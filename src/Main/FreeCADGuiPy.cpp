...
{
public:
    QtApplication(int &argc, char **argv) : QApplication(argc, argv) {}
...
{
    if (qApp) qApp->sendPostedEvents(0, -1); // special DeferredDelete
...
static PyObject *FreeCADGui_showMainWindow(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &inThread)) return nullptr;
...
static PyObject *FreeCADGui_setupWithoutGUI(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!Gui::SoFCDB::isInitialized()) { Gui::SoFCDB::init(); }
...
{
    char *pointer;
    if (!PyArg_ParseTuple(args, "s", &pointer)) return nullptr;
...
static QWidget *setupMainWindow()
{
    ...
    if (!Gui::MainWindow::getInstance()) {
        ...
        if (!appName.isEmpty()) mw->setWindowTitle(appName);
        else
        ...
        if (!init) {
            try {
                ...
                Base::Interpreter().runString(
                    Base::ScriptFactory().ProduceScript("FreeCADGuiInit"));
            }
...
PyMOD_INIT_FUNC(FreeCADGui)
{
    try {
        ...
        App::Application::Config()["CopyrightInfo"] =
            "\xc2\xa9 Juergen Riegel, Werner Mayer, Yorik van Havre and others 2001-2022\n";
        App::Application::Config()["LicenseInfo"] = "FreeCAD is free and open-source software "
                                                    "licensed under the terms of LGPL2+ license.\n";
        App::Application::Config()["CreditsInfo"] =
        ...
        if (Base::Type::fromName("Gui::BaseView").isBad()) Gui::Application::initApplication();

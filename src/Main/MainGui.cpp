...
const char sBanner[] =
    "\xc2\xa9 Juergen Riegel, Werner Mayer, Yorik van Havre and others 2001-2022\n"
...
{
public:
    Redirection(FILE *f) : fi(Base::FileInfo::getTempFileName()), file(f)
    {
#ifdef WIN32
...
#endif
        if (!ptr) { std::cerr << "Failed to reopen file" << std::endl; }
    }
    ~Redirection()
    {
        fclose(file);
...
int main(int argc, char **argv)
{
    ...
    if (fc_py_home) _putenv_s("PYTHONHOME", fc_py_home);
    else
        _putenv("PYTHONHOME=");
#endif
    ...
    try {
        ...
        if (App::Application::Config()["RunMode"] == "Gui") Base::Interpreter().replaceStdOutput();
    }
        ...
        QMessageBox::critical(nullptr, QObject::tr("Initialization of %1 failed").arg(appName),
                              msg);
        exit(100);
    }
    catch (...) {
    ...
    try {
        ...
        if (App::Application::Config()["Console"] == "1") App::Application::runApplication();
...
static LONG __stdcall MyCrashHandlerExceptionFilter(EXCEPTION_POINTERS *pEx)
{
    ...
    bool bFailed = true;
    HANDLE hFile;
    hFile = CreateFileW(s_szMiniDumpFileName.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        ...
        if (s_pMDWD(GetCurrentProcess(), GetCurrentProcessId(), hFile, s_dumpTyp, &stMDEI, NULL,
                    NULL)) {
    ...
    if (bFailed) { return EXCEPTION_CONTINUE_SEARCH; }
...
void InitMiniDumpWriter(const std::string &filename)
{
    ...
    if (s_hDbgHelpMod != NULL) s_pMDWD = (tMDWD)GetProcAddress(s_hDbgHelpMod, "MiniDumpWriteDump");

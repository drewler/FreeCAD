...
wxWidgetsFrame::wxWidgetsFrame(wxFrame *frame, const wxString &title) : wxFrame(frame, -1, title)
{
    if (!Py_IsInitialized()) {
    ...
    freecadMenu->Append(idMenuNewDocument, _("New document"),
                        _("Create new document in the FreeCAD module"));
...
void wxWidgetsFrame::OnClose(wxCloseEvent &event) { Destroy(); }

void wxWidgetsFrame::OnQuit(wxCommandEvent &event) { Destroy(); }

...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        if (cCmdP.HasSwitch("-h") || cCmdP.HasSwitch("--help")) { CImageConvApp::Usage(); }
        ...
        if (cCmdP.HasSwitch("-x") || cCmdP.HasSwitch("--xpm")) {
            ...
            if (cCmdP.GetArgumentCount("-i") > 0) {
                ...
                for (TVector<TString>::iterator it = para.m_strings.begin();
                     it != para.m_strings.end(); it++) {
        ...
        // convert one image file to another image file
            ...
            if (p1.m_strings.size() > 1) throw CICException("Too much input files specified.");
            if (p1.m_strings.size() < 1) throw CICException("No input file specified.");
            if (p2.m_strings.size() > 1) throw CICException("Too much output files specified.");
            if (p2.m_strings.size() < 1) throw CICException("No output file specified.");
            ...
            if (cICApp.Load(QString(fil1.c_str())) == false) {
                ...
                cout << "Perhaps the file does not exist or QT does not support this format."
                     << endl;
                return -1;
            }

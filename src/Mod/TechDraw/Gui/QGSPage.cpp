...
void QGSPage::removeQViewFromScene(QGIView *view)
{
    ...
    if (qgParent) { qgParent->removeChild(view); }
    else {
        removeItem(view);
    }
}
...
QGIView *QGSPage::addViewBalloon(TechDraw::DrawViewBalloon *balloonFeat)
{
    ...
    if (parent) { addBalloonToParent(vBalloon, parent); }
...
void QGSPage::createBalloon(QPointF origin, DrawViewPart *parent)
{
    ...
    if (!balloon) { throw Base::TypeError("QGSP::createBalloon - balloon not found\n"); }
    Command::doCommand(Command::Doc,
...
QGIView *QGSPage::addViewDimension(TechDraw::DrawViewDimension *dimFeat)
{
    ...
    if (parent) { addDimToParent(dimGroup, parent); }
...
QGIView *QGSPage::addViewLeader(TechDraw::DrawLeaderLine *leaderFeat)
{
    ...
    if (parent) { addLeaderToParent(leaderGroup, parent); }
...
QGIView *QGSPage::addRichAnno(TechDraw::DrawRichAnno *richFeat)
{
    ...
    if (parent != nullptr) { addAnnoToParent(annoGroup, parent); }
...
QGIView *QGSPage::addWeldSymbol(TechDraw::DrawWeldSymbol *weldFeat)
{
    ...
    if (parentObj) { parentDV = dynamic_cast<TechDraw::DrawView *>(parentObj); }
    else {
...
QGIView *QGSPage::findQViewForDocObj(App::DocumentObject *obj) const
{
    ...
    if (obj) {
        ...
        for (std::vector<QGIView *>::const_iterator it = qviews.begin(); it != qviews.end(); ++it) {
            if (strcmp(obj->getNameInDocument(), (*it)->getViewName()) == 0) return *it;
        }
    }
    return nullptr;
}
...
QGIView *QGSPage::findParent(QGIView *view) const
{
    ...
    if (balloon) {
        ...
        if (obj) {
            ...
            for (std::vector<QGIView *>::const_iterator it = qviews.begin(); it != qviews.end();
                 ++it) {
                if (strcmp((*it)->getViewName(), obj->getNameInDocument()) == 0) { return *it; }
            }
        }
    }
    ...
    if (lead != nullptr) {
        ...
        if (obj != nullptr) {
            ...
            for (std::vector<QGIView *>::const_iterator it = qviews.begin(); it != qviews.end();
                 ++it) {
                if (strcmp((*it)->getViewName(), parentName.c_str()) == 0) { return *it; }
            }
        }
    }
...
bool QGSPage::hasQView(App::DocumentObject *obj)
{
    ...
    while (qview != views.end()) {
        ...
        if (strcmp((*qview)->getViewName(), obj->getNameInDocument()) == 0) { return true; }
        qview++;
    }
...
void QGSPage::refreshViews()
{
    ...
    for (auto q : list) {
        ...
        if (viewFamily == q->data(0).toString()) { qgiv.push_back(q); }
    }
    for (auto q : qgiv) {
        QGIView *itemView = dynamic_cast<QGIView *>(q);
        if (itemView) { itemView->updateView(true); }
    }
}
                               ...
                               std::vector<App::DocumentObject *> &missing)
{
    for (std::vector<App::DocumentObject *>::const_iterator it = list.begin(); it != list.end();
        ...
        if (!hasQView(*it)) missing.push_back(*it);
...
void QGSPage::fixOrphans(bool force)
{
    ...
    for (auto &qv : qvs) {
        if (!qv) continue; // already deleted?
...
bool QGSPage::orphanExists(const char *viewName, const std::vector<App::DocumentObject *> &list)
{
    for (std::vector<App::DocumentObject *>::const_iterator it = list.begin(); it != list.end();
        ...
        if ((*it)->isDerivedFrom(TechDraw::DrawViewCollection::getClassTypeId())) {
            TechDraw::DrawViewCollection *collection =
                dynamic_cast<TechDraw::DrawViewCollection *>(*it);
            if (orphanExists(viewName, collection->Views.getValues())) return true;
        }

        // Unsure if we can compare pointers so rely on name
        if (strcmp(viewName, (*it)->getNameInDocument()) == 0) { return true; }
    }
    return false;
}
...
void QGSPage::redraw1View(TechDraw::DrawView *dView)
{
    ...
    for (std::vector<QGIView *>::const_iterator it = upviews.begin(); it != upviews.end(); ++it) {
        ...
        if (dvName == qgivName) { (*it)->updateView(true); }
    }
}
...
void QGSPage::setExporting(bool enable)
{
    ...
    for (auto &qgi : sceneItems) {
        ...
        if (qgiRTA) { qgiRTA->setExporting(enable); }
    }
    for (auto &v : dvps) { v->draw(); }
}
...
void QGSPage::saveSvg(QString filename)
{
    ...
    setExporting(false);
    if (templateVisible && svgTemplate) { svgTemplate->show(); }
...
static void removeEmptyGroups(QDomElement e)
{
    while (!e.isNull()) {
        ...
        if (e.hasChildNodes()) { removeEmptyGroups(e.firstChildElement()); }

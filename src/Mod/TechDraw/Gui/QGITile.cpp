...
QGITile::QGITile(TechDraw::DrawTileWeld *dtw)
    : m_textL(QString::fromUtf8(" ")), m_textR(QString::fromUtf8(" ")),
      m_textC(QString::fromUtf8(" ")), m_scale(1.0), m_row(0), m_col(0), m_tailRight(true),
      m_altWeld(false), m_tileFeat(dtw)
{
...
void QGITile::makeText()
{
    ...
    double hMargin = 1;
    if (!m_textL.isEmpty()) { hMargin = (m_wide / 2.0) + (charWidth * 1.5); }
    ...
    hMargin = 1;
    if (!m_textR.isEmpty()) { hMargin = (m_wide / 2.0) + (charWidth); }
...
void QGITile::setTileScale(double s) { m_scale = s; }

void QGITile::setTileTextLeft(std::string s) { m_textL = QString::fromUtf8(s.c_str()); }

void QGITile::setTileTextRight(std::string s) { m_textR = QString::fromUtf8(s.c_str()); }

void QGITile::setTileTextCenter(std::string s) { m_textC = QString::fromUtf8(s.c_str()); }
...
void QGITile::setSymbolFile(std::string s)
{
    ...
    if (!s.empty()) { m_svgPath = QString::fromUtf8(s.c_str()); }
}
...
bool QGITile::isTailRight() { return m_tailRight; }

bool QGITile::getAltWeld() { return m_altWeld; }
...
QString QGITile::prefTextFont() const { return Preferences::labelFontQString(); }

QRectF QGITile::boundingRect() const { return childrenBoundingRect(); }

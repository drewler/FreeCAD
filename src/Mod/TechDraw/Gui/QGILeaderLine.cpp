...
QGILeaderLine::QGILeaderLine()
    : m_parentItem(nullptr), m_lineColor(Qt::black), m_lineStyle(Qt::SolidLine), m_hasHover(false),
      m_saveX(0.0), m_saveY(0.0), m_blockDraw(false)

{
    ...
    QObject::connect(m_editPath, SIGNAL(pointsUpdated(QPointF, std::vector<QPointF>)), this,
...
void QGILeaderLine::onLineEditFinished(QPointF tipDisplace, std::vector<QPointF> points)
{
    ...
    if (featLeader->AutoHorizontal.getValue()) { featLeader->adjustLastSegment(); }
...
void QGILeaderLine::draw()
{
    ...
    if (parent) scale = parent->getScale();

    if (m_editPath->inEdit()) return;

    //********
    if (featLeader->isLocked()) setFlag(QGraphicsItem::ItemIsMovable, false);
    else
...
QPainterPath QGILeaderLine::makeLeaderPath(std::vector<QPointF> qPoints)
{
    ...
    if (qPoints.size() > 1) {
        ...
        for (int i = 1; i < (int)qPoints.size(); i++) { result.lineTo(qPoints.at(i)); }
    }
    return result;
}
...
{
    auto vp = static_cast<ViewProviderLeader *>(getViewProvider(getViewObject()));
    if (!vp) return Rez::guiX(LineGroup::getDefaultWidth("Graphic"));
...
double QGILeaderLine::getEdgeFuzz() const { return PreferencesGui::edgeFuzz(); }
...
QColor QGILeaderLine::prefNormalColor()
{
    ...
    if (vp) { setNormalColor(vp->Color.getValue().asValue<QColor>()); }
    return getNormalColor();
}

QRectF QGILeaderLine::boundingRect() const { return childrenBoundingRect(); }

...
void execHoleCircle(Gui::Command *cmd)
{
    ...
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Hole Circle")) return;
...
void execCircleCenterLines(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Circle Centerlines")) return;
...
void execThreadHoleSide(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Thread Hole Side")) return;
    Gui::Command::openCommand(QT_TRANSLATE_NOOP("Command", "Cosmetic Thread Hole Side"));
    const std::vector<std::string> SubNames = selection[0].getSubNames();
    if (SubNames.size() >= 2) { _createThreadLines(SubNames, objFeat, 1.176f); }
    cmd->getSelection().clearSelection();
    objFeat->refreshCEGeoms();
    objFeat->requestPaint();
    Gui::Command::commitCommand();
...
void execThreadBoltSide(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Thread Bolt Side")) return;
    Gui::Command::openCommand(QT_TRANSLATE_NOOP("Command", "Cosmetic Thread Bolt Side"));
    const std::vector<std::string> SubNames = selection[0].getSubNames();
    if (SubNames.size() >= 2) { _createThreadLines(SubNames, objFeat, 0.85f); }
    cmd->getSelection().clearSelection();
    objFeat->refreshCEGeoms();
    objFeat->requestPaint();
    Gui::Command::commitCommand();
...
void execThreadHoleBottom(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Thread Hole Bottom")) return;
    Gui::Command::openCommand(QT_TRANSLATE_NOOP("Command", "Cosmetic Thread Hole Bottom"));
    const std::vector<std::string> SubNames = selection[0].getSubNames();
    for (const std::string &Name : SubNames) { _createThreadCircle(Name, objFeat, 1.177f); }
    cmd->getSelection().clearSelection();
    objFeat->refreshCEGeoms();
    objFeat->requestPaint();
    Gui::Command::commitCommand();
...
void execThreadBoltBottom(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Thread Bolt Bottom")) return;
    Gui::Command::openCommand(QT_TRANSLATE_NOOP("Command", "Cosmetic Thread Bolt Bottom"));
    const std::vector<std::string> SubNames = selection[0].getSubNames();
    for (const std::string &Name : SubNames) { _createThreadCircle(Name, objFeat, 0.85f); }
    cmd->getSelection().clearSelection();
    objFeat->refreshCEGeoms();
    objFeat->requestPaint();
    Gui::Command::commitCommand();
...
void CmdTechDrawExtensionChangeLineAttributes::activated(int iMsg)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(this, selection, objFeat, "TechDraw Change Line Attributes")) return;
...
void CmdTechDrawExtensionVertexAtIntersection::activated(int iMsg)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(this, selection, objFeat, "TechDraw Cosmetic Intersection Vertex(es)")) return;
...
void execDrawCosmArc(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Cosmetic Arc")) return;
...
void execDrawCosmCircle(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Cosmetic Circle")) return;
...
void execDrawCosmCircle3Points(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Cosmetic Circle 3 Points")) return;
...
void CmdTechDrawExtensionLockUnlockView::activated(int iMsg)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(this, selection, objFeat, "TechDraw Lock/Unlock View")) return;
...
void CmdTechDrawExtensionPositionSectionView::activated(int iMsg)
{
    ...
    if (objFeat && objFeat->isDerivedFrom(TechDraw::DrawViewSection::getClassTypeId())) {
        ...
        if ((direction == "Right") || (direction == "Left")) sectionView->Y.setValue(yPos);
...
void execExtendShortenLine(Gui::Command *cmd, bool extend)
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(cmd, selection, objFeat, "TechDraw Extend/Shorten Line")) return;
...
// calculate the area of selected faces, create output in a balloon
{
    ...
    std::vector<Gui::SelectionObject> selection;
    TechDraw::DrawViewPart *objFeat;
    if (!_checkSel(this, selection, objFeat, "TechDraw calculate selected area")) return;
    ...
    if (!balloon) throw Base::TypeError("CmdTechDrawNewBalloon - balloon not found\n");

...
CmdTechDrawDimension::CmdTechDrawDimension() : Command("TechDraw_Dimension")
{
    sAppModule = "TechDraw";
...
CmdTechDrawRadiusDimension::CmdTechDrawRadiusDimension() : Command("TechDraw_RadiusDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawRadiusDimension::activated(int iMsg)
{
    ...
    if (edgeType == isCircle) {
        ...
        QMessageBox::StandardButton result = QMessageBox::warning(
            Gui::getMainWindow(), QObject::tr("Ellipse Curve Warning"),
        ...
        QMessageBox::StandardButton result = QMessageBox::warning(
            Gui::getMainWindow(), QObject::tr("BSpline Curve Warning"),
        ...
        QMessageBox::critical(
            Gui::getMainWindow(), QObject::tr("BSpline Curve Error"),
    ...
    openCommand(QT_TRANSLATE_NOOP("Command", "Create Dimension"));
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.Type = '%s'", FeatName.c_str(), "Radius");

    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewRadiusDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDrawDiameterDimension::CmdTechDrawDiameterDimension() : Command("TechDraw_DiameterDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawDiameterDimension::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.Type = '%s'", FeatName.c_str(), "Diameter");

    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewDiameterDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDrawLengthDimension::CmdTechDrawLengthDimension() : Command("TechDraw_LengthDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawLengthDimension::activated(int iMsg)
{
    ...
    if ((_isValidSingleEdge(this) == isVertical) || (_isValidSingleEdge(this) == isHorizontal)
        || (_isValidSingleEdge(this) == isDiagonal)) {
        objs.push_back(objFeat);
        subs.push_back(SubNames[0]);
    }
    else if (_isValidVertexes(this) || (_isValidEdgeToEdge(this) == isVertical)
             || (_isValidEdgeToEdge(this) == isHorizontal)
             || (_isValidEdgeToEdge(this) == isDiagonal) || (_isValidVertexToEdge(this))) {
        objs.push_back(objFeat);
        objs.push_back(objFeat);
        subs.push_back(SubNames[0]);
        subs.push_back(SubNames[1]);
    ...
    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewLengthDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
void CmdTechDrawHorizontalDimension::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.Type = '%s'", FeatName.c_str(), "DistanceX");

    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewDistanceXDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDrawVerticalDimension::CmdTechDrawVerticalDimension() : Command("TechDraw_VerticalDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawVerticalDimension::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.Type = '%s'", FeatName.c_str(), "DistanceY");
    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewDistanceYDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDrawAngleDimension::CmdTechDrawAngleDimension() : Command("TechDraw_AngleDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawAngleDimension::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.Type = '%s'", FeatName.c_str(), "Angle");

    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawAngleDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDraw3PtAngleDimension::CmdTechDraw3PtAngleDimension() : Command("TechDraw_3PtAngleDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDraw3PtAngleDimension::activated(int iMsg)
{
    ...
    dim = dynamic_cast<TechDraw::DrawViewDimension *>(getDocument()->getObject(FeatName.c_str()));
    if (!dim) { throw Base::TypeError("CmdTechDrawNewAngle3PtDimension - dim not found\n"); }
    dim->References2D.setValues(objs, subs);
...
CmdTechDrawLinkDimension::CmdTechDrawLinkDimension() : Command("TechDraw_LinkDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
bool CmdTechDrawLinkDimension::isActive()
{
    ...
    if (havePage) { taskInProgress = Gui::Control().activeDialog(); }
...
CmdTechDrawExtentGroup::CmdTechDrawExtentGroup() : Command("TechDraw_ExtentGroup")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawExtentGroup::activated(int iMsg)
{
    ...
    switch (iMsg) {
        case 0: execHExtent(this); break;
        case 1: execVExtent(this); break;
        default: Base::Console().Message("CMD::ExtGrp - invalid iMsg: %d\n", iMsg);
    };
}
...
void execHExtent(Gui::Command *cmd)
{
    ...
    DrawDimHelper::makeExtentDim(baseFeat, edgeNames, 0);
}
...
void execVExtent(Gui::Command *cmd)
{
    ...
    DrawDimHelper::makeExtentDim(baseFeat, edgeNames, 1);
}
...
CmdTechDrawLandmarkDimension::CmdTechDrawLandmarkDimension() : Command("TechDraw_LandmarkDimension")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawLandmarkDimension::activated(int iMsg)
{
    ...
    openCommand(QT_TRANSLATE_NOOP("Command", "Create Dimension"));
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::LandmarkDimension', '%s')",
              FeatName.c_str());
    doCommand(Doc, "App.activeDocument().%s.addView(App.activeDocument().%s)", PageName.c_str(),
              FeatName.c_str());
    if (objects.size() == 2) {
    ...
    if (!dim) { throw Base::TypeError("CmdTechDrawLandmarkDimension - dim not found\n"); }
...
bool _checkPartFeature(Gui::Command *cmd)
{
    ...
    std::vector<Gui::SelectionObject>::iterator itSel = selection.begin();
    for (; itSel != selection.end(); itSel++) {
        if (itSel->isDerivedFrom(Part::Feature::getClassTypeId())) { result = true; }
    }
    if (!result) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Incorrect selection"),
...
int _isValidSingleEdge(Gui::Command *cmd)
{
    ...
    auto objFeat(dynamic_cast<TechDraw::DrawViewPart *>(selection[0].getObject()));
    if (!objFeat) return isInvalid;

    const std::vector<std::string> SubNames = selection[0].getSubNames();
    if (SubNames.size() != 1 || TechDraw::DrawUtil::getGeomTypeFromName(SubNames[0]) != "Edge") {
        return isInvalid;
    }
    ...
    if (geom->geomType == TechDraw::GENERIC) {
        ...
        if (fabs(line.y) < FLT_EPSILON) { return isHorizontal; }
        ...
        if (spline->isCircle()) { return isBSplineCircle; }
        else {
            return isBSpline;
        }
    }
...
bool _isValidVertexes(Gui::Command *cmd, int count)
{
    ...
    if (SubNames.size() != (unsigned)count) { return false; }

    for (auto &s : SubNames) {
        if (TechDraw::DrawUtil::getGeomTypeFromName(s) != "Vertex") { return false; }
    }
...
int _isValidEdgeToEdge(Gui::Command *cmd)
{
    ...
    //there has to be 2
    if (SubNames.size() != 2) { return isInvalid; }
...
{
    std::vector<Gui::SelectionObject> selection = cmd->getSelection().getSelectionEx();
    TechDraw::DrawViewPart *objFeat0 =
        static_cast<TechDraw::DrawViewPart *>(selection[0].getObject());
    const std::vector<std::string> SubNames = selection[0].getSubNames();

    //there has to be 2
    if (SubNames.size() != 2) { return false; }
...
{
    switch (e) {
        case isInvalid: return "invalid";
        case isHorizontal: return "horizontal";
        case isVertical: return "vertical";
        case isDiagonal: return "diagonal";
        case isCircle: return "circle";
        case isEllipse: return "ellipse";
        case isBSpline: return "bspline";
        case isBSplineCircle: return "circular bspline";
        case isAngle: return "angle";
        case isAngle3Pt: return "angle3";

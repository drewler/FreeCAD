...
void QGVPage::setNavigationStyle(std::string navParm)
{
    ...
    if (m_navStyle) { delete m_navStyle; }
...
void QGVPage::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (m_saveContextEvent) { delete m_saveContextEvent; }
    m_saveContextEvent =
...
{
    if (m_saveContextEvent) { m_parentMDI->contextMenuEvent(m_saveContextEvent); }
}
...
QPixmap QGVPage::prepareCursorPixmap(const char *iconName, QPoint &hotspot)
{
    ...
    if (qGuiApp->platformName() == QLatin1String("xcb")) { floatHotspot *= pixelRatio; }
#endif

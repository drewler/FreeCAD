...
void QGIDatumLabel::setPosFromCenter(const double &xCenter, const double &yCenter)
{
    ...
    if (overWidth > underWidth) { width = overWidth; }
...
void QGIDatumLabel::setToleranceString()
{
    ...
    if (labelTexts.first.empty()) { m_tolTextUnder->hide(); }
    else {
        m_tolTextUnder->setPlainText(QString::fromUtf8(labelTexts.first.c_str()));
        m_tolTextUnder->show();
    }
    if (labelTexts.second.empty()) { m_tolTextOver->hide(); }
    else {
...
{
    prepareGeometryChange();
    if (text.isEmpty()) { m_unitText->hide(); }
    else {
...
int QGIDatumLabel::getPrecision()
{
    ...
    if (global) { precision = Base::UnitsApi::getDecimals(); }
    else {
...
QString QGIViewDimension::getLabelText()
{
    ...
    if (second.length() > third.length()) { result = first + second; }
    else {
        result = first + third;
    }
                                                 ...
                                                 double startRotation)
{
    if (startRotation > 0.0) {
        ...
        if (endAngle > M_PI) { endAngle -= M_2PI; }
    }

    if (testAngle > endAngle) { testAngle -= M_2PI; }

    if (testAngle >= endAngle + startRotation) { return +1.0; }
                                               ...
                                               double rightStrikeFactor)
{
    ...
    if (fabs(leftDelta - rightDelta) <= Precision::Confusion()) { return 0; }
                                                   ...
                                                   double &labelAngle)
{
    ...
    if (lineLabelDistance >= rawDistance) { return 0.0; }
...
{
    if (drawMarking.size() < 2) { return 0.0; }
...
QGIViewDimension::computeArcStrikeFactor(const Base::BoundBox2d &labelRectangle,
                                         ...
                                         const std::vector<std::pair<double, bool>> &drawMarking)
{
    if (drawMarking.empty()) { return 0.0; }

    unsigned int entryIndex = 0;
    while (entryIndex < drawMarking.size() && drawMarking[entryIndex].second) { ++entryIndex; }
        ...
        do {
            ...
            if (drawMarking[currentIndex].second != drawMarking[startIndex].second) {
                if (drawMarking[startIndex].second) {
                    ...
                    if (arcRotation < 0.0) { arcRotation += M_2PI; }
    ...
    std::vector<std::pair<double, bool>> &outputMarking) const
{
    ...
    if (jointPosition - labelBorder < startPosition) {
        ...
        if (arrowCount > 1 || startPosition < 0.0) { autoFlipArrows = true; }
    }
    ...
    std::vector<std::pair<double, bool>> &outputMarking) const
{
    ...
    if (standardStyle == ViewProviderDimension::STD_STYLE_ISO_ORIENTED) {
        ...
        if (intersectionPoints.size() == 2) {
            ...
            if ((endDelta < 0.0) == (handednessFactor < 0.0)) { std::swap(startDelta, endDelta); }
    ...
    if (jointRotation - startDelta < startRotation) {
        ...
        // For only one arrow and zero width line skip flipping, it already points correctly
        if (arrowCount > 1 || startRotation < 0.0) { autoFlipArrows = true; }
    }
                                     ...
                                     double arcRadius, double startAngle, double endAngle) const
{
    ...
    if (endAngle < startAngle) { endAngle += M_2PI; }
                                           ...
                                           int renderExtent, bool flipArrow) const
{
    ...
    if (standardStyle == ViewProviderDimension::STD_STYLE_ISO_REFERENCING
        ...
        if (angleFactor < 0.0) { lineAngle = DrawUtil::angleComposition(lineAngle, M_PI); }
        ...
        // Is there point on the arc, where line from center intersects it perpendicularly?
        double angleFactor = getAnglePlacementFactor(lineAngle, endAngle, startRotation);
        if (angleFactor < 0) { lineAngle = DrawUtil::angleComposition(lineAngle, M_PI); }
                                    ...
                                    ViewProviderDimension *viewProvider) const
{
    ...
    double lineAngle;
    if (strcmp(dimensionType, "DistanceX") == 0) { lineAngle = 0.0; }
                                 ...
                                 ViewProviderDimension *viewProvider) const
{
    ...
    if (standardStyle == ViewProviderDimension::STD_STYLE_ISO_REFERENCING
        ...
        arcRadius = labelDirection.Length()
            - placementFactor
                * (labelRectangle.Height() * 0.5 + getDefaultIsoDimensionLineSpacing());
        if (arcRadius < 0.0) { arcRadius = labelDirection.Length(); }
...
Base::Vector3d QGIViewDimension::findIsoDir(Base::Vector3d ortho) const
{
    ...
    for (auto &iso : isoDirs) { angles.push_back(ortho.GetAngle(iso)); }
    int idx = 0;
    double min = angles[0];
    for (int i = 1; i < 6; i++) {
        if (angles[i] < min) {
...
Base::Vector3d QGIViewDimension::findIsoExt(Base::Vector3d dir) const
{
    ...
    if (dir.IsEqual(isoX, FLT_EPSILON)) { dirExt = isoY; }
...
void QGIViewDimension::onPrettyChanged(int state)
{
    ...
    if (state == NORMAL) { setPrettyNormal(); }
    else if (state == PRE) {
        setPrettyPre();
    }
    else { //if state = SEL

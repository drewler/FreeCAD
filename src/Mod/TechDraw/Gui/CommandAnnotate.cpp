...
std::vector<std::string> getSelectedSubElements(Gui::Command *cmd, TechDraw::DrawViewPart *&dvp,
...
CmdTechDrawLeaderLine::CmdTechDrawLeaderLine() : Command("TechDraw_LeaderLine")
{
    sAppModule = "TechDraw";
...
void CmdTechDrawLeaderLine::activated(int iMsg)
{
    ...
    Gui::Control().showDialog(new TechDrawGui::TaskDlgLeaderLine(baseFeat, page));
    updateActive();
...
void CmdTechDrawRichTextAnnotation::activated(int iMsg)
{
    ...
    Gui::Control().showDialog(new TaskDlgRichAnno(baseFeat, page));
    updateActive();
    Gui::Selection().clearSelection();
}
...
void CmdTechDrawCosmeticVertexGroup::activated(int iMsg)
{
    ...
    switch (iMsg) {
        case 0: execCosmeticVertex(this); break;
        case 1: execMidpoints(this); break;
        case 2: execQuadrants(this); break;
...
void CmdTechDrawCosmeticVertexGroup::languageChange()
{
    ...
    arc1->setToolTip(QApplication::translate("TechDraw_CosmeticVertex",
                                             "Inserts a Cosmetic Vertex into a View"));
    arc1->setStatusTip(arc1->toolTip());
    QAction *arc2 = a[1];
    arc2->setText(QApplication::translate("CmdMidpoints", "Add Midpoint Vertices"));
    arc2->setToolTip(QApplication::translate(
        "TechDraw_Midpoints", "Inserts Cosmetic Vertices at Midpoint of selected Edges"));
    arc2->setStatusTip(arc2->toolTip());
    QAction *arc3 = a[2];
    arc3->setText(QApplication::translate("CmdQuadrants", "Add Quadrant Vertices"));
    arc3->setToolTip(QApplication::translate(
        "TechDraw_Quadrants", "Inserts Cosmetic Vertices at Quadrant Points of selected Circles"));
...
void execCosmeticVertex(Gui::Command *cmd)
{
    ...
    Gui::Control().showDialog(new TaskDlgCosVertex(baseFeat, page));
}
...
void execMidpoints(Gui::Command *cmd)
{
    ...
    if (!dvp || selectedEdges.empty()) return;
...
CmdTechDrawCosmeticVertex::CmdTechDrawCosmeticVertex() : Command("TechDraw_CosmeticVertex")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
CmdTechDrawMidpoints::CmdTechDrawMidpoints() : Command("TechDraw_Midpoints")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
CmdTechDrawQuadrants::CmdTechDrawQuadrants() : Command("TechDraw_Quadrants")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawAnnotation::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject('TechDraw::DrawViewAnnotation', '%s')",
              FeatName.c_str());
...
bool CmdTechDrawAnnotation::isActive() { return DrawGuiUtil::needPage(this); }
...
CmdTechDrawCenterLineGroup::CmdTechDrawCenterLineGroup() : Command("TechDraw_CenterLineGroup")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawCenterLineGroup::languageChange()
{
    ...
    arc1->setToolTip(
        QApplication::translate("TechDraw_FaceCenterLine", "Adds a Centerline to Faces"));
    arc1->setStatusTip(arc1->toolTip());
    QAction *arc2 = a[1];
    arc2->setText(QApplication::translate("Cmd2LineCenterLine", "Add Centerline between 2 Lines"));
    arc2->setToolTip(
        QApplication::translate("TechDraw_2LineCenterLine", "Adds a Centerline between 2 Lines"));
    arc2->setStatusTip(arc2->toolTip());
    QAction *arc3 = a[2];
    arc3->setText(
        QApplication::translate("Cmd2PointCenterLine", "Add Centerline between 2 Points"));
    arc3->setToolTip(
        QApplication::translate("TechDraw_2PointCenterLine", "Adds a Centerline between 2 Points"));
    arc3->setStatusTip(arc3->toolTip());
}
...
CmdTechDrawFaceCenterLine::CmdTechDrawFaceCenterLine() : Command("TechDraw_FaceCenterLine")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void execCenterLine(Gui::Command *cmd)
{
    ...
    for (auto &s : subNames) {
        ...
        if (geomType == "Face") { faceNames.push_back(s); }
        else if (geomType == "Edge") {
            edgeNames.push_back(s);
        }
    }

    if ((faceNames.empty()) && (edgeNames.empty())) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong Selection"),
                             QObject::tr("You must select Faces or an existing CenterLine."));
        return;
    }
    if (!faceNames.empty()) {
        Gui::Control().showDialog(new TaskDlgCenterLine(baseFeat, page, faceNames, false));
    }
        ...
        Gui::Control().showDialog(new TaskDlgCenterLine(baseFeat, page, edgeNames.front(), true));
    }
}
...
CmdTechDraw2LineCenterLine::CmdTechDraw2LineCenterLine() : Command("TechDraw_2LineCenterLine")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void exec2LineCenterLine(Gui::Command *cmd)
{
    ...
    if (selectedEdges.size() == 2) {
        Gui::Control().showDialog(new TaskDlgCenterLine(dvp, page, selectedEdges, false));
    }
        ...
        Gui::Control().showDialog(new TaskDlgCenterLine(dvp, page, selectedEdges.front(), true));
    }
...
CmdTechDraw2PointCenterLine::CmdTechDraw2PointCenterLine() : Command("TechDraw_2PointCenterLine")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void exec2PointCenterLine(Gui::Command *cmd)
{
    ...
    for (auto &s : subNames) {
        std::string geomType = DrawUtil::getGeomTypeFromName(s);
        if (geomType == "Vertex") { vertexNames.push_back(s); }
        else if (geomType == "Edge") {
            edgeNames.push_back(s);
        }
    }

    if (vertexNames.empty() && edgeNames.empty()) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong Selection"),
                             QObject::tr("You must select 2 Vertexes or an existing CenterLine."));
        return;
    }
    if (!vertexNames.empty() && (vertexNames.size() == 2)) {
        Gui::Control().showDialog(new TaskDlgCenterLine(baseFeat, page, vertexNames, false));
    }
        ...
        Gui::Control().showDialog(new TaskDlgCenterLine(baseFeat, page, edgeNames.front(), false));
    }
...
void execLine2Points(Gui::Command *cmd)
{
    ...
    if ((subNames2D.empty()) && (objs3D.empty())) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong Selection"),
    ...
    if (!edgeNames.empty() && (edgeNames.size() == 1)) {
        ...
        Gui::Control().showDialog(new TaskDlgCosmeticLine(baseFeat, edgeNames.front()));
        return;
    }
    ...
    Gui::Control().showDialog(new TaskDlgCosmeticLine(baseFeat, points, is3d));
}
...
CmdTechDrawCosmeticEraser::CmdTechDrawCosmeticEraser() : Command("TechDraw_CosmeticEraser")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawCosmeticEraser::activated(int iMsg)
{
    ...
    for (; itSel != selection.end(); itSel++) {
        ...
        for (auto &s : subNames) {
            ...
            if (geomType == "Edge") {
                ...
                if (bg && bg->cosmetic) {
                    ...
                    if (source == COSMETICEDGE) { ce2Delete.push_back(tag); }
                ...
                if (!tdv) Base::Console().Message("CMD::eraser - geom: %d not found!\n", idx);
        ...
        if (!cv2Delete.empty()) { objFeat->removeCosmeticVertex(cv2Delete); }

        if (!ce2Delete.empty()) { objFeat->removeCosmeticEdge(ce2Delete); }
        if (!cl2Delete.empty()) { objFeat->removeCenterLine(cl2Delete); }
...
CmdTechDrawDecorateLine::CmdTechDrawDecorateLine() : Command("TechDraw_DecorateLine")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawDecorateLine::activated(int iMsg)
{
    ...
    for (auto &s : subNames) {
        std::string geomType = DrawUtil::getGeomTypeFromName(s);
        if (geomType == "Edge") { edgeNames.push_back(s); }
    }

    Gui::Control().showDialog(new TaskDlgLineDecor(baseFeat, edgeNames));
    updateActive();
    Gui::Selection().clearSelection();
}
...
CmdTechDrawShowAll::CmdTechDrawShowAll() : Command("TechDraw_ShowAll")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
CmdTechDrawWeldSymbol::CmdTechDrawWeldSymbol() : Command("TechDraw_WeldSymbol")
{
    sAppModule = "TechDraw";
    sGroup = QT_TR_NOOP("TechDraw");
...
void CmdTechDrawWeldSymbol::activated(int iMsg)
{
    ...
    std::vector<App::DocumentObject *> leaders =
        getSelection().getObjectsOfType(TechDraw::DrawLeaderLine::getClassTypeId());
    std::vector<App::DocumentObject *> welds =
        getSelection().getObjectsOfType(TechDraw::DrawWeldSymbol::getClassTypeId());
    TechDraw::DrawLeaderLine *leadFeat = nullptr;
    TechDraw::DrawWeldSymbol *weldFeat = nullptr;
    if ((leaders.size() != 1) && (welds.size() != 1)) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                                                ...
                                                std::string subType)
{
    ...
    for (auto &s : subNames) {
        if (TechDraw::DrawUtil::getGeomTypeFromName(s) == subType) { selectedSubs.push_back(s); }
    }

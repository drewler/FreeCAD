...
TaskSurfaceFinishSymbols::TaskSurfaceFinishSymbols(TechDraw::DrawViewPart *view)
    : selectedView(view), ui(new Ui_TaskSurfaceFinishSymbols)
{
    raValues = {"RA50",   "RA25",   "RA12, 5", "RA6, 3", "RA3, 2",  "RA1, 6",
                "RA0, 8", "RA0, 4", "RA0, 2",  "RA0, 1", "RA0, 05", "RA0, 025"};
    laySymbols = {"", "=", "⟂", "X", "M", "C", "R"};
    roughGrades = {"", "N1", "N2", "N3", "N4", "N5", "N6", "N7", "N8", "N9", "N10", "N11"};
    ui->setupUi(this);
    setUiEdit();
}
...
// return QPixmap showing a base symbol
{
    ...
    if (type == removeRequired || type == removeRequiredAll) painter.drawLine(QLine(0, 44, 24, 44));
    if (type > removeRequired) painter.drawEllipse(QPoint(42, 14), 6, 6);
    painter.end();
...
// return string showing the complete symbol
{
    ...
    if (isISO) {
        ...
        std::string minRoughtText =
            cbMinRought->itemText(cbMinRought->currentIndex()).toStdString();
        symbol.addText(-10, 35, minRoughtText);
        std::string maxRoughtText =
            cbMaxRought->itemText(cbMaxRought->currentIndex()).toStdString();
...
// arrange widget at tool start
{
    ...
    cbRA = new (QComboBox);
    cbRA->resize(90, 20);
    for (const std::string &nextValue : raValues) cbRA->addItem(QString::fromStdString(nextValue));
    ...
    cbLay = new (QComboBox);
    cbLay->resize(40, 20);
    for (const std::string &nextLay : laySymbols) cbLay->addItem(QString::fromStdString(nextLay));
...
bool TaskSurfaceFinishSymbols::reject() { return true; }
    ...
    : TaskDialog()
{
    ...
    taskbox = new Gui::TaskView::TaskBox(
        Gui::BitmapFactory().pixmap("actions/TechDraw_SurfaceFinishSymbols"), widget->windowTitle(),
        true, nullptr);

...
QGMarker::QGMarker(int idx) : QGIVertex(idx), m_dragging(false)
{
...
void QGMarker::keyPressEvent(QKeyEvent *event)
{
    ...
    if (event->key() == Qt::Key_Escape) { Q_EMIT endEdit(); }
...
QGEPath::QGEPath(QGILeaderLine *leader)
    : m_scale(1.0), m_inEdit(false), m_parentLeader(leader), m_startAdj(0.0), m_endAdj(0.0)
{
...
void QGEPath::hoverLeaveEvent(QGraphicsSceneHoverEvent *event)
{
    ...
    if (parent) { parentSel = parent->isSelected(); }
    if (!parentSel && !isSelected()) { setPrettyNormal(); }
...
void QGEPath::showMarkers(std::vector<QPointF> points)
{
    ...
    for (auto &p : points) {
        ...
        QObject::connect(v, SIGNAL(dragFinished(QPointF, int)), this,
                         SLOT(onDragFinished(QPointF, int)));
        QObject::connect(v, SIGNAL(dragging(QPointF, int)), this, SLOT(onDragging(QPointF, int)));
        QObject::connect(v, SIGNAL(doubleClick(QPointF, int)), this,
                         SLOT(onDoubleClick(QPointF, int)));
        QObject::connect(v, SIGNAL(endEdit()), this, SLOT(onEndEdit()));
...
void QGEPath::onDragFinished(QPointF dragEndPos, int markerIndex)
{
    ...
    //                            markerIndex);
    if ((int)m_ghostPoints.size() > markerIndex) { m_ghostPoints.at(markerIndex) = dragEndPos; }
    drawGhost();
}
...
std::vector<QPointF> QGEPath::getDeltasFromLeader()
{
    ...
    if (qDeltas.empty()) { Base::Console().Warning("QGEPath::getDeltasFromLeader - no points\n"); }
    return qDeltas;
}
...
void QGEPath::updateParent()
{
    ...
    if (!inEdit()) { Q_EMIT pointsUpdated(attach, m_ghostPoints); }
}
...
void QGEPath::drawGhost()
{
    ...
    if (!m_ghost->scene()) { m_ghost->setParentItem(this); }
    QPainterPath qpp;
    qpp.moveTo(m_ghostPoints.front());
    for (int i = 1; i < (int)m_ghostPoints.size(); i++) { qpp.lineTo(m_ghostPoints.at(i)); }
    m_ghost->setPath(qpp);
    m_ghost->show();
}

void QGEPath::setStartAdjust(double adj) { m_startAdj = Rez::guiX(adj); }

void QGEPath::setEndAdjust(double adj) { m_endAdj = Rez::guiX(adj); }

QRectF QGEPath::boundingRect() const { return shape().controlPointRect(); }
...
double QGEPath::getEdgeFuzz() const { return PreferencesGui::edgeFuzz(); }
...
void QGEPath::dumpGhostPoints(const char *text)
{
    ...
    for (auto &d : m_ghostPoints) {
        Base::Console().Message("%s - point: %d %s\n", text, idb,
                                TechDraw::DrawUtil::formatVector(d).c_str());
        idb++;
    }
}
...
void QGEPath::dumpMarkerPos(const char *text)
{
    ...
    for (auto &m : m_markers) {
        Base::Console().Message("QGEP - %s - markerPos: %d %s\n", text, idb,
                                TechDraw::DrawUtil::formatVector(m->pos()).c_str());
        idb++;
    }
}

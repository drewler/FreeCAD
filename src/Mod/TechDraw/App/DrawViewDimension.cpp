...
void DrawViewDimension::onChanged(const App::Property *prop)
{
    if (!isRestoring()) {
        if (prop == &MeasureType) {
            ...
            if (!(References3D.getValues()).empty()) { setAll3DMeasurement(); }
            ...
            if (EqualTolerance.getValue()) {
                ...
                if (OverTolerance.getValue() < 0) { OverTolerance.setValue(0.0); }
...
{
    if (has3DReferences()) { setAll3DMeasurement(); }
...
{
    if (!isRestoring()) {
        if (References2D.isTouched() || Type.isTouched() || FormatSpec.isTouched()
            || Arbitrary.isTouched() || FormatSpecOverTolerance.isTouched()
            || FormatSpecUnderTolerance.isTouched() || ArbitraryTolerances.isTouched()
            || MeasureType.isTouched() || TheoreticalExact.isTouched() || EqualTolerance.isTouched()
            || OverTolerance.isTouched() || UnderTolerance.isTouched() || Inverted.isTouched()) {
            return true;
        }
    }
...
App::DocumentObjectExecReturn *DrawViewDimension::execute()
{
    ...
    if (!dvp) return App::DocumentObject::StdReturn;

    if (!has2DReferences()) { //too soon?
        if (isRestoring() || getDocument()->testStatus(App::Document::Status::Restoring)) {
    ...
    if (Type.isValue("Distance") || Type.isValue("DistanceX") || Type.isValue("DistanceY")) {
        if (getRefType() == oneEdge) { m_linearPoints = getPointsOneEdge(); }
        ...
        if ((base && base->geomType == TechDraw::GeomType::CIRCLE)
            ...
            if (ellipse->closed()) {
                ...
                pts.isArc = false;
                pts.onCurve.first =
                    pts.center + Base::Vector3d(1, 0, 0) * rAvg; //arbitrary point on edge
                pts.onCurve.second =
                    pts.center + Base::Vector3d(-1, 0, 0) * rAvg; //arbitrary point on edge
            }
            else {
            ...
            if (spline->isCircle()) {
                ...
                if (arc) {
                    ...
                    pts.onCurve.first =
                        pts.center + Base::Vector3d(1, 0, 0) * rad; //arbitrary point on edge
                    pts.onCurve.second =
                        pts.center + Base::Vector3d(-1, 0, 0) * rad; //arbitrary point on edge
                }
            }
        ...
        if ((gen0->getStartPoint() - apex).Length() > (gen0->getEndPoint() - apex).Length()) {
        ...
        if ((gen1->getStartPoint() - apex).Length() > (gen1->getEndPoint() - apex).Length()) {
        ...
        if (DrawUtil::fpCompare(cross.Length(), 0.0)) {
            ...
            if (DrawUtil::isBetween(projFar0OnL1, gen1->getStartPoint(), gen1->getEndPoint())) {
                extPoint0 = farPoint0;
                extPoint1 = projFar0OnL1;
            }
            else if (DrawUtil::isBetween(projFar1OnL0, gen0->getStartPoint(),
                                         gen0->getEndPoint())) {
...
bool DrawViewDimension::isMultiValueSchema() const
{
    ...
    if (uniSys == Base::UnitSystem::ImperialBuilding && !angularMeasure) { return true; }
    else if (uniSys == Base::UnitSystem::ImperialCivil && !angularMeasure) {
        return true;
    }
    return false;
}
...
{
    switch (system) {
        case Base::UnitSystem::SI1: return "mm";
        case Base::UnitSystem::SI2: return "m";
        case Base::UnitSystem::Imperial1: return "in";
        case Base::UnitSystem::ImperialDecimal: return "in";
        case Base::UnitSystem::Centimeters: return "cm";
        case Base::UnitSystem::ImperialBuilding: return "ft";
        case Base::UnitSystem::MmMin: return "mm";
        case Base::UnitSystem::ImperialCivil: return "ft";
        case Base::UnitSystem::FemMilliMeterNewton: return "mm";
                                           ...
                                           bool isDim)
{
    ...
    if ((unitSystem == Base::UnitSystem::ImperialCivil) && angularMeasure) {
        ...
        if (useDecimals()
            && formatSpecifier.contains(QString::fromLatin1("%g"), Qt::CaseInsensitive)) {
            int globalPrecision = Base::UnitsApi::getDecimals();
            // change formatSpecifier to e.g. "%.2f"
            QString newSpecifier =
        ...
        if (pos != -1) {
            qUserStringUnits =
                rxUnits.cap(0); // entire capture - non numerics at end of qUserString
        }
        ...
        double convertValue =
            Base::Quantity::parse(QString::fromLatin1("1") + QString::fromStdString(BaseLengthUnit))
                .getValue();
        ...
        if (formatSpecifier.contains(QRegExp(QStringLiteral("%.*[wW]")))) {
            ...
            formattedValue.replace(QRegExp(QStringLiteral("([0-9][0-9]*\\.[0-9]*[1-9])00*$")),
                                   QStringLiteral("\\1"));
            formattedValue.replace(QRegExp(QStringLiteral("([0-9][0-9]*)\\.0*$")),
                                   QStringLiteral("\\1"));
        }
        else {
            formattedValue =
                QString::asprintf(Base::Tools::toStdString(formatSpecifier).c_str(), userVal);
        }
        ...
        if (loc.decimalPoint() != dp) { formattedValue.replace(dp, loc.decimalPoint()); }
    }
    ...
    if (partial == 0) { //full text for multi-value schemas
        return Base::Tools::toStdString(formatPrefix) + Base::Tools::toStdString(qMultiValueStr)
            + Base::Tools::toStdString(formatSuffix) + Base::Tools::toStdString(qUserStringUnits);
    }
        ...
        if (angularMeasure && !qUserStringUnits.contains(QString::fromLatin1("deg"))) {
            QRegExp space(QString::fromUtf8("\\s"));
            qUserStringUnits.remove(space);
        }
        if (angularMeasure) {
            //always insert unit after value
            return Base::Tools::toStdString(formatPrefix) + formattedValueString
...
std::string DrawViewDimension::getFormattedToleranceValue(int partial)
{
    ...
    if (ArbitraryTolerances.getValue()) ToleranceString = FormatSpec;
    else
        ToleranceString = QString::fromUtf8(
            formatValue(OverTolerance.getValue(), FormatSpec, partial, false).c_str());
...
std::pair<std::string, std::string> DrawViewDimension::getFormattedToleranceValues(int partial)
{
    ...
    if (ArbitraryTolerances.getValue()) {
        ...
        if (DrawUtil::fpCompare(UnderTolerance.getValue(), 0.0)) {
            underTolerance = QString::fromUtf8(
                formatValue(UnderTolerance.getValue(), QString::fromUtf8("%.0f"), partial, false)
                    .c_str());
        }
        else {
            underTolerance = QString::fromUtf8(
                formatValue(UnderTolerance.getValue(), underFormatSpec, partial, false).c_str());
        }
        if (DrawUtil::fpCompare(OverTolerance.getValue(), 0.0)) {
            overTolerance = QString::fromUtf8(
                formatValue(OverTolerance.getValue(), QString::fromUtf8("%.0f"), partial, false)
                    .c_str());
        }
        else {
            overTolerance = QString::fromUtf8(
                formatValue(OverTolerance.getValue(), overFormatSpec, partial, false).c_str());
        }
    }
...
std::string DrawViewDimension::getFormattedDimensionValue(int partial)
{
    ...
    if (Arbitrary.getValue()) { return FormatSpec.getStrValue(); }
    ...
    if (EqualTolerance.getValue() && !TheoreticalExact.getValue()
        && (!DrawUtil::fpCompare(OverTolerance.getValue(), 0.0)
            || ArbitraryTolerances.getValue())) {
        QString labelText = QString::fromUtf8(formatValue(getDimValue(), qFormatSpec, 1,
                                                          true)
                                                  .c_str()); //just the number pref/spec[unit]/suf
        QString unitText = QString::fromUtf8(formatValue(getDimValue(), qFormatSpec, 2,
                                                         false)
                                                 .c_str()); //just the unit
        ...
        return (labelText + QString::fromUtf8(" \xC2\xB1 ") + // +/- symbol
                tolerance)
            .toStdString();

        if (partial == 2) { return unitText.toStdString(); }
...
QStringList DrawViewDimension::getPrefixSuffixSpec(QString fSpec)
{
    ...
    QRegExp rxFormat(
        QStringLiteral("%[+-]?[0-9]*\\.*[0-9]*[aefgwAEFGW]")); //printf double format spec
...
double DrawViewDimension::getDimValue()
{
    ...
    if (MeasureType.isValue("True")) {
        ...
        if (Type.isValue("Distance") || Type.isValue("DistanceX") || Type.isValue("DistanceY")) {
            ...
            if (Type.isValue("Distance")) { result = dimVec.Length() / getViewPart()->getScale(); }
    ...
    if (Inverted.getValue()) {
        if (Type.isValue("Angle") || Type.isValue("Angle3Pt")) { result = 360 - result; }
        else {
            result = -result;
        }
    }
...
{
    if (References2D.getValues().empty()) { return nullptr; }
...
void DrawViewDimension::setAll3DMeasurement()
{
    ...
    int end = Objs.size();
    int i = 0;
    for (; i < end; i++) { static_cast<void>(measurement->addReference3D(Objs.at(i), Subs.at(i))); }
}
                                    ...
                                    Base::Vector3d e2) const
{
    ...
    int count = extss.NbSolution();
    double minDist = 0.0;
    if (count != 0) { minDist = extss.Value(); } //TODO: else { explode }
...
bool DrawViewDimension::leaderIntersectsArc(Base::Vector3d s, Base::Vector3d pointOnCircle)
{
    ...
    if (base && base->geomType == TechDraw::GeomType::ARCOFCIRCLE) {
        TechDraw::AOCPtr aoc = std::static_pointer_cast<TechDraw::AOC>(base);
        if (aoc->intersectsArc(s, pointOnCircle)) { result = true; }
    }
    else if (base && base->geomType == TechDraw::GeomType::BSPLINE) {
        TechDraw::BSplinePtr spline = std::static_pointer_cast<TechDraw::BSpline>(base);
        if (spline->isCircle()) {
            if (spline->intersectsArc(s, pointOnCircle)) { result = true; }
        }
    }
...
bool DrawViewDimension::has2DReferences() const
{
    ...
    if (objects.empty() || SubNames.empty()) { return false; }
...
{
    if (Type.isValue("Radius")) { return "R"; }
    else if (Type.isValue("Diameter")) {
        Base::Reference<ParameterGrp> hGrp = App::GetApplication()
                                                 .GetUserParameter()
                                                 .GetGroup("BaseApp")
...
std::string DrawViewDimension::getDefaultFormatSpec(bool isToleranceFormat) const
{
    ...
    if (prefFormat.empty()) {
        ...
        int precision;
        if (useDecimals()) { precision = Base::UnitsApi::getDecimals(); }
        else {
        ...
        if (!prefix.empty()) { qPrefix = QString::fromUtf8(prefix.data(), prefix.size()); }
    ...
    if (isToleranceFormat) { formatSpec.replace(QString::fromUtf8("%"), QString::fromUtf8("%+")); }

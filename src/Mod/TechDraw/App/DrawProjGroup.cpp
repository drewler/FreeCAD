...
void DrawProjGroup::onChanged(const App::Property *prop)
{
    ...
    if (isRestoring() || !getPage()) { return TechDraw::DrawViewCollection::onChanged(prop); }
...
App::DocumentObjectExecReturn *DrawProjGroup::execute()
{
    ...
    //if group hasn't been added to page yet, can't scale or distribute projItems
    if (!getPage()) return DrawViewCollection::execute();

    if (!Anchor.getValue())
        //no anchor yet.  nothing to do.
        return DrawViewCollection::execute();

    if (waitingForChildren()) { return DrawViewCollection::execute(); }
    ...
    if (AutoDistribute.getValue()) { autoPositionChildren(); }
    overrideKeepUpdated(false);
    return DrawViewCollection::execute();
}
...
void DrawProjGroup::reportReady()
{
    ...
    if (AutoDistribute.getValue()) { recomputeFeature(); }
}
...
bool DrawProjGroup::checkFit() const
{
    ...
    if (!page) throw Base::RuntimeError("No page is assigned to this feature");
...
bool DrawProjGroup::checkFit(DrawPage *page) const
{
    ...
    if (waitingForChildren()) { return true; }
...
bool DrawProjGroup::hasProjection(const char *viewProjType) const
{
    for (const auto it : Views.getValues()) {
        ...
        if (strcmp(viewProjType, view->Type.getValueAsString()) == 0) { return true; }
    }
    return false;
}
            ...
            if (projPtr) {
                if (strcmp(viewProjType, projPtr->Type.getValueAsString()) == 0) {
                    ...
                    getDocument()->removeObject(
                        it->getNameInDocument()); // Remove from the document
...
Base::Vector3d DrawProjGroup::getXYPosition(const char *viewTypeCStr)
{
    ...
    if (AutoDistribute.getValue()) {
        ...
        if (bboxes[6].IsValid()) { bigWidthFarRight = bboxes[6].LengthX(); }
                                      ...
                                      std::array<Base::BoundBox3d, MAXPROJECTIONCOUNT> bboxes)
{
    ...
    for (auto index : list) {
        if (!bboxes.at(index).IsValid()) { continue; }
...
int DrawProjGroup::getViewIndex(const char *viewTypeCStr) const
{
    ...
    if (ProjectionType.isValue("Default")) {
        if (dp) { projType = dp->ProjectionType.getValueAsString(); }
        else {
    ...
    if (strcmp(projType, "Third Angle") == 0 || strcmp(projType, "First Angle") == 0) {
        ...
        if (strcmp(viewTypeCStr, "Front") == 0) { result = 4; }
...
{
    for (int i = 0; i < MAXPROJECTIONCOUNT; ++i) { viewPtrs[i] = nullptr; }
...
void DrawProjGroup::recomputeChildren()
{
    ...
    for (const auto it : Views.getValues()) {
        ...
        if (!view) { throw Base::TypeError("Error: projection in DPG list is not a DPGI!"); }
        else {
            view->recomputeFeature();
        }
    }
...
App::Enumeration DrawProjGroup::usedProjectionType()
{
    ...
    if (ret.isValue("Default")) {
        TechDraw::DrawPage *page = getPage();
        if (page) { ret.setValue(page->ProjectionType.getValueAsString()); }
    }
    return ret;
}
...
TechDraw::DrawProjGroupItem *DrawProjGroup::getAnchor()
{
    ...
    App::DocumentObject *docObj = Anchor.getValue();
    if (docObj) { result = static_cast<DrawProjGroupItem *>(docObj); }
    return result;
}

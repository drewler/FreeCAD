...
const char *DrawViewBalloon::balloonTypeEnums[] = {"Circular", "None",      "Triangle", "Hexagon",
                                                   "Square",   "Rectangle", "Line",     nullptr};
...
{
    if (!isRestoring() && Text.isTouched()) { return 1; }

    auto dvp = getViewPart();
    if (dvp && dvp->isTouched()) { return 1; }

...
DrawProjGroupItem::DrawProjGroupItem(void)
{
    ...
    ADD_PROPERTY_TYPE(RotationVector, (1.0, 0.0, 0.0), "Base", App::Prop_None,
...
bool DrawProjGroupItem::showLock(void) const
{
    ...
    if (parent) { parentLock = parent->LockPosition.getValue(); }
...
void DrawProjGroupItem::autoPosition()
{
    ...
    if (LockPosition.getValue()) { return; }
...
{
    if (getPGroup() && (getPGroup()->getAnchor() == this)) { return true; }
    return false;
}

/// get a coord system aligned with Direction and Rotation Vector
gp_Ax2 DrawProjGroupItem::getViewAxis(const Base::Vector3d &pt, const Base::Vector3d &axis,
                                      const bool flip) const
{
    ...
    if (DrawUtil::checkParallel(projDir, rotVec)) {
        Base::Console().Warning(
            "DPGI::getVA - %s - Direction and XDirection parallel. using defaults\n",
            getNameInDocument());
    }
    try {
        viewAxis = gp_Ax2(gp_Pnt(pt.x, pt.y, pt.z), gp_Dir(projDir.x, projDir.y, projDir.z),
...
Base::Vector3d DrawProjGroupItem::getXDirection(void) const
{
    ...
    if (prop) {
        ...
        prop = getPropertyByName("RotationVector");
        if (prop) { result = RotationVector.getValue(); }
        else {
            Base::Console().Message(
                "DPGI::getXDirection - missing RotationVector and XDirection\n");
        }
    }
    return result;
}

Base::Vector3d DrawProjGroupItem::getLegacyX(const Base::Vector3d &pt, const Base::Vector3d &axis,
                                             const bool flip) const
{
    ...
    if (prop) {
        ...
        if (DrawUtil::fpCompare(result.Length(), 0.0)) { //have RotationVector property, but not set
            gp_Ax2 va = getViewAxis(pt, axis, flip);
...
double DrawProjGroupItem::getRotateAngle()
{
    ...
    if (rotAxis == Direction.getValue()) { angle *= -1.0; }
    return angle;
}
...
void DrawProjGroupItem::unsetupObject()
{
    ...
    if (getPGroup()->getAnchor() == this && !getPGroup()->isUnsetting()) {
        ...
        getPGroup()->Anchor.setValue(
            nullptr); //this catches situation where DPGI is deleted w/o DPG::removeProjection
    }

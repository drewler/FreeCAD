...
const char *DrawView::ScaleTypeEnums[] = {"Page", "Automatic", "Custom", nullptr};
const double SCALEINCREMENT(0.1);
App::PropertyFloatConstraint::Constraints DrawView::scaleRange = {
    Precision::Confusion(), std::numeric_limits<double>::max(),
    (SCALEINCREMENT)}; // increment by 0.1


PROPERTY_SOURCE(TechDraw::DrawView, App::DocumentObject)

DrawView::DrawView() : autoPos(true), mouseMove(false), m_overrideKeepUpdated(false)
{
...
App::DocumentObjectExecReturn *DrawView::execute()
{
    ...
    if (!findParentPage()) { return App::DocumentObject::execute(); }
    handleXYLock();
...
void DrawView::onChanged(const App::Property *prop)
{
    ...
    else if ((prop == &Caption) || (prop == &Label)) {
        requestPaint();
    }
...
bool DrawView::isLocked() const { return LockPosition.getValue(); }

bool DrawView::showLock() const { return true; }
...
{
    short result = 0;
    if (!isRestoring()) { result = (Scale.isTouched() || ScaleType.isTouched()); }
...
void DrawView::validateScale()
{
    ...
    if (page) {
        if (ScaleType.isValue("Page")) {
            ...
            if (!DrawUtil::fpCompare(pageScale, myScale)) { ScaleType.setValue("Custom"); }
        }
    }
}
...
DrawPage *DrawView::findParentPage() const
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = parent.begin(); it != parent.end();
        ...
        if (page) break; // Found page so leave
    }
...
std::vector<DrawPage *> DrawView::findAllParentPages() const
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = parent.begin(); it != parent.end();
        ...
        if (page) { result.emplace_back(page); }
    }
...
bool DrawView::isInClip()
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = parent.begin(); it != parent.end();
         ++it) {
        if ((*it)->getTypeId().isDerivedFrom(DrawViewClip::getClassTypeId())) { return true; }
    }
    return false;
}
...
bool DrawView::checkFit(TechDraw::DrawPage *p) const
{
    ...
    if (!viewBox.isValid()) { result = true; }
    else {
        width = viewBox.width(); //scaled rect w x h
        height = viewBox.height();
        width *= fudge;
        height *= fudge;
        if ((width > p->getPageWidth()) || (height > p->getPageHeight())) { result = false; }
    }
    return result;
}
...
void DrawView::setPosition(double x, double y, bool force)
{
    ...
    if ((!isLocked()) || (force)) {
        ...
        if (!DrawUtil::fpCompare(currY, y, 0.001)) { Y.setValue(y); }
    }
}
...
double DrawView::getScale() const
{
    ...
    if (ScaleType.isValue("Page")) {
        auto page = findParentPage();
        if (page) { result = page->Scale.getValue(); }
    }
    if (!(result > 0.0)) {
        result = 1.0;
...
void DrawView::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                         App::Property *prop)
{
    if (prop == &Scale) {
        ...
        if (strcmp(tmp.getTypeId().getName(), TypeName) == 0) { //property in file is Float
            ...
            if (tmpValue > 0.0) { Scale.setValue(tmpValue); }
            else {
                Scale.setValue(1.0);
            }
        }
...
bool DrawView::keepUpdated()
{
    ...
    if (overrideKeepUpdated()) { return true; }
    TechDraw::DrawPage *page = findParentPage();
    if (page) { return (page->canUpdate() || page->forceRedraw()); }
    return false;
}

void DrawView::setScaleAttribute()
{
    if (ScaleType.isValue("Page") || ScaleType.isValue("Automatic")) {
        Scale.setStatus(App::Property::ReadOnly, true);
    }
    else {
        Scale.setStatus(App::Property::ReadOnly, false);
...
{
    if (Preferences::reportProgress()) { signalProgressMessage(this, featureName, text); }
}

...
LineFormat::LineFormat(int style, double weight, App::Color color, bool visible)
    : m_style(style), m_weight(weight), m_color(color), m_visible(visible)
{}
...
{
    std::stringstream ss;
    ss << m_style << ", " << m_weight << ", " << m_color.asHexString() << ", " << m_visible;
    return ss.str();
}

//static preference getters.
double LineFormat::getDefEdgeWidth() { return TechDraw::LineGroup::getDefaultWidth("Graphic"); }

App::Color LineFormat::getDefEdgeColor() { return Preferences::normalColor(); }
...
CosmeticVertex::CosmeticVertex() : TechDraw::Vertex()
{
    ...
    size = Preferences::vertexScale() * LineGroup::getDefaultWidth("Thin");
    style = 1;
    visible = true;
    hlrVisible = true;
    cosmetic = true;
...
CosmeticVertex::CosmeticVertex(Base::Vector3d loc) : TechDraw::Vertex(loc)
{
    ...
    linkGeom = -1;
    color = Preferences::vertexColor();
    size = Preferences::vertexScale() * LineGroup::getDefaultWidth("Thick");
...
void CosmeticVertex::move(Base::Vector3d newPos) { permaPoint = newPos; }

void CosmeticVertex::moveRelative(Base::Vector3d movement) { permaPoint += movement; }

std::string CosmeticVertex::toString() const
{
    std::stringstream ss;
    ss << permaPoint.x << ", " << permaPoint.y << ", " << permaPoint.z << ", "
       << " / ";
...
{
    if (!CosmeticVertex::restoreCosmetic()) { return; }
...
Base::Vector3d CosmeticVertex::scaled(double factor) { return permaPoint * factor; }

boost::uuids::uuid CosmeticVertex::getTag() const { return tag; }

std::string CosmeticVertex::getTagAsString() const { return boost::uuids::to_string(getTag()); }
...
{
    if (cv->getTypeId() == this->getTypeId()) this->tag = cv->tag;
    else
...
std::string CosmeticEdge::toString() const
{
    ...
    ss << getTagAsString() << ", $$$, ";
    if (m_geometry) {
        ss << m_geometry->geomType << ", $$$, " << m_geometry->toString() << ", $$$, "
           << m_format.toString();
    }
    return ss.str();
}
...
// Persistence implementers
unsigned int CosmeticEdge::getMemSize() const { return 1; }
...
void CosmeticEdge::Save(Base::Writer &writer) const
{
    ...
    writer.Stream() << writer.ind() << "<Color value=\"" << m_format.m_color.asHexString() << "\"/>"
                    << endl;
    const char v = m_format.m_visible ? '1' : '0';
    writer.Stream() << writer.ind() << "<Visible value=\"" << v << "\"/>" << endl;

    writer.Stream() << writer.ind() << "<GeometryType value=\"" << m_geometry->geomType << "\"/>"
                    << endl;
...
boost::uuids::uuid CosmeticEdge::getTag() const { return tag; }

std::string CosmeticEdge::getTagAsString() const { return boost::uuids::to_string(getTag()); }
...
{
    if (ce->getTypeId() == this->getTypeId()) this->tag = ce->tag;
    else
...
CenterLine::CenterLine(TechDraw::BaseGeomPtr bg, int m, double h, double v, double r, double x)
{
...
CenterLine::CenterLine(Base::Vector3d pt1, Base::Vector3d pt2, int m, double h, double v, double r,
                       double x)
    : CenterLine(BaseGeomPtrFromVectors(pt1, pt2), m, h, v, r, x)
{}
...
CenterLine *CenterLine::CenterLineBuilder(DrawViewPart *partFeat, std::vector<std::string> subNames,
                                          int mode, bool flip)
{
    ...
    if (geomType == "Face") {
        type = CLTYPE::FACE;
        ends = TechDraw::CenterLine::calcEndPoints(partFeat, subNames, mode, 0.0, 0.0, 0.0, 0.0);
        faces = subNames;
    }
    else if (geomType == "Edge") {
        type = CLTYPE::EDGE;
        ends = TechDraw::CenterLine::calcEndPoints2Lines(partFeat, subNames, mode, 0.0, 0.0, 0.0,
                                                         0.0, flip);
        edges = subNames;
    }
    else if (geomType == "Vertex") {
        type = CLTYPE::VERTEX;
        ends = TechDraw::CenterLine::calcEndPoints2Points(partFeat, subNames, mode, 0.0, 0.0, 0.0,
                                                          0.0, flip);
        verts = subNames;
    }
...
TechDraw::BaseGeomPtr CenterLine::scaledGeometry(TechDraw::DrawViewPart *partFeat)
{
    ...
    std::pair<Base::Vector3d, Base::Vector3d> ends;
    try {
        if (m_faces.empty() && m_edges.empty() && m_verts.empty()) {
            //            Base::Console().Message("CL::scaledGeometry - no geometry to scale!\n");
            //CenterLine was created by points without a geometry reference,
            ends =
                calcEndPointsNoRef(m_start, m_end, scale, m_extendBy, m_hShift, m_vShift, m_rotate);
        }
        else if (m_type == CLTYPE::FACE) {
            ends =
                calcEndPoints(partFeat, m_faces, m_mode, m_extendBy, m_hShift, m_vShift, m_rotate);
        }
        else if (m_type == CLTYPE::EDGE) {
            ends = calcEndPoints2Lines(partFeat, m_edges, m_mode, m_extendBy, m_hShift, m_vShift,
                                       m_rotate, m_flip2Line);
        }
        else if (m_type == CLTYPE::VERTEX) {
            ends = calcEndPoints2Points(partFeat, m_verts, m_mode, m_extendBy, m_hShift, m_vShift,
                                        m_rotate, m_flip2Line);
        }
    }
...
{
    std::stringstream ss;
    ss << m_start.x << ", " << m_start.y << ", " << m_start.z << ", " << m_end.x << ", " << m_end.y
       << ", " << m_end.z << ", " << m_mode << ", " << m_type << ", " << m_hShift << ", "
       << m_vShift << ", " << m_rotate << ", " << m_flip2Line << ", " << m_extendBy << ", "
       << m_faces.size();
    if (!m_faces.empty()) {
        for (auto &f : m_faces) {
            if (!f.empty()) { ss << ", " << f; }
...
std::tuple<Base::Vector3d, Base::Vector3d> CenterLine::rotatePointsAroundMid(Base::Vector3d p1,
                                                                             Base::Vector3d p2,
                                                                             Base::Vector3d mid,
                                                                             double rotate)
{
...
CenterLine::calcEndPointsNoRef(Base::Vector3d start, Base::Vector3d end, double scale, double ext,
                               double hShift, double vShift, double rotate)
{
...
std::pair<Base::Vector3d, Base::Vector3d>
CenterLine::calcEndPoints(DrawViewPart *partFeat, std::vector<std::string> faceNames, int mode,
                          double ext, double hShift, double vShift, double rotate)
{
    ...
    for (auto &fn : faceNames) {
        if (TechDraw::DrawUtil::getGeomTypeFromName(fn) != "Face") { continue; }
        int idx = TechDraw::DrawUtil::getIndexFromName(fn);
        std::vector<TechDraw::BaseGeomPtr> faceEdges = partFeat->getFaceEdgesByIndex(idx);
        if (!faceEdges.empty()) {
            for (auto &fe : faceEdges) {
                if (!fe->cosmetic) { BRepBndLib::AddOptimal(fe->occEdge, faceBox); }
            }
        }
    }
...
std::pair<Base::Vector3d, Base::Vector3d>
CenterLine::calcEndPoints2Lines(DrawViewPart *partFeat, std::vector<std::string> edgeNames,
                                int mode, double ext, double hShift, double vShift, double rotate,
                                bool flip)
...
std::pair<Base::Vector3d, Base::Vector3d>
CenterLine::calcEndPoints2Points(DrawViewPart *partFeat, std::vector<std::string> vertNames,
                                 int mode, double ext, double hShift, double vShift, double rotate,
                                 bool flip)

{
    ...
    for (auto &vn : vertNames) {
        if (TechDraw::DrawUtil::getGeomTypeFromName(vn) != "Vertex") { continue; }
...
// Persistence implementers
unsigned int CenterLine::getMemSize() const { return 1; }

void CenterLine::Save(Base::Writer &writer) const
{
    writer.Stream() << writer.ind() << "<Start "
                    << "X=\"" << m_start.x << "\" Y=\"" << m_start.y << "\" Z=\"" << m_start.z
                    << "\"/>" << endl;
    writer.Stream() << writer.ind() << "<End "
                    << "X=\"" << m_end.x << "\" Y=\"" << m_end.y << "\" Z=\"" << m_end.z << "\"/>"
                    << endl;
    ...
    writer.Stream() << writer.ind() << "<Faces "
                    << "FaceCount=\"" << m_faces.size() << "\">" << endl;

    writer.incInd();
    for (auto &f : m_faces) {
        writer.Stream() << writer.ind() << "<Face value=\"" << f << "\"/>" << endl;
    }
    writer.decInd();

    writer.Stream() << writer.ind() << "</Faces>" << endl;

    writer.Stream() << writer.ind() << "<Edges "
                    << "EdgeCount=\"" << m_edges.size() << "\">" << endl;

    writer.incInd();
    for (auto &e : m_edges) {
        writer.Stream() << writer.ind() << "<Edge value=\"" << e << "\"/>" << endl;
    }
    writer.decInd();
    writer.Stream() << writer.ind() << "</Edges>" << endl;

    writer.Stream() << writer.ind() << "<CLPoints "
                    << "CLPointCount=\"" << m_verts.size() << "\">" << endl;

    writer.incInd();
    for (auto &p : m_verts) {
        writer.Stream() << writer.ind() << "<CLPoint value=\"" << p << "\"/>" << endl;
    }
    writer.decInd();
    ...
    //stored geometry
    if (!m_geometry) { return Base::Console().Error("CL::Save - m_geometry is null\n"); }
...
boost::uuids::uuid CenterLine::getTag() const { return tag; }

std::string CenterLine::getTagAsString() const { return boost::uuids::to_string(getTag()); }
...
double CenterLine::getHShift() { return m_hShift; }

double CenterLine::getVShift() { return m_vShift; }

void CenterLine::setRotate(double r) { m_rotate = r; }

double CenterLine::getRotate() { return m_rotate; }

void CenterLine::setExtend(double e) { m_extendBy = e; }

double CenterLine::getExtend() { return m_extendBy; }

void CenterLine::setFlip(bool f) { m_flip2Line = f; }

bool CenterLine::getFlip() { return m_flip2Line; }
//------------------------------------------------------------------------------

TYPESYSTEM_SOURCE(TechDraw::GeomFormat, Base::Persistence)

GeomFormat::GeomFormat() : m_geomIndex(-1)
{
...
GeomFormat::GeomFormat(int idx, TechDraw::LineFormat fmt) : m_geomIndex(idx)
{
...
{
    std::stringstream ss;
    ss << m_geomIndex << ", $$$, " << m_format.toString();
    return ss.str();
}

// Persistence implementer
unsigned int GeomFormat::getMemSize() const { return 1; }
...
boost::uuids::uuid GeomFormat::getTag() const { return tag; }

std::string GeomFormat::getTagAsString() const { return boost::uuids::to_string(getTag()); }

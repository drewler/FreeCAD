...
TopoDS_Face Face::toOccFace() const
{
    ...
    for (; iwire < limit; iwire++) {
        ...
        if (!wOCC.IsNull()) { mkFace.Add(wOCC); }
    }
    if (mkFace.IsDone()) { return mkFace.Face(); }
    return TopoDS_Face();
}

Face::~Face()
{
    for (auto it : wires) { delete it; }
    wires.clear();
}

BaseGeom::BaseGeom()
    : geomType(NOTDEF), extractType(Plain),                              //obs
      classOfEdge(ecNONE), hlrVisible(true), reversed(false), ref3D(-1), //obs?
      cosmetic(false), m_source(0), m_sourceIndex(-1)
{
    occEdge = TopoDS_Edge();
...
BaseGeomPtr BaseGeom::copy()
{
    ...
    if (!occEdge.IsNull()) {
        ...
        if (!result) { result = std::make_shared<BaseGeom>(); }
    }
...
{
    std::stringstream ss;
    ss << geomType << ", " << extractType << ", " << classOfEdge << ", " << hlrVisible << ", "
       << reversed << ", " << ref3D << ", " << cosmetic << ", " << m_source << ", "
       << m_sourceIndex;
    return ss.str();
}

boost::uuids::uuid BaseGeom::getTag() const { return tag; }

std::string BaseGeom::getTagAsString() const { return boost::uuids::to_string(getTag()); }
...
Base::Vector3d BaseGeom::getMidPoint()
{
    ...
    GCPnts_AbscissaPoint abscissa(Precision::Confusion(), curve,
                                  GCPnts_AbscissaPoint::Length(curve) / 2.0,
...
Base::Vector3d BaseGeom::nearPoint(const BaseGeomPtr p)
{
    ...
    if (!extss.IsDone() || extss.NbSolution() == 0) { return Base::Vector3d(0.0, 0.0, 0.0); }
...
std::string BaseGeom::dump()
{
    ...
    ss << "BaseGeom: s:(" << start.x << ", " << start.y << ") e:(" << end.x << ", " << end.y
       << ") ";
    ss << "type: " << geomType << " class: " << classOfEdge << " viz: " << hlrVisible
       << " rev: " << reversed;
...
{
    Base::Vector3d start(getStartPoint().x, getStartPoint().y, 0.0);
    Base::Vector3d end(getEndPoint().x, getEndPoint().y, 0.0);
    if (start.IsEqual(end, 0.00001)) { return true; }
    return false;
}
...
{
    if (edge.IsNull()) { Base::Console().Message("BG::baseFactory - input edge is NULL \n"); }
    //weed out rubbish edges before making geometry
    if (!validateEdge(edge)) { return nullptr; }
    ...
    switch (adapt.GetType()) {
        ...
        case GeomAbs_BezierCurve: {
            ...
            if (edge.Orientation() == TopAbs_REVERSED) { result->reversed = true; }
        ...
        case GeomAbs_BSplineCurve: {
            ...
            try {
                ...
                if (bspline->isLine()) { result = std::make_shared<Generic>(edge); }
                else {
                    circEdge = bspline->asCircle(isArc);
                    if (!circEdge.IsNull()) {
                        if (isArc) { result = std::make_shared<AOC>(circEdge); }
                        else {
                ...
                Base::Console().Error(
                    "Geom::baseFactory - unknown error occurred while making spline\n");
                break;
            }
            break;
        } // end bspline case
...
bool BaseGeom::validateEdge(TopoDS_Edge edge) { return !DrawUtil::isCrazy(edge); }
...
void BaseGeom::intersectionLL(TechDraw::BaseGeomPtr geom1, TechDraw::BaseGeomPtr geom2,
                              std::vector<Base::Vector3d> &interPoints)
{
...
void BaseGeom::intersectionCL(TechDraw::BaseGeomPtr geom1, TechDraw::BaseGeomPtr geom2,
                              std::vector<Base::Vector3d> &interPoints)
{
...
void BaseGeom::intersectionCC(TechDraw::BaseGeomPtr geom1, TechDraw::BaseGeomPtr geom2,
                              std::vector<Base::Vector3d> &interPoints)
{
...
Ellipse::Ellipse(Base::Vector3d c, double mnr, double mjr)
{
    ...
    if (!me.IsDone()) { Base::Console().Message("G:Ellipse - failed to make Ellipse\n"); }
    const Handle(Geom_Ellipse) gEllipse = me.Value();
    BRepBuilderAPI_MakeEdge mkEdge(gEllipse, 0.0, 2 * M_PI);
    if (mkEdge.IsDone()) { occEdge = mkEdge.Edge(); }
}
...
AOE::AOE(const TopoDS_Edge &e) : Ellipse(e)
{
    ...
    if (e.Orientation() == TopAbs_REVERSED) { reversed = true; }
}
...
AOC::AOC(Base::Vector3d c, double r, double sAng, double eAng) : Circle()
{
    ...
    endPnt = Base::Vector3d(ePt.X(), ePt.Y(), s.Z());
    midPnt = Base::Vector3d(m.X(), m.Y(), s.Z());
    if (edge.Orientation() == TopAbs_REVERSED) { reversed = true; }
}
...
bool AOC::isOnArc(Base::Vector3d p)
{
    ...
    TopoDS_Vertex v = BRepBuilderAPI_MakeVertex(pnt);
    BRepExtrema_DistShapeShape extss(occEdge, v);
    if (!extss.IsDone() || extss.NbSolution() == 0) { return false; }
    double minDist = extss.Value();
    if (minDist < Precision::Confusion()) { return true; }
    return false;
}

double AOC::distToArc(Base::Vector3d p) { return minDist(p); }
...
std::string AOC::toString() const
{
    ...
    ss << startPnt.x << ", " << startPnt.y << ", " << startPnt.z << ", " << endPnt.x << ", "
       << endPnt.y << ", " << endPnt.z << ", " << midPnt.x << ", " << midPnt.y << ", " << midPnt.z
       << ", " << startAngle << ", " << endAngle << ", " << cw << ", " << largeArc;
...
void AOC::Save(Base::Writer &writer) const
{
    ...
    writer.Stream() << writer.ind() << "<Start "
                    << "X=\"" << startPnt.x << "\" Y=\"" << startPnt.y << "\" Z=\"" << startPnt.z
                    << "\"/>" << endl;
    writer.Stream() << writer.ind() << "<End "
                    << "X=\"" << endPnt.x << "\" Y=\"" << endPnt.y << "\" Z=\"" << endPnt.z
                    << "\"/>" << endl;
    writer.Stream() << writer.ind() << "<Middle "
                    << "X=\"" << midPnt.x << "\" Y=\"" << midPnt.y << "\" Z=\"" << midPnt.z
                    << "\"/>" << endl;
...
Generic::Generic() { geomType = GENERIC; }
...
std::string Generic::toString() const
{
    ...
    std::stringstream ss;
    ss << points.size() << ", ";
    for (auto &p : points) { ss << p.x << ", " << p.y << ", " << p.z << ", "; }
...
{
    BaseGeom::Save(writer);
    writer.Stream() << writer.ind() << "<Points PointsCount =\"" << points.size() << "\">" << endl;
    writer.incInd();
    for (auto &p : points) {
...
Base::Vector3d Generic::asVector() { return getEndPoint() - getStartPoint(); }

double Generic::slope()
{
    Base::Vector3d v = asVector();
    if (v.x == 0.0) { return DOUBLE_MAX; }
    else {
        return v.y / v.x;
    }
}
...
Base::Vector3d Generic::apparentInter(GenericPtr g)
{
    ...
    if ((det > 0 ? det : -det) < 1e-10) throw Base::ValueError("Invalid selection - Det = 0");
...
BSpline::BSpline(const TopoDS_Edge &e)
{
    ...
    if (approx.IsDone() && approx.HasResult()) { spline = approx.Curve(); }
        ...
        ePt = c.Value(l);
        Base::Console().Log(
            "Error - Geometry::BSpline - no result- from:(%.3f, %.3f) to:(%.3f, %.3f) poles: %d\n",
...
bool BSpline::isLine()
{
    ...
    if (DrawUtil::fpCompare(lenTotal, endLength)) { return true; }
    return false;
}
...
TopoDS_Edge BSpline::asCircle(bool &arc)
{
    ...
    try {
        ...
        if (gce_circ1.Status() != gce_Done) { return result; }
        ...
        if (gce_circ2.Status() != gce_Done) { return result; }
        ...
        Base::Console().Log(
            "Geom::asCircle - unknown error occurred while approx spline as circle\n");
        TopoDS_Edge nullReturn;
        result = nullReturn;
    }
    return result;
...
BezierSegment::BezierSegment(const TopoDS_Edge &e)
{
    ...
    poles = bez->NbPoles();
    degree = bez->Degree();
    if (poles > 4) { Base::Console().Log("Warning - BezierSegment has degree > 3: %d\n", degree); }
...
boost::uuids::uuid Vertex::getTag() const { return tag; }

std::string Vertex::getTagAsString() const { return boost::uuids::to_string(getTag()); }

void Vertex::dump(const char *title)
{
    Base::Console().Message(
        "TD::Vertex - %s - point: %s vis: %d cosmetic: %d  cosLink: %d cosTag: %s\n", title,
    ...
    if (geoms.size() == 1) {
        ...
        for (unsigned int i = 1; i < geoms.size(); i++) { //do size-1 more edges
            ...
            if (next.index) { //found an unused edge with vertex == atPoint
                ...
                if (next.reversed) { atPoint = nextEdge->getStartPoint(); }
                else {
...
/*static*/ GeometryUtils::ReturnType GeometryUtils::nextGeom(Base::Vector3d atPoint,

 ...
 */
template<typename OutputIt> void copy(Py::Dict sourceRange, OutputIt targetIt)
{
    string key;
    string value;
...
public:
    Module() : Py::ExtensionModule<Module>("TechDraw")
    {
        add_varargs_method("edgeWalker", &Module::edgeWalker,
                           "[wires] = edgeWalker(edgePile, inclBiggest) -- Planar graph traversal "
                           "finds wires in edge pile.");
        add_varargs_method("findOuterWire", &Module::findOuterWire,
                           "wire = findOuterWire(edgeList) -- Planar graph traversal finds "
                           "OuterWire in edge pile.");
        add_varargs_method("findShapeOutline", &Module::findShapeOutline,
                           "wire = findShapeOutline(shape, scale, direction) -- Project shape in "
                           "direction and find outer wire of result.");
        add_varargs_method("viewPartAsDxf", &Module::viewPartAsDxf,
                           "string = viewPartAsDxf(DrawViewPart) -- Return the edges of a "
                           "DrawViewPart in Dxf format.");
        add_varargs_method("viewPartAsSvg", &Module::viewPartAsSvg,
                           "string = viewPartAsSvg(DrawViewPart) -- Return the edges of a "
                           "DrawViewPart in Svg format.");
        ...
        add_varargs_method("findCentroid", &Module::findCentroid,
                           "vector = findCentroid(shape, direction): finds geometric centroid of "
                           "shape looking in direction.");
        add_varargs_method("makeExtentDim", &Module::makeExtentDim,
                           "makeExtentDim(DrawViewPart, [edges], direction) -- draw horizontal or "
                           "vertical extent dimension for edges (or all of DrawViewPart if edge "
                           "list is empty. direction:  0 - Horizontal, 1 - Vertical.");
        add_varargs_method(
            "makeDistanceDim", &Module::makeDistanceDim,
            "makeDistanceDim(DrawViewPart, dimType, fromPoint, toPoint) -- draw a Length dimension "
            "between fromPoint to toPoint.  FromPoint and toPoint are unscaled 2d View points. "
            "dimType is one of ['Distance', 'DistanceX', 'DistanceY'.");
        add_varargs_method(
            "makeDistanceDim3d", &Module::makeDistanceDim3d,
            "makeDistanceDim(DrawViewPart, dimType, 3dFromPoint, 3dToPoint) -- draw a Length "
            "dimension between fromPoint to toPoint.  FromPoint and toPoint are unscaled 3d model "
            "points. dimType is one of ['Distance', 'DistanceX', 'DistanceY'.");
        add_varargs_method(
            "makeGeomHatch", &Module::makeGeomHatch,
            "makeGeomHatch(face, [patScale], [patName], [patFile]) -- draw a geom hatch on a given "
            "face, using optionally the given scale (default 1) and a given pattern name (ex. "
            "Diamond) and .pat file (the default pattern name and/or .pat files set in preferences "
            "are used if none are given). Returns a Part compound shape.");
        add_varargs_method("project", &Module::project,
                           "[visiblyG0, visiblyG1, hiddenG0, hiddenG1] = project(TopoShape[, "
                           "App.Vector Direction, string type])\n"
                           " -- Project a shape and return the visible/invisible parts of it.");
        add_varargs_method("projectEx", &Module::projectEx,
                           "[V, V1, VN, VO, VI, H,H1, HN, HO, HI] = projectEx(TopoShape[, "
                           "App.Vector Direction, string type])\n"
                           " -- Project a shape and return the all parts of it.");
        add_keyword_method(
            "projectToSVG", &Module::projectToSVG,
        ...
        add_varargs_method(
            "removeSvgTags", &Module::removeSvgTags,
...
private:
    ...
    Py::Object edgeWalker(const Py::Tuple &args)
    {
        ...
        bool biggie = false;
        if (inclBig == Py_True) { biggie = true; }
    ...
    Py::Object findShapeOutline(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "OdO", &pcObjShape, &scale, &pcObjDir)) {
    ...
    Py::Object viewPartAsSvg(const Py::Tuple &args)
    {
        ...
        Py::String svgReturn;
        std::string grpHead1 =
            "<g fill=\"none\" stroke=\"#000000\" stroke-opacity=\"1\" stroke-width=\"";
        std::string grpHead2 =
        ...
        try {
            ...
            if (PyObject_TypeCheck(viewObj, &(TechDraw::DrawViewPartPy::Type))) {
                ...
                if (dvp->HardHidden.getValue() || dvp->SmoothHidden.getValue()
    ...
    void write1ViewDxf(ImpExpDxfWrite &writer, TechDraw::DrawViewPart *dvp, bool alignPage)
    {
        ...
        for (auto &g : geoms) {
            if (g->hlrVisible && g->cosmetic) { cosmeticEdges.push_back(g->occEdge); }
        }
    ...
    Py::Object writeDXFView(const Py::Tuple &args)
    {
        ...
        PyMem_Free(name);
        bool align = false;
        if (alignObj == Py_True) { align = true; }
            ...
            if (PyObject_TypeCheck(pageObj, &(TechDraw::DrawPagePy::Type))) {
                ...
                for (auto &view : views) {
                    if (view->isDerivedFrom(TechDraw::DrawViewPart::getClassTypeId())) {
                        ...
                        TechDraw::DrawViewAnnotation *dva =
                            static_cast<TechDraw::DrawViewAnnotation *>(view);
                        ...
                        if (dvp->isDerivedFrom(TechDraw::DrawProjGroupItem::getClassTypeId())) {
                            TechDraw::DrawProjGroupItem *dpgi =
                                static_cast<TechDraw::DrawProjGroupItem *>(dvp);
                            TechDraw::DrawProjGroup *dpg = dpgi->getPGroup();
                            if (!dpg) { continue; }
                        ...
                        if (dvd->Type.isValue("Distance") || dvd->Type.isValue("DistanceX")
                            || dvd->Type.isValue("DistanceY")) {
                            Base::Vector3d textLocn(dvd->X.getValue() + parentX,
                                                    dvd->Y.getValue() + parentY, 0.0);
                            Base::Vector3d lineLocn(dvd->X.getValue() + parentX,
                                                    dvd->Y.getValue() + parentY, 0.0);
                            ...
                            Base::Vector3d extLine1Start =
                                Base::Vector3d(pts.first.x, -pts.first.y, 0.0)
                                + Base::Vector3d(parentX, parentY, 0.0);
                            Base::Vector3d extLine2Start =
                                Base::Vector3d(pts.second.x, -pts.second.y, 0.0)
                                + Base::Vector3d(parentX, parentY, 0.0);
                            if (dvd->Type.isValue("DistanceX")) { type = 1; }
                            else if (dvd->Type.isValue("DistanceY")) {
                                type = 2;
                            }
                            writer.exportLinearDim(textLocn, lineLocn, extLine1Start, extLine2Start,
                                                   dimText, type);
                        }
    ...
    Py::Object findCentroid(const Py::Tuple &args)
    {
        PyObject *pcObjShape(nullptr);
        PyObject *pcObjDir(nullptr);
        if (!PyArg_ParseTuple(args.ptr(), "OO", &pcObjShape, &pcObjDir)) {
    ...
    Py::Object makeExtentDim(const Py::Tuple &args)
    {
        ...
        if (PyObject_TypeCheck(pDvp, &(TechDraw::DrawViewPartPy::Type))) {
            App::DocumentObject *obj =
                static_cast<App::DocumentObjectPy *>(pDvp)->getDocumentObjectPtr();
            dvp = static_cast<TechDraw::DrawViewPart *>(obj);
        }
        ...
        DrawDimHelper::makeExtentDim(dvp, edgeList, direction);
        return Py::None();
    }
    ...
    Py::Object makeDistanceDim(const Py::Tuple &args)
    {
        ...
        if (PyUnicode_Check(pDimType)) { dimType = PyUnicode_AsUTF8(pDimType); }
        ...
        DrawViewDimension *dvd = DrawDimHelper::makeDistDim(dvp, dimType, DrawUtil::invertY(from),
                                                            DrawUtil::invertY(to));
    ...
    Py::Object makeDistanceDim3d(const Py::Tuple &args)
    {
        ...
        //DrawViewDimension* =
        DrawDimHelper::makeDistDim(dvp, dimType, from, to);
    ...
    Py::Object makeGeomHatch(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "O|detet", &pFace, &scale, "utf-8", &pPatName, "utf-8",
                              &pPatFile)) {
        ...
        if (PyObject_TypeCheck(pFace, &(TopoShapeFacePy::Type))) {
            const TopoDS_Shape &shape =
                static_cast<TopoShapePy *>(pFace)->getTopoShapePtr()->getShape();
            face = TopoDS::Face(shape);
        }
        else {
            throw Py::TypeError("first argument must be a Part.Face instance");
        }
        if (patName.empty()) { patName = TechDraw::DrawGeomHatch::prefGeomHatchName(); }
        if (patFile.empty()) { patFile = TechDraw::DrawGeomHatch::prefGeomHatchFile(); }
        Base::FileInfo fi(patFile);
        if (!fi.isReadable()) {
            Base::Console().Error(".pat File: %s is not readable\n", patFile.c_str());
            return Py::None();
        }
        std::vector<TechDraw::PATLineSpec> specs =
            TechDraw::DrawGeomHatch::getDecodedSpecsFromFile(patFile, patName);
        ...
        std::vector<LineSet> lsresult =
            TechDraw::DrawGeomHatch::getTrimmedLines(source, lineSets, face, scale);
        if (!lsresult.empty()) {
            ...
            */
            ...
            try {
                for (auto &lsr : lsresult) {
                    std::vector<TopoDS_Edge> edgeList = lsr.getEdges();
                    for (auto &edge : edgeList) {
                        if (!edge.IsNull()) { builder.Add(comp, edge); }
                    }
                }
            }
            catch (Base::Exception &e) {
    ...
    Py::Object project(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "O!|O!", &(Part::TopoShapePy::Type), &pcObjShape,
                              &(Base::VectorPy::Type), &pcObjDir))
            throw Py::Exception();

        Part::TopoShapePy *pShape = static_cast<Part::TopoShapePy *>(pcObjShape);
        Base::Vector3d Vector(0, 0, 1);
        if (pcObjDir) Vector = *static_cast<Base::VectorPy *>(pcObjDir)->getVectorPtr();
    ...
    Py::Object projectEx(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "O!|O!", &(TopoShapePy::Type), &pcObjShape,
                              &(Base::VectorPy::Type), &pcObjDir))
            throw Py::Exception();
    ...
    {
        static char *argNames[] = {"topoShape", "direction", "type",    "tolerance",
                                   "vStyle",    "v0Style",   "v1Style", "hStyle",
                                   "h0Style",   "h1Style",   nullptr};
        PyObject *pcObjShape = nullptr;
        PyObject *pcObjDir = nullptr;
        ...
        if (!PyArg_ParseTupleAndKeywords(args.ptr(), keys.ptr(), "O!|O!sfOOOOOO", argNames,
                                         &(TopoShapePy::Type), &pcObjShape, &(Base::VectorPy::Type),
                                         &pcObjDir, &extractionTypePy, &tol, &vStylePy, &v0StylePy,
                                         &v1StylePy, &hStylePy, &h0StylePy, &h1StylePy))
        ...
        if (pcObjDir) directionVector = static_cast<Base::VectorPy *>(pcObjDir)->value();

        if (extractionTypePy && std::string(extractionTypePy) == "ShowHiddenLines")
            extractionType = ProjectionAlgos::WithHidden;

        if (vStylePy) copy(Py::Dict(vStylePy), inserter(vStyle, vStyle.begin()));
        if (v0StylePy) copy(Py::Dict(v0StylePy), inserter(v0Style, v0Style.begin()));
        if (v1StylePy) copy(Py::Dict(v1StylePy), inserter(v1Style, v1Style.begin()));
        if (hStylePy) copy(Py::Dict(hStylePy), inserter(hStyle, hStyle.begin()));
        if (h0StylePy) copy(Py::Dict(h0StylePy), inserter(h0Style, h0Style.begin()));
        if (h1StylePy) copy(Py::Dict(h1StylePy), inserter(h1Style, h1Style.begin()));
    ...
    Py::Object projectToDXF(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "O!|O!sff", &(TopoShapePy::Type), &pcObjShape,
                              &(Base::VectorPy::Type), &pcObjDir, &type, &scale, &tol))
            throw Py::Exception();

        TopoShapePy *pShape = static_cast<TopoShapePy *>(pcObjShape);
        Base::Vector3d Vector(0, 0, 1);
        if (pcObjDir) Vector = static_cast<Base::VectorPy *>(pcObjDir)->value();
        ProjectionAlgos Alg(pShape->getTopoShapePtr()->getShape(), Vector);

        bool hidden = false;
        if (type && std::string(type) == "ShowHiddenLines") hidden = true;
    ...
    {
        const char *svgcode;
        if (!PyArg_ParseTuple(args.ptr(), "s", &svgcode)) throw Py::Exception();

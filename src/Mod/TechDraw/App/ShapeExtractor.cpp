...
ShapeExtractor::getShapes2d(const std::vector<App::DocumentObject *> links)
{
    //    Base::Console().Message("SE::getShapes2d()\n");

    std::vector<TopoDS_Shape> shapes2d;
    if (!prefAdd2d()) { return shapes2d; }
    for (auto &l : links) {
...
TopoDS_Shape ShapeExtractor::getShapes(const std::vector<App::DocumentObject *> links)
{
    ...
    for (auto &l : links) {
        if (l->getTypeId().isDerivedFrom(App::Link::getClassTypeId())) {
            ...
            if (!shape.IsNull()) { sourceShapes.push_back(shape); }
            else {
    ...
    if (!found) { Base::Console().Error("ShapeExtractor failed to get shape.\n"); }
    else {
        result = comp;
    }
...
std::vector<TopoDS_Shape> ShapeExtractor::getXShapes(const App::Link *xLink)
{
    ...
    if (!xLink) { return xSourceShapes; }

    bool needsTransform = false;
    std::vector<App::DocumentObject *> children = xLink->getLinkedChildren();
    Base::Placement
        linkPlm; // default constructor is an identity placement, i.e. no rotation nor translation
    if (xLink->hasPlacement()) {
        linkPlm = xLink->getLinkPlacementProperty()->getValue();
        needsTransform = true;
    }
    Base::Matrix4D
        linkScale; // default constructor is an identity matrix, possibly scale it with link's scale
        ...
        for (auto &l : children) {
            ...
            if (!shape.IsNull()) {
                if (needsTransform || childNeedsTransform) {
                    ...
                    netTransform =
                        (linkPlm.toMatrix() * linkScale) * (childPlm.toMatrix() * childScale);
...
std::vector<TopoDS_Shape> ShapeExtractor::getShapesFromObject(const App::DocumentObject *docObj)
{
    ...
    if (docObj->getTypeId().isDerivedFrom(Part::Feature::getClassTypeId())) {
        ...
        for (auto &d : objs) {
            ...
            if (!shapes.empty()) { result.insert(result.end(), shapes.begin(), shapes.end()); }
        }
...
{
    bool result = false;
    if (isEdgeType(obj) || isPointType(obj)) { result = true; }
    return result;
}
...
bool ShapeExtractor::isPointType(App::DocumentObject *obj)
{
    ...
    if (obj) {
        Base::Type t = obj->getTypeId();
        if (t.isDerivedFrom(Part::Vertex::getClassTypeId())) { result = true; }
...
bool ShapeExtractor::isDraftPoint(App::DocumentObject *obj)
{
    ...
    if (proxy) {
        ...
        if (pp.find("Point") != std::string::npos) { result = true; }
    }
    return result;
}
...
Base::Vector3d ShapeExtractor::getLocation3dFromFeat(App::DocumentObject *obj)
{
    ...
    if (!isPointType(obj)) { return result; }
    //    if (isDraftPoint(obj) {

...
//* edgeVisior methods
//*******************************************************
template<typename Edge> void edgeVisitor::next_edge(Edge e)
{
...
TechDraw::ewWireList edgeVisitor::getResult() { return graphWires; }

void edgeVisitor::setGraph(TechDraw::graph &g) { m_g = g; }
...
bool EdgeWalker::loadEdges(std::vector<TopoDS_Edge> edges)
{
    ...
    if (edges.empty()) { throw Base::ValueError("EdgeWalker has no edges to load\n"); }
...
bool EdgeWalker::prepare()
{
    ...
    for (boost::tie(ei, ei_end) = edges(m_g); ei != ei_end; ++ei) put(e_index, *ei, edge_count++);

    //make planar embedding
    planar_embedding_storage_t planar_embedding_storage(num_vertices(m_g));
    planar_embedding_t planar_embedding(planar_embedding_storage.begin(), get(vertex_index, m_g));
    for (auto &e : m_embedding) {
        for (auto &i : e.incidenceList) { planar_embedding[e.iVertex].push_back(i.eDesc); }
    }
    ...
    bool isPlanar = boyer_myrvold_planarity_test(
        boyer_myrvold_params::graph = m_g,
        boyer_myrvold_params::embedding =
            &embedding[0], // this is "an" embedding but not one for finding
...
std::vector<TopoDS_Wire> EdgeWalker::getResultWires()
{
    ...
    ewWireList result = m_eV.getResult();
    if (result.wires.empty()) { return fw; }
...
TopoDS_Wire EdgeWalker::makeCleanWire(std::vector<TopoDS_Edge> edges, double tol)
{
    ...
    for (const auto &e : edges) { wireData->Add(e); }
...
std::vector<TopoDS_Vertex> EdgeWalker::makeUniqueVList(std::vector<TopoDS_Edge> edges)
{
    ...
    for (auto &e : edges) {
        ...
        for (const auto &v : uniqueVert) {
            ...
            if (v3d.IsEqual(v1, EWTOLERANCE)) { addv1 = false; }
            if (v3d.IsEqual(v2, EWTOLERANCE)) { addv2 = false; }
        }
        if (addv1) { uniqueVert.push_back(TopExp::FirstVertex(e)); }
        if (addv2) { uniqueVert.push_back(TopExp::LastVertex(e)); }
    }
                                                    ...
                                                    std::vector<TopoDS_Vertex> verts)
{
    ...
    for (const auto &e : edges) {
        ...
        if (vertex1Index == SIZE_MAX) { continue; }
        std::size_t vertex2Index = findUniqueVert(edgeVertex2, verts);
        if (vertex2Index == SIZE_MAX) { continue; }
...
std::vector<TopoDS_Wire> EdgeWalker::sortStrip(std::vector<TopoDS_Wire> fw, bool includeBiggest)
{
    ...
    if (!includeBiggest) { sortedWires.erase(sortedWires.begin()); }
...
std::vector<TopoDS_Wire> EdgeWalker::sortWiresBySize(std::vector<TopoDS_Wire> &w, bool ascend)
{
    ...
    if (ascend) { std::reverse(wires.begin(), wires.end()); }
    return wires;
}
...
std::vector<int> EdgeWalker::getEmbeddingRowIx(int v)
{
    ...
    for (auto &ii : ei.incidenceList) { result.push_back(ii.iEdge); }
    return result;
}
...
std::vector<edge_t> EdgeWalker::getEmbeddingRow(int v)
{
    ...
    embedItem ei = m_embedding[v];
    for (auto &ii : ei.incidenceList) { result.push_back(ii.eDesc); }
    return result;
}
...
{
    bool result = false;
    if (((v1 == w.v1) && (v2 == w.v2)) || ((v1 == w.v2) && (v2 == w.v1))) { result = true; }
    return result;
}
...
{
    bool result = true;
    if (wedges.size() != w2.wedges.size()) { result = false; }
    else {
...
void ewWire::push_back(WalkerEdge w) { wedges.push_back(w); }

std::size_t ewWire::size(void) { return wedges.size(); }
...
ewWireList ewWireList::removeDuplicateWires()
{
    ...
    for (; iWire != wires.end(); iWire++) {
        ...
        if (addToResult) { result.push_back((*iWire)); }
    }
    return result;
}

void ewWireList::push_back(ewWire w) { wires.push_back(w); }

std::size_t ewWireList::size(void) { return wires.size(); }
...
std::vector<incidenceItem> embedItem::sortIncidenceList(std::vector<incidenceItem> &list,
                                                        bool ascend)
{
    ...
    if (ascend) { std::reverse(tempList.begin(), tempList.end()); }
    return tempList;
}

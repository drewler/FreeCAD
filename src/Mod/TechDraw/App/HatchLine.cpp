...
Base::Vector3d LineSet::calcApparentStart(TechDraw::BaseGeomPtr g)
{
    ...
    else if ((angle == 90.0) || (angle == -90.0)) { //vertical
...
{
    Base::Vector3d result;
    Base::Vector3d start(m_geoms.at(0)->getStartPoint().x, m_geoms.at(0)->getStartPoint().y, 0.0);
    Base::Vector3d end(m_geoms.at(0)->getEndPoint().x, m_geoms.at(0)->getEndPoint().y, 0.0);
    result = end - start;
    result.Normalize();
    return result;
}
...
Base::Vector3d LineSet::getUnitOrtho()
{
    ...
    if (unit.IsEqual(X, 0.000001)) { result = Y; }
...
Base::Vector3d LineSet::findAtomStart()
{
    ...
    if (angle == 0.0) { result = Base::Vector3d(getMinX(), origin.y, 0.0); }
...
Base::Vector3d LineSet::getPatternStartPoint(TechDraw::BaseGeomPtr g, double &offset, double scale)
{
    ...
    lineOrigin =
        lineOrigin + (overlayIndex * (scale * getOffset())) * getUnitDir(); //move along line
    ...
    if ((lenStartOrg <= lenStartEnd) && (lenEndOrg <= lenStartEnd)) { //origin is in g
        result = lineOrigin;
        offset = 0.0;
    }
    else {
        ...
        double patsStartOrg =
            lenStartOrg / patternLength; //# pattern repeats from lineOrigin to start
        ...
        if (lenStartOrg < lenEndOrg) { //origin is before start
            ...
            if (c <= patsStartOrg) { //c is an integer pattern count in [patsStartOrg, patsEndOrg]
                ...
                result = gStart;
                offset = ceil(patsStartOrg)
                    - patsStartOrg; //fraction of a patternLength patstartorg to repeat point
                offset = offset * patternLength;
            }
        }
        else {
                                                         ...
                                                         std::string &parmName)
{
    ...
    if (status) { lineSpecs = loadPatternDef(inFile); }
    else {
...
bool PATLineSpec::findPatternStart(std::ifstream &inFile, std::string &parmName)
{
    ...
    while (inFile.good()) {
        ...
        std::size_t commaPos;
        if ((nameTag == ";") || (nameTag == " ") || (line.empty())) { //is cr/lf empty?
            continue;
        }
        else if (nameTag == "*") {
            commaPos = line.find(',', 1);
            if (commaPos != std::string::npos) { patternName = line.substr(1, commaPos - 1); }
            else {
...
{
    if (getAngle() == 0.0) { return getInterval(); }
    else if ((getAngle() == 90.0) || (getAngle() == -90.0)) {
        return 0.0;
    }
    else {

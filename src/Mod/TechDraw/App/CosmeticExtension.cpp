...
CosmeticExtension::CosmeticExtension()
{
    ...
    EXTENSION_ADD_PROPERTY_TYPE(CosmeticVertexes, (nullptr), cgroup, App::Prop_Output,
                                "CosmeticVertex Save/Restore");
    EXTENSION_ADD_PROPERTY_TYPE(CosmeticEdges, (nullptr), cgroup, App::Prop_Output,
                                "CosmeticEdge Save/Restore");
    EXTENSION_ADD_PROPERTY_TYPE(CenterLines, (nullptr), cgroup, App::Prop_Output,
                                "Geometry format Save/Restore");
    EXTENSION_ADD_PROPERTY_TYPE(GeomFormats, (nullptr), cgroup, App::Prop_Output,
                                "Geometry format Save/Restore");
...
void CosmeticExtension::removeCosmeticVertex(std::string delTag)
{
    ...
    for (auto &cv : cVerts) {
        if (cv->getTagAsString() != delTag) { newVerts.push_back(cv); }
    }
...
{
    for (auto &t : delTags) { removeCosmeticVertex(t); }
}
...
std::string CosmeticExtension::addCosmeticEdge(Base::Vector3d start, Base::Vector3d end)
{
...
TechDraw::CosmeticEdge *CosmeticExtension::getCosmeticEdgeBySelection(std::string name) const
{
    ...
    if (!base->getCosmeticTag().empty()) result = getCosmeticEdge(base->getCosmeticTag());
    return result;
}
...
void CosmeticExtension::removeCosmeticEdge(std::string delTag)
{
    ...
    for (auto &ce : cEdges) {
        if (ce->getTagAsString() != delTag) { newEdges.push_back(ce); }
    }
...
{
    for (auto &t : delTags) { removeCosmeticEdge(t); }
}
...
std::string CosmeticExtension::addCenterLine(Base::Vector3d start, Base::Vector3d end)
{
...
TechDraw::CenterLine *CosmeticExtension::getCenterLineBySelection(std::string name) const
{
    ...
    TechDraw::BaseGeomPtr base = dvp->getGeomByIndex(idx);
    if (!base) return result;
    if (!base->getCosmeticTag().empty()) result = getCenterLine(base->getCosmeticTag());
    return result;
}
...
void CosmeticExtension::removeCenterLine(std::string delTag)
{
    ...
    for (auto &cl : cLines) {
        if (cl->getTagAsString() != delTag) { newLines.push_back(cl); }
    }
...
{
    for (auto &t : delTags) { removeCenterLine(t); }
}
...
void CosmeticExtension::removeGeomFormat(std::string delTag)
{
    ...
    for (auto &gf : cFormats) {
        if (gf->getTagAsString() != delTag) { newFormats.push_back(gf); }
    }

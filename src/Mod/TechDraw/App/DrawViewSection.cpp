...
const char *DrawViewSection::SectionDirEnums[] = {"Right", "Left", "Up", "Down", nullptr};
...
DrawViewSection::DrawViewSection() : m_waitingForCut(false)
{
    ...
    ADD_PROPERTY_TYPE(SectionDirection, ((long)0), sgroup, App::Prop_None,
                      "Direction in Base View for this Section");
    ADD_PROPERTY_TYPE(FuseBeforeCut, (false), sgroup, App::Prop_None,
...
{
    if (isRestoring()) { return TechDraw::DrawView::mustExecute(); }

    if (Scale.isTouched() || Direction.isTouched() || BaseView.isTouched()
        || SectionNormal.isTouched() || SectionOrigin.isTouched()) {
        return 1;
    }
...
void DrawViewSection::onChanged(const App::Property *prop)
{
    ...
    if (prop == &SectionSymbol) {
        std::string lblText = "Section " + std::string(SectionSymbol.getValue()) + " - "
            + std::string(SectionSymbol.getValue());
        Label.setValue(lblText);
    }
    else if (prop == &CutSurfaceDisplay) {
        if (CutSurfaceDisplay.isValue("PatHatch")) { makeLineSets(); }
    }
    else if (prop == &FileHatchPattern) {
        if (!FileHatchPattern.isEmpty()) {
            Base::FileInfo fi(FileHatchPattern.getValue());
            if (fi.isReadable()) { replaceSvgIncluded(FileHatchPattern.getValue()); }
        }
    }
    else if (prop == &FileGeomPattern) {
        if (!FileGeomPattern.isEmpty()) {
            Base::FileInfo fi(FileGeomPattern.getValue());
            if (fi.isReadable()) { replacePatIncluded(FileGeomPattern.getValue()); }
        }
    }
    else if (prop == &FileGeomPattern || prop == &NameGeomPattern) {
        makeLineSets();
    }
...
App::DocumentObjectExecReturn *DrawViewSection::execute()
{
    ...
    if (!base) { return new App::DocumentObjectExecReturn("BaseView object not found"); }
    ...
    if (FuseBeforeCut.getValue()) { baseShape = dvp->getSourceShapeFused(); }

    if (baseShape.IsNull()) { return DrawView::execute(); }
...
void DrawViewSection::sectionExec(TopoDS_Shape &baseShape)
{
    ...
    if (waitingForHlr() || waitingForCut()) { return; }
...
void DrawViewSection::makeSectionCut(TopoDS_Shape &baseShape)
{
    ...
    try {
        ...
        gp_Pnt inputCenter;
        inputCenter = TechDraw::findCentroid(rawShape, m_viewAxis);
        Base::Vector3d centroid(inputCenter.X(), inputCenter.Y(), inputCenter.Z());

        centeredShape = TechDraw::moveShape(rawShape, centroid * -1.0);
...
void DrawViewSection::postHlrTasks(void)
{
    ...
    TopoDS_Shape centeredShapeF = TechDraw::moveShape(faceIntersections, m_saveCentroid * -1.0);

    TopoDS_Shape scaledSection = TechDraw::scaleShape(centeredShapeF, getScale());
    if (!DrawUtil::fpCompare(Rotation.getValue(), 0.0)) {
        scaledSection = TechDraw::rotateShape(scaledSection, m_viewAxis, Rotation.getValue());
    }
    if (debugSection()) {
    ...
    gp_Ax3 proj3 = gp_Ax3(gp_Pnt(0.0, 0.0, 0.0), projCS.Direction(), projCS.XDirection());
    gp_Trsf fromR3;
    fromR3.SetTransformation(R3, proj3);
    BRepBuilderAPI_Transform xformer(fromR3);
    xformer.Perform(scaledSection, true);
    if (xformer.IsDone()) { sectionTopoDSFaces = TopoDS::Compound(xformer.Shape()); }
    else {
        Base::Console().Message("DVS::sectionExec - face xform failed\n");
    }

    sectionTopoDSFaces =
        TopoDS::Compound(GeometryObject::invertGeometry(sectionTopoDSFaces)); //handle Qt -y
...
{
    if (DrawViewPart::waitingForResult() || waitingForCut()) { return true; }
    return false;
}
...
{
    if (v.x <= bb.MinX || v.x >= bb.MaxX) return false;
    if (v.y <= bb.MinY || v.y >= bb.MaxY) return false;
    if (v.z <= bb.MinZ || v.z >= bb.MaxZ) return false;
    return true;
}
...
gp_Ax2 DrawViewSection::getCSFromBase(const std::string sectionName) const
{
    ...
    if (debugSection()) { DrawUtil::dumpCS("DVS::getCSFromBase - dvp CS", dvpCS); }
    ...
    if (debugSection()) { DrawUtil::dumpCS("DVS::getCSFromBase - sectionCS out", CS); }
...
TopoDS_Face DrawViewSection::getSectionTopoDSFace(int i)
{
    ...
    for (; expl.More(); expl.Next(), count++) {
        if (count == i + 1) { result = TopoDS::Face(expl.Current()); }
    }
    return result;
}
...
void DrawViewSection::onDocumentRestored()
{
    ...
    if (SvgIncluded.isEmpty()) {
        if (!FileHatchPattern.isEmpty()) {
            ...
            if (tfi.isReadable()) { setupSvgIncluded(); }
        }
    }
    ...
    if (PatIncluded.isEmpty()) {
        if (!FileGeomPattern.isEmpty()) {
            ...
            if (tfi.isReadable()) { setupPatIncluded(); }
        }
    }
...
void DrawViewSection::replaceSvgIncluded(std::string newSvgFile)
{
    ...
    if (SvgIncluded.isEmpty()) { setupSvgIncluded(); }
    else {
...
void DrawViewSection::replacePatIncluded(std::string newPatFile)
{
    ...
    if (PatIncluded.isEmpty()) { setupPatIncluded(); }
    else {

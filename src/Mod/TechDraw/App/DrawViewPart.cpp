...
TopoDS_Shape DrawViewPart::getSourceShape() const
{
    ...
    const std::vector<App::DocumentObject *> &links = getAllSources();
    if (links.empty()) { return TopoDS_Shape(); }
...
std::vector<App::DocumentObject *> DrawViewPart::getAllSources() const
{
    ...
    if (!xLinks.empty()) { result.insert(result.end(), xLinks.begin(), xLinks.end()); }
    return result;
}
...
App::DocumentObjectExecReturn *DrawViewPart::execute(void)
{
    ...
    if (waitingForHlr()) { return DrawView::execute(); }
...
void DrawViewPart::onHlrFinished(void)
{
    ...
    if (geometryObject) { delete geometryObject; }
...
void DrawViewPart::postHlrTasks(void)
{
    ...
    for (auto &d : dims) { d->recomputeFeature(); }
    std::vector<TechDraw::DrawViewBalloon *> bals = getBalloons();
    for (auto &b : bals) { b->recomputeFeature(); }
...
void DrawViewPart::extractFaces()
{
    ...
    if (newFaceFinder()) {
        ...
        if (sortedWires.empty()) {
            ...
            BRepTools::Write(DrawUtil::vectorToCompound(sortedWires),
                             "DVPSortedWires.brep"); //debug
            ...
            for (; itWire != sortedWires.end(); itWire++) {
                ...
                if (geometryObject) { geometryObject->addFaceGeom(f); }
            }
        }
    }
        ...
        for (auto &e : copyEdges) { //drop any zero edges (shouldn't be any by now!!!)
            if (!DrawUtil::isZeroEdge(e)) { nonZero.push_back(e); }
            else {
...
const std::vector<TechDraw::VertexPtr> DrawViewPart::getVertexGeometry() const
{
    ...
    if (geometryObject) { result = geometryObject->getVertexGeometry(); }
    return result;
}
...
const std::vector<TechDraw::FacePtr> DrawViewPart::getFaceGeometry() const
{
    ...
    if (waitingForFaces() || !geometryObject) { return std::vector<TechDraw::FacePtr>(); }
    return geometryObject->getFaceGeometry();
}

const BaseGeomPtrVector DrawViewPart::getEdgeGeometry() const
{
    if (geometryObject) { return geometryObject->getEdgeGeometry(); }
...
Base::Vector3d DrawViewPart::projectPoint(const Base::Vector3d &pt, bool invert) const
{
    ...
    if (invert) { result = DrawUtil::invertY(result); }
    return result;
}
...
std::vector<DrawViewDetail *> DrawViewPart::getDetailRefs() const
{
    ...
    for (auto &o : inObjs) {
        if (o->getTypeId().isDerivedFrom(DrawViewDetail::getClassTypeId())) {
            if (!o->isRemoving()) { result.push_back(static_cast<TechDraw::DrawViewDetail *>(o)); }
        }
    }
    return result;
}
...
void DrawViewPart::removeReferenceVertex(std::string tag)
{
    ...
    for (auto &v : m_referenceVerts) {
        if (v->getTagAsString() != tag) { newRefVerts.push_back(v); }
        else {
...
void DrawViewPart::removeAllReferencesFromGeom()
{
    ...
    if (!m_referenceVerts.empty()) {
        ...
        for (auto &gv : gVerts) {
            if (!gv->reference) { newVerts.push_back(gv); }
        }
...
void DrawViewPart::refreshCEGeoms()
{
    ...
    for (auto &ge : gEdges) {
        if (ge->source() != SourceType::COSEDGE) { oldGEdges.push_back(ge); }
    }
...
void DrawViewPart::refreshCLGeoms()
{
    ...
    for (auto &ge : gEdges) {
        if (ge->source() != SourceType::CENTERLINE) { newGEdges.push_back(ge); }
    }
...
void DrawViewPart::dumpCosVerts(std::string text)
{
    ...
    for (auto &cv : cVerts) { cv->dump("a CV"); }
}
...
void DrawViewPart::dumpCosEdges(std::string text)
{
    ...
    for (auto &ce : cEdges) { ce->dump("a CE"); }
}

...
TopoDS_Edge TechDrawOutput::asCircle(const BRepAdaptor_Curve &c) const
{
    ...
    if (!polygon.IsNull()) {
        ...
        for (int i = nodes.Lower(); i <= nodes.Upper(); i++) {
            ...
            if (std::abs(dist - radius) > 0.001) return TopoDS_Edge();
        }
...
std::string SVGOutput::exportEdges(const TopoDS_Shape &input)
{
    ...
    for (int i = 1; edges.More(); edges.Next(), i++) {
        ...
        if (adapt.GetType() == GeomAbs_Circle) { printCircle(adapt, result); }
...
void SVGOutput::printBezier(const BRepAdaptor_Curve &c, int id, std::ostream &out)
{
    try {
        ...
        str << p1.X() << ", " << p1.Y();
        if (bezier->Degree() == 3) {
            if (poles != 4) Standard_Failure::Raise("do it the generic way");
            ...
            if (poles != 3) Standard_Failure::Raise("do it the generic way");
            gp_Pnt p2 = bezier->Pole(2);
            gp_Pnt p3 = bezier->Pole(3);
            str << " Q" << p2.X() << ", " << p2.Y() << " " << p3.X() << ", " << p3.Y() << " ";
        }
        else if (bezier->Degree() == 1) {
            if (poles != 2) Standard_Failure::Raise("do it the generic way");
            gp_Pnt p2 = bezier->Pole(2);
            str << " L" << p2.X() << ", " << p2.Y() << " ";
        }
        else {
...
void DXFOutput::printBSpline(const BRepAdaptor_Curve &c, int id,
    ...
    try {
        ...
        for (int i = poles.Lower(); i <= poles.Upper(); i++) {
            ...
            str << 10 << endl
                ...
                << 30 << endl
                << pole.Z() << endl;
            if (spline->IsRational()) { str << 41 << endl << spline->Weight(i) << endl; }
        }

...
PropertyGeomFormatList::~PropertyGeomFormatList()
{
    for (std::vector<GeomFormat *>::iterator it = _lValueList.begin(); it != _lValueList.end();
         ++it)
        if (*it) delete *it;
}
...
int PropertyGeomFormatList::getSize() const { return static_cast<int>(_lValueList.size()); }
...
void PropertyGeomFormatList::setValues(const std::vector<GeomFormat *> &lValue)
{
    ...
    // copy all objects
    for (unsigned int i = 0; i < lValue.size(); i++) _lValueList[i] = lValue[i]->clone();
    for (unsigned int i = 0; i < oldVals.size(); i++) delete oldVals[i];
    hasSetValue();
}
...
void PropertyGeomFormatList::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (reader.testStatus(Base::XMLReader::ReaderStatus::PartialRestoreInObject)) {
            Base::Console().Error("GeomFormat \"%s\" within a PropertyGeomFormatList was subject "

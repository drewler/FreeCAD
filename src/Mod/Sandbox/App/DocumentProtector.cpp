...
{
public:
    AbstractCustomProtectorEvent() : QEvent(QEvent::Type(QT_CUSTOM_EVENT_PROTECTOR)), semaphore(0)
    {}
...
{
public:
    CustomAddObjectEvent(App::DocumentObject **o, App::Document *d, const std::string &type,
                         const std::string &name)
        : obj(o), doc(d), type(type), name(name)
    {}
    ~CustomAddObjectEvent() {}
    void execute() { *obj = doc->addObject(this->type.c_str(), this->name.c_str()); }
...
public:
    ...
    void execute() { doc->removeObject(this->name.c_str()); }
...
{
public:
    CustomPropertyEvent(App::Property &p, const App::Property &v) : property(p), value(v) {}
    ~CustomPropertyEvent() {}
    void execute() { property.Paste(value); }
...
{
public:
    CustomCallableEvent(const AbstractCallable &call) : callable(call) {}
    ~CustomCallableEvent() {}
...
{
public:
    CustomPurgeEvent(App::DocumentObject *o) : obj(o) {}
    ~CustomPurgeEvent() {}
    void execute() { obj->purgeTouched(); }
...
DocumentReceiver *DocumentReceiver::globalInstance() { return theInstance(); }
...
DocumentProtector::DocumentProtector(App::Document *doc) : App::DocumentObserver(doc) {}
...
void DocumentProtector::init()
{
    ...
    DocumentReceiver::globalInstance()->moveToThread(QCoreApplication::instance()->thread());
}

void DocumentProtector::slotDeletedDocument(const App::Document &Doc)
{
    if (&Doc == getDocument()) { this->detachDocument(); }
}

void DocumentProtector::validate()
{
    if (!this->getDocument()) throw Base::ValueError("Handled document is null");
}
...
{
    if (!obj) throw Base::ValueError("Handled document object is null");
}

App::DocumentObject *DocumentObjectProtector::getObject() const { return this->obj; }

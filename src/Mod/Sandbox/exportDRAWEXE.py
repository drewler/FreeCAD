...
class Drawexporter(object):
    ...
    def saveSweep(self, ob):
        ...
        if explodeshape:
            ...
            if len(subshapelst) and spine.Shape.ShapeType != "Edge":
                ...
                edgelst = " ".join(
                    ("%s_%s" % (spine.Name, ss[4:]) for ss in subshapelst)
                )
            ...
            if len(subshapelst):
                path = Part.Wire(
                    [
                        spine.Shape.getElement(subshapename)
                        for subshapename in subshapelst
                    ]
                )
            elif spine.Shape.ShapeType == "Edge":
                path = spine.Shape
    ...
    def process_object(self, ob, checksupported=False, toplevel=False):
        ...
        if ob.TypeId in ["Part::Cut", "Part::Fuse", "Part::Common", "Part::Section"]:
            ...
            if (
                ob.Angle1.Value == -90
                and ob.Angle2.Value == 90
                and ob.Angle3.Value == 360
            ):
                self.csg.write("psphere %(name)s %(radius)s\n" % d1)
            else:
                self.csg.write(
                    "psphere %(name)s %(radius)s %(angle1)s "
                    "%(angle2)s %(angle3)s\n" % d1
                )
            ...
            if ob.Angle.Value == 360:
                ...
                self.csg.write(
                    "pcylinder %(name)s %(radius)s %(height)s " "%(angle)s\n" % d1
                )
            ...
            if ob.Angle.Value == 360:
                self.csg.write(
                    "pcone %(name)s %(radius1)s %(radius2)s " "%(height)s\n" % d1
                )
            else:
                self.csg.write(
                    "pcone %(name)s %(radius1)s %(radius2)s "
                    "%(height)s %(angle)s\n" % d1
                )
            ...
            self.csg.write(
                "tmirror %(name)s %(x)s %(y)s %(z)s %(dx)s %(dy)s %(dz)s\n" % d1
            )
            ...
            self.csg.write(
                ...
                % (
                    ...
                    " ".join(
                        (
                            "%s %s" % (f2s(e[1]), "%s_%d" % (ob.Base.Name, e[0]))
                            for e in ob.Edges
                        )
                    ),
                )
            ...
            ob.TypeId == "Part::Thickness"
            and not ob.SelfIntersection
            and ob.Mode == "Skin"
        ):
            if checksupported:
                return True  # The object is supported
            ...
            pointstr = " ".join(
                "%s %s %s" % (f2s(v.x), f2s(v.y), f2s(v.z)) for v in points
            )
            self.csg.write("polyline %s %s\n" % (polyname, pointstr))
            if ob.MakeFace:
            ...
            if len(ob.Objects) == 1:
                ...
                if x == y == z:  # uniform scaling
                    ...
                    self.csg.write(
                        "deform %(name)s %(basename)s" " %(cx)s %(cy)s %(cz)s\n" % d1
                    )
            else:  # compound
                newnames = []
            ...
            if ob.HiddenLines:
                self.csg.write(
                    "compound vl v1l vnl vol vil hl h1l hnl hol hil %s\n" % ob.Name
                )
            else:
        ...
        #    pass
            ...
            self.csg.write(
                "#saved shape of unsupported %s Object\n" % formatobjtype(ob)
            )

...
SceneEventFilter::SceneEventFilter(QObject *parent) : QObject(parent)
{
    ...
    PRIVATE(this)->windowsize =
        SbVec2s(PRIVATE(this)->scene->width(), PRIVATE(this)->scene->height());
...
void SceneEventFilter::unregisterInputDevice(SIM::Coin3D::Quarter::InputDevice *device)
{
    ...
    if (i != -1) { PRIVATE(this)->devices.removeAt(i); }
}
...
/*! Translates Qt Events into Coin events and passes them on to the
  ...
  returns false.
 */
bool SceneEventFilter::eventFilter(QObject *, QEvent *qevent)
{
    ...
    switch (qevent->type()) {
        ...
        case QEvent::GraphicsSceneMouseMove: {
            ...
            sceneev.reset(new QMouseEvent(QEvent::MouseMove, ev->pos().toPoint(), ev->button(),
...
GraphicsScene::GraphicsScene() : m_backgroundColor(0, 170, 255), m_lastTime(0), m_distance(1.4f)
{
    ...
    instructions->layout()->addWidget(
        new QLabel(tr("Use mouse wheel to zoom model, and click and drag to rotate model")));
    instructions->layout()->addWidget(
        new QLabel(tr("Move the sun around to change the light position")));
...
void GraphicsScene::viewAll()
{
    ...
    if (cam) cam->viewAll(sceneNode, sorendermanager->getViewportRegion());
}

SceneEventFilter *GraphicsScene::getEventFilter(void) const { return eventfilter; }

bool GraphicsScene::processSoEvent(const SoEvent *event)
{
    if (event) { return soeventmanager && soeventmanager->processEvent(event); }
...
void GraphicsScene::setNavigationModeFile(const QUrl &url)
{
    ...
    if (url.scheme() == QLatin1String("coin")) {
        ...
        //scheme in Coin 3.0.
        if (filename[0] == QLatin1Char('/')) { filename.remove(0, 1); }
...
SoNode *GraphicsScene::getSceneGraph() const { return sceneNode; }

SoCamera *GraphicsScene::searchForCamera(SoNode *root)
{
    SoSearchAction sa;
...
SoRenderManager *GraphicsScene::getSoRenderManager(void) const { return sorendermanager; }

SoEventManager *GraphicsScene::getSoEventManager(void) const { return soeventmanager; }
...
void GraphicsScene::drawBackground(QPainter *painter, const QRectF &)
{
    if (painter->paintEngine()->type() != QPaintEngine::OpenGL
        ...
        qWarning("GraphicsScene: drawBackground needs a QGLWidget to be set as viewport on the "
                 "graphics view");
        return;
    }
    ...
    : Gui::MDIView(doc, parent), m_scene(new GraphicsScene()), m_view(new GraphicsView)
{
    ...
    m_scene->setNavigationModeFile(
        QUrl(QString::fromLatin1("coin:///scxml/navigation/examiner.xml")));

...
private:
    void slotChangedObject(const App::DocumentObject &Obj, const App::Property &Prop)
    {
        ...
        if (object == &Obj && Prop.getTypeId() == Part::PropertyGeometryList::getClassTypeId()) {
            const Part::PropertyGeometryList &geom =
                static_cast<const Part::PropertyGeometryList &>(Prop);
            const std::vector<Part::Geometry *> &items = geom.getValues();
            if (items.size() != 2) return;
            Part::Geometry *g1 = items[0];
            Part::Geometry *g2 = items[1];
            if (!g1 || g1->getTypeId() != Part::GeomArcOfCircle::getClassTypeId()) return;
            if (!g2 || g2->getTypeId() != Part::GeomLineSegment::getClassTypeId()) return;
...
public:
    Module() : Py::ExtensionModule<Module>("SandboxGui")
    {
        ...
        add_varargs_method("xmlReader", &Module::xmlReader, "Read XML");
    ...
    Py::Object interactiveFilletArc(const Py::Tuple & /*args*/)
    {
        ...
        if (doc) {
            ...
            if (view) {
                ...
                ObjectObserver *obs =
                    new ObjectObserver(doc->getDocument()->getActiveObject(), coords);

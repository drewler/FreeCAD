...
def VShape(SketchFeature):
    ...
    SketchFeature.addGeometry(
        Part.LineSegment(App.Vector(1, 1, 0), App.Vector(2, 0, 0))
    )
...
def BoxCircle(SketchFeature):
    ...
    SketchFeature.fillet(
        top_edge, right_edge, top_midpoint, right_midpoint, 0.25, True, True
    )
...
class TestSketchFillet(unittest.TestCase):
    ...
    def testUnconnected(self):
        ...
        SketchFeature.addConstraint(Sketcher.Constraint("DistanceY", 0, 1, 0, 2, 1))
        SketchFeature.addGeometry(
            Part.LineSegment(App.Vector(2.1, 1, 0), App.Vector(3, 0, 0))
        )
        SketchFeature.addConstraint(Sketcher.Constraint("DistanceX", 1, 1, 1, 2, 0.9))
        SketchFeature.addConstraint(Sketcher.Constraint("DistanceY", 1, 1, 1, 2, -1))

        SketchFeature.addGeometry(
            Part.LineSegment(App.Vector(3, 3, 0), App.Vector(5, 3, 0))
        )
    ...
    def testHorizontalVertical(self):
        SketchFeature = self.Doc.addObject(
            "Sketcher::SketchObject", "HorizontalVertical"
        )
        # Inverted V
        SketchFeature.addGeometry(
            Part.LineSegment(App.Vector(0, 0, 0), App.Vector(1, 1, 0))
    ...
    def testTangent(self):
        ...
        SketchFeature.addConstraint(Sketcher.Constraint("Coincident", 0, 1, -1, 1))
        SketchFeature.addConstraint(
            Sketcher.Constraint("DistanceX", first_line, 2, -1, 1, -1)
        )
        SketchFeature.addConstraint(
            Sketcher.Constraint("DistanceY", first_line, 2, -1, 1, -1)
        )
        ...
        SketchFeature.addConstraint(
            Sketcher.Constraint("DistanceX", second_line, 2, -1, 1, -2)
        )
        SketchFeature.addConstraint(
        ...
        SketchFeature.addGeometry(
            Part.LineSegment(App.Vector(2, 2, 0), App.Vector(3, 3, 0))
        )
        SketchFeature.addConstraint(
            Sketcher.Constraint("Tangent", tangent_line, first_line)
        )
        SketchFeature.addConstraint(
            Sketcher.Constraint("Distance", tangent_line, 1.41421356237)
        )
        ...
        SketchFeature.addConstraint(
            Sketcher.Constraint("DistanceX", tangent_line, 1, -1, 1, -4)
        )
        self.Doc.recompute()

        # The first endpoint should now be at 4,4
        self.assertAlmostEqual(
            App.Vector(4, 4, 0).distanceToPoint(
                SketchFeature.getPoint(tangent_line, 1)
            ),
            0.0,
        )
        ...
        try:
            self.assertAlmostEqual(
                App.Vector(3, 3, 0).distanceToPoint(
                    SketchFeature.getPoint(tangent_line, 2)
                ),
                0.0,
            )
        except AssertionError:
            self.assertAlmostEqual(
                App.Vector(5, 5, 0).distanceToPoint(
                    SketchFeature.getPoint(tangent_line, 2)
                ),
                0.0,
            )

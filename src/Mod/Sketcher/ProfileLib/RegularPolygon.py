...
def makeRegularPolygon(
    ...
    if sides < 3:
        App.Console.PrintError(
            "Number of sides must be at least 3 in 'makeRegularPolygon'"
        )
        return
    ...
    for i in range(0, sides - 1):
        conList.append(
            Sketcher.Constraint("Coincident", geoIndices[i], 2, geoIndices[i + 1], 1)
        )
    conList.append(
        Sketcher.Constraint("Coincident", geoIndices[sides - 1], 2, geoIndices[0], 1)
    )
    for i in range(0, sides - 1):
        conList.append(Sketcher.Constraint("Equal", geoIndices[0], geoIndices[i + 1]))
    for i in range(0, sides):
        conList.append(
            Sketcher.Constraint("PointOnObject", geoIndices[i], 2, geoIndices[-1])
        )

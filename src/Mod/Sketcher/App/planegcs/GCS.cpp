...
#if EIGEN_VERSION                                                                                  \
    > 30290 // This regulates that only starting in Eigen 3.3, the problem with       \
...
template<> FullPivLU<MatrixdType> &FullPivLU<MatrixdType>::compute(const MatrixdType &matrix)
{
...
SolverReportingManager::SolverReportingManager() { initStream(); }
...
{
    if (str.size() < Base::Console().BufferSize) Base::Console().Log(str.c_str());
    else
    ...
    const std::string &str, std::vector<std::vector<Constraint *>> constraintgroups)
{
    ...
    for (const auto &group : constraintgroups) {
        ...
        for (auto c : group) tempstream << c->getTag() << " ";
                                                 ...
                                                 std::set<Constraint *> constraintset)
{
    ...
    for (auto c : constraintset) tempstream << c->getTag() << " ";
...
void SolverReportingManager::LogGroupOfParameters(
    const std::string &str, std::vector<std::vector<double *>> parametergroups)
{
    std::stringstream tempstream;
    ...
    for (size_t i = 0; i < parametergroups.size(); i++) {
        ...
        for (auto p : parametergroups[i]) tempstream << std::hex << p << " ";
...
// System
System::System()
    : plist(0), pdrivenlist(0), pDependentParameters(0), clist(0), c2p(), p2c(), subSystems(0),
      subSystemsAux(0), reference(0), dofs(0), hasUnknowns(false), hasDiagnosis(false),
      isInit(false), emptyDiagnoseMatrix(true), maxIter(100), maxIterRedundant(100),
      sketchSizeMultiplier(false), sketchSizeMultiplierRedundant(false), convergence(1e-10),
      convergenceRedundant(1e-10), qrAlgorithm(EigenSparseQR), dogLegGaussStep(FullPivLU),
      qrpivotThreshold(1E-13), debugMode(Minimal), LM_eps(1E-10), LM_eps1(1E-80), LM_tau(1E-3),
      DL_tolg(1E-80), DL_tolx(1E-80), DL_tolf(1E-10), LM_epsRedundant(1E-10),
      LM_eps1Redundant(1E-80), LM_tauRedundant(1E-3), DL_tolgRedundant(1E-80),
      DL_tolxRedundant(1E-80), DL_tolfRedundant(1E-10)
{
...
void System::clearByTag(int tagId)
{
    ...
    for (std::vector<Constraint *>::const_iterator constr = clist.begin(); constr != clist.end();
         ++constr) {
        if ((*constr)->getTag() == tagId) constrvec.push_back(*constr);
    }
...
void System::removeConstraint(Constraint *constr)
{
    ...
    if (it == clist.end()) return;

    clist.erase(it);
    if (constr->getTag() >= 0) hasDiagnosis = false;
    clearSubSystems();
...
int System::addConstraintTangent(Circle &c, Arc &a, int tagId, bool driving)
{
    ...
    double d = sqrt(dx * dx + dy * dy);
    return addConstraintTangentCircumf(c.center, a.center, c.rad, a.rad, (d < *c.rad || d < *a.rad),
                                       tagId, driving);
}
...
int System::addConstraintInternalAlignmentBSplineControlPoint(BSpline &b, Circle &c, int poleindex,
                                                              int tagId, bool driving)
{
...
double System::calculateConstraintErrorByTag(int tagId)
{
    ...
    switch (cnt) {
        ...
        default: return sqrt(sqErr / (double)cnt);
    }
}

void System::rescaleConstraint(int id, double coeff)
{
    if (id >= static_cast<int>(clist.size()) || id < 0) return;
    if (clist[id]) clist[id]->rescale(coeff);
}
...
void System::declareUnknowns(VEC_pD &params)
{
    plist = params;
    pIndex.clear();
    for (int i = 0; i < int(plist.size()); ++i) pIndex[plist[i]] = i;
    hasUnknowns = true;
}

void System::declareDrivenParams(VEC_pD &params) { pdrivenlist = params; }
...
void System::initSolution(Algorithm alg)
{
    ...
    if (!redundant.empty()) {
        for (std::vector<Constraint *>::const_iterator constr = clist.begin();
            ...
            if (redundant.count(*constr) == 0) clistR.push_back(*constr);
        }
    }
    else
        clistR = clist;
    ...
    Graph g;
    for (int i = 0; i < int(plist.size() + clistR.size()); i++) boost::add_vertex(g);
    ...
    for (std::vector<Constraint *>::const_iterator constr = clistR.begin(); constr != clistR.end();
        ...
        for (VEC_pD::const_iterator param = cparams.begin(); param != cparams.end(); ++param) {
            ...
            if (it != pIndex.end()) boost::add_edge(cvtid, it->second, g);
        }
    }

    VEC_I components(boost::num_vertices(g));
    int componentsSize = 0;
    if (!components.empty()) componentsSize = boost::connected_components(g, &components[0]);
            ...
            if ((*constr)->getTag() >= 0 && (*constr)->getTypeId() == Equal) {
                ...
                if (it1 != pIndex.end() && it2 != pIndex.end()) {
                    ...
                    for (int i = 0; i < int(plist.size()); ++i) {
                        if (reducedParams[i] == p_replaced) reducedParams[i] = p_kept;
                    }
                }
            }
        }
    ...
    for (std::size_t cid = 0; cid < clists.size(); cid++) {
        ...
        for (std::vector<Constraint *>::const_iterator constr = clists[cid].begin();
            ...
            if ((*constr)->getTag() >= 0) clist0.push_back(*constr);
...
void System::resetToReference()
{
    if (reference.size() == plist.size()) {
        ...
        for (; ref != reference.end(); ++ref, ++param) **param = *ref;
    }
}
...
int System::solve_BFGS(SubSystem *subsys, bool /*isFine*/, bool isRedundantsolving)
{
    ...
    if (err <= smallF) return Success;
    if (h.norm() <= (isRedundantsolving ? convergenceRedundant : convergence)) return Converged;
    return Failed;
}
...
int System::solve_LM(SubSystem *subsys, bool isRedundantsolving)
{
    ...
    for (iter = 0; iter < maxIterNumber && !stop; ++iter) {
        ...
        if (iter == 0) mu = tau * diag_A.lpNorm<Eigen::Infinity>();
        ...
        while (k < 50) {
            ...
            for (int i = 0; i < xsize; ++i) A(i, i) += mu;
    ...
    while (!stop) {
        ...
        if (fx_inf <= tolf) // Success
            ...
            switch (dogLegGaussStep) {
                case FullPivLU: h_gn = Jx.fullPivLu().solve(-fx); break;
            ...
            if (h_gn.norm() < delta) {
                ...
                if (gb > 0) beta = c / (gb + sqrt(gb * gb + c * bb));
                else
...
int System::solve(SubSystem *subsysA, SubSystem *subsysB, bool /*isFine*/, bool isRedundantsolving)
{
    ...
    VEC_pD plistAB(xsizeA + xsizeB);
    {
        ...
        it = std::set_union(plistA.begin(), plistA.end(), plistB.begin(), plistB.end(),
                            plistAB.begin());
    ...
    int ret;
    if (subsysA->error() <= smallF) ret = Success;
...
{
    for (int cid = 0; cid < int(subSystems.size()); cid++) {
        if (subSystemsAux[cid]) subSystemsAux[cid]->applySolution();
        if (subSystems[cid]) subSystems[cid]->applySolution();
...
void System::undoSolution() { resetToReference(); }
                                 ...
                                 GCS::VEC_pD &pdiagnoselist, std::map<int, int> &tagmultiplicity)
{
    ...
    for (int j = 0; j < int(plist.size()); j++) {
        ...
        if (result1 == std::end(pdrivenlist)) { pdiagnoselist.push_back(plist[j]); }
    }
...
int System::diagnose(Algorithm alg)
{
    ...
    if (J.rows() > 0) emptyDiagnoseMatrix = false;
    ...
    if (qrAlgorithm == EigenDenseQR) {
        ...
        if (J.rows() > 0) {
            ...
            //
            auto fut = std::async(&System::identifyDependentParametersDenseQR, this, J,
                                  jacobianconstraintmap, pdiagnoselist, true);
            ...
            if (constrNum > rank) { // conflicting or redundant constraints
                ...
                identifyConflictingRedundantConstraints(alg, qrJT, jacobianconstraintmap,
                                                        tagmultiplicity, pdiagnoselist, R,
                                                        constrNum, rank, nonredundantconstrNum);
        ...
        if (J.rows() > 0) {
            ...
            auto fut = std::async(&System::identifyDependentParametersSparseQR, this, J,
                                  jacobianconstraintmap, pdiagnoselist, /*silent=*/true);

            makeSparseQRDecomposition(J, jacobianconstraintmap, SqrJT, rank, R, /*transposed=*/true,
                                      /*silent=*/false);
            ...
            if (constrNum > rank) { // conflicting or redundant constraints
                ...
                identifyConflictingRedundantConstraints(alg, SqrJT, jacobianconstraintmap,
                                                        tagmultiplicity, pdiagnoselist, R,
                                                        constrNum, rank, nonredundantconstrNum);
                                      ...
                                      Eigen::MatrixXd &R, bool transposeJ, bool silent)
{
...
#ifdef _GCS_DEBUG
    if (!silent) SolverReportingManager::Manager().LogMatrix("J", J);
#endif

#ifdef _GCS_DEBUG_SOLVER_JACOBIAN_QR_DECOMPOSITION_TRIANGULAR_MATRIX
    Eigen::MatrixXd Q; // Obtaining the Q matrix with Sparse QR is buggy, see comments below
    Eigen::MatrixXd
        R2; // Intended for a trapezoidal matrix, where R is the top triangular matrix of the R2 trapezoidal matrix
#endif
    ...
    if (J.rows() > 0) {
        Eigen::MatrixXd JG;
        if (transposeJ) JG = J.topRows(jacobianconstraintmap.size()).transpose();
        else
        ...
        if (JG.rows() > 0 && JG.cols() > 0) {
            ...
            if (colsNum >= rowsNum) R = qrJT.matrixQR().triangularView<Eigen::Upper>();
            else
    ...
    Eigen::MatrixXd &R, bool transposeJ, bool silent)
{
    ...
    if (SJ.rows() > 0) {
        ...
        if (transposeJ) SJG = SJ.topRows(jacobianconstraintmap.size()).transpose();
        else
...
//Q = QS;
            ...
            if (colsNum >= rowsNum) R = SqrJT.matrixR().triangularView<Eigen::Upper>();
            else
                                                 ...
                                                 const GCS::VEC_pD &pdiagnoselist, bool silent)
{
    ...
    makeSparseQRDecomposition(J, jacobianconstraintmap, SqrJ, nontransprank, Rparams, false,
...
{
    (void)
        silent; // silent is only used in debug code, but it is important as Base::Console is not thread-safe. Removes warning in non Debug mode.
    ...
    int paramsNum, int rank)
{
    ...
    for (int j = 0; j < paramsNum; j++) {
        ...
        if (result == indepParamCols.end()) { depParamCols.insert(j); }
    }
    ...
    stream << "Indep params: [";
    for (auto indep : indepParamCols) stream << indep;
    stream << "]" << std::endl;

    stream << "Dep params: [";
    for (auto dep : depParamCols) stream << dep;
    stream << "]" << std::endl;
    ...
    for (auto param : depParamCols) { pDependentParameters.push_back(pdiagnoselist[param]); }
}
    ...
    int constrNum, int rank, int &nonredundantconstrNum)
{
    ...
    for (std::vector<Constraint *>::iterator constr = clist.begin(); constr != clist.end();
         ++constr) {
        if ((*constr)->isDriving() && skipped.count(*constr) == 0) clistTmp.push_back(*constr);
    }
    ...
    if (res == Success) {
        ...
        for (std::set<Constraint *>::const_iterator constr = skipped.begin();
            ...
            double err = (*constr)->error();
            if (err * err < convergenceRedundant) redundant.insert(*constr);
        }
        resetToReference();
        ...
        for (int i = conflictGroupsOrig.size() - 1; i >= 0; i--) {
            ...
            if (!isRedundant) conflictGroups.push_back(conflictGroupsOrig[i]);
            else
                constrNum--;
        }
    }
    ...
    for (std::vector<Constraint *>::iterator constr = clist.begin(); constr != clist.end();
         ++constr)
        if (redundant.count(*constr) == 0) redundantTagsSet.erase((*constr)->getTag());

    redundantTags.resize(redundantTagsSet.size());
    std::copy(redundantTagsSet.begin(), redundantTagsSet.end(), redundantTags.begin());

    for (auto r : redundantTagsSet) partiallyRedundantTagsSet.erase(r);
...
void free(std::vector<Constraint *> &constrvec)
{
    for (std::vector<Constraint *>::iterator constr = constrvec.begin(); constr != constrvec.end();
        ...
        if (*constr) {
            switch ((*constr)->getTypeId()) {
                case Equal: delete static_cast<ConstraintEqual *>(*constr); break;
                case Difference: delete static_cast<ConstraintDifference *>(*constr); break;
                case P2PDistance: delete static_cast<ConstraintP2PDistance *>(*constr); break;
                case P2PAngle: delete static_cast<ConstraintP2PAngle *>(*constr); break;
                case P2LDistance: delete static_cast<ConstraintP2LDistance *>(*constr); break;
                case PointOnLine: delete static_cast<ConstraintPointOnLine *>(*constr); break;
                case Parallel: delete static_cast<ConstraintParallel *>(*constr); break;
                case Perpendicular: delete static_cast<ConstraintPerpendicular *>(*constr); break;
                case L2LAngle: delete static_cast<ConstraintL2LAngle *>(*constr); break;
                case MidpointOnLine: delete static_cast<ConstraintMidpointOnLine *>(*constr); break;
                case None:
                default: delete *constr;
            }
        }
    }
    constrvec.clear();

...
enum SolveStatus
{
    ...
    SuccessfulSolutionInvalid =
        3, // This is a solution where the solver succeeded, but the resulting geometry is OCE-invalid
};
...
public:
    ...
    bool
        sketchSizeMultiplier; // if true note that the total number of iterations allowed is MaxIterations *xLength
...
public:
    ...
    int addConstraintInternalAlignmentParabolaFocus(Parabola &e, Point &p1, int tagId = 0,
                                                    bool driving = true);
    int addConstraintInternalAlignmentBSplineControlPoint(BSpline &b, Circle &c, int poleindex,
                                                          int tag = 0, bool driving = true);

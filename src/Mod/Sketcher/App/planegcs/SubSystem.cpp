...
// SubSystem
SubSystem::SubSystem(std::vector<Constraint *> &clist_, VEC_pD &params) : clist(clist_)
{
    MAP_pD_pD dummymap;
...
void SubSystem::initialize(VEC_pD &params, MAP_pD_pD &reductionmap)
{
    ...
    for (std::vector<Constraint *>::iterator constr = clist.begin(); constr != clist.end();
        ...
        for (VEC_pD::const_iterator p = constr_params_orig.begin(); p != constr_params_orig.end();
            ...
            if (pmapfind != pmap.end()) constr_params.insert(pmapfind->second);
        }
...
void SubSystem::getParamMap(MAP_pD_pD &pmapOut) { pmapOut = pmap; }

void SubSystem::getParamList(VEC_pD &plistOut) { plistOut = plist; }

void SubSystem::getParams(VEC_pD &params, Eigen::VectorXd &xOut)
{
    if (xOut.size() != int(params.size())) xOut.setZero(params.size());

    for (int j = 0; j < int(params.size()); j++) {
        MAP_pD_pD::const_iterator pmapfind = pmap.find(params[j]);
        if (pmapfind != pmap.end()) xOut[j] = *(pmapfind->second);
    }
}

void SubSystem::getParams(Eigen::VectorXd &xOut)
{
    if (xOut.size() != psize) xOut.setZero(psize);

    for (int i = 0; i < psize; i++) xOut[i] = pvals[i];
}
...
void SubSystem::setParams(VEC_pD &params, Eigen::VectorXd &xIn)
{
    ...
    for (int j = 0; j < int(params.size()); j++) {
        MAP_pD_pD::const_iterator pmapfind = pmap.find(params[j]);
        if (pmapfind != pmap.end()) *(pmapfind->second) = xIn[j];
    }
}
...
void SubSystem::setParams(Eigen::VectorXd &xIn)
{
    ...
    for (int i = 0; i < psize; i++) pvals[i] = xIn[i];
}

void SubSystem::getConstraintList(std::vector<Constraint *> &clist_) { clist_ = clist; }
...
void SubSystem::calcJacobi(VEC_pD &params, Eigen::MatrixXd &jacobi)
{
    ...
    for (int j = 0; j < int(params.size()); j++) {
        ...
        if (pmapfind != pmap.end())
            for (int i = 0; i < csize; i++) jacobi(i, j) = clist[i]->grad(pmapfind->second);
    }
}

void SubSystem::calcJacobi(Eigen::MatrixXd &jacobi) { calcJacobi(plist, jacobi); }
...
void SubSystem::calcGrad(Eigen::VectorXd &grad) { calcGrad(plist, grad); }
...
double SubSystem::maxStep(VEC_pD &params, Eigen::VectorXd &xdir)
{
    ...
    for (int j = 0; j < int(params.size()); j++) {
        MAP_pD_pD::const_iterator pmapfind = pmap.find(params[j]);
        if (pmapfind != pmap.end()) dir[pmapfind->second] = xdir[j];
    }
...
double SubSystem::maxStep(Eigen::VectorXd &xdir) { return maxStep(plist, xdir); }
...
void SubSystem::analyse(Eigen::MatrixXd & /*J*/, Eigen::MatrixXd & /*ker*/,
                        Eigen::MatrixXd & /*img*/)
{}

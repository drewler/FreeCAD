...
public:
    ...
    double length(
        double &dlength) const; //returns length and writes length deriv into the dlength argument.
    ...
    double scalarProd(const DeriVector2 &v2, double *dprd = nullptr)
        const; //calculates scalar product of two vectors and returns the result. The derivative of the result is written into argument dprd.
    ...
    { //linear combination of two vectors
        return DeriVector2(x * m1 + v2.x * m2, y * m1 + v2.y * m2, dx * m1 + v2.dx * m2,
                           dy * m1 + v2.dy * m2);
    }
};
...
public:
    ...
    virtual Curve *
    Copy() = 0; //DeepSOIC: I haven't found a way to simply copy a curve object provided pointer to a curve object.
};

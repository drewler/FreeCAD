...
void Constraint::redirectParams(MAP_pD_pD redirectionmap)
{
    ...
    for (VEC_pD::iterator param = origpvec.begin(); param != origpvec.end(); ++param, i++) {
        ...
        if (it != redirectionmap.end()) pvec[i] = it->second;
    }
...
ConstraintType Constraint::getTypeId() { return None; }

void Constraint::rescale(double coef) { scale = coef * 1.; }

double Constraint::error() { return 0.; }

double Constraint::grad(double * /*param*/) { return 0.; }

double Constraint::maxStep(MAP_pD_D & /*dir*/, double lim) { return lim; }
...
ConstraintType ConstraintEqual::getTypeId() { return Equal; }

void ConstraintEqual::rescale(double coef) { scale = coef * 1.; }
...
ConstraintType ConstraintDifference::getTypeId() { return Difference; }

void ConstraintDifference::rescale(double coef) { scale = coef * 1.; }
...
ConstraintType ConstraintP2PDistance::getTypeId() { return P2PDistance; }

void ConstraintP2PDistance::rescale(double coef) { scale = coef * 1.; }
...
double ConstraintP2PDistance::maxStep(MAP_pD_D &dir, double lim)
{
    ...
    if (it != dir.end()) {
        if (it->second < 0.) lim = std::min(lim, -(*distance()) / it->second);
    }
    ...
    if (dd > dist) {
        ...
        double dy = (*p1y() - *p2y());
        double d = sqrt(dx * dx + dy * dy);
        if (dd > d) lim = std::min(lim, std::max(d, dist) / dd);
    }
    return lim;
}

// P2PAngle
ConstraintP2PAngle::ConstraintP2PAngle(Point &p1, Point &p2, double *a, double da_) : da(da_)
{
    pvec.push_back(p1.x);
    pvec.push_back(p1.y);
    pvec.push_back(p2.x);
...
ConstraintType ConstraintP2PAngle::getTypeId() { return P2PAngle; }

void ConstraintP2PAngle::rescale(double coef) { scale = coef * 1.; }
...
double ConstraintP2PAngle::maxStep(MAP_pD_D &dir, double lim)
{
    ...
    if (it != dir.end()) {
        ...
        if (step > M_PI / 18.) lim = std::min(lim, (M_PI / 18.) / step);
    }
    return lim;
}
...
ConstraintType ConstraintP2LDistance::getTypeId() { return P2LDistance; }

void ConstraintP2LDistance::rescale(double coef) { scale = coef; }
...
double ConstraintP2LDistance::maxStep(MAP_pD_D &dir, double lim)
{
    ...
    if (darea > 0.) {
        ...
        if (darea > area) {
            ...
            if (darea > area) lim = std::min(lim, area / darea);
        }
    }
    return lim;
}
...
ConstraintType ConstraintPointOnLine::getTypeId() { return PointOnLine; }

void ConstraintPointOnLine::rescale(double coef) { scale = coef; }
...
ConstraintType ConstraintPointOnPerpBisector::getTypeId() { return PointOnPerpBisector; }

void ConstraintPointOnPerpBisector::rescale(double coef) { scale = coef; }
...
void ConstraintPointOnPerpBisector::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = projd1 + projd2;
    if (grad) *grad = dprojd1 + dprojd2;
}
...
ConstraintType ConstraintParallel::getTypeId() { return Parallel; }
...
ConstraintType ConstraintPerpendicular::getTypeId() { return Perpendicular; }
...
ConstraintType ConstraintL2LAngle::getTypeId() { return L2LAngle; }

void ConstraintL2LAngle::rescale(double coef) { scale = coef * 1.; }
...
ConstraintType ConstraintMidpointOnLine::getTypeId() { return MidpointOnLine; }

void ConstraintMidpointOnLine::rescale(double coef) { scale = coef * 1; }
...
ConstraintType ConstraintTangentCircumf::getTypeId() { return TangentCircumf; }

void ConstraintTangentCircumf::rescale(double coef) { scale = coef * 1; }

double ConstraintTangentCircumf::error()
{
    double dx = (*c1x() - *c2x());
    double dy = (*c1y() - *c2y());
    if (internal) return scale * (sqrt(dx * dx + dy * dy) - std::abs(*r1() - *r2()));
    else
...
ConstraintType ConstraintPointOnEllipse::getTypeId() { return PointOnEllipse; }

void ConstraintPointOnEllipse::rescale(double coef) { scale = coef * 1; }
...
ConstraintType ConstraintEllipseTangentLine::getTypeId() { return TangentEllipseLine; }

void ConstraintEllipseTangentLine::rescale(double coef) { scale = coef * 1; }
...
void ConstraintEllipseTangentLine::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = distF1mF2 - 2 * radmaj;
    if (grad) *grad = ddistF1mF2 - 2 * dradmaj;
}
...
void ConstraintInternalAlignmentPoint2Ellipse::rescale(double coef) { scale = coef * 1; }
...
void ConstraintInternalAlignmentPoint2Ellipse::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = by_y_not_by_x ? pv.y - poa.y : pv.x - poa.x;
    if (grad) *grad = by_y_not_by_x ? pv.dy - poa.dy : pv.dx - poa.dx;
}
...
void ConstraintInternalAlignmentPoint2Hyperbola::rescale(double coef) { scale = coef * 1; }
...
ConstraintType ConstraintEqualMajorAxesConic::getTypeId() { return EqualMajorAxesConic; }

void ConstraintEqualMajorAxesConic::rescale(double coef) { scale = coef * 1; }
...
ConstraintType ConstraintEqualFocalDistance::getTypeId() { return EqualFocalDistance; }

void ConstraintEqualFocalDistance::rescale(double coef) { scale = coef * 1; }
...
void ConstraintEqualFocalDistance::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = focal2 - focal1;
    if (grad) *grad = dfocal2 - dfocal1;
}
...
ConstraintType ConstraintCurveValue::getTypeId() { return CurveValue; }

void ConstraintCurveValue::rescale(double coef) { scale = coef * 1; }
...
void ConstraintCurveValue::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (this->pcoord() == this->p.x) { //this constraint is for X projection
        if (err) *err = err_vec.x;
        if (grad) *grad = err_vec.dx;
    }
    else if (this->pcoord() == this->p.y) { //this constraint is for Y projection
        if (err) *err = err_vec.y;
        if (grad) *grad = err_vec.dy;
    }
    else {
...
ConstraintType ConstraintPointOnHyperbola::getTypeId() { return PointOnHyperbola; }

void ConstraintPointOnHyperbola::rescale(double coef) { scale = coef * 1; }
...
ConstraintType ConstraintPointOnParabola::getTypeId() { return PointOnParabola; }

void ConstraintPointOnParabola::rescale(double coef) { scale = coef * 1; }
...
void ConstraintPointOnParabola::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = pf - 2 * focal - proj;
    if (grad) *grad = dpf - 2 * dfocal - dproj;
}
...
ConstraintType ConstraintAngleViaPoint::getTypeId() { return AngleViaPoint; }

void ConstraintAngleViaPoint::rescale(double coef) { scale = coef * 1.; }
...
ConstraintType ConstraintSnell::getTypeId() { return Snell; }

void ConstraintSnell::rescale(double coef) { scale = coef * 1.; }
...
void ConstraintSnell::errorgrad(double *err, double *grad, double *param)
{
    ...
    if (err) *err = *n1() * sin1 - *n2() * sin2;
    if (grad) *grad = dn1 * sin1 + *n1() * dsin1 - dn2 * sin2 - *n2() * dsin2;
}
...
ConstraintType ConstraintEqualLineLength::getTypeId() { return EqualLineLength; }

void ConstraintEqualLineLength::rescale(double coef) { scale = coef * 1; }
    ...
    if (grad) {
        *grad = dlength2 - dlength1;
        ...
        if (fabs(*grad) < 1e-10) {
            ...
            if (param == l1.p1.x) *grad = v1.x > 0 ? surrogate : -surrogate;

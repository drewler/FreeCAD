...
DeriVector2::DeriVector2(const Point &p, const double *derivparam)
{
    ...
    dx = 0.0;
    dy = 0.0;
    if (derivparam == p.x) dx = 1.0;
    if (derivparam == p.y) dy = 1.0;
}
...
{
    double l = length();
    if (l == 0.0) { return DeriVector2(0, 0, dx, dy); }
    else {
        DeriVector2 rtn;
        rtn.x = x / l;
        rtn.y = y / l;
...
{
    if (dprd) { *dprd = dx * v2.x + x * v2.dx + dy * v2.y + y * v2.dy; };
    return x * v2.x + y * v2.y;
}

DeriVector2 DeriVector2::divD(double val, double dval) const
{
    return DeriVector2(x / val, y / val, dx / val - x * dval / (val * val),
                          ...
                          double &ret_dRadMaj) const
{
    ...
    DeriVector2 hack(
        b, cf, db,
        dcf); //hack = a nonsense vector to calculate major radius with derivatives, useful just because the calculation formula is the same as vector length formula

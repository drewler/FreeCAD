...
{
    if (OwnerGeo && Geo) delete Geo;
}

std::unique_ptr<GeometryFacade> GeometryFacade::getFacade(Part::Geometry *geometry, bool owner)
{
    if (geometry) return std::unique_ptr<GeometryFacade>(new GeometryFacade(geometry, owner));
    else
...
{
    if (geometry) return std::unique_ptr<const GeometryFacade>(new GeometryFacade(geometry));
    else
...
{
    if (!geo) THROWM(Base::ValueError, "Geometry is nullptr!");
}

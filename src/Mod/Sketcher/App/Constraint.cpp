...
Constraint::Constraint()
    : Value(0.0), Type(None), AlignmentType(Undef), First(GeoEnum::GeoUndef),
      FirstPos(PointPos::none), Second(GeoEnum::GeoUndef), SecondPos(PointPos::none),
      Third(GeoEnum::GeoUndef), ThirdPos(PointPos::none), LabelDistance(10.f), LabelPosition(0.f),
      isDriving(true), InternalAlignmentIndex(-1), isInVirtualSpace(false), isActive(true)
{
...
Constraint *Constraint::clone() const { return new Constraint(*this); }
...
PyObject *Constraint::getPyObject() { return new ConstraintPy(new Constraint(*this)); }
...
Quantity Constraint::getPresentationValue() const
{
    ...
    switch (Type) {
        ...
        case Angle:
            ...
            break;
        case SnellsLaw:
        case Weight: quantity.setValue(Value); break;
        default: quantity.setValue(Value); break;
    }
...
unsigned int Constraint::getMemSize() const { return 0; }
...
{
    if (this->First == fromGeoId) { this->First = toGeoId; }
    if (this->Second == fromGeoId) { this->Second = toGeoId; }
    if (this->Third == fromGeoId) { this->Third = toGeoId; }
}

std::string Constraint::typeToString(ConstraintType type) { return type2str[type]; }

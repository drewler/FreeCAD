...
PropertyConstraintList::PropertyConstraintList()
    : validGeometryKeys(0), invalidGeometry(true), restoreFromTransaction(false),
...
void PropertyConstraintList::setSize(int newSize)
{
    ...
    if (!removed.empty()) signalConstraintsRemoved(removed);
...
int PropertyConstraintList::getSize() const { return static_cast<int>(_lValueList.size()); }
...
void PropertyConstraintList::set1Value(const int idx, const Constraint *lValue)
{
    if (lValue) {
        ...
        if (oldVal->Name != newVal->Name) {
            ...
            if (!renamed.empty()) signalConstraintsRenamed(renamed);
        }
...
void PropertyConstraintList::setValue(const Constraint *lValue)
{
    if (lValue) {
        ...
        // Cleanup
        for (unsigned int i = 0; i < _lValueList.size(); i++) delete _lValueList[i];
...
{
    auto copy = lValue;
    for (auto &cstr : copy) cstr = cstr->clone();
...
void PropertyConstraintList::applyValues(std::vector<Constraint *> &&lValue)
{
    ...
    for (auto &v : valueMap) removed.insert(makePath(v.second, _lValueList[v.second]));
    ...
    if (!removed.empty() && !restoreFromTransaction) signalConstraintsRemoved(removed);

    /* Signal renames */
    if (!renamed.empty() && !restoreFromTransaction) signalConstraintsRenamed(renamed);

    _lValueList = std::move(lValue);

    /* Clean-up; remove old values */
    for (auto &v : oldVals) delete v;
}
                                            ...
                                            Py::Object &res) const
{
    ...
    if (c1.isArray()) cstr = _lValueList[c1.getIndex(_lValueList.size())];
    else if (c1.isSimple()) {
...
void PropertyConstraintList::Save(Writer &writer) const
{
    ...
    writer.incInd();
    for (int i = 0; i < getSize(); i++) _lValueList[i]->Save(writer);
    writer.decInd();
...
void PropertyConstraintList::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (newC->Type < Sketcher::NumConstraintTypes) { values.push_back(newC); }
        else {
...
void PropertyConstraintList::acceptGeometry(const std::vector<Part::Geometry *> &GeoList)
{
    ...
    for (const auto &it : GeoList) validGeometryKeys.push_back((it)->getTypeId().getKey());
...
bool PropertyConstraintList::checkConstraintIndices(int geomax, int geomin)
{
    ...
    auto cmin = [](int previousmin, int cindex) {
        if (cindex == GeoEnum::GeoUndef) return previousmin;
    ...
    if (maxinternalgeoid > geomax || mininternalgeoid < geomin) invalidIndices = true;
    else
        invalidIndices = false;
...
{
    if (validGeometryKeys.size() != GeoList.size()) { return false; }

    unsigned int i = 0;
    for (std::vector<Part::Geometry *>::const_iterator it = GeoList.begin(); it != GeoList.end();
         ++it, i++) {
        if (validGeometryKeys[i] != (*it)->getTypeId().getKey()) { return false; }
    }
...
bool PropertyConstraintList::validConstraintName(const std::string &name) { return !name.empty(); }
...
void PropertyConstraintList::setPathValue(const ObjectIdentifier &path, const boost::any &value)
{
    ...
    if (value.type() == typeid(double)) dvalue = boost::any_cast<double>(value);
    ...
    if (c1.isArray()) {
        ...
        switch (_lValueList[index]->Type) {
            case Angle: dvalue = Base::toRadians<double>(dvalue); break;
            default: break;
        }
        aboutToSetValue();
...
const Constraint *PropertyConstraintList::getConstraint(const ObjectIdentifier &path) const
{
    ...
    if (c1.isArray()) { return _lValueList[c1.getIndex(_lValueList.size())]; }
    else if (c1.isSimple()) {
        ObjectIdentifier::Component c1 = path.getPropertyComponent(1);

        for (std::vector<Constraint *>::const_iterator it = _lValueList.begin();
             it != _lValueList.end(); ++it) {
            if ((*it)->Name == c1.getName()) return *it;
        }
    }
    FC_THROWM(Base::ValueError, "Invalid constraint path " << path.toString());
}

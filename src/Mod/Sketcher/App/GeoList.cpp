...
GeoListModel<T>::GeoListModel(std::vector<T> &&geometrylist, int intgeocount, bool ownerT)
    : geomlist(std::move(geometrylist)), intGeoCount(intgeocount), OwnerT(ownerT), indexInit(false)
{}
...
GeoListModel<T>::GeoListModel(const std::vector<T> &geometrylist,
    ...
    : geomlist(geometrylist), // copy constructed here
      intGeoCount(intgeocount), OwnerT(false), indexInit(false)
{}

template<typename T> GeoListModel<T>::~GeoListModel()
{
    if (OwnerT) {
        for (auto &g : geomlist) delete g;
    }
}
...
template<typename T> int GeoListModel<T>::getGeoIdFromGeomListIndex(int index) const
{
    assert(index < int(geomlist.size()));

    if (index < intGeoCount) return index;
    else
...
{
    if constexpr (std::is_same<T, GeometryPtr>()) {
        if (geoId >= 0) return GeometryFacade::getFacade(geometrylist[geoId]).release();
        else
            return GeometryFacade::getFacade(geometrylist[geometrylist.size() + geoId]).release();
    }
    else if constexpr (std::is_same<T, GeometryFacadeUniquePtr>()) {
        if (geoId >= 0) return geometrylist[geoId].get();
        else
...
template<typename T> const Part::Geometry *GeoListModel<T>::getGeometryFromGeoId(int geoId) const
{
...
template<typename T> Base::Vector3d GeoListModel<T>::getPoint(const GeoElementId &geid) const
{
...
Base::Vector3d GeoListModel<T>::getPoint(const Part::Geometry *geo, Sketcher::PointPos pos) const
{
    ...
    if (geo->getTypeId() == Part::GeomPoint::getClassTypeId()) {
        ...
        if (pos == PointPos::start) return lineSeg->getStartPoint();
        else if (pos == PointPos::end)
            return lineSeg->getEndPoint();
    }
    else if (geo->getTypeId() == Part::GeomCircle::getClassTypeId()) {
        const Part::GeomCircle *circle = static_cast<const Part::GeomCircle *>(geo);
        if (pos == PointPos::mid) return circle->getCenter();
    }
    else if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()) {
        const Part::GeomEllipse *ellipse = static_cast<const Part::GeomEllipse *>(geo);
        if (pos == PointPos::mid) return ellipse->getCenter();
    }
    else if (geo->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
        const Part::GeomArcOfCircle *aoc = static_cast<const Part::GeomArcOfCircle *>(geo);
        if (pos == PointPos::start) return aoc->getStartPoint(/*emulateCCW=*/true);
        else if (pos == PointPos::end)
        ...
        if (pos == PointPos::start) return aoh->getStartPoint();
        else if (pos == PointPos::end)
        ...
        if (pos == PointPos::start) return aop->getStartPoint();
        else if (pos == PointPos::end)
        ...
        if (pos == PointPos::start) return bsp->getStartPoint();
        else if (pos == PointPos::end)
...
template<typename T> void GeoListModel<T>::rebuildVertexIndex() const
{
    ...
    auto addGeoElement = [this, &pointId](int geoId, PointPos pos) {
        ...
        GeoElementId2VertexId.emplace(std::piecewise_construct, std::forward_as_tuple(geoId, pos),
                                      std::forward_as_tuple(pointId++));
    };

    if (geomlist.size() <= 2) return;
    for (auto it = geomlist.begin(); it != geomlist.end(); ++it, geoId++) {

        Base::Type type;

        if constexpr (std::is_same<T, Part::Geometry *>::value) type = (*it)->getTypeId();
        else if constexpr (std::is_same<T, std::unique_ptr<const Sketcher::GeometryFacade>>::value)
            type = (*it)->getGeometry()->getTypeId();

        if (geoId > getInternalCount()) geoId = -getExternalCount();

        if (type == Part::GeomPoint::getClassTypeId()) { addGeoElement(geoId, PointPos::start); }
...
GeoListModel<GeometryFacadeUniquePtr>::GeoListModel(
    ...
    : geomlist(std::move(geometrylist)), intGeoCount(intgeocount), OwnerT(false), indexInit(false)
{
...
GeoListFacade Sketcher::getGeoListFacade(const GeoList &geolist)
{
    ...
    for (auto geo : geolist.geomlist) facade.push_back(GeometryFacade::getFacade(geo));

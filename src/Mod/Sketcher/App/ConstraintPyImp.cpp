...
int ConstraintPy::PyInit(PyObject *args, PyObject * /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(args, "siiiO", &ConstraintType, &intArg1, &intArg2, &intArg3, &oNumArg4)) {
        ...
        if (PyLong_Check(oNumArg4)) {
            ...
            if (strcmp("Coincident", ConstraintType) == 0) {
                ...
                strstr(
                    ConstraintType,
                    "InternalAlignment")) { // InteralAlignment with InternalElementIndex argument
                this->getConstraintPtr()->Type = InternalAlignment;
    ...
    if (PyArg_ParseTuple(args, "siiiiO", &ConstraintType, &intArg1, &intArg2, &intArg3, &intArg4,
                         &oNumArg5)) {
    ...
    if (PyArg_ParseTuple(args, "siiiiiO", &ConstraintType, &FirstIndex, &FirstPos, &SecondIndex,
                         &SecondPos, &ThirdIndex, &index_or_value)) {
        if (PyLong_Check(index_or_value)) {
...
std::string ConstraintPy::representation() const
{
    ...
    switch (this->getConstraintPtr()->Type) {
        ...
        case Tangent:
            if (this->getConstraintPtr()->Third == GeoEnum::GeoUndef) result << "'Tangent'>";
            else
                result << "'TangentViaPoint'>";
            break;
        case Perpendicular:
            if (this->getConstraintPtr()->Third == GeoEnum::GeoUndef) result << "'Perpendicular'>";
            else
                result << "'PerpendicularViaPoint'>";
            break;
        case Distance: result << "'Distance'>"; break;
        case Angle:
            if (this->getConstraintPtr()->Third == GeoEnum::GeoUndef) result << "'Angle'>";
            else
...
Py::Long ConstraintPy::getFirst() const { return Py::Long(this->getConstraintPtr()->First); }

void ConstraintPy::setFirst(Py::Long arg) { this->getConstraintPtr()->First = arg; }
...
Py::Long ConstraintPy::getSecond() const { return Py::Long(this->getConstraintPtr()->Second); }

void ConstraintPy::setSecond(Py::Long arg) { this->getConstraintPtr()->Second = arg; }
...
Py::Long ConstraintPy::getThird() const { return Py::Long(this->getConstraintPtr()->Third); }

void ConstraintPy::setThird(Py::Long arg) { this->getConstraintPtr()->Third = arg; }
...
Py::String ConstraintPy::getName() const { return Py::String(this->getConstraintPtr()->Name); }

void ConstraintPy::setName(Py::String arg) { this->getConstraintPtr()->Name = arg; }

Py::Float ConstraintPy::getValue() const { return Py::Float(this->getConstraintPtr()->getValue()); }
...
PyObject *ConstraintPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

...
ExternalGeometryFacade::ExternalGeometryFacade()
    : Geo(nullptr), SketchGeoExtension(nullptr), ExternalGeoExtension(nullptr)
{}

ExternalGeometryFacade::ExternalGeometryFacade(const Part::Geometry *geometry) : Geo(geometry)
{
    if (geometry) initExtensions();
    else
...
void ExternalGeometryFacade::ensureSketchGeometryExtensions(Part::Geometry *geometry)
{
    ...
    if (!geometry->hasExtension(ExternalGeometryExtension::getClassTypeId())) {
        geometry->setExtension(
            std::make_unique<ExternalGeometryExtension>()); // Create external geoExtension
    }
}

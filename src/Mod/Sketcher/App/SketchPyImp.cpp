...
std::string SketchPy::representation() const { return std::string("<Sketch object>"); }
...
// constructor method
int SketchPy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *SketchPy::movePoint(PyObject *args)
{
    ...
    PyObject *pcObj;
    int relative = 0;
    if (!PyArg_ParseTuple(args, "iiO!|i", &index1, &index2, &(Base::VectorPy::Type), &pcObj,
                          &relative))
        return nullptr;
    Base::Vector3d *toPoint = static_cast<Base::VectorPy *>(pcObj)->getVectorPtr();

    return Py::new_reference_to(Py::Long(
        getSketchPtr()->movePoint(index1, (Sketcher::PointPos)index2, *toPoint, (relative > 0))));
}
...
Py::Tuple SketchPy::getConflicts() const
{
    ...
    Py::Tuple t(c.size());
    for (std::size_t i = 0; i < c.size(); i++) { t.setItem(i, Py::Long(c[i])); }
...
Py::Tuple SketchPy::getGeometries() const { return getSketchPtr()->getPyGeometry(); }
...
PyObject *SketchPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int SketchPy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

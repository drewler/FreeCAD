...
std::string SketchObjectPy::representation() const { return "<Sketcher::SketchObject>"; }
    ...
    if (PyObject_TypeCheck(pcObj, &(Part::GeometryPy::Type))) {
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            if (PyObject_TypeCheck((*it).ptr(), &(Part::GeometryPy::Type))) {
                Part::Geometry *geo =
                    static_cast<Part::GeometryPy *>((*it).ptr())->getGeometryPtr();
...
PyObject *SketchObjectPy::delGeometries(PyObject *args)
{
    ...
    if (PyObject_TypeCheck(pcObj, &(PyList_Type)) || PyObject_TypeCheck(pcObj, &(PyTuple_Type))) {
        ...
        Py::Sequence list(pcObj);
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            if (PyLong_Check((*it).ptr())) geoIdList.push_back(PyLong_AsLong((*it).ptr()));
        }
...
PyObject *SketchObjectPy::setConstruction(PyObject *args)
{
    int Index;
    PyObject *Mode;
    if (!PyArg_ParseTuple(args, "iO!", &Index, &PyBool_Type, &Mode)) return nullptr;
...
PyObject *SketchObjectPy::getConstruction(PyObject *args)
{
    ...
    if (gf) return Py::new_reference_to(Py::Boolean(gf->getConstruction()));
...
PyObject *SketchObjectPy::renameConstraint(PyObject *args)
{
    int Index;
    char *utf8Name;
    if (!PyArg_ParseTuple(args, "iet", &Index, "utf-8", &utf8Name)) return nullptr;
...
{
    char *utf8Name;
    if (!PyArg_ParseTuple(args, "et", "utf-8", &utf8Name)) return nullptr;
    ...
    for (std::size_t i = 0; i < vals.size(); ++i) {
        if (Name == vals[i]->Name) { return Py_BuildValue("i", i); }
    }
...
{
    int Index, pos = -1;
    if (!PyArg_ParseTuple(args, "i|i", &Index, &pos)) return nullptr;

    if (pos >= static_cast<int>(Sketcher::PointPos::none)
        && pos <= static_cast<int>(
               Sketcher::PointPos::mid)) { // This is the whole range of valid positions
...
PyObject *SketchObjectPy::setDatum(PyObject *args)
{
    ...
    if (err) {
        std::stringstream str;
        if (err == -1) str << "Invalid constraint index: " << Index;
        else if (err == -3)
...
PyObject *SketchObjectPy::setDriving(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iO!", &constrid, &PyBool_Type, &driving)) return nullptr;
...
PyObject *SketchObjectPy::setDatumsDriving(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!", &PyBool_Type, &driving)) return nullptr;
...
PyObject *SketchObjectPy::getDriving(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i", &constrid)) return nullptr;
...
PyObject *SketchObjectPy::setVirtualSpace(PyObject *args)
{
    ...
    if (PyObject_TypeCheck(id_or_ids, &(PyList_Type))
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            if (PyLong_Check((*it).ptr())) constrIds.push_back(PyLong_AsLong((*it).ptr()));
        }
        ...
        try {
            ...
            if (ret == -1) throw Py::TypeError("Impossible to set virtual space!");
        }
...
PyObject *SketchObjectPy::setActive(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iO!", &constrid, &PyBool_Type, &isactive)) return nullptr;
...
PyObject *SketchObjectPy::movePoint(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iiO!|i", &GeoId, &PointType, &(Base::VectorPy::Type), &pcObj,
                          &relative))
        return nullptr;
...
{
    int GeoId, PointType;
    if (!PyArg_ParseTuple(args, "ii", &GeoId, &PointType)) return nullptr;
    ...
    return new Base::VectorPy(
        new Base::Vector3d(obj->getPoint(GeoId, (Sketcher::PointPos)PointType)));
}

PyObject *SketchObjectPy::getAxis(PyObject *args)
{
    int AxId;
    if (!PyArg_ParseTuple(args, "i", &AxId)) return nullptr;
...
PyObject *SketchObjectPy::fillet(PyObject *args)
{
    ...
    // Two Lines, radius
    if (PyArg_ParseTuple(args, "iiO!O!d|iO!", &geoId1, &geoId2, &(Base::VectorPy::Type), &pcObj1,
                         &(Base::VectorPy::Type), &pcObj2, &radius, &trim, &PyBool_Type,
                         &createCorner)) {
        ...
        if (this->getSketchObjectPtr()->fillet(geoId1, geoId2, v1, v2, radius, trim,
    ...
    PyErr_Clear();
    // Point, radius
    if (PyArg_ParseTuple(args, "iid|iO!", &geoId1, &posId1, &radius, &trim, &PyBool_Type,
                         &createCorner)) {
...
PyObject *SketchObjectPy::trim(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iO!", &GeoId, &(Base::VectorPy::Type), &pcObj)) return nullptr;
...
PyObject *SketchObjectPy::extend(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "idi", &GeoId, &increment, &endPoint)) {
        if (this->getSketchObjectPtr()->extend(GeoId, increment,
...
PyObject *SketchObjectPy::addSymmetric(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Oi|i", &pcObj, &refGeoId, &refPosId)) return nullptr;
    ...
    if (PyObject_TypeCheck(pcObj, &(PyList_Type)) || PyObject_TypeCheck(pcObj, &(PyTuple_Type))) {
        ...
        int ret = this->getSketchObjectPtr()->addSymmetric(geoIdList, refGeoId,
                                                           (Sketcher::PointPos)refPosId)
            + 1;

        if (ret == -1) throw Py::TypeError("Symmetric operation unsuccessful!");
...
PyObject *SketchObjectPy::addCopy(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO!|O!", &pcObj, &(Base::VectorPy::Type), &pcVect, &PyBool_Type,
                          &clone))
        return nullptr;
    ...
    if (PyObject_TypeCheck(pcObj, &(PyList_Type)) || PyObject_TypeCheck(pcObj, &(PyTuple_Type))) {
        ...
        try {
            ...
            if (ret == -1) throw Py::TypeError("Copy operation unsuccessful!");
...
PyObject *SketchObjectPy::addMove(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO!", &pcObj, &(Base::VectorPy::Type), &pcVect)) return nullptr;
...
PyObject *SketchObjectPy::addRectangularArray(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO!O!ii|O!d", &pcObj, &(Base::VectorPy::Type), &pcVect,
                          &PyBool_Type, &clone, &rows, &cols, &PyBool_Type, &constraindisplacement,
                          &perpscale))
        return nullptr;
    ...
    if (PyObject_TypeCheck(pcObj, &(PyList_Type)) || PyObject_TypeCheck(pcObj, &(PyTuple_Type))) {
        ...
        try {
            int ret = this->getSketchObjectPtr()->addCopy(
                          geoIdList, vect, false, Base::asBoolean(clone), rows, cols,
                          Base::asBoolean(constraindisplacement), perpscale)
                + 1;
...
PyObject *SketchObjectPy::removeAxesAlignment(PyObject *args)
{
    ...
    if (PyObject_TypeCheck(pcObj, &(PyList_Type)) || PyObject_TypeCheck(pcObj, &(PyTuple_Type))) {
        ...
        if (ret == -1) throw Py::TypeError("Operation unsuccessful!");
...
PyObject *SketchObjectPy::calculateAngleViaPoint(PyObject *args)
{
    ...
    double px = 0, py = 0;
    if (!PyArg_ParseTuple(args, "iidd", &GeoId1, &GeoId2, &px, &py)) return nullptr;
...
PyObject *SketchObjectPy::isPointOnCurve(PyObject *args)
{
    ...
    double px = 0, py = 0;
    if (!PyArg_ParseTuple(args, "idd", &GeoId, &px, &py)) return nullptr;
...
{
    int ic = -1;
    if (!PyArg_ParseTuple(args, "i", &ic)) return nullptr;
...
{
    int bLock = 0;
    if (!PyArg_ParseTuple(args, "i", &bLock)) return nullptr;
...
PyObject *SketchObjectPy::ExposeInternalGeometry(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i", &GeoId)) return nullptr;
...
PyObject *SketchObjectPy::increaseBSplineDegree(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i|i", &GeoId, &incr)) return nullptr;
...
PyObject *SketchObjectPy::decreaseBSplineDegree(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i|i", &GeoId, &decr)) return nullptr;
...
PyObject *SketchObjectPy::modifyBSplineKnotMultiplicity(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ii|i", &GeoId, &knotIndex, &multiplicity)) return nullptr;

    if (!this->getSketchObjectPtr()->modifyBSplineKnotMultiplicity(GeoId, knotIndex,
                                                                   multiplicity)) {
        std::stringstream str;
...
PyObject *SketchObjectPy::insertBSplineKnot(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "id|i", &GeoId, &knotParam, &multiplicity)) return nullptr;
...
PyObject *SketchObjectPy::autoconstraint(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|ddO!", &precision, &angleprecision, &PyBool_Type,
                          &includeconstruction))
        return nullptr;

    if (this->getSketchObjectPtr()->autoConstraint(precision, angleprecision,
...
PyObject *SketchObjectPy::detectMissingVerticalHorizontalConstraints(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|d", &angleprecision)) return nullptr;
...
PyObject *SketchObjectPy::detectMissingEqualityConstraints(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|d", &precision)) return nullptr;
...
PyObject *SketchObjectPy::makeMissingPointOnPointCoincident(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &onebyone)) return nullptr;
...
PyObject *SketchObjectPy::autoRemoveRedundants(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &updategeo)) return nullptr;
...
PyObject *SketchObjectPy::setGeometryId(PyObject *args)
{
    int Index;
    long Id;
    if (!PyArg_ParseTuple(args, "il", &Index, &Id)) return nullptr;
...
PyObject *SketchObjectPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }
...
int SketchObjectPy::setCustomAttributes(const char *attr, PyObject *obj)
{
    ...
    if (prop) {
        ...
        if (strcmp(attr, "Geometry") == 0) getSketchObjectPtr()->rebuildVertexIndex();

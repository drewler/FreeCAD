...
Sketch::Sketch()
    : SolveTime(0), RecalculateInitialSolutionWhileMovingPoint(false),
      resolveAfterGeometryUpdated(false), GCSsys(), ConstraintsCounter(0), isInitMove(false),
      isFine(true), moveStep(0), defaultSolver(GCS::DogLeg), defaultSolverRedundant(GCS::DogLeg),
                        ...
                        const std::vector<Constraint *> &ConstraintList, int extGeoCount)
{
    ...
    for (int i = 0; i < int(GeoList.size()) - extGeoCount; i++) intGeoList.push_back(GeoList[i]);
    for (int i = int(GeoList.size()) - extGeoCount; i < int(GeoList.size()); i++)
        extGeoList.push_back(GeoList[i]);

    std::vector<bool> onlyBlockedGeometry(
        intGeoList.size(),
        false); // these geometries are blocked, frozen and sent as fixed parameters to the solver
    std::vector<bool> unenforceableConstraints(
        ConstraintList.size(),
        false); // these constraints are unenforceable due to a Blocked constraint
    ...
    if (found) Base::Console().Log("\n  None");

    Base::Console().Log("\nNotOnlyBlocked GeoIds:");
    i = 0;
    for (; i < blockedGeoIds.size(); i++) Base::Console().Log("\n  GeoId=%d", blockedGeoIds[i]);
    if (i == 0) Base::Console().Log("\n  None");
    Base::Console().Log("\n");
    ...
    for (int i = extStart; i <= extEnd; i++) Geoms[i].external = true;

    // The Geoms list might be empty after an undo/redo
    if (!Geoms.empty()) { addConstraints(ConstraintList, unenforceableConstraints); }
    ...
    std::vector<int> &blockedGeoIds, std::vector<double *> &params_to_block) const
{
    ...
    for (size_t i = 0; i < groups.size(); i++) {
        for (size_t j = 0; j < groups[i].size(); j++) {
            ...
            if (element != param2geoelement.end()) {
                ...
                auto blockable = std::find(blockedGeoIds.begin(), blockedGeoIds.end(),
    ...
    for (size_t i = 0; i < prop_groups.size(); i++) {
        ...
        if (!prop_groups[i].blocking_param_in_group) { unsatisfied_groups = true; }
    }
...
void Sketch::clearTemporaryConstraints() { GCSsys.clearByTag(GCS::DefaultTemporaryConstraint); }
...
void Sketch::calculateDependentParametersElements()
{
    ...
    for (auto geo : Geoms) {
        ...
        if (GCSsys.isEmptyDiagnoseMatrix()) solvext->init(SolverGeometryExtension::Dependent);
        else
        ...
        if (element != param2geoelement.end()) {
            ...
            switch (geopos) {
                case PointPos::none:
                    ...
                    break;
                case PointPos::start:
                    if (index == 0) solvext->setStartx(SolverGeometryExtension::Dependent);
                    else
                        solvext->setStarty(SolverGeometryExtension::Dependent);
                    break;
                case PointPos::end:
                    if (index == 0) solvext->setEndx(SolverGeometryExtension::Dependent);
                    else
                        solvext->setEndy(SolverGeometryExtension::Dependent);
                    break;
                case PointPos::mid:
                    if (index == 0) solvext->setMidx(SolverGeometryExtension::Dependent);
                    else
    ...
    if (pDependencyGroups.size() > 1) { // only if there is more than 1 group
        ...
        for (size_t i = 0; i < endcount; i++) {
            if (havecommonelement(pDependencyGroups[i].begin(), pDependencyGroups[i].end(),
...
{
    if (geoId >= 0 && geoId < int(solverExtensions.size())) return solverExtensions[geoId];
...
const char *nameByType(Sketch::GeoType type)
{
    switch (type) {
        ...
        case Sketch::ArcOfEllipse: return "arcofellipse";
        case Sketch::ArcOfHyperbola: return "arcofhyperbola";
        case Sketch::ArcOfParabola: return "arcofparabola";
                        ...
                        const std::vector<bool> &blockedGeometry)
{
    ...
    for (it = geo.begin(), bit = blockedGeometry.begin();
         it != geo.end() && bit != blockedGeometry.end(); ++it, ++bit)
...
int Sketch::addPoint(const Part::GeomPoint &point, bool fixed)
{
    ...
    if (!fixed) {
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p1.x),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::start, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p1.y),
...
int Sketch::addLineSegment(const Part::GeomLineSegment &lineSegment, bool fixed)
{
    ...
    if (!fixed) {
        ...
        param2geoelement.emplace(
            ...
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::start, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p2.x),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::end, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p2.y),
...
int Sketch::addArc(const Part::GeomArcOfCircle &circleSegment, bool fixed)
{
    ...
    if (!fixed) GCSsys.addConstraintArcRules(a);
    ...
    if (!fixed) {
        ...
        param2geoelement.emplace(
            ...
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::end, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p3.x),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p3.y),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(r),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(a1),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(a2),
...
int Sketch::addArcOfEllipse(const Part::GeomArcOfEllipse &ellipseSegment, bool fixed)
{
    ...
    // arcs require an ArcRules constraint for the end points
    if (!fixed) GCSsys.addConstraintArcOfEllipseRules(a);
    ...
    if (!fixed) {
        ...
        param2geoelement.emplace(
            ...
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(f1X),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(f1Y),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(rmin),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 2));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(a1),
...
int Sketch::addArcOfHyperbola(const Part::GeomArcOfHyperbola &hyperbolaSegment, bool fixed)
{
    ...
    // arcs require an ArcRules constraint for the end points
    if (!fixed) GCSsys.addConstraintArcOfHyperbolaRules(a);
...
int Sketch::addArcOfParabola(const Part::GeomArcOfParabola &parabolaSegment, bool fixed)
{
    ...
    // arcs require an ArcRules constraint for the end points
    if (!fixed) GCSsys.addConstraintArcOfParabolaRules(a);
    ...
    if (!fixed) {
        ...
        param2geoelement.emplace(
            ...
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p4.x),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(p4.y),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(a1),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 2));
...
int Sketch::addBSpline(const Part::GeomBSplineCurve &bspline, bool fixed)
{
    ...
    for (size_t i = 0; i < weights.size(); i++) {
        if (weights[i] != 1.0) { lastnotone = weights[i]; }
        else { // is 1.0
            lastoneindex = i;
            countones++;
        }
    ...
    if (countones == 1) weights[lastoneindex] = (lastnotone * 0.99);
    ...
    for (std::vector<Base::Vector3d>::const_iterator it = poles.begin(); it != poles.end(); ++it) {
        ...
        if (!fixed) {
            param2geoelement.emplace(
                std::piecewise_construct, std::forward_as_tuple(p.x),
                std::forward_as_tuple(Geoms.size(), Sketcher::PointPos::none, i++));
            param2geoelement.emplace(
                std::piecewise_construct, std::forward_as_tuple(p.y),
                std::forward_as_tuple(Geoms.size(), Sketcher::PointPos::none, i++));
        }
    }
...
int Sketch::addEllipse(const Part::GeomEllipse &elip, bool fixed)
{
    ...
    if (!fixed) {
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(c.x),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 0));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(c.y),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::mid, 1));
        param2geoelement.emplace(
            std::piecewise_construct, std::forward_as_tuple(f1X),
            std::forward_as_tuple(Geoms.size() - 1, Sketcher::PointPos::none, 0));
...
GeoListFacade Sketch::extractGeoListFacade() const
{
    ...
    for (std::vector<GeoDef>::const_iterator it = Geoms.begin(); it != Geoms.end(); ++it) {
        auto gf = GeometryFacade::getFacade(it->geo->clone(),
                                            true); // GeometryFacade is the owner of this allocation
        if (!it->external) internalGeometryCount++;
...
GCS::Curve *Sketch::getGCSCurveByGeoId(int geoId)
{
    ...
    switch (Geoms[geoId].type) {
        case Line: return &Lines[Geoms[geoId].index]; break;
        case Circle: return &Circles[Geoms[geoId].index]; break;
        case Arc: return &Arcs[Geoms[geoId].index]; break;
        case Ellipse: return &Ellipses[Geoms[geoId].index]; break;
        case ArcOfEllipse: return &ArcsOfEllipse[Geoms[geoId].index]; break;
        case ArcOfHyperbola: return &ArcsOfHyperbola[Geoms[geoId].index]; break;
        case ArcOfParabola: return &ArcsOfParabola[Geoms[geoId].index]; break;
        case BSpline: return &BSplines[Geoms[geoId].index]; break;
    ...
    switch (constraint->Type) {
        case DistanceX:
            if (constraint->FirstPos == PointPos::none) { // horizontal length of a line
                ...
                if (c.driving) FixParameters.push_back(c.value);
                else {
        ...
        case Vertical:
            ...
            break;
        case Coincident:
            rtn = addPointCoincidentConstraint(constraint->First, constraint->FirstPos,
                                               constraint->Second, constraint->SecondPos);
            break;
        case PointOnObject:
            rtn = addPointOnObjectConstraint(constraint->First, constraint->FirstPos,
                                             constraint->Second);
            break;
        case Parallel: rtn = addParallelConstraint(constraint->First, constraint->Second); break;
        case Perpendicular:
            if (constraint->FirstPos == PointPos::none && constraint->SecondPos == PointPos::none
                ...
                rtn = addAngleAtPointConstraint(constraint->First, constraint->FirstPos,
                                                constraint->Second, constraint->SecondPos,
                                                constraint->Third, constraint->ThirdPos, c.value,
                                                constraint->Type, c.driving);
            }
            break;
        case Tangent:
            if (constraint->FirstPos == PointPos::none && constraint->SecondPos == PointPos::none
        ...
        case Angle:
            if (constraint->Third != GeoEnum::GeoUndef) {
                ...
                rtn = addAngleAtPointConstraint(constraint->First, constraint->FirstPos,
                     ...
                     != PointPos::none) { // angle between two lines (with explicit start points)
                c.value = new double(constraint->getValue());
                if (c.driving) FixParameters.push_back(c.value);
                else {
                    Parameters.push_back(c.value);
                ...
                rtn =
                    addAngleConstraint(constraint->First, constraint->FirstPos, constraint->Second,
                                       constraint->SecondPos, c.value, c.driving);
            }
        ...
        case Equal: rtn = addEqualConstraint(constraint->First, constraint->Second); break;
        case Symmetric:
        ...
        case InternalAlignment:
            switch (constraint->AlignmentType) {
                ...
                case BSplineKnotPoint:
                    rtn = addInternalAlignmentKnotPoint(constraint->First, constraint->Second,
                                ...
                                const std::vector<Constraint *> &ConstraintList) const
{
    ...
    for (std::vector<Constraint *>::const_iterator it = ConstraintList.begin();
        ...
        switch ((*it)->Type) {
            ...
            case InternalAlignment: internalAlignmentConstraintIndex.push_back(i); break;
            default: break;
        }
    }
    ...
    for (std::vector<int>::iterator it = internalAlignmentConstraintIndex.begin();
         it != internalAlignmentConstraintIndex.end(); it++) {
    ...
    for (std::vector<Constraint *>::const_iterator it = ConstraintList.begin();
        ...
        if ((*it)->isDriving) {
            ...
            for (std::vector<int>::iterator itg = internalAlignmentgeo.begin();
                 itg != internalAlignmentgeo.end(); itg++) {
...
int Sketch::addDistanceXConstraint(int geoId, double *value, bool driving)
{
    ...
    if (Geoms[geoId].type != Line) return -1;
...
int Sketch::addParallelConstraint(int geoId1, int geoId2)
{
    ...
    if (Geoms[geoId1].type != Line || Geoms[geoId2].type != Line) return -1;
                                      ...
                                      bool driving)
{
    ...
    geoId1 = checkGeoId(geoId1);
    geoId2 = checkGeoId(geoId2);
    if (avp) geoId3 = checkGeoId(geoId3);
    ...
    int pointId = -1;
    if (avp) pointId = getPointId(geoId3, pos3);
    else if (e2e || e2c)
                                  ...
                                  bool driving)
{
    ...
    if (Geoms[geoId2].type != Line) return -1;
...
int Sketch::addSymmetricConstraint(int geoId1, PointPos pos1, int geoId2, PointPos pos2, int geoId3)
{
    ...
    if (Geoms[geoId3].type != Line) return -1;
                                   ...
                                   int geoIdBnd, double *value, double *secondvalue, bool driving)
{
    ...
    tag = ++ConstraintsCounter;
    //GCSsys.addConstraintP2PCoincident(p1, p2, tag);
    GCSsys.addConstraintSnellsLaw(*ray1, *ray2, *boundary, p1, n1, n2, posRay1 == PointPos::start,
                                  posRay2 == PointPos::end, tag, driving);
    return ConstraintsCounter;
}
...
int Sketch::addInternalAlignmentEllipseMajorDiameter(int geoId1, int geoId2)
{
    ...
    if (Geoms[geoId1].type != Ellipse && Geoms[geoId1].type != ArcOfEllipse) return -1;
    if (Geoms[geoId2].type != Line) return -1;
...
int Sketch::addInternalAlignmentEllipseFocus1(int geoId1, int geoId2)
{
    ...
    if (Geoms[geoId1].type != Ellipse && Geoms[geoId1].type != ArcOfEllipse) return -1;
    if (Geoms[geoId2].type != Point) return -1;
...
int Sketch::addInternalAlignmentHyperbolaMajorDiameter(int geoId1, int geoId2)
{
    ...
    if (Geoms[geoId1].type != ArcOfHyperbola) return -1;
    if (Geoms[geoId2].type != Line) return -1;
...
int Sketch::addInternalAlignmentParabolaFocus(int geoId1, int geoId2)
{
    ...
    if (Geoms[geoId1].type != ArcOfParabola) return -1;
    if (Geoms[geoId2].type != Point) return -1;
...
int Sketch::addInternalAlignmentBSplineControlPoint(int geoId1, int geoId2, int poleindex)
{
    ...
    if (Geoms[geoId1].type != BSpline) return -1;
    if (Geoms[geoId2].type != Circle) return -1;
    ...
    for (std::vector<GeoDef>::const_iterator it = Geoms.begin(); it != Geoms.end(); ++it, i++) {
        try {
            if (it->type == Point) {
                ...
                if (radmaj
                    >= aoe->getMinorRadius()) { //ensure that ellipse's major radius is always larger than minor raduis... may still cause problems with degenerates.
                ...
                if (radmaj
                    >= ellipse
                           ->getMinorRadius()) { //ensure that ellipse's major radius is always larger than minor raduis... may still cause problems with degenerates.
                ...
                for (it1 = mybsp.poles.begin(), it2 = mybsp.weights.begin();
                     it1 != mybsp.poles.end() && it2 != mybsp.weights.end(); ++it1, ++it2) {
                ...
                int index = 0;
                for (std::vector<int>::const_iterator it5 = mybsp.knotpointGeoids.begin();
                     it5 != mybsp.knotpointGeoids.end(); ++it5, index++) {
                    if (*it5 != GeoEnum::GeoUndef) {
                        if (Geoms[*it5].type == Point) {
                            ...
                            if (pointf->getInternalType() == InternalType::BSplineKnotPoint) {
                                ...
                                point->setPoint(
                                    pointcoords); // update the geompoint of the knot (geometry update)
    ...
    for (std::vector<ConstrDef>::iterator it = Constrs.begin(); it != Constrs.end(); ++it) {
        if (!(*it).driving) {
            if ((*it).constr->Type == SnellsLaw) {
                ...
                if (pos != FixParameters.end()) (*it).constr->setValue(*((*it).value));
                else
            ...
            switch (soltype) {
                ...
                case 3: // last resort: augment the system with a second subsystem and use the SQP solver
                    ...
                    for (std::vector<double *>::iterator it = Parameters.begin();
                        ...
                        InitParameters[i] = **it;
                        GCSsys.addConstraintEqual(*it, &InitParameters[i],
...
void Sketch::resetInitMove() { isInitMove = false; }
                                 ...
                                 bool fine)
{
    ...
    if (bsp.poles.size() <= std::size_t(bsp.degree + 1)) return initMove(geoId, pos, fine);
    ...
    auto upperknot = std::upper_bound(knots.begin(), knots.end(), uNear,
                                      [](double u, double *element) { return u < *element; });

    size_t idx = 0;
    // skipping the first knot for adjustment
    // TODO: ensure this works for periodic as well
    for (size_t i = 1; i < bsp.mult.size() && knots[i] != *upperknot; ++i) idx += bsp.mult[i];
...
int Sketch::movePoint(int geoId, PointPos pos, Base::Vector3d toPoint, bool relative)
{
    ...
    if (!isInitMove) {
        ...
        if (!relative && RecalculateInitialSolutionWhileMovingPoint) {
            if (moveStep == 0) { moveStep = (toPoint - initToPoint).Length(); }
            else {
                if ((toPoint - initToPoint).Length() > 20
                        * moveStep) { // I am getting too far away from the original solution so reinit the solution
                    initMove(geoId, pos);
                    initToPoint = toPoint;
                }
            }
...
int Sketch::setDatum(int /*constrId*/, double /*value*/) { return -1; }

int Sketch::getPointId(int geoId, PointPos pos) const
{
    // do a range check first
    if (geoId < 0 || geoId >= (int)Geoms.size()) return -1;
    switch (pos) {
        case PointPos::start: return Geoms[geoId].startPointId;
        case PointPos::end: return Geoms[geoId].endPointId;
        case PointPos::mid: return Geoms[geoId].midPointId;
        case PointPos::none: break;
    }
    return -1;
}
...
Base::Vector3d Sketch::getPoint(int geoId, PointPos pos) const
{
    geoId = checkGeoId(geoId);
    int pointId = getPointId(geoId, pos);
    if (pointId != -1) return Base::Vector3d(*Points[pointId].x, *Points[pointId].y, 0);
...
TopoShape Sketch::toShape() const
{
    ...
    for (; it != Geoms.end(); ++it) {
        ...
        if (!it->external && !gf->getConstruction()) {
            ...
            if (it->type != Point) {
                ...
                if (!shape.IsNull()) edge_list.push_back(TopoDS::Edge(shape));
            }
            else
    ...
    if (wires.size() == 1 && vertex_list.empty()) { result = *wires.begin(); }
...
unsigned int Sketch::getMemSize() const { return 0; }

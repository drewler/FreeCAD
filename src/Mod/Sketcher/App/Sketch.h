...
public:
    ...
    enum GeoType
    {
        ...
        Ellipse =
            5, // 1 Point(mid), 5 Parameters(x,y,r1,r2,phi)  phi=angle xaxis of ellipse with respect of sketch xaxis
        ArcOfEllipse = 6,
        ArcOfHyperbola = 7,
        ArcOfParabola = 8,
        BSpline = 9
...
protected:
    ...
    struct GeoDef {
        GeoDef()
            : geo(nullptr), type(None), external(false), index(-1), startPointId(-1),
              midPointId(-1), endPointId(-1)
        {}
    ...
    struct ConstrDef {
        ConstrDef() : constr(nullptr), driving(true), value(nullptr), secondvalue(nullptr) {}

...
{
    ADD_PROPERTY_TYPE(Geometry, (nullptr), "Sketch", (App::PropertyType)(App::Prop_None),
                      "Sketch geometry");
    ADD_PROPERTY_TYPE(Constraints, (nullptr), "Sketch", (App::PropertyType)(App::Prop_None),
                      "Sketch constraints");
    ADD_PROPERTY_TYPE(ExternalGeometry, (nullptr, nullptr), "Sketch",
                      (App::PropertyType)(App::Prop_None), "Sketch external geometry");
    ADD_PROPERTY_TYPE(FullyConstrained, (false), "Sketch",
...
{
    if (Geometry.isTouched()) return 1;
    if (Constraints.isTouched()) return 1;
    if (ExternalGeometry.isTouched()) return 1;
...
int SketchObject::solve(bool updateGeoAfterSolving /*=true*/)
{
    Base::StateLocker lock(
        managedoperation,
        true); // no need to check input data validity as this is an sketchobject managed operation.
    ...
    lastSolverStatus =
        GCS::Failed; // Failure is default for notifying the user unless otherwise proven
...
int SketchObject::setDatum(int ConstrId, double Datum)
{
    ...
    if (this->Constraints.hasInvalidGeometry()) return -6;
    const std::vector<Constraint *> &vals = this->Constraints.getValues();
    if (ConstrId < 0 || ConstrId >= int(vals.size())) return -1;
    ConstraintType type = vals[ConstrId]->Type;
    if (!vals[ConstrId]->isDimensional() && type != Tangent
    ...
    if (err) this->Constraints.getValues()[ConstrId]->setValue(oldDatum); // newVals is a shell now
...
int SketchObject::getDriving(int ConstrId, bool &isdriving)
{
    ...
    if (!vals[ConstrId]->isDimensional()) return -1;
...
int SketchObject::toggleDriving(int ConstrId)
{
    ...
    bool extorconstructionpoint1 = (vals[ConstrId]->First == GeoEnum::GeoUndef)
        ...
        || (geof1 && geof1->isGeoType(Part::GeomPoint::getClassTypeId())
            && geof1->getConstruction());
    bool extorconstructionpoint2 = (vals[ConstrId]->Second == GeoEnum::GeoUndef)
        || (vals[ConstrId]->Second < 0)
        || (geof2 && geof2->isGeoType(Part::GeomPoint::getClassTypeId())
            && geof2->getConstruction());
    bool extorconstructionpoint3 = (vals[ConstrId]->Third == GeoEnum::GeoUndef)
        || (vals[ConstrId]->Third < 0)
        || (geof3 && geof3->isGeoType(Part::GeomPoint::getClassTypeId())
            && geof3->getConstruction());

    if (extorconstructionpoint1 && extorconstructionpoint2 && extorconstructionpoint3
        && !vals[ConstrId]->isDriving)
        return -4;
...
int SketchObject::testDrivingChange(int ConstrId, bool isdriving)
{
    ...
    if (!vals[ConstrId]->isDimensional()) return -2;
...
int SketchObject::setDatumsDriving(bool isdriving)
{
    ...
    const std::vector<Constraint *> &uvals =
        this->Constraints.getValues(); // newVals is a shell now
...
int SketchObject::setVirtualSpace(std::vector<int> constrIds, bool isinvirtualspace)
{
    ...
    if (constrIds.empty()) return 0;

    std::sort(constrIds.begin(), constrIds.end());

    const std::vector<Constraint *> &vals = this->Constraints.getValues();

    if (constrIds.front() < 0 || constrIds.back() >= int(vals.size())) return -1;
    ...
    std::vector<Sketcher::Constraint *> additionalconstraints)
{
    ...
    std::copy(additionalconstraints.begin(), additionalconstraints.end(),
...
Base::Vector3d SketchObject::getPoint(int GeoId, PointPos PosId) const
{
    ...
    if (geo->getTypeId() == Part::GeomPoint::getClassTypeId()) {
        ...
        if (PosId == PointPos::start) return lineSeg->getStartPoint();
        else if (PosId == PointPos::end)
            return lineSeg->getEndPoint();
    }
    else if (geo->getTypeId() == Part::GeomCircle::getClassTypeId()) {
        const Part::GeomCircle *circle = static_cast<const Part::GeomCircle *>(geo);
        if (PosId == PointPos::mid) return circle->getCenter();
    }
    else if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()) {
        const Part::GeomEllipse *ellipse = static_cast<const Part::GeomEllipse *>(geo);
        if (PosId == PointPos::mid) return ellipse->getCenter();
    }
    else if (geo->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
        const Part::GeomArcOfCircle *aoc = static_cast<const Part::GeomArcOfCircle *>(geo);
        if (PosId == PointPos::start) return aoc->getStartPoint(/*emulateCCW=*/true);
        else if (PosId == PointPos::end)
        ...
        if (PosId == PointPos::start) return aoh->getStartPoint();
        else if (PosId == PointPos::end)
        ...
        if (PosId == PointPos::start) return aop->getStartPoint();
        else if (PosId == PointPos::end)
        ...
        if (PosId == PointPos::start) return bsp->getStartPoint();
        else if (PosId == PointPos::end)
...
bool SketchObject::isSupportedGeometry(const Part::Geometry *geo) const
{
    ...
    if (geo->getTypeId() == Part::GeomTrimmedCurve::getClassTypeId()) {
        ...
        if (!circle.IsNull() || !ellipse.IsNull()) { return true; }
    }
    return false;
}
...
SketchObject::supportedGeometry(const std::vector<Part::Geometry *> &geoList) const
{
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = geoList.begin(); it != geoList.end();
         ++it) {
        if (isSupportedGeometry(*it)) { supportedGeoList.push_back(*it); }
    }
...
int SketchObject::delGeometry(int GeoId, bool deleteinternalgeo)
{
    ...
    const std::vector<Part::Geometry *> &vals = getInternalGeometry();
    if (GeoId < 0 || GeoId >= int(vals.size())) return -1;
    ...
    for (PointPos PosId = PointPos::start; PosId != PointPos::mid;) {
        ...
        PosId = (PosId == PointPos::start) ? PointPos::end
                                           : PointPos::mid; // loop through [start, end, mid]
    }
    ...
    for (auto cstr : constraints) {
        if (cstr->First == GeoId || cstr->Second == GeoId || cstr->Third == GeoId) continue;
        if (cstr->First > GeoId || cstr->Second > GeoId || cstr->Third > GeoId) {
            cstr = cstr->clone();
            if (cstr->First > GeoId) cstr->First -= 1;
            if (cstr->Second > GeoId) cstr->Second -= 1;
            if (cstr->Third > GeoId) cstr->Third -= 1;
        }
...
int SketchObject::delGeometries(const std::vector<int> &GeoIds)
{
    ...
    for (auto c : Constraints.getValues()) {
        if (c->Type == InternalAlignment) {
            ...
            if (pos != sGeoIds.end()) { sGeoIds.push_back(c->First); }
        }
    }
...
int SketchObject::delGeometriesExclusiveList(const std::vector<int> &GeoIds)
{
    ...
    const std::vector<Part::Geometry *> &vals = getInternalGeometry();
    if (sGeoIds.front() < 0 || sGeoIds.back() >= int(vals.size())) return -1;
    ...
    for (const auto ptr : this->Constraints.getValues()) constraints.push_back(ptr->clone());
    ...
    for (auto itGeo = sGeoIds.rbegin(); itGeo != sGeoIds.rend(); ++itGeo) {
        ...
        for (std::vector<Constraint *>::const_iterator it = constraints.begin();
            ...
            if ((*it)->First != GeoId && (*it)->Second != GeoId && (*it)->Third != GeoId) {
                if (copiedConstr->First > GeoId) copiedConstr->First -= 1;
                if (copiedConstr->Second > GeoId) copiedConstr->Second -= 1;
                if (copiedConstr->Third > GeoId) copiedConstr->Third -= 1;
...
int SketchObject::toggleConstruction(int GeoId)
{
    ...
    if (getGeometryFacade(GeoId)->isInternalAligned()) return -1;
...
int SketchObject::addCopyOfConstraints(const SketchObject &orig)
{
    ...
    for (auto &v : origvals) newVals.push_back(v->copy());
        ...
        if ((*it)->Type == Sketcher::Coincident) {
            if ((*it)->First == GeoId && (*it)->FirstPos == PosId) {
                if (replaceGeoId != GeoEnum::GeoUndef
                    && (replaceGeoId != (*it)->Second
                        || replacePosId != (*it)->SecondPos)) { // redirect this constraint
                ...
                if (replaceGeoId != GeoEnum::GeoUndef
                    && (replaceGeoId != (*it)->First
                        || replacePosId != (*it)->FirstPos)) { // redirect this constraint
                                             ...
                                             PointPos posId2)
{
    ...
    if (!arePointsCoincident(geoId1, posId1, geoId2, posId2)) {
        ...
        for (int i = 0; i < int(constraints.size()); i++) {
            ...
            if (c->Type == Sketcher::Distance || c->Type == Sketcher::Equal) {
                ...
                if (line1 || line2) { deleteme.push_back(i); }
            }
        }
    ...
    for (auto c : this->Constraints.getValues()) {
        ...
        if (c->Type == Sketcher::Coincident) {
            ...
            if (line1First || line2First || line1Second || line2Second) { continue; }
        }
                                      ...
                                      PointPos toPosId, bool doNotTransformTangencies)
{
    ...
    for (int i = 0; i < int(newVals.size()); i++) {
        if (vals[i]->First == fromGeoId && vals[i]->FirstPos == fromPosId
            ...
            if (vals[i]->Type == Sketcher::Tangent || vals[i]->Type == Sketcher::Perpendicular) {
                if (!doNotTransformTangencies) { constNew->Type = Sketcher::Coincident; }
            }
    ...
    if (changed) { this->Constraints.setValues(std::move(newVals)); }
    return 0;
}

int SketchObject::fillet(int GeoId, PointPos PosId, double radius, bool trim, bool createCorner)
{
    if (GeoId < 0 || GeoId > getHighestCurveIndex()) return -1;
                         ...
                         const Base::Vector3d &refPnt2, double radius, bool trim, bool createCorner)
{
    ...
    if (geo1->getTypeId() == Part::GeomLineSegment::getClassTypeId()
        ...
        if (!find2DLinesIntersection(lineSeg1, lineSeg2, intersection)) { return -1; }
        ...
        if (trim) {
            ...
            if (createCorner) { transferFilletConstraints(GeoId1, PosId1, GeoId2, PosId2); }
            else {
                delConstraintOnPoint(GeoId1, PosId1, false);
                delConstraintOnPoint(GeoId2, PosId2, false);
            }
        ...
        auto distancetorefpoints = [](Base::Vector3d ip1, Base::Vector3d ip2, Base::Vector3d ref1,
                                      Base::Vector3d ref2) {
        ...
        try {
            if (!curve1->closestParameter(refPnt1, refparam1)) return -1;
        }
        ...
        try {
            if (!curve2->closestParameter(refPnt2, refparam2)) return -1;
        }
        catch (Base::CADKernelError &e) {
            e.ReportException();
            THROWM(Base::CADKernelError,
        ...
        if (curve1PosId == Sketcher::PointPos::none) {
            ...
            if (geo1->isDerivedFrom(Part::GeomTrimmedCurve::getClassTypeId())
                ...
                try {
                    if (!tcurve1->intersectBasisCurves(tcurve2, points)) return -1;
                }
                catch (Base::CADKernelError &e) {
                    e.ReportException();
        ...
        try {
            if (!curve1->closestParameter(interpoints.first, intparam1)) return -1;
        }
        catch (Base::CADKernelError &e) {
            e.ReportException();
            THROWM(Base::CADKernelError,
        ...
        try {
            if (!curve2->closestParameter(interpoints.second, intparam2)) return -1;
        }
        catch (Base::CADKernelError &e) {
            e.ReportException();
            THROWM(Base::CADKernelError,
        ...
        if (radius == .0f) {
            ...
            try {
                ...
                if (!curve1->tangent(refparam1, tdir1)) return -1;

                if (!curve2->tangent(refparam2, tdir2)) return -1;
                ...
                if (std::abs(det) < Precision::Confusion())
                    throw Base::RuntimeError(
                        "No intersection of normals"); // no intersection of normals
...
#ifdef DEBUG
        Base::Console().Log("tangent vectors: (%f,%f,%f);(%f,%f,%f)\n", tdir1.x, tdir1.y, tdir1.z,
                            tdir2.x, tdir2.y, tdir2.z);
        ...
        Base::Console().Log("start point offset curves: (%f,%f,%f);(%f,%f,%f)\n", oc1pf.x, oc1pf.y,
                            oc1pf.z, oc2pf.x, oc2pf.y, oc2pf.z);
        ...
        try {
            if (!curve1->closestParameter(filletcenterpoint.first, refoparam1)) return -1;
        }
        catch (Base::CADKernelError &e) {
            e.ReportException();
        ...
        try {
            if (!curve2->closestParameter(filletcenterpoint.second, refoparam2)) return -1;
        }
        catch (Base::CADKernelError &e) {
            e.ReportException();
...
#ifdef DEBUG
        Base::Console().Log("refpoints: (%f,%f,%f);(%f,%f,%f)", refp1.x, refp1.y, refp1.z, refp2.x,
                            refp2.y, refp2.z);
#endif
        ...
        if (startAngle < 0) endAngle += 2 * M_PI;
...
int SketchObject::extend(int GeoId, double increment, PointPos endpoint)
{
    ...
    if (retcode == 0 && noRecomputes) { solve(); }
    return retcode;
}
...
{
    auto newConstr = createConstraint(constrType, firstGeoId, firstPos, secondGeoId, secondPos,
                                      thirdGeoId, thirdPos);
...
{
    if (GeoId < 0 || GeoId > getHighestCurveIndex()) return false;
    ...
    if (!Part2DObject::seekTrimPoints(geos, GeoId, point, localindex1, intersect1, localindex2,
                                      intersect2))
        return false;

    GeoId1 = getGeoIdFromCompleteGeometryIndex(
        localindex1); // invalid complete geometry indices are mapped to GeoUndef
...
int SketchObject::trim(int GeoId, const Base::Vector3d &point)
{
    ...
    auto arePointsWithinPrecision = [](Base::Vector3d point1, Base::Vector3d point2) {
        ...
        if ((point1 - point2).Length() < 500 * Precision::Confusion()) return true;

        return false;
    };

    auto isPointAtPosition = [this, arePointsWithinPrecision](int GeoId1, PointPos pos1,
                                                              Base::Vector3d point) {
    ...
    auto transformPreexistingConstraints =
        [this, isPointAtPosition](int GeoId, int GeoId1, Base::Vector3d point1,
            ...
            for (std::vector<Constraint *>::const_iterator it = constraints.begin();
                ...
                if (constr->Type == Sketcher::Tangent) {
                    if (constr->First == GeoId1 && constr->Second == GeoId) {
                        ...
                        if (secondPos == Sketcher::PointPos::none) secondPos = constr->FirstPos;
                        delete_list.push_back(constrId);
                    }
                    else if (constr->First == GeoId && constr->Second == GeoId1) {
                        constrType = Sketcher::Tangent;
                        if (secondPos == Sketcher::PointPos::none) secondPos = constr->SecondPos;
                        delete_list.push_back(constrId);
                    }
                }
    ...
    auto addConstraintIfNoPointOnBSpline = [this](ConstraintType constrType, int GeoId1,
                                                  PointPos pos1, int GeoId2, PointPos pos2) {
        if (constrType == PointOnObject
            && getGeometry(GeoId2)->getTypeId() == Part::GeomBSplineCurve::getClassTypeId())
    ...
    auto getIntersectionParameters = [](const Part::Geometry *geo, const Base::Vector3d point,
                                        double &pointParam, const Base::Vector3d point1,
                                        double &point1Param, const Base::Vector3d point2,
                                        double &point2Param) {
    ...
    if (GeoId1 != GeoEnum::GeoUndef && GeoId2 != GeoEnum::GeoUndef
        && arePointsWithinPrecision(
            point1,
            point2)) { // If both points are detected and are coincident, deletion is the only option.
        delGeometry(GeoId);
    ...
    if (isDerivedFromTrimmedCurve || isNonPeriodicBSpline) {
        ...
        if (geo->isDerivedFrom(Part::GeomConic::getClassTypeId())) {
            ...
            if (tc->isReversed()) {
                const_cast<Part::GeomConic *>(tc)
                    ->reverse(); // reversing does not change the curve as seen by the sketcher.
            }
        }
        ...
        if (!getIntersectionParameters(geo, point, pointParam, point1, point1Param, point2,
                                       point2Param))
            return -1;
        ...
        auto paramDistance = [](double param1, double param2) {
            ...
            if (distance < Precision::Confusion()) return 0.;
            else
                return distance;
        };
        ...
        if (GeoId1 != GeoEnum::GeoUndef && GeoId2 != GeoEnum::GeoUndef && pointParam > point1Param
            && pointParam < point2Param) { // Trim Point between intersection points

            if ((!trimmableStart && !trimmableEnd)
                || !trimmableMiddle) { // if after trimming nothing would be left or if there is nothing to trim
        ...
        if (op.Type == Operation::trim_delete) {
            ...
            if (isDerivedFromTrimmedCurve) {
                ...
                static_cast<Part::GeomBSplineCurve *>(newVals[GeoId])
                    ->Trim(firstParam, point1Param);
            ...
            if (!isLineSegment && !isNonPeriodicBSpline) { constrainAsEqual(GeoId, newGeoId); }
            ...
            addConstraintIfNoPointOnBSpline(constrType1, GeoId, Sketcher::PointPos::end, GeoId1,
                                            secondPos1);

            // Segment comprising the end
            transformPreexistingConstraints(GeoId, GeoId2, point2, constrType2, secondPos2);

            addConstraintIfNoPointOnBSpline(constrType2, newGeoId, Sketcher::PointPos::start,
                                            GeoId2, secondPos2);

            // Both segments have a coincident center
            if (!isLineSegment && !isBSpline) {
                addConstraint(Sketcher::Coincident, GeoId, Sketcher::PointPos::mid, newGeoId,
                              Sketcher::PointPos::mid);
            }

            if (isNonPeriodicBSpline) exposeInternalGeometry(GeoId);
        ...
        else if (op.Type == Operation::trim_start
                 || op.Type == Operation::trim_end) { // drop the second/first intersection point
            geo = getGeometry(GeoId);
            if (isDerivedFromTrimmedCurve) {
                auto newGeo = std::unique_ptr<Part::GeomTrimmedCurve>(
                    static_cast<Part::GeomTrimmedCurve *>(geo->clone()));

                if (op.Type == Operation::trim_start) newGeo->setRange(op.actingParam, lastParam);
                ...
                if (op.Type == Operation::trim_start) newGeo->Trim(op.actingParam, lastParam);
                else if (op.Type == Operation::trim_end)
            ...
            if (isLineSegment) { delEqualConstraintsOnGeoId(GeoId); }

            //****** Step B.1 (4) => Constraint end points ******//
            ConstraintType constrType =
                Sketcher::PointOnObject; // So this is the fallback constraint type here.
            PointPos secondPos = Sketcher::PointPos::none;

            transformPreexistingConstraints(GeoId, op.intersectingGeoId, op.actingPoint, constrType,
                                            secondPos);
            ...
            if (op.Type == Operation::trim_start) {
                ...
                addConstraintIfNoPointOnBSpline(constrType, GeoId, PointPos::start,
                                                op.intersectingGeoId, secondPos);
            }
            else if (op.Type == Operation::trim_end) {
                delConstraintOnPoint(GeoId, PointPos::end, false);
                // constrain the trimming point on the corresponding geometry
                addConstraintIfNoPointOnBSpline(constrType, GeoId, PointPos::end,
                                                op.intersectingGeoId, secondPos);
            }
    ...
    //******************* Step B.2 => Trimming for unbounded periodic geometries ****************************************//
        ...
        if (GeoId1 == GeoEnum::GeoUndef || GeoId2 == GeoEnum::GeoUndef) return -1;
        ...
        if (isCircle) {
            ...
            aoe->setRange(
                ...
                /*emulateCCW=*/false); // CCW curve goes from point2 (start) to point1 (end)
            geoNew = std::move(aoe);
        }
        ...
        if (isBSpline) exposeInternalGeometry(GeoId);
...
int SketchObject::split(int GeoId, const Base::Vector3d &point)
{
    ...
    if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
        ...
        splitPoint = Base::Vector3d(center.x + radius * cos(splitAngle),
                                    center.y + radius * sin(splitAngle));
        ...
        Base::Vector3d dir(point - center);
        splitAngle = atan2(dir.y, dir.x);
        if (splitAngle < startAngle) { splitAngle += M_PI * 2.0; }
        if (endAngle - splitAngle > splitAngle - startAngle) { longestPart = 1; }
                ...
                case Perpendicular: {
                    ...
                    if (geo->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
                        ...
                        if (conGeo && conGeo->isDerivedFrom(Part::GeomCurve::getClassTypeId())) {
                            ...
                            intersects[0] =
                                static_cast<const Part::GeomCurve *>(newGeometries[0])
                                    ->intersect(static_cast<const Part::GeomCurve *>(conGeo),
                                                intersections);
                            intersects[1] =
                                static_cast<const Part::GeomCurve *>(newGeometries[1])
                                    ->intersect(static_cast<const Part::GeomCurve *>(conGeo),
                                                intersections);

                            initial = longestPart;
                            if (intersects[0] != intersects[1]) { initial = intersects[1] ? 1 : 0; }
                            limit = initial + 1;
                        }
                    }
                ...
                case PointOnObject: {
                    if (con->FirstPos == PointPos::none && con->SecondPos == PointPos::none) {
                        ...
                        if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                            ...
                            Base::Vector3d conDir(
                                conPoint
                                - static_cast<const Part::GeomArcOfCircle *>(geo)->getLocation());
                            double conAngle = atan2(conDir.y, conDir.x);
                            if (conAngle < startAngle) { conAngle += M_PI * 2.0; }
                            if (conAngle > splitAngle) { targetId = newIds[1]; }
                        }
    ...
    for (std::vector<Part::Geometry *>::iterator it = newGeometries.begin();
         it != newGeometries.end(); ++it) {
        delete *it;
    }
    for (std::vector<Constraint *>::iterator it = newConstraints.begin();
                                     ...
                                     eReasonList *rsn) const
{
    ...
    try {
        if (!(this->testIfLinkDAGCompatible(pObj))) {
            if (rsn) *rsn = rlCircularReference;
            return false;
        }
    }
                                       ...
                                       bool &yinv, eReasonList *rsn) const
{
        ...
        if (body_this) {
            if (body_this != body_obj) {
                if (!this->allowOtherBody) {
                    ...
                    psObj->getExternalGeometryCount()
                    > 2) { // if the original sketch has external geometry AND it is not in this body prevent link
                    if (rsn) *rsn = rlOtherBodyWithLinks;
                    return false;
                }
            }
        }
    ...
    if (!allowUnaligned && (fabs(fabs(alignment) - 1) > Precision::Confusion())
        ...
        if (rsn) *rsn = rlOriginsMisaligned;
        return false;
    }
                               ...
                               Sketcher::PointPos refPosId /*=Sketcher::PointPos::none*/)
{
    ...
    if (refPosId == Sketcher::PointPos::none) {
        ...
        const Part::GeomLineSegment *refGeoLine =
        ...
        if (georef->getTypeId() == Part::GeomPoint::getClassTypeId()) {
            ...
            switch (refPosId) {
                ...
                case Sketcher::PointPos::mid:
                    ...
                    break;
                default: Base::Console().Error("Wrong PointPosId.\n"); return -1;
            }
        }
    ...
    // Block acceptGeometry in OnChanged to avoid unnecessary checks and updates
    {
        ...
        for (std::vector<Constraint *>::const_iterator it = constrvals.begin();
            ...
            auto fit = geoIdMap.find(
                (*it)
                    ->First); // we look in the map, because we might have skipped internal alignment geometry
            ...
            if (fit != geoIdMap.end()) { // if First of constraint is in geoIdList
                ...
                if ((*it)->Second == GeoEnum::GeoUndef /*&& (*it)->Third == GeoEnum::GeoUndef*/) {
                    if (refIsAxisAligned) {
                        ...
                        && (*it)->Type
                            != Sketcher::
                                Horizontal) { // this includes all non-directional single GeoId constraints, as radius, diameter, weight,...
                          ...
                          double perpscale /*= 1.0*/)
{
    ...
    if (newgeoIdList.empty()) { // default option to operate on all the geometry
        for (int i = 0; i < int(geovals.size()); i++) newgeoIdList.push_back(i);
    }
    ...
    for (y = 0; y < rsize; y++) {
        for (x = 0; x < csize; x++) {
            ...
            if (x == 0
                && y
                    == 0) { // the reference for constraining array elements is the first valid point of the first element
            ...
            for (std::vector<int>::const_iterator it = newgeoIdList.begin();
                ...
                if (geocopy->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                    ...
                    if (it == newgeoIdList.begin()) iterfirstpoint = ssp;
                }
                    ...
                    if (it == newgeoIdList.begin()) iterfirstpoint = scp;
                }
                    ...
                    if (it == newgeoIdList.begin()) iterfirstpoint = geoaoc->getStartPoint(true);
                }
                    ...
                    if (it == newgeoIdList.begin()) iterfirstpoint = geoaoe->getStartPoint(true);
                }
                    ...
                    if (it == newgeoIdList.begin()) iterfirstpoint = geobsp->getStartPoint();
                }
            ...
            if (!moveonly) {
                    ...
                    if (fit != geoIdMap.end()) { // if First of constraint is in geoIdList
                        ...
                        if ((*it)->Second
                            ...
                            if (((*it)->Type != Sketcher::DistanceX
                                ...
                                || (*it)->FirstPos
                                    == Sketcher::PointPos::
                                        none) { // if it is not a point locking DistanceX/Y
                                if (((*it)->Type == Sketcher::DistanceX
                                     || (*it)->Type == Sketcher::DistanceY
                                     || (*it)->Type == Sketcher::Distance
                            ...
                            if (sit != geoIdMap.end()) { // Second is also in the list
                                if ((*it)->Third == GeoEnum::GeoUndef) {
                                    if (((*it)->Type == Sketcher::DistanceX
                                        ...
                                        constNew->Second =
                                            fit->second; // first is already (*it->First)
                ...
                if (constraindisplacement) {
                    ...
                    Base::Vector3d sp = getPoint(refgeoid, refposId)
                        + ((x == 0)
                               ...
                               : (double(x - 1) * displacement
                                  + double(y)
                                      * perpendicularDisplacement)); // position of the reference point
                    Base::Vector3d ep =
                        iterfirstpoint; // position of the current instance corresponding point
                    ...
                    if (x == 0) { // first element of a row
                        ...
                        if (y
                            == 1) { // it is the first added element of this row in the perpendicular to displacementvector direction
                            rowrefgeoid = cgeoid;
                            cgeoid++;
                ...
                geoIdMap
                    .clear(); // after each creation reset map so that the key-value is univoque (only for operations other than move)
            }
        }
    }
...
int SketchObject::removeAxesAlignment(const std::vector<int> &geoIdList)
{
    ...
    if (changeConstraintIndices.empty()) return 0; // nothing to be done
    ...
    if (changed) Constraints.setValues(std::move(newconstrVals));
...
int SketchObject::exposeInternalGeometry(int GeoId)
{
    ...
    if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()
        ...
        for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
            ...
            if ((*it)->Type == Sketcher::InternalAlignment && (*it)->Second == GeoId) {
                switch ((*it)->AlignmentType) {
                    case Sketcher::EllipseMajorDiameter: major = true; break;
                    case Sketcher::EllipseMinorDiameter: minor = true; break;
                    case Sketcher::EllipseFocus1: focus1 = true; break;
                    case Sketcher::EllipseFocus2: focus2 = true; break;
                    default: return -1;
                }
            }
        }
        ...
        for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
            ...
            if ((*it)->Type == Sketcher::InternalAlignment && (*it)->Second == GeoId) {
                switch ((*it)->AlignmentType) {
                    case Sketcher::HyperbolaMajor: major = true; break;
                    case Sketcher::HyperbolaMinor: minor = true; break;
                    case Sketcher::HyperbolaFocus: focus = true; break;
                    default: return -1;
                }
            }
        }
        ...
        if (focus) {
            ...
            getDirectlyCoincidentPoints(focusgeoid, Sketcher::PointPos::start,
                                        focusgeoidlistgeoidlist, focusposidlist);
                ...
                for (i = 0; i < focusgeoidlistgeoidlist.size(); i++) {
                    for (j = 0; j < parabgeoidlistgeoidlist.size(); j++) {
                        if (focusgeoidlistgeoidlist[i] == parabgeoidlistgeoidlist[j]) {
                            ...
                            if (geo
                                && geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                                if ((focusposidlist[i] == Sketcher::PointPos::start
                                     && parabposidlist[j] == Sketcher::PointPos::end)
                                    || (focusposidlist[i] == Sketcher::PointPos::end
                                        && parabposidlist[j] == Sketcher::PointPos::start))
        ...
        for (it = controlpointgeoids.begin(), itb = controlpoints.begin();
             it != controlpointgeoids.end() && itb != controlpoints.end(); ++it, ++itb) {
            (*it) = -1;
            (*itb) = false;
        }

        for (it = knotgeoids.begin(), itb = knotpoints.begin();
             it != knotgeoids.end() && itb != knotpoints.end(); ++it, ++itb) {
            (*it) = -1;
            (*itb) = false;
        }
        ...
        for (it = controlpointgeoids.begin(), itb = controlpoints.begin();
             it != controlpointgeoids.end() && itb != controlpoints.end(); ++it, ++itb, index++) {
        ...
        for (it = knotgeoids.begin(), itb = knotpoints.begin();
             it != knotgeoids.end() && itb != knotpoints.end(); ++it, ++itb, index++) {
...
int SketchObject::deleteUnusedInternalGeometry(int GeoId, bool delgeoid)
{
    ...
    if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()
        ...
        for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
            ...
            if ((*it)->Type == Sketcher::InternalAlignment && (*it)->Second == GeoId) {
                switch ((*it)->AlignmentType) {
                    ...
                    case Sketcher::HyperbolaMajor: majorelementindex = (*it)->First; break;
                    case Sketcher::EllipseMinorDiameter:
                    case Sketcher::HyperbolaMinor: minorelementindex = (*it)->First; break;
                    case Sketcher::EllipseFocus1:
                    case Sketcher::HyperbolaFocus: focus1elementindex = (*it)->First; break;
                    case Sketcher::EllipseFocus2: focus2elementindex = (*it)->First; break;
                    default: return -1;
                }
            }
        }
        ...
        if (focus2constraints < 2) delgeometries.push_back(focus2elementindex);

        if (focus1constraints < 2) delgeometries.push_back(focus1elementindex);

        if (minorconstraints < 2) delgeometries.push_back(minorelementindex);

        if (majorconstraints < 2) delgeometries.push_back(majorelementindex);

        if (delgeoid) delgeometries.push_back(GeoId);

        std::sort(
            delgeometries.begin(),
            delgeometries.end()); // indices over an erased element get automatically updated!!

        if (!delgeometries.empty()) {
            for (std::vector<int>::reverse_iterator it = delgeometries.rbegin();
                 it != delgeometries.rend(); ++it) {
        ...
        for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
            ...
            if ((*it)->Type == Sketcher::InternalAlignment && (*it)->Second == GeoId) {
                switch ((*it)->AlignmentType) {
                    case Sketcher::ParabolaFocus: focus1elementindex = (*it)->First; break;
                    default: return -1;
                }
            }
        }
        ...
        if (focus1elementindex != -1) {
            ...
            std::vector<int> focusgeoidlistgeoidlist;
            std::vector<PointPos> focusposidlist;
            getDirectlyCoincidentPoints(focus1elementindex, Sketcher::PointPos::start,
                                        focusgeoidlistgeoidlist, focusposidlist);
        ...
        // Hide unused geometry here
        int majorconstraints =
            0; // number of constraints associated to the geoid of the major axis other than the coincident ones
        int focus1constraints = 0;
        ...
        if (majorelementindex != -1
            && majorconstraints < 3) { // major as two coincidents to focus and vertex
            delgeometries.push_back(majorelementindex);
            majorelementindex = -1;
        }

        if (majorelementindex == -1 && focus1elementindex != -1
            && focus1constraints < 3) // focus has one coincident and one internal align
        ...
        for (it = controlpointgeoids.begin(), ita = cpassociatedconstraints.begin();
             it != controlpointgeoids.end() && ita != cpassociatedconstraints.end(); ++it, ++ita) {
            (*it) = -1;
            (*ita) = 0;
        }

        for (it = knotgeoids.begin(), ita = kassociatedconstraints.begin();
             it != knotgeoids.end() && ita != kassociatedconstraints.end(); ++it, ++ita) {
            (*it) = -1;
            (*ita) = 0;
        }
        ...
        // search for existing poles
        for (std::vector<Sketcher::Constraint *>::const_iterator jt = vals.begin();
             jt != vals.end(); ++jt) {
                ...
                for (std::vector<Sketcher::Constraint *>::const_iterator itc = vals.begin();
                    ...
                    if ((*itc)->Type == Sketcher::Equal) {
                        ...
                        for (std::vector<int>::iterator its = controlpointgeoids.begin();
                             its != controlpointgeoids.end(); ++its) {
                            if ((*itc)->First == *its) { f = true; }
                        ...
                        if (f
                            != s) { // the equality constraint constraints a pole but it is not interpole
                            (*ita)++;
                        }
                    }
...
bool SketchObject::convertToNURBS(int GeoId)
{
    ...
    if (geo->getTypeId() == Part::GeomPoint::getClassTypeId()) return false;
    ...
    try {
        ...
        if (geo1->isDerivedFrom(Part::GeomArcOfConic::getClassTypeId())) {
            ...
            if (geoaoc->isReversed()) bspline->reverse();
        }
    }
    catch (const Base::Exception &e) {
        Base::Console().Error("%s\n", e.what());
...
bool SketchObject::increaseBSplineDegree(int GeoId, int degreeincrement /*= 1*/)
{
    ...
    if (geo->getTypeId() != Part::GeomBSplineCurve::getClassTypeId()) return false;
...
bool SketchObject::modifyBSplineKnotMultiplicity(int GeoId, int knotIndex, int multiplicityincr)
{
    ...
    if ((curmult + multiplicityincr)
        > degree) // zero is removing the knot, degree is just positional continuity
        THROWMT(Base::ValueError,
                QT_TRANSLATE_NOOP(
                    "Exceptions",
    ...
    if ((curmult + multiplicityincr)
        < 0) // zero is removing the knot, degree is just positional continuity
        THROWMT(
            Base::ValueError,
    ...
    for (int i = 0; i < int(poles.size()); i++) prevpole[i] = -1;
    ...
    for (int i = 0; i < int(knots.size()); i++) prevknot[i] = -1;
    ...
    // Trigger update now
    // Update geometry indices and rebuild vertexindex now via onChanged, so that ViewProvider::UpdateData is triggered.
    if (!delGeoId.empty()) { delGeometriesExclusiveList(delGeoId); }
    else {
        Geometry.touch();
    }
...
bool SketchObject::insertBSplineKnot(int GeoId, double param, int multiplicity)
{
    Base::StateLocker lock(
        managedoperation,
        true); // TODO: Check if this is still valid: no need to check input data validity as this is an sketchobject managed operation.
...
int SketchObject::carbonCopy(App::DocumentObject *pObj, bool construction)
{
    ...
    if (!isCarbonCopyAllowed(pObj->getDocument(), pObj, xinv, yinv)) return -1;
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = svals.begin(); it != svals.end();
         ++it) {
    ...
    for (std::vector<Sketcher::Constraint *>::const_iterator it = scvals.begin();
         it != scvals.end(); ++it) {
        Sketcher::Constraint *newConstr = (*it)->copy();
        if ((*it)->First >= 0) newConstr->First += nextgeoid;
        if ((*it)->Second >= 0) newConstr->Second += nextgeoid;
        if ((*it)->Third >= 0) newConstr->Third += nextgeoid;
    ...
    int sourceid = 0;
    for (std::vector<Sketcher::Constraint *>::const_iterator it = scvals.begin();
         it != scvals.end(); ++it, nextcid++, sourceid++) {

        if ((*it)->Type == Sketcher::Distance || (*it)->Type == Sketcher::Radius
            || (*it)->Type == Sketcher::Diameter || (*it)->Type == Sketcher::Weight
            || (*it)->Type == Sketcher::Angle || (*it)->Type == Sketcher::SnellsLaw
            || (*it)->Type == Sketcher::DistanceX || (*it)->Type == Sketcher::DistanceY) {
...
int SketchObject::addExternal(App::DocumentObject *Obj, const char *SubName)
{
    ...
    // so far only externals to the support of the sketch and datum features
    if (!isExternalAllowed(Obj->getDocument(), Obj)) return -1;
...
int SketchObject::delExternal(int ExtGeoId)
{
    ...
    if (ExtGeoId < 0 || ExtGeoId >= int(SubElements.size())) return -1;
    ...
    for (auto cstr : constraints) {
        if (cstr->First != GeoId && cstr->Second != GeoId && cstr->Third != GeoId) {
            ...
            if (copiedConstr->First < GeoId && copiedConstr->First != GeoEnum::GeoUndef) {
                if (cstr == copiedConstr) copiedConstr = cstr->clone();
            ...
            if (cstr != copiedConstr) copiedConstraints.push_back(copiedConstr);
        }
    }
    ...
    try {
        ...
        // revert to original values
        ExternalGeometry.setValues(originalObjects, originalSubElements);
        for (Constraint *it : copiedConstraints) delete it;
        return -1;
    }
...
int SketchObject::delAllExternal()
{
    ...
    try {
        ...
        // revert to original values
        ExternalGeometry.setValues(originalObjects, originalSubElements);
        for (Constraint *it : newConstraints) delete it;
        return -1;
    }
...
{
    if (GeoId >= 0) {
        if (GeoId < int(Geometry.getSize())) return GeoId;
    }
...
int SketchObject::getGeoIdFromCompleteGeometryIndex(int completeGeometryIndex) const
{
    ...
    if (completeGeometryIndex < Geometry.getSize()) return completeGeometryIndex;
    else
...
bool SketchObject::evaluateSupport()
{
    ...
    if (!link || !link->getTypeId().isDerivedFrom(Part::Feature::getClassTypeId())) return false;
    return true;
}
...
void SketchObject::rebuildExternalGeometry()
{
    ...
    gp_Trsf mov;
    mov.SetValues(invMat[0][0], invMat[0][1], invMat[0][2], invMat[0][3], invMat[1][0],
                  invMat[1][1], invMat[1][2], invMat[1][3], invMat[2][0], invMat[2][1],
                  invMat[2][2], invMat[2][3]);

    gp_Ax3 sketchAx3(gp_Pnt(Pos.x, Pos.y, Pos.z), gp_Dir(dN.x, dN.y, dN.z),
        ...
        switch (refSubShape.ShapeType()) {
            ...
            case TopAbs_EDGE: {
                ...
                if (curve.GetType() == GeomAbs_Line) {
                    ...
                    if (vec1.IsParallel(vec2, Precision::Confusion())) {
                        ...
                        if (vec1.IsNormal(
                            ...
                            if (!curve.IsClosed()) { // arc of circle
                                ...
                                gp_Pnt pntF = curve.Value(
                                    curve.FirstParameter()); // start point of arc of circle
                                gp_Pnt pntL = curve.Value(
                                    curve.LastParameter()); // end point of arc of circle
                                ...
                                double endAngle = curve.LastParameter() + startAngle
                                    - baseAngle; // apply same offset to end angle
                            ...
                            GeomAPI_ProjectPointOnSurf proj(cnt, gPlane);
                            cnt =
                                proj.NearestPoint(); // projection of circle center on sketch plane, 3D space
                            Base::Vector3d p(cnt.X(), cnt.Y(),
                                             cnt.Z()); // converting to FCAD style vector
                            ...
                            double cosTheta;
                            cosTheta = fabs(vec1.Dot(
                                vec2)); // cos of angle between the two planes, assuming vectirs are normalized to 1
                            minorRadius = origCircle.Radius() * cosTheta;

                            Base::Vector3d vectorMajorAxis(
                                vecMajorAxis.X(), vecMajorAxis.Y(),
                                vecMajorAxis.Z()); // maj axis into FCAD style vector
                            invRot.multVec(
                                vectorMajorAxis,
                                vectorMajorAxis); // transforming to sketch's (x,y) coordinates
                            vecMajorAxis.SetXYZ(gp_XYZ(vectorMajorAxis[0], vectorMajorAxis[1],
                                                       vectorMajorAxis[2])); // back to OCC

                            gp_Ax2 refFrameEllipse(
                                gp_Pnt(gp_XYZ(p[0], p[1], p[2])), gp_Vec(0, 0, 1),
                                vecMajorAxis); // NB: force normal of ellipse to be normal of sketch's plane.
                    ...
                    gp_Ax2 destCurveAx2(destCenter, gp_Dir(0, 0, sens > 0.0 ? 1.0 : -1.0),
                                        gp_Dir(destAxisMajor));

                    if ((RDest - rDest)
                        < (double)Precision::Confusion()) { // projection is a circle
                                ...
                                if (projCurve.GetType() == GeomAbs_Line) {
                                    ...
                                    if (P1.SquareDistance(P2) < Precision::Confusion()) {
                                        ...
                                        Handle(Geom_TrimmedCurve) tCurve =
                                            new Geom_TrimmedCurve(curve, projCurve.FirstParameter(),
            ...
            default: throw Base::TypeError("Unknown type of geometry"); break;
        }
    }
...
GeoListFacade SketchObject::getGeoListFacade() const
{
    ...
    for (auto geo : Geometry.getValues()) facade.push_back(GeometryFacade::getFacade(geo));
...
void SketchObject::rebuildVertexIndex()
{
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = geometry.begin();
        ...
        if (i > imax) i = -getExternalGeometryCount();
                                                    ...
                                                    bool &mid_external, bool &end_external)
{
    ...
    for (std::vector<std::map<int, Sketcher::PointPos>>::const_iterator it = coincidenttree.begin();
        ...
        if (geoId1iterator != (*it).end()) {
            ...
            if ((*it).begin()->first < 0) {
                if ((*geoId1iterator).second == Sketcher::PointPos::start) start_external = true;
...
{
    if (GeoId1 == GeoId2 && PosId1 == PosId2) return true;
...
{
    appendConstraintsMsg(conflicting, "Please remove the following conflicting constraint:\n",
...
{
    appendConstraintsMsg(redundant, "Please remove the following redundant constraint:",
                         "Please remove the following redundant constraints:", msg);
}

void SketchObject::appendMalformedConstraintsMsg(const std::vector<int> &malformed,
                                                 std::string &msg)
{
    appendConstraintsMsg(malformed, "Please remove the following malformed constraint:",
                         "Please remove the following malformed constraints:", msg);
}
...
{
    std::stringstream ss;
    if (msg.length() > 0) ss << msg;
    if (!vector.empty()) {
        if (vector.size() == 1) ss << singularmsg << std::endl;
        else
            ss << pluralmsg;
        ss << vector[0] << std::endl;
        for (unsigned int i = 1; i < vector.size(); i++) ss << ", " << vector[i];
        ss << "\n";
    }
    msg = ss.str();
}
...
bool SketchObject::evaluateConstraint(const Constraint *constraint) const
{
    ...
    switch (constraint->Type) {
        ...
        case PointOnObject:
        case Angle: break;
        case Tangent: requireSecond = true; break;
        case Symmetric:
        case SnellsLaw:
            requireSecond = true;
            requireThird = true;
...
bool SketchObject::evaluateConstraints() const
{
    ...
    if (static_cast<int>(geometry.size()) != extGeoCount + intGeoCount) return false;
    if (geometry.size() < 2) return false;

    std::vector<Sketcher::Constraint *>::const_iterator it;
    for (it = constraints.begin(); it != constraints.end(); ++it) {
        if (!evaluateConstraint(*it)) return false;
    }
...
void SketchObject::validateConstraints()
{
    ...
    for (it = constraints.begin(); it != constraints.end(); ++it) {
        ...
        if (valid) newConstraints.push_back(*it);
    }
                                             ...
                                             std::shared_ptr<const App::Expression> expr)
{
    ...
    if (prop == &Constraints) {
        ...
        if (!constraint->isDriving) return "Reference constraints cannot be set!";
    }
    ...
    const std::map<App::ObjectIdentifier, App::ObjectIdentifier> &renamed)
{
    ...
    for (auto doc : App::GetApplication().getDocuments()) doc->renameObjectIdentifiers(renamed);
}
...
double SketchObject::calculateConstraintError(int ConstrId)
{
    ...
    try {
        ...
        for (std::size_t i = 0; i < GeoIdList.size(); i++) {
            g = GeoIdList[i];
            if (g != GeoEnum::GeoUndef) { GeoIdList[i] = sk.addGeometry(this->getGeometry(g)); }
        }
...
unsigned int SketchObject::getMemSize() const { return 0; }
...
void SketchObject::onChanged(const App::Property *prop)
{
    ...
    if (prop == &Geometry || prop == &Constraints) {
            ...
            if (!internaltransaction) { // internal sketchobject operations changing both geometry and constraints will explicitly perform an update
                if (prop == &Geometry) {
                    if (managedoperation || isRestoring()) {
                        ...
                        if (!invalidinput) { acceptGeometry(); }
                        else {
                            Base::Console().Error(
        ...
        if (!isRestoring()) {
            ...
            if (ExternalGeometry.getSize() == 0) { delConstraintsToExternal(); }
        }
    }
#if 0
...
void SketchObject::onUndoRedoFinished()
{
    ...
    // such a recompute
    Constraints.checkConstraintIndices(
        getHighestCurveIndex(),
        -getExternalGeometryCount()); // in case it is redoing an operation with invalid data.
    acceptGeometry();
...
void SketchObject::synchroniseGeometryState()
{
    ...
    for (size_t i = 0; i < vals.size(); i++) {
        ...
        if (constraintBlockedState != facadeBlockedState) gf->setBlocked(constraintBlockedState);
    }
}
    ...
    const Constraint *cstr, Sketcher::InternalType::InternalType &internaltypestate) const
{
    if (cstr->Type == InternalAlignment) {
        ...
        switch (cstr->AlignmentType) {
            case Undef:
            case NumInternalAlignmentType: internaltypestate = InternalType::None; break;
            ...
            case EllipseMinorDiameter:
                ...
                break;
            case EllipseFocus1: internaltypestate = InternalType::EllipseFocus1; break;
            case EllipseFocus2: internaltypestate = InternalType::EllipseFocus2; break;
            case HyperbolaMajor: internaltypestate = InternalType::HyperbolaMajor; break;
            case HyperbolaMinor: internaltypestate = InternalType::HyperbolaMinor; break;
            case HyperbolaFocus: internaltypestate = InternalType::HyperbolaFocus; break;
            case ParabolaFocus: internaltypestate = InternalType::ParabolaFocus; break;
            case BSplineControlPoint: internaltypestate = InternalType::BSplineControlPoint; break;
            case BSplineKnotPoint: internaltypestate = InternalType::BSplineKnotPoint; break;
        }
...
void SketchObject::restoreFinished()
{
    try {
        ...
        if (Shape.getValue().IsNull() && hasConflicts() == 0) {
            if (this->solve(true) == 0) Shape.setValue(solvedSketch.toShape());
        }
    }
    catch (...) {
    }
...
void SketchObject::migrateSketch()
{
    ...
    for (const auto &g : getInternalGeometry())
        if (!g->hasExtension(
                SketchGeometryExtension::getClassTypeId())) // no extension - legacy file
            noextensions = true;
            ...
            if (g->hasExtension(Part::GeometryMigrationExtension::getClassTypeId())) {
                ...
                if (ext->testMigrationType(Part::GeometryMigrationExtension::Construction)) {
                    auto gf = GeometryFacade::getFacade(
                        g); // at this point IA geometry is already migrated
            ...
            if (geoId <= GeoEnum::RefExt
                ...
                if (g->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
                    ...
                    if (segm->isReversed()) {
                        ...
                        //create a constraint copy, affect it, replace the pointer
                        if (!affected) constNew = newVals[ic]->clone();
                        affected = true;
...
bool SketchObject::AutoLockTangencyAndPerpty(Constraint *cstr, bool bForce, bool bLock)
{
    try {
        ...
        if (cstr->getValue() != 0.0
            && !bForce) /*tangency type already set. If not bForce - don't touch.*/
            return true;
        if (!bLock) {
            ...
            if (posPt
                == PointPos::
                    none) { //not endpoint-to-curve and not endpoint-to-endpoint tangent (is simple tangency)
                ...
                double angleOffset =
                    0.0; //the difference between the datum value and the actual angle to apply. (datum=angle+offset)
                double angleDesire =
                    0.0; //the desired angle value (and we are to decide if 180* should be added to it)
                ...
                //the autodetector
                if (fabs(angleErr) > M_PI / 2) angleDesire += M_PI;

                cstr->setValue(
                    angleDesire
                    + angleOffset); //external tangency. The angle stored is offset by Pi/2 so that a value of 0.0 is invalid and treated as "undecided".
            }
        }
    }
    catch (Base::Exception &e) {
...
int SketchObject::autoRemoveRedundants(bool updategeo)
{
    ...
    if (redundants.empty()) return 0;

    for (size_t i = 0; i < redundants.size();
         i++) // getLastRedundant is base 1, while delConstraints is base 0
        redundants[i]--;
...
int SketchObject::renameConstraint(int GeoId, std::string name)
{
    ...
    if (item->Name != name) {
        ...
        solverNeedsUpdate =
            true; // make sure any prospective solver access updates the constraint pointer that just got invalidated
...
std::vector<Base::Vector3d> SketchObject::getOpenVertices() const
{
    ...
    if (analyser) points = analyser->getOpenVertices();
...
int SketchObject::setGeometryId(int GeoId, long id)
{
    ...
    if (GeoId < 0 || GeoId >= int(Geometry.getValues().size())) return -1;

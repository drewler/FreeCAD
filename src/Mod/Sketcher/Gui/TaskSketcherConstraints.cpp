...
public:
    ConstraintItem(const Sketcher::SketchObject *s, ViewProviderSketch *sketchview, int ConstNbr)
        : QListWidgetItem(QString()), sketch(s), sketchView(sketchview), ConstraintNbr(ConstNbr)
    {
    ...
    {
        if (role == Qt::EditRole) this->value = value;
    ...
    {
        if (ConstraintNbr < 0 || ConstraintNbr >= sketch->Constraints.getSize()) return QVariant();
        ...
        if (role == Qt::EditRole) {
            ...
            switch (constraint->Type) {
                ...
                case Sketcher::InternalAlignment: break;
                default: break;
            }
            ...
            auto selicon = [](const Sketcher::Constraint *constr, const QIcon &normal,
            ...
            switch (constraint->Type) {
                case Sketcher::Horizontal: return selicon(constraint, horiz, horiz);
                case Sketcher::Vertical: return selicon(constraint, vert, vert);
                case Sketcher::Coincident: return selicon(constraint, coinc, coinc);
                case Sketcher::Block: return selicon(constraint, block, block);
                case Sketcher::PointOnObject: return selicon(constraint, pntoo, pntoo);
                case Sketcher::Parallel: return selicon(constraint, para, para);
                case Sketcher::Perpendicular: return selicon(constraint, perp, perp);
                case Sketcher::Tangent: return selicon(constraint, tang, tang);
                case Sketcher::Equal: return selicon(constraint, equal, equal);
                case Sketcher::Symmetric: return selicon(constraint, symm, symm);
                case Sketcher::Distance: return selicon(constraint, dist, dist_driven);
                case Sketcher::DistanceX: return selicon(constraint, hdist, hdist_driven);
                case Sketcher::DistanceY: return selicon(constraint, vdist, vdist_driven);
                case Sketcher::Radius:
                case Sketcher::Weight: return selicon(constraint, radi, radi_driven);
                case Sketcher::Diameter: return selicon(constraint, dia, dia_driven);
                case Sketcher::Angle: return selicon(constraint, angl, angl_driven);
                case Sketcher::SnellsLaw: return selicon(constraint, snell, snell_driven);
                case Sketcher::InternalAlignment:
                    switch (constraint->AlignmentType) {
                        ...
                        case Sketcher::Undef:
                        default: return selicon(constraint, iaellipseother, iaellipseother);
                    }
    ...
    bool isEnforceable() const
    {
        ...
        switch (constraint->Type) {
            case Sketcher::None:
            case Sketcher::NumConstraintTypes: assert(false); return false;
            case Sketcher::Horizontal:
            case Sketcher::Vertical:
            case Sketcher::Coincident:
            case Sketcher::Block:
            ...
            case Sketcher::Tangent:
            case Sketcher::Equal:
            case Sketcher::Symmetric: return true;
            case Sketcher::Distance:
            case Sketcher::DistanceX:
            case Sketcher::DistanceY:
            case Sketcher::Radius:
            ...
            case Sketcher::SnellsLaw:
                return (constraint->First >= 0 || constraint->Second >= 0
                        || constraint->Third >= 0);
            case Sketcher::InternalAlignment: return true;
        }
        return false;
    }
...
protected:
               ...
               const QModelIndex &index) const override
    {
        ...
        if (!item || item->sketch->Constraints.getSize() <= item->ConstraintNbr) return;
...
ConstraintView::ConstraintView(QWidget *parent) : QListWidget(parent)
{
...
void ConstraintView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (it) {
        ...
        if (it->isDimensional()) {
            ...
            isQuantity = true;
            if (it->isEnforceable()) isToggleDriving = true;
        }
    ...
    QAction *driven =
        menu.addAction(tr("Toggle to/from reference"), this, SLOT(updateDrivingStatus()));
    driven->setEnabled(isToggleDriving);

    QAction *activate = menu.addAction(isActive ? tr("Deactivate") : tr("Activate"), this,
                                       SLOT(updateActiveStatus()));
    ...
    menu.addSeparator();
    CONTEXT_ITEM("Sketcher_SelectElementsAssociatedWithConstraints", "Select Elements",
                 "Sketcher_SelectElementsAssociatedWithConstraints", doSelectConstraints, true)
    ...
    QAction *swap =
        menu.addAction(tr("Swap constraint names"), this, SLOT(swapNamedOfSelectedItems()));
...
void ConstraintView::updateDrivingStatus()
{
    ...
    ConstraintItem *it = dynamic_cast<ConstraintItem *>(item);
    if (it) { Q_EMIT onUpdateDrivingStatus(item, !it->isDriving()); }
}
...
void ConstraintView::updateActiveStatus()
{
    ...
    ConstraintItem *it = dynamic_cast<ConstraintItem *>(item);
    if (it) { Q_EMIT onUpdateActiveStatus(item, !it->isActive()); }
}

void ConstraintView::showConstraints() { Q_EMIT emitShowSelection3DVisibility(); }

void ConstraintView::hideConstraints() { Q_EMIT emitHideSelection3DVisibility(); }

void ConstraintView::modifyCurrentItem() { Q_EMIT itemActivated(currentItem()); }
...
void ConstraintView::centerSelectedItems() { Q_EMIT emitCenterSelectedItems(); }
...
void ConstraintView::swapNamedOfSelectedItems()
{
    ...
    Gui::cmdAppObjectArgs(item1->sketch, "renameConstraint(%d, u'%s')", item1->ConstraintNbr,
                          tmpname.c_str());
    Gui::cmdAppObjectArgs(item2->sketch, "renameConstraint(%d, u'%s')", item2->ConstraintNbr,
                          escapedstr1.c_str());
    Gui::cmdAppObjectArgs(item1->sketch, "renameConstraint(%d, u'%s')", item1->ConstraintNbr,
                          escapedstr2.c_str());
...
TaskSketcherConstraints::TaskSketcherConstraints(ViewProviderSketch *sketchView)
    : TaskBox(Gui::BitmapFactory().pixmap("document-new"), tr("Constraints"), true, nullptr),
      sketchView(sketchView), inEditMode(false), ui(new Ui_TaskSketcherConstraints)
{
    ...
    QObject::connect(ui->comboBoxFilter, SIGNAL(currentIndexChanged(int)), this,
                     SLOT(on_comboBoxFilter_currentIndexChanged(int)));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(itemSelectionChanged()), this,
                     SLOT(on_listWidgetConstraints_itemSelectionChanged()));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(itemActivated(QListWidgetItem *)), this,
                     SLOT(on_listWidgetConstraints_itemActivated(QListWidgetItem *)));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(itemChanged(QListWidgetItem *)), this,
                     SLOT(on_listWidgetConstraints_itemChanged(QListWidgetItem *)));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(emitCenterSelectedItems()), this,
                     SLOT(on_listWidgetConstraints_emitCenterSelectedItems()));
    QObject::connect(ui->listWidgetConstraints,
                     SIGNAL(onUpdateDrivingStatus(QListWidgetItem *, bool)), this,
                     SLOT(on_listWidgetConstraints_updateDrivingStatus(QListWidgetItem *, bool)));
    QObject::connect(ui->listWidgetConstraints,
                     SIGNAL(onUpdateActiveStatus(QListWidgetItem *, bool)), this,
                     SLOT(on_listWidgetConstraints_updateActiveStatus(QListWidgetItem *, bool)));
    QObject::connect(ui->showAllButton, SIGNAL(clicked(bool)), this,
                     SLOT(on_showAllButton_clicked(bool)));
    QObject::connect(ui->hideAllButton, SIGNAL(clicked(bool)), this,
                     SLOT(on_hideAllButton_clicked(bool)));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(emitHideSelection3DVisibility()), this,
                     SLOT(on_listWidgetConstraints_emitHideSelection3DVisibility()));
    QObject::connect(ui->listWidgetConstraints, SIGNAL(emitShowSelection3DVisibility()), this,
                     SLOT(on_listWidgetConstraints_emitShowSelection3DVisibility()));
    QObject::connect(ui->multipleFilterButton, SIGNAL(clicked(bool)), this,
                     SLOT(on_multipleFilterButton_clicked(bool)));
    QObject::connect(ui->settingsDialogButton, SIGNAL(clicked(bool)), this,
                     SLOT(on_settingsDialogButton_clicked(bool)));
    QObject::connect(ui->visibilityButton, SIGNAL(clicked(bool)), this,
                     SLOT(on_visibilityButton_clicked(bool)));

    QObject::connect(qAsConst(ui->visibilityButton)->actions()[0], SIGNAL(changed()), this,
                     SLOT(on_visibilityButton_trackingaction_changed()));
...
TaskSketcherConstraints::~TaskSketcherConstraints() { connectionConstraintsChanged.disconnect(); }
...
void TaskSketcherConstraints::on_multipleFilterButton_clicked(bool)
{
    ...
    if (!isFilterMatch(ConstraintFilter::SpecialFilterValue::Multiple, filterindex)) {
        ui->comboBoxFilter->setCurrentIndex(
            getFilterIntegral(ConstraintFilter::SpecialFilterValue::
                                  Multiple)); // Change filter to multi filter selection
    }
...
void TaskSketcherConstraints::on_settingsDialogButton_clicked(bool)
{
    ...
    QObject::connect(&cs, SIGNAL(emit_filterInternalAlignment_stateChanged(int)), this,
                     SLOT(on_filterInternalAlignment_stateChanged(int)));
    QObject::connect(&cs, SIGNAL(emit_extendedInformation_stateChanged(int)), this,
                     SLOT(on_extendedInformation_stateChanged(int)));
    QObject::connect(&cs, SIGNAL(emit_visualisationTrackingFilter_stateChanged(int)), this,
...
void TaskSketcherConstraints::changeFilteredVisibility(bool show, ActionTarget target)
{
    ...
    for (int i = 0; i < ui->listWidgetConstraints->count(); ++i) {
        ...
        if (target == ActionTarget::All) { processItem = !item->isHidden(); }
    ...
    if (!constrIds.empty()) {
        ...
        for (size_t i = 0; i < constrIds.size() - 1; i++) { stream << constrIds[i] << ","; }
        stream << constrIds[constrIds.size() - 1] << ']';

        std::string constrIdList = stream.str();

        try {
            Gui::cmdAppObjectArgs(sketch, "setVirtualSpace(%s, %s)", constrIdList,
                                  show ? "False" : "True");
        }
            ...
            QMessageBox::critical(Gui::MainWindow::getInstance(), tr("Error"),
                                  QString::fromLatin1("Impossible to update visibility tracking: ")
...
void TaskSketcherConstraints::on_showAllButton_clicked(bool) { changeFilteredVisibility(true); }

void TaskSketcherConstraints::on_hideAllButton_clicked(bool) { changeFilteredVisibility(false); }
    ...
    if (msg.Type == Gui::SelectionChanges::ClrSelection) {
        ...
        if (strcmp(msg.pDocName, sketchView->getSketchObject()->getDocument()->getName()) == 0
            ...
            if (msg.pSubName) {
                ...
                else if (isFilter(ConstraintFilter::SpecialFilterValue::
                                      AssociatedConstraints)) { // is NOT a constraint
...
void TaskSketcherConstraints::on_comboBoxFilter_currentIndexChanged(int filterindex)
{
    ...
    else if (isFilterMatch(ConstraintFilter::SpecialFilterValue::AssociatedConstraints,
                           filterindex)) {
        updateAssociatedConstraintsFilter();
    }
...
void TaskSketcherConstraints::on_visualisationTrackingFilter_stateChanged(int state)
{
    ...
    if (state == Qt::Checked) change3DViewVisibilityToTrackFilter();
}
...
void TaskSketcherConstraints::on_visibilityButton_trackingaction_changed()
{
    ...
    if (visibilityTracksFilter != bstate) { hGrp->SetBool("VisualisationTrackingFilter", bstate); }

    // Act
    if (bstate) change3DViewVisibilityToTrackFilter();
}
...
void TaskSketcherConstraints::on_listWidgetConstraints_itemChanged(QListWidgetItem *item)
{
    ...
    if (newName != currConstraintName && !basename.empty()) {
        ...
        try {
            Gui::cmdAppObjectArgs(sketch, "renameConstraint(%d, u'%s')", it->ConstraintNbr,
                                  escapedstr.c_str());
            Gui::Command::commitCommand();
        }
        catch (const Base::Exception &e) {
    ...
    try {
        Gui::cmdAppObjectArgs(
            sketch, "setVirtualSpace(%d, %s)", it->ConstraintNbr,
        ...
        auto doSetVirtualSpace = [&sketch](const std::vector<int> &constrIds, bool isvirtualspace) {
            ...
            try {
                ...
                QMessageBox::critical(
                    Gui::MainWindow::getInstance(), tr("Error"),
                    QString::fromLatin1("Impossible to update visibility tracking: ")
                        + QString::fromLatin1(e.what()),
                    QMessageBox::Ok, QMessageBox::Ok);
...
bool TaskSketcherConstraints::isConstraintFiltered(QListWidgetItem *item)
{
    ...
    bool showNonDriving =
        (isFilterMatch(FilterValue::NonDriving, Filter) && !constraint->isDriving);

    auto geometricVisible = [showAll, showGeometric, showNamed, Filter,
                             this](FilterValue filtervalue) {
        return showAll || showGeometric || showNamed || isFilterMatch(filtervalue, Filter)
            || (isFilterMatch(SpecialFilterValue::Multiple, Filter)
                && checkFilterBitset(multiFilterStatus, filtervalue));
    };

    auto datumVisible = [showAll, showDatums, showNamed, showNonDriving, Filter,
                         this](FilterValue filtervalue) {
        return showAll || showDatums || showNamed || showNonDriving
            || isFilterMatch(filtervalue, Filter)
            || (isFilterMatch(SpecialFilterValue::Multiple, Filter)
                && checkFilterBitset(multiFilterStatus, filtervalue));
    };

    switch (constraint->Type) {
        case Sketcher::Horizontal: visible = geometricVisible(FilterValue::Horizontal); break;
        case Sketcher::Vertical: visible = geometricVisible(FilterValue::Vertical); break;
        case Sketcher::Coincident: visible = geometricVisible(FilterValue::Coincident); break;
        case Sketcher::PointOnObject: visible = geometricVisible(FilterValue::PointOnObject); break;
        case Sketcher::Parallel: visible = geometricVisible(FilterValue::Parallel); break;
        case Sketcher::Perpendicular: visible = geometricVisible(FilterValue::Perpendicular); break;
        case Sketcher::Tangent: visible = geometricVisible(FilterValue::Tangent); break;
        case Sketcher::Equal: visible = geometricVisible(FilterValue::Equality); break;
        case Sketcher::Symmetric: visible = geometricVisible(FilterValue::Symmetric); break;
        case Sketcher::Block: visible = geometricVisible(FilterValue::Block); break;
        case Sketcher::Distance: visible = datumVisible(FilterValue::Distance); break;
        case Sketcher::DistanceX: visible = datumVisible(FilterValue::HorizontalDistance); break;
        case Sketcher::DistanceY: visible = datumVisible(FilterValue::VerticalDistance); break;
        case Sketcher::Radius: visible = datumVisible(FilterValue::Radius); break;
        case Sketcher::Weight: visible = datumVisible(FilterValue::Weight); break;
        case Sketcher::Diameter: visible = datumVisible(FilterValue::Diameter); break;
        case Sketcher::Angle: visible = datumVisible(FilterValue::Angle); break;
        case Sketcher::SnellsLaw: visible = datumVisible(FilterValue::SnellsLaw); break;
        case Sketcher::InternalAlignment:
            visible = geometricVisible(FilterValue::InternalAlignment)
                && (!hideInternalAlignment
                    || isFilterMatch(FilterValue::InternalAlignment, Filter));
        default: break;
    }

    // Constraint Type independent, selection filter
    visible = visible
        || (isFilterMatch(SpecialFilterValue::Selection, Filter)
            && std::find(selectionFilter.begin(), selectionFilter.end(), it->ConstraintNbr)
                != selectionFilter.end());

    // Constraint Type independent, associated Constraints Filter
    visible = visible
        || (isFilterMatch(SpecialFilterValue::AssociatedConstraints, Filter)
            && std::find(associatedConstraintsFilter.begin(), associatedConstraintsFilter.end(),
                         it->ConstraintNbr)
...
template<class T> bool TaskSketcherConstraints::isFilter(T filterValue)
{

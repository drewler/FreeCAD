...
bool SketcherGui::tryAutoRecompute(Sketcher::SketchObject *obj, bool &autoremoveredundants)
{
    ...
    if (autoRemoveRedundants && autoRecompute) obj->solve();

    if (autoRemoveRedundants) obj->autoRemoveRedundants();
...
{
    if (GeoId1 == GeoEnum::GeoUndef || GeoId2 == GeoEnum::GeoUndef) return false;
    else
        return (GeoId1 < 0 && GeoId2 < 0);
}

bool SketcherGui::checkBothExternalOrBSplinePoints(const Sketcher::SketchObject *Obj, int GeoId1,
                                                   int GeoId2)
{
    if (GeoId1 == GeoEnum::GeoUndef || GeoId2 == GeoEnum::GeoUndef) return false;
    else
        return (GeoId1 < 0 && GeoId2 < 0)
            || (isBsplineKnot(Obj, GeoId1) && isBsplineKnot(Obj, GeoId2))
            || (GeoId1 < 0 && isBsplineKnot(Obj, GeoId2))
            || (GeoId2 < 0 && isBsplineKnot(Obj, GeoId1));
}
...
bool SketcherGui::isPointOrSegmentFixed(const Sketcher::SketchObject *Obj, int GeoId)
{
    ...
    if (GeoId == GeoEnum::GeoUndef) return false;
    else
        return checkConstraint(vals, Sketcher::Block, GeoId, Sketcher::PointPos::none)
            || GeoId <= Sketcher::GeoEnum::RtPnt || isBsplineKnot(Obj, GeoId);
}
                                               ...
                                               int GeoId2)
{
    ...
    if (GeoId1 == GeoEnum::GeoUndef || GeoId2 == GeoEnum::GeoUndef) return false;
        ...
        return ((checkConstraint(vals, Sketcher::Block, GeoId1, Sketcher::PointPos::none)
                 || GeoId1 <= Sketcher::GeoEnum::RtPnt || isBsplineKnot(Obj, GeoId1))
                && (checkConstraint(vals, Sketcher::Block, GeoId2, Sketcher::PointPos::none)
                    || GeoId2 <= Sketcher::GeoEnum::RtPnt || isBsplineKnot(Obj, GeoId2)));
}
    ...
    if (GeoId1 == GeoEnum::GeoUndef || GeoId2 == GeoEnum::GeoUndef || GeoId3 == GeoEnum::GeoUndef)
        ...
        return ((checkConstraint(vals, Sketcher::Block, GeoId1, Sketcher::PointPos::none)
                ...
                && (checkConstraint(vals, Sketcher::Block, GeoId3, Sketcher::PointPos::none)
                    || GeoId3 <= Sketcher::GeoEnum::RtPnt || isBsplineKnot(Obj, GeoId3)));
}
...
bool SketcherGui::isSimpleVertex(const Sketcher::SketchObject *Obj, int GeoId, PointPos PosId)
{
    ...
    if (geo->getTypeId() == Part::GeomPoint::getClassTypeId()) return true;
                                          ...
                                          Sketcher::PointPos PosId)
{
    ...
    if (isBsplineKnot(Obj, GeoId)) return true;
...
bool SketcherGui::isBsplinePole(const Part::Geometry *geo)
{
    ...
    if (gf) return gf->getInternalType() == InternalType::BSplineControlPoint;
    ...
    if (doc) {
        ...
        if (doc->getInEdit()
            ...
            if (mode == ViewProviderSketch::STATUS_NONE
                ...
                if (!actsOnSelection) return true;
                                                    ...
                                                    std::vector<AutoConstraint> &sug2)
{
    if (!sug1.empty() && !sug2.empty()) {
        ...
        auto detectredundant = [psketch](std::vector<AutoConstraint> &sug, bool &ext, bool &orig,
                                         bool &axis) {
            ext = false;
            orig = false;
            axis = false;
                ...
                if ((*it).Type == Sketcher::Coincident && ext == false) {
                    ...
                    if (!coincidents.empty()) {
                        ...
                        if (geoId1iterator != coincidents.end()) {
                            if ((*geoId1iterator).second == Sketcher::PointPos::start) orig = true;
                        }
                    }
    ...
    if (distance == 0.) {
        ...
        if (attachment.isCurve()) {
            ...
            Gui::cmdAppObjectArgs(
                obj, "addConstraint(Sketcher.Constraint('Coincident',%d,%d,%d,%d)) ", element.GeoId,
                element.posIdAsInt(), attachment.GeoId, attachment.posIdAsInt());
        }
    }
    else {
...
std::string SketcherGui::lengthToDisplayFormat(double value, int digits)
{
    ...
    if (dpPos < 0) {
        ...
        if (hideUnits()) { return Base::Tools::toStdString(matched); }
        else {
    ...
    if (hideUnits()) { return Base::Tools::toStdString(numericPart); }
...
std::string SketcherGui::angleToDisplayFormat(double value, int digits)
{
    ...
    //get the numeric part of the user string
    QRegularExpression rxNoUnits(
        QString::fromUtf8("(.*)\\D*$")); // text before any non digits at end of string
    QRegularExpressionMatch match = rxNoUnits.match(qUserString);
    if (!match.hasMatch()) {
        //no units in userString?
        return Base::Tools::toStdString(qUserString);

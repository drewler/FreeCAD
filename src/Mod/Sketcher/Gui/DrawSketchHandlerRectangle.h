...
public:
    ...
    explicit DrawSketchHandlerBox(ConstructionMethod constrMethod = Diagonal)
        : Mode(STATUS_SEEK_First), EditCurve(5), constructionMethod(constrMethod)
    {}
    ...
    bool releaseButton(Base::Vector2d onSketchPos) override
    {
        ...
        if (Mode == STATUS_End) {
            ...
            if (constructionMethod == Diagonal) {
                ...
                // add auto constraints at the end of the second side
                if (!sugConstr2.empty()) {
                    createAutoConstraints(sugConstr2, getHighestCurveIndex() - 3,
                                          Sketcher::PointPos::end);
                    sugConstr2.clear();
                }
            }
...
public:
    DrawSketchHandlerOblong()
        : Mode(STATUS_SEEK_First), lengthX(0), lengthY(0), radius(0), signX(1), signY(1),
          EditCurve(37)
    {}
    ...
    void mouseMove(Base::Vector2d onSketchPos) override
    {
        ...
        if (Mode == STATUS_SEEK_First) {
            ...
            if (showCursorCoords()) {
                ...
                text.sprintf("  (R%s X%s Y%s)", radiusString.c_str(), xString.c_str(),
                             yString.c_str());
                setPositionText(onSketchPos, text);
            }
        ...
        if (Mode == STATUS_End) {
            ...
            try {
                ...
                Gui::Command::doCommand(
                    ...
                    radius, start, end,            // start and end angle of arc1
                    ...
                    end - M_PI, end - 0.5 * M_PI, EditCurve[17].x, EditCurve[17].y, EditCurve[18].x,
                    EditCurve[18].y, // line 2
                    StartPos.x + lengthX - (signX * radius),
                    StartPos.y + lengthY - (signY * radius), // center of the arc 3
                    radius, end - 1.5 * M_PI, end - M_PI, EditCurve[26].x, EditCurve[26].y,
                    EditCurve[27].x, EditCurve[27].y, // line 3
                    ...
                    radius, end - 2 * M_PI, end - 1.5 * M_PI, EditCurve[35].x, EditCurve[35].y,
                    EditCurve[36].x, EditCurve[36].y,                           // line 4
                    Gui::Command::getObjectCmd(sketchgui->getObject()).c_str(), // the sketch
                    geometryCreationMode == Construction
                        ? "True"
                        : "False",                  // geometry as construction or not
                ...
                if (!sugConstr1.empty()) {
                    createAutoConstraints(sugConstr1, getHighestCurveIndex() - 1,
                                          Sketcher::PointPos::start);
                    sugConstr1.clear();
                }

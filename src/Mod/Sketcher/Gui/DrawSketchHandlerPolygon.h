...
public:
    explicit DrawSketchHandlerRegularPolygon(size_t nof_corners)
        : Corners(nof_corners), AngleOfSeparation(2.0 * M_PI / static_cast<double>(Corners)),
          cos_v(cos(AngleOfSeparation)), sin_v(sin(AngleOfSeparation)), Mode(STATUS_SEEK_First),
          EditCurve(Corners + 1)
    {}
        ...
        if (Mode == STATUS_End) {
            ...
            try {
                Gui::Command::doCommand(Gui::Command::Doc,
                                        ...
                                        Corners, StartPos.x, StartPos.y, EditCurve[0].x,
                                        EditCurve[0].y,
                ...
                if (!sugConstr2.empty()) {
                    createAutoConstraints(sugConstr2, getHighestCurveIndex() - 1,

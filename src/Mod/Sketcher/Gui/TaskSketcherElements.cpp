...
{
public:
    ElementItem(const QIcon &icon, const QString &text, int elementnr, int startingVertex,
                int midVertex, int endVertex, Base::Type geometryType, bool construction,
                bool external)
        : QListWidgetItem(icon, text), ElementNbr(elementnr), StartingVertex(startingVertex),
          MidVertex(midVertex), EndVertex(endVertex), isLineSelected(false),
          isStartingPointSelected(false), isEndPointSelected(false), isMidPointSelected(false),
          GeometryType(geometryType), isConstruction(construction), isExternal(external)
    {}
    ElementItem(const QString &text, int elementnr, int startingVertex, int midVertex,
                int endVertex, Base::Type geometryType, bool construction, bool external)
        : QListWidgetItem(text), ElementNbr(elementnr), StartingVertex(startingVertex),
          MidVertex(midVertex), EndVertex(endVertex), isLineSelected(false),
          isStartingPointSelected(false), isEndPointSelected(false), isMidPointSelected(false),
          GeometryType(geometryType), isConstruction(construction), isExternal(external)
    {}
...
void ElementView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    CONTEXT_ITEM("Constraint_PointOnPoint", "Point Coincidence", "Sketcher_ConstrainCoincident",
                 doPointCoincidence, true)
    CONTEXT_ITEM("Constraint_PointOnObject", "Point on Object", "Sketcher_ConstrainPointOnObject",
                 doPointOnObjectConstraint, true)
    CONTEXT_ITEM("Constraint_Vertical", "Vertical Constraint", "Sketcher_ConstrainVertical",
                 doVerticalConstraint, true)
    CONTEXT_ITEM("Constraint_Horizontal", "Horizontal Constraint", "Sketcher_ConstrainHorizontal",
                 doHorizontalConstraint, true)
    CONTEXT_ITEM("Constraint_Parallel", "Parallel Constraint", "Sketcher_ConstrainParallel",
                 doParallelConstraint, true)
    CONTEXT_ITEM("Constraint_Perpendicular", "Perpendicular Constraint",
                 "Sketcher_ConstrainPerpendicular", doPerpendicularConstraint, true)
    CONTEXT_ITEM("Constraint_Tangent", "Tangent Constraint", "Sketcher_ConstrainTangent",
                 doTangentConstraint, true)
    CONTEXT_ITEM("Constraint_EqualLength", "Equal Length", "Sketcher_ConstrainEqual",
                 doEqualConstraint, true)
    CONTEXT_ITEM("Constraint_Symmetric", "Symmetric", "Sketcher_ConstrainSymmetric",
                 doSymmetricConstraint, true)
    CONTEXT_ITEM("Constraint_Block", "Block Constraint", "Sketcher_ConstrainBlock",
                 doBlockConstraint, true)

    CONTEXT_ITEM("Constraint_Lock", "Lock Constraint", "Sketcher_ConstrainLock", doLockConstraint,
                 true)
    CONTEXT_ITEM("Constraint_HorizontalDistance", "Horizontal Distance",
                 "Sketcher_ConstrainDistanceX", doHorizontalDistance, true)
    CONTEXT_ITEM("Constraint_VerticalDistance", "Vertical Distance", "Sketcher_ConstrainDistanceY",
                 doVerticalDistance, true)
    CONTEXT_ITEM("Constraint_Length", "Length Constraint", "Sketcher_ConstrainDistance",
                 doLengthConstraint, true)
    CONTEXT_ITEM("Constraint_Radius", "Radius Constraint", "Sketcher_ConstrainRadius",
                 doRadiusConstraint, true)
    CONTEXT_ITEM("Constraint_Diameter", "Diameter Constraint", "Sketcher_ConstrainDiameter",
                 doDiameterConstraint, true)
    CONTEXT_ITEM("Constraint_Radiam", "Radiam Constraint", "Sketcher_ConstrainRadiam",
                 doRadiamConstraint, true)
    CONTEXT_ITEM("Constraint_InternalAngle", "Angle Constraint", "Sketcher_ConstrainAngle",
                 doAngleConstraint, true)

    menu.addSeparator();

    CONTEXT_ITEM("Sketcher_ToggleConstruction", "Toggle construction line",
                 "Sketcher_ToggleConstruction", doToggleConstruction, true)

    menu.addSeparator();

    CONTEXT_ITEM("Sketcher_SelectConstraints", "Select Constraints", "Sketcher_SelectConstraints",
                 doSelectConstraints, true)
    CONTEXT_ITEM("Sketcher_SelectOrigin", "Select Origin", "Sketcher_SelectOrigin", doSelectOrigin,
                 false)
    CONTEXT_ITEM("Sketcher_SelectHorizontalAxis", "Select Horizontal Axis",
                 "Sketcher_SelectHorizontalAxis", doSelectHAxis, false)
    CONTEXT_ITEM("Sketcher_SelectVerticalAxis", "Select Vertical Axis",
                 "Sketcher_SelectVerticalAxis", doSelectVAxis, false)
...
void ElementView::deleteSelectedItems()
{
    ...
    for (std::vector<Gui::SelectionObject>::iterator ft = sel.begin(); ft != sel.end(); ++ft) {
        ...
        if (vp) { vp->onDelete(ft->getSubNames()); }
    }
...
void ElementView::keyPressEvent(QKeyEvent *event)
{
    switch (event->key()) {
        case Qt::Key_Z:
            ...
            break;
        default: QListWidget::keyPressEvent(event); break;
    }
}
...
TaskSketcherElements::TaskSketcherElements(ViewProviderSketch *sketchView)
    : TaskBox(Gui::BitmapFactory().pixmap("document-new"), tr("Elements"), true, nullptr),
      sketchView(sketchView), ui(new Ui_TaskSketcherElements()), focusItemIndex(-1),
      previouslySelectedItemIndex(-1), isNamingBoxChecked(false), isautoSwitchBoxChecked(false),
      inhibitSelectionUpdate(false)
{
    ...
    QObject::connect(ui->listWidgetElements, SIGNAL(itemSelectionChanged()), this,
                     SLOT(on_listWidgetElements_itemSelectionChanged()));
    QObject::connect(ui->listWidgetElements, SIGNAL(itemEntered(QListWidgetItem *)), this,
                     SLOT(on_listWidgetElements_itemEntered(QListWidgetItem *)));
    QObject::connect(ui->listWidgetElements, SIGNAL(onFilterShortcutPressed()), this,
                     SLOT(on_listWidgetElements_filterShortcutPressed()));
    QObject::connect(ui->comboBoxElementFilter, SIGNAL(currentIndexChanged(int)), this,
                     SLOT(on_listWidgetElements_currentFilterChanged(int)));
    QObject::connect(ui->comboBoxModeFilter, SIGNAL(currentIndexChanged(int)), this,
                     SLOT(on_listWidgetElements_currentModeFilterChanged(int)));
    QObject::connect(ui->namingBox, SIGNAL(stateChanged(int)), this,
                     SLOT(on_namingBox_stateChanged(int)));
    QObject::connect(ui->autoSwitchBox, SIGNAL(stateChanged(int)), this,
                     SLOT(on_autoSwitchBox_stateChanged(int)));
...
{
    std::string temp;
    if (msg.Type == Gui::SelectionChanges::ClrSelection) { clearWidget(); }
        ...
        if (strcmp(msg.pDocName, sketchView->getSketchObject()->getDocument()->getName()) == 0
            ...
            if (msg.pSubName) {
                ...
                if (shapetype.size() > 4 && shapetype.substr(0, 4) == "Edge") {
                    ...
                    if (pos > -1) {
                        ...
                        if (ok) {
                            ...
                            sketchView->getSketchObject()->getGeoVertexIndex(ElementId, GeoId,
                                                                             PosId);
                ...
                for (int i = 0; i < ui->listWidgetElements->count(); i++) {
                    ...
                    switch (element) {
                        case 0: ite->setSelected(ite->isLineSelected); break;
                        case 1: ite->setSelected(ite->isStartingPointSelected); break;
                        case 2: ite->setSelected(ite->isEndPointSelected); break;
                        case 3: ite->setSelected(ite->isMidPointSelected); break;
                    }
                }
...
void TaskSketcherElements::on_listWidgetElements_itemSelectionChanged(void)
{
    ...
    if (!inhibitSelectionUpdate) {
        if (itf) {
            switch (element) {
                case 0: itf->isLineSelected = !itf->isLineSelected; break;
                case 1: itf->isStartingPointSelected = !itf->isStartingPointSelected; break;
                case 2: itf->isEndPointSelected = !itf->isEndPointSelected; break;
                case 3: itf->isMidPointSelected = !itf->isMidPointSelected; break;
            }
        }
    ...
    for (int i = 0; i < ui->listWidgetElements->count(); i++) {
        ...
        if (multipleconsecutiveselection) {
            if (((i > focusItemIndex && i < previouslySelectedItemIndex)
                ...
                switch (element) {
                    case 0: ite->isLineSelected = true; break;
                    case 1: ite->isStartingPointSelected = true; break;
                    case 2: ite->isEndPointSelected = true; break;
                    case 3: ite->isMidPointSelected = true; break;
                }
            }
        }
...
void TaskSketcherElements::on_listWidgetElements_itemEntered(QListWidgetItem *item)
{
    ...
    switch (element) {
        ...
        case 2:
        case 3:
            vertex = sketchView->getSketchObject()->getVertexIndexGeoPos(
                it->ElementNbr, static_cast<Sketcher::PointPos>(element));
            if (vertex != -1) {
                ss << "Vertex" << vertex + 1;
                Gui::Selection().setPreselect(doc_name.c_str(), obj_name.c_str(), ss.str().c_str());
            }
...
void TaskSketcherElements::slotElementsChanged(void)
{
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = vals.begin(); it != vals.end();
        ...
        ui->listWidgetElements->addItem(new ElementItem(
            (type == Part::GeomPoint::getClassTypeId() && element == 1)
                ? Sketcher_Element_Point_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 0)
                ? Sketcher_Element_Line_Edge.getIcon(construction, false)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 1)
                ? Sketcher_Element_Line_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 2)
                ? Sketcher_Element_Line_EndPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 0)
                ? Sketcher_Element_Arc_Edge.getIcon(construction, false)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 1)
                ? Sketcher_Element_Arc_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 2)
                ? Sketcher_Element_Arc_EndPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 3)
                ? Sketcher_Element_Arc_MidPoint.getIcon(construction, false)
                : (type == Part::GeomCircle::getClassTypeId() && element == 0)
                ? Sketcher_Element_Circle_Edge.getIcon(construction, false)
                : (type == Part::GeomCircle::getClassTypeId() && element == 3)
                ? Sketcher_Element_Circle_MidPoint.getIcon(construction, false)
                : (type == Part::GeomEllipse::getClassTypeId() && element == 0)
                ? Sketcher_Element_Ellipse_Edge.getIcon(construction, false)
                : (type == Part::GeomEllipse::getClassTypeId() && element == 3)
                ? Sketcher_Element_Ellipse_MidPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfEllipse_Edge.getIcon(construction, false)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfEllipse_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfEllipse_EndPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfEllipse_MidPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfHyperbola_Edge.getIcon(construction, false)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfHyperbola_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfHyperbola_EndPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfHyperbola_MidPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfParabola_Edge.getIcon(construction, false)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfParabola_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfParabola_EndPoint.getIcon(construction, false)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfParabola_MidPoint.getIcon(construction, false)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 0)
                ? Sketcher_Element_BSpline_Edge.getIcon(construction, false)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 1)
                ? Sketcher_Element_BSpline_StartingPoint.getIcon(construction, false)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 2)
                ? Sketcher_Element_BSpline_EndPoint.getIcon(construction, false)
                : none.getIcon(construction, false),
            type == Part::GeomPoint::getClassTypeId()
                ? (isNamingBoxChecked
                       ? (tr("Point") + QString::fromLatin1("(Edge%1#ID%2)").arg(i).arg(i - 1))
                           + (construction ? (QString::fromLatin1("-") + tr("Construction"))
                                           : QString::fromLatin1(""))
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = ext_vals.begin(); it != ext_vals.end();
        ...
        if (j > 2) { // we do not want the H and V axes
            ...
            ui->listWidgetElements->addItem(new ElementItem(
                (type == Part::GeomPoint::getClassTypeId() && element == 1)
                    ? Sketcher_Element_Point_StartingPoint.External
                    : (type == Part::GeomLineSegment::getClassTypeId() && element == 0)
                    ? Sketcher_Element_Line_Edge.External
                    : (type == Part::GeomLineSegment::getClassTypeId() && element == 1)
                    ? Sketcher_Element_Line_StartingPoint.External
                    : (type == Part::GeomLineSegment::getClassTypeId() && element == 2)
                    ? Sketcher_Element_Line_EndPoint.External
                    : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 0)
                    ? Sketcher_Element_Arc_Edge.External
                    : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 1)
                    ? Sketcher_Element_Arc_StartingPoint.External
                    : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 2)
                    ? Sketcher_Element_Arc_EndPoint.External
                    : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 3)
                    ? Sketcher_Element_Arc_MidPoint.External
                    : (type == Part::GeomCircle::getClassTypeId() && element == 0)
                    ? Sketcher_Element_Circle_Edge.External
                    : (type == Part::GeomCircle::getClassTypeId() && element == 3)
                    ? Sketcher_Element_Circle_MidPoint.External
                    : (type == Part::GeomEllipse::getClassTypeId() && element == 0)
                    ? Sketcher_Element_Ellipse_Edge.External
                    : (type == Part::GeomEllipse::getClassTypeId() && element == 3)
                    ? Sketcher_Element_Ellipse_MidPoint.External
                    : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 0)
                    ? Sketcher_Element_ArcOfEllipse_Edge.External
                    : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 1)
                    ? Sketcher_Element_ArcOfEllipse_StartingPoint.External
                    : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 2)
                    ? Sketcher_Element_ArcOfEllipse_EndPoint.External
                    : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 3)
                    ? Sketcher_Element_ArcOfEllipse_MidPoint.External
                    : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 0)
                    ? Sketcher_Element_ArcOfHyperbola_Edge.External
                    : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 1)
                    ? Sketcher_Element_ArcOfHyperbola_StartingPoint.External
                    : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 2)
                    ? Sketcher_Element_ArcOfHyperbola_EndPoint.External
                    : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 3)
                    ? Sketcher_Element_ArcOfHyperbola_MidPoint.External
                    : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 0)
                    ? Sketcher_Element_ArcOfParabola_Edge.External
                    : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 1)
                    ? Sketcher_Element_ArcOfParabola_StartingPoint.External
                    : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 2)
                    ? Sketcher_Element_ArcOfParabola_EndPoint.External
                    : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 3)
                    ? Sketcher_Element_ArcOfParabola_MidPoint.External
                    : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 0)
                    ? Sketcher_Element_BSpline_Edge.External
                    : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 1)
                    ? Sketcher_Element_BSpline_StartingPoint.External
                    : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 2)
                    ? Sketcher_Element_BSpline_EndPoint.External
                    : none.External,
                type == Part::GeomPoint::getClassTypeId()
...
void TaskSketcherElements::on_listWidgetElements_filterShortcutPressed()
{
    ...
    if (focusItemIndex > -1 && focusItemIndex < ui->listWidgetElements->count()) {
        ...
        switch (element) {
            ...
            case 0: // Edge
                element = (type == Part::GeomCircle::getClassTypeId()
                           || type == Part::GeomEllipse::getClassTypeId())
                    ? 3
                    : 1;
                break;
            case 1: // StartingPoint
                element = (type == Part::GeomCircle::getClassTypeId()
                           || type == Part::GeomEllipse::getClassTypeId())
                    ? 3
                    : (type == Part::GeomPoint::getClassTypeId()) ? 1
                                                                  : 2;
                break;
            case 2: // EndPoint
        ...
        element =
            (ui->comboBoxElementFilter->currentIndex() + 1) % ui->comboBoxElementFilter->count();
...
void TaskSketcherElements::setItemVisibility(int elementindex, int filterindex)
{
    ...
    if (filterindex == 0) item->setHidden(false);
    else {
...
{
    for (int i = 0; i < ui->listWidgetElements->count(); i++) { setItemVisibility(i, filterindex); }
}
...
void TaskSketcherElements::updateIcons(int element)
{
    ...
    for (int i = 0; i < ui->listWidgetElements->count(); i++) {
        ...
        bool construction =
            static_cast<ElementItem *>(ui->listWidgetElements->item(i))->isConstruction;
        bool external = static_cast<ElementItem *>(ui->listWidgetElements->item(i))->isExternal;

        ui->listWidgetElements->item(i)->setIcon(
            (type == Part::GeomPoint::getClassTypeId() && element == 1)
                ? Sketcher_Element_Point_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 0)
                ? Sketcher_Element_Line_Edge.getIcon(construction, external)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 1)
                ? Sketcher_Element_Line_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomLineSegment::getClassTypeId() && element == 2)
                ? Sketcher_Element_Line_EndPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 0)
                ? Sketcher_Element_Arc_Edge.getIcon(construction, external)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 1)
                ? Sketcher_Element_Arc_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 2)
                ? Sketcher_Element_Arc_EndPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfCircle::getClassTypeId() && element == 3)
                ? Sketcher_Element_Arc_MidPoint.getIcon(construction, external)
                : (type == Part::GeomCircle::getClassTypeId() && element == 0)
                ? Sketcher_Element_Circle_Edge.getIcon(construction, external)
                : (type == Part::GeomCircle::getClassTypeId() && element == 3)
                ? Sketcher_Element_Circle_MidPoint.getIcon(construction, external)
                : (type == Part::GeomEllipse::getClassTypeId() && element == 0)
                ? Sketcher_Element_Ellipse_Edge.getIcon(construction, external)
                : (type == Part::GeomEllipse::getClassTypeId() && element == 3)
                ? Sketcher_Element_Ellipse_MidPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfEllipse_Edge.getIcon(construction, external)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfEllipse_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfEllipse_EndPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfEllipse::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfEllipse_MidPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfHyperbola_Edge.getIcon(construction, external)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfHyperbola_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfHyperbola_EndPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfHyperbola::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfHyperbola_MidPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 0)
                ? Sketcher_Element_ArcOfParabola_Edge.getIcon(construction, external)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 1)
                ? Sketcher_Element_ArcOfParabola_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 2)
                ? Sketcher_Element_ArcOfParabola_EndPoint.getIcon(construction, external)
                : (type == Part::GeomArcOfParabola::getClassTypeId() && element == 3)
                ? Sketcher_Element_ArcOfParabola_MidPoint.getIcon(construction, external)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 0)
                ? Sketcher_Element_BSpline_Edge.getIcon(construction, external)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 1)
                ? Sketcher_Element_BSpline_StartingPoint.getIcon(construction, external)
                : (type == Part::GeomBSplineCurve::getClassTypeId() && element == 2)
                ? Sketcher_Element_BSpline_EndPoint.getIcon(construction, external)

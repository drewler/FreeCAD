    ...
    : TaskDialog(), sketchView(sketchView)
{
    ...
    if (hGrp->GetBool("ShowSolverAdvancedWidget", false)) { Content.push_back(SolverAdvanced); }

    Content.push_back(General);
    Content.push_back(Constraints);
    Content.push_back(Elements);

    if (!hGrp->GetBool("ExpandedMessagesWidget", true)) Messages->hideGroupBox();
    if (!hGrp->GetBool("ExpandedSolverAdvancedWidget", false)) SolverAdvanced->hideGroupBox();
    if (!hGrp->GetBool("ExpandedEditControlWidget", false)) General->hideGroupBox();
    if (!hGrp->GetBool("ExpandedConstraintsWidget", true)) Constraints->hideGroupBox();
    if (!hGrp->GetBool("ExpandedElementsWidget", true)) Elements->hideGroupBox();
}
...
TaskDlgEditSketch::~TaskDlgEditSketch()
{
    ...
    if (it == Content.end()) Content.push_back(SolverAdvanced);
}
...
bool TaskDlgEditSketch::accept() { return true; }
...
bool TaskDlgEditSketch::reject()
{
    ...
    Gui::Command::doCommand(Gui::Command::Gui, "Gui.getDocument('%s').resetEdit()",
                            document.c_str());
    Gui::Command::doCommand(Gui::Command::Doc, "App.getDocument('%s').recompute()",
                            document.c_str());

...
ViewProviderSketch::ParameterObserver::~ParameterObserver() { unsubscribeToParameters(); }
...
void ViewProviderSketch::ParameterObserver::subscribeToParameters()
{
    try {
        ...
        const Base::ValueError &
            e) { // ensure that if parameter strings are not well-formed, the exception is not propagated
...
void ViewProviderSketch::ParameterObserver::initParameters()
{
    ...
    parameterMap = {
        ...
        {"GridSize",
         {[this](const std::string &string, App::Property *property) {
              updateGridSize(string, property);
          },
          &Client.GridSize}},
        {"SketchEdgeColor",
         {[this](const std::string &string, App::Property *property) {
              updateColorProperty(string, property, 1.f, 1.f, 1.f);
          },
          &Client.LineColor}},
        {"SketchVertexColor",
         {[this](const std::string &string, App::Property *property) {
...
ViewProviderSketch::ViewProviderSketch()
    : SelectionObserver(false), Mode(STATUS_NONE), listener(nullptr), editCoinManager(nullptr),
      pObserver(std::make_unique<ViewProviderSketch::ParameterObserver>(*this)),
      sketchHandler(nullptr), viewOrientationFactor(1)
{
    PartGui::ViewProviderAttachExtension::initExtension(this);

    ADD_PROPERTY_TYPE(Autoconstraints, (true), "Auto Constraints",
                      (App::PropertyType)(App::Prop_None), "Create auto constraints");
    ADD_PROPERTY_TYPE(AvoidRedundant, (true), "Auto Constraints",
                      (App::PropertyType)(App::Prop_None), "Avoid redundant autoconstraint");
    ADD_PROPERTY_TYPE(
        TempoVis, (Py::None()), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "Object that handles hiding and showing other objects when entering/leaving sketch.");
    ADD_PROPERTY_TYPE(
        HideDependent, (true), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "If true, all objects that depend on the sketch are hidden when opening editing.");
    ADD_PROPERTY_TYPE(
        ShowLinks, (true), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "If true, all objects used in links to external geometry are shown when opening sketch.");
    ADD_PROPERTY_TYPE(
        ShowSupport, (true), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "If true, all objects this sketch is attached to are shown when opening sketch.");
    ADD_PROPERTY_TYPE(RestoreCamera, (true), "Visibility automation",
                      (App::PropertyType)(App::Prop_ReadOnly),
                      "If true, camera position before entering sketch is remembered, and restored "
                      "after closing it.");
    ADD_PROPERTY_TYPE(
        ForceOrtho, (false), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "If true, camera type will be forced to orthographic view when entering editing mode.");
    ADD_PROPERTY_TYPE(
        SectionView, (false), "Visibility automation", (App::PropertyType)(App::Prop_ReadOnly),
        "If true, only objects (or part of) located behind the sketch plane are visible.");
    ADD_PROPERTY_TYPE(EditingWorkbench, ("SketcherWorkbench"), "Visibility automation",
                      (App::PropertyType)(App::Prop_ReadOnly),
    ...
    switch (key) {
        case SoKeyboardEvent::ESCAPE: {
            ...
            if (isInEditMode() && sketchHandler) {
                if (!pressed) sketchHandler->quit();
                return true;
            }
            if (isInEditMode() && !drag.DragConstraintSet.empty()) {
                if (!pressed) { drag.DragConstraintSet.clear(); }
                return true;
            }
            if (isInEditMode() && drag.isDragCurveValid()) {
                if (!pressed) {
                    getSketchObject()->movePoint(drag.DragCurve, Sketcher::PointPos::none,
                                                 Base::Vector3d(0, 0, 0), true);
            ...
            if (isInEditMode()) {
                ...
                if (!pressed && !viewProviderParameters.buttonPress) return true;
        ...
        default: {
            if (isInEditMode() && sketchHandler) sketchHandler->registerPressedKey(pressed, key);
        }
    }
...
void ViewProviderSketch::snapToGrid(double &x, double &y)
{
    if (GridSnap.getValue() && ShowGrid.getValue()) {
        ...
        if (x < tmpX + snapTol && x > tmpX - snapTol) x = tmpX; // Snap X Mouse Position

        // Check if y within snap tolerance
        if (y < tmpY + snapTol && y > tmpY - snapTol) y = tmpY; // Snap Y Mouse Position
    }
}
...
Base::Placement ViewProviderSketch::getEditingPlacement() const
{
    ...
    if (!doc || doc->getInEdit() != this) return getSketchObject()->globalPlacement();
                ...
                case STATUS_NONE: {
                    ...
                    if (done
                        ...
                        DoubleClick::prvClickPos = SbVec2s(
                            -16000,
                            -16000); //certainly far away from any clickable place, to avoid re-trigger of double-click if next click happens fast.
                        ...
                        if (!done) Mode = STATUS_SKETCH_StartRubberBand;
                    }
            ...
            switch (Mode) {
                case STATUS_SELECT_Point:
                    if (pp) {
                        ...
                        if (isSelected(ss.str())) { rmvSelection(ss.str()); }
                        else {
                            addSelection2(ss.str(), pp->getPoint()[0], pp->getPoint()[1],
                                          pp->getPoint()[2]);
                            drag.resetIds();
                        }
                    }
                        ...
                        if (GeoId != Sketcher::GeoEnum::GeoUndef
                            ...
                            try {
                                Gui::cmdAppObjectArgs(
                                    getObject(), "movePoint(%i,%i,App.Vector(%f,%f,0),%i)", GeoId,
                                    static_cast<int>(PosId), x - drag.xInit, y - drag.yInit, 0);
                ...
                case STATUS_SKETCH_DragCurve:
                    if (drag.isDragCurveValid()) {
                        ...
                        if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                            ...
                            try {
                                Gui::cmdAppObjectArgs(
                                    getObject(), "movePoint(%i,%i,App.Vector(%f,%f,0),%i)",
                                    drag.DragCurve, static_cast<int>(Sketcher::PointPos::none),
                                    vec.x, vec.y, drag.relative ? 1 : 0);
        ...
        if (!pressed) {
            switch (Mode) {
                ...
                case STATUS_NONE: {
                    ...
                    if (preselection.isPreselectPointValid()) { return true; }
                    else if (preselection.isPreselectCurveValid()) {
                        return true;
                    }
                    else if (preselection.PreselectConstraintSet.empty() != true) {
                ...
                case STATUS_SELECT_Point: break;
                case STATUS_SELECT_Edge: {
                    Gui::MenuItem geom;
                ...
                case STATUS_SKETCH_UseRubberBand: break;
            }
        }
    }
    ...
    switch (Mode) {
        ...
        case STATUS_SELECT_Edge:
            if (!getSolvedSketch().hasConflicts() && preselection.isPreselectCurveValid()
                ...
                if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                    ...
                    getCoordsOnSketchPlane(line2.getPosition(), line2.getDirection(), drag.xInit,
                                           drag.yInit);
                ...
                if (geo->getTypeId() == Part::GeomBSplineCurve::getClassTypeId()) {
                    ...
                    getSketchObject()->initTemporaryMove(drag.DragCurve, Sketcher::PointPos::none,
                                                         false);
                }
            }
            else {
        ...
        case STATUS_SKETCH_DragCurve:
            if (drag.isDragCurveValid()) {
                ...
                if (getSketchObject()->moveTemporaryPoint(drag.DragCurve, Sketcher::PointPos::none,
                                                          vec, drag.relative)
                    == 0) {
                    setPositionText(Base::Vector2d(x, y));
                    draw(true, false);
        ...
        case STATUS_SKETCH_DragConstraint:
            if (!drag.DragConstraintSet.empty()) {
                auto idset = drag.DragConstraintSet;
                for (int id : idset) moveConstraint(id, Base::Vector2d(x, y));
            }
            return true;
        case STATUS_SKETCH_UseHandler:
    ...
    if (Constr->Type == Distance || Constr->Type == DistanceX || Constr->Type == DistanceY
        ...
        if (Constr->SecondPos != Sketcher::PointPos::none) { // point to point distance
            ...
            if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                if (Constr->Type == Sketcher::Diameter) p1 = center - dir;
            ...
            if (strcmp(msg.pDocName, getSketchObject()->getDocument()->getName()) == 0
                ...
                if (msg.pSubName) {
                    ...
                    if (shapetype.size() > 4 && shapetype.substr(0, 4) == "Edge") {
                        ...
                        if (sketchHandler) sketchHandler->applyCursor();
                        this->updateColor();
                    }
                    else if (shapetype.size() > 6 && shapetype.substr(0, 6) == "Vertex") {
...
bool ViewProviderSketch::detectAndShowPreselection(SoPickedPoint *Point, const SbVec2s &cursorPos)
{
    ...
    if (Point) {
        ...
        if (result.PointIndex != -1
            ...
            std::stringstream ss;
            if (result.GeoIndex >= 0) ss << "Edge" << result.GeoIndex + 1;
            else // external geometry
                ss << "ExternalEdge"
                                        ...
                                        const Gui::View3DInventorViewer *viewer)
{
    ...
    if (corners[0].getValue()[0] > corners[1].getValue()[0]) touchMode = true;

    for (std::vector<Part::Geometry *>::const_iterator it = geomlist.begin();
         it != geomlist.end() - 2; ++it, ++GeoId) {

        if (GeoId >= intGeoCount) GeoId = -extGeoCount;
        ...
        if ((*it)->getTypeId() == Part::GeomPoint::getClassTypeId()) {
            ...
            if (polygon.Contains(Base::Vector2d(pnt0.x, pnt0.y)) || touchMode) {
                ...
                for (int i = 0; i < countSegments; ++i, angle += segment) {
                    pnt = Base::Vector3d(pnt0.x + radius * cos(angle), pnt0.y + radius * sin(angle),
                                         0.f);
                    Plm.multVec(pnt, pnt);
                    pnt = proj(pnt);
            ...
            const Part::GeomArcOfHyperbola *aoh =
                static_cast<const Part::GeomArcOfHyperbola *>(*it);
            ...
            if ((pnt0Inside && pnt1Inside) || touchMode) {
                ...
                for (int i = 0; i < countSegments; ++i, angle += segment) {
                    pnt = Base::Vector3d(
                        ...
                        pnt0.y + a * cosh(angle) * sin(phi) + b * sinh(angle) * cos(phi), 0.f);
            ...
            if ((pnt0Inside && pnt1Inside) || touchMode) {
                ...
                for (int i = 0; i < countSegments; ++i, angle += segment) {
                    pnt = Base::Vector3d(
                        ...
                        pnt0.y + angle * angle / 4 / focal * sin(phi) + angle * cos(phi), 0.f);
    ...
    GeoListFacade &geolistfacade, bool geometrywithmemoryallocation)
{
    ...
    for (auto it = tempGeo.begin(); it != tempGeo.end() - 2; ++it, GeoId++) {
        if (GeoId >= geolistfacade.getInternalCount()) GeoId = -geolistfacade.getExternalCount();
                    ...
                    if (c->Type == InternalAlignment && c->AlignmentType == BSplineControlPoint
                        ...
                        if (bspline) {
                            ...
                            if (!bspline->isRational()) {
                                ...
                                for (auto ic : getSketchObject()->Constraints.getValues()) {
                                    if (ic->Type == Weight) {
                                        auto pos = std::find(polegeoids.begin(), polegeoids.end(),
                                                             ic->First);
                            ...
                            if (geometrywithmemoryallocation) { // with memory allocation
                                ...
                                tempGeo[GeoId] = GeometryFacade::getFacade(
                                    tmpcircle,
                                    true); // this is the circle that will be drawn, with the updated vradius, the facade takes ownership and will deallocate.
                            }
...
void ViewProviderSketch::updateData(const App::Property *prop)
{
    ...
    if (isInEditMode() && !getSketchObject()->getDocument()->isPerformingTransaction()
        ...
        if (getSketchObject()->getExternalGeometryCount()
            ...
            Gui::MDIView *mdi = Gui::Application::Instance->editDocument()->getActiveView();
            if (mdi->isDerivedFrom(Gui::View3DInventor::getClassTypeId())) draw(false, true);

            signalConstraintsChanged();
        }

        if (prop != &getSketchObject()->Constraints) signalElementsChanged();
    }
}
...
void ViewProviderSketch::attach(App::DocumentObject *pcFeat) { ViewProviderPart::attach(pcFeat); }
...
bool ViewProviderSketch::setEdit(int ModNum)
{
    ...
    if (editDoc) {
        ...
        if (editVp) editObj = editVp->getObject();
    }
    ...
    try {
        ...
        Base::Console().Warning("ViewProviderSketch::setEdit: could not import Show module. "
                                "Visibility automation will not work.\n");
    }

    TightGrid.setValue(false);

    ViewProvider2DObjectGrid::setEdit(
        ModNum); // notify to handle grid according to edit mode property

    // start the edit dialog
    if (sketchDlg) Gui::Control().showDialog(sketchDlg);
    else
    ...
    if (getSketchObject()->Support.getValue()) {
        if (!getSketchObject()->evaluateSupport()) getSketchObject()->validateExternalLinks();
    }
...
QString ViewProviderSketch::appendRedundantMsg(const std::vector<int> &redundant)
{
    return appendConstraintMsg(tr("Please remove the following redundant constraint:"),
                               tr("Please remove the following redundant constraints:"), redundant);
}
...
QString ViewProviderSketch::appendMalformedMsg(const std::vector<int> &malformed)
{
    return appendConstraintMsg(tr("Please remove the following malformed constraint:"),
                               tr("Please remove the following malformed constraints:"), malformed);
}
                                                ...
                                                const std::vector<int> &vector)
{
    ...
    QTextStream ss(&msg);
    if (!vector.empty()) {
        if (vector.size() == 1) ss << singularmsg;
        else
            ss << pluralmsg;
        ss << "\n";
        ss << vector[0];
...
inline QString intListHelper(const std::vector<int> &ints)
{
    ...
    if (ints.size() < 8) { // The 8 is a bit heuristic... more than that and we shift formats
        for (const auto i : ints) {
            if (results.isEmpty()) results.append(QString::fromUtf8("%1").arg(i));
            else
...
void ViewProviderSketch::UpdateSolverInformation()
{
    ...
    if (getSketchObject()->Geometry.getSize() == 0) {
        ...
        signalSetUp(
            QString::fromUtf8("conflicting_constraints"), tr("Over-constrained: "),
        ...
        signalSetUp(QString::fromUtf8("malformed_constraints"), tr("Malformed constraints: "),
        ...
        signalSetUp(
            QString::fromUtf8("redundant_constraints"), tr("Redundant constraints:"),
        ...
        signalSetUp(QString::fromUtf8("partially_redundant_constraints"),
                    tr("Partially redundant:"), QString::fromUtf8("#partiallyredundant"),
                    QString::fromUtf8("(%1)").arg(
                        intListHelper(getSketchObject()->getLastPartiallyRedundant())));
    }
    else if (getSketchObject()->getLastSolverStatus() != 0) {
        signalSetUp(QString::fromUtf8("solver_failed"), tr("Solver failed to converge"),
                    QString::fromUtf8(""), QString::fromUtf8(""));
    }
    else if (dofs > 0) {
        signalSetUp(QString::fromUtf8("under_constrained"), tr("Under constrained:"),
                    QString::fromUtf8("#dofs"), tr("%n DoF(s)", "", dofs));
    }
    else {
        signalSetUp(QString::fromUtf8("fully_constrained"), tr("Fully constrained"), QString(),
                    QString());
    }
}
...
void ViewProviderSketch::unsetEdit(int ModNum)
{
    ...
    if (isInEditMode()) {
        if (sketchHandler) deactivateHandler();
    ...
    try {
        ...
        Base::Console().Error(
            "ViewProviderSketch::unsetEdit: visibility automation failed with an error: \n");
        e.ReportException();
    }
...
void ViewProviderSketch::camSensCB(void *data, SoSensor *)
{
    ...
    auto rotCam =
        Base::Rotation(rotc[0], rotc[1], rotc[2],
...
{
    if (isInEditMode()) return preselection.PreselectPoint;
    return -1;
}

int ViewProviderSketch::getPreselectCurve() const
{
    if (isInEditMode()) return preselection.PreselectCurve;
    return -1;
}
...
int ViewProviderSketch::getPreselectCross() const
{
    ...
    if (isInEditMode()) return static_cast<int>(preselection.PreselectCross);
    return -1;
}
...
bool ViewProviderSketch::onDelete(const std::vector<std::string> &subList)
{
    if (isInEditMode()) {
        ...
        for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
            ...
            if (it->size() > 4 && it->substr(0, 4) == "Edge") {
                int GeoId = std::atoi(it->substr(4, 4000).c_str()) - 1;
                if (GeoId >= 0) { delInternalGeometries.insert(GeoId); }
                else
            ...
            if (GeoId != GeoEnum::GeoUndef) {
                for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
                    ...
                    if (((*it)->Type == Sketcher::Coincident)
                        ...
                        try {
                            Gui::cmdAppObjectArgs(getObject(), "delConstraintOnPoint(%i,%i)", GeoId,
                                                  (int)PosId);
                        }
                        catch (const Base::Exception &e) {
                            Base::Console().Error("%s\n", e.what());
        ...
        if (viewProviderParameters.autoRecompute) { Gui::Command::updateActive(); }
        else {
            editCoinManager->drawConstraintIcons();
            this->updateColor();
        }
...
QIcon ViewProviderSketch::mergeColorfulOverlayIcons(const QIcon &orig) const
{
    ...
    if (!getSketchObject()->FullyConstrained.getValue()) {
        ...
        static const char *const sketcher_notfullyconstrained_xpm[] = {
            "9 9 3 1",   ". c None",  "# c #dbaf00", "a c #ffcc00", "##.....##",
...
void ViewProviderSketch::resetPositionText() { editCoinManager->resetPositionText(); }
...
void ViewProviderSketch::clearSelectPoints() { selection.SelPointSet.clear(); }
...
{
    return Gui::Selection().addSelection(editDocName.c_str(), editObjName.c_str(),
                                         (editSubName + subNameSuffix).c_str(), x, y, z);
}

bool ViewProviderSketch::addSelection2(const std::string &subNameSuffix, float x, float y, float z)
{
    return Gui::Selection().addSelection2(editDocName.c_str(), editObjName.c_str(),
                                          (editSubName + subNameSuffix).c_str(), x, y, z);
}
...
std::unique_ptr<SoRayPickAction> ViewProviderSketch::getRayPickAction() const
{
    ...
    Gui::MDIView *mdi =
        Gui::Application::Instance->editViewOfNode(editCoinManager->getRootEditNode());
    if (!(mdi && mdi->isDerivedFrom(Gui::View3DInventor::getClassTypeId()))) return nullptr;
    Gui::View3DInventorViewer *viewer = static_cast<Gui::View3DInventor *>(mdi)->getViewer();
...
SbVec2f ViewProviderSketch::getScreenCoordinates(SbVec2f sketchcoordinates) const
{
    ...
    Gui::MDIView *mdi = this->getActiveView();
    Gui::View3DInventor *view = qobject_cast<Gui::View3DInventor *>(mdi);
    if (!view || !isInEditMode()) return SbVec2f(0, 0);

    Gui::View3DInventorViewer *viewer = view->getViewer();

    SoCamera *pCam = viewer->getSoRenderManager()->getCamera();

    if (!pCam) return SbVec2f(0, 0);
...
QFont ViewProviderSketch::getApplicationFont() const { return QApplication::font(); }
...
int ViewProviderSketch::getViewOrientationFactor() const { return viewOrientationFactor; }
...
double ViewProviderSketch::getRotation(SbVec3f pos0, SbVec3f pos1) const
{
    ...
    Gui::MDIView *mdi =
        Gui::Application::Instance->editViewOfNode(editCoinManager->getRootEditNode());
    if (!(mdi && mdi->isDerivedFrom(Gui::View3DInventor::getClassTypeId()))) return 0;
    Gui::View3DInventorViewer *viewer = static_cast<Gui::View3DInventor *>(mdi)->getViewer();
    SoCamera *pCam = viewer->getSoRenderManager()->getCamera();
    if (!pCam) return 0;
...
void ViewProviderSketch::addNodeToRoot(SoSeparator *node) { pcRoot->addChild(node); }

void ViewProviderSketch::removeNodeFromRoot(SoSeparator *node) { pcRoot->removeChild(node); }
...
{
    for (const auto v : selection.SelPointSet) operation(v);
}

bool ViewProviderSketch::isInEditMode() const { return editCoinManager != nullptr; }

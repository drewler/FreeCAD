...
public:
    DrawSketchHandlerLineSet()
        : Mode(STATUS_SEEK_First), SegmentMode(SEGMENT_MODE_Line),
          TransitionMode(TRANSITION_MODE_Free), SnapMode(SNAP_MODE_Free), suppressTransition(false),
          EditCurve(2), firstCurve(-1), previousCurve(-1), firstPosId(Sketcher::PointPos::none),
          previousPosId(Sketcher::PointPos::none), startAngle(0), endAngle(0), arcRadius(0),
          firstsegment(true)
    {}
    ...
    void registerPressedKey(bool pressed, int key) override
    {
        ...
        if (key == SoKeyboardEvent::M && pressed && previousCurve != -1) {
            ...
            if (SegmentMode == SEGMENT_MODE_Line) onSketchPos = EditCurve[EditCurve.size() - 1];
            else
        ...
        if (Mode == STATUS_SEEK_First) {
            ...
            if (SegmentMode == SEGMENT_MODE_Line) {
                ...
                if ((x2 * y3 - x3 * y2) - (x1 * y3 - x3 * y1) + (x1 * y2 - x2 * y1) > 0)
                    arcRadius *= -1;
                if (boost::math::isnan(arcRadius) || boost::math::isinf(arcRadius)) arcRadius = 0.f;
                ...
                if (boost::math::isnan(arcAngle) || boost::math::isinf(arcAngle)) arcAngle = 0.f;
                if (arcRadius >= 0 && arcAngle > 0) arcAngle -= 2 * M_PI;
                if (arcRadius < 0 && arcAngle < 0) arcAngle += 2 * M_PI;
    ...
    bool pressButton(Base::Vector2d onSketchPos) override
    {
        if (Mode == STATUS_SEEK_First) {
            ...
            if (getPreselectPoint() != -1 && firstPosId != Sketcher::PointPos::none) {
                ...
                if (sketchgui->getSketchObject()->arePointsCoincident(GeoId, PosId, firstCurve,
                                                                      firstPosId))
                    Mode = STATUS_Close;
            }
            else if (getPreselectCross() == 0 && firstPosId != Sketcher::PointPos::none) {
                // close line started at root point
                if (sketchgui->getSketchObject()->arePointsCoincident(-1, Sketcher::PointPos::start,
        ...
        if (Mode == STATUS_Do || Mode == STATUS_Close) {
            ...
            if (addedGeometry && (previousPosId != Sketcher::PointPos::none)) {
                ...
                if (!suppressTransition && previousCurve != -1) {
                    if (TransitionMode == TRANSITION_MODE_Tangent) constrType = "Tangent";
                    else if (TransitionMode == TRANSITION_MODE_Perpendicular_L
                             || TransitionMode == TRANSITION_MODE_Perpendicular_R)
                        constrType = "Perpendicular";
                }
                Gui::cmdAppObjectArgs(
                    sketchgui->getObject(), "addConstraint(Sketcher.Constraint('%s',%i,%i,%i,%i)) ",
                    constrType.c_str(), previousCurve, static_cast<int>(previousPosId), lastCurve,
                ...
                if (Mode == STATUS_Close) {
                    ...
                    Gui::cmdAppObjectArgs(
                        sketchgui->getObject(),
                        "addConstraint(Sketcher.Constraint('Coincident',%i,%i,%i,%i)) ", lastCurve,
                        static_cast<int>(lastEndPosId), firstCurve, static_cast<int>(firstPosId));
                }
            ...
            if (Mode == STATUS_Close) {
                ...
                if (!sugConstr2.empty()) {
                    ...
                    createAutoConstraints(sugConstr, getHighestCurveIndex(),
                                          Sketcher::PointPos::end);
                    sugConstr2.clear();
                }
                ...
                if (!sugConstr1.empty()) { // this is relevant only to the very first point
                    createAutoConstraints(sugConstr1, getHighestCurveIndex(),
                                          Sketcher::PointPos::start);
                    sugConstr1.clear();
                }
    ...
    void quit() override
    {
        ...
        if (firstsegment) {
            ...
            if (!continuousMode) { DrawSketchHandler::quit(); }
            else {
    ...
    void updateTransitionData(int GeoId, Sketcher::PointPos PosId)
    {
        ...
        if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
            ...
            dirVec.Set(lineSeg->getEndPoint().x - lineSeg->getStartPoint().x,
                       lineSeg->getEndPoint().y - lineSeg->getStartPoint().y, 0.f);

...
EditModeCoinManager::ParameterObserver::~ParameterObserver() { unsubscribeToParameters(); }
...
void EditModeCoinManager::ParameterObserver::initParameters()
{
    ...
    str2updatefunction = {
        ...
        {"MarkerSize", [this](const std::string &param) { updateElementSizeParameters(param); }},
...
EditModeCoinManager::EditModeCoinManager(ViewProviderSketch &vp) : viewProvider(vp)
{
    ...
    pEditModeConstraintCoinManager = std::make_unique<EditModeConstraintCoinManager>(
        viewProvider, drawingParameters, geometryLayerParameters, constraintParameters,
        editModeScenegraphNodes, coinMapping);

    pEditModeGeometryCoinManager = std::make_unique<EditModeGeometryCoinManager>(
        viewProvider, drawingParameters, geometryLayerParameters, analysisResults,
        editModeScenegraphNodes, coinMapping);
...
void EditModeCoinManager::drawEdit(const std::list<std::vector<Base::Vector2d>> &list)
{
    ...
    for (const auto &v : list) ncoords += v.size();
...
void EditModeCoinManager::resetPositionText() { editModeScenegraphNodes.textX->string = ""; }

void EditModeCoinManager::setAxisPickStyle(bool on)
{
    if (on) editModeScenegraphNodes.pickStyleAxes->style = SoPickStyle::SHAPE;
    else
...
EditModeCoinManager::detectPreselection(SoPickedPoint *Point, const SbVec2s &cursorPos)
{
    ...
    for (int l = 0; l < geometryLayerParameters.CoinLayers; l++) {
        ...
        if (tail == editModeScenegraphNodes.PointSet[l]) {
            ...
            if (point_detail && point_detail->getTypeId() == SoPointDetail::getClassTypeId()) {
                ...
                if (result.PointIndex == -1) result.Cross = PreselectionResult::Axes::RootPoint;
    ...
    if (tail == editModeScenegraphNodes.RootCrossSet) {
        ...
        if (cross_detail && cross_detail->getTypeId() == SoLineDetail::getClassTypeId()) {
            ...
            if (CrossIndex == 0) result.Cross = PreselectionResult::Axes::HorizontalAxis;
...
void EditModeCoinManager::updateColor(const GeoListFacade &geolistfacade)
{
    ...
    if (ViewProviderSketchCoinAttorney::haveConstraintsInvalidGeometry(viewProvider)) return;
...
void EditModeCoinManager::createEditModeInventorNodes()
{
    ...
    cursorTextColor.setPackedValue(
        (uint32_t)hGrp->GetUnsigned("CursorTextColor", cursorTextColor.getPackedValue()),
        transparency);
...
void EditModeCoinManager::redrawViewProvider() { viewProvider.draw(false, false); }
...
SoSeparator *EditModeCoinManager::getRootEditNode() { return editModeScenegraphNodes.EditRoot; }

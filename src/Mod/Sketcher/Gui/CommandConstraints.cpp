                                                  ...
                                                  int geoId2)
{
    ...
    try {
        ...
        Gui::cmdAppObjectArgs(
            Obj, "addConstraint(Sketcher.Constraint('TangentViaPoint',%d,%d,%d,%d))", geoId1,
            geoId2, GeoIdPoint, static_cast<int>(Sketcher::PointPos::start));
    }
                                     ...
                                     PointPos PosId1, PointPos PosId2)
{
    ...
    Gui::cmdAppObjectArgs(Obj, "addConstraint(Sketcher.Constraint('Tangent',%d,%d,%d,%d)) ", GeoId1,
...
void SketcherGui::doEndpointToEdgeTangency(Sketcher::SketchObject *Obj, int GeoId1, PointPos PosId1,
                                           int GeoId2)
{
    Gui::cmdAppObjectArgs(Obj, "addConstraint(Sketcher.Constraint('Tangent',%d,%d,%d)) ", GeoId1,
                          static_cast<int>(PosId1), GeoId2);
}

void SketcherGui::notifyConstraintSubstitutions(const QString &message)
{
    Gui::Dialog::DlgCheckableMessageBox::showMessage(
        QObject::tr("Sketcher Constraint Substitution"), message,
...
public:
    explicit GenericConstraintSelection(App::DocumentObject *obj)
        : Gui::SelectionFilterGate(nullPointer()), object(obj), allowedSelTypes(0)
    {}
...
public:
    explicit CmdSketcherConstraint(const char *name) : Command(name) {}

    ~CmdSketcherConstraint() override {}

    const char *className() const override { return "CmdSketcherConstraint"; }
...
protected:
    ...
    bool isActive() override { return isCommandActive(getActiveGuiDocument()); }
};

class DrawSketchHandlerGenConstraint: public DrawSketchHandler
{
public:
    explicit DrawSketchHandlerGenConstraint(CmdSketcherConstraint *_cmd) : cmd(_cmd), seqIndex(0) {}
    ~DrawSketchHandlerGenConstraint() override { Gui::Selection().rmvSelectionGate(); }

    void mouseMove(Base::Vector2d /*onSketchPos*/) override {}

    bool pressButton(Base::Vector2d /*onSketchPos*/) override { return true; }
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainHorizontal"; }
...
void CmdSketcherConstrainHorizontal::activated(int iMsg)
{
        ...
        if (isEdge(GeoId, PosId)) { // it is an edge
            ...
            for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin();
                ...
                if ((*it)->Type == Sketcher::Horizontal && (*it)->First == GeoId
                    ...
                    QMessageBox::warning(
                        Gui::getMainWindow(), QObject::tr("Double constraint"),
                ...
                if ((*it)->Type == Sketcher::Vertical && (*it)->First == GeoId
                    && (*it)->FirstPos == Sketcher::PointPos::none) {
                    QMessageBox::warning(
                        Gui::getMainWindow(), QObject::tr("Impossible constraint"),
            ...
            if (isPointOrSegmentFixed(Obj, GeoId)) fixedpoints++;
    ...
    if (!edgegeoids.empty()) {
        ...
        for (it = pointgeoids.begin(), itp = pointpos.begin();
             it != std::prev(pointgeoids.end()) && itp != std::prev(pointpos.end()); it++, itp++) {
            // issue the actual commands to create the constraint
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('Horizontal',%d,%d,%d,%d)) ",
                                  *it, static_cast<int>(*itp), *std::next(it),
                                  static_cast<int>(*std::next(itp)));
        }
    }
    else { // vertex mode, fixedpoints > 1
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Impossible constraint"),
                             QObject::tr("There are more than one fixed points selected. Select a "
                                         "maximum of one fixed point!"));
        return;
    }
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainVertical"; }
...
void CmdSketcherConstrainVertical::activated(int iMsg)
{
    ...
    if (!edgegeoids.empty()) {
        ...
        for (std::vector<int>::iterator it = edgegeoids.begin(); it != edgegeoids.end(); it++) {
            // issue the actual commands to create the constraint
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('Vertical',%d)) ", *it);
        }
    }
    ...
    switch (seqIndex) {
        case 0: // {Edge}
        {
            ...
            if (CrvId != -1) {
                ...
                // issue the actual commands to create the constraint
                Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                      "addConstraint(Sketcher.Constraint('Vertical',%d)) ", CrvId);
                // finish the transaction and update
                Gui::Command::commitCommand();
                tryAutoRecompute(Obj);
            }
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainLock"; }
...
void CmdSketcherConstrainLock::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if ((it != std::prev(SubNames.end())
            ...
            if (selection.size() == 1) {
                ...
                QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                                     QObject::tr("Select only vertices from the sketch. The last "
                                                 "selected vertex may be the origin."));
            }
            // clear the selection (convenience)
            getSelection().clearSelection();
            return;
    ...
    if (GeoId.size() == 1) { // absolute mode
        ...
        if (isPointOrSegmentFixed(Obj, GeoId[0])) { edgeisblocked = true; }
        ...
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('DistanceX',%d,%d,%f)) ", GeoId[0],
                              static_cast<int>(PosId[0]), pnt.x);
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('DistanceY',%d,%d,%f)) ", GeoId[0],
                              static_cast<int>(PosId[0]), pnt.y);

        lastconstraintindex += 2;

        if (edgeisblocked || GeoId[0] <= Sketcher::GeoEnum::RefExt || isBsplineKnot(Obj, GeoId[0])
        ...
        if (isPointOrSegmentFixed(Obj, GeoId.back())) refpointfixed = true;

        for (itg = GeoId.begin(), itp = PosId.begin();
             itg != std::prev(GeoId.end()) && itp != std::prev(PosId.end()); ++itp, ++itg) {
            bool pointfixed = false;

            if (isPointOrSegmentFixed(Obj, *itg)) pointfixed = true;
            ...
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  ...
                                  *itg, static_cast<int>(*itp), GeoId.back(),
                                  static_cast<int>(PosId.back()), pntr.x - pnt.x);

            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('DistanceY',%d,%d,%d,%d,%f)) ",
                                  *itg, static_cast<int>(*itp), GeoId.back(),
                                  static_cast<int>(PosId.back()), pntr.y - pnt.y);
            lastconstraintindex += 2;
...
void CmdSketcherConstrainLock::applyConstraint(std::vector<SelIdPair> &selSeq, int seqIndex)
{
    switch (seqIndex) {
        case 0: // {Vertex}
            ...
            if (isPointOrSegmentFixed(Obj, selSeq.front().GeoId)) pointfixed = true;
            ...
            Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                  ...
                                  selSeq.front().GeoId, static_cast<int>(selSeq.front().PosId),
                                  pnt.x);
            Gui::cmdAppObjectArgs(sketchgui->getObject(),
            ...
            if (pointfixed || constraintCreationMode == Reference) {
                ...
                Gui::cmdAppObjectArgs(sketchgui->getObject(), "setDriving(%i, %s)",
                                      ConStr.size() - 2, "False");
            ...
            if (autoRecompute) Gui::Command::updateActive();
            break;
    }
}
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainBlock"; }
...
void CmdSketcherConstrainBlock::applyConstraint(std::vector<SelIdPair> &selSeq, int seqIndex)
{
    switch (seqIndex) {
        case 0: // {Edge}
        {
            ...
            if (checkConstraint(vals, Sketcher::Block, selSeq.front().GeoId,
...
public:
    ...
    ~DrawSketchHandlerCoincident() override { Gui::Selection().rmvSelectionGate(); }
    ...
    bool releaseButton(Base::Vector2d onSketchPos) override
    {
        ...
        if (GeoId1 == GeoEnum::GeoUndef) {
            ...
            if (!constraintExists && (GeoId1 != GeoId2)) {
                Gui::cmdAppObjectArgs(
                    sketchgui->getObject(),
                    "addConstraint(Sketcher.Constraint('Coincident',%d,%d,%d,%d)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2));
                Gui::Command::commitCommand();
            }
            else {
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainCoincident"; }

protected:
    void activated(int iMsg) override;
    void applyConstraint(std::vector<SelIdPair> &selSeq, int seqIndex) override;
    // returns true if a substitution took place
    bool substituteConstraintCombinations(SketchObject *Obj, int GeoId1, PointPos PosId1,
                                                                      ...
                                                                      PointPos PosId2)
{
    ...
    for (std::vector<Constraint *>::const_iterator it = cvals.begin(); it != cvals.end();
        ...
        if ((*it)->Type == Sketcher::Tangent && (*it)->FirstPos == Sketcher::PointPos::none
            ...
            Gui::Command::openCommand(
                QT_TRANSLATE_NOOP("Command", "Swap edge tangency with ptp tangency"));

            if (constraintExists) {
                // try to remove any pre-existing direct coincident constraints
                Gui::cmdAppObjectArgs(Obj, "delConstraintOnPoint(%i,%i)", GeoId1,
...
void CmdSketcherConstrainCoincident::activated(int iMsg)
{
    ...
    for (std::size_t i = 1; i < SubNames.size(); i++) {
        ...
        if (substituteConstraintCombinations(Obj, GeoId1, PosId1, GeoId2, PosId2)) return;
    ...
    if (constraintsAdded) commitCommand();
    else
        abortCommand();
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainDistance"; }
...
void CmdSketcherConstrainDistance::activated(int iMsg)
{
    ...
    if (SubNames.empty() || SubNames.size() > 2) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                             QObject::tr("Select exactly one line or one point and one line or two "
    ...
    getIdsFromName(SubNames[0], Obj, GeoId1, PosId1);
    if (SubNames.size() == 2) getIdsFromName(SubNames[1], Obj, GeoId2, PosId2);
    ...
    if ((isVertex(GeoId1, PosId1) || GeoId1 == Sketcher::GeoEnum::VAxis
        ...
        if (arebothpointsorsegmentsfixed || constraintCreationMode == Reference) {
            ...
            Gui::cmdAppObjectArgs(selection[0].getObject(), "setDriving(%i,%s)", ConStr.size() - 1,
                                  "False");
        ...
        if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
            ...
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('Distance',%d,%f)) ", GeoId1,
                                  ActLength);
    ...
    QMessageBox::warning(
        Gui::getMainWindow(), QObject::tr("Wrong selection"),
        QObject::tr(
            "Select exactly one line or one point and one line or two points from the sketch."));
    return;
}
        ...
        case 1: // {SelRoot, SelVertex}
        {
            ...
            if (GeoId1 == Sketcher::GeoEnum::HAxis && PosId1 == Sketcher::PointPos::none) {
                ...
                openCommand(
                    QT_TRANSLATE_NOOP("Command", "Add distance from horizontal axis constraint"));
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('DistanceY',%d,%d,%d,%d,%f)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), pnt2.y);
            }
            else if (GeoId1 == Sketcher::GeoEnum::VAxis && PosId1 == Sketcher::PointPos::none) {
                PosId1 = Sketcher::PointPos::start;
                ...
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('DistanceX',%d,%d,%d,%d,%f)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), pnt2.x);
            }
            else {
                Base::Vector3d pnt1 = Obj->getPoint(GeoId1, PosId1);
                ...
                openCommand(QT_TRANSLATE_NOOP("Command", "Add point to point distance constraint"));
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('Distance',%d,%d,%d,%d,%f)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2),
                    (pnt2 - pnt1).Length());
            }
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainPointOnObject"; }
    ...
    : CmdSketcherConstraint("Sketcher_ConstrainPointOnObject")
{
    ...
    allowedSelSequences = {{SelVertex, SelEdgeOrAxis},   {SelRoot, SelEdge},
                           {SelVertex, SelExternalEdge}, {SelEdge, SelVertexOrRoot},
                           {SelEdgeOrAxis, SelVertex},   {SelExternalEdge, SelVertex}};
}
...
void CmdSketcherConstrainPointOnObject::activated(int iMsg)
{
    ...
    for (std::size_t i = 0; i < SubNames.size(); i++) {
        ...
        if (isEdge(id.GeoId, id.PosId)) curves.push_back(id);
        if (isVertex(id.GeoId, id.PosId)) points.push_back(id);
    }
    ...
    if ((points.size() == 1 && !curves.empty()) || (!points.empty() && curves.size() == 1)) {
        ...
        for (std::size_t iPnt = 0; iPnt < points.size(); iPnt++) {
            for (std::size_t iCrv = 0; iCrv < curves.size(); iCrv++) {
                ...
                if (substituteConstraintCombinations(Obj, points[iPnt].GeoId, points[iPnt].PosId,
                                                     curves[iCrv].GeoId)) {
                    cnt++;
                    continue;
                }
    ...
    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                         QObject::tr("Select either one point and several curves, "
                                     ...
                                     "You have selected %1 curves and %2 points.")
                             .arg(curves.size())
                             .arg(points.size()));
    return;
}
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainDistanceX"; }
...
void CmdSketcherConstrainDistanceX::activated(int iMsg)
{
    ...
    if (isVertex(GeoId1, PosId1) && isVertex(GeoId2, PosId2)) {
        ...
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('DistanceX',%d,%d,%f)) ", GeoId1,
                              static_cast<int>(PosId1), ActX);
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainDistanceY"; }
...
void CmdSketcherConstrainDistanceY::activated(int iMsg)
{
    ...
    if (isVertex(GeoId1, PosId1) && isVertex(GeoId2, PosId2)) {
        ...
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('DistanceY',%d,%d,%f)) ", GeoId1,
                              static_cast<int>(PosId1), ActY);

        if (GeoId1 <= Sketcher::GeoEnum::RefExt || isBsplineKnot(Obj, GeoId1)
            || constraintCreationMode == Reference) {
            // it is a constraint on a external line, make it non-driving
            const std::vector<Sketcher::Constraint *> &ConStr = Obj->Constraints.getValues();
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainParallel"; }
...
void CmdSketcherConstrainParallel::activated(int iMsg)
{
    ...
    for (int i = 0; i < int(ids.size() - 1); i++) {
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('Parallel',%d,%d)) ", ids[i],
                              ids[i + 1]);
    }
    // finish the transaction and update
    commitCommand();
...
void CmdSketcherConstrainParallel::applyConstraint(std::vector<SelIdPair> &selSeq, int seqIndex)
{
    switch (seqIndex) {
        ...
        case 3: // {SelExternalEdge, SelEdge}
            ...
            openCommand(QT_TRANSLATE_NOOP("Command", "Add parallel constraint"));
            Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                  "addConstraint(Sketcher.Constraint('Parallel',%d,%d)) ", GeoId1,
                                  GeoId2);
            // finish the transaction and update
            commitCommand();
            tryAutoRecompute(Obj);
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainPerpendicular"; }
...
void CmdSketcherConstrainPerpendicular::activated(int iMsg)
{
    ...
    QString strBasicHelp =
        QObject::tr("There is a number of ways this constraint can be applied.\n\n"
                    "Accepted combinations: two curves; an endpoint and a curve; two endpoints; "
                    "two curves and a point.",
    ...
    if (areBothPointsOrSegmentsFixed(Obj, GeoId1,
    ...
    if (SubNames.size() == 3) { //perpendicular via point
        ...
        if (isEdge(GeoId1, PosId1) && isEdge(GeoId2, PosId2) && isVertex(GeoId3, PosId3)) {
            ...
            try {
                ...
                if (!IsPointAlreadyOnCurve(GeoId1, GeoId3, PosId3, Obj)) {
                    Gui::cmdAppObjectArgs(
                        selection[0].getObject(),
                        "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ", GeoId3,
                        static_cast<int>(PosId3), GeoId1);
                };
                ...
                if (!IsPointAlreadyOnCurve(GeoId2, GeoId3, PosId3, Obj)) {
                    Gui::cmdAppObjectArgs(
                        selection[0].getObject(),
                        "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ", GeoId3,
                        static_cast<int>(PosId3), GeoId2);
                };

                if (!IsPointAlreadyOnCurve(
                        GeoId1, GeoId3, PosId3,
        ...
        strError = QObject::tr("With 3 objects, there must be 2 curves and 1 point.",
                               "tangent constraint");
    }
        ...
        if (isVertex(GeoId1, PosId1)
            ...
            Gui::cmdAppObjectArgs(
                selection[0].getObject(),
                "addConstraint(Sketcher.Constraint('Perpendicular',%d,%d,%d,%d)) ", GeoId1,
                static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2));
            commitCommand();
            tryAutoRecompute(Obj);
            ...
            if (geo1->getTypeId() == Part::GeomEllipse::getClassTypeId()
                ...
                try {
                    ...
                    Gui::cmdAppObjectArgs(
                        selection[0].getObject(),
                        "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ", GeoIdPoint,
                        static_cast<int>(Sketcher::PointPos::start), GeoId1);
                    // Point on second object
                    Gui::cmdAppObjectArgs(
                        selection[0].getObject(),
                        "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ", GeoIdPoint,
                        static_cast<int>(Sketcher::PointPos::start), GeoId2);
                                                        ...
                                                        int seqIndex)
{
        ...
        case 3: // {SelExternalEdge, SelEdge}
        {
            ...
            if (geo1->getTypeId() == Part::GeomEllipse::getClassTypeId()
                ...
                try {
                    ...
                    // Point on first object (ellipse, arc of ellipse)
                    Gui::cmdAppObjectArgs(
                        Obj, "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ",
    ...
    if (isEdge(GeoId1, PosId1) && isEdge(GeoId2, PosId2) && isVertex(GeoId3, PosId3)) {
        ...
        try {
            ...
            if (!IsPointAlreadyOnCurve(GeoId1, GeoId3, PosId3, Obj)) {
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('PointOnObject',%d,%d,%d)) ", GeoId3,
                    static_cast<int>(PosId3), GeoId1);
            };
            ...
            Gui::cmdAppObjectArgs(
                Obj, "addConstraint(Sketcher.Constraint('PerpendicularViaPoint',%d,%d,%d,%d)) ",
                GeoId1, GeoId2, GeoId3, static_cast<int>(PosId3));
        }
        catch (const Base::Exception &e) {
            Base::Console().Error("%s\n", e.what());
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainTangent"; }
                                                                   ...
                                                                   int GeoId2)
{
    ...
    for (std::vector<Constraint *>::const_iterator it = cvals.begin(); it != cvals.end();
        ...
        if ((*it)->Type == Sketcher::Coincident
            ...
            Gui::cmdAppObjectArgs(Obj, "delConstraint(%i)",
    ...
    if (SubNames.size() == 3) { //tangent via point
        ...
        if (isEdge(GeoId1, PosId1) && isEdge(GeoId2, PosId2) && isVertex(GeoId3, PosId3)) {
            ...
            try {
                ...
                Gui::cmdAppObjectArgs(
                    selection[0].getObject(),
                    "addConstraint(Sketcher.Constraint('TangentViaPoint',%d,%d,%d,%d)) ", GeoId1,
                    GeoId2, GeoId3, static_cast<int>(PosId3));
            }
            catch (const Base::Exception &e) {
                Base::Console().Error("%s\n", e.what());
                QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Error"),
        ...
        if (isVertex(GeoId1, PosId1) && isVertex(GeoId2, PosId2)) { // endpoint-to-endpoint tangency
            ...
            return;
                 ...
                 || (isEdge(GeoId1, PosId1)
                     && isVertex(GeoId2, PosId2))) { // endpoint-to-curve tangency
            if (isVertex(GeoId2, PosId2)) {
                std::swap(GeoId1, GeoId2);
                std::swap(PosId1, PosId2);
            }
            ...
            openCommand(QT_TRANSLATE_NOOP("Command", "Add tangent constraint"));
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('Tangent',%d,%d,%d)) ", GeoId1,
                                  static_cast<int>(PosId1), GeoId2);
            commitCommand();
            tryAutoRecompute(Obj);
            ...
            // - coincidence + tangency on edge
            // - point on object + tangency on edge
            if (substituteConstraintCombinations(Obj, GeoId1, GeoId2)) return;
            ...
            if (geom1 && geom2
                ...
                if (geom2->getTypeId() == Part::GeomEllipse::getClassTypeId()
                    ...
                    makeTangentToArcOfHyperbolaviaNewPoint(
                        Obj, static_cast<const Part::GeomArcOfHyperbola *>(geom2), geom1, GeoId2,
                        GeoId1);
                    getSelection().clearSelection();
                    return;
                }
                    ...
                    makeTangentToArcOfParabolaviaNewPoint(
                        Obj, static_cast<const Part::GeomArcOfParabola *>(geom2), geom1, GeoId2,
                        GeoId1);
                    getSelection().clearSelection();
                    return;
                }
                ...
                if (geom2->getTypeId() == Part::GeomArcOfHyperbola::getClassTypeId()
                    ...
                    makeTangentToArcOfEllipseviaNewPoint(
                        Obj, static_cast<const Part::GeomArcOfEllipse *>(geom1), geom2, GeoId1,
                        GeoId2);
                ...
                if (geom2->getTypeId() == Part::GeomArcOfHyperbola::getClassTypeId()
                    ...
                    Gui::Command::openCommand(
                        QT_TRANSLATE_NOOP("Command", "Add tangent constraint point"));
                    makeTangentToArcOfHyperbolaviaNewPoint(
                        Obj, static_cast<const Part::GeomArcOfHyperbola *>(geom1), geom2, GeoId1,
                        GeoId2);
                    getSelection().clearSelection();
                    return;
                }
                ...
                if (geom2->getTypeId() == Part::GeomArcOfParabola::getClassTypeId()
                    ...
                    Gui::Command::openCommand(
                        QT_TRANSLATE_NOOP("Command", "Add tangent constraint point"));
                    makeTangentToArcOfParabolaviaNewPoint(
                        Obj, static_cast<const Part::GeomArcOfParabola *>(geom1), geom2, GeoId1,
                        GeoId2);
                    getSelection().clearSelection();
                    return;
                }
            ...
            openCommand(QT_TRANSLATE_NOOP("Command", "Add tangent constraint"));
            Gui::cmdAppObjectArgs(selection[0].getObject(),
                                  "addConstraint(Sketcher.Constraint('Tangent',%d,%d)) ", GeoId1,
                                  GeoId2);
            commitCommand();
            tryAutoRecompute(Obj);
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainRadius"; }
...
void CmdSketcherConstrainRadius::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if (geom && geom->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
            ...
            if (issegmentfixed) { externalGeoIdRadiusMap.emplace_back(GeoId, radius); }
            else {
            ...
            if (isBsplinePole(geom)) poles = true;
            else
                nonpoles = true;
        }
    }
    ...
    if (!externalGeoIdRadiusMap.empty()) {
        ...
        for (std::vector<std::pair<int, double>>::iterator it = externalGeoIdRadiusMap.begin();
             it != externalGeoIdRadiusMap.end(); ++it) {
            ...
            Gui::cmdAppObjectArgs(selection[0].getObject(), "setDriving(%i,%s)", constrSize - 1,
                                  "False");
        }
    ...
    if (!geoIdRadiusMap.empty()) {
        if (geoIdRadiusMap.size() > 1 && constraintCreationMode == Driving) {
            ...
            if (!commandopened) openCommand(QT_TRANSLATE_NOOP("Command", "Add radius constraint"));

            // Add the equality constraints
            for (std::vector<std::pair<int, double>>::iterator it = geoIdRadiusMap.begin() + 1;
                 it != geoIdRadiusMap.end(); ++it) {
                Gui::cmdAppObjectArgs(selection[0].getObject(),
    ...
    switch (seqIndex) {
        ...
        case 1: // {SelExternalEdge}
        {
            ...
            if (fixed || constraintCreationMode == Reference) {
                ...
                updateNeeded =
                    true; // We do need to update the solver DoF after setting the constraint driving.
            }
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainDiameter"; }
...
void CmdSketcherConstrainDiameter::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if (geom && geom->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
            ...
            if (issegmentfixed) { externalGeoIdDiameterMap.emplace_back(GeoId, 2 * radius); }
            else {
    ...
    if (!externalGeoIdDiameterMap.empty()) {
        ...
        for (std::vector<std::pair<int, double>>::iterator it = externalGeoIdDiameterMap.begin();
             it != externalGeoIdDiameterMap.end(); ++it) {
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainRadiam"; }
...
void CmdSketcherConstrainRadiam::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if (issegmentfixed) { externalGeoIdRadiamMap.emplace_back(GeoId, radius); }
        else {
    ...
    if (!externalGeoIdRadiamMap.empty()) {
        ...
        for (std::vector<std::pair<int, double>>::iterator it = externalGeoIdRadiamMap.begin();
             it != externalGeoIdRadiamMap.end(); ++it) {
    ...
    if (!geoIdRadiamMap.empty()) {
        if (geoIdRadiamMap.size() > 1 && constraintCreationMode == Driving) {
            ...
            if (!commandopened) openCommand(QT_TRANSLATE_NOOP("Command", "Add radiam constraint"));

            // Add the equality constraints
            for (std::vector<std::pair<int, double>>::iterator it = geoIdRadiamMap.begin() + 1;
                 it != geoIdRadiamMap.end(); ++it) {
                Gui::cmdAppObjectArgs(Obj, "addConstraint(Sketcher.Constraint('Equal',%d,%d)) ",
                                      refGeoId, it->first);
            }
    ...
    switch (seqIndex) {
        ...
        case 1: // {SelExternalEdge}
        {
            ...
            if (geom && geom->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
                ...
                isCircle = true;
                if (isBsplinePole(geom)) isPole = true;
            }
            else {
                QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                                     QObject::tr("Constraint only applies to arcs or circles."));
...
void CmdSketcherCompConstrainRadDia::activated(int iMsg)
{
    ...
    if (iMsg == 0) { rcCmdMgr.runCommandByName("Sketcher_ConstrainRadius"); }
    else if (iMsg == 1) {
...
Gui::Action *CmdSketcherCompConstrainRadDia::createAction()
{
    ...
    switch (curRadDiaCons) {
        case 0: pcAction->setIcon(arc1->icon()); break;
        case 1: pcAction->setIcon(arc2->icon()); break;
        default: pcAction->setIcon(arc3->icon()); curRadDiaCons = 2;
    }
...
bool CmdSketcherCompConstrainRadDia::isActive() { return isCommandActive(getActiveGuiDocument()); }
...
public:
    ...
    void updateAction(int mode) override;
    const char *className() const override { return "CmdSketcherConstrainAngle"; }
...
void CmdSketcherConstrainAngle::activated(int iMsg)
{
    ...
    getIdsFromName(SubNames[0], Obj, GeoId1, PosId1);
    if (SubNames.size() > 1) getIdsFromName(SubNames[1], Obj, GeoId2, PosId2);
    if (SubNames.size() > 2) getIdsFromName(SubNames[2], Obj, GeoId3, PosId3);
    ...
    if (SubNames.size() == 3) { //standalone implementation of angle-via-point
        ...
        if (isEdge(GeoId1, PosId1) && isEdge(GeoId2, PosId2) && isVertex(GeoId3, PosId3)) {
            ...
            Gui::cmdAppObjectArgs(
                selection[0].getObject(),
                "addConstraint(Sketcher.Constraint('AngleViaPoint',%d,%d,%d,%d,%f)) ", GeoId1,
                GeoId2, GeoId3, static_cast<int>(PosId3), ActAngle);
        ...
        if (isEdge(GeoId2, PosId2)) { // line to line angle
            ...
            if (geom1->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                ...
                if (dir3.Length() < Precision::Intersection()) {
                    ...
                    if (dist.Sqr() > Precision::Intersection()) {
                        QMessageBox::warning(
                            Gui::getMainWindow(), QObject::tr("Parallel lines"),
                            QObject::tr(
                ...
                Gui::cmdAppObjectArgs(selection[0].getObject(),
                                      ...
                                      GeoId1, static_cast<int>(PosId1), GeoId2,
                                      static_cast<int>(PosId2), ActAngle);
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                openCommand(QT_TRANSLATE_NOOP("Command", "Add angle constraint"));
                Gui::cmdAppObjectArgs(selection[0].getObject(),
                                      "addConstraint(Sketcher.Constraint('Angle',%d,%f)) ", GeoId1,
                                      ActAngle);
    ...
    switch (seqIndex) {
        ...
        case 4: // {SelExternalEdge, SelExternalEdge}
        {
            ...
            if (geom1->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                ...
                openCommand(QT_TRANSLATE_NOOP("Command", "Add angle constraint"));
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('Angle',%d,%d,%d,%d,%f)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), ActAngle);
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainEqual"; }
...
void CmdSketcherConstrainEqual::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) { lineSel = true; }
        else if (geo->getTypeId() == Part::GeomArcOfCircle::getClassTypeId()) {
            arcSel = true;
        }
        else if (geo->getTypeId() == Part::GeomCircle::getClassTypeId()) {
            if (isBsplinePole(geo)) weightSel = true;
            else
                circSel = true;
        }
    ...
    for (int i = 0; i < int(ids.size() - 1); i++) {
        Gui::cmdAppObjectArgs(selection[0].getObject(),
                              "addConstraint(Sketcher.Constraint('Equal',%d,%d)) ", ids[i],
                              ids[i + 1]);
    }
    // finish the transaction and update
    commitCommand();
...
void CmdSketcherConstrainEqual::applyConstraint(std::vector<SelIdPair> &selSeq, int seqIndex)
{
    ...
    switch (seqIndex) {
        ...
        case 2: // {SelExternalEdge, SelEdge}
        {
            ...
            // undo command open
            openCommand(QT_TRANSLATE_NOOP("Command", "Add equality constraint"));
            Gui::cmdAppObjectArgs(Obj, "addConstraint(Sketcher.Constraint('Equal',%d,%d)) ", GeoId1,
                                  GeoId2);
            // finish the transaction and update
            commitCommand();
            tryAutoRecompute(Obj);
...
public:
    ...
    const char *className() const override { return "CmdSketcherConstrainSymmetric"; }
    ...
    if (SubNames.size() == 2) {
        ...
        if (isEdge(GeoId1, PosId1) && isVertex(GeoId2, PosId2)) {
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                Gui::cmdAppObjectArgs(
                    selection[0].getObject(),
                    "addConstraint(Sketcher.Constraint('Symmetric',%d,%d,%d,%d,%d,%d)) ", GeoId1,
                    static_cast<int>(Sketcher::PointPos::start), GeoId1,
    ...
    if (isVertex(GeoId1, PosId1) && isVertex(GeoId2, PosId2)) {
        ...
        if (isEdge(GeoId3, PosId3)) {
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                Gui::cmdAppObjectArgs(
                    selection[0].getObject(),
                    "addConstraint(Sketcher.Constraint('Symmetric',%d,%d,%d,%d,%d)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), GeoId3);
    ...
    switch (seqIndex) {
        ...
        case 11: // {SelVertex, SelVertex, SelEdgeOrAxis}
        {
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                openCommand(QT_TRANSLATE_NOOP("Command", "Add symmetric constraint"));
                Gui::cmdAppObjectArgs(
                    Obj, "addConstraint(Sketcher.Constraint('Symmetric',%d,%d,%d,%d,%d)) ", GeoId1,
                    static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), GeoId3);
...
void CmdSketcherConstrainSnellsLaw::activated(int iMsg)
{
    ...
    try {
        ...
        if (SubNames.size() != 3) {
            strError = QObject::tr("Number of selected objects is not 3 (is %1).", dmbg)
                           .arg(SubNames.size());
            throw Base::ValueError("");
        }
        ...
        Gui::cmdAppObjectArgs(
            selection[0].getObject(),
            "addConstraint(Sketcher.Constraint('SnellsLaw',%d,%d,%d,%d,%d,%.12f)) ", GeoId1,
            static_cast<int>(PosId1), GeoId2, static_cast<int>(PosId2), GeoId3, n2divn1);
...
void CmdSketcherConstrainInternalAlignment::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if (geo && geo->getTypeId() == Part::GeomPoint::getClassTypeId()) pointids.push_back(GeoId);
    ...
    if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()) {
        ...
        if ((!(focus1 && focus2) && !pointids.empty()) || (!(major && minor) && !lineids.empty())) {
            ...
            if (!pointids.empty()) {
                if (!focus1) {
                    Gui::cmdAppObjectArgs(selection[0].getObject(),
                                          "addConstraint(Sketcher.Constraint('InternalAlignment:"
                                          "EllipseFocus1',%d,%d,%d)) ",
                                          pointids[0], static_cast<int>(Sketcher::PointPos::start),
                                          ellipseids[0]);
                }
                else if (!focus2) {
                    Gui::cmdAppObjectArgs(selection[0].getObject(),
            ...
            if (pointids.size() == 2) {
                if (!focus2) {
                    Gui::cmdAppObjectArgs(selection[0].getObject(),
                                          "addConstraint(Sketcher.Constraint('InternalAlignment:"
                                          "EllipseFocus2',%d,%d,%d)) ",
                                          pointids[1], static_cast<int>(Sketcher::PointPos::start),
                                          ellipseids[0]);
                }
                else
                    extra_elements = true;
            ...
            if (!lineids.empty()) {
                if (!major) {
                    Gui::cmdAppObjectArgs(selection[0].getObject(),
                                          "addConstraint(Sketcher.Constraint('InternalAlignment:"
                                          "EllipseMajorDiameter',%d,%d)) ",
                                          lineids[0], ellipseids[0]);

                    const Part::GeomLineSegment *geo =
                        static_cast<const Part::GeomLineSegment *>(Obj->getGeometry(lineids[0]));
            ...
            if (lineids.size() == 2) {
                if (!minor) {
                    ...
                    const Part::GeomLineSegment *geo =
                        static_cast<const Part::GeomLineSegment *>(Obj->getGeometry(lineids[1]));
...
void CmdSketcherToggleDrivingConstraint::activated(int iMsg)
{
    ...
    if (Gui::Selection().countObjectsOfType(Sketcher::SketchObject::getClassTypeId()) > 0) {
        ...
        for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
            ...
            if (it->size() > 10 && it->substr(0, 10) == "Constraint") modeChange = false;
        }
    }
    ...
    if (modeChange) {
        ...
        if (constraintCreationMode == Driving) { constraintCreationMode = Reference; }
        else {

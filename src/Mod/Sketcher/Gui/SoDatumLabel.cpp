...
void SoDatumLabel::initClass() { SO_NODE_INIT_CLASS(SoDatumLabel, SoShape, "Shape"); }
...
void SoDatumLabel::drawImage()
{
    ...
    if (useAntialiasing) painter.setRenderHint(QPainter::Antialiasing);
...
void SoDatumLabel::generatePrimitives(SoAction *action)
{
    ...
    if (this->imgHeight <= FLT_EPSILON || this->imgWidth <= FLT_EPSILON) return;
    ...
    if (this->datumtype.getValue() == DISTANCE || this->datumtype.getValue() == DISTANCEX
        ...
        SbVec3f dir, normal;
        if (this->datumtype.getValue() == DISTANCE) { dir = (p2 - p1); }
...
void SoDatumLabel::notify(SoNotList *l)
{
    ...
    if (f == &this->string) { this->glimagevalid = false; }
...
void SoDatumLabel::GLRender(SoGLRenderAction *action)
{
    ...
    if (action->handleTransparency(true)) return;
    ...
    */
    ...
    if (this->datumtype.getValue() == DISTANCE || this->datumtype.getValue() == DISTANCEX
        ...
        if (angle > M_PI_2 + M_PI / 12) { angle -= (float)M_PI; }
        ...
        corners.push_back(textOffset + dir * (this->imgWidth / 2 + margin)
                          + normal * (srch + margin));
        corners.push_back(textOffset - dir * (this->imgWidth / 2 + margin)
                          + normal * (srch + margin));
        ...
        if ((p3 - p1).length() > (p2 - p1).length()) p2 = p3;
        ...
        if (range >= 0) range = std::max(0.2f * range, range - this->imgWidth / (2 * r));
        else
    ...
    if (hasText) {
        ...
        if (!npot) {
            ...
            int sx = (w - srcw) / 2;
            int sy = (h - srch) / 2;
            glTexImage2D(GL_TEXTURE_2D, 0, nc, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         (const GLvoid *)imagedata.bits());
            glTexSubImage2D(GL_TEXTURE_2D, 0, sx, sy, srcw, srch, GL_RGBA, GL_UNSIGNED_BYTE,
                            (const GLvoid *)dataptr);
        }
        else {
            glTexImage2D(GL_TEXTURE_2D, 0, nc, srcw, srch, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         (const GLvoid *)dataptr);
        }
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

...
EditModeGeometryCoinConverter::EditModeGeometryCoinConverter(
    ...
    CoinMapping &coinMap)
    : viewProvider(vp), geometryLayerNodes(geometrylayernodes),
      drawingParameters(drawingparameters), geometryLayerParameters(geometryLayerParams),
      coinMapping(coinMap)
{}
...
void EditModeGeometryCoinConverter::convert(const Sketcher::GeoListFacade &geolistfacade)
{
    ...
    auto setTracking = [this](int geoId, int coinLayer,
        ...
        if (pointmode == PointsMode::InsertSingle) {
            ...
            coinMapping.GeoElementId2SetId.emplace(
                std::piecewise_construct, std::forward_as_tuple(geoId, Sketcher::PointPos::start),
            ...
            coinMapping.GeoElementId2SetId.emplace(
                std::piecewise_construct, std::forward_as_tuple(geoId, Sketcher::PointPos::end),
                std::forward_as_tuple(pointCounter[coinLayer]++, coinLayer));
        }
            ...
            coinMapping.GeoElementId2SetId.emplace(
                std::piecewise_construct, std::forward_as_tuple(geoId, Sketcher::PointPos::mid),
                std::forward_as_tuple(pointCounter[coinLayer]++, coinLayer));
        }
        ...
        if (numberCurves > 0) // insert the first segment of the curve into the map
            coinMapping.GeoElementId2SetId.emplace(
                std::piecewise_construct, std::forward_as_tuple(geoId, Sketcher::PointPos::none),
    ...
    for (size_t i = 0; i < geolistfacade.geomlist.size() - 2; i++) {
        ...
        if (type == Part::GeomPoint::getClassTypeId()) { // add a point
            convert<Part::GeomPoint, EditModeGeometryCoinConverter::PointsMode::InsertSingle,
                    EditModeGeometryCoinConverter::CurveMode::NoCurve,
                    EditModeGeometryCoinConverter::AnalyseMode::BoundingBoxMagnitude>(geom, GeoId);
            setTracking(GeoId, coinLayer, EditModeGeometryCoinConverter::PointsMode::InsertSingle,
                        0);
        }
            ...
            convert<Part::GeomLineSegment,
                    ...
                    EditModeGeometryCoinConverter::AnalyseMode::BoundingBoxMagnitude>(geom, GeoId);
            setTracking(GeoId, coinLayer, EditModeGeometryCoinConverter::PointsMode::InsertStartEnd,
                        1);
        }
        else if (type.isDerivedFrom(
                     Part::GeomConic::getClassTypeId())) { // add a closed curve conic
            convert<Part::GeomConic, EditModeGeometryCoinConverter::PointsMode::InsertMidOnly,
                    EditModeGeometryCoinConverter::CurveMode::ClosedCurve,
                    EditModeGeometryCoinConverter::AnalyseMode::BoundingBoxMagnitude>(geom, GeoId);
            setTracking(GeoId, coinLayer, EditModeGeometryCoinConverter::PointsMode::InsertMidOnly,
                        1);
        }
        else if (type.isDerivedFrom(
            ...
            convert<Part::GeomArcOfConic,
                    ...
                    EditModeGeometryCoinConverter::AnalyseMode::BoundingBoxMagnitude>(geom, GeoId);
            setTracking(GeoId, coinLayer,
                        EditModeGeometryCoinConverter::PointsMode::InsertStartEndMid, 1);
        }
        else if (type
                 == Part::GeomBSplineCurve::
                     getClassTypeId()) { // add a bspline (a bounded curve that is not a conic)
    ...
    if constexpr (curvemode == CurveMode::StartEndPointsOnly) {
        ...
        if constexpr (analysemode == AnalyseMode::BoundingBoxMagnitudeAndBSplineCurvature) {
            ...
            for (int i = 0; i < ndiv; i++) {
                ...
                if (curvaturelist[i] > maxcurv) maxcurv = curvaturelist[i];

                double tempf = (pointatcurvelist[i] - midp).Length();

                if (tempf > maxdisttocenterofmass) maxdisttocenterofmass = tempf;
            }
            ...
            if (temprepscale > combrepscale) combrepscale = temprepscale;
        }
    }
}
...
double EditModeGeometryCoinConverter::getCombRepresentationScale() { return combrepscale; }

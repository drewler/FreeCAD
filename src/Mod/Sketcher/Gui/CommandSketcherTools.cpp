...
CmdSketcherSelectConstraints::CmdSketcherSelectConstraints() : Command("Sketcher_SelectConstraints")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
CmdSketcherSelectOrigin::CmdSketcherSelectOrigin() : Command("Sketcher_SelectOrigin")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
bool CmdSketcherSelectOrigin::isActive() { return isCommandActive(getActiveGuiDocument(), false); }
...
void CmdSketcherSelectPartiallyRedundantConstraints::activated(int iMsg)
{
    ...
    for (std::vector<Sketcher::Constraint *>::const_iterator it = vals.begin(); it != vals.end();
        ...
        for (std::vector<int>::const_iterator itc = solverpartiallyredundant.begin();
             itc != solverpartiallyredundant.end(); ++itc) {
            if ((*itc) - 1 == i) {
                constraintSubNames.push_back(
                    Sketcher::PropertyConstraintList::getConstraintName(i));
                break;
                ...
                if (vals[ConstrId]->First != GeoEnum::GeoUndef) {
                    ...
                    switch (vals[ConstrId]->FirstPos) {
                        ...
                        case Sketcher::PointPos::mid:
                            ...
                            if (vertex > -1) ss << "Vertex" << vertex + 1;
                            break;
                    }
...
void CmdSketcherRestoreInternalAlignmentGeometry::activated(int iMsg)
{
    ...
    auto getEdgeGeoId = [&Obj](const std::string &SubName) {
        ...
        if (PosId == Sketcher::PointPos::none) return GeoId;
        else
...
CmdSketcherSymmetry::CmdSketcherSymmetry() : Command("Sketcher_Symmetry")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherSymmetry::activated(int iMsg)
{
    ...
    for (std::vector<std::string>::const_iterator it = SubNames.begin(); it != SubNames.end();
        ...
        if ((it->size() > 4 && it->substr(0, 4) == "Edge")
            ...
            if (LastGeo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) lastgeotype = line;
            else
                lastgeotype = invalid;
    ...
    try {
        Gui::cmdAppObjectArgs(Obj, "addSymmetric(%s, %d, %d)", geoIdList.c_str(), LastGeoId,
...
bool CmdSketcherSymmetry::isActive() { return isCommandActive(getActiveGuiDocument(), true); }
...
public:
    DrawSketchHandlerCopy(string geoidlist, int origingeoid, Sketcher::PointPos originpos,
        ...
        : Mode(STATUS_SEEK_First), snapMode(SnapMode::Free), geoIdList(geoidlist), Origin(),
          OriginGeoId(origingeoid), OriginPos(originpos), nElements(nelements), Op(op), EditCurve(2)
    {}
    ...
    void mouseMove(Base::Vector2d onSketchPos) override
    {
        if (Mode == STATUS_SEEK_First) {
            ...
            drawEdit(EditCurve);
            if (seekAutoConstraint(sugConstr1, endpoint, Base::Vector2d(0.0, 0.0),
    ...
    bool releaseButton(Base::Vector2d onSketchPos) override
    {
        ...
        if (Mode == STATUS_End) {
            ...
            int currentgeoid = static_cast<Sketcher::SketchObject *>(sketchgui->getObject())
            ...
            try {
                if (Op != SketcherCopy::Move) {
                    ...
                    Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                          "addMove(%s, App.Vector(%f, %f, 0))", geoIdList.c_str(),
                                          vector.x, vector.y);
                }
                Gui::Command::commitCommand();
            }
...
public:
    ...
    const char *className() const override { return "CmdSketcherCopy"; }
...
CmdSketcherCopy::CmdSketcherCopy() : SketcherCopy("Sketcher_Copy")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherCopy::activate() { SketcherCopy::activate(SketcherCopy::Copy); }

bool CmdSketcherCopy::isActive() { return isCommandActive(getActiveGuiDocument(), true); }
...
public:
    ...
    const char *className() const override { return "CmdSketcherClone"; }
    void activate() override;
...
CmdSketcherClone::CmdSketcherClone() : SketcherCopy("Sketcher_Clone")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherClone::activate() { SketcherCopy::activate(SketcherCopy::Clone); }

bool CmdSketcherClone::isActive() { return isCommandActive(getActiveGuiDocument(), true); }
...
public:
    ...
    const char *className() const override { return "CmdSketcherMove"; }
    void activate() override;
...
CmdSketcherMove::CmdSketcherMove() : SketcherCopy("Sketcher_Move")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherMove::activate() { SketcherCopy::activate(SketcherCopy::Move); }

bool CmdSketcherMove::isActive() { return isCommandActive(getActiveGuiDocument(), true); }

// ================================================================================

DEF_STD_CMD_ACL(CmdSketcherCompCopy)

CmdSketcherCompCopy::CmdSketcherCompCopy() : Command("Sketcher_CompCopy")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
    sMenuText = QT_TR_NOOP("Copy");
...
bool CmdSketcherCompCopy::isActive() { return isCommandActive(getActiveGuiDocument(), true); }
...
public:
    DrawSketchHandlerRectangularArray(string geoidlist, int origingeoid,
        ...
        : Mode(STATUS_SEEK_First), snapMode(SnapMode::Free), geoIdList(geoidlist),
          OriginGeoId(origingeoid), OriginPos(originpos), nElements(nelements), Clone(clone),
          Rows(rows), Cols(cols), ConstraintSeparation(constraintSeparation),
          EqualVerticalHorizontalSpacing(equalVerticalHorizontalSpacing), EditCurve(2)
    {}
        ...
        if (Mode == STATUS_End) {
            ...
            try {
                Gui::cmdAppObjectArgs(
                    ...
                    geoIdList.c_str(), vector.x, vector.y, (Clone ? "True" : "False"), Cols, Rows,
...
CmdSketcherRectangularArray::CmdSketcherRectangularArray() : Command("Sketcher_RectangularArray")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherRectangularArray::activated(int iMsg)
{
    ...
    if (slad.exec() == QDialog::Accepted) {
        ActivateHandler(getActiveGuiDocument(),
                        new DrawSketchHandlerRectangularArray(geoIdList, LastGeoId, LastPointPos,
                                                              geoids, slad.Clone, slad.Rows,
                                                              slad.Cols, slad.ConstraintSeparation,
...
CmdSketcherDeleteAllGeometry::CmdSketcherDeleteAllGeometry() : Command("Sketcher_DeleteAllGeometry")
{
    sAppModule = "Sketcher";
    sGroup = "Sketcher";
...
void CmdSketcherDeleteAllGeometry::activated(int iMsg)
{
    ...
    int ret = QMessageBox::question(
        Gui::getMainWindow(), QObject::tr("Delete All Geometry"),
...
void CmdSketcherDeleteAllConstraints::activated(int iMsg)
{
    ...
    int ret = QMessageBox::question(
        Gui::getMainWindow(), QObject::tr("Delete All Constraints"),

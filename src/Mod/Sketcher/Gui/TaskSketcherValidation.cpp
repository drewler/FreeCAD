...
SketcherValidation::SketcherValidation(Sketcher::SketchObject *Obj, QWidget *parent)
    : QWidget(parent), ui(new Ui_TaskSketcherValidation()), sketch(Obj), sketchAnalyser(Obj),
      coincidenceRoot(nullptr)
{
    ...
    double tolerances[8] = {Precision::Confusion() / 100,   Precision::Confusion() / 10,
                            Precision::Confusion(),         Precision::Confusion() * 10,
                            Precision::Confusion() * 100,   Precision::Confusion() * 1000,
                            Precision::Confusion() * 10000, Precision::Confusion() * 100000};
...
SketcherValidation::~SketcherValidation() { hidePoints(); }
...
void SketcherValidation::on_findButton_clicked()
{
    ...
    for (auto vc : vertexConstraints) { points.push_back(vc.v); }
...
void SketcherValidation::on_highlightButton_clicked()
{
    ...
    hidePoints();
    if (!points.empty()) showPoints(points);
}
...
void SketcherValidation::on_findConstraint_clicked()
{
    ...
    if (sketch->evaluateConstraints()) {
        ...
        QMessageBox::warning(this, tr("Invalid constraints"), tr("Invalid constraints found"));
...
void SketcherValidation::on_findReversed_clicked()
{
    ...
    if (!points.empty()) {
        ...
        if (nc > 0) {
            QMessageBox::warning(
                this, tr("Reversed external geometry"),
...
void SketcherValidation::on_delConstrExtr_clicked()
{
    ...
    reply = QMessageBox::question(
        this, tr("Delete constraints to external geom."),

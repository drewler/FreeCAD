...
public:
    ...
    virtual ~DrawSketchHandlerCarbonCopy() { Gui::Selection().rmvSelectionGate(); }

    void mouseMove(Base::Vector2d onSketchPos) override
    {
        Q_UNUSED(onSketchPos);
        if (Gui::Selection().getPreselection().pObjectName) applyCursor();
    }
    ...
    bool onSelectionChanged(const Gui::SelectionChanges &msg) override
    {
        if (msg.Type == Gui::SelectionChanges::AddSelection) {
            ...
            if (!obj) throw Base::ValueError("Sketcher: Carbon Copy: Invalid object in selection");

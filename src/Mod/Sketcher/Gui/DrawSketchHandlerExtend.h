...
public:
    explicit ExtendSelection(App::DocumentObject *obj)
        : Gui::SelectionFilterGate(nullPointer()), object(obj), disabled(false)
    {}
...
public:
    DrawSketchHandlerExtend()
        : Mode(STATUS_SEEK_First), EditCurve(2), BaseGeoId(-1), ExtendFromStart(false),
          SavedExtendFromStart(false), Increment(0)
    {}

    virtual ~DrawSketchHandlerExtend() { Gui::Selection().rmvSelectionGate(); }
    enum SelectMode
    {
        STATUS_SEEK_First,
        STATUS_SEEK_Second,
        ...
        if (Mode == STATUS_SEEK_Second) {
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                ...
                bool inCurve =
                    (projection.Length() < recenteredLine.Length()
                     && projection.GetAngle(recenteredLine)
                         < 0.1); // Two possible values here, M_PI and 0, but 0.1 is to avoid floating point problems.
                if (inCurve) {
                ...
                if (ExtendFromStart) {
                    ...
                    if (outOfArc) {
                        if (isCWFromEnd) { modArcAngle += 2 * M_PI - angleToEndAngle; }
                        else {
                            modArcAngle += angleToEndAngle;
                        }
                    }
                    else {
                        if (isCWFromEnd) { modArcAngle -= angleToEndAngle; }
                        else {
    ...
    bool releaseButton(Base::Vector2d onSketchPos) override
    {
        ...
        if (Mode == STATUS_SEEK_First) {
            ...
            try {
                ...
                if (!SugConstr.empty()) {
                    createAutoConstraints(SugConstr, BaseGeoId,

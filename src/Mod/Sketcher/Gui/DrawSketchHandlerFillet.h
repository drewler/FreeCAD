...
public:
    ...
    explicit DrawSketchHandlerFillet(FilletType filletType)
        : filletType(filletType), Mode(STATUS_SEEK_First), firstCurve(0)
    {}
    virtual ~DrawSketchHandlerFillet() { Gui::Selection().rmvSelectionGate(); }
    ...
    bool releaseButton(Base::Vector2d onSketchPos) override
    {
        ...
        if (Mode == STATUS_SEEK_First && VtId != -1) {
            ...
            if (geom->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                ...
                std::vector<int> GeoIdList;
                std::vector<Sketcher::PointPos> PosIdList;
                sketchgui->getSketchObject()->getDirectlyCoincidentPoints(GeoId, PosId, GeoIdList,
                                                                          PosIdList);
                if (GeoIdList.size() == 2 && GeoIdList[0] >= 0 && GeoIdList[1] >= 0) {
                    ...
                    if (geom1->getTypeId() == Part::GeomLineSegment::getClassTypeId()
                        ...
                        if (PosIdList[0] == Sketcher::PointPos::end) dir1 *= -1;
                        if (PosIdList[1] == Sketcher::PointPos::end) dir2 *= -1;
                ...
                try {
                    ...
                    Gui::cmdAppObjectArgs(sketchgui->getObject(), "fillet(%d,%d,%f,%s,%s)", GeoId,
                                          static_cast<int>(PosId), radius, "True",
            ...
            if (geom->getTypeId().isDerivedFrom(Part::GeomBoundedCurve::getClassTypeId())) {
                if (Mode == STATUS_SEEK_First) {
                    ...
                    Gui::Selection().addSelection(
                        ...
                        sketchgui->getSketchObject()->getNameInDocument(), ss.str().c_str(),
                        onSketchPos.x, onSketchPos.y, 0.f);
                }
                    ...
                    try {
                        ...
                        Gui::cmdAppObjectArgs(
                            ...
                            firstCurve, secondCurve, firstPos.x, firstPos.y, secondPos.x,
                            secondPos.y, radius, "True", pointFillet ? "True" : "False");
                        Gui::Command::commitCommand();
                    }

...
CurveConverter::~CurveConverter()
{
    try {
        ...
        const Base::ValueError &
            e) { // ensure that if parameter strings are not well-formed, the program is not terminated when calling the noexcept destructor.
        Base::Console().Error("CurveConverter: Malformed parameter string: %s\n", e.what());
    }
}
...
CurveConverter::toVector2DList(const std::vector<Part::Geometry *> &geometries)
{
    ...
    for (const auto &geo : geometries) list.push_back(toVector2D(geo));
...
void CurveConverter::updateCurvedEdgeCountSegmentsParameter()
{
    ...
    if (stdcountsegments < 6) stdcountsegments = 6;
...
void CurveConverter::OnChange(Base::Subject<const char *> &rCaller, const char *sReason)
{
    ...
    if (strcmp(sReason, "SegmentsPerGeometry") == 0) { updateCurvedEdgeCountSegmentsParameter(); }
}
...
QString DrawSketchHandler::getCrosshairCursorSVGName() const { return QString::fromLatin1("None"); }
                                     ...
                                     const std::map<unsigned long, unsigned long> &colorMapping)
{
    ...
    if (isRatioOne) pointer = pointer.scaled(32, 32);
...
void DrawSketchHandler::addCursorTail(std::vector<QPixmap> &pixmaps)
{
    ...
    if (view && view->isDerivedFrom(Gui::View3DInventor::getClassTypeId())) {
        ...
        int tailWidth = 0;
        for (auto const &p : pixmaps) { tailWidth += p.width(); }
        ...
        qp.drawPixmap(QPointF(0, 0),
                      baseIcon.scaled(baseCursorWidth * pixelRatio, baseCursorHeight * pixelRatio,
...
void DrawSketchHandler::updateCursor()
{
    ...
    if (cursorstring != QString::fromLatin1("None")) setCrosshairCursor(cursorstring);
}

void DrawSketchHandler::applyCursor(void) { applyCursor(actCursor); }
...
DrawSketchHandler::suggestedConstraintsPixmaps(std::vector<AutoConstraint> &suggestedConstraints)
{
    ...
    for (; it != suggestedConstraints.end(); ++it, i++) {
        QString iconType;
        switch (it->Type) {
            case Horizontal: iconType = QString::fromLatin1("Constraint_Horizontal"); break;
            case Vertical: iconType = QString::fromLatin1("Constraint_Vertical"); break;
            case Coincident: iconType = QString::fromLatin1("Constraint_PointOnPoint"); break;
            case PointOnObject: iconType = QString::fromLatin1("Constraint_PointOnObject"); break;
            case Tangent: iconType = QString::fromLatin1("Constraint_Tangent"); break;
            default: break;
        }
        if (!iconType.isEmpty()) {
            qreal pixelRatio = 1;
                                          ...
                                          AutoConstraint::TargetType type)
{
    ...
    if (preSelPnt != -1) sketchgui->getSketchObject()->getGeoVertexIndex(preSelPnt, GeoId, PosId);
    else if (preSelCrv != -1) {
    ...
    if (GeoId != GeoEnum::GeoUndef) {
        ...
        if ((type == AutoConstraint::VERTEX || type == AutoConstraint::VERTEX_NO_TANGENCY)
            ...
            constr.Type = Sketcher::PointOnObject;
                 ...
                 && PosId == Sketcher::PointPos::none
                 && hitobject->getTypeId() != Part::GeomBSplineCurve::getClassTypeId())
            constr.Type = Sketcher::PointOnObject;
        else if (type == AutoConstraint::CURVE && PosId == Sketcher::PointPos::none)
        ...
        if (constr.Type != Sketcher::None) suggestedConstraints.push_back(constr);
    }
    ...
    for (std::vector<Part::Geometry *>::const_iterator it = geomlist.begin(); it != geomlist.end();
        ...
        if ((*it)->getTypeId() == Part::GeomCircle::getClassTypeId()) {
            ...
            if (tmpDir * (center - tmpPos) > 0 || tmpDir * (center - tmpStart) < 0) continue;
            ...
            if (projDist < tangDeviation) {
                ...
                while (angle < startAngle) angle += 2 * D_PI; // Bring it to range of arc
        ...
        for (; it != autoConstrs.end(); ++it) {
            ...
            switch (it->Type) {
                case Sketcher::Coincident: {
                    if (posId1 == Sketcher::PointPos::none) continue;
                    // If the auto constraint has a point create a coincident otherwise it is an edge on a point
                    Gui::cmdAppObjectArgs(
                        sketchgui->getObject(),
                        "addConstraint(Sketcher.Constraint('Coincident',%i,%i,%i,%i)) ", geoId1,
                        static_cast<int>(posId1), it->GeoId, static_cast<int>(it->PosId));
                } break;
                case Sketcher::PointOnObject: {
                    ...
                    Gui::cmdAppObjectArgs(
                        sketchgui->getObject(),
                        "addConstraint(Sketcher.Constraint('PointOnObject',%i,%i,%i)) ", geoId1,
                ...
                case Sketcher::Horizontal: {
                    Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                          "addConstraint(Sketcher.Constraint('Horizontal',%i)) ",
                                          geoId2 != GeoEnum::GeoUndef ? geoId2 : geoId1);
                } break;
                case Sketcher::Vertical: {
                    Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                          "addConstraint(Sketcher.Constraint('Vertical',%i)) ",
                                          geoId2 != GeoEnum::GeoUndef ? geoId2 : geoId1);
                } break;
                case Sketcher::Tangent: {
                    ...
                    if (geom1 && geom2
                        ...
                        if (geom2->getTypeId() == Part::GeomEllipse::getClassTypeId()
                            ...
                            makeTangentToEllipseviaNewPoint(
                                Obj, static_cast<const Part::GeomEllipse *>(geom1), geom2, geoId1,
                                geoId2);
                            return;
                        }
                    }
                    ...
                    if (geom1 && geom2
                        ...
                        if (geom2->getTypeId() == Part::GeomArcOfEllipse::getClassTypeId()
                            ...
                            makeTangentToArcOfEllipseviaNewPoint(
                                Obj, static_cast<const Part::GeomArcOfEllipse *>(geom1), geom2,
                                geoId1, geoId2);
                            return;
                        }
                    }
                    ...
                    Gui::cmdAppObjectArgs(sketchgui->getObject(),
                                          "addConstraint(Sketcher.Constraint('Tangent',%i, %i)) ",
                                          geoId1, it->GeoId);
                } break;
                default: break;
            }

            if (createowncommand) { Gui::Command::commitCommand(); }
...
void DrawSketchHandler::drawEditMarkers(const std::vector<Base::Vector2d> &EditMarkers,
                                        unsigned int augmentationlevel)
{
    ViewProviderSketchDrawSketchHandlerAttorney::drawEditMarkers(*sketchgui, EditMarkers,
                                                                 augmentationlevel);
}

...
EditModeConstraintCoinManager::EditModeConstraintCoinManager(
    ...
    : viewProvider(vp), drawingParameters(drawingParams),
      geometryLayerParameters(geometryLayerParams), constraintParameters(constraintParams),
      editModeScenegraphNodes(editModeScenegraph), coinMapping(coinMap)
{}
...
Restart:
    ...
    if (constrlist.size() != vConstrType.size()) rebuildConstraintNodes(geolistfacade);
    ...
    auto getNormal = [](const GeoListFacade &geolistfacade, const int geoid,
        ...
        try { //because calculateNormalAtPoint, used in there, can throw
            ...
            switch (Constr->Type) {
                ...
                case Vertical: // write the new position of the Vertical constraint
                {
                    ...
                    if (!alignment) {
                        ...
                        if (geo->getTypeId() == Part::GeomLineSegment::getClassTypeId()) {
                            ...
                            double angle,
                                angleplus =
                                    0.; //angle = rotation of object as a whole; angleplus = arc angle (t parameter for ellipses).
                            ...
                            if (geo->getTypeId() == Part::GeomEllipse::getClassTypeId()
                                ...
                                majDir = Base::Vector3d(cos(angle), sin(angle),
                                                        0); //direction of major axis of ellipse
                                minDir = Base::Vector3d(-majDir.y, majDir.x,
                ...
                case Perpendicular: {
                    ...
                    Base::Vector3d midpos2, dir2, norm2;
                    bool twoIcons =
                        false; //a very local flag. It's set to true to indicate that the second dir+norm are valid and should be used


                    if (Constr->Third != GeoEnum::GeoUndef || //perpty via point
                        Constr->FirstPos
                            != Sketcher::PointPos::
                                none) { //endpoint-to-curve or endpoint-to-endpoint perpty
                    ...
                    Base::Vector3d relpos1 =
                        seekConstraintPosition(midpos1, norm1, dir1, 4.0,
                                               editModeScenegraphNodes.constrGroup->getChild(i));
                ...
                case Equal: {
                    ...
                    if (geo1->getTypeId() != Part::GeomLineSegment::getClassTypeId()
                        ...
                        if (Constr->Type == Equal) {
                            ...
                            double angle1,
                                angle1plus = 0., angle2,
                                angle2plus =
                                    0.; //angle1 = rotation of object as a whole; angle1plus = arc angle (t parameter for ellipses).
                            ...
                            if (geo1->getTypeId() == Part::GeomEllipse::getClassTypeId()
                                ...
                                Base::Vector3d majDir, minDir, rvec;
                                majDir = Base::Vector3d(cos(angle1), sin(angle1),
                                                        0); //direction of major axis of ellipse
                                minDir = Base::Vector3d(-majDir.y, majDir.x,
                                                        0); //direction of minor axis of ellipse
                            ...
                            if (geo2->getTypeId() == Part::GeomEllipse::getClassTypeId()
                                ...
                                Base::Vector3d majDir, minDir, rvec;
                                majDir = Base::Vector3d(cos(angle2), sin(angle2),
                                                        0); //direction of major axis of ellipse
                                minDir = Base::Vector3d(-majDir.y, majDir.x,
                                                        0); //direction of minor axis of ellipse
                    ...
                    Base::Vector3d relpos2 =
                        seekConstraintPosition(midpos2, norm2, dir2, 4.0,
                                               editModeScenegraphNodes.constrGroup->getChild(i));
                ...
                case DistanceY: {
                    ...
                    if (Constr->Type == Distance) asciiText->datumtype = SoDatumLabel::DISTANCE;
                ...
                case SnellsLaw: {
                    ...
                    if (Constr->Type == PointOnObject || Constr->Type == SnellsLaw
                        ...
                        (Constr->Type == Tangent
                         && Constr->FirstPos
                             != Sketcher::PointPos::
                                 none) //endpoint-to-curve or endpoint-to-endpoint tangency
                    ) {
    ...
    while (isConstraintAtPosition && multiplier < 10) {
        ...
        rp->apply(
            editModeScenegraphNodes
                .constrGroup); // We could narrow it down to just the SoGroup containing the constraints
        ...
        if (multiplier >= 0) multiplier++; // Increment the multiplier
    }
    if (multiplier == 10) relPos = norm * 0.5f; // no free position found
    return relPos * step;
}
...
{
    if (enabled) { editModeScenegraphNodes.constrGrpSelect->style.setValue(SoPickStyle::SHAPE); }
    else {
    ...
    SbVec3f norm)
{
    ...
    for (std::vector<Sketcher::Constraint *>::const_iterator it = constrlist.begin();
        ...
        switch ((*it)->Type) {
            ...
            default: vConstrType.push_back((*it)->Type);
        }
...
QString EditModeConstraintCoinManager::getPresentationString(const Constraint *constraint)
{
    ...
    if (!constraint->isActive) return QString::fromLatin1(" ");
    ...
    if (constraintParameters.bHideUnits && constraint->Type != Sketcher::Angle) {
        ...
        switch (unitSys) {
            ...
            case Base::UnitSystem::MmMin: baseUnitStr = QString::fromLatin1("mm"); break;

            case Base::UnitSystem::SI2: baseUnitStr = QString::fromLatin1("m"); break;

            case Base::UnitSystem::ImperialDecimal: baseUnitStr = QString::fromLatin1("in"); break;

            case Base::UnitSystem::Centimeters: baseUnitStr = QString::fromLatin1("cm"); break;
                                                                      ...
                                                                      const SbVec2s &cursorPos)
{
    ...
    if (tailFather2 != editModeScenegraphNodes.constrGroup) return constrIndices;
            ...
            if (sep->getNumChildren()
                ...
                if (constrIds) {
                    ...
                    if (combinedConstrBoxes.count(constrIdsStr) && dynamic_cast<SoImage *>(tail)) {
                        ...
                        SbVec3f constrPos = absPos
                            + trans * ViewProviderSketchCoinAttorney::getScaleFactor(viewProvider);
...
#endif
    if (iconPtr->width.getValue() != -1) iconSize[0] = iconPtr->width.getValue();
    if (iconPtr->height.getValue() != -1) iconSize[1] = iconPtr->height.getValue();
    return iconSize;
}
...
void EditModeConstraintCoinManager::drawConstraintIcons(const GeoListFacade &geolistfacade)
{
    ...
    for (int constrId = 0; constrId < maxNumberOfConstraints; ++constrId) {
        ...
        if (icoType.isEmpty()) continue;

        if (constraint->Type != vConstrType[constrId]) break;
        ...
        switch (constraint->Type) {
            ...
            case Parallel: multipleIcons = true; break;
            case Perpendicular:
                ...
                if (constraint->FirstPos == Sketcher::PointPos::none
                    ...
                    multipleIcons = true;
                break;
            case Equal: multipleIcons = true; break;
            default: break;
        }
        ...
        if (multipleIcons) {
            if (constraint->Name.empty()) thisIcon.label = QString::number(constrId + 1);
            else
            ...
            if (constraint->Name.empty()) thisIcon.label = QString();
            else
                thisIcon.label = QString::fromUtf8(constraint->Name.c_str());
        }
...
void EditModeConstraintCoinManager::combineConstraintIcons(IconQueue iconQueue)
{
    ...
    while (!iconQueue.empty()) {
        ...
        if (thisGroup.size() == 1) { drawTypicalConstraintIcon(thisGroup[0]); }
        else {
...
void EditModeConstraintCoinManager::drawMergedConstraintIcons(IconQueue iconQueue)
{
    ...
    while (!iconQueue.empty()) {
        ...
        if (idString.length()) idString.append(QString::fromLatin1(","));
        ...
        if (compositeIcon.isNull()) {
            compositeIcon = renderConstrIcon(thisType, iconColor, labels, labelColors, iconRotation,
                                             &boundingBoxesVec, &lastVPad);
        }
        else {
            int thisVPad;
            QImage partialIcon = renderConstrIcon(thisType, iconColor, labels, labelColors,
                                                  iconRotation, &boundingBoxesVec, &thisVPad);
            ...
            compositeIcon =
                compositeIcon.copy(0, 0, std::max(partialIcon.width(), compositeIcon.width()),
                                                       ...
                                                       std::vector<QRect> *boundingBoxes, int *vPad)
{
    ...
    if (boundingBoxes) boundingBoxes->push_back(QRect(0, 0, roticon.width(), roticon.height()));
    ...
    if (!labels.join(QString()).isEmpty()) {
        ...
        for (labelItr = labels.begin(), colorItr = labelColors.begin();
             labelItr != labels.end() && colorItr != labelColors.end(); ++labelItr, ++colorItr) {
...
void EditModeConstraintCoinManager::drawTypicalConstraintIcon(const constrIconQueueItem &i)
{
    ...
    QImage image = renderConstrIcon(i.type, color, QStringList(i.label), QList<QColor>() << color,
                                    i.iconRotation);
...
QString EditModeConstraintCoinManager::iconTypeFromConstraint(Constraint *constraint)
{
    ...
    switch (constraint->Type) {
        case Horizontal: return QString::fromLatin1("Constraint_Horizontal");
        case Vertical: return QString::fromLatin1("Constraint_Vertical");
        case PointOnObject: return QString::fromLatin1("Constraint_PointOnObject");
        case Tangent: return QString::fromLatin1("Constraint_Tangent");
        case Parallel: return QString::fromLatin1("Constraint_Parallel");
        case Perpendicular: return QString::fromLatin1("Constraint_Perpendicular");
        case Equal: return QString::fromLatin1("Constraint_EqualLength");
        case Symmetric: return QString::fromLatin1("Constraint_Symmetric");
        case SnellsLaw: return QString::fromLatin1("Constraint_SnellsLaw");
        case Block: return QString::fromLatin1("Constraint_Block");
...
QColor EditModeConstraintCoinManager::constrColor(int constraintId)
{
    auto toQColor = [](auto sbcolor) -> QColor {
        return QColor((int)(sbcolor[0] * 255.0f), (int)(sbcolor[1] * 255.0f),

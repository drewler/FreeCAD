...
#define READ_VAL(val)                                                                              \
    ...
    if (in->isBinary()) {
        ...
        // files.
        if (numtoread < 0) {
            SoReadError::post(in, "invalid number of values in field: %d", numtoread);
            return FALSE;
        }
...
SbBool SoSFMeshFacetArray::readBinaryValues(SoInput *in, unsigned long numarg)
{
    ...
    for (unsigned long i = 0; i < numarg; i++)
        if (!this->read1Value(in, i)) return FALSE;
    return TRUE;
}
...
int SoSFMeshFacetArray::getNumValuesPerLine() const { return 2; }
...
void SoFCMeshFacetElement::initClass() { SO_ELEMENT_INIT_CLASS(SoFCMeshFacetElement, inherited); }
...
void SoFCMeshFacetElement::set(SoState *const state, SoNode *const node,
                               const MeshCore::MeshFacetArray *const coords)
{
    SoFCMeshFacetElement *elem =
        (SoFCMeshFacetElement *)SoReplacedElement::getElement(state, classStackIndex, node);
    if (elem) {
...
// Doc from superclass.
void SoFCMeshFacet::GLRender(SoGLRenderAction *action) { SoFCMeshFacet::doAction(action); }

// Doc from superclass.
void SoFCMeshFacet::callback(SoCallbackAction *action) { SoFCMeshFacet::doAction(action); }

// Doc from superclass.
void SoFCMeshFacet::pick(SoPickAction *action) { SoFCMeshFacet::doAction(action); }
...
// Helper functions: draw normal
inline void glNormal(float *n) { glNormal3fv(n); }

// Helper function: convert Vec to SbVec3f
inline SbVec3f sbvec3f(const Base::Vector3f &_v) { return SbVec3f(_v.x, _v.y, _v.z); }

SO_NODE_SOURCE(SoFCMeshFaceSet);

void SoFCMeshFaceSet::initClass() { SO_NODE_INIT_CLASS(SoFCMeshFaceSet, SoShape, "Shape"); }
 ...
 */
void SoFCMeshFaceSet::createProxyModel(const MeshCore::MeshPointArray *rPoints,
                                       const MeshCore::MeshFacetArray *rFaces, SbBool simplest)
{
    ...
    if (simplest) {
        ...
        SbVec3f points[8] = {
            SbVec3f(cBox.MinX, cBox.MinY, cBox.MinZ), SbVec3f(cBox.MaxX, cBox.MinY, cBox.MinZ),
            SbVec3f(cBox.MaxX, cBox.MaxY, cBox.MinZ), SbVec3f(cBox.MinX, cBox.MaxY, cBox.MinZ),
            SbVec3f(cBox.MinX, cBox.MinY, cBox.MaxZ), SbVec3f(cBox.MaxX, cBox.MinY, cBox.MaxZ),
            SbVec3f(cBox.MaxX, cBox.MaxY, cBox.MaxZ), SbVec3f(cBox.MinX, cBox.MaxY, cBox.MaxZ)};
        ...
        for (cIter.Init(); cIter.More(); cIter.Next()) {
            if (cIter.GetCtElements() > 0) {
                ...
                if (ulX + 1 == ulMaxX
                    || (ulX + 1 < ulMaxX && cGrid.GetCtElements(ulX + 1, ulY, ulZ) == 0)) {
                ...
                if (ulY + 1 == ulMaxY
                    || (ulY + 1 < ulMaxY && cGrid.GetCtElements(ulX, ulY + 1, ulZ) == 0)) {
                ...
                if (ulZ + 1 == ulMaxZ
                    || (ulZ + 1 < ulMaxZ && cGrid.GetCtElements(ulX, ulY, ulZ + 1) == 0)) {
        ...
        unsigned int pos = 0;
        for (MeshCore::MeshPointArray::_TConstIterator cP = rPoints.begin(); cP != rPoints.end();
             ++cP)
        ...
        pos = 0;
        for (MeshCore::MeshFacetArray::_TConstIterator cF = rFacets.begin(); cF != rFacets.end();
             ++cF) {
...
void SoFCMeshFaceSet::GLRender(SoGLRenderAction *action)
{
    if (shouldGLRender(action)) {
        ...
        if (mode == false || index->size() <= this->MaximumTriangles) {
            if (mbind != OVERALL) drawFaces(coords, index, &mb, mbind, needNormals, ccw);
            else
...
SoFCMeshFaceSet::Binding SoFCMeshFaceSet::findMaterialBinding(SoState *const state) const
{
    ...
    switch (matbind) {
        ...
        case SoMaterialBindingElement::PER_VERTEX: binding = PER_VERTEX_INDEXED; break;
        case SoMaterialBindingElement::PER_VERTEX_INDEXED: binding = PER_VERTEX_INDEXED; break;
        case SoMaterialBindingElement::PER_PART:
        case SoMaterialBindingElement::PER_FACE: binding = PER_FACE_INDEXED; break;
...
void SoFCMeshFaceSet::drawFaces(const MeshCore::MeshPointArray *rPoints,
                                const MeshCore::MeshFacetArray *rFacets, SoMaterialBundle *mb,
    ...
    if (needNormals) {
        ...
        if (ccw) {
            ...
            for (MeshCore::MeshFacetArray::_TConstIterator it = rFacets->begin();
                ...
                if (perFace) mb->send(it - rFacets->begin(), TRUE);
                glNormal(n);
                if (perVertex) mb->send(it->_aulPoints[0], TRUE);
                glVertex(v0);
                if (perVertex) mb->send(it->_aulPoints[1], TRUE);
                glVertex(v1);
                if (perVertex) mb->send(it->_aulPoints[2], TRUE);
                glVertex(v2);
            }
        }
        else {
...
void SoFCMeshFaceSet::drawPoints(const MeshCore::MeshPointArray *rPoints,
                                 const MeshCore::MeshFacetArray *rFacets, SbBool needNormals,
                                 SbBool ccw) const
{
...
void SoFCMeshFaceSet::generatePrimitives(SoAction *action)
{
    ...
    if (!rPoints || rPoints->size() < 3) return;
    if (!rFacets || rPoints->size() < 1) return;
...
void SoFCMeshOpenEdgeSet::initClass() { SO_NODE_INIT_CLASS(SoFCMeshOpenEdgeSet, SoShape, "Shape"); }

SoFCMeshOpenEdgeSet::SoFCMeshOpenEdgeSet() { SO_NODE_CONSTRUCTOR(SoFCMeshOpenEdgeSet); }
...
void SoFCMeshOpenEdgeSet::getPrimitiveCount(SoGetPrimitiveCountAction *action)
{
    ...
    int ctEdges = 0;
    for (MeshCore::MeshFacetArray::_TConstIterator jt = rFaces->begin(); jt != rFaces->end();
         ++jt) {
        for (int i = 0; i < 3; i++) {
            if (jt->_aulNeighbours[i] == ULONG_MAX) { ctEdges++; }
        }
    }

...
void SoFCMeshNode::initClass() { SO_NODE_INIT_CLASS(SoFCMeshNode, SoShape, "Shape"); }
...
void SoFCMeshNode::notify(SoNotList *node) { SoShape::notify(node); }

/**
 * Sets the mesh.
 */
void SoFCMeshNode::setMesh(const Mesh::MeshObject *mesh) { _mesh = mesh; }
...
void SoFCMeshNode::GLRender(SoGLRenderAction *action)
{
    if (_mesh && shouldGLRender(action)) {
        ...
        if (mode == false || countTriangles() <= this->MaximumTriangles) drawFaces(needNormals);
        else
 ...
 */
SoDetail *SoFCMeshNode::createTriangleDetail(SoRayPickAction *action, const SoPrimitiveVertex *v1,
                                             const SoPrimitiveVertex *v2,
                                             const SoPrimitiveVertex *v3, SoPickedPoint *pp)
{
    if (this->MaximumTriangles < countTriangles()) { return 0; }
    else {
 ...
 */
unsigned int SoFCMeshNode::countTriangles() const { return (_mesh ? _mesh->countFacets() : 0); }
...
void SoFCMeshNode::write(SoWriteAction *action)
{
    ...
    if (out->getStage() == SoOutput::COUNT_REFS) { this->addWriteReference(out, FALSE); }
...
void SoFCMeshOpenEdge::initClass() { SO_NODE_INIT_CLASS(SoFCMeshOpenEdge, SoShape, "Shape"); }

SoFCMeshOpenEdge::SoFCMeshOpenEdge() : _mesh(0) { SO_NODE_CONSTRUCTOR(SoFCMeshOpenEdge); }

/**
 * Sets the mesh.
 */
void SoFCMeshOpenEdge::setMesh(const Mesh::MeshObject *mesh) { _mesh = mesh; }

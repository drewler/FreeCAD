...
CleanupHandler::CleanupHandler() : QObject(qApp)
{
...
void CleanupHandler::cleanup() { DockEvaluateMeshImp::destruct(); }
...
public:
    Private()
        : meshFeature(nullptr), view(nullptr), enableFoldsCheck(false),
          checkNonManfoldPoints(false), strictlyDegenerated(true), epsilonDegenerated(0.0f)
    {}
    ~Private() {}
    ...
    : QDialog(parent, fl), d(new Private())
{
    ...
    connect(d->ui.buttonBox, SIGNAL(helpRequested()), Gui::getMainWindow(), SLOT(whatsThis()));
    ...
    if (d->strictlyDegenerated) d->epsilonDegenerated = 0.0f;
    else
...
DlgEvaluateMeshImp::~DlgEvaluateMeshImp()
{
    ...
    for (std::map<std::string, ViewProviderMeshDefects *>::iterator it = d->vp.begin();
         it != d->vp.end(); ++it) {
        if (d->view) d->view->getViewer()->removeViewProvider(it->second);
        delete it->second;
    }
...
void DlgEvaluateMeshImp::setMesh(Mesh::Feature *m)
{
    ...
    if (doc != getDocument()) attachDocument(doc);
...
void DlgEvaluateMeshImp::on_checkOrientationButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkOrientationButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_checkNonmanifoldsButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkNonmanifoldsButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
...
void DlgEvaluateMeshImp::on_analyzeNonmanifoldsButton_clicked()
{
    if (d->meshFeature) {
        ...
        if (d->checkNonManfoldPoints) {
            ...
            ok2 = p_eval.Evaluate();
            if (!ok2) point_indices = p_eval.GetIndices();
        }
    ...
    if (d->meshFeature) {
        ...
        try {
            Gui::Command::doCommand(
                Gui::Command::App, "App.getDocument(\"%s\").getObject(\"%s\").removeNonManifolds()",
                docName, objName);

            if (d->checkNonManfoldPoints) {
                Gui::Command::doCommand(
                    Gui::Command::App,
                    "App.getDocument(\"%s\").getObject(\"%s\").removeNonManifoldPoints()", docName,
                    objName);
            }
        }
        catch (const Base::Exception &e) {
...
void DlgEvaluateMeshImp::on_checkIndicesButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkIndicesButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_checkDegenerationButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkDegenerationButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_repairDegeneratedButton_clicked()
{
    if (d->meshFeature) {
        ...
        try {
            Gui::Command::doCommand(
                Gui::Command::App, "App.getDocument(\"%s\").getObject(\"%s\").fixDegenerations(%f)",
...
void DlgEvaluateMeshImp::on_checkDuplicatedFacesButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkDuplicatedFacesButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_repairDuplicatedFacesButton_clicked()
{
    if (d->meshFeature) {
        ...
        try {
            Gui::Command::doCommand(
                Gui::Command::App,
                "App.getDocument(\"%s\").getObject(\"%s\").removeDuplicatedFacets()", docName,
                objName);
        }
        catch (const Base::Exception &e) {
...
void DlgEvaluateMeshImp::on_checkDuplicatedPointsButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkDuplicatedPointsButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_repairDuplicatedPointsButton_clicked()
{
    if (d->meshFeature) {
        ...
        try {
            Gui::Command::doCommand(
                Gui::Command::App,
                "App.getDocument(\"%s\").getObject(\"%s\").removeDuplicatedPoints()", docName,
                objName);
        }
        catch (const Base::Exception &e) {
...
void DlgEvaluateMeshImp::on_checkSelfIntersectionButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkSelfIntersectionButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_checkFoldsButton_clicked()
{
    ...
    if (it != d->vp.end()) {
        if (d->ui.checkFoldsButton->isChecked()) it->second->show();
        else
            it->second->hide();
    }
}
...
void DlgEvaluateMeshImp::on_repairFoldsButton_clicked()
{
    if (d->meshFeature) {
        ...
        try {
            Gui::Command::doCommand(
                Gui::Command::App,
                "App.getDocument(\"%s\").getObject(\"%s\").removeFoldsOnSurface()", docName,
                objName);
        }
        catch (const Base::Exception &e) {
...
void DlgEvaluateMeshImp::on_analyzeAllTogether_clicked()
{
    ...
    if (d->enableFoldsCheck) on_analyzeFoldsButton_clicked();
}
...
void DlgEvaluateMeshImp::on_repairAllTogether_clicked()
{
    if (d->meshFeature) {
        ...
        try {
            do {
                ...
                }
                {
                    ...
                    if (!eval.Evaluate()) {
                        Gui::Command::doCommand(
                            Gui::Command::App,
                            "App.getDocument(\"%s\").getObject(\"%s\").harmonizeNormals()", docName,
                            objName);
                        run = true;
                    }
                    qApp->processEvents();
                ...
                }
                {
                    ...
                    if (!rf.Evaluate() || !rp.Evaluate() || !cf.Evaluate() || !nb.Evaluate()) {
                        Gui::Command::doCommand(
                            Gui::Command::App,
                            "App.getDocument(\"%s\").getObject(\"%s\").fixIndices()", docName,
                            objName);
                        run = true;
                    }
                }
            ...
            QMessageBox::warning(this, tr("Mesh repair"),
                                 QString::fromLatin1("Unknown error occurred."));
        }
...
void DlgEvaluateMeshImp::on_buttonBox_clicked(QAbstractButton *button)
{
    ...
    if (type == QDialogButtonBox::Open) {
        ...
        Q_FOREACH (QCheckBox *cb, cbs) cb->setChecked(false);
    }
}
...
bool DockEvaluateMeshImp::hasInstance() { return _instance != nullptr; }
 ...
 */
QSize DockEvaluateMeshImp::sizeHint() const { return QSize(371, 579); }

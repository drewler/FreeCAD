...
void ViewProviderMeshCurvature::onChanged(const App::Property *prop)
{
    if (prop == &TextureMaterial) {
        ...
        pcColorMat->ambientColor.setValue(Mat.ambientColor.r, Mat.ambientColor.g,
                                          Mat.ambientColor.b);
        pcColorMat->specularColor.setValue(Mat.specularColor.r, Mat.specularColor.g,
                                           Mat.specularColor.b);
        pcColorMat->emissiveColor.setValue(Mat.emissiveColor.r, Mat.emissiveColor.g,
                                           Mat.emissiveColor.b);
...
void ViewProviderMeshCurvature::init(const Mesh::PropertyCurvatureList *pCurvInfo)
{
    ...
    for (std::vector<Mesh::CurvatureInfo>::const_iterator jt = fCurvInfo.begin();
         jt != fCurvInfo.end(); ++jt) {
    ...
    // histogram over all values
    aHistogram.clear();
    for (std::vector<float>::const_iterator it2 = aMaxValues.begin(); it2 != aMaxValues.end();
         ++it2) {
...
SoSeparator *ViewProviderMeshCurvature::getFrontRoot() const { return pcColorRoot; }
...
void ViewProviderMeshCurvature::touchShapeNode()
{
    ...
    if (selectionPath) { selectionPath->getTail()->touch(); }
}
...
{
public:
    Annotation(Gui::ViewProviderDocumentObject *vp, const QString &s, const SbVec3f &p,
               const SbVec3f &n)
        : vp(vp), s(s), p(p), n(n)
    {}
    ~Annotation() {}
    ...
    void show()
    {
        ...
        for (std::vector<App::DocumentObject *>::iterator it = groups.begin(); it != groups.end();
             ++it) {
...
std::string ViewProviderMeshCurvature::curvatureInfo(bool detail, int index1, int index2,
                                                     int index3) const
{
    // get the curvature info of the three points of the picked facet
    ...
    if (prop && prop->getTypeId() == Mesh::PropertyCurvatureList::getClassTypeId()) {
        ...
        if (print) {
            if (!detail) { str << mode << ": <" << fVal1 << ", " << fVal2 << ", " << fVal3 << ">"; }
            else {

...
CmdMeshUnion::CmdMeshUnion() : Command("Mesh_Union")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshUnion::activated(int) { return; }
...
CmdMeshDifference::CmdMeshDifference() : Command("Mesh_Difference")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshDifference::activated(int) { return; }
...
CmdMeshIntersection::CmdMeshIntersection() : Command("Mesh_Intersection")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshIntersection::activated(int) { return; }
...
CmdMeshImport::CmdMeshImport() : Command("Mesh_Import")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshImport::activated(int)
{
    ...
    QStringList fn =
        Gui::FileDialog::getOpenFileNames(Gui::getMainWindow(), QObject::tr("Import mesh"),
...
bool CmdMeshImport::isActive() { return (getActiveGuiDocument() ? true : false); }

//--------------------------------------------------------------------------------------

DEF_STD_CMD_A(CmdMeshExport)

CmdMeshExport::CmdMeshExport() : Command("Mesh_Export")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshExport::activated(int)
{
    ...
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.stl)").arg(QObject::tr("Binary STL")), "STL");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.stl)").arg(QObject::tr("ASCII STL")), "AST");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.ast)").arg(QObject::tr("ASCII STL")), "AST");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.bms)").arg(QObject::tr("Binary Mesh")), "BMS");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.obj)").arg(QObject::tr("Alias Mesh")), "OBJ");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.smf)").arg(QObject::tr("Simple Model Format")), "SMF");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.off)").arg(QObject::tr("Object File Format")), "OFF");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.iv)").arg(QObject::tr("Inventor V2.1 ascii")), "IV");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.x3d)").arg(QObject::tr("X3D Extensible 3D")), "X3D");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.x3dz)").arg(QObject::tr("Compressed X3D")), "X3DZ");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.xhtml)").arg(QObject::tr("WebGL/X3D")), "X3DOM");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.ply)").arg(QObject::tr("Stanford Polygon")), "PLY");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.wrl *.vrml)").arg(QObject::tr("VRML V2.0")), "VRML");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.wrz)").arg(QObject::tr("Compressed VRML 2.0")), "WRZ");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.nas *.bdf)").arg(QObject::tr("Nastran")), "NAS");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.py)").arg(QObject::tr("Python module def")), "PY");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.asy)").arg(QObject::tr("Asymptote Format")), "ASY");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.3mf)").arg(QObject::tr("3D Manufacturing Format")), "3MF");
    ext << qMakePair<QString, QByteArray>(
        QString::fromLatin1("%1 (*.*)").arg(QObject::tr("All Files")), ""); // Undefined
    QStringList filter;
    for (QList<QPair<QString, QByteArray>>::iterator it = ext.begin(); it != ext.end(); ++it)
        filter << it->first;

    QString format;
    QString fn = Gui::FileDialog::getSaveFileName(Gui::getMainWindow(), QObject::tr("Export mesh"),
                                                  dir, filter.join(QLatin1String(";;")), &format);
    if (!fn.isEmpty()) {
        QFileInfo fi(fn);
        ...
        if (vp) { vp->exportMesh((const char *)fn.toUtf8(), (const char *)extension); }
    }
}
...
CmdMeshFromGeometry::CmdMeshFromGeometry() : Command("Mesh_FromGeometry")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshFromGeometry::activated(int)
{
    ...
    double tol = QInputDialog::getDouble(Gui::getMainWindow(), QObject::tr("Meshing Tolerance"),
                                         QObject::tr("Enter tolerance for meshing geometry:"), 0.1,
                                         0.01, 10.0, 2, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (!ok) return;
    ...
    for (std::vector<App::DocumentObject *>::iterator it = geo.begin(); it != geo.end(); ++it) {
        if (!(*it)->getTypeId().isDerivedFrom(Mesh::Feature::getClassTypeId())) {
            ...
            for (std::map<std::string, App::Property *>::iterator jt = Map.begin(); jt != Map.end();
                 ++jt) {
                if (jt->first == "Shape"
...
CmdMeshFromPartShape::CmdMeshFromPartShape() : Command("Mesh_FromPartShape")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshVertexCurvature::CmdMeshVertexCurvature() : Command("Mesh_VertexCurvature")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshVertexCurvature::activated(int)
{
    ...
    for (std::vector<App::DocumentObject *>::const_iterator it = meshes.begin(); it != meshes.end();
        ...
        if (grp)
            doCommand(
                Doc, "App.activeDocument().getObject(\"%s\").newObject(\"Mesh::Curvature\",\"%s\")",
                grp->getNameInDocument(), fName.c_str());
        else
            doCommand(Doc, "App.activeDocument().addObject(\"Mesh::Curvature\",\"%s\")",
                      fName.c_str());
...
CmdMeshVertexCurvatureInfo::CmdMeshVertexCurvatureInfo() : Command("Mesh_CurvatureInfo")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
{
    App::Document *doc = App::GetApplication().getActiveDocument();
    if (!doc || doc->countObjectsOfType(Mesh::Curvature::getClassTypeId()) == 0) return false;
...
CmdMeshPolySegm::CmdMeshPolySegm() : Command("Mesh_PolySegm")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshPolySegm::activated(int)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = docObj.begin(); it != docObj.end();
        ...
        if (pVP->isVisible()) pVP->startEditing();
    }
}
...
{
    // Check for the selected mesh feature (all Mesh types)
    if (getSelection().countObjectsOfType(Mesh::Feature::getClassTypeId()) == 0) return false;
...
CmdMeshAddFacet::CmdMeshAddFacet() : Command("Mesh_AddFacet")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
{
    // Check for the selected mesh feature (all Mesh types)
    if (getSelection().countObjectsOfType(Mesh::Feature::getClassTypeId()) != 1) return false;
...
CmdMeshPolyCut::CmdMeshPolyCut() : Command("Mesh_PolyCut")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshPolyTrim::CmdMeshPolyTrim() : Command("Mesh_PolyTrim")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshTrimByPlane::CmdMeshTrimByPlane() : Command("Mesh_TrimByPlane")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshSectionByPlane::CmdMeshSectionByPlane() : Command("Mesh_SectionByPlane")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshCrossSections::CmdMeshCrossSections() : Command("Mesh_CrossSections")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshPolySplit::CmdMeshPolySplit() : Command("Mesh_PolySplit")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshEvaluation::CmdMeshEvaluation() : Command("Mesh_Evaluation")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
{
    App::Document *doc = App::GetApplication().getActiveDocument();
    if (!doc || doc->countObjectsOfType(Mesh::Feature::getClassTypeId()) == 0) return false;
    return true;
}
...
CmdMeshEvaluateFacet::CmdMeshEvaluateFacet() : Command("Mesh_EvaluateFacet")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshRemoveComponents::CmdMeshRemoveComponents() : Command("Mesh_RemoveComponents")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
bool CmdMeshRemoveComponents::isActive()
{
    ...
    if (view) {
        Gui::View3DInventorViewer *viewer = view->getViewer();
        if (viewer->isEditing()) return false;
    }
...
CmdMeshRemeshGmsh::CmdMeshRemeshGmsh() : Command("Mesh_RemeshGmsh")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshRemoveCompByHand::CmdMeshRemoveCompByHand() : Command("Mesh_RemoveCompByHand")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshEvaluateSolid::CmdMeshEvaluateSolid() : Command("Mesh_EvaluateSolid")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshSmoothing::CmdMeshSmoothing() : Command("Mesh_Smoothing")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshDecimating::CmdMeshDecimating() : Command("Mesh_Decimating")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshDecimating::activated(int) { Gui::Control().showDialog(new MeshGui::TaskDecimating()); }
...
CmdMeshHarmonizeNormals::CmdMeshHarmonizeNormals() : Command("Mesh_HarmonizeNormals")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshFlipNormals::CmdMeshFlipNormals() : Command("Mesh_FlipNormals")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshBoundingBox::CmdMeshBoundingBox() : Command("Mesh_BoundingBox")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshBuildRegularSolid::CmdMeshBuildRegularSolid() : Command("Mesh_BuildRegularSolid")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshBuildRegularSolid::activated(int)
{
    ...
    if (!dlg) dlg = new MeshGui::DlgRegularSolidImp(Gui::getMainWindow());
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->show();
}
...
CmdMeshFillupHoles::CmdMeshFillupHoles() : Command("Mesh_FillupHoles")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshFillupHoles::activated(int)
{
    ...
    int FillupHolesOfLength =
        QInputDialog::getInt(Gui::getMainWindow(), QObject::tr("Fill holes"),
                             QObject::tr("Fill holes with maximum number of edges:"), 3, 3, 10000,
...
CmdMeshFillInteractiveHole::CmdMeshFillInteractiveHole() : Command("Mesh_FillInteractiveHole")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshSegmentation::CmdMeshSegmentation() : Command("Mesh_Segmentation")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshSegmentation::activated(int)
{
    ...
    Gui::TaskView::TaskDialog *dlg = Gui::Control().activeDialog();
    if (!dlg) { dlg = new MeshGui::TaskSegmentation(mesh); }
    Gui::Control().showDialog(dlg);
}
...
CmdMeshSegmentationBestFit::CmdMeshSegmentationBestFit() : Command("Mesh_SegmentationBestFit")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshSegmentationBestFit::activated(int)
{
    ...
    Mesh::Feature *mesh = static_cast<Mesh::Feature *>(objs.front());
    Gui::TaskView::TaskDialog *dlg = Gui::Control().activeDialog();
    if (!dlg) { dlg = new MeshGui::TaskSegmentationBestFit(mesh); }
    Gui::Control().showDialog(dlg);
}
...
CmdMeshMerge::CmdMeshMerge() : Command("Mesh_Merge")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshMerge::activated(int)
{
    ...
    std::vector<App::DocumentObject *> objs =
        Gui::Selection().getObjectsOfType(Mesh::Feature::getClassTypeId());
    for (std::vector<App::DocumentObject *>::const_iterator it = objs.begin(); it != objs.end();
         ++it) {
...
CmdMeshSplitComponents::CmdMeshSplitComponents() : Command("Mesh_SplitComponents")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
CmdMeshScale::CmdMeshScale() : Command("Mesh_Scale")
{
    sAppModule = "Mesh";
    sGroup = QT_TR_NOOP("Mesh");
...
void CmdMeshScale::activated(int)
{
    ...
    double factor = QInputDialog::getDouble(Gui::getMainWindow(), QObject::tr("Scaling"),
                                            QObject::tr("Enter scaling factor:"), 1, 0, DBL_MAX, 5,
                                            &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (!ok || factor == 0) return;

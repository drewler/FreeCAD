...
public:
    ...
    void generateGLArrays(SoGLRenderAction *action, SoMaterialBindingElement::Binding matbind,
...
MeshRenderer::Private::Private()
    : vertices(GL_ARRAY_BUFFER), indices(GL_ELEMENT_ARRAY_BUFFER), pcolors(nullptr),
      matbinding(SoMaterialBindingElement::OVERALL), initialized(false)
{}
...
{
    if (vertex.empty() || index.empty()) return;
...
public:
    ...
    Private() : pcolors(0), matbinding(SoMaterialBindingElement::OVERALL) {}
...
bool MeshRenderer::Private::canRenderGLArray(SoGLRenderAction *) const { return true; }
...
public:
    ...
    bool canRenderGLArray(SoGLRenderAction *) const { return false; }
    void generateGLArrays(SoGLRenderAction *, SoMaterialBindingElement::Binding,
...
MeshRenderer::MeshRenderer() : p(new Private) {}

MeshRenderer::~MeshRenderer() { delete p; }

void MeshRenderer::update() { p->update(); }

bool MeshRenderer::needUpdate(SoGLRenderAction *action) { return p->needUpdate(action); }
                                    ...
                                    std::vector<float> &vertex, std::vector<int32_t> &index)
{
    ...
    if (gl) { p->pcolors = gl->getDiffusePointer(); }
...
//
void MeshRenderer::renderCoordsGLArray(SoGLRenderAction *action) { p->renderCoordsGLArray(action); }
...
void MeshRenderer::renderFacesGLArray(SoGLRenderAction *action) { p->renderFacesGLArray(action); }
...
bool MeshRenderer::matchMaterial(SoState *state) const
{
    ...
    if (p->matbinding != matbind) return false;
    // the buffer doesn't contain color information
    if (matbind == SoMaterialBindingElement::OVERALL) return true;
    const SbColor *pcolors = nullptr;
    SoGLLazyElement *gl = SoGLLazyElement::getInstance(state);
    if (gl) { pcolors = gl->getDiffusePointer(); }
...
SoFCIndexedFaceSet::SoFCIndexedFaceSet() : renderTriangleLimit(UINT_MAX), selectBuf(nullptr)
{
...
void SoFCIndexedFaceSet::GLRender(SoGLRenderAction *action)
{
    ...
    if (!render.canRenderGLArray(action)) useVBO = false;
...
void SoFCIndexedFaceSet::drawCoords(const SoGLCoordinateElement *const vertexlist,
                                    const int32_t *vertexindices, int numindices,
                                    const SbVec3f *normals, const int32_t *normalindices,
                                    SoMaterialBundle *materials, const int32_t * /*matindices*/,
                                    ...
                                    const int32_t * /*texindices*/)
{
    ...
    switch (binding) {
        case SoMaterialBindingElement::PER_FACE: per_face = true; break;
        case SoMaterialBindingElement::PER_VERTEX: per_vert = true; break;
        default: break;
    }
    ...
    for (int index = 0; index < numindices; ct++) {
        if (ct % mod == 0) {
            if (per_face) materials->send(ct, true);
            v1 = *viptr++;
            index++;
            if (per_vert) materials->send(v1, true);
            if (normals) currnormal = &normals[*normalindices++];
            glNormal3fv((const GLfloat *)currnormal);
            glVertex3fv((const GLfloat *)(coords3d + v1));

            v2 = *viptr++;
            index++;
            if (per_vert) materials->send(v2, true);
            if (normals) currnormal = &normals[*normalindices++];
            glNormal3fv((const GLfloat *)currnormal);
            ...
            v3 = *viptr++;
            index++;
            if (per_vert) materials->send(v3, true);
            if (normals) currnormal = &normals[*normalindices++];
            glNormal3fv((const GLfloat *)currnormal);
...
void SoFCIndexedFaceSet::invalidate() { updateGLArray.setValue(true); }
...
void SoFCIndexedFaceSet::doAction(SoAction *action)
{
    if (action->getTypeId() == Gui::SoGLSelectAction::getClassTypeId()) {
        ...
        if (!(coords && coords->getTypeId().isDerivedFrom(SoCoordinate3::getClassTypeId()))) return;

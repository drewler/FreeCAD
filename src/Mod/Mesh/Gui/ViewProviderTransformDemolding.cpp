...
void ViewProviderMeshTransformDemolding::attach(App::DocumentObject *pcFeat)
{
    ...
    center =
        static_cast<Feature *>(pcObject)->Mesh.getValue().getKernel().GetBoundBox().GetCenter();
...
{
    if (strcmp("Demold", ModeName) == 0) setDisplayMaskMode("Demold");
    ViewProviderMesh::setDisplayMode(ModeName);
}

const char *ViewProviderMeshTransformDemolding::getDefaultDisplayMode() const { return "Demold"; }

                                         ...
                                         std::vector<SoNode *> &nodes) const
{
    ...
    if (pcPointsCoord && pcFaces) createMesh(prop, pcPointsCoord, pcFaces);
}

void ViewProviderMeshBuilder::createMesh(const App::Property *prop, SoCoordinate3 *coords,
...
void ViewProviderMeshBuilder::createMesh(const MeshCore::MeshKernel &kernel, SoCoordinate3 *coords,
                                         SoIndexedFaceSet *faces) const
{
    ...
    for (MeshCore::MeshFacetArray::_TConstIterator it = cF.begin(); it != cF.end(); ++it, j++) {
        for (int i = 0; i < 3; i++) { indices[4 * j + i] = it->_aulPoints[i]; }
...
const char *ViewProviderExport::getDefaultDisplayMode() const { return ""; }

QIcon ViewProviderExport::getIcon() const
{
    const char *Mesh_Feature_xpm[] = {"22 22 6 1",
                                      ". c None",
                                      "# c #000000",
                                      "c c #ffff00",
                                      "a c #808080",
...
ViewProviderMesh::ViewProviderMesh() : pcOpenEdge(nullptr)
{
    ...
    if (hGrp->GetBool("ShowBoundingBox", false)) { SelectionStyle.setValue(1); }
...
void ViewProviderMesh::onChanged(const App::Property *prop)
{
    ...
    if (prop == &LineTransparency) {
        ...
        if (prop == &ShapeColor) { setOpenEdgeColorFrom(ShapeColor.getValue()); }
...
SoShape *ViewProviderMesh::getShapeNode() const { return nullptr; }

SoNode *ViewProviderMesh::getCoordNode() const { return nullptr; }
...
void ViewProviderMesh::attach(App::DocumentObject *pcFeat)
{
    ...
    if (getColorProperty()) { Coloring.setStatus(App::Property::Hidden, false); }
}
...
void ViewProviderMesh::tryColorPerVertexOrFace(bool on)
{
    if (on) {
        ...
        if (colors) {
            ...
            if (colors->getSize() == numPoints) { setColorPerVertex(colors); }
...
{
    if (strcmp("Shaded", ModeName) == 0) { setDisplayMaskMode("Shaded"); }
...
bool ViewProviderMesh::exportToVrml(const char *filename, const MeshCore::Material &mat,
                                    bool binary) const
{
...
void ViewProviderMesh::exportMesh(const char *filename, const char *fmt) const
{
    ...
    if (mat.diffuseColor.size() == mesh.countPoints()) mat.binding = MeshCore::MeshIO::PER_VERTEX;
                                      ...
                                      std::vector<MeshCore::MeshGeomFacet> &aFaces)
{
    ...
    for (std::vector<SbVec2f>::const_iterator it = rclPoly.begin(); it != rclPoly.end(); ++it) {
        ...
        if (nt == rclPoly.end()) nt = rclPoly.begin();
        else if (*it == *nt)
        ...
        if (face.Area() > 0) aFaces.push_back(face);
...
void ViewProviderMesh::showOpenEdges(bool show) { (void)show; }
...
{
public:
    MeshSplit(ViewProviderMesh *mesh, const std::vector<SbVec2f> &poly,
              const Gui::ViewVolumeProjection &proj)
        : mesh(mesh), poly(poly), proj(proj)
    {}
    ~MeshSplit() {}
    void cutMesh()
    {
...
void ViewProviderMesh::clipMeshCallback(void *ud, SoEventCallback *n)
{
    ...
    std::vector<Gui::ViewProvider *> views =
        view->getDocument()->getViewProvidersOfType(ViewProviderMesh::getClassTypeId());
    if (!views.empty()) {
        Gui::Application::Instance->activeDocument()->openCommand(
            QT_TRANSLATE_NOOP("Command", "Cut"));
        bool commitCommand = false;
        for (std::vector<Gui::ViewProvider *>::iterator it = views.begin(); it != views.end();
             ++it) {
        ...
        if (commitCommand) Gui::Application::Instance->activeDocument()->commitCommand();
        else
    ...
    try {
        ...
        for (std::vector<Gui::ViewProvider *>::iterator it = views.begin(); it != views.end();
            ...
            if (that->getEditingMode() > -1) {
                ...
                if (role == Gui::SelectionRole::Inner) that->splitMesh(copyToolMesh, cNormal, true);
                else
...
void ViewProviderMesh::selectGLCallback(void *ud, SoEventCallback *n)
{
    ...
    for (std::vector<Gui::ViewProvider *>::iterator it = views.begin(); it != views.end(); ++it) {
        ...
        if (that->getEditingMode() > -1) {
            that->finishEditing();
            that->selectArea(x, y, w, h, view->getSoRenderManager()->getViewportRegion(),
...
ViewProviderMesh::getVisibleFacetsAfterZoom(const SbBox2s &rect, const SbViewportRegion &vp,
                                            SoCamera *camera) const
{
...
{
public:
    Vertex(const MeshCore::MeshKernel &kernel, const MeshCore::MeshFacetGrid &grid,
           const Base::Vector3f &pos)
        : kernel(kernel), grid(grid), pos(pos)
    {}
...
void ViewProviderMesh::cutMesh(const std::vector<SbVec2f> &picked, const Base::ViewProjMethod &proj,
                               SbBool inner)
{
...
void ViewProviderMesh::splitMesh(const MeshCore::MeshKernel &toolMesh, const Base::Vector3f &normal,
                                 SbBool clip_inner)
{
    // Get the attached mesh property
    Mesh::PropertyMeshKernel &meshProp = static_cast<Mesh::Feature *>(pcObject)->Mesh;
    ...
    Mesh::Feature *splitMesh =
        static_cast<Mesh::Feature *>(App::GetApplication().getActiveDocument()->addObject(
            "Mesh::Feature", pcObject->getNameInDocument()));
...
void ViewProviderMesh::segmentMesh(const MeshCore::MeshKernel &toolMesh,
                                   const Base::Vector3f &normal, SbBool clip_inner)
{
    // Get the attached mesh property
    Mesh::PropertyMeshKernel &meshProp = static_cast<Mesh::Feature *>(pcObject)->Mesh;
    const MeshCore::MeshKernel &meshPropKernel = meshProp.getValue().getKernel();
...
void ViewProviderMesh::faceInfoCallback(void *ud, SoEventCallback *n)
{
    ...
    if (mbe->getButton() == SoMouseButtonEvent::BUTTON2 && mbe->getState() == SoButtonEvent::UP) {
        ...
        if (cl == id) {
            ...
            for (std::list<Gui::GLGraphicsItem *>::iterator it = glItems.begin();
                 it != glItems.end(); ++it) {
        ...
        Gui::ViewProvider *vp =
            view->getDocument()->getViewProviderByPathFromTail(point->getPath());
        if (!vp || !vp->getTypeId().isDerivedFrom(ViewProviderMesh::getClassTypeId())) return;
...
void ViewProviderMesh::markPartCallback(void *ud, SoEventCallback *n)
{
    ...
    if (n->getEvent()->isOfType(SoMouseButtonEvent::getClassTypeId())) {
        ...
        Gui::View3DInventorViewer *view =
            static_cast<Gui::View3DInventorViewer *>(n->getUserData());
        ...
        if (mbe->getButton() == SoMouseButtonEvent::BUTTON2
            ...
            if (cl == id) {
                ...
                std::vector<ViewProvider *> views =
                    view->getDocument()->getViewProvidersOfType(ViewProviderMesh::getClassTypeId());
                for (std::vector<ViewProvider *>::iterator it = views.begin(); it != views.end();
                     ++it) {
...
void ViewProviderMesh::faceInfo(Mesh::FacetIndex uFacet)
{
    ...
    if (uFacet < facets.size()) {
        ...
        Base::Console().Message(
            ...
            fea->getNameInDocument(), uFacet, face._aulPoints[0], face._aulPoints[1],
            face._aulPoints[2], face._aulNeighbours[0], face._aulNeighbours[1],
            face._aulNeighbours[2], tria._aclPoints[0].x, tria._aclPoints[0].y,
            tria._aclPoints[0].z, tria._aclPoints[1].x, tria._aclPoints[1].y, tria._aclPoints[1].z,
...
void ViewProviderMesh::fillHole(Mesh::FacetIndex uFacet)
{
    ...
    for (std::list<std::vector<Mesh::PointIndex>>::iterator it = boundaries.begin();
         it != boundaries.end(); ++it) {
        if (it->size() < 3 /* || it->size() > 200*/) continue;
        boundary = *it;
        ...
        if (meshAlg.FillupHole(boundary, cTria, faces, points, level, &cPt2Fac)) {
            if (boundary.front() == boundary.back()) boundary.pop_back();
    ...
    if (newFacets.empty()) return; // nothing to do
    ...
    if (prop && prop->getSize() == static_cast<int>(kernel->countPoints())) {
        ...
        if (invalid > 0) {
            ...
            for (std::size_t index = 0; index < numPoints; index++) {
                if (pointDegree[index] > 0) { valid_colors.push_back(colors[index]); }
            }
        ...
        for (auto it : facets) validFacets[it] = false;
        ...
        for (std::size_t index = 0; index < numColors; index++) {
            if (validFacets[index]) valid_colors.push_back(colors[index]);
        }
...
void ViewProviderMesh::selectFacet(Mesh::FacetIndex facet)
{
    ...
    if (uCtFacets != pcShapeMaterial->diffuseColor.getNum()) { highlightSelection(); }
    else {
...
void ViewProviderMesh::deselectComponent(Mesh::FacetIndex uFacet)
{
    ...
    // Colorize the selection
    if (rMesh.hasSelectedFacets()) highlightSelection();
    else
        unhighlightSelection();
}
...
void ViewProviderMesh::setSelection(const std::vector<Mesh::FacetIndex> &indices)
{
    ...
    // Colorize the selection
    if (indices.empty()) unhighlightSelection();
    else
        highlightSelection();
}
...
void ViewProviderMesh::invertSelection()
{
    ...
    for (MeshCore::MeshFacetArray::_TConstIterator jt = beg; jt != end; ++jt) {
        if (!jt->IsFlag(MeshCore::MeshFacet::SELECTED)) notselect.push_back(jt - beg);
    }
...
void ViewProviderMesh::selectArea(short x, short y, short w, short h,
                                  const SbViewportRegion &region, SoCamera *camera)
{
    SbViewportRegion vp;
...
void ViewProviderMesh::highlightSelection()
{
    ...
    SbColor *cols = pcShapeMaterial->diffuseColor.startEditing();
    for (int i = 0; i < uCtFacets; i++) cols[i].setValue(c.r, c.g, c.b);
    for (std::vector<Mesh::FacetIndex>::iterator it = selection.begin(); it != selection.end();
         ++it)
...
void ViewProviderMesh::highlightComponents()
{
    ...
    SbColor *cols = pcShapeMaterial->diffuseColor.startEditing();
    for (std::vector<std::vector<Mesh::FacetIndex>>::iterator it = comps.begin(); it != comps.end();
         ++it) {
...
void ViewProviderMesh::highlightSegments()
{
    ...
    for (unsigned long i = 0; i < numSegm; i++) {
        App::Color col;
        if (col.fromHexString(rMesh.getSegment(i).getColor())) colors[i] = col;
    }
...
void ViewProviderMesh::highlightSegments(const std::vector<App::Color> &colors)
{
    ...
    if (numSegm > 0 && numSegm == colors.size()) {
        ...
        for (unsigned long i = 0; i < numSegm; i++) {
            ...
            for (std::vector<Mesh::FacetIndex>::iterator it = segm.begin(); it != segm.end();
                 ++it) {
...
void ViewProviderMesh::highlightColors()
{
    const Mesh::MeshObject &rMesh = static_cast<Mesh::Feature *>(pcObject)->Mesh.getValue();
    {
        ...
        if (prop && prop->getSize() == int(rMesh.countFacets())) { setColorPerFace(prop); }
    }
    {
        App::PropertyColorList *prop = Base::freecad_dynamic_cast<App::PropertyColorList>(
            pcObject->getPropertyByName("VertexColors"));
        if (prop && prop->getSize() == int(rMesh.countPoints())) { setColorPerVertex(prop); }
    }
}
...
bool ViewProviderMesh::canHighlightColors() const
{
    const Mesh::MeshObject &rMesh = static_cast<Mesh::Feature *>(pcObject)->Mesh.getValue();
    {
        App::PropertyColorList *prop = Base::freecad_dynamic_cast<App::PropertyColorList>(
            pcObject->getPropertyByName("FaceColors"));
        if (prop && prop->getSize() == int(rMesh.countFacets())) return true;
    }
    {
        App::PropertyColorList *prop = Base::freecad_dynamic_cast<App::PropertyColorList>(
            pcObject->getPropertyByName("VertexColors"));
        if (prop && prop->getSize() == int(rMesh.countPoints())) return true;
    }
...
{
    if (!pyViewObject) pyViewObject = new ViewProviderMeshPy(this);
    pyViewObject->IncRef();
    return pyViewObject;
}
...
SoShape *ViewProviderIndexedFaceSet::getShapeNode() const { return this->pcMeshFaces; }

SoNode *ViewProviderIndexedFaceSet::getCoordNode() const { return this->pcMeshCoord; }
...
SoShape *ViewProviderMeshObject::getShapeNode() const { return this->pcMeshShape; }

SoNode *ViewProviderMeshObject::getCoordNode() const { return this->pcMeshNode; }

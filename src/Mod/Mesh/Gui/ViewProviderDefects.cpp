...
{
    if (prop == &LineWidth) { pcDrawStyle->lineWidth = LineWidth.getValue(); }
...
ViewProviderMeshOrientation::~ViewProviderMeshOrientation() { pcFaces->unref(); }
...
ViewProviderMeshNonManifolds::~ViewProviderMeshNonManifolds() { pcLines->unref(); }
...
ViewProviderMeshNonManifoldPoints::~ViewProviderMeshNonManifoldPoints() { pcPoints->unref(); }
...
ViewProviderMeshDuplicatedFaces::~ViewProviderMeshDuplicatedFaces() { pcFaces->unref(); }
...
ViewProviderMeshDuplicatedPoints::~ViewProviderMeshDuplicatedPoints() { pcPoints->unref(); }
...
ViewProviderMeshDegenerations::~ViewProviderMeshDegenerations() { pcLines->unref(); }
    ...
    for (std::vector<Mesh::ElementIndex>::const_iterator it = inds.begin(); it != inds.end();
        ...
        if (rE0 == rE1 && rE0 == rE2) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (cVec1 * cVec2 < 0.0f) {
                    pcCoords->point.set1Value(i++, cF->_aclPoints[(j + 1) % 3].x,
                                              cF->_aclPoints[(j + 1) % 3].y,
                                              cF->_aclPoints[(j + 1) % 3].z);
                    pcCoords->point.set1Value(i++, cF->_aclPoints[(j + 2) % 3].x,
...
ViewProviderMeshIndices::~ViewProviderMeshIndices() { pcFaces->unref(); }
...
ViewProviderMeshSelfIntersections::~ViewProviderMeshSelfIntersections() { pcLines->unref(); }
...
void ViewProviderMeshSelfIntersections::showDefects(const std::vector<Mesh::ElementIndex> &indices)
{
    ...
    int i = 0;
    int j = 0;
    for (std::vector<std::pair<Base::Vector3f, Base::Vector3f>>::const_iterator it = lines.begin();
         it != lines.end(); ++it) {
...
ViewProviderMeshFolds::~ViewProviderMeshFolds() { pcFaces->unref(); }

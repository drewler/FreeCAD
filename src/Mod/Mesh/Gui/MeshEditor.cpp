...
{
    if (strcmp(ModeName, "Face") == 0) setDisplayMaskMode("Face");
    else if (strcmp(ModeName, "Marker") == 0)
        setDisplayMaskMode("Marker");
    ViewProviderDocumentObject::setDisplayMode(ModeName);
}

const char *ViewProviderFace::getDefaultDisplayMode() const { return "Marker"; }
...
MeshFaceAddition::~MeshFaceAddition() { delete faceView; }
...
void MeshFaceAddition::finishEditing()
{
    ...
    viewer->removeEventCallback(SoEvent::getClassTypeId(), MeshFaceAddition::addFacetCallback,
                                this);
    this->deleteLater();
}
...
{
    if (faceView->index.size() < 3) return;
...
{
    if (faceView->current_index < 0) return false;
    if (faceView->index.size() >= 3) return false;
    faceView->index.push_back(faceView->current_index);
    faceView->current_index = -1;
    if (faceView->index.size() == 3) faceView->setDisplayMode("Face");
    return true;
}
...
void MeshFaceAddition::showMarker(SoPickedPoint *pp)
{
    ...
    if (detail) {
        if (detail->isOfType(SoFaceDetail::getClassTypeId())) {
            ...
            if (face_index >= (int)facets.size()) return;
            ...
            if (point_index < 0) return; // picked point is rejected

            int num = faceView->pcCoords->point.getNum();
            if (faceView->current_index >= 0) { num = std::max<int>(num - 1, 0); }
...
void MeshFaceAddition::addFacetCallback(void *ud, SoEventCallback *n)
{
    ...
    if (!view->isRedirectedToSceneGraph()) {
        if (!ev->getTypeId().isDerivedFrom(SoKeyboardEvent::getClassTypeId())) { return; }
    }
    if (ev->getTypeId() == SoLocation2Event::getClassTypeId()) {
        ...
        if (mbe->getButton() == SoMouseButtonEvent::BUTTON1
            ...
            if (face->index.size() == 3) {
                ...
                if (act == add) { QTimer::singleShot(300, that, SLOT(addFace())); }
                else if (act == swp) {
            ...
            QAction *act = menu.exec(QCursor::pos());
            if (act == fin) { QTimer::singleShot(300, that, SLOT(finishEditing())); }
        }
    }
...
MeshFillHole::MeshFillHole(MeshHoleFiller &hf, Gui::View3DInventor *parent)
    : QObject(parent), myMesh(nullptr), myNumPoints(0), myVertex1(0), myVertex2(0), myHoleFiller(hf)
{
...
void MeshFillHole::closeBridge()
{
    ...
    if (it != myPolygon.end() && jt != myPolygon.end()) {
        ...
        if (loop2.empty()) bounds.push_back(loop1);
...
void MeshFillHole::createPolygons()
{
    ...
    int32_t count = 0;
    for (std::list<std::vector<Mesh::PointIndex>>::iterator it = borders.begin();
         it != borders.end(); ++it) {
        if (it->front() == it->back()) it->pop_back();
        count += it->size();
    }
    ...
    for (std::list<std::vector<Mesh::PointIndex>>::iterator it = borders.begin();
        ...
        for (std::vector<Mesh::PointIndex>::iterator jt = it->begin(); jt != it->end(); ++jt) {
            p_iter.Set(*jt);
...
void MeshFillHole::fileHoleCallback(void *ud, SoEventCallback *n)
{
    ...
    if (ev->getTypeId() == SoLocation2Event::getClassTypeId()) {
        ...
        rp.setPickAll(true);
        if (self->myNumPoints == 0) rp.apply(self->myBoundariesRoot);
        else
        ...
        if (node) {
            ...
            if (it != self->myPolygons.end()) {
                ...
                if (minDist < 1.0f) {
                    if (self->myNumPoints == 0) self->myVertex->point.set1Value(0, closestPoint);
                    else
        ...
        if (mbe->getButton() == SoMouseButtonEvent::BUTTON1
            ...
            if (node) {
                ...
                if (it != self->myPolygons.end()) {
                    ...
                    Mesh::PointIndex vertex_index;
                    SbVec3f closestPoint;
                    float minDist = self->findClosestPoint(rp.getLine(), it->second, vertex_index,
                                                           closestPoint);
                    if (minDist < 1.0f) {
            ...
            QAction *act = menu.exec(QCursor::pos());
            if (act == fin) { QTimer::singleShot(300, self, SLOT(finishEditing())); }
        }
    }
}

...
public:
     ...
     */
    void apply(const Mesh::MeshObject &mesh, const App::Color &defaultColor,
    ...
    /*!
     ...
     original material is used.
     */
    void apply(const Mesh::MeshObject &mesh, const App::Color &defaultColor, float max_dist,
               MeshCore::Material &material);
    /*!
...
private:
    void apply(const Mesh::MeshObject &mesh, bool addDefaultColor, const App::Color &defaultColor,
               float max_dist, MeshCore::Material &material);
    PointIndex findIndex(const Base::Vector3f &p, float max_dist) const
    {
        if (max_dist < 0.0f) { return kdTree->FindExact(p); }
        else {
            Base::Vector3f n;
            float dist;

...
std::string MeshPy::representation() const { return getMeshObjectPtr()->representation(); }
...
PyObject *MeshPy::read(PyObject *args, PyObject *kwds)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "Os", keywords_stream, &input, &Ext)) {
        std::string fmt(Ext);
        boost::to_upper(fmt);
        if (ext.find(fmt) != ext.end()) { format = ext[fmt]; }
...
PyObject *MeshPy::write(PyObject *args, PyObject *kwds)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "et|ssO", keywords_path, "utf-8", &Name, &Ext,
                                    &ObjName, &List)) {
        if (Ext) {
            std::string fmt(Ext);
            boost::to_upper(fmt);
    ...
    PyObject *input;
    if (PyArg_ParseTupleAndKeywords(args, kwds, "Os|sO", keywords_stream, &input, &Ext, &ObjName,
                                    &List)) {
        std::string fmt(Ext);
        boost::to_upper(fmt);
        if (ext.find(fmt) != ext.end()) { format = ext[fmt]; }
...
PyObject *MeshPy::writeInventor(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|f", &creaseangle)) return nullptr;
...
{
    float Float;
    if (!PyArg_ParseTuple(args, "f", &Float)) return nullptr;

    PY_TRY { getMeshObjectPtr()->offsetSpecial2(Float); }
    PY_CATCH;
...
PyObject *MeshPy::offsetSpecial(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "fff", &Float, &zmin, &zmax)) return nullptr;

    PY_TRY { getMeshObjectPtr()->offsetSpecial(Float, zmax, zmin); }
    PY_CATCH;
...
PyObject *MeshPy::crossSections(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|fO!", &obj, &min_eps, &PyBool_Type, &poly)) return nullptr;
    ...
    for (std::vector<MeshObject::TPolylines>::iterator it = sections.begin(); it != sections.end();
         ++it) {
        Py::List section;
        for (MeshObject::TPolylines::const_iterator jt = it->begin(); jt != it->end(); ++jt) {
            Py::List polyline;
            for (std::vector<Base::Vector3f>::const_iterator kt = jt->begin(); kt != jt->end();
                 ++kt) {
...
PyObject *MeshPy::unite(PyObject *args)
{
    MeshPy *pcObject;
    PyObject *pcObj;
    if (!PyArg_ParseTuple(args, "O!", &(MeshPy::Type), &pcObj)) return nullptr;
...
PyObject *MeshPy::section(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!|O!f", keywords_section, &(MeshPy::Type),
                                     &pcObj, &PyBool_Type, &connectLines, &fMinDist))
        return nullptr;

    MeshPy *pcObject = static_cast<MeshPy *>(pcObj);

    std::vector<std::vector<Base::Vector3f>> curves = getMeshObjectPtr()->section(
        *pcObject->getMeshObjectPtr(), Base::asBoolean(connectLines), fMinDist);
    Py::List outer;
    for (const auto &it : curves) {
        Py::List inner;
        for (const auto &jt : it) { inner.append(Py::Vector(jt)); }
        outer.append(inner);
    }
...
{
    double x, y, z;
    if (!PyArg_ParseTuple(args, "ddd", &x, &y, &z)) return nullptr;
...
{
    PyObject *mat;
    if (!PyArg_ParseTuple(args, "O!", &(Base::MatrixPy::Type), &mat)) return nullptr;
...
PyObject *MeshPy::addFacet(PyObject *args)
{
    ...
    PyErr_Clear();
    PyObject *v1, *v2, *v3;
    if (PyArg_ParseTuple(args, "O!O!O!", &(Base::VectorPy::Type), &v1, &(Base::VectorPy::Type), &v2,
...
{
    PyObject *mesh;
    if (!PyArg_ParseTuple(args, "O!", &(MeshPy::Type), &mesh)) return nullptr;

    PY_TRY { getMeshObjectPtr()->addMesh(*static_cast<MeshPy *>(mesh)->getMeshObjectPtr()); }
    PY_CATCH;
...
PyObject *MeshPy::setPoint(PyObject *args)
{
    unsigned long index;
    PyObject *pnt;
    if (!PyArg_ParseTuple(args, "kO!", &index, &(Base::VectorPy::Type), &pnt)) return nullptr;

    PY_TRY { getMeshObjectPtr()->setPoint(index, static_cast<Base::VectorPy *>(pnt)->value()); }
    PY_CATCH;
...
PyObject *MeshPy::getPointNormals(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        for (std::size_t i = 0; i < numNormals; i++) { ary.setItem(i, Py::Vector(normals[i])); }
        return Py::new_reference_to(ary);
    }
    PY_CATCH;
}
...
{
    PyObject *pylist;
    if (!PyArg_ParseTuple(args, "O", &pylist)) return nullptr;
    ...
    for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
        ...
        if (index < numFacets) segment.push_back(index);
    }
...
PyObject *MeshPy::getNonUniformOrientedFacets(PyObject *args)
{
    ...
    for (std::size_t i = 0; i < inds.size(); i++) { tuple.setItem(i, Py::Long(inds[i])); }
...
{
    unsigned long count;
    if (!PyArg_ParseTuple(args, "k", &count)) return nullptr;

    PY_TRY
    {
        if (count > 0) { getMeshObjectPtr()->removeComponents(count); }
    }
    PY_CATCH;
...
PyObject *MeshPy::fillupHoles(PyObject *args)
{
    ...
    int level = 0;
    float max_area = 0.0f;
    if (!PyArg_ParseTuple(args, "k|if", &len, &level, &max_area)) return nullptr;
    try {
...
PyObject *MeshPy::fixIndices(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->validateIndices(); }
    PY_CATCH;
...
PyObject *MeshPy::fixCaps(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|ff", &fMaxAngle, &fSplitFactor)) return nullptr;

    PY_TRY { getMeshObjectPtr()->validateCaps(fMaxAngle, fSplitFactor); }
    PY_CATCH;
...
PyObject *MeshPy::fixDeformations(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "f|f", &fMaxAngle, &fEpsilon)) return nullptr;

    PY_TRY { getMeshObjectPtr()->validateDeformations(fMaxAngle, fEpsilon); }
    PY_CATCH;
...
{
    float fEpsilon = MeshCore::MeshDefinitions::_fMinPointDistanceP2;
    if (!PyArg_ParseTuple(args, "|f", &fEpsilon)) return nullptr;

    PY_TRY { getMeshObjectPtr()->validateDegenerations(fEpsilon); }
    PY_CATCH;
...
PyObject *MeshPy::removeDuplicatedPoints(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->removeDuplicatedPoints(); }
    PY_CATCH;
...
PyObject *MeshPy::removeDuplicatedFacets(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->removeDuplicatedFacets(); }
    PY_CATCH;
...
PyObject *MeshPy::refine(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->refine(); }
    PY_CATCH;
...
{
    float length;
    if (!PyArg_ParseTuple(args, "f", &length)) return nullptr;

    PY_TRY { getMeshObjectPtr()->removeNeedles(length); }
    PY_CATCH;
...
PyObject *MeshPy::removeFullBoundaryFacets(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->removeFullBoundaryFacets(); }
    PY_CATCH;
...
PyObject *MeshPy::mergeFacets(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->mergeFacets(); }
    PY_CATCH;
...
PyObject *MeshPy::splitEdges(PyObject *args)
{
    ...
    PY_TRY { getMeshObjectPtr()->splitEdges(); }
    PY_CATCH;
...
{
    unsigned long facet, neighbour;
    if (!PyArg_ParseTuple(args, "kk", &facet, &neighbour)) return nullptr;
...
{
    unsigned long facet;
    if (!PyArg_ParseTuple(args, "k", &facet)) return nullptr;
...
PyObject *MeshPy::insertVertex(PyObject *args)
{
    unsigned long facet;
    PyObject *vertex;
    if (!PyArg_ParseTuple(args, "kO!", &facet, &Base::VectorPy::Type, &vertex)) return nullptr;
...
{
    PyObject *pcObj = nullptr;
    if (!PyArg_ParseTuple(args, "O", &pcObj)) return nullptr;
...
PyObject *MeshPy::foraminate(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO|d", &pnt_p, &dir_p, &maxAngle)) return nullptr;
...
PyObject *MeshPy::cut(PyObject *args)
{
    PyObject *poly;
    int mode;
    if (!PyArg_ParseTuple(args, "Oi", &poly, &mode)) return nullptr;
    ...
    for (std::vector<Base::Vector3f>::const_iterator it = polygon.begin(); it != polygon.end();
         ++it)
...
PyObject *MeshPy::smooth(PyObject *args, PyObject *kwds)
{
    ...
    double maximum = 1000;
    int weight = 1;
    static char *keywords_smooth[] = {"Method",  "Iteration", "Lambda", "Micro",
                                      "Maximum", "Weight",    nullptr};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|sidddi", keywords_smooth, &method, &iter,
                                     &lambda, &micro, &maximum, &weight))
        return nullptr;
...
PyObject *MeshPy::decimate(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "ff", &fTol, &fRed)) {
        PY_TRY { getMeshObjectPtr()->decimate(fTol, fRed); }
        PY_CATCH;
    ...
    if (PyArg_ParseTuple(args, "i", &targetSize)) {
        PY_TRY { getMeshObjectPtr()->decimate(targetSize); }
        PY_CATCH;
...
PyObject *MeshPy::getPlanarSegments(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "f|k", &dev, &minFacets)) return nullptr;
...
PyObject *MeshPy::getSegmentsOfType(PyObject *args)
{
    ...
    float dev;
    unsigned long minFacets = 0;
    if (!PyArg_ParseTuple(args, "sf|k", &type, &dev, &minFacets)) return nullptr;

    Mesh::MeshObject::GeometryType geoType;
    if (strcmp(type, "Plane") == 0) { geoType = Mesh::MeshObject::PLANE; }
...
PyObject *MeshPy::getSegmentsByCurvature(PyObject *args)
{
    ...
    for (Py::Sequence::iterator it = func.begin(); it != func.end(); ++it) {
        ...
        segm.emplace_back(std::make_shared<MeshCore::MeshCurvatureFreeformSegment>(
            meshCurv.GetCurvature(), num, tol1, tol2, c1, c2));
    }

    finder.FindSegments(segm);

    Py::List list;
    for (std::vector<MeshCore::MeshSurfaceSegmentPtr>::iterator segmIt = segm.begin();
         segmIt != segm.end(); ++segmIt) {
...
Py::Long MeshPy::getCountEdges() const { return Py::Long((long)getMeshObjectPtr()->countEdges()); }
...
Py::Float MeshPy::getArea() const { return Py::Float(getMeshObjectPtr()->getSurface()); }

Py::Float MeshPy::getVolume() const { return Py::Float(getMeshObjectPtr()->getVolume()); }

PyObject *MeshPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

...
MeshObject::MeshObject(const MeshCore::MeshKernel &Kernel) : _kernel(Kernel)
{
...
MeshObject::MeshObject(const MeshObject &mesh) : _Mtrx(mesh._Mtrx), _kernel(mesh._kernel)
{
    // copy the mesh structure
    copySegments(mesh);
}
...
Data::Segment *MeshObject::getSubElement(const char *Type, unsigned long n) const
{
    ...
    if (element == "Mesh" && n == 0) {
        ...
        segm->segment.reset(
            new Segment(static_cast<MeshObject *>(segm->mesh), faces.getIndices(), false));
        return segm;
    }
...
void MeshObject::setTransform(const Base::Matrix4D &rclTrf) { _Mtrx = rclTrf; }

Base::Matrix4D MeshObject::getTransform() const { return _Mtrx; }
...
Base::BoundBox3d MeshObject::getBoundBox() const
{
    ...
    if (Bnd.IsValid()) {
        for (int i = 0; i <= 7; i++) Bnd2.Add(transformPointToOutside(Bnd.CalcPoint(i)));
    }
...
void MeshObject::copySegments(const MeshObject &mesh)
{
    ...
    std::for_each(this->_segments.begin(), this->_segments.end(),
                  [this](Segment &s) { s._mesh = this; });
}
...
void MeshObject::swapSegments(MeshObject &mesh)
{
    ...
    std::for_each(this->_segments.begin(), this->_segments.end(),
                  [this](Segment &s) { s._mesh = this; });
    std::for_each(mesh._segments.begin(), mesh._segments.end(),
                  [&mesh](Segment &s) { s._mesh = &mesh; });
}
...
unsigned long MeshObject::countPoints() const { return _kernel.CountPoints(); }

unsigned long MeshObject::countFacets() const { return _kernel.CountFacets(); }

unsigned long MeshObject::countEdges() const { return _kernel.CountEdges(); }

unsigned long MeshObject::countSegments() const { return this->_segments.size(); }
...
double MeshObject::getSurface() const { return _kernel.GetSurface(); }

double MeshObject::getVolume() const { return _kernel.GetVolume(); }
                          ...
                          double /*Accuracy*/, uint16_t /*flags*/) const
{
    ...
    for (unsigned long i = 0; i < ctpoints; i++) { Points.push_back(getPoint(i)); }
...
unsigned int MeshObject::getMemSize() const { return _kernel.GetMemSize(); }
...
void MeshObject::SaveDocFile(Base::Writer &writer) const { _kernel.Write(writer.Stream()); }
...
void MeshObject::RestoreDocFile(Base::Reader &reader) { load(reader); }

void MeshObject::save(const char *file, MeshCore::MeshIO::Format f, const MeshCore::Material *mat,
                      const char *objectname) const
{
    MeshCore::MeshOutput aWriter(this->_kernel, mat);
    if (objectname) aWriter.SetObjectName(objectname);
    ...
    if (aWriter.SaveAny(file, f)) {
        if (mat && mat->binding == MeshCore::MeshIO::PER_FACE) {
            if (f == MeshCore::MeshIO::Undefined) f = MeshCore::MeshOutput::GetFormat(file);
...
void MeshObject::save(std::ostream &str, MeshCore::MeshIO::Format f, const MeshCore::Material *mat,
                      const char *objectname) const
{
    MeshCore::MeshOutput aWriter(this->_kernel, mat);
    if (objectname) aWriter.SetObjectName(objectname);
...
bool MeshObject::load(const char *file, MeshCore::Material *mat)
{
    ...
    if (!aReader.LoadAny(file)) return false;
...
bool MeshObject::load(std::istream &str, MeshCore::MeshIO::Format f, MeshCore::Material *mat)
{
    MeshCore::MeshKernel kernel;
    MeshCore::MeshInput aReader(kernel, mat);
    if (!aReader.LoadFormat(str, f)) return false;
...
void MeshObject::save(std::ostream &out) const { _kernel.Write(out); }
...
void MeshObject::addFacet(const MeshCore::MeshGeomFacet &facet) { _kernel.AddFacet(facet); }
...
void MeshObject::addFacets(const std::vector<MeshCore::MeshFacet> &facets,
                           const std::vector<Base::Vector3f> &points, bool checkManifolds)
{
...
void MeshObject::addFacets(const std::vector<Data::ComplexGeoData::Facet> &facets,
                           const std::vector<Base::Vector3d> &points, bool checkManifolds)
{
...
void MeshObject::setFacets(const std::vector<MeshCore::MeshGeomFacet> &facets) { _kernel = facets; }
...
void MeshObject::addMesh(const MeshObject &mesh) { _kernel.Merge(mesh._kernel); }

void MeshObject::addMesh(const MeshCore::MeshKernel &kernel) { _kernel.Merge(kernel); }
...
{
    if (remFacets.empty()) return;       // nothing has changed
    if (this->_segments.empty()) return; // nothing to do
    ...
    for (std::vector<FacetIndex>::const_iterator it = remFacets.begin(); it != remFacets.end();
         ++it) {
        f_indices[*it] = MeshCore::FACET_INDEX_MAX;
    }

    FacetIndex index = 0;
    for (std::vector<FacetIndex>::iterator it = f_indices.begin(); it != f_indices.end(); ++it) {
        if (*it == 0) *it = index++;
    }

    // the array serves now as LUT to set the new indices in the segments
    for (std::vector<Segment>::iterator it = this->_segments.begin(); it != this->_segments.end();
         ++it) {
        std::vector<FacetIndex> segm = it->_indices;
        for (std::vector<FacetIndex>::iterator jt = segm.begin(); jt != segm.end(); ++jt) {
            *jt = f_indices[*jt];
        }
        ...
        std::vector<FacetIndex>::iterator ft = std::find_if(
            segm.begin(), segm.end(), [](FacetIndex v) { return v == MeshCore::FACET_INDEX_MAX; });
        if (ft != segm.end()) segm.erase(ft, segm.end());
        it->_indices = segm;
    }
}
...
bool MeshObject::hasSelectedFacets() const { return (countSelectedFacets() > 0); }
...
bool MeshObject::hasSelectedPoints() const { return (countSelectedPoints() > 0); }
...
bool MeshObject::nearestFacetOnRay(const MeshObject::TRay &ray, double maxAngle,
                                         ...
                                         std::vector<PointIndex> &point_degree) const
{
    ...
    unsigned long countInvalids =
        std::count_if(pointDeg.begin(), pointDeg.end(), [](PointIndex v) { return v == 0; });
...
void MeshObject::offsetSpecial2(float fSize)
{
    ...
    for (it.Init(); it.More(); it.Next()) FaceNormals.push_back(it->GetNormal().Normalize());
    ...
    for (int l = 0; l < 1; l++) {
        for (it.Init(), i = 0; it.More(); it.Next(), i++) {
            if (it->IsFlag(MeshCore::MeshFacet::INVALID)) continue;
...
void MeshObject::smooth(int iterations, float d_max) { _kernel.Smooth(iterations, d_max); }
...
void MeshObject::crossSections(const std::vector<MeshObject::TPlane> &planes,
                               std::vector<MeshObject::TPolylines> &sections, float fMinEps,
                               bool bConnectPolygons) const
{
    ...
    for (std::vector<MeshObject::TPlane>::const_iterator it = planes.begin(); it != planes.end();
         ++it) {
...
void MeshObject::cut(const Base::Polygon2d &polygon2d, const Base::ViewProjMethod &proj,
                     MeshObject::CutType type)
{
    ...
    bool inner;
    switch (type) {
        case INNER: inner = true; break;
        case OUTER: inner = false; break;
        default: inner = true; break;
    }

    MeshCore::MeshFacetGrid meshGrid(kernel);
    meshAlg.CheckFacets(meshGrid, &proj, polygon2d, inner, check);
    if (!check.empty()) this->deleteFacets(check);
}

void MeshObject::trim(const Base::Polygon2d &polygon2d, const Base::ViewProjMethod &proj,
                      MeshObject::CutType type)
{
    MeshCore::MeshKernel kernel(this->_kernel);
    kernel.Transform(getTransform());
    ...
    switch (type) {
        case INNER: trim.SetInnerOrOuter(MeshCore::MeshTrimming::INNER); break;
        case OUTER: trim.SetInnerOrOuter(MeshCore::MeshTrimming::OUTER); break;
    }
    ...
    if (!triangle.empty()) {
        ...
        mat.inverse();
        for (auto &it : triangle) it.Transform(mat);
...
void MeshObject::trimByPlane(const Base::Vector3f &base, const Base::Vector3f &normal)
{
    ...
    if (!removeFacets.empty()) this->deleteFacets(removeFacets);
    if (!triangle.empty()) this->_kernel.AddFacets(triangle);
}
...
MeshObject *MeshObject::unite(const MeshObject &mesh) const
{
    ...
    MeshCore::SetOperations setOp(kernel1, kernel2, result, MeshCore::SetOperations::Union,
                                  Epsilon);
    setOp.Do();
...
MeshObject *MeshObject::intersect(const MeshObject &mesh) const
{
    ...
    MeshCore::MeshKernel kernel2(mesh._kernel);
    kernel2.Transform(mesh._Mtrx);
    MeshCore::SetOperations setOp(kernel1, kernel2, result, MeshCore::SetOperations::Intersect,
                                  Epsilon);
    setOp.Do();
    return new MeshObject(result);
}
...
MeshObject *MeshObject::subtract(const MeshObject &mesh) const
{
    ...
    MeshCore::MeshKernel kernel2(mesh._kernel);
    kernel2.Transform(mesh._Mtrx);
    MeshCore::SetOperations setOp(kernel1, kernel2, result, MeshCore::SetOperations::Difference,
                                  Epsilon);
    setOp.Do();
    return new MeshObject(result);
}
...
MeshObject *MeshObject::inner(const MeshObject &mesh) const
{
    ...
    MeshCore::MeshKernel kernel2(mesh._kernel);
    kernel2.Transform(mesh._Mtrx);
    MeshCore::SetOperations setOp(kernel1, kernel2, result, MeshCore::SetOperations::Inner,
                                  Epsilon);
    setOp.Do();
    return new MeshObject(result);
}
...
MeshObject *MeshObject::outer(const MeshObject &mesh) const
{
    ...
    MeshCore::MeshKernel kernel2(mesh._kernel);
    kernel2.Transform(mesh._Mtrx);
    MeshCore::SetOperations setOp(kernel1, kernel2, result, MeshCore::SetOperations::Outer,
                                  Epsilon);
    setOp.Do();
    return new MeshObject(result);
}
...
MeshObject::section(const MeshObject &mesh, bool connectLines, float fMinDist) const
{
    ...
    if (!connectLines) {
        ...
        for (const auto &it : triple) {
            ...
            for (const auto &jt : it) curve.push_back(jt.p);
            lines.push_back(curve);
        }
    }
...
void MeshObject::removeNeedles(float length)
{
    ...
    eval.Fixup();
    if (_kernel.CountFacets() < count) this->_segments.clear();
}
...
{
    MeshCore::MeshTopoAlgorithm topalg(_kernel);
    if (fMaxAngle > 0.0f) topalg.OptimizeTopology(fMaxAngle);
    else
...
void MeshObject::splitEdges()
{
    ...
    MeshCore::MeshTopoAlgorithm topalg(_kernel);
    for (std::vector<std::pair<FacetIndex, FacetIndex>>::iterator it = adjacentFacet.begin();
         it != adjacentFacet.end(); ++it) {
        cIter.Set(it->first);
...
MeshObject::getSelfIntersections(const MeshObject::TFacePairs &facets) const
{
    ...
    Base::Matrix4D mat(getTransform());
    std::transform(selfPoints.begin(), selfPoints.end(), std::back_inserter(lines),
                   [&mat](const Section &l) {
...
{
    std::vector<FacetIndex> facets;
    if (!MeshCore::MeshEvalBorderFacet(_kernel, facets).Evaluate()) { deleteFacets(facets); }
}
...
{
    unsigned long count = _kernel.CountFacets();
    MeshCore::MeshFixDeformedFacets eval(_kernel, Base::toRadians(15.0f), Base::toRadians(150.0f),
                                         fMaxAngle, fEps);
    eval.Fixup();
    if (_kernel.CountFacets() < count) this->_segments.clear();
}
...
MeshObject *MeshObject::createSphere(float radius, int sampling)
{
    ...
    try {
        ...
        if (module.isNull()) return nullptr;
...
MeshObject *MeshObject::createCylinder(float radius, float length, int closed, float edgelen,
                                       int sampling)
{
    // load the 'BuildRegularGeoms' module
    Base::PyGILStateLocker lock;
...
MeshObject *MeshObject::createCone(float radius1, float radius2, float len, int closed,
                                   float edgelen, int sampling)
{
    // load the 'BuildRegularGeoms' module
    Base::PyGILStateLocker lock;
    try {
...
void MeshObject::addSegment(const std::vector<FacetIndex> &inds)
{
    ...
    for (std::vector<FacetIndex>::const_iterator it = inds.begin(); it != inds.end(); ++it) {
        if (*it >= maxIndex) throw Base::IndexError("Index out of range");
    }

    this->_segments.emplace_back(this, inds, true);
}

const Segment &MeshObject::getSegment(unsigned long index) const { return this->_segments[index]; }

Segment &MeshObject::getSegment(unsigned long index) { return this->_segments[index]; }
...
std::vector<Segment> MeshObject::getSegmentsOfType(MeshObject::GeometryType type, float dev,
                                                   unsigned long minFacets) const
{
    std::vector<Segment> segm;
    if (this->_kernel.CountFacets() == 0) return segm;
    ...
    switch (type) {
        case PLANE:
            ...
            surf.reset(new MeshCore::MeshDistanceGenericSurfaceFitSegment(
                new MeshCore::PlaneSurfaceFit, this->_kernel, minFacets, dev));
            break;
        case CYLINDER:
            surf.reset(new MeshCore::MeshDistanceGenericSurfaceFitSegment(
                new MeshCore::CylinderSurfaceFit, this->_kernel, minFacets, dev));
            break;
        case SPHERE:
            surf.reset(new MeshCore::MeshDistanceGenericSurfaceFitSegment(
                new MeshCore::SphereSurfaceFit, this->_kernel, minFacets, dev));
            break;
        default: break;
    }
    ...
    if (surf.get()) {
        ...
        for (std::vector<MeshCore::MeshSegment>::const_iterator it = data.begin(); it != data.end();
             ++it) {
...
MeshObject::const_point_iterator::operator=(const MeshObject::const_point_iterator &pi)
{
    this->_mesh = pi._mesh;
...
MeshObject::const_facet_iterator::operator=(const MeshObject::const_facet_iterator &fi)
{
    this->_mesh = fi._mesh;

...
Segment::Segment(const MeshObject *mesh, bool mod) : _mesh(mesh), _save(false), _modifykernel(mod)
{}

Segment::Segment(const MeshObject *mesh, const std::vector<FacetIndex> &inds, bool mod)
    : _mesh(mesh), _indices(inds), _save(false), _modifykernel(mod)
{
    if (_modifykernel) _mesh->updateMesh(inds);
}
...
void Segment::removeIndices(const std::vector<FacetIndex> &inds)
{
    ...
    _indices = result;
    if (_modifykernel) _mesh->updateMesh();
}

const std::vector<FacetIndex> &Segment::getIndices() const { return _indices; }

Segment::Segment(const Segment &s)
    : _mesh(s._mesh), _indices(s._indices), _name(s._name), _color(s._color), _save(s._save),
...
const Segment &Segment::operator=(const Segment &s)
{
    ...
    if (this != &s) this->_indices = s._indices;
    if (_modifykernel) _mesh->updateMesh();
    return *this;
}

bool Segment::operator==(const Segment &s) const { return this->_indices == s._indices; }

// ----------------------------------------------------------------------------

Segment::const_facet_iterator::const_facet_iterator(const Segment *segm,
...
Segment::const_facet_iterator &
Segment::const_facet_iterator::operator=(const Segment::const_facet_iterator &fi)
{

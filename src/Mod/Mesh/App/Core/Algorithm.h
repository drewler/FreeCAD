...
public:
   ...
   * occasionally.
   */
    bool NearestFacetOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
   ...
   */
    bool NearestFacetOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                           float fMaxAngle, Base::Vector3f &rclRes, FacetIndex &rulFacet) const;
    /**
   * Searches for the nearest facet to the ray defined by
   * (\a rclPt, \a rclDir).
   * The point \a rclRes holds the intersection point with the ray and the
   ...
   */
    bool NearestFacetOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                           const std::vector<FacetIndex> &raulFacets, Base::Vector3f &rclRes,
                           FacetIndex &rulFacet) const;
    /**
   ...
   */
    bool FirstFacetToVertex(const Base::Vector3f &rclPt, float fMaxDistance,
                            const MeshFacetGrid &rclGrid, FacetIndex &rulFacet) const;
    /**
   ...
   */
    bool IsVertexVisible(const Base::Vector3f &rcVertex, const Base::Vector3f &rcView,
   ...
   */
    void GetFacetBorders(const std::vector<FacetIndex> &raulInd,
   ...
   * boundary is empty.
   */
    void GetFacetsBorders(const std::vector<FacetIndex> &uFacets,
   ...
   */
    bool FillupHole(const std::vector<PointIndex> &boundary, AbstractPolygonTriangulator &cTria,
                    MeshFacetArray &rFaces, MeshPointArray &rPoints, int level,
                    const MeshRefPointToFacets *pP2FStructure = nullptr) const;
    /** Sets to all facets in \a raulInds the properties in raulProps. 
   * \note Both arrays must have the same size.
   */
    void SetFacetsProperty(const std::vector<FacetIndex> &raulInds,
    ...
    void PointsFromFacetsIndices(const std::vector<FacetIndex> &rvecIndices,
   ...
   */
    void GetFacetsFromToolMesh(const MeshKernel &rToolMesh, const Base::Vector3f &rcDir,
   ...
   */
    void CheckFacets(const MeshFacetGrid &rclGrid, const Base::ViewProjMethod *pclProj,
   ...
   */
    void CheckBorderFacets(const std::vector<FacetIndex> &raclFacetIndices,
                           ...
                           unsigned short usLevel = 1) const;
    /**
   ...
   */
    void CutBorderFacets(std::vector<FacetIndex> &raclFacetIndices,
                         unsigned short usLevel = 1) const;
   ...
   */
    void GetBorderPoints(const std::vector<FacetIndex> &raclFacetIndices,
   ...
   */
    bool NearestPointFromPoint(const Base::Vector3f &rclPt, FacetIndex &rclResFacetIndex,
    ...
    bool CutWithPlane(const Base::Vector3f &clBase, const Base::Vector3f &clNormal,
                      const MeshFacetGrid &rclGrid,
                      std::list<std::vector<Base::Vector3f>> &rclResult, float fMinEps = 1.0e-2f,
   ...
   */
    void GetFacetsFromPlane(const MeshFacetGrid &rclGrid, const Base::Vector3f &clNormal,
                            float dist, const Base::Vector3f &rclLeft,
   ...
   */
    bool Distance(const Base::Vector3f &rclPt, FacetIndex ulFacetIdx, float fMaxDistance,
   ...
   */
    float CalculateMinimumGridLength(float fLength, const Base::BoundBox3f &rBBox,
                                     unsigned long maxElements) const;

protected:
    /** Helper method to connect the intersection points to polylines. */
    bool ConnectLines(std::list<std::pair<Base::Vector3f, Base::Vector3f>> &rclLines,
                      std::list<std::vector<Base::Vector3f>> &rclPolylines, float fMinEps) const;
    bool ConnectPolygons(std::list<std::vector<Base::Vector3f>> &clPolyList,
    ...
    bool RayNearestField(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                         const std::vector<FacetIndex> &raulFacets, Base::Vector3f &rclRes,
                         FacetIndex &rulFacet, float fMaxAngle = Mathf::PI) const;
    /** 
   * Splits the boundary \a rBound in several loops and append this loops to the list of borders.
   */
    void SplitBoundaryLoops(const std::vector<PointIndex> &rBound,
...
public:
    /// Construction
    explicit MeshRefPointToFacets(const MeshKernel &rclM) : _rclMesh(rclM) { Rebuild(); }
    /// Destruction
...
protected:
    void SearchNeighbours(const MeshFacetArray &rFacets, FacetIndex index,
                          const Base::Vector3f &rclCenter, float fMaxDist,
                          std::set<FacetIndex> &visit, MeshCollector &collect) const;
...
public:
    /// Construction
    explicit MeshRefFacetToFacets(const MeshKernel &rclM) : _rclMesh(rclM) { Rebuild(); }
    /// Destruction
...
public:
    /// Construction
    explicit MeshRefPointToPoints(const MeshKernel &rclM) : _rclMesh(rclM) { Rebuild(); }
    /// Destruction
...
public:
    /// Construction
    explicit MeshRefEdgeToFacets(const MeshKernel &rclM) : _rclMesh(rclM) { Rebuild(); }
    /// Destruction
    ~MeshRefEdgeToFacets() {}
...
public:
    /// Construction
    explicit MeshRefNormalToPoints(const MeshKernel &rclM) : _rclMesh(rclM) { Rebuild(); }
    /// Destruction
    ...
    const std::vector<Base::Vector3f> &GetValues() const { return _norm; }

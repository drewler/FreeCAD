...
bool MeshSurfaceSegment::TestInitialFacet(FacetIndex) const { return true; }

void MeshSurfaceSegment::AddFacet(const MeshFacet &) {}

void MeshSurfaceSegment::AddSegment(const std::vector<FacetIndex> &segm)
{
    if (segm.size() >= minFacets) { segments.push_back(segm); }
}

MeshSegment MeshSurfaceSegment::FindSegment(FacetIndex index) const
{
    for (std::vector<MeshSegment>::const_iterator it = segments.begin(); it != segments.end();
         ++it) {
        if (std::find(it->begin(), it->end(), index) != it->end()) return *it;
    }
...
MeshDistancePlanarSegment::~MeshDistancePlanarSegment() { delete fitter; }
...
{
    if (!fitter->Done()) fitter->Fit();
    MeshGeomFacet triangle = kernel.GetFacet(face);
    for (int i = 0; i < 3; i++) {
        if (fabs(fitter->GetDistanceToPlane(triangle._aclPoints[i])) > tolerance) return false;
    }
...
PlaneSurfaceFit::PlaneSurfaceFit() : fitter(new PlaneFit) {}

PlaneSurfaceFit::PlaneSurfaceFit(const Base::Vector3f &b, const Base::Vector3f &n)
    : basepoint(b), normal(n), fitter(nullptr)
{}

PlaneSurfaceFit::~PlaneSurfaceFit() { delete fitter; }
...
bool PlaneSurfaceFit::TestTriangle(const MeshGeomFacet &) const { return true; }

void PlaneSurfaceFit::AddTriangle(const MeshCore::MeshGeomFacet &tria)
{
    if (fitter) fitter->AddPoint(tria.GetGravityPoint());
}

bool PlaneSurfaceFit::Done() const
{
    if (!fitter) return true;
    else
        return fitter->Done();
}
...
{
    if (!fitter) return pnt.DistanceToPlane(basepoint, normal);
    else
...
CylinderSurfaceFit::CylinderSurfaceFit() : fitter(new CylinderFit)
{
    axis.Set(0, 0, 0);
    radius = FLOAT_MAX;
}
...
CylinderSurfaceFit::CylinderSurfaceFit(const Base::Vector3f &b, const Base::Vector3f &a, float r)
    : basepoint(b), axis(a), radius(r), fitter(nullptr)
{}

CylinderSurfaceFit::~CylinderSurfaceFit() { delete fitter; }
...
SphereSurfaceFit::SphereSurfaceFit() : fitter(new SphereFit)
{
    center.Set(0, 0, 0);
    radius = FLOAT_MAX;
}
...
SphereSurfaceFit::SphereSurfaceFit(const Base::Vector3f &c, float r)
    : center(c), radius(r), fitter(nullptr)
{}

SphereSurfaceFit::~SphereSurfaceFit() { delete fitter; }
...
MeshDistanceGenericSurfaceFitSegment::MeshDistanceGenericSurfaceFitSegment(AbstractSurfaceFit *fit,
                                                                           ...
                                                                           float tol)
    : MeshDistanceSurfaceSegment(mesh, minFacets, tol), fitter(fit)
{}

MeshDistanceGenericSurfaceFitSegment::~MeshDistanceGenericSurfaceFitSegment() { delete fitter; }
...
bool MeshDistanceGenericSurfaceFitSegment::TestInitialFacet(FacetIndex index) const
{
    MeshGeomFacet triangle = kernel.GetFacet(index);
    for (int i = 0; i < 3; i++) {
        if (fabs(fitter->GetDistanceToSurface(triangle._aclPoints[i])) > tolerance) return false;
    }
...
bool MeshCurvaturePlanarSegment::TestFacet(const MeshFacet &rclFacet) const
{
    for (int i = 0; i < 3; i++) {
        ...
        if (fabs(ci.fMinCurvature) > tolerance) return false;
        if (fabs(ci.fMaxCurvature) > tolerance) return false;
    }
...
bool MeshCurvatureCylindricalSegment::TestFacet(const MeshFacet &rclFacet) const
{
    for (int i = 0; i < 3; i++) {
        ...
        if (fMin > toleranceMin) return false;
        if (fabs(fMax - curvature) > toleranceMax) return false;
    }
...
bool MeshCurvatureSphericalSegment::TestFacet(const MeshFacet &rclFacet) const
{
    for (int i = 0; i < 3; i++) {
        const CurvatureInfo &ci = info[rclFacet._aulPoints[i]];
        if (ci.fMaxCurvature * ci.fMinCurvature < 0) return false;
        float diff;
...
bool MeshCurvatureFreeformSegment::TestFacet(const MeshFacet &rclFacet) const
{
    for (int i = 0; i < 3; i++) {
        const CurvatureInfo &ci = info[rclFacet._aulPoints[i]];
        if (fabs(ci.fMinCurvature - c2) > toleranceMin) return false;
        if (fabs(ci.fMaxCurvature - c1) > toleranceMax) return false;
    }
...
bool MeshSurfaceVisitor::AllowVisit(const MeshFacet &face, const MeshFacet &, FacetIndex,
                                    unsigned long, unsigned short)
{
    return segm.TestFacet(face);
}

bool MeshSurfaceVisitor::Visit(const MeshFacet &face, const MeshFacet &, FacetIndex ulFInd,
                               unsigned long)
{
...
void MeshSegmentAlgorithm::FindSegments(std::vector<MeshSurfaceSegmentPtr> &segm)
{
    ...
    for (std::vector<MeshSurfaceSegmentPtr>::iterator it = segm.begin(); it != segm.end(); ++it) {
        ...
        iCur = std::find_if(iBeg, iEnd,
                            [flag](const MeshFacet &f) { return flag(f, MeshFacet::VISIT); });
        if (iCur < iEnd) startFacet = iCur - iBeg;
        else

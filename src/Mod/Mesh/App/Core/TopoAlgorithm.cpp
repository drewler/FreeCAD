...
bool MeshTopoAlgorithm::SnapVertex(FacetIndex ulFacetPos, const Base::Vector3f &rP)
{
    ...
    if (!rFace.HasOpenEdge()) return false;
    ...
    for (unsigned short i = 0; i < 3; i++) {
        if (rFace._aulNeighbours[i] == FACET_INDEX_MAX) {
            ...
            if (cNo3.Length() < FLOAT_EPS) { return SplitOpenEdge(ulFacetPos, i, rP); }
...
void MeshTopoAlgorithm::OptimizeTopology(float fMaxAngle)
{
    ...
    for (MeshFacetArray::_TIterator pI = _rclMesh._aclFacetArray.begin();
         pI != _rclMesh._aclFacetArray.end(); ++pI) {
        for (int i = 0; i < 3; i++) {
            // ignore open edges
    ...
    while (!aEdgeList.empty() && uMaxIter > 0) {
        ...
        if (!ShouldSwapEdge(pE->second[0], pE->second[1], fMaxAngle)) continue;
        ...
        if (/*fMax12 > fMax34*/ true) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (it != aEdge2Face.end()) {
                    if (it->second[0] == pE->second[1]) it->second[0] = pE->second[0];
                ...
                it = aEdge2Face.find(std::make_pair(ulPt0, ulPt1));
                if (it != aEdge2Face.end()) {
                    if (it->second[0] == pE->second[0]) it->second[0] = pE->second[1];
...
static float cos_maxangle(const Base::Vector3f &v1, const Base::Vector3f &v2,
                          ...
                          const Base::Vector3f &v3, const Base::Vector3f &v4)
{
    ...
    if ((n124 * n234) <= 0.0f) return 0.0f; // avoid normal flip
...
float MeshTopoAlgorithm::SwapEdgeBenefit(FacetIndex f, int e) const
{
    ...
    if (n == FACET_INDEX_MAX) return 0.0f; // border edge
...
void MeshTopoAlgorithm::OptimizeTopology()
{
    ...
    for (FacetIndex i = 0; i < nf; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (b > 0.0f) todo.push(std::make_pair(b, std::make_pair(i, j)));
        }
    }
    ...
    while (!todo.empty()) {
        ...
        if (SwapEdgeBenefit(f, e) <= 0.0f) continue;
        // OK, swap the edge
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (b > 0.0f) todo.push(std::make_pair(b, std::make_pair(f, j)));
        }
        for (int j = 0; j < 3; j++) {
            float b = SwapEdgeBenefit(f2, j);
            if (b > 0.0f) todo.push(std::make_pair(b, std::make_pair(f2, j)));
        }
    }
}
...
void MeshTopoAlgorithm::DelaunayFlip(float fMaxAngle)
{
    ...
    FacetIndex index = 0;
    for (MeshFacetArray::_TIterator pI = _rclMesh._aclFacetArray.begin();
         pI != _rclMesh._aclFacetArray.end(); ++pI, index++) {
        for (int i = 0; i < 3; i++) {
            // ignore open edges
            if (pI->_aulNeighbours[i] != FACET_INDEX_MAX) {
...
void MeshTopoAlgorithm::AdjustEdgesToCurvatureDirection()
{
    ...
    for (cPIt.Init(); cPIt.More(); cPIt.Next()) aPnts.emplace_back(cPIt->x, cPIt->y, cPIt->z);
    ...
    for (std::map<std::pair<PointIndex, PointIndex>, std::list<FacetIndex>>::iterator kt =
             aclEdgeMap.begin();
         kt != aclEdgeMap.end(); ++kt) {
        if (kt->second.size() == 2) {
            ...
            if (rFace1.IsFlag(MeshFacet::VISIT) || rFace2.IsFlag(MeshFacet::VISIT)) continue;
...
bool MeshTopoAlgorithm::InsertVertexAndSwapEdge(FacetIndex ulFacetPos,
                                                const Base::Vector3f &rclPoint, float fMaxAngle)
{
    if (!InsertVertex(ulFacetPos, rclPoint)) return false;
...
bool MeshTopoAlgorithm::IsSwapEdgeLegal(FacetIndex ulFacetPos, FacetIndex ulNeighbour) const
{
    ...
    if (uFSide == USHRT_MAX || uNSide == USHRT_MAX) return false; // not neighbours
    ...
    if (cT3.IsDegenerated(MeshDefinitions::_fMinPointDistanceP2)) return false;
    MeshGeomFacet cT4(cP3, cP4, cP2);
    if (cT4.IsDegenerated(MeshDefinitions::_fMinPointDistanceP2)) return false;
    ...
    if (((cP3 - cP1) * cN1) * ((cP4 - cP1) * cN1) >= 0.0f) return false; // not convex
    // build a helper plane through cP3 that must separate cP1 and cP2
    Base::Vector3f cN2 = (cU % cV) % cV;
    if (((cP1 - cP3) * cN2) * ((cP2 - cP3) * cN2) >= 0.0f) return false; // not convex

    return true;
}

bool MeshTopoAlgorithm::ShouldSwapEdge(FacetIndex ulFacetPos, FacetIndex ulNeighbour,
                                       float fMaxAngle) const
{
    if (!IsSwapEdgeLegal(ulFacetPos, ulNeighbour)) return false;
    ...
    if (cN1.GetAngle(cN2) > fMaxAngle) return false;
...
void MeshTopoAlgorithm::SwapEdge(FacetIndex ulFacetPos, FacetIndex ulNeighbour)
{
    ...
    if (uFSide == USHRT_MAX || uNSide == USHRT_MAX) return; // not neighbours
...
bool MeshTopoAlgorithm::SplitEdge(FacetIndex ulFacetPos, FacetIndex ulNeighbour,
...
bool MeshTopoAlgorithm::SplitOpenEdge(FacetIndex ulFacetPos, unsigned short uSide,
                                      const Base::Vector3f &rP)
{
    MeshFacet &rclF = _rclMesh._aclFacetArray[ulFacetPos];
    if (rclF._aulNeighbours[uSide] != FACET_INDEX_MAX) return false; // not open
...
{
    if (fabs(u.x - v.x) > FLOAT_EPS) return u.x < v.x;
    if (fabs(u.y - v.y) > FLOAT_EPS) return u.y < v.y;
    if (fabs(u.z - v.z) > FLOAT_EPS) return u.z < v.z;
    return false;
}
...
{
    if (!_cache) return _rclMesh._aclPointArray.GetOrAddIndex(rclPoint);
    ...
    if (retval.second) _rclMesh._aclPointArray.push_back(rclPoint);
...
{
    if (vc._circumFacets.size() != vc._circumPoints.size()) return false;

    if (vc._circumFacets.size() != 3) return false;
    ...
    if (ptIndex == POINT_INDEX_MAX) return false;
...
bool MeshTopoAlgorithm::IsCollapseEdgeLegal(const EdgeCollapse &ec) const
{
    ...
    std::set_intersection(ec._adjacentFrom.begin(), ec._adjacentFrom.end(), ec._adjacentTo.begin(),
                          ec._adjacentTo.end(),
                          std::back_insert_iterator<std::vector<PointIndex>>(commonPoints));
    if (commonPoints.size() > 2) { return false; }
    ...
    if (!_rclMesh._aclPointArray[ec._fromPoint].IsValid()) return false;

    if (!_rclMesh._aclPointArray[ec._toPoint].IsValid()) return false;
    ...
    for (it = ec._removeFacets.begin(); it != ec._removeFacets.end(); ++it) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (f._aulNeighbours[i] != FACET_INDEX_MAX) {
                if (std::find(ec._removeFacets.begin(), ec._removeFacets.end(), f._aulNeighbours[i])
...
{
    MeshFacet &rclF = _rclMesh._aclFacetArray[ulFacetPos];
    if (!rclF.IsValid()) return false; // the facet is marked invalid from a previous run
    ...
    for (int i = 0; i < 3; i++) {
        if (rclF._aulNeighbours[i] == FACET_INDEX_MAX) continue;
        MeshFacet &rclN = _rclMesh._aclFacetArray[rclF._aulNeighbours[i]];
        unsigned short uNSide = rclN.Side(rclF);

        if (rclN._aulNeighbours[(uNSide + 1) % 3] != FACET_INDEX_MAX) {
            _rclMesh._aclFacetArray[rclN._aulNeighbours[(uNSide + 1) % 3]].ReplaceNeighbour(
                rclF._aulNeighbours[i], rclN._aulNeighbours[(uNSide + 2) % 3]);
        }
        if (rclN._aulNeighbours[(uNSide + 2) % 3] != FACET_INDEX_MAX) {
            _rclMesh._aclFacetArray[rclN._aulNeighbours[(uNSide + 2) % 3]].ReplaceNeighbour(
                rclF._aulNeighbours[i], rclN._aulNeighbours[(uNSide + 1) % 3]);
        }
...
void MeshTopoAlgorithm::SplitFacet(FacetIndex ulFacetPos, const Base::Vector3f &rP1,
                                   const Base::Vector3f &rP2)
{
    ...
    auto pointIndex = [=](const Base::Vector3f &rP) {
        ...
        if (Base::Distance(rVertex0, rP) < fEps) equalP = 0;
    ...
    if (equalP1 != USHRT_MAX && equalP2 != USHRT_MAX) return; // must not split the facet
...
void MeshTopoAlgorithm::SplitFacetOnTwoEdges(FacetIndex ulFacetPos, const Base::Vector3f &rP1,
                                             const Base::Vector3f &rP2)
{
    ...
    for (FacetIndex i = cntFts; i < cntFts2; i++) { fixIndices.push_back(i); }
...
void MeshTopoAlgorithm::SplitFacet(FacetIndex ulFacetPos, PointIndex P1, PointIndex P2,
                                   PointIndex Pn)
{
    MeshFacet &rFace = _rclMesh._aclFacetArray[ulFacetPos];
    ...
    if (side != USHRT_MAX) {
        ...
        if (N1 != FACET_INDEX_MAX) _rclMesh._aclFacetArray[N1].ReplaceNeighbour(ulFacetPos, size);
...
void MeshTopoAlgorithm::AddFacet(PointIndex P1, PointIndex P2, PointIndex P3, FacetIndex N1,
                                 FacetIndex N2, FacetIndex N3)
{
    MeshFacet facet;
    facet._aulPoints[0] = P1;
    facet._aulPoints[1] = P2;
...
void MeshTopoAlgorithm::HarmonizeNeighbours(FacetIndex facet1, FacetIndex facet2)
{
    ...
    if (side != USHRT_MAX) { rFace1._aulNeighbours[side] = facet2; }

    side = rFace2.Side(rFace1);
    if (side != USHRT_MAX) { rFace2._aulNeighbours[side] = facet1; }
}

void MeshTopoAlgorithm::SplitNeighbourFacet(FacetIndex ulFacetPos, unsigned short uFSide,
...
bool MeshTopoAlgorithm::RemoveDegeneratedFacet(FacetIndex index)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (rE0 == rE1) {
            ...
            if (uN2 != FACET_INDEX_MAX) _rclMesh._aclFacetArray[uN2].ReplaceNeighbour(index, uN1);
            if (uN1 != FACET_INDEX_MAX) _rclMesh._aclFacetArray[uN1].ReplaceNeighbour(index, uN2);
                                    ...
                                    std::list<std::vector<PointIndex>> &aFailed)
{
    ...
    for (std::list<std::vector<PointIndex>>::iterator it = aBorders.begin(); it != aBorders.end();
         ++it) {
        if (it->size() - 1 <= length) // ignore boundary with too many edges
            aFillBorders.push_back(*it);
    }

    if (!aFillBorders.empty()) FillupHoles(level, cTria, aFillBorders, aFailed);
}
                                    ...
                                    std::list<std::vector<PointIndex>> &aFailed)
{
    ...
    for (std::list<std::vector<PointIndex>>::const_iterator it = aBorders.begin();
         it != aBorders.end(); ++it) {
    ...
    _rclMesh._aclPointArray.insert(_rclMesh._aclPointArray.end(), newPoints.begin(),
                                   newPoints.end());
    ...
    if (!newFacets.empty()) {
        ...
        for (MeshFacetArray::_TIterator it = newFacets.begin(); it != newFacets.end(); ++it) {
            if (it->_aulPoints[0] >= ctPoints || it->_aulPoints[1] >= ctPoints
                || it->_aulPoints[2] >= ctPoints) {
                Base::Console().Log("Ignore invalid face <%d, %d, %d> (%d vertices)\n",
                                    it->_aulPoints[0], it->_aulPoints[1], it->_aulPoints[2],
                                    ctPoints);
            }
            else {
                                  ...
                                  std::list<std::vector<PointIndex>> &aBorders) const
{
    ...
    for (std::list<std::vector<PointIndex>>::iterator it = border.begin(); it != border.end();
         ++it) {
...
void MeshTopoAlgorithm::FindComponents(unsigned long count, std::vector<FacetIndex> &findIndices)
{
    ...
    for (std::vector<std::vector<FacetIndex>>::iterator it = segments.begin(); it != segments.end();
         ++it) {
        if (it->size() <= count) findIndices.insert(findIndices.end(), it->begin(), it->end());
    }
}
...
void MeshTopoAlgorithm::RemoveComponents(unsigned long count)
{
    ...
    if (!removeFacets.empty()) _rclMesh.DeleteFacets(removeFacets);
}
...
void MeshTopoAlgorithm::FlipNormals()
{
    for (MeshFacetArray::_TIterator i = _rclMesh._aclFacetArray.begin();
         i < _rclMesh._aclFacetArray.end(); ++i)
        i->FlipNormal();
}
...
MeshComponents::MeshComponents(const MeshKernel &rclMesh) : _rclMesh(rclMesh) {}
                                         ...
                                         std::vector<std::vector<FacetIndex>> &aclT) const
{
    ...
    FacetIndex k = 0;
    for (std::vector<FacetIndex>::iterator pI = aulAllFacets.begin(); pI != aulAllFacets.end();
         ++pI)
        *pI = k++;
...
void MeshComponents::SearchForComponents(TMode tMode, const std::vector<FacetIndex> &aSegment,
                                         std::vector<std::vector<FacetIndex>> &aclT) const
{
    FacetIndex ulStartFacet;

    if (_rclMesh.CountFacets() == 0) return;
    ...
    iTri =
        std::find_if(iTri, iEnd, [flag](const MeshFacet &f) { return flag(f, MeshFacet::VISIT); });
    ...
    while (ulStartFacet != FACET_INDEX_MAX) {
        ...
        aclComponent.clear();
        if (tMode == OverEdge) ulVisited += _rclMesh.VisitNeighbourFacets(clFVisitor, ulStartFacet);
        ...
        iTri = std::find_if(iTri, iEnd,
                            [flag](const MeshFacet &f) { return flag(f, MeshFacet::VISIT); });

        if (iTri < iEnd) ulStartFacet = iTri - iBeg;
        else

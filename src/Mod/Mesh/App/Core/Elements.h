...
public:
   ...
   */
    //@{
    void SetFlag(TFlagType tF) const { _ucFlag |= static_cast<unsigned char>(tF); }
    void ResetFlag(TFlagType tF) const { _ucFlag &= ~static_cast<unsigned char>(tF); }
    bool IsFlag(TFlagType tF) const
    {
        return (_ucFlag & static_cast<unsigned char>(tF)) == static_cast<unsigned char>(tF);
    }
    void ResetInvalid() const { ResetFlag(INVALID); }
    void SetInvalid() const { SetFlag(INVALID); }
    bool IsValid() const { return !IsFlag(INVALID); }
    void SetProperty(unsigned long uP) const { _ulProp = uP; }
    //@}
...
public:
   ...
   */
    bool IntersectWithLine(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
   ...
   * with the edge. The intersection must be inside the edge. If there is no intersection false is returned.
   */
    bool IntersectWithPlane(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                            Base::Vector3f &rclRes) const;
    /**
...
public:
    ...
    inline MeshFacet(PointIndex p1, PointIndex p2, PointIndex p3, FacetIndex n1 = FACET_INDEX_MAX,
                     FacetIndex n2 = FACET_INDEX_MAX, FacetIndex n3 = FACET_INDEX_MAX);
    ~MeshFacet() {}
    //@}
   ...
   */
    bool IsNeighbour(FacetIndex index) const { return Side(index) < 3; }
...
public:
   ...
   */
    ...
    /** Set flag state */
    void SetFlag(MeshFacet::TFlagType tF) { _ucFlag |= static_cast<unsigned char>(tF); }
    /** Reset flag state */
    void ResetFlag(MeshFacet::TFlagType tF) { _ucFlag &= ~static_cast<unsigned char>(tF); }
   ...
   */
    int IntersectWithFacet(const MeshGeomFacet &facet, Base::Vector3f &rclPt0,
   ...
   */
    bool Foraminate(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                    Base::Vector3f &rclRes, float fMaxAngle = Mathf::PI) const;
   ...
   */
    bool IntersectWithPlane(const Base::Vector3f &rclBase, const Base::Vector3f &rclNormal,
                            Base::Vector3f &rclP1, Base::Vector3f &rclP2) const;
    /**
   * Checks if the facet intersects with the plane defined by the base \a rclBase and the normal
   * \a rclNormal.
   */
   ...
   */
    bool IntersectPlaneWithLine(const Base::Vector3f &rclBase, const Base::Vector3f &rclNormal,
                                Base::Vector3f &rclRes) const;
   ...
   */
    ...
    void NearestEdgeToPoint(const Base::Vector3f &rclPt, float &fDistance,
                            unsigned short &side) const;
...
{
public:
    explicit MeshPointModifier(MeshPointArray &points) : rPoints(points) {}

    MeshPointModifier(const MeshPointModifier &c) : rPoints(c.rPoints) {}
...
{
public:
    explicit MeshFacetModifier(MeshFacetArray &facets) : rFacets(facets) {}

    MeshFacetModifier(const MeshFacetModifier &c) : rFacets(c.rFacets) {}
...
inline MeshPoint::MeshPoint(const MeshPoint &rclPt)
    ...
    : Base::Vector3f(rclPt),
#endif
      _ucFlag(rclPt._ucFlag), _ulProp(rclPt._ulProp)
{}
...
inline bool MeshPoint::operator<(const MeshPoint &rclPt) const
{
    if (fabs(this->x - rclPt.x) >= MeshDefinitions::_fMinPointDistanceD1) return this->x < rclPt.x;
    if (fabs(this->y - rclPt.y) >= MeshDefinitions::_fMinPointDistanceD1) return this->y < rclPt.y;
    if (fabs(this->z - rclPt.z) >= MeshDefinitions::_fMinPointDistanceD1) return this->z < rclPt.z;
...
inline void MeshGeomFacet::ArrangeNormal(const Base::Vector3f &rclN)
{
    ...
    if ((rclN * GetNormal()) < 0.0f) _clNormal = -_clNormal;
}
...
inline void MeshGeomFacet::AdjustCirculationDirection()
{
    ...
    if ((clN * _clNormal) < 0.0f) std::swap(_aclPoints[1], _aclPoints[2]);
}
...
inline bool MeshGeomFacet::ContainedByOrIntersectBoundingBox(const Base::BoundBox3f &rclBB) const
{
    ...
    if (!(GetBoundBox() && rclBB)) return false;

    // Test, whether Facet-BB is completely within BB
    if (rclBB.IsInBox(GetBoundBox())) return true;

    // Test, whether one of the corner points is in BB
    for (int i = 0; i < 3; i++) {
        if (rclBB.IsInBox(_aclPoints[i])) return true;
    }

    // "real" test for cutting
    if (IntersectBoundingBox(rclBB)) return true;
...
inline MeshFacet::MeshFacet() : _ucFlag(0), _ulProp(0)
{
    memset(_aulNeighbours, 0xff, sizeof(FacetIndex) * 3);
    memset(_aulPoints, 0xff, sizeof(PointIndex) * 3);
}

inline MeshFacet::MeshFacet(const MeshFacet &rclF) : _ucFlag(rclF._ucFlag), _ulProp(rclF._ulProp)
{
...
inline MeshFacet::MeshFacet(PointIndex p1, PointIndex p2, PointIndex p3, FacetIndex n1,
...
{
    if (_aulPoints[0] == ulOrig) _aulPoints[0] = ulNew;
...
{
    if (_aulPoints[0] == ulIndex) return true;
    if (_aulPoints[1] == ulIndex) return true;
    if (_aulPoints[2] == ulIndex) return true;
    return false;
}

inline void MeshFacet::ReplaceNeighbour(FacetIndex ulOrig, FacetIndex ulNew)
{
    if (_aulNeighbours[0] == ulOrig) _aulNeighbours[0] = ulNew;
...
inline bool MeshFacet::HasOpenEdge() const { return (CountOpenEdges() != 0); }
...
{
    if (_aulPoints[0] == _aulPoints[1]) return true;
    if (_aulPoints[1] == _aulPoints[2]) return true;
    if (_aulPoints[2] == _aulPoints[0]) return true;
    return false;
}

inline unsigned short MeshFacet::Side(FacetIndex ulNIndex) const
{
    if (_aulNeighbours[0] == ulNIndex) return 0;
...
inline unsigned short MeshFacet::Side(PointIndex ulP0, PointIndex ulP1) const
{
    if (_aulPoints[0] == ulP0) {
        if (_aulPoints[1] == ulP1) return 0; // Edge 0-1 ==> 0
        else if (_aulPoints[2] == ulP1)
            return 2; // Edge 0-2 ==> 2
    }
    else if (_aulPoints[1] == ulP0) {
        if (_aulPoints[0] == ulP1) return 0; // Edge 1-0 ==> 0
        else if (_aulPoints[2] == ulP1)
            return 1; // Edge 1-2 ==> 1
    }
    else if (_aulPoints[2] == ulP0) {
        if (_aulPoints[0] == ulP1) return 2; // Edge 2-0 ==> 2
 ...
 */
template<class TCLASS> class MeshIsFlag
{
public:
 ...
 * Binary function to query the flags for use with generic STL functions.
 */
template<class TCLASS> class MeshIsNotFlag
{
public:
    using first_argument_type = TCLASS;
    using second_argument_type = typename TCLASS::TFlagType;
 ...
 */
template<class TCLASS> class MeshSetFlag
{
public:
    using first_argument_type = TCLASS;
    using second_argument_type = typename TCLASS::TFlagType;
 ...
 */
template<class TCLASS> class MeshResetFlag
{
public:
    using first_argument_type = TCLASS;
    using second_argument_type = typename TCLASS::TFlagType;

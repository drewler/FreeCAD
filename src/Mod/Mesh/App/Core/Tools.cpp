...
MeshSearchNeighbours::MeshSearchNeighbours(const MeshKernel &rclM, float fSampleDistance)
    : _rclMesh(rclM), _rclFAry(rclM.GetFacets()), _rclPAry(rclM.GetPoints()), _clPt2Fa(rclM),
      _fMaxDistanceP2(0), _fSampleDistance(fSampleDistance), _bTooFewPoints(false)
{
...
unsigned long
MeshSearchNeighbours::NeighboursFromFacet(FacetIndex ulFacetIdx, float fDistance,
                                          ...
                                          std::vector<Base::Vector3f> &raclResultPoints)
{
    ...
    while (bFound && (nCtExpandRadius < 10)) {
        ...
        for (std::set<FacetIndex>::iterator pI = aclTmp.begin(); pI != aclTmp.end(); ++pI) {
            ...
            for (std::set<FacetIndex>::const_iterator pJ = rclISet.begin(); pJ != rclISet.end();
                 ++pJ) {
...
{
    if (_aclSampledFacets.size() == _rclMesh.CountFacets()) return; // already sampled, do nothing
...
unsigned long
MeshSearchNeighbours::NeighboursFromSampledFacets(FacetIndex ulFacetIdx, float fDistance,
                                                  std::vector<Base::Vector3f> &raclResultPoints)
{
    SampleAllFacets();
    ...
    for (std::set<PointIndex>::iterator pI = _aclResult.begin(); pI != _aclResult.end(); ++pI) {
        if (InnerPoint(_rclPAry[*pI])) raclResultPoints.push_back(_rclPAry[*pI]);
    }
...
bool MeshSearchNeighbours::AccumulateNeighbours(const MeshFacet &rclF, FacetIndex ulFIdx)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (Base::DistanceP2(_clCenter, _rclPAry[ulPIdx]) < _fMaxDistanceP2) k++;
    }

    bool bFound = false;
    if (k == 3) { // add all sample points
        _aclPointsResult.insert(_aclPointsResult.end(), _aclSampledFacets[ulFIdx].begin(),
        ...
        if (bFound) {
            ...
            for (std::vector<Base::Vector3f>::iterator pI = rclT.begin(); pI != rclT.end(); ++pI) {
                if (InnerPoint(*pI)) clTmp.push_back(*pI);
            }
...
unsigned long
MeshSearchNeighbours::NeighboursFacetFromFacet(FacetIndex ulFacetIdx, float fDistance,
                                               std::vector<Base::Vector3f> &raclResultPoints,
                                               std::vector<FacetIndex> &raclResultFacets)
{
    ...
    while (bFound) {
        ...
        for (std::set<PointIndex>::iterator pI = aclTmp.begin(); pI != aclTmp.end(); ++pI) {
            const std::set<FacetIndex> &rclISet = _clPt2Fa[*pI];
            // search all facets hanging on this point
            for (std::set<PointIndex>::const_iterator pJ = rclISet.begin(); pJ != rclISet.end();
                 ++pJ) {
                const MeshFacet &rclF = f_beg[*pJ];

                for (int i = 0; i < 3; i++) {
                    if (Base::DistanceP2(_clCenter, _rclPAry[rclF._aulPoints[i]])
                        < _fMaxDistanceP2) {

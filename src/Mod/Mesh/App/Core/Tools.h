...
public:
   ...
   * MARKED flags.
   */
    unsigned long NeighboursFromFacet(FacetIndex ulFacetIdx, float fDistance,
    ...
    unsigned long NeighboursFromSampledFacets(FacetIndex ulFacetIdx, float fDistance,
                                              std::vector<Base::Vector3f> &raclResultPoints);
    /** Searches for facets from the start facet. */
    unsigned long NeighboursFacetFromFacet(FacetIndex ulFacetIdx, float fDistance,
                                           std::vector<Base::Vector3f> &raclResultPoints,
...
{
public:
    explicit MeshFaceIterator(const MeshKernel &mesh) : it(mesh) {}
...
{
public:
    explicit MeshVertexIterator(const MeshKernel &mesh) : it(mesh) {}
...
template<class T> class MeshNearestIndexToPlane
{
public:
    using Index = typename T::Index;
    MeshNearestIndexToPlane(const MeshKernel &mesh, const Base::Vector3f &b,

...
{
    switch (tMode) {
        case INNER: myInner = true; break;
        case OUTER: myInner = false; break;
    }
}
                               ...
                               std::vector<FacetIndex> &raulFacets) const
{
    ...
    if (myInner) {
        ...
        for (it = aulAllElements.begin(); it != aulAllElements.end(); ++it) {
            ...
            if (HasIntersection(clFacet)) raulFacets.push_back(*it);
            seq.next();
        }
    }
    // cut outer
        ...
        for (clIter.Init(); clIter.More(); clIter.Next()) {
            if (HasIntersection(*clIter)) raulFacets.push_back(clIter.Position());
            seq.next();
        }
    }
}
...
bool MeshTrimming::HasIntersection(const MeshGeomFacet &rclFacet) const
{
    ...
    for (size_t i = 0; i < 3; i++) {
        ...
        if (myPoly.Contains(Base::Vector2d(clPt2d.x, clPt2d.y)) == myInner) return true;
        else
    ...
    for (size_t j = 0; j < myPoly.GetCtVectors(); j++) {
        if (clPoly.Contains(myPoly[j])) return true;
    }
    ...
    for (size_t j = 0; j < myPoly.GetCtVectors(); j++) {
        ...
        for (size_t i = 0; i < 3; i++) {
            ...
            if (clPolyLine.IntersectAndContain(clFacLine, S)) return true;
        }
    }
...
bool MeshTrimming::PolygonContainsCompleteFacet(bool bInner, FacetIndex ulIndex) const
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (myPoly.Contains(Base::Vector2d(clPt.x, clPt.y)) != bInner) return false;
    }
...
bool MeshTrimming::GetIntersectionPointsOfPolygonAndFacet(
    FacetIndex ulIndex, int &iSide, std::vector<Base::Vector3f> &raclPoints) const
{
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (clPolyLine.Intersect(P1, double(MESH_MIN_PT_DIST))) {
                ...
                if ((fabs(l + m - 1.0f) < 0.001f) && (fabs(r + s - 1.0f) < 0.001f)) {
                    ...
                    if (j == 0) {
                        if (iIntsctWithEdge0 == 2) bPushBack = false;
                        else
                            iIntsctWithEdge0++;
                    }
                    else if (j == 1) {
                        if (iIntsctWithEdge1 == 2) bPushBack = false;
                        else
                            iIntsctWithEdge1++;
                    }
                    else {
                        if (iIntsctWithEdge2 == 2) bPushBack = false;
                        else
                            iIntsctWithEdge2++;
                    }

                    if (bPushBack) raclPoints.push_back(clIntersection);
                }
            }
        }
    }
    ...
    if (iIntsctWithEdge0 == 0 && iIntsctWithEdge1 == 0) iSide = 1;
...
bool MeshTrimming::CreateFacets(FacetIndex ulFacetPos, int iSide,
                                ...
                                std::vector<MeshGeomFacet> &aclNewFacets)
{
    ...
    if (raclPoints.empty()) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (myPoly.Intersect(clProjPnt, double(MESH_MIN_PT_DIST))) ++iCtPtsOn;
            else if (myPoly.Contains(clProjPnt) == myInner)
                ++iCtPtsIn;
        }

        // in this case we can use the original triangle
        if (iCtPtsIn != (3 - iCtPtsOn)) aclNewFacets.push_back(myMesh.GetFacet(ulFacetPos));
    }
    ...
    // two intersection points found
        ...
        for (int i = 0; i < 3; i++) {
            clFacPnt = (*myProj)(myMesh._aclPointArray[facet._aulPoints[i]]);
            clProjPnt = Base::Vector2d(clFacPnt.x, clFacPnt.y);
            if (myPoly.Contains(clProjPnt) == myInner) ++iCtPts;
        }
...
bool MeshTrimming::CreateFacets(FacetIndex ulFacetPos, int iSide,
                                const std::vector<Base::Vector3f> &raclPoints, Base::Vector3f &clP3,
                                std::vector<MeshGeomFacet> &aclNewFacets)
{
    // no valid triangulation possible
    if (iSide == -1 || raclPoints.size() < 2) return false;
                              ...
                              std::vector<MeshGeomFacet> &aclNewFacets)
{
    ...
    for (std::vector<FacetIndex>::const_iterator it = raulFacets.begin(); it != raulFacets.end();
         ++it) {
        clIntsct.clear();

...
struct MeshExport MeshHelpPoint {
    ...
    FacetIndex Index() const { return _ulInd >> 2; }

    FacetIndex Corner() const { return _ulInd & 3; }
...
struct MeshExport MeshHelpBuilderEdge {
    FacetIndex Side() const { return _ulFIndex & 3; }

    FacetIndex Index() const { return _ulFIndex >> 2; }
...
inline bool MeshHelpPoint::operator<(const MeshHelpPoint &rclObj) const
{
    if (_clPt.x == rclObj._clPt.x) {
        if (_clPt.y == rclObj._clPt.y) return _clPt.z < rclObj._clPt.z;
        else
...
inline void MeshPointBuilder::Add(FacetIndex ulCorner, FacetIndex ulFacet,
...
inline void MeshHelpBuilderEdge::Set(PointIndex ulInd1, PointIndex ulInd2, FacetIndex ulSide,
                                     FacetIndex ulFInd)
{
    if (ulInd1 < ulInd2) {
        _aulInd[0] = ulInd1;
...
{
    if (_aulInd[0] == rclObj._aulInd[0]) return _aulInd[1] < rclObj._aulInd[1];
    else
...
inline void MeshEdgeBuilder::Add(PointIndex ulInd1, PointIndex ulInd2, FacetIndex ulSide,
                                 FacetIndex ulFInd)
{

...
MeshPointArray::MeshPointArray(const MeshPointArray &ary) : TMeshPointArray(ary) {}
...
PointIndex MeshPointArray::Get(const MeshPoint &rclPoint)
{
    ...
    if (clIter != end()) return clIter - begin();
    else
...
{
    for (_TIterator pP = begin(); pP != end(); ++pP) mat.multVec(*pP, *pP);
}

MeshFacetArray::MeshFacetArray(const MeshFacetArray &ary) : TMeshFacetArray(ary) {}
...
void MeshFacetArray::Erase(_TIterator pIter)
{
    ...
    while (pPass < pEnd) {
        for (i = 0; i < 3; i++) {
            ...
            if ((*pulN > ulInd) && (*pulN != FACET_INDEX_MAX)) (*pulN)--;
        }
        pPass++;
    }
}
...
Base::BoundBox3f MeshGeomEdge::GetBoundBox() const { return Base::BoundBox3f(_aclPoints, 2); }
...
bool MeshGeomEdge::IntersectWithLine(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                                     Base::Vector3f &rclRes) const
{
    ...
    if (fabs(rclPt.DistanceToPlane(_aclPoints[0], normal)) > eps) return false;
...
bool MeshGeomEdge::IntersectWithEdge(const MeshGeomEdge &edge, Base::Vector3f &res) const
{
    ...
    if (n.IsNull()) {
        ...
        // lines are warped
        if (fabs(distance) > eps) return false;

        float t = d.Cross(s).Dot(n) / n.Sqr();
        float u = d.Cross(r).Dot(n) / n.Sqr();

        auto is_in_range = [](float v) { return v >= 0.0f && v <= 1.0f; };
...
bool MeshGeomEdge::IntersectWithPlane(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                                      Base::Vector3f &rclRes) const
{
...
bool MeshGeomEdge::IsPointOf(const Base::Vector3f &rclPoint, float fDistance) const
{
    ...
    if (len2 == 0.0f) { return _aclPoints[0].IsEqual(rclPoint, 0.0f); }
...
MeshGeomFacet::MeshGeomFacet() : _bNormalCalculated(false), _ucFlag(0), _ulProp(0) {}


MeshGeomFacet::MeshGeomFacet(const Base::Vector3f &v1, const Base::Vector3f &v2,
                             const Base::Vector3f &v3)
    : _bNormalCalculated(false), _ucFlag(0), _ulProp(0)
{
    _aclPoints[0] = v1;
    _aclPoints[1] = v2;
    _aclPoints[2] = v3;
...
{
    if (DistancePlaneToPoint(rclPoint) > fDistance) return false;
    ...
    if (fLP > 0.0f) {
        ...
        if (fLP <= fLE) {
            if (clProjPt.DistanceToLine(clP2, clEdge) > fLE) return false;
        }
        else
            return false;
    }
    ...
    if (fLP > 0.0f) {
        ...
        if (fLP <= fLE) {
            if (clProjPt.DistanceToLine(clP1, clEdge) > fLE) return false;
        }
        else
            return false;
    }
    ...
    if (fLP > 0.0f) {
        ...
        if (fLP <= fLE) {
            if (clProjPt.DistanceToLine(clP0, clEdge) > fLE) return false;
        }
        else
            return false;
    }
...
bool MeshGeomFacet::IsPointOfFace(const Base::Vector3f &rclP, float fDistance) const
{
    ...
    if (n * (p - a) > fDistance * n.Length()) return false;

    if (n * (a - p) > fDistance * n.Length()) return false;

    if (n * n1 <= 0.0f) return false;

    if (n * n2 <= 0.0f) return false;

    if (n * n3 <= 0.0f) return false;
...
bool MeshGeomFacet::IsDegenerated(float epsilon) const
{
    ...
    double uu = u * u;
    if (uu <= eps) return true;
    double vv = v * v;
    if (vv <= eps) return true;
    double uv = u * v;
    double crosssqr = uu * vv - uv * uv;
    if (crosssqr <= eps * std::max<double>(uu, vv)) return true;
    return false;
}
...
bool MeshGeomFacet::IsDeformed(float fCosOfMinAngle, float fCosOfMaxAngle) const
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (fCosAngle > fCosOfMinAngle || fCosAngle < fCosOfMaxAngle) return true;
    }
...
bool MeshGeomFacet::IntersectBoundingBox(const Base::BoundBox3f &rclBB) const
{
    ...
    if (rclBB.IsInBox(v0) || rclBB.IsInBox(v1) || rclBB.IsInBox(v2)) return true;
...
bool MeshGeomFacet::IntersectWithPlane(const Base::Vector3f &rclBase,
                                       const Base::Vector3f &rclNormal, Base::Vector3f &rclP1,
...
bool MeshGeomFacet::Foraminate(const Base::Vector3f &P, const Base::Vector3f &dir,
                               Base::Vector3f &I, float fMaxAngle) const
{
    const float eps = 1e-06f;
    ...
    if (fAngle > fMaxAngle) return false;
    ...
    if ((nd * nd) <= (eps * dd * nn)) return false;
                                           ...
                                           Base::Vector3f &rclRes) const
{
    ...
    if (fabs(rclDir * GetNormal()) < 1e-3f) return false; // line and plane are parallel
...
bool MeshGeomFacet::IntersectWithLine(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                                      Base::Vector3f &rclRes) const
{
    if (!IntersectPlaneWithLine(rclPt, rclDir, rclRes)) return false; // line and plane are parallel
...
void MeshGeomFacet::SubSample(float fStep, std::vector<Base::Vector3f> &rclPoints) const
{
    ...
    if (clPoints.empty()) clPoints.push_back(this->GetGravityPoint());
 ...
 * http://www.cs.lth.se/home/Tomas_Akenine_Moller/code/
 */
int MeshGeomFacet::IntersectWithFacet(const MeshGeomFacet &rclFacet, Base::Vector3f &rclPt0,
                                      Base::Vector3f &rclPt1) const
{
    ...
    if (IsCoplanar(rclFacet)) {
        ...
        for (short i = 0; i < 3; i++) {
            ...
            for (short j = 0; j < 3; j++) {
                ...
                Base::Vector3f point;
                if (edge1.IntersectWithEdge(edge2, point)) { intersections.push_back(point); }
            }
        }
    ...
    if (tri_tri_intersect_with_isectline(V[0], V[1], V[2], U[0], U[1], U[2], &coplanar, isectpt1,
                                         isectpt2)
        == 0)
    ...
    if (rclPt0 == rclPt1) {
        if (mult < 0.995f) // not co-planar, thus no test needed
            return 1;
        if (this->IsPointOf(rclPt0) && rclFacet.IsPointOf(rclPt0)) return 1;
    }
    else {
        if (mult < 0.995f) // not co-planar, thus no test needed
            return 2;
...
bool MeshGeomFacet::IsPointOf(const Base::Vector3f &P) const
{
    ...
    if ((s >= -eps) && (t >= -eps) && ((s + t) <= det + eps)) { return true; }
...
unsigned short MeshGeomFacet::NearestEdgeToPoint(const Base::Vector3f &rclPt) const
{
    ...
    if (t < 0.0f) fD1 = Base::Distance(rclPt, rcP1);
    else if (t > 1.0f)
    ...
    if (t < 0.0f) fD2 = Base::Distance(rclPt, rcP2);
    else if (t > 1.0f)
    ...
    if (t < 0.0f) fD3 = Base::Distance(rclPt, rcP3);
    else if (t > 1.0f)
...
void MeshGeomFacet::NearestEdgeToPoint(const Base::Vector3f &rclPt, float &fDistance,
...
float MeshGeomFacet::MaximumAngle() const
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (fAngle > fMaxAngle) fMaxAngle = fAngle;
    }
...
float MeshGeomFacet::MinimumAngle() const
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        Base::Vector3f dir2(_aclPoints[(i + 2) % 3] - _aclPoints[i]);
        float fAngle = dir1.GetAngle(dir2);
        if (fAngle < fMinAngle) fMinAngle = fAngle;
    }

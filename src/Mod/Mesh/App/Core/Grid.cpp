...
MeshGrid::MeshGrid(const MeshKernel &rclM)
    : _pclMesh(&rclM), _ulCtElements(0), _ulCtGridsX(0), _ulCtGridsY(0), _ulCtGridsZ(0),
      _fGridLenX(0.0f), _fGridLenY(0.0f), _fGridLenZ(0.0f), _fMinX(0.0f), _fMinY(0.0f), _fMinZ(0.0f)
{}

MeshGrid::MeshGrid()
    : _pclMesh(nullptr), _ulCtElements(0), _ulCtGridsX(MESH_CT_GRID), _ulCtGridsY(MESH_CT_GRID),
      _ulCtGridsZ(MESH_CT_GRID), _fGridLenX(0.0f), _fGridLenY(0.0f), _fGridLenZ(0.0f), _fMinX(0.0f),
      _fMinY(0.0f), _fMinZ(0.0f)
{}
...
void MeshGrid::InitGrid()
{
    ...
    for (i = 0; i < _ulCtGridsX; i++) {
        ...
        for (j = 0; j < _ulCtGridsY; j++) _aulGrid[i][j].resize(_ulCtGridsZ);
    }
}

unsigned long MeshGrid::Inside(const Base::BoundBox3f &rclBB,
                               std::vector<ElementIndex> &raulElements, bool bDelDoubles) const
{
    ...
    for (i = ulMinX; i <= ulMaxX; i++) {
        for (j = ulMinY; j <= ulMaxY; j++) {
            for (k = ulMinZ; k <= ulMaxZ; k++) {
                raulElements.insert(raulElements.end(), _aulGrid[i][j][k].begin(),
                                    _aulGrid[i][j][k].end());
            }
        }
    }
...
bool MeshGrid::CheckPosition(const Base::Vector3f &rclPoint, unsigned long &rulX,
                             unsigned long &rulY, unsigned long &rulZ) const
{
    ...
    if ((rulX < _ulCtGridsX) && (rulY < _ulCtGridsY) && (rulZ < _ulCtGridsZ)) return true;

    return false;
}

void MeshGrid::Position(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
                        unsigned long &rulZ) const
{
    if (rclPoint.x <= _fMinX) rulX = 0;
    else
        rulX = std::min<unsigned long>(
            static_cast<unsigned long>((rclPoint.x - _fMinX) / _fGridLenX), _ulCtGridsX - 1);

    if (rclPoint.y <= _fMinY) rulY = 0;
    else
        rulY = std::min<unsigned long>(
            static_cast<unsigned long>((rclPoint.y - _fMinY) / _fGridLenY), _ulCtGridsY - 1);

    if (rclPoint.z <= _fMinZ) rulZ = 0;
    else
        rulZ = std::min<unsigned long>(
            static_cast<unsigned long>((rclPoint.z - _fMinZ) / _fGridLenZ), _ulCtGridsZ - 1);
}
...
void MeshGrid::CalculateGridLength(int iCtGridPerAxis)
{
    ...
    if (bLenghtXisZero) iFlag += 1;
    else
        iMaxGrids *= iCtGridPerAxis;

    if (bLenghtYisZero) iFlag += 2;
    else
        iMaxGrids *= iCtGridPerAxis;

    if (bLenghtZisZero) iFlag += 4;
    else
        iMaxGrids *= iCtGridPerAxis;
    ...
    switch (iFlag) {
        ...
        case 1: {
            ...
            if ((fAreaGrid * iMaxGrids) < fArea) fAreaGrid = fArea / static_cast<float>(iMaxGrids);
                                      ...
                                      std::set<ElementIndex> &raclInd) const
{
    ...
    if (clBB.IsInBox(rclPt)) { // Point lies within
        ...
        while (raclInd.empty() && ulLevel <= ulMaxLevel) GetHull(ulX, ulY, ulZ, ulLevel++, raclInd);
                       ...
                       unsigned long ulDistance, std::set<ElementIndex> &raclInd) const
{
    ...
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) {
            GetElements(static_cast<unsigned long>(i), static_cast<unsigned long>(j),
                        static_cast<unsigned long>(nZ1), raclInd);
        }
    }
    // bottom plane
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) {
            GetElements(static_cast<unsigned long>(i), static_cast<unsigned long>(j),
                        static_cast<unsigned long>(nZ2), raclInd);
        }
    }
    // left plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            GetElements(static_cast<unsigned long>(nX1), static_cast<unsigned long>(i),
                        static_cast<unsigned long>(j), raclInd);
        }
    }
    // right plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            GetElements(static_cast<unsigned long>(nX2), static_cast<unsigned long>(i),
                        static_cast<unsigned long>(j), raclInd);
        }
    }
    // front plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            GetElements(static_cast<unsigned long>(i), static_cast<unsigned long>(nY1),
                        static_cast<unsigned long>(j), raclInd);
        }
    }
    // back plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            GetElements(static_cast<unsigned long>(i), static_cast<unsigned long>(nY2),
                        static_cast<unsigned long>(j), raclInd);
        }
    }
}
...
{
    unsigned long ulX, ulY, ulZ;
    if (!CheckPosition(rclPoint, ulX, ulY, ulZ)) return 0;
...
unsigned long MeshGrid::GetIndexToPosition(unsigned long ulX, unsigned long ulY,
                                           unsigned long ulZ) const
{
    if (!CheckPos(ulX, ulY, ulZ)) return ULONG_MAX;
    return (ulZ * _ulCtGridsY + ulY) * _ulCtGridsX + ulX;
}

bool MeshGrid::GetPositionToIndex(unsigned long id, unsigned long &ulX, unsigned long &ulY,
                                  unsigned long &ulZ) const
{
    ulX = id % _ulCtGridsX;
...
MeshFacetGrid::MeshFacetGrid(const MeshKernel &rclM) : MeshGrid(rclM) { RebuildGrid(); }

MeshFacetGrid::MeshFacetGrid(const MeshKernel &rclM, int iCtGridPerAxis) : MeshGrid(rclM)
{
    Rebuild(iCtGridPerAxis);
}

MeshFacetGrid::MeshFacetGrid(const MeshKernel &rclM, unsigned long ulX, unsigned long ulY,
                             unsigned long ulZ)
    : MeshGrid(rclM)
{
    Rebuild(ulX, ulY, ulZ);
...
MeshFacetGrid::MeshFacetGrid(const MeshKernel &rclM, float fGridLen) : MeshGrid(rclM)
{
    Base::BoundBox3f clBBMesh = _pclMesh->GetBoundBox();
...
{
    if (_pclMesh != &rclMesh) Attach(rclMesh);
...
void MeshFacetGrid::Validate()
{
    ...
    if (_pclMesh->CountFacets() != _ulCtElements) RebuildGrid();
}

bool MeshFacetGrid::Verify() const
{
    if (!_pclMesh) return false;                                // no mesh attached
    if (_pclMesh->CountFacets() != _ulCtElements) return false; // not up-to-date
    ...
    for (it.Init(); it.More(); it.Next()) {
        ...
        for (std::vector<ElementIndex>::iterator itF = aulElements.begin();
             itF != aulElements.end(); ++itF) {
            cF.Set(*itF);
                                                    ...
                                                    float fMaxSearchArea) const
{
    ...
    Base::BoundBox3f clBB(rclPt.x - fMaxSearchArea, rclPt.y - fMaxSearchArea,
                          rclPt.z - fMaxSearchArea, rclPt.x + fMaxSearchArea,
                          rclPt.y + fMaxSearchArea, rclPt.z + fMaxSearchArea);

    Inside(clBB, aulFacets, rclPt, fMaxSearchArea, true);

    for (std::vector<ElementIndex>::const_iterator pI = aulFacets.begin(); pI != aulFacets.end();
         ++pI) {
        float fDist;
...
void MeshFacetGrid::SearchNearestFacetInHull(unsigned long ulX, unsigned long ulY,
                                             unsigned long ulZ, unsigned long ulDistance,
                                             const Base::Vector3f &rclPt, ElementIndex &rulFacetInd,
                                             float &rfMinDist) const
{
    ...
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(i), static_cast<unsigned long>(j),
                                     static_cast<unsigned long>(nZ1), rclPt, rfMinDist,
                                     rulFacetInd);
        }
    }
    // bottom plane
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(i), static_cast<unsigned long>(j),
                                     static_cast<unsigned long>(nZ2), rclPt, rfMinDist,
                                     rulFacetInd);
        }
    }
    // left plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(nX1), static_cast<unsigned long>(i),
                                     static_cast<unsigned long>(j), rclPt, rfMinDist, rulFacetInd);
        }
    }
    // right plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(nX2), static_cast<unsigned long>(i),
                                     static_cast<unsigned long>(j), rclPt, rfMinDist, rulFacetInd);
        }
    }
    // front plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(i), static_cast<unsigned long>(nY1),
                                     static_cast<unsigned long>(j), rclPt, rfMinDist, rulFacetInd);
        }
    }
    // back plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) {
            SearchNearestFacetInGrid(static_cast<unsigned long>(i), static_cast<unsigned long>(nY2),
                                     static_cast<unsigned long>(j), rclPt, rfMinDist, rulFacetInd);
        }
    }
}

void MeshFacetGrid::SearchNearestFacetInGrid(unsigned long ulX, unsigned long ulY,
                                             unsigned long ulZ, const Base::Vector3f &rclPt,
...
MeshPointGrid::MeshPointGrid(const MeshKernel &rclM) : MeshGrid(rclM) { RebuildGrid(); }

MeshPointGrid::MeshPointGrid() : MeshGrid() {}

MeshPointGrid::MeshPointGrid(const MeshKernel &rclM, unsigned long ulX, unsigned long ulY,
                             unsigned long ulZ)
    : MeshGrid(rclM)
{
    Rebuild(ulX, ulY, ulZ);
...
MeshPointGrid::MeshPointGrid(const MeshKernel &rclM, int iCtGridPerAxis) : MeshGrid(rclM)
{
    Rebuild(iCtGridPerAxis);
}

MeshPointGrid::MeshPointGrid(const MeshKernel &rclM, float fGridLen) : MeshGrid(rclM)
{
    Base::BoundBox3f clBBMesh = _pclMesh->GetBoundBox();
    Rebuild(std::max<unsigned long>(static_cast<unsigned long>(clBBMesh.LengthX() / fGridLen), 1),
            std::max<unsigned long>(static_cast<unsigned long>(clBBMesh.LengthY() / fGridLen), 1),
...
void MeshPointGrid::Validate()
{
    ...
    if (_pclMesh->CountPoints() != _ulCtElements) RebuildGrid();
}
...
bool MeshPointGrid::Verify() const
{
    ...
    for (it.Init(); it.More(); it.Next()) {
        std::vector<ElementIndex> aulElements;
        it.GetElements(aulElements);
        for (std::vector<ElementIndex>::iterator itP = aulElements.begin();
             itP != aulElements.end(); ++itP) {
            cP.Set(*itP);
...
void MeshPointGrid::RebuildGrid()
{
    ...
    unsigned long i = 0;
    for (cPIter.Init(); cPIter.More(); cPIter.Next()) { AddPoint(*cPIter, i++); }
}

void MeshPointGrid::Pos(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
                        unsigned long &rulZ) const
{
    rulX = static_cast<unsigned long>((rclPoint.x - _fMinX) / _fGridLenX);
    rulY = static_cast<unsigned long>((rclPoint.y - _fMinY) / _fGridLenY);
...
MeshGridIterator::MeshGridIterator(const MeshGrid &rclG)
    : _rclGrid(rclG), _ulX(0), _ulY(0), _ulZ(0), _clPt(0.0f, 0.0f, 0.0f), _clDir(0.0f, 0.0f, 0.0f),
      _bValidRay(false), _fMaxSearchArea(FLOAT_MAX)

{}

bool MeshGridIterator::InitOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                                 float fMaxSearchArea, std::vector<ElementIndex> &raulElements)
{
                                 ...
                                 std::vector<ElementIndex> &raulElements)
{
    ...
    if (_rclGrid.GetBoundBox().IsInBox(rclPt)) { // Determine the voxel by the starting point
        ...
        raulElements.insert(raulElements.end(), _rclGrid._aulGrid[_ulX][_ulY][_ulZ].begin(),
                            _rclGrid._aulGrid[_ulX][_ulY][_ulZ].end());
        _bValidRay = true;
    }
    else { // Start point outside
        Base::Vector3f cP0, cP1;
        if (_rclGrid.GetBoundBox().IntersectWithLine(rclPt, rclDir, cP0,
...
{
    if (!_bValidRay) return false; // not initialized or beam exited
    ...
    // Search area
    //
    if ((_clPt - clIntersectPoint).Length() > _fMaxSearchArea) { _bValidRay = false; }
    else {
        switch (tSide) {
            ...
            default:
            case Base::BoundBox3f::INVALID: _bValidRay = false; break;
        }

...
public:
    ...
    {
        for (std::size_t i = 0; i < 10; ++i) m[i] = c;
    }

    SymmetricMatrix(double m11, double m12, double m13, double m14, double m22, double m23,
    ...
    {
        return SymmetricMatrix(m[0] + n[0], m[1] + n[1], m[2] + n[2], m[3] + n[3], m[4] + n[4],
...
void Simplify::simplify_mesh(int target_count, double tolerance, double aggressiveness)
{
    ...
    for (std::size_t i = 0; i < triangles.size(); ++i) triangles[i].deleted = 0;
    ...
    for (int iteration = 0; iteration < 100; ++iteration) {
        ...
        // clear dirty flag
        for (std::size_t i = 0; i < triangles.size(); ++i) triangles[i].dirty = 0;
...
void Simplify::update_triangles(int i0, Vertex &v, std::vector<int> &deleted,
...
void Simplify::update_mesh(int iteration)
{
    ...
    //
    if (iteration == 0) {
        for (std::size_t i = 0; i < vertices.size(); ++i) vertices[i].q = SymmetricMatrix(0.0);

        for (std::size_t i = 0; i < triangles.size(); ++i) {
            Triangle &t = triangles[i];
            vec3f n, p[3];
            for (std::size_t j = 0; j < 3; ++j) p[j] = vertices[t.v[j]].p;
    ...
    for (std::size_t i = 0; i < triangles.size(); ++i) {
        Triangle &t = triangles[i];
        for (std::size_t j = 0; j < 3; ++j) vertices[t.v[j]].tcount++;
    }
    int tstart = 0;
    for (std::size_t i = 0; i < vertices.size(); ++i) {
        Vertex &v = vertices[i];
    ...
    if (iteration == 0) {
        ...
        for (std::size_t i = 0; i < vertices.size(); ++i) vertices[i].border = 0;
...
{
    int dst = 0;
    for (std::size_t i = 0; i < vertices.size(); ++i) { vertices[i].tcount = 0; }
    for (std::size_t i = 0; i < triangles.size(); ++i) {
        if (!triangles[i].deleted) {
            Triangle &t = triangles[i];
            triangles[dst++] = t;
            for (std::size_t j = 0; j < 3; ++j) vertices[t.v[j]].tcount = 1;
        }
    }
    ...
    for (std::size_t i = 0; i < triangles.size(); ++i) {
        Triangle &t = triangles[i];
        for (std::size_t j = 0; j < 3; ++j) t.v[j] = vertices[t.v[j]].tstart;
    }
    vertices.resize(dst);
}

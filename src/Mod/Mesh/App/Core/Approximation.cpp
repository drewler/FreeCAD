...
Approximation::Approximation() : _bIsFitted(false), _fLastResult(FLOAT_MAX) {}

Approximation::~Approximation() { Clear(); }
...
std::size_t Approximation::CountPoints() const { return _vPoints.size(); }
...
float Approximation::GetLastResult() const { return _fLastResult; }

bool Approximation::Done() const { return _bIsFitted; }

// -------------------------------------------------------------------------------

PlaneFit::PlaneFit() : _vBase(0, 0, 0), _vDirU(1, 0, 0), _vDirV(0, 1, 0), _vDirW(0, 0, 1) {}
...
{
    _bIsFitted = true;
    if (CountPoints() < 3) return FLOAT_MAX;
    ...
    if (rkDiag(1, 1) <= 0) return FLOAT_MAX;
    ...
    for (int i = 0; i < 3; i++) {
        if (boost::math::isnan(W[i])) return FLOAT_MAX;
    }
    ...
    if (!validUV) { Wm4::Vector3<double>::GenerateOrthonormalBasis(U, V, W); }
    ...
    if (boost::math::isnan(sigma)) return FLOAT_MAX;
    ...
    if (nSize > 3) sigma = sqrt(sigma / (nSize - 3));
    else
        sigma = 0;
...
float PlaneFit::GetDistanceToPlane(const Base::Vector3f &rcPoint) const
{
    ...
    if (_bIsFitted) fResult = (rcPoint - _vBase) * _vDirW;
    return fResult;
}
...
float PlaneFit::GetStdDeviation() const
{
    ...
    if (!_bIsFitted) return FLOAT_MAX;
...
float PlaneFit::GetSignedStdDeviation() const
{
    ...
    std::list<Base::Vector3f>::const_iterator cIt;
    for (cIt = _vPoints.begin(); cIt != _vPoints.end(); ++cIt) clGravity += *cIt;
    ...
    // which side
    if ((clPt - clGravity) * GetNormal() > 0) fFactor = 1.0f;
    else
        fFactor = -1.0f;
...
std::vector<Base::Vector3f> PlaneFit::GetLocalPoints() const
{
    ...
    if (_bIsFitted && _fLastResult < FLOAT_MAX) {
        ...
        for (std::vector<Base::Vector3f>::iterator it = localPoints.begin();
             it != localPoints.end(); ++it) {
            Base::Vector3d clPoint = Base::convertTo<Base::Vector3d>(*it);
            clPoint.TransformToCoordinateSystem(bs, ex, ey);
            it->Set(static_cast<float>(clPoint.x), static_cast<float>(clPoint.y),
...
Base::BoundBox3f PlaneFit::GetBoundings() const
{
    ...
    for (const auto &it : pts) bbox.Add(it);
    return bbox;
}
...
bool QuadraticFit::GetCurvatureInfo(double x, double y, double z, double &rfCurv0, double &rfCurv1,
                                    Base::Vector3f &rkDir0, Base::Vector3f &rkDir1,
                                    double &dDistance)
{
    assert(_bIsFitted);
    bool bResult = false;
...
const double &QuadraticFit::GetCoeffArray() const { return _fCoeff[0]; }

double QuadraticFit::GetCoeff(std::size_t ulIndex) const
{
    assert(ulIndex < 10);

    if (_bIsFitted) return _fCoeff[ulIndex];
    else
...
void QuadraticFit::CalcEigenValues(double &dLambda1, double &dLambda2, double &dLambda3,
                                   Base::Vector3f &clEV1, Base::Vector3f &clEV2,
                                   Base::Vector3f &clEV3) const
{
    ...
    Wm4::Matrix3<double> akMat(_fCoeff[4], _fCoeff[7] / 2.0, _fCoeff[8] / 2.0, _fCoeff[7] / 2.0,
                               _fCoeff[5], _fCoeff[9] / 2.0, _fCoeff[8] / 2.0, _fCoeff[9] / 2.0,
                               _fCoeff[6]);
...
SurfaceFit::SurfaceFit() : PlaneFit()
{
    _fCoeff[0] = 0.0;
    _fCoeff[1] = 0.0;
    _fCoeff[2] = 0.0;
...
{
    if (PlaneFit::Fit() >= FLOAT_MAX) return double(FLOAT_MAX);
    ...
    double sigma = 0;
    FunctionContainer clFuncCont(_fCoeff);
    for (std::vector<Base::Vector3d>::const_iterator it = transform.begin(); it != transform.end();
         ++it) {
        double u = it->x;
        double v = it->y;
    ...
    if (sigma < 0) sigma = 0;
    if (!_vPoints.empty()) sigma = sqrt(sigma / _vPoints.size());
...
void SurfaceFit::GetCoefficients(double &a, double &b, double &c, double &d, double &e,
                                 double &f) const
{
    a = _fCoeff[4];
    b = _fCoeff[5];
 ...
 */
std::vector<Base::Vector3d> SurfaceFit::toBezier(double umin, double umax, double vmin,
                                                 double vmax) const
{
...
CylinderFit::CylinderFit() : _vBase(0, 0, 0), _vAxis(0, 0, 1), _fRadius(0), _initialGuess(false) {}
...
{
    if (CountPoints() < 7) return FLOAT_MAX;
    _bIsFitted = true;
    ...
    if (_initialGuess)
        cylFit.SetApproximations(_fRadius, Base::Vector3d(_vBase.x, _vBase.y, _vBase.z),
    ...
    if (result < FLOAT_MAX) {
        ...
        Base::Console().Log(
            ...
            base.x, base.y, base.z, dir.x, dir.y, dir.z, cylFit.GetRadius(),
            cylFit.GetStdDeviation(), cylFit.GetNumIterations());
#endif
...
float CylinderFit::GetRadius() const { return _fRadius; }
...
{
    float fResult = FLOAT_MAX;
    if (_bIsFitted) fResult = rcPoint.DistanceToLine(_vBase, _vAxis) - _fRadius;
    return fResult;
}
...
SphereFit::SphereFit() : _vCenter(0, 0, 0), _fRadius(0) {}

SphereFit::~SphereFit() {}

float SphereFit::GetRadius() const
{
    if (_bIsFitted) return _fRadius;
    else
        return FLOAT_MAX;
}
...
{
    _bIsFitted = true;
    if (CountPoints() < 4) return FLOAT_MAX;
    ...
    if (result < FLOAT_MAX) {
        ...
        Base::Console().Message("MeshCoreFit::Sphere Fit:  Center: (%0.4f, %0.4f, %0.4f),  Radius: "
                                ...
                                center.x, center.y, center.z, sphereFit.GetRadius(),
                                sphereFit.GetStdDeviation(), sphereFit.GetNumIterations());
#endif
...
{
    float fResult = FLOAT_MAX;
    if (_bIsFitted) { fResult = Base::Vector3f(rcPoint - _vCenter).Length() - _fRadius; }
    return fResult;
}
...
{
    for (int i = 0; i < 9; i++) _fCoeff[i] = 0.0f;
}
...
float PolynomialFit::Fit()
{
    ...
    try {
        ...
        for (int i = 0; i < 9; i++) _fCoeff[i] = coeff[i];
    }
    catch (const std::exception &) {
        return FLOAT_MAX;
    }

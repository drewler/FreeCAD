...
public:
     ...
     */
    virtual bool AllowVisit(const MeshFacet &rclFacet, const MeshFacet &rclFrom, FacetIndex ulFInd,
                            unsigned long ulLevel, unsigned short neighbourIndex)
    {
        (void)rclFacet;
        (void)rclFrom;
        (void)ulFInd;
...
{
public:
    MeshSearchNeighbourFacetsVisitor(const MeshKernel &rclMesh, float fRadius,
                                     FacetIndex ulStartFacetIdx);
    ~MeshSearchNeighbourFacetsVisitor() override {}
    /** Checks the facet if it lies inside the search radius. */
    inline bool Visit(const MeshFacet &rclFacet, const MeshFacet &rclFrom, FacetIndex ulFInd,
                                                    ...
                                                    unsigned long ulLevel)
{
    ...
    if (ulLevel > _ulCurrentLevel) {
        if (!_bFacetsFoundInCurrentLevel) return false;
...
{
public:
    MeshPlaneVisitor(const MeshKernel &mesh, FacetIndex index, float deviation,
                     std::vector<FacetIndex> &indices);
    ~MeshPlaneVisitor() override;
    bool AllowVisit(const MeshFacet &face, const MeshFacet &, FacetIndex, unsigned long,
...
public:
     ...
     */
    virtual bool Visit(const MeshPoint &rclPoint, const MeshPoint &rclFrom, FacetIndex ulPInd,
                       unsigned long ulLevel) = 0;
};

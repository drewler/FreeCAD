...
SphereFit::SphereFit()
    : _vCenter(0, 0, 0), _dRadius(0), _numIter(0), _posConvLimit(0.0001), _vConvLimit(0.001),
      _maxIter(50)
{}
...
float SphereFit::GetDistanceToSphere(const Base::Vector3f &rcPoint) const
{
    ...
    if (_bIsFitted) {
        fResult = Base::Vector3d((double)rcPoint.x - _vCenter.x, (double)rcPoint.y - _vCenter.y,
...
void SphereFit::ProjectToSphere()
{
    for (std::list<Base::Vector3f>::iterator it = _vPoints.begin(); it != _vPoints.end(); ++it) {
        ...
        Base::Vector3d diff((double)cPnt.x - _vCenter.x, (double)cPnt.y - _vCenter.y,
...
void SphereFit::ComputeApproximations()
{
    ...
    if (!_vPoints.empty()) {
        ...
        for (cIt = _vPoints.begin(); cIt != _vPoints.end(); ++cIt) {
            Base::Vector3d diff((double)cIt->x - _vCenter.x, (double)cIt->y - _vCenter.y,
...
float SphereFit::Fit()
{
    ...
    if (_dRadius == 0.0) ComputeApproximations();
    ...
    while (cont && (_numIter < _maxIter)) {
        ...
        bool vConverged;
        if (!computeResiduals(x, residuals, sigma0, _vConvLimit, vConverged)) return FLOAT_MAX;
...
void SphereFit::setupNormalEquationMatrices(const std::vector<Base::Vector3d> &residuals,
                                            Matrix4x4 &atpa, Eigen::VectorXd &atpl) const
{
    // Zero matrices
    atpa.setZero();
    atpl.setZero();
...
void SphereFit::setupObservation(const Base::Vector3f &point, const Base::Vector3d &residual,
                                 double a[4], double &f0, double &qw, double b[3]) const
{
...
void SphereFit::addObservationU(double a[4], double li, double pi, Matrix4x4 &atpa,
...
bool SphereFit::computeResiduals(const Eigen::VectorXd &x, std::vector<Base::Vector3d> &residuals,

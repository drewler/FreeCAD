...
bool TriangulationVerifier::Accept(const Base::Vector3f &n, const Base::Vector3f &p1,
                                   const Base::Vector3f &p2, const Base::Vector3f &p3) const
{
...
bool TriangulationVerifierV2::Accept(const Base::Vector3f &n, const Base::Vector3f &p1,
                                     const Base::Vector3f &p2, const Base::Vector3f &p3) const
{
    float ref_dist = (p2 - p1) * n;
    float tri_dist = (p3 - p1) * n;
...
AbstractPolygonTriangulator::~AbstractPolygonTriangulator() { delete _verifier; }

TriangulationVerifier *AbstractPolygonTriangulator::GetVerifier() const { return _verifier; }
...
void AbstractPolygonTriangulator::SetPolygon(const std::vector<Base::Vector3f> &raclPoints)
{
    ...
    if (!this->_points.empty()) {
        if (this->_points.front() == this->_points.back()) this->_points.pop_back();
    }
}

std::vector<Base::Vector3f> AbstractPolygonTriangulator::GetPolygon() const { return _points; }
...
std::vector<Base::Vector3f> AbstractPolygonTriangulator::AddedPoints() const
{
    ...
    for (std::vector<Base::Vector3f>::const_iterator pt = _newpoints.begin();
         pt != _newpoints.end(); ++pt)
...
Base::Matrix4D AbstractPolygonTriangulator::GetTransformToFitPlane() const
{
    ...
    if (planeFit.Fit() >= FLOAT_MAX) throw Base::RuntimeError("Plane fit failed");
...
std::vector<Base::Vector3f> AbstractPolygonTriangulator::ProjectToFitPlane()
{
    ...
    Base::Vector3f bs(static_cast<float>(_inverse[0][3]), static_cast<float>(_inverse[1][3]),
                      static_cast<float>(_inverse[2][3]));
    Base::Vector3f ex(static_cast<float>(_inverse[0][0]), static_cast<float>(_inverse[1][0]),
                      static_cast<float>(_inverse[2][0]));
    Base::Vector3f ey(static_cast<float>(_inverse[0][1]), static_cast<float>(_inverse[1][1]),
...
void AbstractPolygonTriangulator::PostProcessing(const std::vector<Base::Vector3f> &points)
{
    ...
    for (std::vector<Base::Vector3f>::const_iterator it = points.begin(); it != points.end();
         ++it) {
        Base::Vector3f pt = *it;
    ...
    if (polyFit.CountPoints() >= uMinPts && polyFit.Fit() < FLOAT_MAX) {
        for (std::vector<Base::Vector3f>::iterator pt = _newpoints.begin(); pt != _newpoints.end();
             ++pt)
...
std::vector<PointIndex> AbstractPolygonTriangulator::GetInfo() const { return _info; }
...
bool EarClippingTriangulator::Triangulate()
{
    ...
    if (tcount > _points.size()) return false; // no valid triangulation
...
/*
  ...
  defined by A, B, C.
*/
bool EarClippingTriangulator::Triangulate::InsideTriangle(float Ax, float Ay, float Bx, float By,
...
bool EarClippingTriangulator::Triangulate::Snip(const std::vector<Base::Vector3f> &contour, int u,
                                                int v, int w, int n, int *V)
{
    int p;
...
{
    if (!EarClippingTriangulator::Triangulate()) return false; // no valid triangulation
    ...
    for (pE = aEdge2Face.begin(); pE != aEdge2Face.end(); ++pE) aEdgeList.push_back(pE->first);
    ...
    while (!aEdgeList.empty() && uMaxIter > 0) {
        ...
        if (((cP3 - cP1) * cN1) * ((cP4 - cP1) * cN1) >= 0.0f) continue; // not convex
        // build a helper plane through cP3 that must separate cP1 and cP2
        Base::Vector3f cN2 = (cU % cV) % cV;
        if (((cP1 - cP3) * cN2) * ((cP2 - cP3) * cN2) >= 0.0f) continue; // not convex
...
struct Vertex2d_Less {
    bool operator()(const Base::Vector3f &p, const Base::Vector3f &q) const
    {
        if (fabs(p.x - q.x) < MeshDefinitions::_fMinPointDistanceD1) {
            if (fabs(p.y - q.y) < MeshDefinitions::_fMinPointDistanceD1) { return false; }
            else {
                return p.y < q.y;
            }
        }
...
bool DelaunayTriangulator::Triangulate()
{
    ...
    Wm4::Delaunay2d del(static_cast<int>(akVertex.size()), &(akVertex[0]), 0.001, false,
                        Wm4::Query::QT_INT64);
...
ConstraintDelaunayTriangulator::ConstraintDelaunayTriangulator(float area) : fMaxArea(area)
{

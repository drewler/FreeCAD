    ...
    while (!clCurrentLevel.empty()) {
        ...
        for (clCurrIter = clCurrentLevel.begin(); clCurrIter < clCurrentLevel.end(); ++clCurrIter) {
            ...
            for (unsigned short i = 0; i < 3; i++) {
                ...
                if (j == FACET_INDEX_MAX) continue; // no neighbour facet

                if (j >= ulCount) continue; // error in data structure

                clNBFacet = _aclFacetArray.begin() + j;

                if (!rclFVisitor.AllowVisit(*clNBFacet, *clCurrFacet, j, ulLevel, i)) continue;
                if (clNBFacet->IsFlag(MeshFacet::VISIT))
                    ...
                    if (!rclFVisitor.Visit(*clNBFacet, *clCurrFacet, j, ulLevel)) return ulVisited;
                }
            }
        }
                                                          ...
                                                          FacetIndex ulStartFacet) const
{
    ...
    while (!aclCurrentLevel.empty()) {
        ...
        for (std::vector<FacetIndex>::iterator pCurrFacet = aclCurrentLevel.begin();
             pCurrFacet < aclCurrentLevel.end(); ++pCurrFacet) {
            for (int i = 0; i < 3; i++) {
                const MeshFacet &rclFacet = raclFAry[*pCurrFacet];
                const std::set<FacetIndex> &raclNB = clRPF[rclFacet._aulPoints[i]];
                for (std::set<FacetIndex>::const_iterator pINb = raclNB.begin();
                     pINb != raclNB.end(); ++pINb) {
                    if (!pFBegin[*pINb].IsFlag(MeshFacet::VISIT)) {
                        ...
                        if (!rclFVisitor.Visit(pFBegin[*pINb], raclFAry[*pCurrFacet], ulFInd,
                                               ulLevel))
                            return ulVisited;
                    }
                }
                                               ...
                                               PointIndex ulStartPoint) const
{
    ...
    while (!aclCurrentLevel.empty()) {
        ...
        for (clCurrIter = aclCurrentLevel.begin(); clCurrIter < aclCurrentLevel.end();
            ...
            for (std::set<PointIndex>::const_iterator pINb = raclNB.begin(); pINb != raclNB.end();
                 ++pINb) {
                if (!pPBegin[*pINb].IsFlag(MeshPoint::VISIT)) {
                    ...
                    if (!rclPVisitor.Visit(pPBegin[*pINb], *(pPBegin + (*clCurrIter)), ulPInd,
                                           ulLevel))
                        return ulVisited;
                }
            }
...
MeshSearchNeighbourFacetsVisitor::MeshSearchNeighbourFacetsVisitor(const MeshKernel &rclMesh,
    ...
    : _rclMeshBase(rclMesh), _clCenter(rclMesh.GetFacet(ulStartFacetIdx).GetGravityPoint()),
      _fRadius(fRadius), _ulCurrentLevel(0), _bFacetsFoundInCurrentLevel(false)
{}
...
MeshPlaneVisitor::MeshPlaneVisitor(const MeshKernel &mesh, FacetIndex index, float deviation,
...
MeshPlaneVisitor::~MeshPlaneVisitor() { delete fitter; }

bool MeshPlaneVisitor::AllowVisit(const MeshFacet &face, const MeshFacet &, FacetIndex,
                                  unsigned long, unsigned short)
{
    if (!fitter->Done()) fitter->Fit();
    MeshGeomFacet triangle = mesh.GetFacet(face);
    for (int i = 0; i < 3; i++) {
        if (fabs(fitter->GetDistanceToPlane(triangle._aclPoints[i])) > max_deviation) return false;
    }
    return true;
}

bool MeshPlaneVisitor::Visit(const MeshFacet &face, const MeshFacet &, FacetIndex ulFInd,
                             unsigned long)
{
    MeshGeomFacet triangle = mesh.GetFacet(face);

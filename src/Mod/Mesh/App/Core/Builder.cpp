...
void MeshBuilder::SetTolerance(float fTol) { MeshDefinitions::_fMinPointDistanceD1 = fTol; }
...
void MeshBuilder::Initialize(size_t ctFacets, bool deletion)
{
    if (deletion) {
        ...
        for (MeshPointArray::_TConstIterator it1 = _meshKernel._aclPointArray.begin();
             it1 != _meshKernel._aclPointArray.end(); ++it1) {
...
void MeshBuilder::AddFacet(const MeshGeomFacet &facet, bool takeFlag, bool takeProperty)
{
    ...
    if (takeFlag) flag = facet._ucFlag;
    if (takeProperty) prop = facet._ulProp;

    AddFacet(facet._aclPoints[0], facet._aclPoints[1], facet._aclPoints[2], facet.GetNormal(), flag,
             prop);
}

void MeshBuilder::AddFacet(const Base::Vector3f &pt1, const Base::Vector3f &pt2,
                           const Base::Vector3f &pt3, const Base::Vector3f &normal,
                           unsigned char flag, unsigned long prop)
{
...
void MeshBuilder::SetNeighbourhood()
{
    ...
    for (MeshFacetArray::_TIterator it = _meshKernel._aclFacetArray.begin();
         it != _meshKernel._aclFacetArray.end(); ++it) {
        this->_seq->next(true); // allow to cancel
        MeshFacet &mf = *it;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (e != edges.end()) { // edge exists, set neighbourhood
                ...
                if (mf1._aulPoints[0] == edge.pt1) {
                    if (mf1._aulPoints[1] == edge.pt2) mf1._aulNeighbours[0] = facetIdx;
                    else
                        mf1._aulNeighbours[2] = facetIdx;
                }
                else if (mf1._aulPoints[0] == edge.pt2) {
                    if (mf1._aulPoints[1] == edge.pt1) mf1._aulNeighbours[0] = facetIdx;
                    else
                        mf1._aulNeighbours[2] = facetIdx;
                }
                else
...
void MeshBuilder::RemoveUnreferencedPoints()
{
    ...
    for (MeshFacetArray::_TConstIterator it = _meshKernel._aclFacetArray.begin();
         it != _meshKernel._aclFacetArray.end(); ++it) {
        for (int i = 0; i < 3; i++) _meshKernel._aclPointArray[it->_aulPoints[i]].ResetInvalid();
    }

    unsigned long uValidPts =
        std::count_if(_meshKernel._aclPointArray.begin(), _meshKernel._aclPointArray.end(),
                      [](const MeshPoint &p) { return p.IsValid(); });
    if (uValidPts < _meshKernel.CountPoints()) _meshKernel.RemoveInvalids();
}
...
void MeshBuilder::Finish(bool freeMemory)
{
    ...
    for (std::vector<MeshPointIterator>::iterator it = _pointsIterator.begin();
         it != _pointsIterator.end(); ++it)
...
struct MeshFastBuilder::Private {
    struct Vertex {
        ...
        bool operator!=(const Vertex &rhs) const { return x != rhs.x || y != rhs.y || z != rhs.z; }
...
MeshFastBuilder::~MeshFastBuilder() { delete p; }

void MeshFastBuilder::Initialize(size_type ctFacets) { p->verts.reserve(ctFacets * 3); }
...
void MeshFastBuilder::Finish()
{
    ...
    for (size_type i = 0; i < ulCtPts; ++i) { verts[i].i = i; }
    ...
    for (QVector<Private::Vertex>::iterator v = verts.begin(); v != verts.end(); ++v) {
        if (!vertex_count || *v != verts[vertex_count - 1]) verts[vertex_count++] = *v;

...
Writer3MF::Writer3MF(std::ostream &str) : zip(str), objectIndex(0)
{
    zip.putNextEntry("3D/3dmodel.model");
    Initialize(zip);
}

Writer3MF::Writer3MF(const std::string &filename) : zip(filename), objectIndex(0)
{
    zip.putNextEntry("3D/3dmodel.model");
    Initialize(zip);
}
...
void Writer3MF::Finish(std::ostream &str)
{
    ...
    for (const auto &it : items) str << Base::blanks(2) << it;
    str << Base::blanks(1) << "</build>\n";
    str << "</model>\n";
}
...
void Writer3MF::AddResource(const Resource3MF &res) { resources.emplace_back(res); }
...
bool Writer3MF::Save()
{
    ...
    if (!SaveRels(zip)) return false;
    zip.closeEntry();

    zip.putNextEntry("[Content_Types].xml");
    if (!SaveContent(zip)) return false;
    zip.closeEntry();
...
{
    if (MeshEvalSolid(mesh).Evaluate()) return "model";
    else
        return "surface";
}

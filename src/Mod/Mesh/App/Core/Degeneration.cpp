...
std::vector<FacetIndex> MeshEvalInvalids::GetIndices() const
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it, ind++) {
        if (!it->IsValid()) aInds.push_back(ind);
...
struct Vertex_Less {
    bool operator()(const VertexIterator &x, const VertexIterator &y) const { return (*x) < (*y); }
};
...
bool MeshFixDuplicatePoints::Fixup()
{
    ...
    for (MeshFacetArray::_TIterator it = rFacets.begin(); it != rFacets.end(); ++it) {
        for (int i = 0; i < 3; i++) {
            ...
            if (pt != mapPointIndex.end()) it->_aulPoints[i] = pt->second;
        }
    }
...
std::vector<FacetIndex> MeshEvalDuplicateFacets::GetIndices() const
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it, uIndex++) {
        ...
        if (!pI.second) aInds.push_back(uIndex);
    }
...
bool MeshFixDuplicateFacets::Fixup()
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it, uIndex++) {
        std::pair<std::set<FaceIterator, MeshFacet_Less>::iterator, bool> pI = aFaceSet.insert(it);
        if (!pI.second) aRemoveFaces.push_back(uIndex);
    }
...
bool MeshEvalDegeneratedFacets::Evaluate()
{
    ...
    for (it.Init(); it.More(); it.Next()) {
        if (it->IsDegenerated(fEpsilon)) return false;
    }
...
std::vector<FacetIndex> MeshEvalDegeneratedFacets::GetIndices() const
{
    ...
    MeshFacetIterator it(_rclMesh);
    for (it.Init(); it.More(); it.Next()) {
        if (it->IsDegenerated(fEpsilon)) aInds.push_back(it.Position());
    }
...
bool MeshRemoveNeedles::Fixup()
{
    ...
    std::priority_queue<FaceEdgePriority, std::vector<FaceEdgePriority>,
                        std::greater<FaceEdgePriority>>
        todo;
    ...
    while (!todo.empty()) {
        ...
        if (!rclFAry[faceedge.first].IsValid()) continue;
        ...
        if (neighbour != FACET_INDEX_MAX) ce._removeFacets.push_back(neighbour);

        std::set<FacetIndex> vf = vf_it[ce._fromPoint];
        vf.erase(faceedge.first);
        if (neighbour != FACET_INDEX_MAX) vf.erase(neighbour);
        ...
        if (topAlg.IsCollapseEdgeLegal(ce)) {
            ...
            for (auto it : ce._removeFacets) { vf_it.RemoveFacet(it); }
...
bool MeshFixCaps::Fixup()
{
    ...
    std::priority_queue<FaceVertexPriority, std::vector<FaceVertexPriority>,
                        std::greater<FaceVertexPriority>>
        todo;
    for (std::size_t index = 0; index < facetCount; index++) {
        for (int i = 0; i < 3; i++) {
            const MeshFacet &facet = rclFAry[index];
    ...
    while (!todo.empty()) {
        ...
        float fCosAngle = dir1.Dot(dir2);
        if (fCosAngle >= fCosMaxAngle) continue;
        ...
        if (distP2P4 / distP2P3 < fSplitFactor || distP3P4 / distP2P3 < fSplitFactor) continue;

        FacetIndex facetpos = facevertex.first;
        FacetIndex neighbour = rclFAry[facetpos]._aulNeighbours[(facevertex.second + 1) % 3];
        if (neighbour != FACET_INDEX_MAX) topAlg.SwapEdge(facetpos, neighbour);
    }
...
bool MeshEvalDeformedFacets::Evaluate()
{
    ...
    MeshFacetIterator it(_rclMesh);
    for (it.Init(); it.More(); it.Next()) {
        if (it->IsDeformed(fCosMinAngle, fCosMaxAngle)) return false;
    }
...
std::vector<FacetIndex> MeshEvalDeformedFacets::GetIndices() const
{
    ...
    MeshFacetIterator it(_rclMesh);
    for (it.Init(); it.More(); it.Next()) {
        if (it->IsDeformed(fCosMinAngle, fCosMaxAngle)) aInds.push_back(it.Position());
    }
...
bool MeshEvalDentsOnSurface::Evaluate()
{
    ...
    for (unsigned long index = 0; index < ctPoints; index++) {
        ...
        for (std::set<PointIndex>::iterator pt = nb.begin(); pt != nb.end(); ++pt) {
            ...
            for (std::set<FacetIndex>::const_iterator ft = faces.begin(); ft != faces.end(); ++ft) {
                // the point must not be part of the facet we test
                if (f_beg[*ft]._aulPoints[0] == *pt) continue;
                if (f_beg[*ft]._aulPoints[1] == *pt) continue;
                if (f_beg[*ft]._aulPoints[2] == *pt) continue;
...
std::vector<FacetIndex> MeshEvalDentsOnSurface::GetIndices() const { return this->indices; }
...
std::vector<FacetIndex> MeshEvalFoldsOnSurface::GetIndices() const { return this->indices; }
...
std::vector<FacetIndex> MeshEvalFoldsOnBoundary::GetIndices() const { return this->indices; }
...
bool MeshEvalBorderFacet::Evaluate()
{
    ...
    for (f_it = facets.begin(); f_it != f_end; ++f_it) {
        ...
        if (ok) _facets.push_back(f_it - f_beg);
    }
...
bool MeshEvalRangeFacet::Evaluate()
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it) {
        for (int i = 0; i < 3; i++) {
            if ((it->_aulNeighbours[i] >= ulCtFacets)
...
bool MeshEvalRangePoint::Evaluate()
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it) {
        if (std::find_if(it->_aulPoints, it->_aulPoints + 3,
...
bool MeshFixRangePoint::Fixup()
{
    ...
    if (_rclMesh.CountPoints() == 0) {
        ...
        if (!invalid.empty()) {
            for (std::vector<PointIndex>::iterator it = invalid.begin(); it != invalid.end();
                 ++it) {
...
bool MeshEvalCorruptedFacets::Evaluate()
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it) {
        ...
        if (it->IsDegenerated()) return false;
    }
...
std::vector<FacetIndex> MeshEvalCorruptedFacets::GetIndices() const
{
    ...
    for (MeshFacetArray::_TConstIterator it = rFaces.begin(); it != rFaces.end(); ++it, ind++) {
        if (it->IsDegenerated()) aInds.push_back(ind);
    }
        ...
        if (!facet.HasPoint(idx)) {
            for (int i = 0; i < 3; i++) {
                ...
                if (edge.GetBoundBox().IsInBox(points[idx])) {
                    if (edge.IsPointOf(points[idx], 0.001f)) return true;
                }
            }
        }
        return false;
    ...
    for (PointIndex i = 0; i < maxPoints; i++) {
        ...
        for (const auto &it : elements) {
            ...
            if (IsPointOnEdge(i, face)) {
                ...
                if (face.HasOpenEdge()) facetsIndices.push_back(it);
            }
        }
    }
...
std::vector<PointIndex> MeshEvalPointOnEdge::GetPointIndices() const { return pointsIndices; }

std::vector<FacetIndex> MeshEvalPointOnEdge::GetFacetIndices() const { return facetsIndices; }
...
bool MeshFixPointOnEdge::Fixup()
{
    ...
    if (!pointsIndices.empty()) {
        if (fillBoundary) { MarkBoundaries(facetsIndices); }
        ...
        if (fillBoundary) {
            ...
            if (!borderList.empty()) FillBoundaries(borderList);
        }
    }
...
void MeshFixPointOnEdge::FindBoundaries(std::list<std::vector<PointIndex>> &borderList)
{
    ...
    if (!tmp.empty()) { meshalg.GetFacetsBorders(tmp, borderList); }
}

...
public:
   ...
   * automatically. */
    ...
    virtual unsigned long Inside(const Base::BoundBox3f &rclBB,
                                 std::vector<ElementIndex> &raulElements,
                                 const Base::Vector3f &rclOrg, float fMaxDist,
                                 bool bDelDoubles = true) const;
    ...
    unsigned long GetElements(unsigned long ulX, unsigned long ulY, unsigned long ulZ,
    ...
    inline Base::BoundBox3f GetBoundBox(unsigned long ulX, unsigned long ulY,
   ...
   */
    bool GetPositionToIndex(unsigned long id, unsigned long &ulX, unsigned long &ulY,
                            unsigned long &ulZ) const;
   ...
   */
    bool CheckPosition(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
   ...
   * the nearest grid element are taken.*/
    virtual void Position(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
                          unsigned long &rulZ) const;
    ...
    void GetHull(unsigned long ulX, unsigned long ulY, unsigned long ulZ, unsigned long ulDistance,
                 std::set<ElementIndex> &raclInd) const;
...
public:
    ...
    void SearchNearestFacetInGrid(unsigned long ulX, unsigned long ulY, unsigned long ulZ,
   ...
   * are introduced into the search. */
    void SearchNearestFacetInHull(unsigned long ulX, unsigned long ulY, unsigned long ulZ,
...
protected:
    ...
    inline void Pos(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
                    unsigned long &rulZ) const;
    /** Returns the grid numbers to the given point \a rclPoint. */
    inline void PosWithCheck(const Base::Vector3f &rclPoint, unsigned long &rulX,
   ...
   * the facet. */
    inline void AddFacet(const MeshGeomFacet &rclFacet, ElementIndex ulFacetIndex,
                         float fEpsilon = 0.0f);
    /** Returns the number of stored elements. */
    unsigned long HasElements() const override { return _pclMesh->CountFacets(); }
    /** Rebuilds the grid structure. */
...
protected:
   ...
   * the corresponding index in the mesh kernel. */
    ...
    /** Returns the grid numbers to the given point \a rclPoint. */
    void Pos(const Base::Vector3f &rclPoint, unsigned long &rulX, unsigned long &rulY,
             unsigned long &rulZ) const;
    /** Returns the number of stored elements. */
    unsigned long HasElements() const override { return _pclMesh->CountPoints(); }
    /** Rebuilds the grid structure. */
    void RebuildGrid() override;
};
...
public:
    ...
    Base::BoundBox3f GetBoundBox() const { return _rclGrid.GetBoundBox(_ulX, _ulY, _ulZ); }
    ...
    unsigned long GetCtElements() const { return _rclGrid.GetCtElements(_ulX, _ulY, _ulZ); }
    /** @name Iteration */
    //@{
    /** Sets the iterator to the first element*/
    void Init() { _ulX = _ulY = _ulZ = 0; }
    /** Checks if the iterator has not yet reached the end position. */
    bool More() const { return (_ulZ < _rclGrid._ulCtGridsZ); }
    /** Go to the next grid. */
    void Next()
    {
        if (++_ulX >= (_rclGrid._ulCtGridsX)) _ulX = 0;
        else
            return;
    ...
    bool InitOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir,
                   std::vector<ElementIndex> &raulElements);
    /** Searches for facets around the ray. */
    bool InitOnRay(const Base::Vector3f &rclPt, const Base::Vector3f &rclDir, float fMaxSearchArea,
                   std::vector<ElementIndex> &raulElements);
    /** Searches for facets around the ray. */
...
inline Base::BoundBox3f MeshGrid::GetBoundBox(unsigned long ulX, unsigned long ulY,
                                              unsigned long ulZ) const
{
    float fX, fY, fZ;
...
inline void MeshFacetGrid::Pos(const Base::Vector3f &rclPoint, unsigned long &rulX,
                               unsigned long &rulY, unsigned long &rulZ) const
{
...
inline void MeshFacetGrid::PosWithCheck(const Base::Vector3f &rclPoint, unsigned long &rulX,
                                        unsigned long &rulY, unsigned long &rulZ) const
{
    if (rclPoint.x < _fMinX) rulX = 0;
    else {
        rulX = static_cast<unsigned long>((rclPoint.x - _fMinX) / _fGridLenX);
        if (rulX >= _ulCtGridsX) rulX = (_ulCtGridsX - 1);
    }

    if (rclPoint.y < _fMinY) rulY = 0;
    else {
        rulY = static_cast<unsigned long>((rclPoint.y - _fMinY) / _fGridLenY);
        if (rulY >= _ulCtGridsY) rulY = (_ulCtGridsY - 1);
    }

    if (rclPoint.z < _fMinZ) rulZ = 0;
    else {
        rulZ = static_cast<unsigned long>((rclPoint.z - _fMinZ) / _fGridLenZ);
        if (rulZ >= _ulCtGridsZ) rulZ = (_ulCtGridsZ - 1);
    }

    assert((rulX < _ulCtGridsX) && (rulY < _ulCtGridsY) && (rulZ < _ulCtGridsZ));
}

inline void MeshFacetGrid::AddFacet(const MeshGeomFacet &rclFacet, ElementIndex ulFacetIndex,
                                    float /*fEpsilon*/)
{

...
CylinderFit::CylinderFit()
    : _vBase(0, 0, 0), _vAxis(0, 0, 1), _dRadius(0), _numIter(0), _posConvLimit(0.0001),
      _dirConvLimit(0.000001), _vConvLimit(0.001), _maxIter(50)
{}

CylinderFit::~CylinderFit() {}

// Set approximations before calling the fitting
void CylinderFit::SetApproximations(double radius, const Base::Vector3d &base,
...
void CylinderFit::SetConvergenceCriteria(double posConvLimit, double dirConvLimit,
                                         double vConvLimit, int maxIter)
{
    if (posConvLimit > 0.0) _posConvLimit = posConvLimit;
    if (dirConvLimit > 0.0) _dirConvLimit = dirConvLimit;
    if (vConvLimit > 0.0) _vConvLimit = vConvLimit;
    if (maxIter > 0) _maxIter = maxIter;
}


double CylinderFit::GetRadius() const
{
    if (_bIsFitted) return _dRadius;
    else
        return 0.0;
}
...
float CylinderFit::Fit()
{
    ...
    while (cont && (_numIter < _maxIter)) {
        ...
        if (llt.info() != Eigen::Success) return FLOAT_MAX;
        ...
        if (!computeResiduals(solDir, x, residuals, sigma0, _vConvLimit, vConverged))
            return FLOAT_MAX;
        if (!vConverged) cont = true;

        // Update the parameters
        if (!updateParameters(solDir, x)) return FLOAT_MAX;
    }

    // Check for convergence
    if (cont) return FLOAT_MAX;
...
void CylinderFit::findBestSolDirection(SolutionD &solDir)
{
    ...
    if (biggest < 0.0) dir.Set(-dir.x, -dir.y, -dir.z); // multiplies by -1
...
void CylinderFit::setupNormalEquationMatrices(SolutionD solDir,
                                              ...
                                              Matrix5x5 &atpa, Eigen::VectorXd &atpl) const
{
    // Zero matrices
    atpa.setZero();
    atpl.setZero();
...
void CylinderFit::setupObservation(SolutionD solDir, const Base::Vector3f &point,
                                   const Base::Vector3d &residual, double a[5], double &f0,
                                   double &qw, double b[3]) const
{
...
void CylinderFit::addObservationU(double a[5], double li, double pi, Matrix5x5 &atpa,
...
bool CylinderFit::computeResiduals(SolutionD solDir, const Eigen::VectorXd &x,
                                   std::vector<Base::Vector3d> &residuals, double &sigma0,
                                   double vConvLimit, bool &vConverged) const
{
    vConverged = true;
    int nPtsUsed = 0;
    sigma0 = 0.0;
    ...
    for (cIt = _vPoints.begin(); cIt != _vPoints.end(); ++cIt, ++vIt) {
        ...
        if ((dVx > vConvLimit) || (dVy > vConvLimit) || (dVz > vConvLimit)) vConverged = false;
...
bool CylinderFit::updateParameters(SolutionD solDir, const Eigen::VectorXd &x)
{
    ...
    switch (solDir) {
        case solL:
            ...
            if (l2 <= 0.0) return false; // L*L <= 0 !
            _vAxis.x = sqrt(l2);
            //_vBase.x is fixed
            break;
        case solM:
            m2 = 1.0 - _vAxis.x * _vAxis.x - _vAxis.z * _vAxis.z;
            if (m2 <= 0.0) return false; // M*M <= 0 !
            _vAxis.y = sqrt(m2);
            //_vBase.y is fixed
            break;
        case solN:
            n2 = 1.0 - _vAxis.x * _vAxis.x - _vAxis.y * _vAxis.y;
            if (n2 <= 0.0) return false; // N*N <= 0 !
            _vAxis.z = sqrt(n2);
            //_vBase.z is fixed
            break;
    }

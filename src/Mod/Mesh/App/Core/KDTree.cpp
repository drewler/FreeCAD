...
struct Point3d {
    ...
    inline value_type operator[](const int N) const { return p[N]; }

    inline bool operator==(const Point3d &other) const { return (this->p) == (other.p); }
...
MeshKDTree::~MeshKDTree() { delete d; }
...
bool MeshKDTree::IsEmpty() const { return d->kd_tree.empty(); }

void MeshKDTree::Clear() { d->kd_tree.clear(); }

void MeshKDTree::Optimize() { d->kd_tree.optimize(); }
...
PointIndex MeshKDTree::FindNearest(const Base::Vector3f &p, Base::Vector3f &n, float &dist) const
{
    ...
    if (it.first == d->kd_tree.end()) return POINT_INDEX_MAX;
...
PointIndex MeshKDTree::FindNearest(const Base::Vector3f &p, float max_dist, Base::Vector3f &n,
                                   float &dist) const
{
    std::pair<MyKDTree::const_iterator, MyKDTree::distance_type> it =
...
PointIndex MeshKDTree::FindExact(const Base::Vector3f &p) const
{
    ...
    if (it == d->kd_tree.end()) return POINT_INDEX_MAX;
    PointIndex index = it->i;
    return index;
}

void MeshKDTree::FindInRange(const Base::Vector3f &p, float range,

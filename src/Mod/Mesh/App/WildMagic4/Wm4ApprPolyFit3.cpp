...
//----------------------------------------------------------------------------
template<class Real>
Real *PolyFit3(int iSamples, const Real *afX, const Real *afY, const Real *afW, int iXDegree,
               int iYDegree)
{
    ...
    for (iS = 0; iS < iSamples; iS++) {
        aafXP[iS][0] = (Real)1.0;
        for (i0 = 1; i0 <= 2 * iXDegree; i0++) { aafXP[iS][i0] = afX[iS] * aafXP[iS][i0 - 1]; }

        aafYP[iS][0] = (Real)1.0;
        for (j0 = 1; j0 <= 2 * iYDegree; j0++) { aafYP[iS][j0] = afY[iS] * aafYP[iS][j0 - 1]; }
    }
    ...
    for (j0 = 0; j0 <= iYDegree; j0++) {
        for (i0 = 0; i0 <= iXDegree; i0++) {
            ...
            Real fSum = (Real)0.0;
            for (iS = 0; iS < iSamples; iS++) { fSum += afW[iS] * aafXP[iS][i0] * aafYP[iS][j0]; }
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM float *PolyFit3<float>(int, const float *, const float *,
                                                    const float *, int, int);

template WM4_FOUNDATION_ITEM double *PolyFit3<double>(int, const double *, const double *,

...
//----------------------------------------------------------------------------
template<class Real>
IntrTriangle2Triangle2<Real>::IntrTriangle2Triangle2(const Triangle2<Real> &rkTriangle0,
...
//----------------------------------------------------------------------------
template<class Real> const Triangle2<Real> &IntrTriangle2Triangle2<Real>::GetTriangle0() const
{
    return m_rkTriangle0;
}
//----------------------------------------------------------------------------
template<class Real> const Triangle2<Real> &IntrTriangle2Triangle2<Real>::GetTriangle1() const
{
    return m_rkTriangle1;
}
//----------------------------------------------------------------------------
template<class Real> bool IntrTriangle2Triangle2<Real>::Test()
{
    int i0, i1;
    Vector2<Real> kDir;
...
//----------------------------------------------------------------------------
template<class Real> bool IntrTriangle2Triangle2<Real>::Find()
{
    ...
    m_iQuantity = 3;
    for (int i = 0; i < 3; i++) { m_akPoint[i] = m_rkTriangle1.V[i]; }

    for (int i1 = 2, i0 = 0; i0 < 3; i1 = i0, i0++) {
        // clip against edge <V0[i1],V0[i0]>
        Vector2<Real> kN(m_rkTriangle0.V[i1].Y() - m_rkTriangle0.V[i0].Y(),
...
//----------------------------------------------------------------------------
template<class Real>
bool IntrTriangle2Triangle2<Real>::Test(Real fTMax, const Vector2<Real> &rkVelocity0,
                                        const Vector2<Real> &rkVelocity1)
{
    ...
    for (i0 = 1, i1 = 2, i2 = 0; i2 < 3; i0 = i1, i1 = i2, i2++) {
        ...
        if (NoIntersect(kCfg0, kCfg1, fTMax, fSpeed, iSide, kTCfg0, kTCfg1, fTFirst, fTLast)) {
            return false;
        }
    }
...
//----------------------------------------------------------------------------
template<class Real>
bool IntrTriangle2Triangle2<Real>::Find(Real fTMax, const Vector2<Real> &rkVelocity0,
                                        const Vector2<Real> &rkVelocity1)
{
    // process as if V0-triangle is stationary and V1-triangle is moving
    Vector2<Real> kW = rkVelocity1 - rkVelocity0;
    ...
    GetIntersection(kTCfg0, kTCfg1, iSide, akMoveV0, akMoveV1, m_iQuantity, m_akPoint);

    m_fContactTime = fTFirst;
    return m_iQuantity > 0;
}
//----------------------------------------------------------------------------
template<class Real> int IntrTriangle2Triangle2<Real>::GetQuantity() const { return m_iQuantity; }
//----------------------------------------------------------------------------
template<class Real> const Vector2<Real> &IntrTriangle2Triangle2<Real>::GetPoint(int i) const
{
...
//----------------------------------------------------------------------------
template<class Real>
int IntrTriangle2Triangle2<Real>::WhichSide(const Vector2<Real> akV[3], const Vector2<Real> &rkP,
                                            const Vector2<Real> &rkD)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (iPositive > 0 && iNegative > 0) { return 0; }
    }
    return (iZero == 0 ? (iPositive > 0 ? 1 : -1) : 0);
}
//----------------------------------------------------------------------------
template<class Real>
void IntrTriangle2Triangle2<Real>::ClipConvexPolygonAgainstLine(const Vector2<Real> &rkN, Real fC,
                                                                int &riQuantity,
                                                                Vector2<Real> akV[6])
{
    ...
    if (iPositive > 0) {
        if (iNegative > 0) {
            ...
            if (iPIndex > 0) {
                ...
                if (iCur < riQuantity) { iPrv = iCur - 1; }
                else {
                    iCur = 0;
                    iPrv = riQuantity - 1;
                }
            ...
            else // iPIndex is 0
            {
                ...
                while (iCur < riQuantity && afTest[iCur] <= (Real)0.0) { iCur++; }
...
//----------------------------------------------------------------------------
template<class Real>
void IntrTriangle2Triangle2<Real>::ComputeTwo(Configuration &rkCfg, const Vector2<Real> akV[3],
                                              const Vector2<Real> &rkD, int i0, int i1, int i2)
{
    rkCfg.Map = M12;
    rkCfg.Index[0] = i0;
    rkCfg.Index[1] = i1;
...
//----------------------------------------------------------------------------
template<class Real>
void IntrTriangle2Triangle2<Real>::ComputeThree(Configuration &rkCfg, const Vector2<Real> akV[3],
                                                const Vector2<Real> &rkD, const Vector2<Real> &rkP)
{
    ...
    if (fD0 <= fD1) {
        if (fD1 <= fD2) // d0 <= d1 <= d2
        {
            if (fD0 != fD1) { rkCfg.Map = (fD1 != fD2 ? M11 : M12); }
            else {
                rkCfg.Map = M21;
            }
...
//----------------------------------------------------------------------------
template<class Real>
bool IntrTriangle2Triangle2<Real>::NoIntersect(const Configuration &rkCfg0,
                                               const Configuration &rkCfg1, Real fTMax, Real fSpeed,
                                               int &riSide, Configuration &rkTCfg0,
                                               Configuration &rkTCfg1, Real &rfTFirst,
                                               Real &rfTLast)
{
    Real fInvSpeed, fT;
    ...
    if (rkCfg1.Max < rkCfg0.Min) {
        ...
        // test for separation
        if (rfTFirst > rfTLast) { return true; }
    }
...
//----------------------------------------------------------------------------
template<class Real>
void IntrTriangle2Triangle2<Real>::GetIntersection(const Configuration &rkCfg0,
                                                   const Configuration &rkCfg1, int iSide,
                                                   const Vector2<Real> akV0[3],
                                                   ...
                                                   Vector2<Real> akVertex[6])
{
    ...
    else // triangles were initially intersecting
    {
        ...
        kIntr.Find();
        riQuantity = kIntr.GetQuantity();
        for (i = 0; i < riQuantity; i++) { akVertex[i] = kIntr.GetPoint(i); }
    }
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class IntrTriangle2Triangle2<float>;

template WM4_FOUNDATION_ITEM class IntrTriangle2Triangle2<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

...
//----------------------------------------------------------------------------
template<class Real>
ConvexHull3<Real>::ConvexHull3(int iVertexQuantity, Vector3<Real> *akVertex, Real fEpsilon,
...
//----------------------------------------------------------------------------
template<class Real> ConvexHull3<Real>::~ConvexHull3()
{
...
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &ConvexHull3<Real>::GetLineOrigin() const
{
    return m_kLineOrigin;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &ConvexHull3<Real>::GetLineDirection() const
{
    return m_kLineDirection;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &ConvexHull3<Real>::GetPlaneOrigin() const
{
    return m_kPlaneOrigin;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &ConvexHull3<Real>::GetPlaneDirection(int i) const
{
    assert(0 <= i && i < 2);
    return m_akPlaneDirection[i];
}
//----------------------------------------------------------------------------
template<class Real> ConvexHull1<Real> *ConvexHull3<Real>::GetConvexHull1() const
{
    ...
    return WM4_NEW ConvexHull1<Real>(m_iVertexQuantity, afProjection, m_fEpsilon, true,
                                     m_eQueryType);
}
//----------------------------------------------------------------------------
template<class Real> ConvexHull2<Real> *ConvexHull3<Real>::GetConvexHull2() const
{
    ...
    return WM4_NEW ConvexHull2<Real>(m_iVertexQuantity, akProjection, m_fEpsilon, true,
                                     m_eQueryType);
}
//----------------------------------------------------------------------------
template<class Real> bool ConvexHull3<Real>::Update(int i)
{
        ...
        for (j = 0; j < 3; j++) {
            ...
            if (pkAdj) {
                ...
                if (pkAdj->GetSign(i, m_pkQuery) > 0) {
                    ...
                    iV0 = pkTri->V[j];
                    iV1 = pkTri->V[(j + 1) % 3];
                    kTerminator[iV0] = TerminatorData(iV0, iV1, iNullIndex, pkAdj);
                }
            }
        }
        m_kHull.erase(pkTri);
    ...
    assert(iSize >= 3);
    typename std::map<int, TerminatorData>::iterator pkEdge = kTerminator.begin();
...
//----------------------------------------------------------------------------
template<class Real> void ConvexHull3<Real>::ExtractIndices()
{
    ...
    for (pkIter = m_kHull.begin(); pkIter != m_kHull.end(); pkIter++) {
        ...
        for (int j = 0; j < 3; j++) { m_aiIndex[i++] = pkTri->V[j]; }
        WM4_DELETE pkTri;
    }
    m_kHull.clear();
}
//----------------------------------------------------------------------------
template<class Real> void ConvexHull3<Real>::DeleteHull()
{
    typename std::set<Triangle *>::iterator pkIter;
    for (pkIter = m_kHull.begin(); pkIter != m_kHull.end(); pkIter++) {
        Triangle *pkTri = *pkIter;
...
//----------------------------------------------------------------------------
template<class Real> bool ConvexHull3<Real>::Load(const char *acFilename)
{
...
//----------------------------------------------------------------------------
template<class Real> bool ConvexHull3<Real>::Save(const char *acFilename) const
{
...
//----------------------------------------------------------------------------
template<class Real> ConvexHull3<Real>::Triangle::Triangle(int iV0, int iV1, int iV2)
{
    V[0] = iV0;
    V[1] = iV1;
    V[2] = iV2;
...
//----------------------------------------------------------------------------
template<class Real> int ConvexHull3<Real>::Triangle::GetSign(int i, const Query3<Real> *pkQuery)
{
    if (i != Time) {
        Time = i;
...
//----------------------------------------------------------------------------
template<class Real>
void ConvexHull3<Real>::Triangle::AttachTo(Triangle *pkAdj0, Triangle *pkAdj1, Triangle *pkAdj2)
{
...
//----------------------------------------------------------------------------
template<class Real> int ConvexHull3<Real>::Triangle::DetachFrom(int iAdj, Triangle *pkAdj)
{
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class ConvexHull3<float>;

template WM4_FOUNDATION_ITEM class ConvexHull3<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

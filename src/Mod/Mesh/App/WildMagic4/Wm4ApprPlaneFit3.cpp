...
bool HeightPlaneFit3(int iQuantity, const Vector3<Real> *akPoint, Real &rfA, Real &rfB, Real &rfC)
{
    ...
    Real aafA[3][3] = {{fSumXX, fSumXY, fSumX},
                       {fSumXY, fSumYY, fSumY},
...
//----------------------------------------------------------------------------
template<class Real> Plane3<Real> OrthogonalPlaneFit3(int iQuantity, const Vector3<Real> *akPoint)
{
    // compute the mean of the points
    Vector3<Real> kOrigin = Vector3<Real>::ZERO;
    int i;
    for (i = 0; i < iQuantity; i++) { kOrigin += akPoint[i]; }
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM bool HeightPlaneFit3<float>(int, const Vector3<float> *, float &,
                                                         float &, float &);

template WM4_FOUNDATION_ITEM Plane3<float> OrthogonalPlaneFit3<float>(int, const Vector3<float> *);

template WM4_FOUNDATION_ITEM bool HeightPlaneFit3<double>(int, const Vector3<double> *, double &,
                                                          double &, double &);

template WM4_FOUNDATION_ITEM Plane3<double> OrthogonalPlaneFit3<double>(int,

...
//----------------------------------------------------------------------------
template<class Real>
Delaunay3<Real>::Delaunay3(int iVertexQuantity, Vector3<Real> *akVertex, Real fEpsilon, bool bOwner,
                           Query::Type eQueryType)
    : Delaunay<Real>(iVertexQuantity, fEpsilon, bOwner, eQueryType),
      m_kLineOrigin(Vector3<Real>::ZERO), m_kLineDirection(Vector3<Real>::ZERO),
      m_kPlaneOrigin(Vector3<Real>::ZERO)
{
    ...
    if (eQueryType != Query::QT_RATIONAL && eQueryType != Query::QT_FILTERED) {
        ...
        m_akSVertex[m_aiSV[0]] = Vector3<Real>((Real)-1.0, (Real)-1.0, (Real)-1.0);
        m_akSVertex[m_aiSV[1]] = Vector3<Real>((Real) + 6.0, (Real)-1.0, (Real)-1.0);
        m_akSVertex[m_aiSV[2]] = Vector3<Real>((Real)-1.0, (Real) + 6.0, (Real)-1.0);
        m_akSVertex[m_aiSV[3]] = Vector3<Real>((Real)-1.0, (Real)-1.0, (Real) + 6.0);
        ...
        if (eQueryType == Query::QT_INT64) {
            ...
            // tetrahedralization.
            fExpand = (Real)(1 << 20);
            m_pkQuery = WM4_NEW Query3TInteger<Real>(m_iVertexQuantity, m_akSVertex);
        }
        ...
        m_fScale *= fExpand;
        for (i = 0; i < m_iVertexQuantity; i++) { m_akSVertex[i] *= fExpand; }
    }
    else {
        ...
        if (eQueryType == Query::QT_RATIONAL) {
            m_pkQuery = WM4_NEW Query3TRational<Real>(m_iVertexQuantity, m_akSVertex);
        }
        else // eQueryType == Query::QT_FILTERED
        {
            m_pkQuery = WM4_NEW Query3Filtered<Real>(m_iVertexQuantity, m_akSVertex, fEpsilon);
        }
    }

    DelTetrahedron<Real> *pkTetra =
        WM4_NEW DelTetrahedron<Real>(m_aiSV[0], m_aiSV[1], m_aiSV[2], m_aiSV[3]);
    ...
    typename std::set<DelTetrahedron<Real> *>::iterator pkTIter = m_kTetrahedron.begin();
    ...
    pkTIter = m_kTetrahedron.begin();
    for (/**/; pkTIter != m_kTetrahedron.end(); ++pkTIter) { WM4_DELETE *pkTIter; }
}
//----------------------------------------------------------------------------
template<class Real> Delaunay3<Real>::~Delaunay3()
{
    WM4_DELETE m_pkQuery;
    WM4_DELETE[] m_akSVertex;
    WM4_DELETE[] m_aiPath;
    if (m_bOwner) { WM4_DELETE[] m_akVertex; }
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> *Delaunay3<Real>::GetVertices() const
{
    return m_akVertex;
}
//----------------------------------------------------------------------------
template<class Real> int Delaunay3<Real>::GetUniqueVertexQuantity() const
{
    return m_iUniqueVertexQuantity;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &Delaunay3<Real>::GetLineOrigin() const
{
    return m_kLineOrigin;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &Delaunay3<Real>::GetLineDirection() const
{
    return m_kLineDirection;
}
//----------------------------------------------------------------------------
template<class Real> Delaunay1<Real> *Delaunay3<Real>::GetDelaunay1() const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return nullptr; }
    ...
    return WM4_NEW Delaunay1<Real>(m_iVertexQuantity, afProjection, m_fEpsilon, true, m_eQueryType);
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &Delaunay3<Real>::GetPlaneOrigin() const
{
    return m_kPlaneOrigin;
}
//----------------------------------------------------------------------------
template<class Real> const Vector3<Real> &Delaunay3<Real>::GetPlaneDirection(int i) const
{
    assert(0 <= i && i < 2);
    return m_akPlaneDirection[i];
}
//----------------------------------------------------------------------------
template<class Real> Delaunay2<Real> *Delaunay3<Real>::GetDelaunay2() const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return nullptr; }
    ...
    return WM4_NEW Delaunay2<Real>(m_iVertexQuantity, akProjection, m_fEpsilon, true, m_eQueryType);
}
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::GetHull(int &riTQuantity, int *&raiIndex) const
{
    ...
    for (i = 0; i < iAdjQuantity; i++) {
        if (m_aiAdjacent[i] == -1) { riTQuantity++; }
    }
    assert(riTQuantity > 0);
    if (riTQuantity == 0) { return false; }
    ...
    for (i = 0; i < iAdjQuantity; i++) {
        if (m_aiAdjacent[i] == -1) {
            ...
            for (int j = 0; j < 4; j++) {
                if (j != iFace) { *piIndex++ = m_aiIndex[4 * iTetra + j]; }
            }
            if ((iFace % 2) == 0) {
                int iSave = *(piIndex - 1);
                *(piIndex - 1) = *(piIndex - 2);
...
//----------------------------------------------------------------------------
template<class Real> int Delaunay3<Real>::GetContainingTetrahedron(const Vector3<Real> &rkP) const
{
    assert(m_iDimension == 3);
    if (m_iDimension != 3) { return -1; }
...
//----------------------------------------------------------------------------
template<class Real> int Delaunay3<Real>::GetPathLast() const { return m_iPathLast; }
//----------------------------------------------------------------------------
template<class Real> const int *Delaunay3<Real>::GetPath() const { return m_aiPath; }
//----------------------------------------------------------------------------
template<class Real>
int Delaunay3<Real>::GetLastFace(int &riV0, int &riV1, int &riV2, int &riV3) const
{
    riV0 = m_iLastFaceV0;
    riV1 = m_iLastFaceV1;
    riV2 = m_iLastFaceV2;
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::GetVertexSet(int i, Vector3<Real> akV[4]) const
{
    assert(m_iDimension == 3);
    if (m_iDimension != 3) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::GetIndexSet(int i, int aiIndex[4]) const
{
    assert(m_iDimension == 3);
    if (m_iDimension != 3) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::GetAdjacentSet(int i, int aiAdjacent[4]) const
{
    assert(m_iDimension == 3);
    if (m_iDimension != 3) { return false; }
...
//----------------------------------------------------------------------------
template<class Real>
bool Delaunay3<Real>::GetBarycentricSet(int i, const Vector3<Real> &rkP, Real afBary[4]) const
{
    assert(m_iDimension == 3);
    if (m_iDimension != 3) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> void Delaunay3<Real>::Update(int i)
{
        ...
        for (j = 0; j < 4; j++) {
            ...
            if (pkAdj) {
                ...
                if (pkAdj->IsInsertionComponent(i, pkTetra, m_pkQuery, m_aiSV)) {
                    ...
                    pkFace = (DelPolyhedronFace<Real> *)kPolyhedron.InsertTriangle(iV0, iV1, iV2);
                ...
                if (IsSupervertex(iV0)) {
                    ...
                    if (IsSupervertex(iV1)) {
                        ...
                        if (IsSupervertex(iV2)) {
                            pkFace = (DelPolyhedronFace<Real> *)kPolyhedron.InsertTriangle(iV0, iV1,
                                                                                           iV2);
                            pkFace->NullIndex = -1;
    ...
    for (pkTIter = rkTMap.begin(); pkTIter != rkTMap.end(); pkTIter++) {
        ...
        pkTetra = WM4_NEW DelTetrahedron<Real>(i, pkFace->V[0], pkFace->V[1], pkFace->V[2]);
        m_kTetrahedron.insert(pkTetra);

        // Establish the adjacency links across the polyhedron face.
        pkTetra->A[0] = pkFace->Tetra;
        if (pkFace->Tetra) { pkFace->Tetra->A[pkFace->NullIndex] = pkTetra; }
...
//----------------------------------------------------------------------------
template<class Real> DelTetrahedron<Real> *Delaunay3<Real>::GetContainingTetrahedron(int i) const
{
...
//----------------------------------------------------------------------------
template<class Real> void Delaunay3<Real>::RemoveTetrahedra()
{
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::IsSupervertex(int i) const
{
    for (int j = 0; j < 4; j++) {
        if (i == m_aiSV[j]) { return true; }
    }
    return false;
}
//----------------------------------------------------------------------------
template<class Real>
bool Delaunay3<Real>::SharesFace(int i, DelTetrahedron<Real> *pkFace, DelTetrahedron<Real> *pkAdj)
{
    int aiF[3], iCount = 0, j;
    for (j = 0; j < 4; j++) {
        if (j != i) { aiF[iCount++] = pkFace->V[j]; }
    }

    for (i = 0; i < 4; i++) {
        if (pkAdj->V[i] != aiF[0] && pkAdj->V[i] != aiF[1] && pkAdj->V[i] != aiF[2]) { break; }
    }
    if (i == 4) { return false; }

    int aiA[3];
    for (j = 0, iCount = 0; j < 4; j++) {
        if (j != i) { aiA[iCount++] = pkAdj->V[j]; }
    }
    ...
    if (aiA[0] != aiF[0] || aiA[1] != aiF[1] || aiA[2] != aiF[2]) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::Load(const char *acFilename)
{
    FILE *pkIFile = System::Fopen(acFilename, "rb");
    if (!pkIFile) { return false; }
    ...
    switch (m_eQueryType) {
        ...
        case Query::QT_FILTERED:
            m_pkQuery = WM4_NEW Query3Filtered<Real>(m_iVertexQuantity, m_akSVertex, m_fEpsilon);
            break;
    }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay3<Real>::Save(const char *acFilename) const
{
    FILE *pkOFile = System::Fopen(acFilename, "wb");
    if (!pkOFile) { return false; }

...
public:
    ...
    Quaternion &SlerpExtraSpins(Real fT, const Quaternion &rkP, const Quaternion &rkQ,
                                int iExtraSpins);

    // intermediate terms for spherical quadratic interpolation
    Quaternion &Intermediate(const Quaternion &rkQ0, const Quaternion &rkQ1,
                             const Quaternion &rkQ2);

    // spherical quadratic interpolation
    Quaternion &Squad(Real fT, const Quaternion &rkQ0, const Quaternion &rkA0,
                      const Quaternion &rkA1, const Quaternion &rkQ1);
    ...
    void DecomposeTwistTimesSwing(const Vector3<Real> &rkV1, Quaternion &rkTwist,
                                  Quaternion &rkSwing);
    ...
    // q, q_swing represents the rotation about the axis perpendicular to
    // V1 and V2 (see Quaternion::Align), and q_twist is a rotation about V1.
    void DecomposeSwingTimesTwist(const Vector3<Real> &rkV1, Quaternion &rkSwing,
                                  Quaternion &rkTwist);
    ...
    public:
        ...
        Constraints(Real fMinAngle, Real fMaxAngle) { SetAngles(fMinAngle, fMaxAngle); }
        ...
        bool IsValid(Real fX, Real fY) const
        {
            ...
            if (fXm * m_fDiffSinMaxMin >= fYm * m_fDiffCosMaxMin) { return true; }

            // Test whether (-x,-y) satisfies the constraints.
            Real fXp = fX + m_fCosMinAngle;
            Real fYp = fY + m_fSinMinAngle;
            if (fXp * m_fDiffSinMaxMin <= fYp * m_fDiffCosMaxMin) { return true; }
...
template<class Real> inline Quaternion<Real> operator*(Real fScalar, const Quaternion<Real> &rkQ);

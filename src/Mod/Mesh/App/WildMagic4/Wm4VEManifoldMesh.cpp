...
//----------------------------------------------------------------------------
VEManifoldMesh::VPtr VEManifoldMesh::CreateVertex(int iV) { return WM4_NEW Vertex(iV); }
//----------------------------------------------------------------------------
VEManifoldMesh::EPtr VEManifoldMesh::CreateEdge(int iV0, int iV1) { return WM4_NEW Edge(iV0, iV1); }
//----------------------------------------------------------------------------
...
bool VEManifoldMesh::IsClosed() const
{
    ...
    for (pkVIter = m_kVMap.begin(); pkVIter != m_kVMap.end(); pkVIter++) {
        ...
        if (!pkVertex->E[0] || !pkVertex->E[1]) { return false; }
    }
    return true;
}
//----------------------------------------------------------------------------
void VEManifoldMesh::Print(const char *acFilename)
{
    ...
    for (pkVIter = m_kVMap.begin(); pkVIter != m_kVMap.end(); pkVIter++) {
        ...
        kOStr << 'v' << rkVertex.V << " <";
        if (rkVertex.E[0]) { kOStr << 'e' << kEIndex[rkVertex.E[0]]; }
        else {
            kOStr << '*';
        }
        kOStr << ',';
        if (rkVertex.E[1]) { kOStr << 'e' << kEIndex[rkVertex.E[1]]; }
        else {
            kOStr << '*';
        }
        kOStr << '>' << std::endl;
    ...
    for (pkEIter = m_kEMap.begin(); pkEIter != m_kEMap.end(); pkEIter++) {
        ...
        kOStr << 'e' << kEIndex[pkEIter->second] << " <" << 'v' << rkEdge.V[0] << ",v"
              << rkEdge.V[1] << "; ";
        if (rkEdge.E[0]) { kOStr << 'e' << kEIndex[rkEdge.E[0]]; }
        else {
            kOStr << '*';
        }
        kOStr << ',';
        if (rkEdge.E[1]) { kOStr << 'e' << kEIndex[rkEdge.E[1]]; }
        else {
            kOStr << '*';
        }
        kOStr << '>' << std::endl;

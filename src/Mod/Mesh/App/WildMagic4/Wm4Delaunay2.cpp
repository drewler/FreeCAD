...
//----------------------------------------------------------------------------
template<class Real>
Delaunay2<Real>::Delaunay2(int iVertexQuantity, Vector2<Real> *akVertex, Real fEpsilon, bool bOwner,
                           Query::Type eQueryType)
    : Delaunay<Real>(iVertexQuantity, fEpsilon, bOwner, eQueryType),
      m_kLineOrigin(Vector2<Real>::ZERO), m_kLineDirection(Vector2<Real>::ZERO)
{
    assert(akVertex);
    m_akVertex = akVertex;
    ...
    if (eQueryType != Query::QT_RATIONAL && eQueryType != Query::QT_FILTERED) {
        ...
        if (eQueryType == Query::QT_INT64) {
            ...
            // triangulation.
            fExpand = (Real)(1 << 20);
            m_pkQuery = WM4_NEW Query2TInteger<Real>(m_iVertexQuantity, m_akSVertex);
        }
        ...
        if (eQueryType == Query::QT_RATIONAL) {
            m_pkQuery = WM4_NEW Query2TRational<Real>(m_iVertexQuantity, m_akSVertex);
        }
        else // eQueryType == Query::QT_FILTERED
        {
            m_pkQuery = WM4_NEW Query2Filtered<Real>(m_iVertexQuantity, m_akSVertex, m_fEpsilon);
        }
    }

    DelTriangle<Real> *pkTri = WM4_NEW DelTriangle<Real>(m_aiSV[0], m_aiSV[1], m_aiSV[2]);
    ...
    typename std::set<DelTriangle<Real> *>::iterator pkTIter = m_kTriangle.begin();
    ...
    pkTIter = m_kTriangle.begin();
    for (/**/; pkTIter != m_kTriangle.end(); ++pkTIter) { WM4_DELETE *pkTIter; }
}
//----------------------------------------------------------------------------
template<class Real> Delaunay2<Real>::~Delaunay2()
{
...
//----------------------------------------------------------------------------
template<class Real> const Vector2<Real> *Delaunay2<Real>::GetVertices() const
{
    return m_akVertex;
}
//----------------------------------------------------------------------------
template<class Real> int Delaunay2<Real>::GetUniqueVertexQuantity() const
{
    return m_iUniqueVertexQuantity;
}
//----------------------------------------------------------------------------
template<class Real> const Vector2<Real> &Delaunay2<Real>::GetLineOrigin() const
{
    return m_kLineOrigin;
}
//----------------------------------------------------------------------------
template<class Real> const Vector2<Real> &Delaunay2<Real>::GetLineDirection() const
{
    return m_kLineDirection;
}
//----------------------------------------------------------------------------
template<class Real> Delaunay1<Real> *Delaunay2<Real>::GetDelaunay1() const
{
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::GetHull(int &riEQuantity, int *&raiIndex)
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return false; }
    ...
    for (i = 0; i < iAdjQuantity; i++) {
        if (m_aiAdjacent[i] == -1) { riEQuantity++; }
    }
    assert(riEQuantity > 0);
    if (riEQuantity == 0) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> int Delaunay2<Real>::GetContainingTriangle(const Vector2<Real> &rkP) const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return -1; }
...
//----------------------------------------------------------------------------
template<class Real> int Delaunay2<Real>::GetPathLast() const { return m_iPathLast; }
//----------------------------------------------------------------------------
template<class Real> const int *Delaunay2<Real>::GetPath() const { return m_aiPath; }
//----------------------------------------------------------------------------
template<class Real> int Delaunay2<Real>::GetLastEdge(int &riV0, int &riV1, int &riV2) const
{
    riV0 = m_iLastEdgeV0;
    riV1 = m_iLastEdgeV1;
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::GetVertexSet(int i, Vector2<Real> akV[3]) const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::GetIndexSet(int i, int aiIndex[3]) const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::GetAdjacentSet(int i, int aiAdjacent[3]) const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return false; }
...
//----------------------------------------------------------------------------
template<class Real>
bool Delaunay2<Real>::GetBarycentricSet(int i, const Vector2<Real> &rkP, Real afBary[3]) const
{
    assert(m_iDimension == 2);
    if (m_iDimension != 2) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> void Delaunay2<Real>::Update(int i)
{
        ...
        for (j = 0; j < 3; j++) {
            ...
            if (pkAdj) {
                ...
                if (pkAdj->IsInsertionComponent(i, pkTri, m_pkQuery, m_aiSV)) {
                    ...
                    iV0 = pkTri->V[j];
                    iV1 = pkTri->V[(j + 1) % 3];
                    pkEdge = (DelPolygonEdge<Real> *)kPolygon.InsertEdge(iV0, iV1);
    ...
    for (pkEIter = rkEMap.begin(); pkEIter != rkEMap.end(); pkEIter++) {
        ...
        if (pkEdge->Tri) { pkEdge->Tri->A[pkEdge->NullIndex] = pkTri; }
...
//----------------------------------------------------------------------------
template<class Real> DelTriangle<Real> *Delaunay2<Real>::GetContainingTriangle(int i) const
{
...
//----------------------------------------------------------------------------
template<class Real> void Delaunay2<Real>::RemoveTriangles()
{
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::IsSupervertex(int i) const
{
    for (int j = 0; j < 3; j++) {
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::Load(const char *acFilename)
{
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay2<Real>::Save(const char *acFilename) const
{

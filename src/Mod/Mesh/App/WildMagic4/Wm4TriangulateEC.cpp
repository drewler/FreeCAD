...
//----------------------------------------------------------------------------
template<class Real>
TriangulateEC<Real>::TriangulateEC(const Positions &rkPositions, Query::Type eQueryType,
                                   ...
                                   Indices &rkTriangles)
{
    ...
    IndexMap kMap;
    Indices kCombined;
    CombinePolygons(eQueryType, fEpsilon, iNextElement, rkOuter, rkInner, kMap, kCombined);
...
template<class Real>
TriangulateEC<Real>::TriangulateEC(const Positions &rkPositions, Query::Type eQueryType,
                                   Real fEpsilon, const Indices &rkOuter,
                                   const IndicesArray &rkInners, Indices &rkTriangles)
{
    ...
    Indices kCombined;
    IndexMap kMap;
    ProcessOuterAndInners(eQueryType, fEpsilon, rkOuter, rkInners, iNextElement, kMap, kCombined);
    ...
    while (kQueue.size() > 0) {
        ...
        if (iNumChildren == 0) {
            ...
            for (int i = 0; i < iNumChildren; i++) {
                ...
                for (int j = 0; j < iNumGrandChildren; j++) { kQueue.push(pkInnerNode->Child[j]); }
            }
            ...
            ProcessOuterAndInners(eQueryType, fEpsilon, pkOuterNode->Polygon, kInners, iNextElement,
                                  kMap, kCombined);
...
//----------------------------------------------------------------------------
template<class Real> TriangulateEC<Real>::~TriangulateEC() { WM4_DELETE m_pkQuery; }
//----------------------------------------------------------------------------
template<class Real>
void TriangulateEC<Real>::InitializePositions(const Positions &rkPositions, Query::Type eQueryType,
                                              Real fEpsilon, int iExtraElements)
{
    ...
    switch (eQueryType) {
        ...
        case Query::QT_INTEGER:
            ...
            m_pkQuery = WM4_NEW Query2TInteger<Real>(iPEQuantity, &m_kSPositions[0]);
            return;
        ...
        case Query::QT_RATIONAL:
            ...
            for (i = 0; i < iPQuantity; i++) { m_kSPositions[i] = rkPositions[i]; }
            m_pkQuery = WM4_NEW Query2TRational<Real>(iPEQuantity, &m_kSPositions[0]);
            return;
        ...
        case Query::QT_FILTERED:
            ...
            // expanded when triangulating polygons with holes.
            for (i = 0; i < iPQuantity; i++) { m_kSPositions[i] = rkPositions[i]; }
            m_pkQuery = WM4_NEW Query2Filtered<Real>(iPEQuantity, &m_kSPositions[0], fEpsilon);
            return;
    }
...
//----------------------------------------------------------------------------
template<class Real>
void TriangulateEC<Real>::InitializeVertices(int iVQuantity, const int *aiIndex,
                                        ...
                                        std::vector<int> &rkTriangle)
{
    ...
    while (true) {
        ...
        if (rkVPrev.IsEar) {
            if (!IsEar(iVPrev)) { RemoveE(iVPrev); }
        }
        else {
            bool bWasReflex = !rkVPrev.IsConvex;
            if (IsConvex(iVPrev)) {
                if (bWasReflex) { RemoveR(iVPrev); }

                if (IsEar(iVPrev)) { InsertBeforeE(iVPrev); }
            }
        }

        Vertex &rkVNext = V(iVNext);
        if (rkVNext.IsEar) {
            if (!IsEar(iVNext)) { RemoveE(iVNext); }
        }
        else {
            bool bWasReflex = !rkVNext.IsConvex;
            if (IsConvex(iVNext)) {
                if (bWasReflex) { RemoveR(iVNext); }

                if (IsEar(iVNext)) { InsertAfterE(iVNext); }
            }
        }
...
//----------------------------------------------------------------------------
template<class Real>
int TriangulateEC<Real>::TriangleQuery(const Vector2<Real> &rkPoint, Query::Type eQueryType,
                                       Real fEpsilon, const Vector2<Real> akSTriangle[3]) const
{
    switch (eQueryType) {
        case Query::QT_INT64: return Query2Int64<Real>(3, akSTriangle).ToTriangle(rkPoint, 0, 1, 2);

        case Query::QT_INTEGER:
            return Query2TInteger<Real>(3, akSTriangle).ToTriangle(rkPoint, 0, 1, 2);

        case Query::QT_REAL: return Query2<Real>(3, akSTriangle).ToTriangle(rkPoint, 0, 1, 2);

        case Query::QT_RATIONAL:
            return Query2TRational<Real>(3, akSTriangle).ToTriangle(rkPoint, 0, 1, 2);

        case Query::QT_FILTERED:
            return Query2Filtered<Real>(3, akSTriangle, fEpsilon).ToTriangle(rkPoint, 0, 1, 2);
    }
...
//----------------------------------------------------------------------------
template<class Real>
void TriangulateEC<Real>::CombinePolygons(Query::Type eQueryType, Real fEpsilon, int iNextElement,
                                          const Indices &rkOuter, const Indices &rkInner,
                                          IndexMap &rkMap, Indices &rkCombined)
{
    ...
    for (i0 = iOQuantity - 1, i1 = 0; i1 < iOQuantity; i0 = i1++) {
        ...
        if (kDiff0[1] > (Real)0.0) { continue; }
        Vector2<Real> kDiff1 = m_kSPositions[rkOuter[i1]] - kM;
        if (kDiff1[1] < (Real)0.0) { continue; }
    ...
    if (iEndMin == -1) {
        ...
        int iPIndex;
        if (m_kSPositions[rkOuter[iV0Min]][0] > m_kSPositions[rkOuter[iV1Min]][0]) {
    ...
    int iCIndex = 0;
    for (i = 0; i <= iMaxCosIndex; i++, iCIndex++) { rkCombined[iCIndex] = rkOuter[i]; }
    ...
    if (pkIter != rkMap.end()) { iInnerIndex = pkIter->second; }
    ...
    if (pkIter != rkMap.end()) { iOuterIndex = pkIter->second; }
    rkMap[iNextElement] = iOuterIndex;
    iCIndex++;
    iNextElement++;

    for (i = iMaxCosIndex + 1; i < iOQuantity; i++, iCIndex++) { rkCombined[iCIndex] = rkOuter[i]; }
}
//----------------------------------------------------------------------------
template<class Real>
void TriangulateEC<Real>::ProcessOuterAndInners(Query::Type eQueryType, Real fEpsilon,
                                                const Indices &rkOuter,
                                                const IndicesArray &rkInners, int &riNextElement,
                                                IndexMap &rkMap, Indices &rkCombined)
{
    ...
    for (i = iNumInners - 1; i >= 0; i--) {
        ...
        CombinePolygons(eQueryType, fEpsilon, riNextElement, kCurrentOuter, rkInner, rkMap,
                        kCurrentCombined);
        kCurrentOuter = kCurrentCombined;
        riNextElement += 2;
    }

    for (i = 0; i < (int)kCurrentOuter.size(); i++) { rkCombined.push_back(kCurrentOuter[i]); }
}
//----------------------------------------------------------------------------
template<class Real>
void TriangulateEC<Real>::RemapIndices(const IndexMap &rkMap, Indices &rkTriangles) const
{
    ...
    for (int i = 0; i < (int)rkTriangles.size(); i++) {
        ...
        if (pkIter != rkMap.end()) { rkTriangles[i] = pkIter->second; }
    }
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
template<class Real> typename TriangulateEC<Real>::Vertex &TriangulateEC<Real>::V(int i)
{
    return m_kVertex[i];
}
//----------------------------------------------------------------------------
template<class Real> bool TriangulateEC<Real>::IsConvex(int i)
{
    Vertex &rkV = V(i);
    int iCurr = rkV.Index;
...
//----------------------------------------------------------------------------
template<class Real> bool TriangulateEC<Real>::IsEar(int i)
{
    Vertex &rkV = V(i);
    ...
    for (int j = m_iRFirst; j != -1; j = V(j).SNext) {
        ...
        if (j == rkV.VPrev || j == i || j == rkV.VNext) { continue; }
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::InsertAfterC(int i)
{
    if (m_iCFirst == -1) {
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::InsertAfterR(int i)
{
    if (m_iRFirst == -1) {
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::InsertEndE(int i)
{
    if (m_iEFirst == -1) {
        // add first ear
        m_iEFirst = i;
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::InsertAfterE(int i)
{
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::InsertBeforeE(int i)
{
    Vertex &rkVFirst = V(m_iEFirst);
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::RemoveV(int i)
{
...
//----------------------------------------------------------------------------
template<class Real> int TriangulateEC<Real>::RemoveE(int i)
{
...
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::RemoveR(int i)
{
    assert(m_iRFirst != -1 && m_iRLast != -1);

    if (i == m_iRFirst) {
        m_iRFirst = V(i).SNext;
        if (m_iRFirst != -1) { V(m_iRFirst).SPrev = -1; }
        V(i).SNext = -1;
    }
    else if (i == m_iRLast) {
        m_iRLast = V(i).SPrev;
        if (m_iRLast != -1) { V(m_iRLast).SNext = -1; }
        V(i).SPrev = -1;
    }
    else {
...
// Tree support.
//----------------------------------------------------------------------------
template<class Real> void TriangulateEC<Real>::Delete(Tree *&rpkRoot)
{
    if (rpkRoot) {
        ...
        while (kQueue.size() > 0) {
            ...
            kQueue.pop();
            for (int i = 0; i < (int)pkTree->Child.size(); i++) { kQueue.push(pkTree->Child[i]); }
            WM4_DELETE pkTree;
        }
...
//----------------------------------------------------------------------------
template<class Real> int TriangulateEC<Real>::GetExtraElements(const Tree *pkTree)
{
    ...
    while (kQueue.size() > 0) {
        ...
        for (int i = 0; i < iNumChildren; i++) {
            ...
            for (int j = 0; j < iNumGrandChildren; j++) { kQueue.push(pkChild->Child[j]); }
        }
    }
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class TriangulateEC<float>;

template WM4_FOUNDATION_ITEM class TriangulateEC<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

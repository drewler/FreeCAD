...
template<class Real> class WM4_FOUNDATION_ITEM TriangulateEC
{
public:
    ...
    TriangulateEC(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                  Indices &rkTriangles);
    ...
    TriangulateEC(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                  const Indices &rkPolygon, Indices &rkTriangles);
    ...
    TriangulateEC(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                  const Indices &rkOuter, const Indices &rkInner, Indices &rkTriangles);
    ...
    TriangulateEC(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                  const Indices &rkOuter, const IndicesArray &rkInners, Indices &rkTriangles);
    ...
    TriangulateEC(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                  const Tree *pkTree, Indices &rkTriangles);
...
private:
    ...
    // the triangulation.
    void InitializePositions(const Positions &rkPositions, Query::Type eQueryType, Real fEpsilon,
                             int iExtraElements);

    // Create the vertex objects that store the various lists required by the
    // ear-clipping algorithm.
    void InitializeVertices(int iVQuantity, const int *aiIndex, Indices &rkTriangles);
    ...
    void DoEarClipping(int iVQuantity, const int *aiIndex, Indices &rkTriangles);
    ...
    int TriangleQuery(const Vector2<Real> &rkPosition, Query::Type eQueryType, Real fEpsilon,
    ...
    void CombinePolygons(Query::Type eQueryType, Real fEpsilon, int iNextElement,
                         const Indices &rkOuter, const Indices &rkInner, IndexMap &rkMap,
                         Indices &rkCombined);
    ...
    // polygon.
    void ProcessOuterAndInners(Query::Type eQueryType, Real fEpsilon, const Indices &rkOuter,
                               const IndicesArray &rkInners, int &rkNextElement, IndexMap &rkMap,
                               Indices &rkCombined);

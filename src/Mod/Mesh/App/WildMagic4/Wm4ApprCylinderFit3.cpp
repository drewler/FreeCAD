...
//----------------------------------------------------------------------------
template<class Real>
CylinderFit3<Real>::CylinderFit3(int iQuantity, const Vector3<Real> *akPoint, Vector3<Real> &rkC,
                                 Vector3<Real> &rkU, Real &rfR, Real &rfH,
...
//----------------------------------------------------------------------------
template<class Real> CylinderFit3<Real>::operator Real() { return m_fError; }
//----------------------------------------------------------------------------
template<class Real>
Real CylinderFit3<Real>::UpdateInvRSqr(int iQuantity, const Vector3<Real> *akPoint,
                                       const Vector3<Real> &rkC, const Vector3<Real> &rkU,
                                       Real &rfInvRSqr)
{
...
//----------------------------------------------------------------------------
template<class Real>
Real CylinderFit3<Real>::UpdateDirection(int iQuantity, const Vector3<Real> *akPoint,
                                         const Vector3<Real> &rkC, Vector3<Real> &rkU,
                                         Real &rfInvRSqr)
{
    ...
    for (i = 0; i < iQuantity; i++) {
        ...
        fAAMean += fA * fA;
        kVDir.X() += fA
            * (rkU.X() * (kDelta.Y() * kDelta.Y() + kDelta.Z() * kDelta.Z())
               - kDelta.X() * (rkU.Y() * kDelta.Y() + rkU.Z() * kDelta.Z()));
        kVDir.Y() += fA
            * (rkU.Y() * (kDelta.X() * kDelta.X() + kDelta.Z() * kDelta.Z())
               - kDelta.Y() * (rkU.X() * kDelta.X() + rkU.Z() * kDelta.Z()));
        kVDir.Z() += fA
            * (rkU.Z() * (kDelta.X() * kDelta.X() + kDelta.Y() * kDelta.Y())
               - kDelta.Z() * (rkU.X() * kDelta.X() + rkU.Y() * kDelta.Y()));
    }
    fAMean *= fInvQuantity;
    fAAMean *= fInvQuantity;
    if (kVDir.Normalize() < Math<Real>::ZERO_TOLERANCE) { return fAAMean; }
...
//----------------------------------------------------------------------------
template<class Real>
Real CylinderFit3<Real>::UpdateCenter(int iQuantity, const Vector3<Real> *akPoint,
                                      Vector3<Real> &rkC, const Vector3<Real> &rkU,
                                      const Real &rfInvRSqr)
{
    ...
    fAMean *= fInvQuantity;
    fAAMean *= fInvQuantity;
    if (kCDir.Normalize() < Math<Real>::ZERO_TOLERANCE) { return fAAMean; }
    ...
    if (iMin >= 0) { rkC -= afRoot[iMin] * kCDir; }
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class CylinderFit3<float>;

template WM4_FOUNDATION_ITEM class CylinderFit3<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

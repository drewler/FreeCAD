...
//----------------------------------------------------------------------------
template<class Real>
Delaunay1<Real>::Delaunay1(int iVertexQuantity, Real *afVertex, Real fEpsilon, bool bOwner,
...
//----------------------------------------------------------------------------
template<class Real> Delaunay1<Real>::~Delaunay1()
{
    if (m_bOwner) { WM4_DELETE[] m_afVertex; }
}
//----------------------------------------------------------------------------
template<class Real> const Real *Delaunay1<Real>::GetVertices() const { return m_afVertex; }
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::GetHull(int aiIndex[2])
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> int Delaunay1<Real>::GetContainingSegment(const Real fP) const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return -1; }

    if (fP < m_afVertex[m_aiIndex[0]]) { return -1; }

    if (fP > m_afVertex[m_aiIndex[2 * m_iSimplexQuantity - 1]]) { return -1; }

    int i;
    for (i = 0; i < m_iSimplexQuantity; i++) {
        if (fP < m_afVertex[m_aiIndex[2 * i + 1]]) { break; }
    }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::GetVertexSet(int i, Real afV[2]) const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::GetIndexSet(int i, int aiIndex[2]) const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::GetAdjacentSet(int i, int aiAdjacent[2]) const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return false; }
...
//----------------------------------------------------------------------------
template<class Real>
bool Delaunay1<Real>::GetBarycentricSet(int i, const Real fP, Real afBary[2]) const
{
    assert(m_iDimension == 1);
    if (m_iDimension != 1) { return false; }
    ...
    if (0 <= i && i < m_iSimplexQuantity) {
        ...
        Real fDenom = fV1 - fV0;
        if (fDenom > m_fEpsilon) { afBary[0] = (fV1 - fP) / fDenom; }
        else {
            afBary[0] = (Real)1.0;
        }
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::Load(const char *acFilename)
{
    ...
    if (uiSize == 4) { System::Read4le(pkIFile, m_iVertexQuantity, m_afVertex); }
    else // uiSize == 8
    {
...
//----------------------------------------------------------------------------
template<class Real> bool Delaunay1<Real>::Save(const char *acFilename) const
{
    ...
    size_t uiSize = sizeof(Real);
    if (uiSize == 4) { System::Write4le(pkOFile, m_iVertexQuantity, m_afVertex); }
    else // uiSize == 8
    {
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class Delaunay1<float>;

template WM4_FOUNDATION_ITEM class Delaunay1<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

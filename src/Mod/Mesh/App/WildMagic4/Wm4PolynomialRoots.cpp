...
{
//----------------------------------------------------------------------------
template<class Real> PolynomialRoots<Real>::PolynomialRoots(Real fEpsilon)
{
...
//----------------------------------------------------------------------------
template<class Real> PolynomialRoots<Real>::~PolynomialRoots() { WM4_DELETE[] m_afRoot; }
//----------------------------------------------------------------------------
template<class Real> int PolynomialRoots<Real>::GetCount() const { return m_iCount; }
//----------------------------------------------------------------------------
template<class Real> const Real *PolynomialRoots<Real>::GetRoots() const { return m_afRoot; }
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetRoot(int i) const
{
    assert(0 <= i && i < m_iCount);
    if (0 <= i && i < m_iCount) { return m_afRoot[i]; }

    return Math<Real>::MAX_REAL;
}
//----------------------------------------------------------------------------
template<class Real> Real &PolynomialRoots<Real>::Epsilon() { return m_fEpsilon; }
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::Epsilon() const { return m_fEpsilon; }
//----------------------------------------------------------------------------
template<class Real> int &PolynomialRoots<Real>::MaxIterations() { return m_iMaxIterations; }
//----------------------------------------------------------------------------
template<class Real> int PolynomialRoots<Real>::MaxIterations() const { return m_iMaxIterations; }
//----------------------------------------------------------------------------

//----------------------------------------------------------------------------
// degree 1
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindA(Real fC0, Real fC1)
{
...
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetBound(Real fC0, Real fC1)
{
    if (Math<Real>::FAbs(fC1) <= m_fEpsilon) {
...
// degree 2
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindA(Real fC0, Real fC1, Real fC2)
{
    ...
    if (Math<Real>::FAbs(fDiscr) <= m_fEpsilon) { fDiscr = (Real)0.0; }
...
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetBound(Real fC0, Real fC1, Real fC2)
{
    if (Math<Real>::FAbs(fC2) <= m_fEpsilon) {
        // polynomial is linear
...
// degree 3
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindA(Real fC0, Real fC1, Real fC2, Real fC3)
{
    ...
    if (fDiscr > (Real)0.0) // 1 real, 2 complex roots
    {
        ...
        if (fTemp >= (Real)0.0) { m_afRoot[0] = Math<Real>::Pow(fTemp, fThird); }
        else {
        ...
        Real fAngle = fThird * Math<Real>::ATan2(Math<Real>::Sqrt(-fDiscr), -fHalfB);
        ...
        Real fTemp;
        if (fHalfB >= (Real)0.0) { fTemp = -Math<Real>::Pow(fHalfB, fThird); }
        else {
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::FindE(Real fC0, Real fC1, Real fC2, Real fC3, bool bDoBalancing)
{
    if (Math<Real>::FAbs(fC3) <= m_fEpsilon) {
        // polynomial is quadratic
        return FindA(fC0, fC1, fC2);
    ...
    if (bDoBalancing) { BalanceCompanion3(kMat); }

    return QRIteration3(kMat);
}
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetBound(Real fC0, Real fC1, Real fC2, Real fC3)
{
    if (Math<Real>::FAbs(fC3) <= m_fEpsilon) {
        // polynomial is quadratic
...
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::SpecialCubic(Real fA, Real fB, Real fC)
{
...
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetRowNorm(int iRow, GMatrix<Real> &rkMat)
{
    Real fNorm = Math<Real>::FAbs(rkMat[iRow][0]);
    for (int iCol = 1; iCol < rkMat.GetColumns(); iCol++) {
        Real fAbs = Math<Real>::FAbs(rkMat[iRow][iCol]);
        if (fAbs > fNorm) { fNorm = fAbs; }
    }
    return fNorm;
}
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetColNorm(int iCol, GMatrix<Real> &rkMat)
{
...
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::ScaleRow(int iRow, Real fScale, GMatrix<Real> &rkMat)
{
    for (int iCol = 0; iCol < rkMat.GetColumns(); iCol++) { rkMat[iRow][iCol] *= fScale; }
}
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::ScaleCol(int iCol, Real fScale, GMatrix<Real> &rkMat)
{
    for (int iRow = 0; iRow < rkMat.GetRows(); iRow++) { rkMat[iRow][iCol] *= fScale; }
}
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::IsBalanced3(GMatrix<Real> &rkMat)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (fTest > fTolerance) { return false; }
    }
    return true;
}
//----------------------------------------------------------------------------
template<class Real> void PolynomialRoots<Real>::Balance3(GMatrix<Real> &rkMat)
{
    const int iMax = 16;
    int i;
    for (i = 0; i < iMax; i++) {
        ...
        if (IsBalanced3(rkMat)) { break; }
    }
    assert(i < iMax);
}
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::IsBalancedCompanion3(Real fA10, Real fA21, Real fA02, Real fA12,
                                                 Real fA22)
{
    const Real fTolerance = (Real)0.001;
    ...
    if (fA22 > fColNorm) { fColNorm = fA22; }
    fTest = Math<Real>::FAbs((Real)1.0 - fColNorm / fRowNorm);
    return fTest <= fTolerance;
}
//----------------------------------------------------------------------------
template<class Real> void PolynomialRoots<Real>::BalanceCompanion3(GMatrix<Real> &rkMat)
{
    ...
    for (i = 0; i < iMax; i++) {
        ...
        if (IsBalancedCompanion3(fA10, fA21, fA02, fA12, fA22)) { break; }
    }
    assert(i < iMax);
...
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::QRIteration3(GMatrix<Real> &rkMat)
{
    GVector<Real> kW(3);
    Real fRHS, fTrace, fDet;
    for (int i = 0; i < m_iMaxIterations; i++) {
        fRHS = m_fEpsilon * (Math<Real>::FAbs(rkMat[0][0]) + Math<Real>::FAbs(rkMat[1][1]));
        ...
        fRHS = m_fEpsilon * (Math<Real>::FAbs(rkMat[1][1]) + Math<Real>::FAbs(rkMat[2][2]));
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::FindA(Real fC0, Real fC1, Real fC2, Real fC3, Real fC4)
{
    ...
    if (fDiscr > (Real)0.0) {
        ...
        Real fT2 =
            (((Real)4.0) * fC3 * fC2 - ((Real)8.0) * fC1 - fC3 * fC3 * fC3) / (((Real)4.0) * fR);

        Real fTplus = fT1 + fT2;
        Real fTminus = fT1 - fT2;
        if (Math<Real>::FAbs(fTplus) <= m_fEpsilon) { fTplus = (Real)0.0; }
        if (Math<Real>::FAbs(fTminus) <= m_fEpsilon) { fTminus = (Real)0.0; }
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::FindE(Real fC0, Real fC1, Real fC2, Real fC3, Real fC4,
                                  bool bDoBalancing)
{
    if (Math<Real>::FAbs(fC4) <= m_fEpsilon) {
        // polynomial is cubic
    ...
    if (bDoBalancing) { BalanceCompanion4(kMat); }

    return QRIteration4(kMat);
}
//----------------------------------------------------------------------------
template<class Real>
Real PolynomialRoots<Real>::GetBound(Real fC0, Real fC1, Real fC2, Real fC3, Real fC4)
{
    if (Math<Real>::FAbs(fC4) <= m_fEpsilon) {
        // polynomial is cubic
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::IsBalancedCompanion4(Real fA10, Real fA21, Real fA32, Real fA03,
                                                 Real fA13, Real fA23, Real fA33)
{
    ...
    if (fA23 > fColNorm) { fColNorm = fA23; }
    if (fA33 > fColNorm) { fColNorm = fA33; }
    fTest = Math<Real>::FAbs(1.0f - fColNorm / fRowNorm);
    return fTest <= fTolerance;
}
//----------------------------------------------------------------------------
template<class Real> void PolynomialRoots<Real>::BalanceCompanion4(GMatrix<Real> &rkMat)
{
    Real fA10 = Math<Real>::FAbs(rkMat[1][0]);
    Real fA21 = Math<Real>::FAbs(rkMat[2][1]);
    ...
    for (i = 0; i < iMax; i++) {
        ...
        if (IsBalancedCompanion4(fA10, fA21, fA32, fA03, fA13, fA23, fA33)) { break; }
    }
    assert(i < iMax);
...
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::QRIteration4(GMatrix<Real> &rkMat)
{
    GVector<Real> kW(4);
    ...
    for (i = 0; i < m_iMaxIterations; i++) {
        ...
        if (Math<Real>::FAbs(rkMat[2][1]) <= fRHS) {
            ...
            FindA(fDet, -fTrace, (Real)1.0);
            iSaveCount = m_iCount;
            for (j = 0; j < iSaveCount; j++) { afSaveRoot[j] = m_afRoot[j]; }

            fTrace = rkMat[2][2] + rkMat[3][3];
            fDet = rkMat[2][2] * rkMat[3][3] - rkMat[2][3] * rkMat[3][2];
            FindA(fDet, -fTrace, (Real)1.0);
            for (j = 0; j < iSaveCount; j++) { m_afRoot[m_iCount++] = afSaveRoot[j]; }
            return m_iCount > 0;
        }

        fRHS = m_fEpsilon * (Math<Real>::FAbs(rkMat[2][2]) + Math<Real>::FAbs(rkMat[3][3]));
...
// degree N
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindB(const Polynomial1<Real> &rkPoly, int iDigits)
{
    Real fBound = GetBound(rkPoly);
    return FindB(rkPoly, -fBound, fBound, iDigits);
}
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindN(const Polynomial1<Real> &, int)
{
...
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindE(const Polynomial1<Real> &, bool)
{
    // TO DO:  Implement this.
    assert(false);
    return false;
...
//----------------------------------------------------------------------------
template<class Real> Real PolynomialRoots<Real>::GetBound(const Polynomial1<Real> &rkPoly)
{
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::FindB(const Polynomial1<Real> &rkPoly, Real fXMin, Real fXMax,
                                  int iDigits)
{
    ...
    if (m_iCount > 0) {
        ...
        if (Bisection(rkPoly, fXMin, fXMax, iDigits, fRoot)) { afNewRoot[iNewCount++] = fRoot; }
    }
    ...
    if (iNewCount > 0) {
        ...
        for (i = 1; i < iNewCount; i++) {
            ...
            if (Math<Real>::FAbs(fRootDiff) > m_fEpsilon) { m_afRoot[m_iCount++] = afNewRoot[i]; }
        }
    }
    else {
        m_iCount = 0;
...
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::FindN(const Polynomial1<Real> &, Real, Real, int)
{
    // TO DO:  Implement this.
    assert(false);
    return false;
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::AllRealPartsNegative(const Polynomial1<Real> &rkPoly)
{
    ...
    if (afCoeff[iDegree] != (Real)1.0) {
        ...
        for (int i = 0; i < iDegree; i++) { afCoeff[i] *= fInv; }
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::AllRealPartsPositive(const Polynomial1<Real> &rkPoly)
{
    // make a copy of coefficients, later calls will change the copy
    int iDegree = rkPoly.GetDegree();
    const Real *afPolyCoeff = (const Real *)rkPoly;
    ...
    if (afCoeff[iDegree] != (Real)1.0) {
        Real fInv = ((Real)1.0) / afCoeff[iDegree];
        for (i = 0; i < iDegree; i++) { afCoeff[i] *= fInv; }
        afCoeff[iDegree] = (Real)1.0;
    }

    // reflect z -> -z
    int iSign = -1;
    for (i = iDegree - 1; i >= 0; i--, iSign = -iSign) { afCoeff[i] *= iSign; }

    return AllRealPartsNegative(iDegree, afCoeff);
}
//----------------------------------------------------------------------------
template<class Real> bool PolynomialRoots<Real>::AllRealPartsNegative(int iDegree, Real *afCoeff)
{
    // assert:  afCoeff[iDegree] = 1

    if (afCoeff[iDegree - 1] <= (Real)0.0) { return false; }
    if (iDegree == 1) { return true; }
    ...
    for (i = 1; i <= iDegree - 2; i++) {
        ...
        if (((iDegree - i) % 2) == 0) { afTmpCoeff[i] -= afCoeff[i - 1]; }
    ...
    for (iNextDegree = iDegree - 1; iNextDegree >= 0; iNextDegree--) {
        if (afTmpCoeff[iNextDegree] != (Real)0.0) { break; }
    }
    for (i = 0; i <= iNextDegree - 1; i++) { afCoeff[i] = afTmpCoeff[i] / afTmpCoeff[iNextDegree]; }
...
//----------------------------------------------------------------------------
template<class Real>
int PolynomialRoots<Real>::GetRootCount(const Polynomial1<Real> &rkPoly, Real fT0, Real fT1)
{
    int iDegree = rkPoly.GetDegree();
    ...
    if (iDegree == 0) {
        ...
        if (afCoeff[0] != (Real)0.0) { return 0; }
        else {
    ...
    Polynomial1<Real> *pkF1 = WM4_NEW Polynomial1<Real>(pkF0->GetDerivative());
    ...
    if (fT0 == -Math<Real>::MAX_REAL) {
        pkF0 = kSturm[0];
        if (pkF0->GetDegree() & 1) { fValue0 = -(*pkF0)[pkF0->GetDegree()]; }
        else {
            fValue0 = (*pkF0)[pkF0->GetDegree()];
        }

        if (Math<Real>::FAbs(fValue0) < m_fEpsilon) { fValue0 = (Real)0.0; }

        for (i = 1; i < (int)kSturm.size(); i++) {
            pkF1 = kSturm[i];

            if (pkF1->GetDegree() & 1) { fValue1 = -(*pkF1)[pkF1->GetDegree()]; }
            else {
                fValue1 = (*pkF1)[pkF1->GetDegree()];
            }

            if (Math<Real>::FAbs(fValue1) < m_fEpsilon) { fValue1 = (Real)0.0; }

            if (fValue0 * fValue1 < (Real)0.0 || fValue0 == (Real)0.0) { iSignChanges0++; }
    ...
    if (fT1 == Math<Real>::MAX_REAL) {
        ...
        for (i = 1; i < (int)kSturm.size(); i++) {
            ...
            if (fValue0 * fValue1 < (Real)0.0 || fValue0 == (Real)0.0) { iSignChanges1++; }
    ...
    // clean up
    for (i = 0; i < (int)kSturm.size(); i++) { WM4_DELETE kSturm[i]; }

    if (iSignChanges0 >= iSignChanges1) { return iSignChanges0 - iSignChanges1; }
...
//----------------------------------------------------------------------------
template<class Real>
bool PolynomialRoots<Real>::Bisection(const Polynomial1<Real> &rkPoly, Real fXMin, Real fXMax,
                                      int iDigits, Real &rfRoot)
{
    ...
    if (fP0 * fP1 > (Real)0.0) return false;
...
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::GetHouseholderVector(int iSize, const Vector3<Real> &rkU,
                                                 Vector3<Real> &rkV)
{
    ...
    int i;
    for (i = 1; i < iSize; i++) { fLength += rkU[i] * rkU[i]; }
    fLength = Math<Real>::Sqrt(fLength);

    if (fLength > m_fEpsilon) {
        Real fInv = ((Real)1.0) / (rkU[0] + Math<Real>::Sign(rkU[0]) * fLength);
        rkV[0] = (Real)1.0;
        for (i = 1; i < iSize; i++) { rkV[i] = fInv * rkU[i]; }
    }
    else {
        // U is the zero vector, any vector will do
        rkV[0] = (Real)1.0;
        for (i = 1; i < iSize; i++) { rkV[i] = (Real)0.0; }
    }
}
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::PremultiplyHouseholder(GMatrix<Real> &rkMat, GVector<Real> &rkW,
                                                   int iRMin, int iRMax, int iCMin, int iCMax,
                                                   int iVSize, const Vector3<Real> &rkV)
{
    ...
    for (int i = 1; i < iVSize; i++) { fSqrLen += rkV[i] * rkV[i]; }
...
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::PostmultiplyHouseholder(GMatrix<Real> &rkMat, GVector<Real> &rkW,
                                                    int iRMin, int iRMax, int iCMin, int iCMax,
                                                    int iVSize, const Vector3<Real> &rkV)
{
...
//----------------------------------------------------------------------------
template<class Real>
void PolynomialRoots<Real>::FrancisQRStep(GMatrix<Real> &rkH, GVector<Real> &rkW)
{
    ...
    Real fDet =
        rkH[iN - 2][iN - 2] * rkH[iN - 1][iN - 1] - rkH[iN - 2][iN - 1] * rkH[iN - 1][iN - 2];
    Vector3<Real> kU;
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM class PolynomialRoots<float>;

template WM4_FOUNDATION_ITEM class PolynomialRoots<double>;
//----------------------------------------------------------------------------
} // namespace Wm4

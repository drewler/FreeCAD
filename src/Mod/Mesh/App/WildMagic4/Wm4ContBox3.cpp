...
{
//----------------------------------------------------------------------------
template<class Real> Box3<Real> ContAlignedBox(int iQuantity, const Vector3<Real> *akPoint)
{
    ...
    for (int i = 0; i < 3; i++) { kBox.Extent[i] = kHalfDiagonal[i]; }

    return kBox;
}
//----------------------------------------------------------------------------
template<class Real> Box3<Real> ContOrientedBox(int iQuantity, const Vector3<Real> *akPoint)
{
    ...
    Vector3<Real> kMin(kDiff.Dot(kBox.Axis[0]), kDiff.Dot(kBox.Axis[1]), kDiff.Dot(kBox.Axis[2]));
...
//----------------------------------------------------------------------------
template<class Real>
Box3<Real> ContMinBox(int iQuantity, const Vector3<Real> *akPoint, Real fEpsilon,
                      Query::Type eQueryType)
{
    Box3<Real> kBox;

    // Get the convex hull of the points.
    ConvexHull3<Real> kHull(iQuantity, (Vector3<Real> *)akPoint, fEpsilon, false, eQueryType);
    ...
    if (iHDim == 1) {
        ...
        kBox.Extent[2] = (Real)0.0;
        kBox.Axis[0] = kDiff;
        Vector3<Real>::GenerateComplementBasis(kBox.Axis[1], kBox.Axis[2], kBox.Axis[0]);
    ...
    if (iHDim == 2) {
        ...
        kBox2 = ContMinBox<Real>(iQuantity, akPoint2, fEpsilon, eQueryType, false);
        WM4_DELETE[] akPoint2;
    ...
    for (i = 0; i < 3 * iHQuantity; i++) { kUniqueIndices.insert(aiHIndex[i]); }
    ...
    for (i = 0; i < iHQuantity; i++) {
        ...
        kBox2 = ContMinBox<Real>((int)kUniqueIndices.size(), akPoint2, fEpsilon, eQueryType, false);
        ...
        if (fVolume < fMinVolume) {
            ...
            kBox.Axis[2] = kW;
            kBox.Center =
                kOrigin + kBox2.Center.X() * kU + kBox2.Center.Y() * kV + kBox.Extent[2] * kW;
        }
    }
    ...
    for (pkE2 = kEdges.begin(); pkE2 != kEdges.end(); pkE2++) {
        ...
        for (++pkE1; pkE1 != kEdges.end(); pkE1++) {
            ...
            kV.Normalize();
            Real fDot = kV.Dot(kW);
            if (Math<Real>::FAbs(fDot) > Math<Real>::ZERO_TOLERANCE) { continue; }
            ...
            for (++pkE0; pkE0 != kEdges.end(); pkE0++) {
                ...
                while (pkUI != kUniqueIndices.end()) {
                    ...
                    if (fU < fUMin) { fUMin = fU; }
                    else if (fU > fUMax) {
                        fUMax = fU;
                    }

                    Real fV = kV.Dot(kDiff);
                    if (fV < fVMin) { fVMin = fV; }
                    else if (fV > fVMax) {
                        fVMax = fV;
                    }

                    Real fW = kW.Dot(kDiff);
                    if (fW < fWMin) { fWMin = fW; }
                    else if (fW > fWMax) {
                        fWMax = fW;
                    }
                }
                ...
                if (fVolume < fMinVolume) {
                    ...
                    kBox.Axis[1] = kV;
                    kBox.Axis[2] = kW;
                    kBox.Center = kOrigin + ((Real)0.5) * (fUMin + fUMax) * kU
                        + ((Real)0.5) * (fVMin + fVMax) * kV + ((Real)0.5) * (fWMin + fWMax) * kW;
                }
            }
        }
    }
...
//----------------------------------------------------------------------------
template<class Real> bool InBox(const Vector3<Real> &rkPoint, const Box3<Real> &rkBox)
{
...
//----------------------------------------------------------------------------
template<class Real> Box3<Real> MergeBoxes(const Box3<Real> &rkBox0, const Box3<Real> &rkBox1)
{
    ...
    if (kQ0.Dot(kQ1) < (Real)0.0) { kQ1 = -kQ1; }
...
//----------------------------------------------------------------------------
template WM4_FOUNDATION_ITEM Box3<float> ContAlignedBox<float>(int, const Vector3<float> *);

template WM4_FOUNDATION_ITEM Box3<float> ContOrientedBox<float>(int, const Vector3<float> *);

template WM4_FOUNDATION_ITEM Box3<float> ContMinBox<float>(int, const Vector3<float> *, float,
                                                           Query::Type);

template WM4_FOUNDATION_ITEM bool InBox<float>(const Vector3<float> &, const Box3<float> &);

template WM4_FOUNDATION_ITEM Box3<float> MergeBoxes<float>(const Box3<float> &,
                                                           const Box3<float> &);

template WM4_FOUNDATION_ITEM Box3<double> ContAlignedBox<double>(int, const Vector3<double> *);

template WM4_FOUNDATION_ITEM Box3<double> ContOrientedBox<double>(int, const Vector3<double> *);

template WM4_FOUNDATION_ITEM Box3<double> ContMinBox<double>(int, const Vector3<double> *, double,
                                                             Query::Type);

template WM4_FOUNDATION_ITEM bool InBox<double>(const Vector3<double> &, const Box3<double> &);

template WM4_FOUNDATION_ITEM Box3<double> MergeBoxes<double>(const Box3<double> &,
                                                             const Box3<double> &);
//----------------------------------------------------------------------------
} // namespace Wm4

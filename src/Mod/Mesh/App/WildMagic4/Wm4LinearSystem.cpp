...
{
//----------------------------------------------------------------------------
template<class Real> LinearSystem<Real>::LinearSystem()
{
    ZeroTolerance = Math<Real>::ZERO_TOLERANCE;
}
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::Solve2(const Real aafA[2][2], const Real afB[2], Real afX[2])
{
    Real fDet = aafA[0][0] * aafA[1][1] - aafA[0][1] * aafA[1][0];
    if (Math<Real>::FAbs(fDet) < ZeroTolerance) { return false; }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::Solve3(const Real aafA[3][3], const Real afB[3], Real afX[3])
{
    Real aafAInv[3][3];
    ...
    for (int iRow = 0; iRow < 3; iRow++) {
        for (int iCol = 0; iCol < 3; iCol++) { aafAInv[iRow][iCol] *= fInvDet; }
    }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::Inverse(const GMatrix<Real> &rkA, GMatrix<Real> &rkInvA)
{
    ...
    for (int i0 = 0; i0 < iSize; i0++) {
        ...
        if (iRow != iCol) { rkInvA.SwapRows(iRow, iCol); }
        ...
        for (i2 = 0; i2 < iSize; i2++) { rkInvA[iCol][i2] *= fInv; }
        ...
        for (i1 = 0; i1 < iSize; i1++) {
            if (i1 != iCol) {
                ...
                for (i2 = 0; i2 < iSize; i2++) { rkInvA[i1][i2] -= rkInvA[iCol][i2] * fSave; }
            }
        }
    }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::Solve(const GMatrix<Real> &rkA, const Real *afB, Real *afX)
{
    // computations are performed in-place
    ...
    for (int i0 = 0; i0 < iSize; i0++) {
        ...
        for (i2 = 0; i2 < iSize; i2++) { kInvA[iCol][i2] *= fInv; }
        afX[iCol] *= fInv;
        ...
        for (i1 = 0; i1 < iSize; i1++) {
            if (i1 != iCol) {
                ...
                for (i2 = 0; i2 < iSize; i2++) kInvA[i1][i2] -= kInvA[iCol][i2] * fSave;
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::SolveTri(int iSize, Real *afA, Real *afB, Real *afC, Real *afR, Real *afU)
{
    if (afB[0] == (Real)0.0) { return false; }
    ...
    for (i0 = iSize - 1, i1 = iSize - 2; i1 >= 0; i0--, i1--) { afU[i1] -= afD[i1] * afU[i0]; }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::SolveConstTri(int iSize, Real fA, Real fB, Real fC, Real *afR, Real *afU)
{
    if (fB == (Real)0.0) { return false; }
...
//----------------------------------------------------------------------------
template<class Real> Real LinearSystem<Real>::Dot(int iSize, const Real *afU, const Real *afV)
{
    Real fDot = (Real)0.0;
    for (int i = 0; i < iSize; i++) { fDot += afU[i] * afV[i]; }
    return fDot;
}
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::Multiply(const GMatrix<Real> &rkA, const Real *afX, Real *afProd)
{
    int iSize = rkA.GetRows();
    memset(afProd, 0, iSize * sizeof(Real));
    for (int iRow = 0; iRow < iSize; iRow++) {
        for (int iCol = 0; iCol < iSize; iCol++) { afProd[iRow] += rkA[iRow][iCol] * afX[iCol]; }
    }
}
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::Multiply(int iSize, const SparseMatrix &rkA, const Real *afX, Real *afProd)
{
    memset(afProd, 0, iSize * sizeof(Real));
    ...
    for (/**/; pkIter != rkA.end(); pkIter++) {
        ...
        if (i != j) { afProd[j] += fValue * afX[i]; }
    }
}
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::UpdateX(int iSize, Real *afX, Real fAlpha, const Real *afP)
{
    for (int i = 0; i < iSize; i++) { afX[i] += fAlpha * afP[i]; }
}
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::UpdateR(int iSize, Real *afR, Real fAlpha, const Real *afW)
{
    for (int i = 0; i < iSize; i++) { afR[i] -= fAlpha * afW[i]; }
}
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::UpdateP(int iSize, Real *afP, Real fBeta, const Real *afR)
{
    for (int i = 0; i < iSize; i++) { afP[i] = afR[i] + fBeta * afP[i]; }
}
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::SolveSymmetricCG(const GMatrix<Real> &rkA, const Real *afB, Real *afX)
{
    ...
    for (i = 1; i < iMax; i++) {
        ...
        if (fRoot0 <= ZeroTolerance * fRoot1) { break; }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::SolveSymmetricCG(int iSize, const SparseMatrix &rkA, const Real *afB,
                                          Real *afX)
{
    // based on the algorithm in "Matrix Computations" by Golum and Van Loan
    Real *afR = WM4_NEW Real[iSize];
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::ForwardEliminate(int iReduceRow, BandedMatrix<Real> &rkA, Real *afB)
{
    // the pivot must be nonzero in order to proceed
    Real fDiag = rkA(iReduceRow, iReduceRow);
    if (fDiag == (Real)0.0) { return false; }
    ...
    if (iColMax > rkA.GetSize()) { iColMax = rkA.GetSize(); }

    int iCol;
    for (iCol = iColMin; iCol < iColMax; iCol++) { rkA(iReduceRow, iCol) *= fInvDiag; }
    ...
    if (iRowMax > rkA.GetSize()) { iRowMax = rkA.GetSize(); }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::SolveBanded(const BandedMatrix<Real> &rkA, const Real *afB, Real *afX)
{
    ...
    for (iRow = 0; iRow < iSize; iRow++) {
        if (!ForwardEliminate(iRow, kTmp, afX)) { return false; }
    }
    ...
    for (iRow = iSize - 2; iRow >= 0; iRow--) {
        ...
        if (iColMax > iSize) { iColMax = iSize; }
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::ForwardEliminate(int iReduceRow, BandedMatrix<Real> &rkA,
                                          GMatrix<Real> &rkB)
{
    // the pivot must be nonzero in order to proceed
    Real fDiag = rkA(iReduceRow, iReduceRow);
    ...
    int iCol;
    for (iCol = iColMin; iCol < iColMax; iCol++) { rkA(iReduceRow, iCol) *= fInvDiag; }
    for (iCol = 0; iCol <= iReduceRow; iCol++) { rkB(iReduceRow, iCol) *= fInvDiag; }
...
//----------------------------------------------------------------------------
template<class Real>
void LinearSystem<Real>::BackwardEliminate(int iReduceRow, BandedMatrix<Real> &rkA,
                                           GMatrix<Real> &rkB)
{
...
//----------------------------------------------------------------------------
template<class Real>
bool LinearSystem<Real>::Invert(const BandedMatrix<Real> &rkA, GMatrix<Real> &rkInvA)
{
    int iSize = rkA.GetSize();
    BandedMatrix<Real> kTmp = rkA;
    int iRow;
    for (iRow = 0; iRow < iSize; iRow++) {
        for (int iCol = 0; iCol < iSize; iCol++) {
            if (iRow != iCol) { rkInvA(iRow, iCol) = (Real)0.0; }
            else {
    ...
    // forward elimination
    for (iRow = 0; iRow < iSize; iRow++) {
        if (!ForwardEliminate(iRow, kTmp, rkInvA)) { return false; }
    }

    // backward elimination
    for (iRow = iSize - 1; iRow >= 1; iRow--) { BackwardEliminate(iRow, kTmp, rkInvA); }

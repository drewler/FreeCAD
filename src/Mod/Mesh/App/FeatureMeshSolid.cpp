...
{
    if (Radius.isTouched() || Sampling.isTouched()) return 1;
...
void Sphere::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                       App::Property *prop)
{
...
{
    if (Radius1.isTouched() || Radius2.isTouched() || Sampling.isTouched()) return 1;
    return Feature::mustExecute();
}

App::DocumentObjectExecReturn *Ellipsoid::execute()
{
    std::unique_ptr<MeshObject> mesh(MeshObject::createEllipsoid(
        (float)Radius1.getValue(), (float)Radius2.getValue(), Sampling.getValue()));
    if (mesh.get()) {
        mesh->setPlacement(this->Placement.getValue());
        Mesh.setValue(mesh->getKernel());
        return App::DocumentObject::StdReturn;
...
void Ellipsoid::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                          App::Property *prop)
{
...
{
    std::unique_ptr<MeshObject> mesh(MeshObject::createCylinder(
        (float)Radius.getValue(), (float)Length.getValue(), Closed.getValue(),
        (float)EdgeLength.getValue(), Sampling.getValue()));
    if (mesh.get()) {
        mesh->setPlacement(this->Placement.getValue());
        Mesh.setValue(mesh->getKernel());
        return App::DocumentObject::StdReturn;
...
void Cylinder::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                         App::Property *prop)
{
...
{
    std::unique_ptr<MeshObject> mesh(MeshObject::createCone(
        (float)Radius1.getValue(), (float)Radius2.getValue(), (float)Length.getValue(),
...
void Cone::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                     App::Property *prop)
{
...
{
    std::unique_ptr<MeshObject> mesh(MeshObject::createTorus(
        (float)Radius1.getValue(), (float)Radius2.getValue(), Sampling.getValue()));
    if (mesh.get()) {
        mesh->setPlacement(this->Placement.getValue());
        Mesh.setValue(mesh->getKernel());
...
void Torus::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                      App::Property *prop)
{
    if ((prop == &Radius1 || prop == &Radius2)
        && strcmp(TypeName, "App::PropertyFloatConstraint") == 0) {
...
{
    if (Length.isTouched() || Width.isTouched() || Height.isTouched()) return 1;
    return Feature::mustExecute();
}

App::DocumentObjectExecReturn *Cube::execute()
{
    std::unique_ptr<MeshObject> mesh(MeshObject::createCube(
        (float)Length.getValue(), (float)Width.getValue(), (float)Height.getValue()));
    if (mesh.get()) {
        mesh->setPlacement(this->Placement.getValue());
        Mesh.setValue(mesh->getKernel());
        return App::DocumentObject::StdReturn;
...
void Cube::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                     App::Property *prop)
{

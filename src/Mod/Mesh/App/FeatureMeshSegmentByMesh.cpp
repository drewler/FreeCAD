...
{
    if (Source.isTouched() || Tool.isTouched()) return 1;
    if (Source.getValue() && Source.getValue()->isTouched()) return 1;
    if (Tool.getValue() && Tool.getValue()->isTouched()) return 1;
    return 0;
}
...
App::DocumentObjectExecReturn *SegmentByMesh::execute()
{
    ...
    if (!kernel) return new App::DocumentObjectExecReturn("No mesh specified.\n");
    ...
    if (!toolmesh) { return new App::DocumentObjectExecReturn("No toolmesh specified.\n"); }
    ...
    if (cNormal.Length() > 0.1f) { // not a null vector
        ...
        for (std::vector<MeshCore::FacetIndex>::iterator it = faces.begin(); it != faces.end();
             ++it) {
            cFIt.Set(*it);

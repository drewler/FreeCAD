...
static std::vector<std::string> expandSubObjectNames(
    ...
    std::map<const App::DocumentObject *, std::vector<std::string>> &subObjectNameCache, int depth)
{
    if (!App::GetApplication().checkLinkDepth(depth)) return {};
    ...
    for (auto &sub : subs) {
        ...
        if (!sobj || (vis < 0 && !sobj->Visibility.getValue())) continue;
        ...
        if (it == subObjectNameCache.end())
            it = subObjectNameCache
                     ...
                     .first;
        for (auto &ssub : it->second) res.push_back(sub + ssub);
    }
    return res;
}
    ...
    for (std::string &sub : expandSubObjectNames(obj, subObjectNameCache, 0)) {
        ...
        if (it == meshCache.end()) {
            if (linked->isDerivedFrom(Mesh::Feature::getClassTypeId())) {
                it =
                    meshCache.emplace(linked, static_cast<Mesh::Feature *>(linked)->Mesh.getValue())
                        .first;
        ...
        // Add a new mesh
        if (it != meshCache.end()) {
            if (addMesh(sobj->Label.getValue(), it->second)) ++count;
        }
    }
    return count;
}
...
MergeExporter::MergeExporter(std::string fileName, MeshIO::Format) : fName(fileName) {}

MergeExporter::~MergeExporter() { write(); }
...
bool MergeExporter::addMesh(const char *name, const MeshObject &mesh)
{
    ...
    if (countFacets == 0) { mergingMesh.setKernel(kernel); }
    else {
    ...
    for (unsigned long i = 0; i < numSegm; i++) {
        if (mesh.getSegment(i).isSaved()) canSave++;
    }
        ...
        for (unsigned long i = 0; i < numSegm; i++) {
            ...
            if (segm.isSaved()) {
                ...
                std::for_each(indices.begin(), indices.end(),
                              [countFacets](FacetIndex &v) { v += countFacets; });
...
AbstractFormatExtensionPtr GuiExtension3MFProducer::create() const { return nullptr; }
...
void Extension3MFFactory::addProducer(Extension3MFProducer *ext) { producer.emplace_back(ext); }

void Extension3MFFactory::initialize()
{
    std::vector<Extension3MFProducerPtr> ext = producer;
    for (const auto &it : ext) { it->initialize(); }
}
...
{
public:
    explicit Private(const std::string &filename) : writer3mf(filename)
    {
...
bool Exporter3MF::addMesh(const char *name, const MeshObject &mesh)
{
    ...
    if (ok) {
        for (const auto &it : d->ext) { d->writer3mf.AddResource(it->addMesh(mesh)); }
    }

    return ok;
}

void Exporter3MF::write() { d->writer3mf.Save(); }

// ----------------------------------------------------------------------------

ExporterAMF::ExporterAMF(std::string fileName, const std::map<std::string, std::string> &meta,
                         bool compress)
...
bool ExporterAMF::addMesh(const char *name, const MeshObject &mesh)
{
    ...
    if (!outputStreamPtr || outputStreamPtr->bad()) { return false; }

    auto numFacets(kernel.CountFacets());

    if (numFacets == 0) { return false; }
        ...
        for (auto i(0); i < 3; ++i) {
            ...
            if (vertItr == vertices.end()) {
                ...
                *outputStreamPtr << "\t\t\t\t<vertex>\n"
                ...
                for (auto j(0); j < 3; ++j) {
                    char axis('x' + j);
                    *outputStreamPtr << "\t\t\t\t\t\t<" << axis << '>' << facet->_aclPoints[i][j]
                                     << "</" << axis << ">\n";
                }

...
public:
    ...
    virtual void clear();
    virtual int createCopy(void *pSrcPixelData, unsigned long width, unsigned long height,
                           int format, unsigned short numSigBitsPerSample);
    virtual int pointTo(void *pSrcPixelData, unsigned long width, unsigned long height, int format,
...
protected:
    ...
    unsigned short
        _numSigBitsPerSample; // number of significant bits per sample (always <= _numBitsPerSample)

    // Dependent parameters
    unsigned short
        _numSamples; // number of samples per pixel (e.g. 1 for grey, 3 for rgb, 4 for rgba)

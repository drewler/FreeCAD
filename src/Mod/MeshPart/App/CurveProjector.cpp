...
void CurveProjector::writeIntersectionPointsToFile(const char *name)
{
    ...
    for (result_type::const_iterator it1 = mvEdgeSplitPoints.begin();
         it1 != mvEdgeSplitPoints.end(); ++it1) {
        for (std::vector<FaceSplitEdge>::const_iterator it2 = it1->second.begin();
                                       ...
                                       std::vector<FaceSplitEdge> &vSplitEdges)
{
    ...
    if (!findStartPoint(_Mesh, cStartPoint, cResultPoint, uStartFacetIdx)) return;

    uCurFacetIdx = uStartFacetIdx;
    do {
        MeshGeomFacet cCurFacet = _Mesh.GetFacet(uCurFacetIdx);
        _Mesh.GetFacetNeighbours(uCurFacetIdx, auNeighboursIdx[0], auNeighboursIdx[1],
                                 auNeighboursIdx[2]);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (auNeighboursIdx[i] == uLastFacetIdx) continue;
            ...
            if (Alg.IsDone()) {
                ...
                if (Alg.NbPoints() == 1) {
                    ...
                    if (l < 0.0 || l > 1.0) PointOnEdge[i] = Base::Vector3f(FLOAT_MAX, 0, 0);
                    else {
        ...
        if (uCurFacetIdx == uStartFacetIdx) GoOn = false;

    } while (GoOn);
}

bool CurveProjectorShape::findStartPoint(const MeshKernel &MeshK, const Base::Vector3f &Pnt,
                                         Base::Vector3f &Rslt, MeshCore::FacetIndex &FaceIndex)
{
...
bool CurveProjectorSimple::findStartPoint(const MeshKernel &MeshK, const Base::Vector3f &Pnt,
                                          Base::Vector3f &Rslt, MeshCore::FacetIndex &FaceIndex)
{
    Base::Vector3f TempResultPoint;
    float MinLength = FLOAT_MAX;
    ...
    for (unsigned long i = 0; i < ulNbOfPoints; i++) {
        ...
        for (It.Init(); It.More(); It.Next()) {
            ...
            if (It->IntersectWithLine(
                ...
                if (Base::Distance(LinePoint, cResultPoint) < 0.5) ResultNormal += It->GetNormal();
            }
        }
        LineSeg s;
...
MeshProjection::MeshProjection(const MeshKernel &rMesh) : _rcMesh(rMesh) {}

MeshProjection::~MeshProjection() {}

void MeshProjection::discretize(const TopoDS_Edge &aEdge, std::vector<Base::Vector3f> &polyline,
                                std::size_t minPoints) const
{
    ...
    if (polyline.size() < minPoints) {
        GCPnts_UniformAbscissa clAbsc(clCurve, static_cast<Standard_Integer>(minPoints), fFirst,
                                      fLast);
...
void MeshProjection::splitMeshByShape(const TopoDS_Shape &aShape, float fMaxDist) const
{
    ...
    str.precision(4);
    str.setf(std::ios::fixed | std::ios::showpoint);
    for (std::vector<PolyLine>::const_iterator it = rPolyLines.begin(); it != rPolyLines.end();
         ++it) {
        for (std::vector<Base::Vector3f>::const_iterator jt = it->points.begin();
             jt != it->points.end(); ++jt)
...
void MeshProjection::findSectionParameters(const TopoDS_Edge &edge, const Base::Vector3f &dir,
...
void MeshProjection::projectToMesh(const TopoDS_Shape &aShape, float fMaxDist,
                                   std::vector<PolyLine> &rPolyLines) const
{
    ...
    int iCnt = 0;
    for (Ex.Init(aShape, TopAbs_EDGE); Ex.More(); Ex.Next()) iCnt++;
    ...
    for (Ex.Init(aShape, TopAbs_EDGE); Ex.More(); Ex.Next()) {
        ...
        for (auto it : rSplitEdges) polyline.points.push_back(it.cPt);
...
void MeshProjection::projectOnMesh(const std::vector<Base::Vector3f> &pointsIn,
                                   const Base::Vector3f &dir, float tolerance,
    ...
    for (auto it : pointsIn) {
        ...
        if (clAlg.NearestFacetOnRay(it, dir, cGrid, result, index)) {
            ...
            if (tolerance > 0 && geomFacet.IntersectPlaneWithLine(it, dir, result)) {
                if (geomFacet.IsPointOfFace(result, tolerance)) pointsOut.push_back(result);
            }
            else {
            ...
            if (boundaryPnt != boundaryPoints.end()) { pointsOut.push_back(*boundaryPnt); }
            else {
...
void MeshProjection::projectParallelToMesh(const TopoDS_Shape &aShape, const Base::Vector3f &dir,
                                           std::vector<PolyLine> &rPolyLines) const
{
    // calculate the average edge length and create a grid
    MeshAlgorithm clAlg(_rcMesh);
...
void MeshProjection::projectEdgeToEdge(const TopoDS_Edge &aEdge, float fMaxDist,
                                       ...
                                       std::vector<SplitEdge> &rSplitEdges) const
{
    ...
    // facet to edge
    for (std::vector<MeshCore::FacetIndex>::iterator pI = auFInds.begin(); pI != auFInds.end();
         ++pI) {
    ...
    for (it = pEdgeToFace.begin(); it != pEdgeToFace.end(); ++it) {
        ...
        if (auFaces.size() > 2) continue; // non-manifold edge -> don't handle this
        //      if ( clBB.IsOut( gp_Pnt(cE0.x, cE0.y, cE0.z) ) && clBB.IsOut( gp_Pnt(cE1.x, cE1.y, cE1.z) ) )
        //          continue;

        Base::Vector3f cEdgeNormal;
        for (std::list<MeshCore::FacetIndex>::const_iterator itF = auFaces.begin();
             itF != auFaces.end(); ++itF) {
            cFI.Set(*itF);
        ...
        Handle(Geom_Plane) hPlane = new Geom_Plane(gp_Pln(
            gp_Pnt(cE0.x, cE0.y, cE0.z), gp_Dir(cPlaneNormal.x, cPlaneNormal.y, cPlaneNormal.z)));
    ...
    // sorted by parameter
    for (std::map<Standard_Real, SplitEdge>::iterator itS = rParamSplitEdges.begin();
         itS != rParamSplitEdges.end(); ++itS) {

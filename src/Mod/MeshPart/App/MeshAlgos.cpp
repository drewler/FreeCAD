...
void MeshAlgos::offsetSpecial2(MeshCore::MeshKernel *Mesh, float fSize)
{
    ...
    for (int l = 0; l < 1; l++) {
        for (it.Init(), i = 0; it.More(); it.Next(), i++) {
            if (it->IsFlag(MeshFacet::INVALID)) continue;
...
void MeshAlgos::coarsen(MeshCore::MeshKernel * /*Mesh*/, float /*f*/)
{
    ...
    gts_surface_coarsen(surface, NULL, NULL, NULL, NULL, (GtsStopFunc)gts_coarsen_stop_number,
                        &stop_number, fold);
...
MeshCore::MeshKernel *MeshAlgos::boolean(MeshCore::MeshKernel *pMesh1,
                                         ...
                                         MeshCore::MeshKernel * /*pResult*/, int /*Type*/)
{
#ifdef FC_USE_GTS
    GtsSurface *s1, *s2, *s3;
    GtsSurfaceInter *si;
...
static GtsEdge *new_edge(GtsVertex *v1, GtsVertex *v2)
{
    ...
    if (s == NULL) return gts_edge_new(gts_edge_class(), v1, v2);
    else
        return GTS_EDGE(s);
}
...
{
    GtsSurface *Surf = gts_surface_new(gts_surface_class(), gts_face_class(), gts_edge_class(),
                                       gts_vertex_class());
    ...
    for (unsigned int pFIter = 0; pFIter < Mesh->CountFacets(); pFIter++) {
        ...
        gts_surface_add_face(Surf,
                             gts_face_new(Surf->face_class, new_edge(aVertex[p1], aVertex[p2]),
                                          new_edge(aVertex[p2], aVertex[p3]),
                                          new_edge(aVertex[p3], aVertex[p1])));
    }

    Base::Console().Log(
        "GTS [%d faces, %d Points, %d Edges,%s ,%s]\n", gts_surface_face_number(Surf),
        gts_surface_vertex_number(Surf), gts_surface_edge_number(Surf),
...
void MeshAlgos::cutByShape(const TopoDS_Shape &aShape, const MeshCore::MeshKernel *pMesh,
                           ...
                           const std::vector<CurveProjector::FaceSplitEdge> &vSplitEdges)
{
    ...
    for (std::vector<CurveProjector::FaceSplitEdge>::const_iterator it = vSplitEdges.begin();
         it != vSplitEdges.end(); ++it) {
...
void MeshAlgos::LoftOnCurve(MeshCore::MeshKernel &ResultMesh, const TopoDS_Shape &Shape,
                            const std::vector<Base::Vector3f> &poly, const Base::Vector3f &up,
                            float MaxSize)
{
    TopExp_Explorer Ex;
    ...
    for (Ex.Init(Shape, TopAbs_EDGE); Ex.More(); Ex.Next()) {
        ...
        if (ConnectMap.find(V2) != ConnectMap.end()) bEnd = true;
        ...
        for (long i = 0; i < res; i++) {
            ...
            Base::Vector3f Ptn((float)prop.Value().X(), (float)prop.Value().Y(),
                               (float)prop.Value().Z());
            Base::Vector3f Up(up);

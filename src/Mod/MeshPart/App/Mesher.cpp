...
MeshingOutput::MeshingOutput() { buffer.reserve(80); }

int MeshingOutput::overflow(int c)
{
    if (c != EOF) buffer.push_back((char)c);
    return c;
}
...
struct Vertex {
    ...
    Vertex(Standard_Real X, Standard_Real Y, Standard_Real Z) : x(X), y(Y), z(Z), i(0)
    {
    ...
    const MeshCore::MeshPoint &toPoint() const { return p; }

    bool operator<(const Vertex &v) const
    {
        if (fabs(this->x - v.x) >= deflection) return this->x < v.x;
        if (fabs(this->y - v.y) >= deflection) return this->y < v.y;
        if (fabs(this->z - v.z) >= deflection) return this->z < v.z;
...
public:
    BrepMesh(bool s, const std::vector<uint32_t> &c) : segments(s), colors(c) {}
    ...
    Mesh::MeshObject *create(const std::vector<Part::TopoShape::Domain> &domains) const
    {
        ...
        for (std::size_t i = 0; i < colors.size(); i++) { colorMap[colors[i]].push_back(i); }

        bool createSegm = (colors.size() == domains.size());

        MeshCore::MeshFacetArray faces;
        std::size_t numTriangles = 0;
        for (const auto &it : domains) numTriangles += it.facets.size();
        ...
        for (std::size_t i = 0; i < domains.size(); ++i) {
            ...
            if (createSegm || this->segments) {
                ...
                std::generate(segment.begin(), segment.end(),
        ...
        for (const auto &it : vertices) verts[it.i] = it.toPoint();
        ...
        if (createSegm) {
            ...
            for (const auto &it : colorMap) {
                ...
                for (auto jt : it.second) { segm.addIndices(meshSegments[jt]); }
                segm.save(true);
            ...
            for (const auto &it : meshSegments) { meshdata->addSegment(it); }
        }
        return meshdata;
    }
};
...
Mesher::Mesher(const TopoDS_Shape &s)
    : shape(s), method(None), maxLength(0), maxArea(0), localLength(0), deflection(0),
      angularDeflection(0.5), minLen(0), maxLen(0), relative(false), regular(false), segments(false)
#if defined(HAVE_NETGEN)
      ,
      fineness(5), growthRate(0), nbSegPerEdge(0), nbSegPerRadius(0), secondOrder(false),
      optimize(true), allowquad(false)
#endif
{}
...
{
    // OCC standard mesher
    if (method == Standard) { return createStandard(); }
    ...
    if (!Mesher::_mesh_gen) Mesher::_mesh_gen = new SMESH_Gen();
    ...
    switch (method) {
        ...
        case Netgen: {
            ...
            // user defined values
                ...
                if (growthRate > 0) hyp2d->SetGrowthRate(growthRate);
                if (nbSegPerEdge > 0) hyp2d->SetNbSegPerEdge(nbSegPerEdge);
                if (nbSegPerRadius > 0) hyp2d->SetNbSegPerRadius(nbSegPerRadius);
            }

            if (maxLen > 0) hyp2d->SetMaxSize(maxLen);
            if (minLen > 0) hyp2d->SetMinSize(minLen);
    ...
    for (int i = 0; i < hyp; i++) mesh->AddHypothesis(shape, i);

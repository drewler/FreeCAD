...
std::vector<ColMat<double, 3>> getBoundaries(ColMat<double, 3> vertices, ColMat<long, 3> tris)
{
    ...
    for (long i = 0; i < tris.rows(); i++) {
        for (long j = 0; j < 3; j++) {
            ...
            if (v1 < v2) hash_map[hash] = std::vector<long> {v1, v2, 0};
            else
                hash_map[hash] = std::vector<long> {v2, v1, 0};
        }
    }
    for (auto &hash : hash_list) hash_map[hash][2] += 1;
...
void FaceUnwrapper::findFlatNodes(int steps, double val)
{
    ...
    lscmrelax::LscmRelax mesh_flattener(this->xyz_nodes.transpose(), this->tris.transpose(),
                                        fixed_pins);
    mesh_flattener.lscm();
    for (int j = 0; j < steps; j++) mesh_flattener.relax(val);
...
ColMat<double, 3> FaceUnwrapper::interpolateFlatFace(const TopoDS_Face &face)
{
    ...
    for (long u = 1; u <= _uknots.Length(); u++) { u_knots[u - 1] = _uknots.Value(u); }
    for (long v = 1; v <= _vknots.Length(); v++) { v_knots[v - 1] = _vknots.Value(v); }
...
{
    if (this->ze_nodes.size() == 0) throw(std::runtime_error("flat vertices not xet computed"));

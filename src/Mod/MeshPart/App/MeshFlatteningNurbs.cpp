...
Eigen::VectorXd NurbsBase1D::getKnotSequence(double u_min, double u_max, int u_deg, int num_u_poles)
{
    ...
    for (int i = 0; i < u_deg; i++) u_knots.push_back(u_min);
    for (int i = 0; i < num_u_poles; i++)
        u_knots.push_back(u_min + (u_max - u_min) * i / (num_u_poles - 1));
    for (int i = 0; i < u_deg; i++) u_knots.push_back(u_max);
...
// Return a basis_function for the given degree """
{
    if (degree == 0) {
        return [degree, i, knots](double t) {
            ...
            if (t == knots[0]) return (double)(t_next >= t && t >= t_this);
        ...
        return [degree, i, knots](double t) {
            ...
            if (t < knots[0]) return get_basis(degree, i, knots)(knots[0]);
...
std::function<double(double)> get_basis_derivative(int order, int degree, int i,
    ...
    if (order == 1) {
        return [degree, i, knots, order](double t) {
            ...
            if (!(knots[i + degree] - knots[i] == 0)) {
                out += get_basis(degree - 1, i, knots)(t) * degree / (knots[i + degree] - knots[i]);
            }
            if (!(knots[i + degree + 1] - knots[i + 1] == 0)) {
                out -= get_basis(degree - 1, i + 1, knots)(t) * degree
        ...
        return [degree, i, knots, order](double t) {
            double out = 0;
            if (!(knots[i + degree] - knots[i] == 0)) {
                out += get_basis_derivative(order - 1, degree - 1, i, knots)(t) * degree
                    / (knots[i + degree] - knots[i]);
            }
            if (!(knots[i + degree + 1] - knots[i + 1] == 0)) {
                out -= get_basis_derivative(order - 1, degree - 1, i + 1, knots)(t) * degree
                    / (knots[i + degree + 1] - knots[i + 1]);
            }
            return out;
        };
...
NurbsBase2D::NurbsBase2D(Eigen::VectorXd u_knots, Eigen::VectorXd v_knots, Eigen::VectorXd weights,
...
void add_triplets(Eigen::VectorXd values, double row, std::vector<trip> &triplets)
{
    for (unsigned int i = 0; i < values.size(); i++) {
        if (values(i) != 0.) triplets.push_back(trip(row, i, values(i)));
    }
}
...
std::tuple<NurbsBase2D, Eigen::MatrixXd>
NurbsBase2D::interpolateUBS(Eigen::Matrix<double, Eigen::Dynamic, 3> poles, int degree_u,
                            int degree_v, int num_u_poles, int num_v_poles, int num_u_points,
                            int num_v_points)
{
    ...
    Eigen::Matrix<double, Eigen::Dynamic, 2> uv_points =
        this->getUVMesh(num_u_points, num_v_points);
    Eigen::Matrix<double, Eigen::Dynamic, 3> xyz_points =
        this->getInfluenceMatrix(uv_points) * poles;
...
Eigen::VectorXd NurbsBase1D::getInfluenceVector(double u)
{
    ...
    for (unsigned int i = 0; i < this->u_functions.size(); i++) n_u[i] = this->u_functions[i](u);
...
std::tuple<NurbsBase1D, Eigen::Matrix<double, Eigen::Dynamic, 3>>
NurbsBase1D::interpolateUBS(Eigen::Matrix<double, Eigen::Dynamic, 3> poles, int degree,
                            int num_poles, int num_points)
{
    double u_min = this->u_knots(0);
    double u_max = this->u_knots(this->u_knots.size() - 1);

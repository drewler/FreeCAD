...
public:
    ...
    template<class T> struct TopoDSLess {
...
public:
    ...
    bool findStartPoint(const MeshKernel &MeshK, const Base::Vector3f &Pnt, Base::Vector3f &Rslt,
...
public:
    ...
    void GetSampledCurves(const TopoDS_Edge &aEdge, std::vector<Base::Vector3f> &rclPoints,
                          unsigned long ulNbOfPoints = 30);


    void projectCurve(const TopoDS_Edge &aEdge, const std::vector<Base::Vector3f> &rclPoints,
...
public:
    ...
    CurveProjectorWithToolMesh(const TopoDS_Shape &aShape, const MeshKernel &pMesh,
...
public:
     ...
     * @param parameters
     */
    void findSectionParameters(const TopoDS_Edge &edge, const Base::Vector3f &dir,
                               std::set<double> &parameters) const;
    void discretize(const TopoDS_Edge &aEdge, std::vector<Base::Vector3f> &polyline,
     ...
     */
    void projectToMesh(const TopoDS_Shape &aShape, float fMaxDist,
     ...
     */
    void projectParallelToMesh(const TopoDS_Shape &aShape, const Base::Vector3f &dir,
                               std::vector<PolyLine> &rPolyLines) const;
    /**
     * Project all polylines onto the mesh using parallel projection.
     */
    void projectParallelToMesh(const std::vector<PolyLine> &aEdges, const Base::Vector3f &dir,
                               std::vector<PolyLine> &rPolyLines) const;
    /**
...
protected:
    void projectEdgeToEdge(const TopoDS_Edge &aCurve, float fMaxDist,
                           const MeshCore::MeshFacetGrid &rGrid,
                           std::vector<SplitEdge> &rSplitEdges) const;
    bool findIntersection(const Edge &, const Edge &, const Base::Vector3f &dir,

...
ColMat<double, 2> map_to_2D(ColMat<double, 3> points)
{
    ...
    n.normalize();
    Vector3 y(0, 1, 0);
    if ((n - y).norm() < 0.0001) y = Vector3(1, 0, 0);
    Vector3 x = n.cross(y);
    x.normalize();
    y = n.cross(x);
...
LscmRelax::LscmRelax(RowMat<double, 3> vertices, RowMat<long, 3> triangles,
                     std::vector<long> fixed_pins)
{
    ...
    for (long i = 0; i < this->vertices.cols(); i++) {
        if (fixed_count < this->fixed_pins.size()) {
            if (i == this->fixed_pins[fixed_count]) fixed_count++;
            else
    ...
    for (auto fixed_index : this->fixed_pins) this->old_order.push_back(fixed_index);
    ...
    // set the C-mat
    this->C << 1, nue, 0, nue, 1, 0, 0, 0, (1 - nue) / 2;
...
void LscmRelax::relax(double weight)
{
    ...
    if (this->sol.size() == 0) this->sol.Zero(this->vertices.cols() * 2 + 3);
    ...
    for (long i = 0; i < this->triangles.cols(); i++) {
        ...
        v23 = v3 - v2;
        v31 = v1 - v3;
        B << -v23.y(), 0, -v31.y(), 0, -v12.y(), 0, 0, v23.x(), 0, v31.x(), 0, v12.x(), -v23.x(),
            v23.y(), -v31.x(), v31.y(), -v12.x(), v12.y();
        T << v12.x(), -v12.y(), v12.y(), v12.x();
        T /= v12.norm();
        A = std::abs(this->q_l_m(i, 0) * this->q_l_m(i, 2) / 2);
        B /= A * 2; // (2*area)

        // 2: sigma due dqlg in m-system
        u_m << Vector2(0, 0), T * Vector2(d_q_l_g(i, 0), 0),
            T * Vector2(d_q_l_g(i, 1), d_q_l_g(i, 2));
        ...
        for (int j = 0; j < 3; j++) {
            ...
            for (int k = 0; k < 3; k++) {
                ...
                K_g_triplets.push_back(
                    trip(row_pos * 2 + 1, col_pos * 2 + 1, K_m(j * 2 + 1, k * 2 + 1)));
    ...
    for (long i = 0; i < this->flat_vertices.cols(); i++) {
        ...
        // fixing ux*y-uy*x
        K_g_triplets.push_back(
            trip(i * 2, this->flat_vertices.cols() * 2 + 2, -this->flat_vertices(1, i)));
        K_g_triplets.push_back(
            trip(this->flat_vertices.cols() * 2 + 2, i * 2, -this->flat_vertices(1, i)));
        K_g_triplets.push_back(
            trip(i * 2 + 1, this->flat_vertices.cols() * 2 + 2, this->flat_vertices(0, i)));
        K_g_triplets.push_back(
            trip(this->flat_vertices.cols() * 2 + 2, i * 2 + 1, this->flat_vertices(0, i)));
    }
...
void LscmRelax::area_relax(double weight)
{
    ...
    if (this->sol.size() == 0) this->sol.Zero(this->vertices.cols());
    std::vector<trip> K_g_triplets;
    ...
    for (long i = 0; i < this->triangles.cols(); i++) {
        ...
        for (auto col : range_6) { K_g_triplets.push_back(trip(i, indices[col], (double)B[col])); }
    }
...
void LscmRelax::lscm()
{
    ...
    for (i = 0; i < this->triangles.cols(); i++) {
        ...
        triple_list.push_back(
            trip(2 * i + 1, this->new_order[this->triangles(1, i)] * 2 + 1, -x31));
    ...
    for (auto triplet : triple_list) {
        if (triplet.col()
            > static_cast<int>((this->vertices.cols() - this->fixed_pins.size()) * 2 - 1))
...
void LscmRelax::set_fixed_pins()
{
    ...
    if (this->fixed_pins.size() == 0) this->fixed_pins.push_back(0);
    if (this->fixed_pins.size() == 1) {

        double dist;
        this->fixed_pins.push_back(
            get_max_distance(this->vertices.col(this->fixed_pins[0]), this->vertices, dist));
...
void LscmRelax::transform(bool scale)
{
    ...
    for (long i = 0; i < this->flat_vertices.cols(); i++) this->flat_vertices.col(i) -= center;
    if (scale) this->flat_vertices *= std::pow(global_area / flat_area, 0.5);
    this->set_q_l_m();
}
...
void LscmRelax::rotate_by_min_bound_area()
{
    ...
    for (int i = 0; i < n + 1; i++) {
        phi = i * M_PI / n;
        Eigen::VectorXd x_proj =
            this->flat_vertices.transpose() * Vector2(std::cos(phi), std::sin(phi));
        Eigen::VectorXd y_proj =
            this->flat_vertices.transpose() * Vector2(-std::sin(phi), std::cos(phi));

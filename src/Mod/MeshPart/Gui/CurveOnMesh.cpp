...
/* XPM */
static const char *cursor_curveonmesh[] = {"32 32 3 1",
                                           "+ c white",
                                           "# c red",
                                           ". c None",
                                           "......+.........................",
...
void ViewProviderCurveOnMesh::clearVertex() { pcNodes->point.setNum(0); }
...
void ViewProviderCurveOnMesh::clearPoints() { pcCoords->point.setNum(0); }
...
public:
    ...
    Private()
        : wireClosed(false), distance(1.0), cosAngle(0.7071) // 45 degree
          ,
          approximate(true), curve(new ViewProviderCurveOnMesh), mesh(nullptr), grid(nullptr),
          viewer(nullptr), editcursor(QPixmap(cursor_curveonmesh), 7, 7)
    {}
    ~Private()
    {
        delete curve;
    ...
    bool projectLineOnMesh(const PickedPoint &pick)
    {
        ...
        if (meshProjection.projectLineOnMesh(*grid, v1, last.facet, v2, pick.facet, vd, polyline)) {
            if (polyline.size() > 1) {
                if (cutLines.empty()) { cutLines.push_back(polyline); }
                else {
                    ...
                    if (dir1.dot(dir2) < cosAngle) { cutLines.push_back(polyline); }
                    else {
...
CurveOnMeshHandler::CurveOnMeshHandler(QObject *parent) : QObject(parent), d_ptr(new Private) {}

CurveOnMeshHandler::~CurveOnMeshHandler() { disableCallback(); }

void CurveOnMeshHandler::enableApproximation(bool on) { d_ptr->approximate = on; }

void CurveOnMeshHandler::setParameters(int maxDegree, GeomAbs_Shape cont, double tol3d,
                                       double angle)
{
...
void CurveOnMeshHandler::onCreate()
{
    for (auto it = d_ptr->cutLines.begin(); it != d_ptr->cutLines.end(); ++it) {
        ...
        if (d_ptr->approximate) {
            ...
            if (!spline.IsNull()) displaySpline(spline);
        }
        else {
            TopoDS_Wire wire;
            if (makePolyline(segm, wire)) displayPolyline(wire);
        }
    }
...
{
    if (d_ptr->wireClosed || d_ptr->pickedPoints.size() < 3) { return; }
...
std::vector<SbVec3f> CurveOnMeshHandler::getVertexes() const
{
    ...
    for (std::vector<Private::PickedPoint>::const_iterator it = d_ptr->pickedPoints.begin();
         it != d_ptr->pickedPoints.end(); ++it)
...
Handle(Geom_BSplineCurve) CurveOnMeshHandler::approximateSpline(const std::vector<SbVec3f> &points)
{
    ...
    try {
        ...
        GeomAPI_PointsToBSpline fit(pnts, 1, d_ptr->par.maxDegree, d_ptr->par.cont,
                                    d_ptr->par.tol3d);
...
void CurveOnMeshHandler::displaySpline(const Handle(Geom_BSplineCurve) & spline)
{
    if (d_ptr->viewer) {
        ...
        Part::Feature *part =
            static_cast<Part::Feature *>(doc->addObject("Part::Spline", "Spline"));
...
bool CurveOnMeshHandler::tryCloseWire(const SbVec3f &p) const
{
    if (d_ptr->pickedPoints.size() >= 3) {
        ...
        if (len < d_ptr->distance) { return true; }
    }
        ...
        if (mbe->getButton() == SoMouseButtonEvent::BUTTON1
            ...
            if (pp) {
                ...
                if (!self->d_ptr->wireClosed) {
                    Gui::ViewProvider *vp =
                        view->getDocument()->getViewProviderByPathFromTail(pp->getPath());
                    if (vp
                        ...
                        if (detail && detail->getTypeId() == SoFaceDetail::getClassTypeId()) {
                            ...
                            if (self->d_ptr->pickedPoints.empty()) {
                                ...
                                if (self->tryCloseWire(p)) { self->closeWire(); }

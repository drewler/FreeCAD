...
CurveOnMeshWidget::CurveOnMeshWidget(Gui::View3DInventor *view, QWidget *parent)
    : QWidget(parent), ui(new Ui_TaskCurveOnMesh()), myCurveHandler(new CurveOnMeshHandler(this)),
      myView(view)
{
    ui->setupUi(this);
    this->setup();
...
void CurveOnMeshWidget::setup()
{
    ...
    for (int i = 0; i < 8; i++) ui->maxDegree->addItem(QString::number(i + 1));
...
void CurveOnMeshWidget::on_startButton_clicked()
{
    ...
    myCurveHandler->setParameters(ui->maxDegree->currentIndex() + 1,
                                  static_cast<GeomAbs_Shape>(cont), ui->meshTolerance->value(),
                                  ui->splitAngle->value().getValue());
    myCurveHandler->enableCallback(myView);
}

void CurveOnMeshWidget::reject() { myCurveHandler->recomputeDocument(); }

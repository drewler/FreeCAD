...
{
    // not initialized?
    if (!_instance) _instance = new UnitTestDialog(Gui::getMainWindow());
    return _instance;
}
 ...
 */
bool UnitTestDialog::hasInstance() { return _instance != nullptr; }
...
void UnitTestDialog::on_treeViewFailure_itemDoubleClicked(QTreeWidgetItem *item, int column)
{
    ...
    if (text.size() > 1000) { text = text.left(1000); }
...
{
    QMessageBox::information(
        this, tr("About FreeCAD UnitTest"),
...
void UnitTestDialog::addUnitTest(const QString &unit)
{
    ...
    for (int i = 0; i < ct; i++) {
        if (ui->comboTests->itemText(i) == unit) return;
    }
 ...
 */
void UnitTestDialog::clearUnitTests() { ui->comboTests->clear(); }

/**
 * Returns the unit test.
 */
QString UnitTestDialog::getUnitTest() const { return ui->comboTests->currentText(); }

/**
 * Sets the text in the status bar.
 */
void UnitTestDialog::setStatusText(const QString &text) { ui->textLabelStatus->setText(text); }
...
{
    if (fraction == 0.0f) { ui->progressBar->setRange(0, 100); }
    else {
        if (color == QLatin1String("red")) { setProgressColor(Qt::red); }
 ...
 */
void UnitTestDialog::clearErrorList() { ui->treeViewFailure->clear(); }

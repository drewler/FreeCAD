...
void UnitTestDialogPy::init_type()
{
    ...
    add_varargs_method("setProgressFraction", &UnitTestDialogPy::setProgressFrac,
                       "setProgressFraction");
...
Py::Object UnitTestDialogPy::repr() { return Py::String("UnitTest"); }
...
Py::Object UnitTestDialogPy::insertError(const Py::Tuple &args)
{
    ...
    if (!PyArg_ParseTuple(args.ptr(), "ss", &failure, &details)) throw Py::Exception();
...
Py::Object UnitTestDialogPy::setProgressFrac(const Py::Tuple &args)
{
    float fraction;
    char *pColor = nullptr;
    if (!PyArg_ParseTuple(args.ptr(), "f|s", &fraction, &pColor)) throw Py::Exception();
...
Py::Object UnitTestDialogPy::errorDialog(const Py::Tuple &args)
{
    ...
    if (!PyArg_ParseTuple(args.ptr(), "ss", &title, &message)) throw Py::Exception();
...
{
    int count;
    if (!PyArg_ParseTuple(args.ptr(), "i", &count)) throw Py::Exception();

...
__doc__ = (
    "Implementation of GeneralFuseResult class, which parses return of generalFuse."
)
    ...
    def parse(self):
        ...
        for iPiece in range(len(self.pieces)):
            ...
            if not ha_piece in self._piece_to_index:
                ...
                raise ValueError(
                    "GeneralFuseAnalyzer.parse: duplicate piece shape detected."
                )
        ...
        for iSource in range(len(self.source_shapes)):
            ...
            if not ha_source in self._source_to_index:
                ...
                raise ValueError(
                    "GeneralFuseAnalyzer.parse: duplicate source shape detected."
                )
        ...
        if map_needs_repairing:
            ...
            aggregate_pieces = [
                sh for sh in self.pieces if sh.ShapeType in aggregate_types
            ]
        ...
        for iPiece in range(len(self.pieces)):
            ...
            for element in piece.Vertexes + piece.Edges + piece.Faces + piece.Solids:
                ...
                if el_h in self._element_to_source:
                    self._element_to_source[el_h].update(
                        set(self._sources_of_piece[iPiece])
                    )
                else:
                ...
                for bit in bit_extractor(self.gfa_return[0]):
                    for joint_bit in joint_extractor(bit):
                        if joint_bit.isSame(joint):
                            ...
                            bit_overlap_count = len(
                                self._element_to_source[HashableShape(bit)]
                            )
...
class GeneralFuseReturnBuilder(FrozenClass):
    ...
    def getGFReturn(self):
        return (
            ...
            [
                [self.pieces[iPiece] for iPiece in ilist]
                for ilist in self._pieces_from_source
            ],
        )

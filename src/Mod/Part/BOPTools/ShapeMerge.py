...
def splitIntoGroupsBySharing(list_of_shapes, element_extractor, split_connections=[]):
    ...
    for shape in list_of_shapes:
        shape_elements = set(
            [HashableShape(element) for element in element_extractor(shape)]
        )
...
def mergeSolids(
    ...
    if flag_single:
        ...
        groups = splitIntoGroupsBySharing(
            solids, lambda sh: sh.Faces, split_connections
        )
        if bool_compsolid:
...
def mergeWires(list_of_edges_wires, flag_single=False, split_connections=[]):
    ...
    if flag_single:
        ...
        groups = splitIntoGroupsBySharing(
            edges, lambda sh: sh.Vertexes, split_connections
        )
        return Part.makeCompound(
            [Part.Wire(Part.sortEdges(group)[0]) for group in groups]
        )
...
def mergeShapes(
    list_of_shapes, flag_single=False, split_connections=[], bool_compsolid=False
):

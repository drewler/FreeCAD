...
std::list<TopoDS_Wire> CrossSection::slice(double d) const
{
    ...
    TopExp_Explorer xp;
    for (xp.Init(s, TopAbs_SOLID); xp.More(); xp.Next()) { sliceSolid(d, xp.Current(), wires); }
    ...
    for (const auto &wire : wires) {
        ...
        auto it = std::find_if(wires_reduce.begin(), wires_reduce.end(),
                               [&mapOfEdges1](const TopoDS_Wire &w) {
                                   ...
                                   if (numEdges1 != numEdges2) return false;
                                   ...
                                   for (; it1.More() && it2.More(); it1.Next(), it2.Next()) {
                                       if (!it1.Value().IsSame(it2.Value())) return false;
                                   }

                                   return true;
                               });

        if (it == wires_reduce.end()) { wires_reduce.push_back(wire); }
    }
    return wires_reduce;
}

void CrossSection::sliceNonSolid(double d, const TopoDS_Shape &shape,
...
void CrossSection::sliceSolid(double d, const TopoDS_Shape &shape,
                              std::list<TopoDS_Wire> &wires) const
{
#if 0
...
void CrossSection::connectWires(const TopTools_IndexedMapOfShape &wireMap,
                                std::list<TopoDS_Wire> &wires) const
{

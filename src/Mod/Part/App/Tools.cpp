                                              ...
                                              const Standard_Integer theMaxDeg)
{
    ...
    GeomPlate_BuildPlateSurface aPlateBuilder(aDeg, aNbPnts, aNbIter, aTol2d, aTol3d, anAngTol,
                                              aCurvTol);
        ...
        for (; anIt.More(); anIt.Next(), i++) {
            ...
            if (aCur.IsNull()) {
                ...
                Handle(GeomPlate_CurveConstraint) aConst = new GeomPlate_CurveConstraint(
                    aHCOS, 1 /*GeomAbs_G1*/, aNbPnts, aTol3d, anAngTol, aCurvTol);
    ...
    if (!aPlateBuilder.IsDone()) { return aRes; }
...
bool Part::Tools::getTriangulation(const TopoDS_Face &face, std::vector<gp_Pnt> &points,
                                   std::vector<Poly_Triangle> &facets)
{
    ...
    for (int i = 1; i <= nbTriangles; i++) {
        ...
        if (orient != TopAbs_FORWARD) { std::swap(n1, n2); }
...
bool Part::Tools::getPolygonOnTriangulation(const TopoDS_Edge &edge, const TopoDS_Face &face,
                                  ...
                                  std::vector<gp_Vec> &vertexnormals)
{
    ...
    for (auto &it : vertexnormals) it.Normalize();
}

void Part::Tools::getPointNormals(const std::vector<gp_Pnt> &points, const TopoDS_Face &face,
                                  std::vector<gp_Vec> &vertexnormals)
{
...
void Part::Tools::getPointNormals(const TopoDS_Face &theFace, Handle(Poly_Triangulation) aPolyTri,
                                  TColgp_Array1OfDir &theNormals)
{
    ...
    if (aPolyTri->HasNormals()) {
        ...
        for (Standard_Integer aNodeIter = aNodes.Lower(); aNodeIter <= aNodes.Upper();
            ...
            if (!aNodesUV
                || GeomLib::NormEstim(aSurf, aNodesUV->Value(aNodeIter), aTol,
    ...
    if (aPolyTri->HasNormals()) {
        ...
        for (Standard_Integer aNodeIter = 1; aNodeIter <= numNodes; ++aNodeIter) {
            // try to retrieve normal from real surface first, when UV coordinates are available
            if (!hasNodesUV
                || GeomLib::NormEstim(aSurf, aPolyTri->UVNode(aNodeIter), aTol,
                                      theNormals(aNodeIter))
                    > 1) {
                // compute flat normals
                gp_XYZ eqPlan(0.0, 0.0, 0.0);
...
void Part::Tools::getPointNormals(const TopoDS_Face &face, Handle(Poly_Triangulation) aPoly,
                                  std::vector<gp_Vec> &normals)
{
    ...
    for (int i = dirs.Lower(); i <= dirs.Upper(); ++i) { normals.emplace_back(dirs(i).XYZ()); }
}
                                               ...
                                               std::vector<gp_Vec> &normals)
{
    if (!loc.IsIdentity()) {
        ...
        for (auto &it : normals) { it.Transform(myTransf); }
    }
}
...
Handle(Poly_Polygon3D) Part::Tools::polygonOfEdge(const TopoDS_Edge &edge, TopLoc_Location &loc)
{
    ...
    if (!aPoly.IsNull() && !Precision::IsInfinite(u) && !Precision::IsInfinite(v)) return aPoly;
...
void Part::Tools::getNormal(const TopoDS_Face &face, double u, double v, const Standard_Real tol,
                            gp_Dir &dir, Standard_Boolean &done)
{
    BRepAdaptor_Surface adapt(face);

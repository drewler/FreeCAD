...
std::string HLRBRep_AlgoPy::representation() const { return std::string("<HLRBRep_Algo object>"); }

PyObject *HLRBRep_AlgoPy::add(PyObject *args)
{
    PyObject *shape;
    int nbIso = 0;
    if (!PyArg_ParseTuple(args, "O!|i", &Part::TopoShapePy::Type, &shape, &nbIso)) return nullptr;
...
PyObject *HLRBRep_AlgoPy::setProjector(PyObject *args, PyObject *kwds)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "|O!O!O!d", kwlist, &Base::VectorPy::Type, &ps,
                                    &Base::VectorPy::Type, &zd, &Base::VectorPy::Type, &xd,
                                    &focus)) {
        gp_Ax2 ax2;
        if (ps && zd && xd) {
        ...
        if (boost::math::isnan(focus)) getHLRBRep_AlgoPtr()->Projector(HLRAlgo_Projector(ax2));
        else
...
PyObject *HLRBRep_AlgoPy::showAll(PyObject *args)
{
    ...
    if (i < 1) getHLRBRep_AlgoPtr()->ShowAll();
    else
...
{
    int i = -1, j = -1;
    if (!PyArg_ParseTuple(args, "|ii", &i, &j)) return nullptr;

    if (i < 1) getHLRBRep_AlgoPtr()->Hide();
    else if (j < 1)
...
PyObject *HLRBRep_AlgoPy::hideAll(PyObject *args)
{
    ...
    if (i < 1) getHLRBRep_AlgoPtr()->HideAll();
    else
...
PyObject *HLRBRep_AlgoPy::select(PyObject *args)
{
    ...
    if (i < 1) getHLRBRep_AlgoPtr()->Select();
    else
...
PyObject *HLRBRep_AlgoPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int HLRBRep_AlgoPy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

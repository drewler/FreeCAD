...
{
    PyObject *l;
    if (!PyArg_ParseTuple(args, "O", &l)) return nullptr;
    ...
    try {
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            shapes.emplace_back(sh1.extensionObject()->getTopoShapePtr()->getShape(),
...
{
    char *Name;
    if (!PyArg_ParseTuple(args, "et", "utf-8", &Name)) return nullptr;
...
PyObject *TopoShapePy::writeInventor(PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "|iddO", kwlist, &mode, &dev, &angle, &pylist))
        return nullptr;
    ...
    if (mode == 0) { getTopoShapePtr()->exportFaceSet(dev, angle, faceColors, result); }
    else if (mode == 1) {
...
{
    char *input;
    if (!PyArg_ParseTuple(args, "s", &input)) return nullptr;
...
PyObject *TopoShapePy::importBrepFromString(PyObject *args)
{
    char *input;
    int indicator = 1;
    if (!PyArg_ParseTuple(args, "s|i", &input, &indicator)) return nullptr;
...
PyObject *TopoShapePy::__getstate__(PyObject *args) { return exportBrepToString(args); }
...
PyObject *TopoShapePy::exportStl(PyObject *args)
{
    ...
    char *Name;
    if (!PyArg_ParseTuple(args, "et|d", "utf-8", &Name, &deflection)) return nullptr;
...
{
    PyObject *pVec;
    if (!PyArg_ParseTuple(args, "O!", &(Base::VectorPy::Type), &pVec)) return nullptr;
    ...
    try {
        ...
        switch (type) {
            case TopAbs_COMPOUND: return new TopoShapeCompoundPy(new TopoShape(shape));
            case TopAbs_COMPSOLID: return new TopoShapeCompSolidPy(new TopoShape(shape));
            case TopAbs_SOLID: return new TopoShapeSolidPy(new TopoShape(shape));
            case TopAbs_SHELL: return new TopoShapeShellPy(new TopoShape(shape));
            case TopAbs_FACE: return new TopoShapeFacePy(new TopoShape(shape));
            case TopAbs_WIRE: break;
            case TopAbs_EDGE: return new TopoShapeEdgePy(new TopoShape(shape));
...
PyObject *TopoShapePy::revolve(PyObject *args)
{
    PyObject *pPos, *pDir;
    double d = 360;
    if (!PyArg_ParseTuple(args, "O!O!|d", &(Base::VectorPy::Type), &pPos, &(Base::VectorPy::Type),
                          &pDir, &d))
        return nullptr;
...
PyObject *TopoShapePy::check(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &(PyBool_Type), &runBopCheck)) return nullptr;
...
PyObject *TopoShapePy::multiFuse(PyObject *args)
{
    double tolerance = 0.0;
    PyObject *pcObj;
    if (!PyArg_ParseTuple(args, "O|d", &pcObj, &tolerance)) return nullptr;
...
{
    PyObject *pcObj;
    if (!PyArg_ParseTuple(args, "O!", &(TopoShapePy::Type), &pcObj)) return nullptr;
...
PyObject *TopoShapePy::section(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "O!|O!", &(TopoShapePy::Type), &pcObj, &(PyBool_Type), &approx)) {
        ...
        try {
            ...
            TopoDS_Shape secShape =
                this->getTopoShapePtr()->section(shape, Base::asBoolean(approx));
    ...
    PyErr_Clear();
    double tolerance = 0.0;
    if (PyArg_ParseTuple(args, "O!d|O!", &(TopoShapePy::Type), &pcObj, &tolerance, &(PyBool_Type),
                         &approx)) {
        std::vector<TopoDS_Shape> shapeVec;
        shapeVec.push_back(static_cast<TopoShapePy *>(pcObj)->getTopoShapePtr()->getShape());
        try {
            TopoDS_Shape sectionShape =
                this->getTopoShapePtr()->section(shapeVec, tolerance, Base::asBoolean(approx));
    ...
    if (PyArg_ParseTuple(args, "O|dO!", &pcObj, &tolerance, &(PyBool_Type), &approx)) {
        ...
        try {
            TopoDS_Shape multiSectionShape =
                this->getTopoShapePtr()->section(shapeVec, tolerance, Base::asBoolean(approx));
...
PyObject *TopoShapePy::slice(PyObject *args)
{
    PyObject *dir;
    double d;
    if (!PyArg_ParseTuple(args, "O!d", &(Base::VectorPy::Type), &dir, &d)) return nullptr;
...
{
    PyObject *dir, *dist;
    if (!PyArg_ParseTuple(args, "O!O", &(Base::VectorPy::Type), &dir, &dist)) return nullptr;
...
PyObject *TopoShapePy::generalFuse(PyObject *args)
{
    ...
    try {
        ...
        TopoDS_Shape gfaResultShape =
            this->getTopoShapePtr()->generalFuse(shapeVec, tolerance, &map);
...
PyObject *TopoShapePy::sewShape(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|d", &tolerance)) return nullptr;
            ...
            if (!aChild.IsNull()) {
                ...
                switch (type) {
                    ...
                    case TopAbs_COMPSOLID:
                        ...
                        break;
                    case TopAbs_SOLID: pyChild = new TopoShapeSolidPy(new TopoShape(aChild)); break;
                    case TopAbs_SHELL: pyChild = new TopoShapeShellPy(new TopoShape(aChild)); break;
                    case TopAbs_FACE: pyChild = new TopoShapeFacePy(new TopoShape(aChild)); break;
                    case TopAbs_WIRE: pyChild = new TopoShapeWirePy(new TopoShape(aChild)); break;
                    case TopAbs_EDGE: pyChild = new TopoShapeEdgePy(new TopoShape(aChild)); break;
                    case TopAbs_VERTEX:
                ...
                if (pyChild) { list.append(Py::Object(pyChild, true)); }
            }
        }
...
const std::map<PyTypeObject *, TopAbs_ShapeEnum> mapTypeShape(vecTypeShape.begin(),
                                                              vecTypeShape.end());
...
static TopAbs_ShapeEnum ShapeTypeFromPyType(PyTypeObject *pyType)
{
    for (auto it = vecTypeShape.begin(); it != vecTypeShape.end(); ++it) {
        if (PyType_IsSubtype(pyType, it->first)) return it->second;
    }
    return TopAbs_SHAPE;
};
} // namespace Part
...
{
    double minArea;
    if (!PyArg_ParseTuple(args, "d", &minArea)) return nullptr;
...
PyObject *TopoShapePy::transformShape(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!|O!O!", &(Base::MatrixPy::Type), &obj, &(PyBool_Type), &copy,
                          &(PyBool_Type), &checkScale))
        return nullptr;

    Base::Matrix4D mat = static_cast<Base::MatrixPy *>(obj)->value();
    PY_TRY
    {
        this->getTopoShapePtr()->transformShape(mat, Base::asBoolean(copy),
...
PyObject *TopoShapePy::transformed(PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "O!|O!O!s", kwlist, &Base::MatrixPy::Type,
                                     &pymat, &PyBool_Type, &copy, &PyBool_Type, &checkScale, &op))
        return nullptr;
...
PyObject *TopoShapePy::rotate(PyObject *args)
{
    PyObject *obj1, *obj2;
    double angle;
    if (!PyArg_ParseTuple(args, "OOd", &obj1, &obj2, &angle)) return nullptr;
...
PyObject *TopoShapePy::scale(PyObject *args)
{
    double factor;
    PyObject *p = nullptr;
    if (!PyArg_ParseTuple(args, "d|O!", &factor, &(Base::VectorPy::Type), &p)) return nullptr;
...
PyObject *TopoShapePy::makeThickness(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Odd|O!O!hh", &obj, &offset, &tolerance, &(PyBool_Type), &inter,
                          &(PyBool_Type), &self_inter, &offsetMode, &join))
        return nullptr;
    ...
    try {
        ...
        TopoDS_Shape shape = this->getTopoShapePtr()->makeThickSolid(
            facesToRemove, offset, tolerance, Base::asBoolean(inter), Base::asBoolean(self_inter),
            offsetMode, join);
...
{
    static char *kwlist[] = {"offset",     "tolerance", "inter", "self_inter",
                             "offsetMode", "join",      "fill",  nullptr};
    double offset, tolerance;
    PyObject *inter = Py_False;
    PyObject *self_inter = Py_False;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "dd|O!O!hhO!", kwlist, &offset, &tolerance,
                                     &(PyBool_Type), &inter, &(PyBool_Type), &self_inter,
                                     &offsetMode, &join, &(PyBool_Type), &fill))
        return nullptr;

    try {
        TopoDS_Shape shape = this->getTopoShapePtr()->makeOffsetShape(
            offset, tolerance, Base::asBoolean(inter), Base::asBoolean(self_inter), offsetMode,
            join, Base::asBoolean(fill));
        return new TopoShapePy(new TopoShape(shape));
    }
    catch (Standard_Failure &e) {
        PyErr_SetString(PartExceptionOCCError, e.GetMessageString());
...
PyObject *TopoShapePy::makeOffset2D(PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "d|hO!O!O!", kwlist, &offset, &join,
                                     &(PyBool_Type), &fill, &(PyBool_Type), &openResult,
                                     &(PyBool_Type), &inter))
        return nullptr;

    try {
        TopoDS_Shape resultShape = this->getTopoShapePtr()->makeOffset2D(
            offset, join, Base::asBoolean(fill), Base::asBoolean(openResult),
...
PyObject *TopoShapePy::reversed(PyObject *args)
{
    ...
    if (!shape.IsNull()) { static_cast<TopoShapePy *>(cpy)->getTopoShapePtr()->setShape(shape); }
    return cpy;
}
...
PyObject *TopoShapePy::isValid(PyObject *args)
{
    ...
    PY_TRY { return Py_BuildValue("O", (getTopoShapePtr()->isValid() ? Py_True : Py_False)); }
    PY_CATCH_OCC
}
...
PyObject *TopoShapePy::isCoplanar(PyObject *args)
{
    PyObject *pyObj;
    double tol = -1;
    if (!PyArg_ParseTuple(args, "O!|d", &TopoShapePy::Type, &pyObj, &tol)) return nullptr;
...
PyObject *TopoShapePy::isInfinite(PyObject *args)
{
    ...
    PY_TRY { return Py::new_reference_to(Py::Boolean(getTopoShapePtr()->isInfinite())); }
    PY_CATCH_OCC
}
...
PyObject *TopoShapePy::fix(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ddd", &prec, &mintol, &maxtol)) return nullptr;
...
PyObject *TopoShapePy::hashCode(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|i", &upper)) return nullptr;
...
PyObject *TopoShapePy::tessellate(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "d|O!", &tolerance, &PyBool_Type, &ok)) return nullptr;

    try {
        std::vector<Base::Vector3d> Points;
        std::vector<Data::ComplexGeoData::Facet> Facets;
        if (Base::asBoolean(ok)) BRepTools::Clean(getTopoShapePtr()->getShape());
        getTopoShapePtr()->getFaces(Points, Facets, tolerance);
        Py::Tuple tuple(2);
        Py::List vertex;
        for (std::vector<Base::Vector3d>::const_iterator it = Points.begin(); it != Points.end();
             ++it)
            vertex.append(Py::asObject(new Base::VectorPy(*it)));
        tuple.setItem(0, vertex);
        Py::List facet;
        for (std::vector<Data::ComplexGeoData::Facet>::const_iterator it = Facets.begin();
             it != Facets.end(); ++it) {
            Py::Tuple f(3);
...
PyObject *TopoShapePy::makeParallelProjection(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!O!", &(Part::TopoShapePy::Type), &pShape, &Base::VectorPy::Type,
                          &pDir))
        return nullptr;
...
{
    static char *kwlist[] = {"ViewDir", "ViewPos", "UpDir", "EdgeType",
                             "Visible", "OnShape", nullptr};
    ...
    PyObject *pView;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!|O!O!sO!O!", kwlist, &Base::VectorPy::Type,
                                     &pView, &Base::VectorPy::Type, &pPos, &Base::VectorPy::Type,
    ...
    try {
        ...
        std::string str = type;
        if (str == "IsoLine") t = HLRBRep_IsoLine;
        else if (str == "Rg1Line")
            t = HLRBRep_Rg1Line;
        else if (str == "RgNLine")
            t = HLRBRep_RgNLine;
        ...
        if (pPos) p = Py::Vector(pPos, false).toVector();
        Base::Vector3d u(0.0, 1.0, 0.0);
        if (pUp) u = Py::Vector(pUp, false).toVector();
...
PyObject *TopoShapePy::makeShapeFromMesh(PyObject *args)
{
    ...
    try {
        ...
        if (Base::asBoolean(sewShape)) getTopoShapePtr()->sewShape(tolerance);
...
{
    const char *op = nullptr;
    if (!PyArg_ParseTuple(args, "s", &op)) return nullptr;

    PY_TRY { return Py::new_reference_to(shape2pyshape(getTopoShapePtr()->makeWires(op))); }
    PY_CATCH_OCC
}
...
PyObject *TopoShapePy::isInside(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!dO!", &(Base::VectorPy::Type), &point, &tolerance, &PyBool_Type,
                          &checkFace))
        return nullptr;
    ...
    try {
        ...
        if (shape.ShapeType() == TopAbs_VERTEX || shape.ShapeType() == TopAbs_EDGE
            ...
            if (Base::asBoolean(checkFace) && solidClassifier.IsOnAFace()) test = Standard_True;
            return Py_BuildValue("O", (test ? Py_True : Py_False));
        }
    }
    catch (Standard_Failure &e) {
...
PyObject *TopoShapePy::getElement(PyObject *args)
{
    ...
    try {
        if (boost::regex_match(input, ex)) {
            std::unique_ptr<Part::ShapeSegment> s(
                static_cast<Part::ShapeSegment *>(getTopoShapePtr()->getSubElementByName(input)));
            TopoDS_Shape shape = s->Shape;
            switch (shape.ShapeType()) {
                case TopAbs_FACE: return new TopoShapeFacePy(new TopoShape(shape));
                case TopAbs_EDGE: return new TopoShapeEdgePy(new TopoShape(shape));
                case TopAbs_VERTEX: return new TopoShapeVertexPy(new TopoShape(shape));
                default: break;
            }
        }
...
PyObject *TopoShapePy::countElement(PyObject *args)
{
    ...
    PY_TRY { return Py::new_reference_to(Py::Int((long)getTopoShapePtr()->countSubShapes(input))); }
    PY_CATCH_OCC
}
...
PyObject *TopoShapePy::getTolerance(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i|O!", &mode, &PyType_Type, &type)) return nullptr;
...
PyObject *TopoShapePy::overTolerance(PyObject *args)
{
    double value;
    PyObject *type = reinterpret_cast<PyObject *>(&TopoShapePy::Type);
    if (!PyArg_ParseTuple(args, "d|O!", &value, &PyType_Type, &type)) return nullptr;
...
PyObject *TopoShapePy::inTolerance(PyObject *args)
{
    ...
    double valmax;
    PyObject *type = reinterpret_cast<PyObject *>(&TopoShapePy::Type);
    if (!PyArg_ParseTuple(args, "dd|O!", &valmin, &valmax, &PyType_Type, &type)) return nullptr;
...
PyObject *TopoShapePy::limitTolerance(PyObject *args)
{
    ...
    double tmax = 0;
    PyObject *type = reinterpret_cast<PyObject *>(&TopoShapePy::Type);
    if (!PyArg_ParseTuple(args, "d|dO!", &tmin, &tmax, &PyType_Type, &type)) return nullptr;
...
PyObject *TopoShapePy::proximity(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!|d", &(TopoShapePy::Type), &ps2, &tol)) return nullptr;
    ...
    if (tol > 0.0) proximity.SetTolerance(tol);
    proximity.Perform();
    if (!proximity.IsDone()) {
        ...
        while (xp.More()) {
            ...
            if (aTriangulation.IsNull()) {
                PyErr_SetString(
                    ...
                    "BRepExtrema_ShapeProximity not done, call 'tessellate' beforehand");
                return nullptr;
            }
        }
...
PyObject *TopoShapePy::distToShape(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!", &(TopoShapePy::Type), &ps2)) return nullptr;
...
PyObject *TopoShapePy::optimalBoundingBox(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!O!", &PyBool_Type, &PyBool_Type, &useT, &useS)) return nullptr;
...
Py::String TopoShapePy::getOrientation() const
{
    ...
    std::string name;
    switch (type) {
        case TopAbs_FORWARD: name = "Forward"; break;
        case TopAbs_REVERSED: name = "Reversed"; break;
        case TopAbs_INTERNAL: name = "Internal"; break;
        case TopAbs_EXTERNAL: name = "External"; break;
    }
...
void TopoShapePy::setOrientation(Py::String arg)
{
    ...
    if (name == "Forward") { type = TopAbs_FORWARD; }
...
Py::List TopoShapePy::getFaces() const { return getShapes<TopoShapeFacePy>(getTopoShapePtr()); }
...
Py::List TopoShapePy::getShells() const { return getShapes<TopoShapeShellPy>(getTopoShapePtr()); }

Py::List TopoShapePy::getSolids() const { return getShapes<TopoShapeSolidPy>(getTopoShapePtr()); }
...
Py::List TopoShapePy::getEdges() const { return getShapes<TopoShapeEdgePy>(getTopoShapePtr()); }

Py::List TopoShapePy::getWires() const { return getShapes<TopoShapeWirePy>(getTopoShapePtr()); }
...
{
    const TopoDS_Shape &shape = getTopoShapePtr()->getShape();
    if (shape.IsNull()) throw Py::RuntimeError("shape is invalid");
    GProp_GProps props;
...
PyObject *TopoShapePy::getCustomAttributes(const char *attr) const
{
    ...
    PY_TRY
    {
        ...
        if (!res.IsNull()) return Py::new_reference_to(shape2pyshape(res));
    }
    PY_CATCH_OCC
    return nullptr;
}

...
std::string GeometrySurfacePy::representation() const { return "<Surface object>"; }
...
// constructor method
int GeometrySurfacePy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *GeometrySurfacePy::toShell(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|O!O!", kwlist, &PyTuple_Type, &bound,
                                     &PyBool_Type, &segm))
        return nullptr;
...
PyObject *GeometrySurfacePy::getDN(PyObject *args)
{
    try {
        int nu, nv;
        double u, v;
        if (!PyArg_ParseTuple(args, "ddii", &u, &v, &nu, &nv)) return nullptr;
...
PyObject *GeometrySurfacePy::curvature(PyObject *args)
{
    try {
        ...
        if (s) {
            double u, v;
            char *type;
            if (!PyArg_ParseTuple(args, "dds", &u, &v, &type)) return nullptr;

            GeomSurface::Curvature t;
            if (strcmp(type, "Max") == 0) { t = GeomSurface::Maximum; }
...
PyObject *GeometrySurfacePy::parameter(PyObject *args)
{
    ...
    try {
        if (!surf.IsNull()) {
            ...
            if (!PyArg_ParseTuple(args, "O!|d", &(Base::VectorPy::Type), &p, &prec)) return nullptr;
...
{
    double v;
    if (!PyArg_ParseTuple(args, "d", &v)) return nullptr;
...
PyObject *GeometrySurfacePy::toBSpline(PyObject *args, PyObject *kwds)
{
    ...
    static char *kwlist[] = {"Tol3d",      "UContinuity", "VContinuity", "MaxDegreeU",
                             "MaxDegreeV", "MaxSegments", "PrecisCode",  nullptr};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|dssiiii", kwlist, &tol3d, &ucont, &vcont,
                                     &maxDegU, &maxDegV, &maxSegm, &prec))
        return nullptr;

    GeomAbs_Shape absU, absV;
    std::string uc = ucont;
    if (maxDegU <= 1) absU = GeomAbs_C0;
    else if (uc == "C0")
        absU = GeomAbs_C0;
    else if (uc == "C1")
        absU = GeomAbs_C1;
...
PyObject *GeometrySurfacePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int GeometrySurfacePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

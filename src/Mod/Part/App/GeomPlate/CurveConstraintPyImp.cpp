...
int CurveConstraintPy::PyInit(PyObject *args, PyObject *kwds)
{
    ...
    static char *keywords[] = {"Boundary", "Order",   "NbPts", "TolDist",
                               "TolAng",   "TolCurv", nullptr};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!|iiddd", keywords, &(GeometryCurvePy::Type),
                                     &bound, &order, &nbPts, &tolDist, &tolAng, &tolCurv))
        return -1;
    ...
    try {
        ...
        if (bound) {
            ...
            if (curve->getTypeId().isDerivedFrom(GeomTrimmedCurve::getClassTypeId())) {
                ...
                hCurve = new GeomAdaptor_Curve(handle, trim->getFirstParameter(),
            ...
            if (curve->getTypeId().isDerivedFrom(GeomTrimmedCurve::getClassTypeId())) {
                GeomTrimmedCurve *trim = static_cast<GeomTrimmedCurve *>(curve);
                GeomAdaptor_Curve adapt(handle, trim->getFirstParameter(),
                                        trim->getLastParameter());
            ...
            ptr.reset(
                new GeomPlate_CurveConstraint(hCurve, order, nbPts, tolDist, tolAng, tolCurv));
        }
        else {
...
{
    PyObject *c;
    if (!PyArg_ParseTuple(args, "O!", &Part::Curve2dPy::Type, &c)) return nullptr;
...
PyObject *CurveConstraintPy::setProjectedCurve(PyObject *args)
{
    PyObject *c;
    double tolU, tolV;
    if (!PyArg_ParseTuple(args, "O!dd", &Part::Curve2dPy::Type, &c, &tolU, &tolV)) return nullptr;
...
PyObject *CurveConstraintPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

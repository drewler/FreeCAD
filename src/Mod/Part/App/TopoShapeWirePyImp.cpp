...
int TopoShapeWirePy::PyInit(PyObject *args, PyObject * /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(args, "O!", &(Part::TopoShapePy::Type), &pcObj)) {
        ...
        if (sh.ShapeType() == TopAbs_EDGE) mkWire.Add(TopoDS::Edge(sh));
...
{
    PyObject *edge;
    if (!PyArg_ParseTuple(args, "O!", &(TopoShapePy::Type), &edge)) return nullptr;
...
PyObject *TopoShapeWirePy::fixWire(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!d", &(TopoShapeFacePy::Type), &face, &tol)) return nullptr;
...
{
    double dist;
    if (!PyArg_ParseTuple(args, "d", &dist)) return nullptr;
    const TopoDS_Wire &w = TopoDS::Wire(getTopoShapePtr()->getShape());
...
PyObject *TopoShapeWirePy::makePipeShell(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "O|O!O!i", &obj, &PyBool_Type, &make_solid, &PyBool_Type, &is_Frenet,
                         &transition)) {
        try {
            ...
            TopoDS_Shape shape = this->getTopoShapePtr()->makePipeShell(
                sections, Base::asBoolean(make_solid), Base::asBoolean(is_Frenet), transition);
            return new TopoShapePy(new TopoShape(shape));
        }
        catch (Standard_Failure &e) {
...
PyObject *TopoShapeWirePy::makeEvolved(PyObject *args, PyObject *kwds)
{
    ...
    static char *kwds_evolve[] = {"Profile",     "Join",      "AxeProf", "Solid",
                                  "ProfOnSpine", "Tolerance", nullptr};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!|iO!O!O!d", kwds_evolve, &TopoShapeWirePy::Type,
                                     &Profile, &JoinType, &PyBool_Type, &AxeProf, &PyBool_Type,
                                     &Solid, &PyBool_Type, &ProfOnSpine, &Tolerance))
        return nullptr;
    ...
    const TopoDS_Wire &profile =
        TopoDS::Wire(static_cast<TopoShapeWirePy *>(Profile)->getTopoShapePtr()->getShape());

    GeomAbs_JoinType joinType;
    switch (JoinType) {
        case GeomAbs_Tangent: joinType = GeomAbs_Tangent; break;
        case GeomAbs_Intersection: joinType = GeomAbs_Intersection; break;
        default: joinType = GeomAbs_Arc; break;
    }

    try {
        BRepOffsetAPI_MakeEvolved evolved(spine, profile, joinType, Base::asBoolean(AxeProf),
                                          Base::asBoolean(Solid), Base::asBoolean(ProfOnSpine),
                                          Tolerance);
...
{
    PyObject *wire;
    if (!PyArg_ParseTuple(args, "O!", &(Part::TopoShapeWirePy::Type), &wire)) return nullptr;
    try {
        TopoDS_Wire o1, o2;
...
PyObject *TopoShapeWirePy::approximate(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|ddii", kwds_approx, &tol2d, &tol3d, &maxseg,
                                     &maxdeg))
        return nullptr;
    try {
        ...
        if (approx.IsDone()) { return new BSplineCurvePy(new GeomBSplineCurve(approx.Curve())); }
        else {
    ...
    try {
        ...
        if (PyArg_ParseTuple(args, "O", &dist_or_num)) {
            ...
            if (PyArg_ParseTupleAndKeywords(args, kwds, "i|dd", kwds_numPoints, &numPoints, &first,
                ...
                PyErr_Clear();
                if (PyArg_ParseTupleAndKeywords(args, kwds, "d|dd", kwds_Distance, &distance,
                                                &first, &last)) {
                    uniformAbscissaDistance = true;
                }
            }
        ...
        if (uniformAbscissaPoints || uniformAbscissaDistance) {
            ...
            if (uniformAbscissaPoints) discretizer.Initialize(adapt, numPoints, first, last);
            else
...
Py::String TopoShapeWirePy::getContinuity() const
{
    ...
    switch (adapt.Continuity()) {
        case GeomAbs_C0: cont = "C0"; break;
        case GeomAbs_G1: cont = "G1"; break;
        case GeomAbs_C1: cont = "C1"; break;
        case GeomAbs_G2: cont = "G2"; break;
        case GeomAbs_C2: cont = "C2"; break;
        case GeomAbs_C3: cont = "C3"; break;
        case GeomAbs_CN: cont = "CN"; break;
    }
...
Py::Object TopoShapeWirePy::getMatrixOfInertia() const
{
    ...
    Base::Matrix4D mat;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) { mat[i][j] = m(i + 1, j + 1); }
    }
...
Py::List TopoShapeWirePy::getOrderedVertexes() const
{
    ...
    if (!Vfirst.IsNull() && !Vlast.IsNull()) {
        if (!Vfirst.IsSame(Vlast)) { ret.append(shape2pyshape(Vlast)); }
    }

    return ret;
}

PyObject *TopoShapeWirePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int TopoShapeWirePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

...
PartExport std::list<TopoDS_Edge> sort_Edges(double tol3d, std::list<TopoDS_Edge> &edges)
{
    ...
    if (edge_points.empty()) return std::list<TopoDS_Edge>();
...
private:
    Py::Object sameParameter(const Py::Tuple &args)
    {
        ...
        PyObject *enforce;
        double prec = 0.0;
        if (!PyArg_ParseTuple(args.ptr(), "O!O!|d", &TopoShapePy::Type, &shape, &PyBool_Type,
                              &enforce, &prec))
            throw Py::Exception();
    ...
    {
        PyObject *shape;
        if (!PyArg_ParseTuple(args.ptr(), "O!", &TopoShapePy::Type, &shape)) throw Py::Exception();
...
public:
    Module() : Py::ExtensionModule<Module>("Part")
    {
        ...
        add_varargs_method(
            "makeRevolution", &Module::makeRevolution,
        ...
        add_varargs_method(
            "makeRuledSurface", &Module::makeRuledSurface,
        ...
        add_varargs_method(
            "makeSplitShape", &Module::makeSplitShape,
        ...
        add_varargs_method("getSortedClusters", &Module::getSortedClusters,
                           ...
                           "a variety of edges");
        add_varargs_method(
            "__sortEdges__", &Module::sortEdges,
        ...
        add_keyword_method(
            ...
            "           1: return (shape,subObj,mat), where subObj is the object referenced in "
            "'subname',\n"
...
private:
    Py::Object invoke_method_keyword(void *method_def, const Py::Tuple &args,
    ...
    {
        char *Name;
        if (!PyArg_ParseTuple(args.ptr(), "et", "utf-8", &Name)) throw Py::Exception();
        ...
        // extract ending
        if (file.extension().empty()) throw Py::RuntimeError("No file extension");
...
#else
            Part::ImportStep *pcFeature = (Part::ImportStep *)pcDoc->addObject(
                "Part::ImportStep", file.fileNamePure().c_str());
    ...
    Py::Object insert(const Py::Tuple &args)
    {
        char *Name;
        const char *DocName;
        if (!PyArg_ParseTuple(args.ptr(), "ets", "utf-8", &Name, &DocName)) throw Py::Exception();
        ...
        if (!pcDoc) { pcDoc = App::GetApplication().newDocument(DocName); }
    ...
    Py::Object exporter(const Py::Tuple &args)
    {
        PyObject *object;
        char *Name;
        if (!PyArg_ParseTuple(args.ptr(), "Oet", &object, "utf-8", &Name)) throw Py::Exception();
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            if (PyObject_TypeCheck(item, &(App::DocumentObjectPy::Type))) {
                ...
                if (obj->getTypeId().isDerivedFrom(Part::Feature::getClassTypeId())) {
                    ...
                    if (!shape.IsNull()) builder.Add(comp, shape);
                }
                else {
    ...
    Py::Object show(const Py::Tuple &args)
    {
        ...
        if (!pcDoc) pcDoc = App::GetApplication().newDocument();
    ...
    Py::Object getFacets(const Py::Tuple &args)
    {
        PyObject *shape;
        Py::List list;
        if (!PyArg_ParseTuple(args.ptr(), "O", &shape)) throw Py::Exception();
            ...
            if (Tools::getTriangulation(currentFace, points, facets)) {
                for (const auto &it : facets) {
                    ...
                    if (!p1.IsEqual(p2, 0.01) && !p2.IsEqual(p3, 0.01) && !p3.IsEqual(p1, 0.01)) {
                        PyObject *t1 =
                            PyTuple_Pack(3, PyFloat_FromDouble(p1.X()), PyFloat_FromDouble(p1.Y()),
                                         PyFloat_FromDouble(p1.Z()));
                        PyObject *t2 =
                            PyTuple_Pack(3, PyFloat_FromDouble(p2.X()), PyFloat_FromDouble(p2.Y()),
                                         PyFloat_FromDouble(p2.Z()));
                        PyObject *t3 =
                            PyTuple_Pack(3, PyFloat_FromDouble(p3.X()), PyFloat_FromDouble(p3.Y()),
    ...
    {
        PyObject *pcObj;
        if (!PyArg_ParseTuple(args.ptr(), "O", &pcObj)) throw Py::Exception();
    ...
    {
        PyObject *obj;
        if (!PyArg_ParseTuple(args.ptr(), "O", &obj)) throw Py::Exception();
            ...
            for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                if (PyObject_TypeCheck((*it).ptr(), &(Part::TopoShapeFacePy::Type))) {
                    const TopoDS_Shape &sh =
                        static_cast<TopoShapeFacePy *>((*it).ptr())->getTopoShapePtr()->getShape();
                    if (!sh.IsNull()) builder.Add(shell, sh);
                }
            }
    ...
    Py::Object makeFace(const Py::Tuple &args)
    {
        try {
                ...
                if (PySequence_Check(pcPyShapeOrList)) {
                    ...
                    for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                        ...
                        if (PyObject_TypeCheck(item, &(Part::TopoShapePy::Type))) {
                            const TopoDS_Shape &sh = static_cast<Part::TopoShapePy *>(item)
                                                         ->getTopoShapePtr()
                                                         ->getShape();
                            fm->addShape(sh);
                        }
                    ...
                    const TopoDS_Shape &sh = static_cast<Part::TopoShapePy *>(pcPyShapeOrList)
                                                 ->getTopoShapePtr()
                                                 ->getShape();
                    if (sh.IsNull()) throw NullShapeException("Shape is null!");
                    if (sh.ShapeType() == TopAbs_COMPOUND) fm->useCompound(TopoDS::Compound(sh));
                    else
                        fm->addShape(sh);
                }
                else {
                    throw Py::Exception(PyExc_TypeError,
                                        "First argument is neither a shape nor list of shapes.");
                }
            ...
            throw Py::TypeError(std::string("Argument type signature not recognized. Should be "
                                            "either (list, string), or (shape, string)"));
        }
        catch (Standard_Failure &e) {
            throw Py::Exception(PartExceptionOCCError, e.GetMessageString());
        }
    ...
    Py::Object makeFilledSurface(const Py::Tuple &args)
    {
        PyObject *obj;
        double tolerance;
        if (!PyArg_ParseTuple(args.ptr(), "Od", &obj, &tolerance)) throw Py::Exception();
        ...
        try {
            ...
            for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                if (PyObject_TypeCheck((*it).ptr(), &(Part::GeometryCurvePy::Type))) {
                    Handle(Geom_Curve) hCurve = Handle(Geom_Curve)::DownCast(
                        static_cast<Part::GeometryCurvePy *>((*it).ptr())
                            ->getGeomCurvePtr()
                            ->handle());
                    if (!hCurve.IsNull()) { generator.AddCurve(hCurve); }
                }
            }
            ...
            if (!hSurface.IsNull()) {
                ...
                throw Py::Exception(PartExceptionOCCError,
                                    "Failed to created surface by filling curves");
            }
        }
        catch (Standard_Failure &e) {
            throw Py::Exception(PartExceptionOCCError, e.GetMessageString());
        ...
        try {
            if (surf) {
                const TopoDS_Shape &face =
                    static_cast<TopoShapeFacePy *>(surf)->getTopoShapePtr()->getShape();
            ...
            if (numConstraints == 0) {
                throw Py::Exception(PartExceptionOCCError,
                                    "Failed to create face with no constraints");
            }
    ...
    {
        PyObject *obj;
        if (!PyArg_ParseTuple(args.ptr(), "O!", &(TopoShapePy::Type), &obj)) throw Py::Exception();

        try {
            const TopoDS_Shape &shape =
                static_cast<TopoShapePy *>(obj)->getTopoShapePtr()->getShape();
            ...
            if (count == 0) {
                ...
                Standard_Failure::Raise("Only one compsolid can be accepted. Provided shape has "
                                        "more than one compsolid.");
    ...
    Py::Object makePlane(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "dd|O!O!O!", &length, &width, &(Base::VectorPy::Type),
                              &pPnt, &(Base::VectorPy::Type), &pDirZ, &(Base::VectorPy::Type),
                              &pDirX))
            throw Py::Exception();

        if (length < Precision::Confusion()) { throw Py::ValueError("length of plane too small"); }
        if (width < Precision::Confusion()) { throw Py::ValueError("width of plane too small"); }
    ...
    Py::Object makeBox(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "ddd|O!O!", &length, &width, &height,
                              &(Base::VectorPy::Type), &pPnt, &(Base::VectorPy::Type), &pDir))
            throw Py::Exception();

        if (length < Precision::Confusion()) { throw Py::ValueError("length of box too small"); }
        if (width < Precision::Confusion()) { throw Py::ValueError("width of box too small"); }
        if (height < Precision::Confusion()) { throw Py::ValueError("height of box too small"); }
    ...
    Py::Object makeWedge(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "dddddddddd|O!O!", &xmin, &ymin, &zmin, &z2min, &x2min,
                              &xmax, &ymax, &zmax, &z2max, &x2max, &(Base::VectorPy::Type), &pPnt,
        ...
        if (dx < Precision::Confusion()) { throw Py::ValueError("delta x of wedge too small"); }
        if (dy < Precision::Confusion()) { throw Py::ValueError("delta y of wedge too small"); }
        if (dz < Precision::Confusion()) { throw Py::ValueError("delta z of wedge too small"); }
        if (dz2 < 0) { throw Py::ValueError("delta z2 of wedge is negative"); }
        if (dx2 < 0) { throw Py::ValueError("delta x2 of wedge is negative"); }
        ...
        try {
            ...
            BRepPrim_Wedge mkWedge(gp_Ax2(p, d), xmin, ymin, zmin, z2min, x2min, xmax, ymax, zmax,
                                   z2max, x2max);
            BRepBuilderAPI_MakeSolid mkSolid;
    ...
    {
        PyObject *obj1, *obj2;
        if (!PyArg_ParseTuple(args.ptr(), "OO", &obj1, &obj2)) throw Py::Exception();
        ...
        switch (makeEdge.Error()) {
            case BRepBuilderAPI_EdgeDone: break; // ok
            case BRepBuilderAPI_PointProjectionFailed: error = "Point projection failed"; break;
            case BRepBuilderAPI_ParameterOutOfRange: error = "Parameter out of range"; break;
        ...
        // Error
        if (error) { throw Py::Exception(PartExceptionOCCError, error); }
    ...
    Py::Object makeCircle(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "d|O!O!dd", &radius, &(Base::VectorPy::Type), &pPnt,
                              &(Base::VectorPy::Type), &pDir, &angle1, &angle2))
            throw Py::Exception();
        ...
        try {
            ...
            BRepBuilderAPI_MakeEdge aMakeEdge(hCircle, angle1 * (M_PI / 180),
                                              angle2 * (M_PI / 180));
    ...
    Py::Object makeSphere(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "d|O!O!ddd", &radius, &(Base::VectorPy::Type), &pPnt,
                              &(Base::VectorPy::Type), &pDir, &angle1, &angle2, &angle3))
            throw Py::Exception();
        ...
        try {
            ...
            BRepPrimAPI_MakeSphere mkSphere(gp_Ax2(p, d), radius, angle1 * (M_PI / 180),
                                            angle2 * (M_PI / 180), angle3 * (M_PI / 180));
    ...
    Py::Object makeCylinder(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "dd|O!O!d", &radius, &height, &(Base::VectorPy::Type),
                              &pPnt, &(Base::VectorPy::Type), &pDir, &angle))
            throw Py::Exception();
    ...
    Py::Object makeCone(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "ddd|O!O!d", &radius1, &radius2, &height,
                              &(Base::VectorPy::Type), &pPnt, &(Base::VectorPy::Type), &pDir,
                              &angle))
            throw Py::Exception();
    ...
    Py::Object makeTorus(const Py::Tuple &args)
    {
        ...
        PyObject *pPnt = nullptr, *pDir = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "dd|O!O!ddd", &radius1, &radius2, &(Base::VectorPy::Type),
                              &pPnt, &(Base::VectorPy::Type), &pDir, &angle1, &angle2, &angle))
            throw Py::Exception();
        ...
        try {
            ...
            BRepPrimAPI_MakeTorus mkTorus(gp_Ax2(p, d), radius1, radius2, angle1 * (M_PI / 180),
                                          angle2 * (M_PI / 180), angle * (M_PI / 180));
    ...
    Py::Object makeHelix(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "ddd|dO!O!", &pitch, &height, &radius, &angle,
                              &(PyBool_Type), &pleft, &(PyBool_Type), &pvertHeight))
            throw Py::Exception();
    ...
    Py::Object makeRevolution(const Py::Tuple &args)
    {
        ...
        do {
            if (PyArg_ParseTuple(args.ptr(), "O!|dddO!O!O!", &(GeometryPy::Type), &pCrv, &vmin,
                                 &vmax, &angle, &(Base::VectorPy::Type), &pPnt,
                                 &(Base::VectorPy::Type), &pDir, &(PyType_Type), &type)) {
                ...
                if (vmax == -DBL_MAX) vmax = curve->LastParameter();
                break;
            }

            PyErr_Clear();
            if (PyArg_ParseTuple(args.ptr(), "O!|dddO!O!O!", &(TopoShapePy::Type), &pCrv, &vmin,
                                 &vmax, &angle, &(Base::VectorPy::Type), &pPnt,
                                 &(Base::VectorPy::Type), &pDir, &(PyType_Type), &type)) {
                const TopoDS_Shape &shape =
                ...
                if (vmin == DBL_MAX) vmin = adapt.FirstParameter();
                if (vmax == -DBL_MAX) vmax = adapt.LastParameter();
                break;
            }
    ...
    Py::Object makeRuledSurface(const Py::Tuple &args)
    {
        ...
        PyObject *sh1, *sh2;
        if (!PyArg_ParseTuple(args.ptr(), "O!O!", &(TopoShapePy::Type), &sh1, &(TopoShapePy::Type),
                              &sh2))
            throw Py::Exception();
    ...
    {
        PyObject *pylist;
        if (!PyArg_ParseTuple(args.ptr(), "O", &pylist)) throw Py::Exception();
        ...
        try {
            ...
            for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                ...
                if (!s.IsNull() && s.ShapeType() == TopAbs_WIRE) { fill.AddWire(TopoDS::Wire(s)); }
            }
    ...
    Py::Object makeTube(const Py::Tuple &args)
    {
        ...
        // Path + radius
        if (!PyArg_ParseTuple(args.ptr(), "O!d|sii", &(TopoShapePy::Type), &pshape, &radius, &scont,
                              &maxdegree, &maxsegment))
            throw Py::Exception();

        std::string str_cont = scont;
        int cont;
        if (str_cont == "C0") cont = (int)GeomAbs_C0;
        else if (str_cont == "C1")
            cont = (int)GeomAbs_C1;
        else if (str_cont == "C2")
            cont = (int)GeomAbs_C2;
    ...
    Py::Object makeSweepSurface(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "O!O!|di", &(TopoShapePy::Type), &path,
                              &(TopoShapePy::Type), &profile, &tolerance, &fillMode))
            throw Py::Exception();

        try {
            const TopoDS_Shape &path_shape =
                static_cast<TopoShapePy *>(path)->getTopoShapePtr()->getShape();
            const TopoDS_Shape &prof_shape =
                static_cast<TopoShapePy *>(profile)->getTopoShapePtr()->getShape();
    ...
    Py::Object makeLoft(const Py::Tuple &args)
    {
        ...
        PyObject *pclosed = Py_False;
        int degMax = 5;
        if (!PyArg_ParseTuple(args.ptr(), "O|O!O!O!i", &pcObj, &(PyBool_Type), &psolid,
                              &(PyBool_Type), &pruled, &(PyBool_Type), &pclosed, &degMax)) {
            throw Py::Exception();
        }
    ...
    Py::Object makeWireString(const Py::Tuple &args)
    {
        ...
        if (PyBytes_Check(intext)) {
            ...
            if (!p) { throw Py::TypeError("** makeWireString can't convert PyString."); }
...
#if PY_VERSION_HEX >= 0x03090000
            if (unichars) { PyMem_Free(unichars); }
#endif
        }
    ...
    {
        char *unit = nullptr;
        if (!PyArg_ParseTuple(args.ptr(), "|s", &unit)) throw Py::Exception();
    ...
    Py::Object getSortedClusters(const Py::Tuple &args)
    {
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            if (PyObject_TypeCheck(item, &(Part::TopoShapePy::Type))) {
                ...
                if (sh.ShapeType() == TopAbs_EDGE) edges.push_back(TopoDS::Edge(sh));
                else {
    ...
    Py::Object toPythonOCC(const Py::Tuple &args)
    {
        ...
        try {
            ...
            proxy = Base::Interpreter().createSWIGPointerObj("OCC.TopoDS", "TopoDS_Shape *",
                                                             (void *)shape, 1);
    ...
    {
        PyObject *proxy;
        if (!PyArg_ParseTuple(args.ptr(), "O", &proxy)) throw Py::Exception();

        void *ptr;
        try {
            TopoShape *shape = new TopoShape();
            Base::Interpreter().convertSWIGPointerObj("OCC.TopoDS", "TopoDS_Shape *", proxy, &ptr,
                                                      0);
    ...
    Py::Object getShape(const Py::Tuple &args, const Py::Dict &kwds)
    {
        ...
        short retType = 0;
        static char *kwd_list[] = {"obj",       "subname", "mat",          "needSubElement",
                                   "transform", "retType", "noElementMap", "refine",
                                   nullptr};
        if (!PyArg_ParseTupleAndKeywords(args.ptr(), kwds.ptr(), "O!|sO!O!O!hO!O!", kwd_list,
                                         &App::DocumentObjectPy::Type, &pObj, &subname,
                                         &Base::MatrixPy::Type, &pyMat, &PyBool_Type,
                                         &needSubElement, &PyBool_Type, &transform, &retType,
        ...
        Base::Matrix4D mat;
        if (pyMat) mat = *static_cast<Base::MatrixPy *>(pyMat)->getMatrixPtr();
        auto shape = Feature::getTopoShape(obj, subname, Base::asBoolean(needSubElement), &mat,
                                           &subObj, retType == 2, Base::asBoolean(transform),
    ...
    Py::Object joinSubname(const Py::Tuple &args)
    {
        ...
        const char *mapped;
        const char *element;
        if (!PyArg_ParseTuple(args.ptr(), "sss", &sub, &mapped, &element)) throw Py::Exception();
        std::string subname(sub);
        if (!subname.empty() && subname[subname.size() - 1] != '.') subname += '.';
        if (mapped && mapped[0]) {

...
AttachEngine::AttachEngine()
    : mapMode(mmDeactivated), mapReverse(false), attachParameter(0.0), surfU(0.0), surfV(0.0)
{}

void AttachEngine::setUp(const App::PropertyLinkSubList &references, eMapMode mapMode,
                         bool mapReverse, double attachParameter, double surfU, double surfV,
...
{
    setUp(another.references, another.mapMode, another.mapReverse, another.attachParameter,
          another.surfU, another.surfV, another.attachmentOffset);
}

Base::Placement AttachEngine::placementFactory(const gp_Dir &ZAxis, gp_Vec XAxis, gp_Pnt Origin,
                                               gp_Pnt refOrg, bool useRefOrg_Line,
                                               bool useRefOrg_Plane, bool makeYVertical,
                                               ...
                                               Base::Placement *placeOfRef) const
{
    ...
    if (XAxis.Magnitude() < Precision::Confusion()) makeYVertical = true;

    gp_Ax3 ax3; //OCC representation of the final placement
    if (!makeYVertical) { ax3 = gp_Ax3(Origin, ZAxis, XAxis); }
        ...
        // +X/-X
        if (pos == 0) {
            if (cosNX > 0) ax3 = gp_Ax3(Origin, ZAxis, dirY);
            else
                ax3 = gp_Ax3(Origin, ZAxis, -dirY);
        }
        // +Y/-Y
        else if (pos == 1) {
            if (cosNY > 0) ax3 = gp_Ax3(Origin, ZAxis, -dirX);
            else
...
void AttachEngine::suggestMapModes(SuggestResult &result) const
{
    ...
    for (std::size_t iMode = 0; iMode < this->modeRefTypes.size(); ++iMode) {
        if (!this->modeEnabled[iMode]) continue;
        const refTypeStringList &listStrings = modeRefTypes[iMode];
        for (std::size_t iStr = 0; iStr < listStrings.size(); ++iStr) {
            int score =
                1; //-1 = topo incompatible, 0 = topo compatible, geom incompatible; 1+ = compatible (the higher - the more specific is the mode for the support)
            ...
            if (str.size() != typeStr.size()) score = -1;
            ...
            if (score > 0) {
                if (mlist.empty()) mlist.push_back(eMapMode(iMode));
...
eRefType AttachEngine::getShapeType(const TopoDS_Shape &sh)
{
    ...
    switch (sh.ShapeType()) {
        ...
        case TopAbs_SHELL: return rtAnything; break;
        case TopAbs_FACE: {
            ...
            switch (surf.GetType()) {
                ...
                case GeomAbs_BezierSurface: break;
                case GeomAbs_BSplineSurface: break;
                case GeomAbs_SurfaceOfRevolution: return rtSurfaceRev;
                case GeomAbs_SurfaceOfExtrusion: break;
                case GeomAbs_OffsetSurface: break;
        ...
        case TopAbs_EDGE: {
            ...
            switch (crv.GetType()) {
                case GeomAbs_Line: return rtLine;
                case GeomAbs_Circle: return rtCircle;
                case GeomAbs_Ellipse: return rtEllipse;
                case GeomAbs_Hyperbola: return rtHyperbola;
                case GeomAbs_Parabola: return rtParabola;
                case GeomAbs_BezierCurve:
                case GeomAbs_BSplineCurve:
                case GeomAbs_OtherCurve:
                case GeomAbs_OffsetCurve: return rtCurve;
            }
        } break;
        case TopAbs_WIRE: return rtWire;
        case TopAbs_VERTEX: return rtVertex;
        default:
            throw AttachEngineException(
...
eRefType AttachEngine::downgradeType(eRefType type)
{
    ...
    switch (type) {
        case rtVertex:
        case rtEdge:
        case rtFace: return rtAnything;
        case rtAnything: return rtAnything;
        case rtLine:
        case rtCurve: return rtEdge;
        case rtConic:
        case rtCircle: return rtCurve;
        case rtEllipse:
        case rtParabola:
        case rtHyperbola: return rtConic;
        case rtFlatFace:
        case rtSphericalFace:
        case rtSurfaceRev: return rtFace;
        case rtCylindricalFace:
        case rtToroidalFace:
        case rtConicalFace: return rtSurfaceRev;
        case rtSolid:
        case rtWire: return rtPart;
        case rtPart: return rtAnything;
        default: throw AttachEngineException("AttachEngine::downgradeType: unknown type");
    }
}
...
int AttachEngine::isShapeOfType(eRefType shapeType, eRefType requirement)
{
    ...
    if (requirement & rtFlagHasPlacement) {
        if (!(shapeType & rtFlagHasPlacement)) return -1;
    }
    ...
    if (requirement == rtAnything) return 1;
    ...
    while (shDeg != rtAnything) {
        if (shDeg == requirement) return reqRank;
...
eMapMode AttachEngine::getModeByName(const std::string &modeName)
{
    for (int mmode = 0; mmode < mmDummy_NumberOfModes; mmode++) {
        if (strcmp(eMapModeStrings[mmode], modeName.c_str()) == 0) { return eMapMode(mmode); }
    }
...
std::string AttachEngine::getRefTypeName(eRefType shapeType)
{
    ...
    if (shapeType & rtFlagHasPlacement) { result.append("|Placement"); }
    return result;
}
...
eRefType AttachEngine::getRefTypeByName(const std::string &typeName)
{
    ...
    if (seppos != std::string::npos) { flags = typeName.substr(seppos + 1); }
                             ...
                             std::vector<TopoDS_Shape> &storage, std::vector<eRefType> &types)
{
    ...
    for (std::size_t i = 0; i < objs.size(); i++) {
        ...
        if (sub[i].length() == 0) types[i] = eRefType(types[i] | rtFlagHasPlacement);
    }
}
...
Base::Placement
AttachEngine3D::calculateAttachedPlacement(const Base::Placement &origPlacement) const
{
    ...
    if (parts.empty()) throw ExceptionCancel();
    ...
    switch (mmode) {
        ...
        case mmObjectYZ: {
            ...
            if (types[0] & rtFlagHasPlacement) {
                ...
                switch (adapt.GetType()) {
                    ...
                    default:
                        assert(
                            0); //conics should have been filtered out by testing shape type in the above if.
                }
                dirX = pos.XDirection();
                dirY = pos.YDirection();
        ...
        case mmInertialCS: {
            ...
            if (pr.HasSymmetryAxis()) {
                ...
                d23 = fabs(I2 - I3);
                d31 = fabs(I3 - I1);
                if (d12 < d23 && d12 < d31) { SketchNormal = pr.ThirdAxisOfInertia(); }
        ...
        case mmFlatFace: {
            ...
            if (adapt.GetType() == GeomAbs_Plane) { plane = adapt.Plane(); }
            else {
                TopLoc_Location loc;
                Handle(Geom_Surface) surf = BRep_Tool::Surface(face, loc);
                GeomLib_IsPlanarSurface check(surf);
                if (check.IsPlanar()) plane = check.Plan();
                else
                    throw Base::ValueError(
            ...
            bool Reverse = false;
            if (face.Orientation() == TopAbs_REVERSED) Reverse = true;
        ...
        case mmTangentPlane: {
            ...
            if (prop.IsTangentUDefined()) {
                prop.TangentU(dirX);
                if (face.Orientation() == TopAbs_REVERSED) dirX.Reverse();
            }
            ...
            if (bThruVertex) { SketchBasePoint = p; }
            else {
                SketchBasePoint = projector.NearestPoint();
            }
        } break;
        ...
        case mmConcentric: { //all alignments to point on curve
            ...
            if (bThruVertex) { SketchBasePoint = p_in; }
            else {
                SketchBasePoint = p;
            }
            ...
            if (mmode == mmRevolutionSection || mmode == mmConcentric || mmode == mmFrenetNB
                ...
                switch (mmode) {
                    ...
                    case mmRevolutionSection:
                        SketchNormal =
                            T.Reversed(); //to avoid sketches upside-down for regular curves like circles
                    ...
                    case mmFrenetTB:
                        ...
                        SketchNormal =
                            N.Reversed(); //it is more convenient to sketch on something looking at it so it is convex.
                        SketchXAxis = T;
                        break;
                    default: assert(0); //mode forgotten?
                }
                ...
                SketchNormal = gp_Dir(
                    d.Reversed()); //sketch normal looks at user. It is natural to have the curve directed away from user, so reversed.
            }
        ...
        case mmFolding: {
            ...
            double signs[4] = {
                0, 0, 0,
                0}; //flags whether to reverse line directions, for all directions to point away from the common vertex
            ...
            for (int i = 2; i < 4; i++) {
                ...
                if (p.Distance(p1) < Precision::Confusion()) signs[i] = +1.0;
        ...
        case mmOYX: {
            ...
            int order[3];
            for (int i = 0; i < 3; ++i) { order[i] = orderString[i] - 'X'; }
            ...
            Base::Rotation rot = Base::Rotation::makeRotationByAxes(
                ...
                Base::Vector3d(dirs[2].X(), dirs[2].Y(), dirs[2].Z()), orderString);
            if (this->mapReverse) { rot = rot * Base::Rotation(Base::Vector3d(0, 1, 0), D_PI); }

            Base::Placement plm = Base::Placement(
                Base::Vector3d(SketchBasePoint.X(), SketchBasePoint.Y(), SketchBasePoint.Z()), rot);
            plm *= this->attachmentOffset;
            return plm;
        } break;
        default: throwWrongMode(mmode);
    } //switch (MapMode)
    ...
    Base::Placement plm =
        this->placementFactory(SketchNormal, SketchXAxis, SketchBasePoint, gp_Pnt(),
                               ...
                               /*makeYVertical = */ false,
                               /*makeLegacyFlatFaceOrientation = */ mmode == mmFlatFace, &Place);
    plm *= this->attachmentOffset;
    return plm;
}
...
Base::Placement
AttachEnginePlane::calculateAttachedPlacement(const Base::Placement &origPlacement) const
{
    //re-use Attacher3d
    Base::Placement plm;
    AttachEngine3D attacher3D;
...
Base::Placement
AttachEngineLine::calculateAttachedPlacement(const Base::Placement &origPlacement) const
{
    ...
    switch (mmode) {
        case mmDeactivated:
            throw ExceptionCancel(); //to be handled in positionBySupport, to not do anything if disabled
        case mm1AxisX: mmode = mmObjectYZ; break;
        case mm1AxisY: mmode = mmObjectXZ; break;
        case mm1AxisZ: mmode = mmObjectXY; break;
        case mm1AxisCurv:
            ...
            break;
        case mm1Binormal: mmode = mmFrenetTN; break;
        case mm1Normal: mmode = mmFrenetTB; break;
        case mm1Tangent: mmode = mmNormalToPath; break;
        case mm1FaceNormal: mmode = mmTangentPlane; break;
        default: bReUsed = false; break;
    }
    ...
    if (!bReUsed) {
        ...
        switch (mmode) {
            ...
            case mm1Asymptote2: {
                ...
                if (mmode == mm1Asymptote1) LineDir = hyp.Asymptote1().Direction();
                else
        ...
        attacher3D.attachmentOffset = Base::
            Placement(); //AttachmentOffset is applied separately here, afterwards. So we are resetting it in sub-attacher to avoid applying it twice!
        plm = attacher3D.calculateAttachedPlacement(origPlacement);
...
Base::Placement
AttachEnginePoint::calculateAttachedPlacement(const Base::Placement &origPlacement) const
{
    eMapMode mmode = this->mapMode;
    ...
    switch (mmode) {
        case mmDeactivated:
            throw ExceptionCancel(); //to be handled in positionBySupport, to not do anything if disabled
        case mm0Origin: mmode = mmObjectXY; break;
        case mm0CenterOfCurvature: mmode = mmRevolutionSection; break;
        case mm0OnEdge:
            //todo: prevent thruPoint
            mmode = mmNormalToPath;
            break;
        default: bReUsed = false;
    }
    ...
    if (!bReUsed) {
        ...
        switch (mmode) {
            case mm0Vertex: {
                ...
                if (sh.ShapeType() == TopAbs_VERTEX) {
                    ...
                    if (Precision::IsInfinite(u)) throw Base::ValueError("Edge is infinite");
            ...
            case mm0Focus2: {
                ...
                if (mmode == mm0Focus1) BasePoint = f1;
                else
                    BasePoint = f2;
            } break;
        ...
        plm = this->placementFactory(gp_Vec(0.0, 0.0, 1.0), gp_Vec(1.0, 0.0, 0.0), BasePoint,
                                     gp_Pnt());
    }
    else { //re-use 3d mode
        AttachEngine3D attacher3D;
...
gp_Pnt AttachEnginePoint::getProximityPoint(eMapMode mmode, const TopoDS_Shape &s1,
                                            const TopoDS_Shape &s2) const
{
    ...
    gp_Pnt p1 = distancer.PointOnShape1(1);
    gp_Pnt p2 = distancer.PointOnShape2(1);
    if (mmode == mm0ProximityPoint1) return p1;
    else
        return p2;
}

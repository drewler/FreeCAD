...
BodyBase *BodyBase::findBodyOf(const App::DocumentObject *f)
{
    ...
    if (doc) {
        ...
        for (std::vector<App::DocumentObject *>::const_iterator b = bodies.begin();
            ...
            if (body->hasObject(f)) return body;
        }
    }
...
bool BodyBase::isAfter(const App::DocumentObject *feature, const App::DocumentObject *target) const
{
    ...
    if (!target || target == BaseFeature.getValue()) { return hasObject(feature); }
    ...
    if (featureIt == features.end()) { return false; }
    else {
...
void BodyBase::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                         const char *PropName)
{
    // The App::PropertyLinkList property was Model in the past (#0002642)
    Base::Type type = Base::Type::fromName(TypeName);
    if (Group.getClassTypeId() == type && strcmp(PropName, "Model") == 0) { Group.Restore(reader); }
    else {

...
TopoDS_Face FaceMakerCheese::validateFace(const TopoDS_Face &face)
{
    ...
    if (!aChecker.IsValid()) {
        ...
        if (!aChecker.IsValid()) {
            ...
            if (!aChecker.IsValid()) Standard_Failure::Raise("Failed to validate broken face");
            return fixedFace;
        }
        return mkFace.Face();
    }
...
bool FaceMakerCheese::isInside(const TopoDS_Wire &wire1, const TopoDS_Wire &wire2)
{
    ...
    if (box1.IsOut(box2)) return false;

    double prec = Precision::Confusion();

    BRepBuilderAPI_MakeFace mkFace(wire1);
    if (!mkFace.IsDone()) Standard_Failure::Raise("Failed to create a face from wire in sketch");
    ...
    while (xp.More()) {
        ...
        if (class2d.Perform(uv) == TopAbs_IN) return true;
...
TopoDS_Shape FaceMakerCheese::makeFace(std::list<TopoDS_Wire> &wires)
{
    ...
    gp_Dir axis(0, 0, 1);
    BRepAdaptor_Surface adapt(face);
    if (adapt.GetType() == GeomAbs_Plane) { axis = adapt.Plane().Axis().Direction(); }
...
{
    if (w.empty()) return TopoDS_Shape();
    ...
    if (sep_wire_list.size() == 1) {
        ...
        for (std::list<std::list<TopoDS_Wire>>::iterator it = sep_wire_list.begin();
             it != sep_wire_list.end(); ++it) {
            TopoDS_Shape aFace = makeFace(*it);
            if (!aFace.IsNull()) builder.Add(comp, aFace);
        }
...
std::string FaceMakerCheese::getBriefExplanation() const
{
    return std::string(QT_TRANSLATE_NOOP(
        "Part_FaceMaker", "Supports making planar faces with holes, but no islands inside holes."));
}
...
void FaceMakerCheese::Build_Essence()
{
    ...
    for (int i = 0; i < seq->Length(); i++) { this->myShapesToReturn.push_back(seq->Value(i + 1)); }
}

...
{
    switch (et) {
        case gce_Done: return "Construction was successful";
        case gce_ConfusedPoints: return "Two points are coincident";
        case gce_NegativeRadius: return "Radius value is negative";
        case gce_ColinearPoints: return "Three points are collinear";
        case gce_IntersectionError: return "Intersection cannot be computed";
        case gce_NullAxis: return "Axis is undefined";
        case gce_NullAngle: return "Angle value is invalid (usually null)";
        case gce_NullRadius: return "Radius is null";
        case gce_InvertAxis: return "Axis value is invalid";
        case gce_BadAngle: return "Angle value is invalid";
        case gce_InvertRadius:
            return "Radius value is incorrect (usually with respect to another radius)";
        case gce_NullFocusLength: return "Focal distance is null";
        case gce_NullVector: return "Vector is null";
        case gce_BadEquation:
            return "Coefficients are incorrect (applies to the equation of a geometric object)";
        default: return "Creation of geometry failed";
    }
}
...
Geometry::Geometry() { createNewTag(); }

Geometry::~Geometry() {}

// Persistence implementer
unsigned int Geometry::getMemSize() const { return 1; }
...
void Geometry::Save(Base::Writer &writer) const
{
    ...
    for (const auto &att : extensions) {
        if (att->isDerivedFrom(Part::GeometryPersistenceExtension::getClassTypeId())) counter++;
    }
...
void Geometry::Restore(Base::XMLReader &reader)
{
    ...
    if (strcmp(reader.localName(), "GeoExtensions") == 0) { // new format
        ...
        for (int i = 0; i < count; i++) {
            ...
            GeometryPersistenceExtension *newE =
                (GeometryPersistenceExtension *)type.createInstance();
            newE->Restore(reader);
...
boost::uuids::uuid Geometry::getTag() const { return tag; }
...
std::vector<std::weak_ptr<const GeometryExtension>> Geometry::getExtensions() const
{
    ...
    for (auto &ext : extensions) wp.push_back(ext);
...
bool Geometry::hasExtension(Base::Type type) const
{
    for (const auto &ext : extensions) {
        if (ext->getTypeId() == type) return true;
    }
...
{
    for (const auto &ext : extensions) {
        if (ext->getName() == name) return true;
    }
...
{
    for (const auto &ext : extensions) {
        if (ext->getTypeId() == type) return ext;
    }
...
{
    for (const auto &ext : extensions) {
        if (ext->getName() == name) return ext;
    }
...
{
    if (geo->getTypeId() == this->getTypeId()) this->tag = geo->tag;
    else
...
{
    gp_Trsf trf;
    trf.SetValues(mat[0][0], mat[0][1], mat[0][2], mat[0][3], mat[1][0], mat[1][1], mat[1][2],
                  mat[1][3], mat[2][0], mat[2][1], mat[2][2], mat[2][3]);
...
GeomPoint::GeomPoint() { this->myPoint = new Geom_CartesianPoint(0, 0, 0); }

GeomPoint::GeomPoint(const Handle(Geom_CartesianPoint) & p) { setHandle(p); }
...
const Handle(Geom_Geometry) & GeomPoint::handle() const { return myPoint; }
...
TopoDS_Shape GeomPoint::toShape() const { return BRepBuilderAPI_MakeVertex(myPoint->Pnt()); }
...
void GeomPoint::setPoint(const Base::Vector3d &p) { this->myPoint->SetCoord(p.x, p.y, p.z); }

// Persistence implementer
unsigned int GeomPoint::getMemSize() const { return sizeof(Geom_CartesianPoint); }
...
PyObject *GeomPoint::getPyObject() { return new PointPy(new GeomPoint(getPoint())); }
...
GeomBSplineCurve *GeomCurve::toBSpline(double first, double last) const
{
    ...
    if (spline.IsNull()) THROWM(Base::CADKernelError, "Conversion to B-spline failed")
                          ...
                          double tol) const
{
    ...
    if (!curve1.IsNull() && !curve2.IsNull()) { return intersect(curve1, curve2, points, tol); }
    else
        return false;
}
                          ...
                          double tol)
{
    ...
    try {
        ...
        for (int i = 1; i <= intersector.NbExtrema(); i++) {
            if (intersector.Distance(i) > tol) continue;
...
GeomBezierCurve::GeomBezierCurve(const Handle(Geom_BezierCurve) & b) { setHandle(b); }

GeomBezierCurve::GeomBezierCurve(const std::vector<Base::Vector3d> &poles,
                                 const std::vector<double> &weights)
{
    if (poles.size() != weights.size()) throw Base::ValueError("poles and weights mismatch");
...
const Handle(Geom_Geometry) & GeomBezierCurve::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomBezierCurve::getMemSize() const { return sizeof(Geom_BezierCurve); }
...
void GeomBezierCurve::Restore(Base::XMLReader &reader)
{
    ...
    try {
        ...
        if (!bezier.IsNull()) this->myCurve = bezier;
        else
...
GeomBSplineCurve::GeomBSplineCurve(const Handle(Geom_BSplineCurve) & b) { setHandle(b); }
...
GeomBSplineCurve::GeomBSplineCurve(const std::vector<Base::Vector3d> &poles,
                                   ...
                                   bool periodic, bool checkrational)
{
    if (poles.size() != weights.size()) throw Base::ValueError("poles and weights mismatch");
    ...
    this->myCurve =
        new Geom_BSplineCurve(p, w, k, m, degree, periodic ? Standard_True : Standard_False,
...
const Handle(Geom_Geometry) & GeomBSplineCurve::handle() const { return myCurve; }
...
int GeomBSplineCurve::countPoles() const { return myCurve->NbPoles(); }

int GeomBSplineCurve::countKnots() const { return myCurve->NbKnots(); }

void GeomBSplineCurve::setPole(int index, const Base::Vector3d &pole, double weight)
{
    try {
        gp_Pnt pnt(pole.x, pole.y, pole.z);
        if (weight < 0.0) myCurve->SetPole(index, pnt);
        else
...
void GeomBSplineCurve::setPoles(const std::vector<Base::Vector3d> &poles)
{
    ...
    for (std::vector<Base::Vector3d>::const_iterator it = poles.begin(); it != poles.end();
         ++it, index++) {
        setPole(index, *it);
    }
}
...
void GeomBSplineCurve::setWeights(const std::vector<double> &weights)
{
    ...
    try {
        ...
        for (std::vector<double>::const_iterator it = weights.begin(); it != weights.end();
             ++it, index++) {
...
{
    try {
        if (mult < 0) myCurve->SetKnot(index, val);
        else
...
int GeomBSplineCurve::getDegree() const { return myCurve->Degree(); }

bool GeomBSplineCurve::isPeriodic() const { return myCurve->IsPeriodic() == Standard_True; }

bool GeomBSplineCurve::isRational() const { return myCurve->IsRational() == Standard_True; }

bool GeomBSplineCurve::join(const Handle(Geom_BSplineCurve) & spline)
{
    GeomConvert_CompCurveToBSplineCurve ccbc(this->myCurve);
    if (!ccbc.Add(spline, Precision::Approximation())) return false;
...
{
    if (p.size() < 2) Standard_ConstructionError::Raise();

    double tol3d = Precision::Approximation();
    Handle(TColgp_HArray1OfPnt) pts = new TColgp_HArray1OfPnt(1, p.size());
    for (std::size_t i = 0; i < p.size(); i++) { pts->SetValue(i + 1, p[i]); }
...
{
    if (p.size() < 2) Standard_ConstructionError::Raise();
    if (p.size() != t.size()) Standard_ConstructionError::Raise();
                                                 ...
                                                 std::vector<gp_Vec> &t) const
{
    ...
    if (p.size() < 2) Standard_ConstructionError::Raise();
    if (p.size() != c.size()) Standard_ConstructionError::Raise();
...
bool GeomBSplineCurve::approximate(double tol3d, int maxSegments, int maxDegree, int continuity)
{
    try {
        ...
        if (continuity >= 0 && continuity <= 6) cont = static_cast<GeomAbs_Shape>(continuity);
    ...
    try {
        ...
        if (curve->RemoveKnot(index, multiplicity, tolerance)) {
            ...
            for (Standard_Integer i = weights.Lower(); i <= weights.Upper(); i++) {
                double v = weights(i);
                if (v <= gp::Resolution()) return false;
            }
...
void GeomBSplineCurve::Trim(double u, double v)
{
    ...
    try {
        if (!isPeriodic()) { splitUnwrappedBSpline(u, v); }
        else {               // periodic
...
// Persistence implementer
unsigned int GeomBSplineCurve::getMemSize() const { return sizeof(Geom_BSplineCurve); }
...
void GeomBSplineCurve::Restore(Base::XMLReader &reader)
{
    ...
    try {
        Handle(Geom_BSplineCurve) spline = new Geom_BSplineCurve(
            p, w, k, m, degree, isperiodic ? Standard_True : Standard_False, Standard_False);

        if (!spline.IsNull()) this->myCurve = spline;
        else
...
GeomTrimmedCurve::GeomTrimmedCurve(const Handle(Geom_TrimmedCurve) & c) { setHandle(c); }
...
const Handle(Geom_Geometry) & GeomTrimmedCurve::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomTrimmedCurve::getMemSize() const { return sizeof(Geom_TrimmedCurve); }
...
bool GeomTrimmedCurve::intersectBasisCurves(
    const GeomTrimmedCurve *c, std::vector<std::pair<Base::Vector3d, Base::Vector3d>> &points,
    double tol) const
{
    ...
    if (!bcurve1.IsNull() && !bcurve2.IsNull()) { return intersect(bcurve1, bcurve2, points, tol); }
    else
        return false;
}
...
Base::Vector3d GeomArcOfConic::getStartPoint(bool emulateCCWXY) const
{
    ...
    gp_Pnt pnt = curve->StartPoint();
    if (emulateCCWXY) {
        if (isReversed()) pnt = curve->EndPoint();
    }
    return Base::Vector3d(pnt.X(), pnt.Y(), pnt.Z());
}
...
Base::Vector3d GeomArcOfConic::getEndPoint(bool emulateCCWXY) const
{
    ...
    gp_Pnt pnt = curve->EndPoint();
    if (emulateCCWXY) {
        if (isReversed()) pnt = curve->StartPoint();
    }
    return Base::Vector3d(pnt.X(), pnt.Y(), pnt.Z());
}
...
GeomCircle::GeomCircle(const Handle(Geom_Circle) & c) { setHandle(c); }

GeomCircle::~GeomCircle() {}

const Handle(Geom_Geometry) & GeomCircle::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomCircle::getMemSize() const { return sizeof(Geom_Circle); }
...
void GeomCircle::Restore(Base::XMLReader &reader)
{
    ...
    if (reader.hasAttribute("AngleXU")) AngleXU = reader.getAttributeAsFloat("AngleXU");
    ...
    try {
        ...
        if (!mc.IsDone()) THROWM(Base::CADKernelError, gce_ErrorStatusText(mc.Status()))
...
GeomArcOfCircle::GeomArcOfCircle(const Handle(Geom_Circle) & c) { setHandle(c); }
...
void GeomArcOfCircle::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
    ...
    if (basis.IsNull()) Standard_Failure::Raise("Basis curve is not a circle");
    this->myCurve = Handle(Geom_TrimmedCurve)::DownCast(c->Copy());
}
...
const Handle(Geom_Geometry) & GeomArcOfCircle::handle() const { return myCurve; }
...
void GeomArcOfCircle::getRange(double &u, double &v, bool emulateCCWXY) const
{
    ...
    if (emulateCCWXY) {
        ...
        double
            u1 = u,
            v1 =
                v; //the true arc curve parameters, cached. u,v will contain the rotation-corrected and swapped angles.
    ...
    try {
        ...
        if (emulateCCWXY) {
            ...
            double
                u1 = u,
                v1 =
                    v; //the values that were passed, ccw angles from X axis. u,v will contain the rotation-corrected and swapped angles.
            if (conic->Axis().Direction().Z() > 0.0) {
                //normal CCW arc
                u = u1 - angleXU;
                v = v1 - angleXU;
...
void GeomArcOfCircle::Restore(Base::XMLReader &reader)
{
    ...
    try {
        ...
        if (!ma.IsDone()) THROWM(Base::CADKernelError, gce_ErrorStatusText(ma.Status()))
...
GeomEllipse::GeomEllipse(const Handle(Geom_Ellipse) & e) { setHandle(e); }

GeomEllipse::~GeomEllipse() {}

const Handle(Geom_Geometry) & GeomEllipse::handle() const { return myCurve; }
...
GeomBSplineCurve *GeomEllipse::toNurbs(double first, double last) const
{
    ...
    if (first != 0 || last != 2 * M_PI) { return GeomCurve::toNurbs(first, last); }
...
void GeomEllipse::setMajorAxisDir(Base::Vector3d newdir)
{
    ...
    try {
        ...
        pos.SetXDirection(gp_Dir(
            newdir.x, newdir.y,
            newdir
                .z)); //OCC should keep the old main Direction (Z), and change YDirection to accommodate the new XDirection.
...
// Persistence implementer
unsigned int GeomEllipse::getMemSize() const { return sizeof(Geom_Ellipse); }
...
GeomArcOfEllipse::GeomArcOfEllipse(const Handle(Geom_Ellipse) & e) { setHandle(e); }
...
void GeomArcOfEllipse::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
    ...
    if (basis.IsNull()) Standard_Failure::Raise("Basis curve is not an ellipse");
    this->myCurve = Handle(Geom_TrimmedCurve)::DownCast(c->Copy());
}
...
const Handle(Geom_Geometry) & GeomArcOfEllipse::handle() const { return myCurve; }
...
GeomHyperbola::GeomHyperbola(const Handle(Geom_Hyperbola) & h) { setHandle(h); }

GeomHyperbola::~GeomHyperbola() {}

const Handle(Geom_Geometry) & GeomHyperbola::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomHyperbola::getMemSize() const { return sizeof(Geom_Hyperbola); }
...
GeomArcOfHyperbola::GeomArcOfHyperbola(const Handle(Geom_Hyperbola) & h) { setHandle(h); }
...
void GeomArcOfHyperbola::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
    ...
    if (basis.IsNull()) Standard_Failure::Raise("Basis curve is not an hyperbola");
    this->myCurve = Handle(Geom_TrimmedCurve)::DownCast(c->Copy());
}
...
const Handle(Geom_Geometry) & GeomArcOfHyperbola::handle() const { return myCurve; }
...
GeomParabola::GeomParabola(const Handle(Geom_Parabola) & p) { setHandle(p); }

GeomParabola::~GeomParabola() {}

const Handle(Geom_Geometry) & GeomParabola::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomParabola::getMemSize() const { return sizeof(Geom_Parabola); }
...
GeomArcOfParabola::GeomArcOfParabola(const Handle(Geom_Parabola) & h) { setHandle(h); }
...
void GeomArcOfParabola::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
    ...
    if (basis.IsNull()) Standard_Failure::Raise("Basis curve is not a parabola");
    this->myCurve = Handle(Geom_TrimmedCurve)::DownCast(c->Copy());
}
...
const Handle(Geom_Geometry) & GeomArcOfParabola::handle() const { return myCurve; }
...
GeomLine::GeomLine(const Handle(Geom_Line) & l) { setHandle(l); }
...
const Handle(Geom_Geometry) & GeomLine::handle() const { return myCurve; }
...
// Persistence implementer
unsigned int GeomLine::getMemSize() const { return sizeof(Geom_Line); }
...
PyObject *GeomLine::getPyObject() { return new LinePy(static_cast<GeomLine *>(this->clone())); }
...
GeomLineSegment::GeomLineSegment(const Handle(Geom_Line) & l) { setHandle(l); }
...
void GeomLineSegment::setHandle(const Handle(Geom_TrimmedCurve) & c)
{
    ...
    if (basis.IsNull()) Standard_Failure::Raise("Basis curve is not a line");
    this->myCurve = Handle(Geom_TrimmedCurve)::DownCast(c->Copy());
}
...
const Handle(Geom_Geometry) & GeomLineSegment::handle() const { return myCurve; }
...
void GeomLineSegment::setPoints(const Base::Vector3d &Start, const Base::Vector3d &End)
{
    ...
    try {
        ...
        if (p1.Distance(p2) < gp::Resolution()) THROWM(Base::ValueError, "Both points are equal");

        GC_MakeSegment ms(p1, p2);
        if (!ms.IsDone()) { THROWM(Base::CADKernelError, gce_ErrorStatusText(ms.Status())) }
...
void GeomLineSegment::Restore(Base::XMLReader &reader)
{
    ...
    try {
        ...
        if (start.x == 0) { end = start + Base::Vector3d(DBL_EPSILON, 0, 0); }
        else {
...
GeomOffsetCurve::GeomOffsetCurve(const Handle(Geom_OffsetCurve) & c) { setHandle(c); }
...
const Handle(Geom_Geometry) & GeomOffsetCurve::handle() const { return this->myCurve; }

// Persistence implementer
unsigned int GeomOffsetCurve::getMemSize() const { return sizeof(Geom_OffsetCurve); }
...
bool GeomSurface::isUmbillic(double u, double v) const
{
    Handle(Geom_Surface) s = Handle(Geom_Surface)::DownCast(handle());
    GeomLProp_SLProps prop(s, u, v, 2, Precision::Confusion());
    if (prop.IsCurvatureDefined()) { return prop.IsUmbilic(); }
...
double GeomSurface::curvature(double u, double v, Curvature type) const
{
    ...
    if (prop.IsCurvatureDefined()) {
        double value = 0;
        switch (type) {
            case Maximum: value = prop.MaxCurvature(); break;
            case Minimum: value = prop.MinCurvature(); break;
            case Mean: value = prop.MeanCurvature(); break;
            case Gaussian: value = prop.GaussianCurvature(); break;
        }
...
GeomBezierSurface::GeomBezierSurface(const Handle(Geom_BezierSurface) & b) { setHandle(b); }

GeomBezierSurface::~GeomBezierSurface() {}

const Handle(Geom_Geometry) & GeomBezierSurface::handle() const { return mySurface; }
...
GeomBSplineSurface::GeomBSplineSurface(const Handle(Geom_BSplineSurface) & b) { setHandle(b); }
...
const Handle(Geom_Geometry) & GeomBSplineSurface::handle() const { return mySurface; }
...
GeomCylinder::GeomCylinder(const Handle(Geom_CylindricalSurface) & c) { setHandle(c); }
...
const Handle(Geom_Geometry) & GeomCylinder::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomCylinder::getMemSize() const { return sizeof(Geom_CylindricalSurface); }
...
GeomCone::GeomCone(const Handle(Geom_ConicalSurface) & c) { setHandle(c); }
...
const Handle(Geom_Geometry) & GeomCone::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomCone::getMemSize() const { return sizeof(Geom_ConicalSurface); }
...
PyObject *GeomCone::getPyObject() { return new ConePy(static_cast<GeomCone *>(this->clone())); }
...
gp_Vec GeomCone::getDN(double u, double v, int Nu, int Nv) const
{
    ...
    auto ElSLib__ConeDN = [](const Standard_Real U, const Standard_Real V, const gp_Ax3 &Pos,
        ...
        if (Nv == 0) {
            ...
            if (Nu == 0) Xdir.Add(Pos.Direction().XYZ() * cos(SAngle));
            return gp_Vec(Xdir);
        }
    ...
    if (Nv > 1) { return gp_Vec(0.0, 0.0, 0.0); }
    else {
...
GeomToroid::GeomToroid(const Handle(Geom_ToroidalSurface) & t) { setHandle(t); }
...
const Handle(Geom_Geometry) & GeomToroid::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomToroid::getMemSize() const { return sizeof(Geom_ToroidalSurface); }
...
GeomSphere::GeomSphere(const Handle(Geom_SphericalSurface) & s) { setHandle(s); }
...
const Handle(Geom_Geometry) & GeomSphere::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomSphere::getMemSize() const { return sizeof(Geom_SphericalSurface); }
...
GeomPlane::GeomPlane(const Handle(Geom_Plane) & p) { setHandle(p); }
...
const Handle(Geom_Geometry) & GeomPlane::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomPlane::getMemSize() const { return sizeof(Geom_Plane); }
...
PyObject *GeomPlane::getPyObject() { return new PlanePy(static_cast<GeomPlane *>(this->clone())); }
...
GeomOffsetSurface::GeomOffsetSurface(const Handle(Geom_OffsetSurface) & s) { setHandle(s); }
...
const Handle(Geom_Geometry) & GeomOffsetSurface::handle() const { return mySurface; }
...
// Persistence implementer
unsigned int GeomOffsetSurface::getMemSize() const { return sizeof(Geom_OffsetSurface); }
...
GeomPlateSurface::GeomPlateSurface(const Handle(GeomPlate_Surface) & s) { setHandle(s); }
...
const Handle(Geom_Geometry) & GeomPlateSurface::handle() const { return mySurface; }
...
const Handle(Geom_Geometry) & GeomTrimmedSurface::handle() const { return mySurface; }
...
const Handle(Geom_Geometry) & GeomSurfaceOfRevolution::handle() const { return mySurface; }
...
const Handle(Geom_Geometry) & GeomSurfaceOfExtrusion::handle() const { return mySurface; }
                             ...
                             Base::Vector3d &point)
{
    ...
    if ((det > 0 ? det : -det) < 1e-10) return false;
...
bool findFilletCenter(const GeomLineSegment *lineSeg1, const GeomLineSegment *lineSeg2,
                      double radius, Base::Vector3d &center)
{
...
bool findFilletCenter(const GeomLineSegment *lineSeg1, const GeomLineSegment *lineSeg2,
                      double radius, const Base::Vector3d &refPnt1, const Base::Vector3d &refPnt2,
                      Base::Vector3d &center)
{
    ...
    if (!find2DLinesIntersection(lineSeg1, lineSeg2, corner)) return false;
    ...
    // found center point
    if (find2DLinesIntersection(tmpPoint1, dir1, tmpPoint2, dir2, center)) return true;
    else
        return false;
}
...
{
    Base::Vector3d corner;
    if (!Part::find2DLinesIntersection(lineSeg1, lineSeg2, corner)) return -1;

    Base::Vector3d dir1 = lineSeg1->getEndPoint() - lineSeg1->getStartPoint();
    Base::Vector3d dir2 = lineSeg2->getEndPoint() - lineSeg2->getStartPoint();

    // Decide the line directions depending on the reference points
    if (dir1 * (refPnt1 - corner) < 0) dir1 *= -1;
    if (dir2 * (refPnt2 - corner) < 0) dir2 *= -1;
...
GeomArcOfCircle *createFilletGeometry(const GeomLineSegment *lineSeg1,
                                      const GeomLineSegment *lineSeg2, const Base::Vector3d &center,
                                      double radius)
{
    Base::Vector3d corner;
...
std::unique_ptr<GeomCurve> makeFromCurveAdaptor(const Adaptor3d_Curve &adapt)
{
    ...
    if (!geoCurve) throw Base::TypeError("Unhandled curve type");

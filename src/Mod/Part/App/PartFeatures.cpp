...
RuledSurface::RuledSurface()
{
    ...
    ADD_PROPERTY_TYPE(Orientation, ((long)0), "Ruled Surface", App::Prop_None,
...
{
    if (Curve1.isTouched()) return 1;
    if (Curve2.isTouched()) return 1;
    if (Orientation.isTouched()) return 1;
    return 0;
}

void RuledSurface::onChanged(const App::Property *prop) { Part::Feature::onChanged(prop); }
                                                      ...
                                                      TopoDS_Shape &shape) const
{
    ...
    if (part.isNull()) { return new App::DocumentObjectExecReturn("No shape linked."); }
...
App::DocumentObjectExecReturn *RuledSurface::execute()
{
    try {
        ...
        if (S1.ShapeType() == TopAbs_WIRE) isWire = Standard_True;

        if (isWire) {
            if (S2.ShapeType() == TopAbs_EDGE) S2 = BRepLib_MakeWire(TopoDS::Edge(S2));
        }
        else {
        ...
        if (!isWire) { ruledShape = BRepFill::Face(TopoDS::Edge(S1), TopoDS::Edge(S2)); }
        else {
...
App::PropertyIntegerConstraint::Constraints Loft::Degrees = {2, Geom_BSplineSurface::MaxDegree(),
                                                             1};
...
{
    if (Sections.isTouched()) return 1;
    if (Solid.isTouched()) return 1;
    if (Ruled.isTouched()) return 1;
    if (Closed.isTouched()) return 1;
    if (MaxDegree.isTouched()) return 1;
    return 0;
}

void Loft::onChanged(const App::Property *prop) { Part::Feature::onChanged(prop); }

App::DocumentObjectExecReturn *Loft::execute()
{
    if (Sections.getSize() == 0) return new App::DocumentObjectExecReturn("No sections linked.");
        ...
        for (it = shapes.begin(); it != shapes.end(); ++it) {
            ...
            if (shape.ShapeType() == TopAbs_COMPOUND) {
                ...
                for (; it.More(); it.Next(), numChilds++) {
                    if (!it.Value().IsNull()) {
                        child = it.Value();
                        if (child.ShapeType() == TopAbs_EDGE) { hEdges->Append(child); }
                    }
                }

                // a single child
                if (numChilds == 1) { shape = child; }
                // or all children are edges
                else if (hEdges->Length() == numChilds) {
                    ShapeAnalysis_FreeBounds::ConnectEdgesToWires(hEdges, Precision::Confusion(),
                                                                  Standard_False, hWires);
                    if (hWires->Length() == 1) shape = hWires->Value(1);
                }
            }
...
const char *Part::Sweep::TransitionEnums[] = {"Transformed", "Right corner", "Round corner",
                                              nullptr};
...
{
    if (Sections.isTouched()) return 1;
    if (Spine.isTouched()) return 1;
    if (Solid.isTouched()) return 1;
    if (Frenet.isTouched()) return 1;
    if (Transition.isTouched()) return 1;
    return 0;
}

void Sweep::onChanged(const App::Property *prop) { Part::Feature::onChanged(prop); }

App::DocumentObjectExecReturn *Sweep::execute()
{
    if (Sections.getSize() == 0) return new App::DocumentObjectExecReturn("No sections linked.");
    App::DocumentObject *spine = Spine.getValue();
    if (!spine) return new App::DocumentObjectExecReturn("No spine linked.");
    ...
    if (!shape.getShape().IsNull()) {
        try {
            if (!subedge.empty()) {
                ...
                ShapeAnalysis_FreeBounds::ConnectEdgesToWires(hEdges, Precision::Confusion(),
                                                              Standard_True, hWires);
                int len = hWires->Length();
                if (len != 1) return new App::DocumentObjectExecReturn("Spine is not connected.");
                path = hWires->Value(1);
            }
            else {
    ...
    try {
        ...
        mkPipeShell.Build();
        if (isSolid) mkPipeShell.MakeSolid();
...
{
    if (Faces.isTouched()) return 1;
    if (Value.isTouched()) return 1;
    if (Mode.isTouched()) return 1;
    if (Join.isTouched()) return 1;
    if (Intersection.isTouched()) return 1;
    if (SelfIntersection.isTouched()) return 1;
    return 0;
}

void Thickness::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                          App::Property *prop)
{
...
App::DocumentObjectExecReturn *Thickness::execute()
{
    ...
    if (!source) return new App::DocumentObjectExecReturn("No source shape linked.");
    const TopoShape &shape = Feature::getTopoShape(source);
    if (shape.isNull()) return new App::DocumentObjectExecReturn("Source shape is empty.");

    int countSolids = 0;
    TopExp_Explorer xp;
    xp.Init(shape.getShape(), TopAbs_SOLID);
    for (; xp.More(); xp.Next()) { countSolids++; }
    if (countSolids != 1) return new App::DocumentObjectExecReturn("Source shape is not a solid.");
...
Refine::Refine() { ADD_PROPERTY_TYPE(Source, (nullptr), "Refine", App::Prop_None, "Source shape"); }

App::DocumentObjectExecReturn *Refine::execute()
{
    Part::Feature *source = Source.getValue<Part::Feature *>();
    if (!source) return new App::DocumentObjectExecReturn("No part object linked.");
...
App::DocumentObjectExecReturn *Reverse::execute()
{
    ...
    if (topoShape.isNull()) return new App::DocumentObjectExecReturn("No part object linked.");

...
Revolution::Revolution()
{
    ...
    ADD_PROPERTY_TYPE(Base, (Base::Vector3d(0.0, 0.0, 0.0)), "Revolve", App::Prop_None,
                      "Base point of revolution axis");
    ADD_PROPERTY_TYPE(Axis, (Base::Vector3d(0.0, 0.0, 1.0)), "Revolve", App::Prop_None,
                      "Direction of revolution axis");
    ADD_PROPERTY_TYPE(AxisLink, (nullptr), "Revolve", App::Prop_None,
                      "Link to edge to use as revolution axis.");
    ADD_PROPERTY_TYPE(Angle, (360.0), "Revolve", App::Prop_None,
                      "Angle span of revolution. If angle is zero, and an arc is used for axis "
                      "link, angle span of arc will be used.");
    Angle.setConstraints(&angleRangeU);
    ADD_PROPERTY_TYPE(Symmetric, (false), "Revolve", App::Prop_None,
                      "Extend revolution symmetrically from the profile.");
    ADD_PROPERTY_TYPE(Solid, (false), "Revolve", App::Prop_None,
                      "Make revolution a solid if possible");
    ADD_PROPERTY_TYPE(
        FaceMakerClass, (""), "Revolve", App::Prop_None,
        "Facemaker to use if Solid is true."); //default for old documents. For default for new objects, refer to setupObject().
}
...
bool Revolution::fetchAxisLink(const App::PropertyLinkSub &axisLink, Base::Vector3d &center,
                               Base::Vector3d &dir, double &angle)
{
    ...
    if (axEdge.IsNull()) throw Base::ValueError("AxisLink shape is null");
    if (axEdge.ShapeType() != TopAbs_EDGE) throw Base::TypeError("AxisLink shape is not an edge");
...
App::DocumentObjectExecReturn *Revolution::execute()
{
    ...
    try {
        ...
        if (fabs(angle) < Precision::Angular()) angle = angle_edge;
        ...
        if (makeSolid && strlen(this->FaceMakerClass.getValue()) > 0) {
            ...
            makeSolid =
                Standard_False; //don't ask TopoShape::revolve to make solid, as we've made faces...
        }

        // actual revolution!
        TopoDS_Shape revolve = sourceShape.revolve(revAx, angle, makeSolid);

        if (revolve.IsNull()) return new App::DocumentObjectExecReturn("Resulting shape is null");

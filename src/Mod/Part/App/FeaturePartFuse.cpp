...
App::DocumentObjectExecReturn *MultiFuse::execute()
{
    ...
    if (s.size() >= 2) {
        try {
            ...
            if (shape.IsNull()) throw Base::RuntimeError("Input shape is null");
            ...
            mkFuse.Build();
            if (!mkFuse.IsDone()) throw Base::RuntimeError("MultiFusion failed");
            ...
            if (resShape.IsNull()) throw Base::RuntimeError("Resulting shape is null");

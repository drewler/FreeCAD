...
std::string BezierSurfacePy::representation() const { return "<BezierSurface object>"; }
...
// constructor method
int BezierSurfacePy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *BezierSurfacePy::insertPoleColAfter(PyObject *args)
{
    ...
    try {
        ...
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->InsertPoleColAfter(vindex, poles); }
        else {
...
PyObject *BezierSurfacePy::insertPoleRowAfter(PyObject *args)
{
    ...
    try {
        ...
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->InsertPoleRowAfter(uindex, poles); }
        else {
            Py::Sequence list(obj2);
            TColStd_Array1OfReal weights(1, list.size());
            int index = 1;
...
PyObject *BezierSurfacePy::insertPoleColBefore(PyObject *args)
{
    ...
    try {
        ...
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->InsertPoleColBefore(vindex, poles); }
        else {
            Py::Sequence list(obj2);
            TColStd_Array1OfReal weights(1, list.size());
            int index = 1;
...
PyObject *BezierSurfacePy::insertPoleRowBefore(PyObject *args)
{
    ...
    try {
        ...
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->InsertPoleRowBefore(uindex, poles); }
        else {
            Py::Sequence list(obj2);
            TColStd_Array1OfReal weights(1, list.size());
            int index = 1;
...
{
    int vindex;
    if (!PyArg_ParseTuple(args, "i", &vindex)) return nullptr;
    try {
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
...
{
    int uindex;
    if (!PyArg_ParseTuple(args, "i", &uindex)) return nullptr;
    try {
        Handle(Geom_BezierSurface) surf =
            Handle(Geom_BezierSurface)::DownCast(getGeometryPtr()->handle());
...
PyObject *BezierSurfacePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int BezierSurfacePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

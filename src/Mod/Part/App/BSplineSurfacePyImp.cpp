...
std::string BSplineSurfacePy::representation(void) const { return "<BSplineSurface object>"; }
...
// constructor method
int BSplineSurfacePy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
{
    int udegree, vdegree;
    if (!PyArg_ParseTuple(args, "ii", &udegree, &vdegree)) return nullptr;
...
PyObject *BSplineSurfacePy::insertUKnot(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "did|O!", &U, &M, &tol, &PyBool_Type, &add)) return nullptr;
...
PyObject *BSplineSurfacePy::insertVKnot(PyObject *args)
{
    ...
    int M = 1;
    PyObject *add = Py_True;
    if (!PyArg_ParseTuple(args, "did|O!", &V, &M, &tol, &PyBool_Type, &add)) return nullptr;
...
PyObject *BSplineSurfacePy::setPole(PyObject *args)
{
    ...
    try {
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
        if (weight < 0.0) surf->SetPole(uindex, vindex, pnt);
        else
...
PyObject *BSplineSurfacePy::setPoleCol(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iO|O", &vindex, &obj, &obj2)) return nullptr;
    try {
        Py::Sequence list(obj);
        ...
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->SetPoleCol(vindex, poles); }
        else {
...
PyObject *BSplineSurfacePy::setPoleRow(PyObject *args)
{
    ...
    PyObject *obj;
    PyObject *obj2 = nullptr;
    if (!PyArg_ParseTuple(args, "iO|O", &uindex, &obj, &obj2)) return nullptr;
    try {
        Py::Sequence list(obj);
        TColgp_Array1OfPnt poles(1, list.size());
        int index = 1;
        ...
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
        if (!obj2) { surf->SetPoleRow(uindex, poles); }
        else {
            Py::Sequence list(obj2);
            TColStd_Array1OfReal weights(1, list.size());
            int index = 1;
...
{
    int uindex, vindex;
    if (!PyArg_ParseTuple(args, "ii", &uindex, &vindex)) return nullptr;
    try {
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
...
PyObject *BSplineSurfacePy::setWeight(PyObject *args)
{
    int uindex, vindex;
    double weight;
    if (!PyArg_ParseTuple(args, "iid", &uindex, &vindex, &weight)) return nullptr;
    try {
        Handle(Geom_BSplineSurface) surf =
            Handle(Geom_BSplineSurface)::DownCast(getGeometryPtr()->handle());
...
PyObject *BSplineSurfacePy::setWeightCol(PyObject *args)
{
    int vindex;
    PyObject *obj;
    if (!PyArg_ParseTuple(args, "iO", &vindex, &obj)) return nullptr;
    try {
        Py::Sequence list(obj);
        TColStd_Array1OfReal weights(1, list.size());
        int index = 1;
...
PyObject *BSplineSurfacePy::setWeightRow(PyObject *args)
{
    int uindex;
    PyObject *obj;
    if (!PyArg_ParseTuple(args, "iO", &uindex, &obj)) return nullptr;
    try {
        Py::Sequence list(obj);
        TColStd_Array1OfReal weights(1, list.size());
        int index = 1;
...
PyObject *BSplineSurfacePy::movePoint(PyObject *args)
{
    ...
    int vindex1, vindex2;
    PyObject *pnt;
    if (!PyArg_ParseTuple(args, "ddO!iiii", &U, &V, &(Base::VectorPy::Type), &pnt, &uindex1,
                          &uindex2, &vindex1, &vindex2))
        return nullptr;
    try {
        ...
        surf->MovePoint(U, V, gp_Pnt(p.x, p.y, p.z), uindex1, uindex2, vindex1, vindex2, ufirst,
                        ulast, vfirst, vlast);
...
PyObject *BSplineSurfacePy::reparametrize(PyObject *args)
{
    int u, v;
    double tol = 0.000001;
    if (!PyArg_ParseTuple(args, "ii|d", &u, &v, &tol)) return nullptr;
    ...
    try {
        ...
        for (int j = 0; j < u; j++) {
            ...
            for (int k = 0; k < v; k++) {
                ...
                int ufirst, ulast, vfirst, vlast;
                spline->MovePoint(newU, newV, point, j + 1, j + 1, k + 1, k + 1, ufirst, ulast,
                                  vfirst, vlast);
            }
        }
...
PyObject *BSplineSurfacePy::approximate(PyObject *args, PyObject *kwds)
{
    ...
    static char *kwds_interp[] = {"Points",
                                  "DegMin",
                                  "DegMax",
                                  "Continuity",
                                  "Tolerance",
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|iiidddddsddd", kwds_interp, &obj, &degMin,
                                     &degMax, &continuity, &tol3d, &X0, &dX, &Y0, &dY, &parType,
    ...
    try {
        ...
        switch (continuity) {
            case 0: c = GeomAbs_C0; break;
            case 1: c = GeomAbs_C1; break;
            case 2: c = GeomAbs_C2; break;
        }
...
PyObject *BSplineSurfacePy::interpolate(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|dddd", &obj, &X0, &dX, &Y0, &dY)) return nullptr;
    try {
        Py::Sequence list(obj);
        Standard_Integer lu = list.size();
        Py::Sequence col(list.getItem(0));
        ...
        GeomAPI_PointsToBSplineSurface surInterpolation;
        if (len == 1) { surInterpolation.Interpolate(interpolationPoints); }
        else {
...
{
    static char *kwlist[] = {"poles",     "umults",  "vmults",  "uknots",  "vknots", "uperiodic",
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOO|OOO!O!iiO", kwlist, &poles, &umults,
                                     &vmults,                                            //required
    ...
    try {
        ...
        for (Py::Sequence::iterator it = umultssq.begin();
             it != umultssq.end() && index <= occumults.Length(); ++it) {
            Py::Long mult(*it);
        ...
        for (Py::Sequence::iterator it = vmultssq.begin();
             it != vmultssq.end() && index <= occvmults.Length(); ++it) {
            Py::Long mult(*it);
        ...
        if (uknots != Py_None) { //uknots are given
            ...
            for (Py::Sequence::iterator it = uknotssq.begin();
                 it != uknotssq.end() && index <= occuknots.Length(); ++it) {
                Py::Float knot(*it);
        ...
        if (vknots != Py_None) { //vknots are given
            ...
            for (Py::Sequence::iterator it = vknotssq.begin();
                 it != vknotssq.end() && index <= occvknots.Length(); ++it) {
                Py::Float knot(*it);
        ...
        Handle(Geom_BSplineSurface) spline = new Geom_BSplineSurface(
            occpoles, occweights, occuknots, occvknots, occumults, occvmults, udegree, vdegree,
            Base::asBoolean(uperiodic), Base::asBoolean(vperiodic));
        if (!spline.IsNull()) {
...
PyObject *BSplineSurfacePy::buildFromNSections(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|O!", &list, &PyBool_Type, &refSurf)) return nullptr;
...
Py::List BSplineSurfacePy::getUKnotSequence(void) const
{
    ...
    if (surf->IsUPeriodic()) {
        ...
        for (int i = 1; i <= surf->NbUKnots(); i++) m += surf->UMultiplicity(i);
    }
...
Py::List BSplineSurfacePy::getVKnotSequence(void) const
{
    ...
    if (surf->IsVPeriodic()) {
        ...
        for (int i = 1; i <= surf->NbVKnots(); i++) m += surf->VMultiplicity(i);
    }
    TColStd_Array1OfReal k(1, m);
...
PyObject *BSplineSurfacePy::scaleKnotsToBounds(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|dddd", &u0, &u1, &v0, &v1)) return nullptr;
    try {
        if (u0 >= u1 || v0 >= v1) {
...
int BSplineSurfacePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

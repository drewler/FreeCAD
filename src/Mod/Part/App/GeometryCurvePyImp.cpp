...
std::string GeometryCurvePy::representation() const { return "<Curve object>"; }
...
// constructor method
int GeometryCurvePy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *GeometryCurvePy::getDN(PyObject *args)
{
    ...
    try {
        if (!c.IsNull()) {
            int n;
            double u;
            if (!PyArg_ParseTuple(args, "di", &u, &n)) return nullptr;
            gp_Vec v = c->DN(u, n);
...
PyObject *GeometryCurvePy::projectPoint(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O!|s", kwlist, &Base::VectorPy::Type, &v, &meth))
        return nullptr;
...
PyObject *GeometryCurvePy::parameter(PyObject *args)
{
    try {
        PyObject *p;
        if (!PyArg_ParseTuple(args, "O!", &(Base::VectorPy::Type), &p)) return nullptr;
...
{
    PyObject *curve;
    if (!PyArg_ParseTuple(args, "O!", &(Part::GeometryCurvePy::Type), &curve)) return nullptr;
...
{
    PyObject *c, *p;
    if (!PyArg_ParseTuple(args, "O!O!", &(Part::GeometryCurvePy::Type), &c, &(Part::PlanePy::Type),
                          &p))
        return nullptr;
...
PyObject *GeometryCurvePy::continuityWith(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!|ddO!O!dd", &GeometryCurvePy::Type, &curve, &u1, &u2,
                          &PyBool_Type, &rev1, &PyBool_Type, &rev2, &tl, &ta))
        return nullptr;
    ...
    try {
        if (!c1.IsNull() && !c2.IsNull()) {
            GeomAbs_Shape c;
            if (tl >= 0.0 && ta >= 0.0) c = GeomLProp::Continuity(c1, c2, u1, u2, r1, r2, tl, ta);
            else
...
PyObject *GeometryCurvePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int GeometryCurvePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }
...
PyObject *GeometryCurvePy::intersectCC(PyObject *args)
{
    PyObject *p;
    double prec = Precision::Confusion();
    if (!PyArg_ParseTuple(args, "O!|d", &(Part::GeometryCurvePy::Type), &p, &prec)) return nullptr;
...
{
    double p;
    if (!PyArg_ParseTuple(args, "d", &p)) return nullptr;
    try {
        Handle(Geom_Geometry) g = getGeometryPtr()->handle();
        Handle(Geom_Curve) c = Handle(Geom_Curve)::DownCast(g);

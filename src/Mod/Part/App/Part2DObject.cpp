...
App::DocumentObjectExecReturn *Part2DObject::execute(void) { return Feature::execute(); }
...
int Part2DObject::getAxisCount(void) const { return 0; }

Base::Axis Part2DObject::getAxis(int axId) const
{
    if (axId == H_Axis) { return Base::Axis(Base::Vector3d(0, 0, 0), Base::Vector3d(1, 0, 0)); }
    else if (axId == V_Axis) {
...
bool Part2DObject::seekTrimPoints(const std::vector<Geometry *> &geomlist, int geometryIndex,
                                  const Base::Vector3d &point, int &geometryIndex1,
                                  Base::Vector3d &intersect1, int &geometryIndex2,
                                  Base::Vector3d &intersect2)
{
    if (geometryIndex >= int(geomlist.size())) return false;
    ...
    for (int id = 0; id < int(geomlist.size()); id++) {
        ...
        if (id != geometryIndex /* && !geomlist[id]->Construction*/) {
            ...
            if (!curve3d.IsNull()) {
                ...
                if (geomlist[id]->getTypeId().isDerivedFrom(
                        Part::GeomBoundedCurve::getClassTypeId())) {
                ...
                for (auto p : points) {
                    ...
                    if (periodic) {
                        ...
                        param -=
                            period; // transfer param into the interval (pickedParam pickedParam+period]
                        if (param < param2) {
                            param2 = param;
                            p2 = p;
                            geometryIndex2 = id;
    ...
    if (periodic) {
        ...
        if (fabs(param2 - param1 - period) < 1e-10) {
            if (param2 - pickedParam >= pickedParam - param1) geometryIndex2 = -1;
            else
                geometryIndex1 = -1;
        }
    }

    if (geometryIndex1 < 0 && geometryIndex2 < 0) return false;

    if (geometryIndex1 >= 0) intersect1 = Base::Vector3d(p1.X(), p1.Y(), 0.f);
    if (geometryIndex2 >= 0) intersect2 = Base::Vector3d(p2.X(), p2.Y(), 0.f);
    return true;
}
...
void Part2DObject::Restore(Base::XMLReader &reader) { Part::Feature::Restore(reader); }

void Part2DObject::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                             App::Property *prop)
{
    ...
    if (prop->isDerivedFrom(App::PropertyLinkSubList::getClassTypeId())) {
        ...
        if (0 == strcmp(tmp.getTypeId().getName(), TypeName)) {
            ...
            tmp.Restore(reader);
            static_cast<App::PropertyLinkSubList *>(prop)->setValue(tmp.getValue(),
                                                                    tmp.getSubValues());
        }
...
void Part2DObject::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                             const char *PropName)
{

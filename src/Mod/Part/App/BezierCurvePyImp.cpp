...
std::string BezierCurvePy::representation() const { return "<BezierCurve object>"; }
...
// constructor method
int BezierCurvePy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *BezierCurvePy::insertPoleAfter(PyObject *args)
{
    ...
    try {
        Handle(Geom_BezierCurve) curve =
            Handle(Geom_BezierCurve)::DownCast(getGeometryPtr()->handle());
...
{
    PyObject *plist;
    if (!PyArg_ParseTuple(args, "O", &plist)) return nullptr;
    try {
...
PyObject *BezierCurvePy::interpolate(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|O", &obj, &par)) return nullptr;
    try {
        Handle(Geom_BezierCurve) curve =
            Handle(Geom_BezierCurve)::DownCast(getGeometryPtr()->handle());
        Py::Sequence constraints(obj);
        int nb_pts = constraints.size();
        if (nb_pts < 2) Standard_Failure::Raise("not enough points given");
        ...
        for (Py::Sequence::iterator it1 = constraints.begin(); it1 != constraints.end(); ++it1) {
            ...
            BSplCLib::EvalBsplineBasis(row.size() - 1, num_poles, knots, params(cons_idx),
                                       first_non_zero, bezier_eval, Standard_False);
            int idx2 = 1;
...
PyObject *BezierCurvePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int BezierCurvePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

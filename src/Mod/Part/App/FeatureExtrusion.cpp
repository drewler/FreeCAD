...
Extrusion::Extrusion()
{
    ...
    ADD_PROPERTY_TYPE(LengthFwd, (0.0), "Extrude", App::Prop_None,
                      "Length of extrusion along direction. If both LengthFwd and LengthRev are "
                      "zero, magnitude of Dir is used.");
    ...
    ADD_PROPERTY_TYPE(Symmetric, (false), "Extrude", App::Prop_None,
                      "If true, extrusion is done in both directions to a total of LengthFwd. "
                      "LengthRev is ignored.");
    ADD_PROPERTY_TYPE(TaperAngle, (0.0), "Extrude", App::Prop_None,
                      "Sets the angle of slope (draft) to apply to the sides. The angle is for "
                      "outward taper; negative value yields inward tapering.");
    ADD_PROPERTY_TYPE(TaperAngleRev, (0.0), "Extrude", App::Prop_None,
                      "Taper angle of reverse part of extrusion.");
    ADD_PROPERTY_TYPE(
        FaceMakerClass, ("Part::FaceMakerExtrusion"), "Extrude", App::Prop_None,
        "If Solid is true, this sets the facemaker class to use when converting wires to faces. "
        "Otherwise, ignored."); //default for old documents. See setupObject for default for new extrusions.
}
...
bool Extrusion::fetchAxisLink(const App::PropertyLinkSub &axisLink, Base::Vector3d &basepoint,
                              Base::Vector3d &dir)
{
    if (!axisLink.getValue()) return false;
    ...
    if (crv.GetType() == GeomAbs_Line) {
        ...
        if (axEdge.Orientation() == TopAbs_REVERSED) std::swap(startpoint, endpoint);
    }
    else {
...
Extrusion::ExtrusionParameters Extrusion::computeFinalParameters()
{
    ...
    switch (this->DirMode.getValue()) {
        case dmCustom: dir = this->Dir.getValue(); break;
        case dmEdge: {
            bool fetched;
            Base::Vector3d base;
        ...
        case dmNormal:
            ...
            this->Dir.setValue(dir);
            break;
        default: throw Base::ValueError("Unexpected enum value");
    }
    if (dir.Length() < Precision::Confusion()) throw Base::ValueError("Direction is zero-length");
    result.dir = gp_Dir(dir.x, dir.y, dir.z);
    if (this->Reversed.getValue()) result.dir.Reverse();
...
Base::Vector3d Extrusion::calculateShapeNormal(const App::PropertyLink &shapeLink)
{
    ...
    if (!docobj) throw Base::ValueError("calculateShapeNormal: link is empty");
    ...
    if (ex.More()) {
        ...
        if (ex.Current().Orientation() == TopAbs_REVERSED) { normal.Reverse(); }
    }
...
{
    TopoDS_Shape result;
    gp_Vec vec = gp_Vec(params.dir)
                     .Multiplied(params.lengthFwd + params.lengthRev); //total vector of extrusion
    ...
    if (std::fabs(params.taperAngleFwd) >= Precision::Angular()
        ...
        if (myShape.IsNull()) Standard_Failure::Raise("Cannot extrude empty shape");
        // #0000910: Circles Extrude Only Surfaces, thus use BRepBuilderAPI_Copy
        myShape = BRepBuilderAPI_Copy(myShape).Shape();

        std::list<TopoDS_Shape> drafts;
        bool isPartDesign =
            false; // there is an OCC bug with single-edge wires (circles) we need to treat differently for PD and Part
        ExtrusionHelper::makeDraft(myShape, params.dir, params.lengthFwd, params.lengthRev,
                                   params.taperAngleFwd, params.taperAngleRev, params.solid, drafts,
                                   isPartDesign);
        if (drafts.empty()) { Standard_Failure::Raise("Drafting shape failed"); }
        ...
        if (params.solid) {
            ...
            if (xp.More()) {
                ...
                std::unique_ptr<FaceMaker> mkFace =
                    FaceMaker::ConstructFromType(params.faceMakerClass.c_str());
    ...
    if (result.IsNull()) throw NullShapeException("Result of extrusion is null shape.");
...
{
    return std::string(QT_TRANSLATE_NOOP(
        "Part_FaceMaker", "Supports making faces with holes, does not support nesting."));
}
...
#endif
{
    ...
    if (mySourceShapes.empty()) throw Base::ValueError("No input shapes!");
    if (mySourceShapes.size() == 1) { inputShape = mySourceShapes[0]; }
    else {
        TopoDS_Builder builder;
        TopoDS_Compound cmp;
        builder.MakeCompound(cmp);
        for (const TopoDS_Shape &sh : mySourceShapes) { builder.Add(cmp, sh); }
        inputShape = cmp;
    }
    ...
    if (!wires.empty()) {
        ...
        if (!res.IsNull()) this->myShape = res;
        //}
        //catch (...) {

...
int PropertyGeometryList::getSize() const { return static_cast<int>(_lValueList.size()); }
...
void PropertyGeometryList::setValues(std::vector<Geometry *> &&lValue)
{
    ...
    for (auto v : lValue) valueSet.erase(v);
...
{
    if (idx >= (int)_lValueList.size()) throw Base::IndexError("Index out of bound");
    aboutToSetValue();
    if (idx < 0) _lValueList.push_back(lValue.release());
    else {
...
void PropertyGeometryList::setPyObject(PyObject *value)
{
    ...
    if (PySequence_Check(value)) {
        ...
        setValues(values);
        if (part2d) part2d->acceptGeometry();
    }

...
int TopoShapeSolidPy::PyInit(PyObject *args, PyObject * /*kwd*/)
{
    ...
    PyErr_Clear();
    PyObject *obj;
    if (!PyArg_ParseTuple(args, "O!", &(TopoShapePy::Type), &obj)) return -1;

    try {
        const TopoDS_Shape &shape = static_cast<TopoShapePy *>(obj)->getTopoShapePtr()->getShape();
        ...
        if (count == 0) {
            ...
            Standard_Failure::Raise(
                "Only one compsolid can be accepted. Provided shape has more than one compsolid.");
        }
    }
...
PyObject *TopoShapeSolidPy::offsetFaces(PyObject *args)
{
    ...
    builder.Initialize(shape, 1.0, Precision::Confusion(), BRepOffset_Skin, Standard_False,
...
PyObject *TopoShapeSolidPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

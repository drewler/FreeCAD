...
Offset2D::Offset2D()
{
    ...
    this->Mode.setValue(
        1); //switch to Pipe mode by default, because skin mode does not function properly on closed profiles.
}
...
App::DocumentObjectExecReturn *Offset2D::execute(void)
{
    ...
    if (shape.isNull()) { return new App::DocumentObjectExecReturn("No source shape linked."); }
    double offset = Value.getValue();
    short mode = (short)Mode.getValue();
    short join = (short)Join.getValue();
    bool fill = Fill.getValue();

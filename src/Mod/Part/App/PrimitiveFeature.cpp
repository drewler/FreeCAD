...
short Primitive::mustExecute() const { return Feature::mustExecute(); }

App::DocumentObjectExecReturn *Primitive::execute() { return Part::Feature::execute(); }
...
void Primitive::Restore(Base::XMLReader &reader) { Part::Feature::Restore(reader); }

void Primitive::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                          const char *PropName)
{
    extHandleChangedPropertyName(reader, TypeName, PropName); // AttachExtension
}

void Primitive::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                          App::Property *prop)
{
...
{
    if (X.isTouched() || Y.isTouched() || Z.isTouched()) return 1;
...
Line::Line()
{
    ...
    ADD_PROPERTY_TYPE(X2, (0.0), "Vertex 2 - Finish", App::Prop_None,
                      "X value of the finish vertex");
    ADD_PROPERTY_TYPE(Y2, (0.0), "Vertex 2 - Finish", App::Prop_None,
                      "Y value of the finish vertex");
    ADD_PROPERTY_TYPE(Z2, (1.0), "Vertex 2 - Finish", App::Prop_None,
                      "Z value of the finish vertex");
}
...
App::DocumentObjectExecReturn *Line::execute()
{
    ...
    if (!mkEdge.IsDone()) return new App::DocumentObjectExecReturn("Failed to create edge");
...
{
    if (!isRestoring()) {
        if (prop == &X1 || prop == &Y1 || prop == &Z1 || prop == &X2 || prop == &Y2
            || prop == &Z2) {
            try {
                App::DocumentObjectExecReturn *ret = recompute();
                delete ret;
...
{
    if (Length.isTouched() || Width.isTouched()) return 1;
...
App::DocumentObjectExecReturn *Plane::execute()
{
    ...
    switch (mkFace.Error()) {
        case BRepBuilderAPI_FaceDone: break; // ok
        case BRepBuilderAPI_NoFace: error = "no face"; break;
        case BRepBuilderAPI_NotPlanar: error = "not planar"; break;
        case BRepBuilderAPI_CurveProjectionFailed: break;
        case BRepBuilderAPI_ParametersOutOfRange: error = "parameters out of range"; break;
        default: error = "unknown error"; break;
    }
    // Error ?
    if (error) { return new App::DocumentObjectExecReturn(error); }
...
{
    if (Radius.isTouched()) return 1;
    if (Angle1.isTouched()) return 1;
    if (Angle2.isTouched()) return 1;
    if (Angle3.isTouched()) return 1;
    return Primitive::mustExecute();
}
...
App::DocumentObjectExecReturn *Sphere::execute()
{
    ...
    try {
        BRepPrimAPI_MakeSphere mkSphere(Radius.getValue(), Angle1.getValue() / 180.0f * M_PI,
...
{
    if (Radius1.isTouched()) return 1;
    if (Radius2.isTouched()) return 1;
    if (Radius3.isTouched()) return 1;
    if (Angle1.isTouched()) return 1;
    if (Angle2.isTouched()) return 1;
    if (Angle3.isTouched()) return 1;
    return Primitive::mustExecute();
...
App::DocumentObjectExecReturn *Ellipsoid::execute()
{
    ...
    try {
        ...
        gp_Dir dir(0.0, 0.0, 1.0);
        gp_Ax2 ax2(pnt, dir);
        BRepPrimAPI_MakeSphere mkSphere(ax2, Radius2.getValue(), Angle1.getValue() / 180.0f * M_PI,
                                        Angle2.getValue() / 180.0f * M_PI,
                                        Angle3.getValue() / 180.0f * M_PI);
...
Cylinder::Cylinder()
{
    ...
    ADD_PROPERTY_TYPE(Angle, (360.0f), "Cylinder", App::Prop_None,
...
{
    if (Radius.isTouched()) return 1;
    if (Height.isTouched()) return 1;
    if (Angle.isTouched()) return 1;
    return Primitive::mustExecute();
}
...
App::DocumentObjectExecReturn *Cylinder::execute()
{
    ...
    try {
        BRepPrimAPI_MakeCylinder mkCylr(Radius.getValue(), Height.getValue(),
...
Prism::Prism()
{
    ADD_PROPERTY_TYPE(Polygon, (6.0), "Prism", App::Prop_None,
                      "Number of sides in the polygon, of the prism");
    ADD_PROPERTY_TYPE(Circumradius, (2.0), "Prism", App::Prop_None,
...
{
    if (Polygon.isTouched()) return 1;
    if (Circumradius.isTouched()) return 1;
    if (Height.isTouched()) return 1;
    return Primitive::mustExecute();
}
...
{
    ADD_PROPERTY_TYPE(Polygon, (6.0), "RegularPolygon", App::Prop_None,
                      "Number of sides in the regular polygon");
    ADD_PROPERTY_TYPE(Circumradius, (2.0), "RegularPolygon", App::Prop_None,
...
App::DocumentObjectExecReturn *RegularPolygon::execute()
{
    ...
    if (Polygon.getValue() < 3)
        return new App::DocumentObjectExecReturn(
            "the polygon is invalid, must have 3 or more sides");
    if (Circumradius.getValue() < Precision::Confusion())
...
App::DocumentObjectExecReturn *Cone::execute()
{
    ...
    try {
        // Build a cone
        BRepPrimAPI_MakeCone mkCone(Radius1.getValue(), Radius2.getValue(), Height.getValue(),
...
App::DocumentObjectExecReturn *Torus::execute()
{
    ...
    try {
        TopoShape shape;
        this->Shape.setValue(shape.makeTorus(Radius1.getValue(), Radius2.getValue(),
                                             Angle1.getValue(), Angle2.getValue(),
                                             Angle3.getValue()));
    }
    catch (Standard_Failure &e) {
        return new App::DocumentObjectExecReturn(e.GetMessageString());
...
{
    if (Pitch.isTouched()) return 1;
    if (Height.isTouched()) return 1;
    if (Radius.isTouched()) return 1;
    if (Angle.isTouched()) return 1;
    if (LocalCoord.isTouched()) return 1;
    if (Style.isTouched()) return 1;
    return Primitive::mustExecute();
}
...
App::DocumentObjectExecReturn *Helix::execute()
{
    try {
        ...
        if (myPitch < Precision::Confusion()) Standard_Failure::Raise("Pitch too small");
        Standard_Real nbTurns = myHeight / myPitch;
        if (nbTurns > 1e4) Standard_Failure::Raise("Number of turns too high (> 1e4)");
        Standard_Real myRadiusTop = myRadius + myHeight * tan(myAngle / 180.0f * M_PI);

        this->Shape.setValue(TopoShape().makeSpiralHelix(myRadius, myRadiusTop, myHeight, nbTurns,
                                                         mySegLen, myLocalCS));
    }
    catch (Standard_Failure &e) {
...
Spiral::Spiral()
{
    ADD_PROPERTY_TYPE(Growth, (1.0), "Spiral", App::Prop_None,
                      "The growth of the spiral per rotation");
    ...
    ADD_PROPERTY_TYPE(SegmentLength, (1.0), "Spiral", App::Prop_None,
...
{
    if (Growth.isTouched()) return 1;
    if (Rotations.isTouched()) return 1;
    if (Radius.isTouched()) return 1;
    return Primitive::mustExecute();
}
...
App::DocumentObjectExecReturn *Spiral::execute()
{
    try {
        ...
        this->Shape.setValue(TopoShape().makeSpiralHelix(myRadius, myRadiusTop, 0, myNumRot,
                                                         mySegLen, Standard_False));
        return Primitive::execute();
    }
    catch (Standard_Failure &e) {
        return new App::DocumentObjectExecReturn(e.GetMessageString());
...
App::DocumentObjectExecReturn *Wedge::execute()
{
    ...
    if (dz2 < 0) return new App::DocumentObjectExecReturn("delta z2 of wedge is negative");

    if (dx2 < 0) return new App::DocumentObjectExecReturn("delta x2 of wedge is negative");

    try {
        gp_Pnt pnt(0.0, 0.0, 0.0);
        gp_Dir dir(0.0, 0.0, 1.0);
        BRepPrim_Wedge mkWedge(gp_Ax2(pnt, dir), xmin, ymin, zmin, z2min, x2min, xmax, ymax, zmax,
                               z2max, x2max);
...
{
    Base::ObjectStatusLocker<App::Property::Status, App::Property> lock(App::Property::User1,
                                                                        &Angle2, false);
...
void Ellipse::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                        const char *PropName)
{

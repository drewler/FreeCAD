...
std::string ConePy::representation() const { return "<Cone object>"; }
...
int ConePy::PyInit(PyObject *args, PyObject *kwds)
{
    ...
    PyErr_Clear();
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O!O!dd", keywords_pprr, &(Base::VectorPy::Type),
                                    &pV1, &(Base::VectorPy::Type), &pV2, &radius1, &radius2)) {
        Base::Vector3d v1 = static_cast<Base::VectorPy *>(pV1)->value();
        Base::Vector3d v2 = static_cast<Base::VectorPy *>(pV2)->value();
        GC_MakeConicalSurface mc(gp_Pnt(v1.x, v1.y, v1.z), gp_Pnt(v2.x, v2.y, v2.z), radius1,
                                 radius2);
        if (!mc.IsDone()) {
    ...
    PyErr_Clear();
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O!O!O!O!", keywords_pppp, &(Base::VectorPy::Type),
                                    &pV1, &(Base::VectorPy::Type), &pV2, &(Base::VectorPy::Type),
                                    &pV3, &(Base::VectorPy::Type), &pV4)) {
        Base::Vector3d v1 = static_cast<Base::VectorPy *>(pV1)->value();
        Base::Vector3d v2 = static_cast<Base::VectorPy *>(pV2)->value();
        Base::Vector3d v3 = static_cast<Base::VectorPy *>(pV3)->value();
        Base::Vector3d v4 = static_cast<Base::VectorPy *>(pV4)->value();
        GC_MakeConicalSurface mc(gp_Pnt(v1.x, v1.y, v1.z), gp_Pnt(v2.x, v2.y, v2.z),
                                 gp_Pnt(v3.x, v3.y, v3.z), gp_Pnt(v4.x, v4.y, v4.z));
        if (!mc.IsDone()) {
            PyErr_SetString(PartExceptionOCCError, gce_ErrorStatusText(mc.Status()));
            return -1;
        }
    ...
    PyErr_Clear();
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O!d", keywords_c, &(ConePy::Type), &pCone)) {
...
PyObject *ConePy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int ConePy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }

...
void ModelRefine::boundaryEdges(const FaceVectorType &faces, EdgeVectorType &edgesOut)
{
    ...
    for (faceIt = faces.begin(); faceIt != faces.end(); ++faceIt) {
        ...
        for (faceEdgesIt = faceEdges.begin(); faceEdgesIt != faceEdges.end(); ++faceEdgesIt) {
            ...
            if (!foundSignal) edges.push_back(*faceEdgesIt);
        }
    }
...
TopoDS_Shell ModelRefine::removeFaces(const TopoDS_Shell &shell, const FaceVectorType &faces)
{
    ...
    for (it = faces.begin(); it != faces.end(); ++it) rebuilder.Remove(*it);
...
void FaceTypeSplitter::addShell(const TopoDS_Shell &shellIn) { shell = shellIn; }
...
{
    if (this->hasType(type)) return (*(typeMap.find(type))).second;
    //error here.
...
void FaceAdjacencySplitter::split(const FaceVectorType &facesIn)
{
    ...
    FaceVectorType::const_iterator it;
    for (it = facesIn.begin(); it != facesIn.end(); ++it) facesInMap.Add(*it);
    ...
    for (it = facesIn.begin(); it != facesIn.end(); ++it) {
        ...
        if (processedMap.Contains(*it)) continue;

        tempFaces.clear();
        processedMap.Add(*it);
        recursiveFind(*it, tempFaces);
        if (tempFaces.size() > 1) { adjacencyArray.push_back(tempFaces); }
    }
}
...
void FaceAdjacencySplitter::recursiveFind(const TopoDS_Face &face, FaceVectorType &outVector)
{
    ...
    for (edgeIt.Initialize(edges); edgeIt.More(); edgeIt.Next()) {
        ...
        for (faceIt.Initialize(faces); faceIt.More(); faceIt.Next()) {
            if (!facesInMap.Contains(faceIt.Value())) continue;
            if (processedMap.Contains(faceIt.Value())) continue;
                                  ...
                                  std::vector<EdgeVectorType> &boundariesOut) const
{
    ...
    while (!edges.empty()) {
        ...
        if (closedSignal) boundariesOut.push_back(boundary);
    }
}
...
bool FaceTypedPlane::isEqual(const TopoDS_Face &faceOne, const TopoDS_Face &faceTwo) const
{
    ...
    if (planeSurfaceOne.IsNull() || planeSurfaceTwo.IsNull()) return false; //error?

    gp_Pln planeOne(planeSurfaceOne->Pln());
    gp_Pln planeTwo(planeSurfaceTwo->Pln());
    return (planeOne.Position().Direction().IsParallel(planeTwo.Position().Direction(),
                                                       Precision::Confusion())
            && planeOne.Distance(planeTwo.Position().Location()) < Precision::Confusion());
}

GeomAbs_SurfaceType FaceTypedPlane::getType() const { return GeomAbs_Plane; }
...
TopoDS_Face FaceTypedPlane::buildFace(const FaceVectorType &faces) const
{
    ...
    if (splitEdges.empty()) return TopoDS_Face();
    ...
    for (splitIt = splitEdges.begin(); splitIt != splitEdges.end(); ++splitIt) {
        ...
        for (it = (*splitIt).begin(); it != (*splitIt).end(); ++it) wireMaker.Add(*it);
    ...
    if (faceMaker.Error() != BRepLib_FaceDone) return TopoDS_Face();
    TopoDS_Face current = faceMaker.Face();
    if (wires.size() > 1) {
        ShapeFix_Face faceFix(current);
        faceFix.SetContext(new ShapeBuild_ReShape());
        for (size_t index(1); index < wires.size(); ++index) faceFix.Add(wires.at(index));
        faceFix.Perform();
        if (faceFix.Status(ShapeExtend_FAIL)) return TopoDS_Face();
...
static Handle(Geom_CylindricalSurface) getGeomCylinder(const TopoDS_Face &faceIn)
{
    ...
    if (!surface.IsNull()) {
        ...
        if (cylinderSurfaceOut.IsNull()) {
            ...
            if (!trimmedSurface.IsNull())
                cylinderSurfaceOut =
                    Handle(Geom_CylindricalSurface)::DownCast(trimmedSurface->BasisSurface());
        }
    }
...
bool FaceTypedCylinder::isEqual(const TopoDS_Face &faceOne, const TopoDS_Face &faceTwo) const
{
    ...
    if (surfaceOne.IsNull() || surfaceTwo.IsNull()) return false; //probably need an error
    gp_Cylinder cylinderOne = surfaceOne->Cylinder();
    gp_Cylinder cylinderTwo = surfaceTwo->Cylinder();

    if (fabs(cylinderOne.Radius() - cylinderTwo.Radius()) > Precision::Confusion()) return false;
    if (!cylinderOne.Axis().IsCoaxial(cylinderTwo.Axis(), Precision::Angular(),
                                      Precision::Confusion())
        && !cylinderOne.Axis().IsCoaxial(cylinderTwo.Axis().Reversed(), Precision::Angular(),
                                         Precision::Confusion()))
        return false;
...
GeomAbs_SurfaceType FaceTypedCylinder::getType() const { return GeomAbs_Cylinder; }
...
const TopoDS_Face fixFace(const TopoDS_Face &f)
{
    ...
    faceFixer.Perform();
    if (faceFixer.Status(ShapeExtend_FAIL)) return dummy;
...
bool wireEncirclesAxis(const TopoDS_Wire &wire, const Handle(Geom_CylindricalSurface) & cylinder)
{
    ...
    gp_Vec cv(cylAxis.Location().X(), cylAxis.Location().Y(),
              cylAxis.Location().Z()); // center of cylinder
    gp_Vec av(cylAxis.Direction().X(), cylAxis.Direction().Y(),
              cylAxis.Direction().Z()); // axis of cylinder
    ...
    for (TopExp_Explorer ex(wire, TopAbs_EDGE); ex.More(); ex.Next()) {
        ...
        if (adapt.GetType() == GeomAbs_Line) {
            ...
            for (Standard_Integer j = 1; j <= SeqPnt.Length(); j++) {
                ...
                if (!proj.IsDone()) return false; // FIXME: What else can we do?
...
TopoDS_Face FaceTypedCylinder::buildFace(const FaceVectorType &faces) const
{
    ...
    if (boundaries.empty()) return dummy;
    ...
    for (boundaryIt = boundaries.begin(); boundaryIt != boundaries.end(); ++boundaryIt) {
        BRepLib_MakeWire wireMaker;
        EdgeVectorType::iterator it;
        for (it = (*boundaryIt).begin(); it != (*boundaryIt).end(); ++it) wireMaker.Add(*it);
        if (wireMaker.Error() != BRepLib_WireDone) return dummy;
        allWires.push_back(wireMaker.Wire());
    }
    if (allWires.empty()) return dummy;
    ...
    if (surface.IsNull()) return dummy;
    ...
    for (wireIt = allWires.begin(); wireIt != allWires.end(); ++wireIt) {
        if (wireEncirclesAxis(*wireIt, surface)) encirclingWires.push_back(*wireIt);
        else
    ...
    if (encirclingWires.empty()) {
        ...
        if (!faceMaker.IsDone()) return dummy;
        ...
        if (encirclingWires.size() != 2) return dummy;
...
void collectConicEdges(const TopoDS_Shell &shell, TopTools_IndexedMapOfShape &map)
{
    ...
    for (int index = 1; index <= edges.Extent(); ++index) {
        ...
        if (curve->IsKind(STANDARD_TYPE(Geom_Conic))) map.Add(currentEdge);
    }
}
#endif
...
{
    try {
        Handle(Geom_BSplineSurface) surfaceOne =
            Handle(Geom_BSplineSurface)::DownCast(BRep_Tool::Surface(faceOne));
        Handle(Geom_BSplineSurface) surfaceTwo =
            Handle(Geom_BSplineSurface)::DownCast(BRep_Tool::Surface(faceTwo));

        if (surfaceOne.IsNull() || surfaceTwo.IsNull()) return false;
        ...
        if (uPoleCountOne != uPoleCountTwo || vPoleCountOne != vPoleCountTwo) return false;
        ...
        if (uKnotCountOne != uKnotCountTwo || vKnotCountOne != vKnotCountTwo) return false;
        ...
        for (int indexU = 1; indexU <= uKnotCountOne; ++indexU)
            if (uKnotsOne.Value(indexU) != uKnotsTwo.Value(indexU)) return false;
        for (int indexV = 1; indexV <= vKnotCountOne; ++indexV)
            if (vKnotsOne.Value(indexV) != vKnotsTwo.Value(indexV)) return false;
        ...
        for (int indexU = 1; indexU <= uKnotSequenceOneCount; ++indexU)
            if (uKnotSequenceOne.Value(indexU) != uKnotSequenceTwo.Value(indexU)) return false;
        for (int indexV = 1; indexV <= vKnotSequenceOneCount; ++indexV)
            if (vKnotSequenceOne.Value(indexV) != vKnotSequenceTwo.Value(indexV)) return false;
        return true;
    }
...
GeomAbs_SurfaceType FaceTypedBSpline::getType() const { return GeomAbs_BSplineSurface; }
...
TopoDS_Face FaceTypedBSpline::buildFace(const FaceVectorType &faces) const
{
    ...
    Handle(Geom_BSplineSurface) surface =
        Handle(Geom_BSplineSurface)::DownCast(BRep_Tool::Surface(faces.at(0)));
    if (!surface) return TopoDS_Face();
    std::vector<TopoDS_Wire>::iterator wireIt;
    wireIt = wires.begin();
    BRepBuilderAPI_MakeFace faceMaker(surface, *wireIt);
    ...
    faceFixer.SetContext(new ShapeBuild_ReShape());
    faceFixer.Perform();
    if (faceFixer.Status(ShapeExtend_FAIL)) return TopoDS_Face();
    faceFixer.FixOrientation();
    faceFixer.Perform();
    if (faceFixer.Status(ShapeExtend_FAIL)) return TopoDS_Face();
...
FaceUniter::FaceUniter(const TopoDS_Shell &shellIn) : modifiedSignal(false) { workShell = shellIn; }

bool FaceUniter::process()
{
    if (workShell.IsNull()) return false;
    ...
    for (typeIt = typeObjects.begin(); typeIt != typeObjects.end(); ++typeIt) {
        ...
        for (std::size_t indexEquality(0); indexEquality < equalitySplitter.getGroupCount();
            ...
            for (std::size_t adjacentIndex(0); adjacentIndex < adjacencySplitter.getGroupCount();
                ...
                TopoDS_Face newFace =
                    (*typeIt)->buildFace(adjacencySplitter.getGroup(adjacentIndex));
                if (!newFace.IsNull()) {
                    ...
                    if (facesToRemove.capacity()
                        <= facesToRemove.size() + adjacencySplitter.getGroup(adjacentIndex).size())
    ...
    if (!facesToSew.empty()) {
        ...
        if (!emptyShell || facesToSew.size() > 1) {
            ...
            for (sewIt = facesToSew.begin(); sewIt != facesToSew.end(); ++sewIt) sew.Add(*sewIt);
            sew.Perform();
            try {
            ...
            for (std::vector<ShapePairType>::iterator it = modifiedShapes.begin();
                 it != modifiedShapes.end(); ++it) {
...
// TODO: change this version after occ fix. Freecad Mantis 1450
        ...
        if (checkFinalShell) {
            ...
            if (!check.HasFreeEdges()) {
                ...
                if (props.Mass() < 0) workShell = TopoDS::Shell(workShell.Reversed());
            }
        }
        // update the list of modifications
...
#endif
{
    if (myShape.IsNull()) Standard_Failure::Raise("Cannot remove splitter from empty shape");
...
void Part::BRepBuilderAPI_RefineModel::LogModifications(const ModelRefine::FaceUniter &uniter)
{
    ...
    for (std::vector<ShapePairType>::const_iterator it = modShapes.begin(); it != modShapes.end();
         ++it) {
...
{
    if (myModified.IsBound(S)) return myModified.Find(S);
    else
        return myEmptyList;
}

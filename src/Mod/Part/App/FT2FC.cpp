...
PyObject *FT2FC(const Py_UNICODE *PyUString, const size_t length, const char *FontPath,
                const char *FontName, const double stringheight, const double tracking)
{
    std::string FontSpec;
...
PyObject *FT2FC(const Py_UNICODE *PyUString, const size_t length, const char *FontSpec,
                ...
                const double tracking)     // fc coords
{
    ...
    error = FT_Set_Char_Size(FTFont, 0, /* char_width in 1/64th of points */
    ...
    for (i = 0; i < length; i++) {
        ...
        error = FT_Load_Char(FTFont, currchar, FTLoadFlags);
        if (error) {
        ...
        PenPos += kern.x;
        try {
            Py::List WireList(getGlyphContours(FTFont, currchar, PenPos, scalefactor, i, tracking),
                              true);
...
static int move_cb(const FT_Vector *pt, void *p)
{
    ...
    dc->LastVert = *pt;
    if (dc->polyPoints.empty()) { dc->polyPoints.emplace_back(pt->x, pt->y, 0.0); }
...
static int quad_cb(const FT_Vector *pt0, const FT_Vector *pt1, void *p)
{
    ...
    if (spline.IsNull()) { Base::Console().Message("Conversion to B-spline failed"); }
                           ...
                           int charNum, double tracking)
{
    ...
    error = FT_Outline_Decompose(&FTFont->glyph->outline, &FTcbFuncs, &ctx);
    if (error) {
    ...
    bool isTTF = false;
    if (ftOrient == FT_ORIENTATION_TRUETYPE) { isTTF = true; }
...
int calcClockDir(std::vector<Base::Vector3d> points)
{
    ...
    bigArea += area;
    if (bigArea < 0) { result = ANTICLOCKWISE; }
    return result;
}

...
{
    switch (et) {
        case BRepBuilderAPI_FaceDone: return "Construction was successful";
        case BRepBuilderAPI_NoFace: return "No face";
        case BRepBuilderAPI_NotPlanar: return "Face is not planar";
        case BRepBuilderAPI_CurveProjectionFailed: return "Curve projection failed";
        case BRepBuilderAPI_ParametersOutOfRange: return "Parameters out of range";
...
std::string ShapeSegment::getName() const { return std::string(); }
...
TopoShape::TopoShape(const TopoDS_Shape &shape) : _Shape(shape) {}

TopoShape::TopoShape(const TopoShape &shape) : _Shape(shape._Shape) { Tag = shape.Tag; }
...
unsigned long TopoShape::countSubElements(const char *Type) const { return countSubShapes(Type); }
...
TopoDS_Shape TopoShape::getSubShape(TopAbs_ShapeEnum type, int index, bool silent) const
{
    ...
    try {
        if (type == TopAbs_SHAPE) {
            ...
            if (index <= anIndices.Extent()) return anIndices.FindKey(index);
        }
    }
    ...
    if (!silent) Standard_Failure::Raise("Index out of bound");
    return TopoDS_Shape();
}
...
{
    if (!Type) return 0;
    if (strcmp(Type, "SubShape") == 0) return countSubShapes(TopAbs_SHAPE);
    auto type = shapeType(Type, true);
    if (type == TopAbs_SHAPE) return 0;
...
unsigned long TopoShape::countSubShapes(TopAbs_ShapeEnum Type) const
{
    if (Type == TopAbs_SHAPE) {
        int count = 0;
        for (TopoDS_Iterator it(_Shape); it.More(); it.Next()) ++count;
        return count;
    }
    TopTools_IndexedMapOfShape anIndices;
    TopExp::MapShapes(this->_Shape, Type, anIndices);
...
{
    std::vector<T> shapes;
    if (s.IsNull()) return shapes;

    if (type == TopAbs_SHAPE) {
        for (TopoDS_Iterator it(s); it.More(); it.Next()) shapes.emplace_back(it.Value());
        return shapes;
    }
    ...
    for (int i = 1; i <= count; ++i) shapes.emplace_back(anIndices.FindKey(i));
    return shapes;
}
...
TopAbs_ShapeEnum TopoShape::shapeType(char type, bool silent)
{
    switch (type) {
        ...
        default:
            if (!silent) FC_THROWM(Base::CADKernelError, "invalid shape type '" << type << "'");
            return TopAbs_SHAPE;
    }
}
...
{
    if (isNull()) {
        if (!silent) FC_THROWM(NullShapeException, "Input shape is null");
        return TopAbs_SHAPE;
    }
...
PyObject *TopoShape::getPyObject()
{
    ...
    if (_Shape.IsNull()) { prop = new TopoShapePy(new TopoShape(_Shape)); }
    else {
        TopAbs_ShapeEnum type = _Shape.ShapeType();
        switch (type) {
            case TopAbs_COMPOUND: prop = new TopoShapeCompoundPy(new TopoShape(_Shape)); break;
            case TopAbs_COMPSOLID: prop = new TopoShapeCompSolidPy(new TopoShape(_Shape)); break;
            case TopAbs_SOLID: prop = new TopoShapeSolidPy(new TopoShape(_Shape)); break;
            case TopAbs_SHELL: prop = new TopoShapeShellPy(new TopoShape(_Shape)); break;
            case TopAbs_FACE: prop = new TopoShapeFacePy(new TopoShape(_Shape)); break;
            case TopAbs_WIRE: prop = new TopoShapeWirePy(new TopoShape(_Shape)); break;
            case TopAbs_EDGE: prop = new TopoShapeEdgePy(new TopoShape(_Shape)); break;
            case TopAbs_VERTEX: prop = new TopoShapeVertexPy(new TopoShape(_Shape)); break;
            case TopAbs_SHAPE:
            default: prop = new TopoShapePy(new TopoShape(_Shape)); break;
        }
    }
...
{
    trsf.SetValues(mtrx[0][0], mtrx[0][1], mtrx[0][2], mtrx[0][3], mtrx[1][0], mtrx[1][1],
                   mtrx[1][2], mtrx[1][3], mtrx[2][0], mtrx[2][1], mtrx[2][2], mtrx[2][3]);
}
...
void TopoShape::importBinary(std::istream &str)
{
    ...
    if (shapeId <= 0 || shapeId > theShapeSet.NbShapes()) return;
...
void TopoShape::exportIges(const char *filename) const
{
    try {
        ...
        header.SetSendName(
            new TCollection_HAsciiString(Interface_Static::CVal("write.iges.header.product")));
...
{
#if OCC_VERSION_HEX >= 0x070600
    if (!BRepTools::Write(this->_Shape, encodeFilename(filename).c_str(), Standard_False,
                          Standard_False, TopTools_FormatVersion_VERSION_1))
...
void TopoShape::dump(std::ostream &out) const { BRepTools::Dump(this->_Shape, out); }
...
void TopoShape::exportFaceSet(double dev, double ca, const std::vector<App::Color> &colors,
                              std::ostream &str) const
{
    Base::InventorBuilder builder(str);
    TopExp_Explorer ex;
    std::size_t numFaces = 0;
    for (ex.Init(this->_Shape, TopAbs_FACE); ex.More(); ex.Next()) { numFaces++; }
    ...
    for (ex.Init(this->_Shape, TopAbs_FACE); ex.More(); ex.Next(), index++) {
        ...
        if (!Tools::getTriangulation(aFace, points, facets)) continue;
...
void TopoShape::exportLineSet(std::ostream &str) const
{
    ...
    for (int i = 0; i < M.Extent(); i++) {
        ...
        if (!Tools::getPolygon3D(aEdge, points)) {
            ...
            if (!Tools::getPolygonOnTriangulation(aEdge, aFace, points)) continue;
        }
...
{
    if (_Shape.IsNull()) return false;

    // Computing of CentreOfMass
    gp_Pnt pnt;

    if (_Shape.ShapeType() == TopAbs_VERTEX) { pnt = BRep_Tool::Pnt(TopoDS::Vertex(_Shape)); }
    else {
        GProp_GProps prop;
            ...
            switch (shape.ShapeType()) {
                case TopAbs_FACE: {
                    ...
                    switch (surface.GetType()) {
                        case GeomAbs_Plane: memsize += sizeof(Geom_Plane); break;
                        case GeomAbs_Cylinder: memsize += sizeof(Geom_CylindricalSurface); break;
                        case GeomAbs_Cone: memsize += sizeof(Geom_ConicalSurface); break;
                        case GeomAbs_Sphere: memsize += sizeof(Geom_SphericalSurface); break;
                        case GeomAbs_Torus: memsize += sizeof(Geom_ToroidalSurface); break;
                ...
                case TopAbs_EDGE: {
                    ...
                    switch (curve.GetType()) {
                        case GeomAbs_Line: memsize += sizeof(Geom_Line); break;
                        case GeomAbs_Circle: memsize += sizeof(Geom_Circle); break;
                        case GeomAbs_Ellipse: memsize += sizeof(Geom_Ellipse); break;
                        case GeomAbs_Hyperbola: memsize += sizeof(Geom_Hyperbola); break;
                        case GeomAbs_Parabola: memsize += sizeof(Geom_Parabola); break;
...
bool TopoShape::isNull() const { return this->_Shape.IsNull() ? true : false; }
...
bool TopoShape::analyze(bool runBopCheck, std::ostream &str) const
{
    if (!this->_Shape.IsNull()) {
        ...
        if (!aChecker.IsValid()) {
            ...
            for (std::vector<TopoDS_Shape>::iterator xp = shapes.begin(); xp != shapes.end();
                ...
                if (!aChecker.IsValid(*xp)) {
                    ...
                    while (it.More()) {
                        ...
                        switch (val) {
                            case BRepCheck_NoError: str << "No error" << std::endl; break;
                            ...
                            case BRepCheck_InvalidPointOnSurface:
                                ...
                                break;
                            case BRepCheck_No3DCurve: str << "No 3D curve" << std::endl; break;
                            ...
                            case BRepCheck_InvalidDegeneratedFlag:
                                ...
                                break;
                            case BRepCheck_FreeEdge: str << "Free edge" << std::endl; break;
                            case BRepCheck_InvalidMultiConnexity:
                                str << "Invalid multi-connexity" << std::endl;
                                break;
                            case BRepCheck_InvalidRange: str << "Invalid range" << std::endl; break;
                            case BRepCheck_EmptyWire: str << "Empty wire" << std::endl; break;
                            ...
                            case BRepCheck_SelfIntersectingWire:
                                ...
                                break;
                            case BRepCheck_NoSurface: str << "No surface" << std::endl; break;
                            case BRepCheck_InvalidWire: str << "Invalid wires" << std::endl; break;
                            ...
                            case BRepCheck_InvalidImbricationOfWires:
                                ...
                                break;
                            case BRepCheck_EmptyShell: str << "Empty shell" << std::endl; break;
                            ...
                            case BRepCheck_UnorientableShape:
                                ...
                                break;
                            case BRepCheck_NotClosed: str << "Not closed" << std::endl; break;
                            case BRepCheck_NotConnected: str << "Not connected" << std::endl; break;
                            ...
                            case BRepCheck_InvalidToleranceValue:
                                ...
                                break;
                            case BRepCheck_CheckFail: str << "Check failed" << std::endl; break;
                            default: str << "Undetermined error" << std::endl; break;
                        }
            ...
            BOPCheck.Perform();
            if (!BOPCheck.HasFaulty()) return true;
...
{
    if (this->_Shape.IsNull()) return false;
    bool closed = false;
    switch (this->_Shape.ShapeType()) {
        case TopAbs_SHELL:
        case TopAbs_WIRE:
        case TopAbs_EDGE: closed = BRep_Tool::IsClosed(this->_Shape) ? true : false; break;
        ...
        case TopAbs_FACE:
        case TopAbs_VERTEX:
        case TopAbs_SHAPE: closed = BRep_Tool::IsClosed(this->_Shape) ? true : false; break;
    }
    return closed;
}
...
{
    if (this->_Shape.IsNull()) return this->_Shape;
    if (shape.IsNull()) return this->_Shape;
...
TopoDS_Shape TopoShape::cut(const std::vector<TopoDS_Shape> &shapes, Standard_Real tolerance) const
{
    ...
    for (std::vector<TopoDS_Shape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it) {
        if (it->IsNull()) throw Base::ValueError("Tool shape is null");
        if (tolerance > 0.0)
    ...
    if (tolerance > 0.0) mkCut.SetFuzzyValue(tolerance);
    mkCut.Build();
    if (!mkCut.IsDone()) throw Base::RuntimeError("Multi cut failed");
                               ...
                               Standard_Real tolerance) const
{
    ...
    if (tolerance > 0.0) mkCommon.SetFuzzyValue(tolerance);
    mkCommon.Build();
    if (!mkCommon.IsDone()) throw Base::RuntimeError("Multi common failed");
...
{
    if (this->_Shape.IsNull()) return shape;
    if (shape.IsNull()) return this->_Shape;
...
{
    if (this->_Shape.IsNull()) Standard_Failure::Raise("Base shape is null");
#if OCC_VERSION_HEX <= 0x060800
    if (tolerance > 0.0)
        Standard_Failure::Raise("Fuzzy Booleans are not supported in this version of OCCT");
    TopoDS_Shape resShape = this->_Shape;
    if (resShape.IsNull()) throw Base::ValueError("Object shape is null");
    for (std::vector<TopoDS_Shape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it) {
        if (it->IsNull()) throw NullShapeException("Input shape is null");
        // Let's call algorithm computing a fuse operation:
        BRepAlgoAPI_Fuse mkFuse(resShape, *it);
        // Let's check if the fusion has been successful
        if (!mkFuse.IsDone()) throw Base::RuntimeError("Fusion failed");
    ...
    shapeArguments.Append(this->_Shape);
    for (std::vector<TopoDS_Shape>::const_iterator it = shapes.begin(); it != shapes.end(); ++it) {
        if (it->IsNull()) throw NullShapeException("Tool shape is null");
        if (tolerance > 0.0)
            // workaround for http://dev.opencascade.org/index.php?q=node/1056#comment-520
            shapeTools.Append(BRepBuilderAPI_Copy(*it).Shape());
        else
    ...
    if (tolerance > 0.0) mkFuse.SetFuzzyValue(tolerance);
    mkFuse.Build();
    if (!mkFuse.IsDone()) throw Base::RuntimeError("Multi fuse failed");
...
{
    if (this->_Shape.IsNull()) Standard_Failure::Raise("Base shape is null");
    if (shape.IsNull()) Standard_Failure::Raise("Tool shape is null");
#if OCC_VERSION_HEX < 0x070300
...
TopoDS_Shape TopoShape::section(TopoDS_Shape shape, Standard_Boolean approximate) const
{
    ...
    mkSection.Build();
#endif
    if (!mkSection.IsDone()) throw Base::RuntimeError("Section failed");
    return mkSection.Shape();
}

TopoDS_Shape TopoShape::section(const std::vector<TopoDS_Shape> &shapes, Standard_Real tolerance,
                                Standard_Boolean approximate) const
{
    ...
    if (tolerance > 0.0) mkSection.SetFuzzyValue(tolerance);
    mkSection.Build();
    if (!mkSection.IsDone()) throw Base::RuntimeError("Multi section failed");
                                    ...
                                    std::vector<TopTools_ListOfShape> *mapInOut) const
{
    ...
    if (tolerance > 0.0) mkGFA.SetFuzzyValue(tolerance);
#if OCC_VERSION_HEX >= 0x070000
    mkGFA.SetNonDestructive(Standard_True);
#endif
    mkGFA.Build();
    if (!mkGFA.IsDone()) throw BooleanException("MultiFusion failed");
...
{
    if (this->_Shape.IsNull()) Standard_Failure::Raise("Cannot sweep along empty spine");
    if (this->_Shape.ShapeType() != TopAbs_WIRE)
        Standard_Failure::Raise("Spine shape is not a wire");
    if (profile.IsNull()) Standard_Failure::Raise("Cannot sweep empty profile");
...
TopoDS_Shape TopoShape::makePipeShell(const TopTools_ListOfShape &profiles,
                                      ...
                                      const Standard_Boolean isFrenet, int transition) const
{
    if (this->_Shape.IsNull()) Standard_Failure::Raise("Cannot sweep along empty spine");
    if (this->_Shape.ShapeType() != TopAbs_WIRE)
        Standard_Failure::Raise("Spine shape is not a wire");
    ...
    switch (transition) {
        case 1: transMode = BRepBuilderAPI_RightCorner; break;
        case 2: transMode = BRepBuilderAPI_RoundCorner; break;
        default: transMode = BRepBuilderAPI_Transformed; break;
    }
    ...
    if (!mkPipeShell.IsReady()) throw Standard_Failure("shape is not ready to build");
...
#else
static Handle(Law_Function)
    CreateBsFunction(const Standard_Real theFirst, const Standard_Real theLast,
...
TopoDS_Shape TopoShape::makeTube(double radius, double tol, int cont, int maxdegree,
                                 int maxsegm) const
{
    // http://opencascade.blogspot.com/2009/11/surface-modeling-part3.html
...
TopoDS_Shape TopoShape::makeSweep(const TopoDS_Shape &profile, double tol, int fillMode) const
{
    ...
    if (profile.IsNull()) Standard_Failure::Raise("Cannot sweep with empty profile");
    if (profile.ShapeType() != TopAbs_EDGE) Standard_Failure::Raise("Profile shape is not an edge");
    ...
    if (hPath.IsNull()) Standard_Failure::Raise("invalid curve in path edge");
    ...
    if (hProfile.IsNull()) Standard_Failure::Raise("invalid curve in profile edge");
...
#else
    (void)isSolid;
    BRepPrimAPI_MakeTorus mkTorus(radius1, radius2, Base::toRadians<double>(angle1),
                                  Base::toRadians<double>(angle2), Base::toRadians<double>(angle3));
    return mkTorus.Solid();
#endif
}

TopoDS_Shape TopoShape::makeHelix(Standard_Real pitch, Standard_Real height, Standard_Real radius,
                                  Standard_Real angle, Standard_Boolean leftHanded,
                                  Standard_Boolean newStyle) const
{
    if (fabs(pitch) < Precision::Confusion()) Standard_Failure::Raise("Pitch of helix too small");

    if (fabs(height) < Precision::Confusion()) Standard_Failure::Raise("Height of helix too small");
    ...
    if (angle < Precision::Confusion()) {
        if (radius < Precision::Confusion()) Standard_Failure::Raise("Radius of helix too small");
        surf = new Geom_CylindricalSurface(cylAx2, radius);
    }
    else {
        angle = Base::toRadians(angle);
        if (angle < Precision::Confusion()) Standard_Failure::Raise("Angle of helix too small");
...
{
    if (pitch < Precision::Confusion()) Standard_Failure::Raise("Pitch of helix too small");

    if (height < Precision::Confusion()) Standard_Failure::Raise("Height of helix too small");
    ...
    for (unsigned long i = 0; i < wholeTurns; i++) {
        if (isCylinder) { end = line->Value(sqrt(4.0 * M_PI * M_PI + pitch * pitch) * (i + 1)); }
        else {
    ...
    if (partTurn > Precision::Confusion()) {
        if (isCylinder) { end = line->Value(sqrt(4.0 * M_PI * M_PI + pitch * pitch) * turns); }
        else {
                                        ...
                                        Standard_Boolean leftHanded) const
{
    ...
    if (breakperiod == 0) breakperiod = 1000;
    if (nbturns <= 0) Standard_Failure::Raise("Number of turns must be greater than 0");
    ...
    if (leftHanded == Standard_True) dir = gp_Vec2d(-breakperiod * 2.0 * M_PI, 1 / nbPeriods);
    Handle(Geom2d_TrimmedCurve) segm;
    TopoDS_Edge edgeOnSurf;
    BRepBuilderAPI_MakeWire mkWire;
...
TopoDS_Shape TopoShape::makeThread(Standard_Real pitch, Standard_Real depth, Standard_Real height,
                                   Standard_Real radius) const
{
    if (pitch < Precision::Confusion()) Standard_Failure::Raise("Pitch of thread too small");

    if (depth < Precision::Confusion()) Standard_Failure::Raise("Depth of thread too small");

    if (height < Precision::Confusion()) Standard_Failure::Raise("Height of thread too small");

    if (radius < Precision::Confusion()) Standard_Failure::Raise("Radius of thread too small");
...
TopoDS_Shape TopoShape::makeLoft(const TopTools_ListOfShape &profiles, Standard_Boolean isSolid,
                                 Standard_Boolean isRuled, Standard_Boolean isClosed,
                                 Standard_Integer maxDegree) const
{
    ...
    aGenerator.CheckCompatibility(
        anIsCheck); // use BRepFill_CompatibleWires on profiles. force #edges, orientation, "origin" to match.
    aGenerator.Build();
    if (!aGenerator.IsDone()) Standard_Failure::Raise("Failed to create loft face");
...
TopoDS_Shape TopoShape::revolve(const gp_Ax1 &axis, double d, Standard_Boolean isSolid) const
{
    ...
    if ((isSolid) && (BRep_Tool::IsClosed(base))
        ...
        if (base.ShapeType() == TopAbs_EDGE) {
            ...
            if (mkWire.IsDone()) { w = mkWire.Wire(); }
            else {
                convertFailed = true;
            }
        }
                                        ...
                                        bool fill) const
{
    ...
    if (!xp.More()) {
        ...
        if (xp.More()) {
            ...
            xp.Next();
            if (xp.More() == Standard_False) inputShape = inputSolid;
        }
    }
    ...
    mkOffset.PerformByJoin(inputShape, offset, tol, BRepOffset_Mode(offsetMode),
                           intersection ? Standard_True : Standard_False,
                           selfInter ? Standard_True : Standard_False, GeomAbs_JoinType(join));
#endif

    if (!mkOffset.IsDone()) Standard_Failure::Raise("BRepOffsetAPI_MakeOffsetShape not done");
    ...
    freeCheck.Perform();
    if (freeCheck.NbClosedFreeBounds() < 1) { Standard_Failure::Raise("no closed bounds"); }
    ...
    for (int index = 1; index <= freeCheck.NbClosedFreeBounds(); ++index) {
        ...
        for (xp.Init(originalWire, TopAbs_EDGE); xp.More(); xp.Next()) {
            if (!img.HasImage(xp.Current())) { Standard_Failure::Raise("no image for shape"); }
            ...
            for (listIt.Initialize(currentImage); listIt.More(); listIt.Next()) {
                if (listIt.Value().ShapeType() != TopAbs_EDGE) continue;
                edgeCount++;
        ...
        aGenerator.Build();
        if (!aGenerator.IsDone()) { Standard_Failure::Raise("ThruSections failed"); }
    ...
    if ((outputShape.ShapeType() == TopAbs_SHELL) && (outputShape.Closed())) {
        ...
        if (solidMaker.IsDone()) {
            ...
            if (BRepLib::OrientClosedSolid(temp)) outputShape = temp;
        }
    }
...
TopoDS_Shape TopoShape::makeOffset2D(double offset, short joinType, bool fill, bool allowOpenResult,
                                     bool intersection) const
{
    if (_Shape.IsNull()) throw Base::ValueError("makeOffset2D: input shape is null!");
#if OCC_VERSION_HEX < 0x060900
    if (allowOpenResult)
    ...
    if (!shapesToProcess.empty()) {
        ...
        for (TopoDS_Shape &sh : shapesToProcess) {
            switch (sh.ShapeType()) {
                ...
                case TopAbs_WIRE: {
                    ...
                    if (sh.ShapeType() == TopAbs_WIRE) { sourceWire = TopoDS::Wire(sh); }
                    else { //edge
        ...
        if (haveFaces) allowOpenResult = false;
        ...
        TopoDS_Compound compoundSourceWires;
        {
            ...
            for (TopoDS_Wire &w : sourceWires) builder.Add(compoundSourceWires, w);
        ...
        for (TopoDS_Wire &w : sourceWires) { mkOffset.AddWire(w); }
        ...
        if (!fill) {
            if (haveFaces) { wiresForMakingFaces = offsetWires; }
            else {
                for (TopoDS_Wire &w : offsetWires) shapesToReturn.push_back(w);
            }
        }
        else {
            //fill offset
            ...
            for (TopoDS_Wire &w : sourceWires)
                if (BRep_Tool::IsClosed(w)) closedWires.push_back(w);
                else
            ...
            if (!allowOpenResult || openWires.empty()) {
                ...
                for (it.Init(openWire1); it.More(); it.Next()) { mkWire.Add(it.Current()); }
        ...
        if (!wiresForMakingFaces.empty()) {
            ...
            for (TopoDS_Wire &w : wiresForMakingFaces) { mkFace.addWire(w); }
            mkFace.Build();
...
TopoDS_Shape TopoShape::makeThickSolid(const TopTools_ListOfShape &remFace, double offset,
                                       double tol, bool intersection, bool selfInter,
                                       short offsetMode, short join) const
{
#if OCC_VERSION_HEX < 0x070200
    BRepOffsetAPI_MakeThickSolid mkThick(
        this->_Shape, remFace, offset, tol, BRepOffset_Mode(offsetMode),
        intersection ? Standard_True : Standard_False, selfInter ? Standard_True : Standard_False,
        GeomAbs_JoinType(join));
#else
    BRepOffsetAPI_MakeThickSolid mkThick;
...
{
    try {
        if (this->_Shape.IsNull()) Standard_Failure::Raise("Cannot transform null shape");
...
{
    if (this->_Shape.IsNull()) Standard_Failure::Raise("Cannot convert null shape to NURBS");
...
TopoShape::replaceShape(const std::vector<std::pair<TopoDS_Shape, TopoDS_Shape>> &s) const
{
    ...
    for (it = s.begin(); it != s.end(); ++it) reshape.Replace(it->first, it->second);
...
TopoDS_Shape TopoShape::removeSplitter() const
{
    ...
    if (_Shape.ShapeType() == TopAbs_SOLID) {
        ...
        if (uniter.process()) { return uniter.getShell(); }
        else {
            Standard_Failure::Raise("Removing splitter failed");
        }
    }
        ...
        for (xp.Init(_Shape, TopAbs_SHELL, TopAbs_SOLID); xp.More(); xp.Next()) {
            ...
            ModelRefine::FaceUniter uniter(shell);
            if (uniter.process()) { builder.Add(comp, uniter.getShell()); }
        }
        // the rest
        for (xp.Init(_Shape, TopAbs_FACE, TopAbs_SHELL); xp.More(); xp.Next()) {
            if (!xp.Current().IsNull()) builder.Add(comp, xp.Current());
        }
...
void TopoShape::getDomains(std::vector<Domain> &domains) const
{
    ...
    for (TopExp_Explorer xp(this->_Shape, TopAbs_FACE); xp.More(); xp.Next()) { ++countFaces; }
...
struct MeshVertex {
    ...
    MeshVertex(Standard_Real X, Standard_Real Y, Standard_Real Z) : x(X), y(Y), z(Z), i(0) {}
    explicit MeshVertex(const Base::Vector3d &p) : x(p.x), y(p.y), z(p.z), i(0) {}

    Base::Vector3d toPoint() const { return Base::Vector3d(x, y, z); }

    bool operator<(const MeshVertex &v) const
    {
        if (fabs(this->x - v.x) >= MESH_MIN_PT_DIST) return this->x < v.x;
        if (fabs(this->y - v.y) >= MESH_MIN_PT_DIST) return this->y < v.y;
        if (fabs(this->z - v.z) >= MESH_MIN_PT_DIST) return this->z < v.z;
                                    ...
                                    std::vector<Facet> &faces) const
{
    ...
    for (std::vector<Domain>::const_iterator it = domains.begin(); it != domains.end(); ++it) {
        ...
        for (std::vector<Facet>::const_iterator jt = domain.facets.begin();
             jt != domain.facets.end(); ++jt) {
...
void TopoShape::getFaces(std::vector<Base::Vector3d> &aPoints, std::vector<Facet> &aTopo,
                         double accuracy, uint16_t /*flags*/) const
{
    if (this->_Shape.IsNull()) return;
...
void TopoShape::setFaces(const std::vector<Base::Vector3d> &Points, const std::vector<Facet> &Topo,
                         double tolerance)
{
    gp_XYZ p1, p2, p3;
    ...
    for (std::vector<Facet>::const_iterator it = Topo.begin(); it != Topo.end(); ++it) {
        if (it->I1 < ctPoints) {
            if (Vertexes[it->I1].IsNull()) Vertexes[it->I1] = CreateVertex(Points[it->I1]);
        }
        if (it->I2 < ctPoints) {
            if (Vertexes[it->I2].IsNull()) Vertexes[it->I2] = CreateVertex(Points[it->I2]);
        }
        if (it->I3 < ctPoints) {
            if (Vertexes[it->I3].IsNull()) Vertexes[it->I3] = CreateVertex(Points[it->I3]);
        }
    }
    ...
    auto CreateEdge = [&Vertexes, &Edges](uint32_t p1, uint32_t p2) {
        ...
        if (it != Edges.end()) {
            ...
            if (mkEdge.IsDone()) Edges[key2] = mkEdge.Edge();
        }
    };
    ...
    for (std::vector<Facet>::const_iterator it = Topo.begin(); it != Topo.end(); ++it) {
        if (it->I1 >= ctPoints || it->I2 >= ctPoints || it->I3 >= ctPoints) continue;
        x1 = Points[it->I1].x;
        y1 = Points[it->I1].y;
        z1 = Points[it->I1].z;
        x2 = Points[it->I2].x;
        ...
        if ((!(p1.IsEqual(p2, Precision::Approximation())))
            ...
            if (e1.IsNull() || e2.IsNull() || e3.IsNull()) continue;

            newWire = BRepBuilderAPI_MakeWire(e1, e2, e3);
            if (!newWire.IsNull()) {
                newFace = BRepBuilderAPI_MakeFace(newWire);
                if (!newFace.IsNull()) BuildTool.Add(aComp, newFace);
            }
        }
    }
                                     ...
                                     std::vector<Line> &lines) const
{
    ...
    for (TopExp_Explorer exp(shape, TopAbs_EDGE); exp.More(); exp.Next()) {
        ...
        if (!Tools::getPolygon3D(aEdge, points)) {
            ...
            if (!Part::Tools::getPolygonOnTriangulation(aEdge, aFace, points)) continue;
        }
...
void TopoShape::getLines(std::vector<Base::Vector3d> &vertices, std::vector<TopoShape::Line> &lines,
                         double /*Accuracy*/, uint16_t /*flags*/) const
{
    getLinesFromSubShape(_Shape, vertices, lines);
}

void TopoShape::getPoints(std::vector<Base::Vector3d> &Points, std::vector<Base::Vector3d> &Normals,
                          double Accuracy, uint16_t /*flags*/) const
{
    ...
    for (TopExp_Explorer xp(_Shape, TopAbs_EDGE, TopAbs_FACE); xp.More(); xp.Next()) {
        ...
        GCPnts_UniformAbscissa discretizer(curve, lateralDistance, curve.FirstParameter(),
                                           curve.LastParameter());
...
TopoDS_Shape TopoShape::makeShell(const TopoDS_Shape &input) const
{
    ...
    try {
        for (it.Init(input, TopAbs_FACE); it.More(); it.Next()) {
            if (!it.Current().IsNull()) builder.Add(shell, it.Current());
        }
        ...
        if (shape.IsNull()) return input;

        if (shape.ShapeType() != TopAbs_SHELL) return input;
...
#define _HANDLE_NULL_SHAPE(_msg, _throw)                                                           \
    do {                                                                                           \
        if (_throw) { FC_THROWM(NullShapeException, _msg); }                                       \
        FC_WARN(_msg);                                                                             \
    } while (0)
...
TopoShape &TopoShape::makeWires(const TopoShape &shape, const char *op, bool fix, double tol)
{
    ...
    if (shape.isNull()) HANDLE_NULL_INPUT;
    ...
    for (int i = 1; i <= anIndices.Extent(); ++i) edge_list.emplace_back(anIndices.FindKey(i));
...
TopoShape &TopoShape::makeCompound(const std::vector<TopoShape> &shapes, const char *op, bool force)
{
    ...
    if (shapes.empty()) HANDLE_NULL_INPUT;
...
TopoShape &TopoShape::makeFace(const std::vector<TopoShape> &shapes, const char *op,
                               const char *maker)
{
    (void)op;
    _Shape.Nullify();
...
bool TopoShape::isInfinite() const
{
    ...
    try {
        ...
        if (Precision::IsInfinite(xMax - xMin)) return true;
        if (Precision::IsInfinite(yMax - yMin)) return true;
        if (Precision::IsInfinite(zMax - zMin)) return true;
...
bool TopoShape::isCoplanar(const TopoShape &other, double tol) const
{
    ...
    if (_Shape.IsEqual(other._Shape)) return true;
    gp_Pln pln1, pln2;
    if (!findPlane(pln1, tol) || !other.findPlane(pln2, tol)) return false;
    if (tol < 0.0) tol = Precision::Confusion();
    return pln1.Position().IsCoplanar(pln2.Position(), tol, tol);
}

bool TopoShape::_makeTransform(const TopoShape &shape, const Base::Matrix4D &rclTrf, const char *op,
                               bool checkScale, bool copy)
{
    if (checkScale) {
        try {
...
TopoShape &TopoShape::makeGTransform(const TopoShape &shape, const Base::Matrix4D &rclTrf,
                                     const char *op, bool copy)
{

...
std::string BSplineCurve2dPy::representation() const { return "<BSplineCurve2d object>"; }
...
PyObject *BSplineCurve2dPy::insertKnot(PyObject *args)
{
    double U, tol = 0.0;
    int M = 1;
    if (!PyArg_ParseTuple(args, "d|idO!", &U, &M, &tol)) return nullptr;
...
PyObject *BSplineCurve2dPy::movePoint(PyObject *args)
{
    ...
    int index1, index2;
    PyObject *pnt;
    if (!PyArg_ParseTuple(args, "dO!ii", &U, Base::Vector2dPy::type_object(), &pnt, &index1,
                          &index2))
        return nullptr;
    try {
...
Py::List BSplineCurve2dPy::getKnotSequence() const
{
    ...
    if (curve->IsPeriodic()) {
        ...
        for (int i = 1; i <= curve->NbKnots(); i++) m += (int)curve->Multiplicity(i);
    }
...
PyObject *BSplineCurve2dPy::approximate(PyObject *args, PyObject *kwds)
{
    ...
    try {
        ...
        if (weight1 || weight2 || weight3) {
            ...
            if (!(c == GeomAbs_C0 || c == GeomAbs_C1 || c == GeomAbs_C2)) { c = GeomAbs_C2; }
...
PyObject *BSplineCurve2dPy::getCardinalSplineTangents(PyObject *args, PyObject *kwds)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "Od", kwds_interp1, &pts, &parameter)) {
        ...
        Py::List vec;
        for (gp_Vec2d it : tangents) { vec.append(Base::Vector2dPy::create(it.X(), it.Y())); }
...
PyObject *BSplineCurve2dPy::interpolate(PyObject *args, PyObject *kwds)
{
    ...
    static char *kwds_interp[] = {"Points",         "PeriodicFlag", "Tolerance",
                                  "InitialTangent", "FinalTangent", "Tangents",
                                  "TangentFlags",   "Parameters",   nullptr};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|O!dO!O!OOO", kwds_interp, &obj, &PyBool_Type,
                                     &periodic, &tol3d, Base::Vector2dPy::type_object(), &t1,
                                     Base::Vector2dPy::type_object(), &t2, &ts, &fl, &par))
        return nullptr;
    ...
    try {
        ...
        if (parameters.IsNull()) {
            aBSplineInterpolation.reset(
                new Geom2dAPI_Interpolate(interpolationPoints, Base::asBoolean(periodic), tol3d));
        }
        else {
            aBSplineInterpolation.reset(new Geom2dAPI_Interpolate(
                interpolationPoints, parameters, Base::asBoolean(periodic), tol3d));
        }
...
PyObject *BSplineCurve2dPy::buildFromPolesMultsKnots(PyObject *args, PyObject *keywds)
{
    ...
    int sum_of_mults = 0;
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|OOO!iO", kwlist, &poles, &mults, &knots,
                                     &PyBool_Type, &periodic, &degree, &weights))
        return nullptr;
    try {
        ...
        Handle(Geom2d_BSplineCurve) spline = new Geom2d_BSplineCurve(
            occpoles, occweights, occknots, occmults, degree, Base::asBoolean(periodic));
        if (!spline.IsNull()) {
            this->getGeom2dBSplineCurvePtr()->setHandle(spline);
            Py_Return;
        }
...
{
    PyObject *c;
    if (!PyArg_ParseTuple(args, "O!", &BSplineCurve2dPy::Type, &c)) return nullptr;
...
PyObject *BSplineCurve2dPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

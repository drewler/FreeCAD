...
class _CompoundFilter:
    ...
    def __init__(self, obj):
        obj.addProperty(
            "App::PropertyLink", "Base", "CompoundFilter", "Compound to be filtered"
        )
    ...
    def execute(self, obj):
        ...
        if len(rst) == 0:
            ...
            if not obj.Base.Shape.isNull():
                scale = (
                    obj.Base.Shape.BoundBox.DiagonalLength
                    / math.sqrt(3)
                    / math.sqrt(len(shps))
                )
    ...
    try:
        ...
        if not _nullShapeShape:
            ...
            shapePath = os.path.join(
                os.path.dirname(__file__), "shapes", "empty-shape.brep"
            )
            f = open(shapePath)

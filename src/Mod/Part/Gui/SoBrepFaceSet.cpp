...
public:
    ...
    void render(SoGLRenderAction *action, const SoGLCoordinateElement *const vertexlist,
                const int32_t *vertexindices, int num_vertexindices, const int32_t *partindices,
                int num_partindices, const SbVec3f *normals, const int32_t *normindices,
        ...
        switch (selaction->getType()) {
            ...
            case Gui::SoSelectionElementAction::None:
                if (selaction->isSecondary()) {
                    if (Gui::SoFCSelectionRoot::removeActionContext(action, this)) touch();
                }
                else {
                    SelContextPtr ctx =
                        Gui::SoFCSelectionRoot::getActionContext(action, this, selContext, false);
            ...
            case Gui::SoSelectionElementAction::Color:
                if (selaction->isSecondary()) {
                    ...
                    if (colors.begin()->first.empty()
                        ...
                        if (ctx->setColors(selaction->getColors(), element)) touch();
                    }
                }
                return;
            ...
            case Gui::SoSelectionElementAction::Append: {
                ...
                if (selaction->getType() == Gui::SoSelectionElementAction::Append) {
                    ...
                    selCounter.checkAction(selaction, ctx);
                    ctx->selectionColor = selaction->getColor();
                    if (ctx->isSelectAll()) ctx->selectionIndex.clear();
                    if (ctx->selectionIndex.insert(index).second) touch();
                }
                else {
                    auto ctx =
                        Gui::SoFCSelectionRoot::getActionContext(action, this, selContext, false);
                    if (ctx && ctx->removeIndex(index)) touch();
                }
                break;
            }
            default: break;
...
void SoBrepFaceSet::GLRender(SoGLRenderAction *action)
{
    ...
    if (ctx->coordIndex.getNum() < 3) return;

    SelContextPtr ctx2;
    SelContextPtr ctx =
        Gui::SoFCSelectionRoot::getRenderContext<SelContext>(this, selContext, ctx2);
    if (ctx2 && ctx2->selectionIndex.empty()) return;
    ...
    if (ctx2 && ctx2->selectionIndex.size()) { renderSelection(action, ctx2, false); }
    else {
        ...
        this->getVertexData(state, coords, normals, cindices, nindices, tindices, mindices,
        ...
        renderShape(state, vboAvailable, static_cast<const SoGLCoordinateElement *>(coords),
                    cindices, numindices, pindices, numparts, normals, nindices, &mb, mindices, &tb,
                    tindices, nbind, mbind, doTextures ? 1 : 0);
...
void SoBrepFaceSet::GLRender(SoGLRenderAction *action)
{
    ...
    if (ctx2 && ctx2->selectionIndex.empty()) return;
    if (selContext2->checkGlobal(ctx)) ctx = selContext2;
    if (ctx && (ctx->selectionIndex.empty() && ctx->highlightIndex < 0)) ctx.reset();
    ...
    if (!pushed) {
        ...
        if (ctx && ctx->highlightIndex == INT_MAX) {
            if (ctx->selectionIndex.empty() || ctx->isSelectAll()) {
                ...
                if (!action->isRenderingDelayedPaths()) renderSelection(action, ctx);
                if (ctx2) {
                    ctx2->selectionColor = ctx->highlightColor;
                    renderSelection(action, ctx2);
                }
        ...
        if (!action->isRenderingDelayedPaths()) renderHighlight(action, ctx);
    ...
    if (this->shouldGLRender(action)) {
        ...
        renderShape(action, hasVBO, static_cast<const SoGLCoordinateElement *>(coords), cindices,
                    numindices, pindices, numparts, normals, nindices, &mb, mindices, &tb, tindices,
                    nbind, mbind, doTextures ? 1 : 0);
...
bool SoBrepFaceSet::overrideMaterialBinding(SoGLRenderAction *action, SelContextPtr ctx,
                                            SelContextPtr ctx2)
{
    ...
    if ((mb == SoMaterialBindingElement::OVERALL
        ...
        if (ctx && (ctx->isSelectAll() || ctx->isHighlightAll())) {
            ...
            if (!partialRender) packedColors.push_back(diffuseColor);
            else {
        ...
        SoLazyElement::setPacked(state, this, packedColors.size(), &packedColors[0],
                                 hasTransparency);
        SoTextureEnabledElement::set(state, this, false);
...
void SoBrepFaceSet::getBoundingBox(SoGetBoundingBoxAction *action)
{
    ...
    if (ctx2->selectionIndex.empty()) return;
...
void SoBrepFaceSet::generatePrimitives(SoAction *action)
{
    ...
    this->getVertexData(state, coords, normals, cindices, nindices, tindices, mindices, numindices,
    ...
    if (doTextures) {
        ...
        // the if-check? 20020110 mortene.
            ...
            if (!tindices) tindices = cindices;
        }
    }

    if (nbind == PER_VERTEX_INDEXED && !nindices) { nindices = cindices; }
    if (mbind == PER_VERTEX_INDEXED && !mindices) { mindices = cindices; }
...
{
    if (!ctx || ctx->highlightIndex < 0) return;
    ...
    if (id != INT_MAX && id >= this->partIndex.getNum()) {
        ...
        // normals
        if (nbind == PER_VERTEX_INDEXED) nindices = &(nindices[start]);
        ...
        renderShape(action, false, static_cast<const SoGLCoordinateElement *>(coords),
                    &(cindices[start]), length, &(pindices[id]), 1, normals, nindices, &mb,
                    mindices, &tb, tindices, nbind, mbind, doTextures);
    }
    state->pop();
...
{
    if (!ctx || ctx->selectionIndex.empty()) return;
    ...
    for (auto id : ctx->selectionIndex) {
        ...
        if (id >= 0 && id == ctx->highlightIndex) continue;
        ...
        if (nbind == PER_VERTEX_INDEXED) nindices_s = &(nindices[start]);
        else if (nbind == PER_VERTEX)
        ...
        renderShape(action, false, static_cast<const SoGLCoordinateElement *>(coords),
                    &(cindices[start]), length, &(pindices[id]), numparts, normals_s, nindices_s,
                    &mb, mindices, &tb, tindices, nbind, mbind, doTextures ? 1 : 0);
    }
    if (push) {
        state->pop();
...
void SoBrepFaceSet::VBO::render(
    SoGLRenderAction *action, const SoGLCoordinateElement *const vertexlist,
    const int32_t *vertexindices, int num_indices, const int32_t *partindices, int num_partindices,
    const SbVec3f *normals, const int32_t *normalindices, SoMaterialBundle *const materials,
    const int32_t *matindices, SoTextureCoordinateBundle *const texcoords,
    const int32_t *texindices, const int nbind, const int mbind, SbBool texture)
{
    (void)texcoords;
    (void)texindices;
    (void)texture;
    ...
    if ((buf.vertex_array_size != (sizeof(float) * num_indices * 10))
        ...
        if ((buf.vertex_array_size != 0) && (buf.index_array_size != 0)) buf.updateVbo = true;
    }
    ...
    if (!buf.vboLoaded || buf.updateVbo) {
        ...
        while (viptr + 2 < viendptr) {
            ...
            if (mbind == PER_PART) {
                ...
                if (trinr == 0) materials->send(*matindices++, true);
            }
            else if (mbind == PER_VERTEX || mbind == PER_FACE) {
                materials->send(matnr++, true);
            }
            ...
            if (mbind == PER_VERTEX) materials->send(matnr++, true);
            else if (mbind == PER_VERTEX_INDEXED)
                materials->send(*matindices++, true);
            ...
            if (nbind == PER_VERTEX_INDEXED) normalindices++;
            ...
            ((SbVec3f *)(cur_coords3d + v1))
                ->getValue(vertex_array[indice + 0], vertex_array[indice + 1],
                           vertex_array[indice + 2]);

            ((SbVec3f *)(mynormal1))
                ->getValue(vertex_array[indice + 3], vertex_array[indice + 4],
        ...
        glBufferDataARB(GL_ARRAY_BUFFER_ARB, sizeof(float) * indice, vertex_array,
                        GL_DYNAMIC_DRAW_ARB);

        glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, buf.myvbo[1]);
        glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, sizeof(GLuint) * this->indice_array,
                        &index_array[0], GL_DYNAMIC_DRAW_ARB);
...
void SoBrepFaceSet::renderShape(
    SoGLRenderAction *action, SbBool hasVBO, const SoGLCoordinateElement *const vertexlist,
    const int32_t *vertexindices, int num_indices, const int32_t *partindices, int num_partindices,
    const SbVec3f *normals, const int32_t *normalindices, SoMaterialBundle *const materials,
    const int32_t *matindices, SoTextureCoordinateBundle *const texcoords,
    const int32_t *texindices, const int nbind, const int mbind, SbBool texture)
{
    ...
    if (hasVBO) {
        ...
        PRIVATE(this)->render(action, vertexlist, vertexindices, num_indices, partindices,
                              num_partindices, normals, normalindices, materials, matindices,
                              texcoords, texindices, nbinding, mbind, texture);
        return;
    }
    ...
    while (viptr + 2 < viendptr) {
        ...
        /* vertex 1 *********************************************************/
        if (mbind == PER_PART) {
            if (trinr == 0) materials->send(matnr++, true);
        }
        else if (mbind == PER_PART_INDEXED) {
            if (trinr == 0) materials->send(*matindices++, true);
        }
        ...
        if (texture) {
            texcoords->send(texindices ? *texindices++ : texidx++, vertexlist->get3(v1),
                            *currnormal);
        }
        ...
        if (texture) {
            texcoords->send(texindices ? *texindices++ : texidx++, vertexlist->get3(v2),
                            *currnormal);
        }
        ...
        if (texture) {
            texcoords->send(texindices ? *texindices++ : texidx++, vertexlist->get3(v3),
                            *currnormal);
        }
...
SoDetail *SoBrepFaceSet::createTriangleDetail(SoRayPickAction *action, const SoPrimitiveVertex *v1,
                                              const SoPrimitiveVertex *v2,
                                              const SoPrimitiveVertex *v3, SoPickedPoint *pp)
{
...
SoBrepFaceSet::Binding SoBrepFaceSet::findMaterialBinding(SoState *const state) const
{
    ...
    switch (matbind) {
        case SoMaterialBindingElement::OVERALL: binding = OVERALL; break;
        case SoMaterialBindingElement::PER_VERTEX: binding = PER_VERTEX; break;
        case SoMaterialBindingElement::PER_VERTEX_INDEXED: binding = PER_VERTEX_INDEXED; break;
        case SoMaterialBindingElement::PER_PART: binding = PER_PART; break;
        case SoMaterialBindingElement::PER_FACE: binding = PER_FACE; break;
        case SoMaterialBindingElement::PER_PART_INDEXED: binding = PER_PART_INDEXED; break;
        case SoMaterialBindingElement::PER_FACE_INDEXED: binding = PER_FACE_INDEXED; break;
        default: break;
    }
    return binding;
}
...
SoBrepFaceSet::Binding SoBrepFaceSet::findNormalBinding(SoState *const state) const
{
    ...
    switch (normbind) {
        case SoNormalBindingElement::OVERALL: binding = OVERALL; break;
        case SoNormalBindingElement::PER_VERTEX: binding = PER_VERTEX; break;
        case SoNormalBindingElement::PER_VERTEX_INDEXED: binding = PER_VERTEX_INDEXED; break;
        case SoNormalBindingElement::PER_PART: binding = PER_PART; break;
        case SoNormalBindingElement::PER_FACE: binding = PER_FACE; break;
        case SoNormalBindingElement::PER_PART_INDEXED: binding = PER_PART_INDEXED; break;
        case SoNormalBindingElement::PER_FACE_INDEXED: binding = PER_FACE_INDEXED; break;
        default: break;
    }
    return binding;
}

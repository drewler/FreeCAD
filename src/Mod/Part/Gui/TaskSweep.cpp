...
public:
    ...
    {
    public:
        EdgeSelection() : Gui::SelectionFilterGate(nullPointer()) {}
        bool allow(App::Document * /*pDoc*/, App::DocumentObject *pObj,
                   const char *sSubName) override
        {
            if (!sSubName || sSubName[0] == '\0') {
                ...
                if (topoShape.isNull()) { return false; }
                const TopoDS_Shape shape = topoShape.getShape();
                if (!shape.IsNull()) {
                    // a single edge
                    if (shape.ShapeType() == TopAbs_EDGE) { return true; }
                    // a single wire
                    if (shape.ShapeType() == TopAbs_WIRE) { return true; }
...
SweepWidget::SweepWidget(QWidget *parent) : d(new Private())
{
    Q_UNUSED(parent);
    ...
    connect(d->ui.selector->availableTreeWidget(),
            SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)), this,
            SLOT(onCurrentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)));
    connect(d->ui.selector->selectedTreeWidget(),
            SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)), this,
            SLOT(onCurrentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)));
...
{
    if (d->ui.buttonPath->isChecked()) return false;
    ...
    if (selobjs.size() == 1) {
        ...
        if (!topoShape.isNull()) {
            for (std::vector<std::string>::const_iterator it = subnames.begin();
                 it != subnames.end(); ++it) {
                subShapes.push_back(Part::Feature::getTopoShape(docobj, subnames[0].c_str(),
                                                                true /*need element*/));
            }
    ...
    if (d->ui.checkSolid->isChecked()) solid = QString::fromLatin1("True");
    else
        solid = QString::fromLatin1("False");

    if (d->ui.checkFrenet->isChecked()) frenet = QString::fromLatin1("True");
    else
    ...
    if (count < 1) {
        QMessageBox::critical(this, tr("Too few elements"),
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (name == QLatin1String(spineObject.c_str())) {
            QMessageBox::critical(this, tr("Wrong selection"),
    ...
    try {
        ...
        cmd = QString::fromLatin1("App.getDocument('%5').addObject('Part::Sweep','Sweep')\n"
                  ...
                  .arg(list, QLatin1String(selection.c_str()), solid, frenet,
                       QString::fromLatin1(d->document.c_str()));

        Gui::Document *doc = Gui::Application::Instance->getDocument(d->document.c_str());
        if (!doc) throw Base::RuntimeError("Document doesn't exist anymore");
    ...
    if (on) {
        ...
        if (matchEdge) {
            ...
            if (!isPathValid(result.front())) {
                QMessageBox::critical(this, tr("Sweep path"),
...
TaskSweep::TaskSweep() : label(nullptr)
{
    ...
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_Sweep"),
                                         widget->windowTitle(), true, nullptr);
    taskbox->groupLayout()->addWidget(widget);
    Content.push_back(taskbox);
}

TaskSweep::~TaskSweep() { delete label; }
...
bool TaskSweep::accept() { return widget->accept(); }

bool TaskSweep::reject() { return widget->reject(); }

...
void ViewProviderPart::applyTransparency(const float &transparency, std::vector<App::Color> &colors)
{
    if (transparency != 0.0) {
        ...
        for (j = colors.begin(); j != colors.end(); ++j) {
            ...
            if (j->a == 0.0) j->a = transparency / 100.0; // transparency comes in percent
        }
    }
}

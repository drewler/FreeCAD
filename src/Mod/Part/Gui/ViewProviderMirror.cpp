...
ViewProviderMirror::~ViewProviderMirror() { pcEditNode->unref(); }
...
bool ViewProviderMirror::onDelete(const std::vector<std::string> &)
{
    ...
    if (pSource) Gui::Application::Instance->showViewProvider(pSource);
...
ViewProviderFillet::ViewProviderFillet() { sPixmap = "Part_Fillet"; }
...
bool ViewProviderFillet::onDelete(const std::vector<std::string> &)
{
    ...
    if (pBase) Gui::Application::Instance->showViewProvider(pBase);
...
ViewProviderChamfer::ViewProviderChamfer() { sPixmap = "Part_Chamfer"; }
    ...
    if (prop->getTypeId() == Part::PropertyShapeHistory::getClassTypeId()) {
        ...
        if (objBase) {
            ...
            if (vpBase) {
                ...
                colCham.resize(
                    chamMap.Extent(),
                    static_cast<PartGui::ViewProviderPart *>(vpBase)->ShapeColor.getValue());
                applyTransparency(
...
ViewProviderRevolution::ViewProviderRevolution() { sPixmap = "Part_Revolve"; }
...
ViewProviderLoft::ViewProviderLoft() { sPixmap = "Part_Loft"; }
...
bool ViewProviderLoft::onDelete(const std::vector<std::string> &) { return true; }

// ---------------------------------------

PROPERTY_SOURCE(PartGui::ViewProviderSweep, PartGui::ViewProviderPart)

ViewProviderSweep::ViewProviderSweep() { sPixmap = "Part_Sweep"; }
...
std::vector<App::DocumentObject *> ViewProviderSweep::claimChildren() const
{
    ...
    if (obj->Spine.getValue()) children.push_back(obj->Spine.getValue());
    return children;
}

bool ViewProviderSweep::onDelete(const std::vector<std::string> &) { return true; }

// ---------------------------------------

PROPERTY_SOURCE(PartGui::ViewProviderOffset, PartGui::ViewProviderPart)

ViewProviderOffset::ViewProviderOffset() { sPixmap = "Part_Offset"; }
...
bool ViewProviderOffset::setEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
        ...
        if (dlg && !offsetDlg) {
            if (dlg->canClose()) Gui::Control().closeDialog();
            else
                return false;
        }
        ...
        if (offsetDlg) Gui::Control().showDialog(offsetDlg);
        else
...
ViewProviderThickness::ViewProviderThickness() { sPixmap = "Part_Thickness"; }
...
bool ViewProviderThickness::setEdit(int ModNum)
{
    if (ModNum == ViewProvider::Default) {
        ...
        // start the edit dialog
        if (thicknessDlg) Gui::Control().showDialog(thicknessDlg);
        else
...
ViewProviderRefine::ViewProviderRefine() { sPixmap = "Part_Refine_Shape"; }

...
void ViewProviderBoolean::updateData(const App::Property *prop)
{
    ...
    if (prop->getTypeId() == Part::PropertyShapeHistory::getClassTypeId()) {
        ...
        Part::Feature *objBase =
            dynamic_cast<Part::Feature *>(Part::Feature::getShapeOwner(objBool->Base.getValue()));
        Part::Feature *objTool =
            dynamic_cast<Part::Feature *>(Part::Feature::getShapeOwner(objBool->Tool.getValue()));
        if (objBase && objTool) {
...
bool ViewProviderBoolean::onDelete(const std::vector<std::string> &)
{
    ...
    if (pTool) Gui::Application::Instance->showViewProvider(pTool);
...
std::vector<App::DocumentObject *> ViewProviderMultiFuse::claimChildren() const
{
    return std::vector<App::DocumentObject *>(
        static_cast<Part::MultiFuse *>(getObject())->Shapes.getValues());
}
...
void ViewProviderMultiFuse::updateData(const App::Property *prop)
{
    ...
    if (prop->getTypeId() == Part::PropertyShapeHistory::getClassTypeId()) {
        ...
        // If the view provider has set a transparency then override the values
        // of the input shapes
        if (Transparency.getValue() > 0) { applyTransparency(Transparency.getValue(), colBool); }
...
bool ViewProviderMultiFuse::onDelete(const std::vector<std::string> &)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = pShapes.begin(); it != pShapes.end();
         ++it) {
        if (*it) Gui::Application::Instance->showViewProvider(*it);
    }

    return true;
}

bool ViewProviderMultiFuse::canDragObjects() const { return true; }
...
bool ViewProviderMultiFuse::canDropObjects() const { return true; }
...
{
    return std::vector<App::DocumentObject *>(
        static_cast<Part::MultiCommon *>(getObject())->Shapes.getValues());
}
...
bool ViewProviderMultiCommon::canDragObjects() const { return true; }
...
bool ViewProviderMultiCommon::canDropObjects() const { return true; }

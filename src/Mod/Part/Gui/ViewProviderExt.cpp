...
App::PropertyQuantityConstraint::Constraints ViewProviderPartExt::angDeflectionRange = {1.0, 180.0,
                                                                                        0.05};
const char *ViewProviderPartExt::LightingEnums[] = {"One side", "Two side", nullptr};
const char *ViewProviderPartExt::DrawStyleEnums[] = {"Solid", "Dashed", "Dotted", "Dashdot",
                                                     nullptr};
...
ViewProviderPartExt::ViewProviderPartExt()
{
    ...
    ADD_PROPERTY_TYPE(LineColor, (lmat.diffuseColor), osgroup, App::Prop_None,
                      "Set object line color.");
    ADD_PROPERTY_TYPE(PointColor, (vmat.diffuseColor), osgroup, App::Prop_None,
                      "Set object point color");
    ADD_PROPERTY_TYPE(PointColorArray, (PointColor.getValue()), osgroup, App::Prop_None,
                      "Object point color array.");
    ADD_PROPERTY_TYPE(DiffuseColor, (ShapeColor.getValue()), osgroup, App::Prop_None,
                      "Object diffuse color.");
    ADD_PROPERTY_TYPE(LineColorArray, (LineColor.getValue()), osgroup, App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(
        AngularDeflection, (28.65), osgroup, App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(DrawStyle, ((long int)0), osgroup, App::Prop_None,
...
void ViewProviderPartExt::onChanged(const App::Property *prop)
{
    ...
    if (prop == &Deviation) {
        if (isUpdateForced() || Visibility.getValue()) updateVisual();
        else
            VisualTouched = true;
    }
    ...
    if (prop == &LineWidth) { pcLineStyle->lineWidth = LineWidth.getValue(); }
        ...
        if (c != LineMaterial.getValue().diffuseColor) LineMaterial.setDiffuseColor(c);
        LineColorArray.setValue(LineColor.getValue());
    }
    else if (prop == &PointColor) {
        const App::Color &c = PointColor.getValue();
        pcPointMaterial->diffuseColor.setValue(c.r, c.g, c.b);
        if (c != PointMaterial.getValue().diffuseColor) PointMaterial.setDiffuseColor(c);
        PointColorArray.setValue(PointColor.getValue());
    }
    else if (prop == &LineMaterial) {
        const App::Material &Mat = LineMaterial.getValue();
        if (LineColor.getValue() != Mat.diffuseColor) LineColor.setValue(Mat.diffuseColor);
        ...
        if (PointColor.getValue() != Mat.diffuseColor) PointColor.setValue(Mat.diffuseColor);
        ...
        if (Lighting.getValue() == 0) pShapeHints->vertexOrdering = SoShapeHints::UNKNOWN_ORDERING;
        else
            pShapeHints->vertexOrdering = SoShapeHints::COUNTERCLOCKWISE;
    }
    else if (prop == &DrawStyle) {
        if (DrawStyle.getValue() == 0) pcLineStyle->linePattern = 0xffff;
...
{
    if (strcmp("Flat Lines", ModeName) == 0) setDisplayMaskMode("Flat Lines");
...
void ViewProviderPartExt::setHighlightedFaces(const std::vector<App::Material> &colors)
{
    ...
    if (size > 1 && size == this->faceset->partIndex.getNum()) {
        ...
        for (int i = 0; i < size; i++) {
            dc[i].setValue(colors[i].diffuseColor.r, colors[i].diffuseColor.g,
                           colors[i].diffuseColor.b);
            ac[i].setValue(colors[i].ambientColor.r, colors[i].ambientColor.g,
                           colors[i].ambientColor.b);
            sc[i].setValue(colors[i].specularColor.r, colors[i].specularColor.g,
                           colors[i].specularColor.b);
            ec[i].setValue(colors[i].emissiveColor.r, colors[i].emissiveColor.g,
        ...
        pcFaceBind->value = SoMaterialBinding::OVERALL;
        pcShapeMaterial->diffuseColor.setValue(colors[0].diffuseColor.r, colors[0].diffuseColor.g,
                                               colors[0].diffuseColor.b);
        pcShapeMaterial->ambientColor.setValue(colors[0].ambientColor.r, colors[0].ambientColor.g,
                                               colors[0].ambientColor.b);
        pcShapeMaterial->specularColor.setValue(
            colors[0].specularColor.r, colors[0].specularColor.g, colors[0].specularColor.b);
        pcShapeMaterial->emissiveColor.setValue(
            colors[0].emissiveColor.r, colors[0].emissiveColor.g, colors[0].emissiveColor.b);
    }
}
...
std::map<std::string, App::Color> ViewProviderPartExt::getElementColors(const char *element) const
{
    ...
    if (boost::starts_with(element, "Face")) {
        ...
        if (element[4] == '*') {
            ...
            if (idx > 0 && idx <= size) ret[element] = DiffuseColor[idx - 1];
            else
                ret[element] = ShapeColor.getValue();
            if (size == 1) ret[element].a = Transparency.getValue() / 100.0f;
        }
    }
        ...
        if (element[4] == '*') {
            ...
            int idx = atoi(element + 4);
            if (idx > 0 && idx <= size) ret[element] = LineColorArray[idx - 1];
            else
        ...
        if (element[5] == '*') {
            ...
            if (idx > 0 && idx <= size) ret[element] = PointColorArray[idx - 1];
            else
...
void ViewProviderPartExt::setHighlightedEdges(const std::vector<App::Color> &colors)
{
    ...
    if (size > 1) {
        ...
        for (int i = 0; i < numindices; ++i) {
            if (cindices[i] < 0) {
                ca[linecount].setValue(colors[linecount].r, colors[linecount].g,
                                       colors[linecount].b);
                linecount++;
                if (linecount >= size) break;
            }
        }
...
void ViewProviderPartExt::setHighlightedPoints(const std::vector<App::Color> &colors)
{
    ...
    if (size > 1) {
        ...
        for (int i = 0; i < size; ++i) ca[i].setValue(colors[i].r, colors[i].g, colors[i].b);
...
void ViewProviderPartExt::unsetHighlightedPoints() { PointColorArray.touch(); }
...
void ViewProviderPartExt::updateVisual()
{
    ...
    try {
        ...
        // should be displayed.
        if (deflection < gp::Resolution()) deflection = Precision::Confusion();

        // create or use the mesh on the data structure
        Standard_Real AngDeflectionRads = AngularDeflection.getValue() / 180.0 * M_PI;
        BRepMesh_IncrementalMesh(cShape, deflection, Standard_False, AngDeflectionRads,
                                 Standard_True);
        ...
        for (int i = 0; i < numNorms; i++) norms[i] = SbVec3f(0.0, 0.0, 0.0);
        ...
        for (int i = 1; i <= faceMap.Extent(); i++, ii++) {
            ...
            if (mesh.IsNull()) { mesh = Part::Tools::triangulationOfFace(actFace); }
            if (mesh.IsNull()) {
                parts[ii] = 0;
                continue;
            }
...
#endif
            if (NormalsFromUV) Part::Tools::getPointNormals(actFace, mesh, Normals);
            ...
            for (int g = 1; g <= nbTriInFace; g++) {
                ...
                // set the vertices
                verts[faceNodeOffset + N1 - 1].setValue((float)(V1.X()), (float)(V1.Y()),
                                                        (float)(V1.Z()));
                verts[faceNodeOffset + N2 - 1].setValue((float)(V2.X()), (float)(V2.Y()),
                                                        (float)(V2.Z()));
                verts[faceNodeOffset + N3 - 1].setValue((float)(V3.X()), (float)(V3.Y()),
                                                        (float)(V3.Z()));
        ...
        for (int i = 0; i < vertexMap.Extent(); i++) {
            ...
            verts[faceNodeOffset + i].setValue((float)(pnt.X()), (float)(pnt.Y()),
                                               (float)(pnt.Z()));
        }

        // normalize all normals
        for (int i = 0; i < numNorms; i++) norms[i].normalize();

        std::vector<int32_t> lineSetCoords;
        for (std::map<int, std::vector<int32_t>>::iterator it = lineSetMap.begin();
             it != lineSetMap.end(); ++it) {
...
void ViewProviderPartExt::forceUpdate(bool enable)
{
    if (enable) {
        if (++forceUpdateCount == 1) {
            if (!isShow() && VisualTouched) updateVisual();
        }
    }

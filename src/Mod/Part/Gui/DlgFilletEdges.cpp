...
Qt::ItemFlags FilletRadiusModel::flags(const QModelIndex &index) const
{
    ...
    if (index.column() == 0) fl = fl | Qt::ItemIsUserCheckable;
    return fl;
}
...
bool FilletRadiusModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if (role == Qt::CheckStateRole) { Q_EMIT toggleCheckState(index); }
    return ok;
}
...
public:
    ...
    bool allow(App::Document * /*pDoc*/, App::DocumentObject *pObj, const char *sSubName) override
    {
        ...
        if (allowEdge) return element.substr(0, 4) == "Edge";
        else
...
DlgFilletEdges::DlgFilletEdges(FilletType type, Part::FilletBase *fillet, QWidget *parent,
                               Qt::WindowFlags fl)
    : QWidget(parent, fl), ui(new Ui_DlgFilletEdges()), d(new Private())
{
    ...
    d->connectApplicationDeletedObject = App::GetApplication().signalDeletedObject.connect(
        boost::bind(&DlgFilletEdges::onDeleteObject, this, bp::_1));
    d->connectApplicationDeletedDocument = App::GetApplication().signalDeleteDocument.connect(
        boost::bind(&DlgFilletEdges::onDeleteDocument, this, bp::_1));
...
void DlgFilletEdges::onSelectionChanged(const Gui::SelectionChanges &msg)
{
    ...
    if (!d->object || !msg.pSubName) return;
...
{
    if (d->fillet == &obj) { d->fillet = nullptr; }
...
void DlgFilletEdges::findShapes()
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = objs.begin(); it != objs.end();
        ...
        if (current_index == 0) {
            if (Gui::Selection().isSelected(*it)) { current_index = index; }
        }
    }

    // if only one object is in the document then simply use that
    if (objs.size() == 1) current_index = 1;
...
void DlgFilletEdges::setupFillet(const std::vector<App::DocumentObject *> &objs)
{
    ...
    std::vector<App::DocumentObject *>::const_iterator it =
        std::find(objs.begin(), objs.end(), base);
    if (it != objs.end()) {
        // toggle visibility
        Gui::ViewProvider *vp;
        ...
        for (std::vector<Part::FilletElement>::const_iterator et = e.begin(); et != e.end(); ++et) {
            std::vector<int>::iterator it =
                std::find(d->edge_ids.begin(), d->edge_ids.end(), et->edgeid);
            if (it != d->edge_ids.end()) {
                ...
                model->setData(model->index(index, 1),
                               QVariant::fromValue<Base::Quantity>(
                                   Base::Quantity(et->radius1, Base::Unit::Length)));
                ...
                if (startRadius != endRadius) twoRadii = true;
        ...
        */
        ...
        if (!subElements.empty()) {
            Gui::Selection().addSelections(doc->getName(), d->object->getNameInDocument(),
                                           subElements);
        }
    }
}
    ...
    if (part && part->getTypeId().isDerivedFrom(Part::Feature::getClassTypeId())) {
        ...
        for (int i = 1; i <= edge2Face.Extent(); ++i) {
            ...
            if (los.Extent() == 2) {
                ...
                GeomAbs_Shape cont = BRep_Tool::Continuity(TopoDS::Edge(edge), TopoDS::Face(face1),
                                                           TopoDS::Face(face2));
                if (cont == GeomAbs_C0) {
...
const char *DlgFilletEdges::getFilletType() const { return "Fillet"; }
...
bool DlgFilletEdges::accept()
{
    ...
    if (d->fillet) name = QString::fromLatin1(d->fillet->getNameInDocument());
    else
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        if (checkState & Qt::Checked) {
            ...
            Base::Quantity r2 = r1;
            if (end_radius) r2 = model->index(i, 2).data(Qt::EditRole).value<Base::Quantity>();
...
FilletEdgesDialog::FilletEdgesDialog(DlgFilletEdges::FilletType type, Part::FilletBase *fillet,
...
{
    if (widget->accept()) QDialog::accept();
}
...
TaskFilletEdges::TaskFilletEdges(Part::Fillet *fillet)
{
    ...
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_Fillet"),
...
bool TaskFilletEdges::accept()
{
    ...
    if (ok) Gui::Command::doCommand(Gui::Command::Gui, "Gui.activeDocument().resetEdit()");
    return ok;
}
...
const char *DlgChamferEdges::getFilletType() const { return "Chamfer"; }

TaskChamferEdges::TaskChamferEdges(Part::Chamfer *chamfer)
{
    widget = new DlgChamferEdges(chamfer);
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_Chamfer"),
                                         widget->windowTitle(), true, nullptr);
    taskbox->groupLayout()->addWidget(widget);
    Content.push_back(taskbox);
}

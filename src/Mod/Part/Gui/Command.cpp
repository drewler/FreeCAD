...
CmdPartPickCurveNet::CmdPartPickCurveNet() : Command("Part_PickCurveNet")
{
    sAppModule = "Part";
...
void CmdPartPickCurveNet::activated(int iMsg) { Q_UNUSED(iMsg); }
...
CmdPartNewDoc::CmdPartNewDoc() : Command("Part_NewDoc")
{
    sAppModule = "Part";
    sGroup = "Part";
...
CmdPartBox2::CmdPartBox2() : Command("Part_Box2")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
CmdPartBox3::CmdPartBox3() : Command("Part_Box3")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
CmdPartPrimitives::CmdPartPrimitives() : Command("Part_Primitives")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
std::vector<Part::TopoShape> getShapesFromSelection()
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = objs.begin(); it != objs.end(); ++it) {
        ...
        if (!shp.isNull()) { shapes.push_back(shp); }
    }
    return shapes;
}
/*
...
CmdPartCut::CmdPartCut() : Command("Part_Cut")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartCut::activated(int iMsg)
{
    ...
    for (std::vector<Gui::SelectionObject>::iterator it = Sel.begin(); it != Sel.end(); ++it) {
        ...
        if (!PartGui::checkForSolids(shape) && !askUser) {
            int ret = QMessageBox::warning(
                Gui::getMainWindow(), QObject::tr("Non-solids selected"),
                QObject::tr(
    ...
    doCommand(Doc, "App.activeDocument().%s.Base = App.activeDocument().%s", FeatName.c_str(),
              Sel[0].getFeatName());
    doCommand(Doc, "App.activeDocument().%s.Tool = App.activeDocument().%s", FeatName.c_str(),
              Sel[1].getFeatName());
...
{
    return getSelection().countObjectsOfType(App::DocumentObject::getClassTypeId(), nullptr,
...
CmdPartCommon::CmdPartCommon() : Command("Part_Common")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartCommon::activated(int iMsg)
{
    ...
    doCommand(Doc, "App.activeDocument().addObject(\"Part::MultiCommon\",\"%s\")",
              FeatName.c_str());
    runCommand(Doc, str.str().c_str());

    // hide the input objects and remove them from the parent group
    App::DocumentObjectGroup *targetGroup = nullptr;
    for (std::vector<Gui::SelectionObject>::iterator it = partObjects.begin();
         it != partObjects.end(); ++it) {
        doCommand(Gui, "Gui.activeDocument().%s.Visibility=False", it->getFeatName());
        App::DocumentObjectGroup *group = it->getObject()->getGroup();
        if (group) {
            targetGroup = group;
...
CmdPartFuse::CmdPartFuse() : Command("Part_Fuse")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
CmdPartCompJoinFeatures::CmdPartCompJoinFeatures() : Command("Part_CompJoinFeatures")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartCompJoinFeatures::activated(int iMsg)
{
    ...
    if (iMsg == 0) rcCmdMgr.runCommandByName("Part_JoinConnect");
    else if (iMsg == 1)
...
CmdPartCompSplitFeatures::CmdPartCompSplitFeatures() : Command("Part_CompSplitFeatures")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
{
    Gui::CommandManager &rcCmdMgr = Gui::Application::Instance->commandManager();
    if (iMsg == 0) rcCmdMgr.runCommandByName("Part_BooleanFragments");
    else if (iMsg == 1)
...
CmdPartCompCompoundTools::CmdPartCompCompoundTools() : Command("Part_CompCompoundTools")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
{
    Gui::CommandManager &rcCmdMgr = Gui::Application::Instance->commandManager();
    if (iMsg == 0) rcCmdMgr.runCommandByName("Part_Compound");
    else if (iMsg == 1)
...
CmdPartCompound::CmdPartCompound() : Command("Part_Compound")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartCompound::activated(int iMsg)
{
    ...
    unsigned int n = getSelection().countObjectsOfType(App::DocumentObject::getClassTypeId(),
                                                       nullptr, Gui::ResolveMode::FollowLink);
    if (n < 1) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
    ...
    for (std::vector<Gui::SelectionSingleton::SelObj>::iterator it = Sel.begin(); it != Sel.end();
         ++it) {
        auto pos = tempSelNames.insert(it->FeatName);
        if (pos.second) { str << "App.activeDocument()." << it->FeatName << ","; }
    }
    str << "]";
...
CmdPartSection::CmdPartSection() : Command("Part_Section")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartSection::activated(int iMsg)
{
    ...
    doCommand(Gui, "Gui.activeDocument().%s.LineColor = Gui.activeDocument().%s.ShapeColor",
              FeatName.c_str(), BaseName.c_str());
    updateActive();
    commitCommand();
}
...
CmdPartImport::CmdPartImport() : Command("Part_Import")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartImport::activated(int iMsg)
{
    ...
    if (!fn.isEmpty()) {
        ...
        if (select == filter[1] || select == filter[3]) {
            ...
            doCommand(Doc, "ImportGui.insert(\"%s\",\"%s\")", (const char *)fn.toUtf8(),
                      pDoc->getName());
        }
        else {
            doCommand(Doc, "import Part");
            doCommand(Doc, "Part.insert(\"%s\",\"%s\")", (const char *)fn.toUtf8(),
                      pDoc->getName());
        }
        commitCommand();
...
CmdPartExport::CmdPartExport() : Command("Part_Export")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartExport::activated(int iMsg)
{
    ...
    QString select;
    QString fn = Gui::FileDialog::getSaveFileName(Gui::getMainWindow(), QString(), QString(),
                                                  filter.join(QLatin1String(";;")), &select);
    if (!fn.isEmpty()) {
        App::Document *pDoc = getDocument();
        if (!pDoc) // no document
            return;
        if (select == filter[1] || select == filter[3]) {
            Gui::Application::Instance->exportTo((const char *)fn.toUtf8(), pDoc->getName(),
                                                 "ImportGui");
        }
        else {
            Gui::Application::Instance->exportTo((const char *)fn.toUtf8(), pDoc->getName(),
...
{
    return Gui::Selection().countObjectsOfType(App::DocumentObject::getClassTypeId(), nullptr,
                                               Gui::ResolveMode::FollowLink)
        > 0;
}
...
CmdPartImportCurveNet::CmdPartImportCurveNet() : Command("Part_ImportCurveNet")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartImportCurveNet::activated(int iMsg)
{
    ...
    if (!fn.isEmpty()) {
        ...
        doCommand(Doc, "f = App.activeDocument().addObject(\"Part::CurveNet\",\"%s\")",
...
CmdPartMakeSolid::CmdPartMakeSolid() : Command("Part_MakeSolid")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartMakeSolid::activated(int iMsg)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = objs.begin(); it != objs.end(); ++it) {
        ...
        if (!shape.IsNull()) {
            ...
            try {
                if (!str.isEmpty()) runCommand(Doc, str.toLatin1());
            }
...
CmdPartReverseShape::CmdPartReverseShape() : Command("Part_ReverseShape")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartReverseShape::isActive() { return PartGui::hasShapesInSelection(); }
...
CmdPartBoolean::CmdPartBoolean() : Command("Part_Boolean")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartBoolean::activated(int iMsg)
{
    ...
    if (!dlg) dlg = new PartGui::TaskBooleanOperation();
    Gui::Control().showDialog(dlg);
}

bool CmdPartBoolean::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartExtrude::CmdPartExtrude() : Command("Part_Extrude")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartExtrude::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartMakeFace::CmdPartMakeFace() : Command("Part_MakeFace")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartMakeFace::activated(int iMsg)
{
    ...
    try {
        ...
        std::stringstream str;
        str << doc.getDocumentPython() << ".addObject(\"Part::Face\", \"Face\").Sources = (";
        for (auto &obj : sketches) { str << App::DocumentObjectT(obj).getObjectPython() << ", "; }
...
CmdPartRevolve::CmdPartRevolve() : Command("Part_Revolve")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartRevolve::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartFillet::CmdPartFillet() : Command("Part_Fillet")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartFillet::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartChamfer::CmdPartChamfer() : Command("Part_Chamfer")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartChamfer::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartMirror::CmdPartMirror() : Command("Part_Mirror")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartMirror::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartCrossSections::CmdPartCrossSections() : Command("Part_CrossSections")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
CmdPartBuilder::CmdPartBuilder() : Command("Part_Builder")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartBuilder::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartLoft::CmdPartLoft() : Command("Part_Loft")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartLoft::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartSweep::CmdPartSweep() : Command("Part_Sweep")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdPartSweep::isActive() { return (hasActiveDocument() && !Gui::Control().activeDialog()); }
...
CmdPartOffset::CmdPartOffset() : Command("Part_Offset")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartOffset::activated(int iMsg)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = docobjs.begin(); it != docobjs.end();
         ++it) {
        if (!Part::Feature::getTopoShape(*it).isNull()) { shapes.push_back(*it); }
    }
    ...
    doCommand(Doc, "App.ActiveDocument.%s.Source = App.ActiveDocument.%s", offset.c_str(),
...
CmdPartOffset2D::CmdPartOffset2D() : Command("Part_Offset2D")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
{
    Gui::CommandManager &rcCmdMgr = Gui::Application::Instance->commandManager();
    if (iMsg == 0) rcCmdMgr.runCommandByName("Part_Offset");
    else if (iMsg == 1)
...
CmdPartThickness::CmdPartThickness() : Command("Part_Thickness")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartThickness::activated(int iMsg)
{
    ...
    if (selobjs.size() == 1) {
        ...
        if (!topoShape.isNull()) {
            ...
            for (std::vector<Part::TopoShape>::iterator it = subShapes.begin();
                 it != subShapes.end(); ++it) {
    ...
    if (!topoShape.isNull()) {
        ...
        for (; xp.More(); xp.Next()) { countSolids++; }
    }
    if (countSolids != 1 || !ok) {
        QMessageBox::warning(
            Gui::getMainWindow(), QApplication::translate("CmdPartThickness", "Wrong selection"),
...
CmdShapeInfo::CmdShapeInfo() : Command("Part_ShapeInfo")
{
    sAppModule = "Part";
    sGroup = "Part";
...
bool CmdShapeInfo::isActive()
{
    ...
    if (!doc || doc->countObjectsOfType(Part::Feature::getClassTypeId()) == 0) return false;
...
CmdPartRuledSurface::CmdPartRuledSurface() : Command("Part_RuledSurface")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdPartRuledSurface::activated(int iMsg)
{
    ...
    if (selobjs.size() != 1 && selobjs.size() != 2) { ok = false; }
    ...
    if (ok && selobjs.size() <= 2) {
        if (!selobjs.empty()) {
            ...
            if (shape1.isNull()) { ok = false; }
            if (ok && subnames1.size() <= 2) {
                if (!subnames1.empty()) {
                    curve1 = Part::Feature::getTopoShape(docobj1, subnames1[0].c_str(),
                                                         true /*need element*/)
                                 .getShape();
                    link1 = subnames1[0];
                }
                if (subnames1.size() == 2) {
                    curve2 = Part::Feature::getTopoShape(docobj1, subnames1[1].c_str(),
                                                         true /*need element*/)
                                 .getShape();
                    link2 = subnames1[1];
                }
                if (subnames1.empty()) { curve1 = shape1.getShape(); }
            }
            else {
                ok = false;
            }
        ...
        if (selobjs.size() == 2) {
            ...
            if (shape2.isNull()) { ok = false; }
            if (ok && subnames2.size() == 1) {
                curve2 = Part::Feature::getTopoShape(docobj2, subnames2[0].c_str(),
                                                     true /*need element*/)
                             .getShape();
                link2 = subnames2[0];
            }
            else {
                if (subnames2.empty()) { curve2 = shape2.getShape(); }
            }
        }
...
bool CmdPartRuledSurface::isActive() { return getActiveGuiDocument(); }
...
CmdCheckGeometry::CmdCheckGeometry() : Command("Part_CheckGeometry")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
void CmdCheckGeometry::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    Gui::TaskView::TaskDialog *dlg = Gui::Control().activeDialog();
    if (!dlg) dlg = new PartGui::TaskCheckGeometryDialog();
    Gui::Control().showDialog(dlg);
}
...
CmdColorPerFace::CmdColorPerFace() : Command("Part_ColorPerFace")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
{
    Q_UNUSED(iMsg);
    if (getActiveGuiDocument()->getInEdit()) getActiveGuiDocument()->resetEdit();
...
CmdMeasureLinear::CmdMeasureLinear() : Command("Part_Measure_Linear")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureLinear::isActive() { return hasActiveDocument(); }
...
CmdMeasureAngular::CmdMeasureAngular() : Command("Part_Measure_Angular")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureAngular::isActive() { return hasActiveDocument(); }
...
CmdMeasureRefresh::CmdMeasureRefresh() : Command("Part_Measure_Refresh")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureRefresh::isActive() { return hasActiveDocument(); }
...
CmdMeasureClearAll::CmdMeasureClearAll() : Command("Part_Measure_Clear_All")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureClearAll::isActive() { return hasActiveDocument(); }
...
CmdMeasureToggleAll::CmdMeasureToggleAll() : Command("Part_Measure_Toggle_All")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureToggleAll::isActive() { return hasActiveDocument(); }
...
CmdMeasureToggle3d::CmdMeasureToggle3d() : Command("Part_Measure_Toggle_3D")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureToggle3d::isActive() { return hasActiveDocument(); }
...
CmdMeasureToggleDelta::CmdMeasureToggleDelta() : Command("Part_Measure_Toggle_Delta")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdMeasureToggleDelta::isActive() { return hasActiveDocument(); }
...
CmdBoxSelection::CmdBoxSelection() : Command("Part_BoxSelection")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
bool CmdBoxSelection::isActive() { return hasActiveDocument(); }
...
CmdPartProjectionOnSurface::CmdPartProjectionOnSurface() : Command("Part_ProjectionOnSurface")
{
    sAppModule = "Part";
    sGroup = QT_TR_NOOP("Part");
...
CmdPartSectionCut::CmdPartSectionCut() : Command("Part_SectionCut")
{
    sAppModule = "Part";
    sGroup = "View";
...
void CmdPartSectionCut::activated(int iMsg)
{
    ...
    if (!sectionCut) { sectionCut = PartGui::SectionCut::makeDockWidget(Gui::getMainWindow()); }
}

bool CmdPartSectionCut::isActive() { return hasActiveDocument(); }

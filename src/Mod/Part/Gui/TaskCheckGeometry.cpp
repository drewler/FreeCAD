...
QString shapeEnumToString(const int &index)
{
    ...
    if (index < 0 || index > TopAbs_SHAPE) return names.at(8);
...
QVector<QString> buildCheckStatusStringVector()
{
    ...
    names.push_back(
        QObject::tr("Invalid Curve On Closed Surface")); //    BRepCheck_InvalidCurveOnClosedSurface
...
QString checkStatusToString(const int &index)
{
    ...
    if (index == -1) { return QString(QObject::tr("No Result")); }
    if (index > 33 || index < 0) {
...
QString getBOPCheckString(const BOPAlgo_CheckStatus &status)
{
    ...
    if (index < 0 || index > strings.size()) index = 0;
...
ResultEntry::~ResultEntry()
{
    if (boxSep && viewProviderRoot) viewProviderRoot->removeChild(boxSep);
    if (viewProviderRoot) viewProviderRoot->unref();
    qDeleteAll(children);
}
...
void ResultEntry::buildEntryName()
{
    ...
    switch (this->shape.ShapeType()) {
        ...
        case TopAbs_VERTEX:
            ...
            stream << "Vertex";
            break;
        default: stream << "Unexpected shape type"; break;
    }
...
ResultModel::ResultModel(QObject *parent) : QAbstractItemModel(parent) { root = nullptr; }
...
QVariant ResultModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (index.column()) {
        case 0: return QVariant(node->name);
        case 1: return QVariant(node->type);
        case 2: return QVariant(node->error);
    }
    return QVariant();
}
...
QVariant ResultModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    switch (section) {
        case 0: return QVariant(QString(tr("Name")));
        case 1: return QVariant(QString(tr("Type")));
        case 2: return QVariant(QString(tr("Error")));
    }
    return QVariant();
}
...
{
    if (index.isValid()) return static_cast<ResultEntry *>(index.internalPointer());
    else
        return root;
}
...
void TaskCheckGeometryResults::setupInterface()
{
    ...
    connect(treeView->selectionModel(), SIGNAL(currentRowChanged(QModelIndex, QModelIndex)), this,
...
void TaskCheckGeometryResults::goCheck()
{
    ...
    Message_ProgressScope theScope(theRange, TCollection_AsciiString("BOP check..."),
                                   selection.size());
    theScope.Show();
#endif // 0x070500
...
void TaskCheckGeometryResults::recursiveCheck(const BRepCheck_Analyzer &shapeCheck,
                                              const TopoDS_Shape &shape, ResultEntry *parent)
{
    ...
    if (shape.ShapeType() == TopAbs_SOLID) checkSub(shapeCheck, shape, TopAbs_SHELL, branchNode);
    if (shape.ShapeType() == TopAbs_EDGE) checkSub(shapeCheck, shape, TopAbs_VERTEX, branchNode);
...
void TaskCheckGeometryResults::checkSub(const BRepCheck_Analyzer &shapeCheck,
                                        const TopoDS_Shape &shape, const TopAbs_ShapeEnum subType,
                                        ResultEntry *parent)
{
        ...
        for (res->InitContextIterator(); res->MoreShapeInContext(); res->NextShapeInContext()) {
            if (res->ContextualShape().IsSame(shape)) {
                for (itl.Initialize(res->StatusOnShape()); itl.More(); itl.Next()) {
                    if (itl.Value() == BRepCheck_NoError) break;
                    checkedMap.Add(sub);
                    ResultEntry *entry = new ResultEntry();
                    entry->parent = parent;
                    entry->shape = sub;
...
void TaskCheckGeometryResults::buildShapeContent(App::DocumentObject *pObject,
                                                 const QString &baseName, const TopoDS_Shape &shape)
{
...
#if OCC_VERSION_HEX < 0x070500
int TaskCheckGeometryResults::goBOPSingleCheck(const TopoDS_Shape &shapeIn, ResultEntry *theRoot,
...
#endif
{
    ...
    if (!BOPCheck.HasFaulty()) return 0;
...
void TaskCheckGeometryResults::setupFunctionMap()
{
    ...
    functionMap.emplace_back(TopAbs_VERTEX, BRepCheck_InvalidPointOnCurve,
                             goSetupResultInvalidPointCurve);
    functionMap.emplace_back(TopAbs_FACE, BRepCheck_IntersectingWires,
                             goSetupResultIntersectingWires);
    functionMap.emplace_back(TopAbs_EDGE, BRepCheck_InvalidCurveOnSurface,
                             goSetupResultInvalidCurveSurface);
    functionMap.emplace_back(TopAbs_EDGE, BRepCheck_InvalidSameParameterFlag,
                             goSetupResultInvalidSameParameterFlag);
    functionMap.emplace_back(TopAbs_FACE, BRepCheck_UnorientableShape,
                                                 ...
                                                 const QModelIndex &previous)
{
    ...
    if (previous.isValid()) {
        ...
        if (entry) {
            if (entry->boxSwitch) entry->boxSwitch->whichChild.setValue(SO_SWITCH_NONE);
        }
    }
    if (current.isValid()) {
        ResultEntry *entry = model->getEntry(current);
        if (entry) {
            if (entry->boxSwitch) entry->boxSwitch->whichChild.setValue(0);
            QStringList::Iterator stringIt;
            for (stringIt = entry->selectionStrings.begin();
                 stringIt != entry->selectionStrings.end(); ++stringIt) {
                //need unique delimiter.
                QString doc, object, sub;
                if (!this->split((*stringIt), doc, object, sub)) continue;
...
void PartGui::goSetupResultTypedSelection(ResultEntry *entry, const TopoDS_Shape &shape,
                                          TopAbs_ShapeEnum type)
{
    TopExp_Explorer it;
    for (it.Init(shape, type); it.More(); it.Next()) {
        QString name = buildSelectionName(entry, (it.Current()));
        if (!name.isEmpty()) entry->selectionStrings.append(name);
    }
}
...
void PartGui::goSetupResultBoundingBox(ResultEntry *entry)
{
    ...
    try {
        ...
        SbVec3f boundCenter((xmax - xmin) / 2 + xmin, (ymax - ymin) / 2 + ymin,
                            (zmax - zmin) / 2 + zmin);
...
TaskCheckGeometryDialog::TaskCheckGeometryDialog()
    : widget(nullptr), contentLabel(nullptr), okBtn(nullptr), settingsBtn(nullptr),
      resultsBtn(nullptr)
{
    ParameterGrp::handle group = App::GetApplication()
                                     .GetUserParameter()
    ...
    if (!expandShapeContent) { shapeContentBox->hideGroupBox(); }
...
Default: false"));
    ...
    connect(autoRunCheckBox, SIGNAL(toggled(bool)), this, SLOT(on_autoRunCheckBox_toggled(bool)));
...
mean the checked object is unusable.  Default: false"));
    ...
    connect(runBOPCheckBox, SIGNAL(toggled(bool)), this, SLOT(on_runBOPCheckBox_toggled(bool)));
...
but more stable.  Default: false"));
    ...
    connect(runSingleThreadedCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_runSingleThreadedCheckBox_toggled(bool)));
    ...
    connect(logErrorsCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_logErrorsCheckBox_toggled(bool)));
...
the check geometry tool.  Default: false"));
    ...
    connect(expandShapeContentCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_expandShapeContentCheckBox_toggled(bool)));
...
the check geometry tool.  Default: false"));
    ...
    connect(advancedShapeContentCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_advancedShapeContentCheckBox_toggled(bool)));
    ...
    connect(argumentTypeModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_argumentTypeModeCheckBox_toggled(bool)));
    ...
    connect(selfInterModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_selfInterModeCheckBox_toggled(bool)));
    ...
    connect(smallEdgeModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_smallEdgeModeCheckBox_toggled(bool)));
    ...
    connect(rebuildFaceModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_rebuildFaceModeCheckBox_toggled(bool)));
    ...
    connect(continuityModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_continuityModeCheckBox_toggled(bool)));
    ...
    connect(tangentModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_tangentModeCheckBox_toggled(bool)));
    ...
    connect(mergeVertexModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_mergeVertexModeCheckBox_toggled(bool)));
    ...
    connect(mergeEdgeModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_mergeEdgeModeCheckBox_toggled(bool)));
    ...
    connect(curveOnSurfaceModeCheckBox, SIGNAL(toggled(bool)), this,
            SLOT(on_curveOnSurfaceModeCheckBox_toggled(bool)));
...
bool TaskCheckGeometryDialog::reject() { return true; }
...
void TaskCheckGeometryDialog::on_clicked(QAbstractButton *btn)
{
    ...
    if (btn == okBtn) { settingsBtn->setEnabled(true); }
...
void TaskCheckGeometryDialog::modifyStandardButtons(QDialogButtonBox *box)
{
    ...
    ParameterGrp::handle group = App::GetApplication()
                                     ...
                                     ->GetGroup("Part")
                                     ->GetGroup("CheckGeometry");
    if (!group->GetBool("AutoRun", false)) settingsBtn->setEnabled(false);
...
BOPProgressIndicator::~BOPProgressIndicator() { myProgress->close(); }
...
Standard_Boolean BOPProgressIndicator::Show(const Standard_Boolean theForce)
{
    if (theForce) {
        ...
        if (!aName.IsNull()) myProgress->setLabelText(QString::fromLatin1(aName->ToCString()));
    }
...
Standard_Boolean BOPProgressIndicator::UserBreak()
{
    ...
    if (currentThread == myProgress->thread()) {
        ...
        if (canceled) return Standard_True;

...
public:
    ...
    Type mode;
    ShapeSelection() : Gui::SelectionFilterGate(nullPointer()), mode(ALL) {}
    void setMode(Type mode) { this->mode = mode; }
    bool allow(App::Document *, App::DocumentObject *obj, const char *sSubName) override
    {
        if (!obj || !obj->isDerivedFrom(Part::Feature::getClassTypeId())) return false;
        if (!sSubName || sSubName[0] == '\0') return (mode == ALL);
        std::string element(sSubName);
        switch (mode) {
            case VERTEX: return element.substr(0, 6) == "Vertex";
            case EDGE: return element.substr(0, 4) == "Edge";
            case FACE: return element.substr(0, 4) == "Face";
            default: return true;
        }
    }
};
...
ShapeBuilderWidget::ShapeBuilderWidget(QWidget *parent) : d(new Private())
{
    Q_UNUSED(parent);
    d->ui.setupUi(this);
    ...
    connect(&d->bg, SIGNAL(buttonClicked(int)), this, SLOT(switchMode(int)));
...
void ShapeBuilderWidget::on_createButton_clicked()
{
    ...
    try {
        if (mode == 0) { createEdgeFromVertex(); }
        else if (mode == 1) {
            createWireFromEdge();
        }
        else if (mode == 2) {
...
void ShapeBuilderWidget::on_selectButton_clicked()
{
    ...
    if (id == 0 || id == 2) { d->selection.start(TopAbs_VERTEX); }
    else if (id == 1 || id == 3) {
...
void ShapeBuilderWidget::createShellFromFace()
{
    ...
    if (d->ui.checkFaces->isChecked()) {
        ...
        for (it = sel.begin(); it != sel.end(); ++it) obj.insert(it->getObject());
        str << "[]";
...
bool ShapeBuilderWidget::accept() { return true; }

bool ShapeBuilderWidget::reject() { return true; }
...
TaskShapeBuilder::TaskShapeBuilder()
{
    ...
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_Shapebuilder"),
...
bool TaskShapeBuilder::accept() { return widget->accept(); }

bool TaskShapeBuilder::reject() { return widget->reject(); }

...
QIcon ViewProviderSpline::getIcon(void) const { return Gui::BitmapFactory().pixmap(sPixmap); }
...
ViewProviderSplineExtension::ViewProviderSplineExtension() : pcControlPoints(nullptr)
{
    initExtensionType(ViewProviderSplineExtension::getExtensionClassTypeId());
    EXTENSION_ADD_PROPERTY(ControlPoints, (false));
}

void ViewProviderSplineExtension::toggleControlPoints(bool on) { ControlPoints.setValue(on); }
...
void ViewProviderSplineExtension::showControlPoints(bool show, const App::Property *prop)
{
    ...
    if (pcControlPoints) { pcControlPoints->whichChild = (show ? SO_SWITCH_ALL : SO_SWITCH_NONE); }

    if (!show || !pcControlPoints || pcControlPoints->getNumChildren() > 0) return;
    ...
    if (prop && prop->getTypeId() == Part::PropertyPartShape::getClassTypeId()) {
        ...
        if (shape.IsNull()) return; // empty shape
...
void ViewProviderSplineExtension::showControlPointsOfEdge(const TopoDS_Edge &edge)
{
    ...
    switch (curve.GetType()) {
        case GeomAbs_BezierCurve: {
            ...
            nCt = hBezier->NbPoles();
            for (Standard_Integer i = 1; i <= nCt; i++) poles.push_back(hBezier->Pole(i));
            if (hBezier->IsClosed()) {
                nCt++;
...
namespace Gui
{
EXTENSION_PROPERTY_SOURCE_TEMPLATE(PartGui::ViewProviderSplineExtensionPython,

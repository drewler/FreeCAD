...
void SoFCControlPoints::initClass() { SO_NODE_INIT_CLASS(SoFCControlPoints, SoShape, "Shape"); }
...
void SoFCControlPoints::drawControlPoints(const SbVec3f *points, int32_t len) const
{
    ...
    if (poles > (uint32_t)len) return; // wrong setup, too few points
    // draw control mesh
    glBegin(GL_LINES);
    ...
    glPointSize(5.0f);
    glBegin(GL_POINTS);
    for (uint32_t p = 0; p < poles; p++) { glVertex3fv(points[p].getValue()); }
    glEnd();

    // draw knots if available
    uint32_t knots = numKnotsU.getValue() * numKnotsV.getValue();
    uint32_t index = poles + knots;
    if (index > (uint32_t)len) return; // wrong setup, too few points
    glColor3f(1.0f, 1.0f, 0.0f);
    glPointSize(6.0f);
    glBegin(GL_POINTS);
    for (uint32_t p = poles; p < index; p++) { glVertex3fv(points[p].getValue()); }
    glEnd();
}

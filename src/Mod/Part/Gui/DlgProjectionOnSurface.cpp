...
public:
    ...
    FaceSelection() : Gui::SelectionFilterGate(nullPointer()) { canSelect = false; }
...
DlgProjectionOnSurface::DlgProjectionOnSurface(QWidget *parent)
    : QWidget(parent), ui(new Ui::DlgProjectionOnSurface),
      m_projectionObjectName(tr("Projection Object")), filterEdge(nullptr), filterFace(nullptr)
{
    ui->setupUi(this);
...
{
    if (m_projectionObject == &Obj) { m_projectionObject = nullptr; }
}

void PartGui::DlgProjectionOnSurface::apply()
{
    if (m_partDocument) m_partDocument->commitTransaction();
}

void PartGui::DlgProjectionOnSurface::reject()
{
    if (m_partDocument) m_partDocument->abortTransaction();
}
        ...
        for (auto it = selObj.begin(); it != selObj.end(); ++it) {
            ...
            if (aPart) {
                ...
                if (!it->getSubNames().empty()) {
                    ...
                    for (auto itName = selObj.front().getSubNames().begin();
                         itName != selObj.front().getSubNames().end(); ++itName) {
        ...
        for (auto &itCurrentShape : iCurrentShape) {
            ...
            if (!itCurrentShape.aFace.IsNull()) {
                ...
                for (const auto &itWire : itCurrentShape.aWireVec) {
                    BRepProj_Projection aProjection(itWire, itCurrentShape.surfaceToProject,
...
PartGui::DlgProjectionOnSurface::create_compound(const std::vector<SShapeStore> &iShapeVec)
{
    ...
    for (const auto &it : iShapeVec) {
        if (m_currentShowType == "edges") {
            for (const auto &it2 : it.aProjectedEdgeVec) { aBuilder.Add(aCompound, it2); }
            for (const auto &it2 : it.aProjectedWireVec) { aBuilder.Add(aCompound, it2); }
            continue;
        }
        else if (m_currentShowType == "faces") {
            if (it.aProjectedFace.IsNull()) {
                for (const auto &it2 : it.aProjectedWireVec) {
                    if (!it2.IsNull()) { aBuilder.Add(aCompound, it2); }
                }
            }
            else
            ...
            if (!it.aProjectedSolid.IsNull()) { aBuilder.Add(aCompound, it.aProjectedSolid); }
                                                      ...
                                                      bool iHighlight, const unsigned int iColor)
{
    ...
    if (vp) {
        ...
        if (currentShapeType == TopAbs_FACE) { vp->DiffuseColor.setValues(colors); }
        else if (currentShapeType == TopAbs_EDGE) {
            ...
            for (const auto &itWireVec : itCurrentShape.aProjectedWireVec) {
                ...
                for (auto itEdge : edgeVec) {
                    ...
                    auto currentCurve = BRep_Tool::CurveOnSurface(
                        TopoDS::Edge(itEdge), itCurrentShape.surfaceToProject, first, last);
                                                    ...
                                                    const TopoDS_Face &iFaceToProject)
{
    ...
    for (const auto &it : iEdgeVec) { shapeList->Append(it); }
...
void PartGui::DlgProjectionOnSurface::set_xyz_dir_spinbox(QDoubleSpinBox *icurrentSpinBox)
{
    ...
    auto newVal = 0.0;
    if (currentVal != 1.0 && currentVal != -1.0) { newVal = -1; }

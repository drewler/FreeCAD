...
public:
    ...
    EdgeSelection() : Gui::SelectionFilterGate(nullPointer()) { canSelect = false; }
...
void DlgExtrusion::on_btnX_clicked()
{
    ...
    if ((getDir() - axis).Length() < 1e-7) axis = axis * (-1);
...
void DlgExtrusion::on_chkSymmetric_toggled(bool on) { ui->spinLenRev->setEnabled(!on); }

void DlgExtrusion::on_txtLink_textChanged(QString) { this->fetchDir(); }
...
App::DocumentObject &DlgExtrusion::getShapeToExtrude() const
{
    ...
    if (objs.empty()) throw Base::ValueError("No shapes selected");
    return *(objs[0]);
}
...
void DlgExtrusion::fetchDir()
{
    ...
    try {
        ...
        if (dir_has_valid_magnitude && lengths_are_at_defaults) { ui->spinLenFwd->setValue(0); }
        else if (!dir_has_valid_magnitude && lengths_are_zero) {
            ui->spinLenFwd->setValue(1.0);
        }
        if (fetched) { this->setDir(dir); }
    }
    ...
    try {
        ...
        for (int i = 0; i < leaves->Length(); i++) {
            ...
            if (leaf.ShapeType() == TopAbs_WIRE || leaf.ShapeType() == TopAbs_EDGE) {
                if (BRep_Tool::IsClosed(leaf)) { cntClosedWires++; }
            }
        }
...
{
    try {
        if (!validate()) throw Base::AbortException();
...
{
    if (ui->rbDirModeCustom->isChecked()) return Part::Extrusion::dmCustom;
    if (ui->rbDirModeEdge->isChecked()) return Part::Extrusion::dmEdge;
    if (ui->rbDirModeNormal->isChecked()) return Part::Extrusion::dmNormal;
...
void DlgExtrusion::getAxisLink(App::PropertyLinkSub &lnk) const
{
    ...
    if (text.length() == 0) { lnk.setValue(nullptr); }
    else {
...
std::vector<App::DocumentObject *> DlgExtrusion::getShapesToExtrude() const
{
    ...
    if (!doc) throw Base::RuntimeError("Document lost");

    std::vector<App::DocumentObject *> objects;
    for (int i = 0; i < items.size(); i++) {
        App::DocumentObject *obj =
            doc->getObject(items[i]->data(0, Qt::UserRole).toString().toLatin1());
        if (!obj) throw Base::RuntimeError("Object not found");
...
{
    Gui::Command::doCommand(Gui::Command::Doc, "f = App.getDocument('%s').getObject('%s')",
                            feature.getDocument()->getName(), feature.getNameInDocument());

    if (base)
        Gui::Command::doCommand(Gui::Command::Doc, "f.Base = App.getDocument('%s').getObject('%s')",
                                base->getDocument()->getName(), base->getNameInDocument());
    ...
    if (dirMode == Part::Extrusion::dmCustom) {
        ...
        Gui::Command::doCommand(Gui::Command::Doc, "f.Dir = App.Vector(%.15f, %.15f, %.15f)", dir.x,
                                dir.y, dir.z);
    }
    ...
    if (!lnk.getValue()) { linkstr << "None"; }
    else {
        linkstr << "(App.getDocument(\"" << lnk.getValue()->getDocument()->getName() << "\")."
                << lnk.getValue()->getNameInDocument();
        linkstr << ", [";
        for (const std::string &str : lnk.getSubValues()) { linkstr << "\"" << str << "\""; }
        linkstr << "])";
    }
    Gui::Command::doCommand(Gui::Command::Doc, "f.DirLink = %s", linkstr.str().c_str());

    Gui::Command::doCommand(Gui::Command::Doc, "f.LengthFwd = %.15f",
                            ui->spinLenFwd->value().getValue());
    Gui::Command::doCommand(Gui::Command::Doc, "f.LengthRev = %.15f",
                            ui->spinLenRev->value().getValue());

    Gui::Command::doCommand(Gui::Command::Doc, "f.Solid = %s",
                            ui->chkSolid->isChecked() ? "True" : "False");
    Gui::Command::doCommand(Gui::Command::Doc, "f.Reversed = %s",
                            ui->chkReversed->isChecked() ? "True" : "False");
    Gui::Command::doCommand(Gui::Command::Doc, "f.Symmetric = %s",
                            ui->chkSymmetric->isChecked() ? "True" : "False");
    Gui::Command::doCommand(Gui::Command::Doc, "f.TaperAngle = %.15f",
                            ui->spinTaperAngle->value().getValue());
    Gui::Command::doCommand(Gui::Command::Doc, "f.TaperAngleRev = %.15f",

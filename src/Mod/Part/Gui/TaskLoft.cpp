...
LoftWidget::LoftWidget(QWidget *parent) : d(new Private())
{
    Q_UNUSED(parent);
...
LoftWidget::~LoftWidget() { delete d; }
...
bool LoftWidget::accept()
{
    ...
    if (d->ui.checkRuledSurface->isChecked()) ruled = QString::fromLatin1("True");
    else
        ruled = QString::fromLatin1("False");

    if (d->ui.checkClosed->isChecked()) closed = QString::fromLatin1("True");
    else
...
bool LoftWidget::reject() { return true; }
...
TaskLoft::TaskLoft()
{
    ...
    taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_Loft"),
...
bool TaskLoft::accept() { return widget->accept(); }

bool TaskLoft::reject() { return widget->reject(); }

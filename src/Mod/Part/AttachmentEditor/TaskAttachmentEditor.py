        ...
        try:
            ...
            for m in sugr["reachableModes"].keys():
                ...
                if len(listlistrefs) == 1:
                    ...
                    txt = translate(
                        "AttachmentEditor", "{} (add more references)", None
                    ).format(txt)
                item.setText(txt)
                item.setData(self.KEYmode, m)
                item.setData(self.KEYon, True)
    ...
    def updateRefButtons(self):
        try:
            ...
            for i in range(len(self.refButtons)):
                ...
                typ = translate("AttachmentEditor", "Reference{}", None).format(
                    str(i + 1)
                )
    ...
    def updatePreview(self):
        ...
        try:
            ...
            if self.last_sugr["message"] == "LinkBroken":
                raise ValueError(
                    translate(
                        "AttachmentEditor", "Failed to resolve links. {}", None
                    ).format(self.last_sugr["error"])
                )
            ...
            if new_plm is None:
                ...
                self.form.message.setText(
                    translate("AttachmentEditor", "Attached with mode {}", None).format(
                        self.attacher.getModeInfo(self.getCurrentMode())[
                            "UserFriendlyName"
                        ]
                    )
                )

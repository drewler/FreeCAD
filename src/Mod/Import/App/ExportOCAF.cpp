...
ExportOCAF::ExportOCAF(Handle(TDocStd_Document) h, bool explicitPlacement)
    : pDoc(h), keepExplicitPlacement(explicitPlacement), filterBaseFeature(true)
{
...
std::vector<App::DocumentObject *> ExportOCAF::filterPart(App::Part *part) const
{
    ...
    for (auto it : entries) {
        ...
        for (auto jt : outList) {
            if (jt->getTypeId() == featureBase) { filterType.push_back(jt); }
        }
    }
    ...
    if (!filterType.empty()) {
        ...
        for (auto it : entries) {
            ...
            if (accept) keepObjects.push_back(it);
        }
...
int ExportOCAF::exportObject(App::DocumentObject *obj, std::vector<TDF_Label> &hierarchical_label,
                             ...
                             std::vector<App::DocumentObject *> &hierarchical_part)
{
    ...
    if (obj->getTypeId().isDerivedFrom(App::Part::getClassTypeId())) {
        ...
        if (filterBaseFeature) { entries = filterPart(part); }
    ...
    if (colors.size() == face_index.size()) {
        ...
        while (xp.More()) {
            ...
            if (face_index.find(index) != face_index.end()) {
                ...
                if (!faceLabel.IsNull()) { aShapeTool->SetShape(faceLabel, xp.Current()); }
                else {
...
void ExportOCAF::getFreeLabels(std::vector<TDF_Label> &hierarchical_label,
                               std::vector<TDF_Label> &labels, std::vector<int> &label_part_id)
{
...
void ExportOCAF::getPartColors(std::vector<App::DocumentObject *> hierarchical_part,
                               std::vector<TDF_Label> FreeLabels, std::vector<int> part_id,
...
void ExportOCAF::pushNode(int root_id, int node_id, std::vector<TDF_Label> &hierarchical_label,
                          std::vector<TopLoc_Location> &hierarchical_loc)
{
    TDF_Label root;
    TDF_Label node;
    ...
    XCAFDoc_DocumentTool::ShapeTool(root)->AddComponent(root, node,
...
void ExportOCAFCmd::findColors(Part::Feature *part, std::vector<App::Color> &colors) const
{
    ...
    if (it != partColors.end()) colors = it->second;
}

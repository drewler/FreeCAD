...
def addPart(partElement):
    ...
    representation = partElement.find(
        "{http://www.plmxml.org/Schemas/PLMXMLSchema}Representation"
    )
...
def parse(fileName):
    ...
    res = ParseUserData(
        ProductDef.find("{http://www.plmxml.org/Schemas/PLMXMLSchema}UserData")
    )

    InstanceGraph = ProductDef.find(
        "{http://www.plmxml.org/Schemas/PLMXMLSchema}InstanceGraph"
    )

    # get all the special elements we can read
    Instances = InstanceGraph.findall(
        "{http://www.plmxml.org/Schemas/PLMXMLSchema}Instance"
    )

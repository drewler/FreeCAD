...
void ImpExpDxfRead::OnReadLine(const double *s, const double *e, bool /*hidden*/)
{
    ...
    if (p0.IsEqual(p1, 0.00000001)) return;
...
void ImpExpDxfRead::OnReadArc(const double *s, const double *e, const double *c, bool dir,
                              bool /*hidden*/)
{
    gp_Pnt p0 = makePoint(s);
    gp_Pnt p1 = makePoint(e);
...
Handle(Geom_BSplineCurve) getSplineFromPolesAndKnots(struct SplineData &sd)
{
    ...
    int index = 1;
    for (auto x : sd.controlx) { occpoles(index++).SetX(x); }

    index = 1;
    for (auto y : sd.controly) { occpoles(index++).SetY(y); }

    index = 1;
    for (auto z : sd.controlz) { occpoles(index++).SetZ(z); }
    ...
    if (sd.weight.size() == std::size_t(sd.control_points)) {
        index = 1;
        for (auto w : sd.weight) { occweights(index++) = w; }
    }
    else {
        // non-rational
        for (int i = occweights.Lower(); i <= occweights.Upper(); i++) { occweights(i) = 1.0; }
    }
...
Handle(Geom_BSplineCurve) getInterpolationSpline(struct SplineData &sd)
{
    ...
    int index = 1;
    for (auto x : sd.fitx) { fitpoints->ChangeValue(index++).SetX(x); }

    index = 1;
    for (auto y : sd.fity) { fitpoints->ChangeValue(index++).SetY(y); }

    index = 1;
    for (auto z : sd.fitz) { fitpoints->ChangeValue(index++).SetZ(z); }
...
void ImpExpDxfRead::OnReadSpline(struct SplineData &sd)
{
    ...
    try {
        Handle(Geom_BSplineCurve) geom;
        if (sd.control_points > 0) geom = getSplineFromPolesAndKnots(sd);
        else if (sd.fit_points > 0)
            geom = getInterpolationSpline(sd);

        if (geom.IsNull()) throw Standard_Failure();
...
void ImpExpDxfRead::OnReadEllipse(const double *c, double major_radius, double minor_radius,
...
void ImpExpDxfRead::OnReadText(const double *point, const double /*height*/, const char *text)
{
    if (optionImportAnnotations) {
        Base::Vector3d pt(point[0] * optionScaling, point[1] * optionScaling,
                          point[2] * optionScaling);
        if (LayerName().substr(0, 6) != "BLOCKS") {
            App::Annotation *pcFeature =
                (App::Annotation *)document->addObject("App::Annotation", "Text");
...
void ImpExpDxfRead::OnReadInsert(const double *point, const double *scale, const char *name,
                                 double rotation)
{
    ...
    prefix += name;
    prefix += " ";
    auto checkScale = [=](double v) { return v != 0.0 ? v : 1.0; };
    for (std::map<std::string, std::vector<Part::TopoShape *>>::const_iterator i = layers.begin();
        ...
        if (k.substr(0, prefix.size()) == prefix) {
            ...
            if (!comp.IsNull()) {
                ...
                mat.rotZ(rotation);
                mat.move(point[0] * optionScaling, point[1] * optionScaling,
                         point[2] * optionScaling);
...
void ImpExpDxfRead::OnReadDimension(const double *s, const double *e, const double *point,
                                    double /*rotation*/)
{
    if (optionImportAnnotations) {
        Base::Interpreter().runString("import Draft");
        Base::Interpreter().runStringArg("p1=FreeCAD.Vector(%f,%f,%f)", s[0] * optionScaling,
                                         s[1] * optionScaling, s[2] * optionScaling);
        Base::Interpreter().runStringArg("p2=FreeCAD.Vector(%f,%f,%f)", e[0] * optionScaling,
                                         e[1] * optionScaling, e[2] * optionScaling);
        Base::Interpreter().runStringArg("p3=FreeCAD.Vector(%f,%f,%f)", point[0] * optionScaling,
                                         point[1] * optionScaling, point[2] * optionScaling);
...
void ImpExpDxfRead::AddObject(Part::TopoShape *shape)
{
    ...
    if (layers.count(LayerName())) vec = layers[LayerName()];
    vec.push_back(shape);
    layers[LayerName()] = vec;
    if (!optionGroupLayers) {
        if (LayerName().substr(0, 6) != "BLOCKS") {
            Part::Feature *pcFeature =
                (Part::Feature *)document->addObject("Part::Feature", "Shape");
    ...
    if (optionGroupLayers) {
        for (std::map<std::string, std::vector<Part::TopoShape *>>::const_iterator i =
            ...
            if (k.substr(0, 6) != "BLOCKS") {
                ...
                if (!comp.IsNull()) {
                    Part::Feature *pcFeature =
                        (Part::Feature *)document->addObject("Part::Feature", k.c_str());
...
ImpExpDxfWrite::ImpExpDxfWrite(std::string filepath) : CDxfWrite(filepath.c_str())
{
...
void ImpExpDxfWrite::exportShape(const TopoDS_Shape input)
{
    ...
    for (int i = 1; edges.More(); edges.Next(), i++) {
        ...
        if (adapt.GetType() == GeomAbs_Circle) {
            ...
            if (fabs(l - f) > 1.0 && s.SquareDistance(e) < 0.001) { exportCircle(adapt); }
            else {
                exportArc(adapt);
            }
        }
            ...
            if (fabs(l - f) > 1.0 && s.SquareDistance(e) < 0.001) {
                if (m_polyOverride) {
                    if (m_version >= 14) { exportLWPoly(adapt); }
                    else { //m_version < 14
                    ...
                    if (m_version < 14) { exportPolyline(adapt); }
                    else {
                        exportEllipse(adapt);
                    }
                }
...
{
    bool result = false;
    if (p1.IsEqual(p2, Precision::Confusion())) { result = true; }
    return result;
}
...
void ImpExpDxfWrite::exportBSpline(BRepAdaptor_Curve &c)
{
    ...
    if (spline->IsPeriodic()) {
        ...
        for (int i = 1; i <= spline->NbKnots(); i++) m += spline->Multiplicity(i);
    }
...
void ImpExpDxfWrite::exportText(const char *text, Base::Vector3d position1,
                                Base::Vector3d position2, double size, int just)
{
...
void ImpExpDxfWrite::exportAngularDim(Base::Vector3d textLocn, Base::Vector3d lineLocn,
                                      ...
                                      Base::Vector3d apexPoint, char *dimText)
{
    double text[3] = {0, 0, 0};
    text[0] = textLocn.x;
    text[1] = textLocn.y;
...
void ImpExpDxfWrite::exportRadialDim(Base::Vector3d centerPoint, Base::Vector3d textLocn,
                                     Base::Vector3d arcPoint, char *dimText)
{
    double center[3] = {0, 0, 0};
...
void ImpExpDxfWrite::exportDiametricDim(Base::Vector3d textLocn, Base::Vector3d arcPoint1,
                                        Base::Vector3d arcPoint2, char *dimText)
{
    double text[3] = {0, 0, 0};
    text[0] = textLocn.x;
    text[1] = textLocn.y;

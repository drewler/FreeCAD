...
public:
    Module() : Py::ExtensionModule<Module>("Import")
    {
        ...
        add_varargs_method("readDXF", &Module::readDXF,
                           "readDXF(filename,[document,ignore_errors]): Imports a DXF file into "
                           "the given document. ignore_errors is True by default.");
        ...
        add_varargs_method(
            "writeDXFObject", &Module::writeDXFObject,
...
private:
    Py::Object importer(const Py::Tuple &args, const Py::Dict &kwds)
    {
        ...
        int mode = -1;
        static char *kwd_list[] = {"name",         "docName", "importHidden", "merge",
                                   "useLinkGroup", "mode",    nullptr};
        if (!PyArg_ParseTupleAndKeywords(args.ptr(), kwds.ptr(), "et|sO!O!O!i", kwd_list, "utf-8",
                                         &Name, &DocName, &PyBool_Type, &importHidden, &PyBool_Type,
                                         &merge, &PyBool_Type, &useLinkGroup, &mode))
            throw Py::Exception();
        ...
        try {
            ...
            if (DocName) { pcDoc = App::GetApplication().getDocument(DocName); }
            if (!pcDoc) { pcDoc = App::GetApplication().newDocument("Unnamed"); }
            ...
            if (merge != Py_None) ocaf.setMerge(Base::asBoolean(merge));
            if (importHidden != Py_None) ocaf.setImportHiddenObject(Base::asBoolean(importHidden));
            if (useLinkGroup != Py_None) ocaf.setUseLinkGroup(Base::asBoolean(useLinkGroup));
            if (mode >= 0) ocaf.setMode(mode);
            ocaf.loadShapes();
#elif 1
    ...
    Py::Object exporter(const Py::Tuple &args, const Py::Dict &kwds)
    {
        ...
        static char *kwd_list[] = {"obj",    "name",          "exportHidden",
                                   "legacy", "keepPlacement", nullptr};
        if (!PyArg_ParseTupleAndKeywords(args.ptr(), kwds.ptr(), "Oet|O!O!O!", kwd_list, &object,
                                         "utf-8", &Name, &PyBool_Type, &exportHidden, &PyBool_Type,
                                         &legacy, &PyBool_Type, &keepPlacement))
            throw Py::Exception();
        ...
        try {
            ...
            if (!Base::asBoolean(legacy) || !ocaf.canFallback(objs)) {
                ...
                if (keepPlacement != Py_None) ocaf.setKeepPlacement(Base::asBoolean(keepPlacement));
            ...
            if (file.hasExtension("stp") || file.hasExtension("step")) {
                ...
                makeHeader.SetAuthorValue(
                    1, new TCollection_HAsciiString(hGrp->GetASCII("Author", "Author").c_str()));
                makeHeader.SetOrganizationValue(
                    1, new TCollection_HAsciiString(hGrp->GetASCII("Company").c_str()));
                ...
                header.SetAuthorName(new TCollection_HAsciiString(
                    Interface_Static::CVal("write.iges.header.author")));
                header.SetCompanyName(new TCollection_HAsciiString(
                    Interface_Static::CVal("write.iges.header.company")));
                header.SetSendName(new TCollection_HAsciiString(
    ...
    Py::Object readDXF(const Py::Tuple &args)
    {
        ...
        if (!PyArg_ParseTuple(args.ptr(), "et|sbs", "utf-8", &Name, &DocName, &IgnoreErrors,
                              &optionSource))
            throw Py::Exception();
        ...
        if (!file.exists()) throw Py::RuntimeError("File doesn't exist");

        if (optionSource) defaultOptions = optionSource;
    ...
    Py::Object writeDXFShape(const Py::Tuple &args)
    {
        ...
        if (PyArg_ParseTuple(args.ptr(), "O!et|iOs", &(PyList_Type), &shapeObj, "utf-8", &fname,
                             &versionParm, &usePolyline, &optionSource)) {
            filePath = std::string(fname);
            layerName = "none";
            PyMem_Free(fname);

            if ((versionParm == 12) || (versionParm == 14)) { versionOverride = true; }
            if (usePolyline == Py_True) { polyOverride = true; }
            if (optionSource) { defaultOptions = optionSource; }
            ...
            try {
                ...
                writer.setOptions();
                if (versionOverride) { writer.setVersion(versionParm); }
        ...
        PyErr_Clear();
        if (PyArg_ParseTuple(args.ptr(), "O!et|iOs", &(Part::TopoShapePy::Type), &shapeObj, "utf-8",
                             &fname, &versionParm, &usePolyline, &optionSource)) {
            filePath = std::string(fname);
            layerName = "none";
            PyMem_Free(fname);
            ...
            try {
                ...
                writer.init();
                Part::TopoShape *obj =
                    static_cast<Part::TopoShapePy *>(shapeObj)->getTopoShapePtr();
    ...
    Py::Object writeDXFObject(const Py::Tuple &args)
    {
        ...
        if (PyArg_ParseTuple(args.ptr(), "O!et|iOs", &(PyList_Type), &docObj, "utf-8", &fname,
                             &versionParm, &usePolyline, &optionSource)) {
            filePath = std::string(fname);
            layerName = "none";
            PyMem_Free(fname);
        ...
        PyErr_Clear();
        if (PyArg_ParseTuple(args.ptr(), "O!et|iOs", &(App::DocumentObjectPy::Type), &docObj,
                             "utf-8", &fname, &versionParm, &usePolyline, &optionSource)) {
            filePath = std::string(fname);
            layerName = "none";
            PyMem_Free(fname);

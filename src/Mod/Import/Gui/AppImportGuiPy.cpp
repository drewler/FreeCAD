...
{
public:
    explicit OCAFBrowser(Handle(TDocStd_Document) h) : pDoc(h)
    {
...
void OCAFBrowser::load(const TDF_Label &label, QTreeWidgetItem *item, const QString &s)
{
    ...
    for (; itr.More(); itr.Next()) { localList.Append(itr.Value()->ID()); }
#endif
            ...
            if (it.Value() == TDataStd_Name::GetID()) {
                ...
                if (!shape.IsNull()) {
                    switch (shape.ShapeType()) {
                        case TopAbs_COMPOUND: str << "COMPOUND PRIMITIVE"; break;
                        case TopAbs_COMPSOLID: str << "COMPSOLID PRIMITIVE"; break;
                        case TopAbs_SOLID: str << "SOLID PRIMITIVE"; break;
                        case TopAbs_SHELL: str << "SHELL PRIMITIVE"; break;
                        case TopAbs_FACE: str << "FACE PRIMITIVE"; break;
                        case TopAbs_WIRE: str << "WIRE PRIMITIVE"; break;
                        case TopAbs_EDGE: str << "EDGE PRIMITIVE"; break;
                        case TopAbs_VERTEX: str << "VERTEX PRIMITIVE"; break;
                        case TopAbs_SHAPE: str << "SHAPE PRIMITIVE"; break;
                    }
                }
                child->setText(0, text);
            }
...
private:
    ...
    void applyEdgeColors(Part::Feature *part, const std::vector<App::Color> &colors) override
    {
        ...
        if (colors.size() == 1) vp->LineColor.setValue(colors.front());
        else
    ...
    void applyLinkColor(App::DocumentObject *obj, int index, App::Color color) override
    {
        ...
        if (vp->MaterialList.getSize() <= index) vp->MaterialList.setSize(index + 1, mat);
...
private:
    ...
    {
        auto vp = Gui::Application::Instance->getViewProvider(obj);
        if (vp) return vp->getElementColors(subname);
    ...
    Py::Object exporter(const Py::Tuple &args, const Py::Dict &kwds)
    {
        ...
        try {
            ...
            if (file.hasExtension("stp") || file.hasExtension("step")) {
                ...
                std::string scheme =
                    hGrp_stp->GetASCII("Scheme", Interface_Static::CVal("write.step.schema"));

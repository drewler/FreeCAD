...
void PropertyGreyValueList::setSize(int newSize) { _lValueList.resize(newSize); }

int PropertyGreyValueList::getSize() const { return static_cast<int>(_lValueList.size()); }
...
void PropertyGreyValueList::SaveDocFile(Base::Writer &writer) const
{
    ...
    str << uCt;
    for (std::vector<float>::const_iterator it = _lValueList.begin(); it != _lValueList.end();
         ++it) {
        str << *it;
    }
}
...
void PropertyGreyValueList::RestoreDocFile(Base::Reader &reader)
{
    ...
    for (std::vector<float>::iterator it = values.begin(); it != values.end(); ++it) { str >> *it; }
    setValues(values);
}
...
void PropertyGreyValueList::removeIndices(const std::vector<unsigned long> &uIndices)
{
    ...
    if (uSortedInds.size() > rValueList.size()) return;
    ...
    for (std::vector<float>::const_iterator it = rValueList.begin(); it != rValueList.end(); ++it) {
        ...
        if (pos == uSortedInds.end()) remainValue.push_back(*it);
        else if (index != *pos)
...
#ifdef _MSC_VER
    Concurrency::parallel_for_each(_lValueList.begin(), _lValueList.end(),
                                   [rot](Base::Vector3f &value) { value = rot * value; });
#else
    QtConcurrent::blockingMap(_lValueList,
                              [rot](Base::Vector3f &value) { rot.multVec(value, value); });
#endif
...
std::vector<float> PropertyCurvatureList::getCurvature(int mode) const
{
    ...
    if (mode == MeanCurvature) {
        for (std::vector<Points::CurvatureInfo>::const_iterator it = fCurvInfo.begin();
             it != fCurvInfo.end(); ++it) {
...
void PropertyCurvatureList::removeIndices(const std::vector<unsigned long> &uIndices)
{
    ...
    if (uSortedInds.size() > _lValueList.size()) return;

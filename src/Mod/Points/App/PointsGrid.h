...
public:
   ...
   * automatically. */
    ...
    inline Base::BoundBox3d GetBoundBox(unsigned long ulX, unsigned long ulY,
   ...
   * the nearest grid element are taken.*/
    virtual void Position(const Base::Vector3d &rclPoint, unsigned long &rulX, unsigned long &rulY,
...
protected:
    ...
    unsigned long HasElements() const { return _pclPoints->size(); }
...
protected:
   ...
   * the corresponding index in the point kernel. */
    ...
    void Pos(const Base::Vector3d &rclPoint, unsigned long &rulX, unsigned long &rulY,
             unsigned long &rulZ) const;
};
...
public:
    ...
    Base::BoundBox3d GetBoundBox() const { return _rclGrid.GetBoundBox(_ulX, _ulY, _ulZ); }
    ...
    bool InitOnRay(const Base::Vector3d &rclPt, const Base::Vector3d &rclDir,
                   std::vector<unsigned long> &raulElements);
    /** Searches for facets around the ray. */
    bool InitOnRay(const Base::Vector3d &rclPt, const Base::Vector3d &rclDir, float fMaxSearchArea,
                   std::vector<unsigned long> &raulElements);
    /** Searches for facets around the ray. */
...
inline Base::BoundBox3d PointsGrid::GetBoundBox(unsigned long ulX, unsigned long ulY,
                                                unsigned long ulZ) const
{
    double fX, fY, fZ;

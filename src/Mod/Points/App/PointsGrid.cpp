...
PointsGrid::PointsGrid(const PointKernel &rclM)
    : _pclPoints(&rclM), _ulCtElements(0), _ulCtGridsX(0), _ulCtGridsY(0), _ulCtGridsZ(0),
...
PointsGrid::PointsGrid()
    : _pclPoints(nullptr), _ulCtElements(0), _ulCtGridsX(POINTS_CT_GRID),
      _ulCtGridsY(POINTS_CT_GRID), _ulCtGridsZ(POINTS_CT_GRID), _fGridLenX(0.0f), _fGridLenY(0.0f),
      _fGridLenZ(0.0f), _fMinX(0.0f), _fMinY(0.0f), _fMinZ(0.0f)
{}

PointsGrid::PointsGrid(const PointKernel &rclM, unsigned long ulX, unsigned long ulY,
                       unsigned long ulZ)
    : _pclPoints(&rclM), _ulCtElements(0), _ulCtGridsX(0), _ulCtGridsY(0), _ulCtGridsZ(0),
      _fGridLenX(0.0f), _fGridLenY(0.0f), _fGridLenZ(0.0f), _fMinX(0.0f), _fMinY(0.0f), _fMinZ(0.0f)
{
...
unsigned long PointsGrid::InSide(const Base::BoundBox3d &rclBB,
                                 std::vector<unsigned long> &raulElements, bool bDelDoubles) const
{
...
unsigned long PointsGrid::InSide(const Base::BoundBox3d &rclBB,
                                 ...
                                 const Base::Vector3d &rclOrg, double fMaxDist,
...
void PointsGrid::Position(const Base::Vector3d &rclPoint, unsigned long &rulX, unsigned long &rulY,
...
void PointsGrid::CalculateGridLength(int iCtGridPerAxis)
{
    ...
    switch (iFlag) {
        case 0: {
            ...
            if ((fVolumenGrid * iMaxGrids) < fVolumen) fVolumenGrid = fVolumen / (float)iMaxGrids;
        ...
        case 1: {
            ...
            if ((fAreaGrid * iMaxGrids) < fArea) fAreaGrid = fArea / (double)iMaxGrids;
        ...
        case 4: {
            ...
            if ((fAreaGrid * iMaxGrids) < fArea) fAreaGrid = fArea / (float)iMaxGrids;
                                        ...
                                        std::set<unsigned long> &raclInd) const
{
    ...
    if (clBB.IsInBox(rclPt)) { // Point lies within
        ...
        while (raclInd.empty()) GetHull(ulX, ulY, ulZ, ulLevel++, raclInd);
                         ...
                         unsigned long ulDistance, std::set<unsigned long> &raclInd) const
{
    ...
    // top plane
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) GetElements(i, j, nZ1, raclInd);
    }
    // bottom plane
    for (i = nX1; i <= nX2; i++) {
        for (j = nY1; j <= nY2; j++) GetElements(i, j, nZ2, raclInd);
    }
    // left plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) GetElements(nX1, i, j, raclInd);
    }
    // right plane
    for (i = nY1; i <= nY2; i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) GetElements(nX2, i, j, raclInd);
    }
    // front plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) GetElements(i, nY1, j, raclInd);
    }
    // back plane
    for (i = (nX1 + 1); i <= (nX2 - 1); i++) {
        for (j = (nZ1 + 1); j <= (nZ2 - 1); j++) GetElements(i, nY2, j, raclInd);
    }
}
...
{
    if (_pclPoints != &rclPoints) Attach(rclPoints);
...
{
    if (!_pclPoints) return;

    if (_pclPoints->size() != _ulCtElements) RebuildGrid();
}

bool PointsGrid::Verify() const
{
    if (!_pclPoints) return false;                         // no point cloud attached
    if (_pclPoints->size() != _ulCtElements) return false; // not up-to-date

    PointsGridIterator it(*this);
    for (it.Init(); it.More(); it.Next()) {
        std::vector<unsigned long> aulElements;
        it.GetElements(aulElements);
        for (std::vector<unsigned long>::iterator itP = aulElements.begin();
...
void PointsGrid::Pos(const Base::Vector3d &rclPoint, unsigned long &rulX, unsigned long &rulY,
                     unsigned long &rulZ) const
{
...
bool PointsGridIterator::InitOnRay(const Base::Vector3d &rclPt, const Base::Vector3d &rclDir,
                                   float fMaxSearchArea, std::vector<unsigned long> &raulElements)
{
...
bool PointsGridIterator::NextOnRay(std::vector<unsigned long> &raulElements)
{
    ...
    if ((_clPt - clIntersectPoint).Length() > _fMaxSearchArea) { _bValidRay = false; }
        ...
        switch (tSide) {
            ...
            default:
            case Base::BoundBox3d::INVALID: _bValidRay = false; break;
        }

...
private:
    ...
    Py::Object open(const Py::Tuple &args)
    {
        ...
        try {
            ...
            if (file.hasExtension("asc")) { reader.reset(new AscReader); }
            else if (file.hasExtension("e57")) {
                auto setting = readE57Settings();
                reader.reset(new E57Reader(std::get<0>(setting), std::get<1>(setting),
                                           std::get<2>(setting)));
            }
            ...
            if (reader->hasProperties()) {
                ...
                if (reader->hasIntensities()) {
                    Points::PropertyGreyValueList *prop =
                        static_cast<Points::PropertyGreyValueList *>(pcFeature->addDynamicProperty(
                            "Points::PropertyGreyValueList", "Intensity"));
                    if (prop) { prop->setValues(reader->getIntensities()); }
                }
                // add colors
                if (reader->hasColors()) {
                    App::PropertyColorList *prop = static_cast<App::PropertyColorList *>(
                        pcFeature->addDynamicProperty("App::PropertyColorList", "Color"));
                    if (prop) { prop->setValues(reader->getColors()); }
                }
                // add normals
                if (reader->hasNormals()) {
                    ...
                    if (prop) { prop->setValues(reader->getNormals()); }
                }
        ...
        for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
            ...
            if (PyObject_TypeCheck(item, &(App::DocumentObjectPy::Type))) {
                ...
                if (obj->getTypeId().isDerivedFrom(pointsId)) {
                    ...
                    if (file.hasExtension("asc")) { writer.reset(new AscWriter(kernel)); }
                    else if (file.hasExtension("ply")) {

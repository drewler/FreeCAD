...
CmdPointsImport::CmdPointsImport() : Command("Points_Import")
{
    sAppModule = "Points";
...
void CmdPointsImport::activated(int iMsg)
{
    ...
    if (!fn.isEmpty()) {
        ...
        doCommand(Command::Doc, "Points.insert(\"%s\", \"%s\")", fn.toUtf8().data(),
...
CmdPointsExport::CmdPointsExport() : Command("Points_Export")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
void CmdPointsExport::activated(int iMsg)
{
    ...
    for (std::vector<App::DocumentObject *>::const_iterator it = points.begin(); it != points.end();
         ++it) {
...
CmdPointsTransform::CmdPointsTransform() : Command("Points_Transform")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
CmdPointsConvert::CmdPointsConvert() : Command("Points_Convert")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
void CmdPointsConvert::activated(int iMsg)
{
    ...
    double tol = QInputDialog::getDouble(
        Gui::getMainWindow(), QObject::tr("Distance"), QObject::tr("Enter maximum distance:"), 0.1,
        minimal_tolerance, 10.0, decimals, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (!ok) return;
    ...
    auto run_python = [](const std::vector<App::GeoFeature *> &geoObject, double tol) -> bool {
        ...
        for (auto it : geoObject) {
            ...
            if (prop) { list.append(Py::asObject(it->getPyObject())); }
        }
    ...
    try {
        if (run_python(geoObject, tol)) commitCommand();
        else
            abortCommand();
    }
...
CmdPointsPolyCut::CmdPointsPolyCut() : Command("Points_PolyCut")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
CmdPointsMerge::CmdPointsMerge() : Command("Points_Merge")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
void CmdPointsMerge::activated(int iMsg)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = docObj.begin(); it != docObj.end();
        ...
        for (std::size_t i = 0; i < k.size(); ++i) { kernel->setPoint(i + numPts, k.getPoint(i)); }
    }
...
CmdPointsStructure::CmdPointsStructure() : Command("Points_Structure")
{
    sAppModule = "Points";
    sGroup = QT_TR_NOOP("Points");
...
void CmdPointsStructure::activated(int iMsg)
{
    ...
    for (auto it : docObj) {
        ...
        if (it->getTypeId().isDerivedFrom(Points::Structured::getClassTypeId())) {
            ...
            for (std::size_t i = 0; i < k.size(); ++i) { kernel->setPoint(i, k.getPoint(i)); }

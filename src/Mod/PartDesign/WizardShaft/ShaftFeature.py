...
class ShaftFeature:
    ...
    def addSegment(self, length, diameter, innerdiameter):
        ...
        if oldLength == 0:
            ...
            self.sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(0, 0, 0), self.App.Vector(self.totalLength, 0, 0)
                )
            )
            ...
            self.sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(0, innerradius, 0), self.App.Vector(0, radius, 0)
                )
            )
            self.sketch.addConstraint(
            ...
            self.sketch.addGeometry(
                Part.LineSegment(
                    self.App.Vector(0, radius, 0), self.App.Vector(length, radius, 0)
                )
            )
            self.sketch.addConstraint(
                Sketcher.Constraint("DistanceX", 2, length)
            )  # Constraint8
            self.sketch.addConstraint(
            ...
            self.sketch.addConstraint(
                Sketcher.Constraint("DistanceY", -1, 1, segRadius, 2, radius)
            )
            self.sketch.addConstraint(
            ...
            self.sketch.addConstraint(
                Sketcher.Constraint("DistanceX", segLength, length)
            )
            self.sketch.addConstraint(
                Sketcher.Constraint("Coincident", segLength, 1, segRadius, 2)
            )
        ...
        self.sketch.addConstraint(
            Sketcher.Constraint("Coincident", segEnd, 1, segLength, 2)
        )
        ...
        if oldLength == 0:
            ...
            self.feature = self.Doc.addObject(
                "PartDesign::Revolution", "RevolutionShaft"
            )
    ...
    def updateSegment(self, segment, oldLength, length, diameter, innerdiameter):
        ...
        constrInnerRadius = (
            1  # Currently we don't allow multiple different innner diameters
        )
        # update total length

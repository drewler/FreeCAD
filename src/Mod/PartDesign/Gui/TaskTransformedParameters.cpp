...
TaskTransformedParameters::TaskTransformedParameters(ViewProviderTransformed *TransformedView,
                                                     QWidget *parent)
    : TaskBox(Gui::BitmapFactory().pixmap(
                  (std::string("PartDesign_") + TransformedView->featureName).c_str()),
              TransformedView->menuName, true, parent),
      proxy(nullptr), TransformedView(TransformedView), parentTask(nullptr),
      insideMultiTransform(false), blockUpdate(false)
{
    selectionMode = none;
...
TaskTransformedParameters::TaskTransformedParameters(TaskMultiTransformParameters *parentTask)
    : TaskBox(QPixmap(), tr(""), true, parentTask), proxy(nullptr), TransformedView(nullptr),
      parentTask(parentTask), insideMultiTransform(true), blockUpdate(false)
{
...
{
    if (TransformedView == &Obj) TransformedView = nullptr;
}

bool TaskTransformedParameters::isViewUpdated() const { return (blockUpdate == false); }

int TaskTransformedParameters::getUpdateViewTimeout() const { return 500; }
...
bool TaskTransformedParameters::originalSelected(const Gui::SelectionChanges &msg)
{
    if (msg.Type == Gui::SelectionChanges::AddSelection
        ...
        if (strcmp(msg.pDocName, getObject()->getDocument()->getName()) != 0) return false;
...
{
    if (!isEnabledTransaction()) return;
    ...
    if (tid && tid == transactionID) return;
...
void TaskTransformedParameters::setEnabledTransaction(bool on) { enableTransaction = on; }

bool TaskTransformedParameters::isEnabledTransaction() const { return enableTransaction; }
...
void TaskTransformedParameters::recomputeFeature() { getTopTransformedView()->recomputeFeature(); }
...
PartDesignGui::ViewProviderTransformed *TaskTransformedParameters::getTopTransformedView() const
{
    ...
    if (insideMultiTransform) { rv = parentTask->TransformedView; }
    else {
        rv = TransformedView;
    }
    return rv;
...
{
    if (insideMultiTransform) return parentTask->getSubFeature();
...
App::DocumentObject *TaskTransformedParameters::getBaseObject() const
{
    ...
    if (!base) {
        ...
        if (body) base = body->getPrevSolidFeature(feature);
    }
    return base;
}
...
ComboLinks::ComboLinks(QComboBox &combo) : doc(nullptr)
{
    this->_combo = &combo;
    _combo->clear();
}
...
int ComboLinks::addLink(const App::PropertyLinkSub &lnk, QString itemText)
{
    ...
    newitem.Paste(lnk);
    if (newitem.getValue() && !this->doc) this->doc = newitem.getValue()->getDocument();
...
{
    for (size_t i = 0; i < this->linksInList.size(); i++) { delete linksInList[i]; }
    if (this->_combo) _combo->clear();
}

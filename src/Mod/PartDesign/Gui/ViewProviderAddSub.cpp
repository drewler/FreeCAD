...
void ViewProviderAddSub::updateAddSubShapeIndicator()
{
    ...
    try {
        ...
        for (int i = 0; i < numNorms; i++) previewNorms[i] = SbVec3f(0.0, 0.0, 0.0);
        ...
        for (int i = 0; i < numNorms; i++) previewNorms[i].normalize();
...
void ViewProviderAddSub::updateData(const App::Property *p)
{
    ...
    if (p->getName() && strcmp(p->getName(), "AddSubShape") == 0) updateAddSubShapeIndicator();
...
void ViewProviderAddSub::setPreviewDisplayMode(bool onoff)
{
    ...
    if (onoff) {
        if (pcModeSwitch->getChild(getDefaultMode()) == previewShape) return;

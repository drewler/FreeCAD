...
TaskDatumParameters::TaskDatumParameters(ViewProviderDatum *ViewProvider, QWidget *parent)
    : PartGui::TaskAttacher(ViewProvider, parent,
...
bool TaskDlgDatumParameters::reject() { return PartGui::TaskDlgAttacher::reject(); }
...
bool TaskDlgDatumParameters::accept()
{
    ...
    if (parameter->getActiveMapMode() == mmDeactivated) {
        ...
        msg.exec();
        if (msg.buttonRole(msg.clickedButton()) == QMessageBox::NoRole) return false;
    }
            ...
            for (App::DocumentObject *obj : pcDatum->Support.getValues()) {
                if (pcActiveBody && !pcActiveBody->hasObject(obj)
                    ...
                    auto *copy = PartDesignGui::TaskFeaturePick::makeCopy(
                        obj, subs[index], dlg.radioIndependent->isChecked());
                    if (copy) {
    ...
    if (!PartGui::TaskDlgAttacher::accept()) return false;

...
WorkflowManager::WorkflowManager()
{
    ...
    for (auto doc : App::GetApplication().getDocuments()) { slotFinishRestoreDocument(*doc); }
...
{
    if (!_instance) { _instance = new WorkflowManager(); }
    else {
...
{
    if (!_instance) { WorkflowManager::init(); }
    return _instance;
}
...
void WorkflowManager::slotFinishRestoreDocument(const App::Document &doc)
{
    ...
    if (wf != Workflow::Modern) { wf = Workflow::Undetermined; }
    dwMap[&doc] = wf;
}

void WorkflowManager::slotDeleteDocument(const App::Document &doc) { dwMap.erase(&doc); }
...
Workflow WorkflowManager::getWorkflowForDocument(App::Document *doc)
{
    ...
    if (it != dwMap.end()) { return it->second; }
    else {
    ...
    if (rv != Workflow::Modern) {
        ...
        if (rv == Workflow::Legacy) { // legacy messages
            ...
            msgBox.setText(QObject::tr("The document \"%1\" seems to be either in the middle of"
                                       " the migration process from legacy PartDesign or have a "
                                       "slightly broken structure.")
                               .arg(QString::fromStdString(doc->getName())));
            msgBox.setInformativeText(
        ...
        if (rv != Workflow::Undetermined) { msgBox.addButton(QMessageBox::No); }
...
void WorkflowManager::forceWorkflow(const App::Document *doc, Workflow wf) { dwMap[doc] = wf; }

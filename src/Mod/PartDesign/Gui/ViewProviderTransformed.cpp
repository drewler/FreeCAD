...
{
    QAction *act;
    act = menu->addAction(QObject::tr("Edit %1").arg(QString::fromStdString(featureName)), receiver,
                          member);
    ...
    if (!pcTransformed->Originals.getSize()) {
        for (auto obj : pcTransformed->getInList()) {
            if (obj->isDerivedFrom(PartDesign::MultiTransform::getClassTypeId())) {
                ...
                if (vp) return vp->startEditing(ModNum);
                return nullptr;
            }
        }
    }
...
bool ViewProviderTransformed::setEdit(int ModNum)
{
    ...
    pcRejectedRoot->addChild(
        rejectedNormb); // NOTE: The code relies on the last child added here being index 6
...
void ViewProviderTransformed::recomputeFeature(bool recompute)
{
    ...
    if (rejected > 0) { showRejectedShape(cShape); }
}
...
void ViewProviderTransformed::showRejectedShape(TopoDS_Shape shape)
{
    try {
        ...
        BRepMesh_IncrementalMesh(shape, deflection, Standard_False, AngDeflectionRads,
                                 Standard_True);
        ...
        for (int i = 0; i < nbrNodes; i++) norms[i] = SbVec3f(0.0, 0.0, 0.0);
        ...
        for (Ex.Init(shape, TopAbs_FACE); Ex.More(); Ex.Next()) {
            ...
            if (!Part::Tools::getTriangulation(actFace, points, facets)) continue;
            ...
            for (int i = 0; i < nbrNodes; i++) norms[i].normalize();
        ...
        Base::Console().Error("Cannot compute Inventor representation for the rejected "
                              "transformations of shape of %s.\n",

...
public:
    ReferenceSelection(const App::DocumentObject *support_, AllowSelectionFlags type)
        : Gui::SelectionFilterGate(nullPointer()), support(support_), type(type)
    {}
    /**
...
private:
    ...
    bool allowOrigin(PartDesign::Body *body, App::OriginGroupExtension *originGroup,
...
public:
    CombineSelectionFilterGates(std::unique_ptr<Gui::SelectionFilterGate> &filter1_,
                                std::unique_ptr<Gui::SelectionFilterGate> &filter2_)
        : Gui::SelectionFilterGate(nullPointer()), filter1(std::move(filter1_)),

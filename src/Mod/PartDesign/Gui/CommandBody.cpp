...
CmdPartDesignBody::CmdPartDesignBody() : Command("PartDesign_Body")
{
...
void CmdPartDesignBody::activated(int iMsg)
{
    ...
    if (PartDesignGui::assureLegacyWorkflow(getDocument())) return;
    ...
    if (!features.empty()) {
        if (features.size() == 1) {
            ...
            if (baseFeature->isDerivedFrom(PartDesign::Feature::getClassTypeId())
                ...
                QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Bad base feature"),
                                     QObject::tr("%1 already belongs to a body, can't use it as "
                                                 "base feature for another body.")
                ...
                if (partOfBaseFeature && partOfBaseFeature != actPart) {
                    ...
                    QMessageBox::warning(
                        Gui::getMainWindow(), QObject::tr("Bad base feature"),
                ...
                // the number of solids/shells
                    ...
                    if (!shape.IsNull()) {
                        ...
                        if (numSolids > 1 && numShells == 0) {
                            warning =
                                QObject::tr("The selected shape consists of multiple solids.\n"
                                            "This may lead to unexpected results.");
                        }
                        else if (numShells > 1 && numSolids == 0) {
                            warning =
                                QObject::tr("The selected shape consists of multiple shells.\n"
                                            "This may lead to unexpected results.");
                        }
                        else if (numShells == 1 && numSolids == 0) {
                        ...
                        if (!warning.isEmpty()) {
                            QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Base feature"),
                                                 warning);
                        }
                    }
                }
        ...
        if (body) {
            ...
            if (addtogroup) {
                ...
                if (validPlaneCount > 1) {
                    ...
                    auto worker =
                        [baseFeature](const std::vector<App::DocumentObject *> &features) {
                            ...
                            FCMD_OBJ_CMD(baseFeature,
                                         "MapMode = '" << Attacher::AttachEngine::getModeName(
                                             Attacher::mmFlatFace)
                                                       << "'");
                    ...
                    auto quitter = [docname]() {
                        Gui::Document *document =
                            Gui::Application::Instance->getDocument(docname.c_str());
...
CmdPartDesignMigrate::CmdPartDesignMigrate() : Command("PartDesign_Migrate")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignMigrate::activated(int iMsg)
{
    ...
    if (migrateFeatures.empty()) {
        if (!PartDesignGui::isModernWorkflow(doc)) {
            ...
            QMessageBox::warning(
                nullptr, QObject::tr("Nothing to migrate"),
    ...
    for (auto featIt = migrateFeatures.begin(); !migrateFeatures.empty();) {
        ...
        if (!base || !base->isDerivedFrom(PartDesign::Feature::getClassTypeId())
            ...
            if (baseFeatSetIt != migrateFeatures.end()) {
                ...
                auto chainIt =
                    std::find_if(featureChains.begin(), featureChains.end(),
                                 [baseFeat, &baseFeatIt](
                                     std::list<PartDesign::Feature *> &chain) mutable -> bool {
                ...
                if (chainIt != featureChains.end()) {
                    ...
                    if (std::next(baseFeatIt) == chainIt->end()) {
                        ...
                        newChainIt->splice(newChainIt->end(), *chainIt, std::next(baseFeatIt),
                                           chainIt->end());
                    }
                }
                else {
    ...
    for (auto chainIt = featureChains.begin(); !featureChains.empty();
        ...
        for (auto feature : *chainIt) {
            if (feature->isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
                ...
                PartDesign::ProfileBased *sketchBased =
                    static_cast<PartDesign::ProfileBased *>(feature);
...
bool CmdPartDesignMigrate::isActive() { return hasActiveDocument(); }
...
CmdPartDesignMoveTip::CmdPartDesignMoveTip() : Command("PartDesign_MoveTip")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignMoveTip::activated(int iMsg)
{
    ...
    if (!selFeature) {
        ...
        QMessageBox::warning(nullptr, QObject::tr("Selection error"),
                             QObject::tr("Couldn't determine a body for the selected feature '%s'.",
    ...
    if (selFeature == body) { FCMD_OBJ_CMD(body, "Tip = None"); }
    else {
...
bool CmdPartDesignMoveTip::isActive() { return hasActiveDocument(); }
...
void CmdPartDesignDuplicateSelection::activated(int iMsg)
{
    ...
    if (pcActiveBody) {
        ...
        std::set_difference(afterFeatures.begin(), afterFeatures.end(), beforeFeatures.begin(),
                            beforeFeatures.end(), std::back_inserter(newFeatures));
        ...
        if (!newFeatures.empty()) FCMD_OBJ_SHOW(newFeatures.back());
    }

    updateActive();
}

bool CmdPartDesignDuplicateSelection::isActive() { return hasActiveDocument(); }
...
CmdPartDesignMoveFeature::CmdPartDesignMoveFeature() : Command("PartDesign_MoveFeature")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignMoveFeature::activated(int iMsg)
{
    ...
    if (std::any_of(std::begin(features), std::end(features), [](App::DocumentObject *obj) {
        ...
        QMessageBox::warning(
            Gui::getMainWindow(), QObject::tr("Features cannot be moved"),
    ...
    std::vector<App::DocumentObject *> dependencies =
        PartDesignGui::collectMovableDependencies(features);
    ...
    std::vector<App::DocumentObject *> bodies =
        getDocument()->getObjectsOfType(Part::BodyBase::getClassTypeId());
    ...
    for (auto body : bodies) {
        if (!source_bodies.count(body)) target_bodies.push_back(body);
    }
    ...
    bool ok;
    QStringList items;
    for (auto body : target_bodies) { items.push_back(QString::fromUtf8(body->Label.getValue())); }
    QString text = QInputDialog::getItem(
        Gui::getMainWindow(), qApp->translate("PartDesign_MoveFeature", "Select body"),
        qApp->translate("PartDesign_MoveFeature", "Select a body from the list"), items, 0, false,
    ...
    for (auto feat : features) stream << ", " << getObjectCmd(feat);
...
void CmdPartDesignMoveFeatureInTree::activated(int iMsg)
{
    ...
    bool ok;
    QStringList items;
    if (bodyBase) { items.push_back(QString::fromUtf8(bodyBase->Label.getValue())); }
    else {
        items.push_back(QObject::tr("Beginning of the body"));
    }
    for (auto feat : model) { items.push_back(QString::fromUtf8(feat->Label.getValue())); }

    QString text = QInputDialog::getItem(
        Gui::getMainWindow(), qApp->translate("PartDesign_MoveFeatureInTree", "Select feature"),
        qApp->translate("PartDesign_MoveFeatureInTree", "Select a feature from the list"), items, 0,
        false, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (!ok) return;
    int index = items.indexOf(text);
    ...
    for (auto feat : features) {
        ...
        FCMD_OBJ_CMD(body,
                     "insertObject(" << getObjectCmd(feat) << "," << getObjectCmd(target)
                                     << ", True)");

        if (!lastObject) lastObject = feat;
    }
    ...
    for (size_t i = 0; i < bodyFeatures.size(); ++i) {
        ...
        for (auto obj : feat->getOutList()) {
            if (obj->isDerivedFrom(PartDesign::Feature::getClassTypeId())) continue;
    ...
    if (lastObject && body->Tip.getValue() == target
        ...
        msgBox.setText(qApp->translate("PartDesign_MoveFeatureInTree",
                                       "The moved feature appears after the currently set tip."));
        msgBox.setInformativeText(qApp->translate(
            "PartDesign_MoveFeatureInTree", "Do you want the last feature to be the new tip?"));
        msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
        msgBox.setDefaultButton(QMessageBox::No);
        int ret = msgBox.exec();
        if (ret == QMessageBox::Yes) FCMD_OBJ_CMD(body, "Tip = " << getObjectCmd(lastObject));
    }

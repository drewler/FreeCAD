    ...
    if (activeView) {
        ...
        if (assertModern && PartDesignGui::assureModernWorkflow(doc)) {
            ...
            if (!activeBody && messageIfNot) {
                ...
                if (dia.exec() == QDialog::DialogCode::Accepted) activeBody = dia.getActiveBody();
            }
        }
    }
                                 ...
                                 App::DocumentObject **topParent, std::string *subname)
{
    ...
    for (auto &v : body->getParents()) {
        if (v.first->getDocument() != doc) continue;
        if (parent) {
            body = nullptr;
            break;
        }
    ...
    if (body) {
        ...
        _FCMD_DOC_CMD(Gui, _doc,
                      "ActiveView.setActiveObject('"
                          << PDBODYKEY << "'," << Gui::Command::getObjectCmd(parent ? parent : body)
                          << ",'" << sub << "')");
        return Gui::Application::Instance->activeView()->getActiveObject<PartDesign::Body *>(
            PDBODYKEY, topParent, subname);
    }
...
{
    QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Active Body Required"),
                             ...
                             std::string *subname)
{
    ...
    if (rv && rv->hasObject(obj)) return rv;
...
{
    Gui::MDIView *activeView = Gui::Application::Instance->activeView();
    if (activeView) { return activeView->getActiveObject<App::Part *>(PARTKEY); }
    else {
        return nullptr;
    }
}
...
void fixSketchSupport(Sketcher::SketchObject *sketch)
{
    ...
    if (support) return; // Sketch is on a face of a solid, do nothing

    const App::Document *doc = sketch->getDocument();
    PartDesign::Body *body = getBodyFor(sketch, /*messageIfNot*/ 0);
    if (!body) { throw Base::RuntimeError("Couldn't find body for the sketch"); }
    ...
    if (sketchVector == Base::Vector3d(0, 0, 1)) plane = origin->getXY();
    ...
    if (fabs(offset) < Precision::Confusion()) {
        ...
        if ((a < -M_PI_2) || (a > M_PI_2)) offset *= -1.0;
...
void relinkToBody(PartDesign::Feature *feature)
{
    ...
    if (!body) { throw Base::RuntimeError("Couldn't find body for the feature"); }
        ...
        if (!isPartDesignAwareObjecta(obj)) {
            ...
            for (auto prop : properties) {
                ...
                if (prop->isDerivedFrom(App::PropertyLink::getClassTypeId())) {
                    ...
                    if (propLink->getValue() != feature) { continue; }
...
bool isFeatureMovable(App::DocumentObject *const feat)
{
    ...
    if (feat->getTypeId().isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
        ...
        if (!isFeatureMovable(sk)) return false;

        if (auto prop = static_cast<App::PropertyLinkList *>(prim->getPropertyByName("Sections"))) {
            if (std::any_of(prop->getValues().begin(), prop->getValues().end(),
        ...
        if (auto prop =
            ...
            if (axis && !isFeatureMovable(axis)) return false;
        }

        if (auto prop = static_cast<App::PropertyLinkSub *>(prim->getPropertyByName("Spine"))) {
            App::DocumentObject *spine = prop->getValue();
            if (spine && !isFeatureMovable(spine)) return false;
        }

        if (auto prop =
                static_cast<App::PropertyLinkSub *>(prim->getPropertyByName("AuxillerySpine"))) {
            App::DocumentObject *auxSpine = prop->getValue();
            if (auxSpine && !isFeatureMovable(auxSpine)) return false;
        }
    }
...
collectMovableDependencies(std::vector<App::DocumentObject *> &features)
{
    ...
    for (auto const &feat : features) {
        ...
        if (feat->getTypeId().isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
            ...
            if (sk) { unique_objs.insert(static_cast<App::DocumentObject *>(sk)); }
            if (auto prop =
                    static_cast<App::PropertyLinkList *>(prim->getPropertyByName("Sections"))) {
                for (App::DocumentObject *obj : prop->getValues()) { unique_objs.insert(obj); }
            }
            if (auto prop =
                    static_cast<App::PropertyLinkSub *>(prim->getPropertyByName("ReferenceAxis"))) {
                App::DocumentObject *axis = prop->getValue();
                if (axis
                    && !axis->getTypeId().isDerivedFrom(App::OriginFeature::getClassTypeId())) {
            ...
            if (auto prop = static_cast<App::PropertyLinkSub *>(

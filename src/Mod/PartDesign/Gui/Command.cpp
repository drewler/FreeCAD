...
void UnifiedDatumCommand(Gui::Command &cmd, Base::Type type, std::string name)
{
    try {
        ...
        if (support.getSize() == 1 && support.getValue()) {
            if (support.getValue()->isDerivedFrom(type)) bEditSelected = true;
        }
        ...
        if (bEditSelected) {
            ...
            if (support.getSize() > 0) {
                ...
                if (sugr.message == Attacher::SuggestResult::srOK) {
                    ...
                    FCMD_OBJ_CMD(
                        Feat, "MapMode = '" << AttachEngine::getModeName(sugr.bestFitMode) << "'");
                }
                else {
                    QMessageBox::information(
                        Gui::getMainWindow(), QObject::tr("Invalid selection"),
            ...
            cmd.doCommand(
                Gui::Command::Doc,
                "App.activeDocument().recompute()"); // recompute the feature based on its references
...
CmdPartDesignPlane::CmdPartDesignPlane() : Command("PartDesign_Plane")
{
...
CmdPartDesignLine::CmdPartDesignLine() : Command("PartDesign_Line")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
CmdPartDesignPoint::CmdPartDesignPoint() : Command("PartDesign_Point")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
CmdPartDesignCS::CmdPartDesignCS() : Command("PartDesign_CoordinateSystem")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
CmdPartDesignShapeBinder::CmdPartDesignShapeBinder() : Command("PartDesign_ShapeBinder")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignShapeBinder::isActive() { return hasActiveDocument(); }
...
CmdPartDesignSubShapeBinder::CmdPartDesignSubShapeBinder() : Command("PartDesign_SubShapeBinder")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignSubShapeBinder::activated(int iMsg)
{
    ...
    for (auto &sel : Gui::Selection().getCompleteSelection(Gui::ResolveMode::NoResolve)) {
        ...
        if (sel.SubName && sel.SubName[0]) subs.emplace_back(sel.SubName);
    }
    ...
    if (parent) {
        ...
        for (auto &v : values) {
            ...
            for (auto &sub : v.second) {
                ...
                if (link && link != pcActiveBody) links[link].push_back(sub);
            }
        }
    ...
    try {
        ...
        e.ReportException();
        QMessageBox::critical(Gui::getMainWindow(), QObject::tr("Sub-Shape Binder"),
                              QString::fromUtf8(e.what()));
        abortCommand();
    }
}

bool CmdPartDesignSubShapeBinder::isActive() { return hasActiveDocument(); }
...
CmdPartDesignClone::CmdPartDesignClone() : Command("PartDesign_Clone")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
CmdPartDesignNewSketch::CmdPartDesignNewSketch() : Command("PartDesign_NewSketch")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignNewSketch::activated(int iMsg)
{
    ...
    if (PartDesignGui::assureModernWorkflow(doc)) {
        ...
        if (!pcActiveBody) {
            if (doc->countObjectsOfType(PartDesign::Body::getClassTypeId()) == 0) {
                ...
                if (dia.exec() == QDialog::DialogCode::Accepted) pcActiveBody = dia.getActiveBody();
                if (!pcActiveBody) return;
            }
        }
    }
    ...
    if (PlaneFilter2.match()) PlaneFilter = PlaneFilter2;
    ...
    if (FaceFilter.match() || PlaneFilter.match()) {
        ...
        if (FaceFilter.match()) {
            ...
            if (!obj->isDerivedFrom(Part::Feature::getClassTypeId())) return;
            ...
            if (obj == pcActiveBody) {
                ...
                if (tip && tip->isDerivedFrom(Part::Feature::getClassTypeId())
                    ...
                    PartDesignGui::ViewProviderBody *vpBody =
                        dynamic_cast<PartDesignGui::ViewProviderBody *>(
                            Gui::Application::Instance->getViewProvider(pcActiveBody));
                    if (vpBody) { vpBody->DisplayModeBody.setValue("Through"); }
                }
            }
            ...
            if (subNames.size() > 1) {
                ...
                QMessageBox::warning(
                    Gui::getMainWindow(), QObject::tr("Several sub-elements selected"),
            ...
            if (face.IsNull()) {
                // No assert for wrong user input!
                QMessageBox::warning(
                    Gui::getMainWindow(), QObject::tr("No support face selected"),
            ...
            if (adapt.GetType() != GeomAbs_Plane) {
                ...
                if (surf.IsNull() || !GeomLib_IsPlanarSurface(surf).IsPlanar()) {
                    QMessageBox::warning(
                        Gui::getMainWindow(), QObject::tr("No planar support"),
        ...
        if (!pcActiveBody->hasObject(obj)) {
            if (!obj->isDerivedFrom(App::Plane::getClassTypeId())) {
                ...
                dia.setModal(true);
                int result = dia.exec();
                if (result == QDialog::DialogCode::Rejected) return;
                else if (!dlg.radioXRef->isChecked()) {
                    openCommand(QT_TRANSLATE_NOOP("Command", "Make copy"));
                    std::string sub;
                    if (FaceFilter.match()) sub = FaceFilter.Result[0][0].getSubNames()[0];
                    auto copy = PartDesignGui::TaskFeaturePick::makeCopy(
                        obj, sub, dlg.radioIndependent->isChecked());

                    if (pcActiveBody) pcActiveBody->addObject(copy);
                    else if (pcActivePart)
                        pcActivePart->addObject(copy);

                    if (PlaneFilter.match()) supportString = getObjectCmd(copy, "(", ",'')");
                    else
        ...
        if (pcActiveBody) {
            ...
            if (group) { geoGroup = group->getExtensionByType<App::GeoFeatureGroupExtension>(); }
        }
        ...
        auto shapeBinders(
            getDocument()->getObjectsOfType(PartDesign::ShapeBinder::getClassTypeId()));
        ...
        auto quitter = [docname]() {
            ...
            if (document) document->abortCommand();
        };

        if (validPlaneCount == 0) {
            QMessageBox::warning(
                Gui::getMainWindow(), QObject::tr("No valid planes in this document"),
            ...
            Gui::Control().showDialog(new PartDesignGui::TaskDlgFeaturePick(
                planes, status, accepter, worker, true, quitter));
        }
    }
}
                   ...
                   const bool updateDocument = true)
{
    ...
    if (hidePrevSolid && prevSolidFeature) FCMD_OBJ_HIDE(prevSolidFeature);

    if (updateDocument) cmd->updateActive();

    auto base = dynamic_cast<PartDesign::Feature *>(Feat);
    if (base) base = dynamic_cast<PartDesign::Feature *>(base->getBaseObject(true));
                          ...
                          std::vector<App::DocumentObject *>::iterator &firstFreeSketch)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator s = sketches.begin(); s != sketches.end();
        ...
        if (!pcActiveBody) {
            ...
            if (!b) status.push_back(PartDesignGui::TaskFeaturePick::notInBody);
        ...
        int ctWires = 0;
        TopExp_Explorer ex;
        for (ex.Init(shape, TopAbs_WIRE); ex.More(); ex.Next()) { ctWires++; }
        if (ctWires == 0) {
        ...
        if (firstFreeSketch == sketches.end()) firstFreeSketch = s;
        freeSketches++;
        status.push_back(PartDesignGui::TaskFeaturePick::validFeature);
    }
...
void prepareProfileBased(PartDesign::Body *pcActiveBody, Gui::Command *cmd,
                         ...
                         std::function<void(Part::Feature *, App::DocumentObject *)> func)
{
    auto base_worker = [=](App::DocumentObject *feature, const std::vector<string> &subs) {
        if (!feature || !feature->isDerivedFrom(Part::Feature::getClassTypeId())) return;
        ...
        if (feature->isTouched()) feature->recomputeFeature();
        ...
        auto runProfileCmd = [=]() { FCMD_OBJ_CMD(Feat, "Profile = " << objCmd); };
        ...
        if (which.compare("AdditiveLoft") == 0 || which.compare("SubtractiveLoft") == 0) {
            ...
            if (selection.size() == 2) { //treat additional selected object as spine
                ...
                if (selection[1].getObject()->isDerivedFrom(Part::Part2DObject::getClassTypeId())
                    ...
                    std::ostringstream ss;
                    for (auto &s : subnames) {
                        if (s.find("Edge") != std::string::npos) ss << "'" << s << "',";
                    }
    ...
    if (sketches
            .empty()) { //no sketches were selected. Let user pick an object from valid ones available in document
    ...
    if (pcActiveBody && !bNoSketchWasSelected && extReference) {
        ...
        if (!dlg.radioXRef->isChecked()) {
            ...
            if (oBody) pcActiveBody->addObject(copy);
            else if (pcActivePart)
                pcActivePart->addObject(copy);
    ...
    if (bNoSketchWasSelected && (freeSketches != 1)) {
        ...
        if (!bNoSketchWasSelected) theSketch.push_back(sketches[0]);
        else
...
void finishProfileBased(const Gui::Command *cmd, const Part::Feature *sketch,
...
CmdPartDesignPad::CmdPartDesignPad() : Command("PartDesign_Pad")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignPad::activated(int iMsg)
{
    Q_UNUSED(iMsg);
    App::Document *doc = getDocument();
    if (!PartDesignGui::assureModernWorkflow(doc)) return;

    prepareProfileBased(this, "Pad", 10.0);
}

bool CmdPartDesignPad::isActive() { return hasActiveDocument(); }
...
CmdPartDesignPocket::CmdPartDesignPocket() : Command("PartDesign_Pocket")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignPocket::isActive() { return hasActiveDocument(); }
...
CmdPartDesignHole::CmdPartDesignHole() : Command("PartDesign_Hole")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignHole::isActive() { return hasActiveDocument(); }
...
CmdPartDesignRevolution::CmdPartDesignRevolution() : Command("PartDesign_Revolution")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignRevolution::activated(int iMsg)
{
    ...
    auto worker = [cmd, &pcActiveBody](Part::Feature *sketch, App::DocumentObject *Feat) {
        ...
        if (pcRevolution && pcRevolution->suggestReversed()) FCMD_OBJ_CMD(Feat, "Reversed = 1");
...
bool CmdPartDesignRevolution::isActive() { return hasActiveDocument(); }
...
CmdPartDesignGroove::CmdPartDesignGroove() : Command("PartDesign_Groove")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignGroove::activated(int iMsg)
{
    ...
    auto worker = [cmd, &pcActiveBody](Part::Feature *sketch, App::DocumentObject *Feat) {
        ...
        try {
            ...
            if (pcGroove && pcGroove->suggestReversed()) FCMD_OBJ_CMD(Feat, "Reversed = 1");
        }
...
bool CmdPartDesignGroove::isActive() { return hasActiveDocument(); }
...
CmdPartDesignAdditivePipe::CmdPartDesignAdditivePipe() : Command("PartDesign_AdditivePipe")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignAdditivePipe::isActive() { return hasActiveDocument(); }
...
CmdPartDesignSubtractivePipe::CmdPartDesignSubtractivePipe() : Command("PartDesign_SubtractivePipe")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignSubtractivePipe::isActive() { return hasActiveDocument(); }
...
CmdPartDesignAdditiveLoft::CmdPartDesignAdditiveLoft() : Command("PartDesign_AdditiveLoft")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignAdditiveLoft::isActive() { return hasActiveDocument(); }
...
CmdPartDesignSubtractiveLoft::CmdPartDesignSubtractiveLoft() : Command("PartDesign_SubtractiveLoft")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignSubtractiveLoft::isActive() { return hasActiveDocument(); }
...
CmdPartDesignAdditiveHelix::CmdPartDesignAdditiveHelix() : Command("PartDesign_AdditiveHelix")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
    ...
    auto worker = [cmd, &pcActiveBody](Part::Feature *sketch, App::DocumentObject *Feat) {
        ...
        if (Feat->isError()) {
            ...
            if (base) {
                ...
                if (view) view->makeTemporaryVisible(true);
            }
        }
...
bool CmdPartDesignAdditiveHelix::isActive() { return hasActiveDocument(); }
...
bool CmdPartDesignSubtractiveHelix::isActive() { return hasActiveDocument(); }

//===========================================================================
// Common utility functions for Dressup features
//===========================================================================

bool dressupGetSelected(Gui::Command *cmd, const std::string &which, Gui::SelectionObject &selected,
                        bool &useAllEdges)
{
    // No PartDesign feature without Body past FreeCAD 0.16
    App::Document *doc = cmd->getDocument();
    if (!PartDesignGui::assureModernWorkflow(doc)) return false;

    PartDesign::Body *pcActiveBody = PartDesignGui::getBody(true);

    if (!pcActiveBody) return false;
    ...
    if (!selected.isObjectTypeOf(Part::Feature::getClassTypeId())) {
        QMessageBox::warning(
            Gui::getMainWindow(), QObject::tr("Wrong object type"),
    ...
    if (selection[0].getSubNames().empty()
        ...
        if (selection.size() == 1) { selected = selection[0]; }
    }
    return true;
}

void finishDressupFeature(const Gui::Command *cmd, const std::string &which, Part::Feature *base,
                          const std::vector<std::string> &SubNames, const bool useAllEdges)
{
    if (SubNames.empty()) {
        QMessageBox::warning(Gui::getMainWindow(), QObject::tr("Wrong selection"),
                             QString::fromStdString(which)
                                 + QObject::tr(" not possible on selected faces/edges."));
        return;
    }
    ...
    if (baseFeature) {
        ...
        if (view && Feat->isError()) view->Visibility.setValue(true);
    }
}
...
void makeChamferOrFillet(Gui::Command *cmd, const std::string &which)
{
    ...
    if (!dressupGetSelected(cmd, which, selected, useAllEdges)) return;
...
CmdPartDesignFillet::CmdPartDesignFillet() : Command("PartDesign_Fillet")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignFillet::isActive() { return hasActiveDocument(); }
...
CmdPartDesignChamfer::CmdPartDesignChamfer() : Command("PartDesign_Chamfer")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignChamfer::isActive() { return hasActiveDocument(); }
...
CmdPartDesignDraft::CmdPartDesignDraft() : Command("PartDesign_Draft")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignDraft::activated(int iMsg)
{
    ...
    Gui::SelectionObject selected;
    bool useAllEdges = false;
    if (!dressupGetSelected(this, "Draft", selected, useAllEdges)) return;
...
bool CmdPartDesignDraft::isActive() { return hasActiveDocument(); }
...
CmdPartDesignThickness::CmdPartDesignThickness() : Command("PartDesign_Thickness")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignThickness::activated(int iMsg)
{
    ...
    Gui::SelectionObject selected;
    bool useAllEdges = false;
    if (!dressupGetSelected(this, "Thickness", selected, useAllEdges)) return;
...
bool CmdPartDesignThickness::isActive() { return hasActiveDocument(); }
...
void prepareTransformed(
    PartDesign::Body *pcActiveBody, Gui::Command *cmd, const std::string &which,
    std::function<void(App::DocumentObject *, std::vector<App::DocumentObject *>)> func)
{
    ...
    auto worker = [=](std::vector<App::DocumentObject *> features) {
        ...
        for (std::vector<App::DocumentObject *>::iterator it = features.begin();
             it != features.end(); ++it) {
...
void finishTransformed(Gui::Command *cmd, App::DocumentObject *Feat) { finishFeature(cmd, Feat); }
...
CmdPartDesignMirrored::CmdPartDesignMirrored() : Command("PartDesign_Mirrored")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignMirrored::isActive() { return hasActiveDocument(); }
...
CmdPartDesignLinearPattern::CmdPartDesignLinearPattern() : Command("PartDesign_LinearPattern")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
void CmdPartDesignLinearPattern::activated(int iMsg)
{
    ...
    auto worker = [cmd](App::DocumentObject *Feat, std::vector<App::DocumentObject *> features) {
        ...
        if (features.front()->isDerivedFrom(PartDesign::ProfileBased::getClassTypeId())) {
            ...
            if (sketch) {
                FCMD_OBJ_CMD(
                    Feat, "Direction = (" << Gui::Command::getObjectCmd(sketch) << ", ['H_Axis'])");
                direction = true;
            }
        }
        if (!direction) {
...
bool CmdPartDesignLinearPattern::isActive() { return hasActiveDocument(); }
...
CmdPartDesignPolarPattern::CmdPartDesignPolarPattern() : Command("PartDesign_PolarPattern")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignPolarPattern::isActive() { return hasActiveDocument(); }
...
CmdPartDesignScaled::CmdPartDesignScaled() : Command("PartDesign_Scaled")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignScaled::isActive() { return hasActiveDocument(); }
...
CmdPartDesignMultiTransform::CmdPartDesignMultiTransform() : Command("PartDesign_MultiTransform")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
...
bool CmdPartDesignMultiTransform::isActive() { return hasActiveDocument(); }
...
CmdPartDesignBoolean::CmdPartDesignBoolean() : Command("PartDesign_Boolean")
{
    sAppModule = "PartDesign";
    sGroup = QT_TR_NOOP("PartDesign");
    ...
    if (BodyFilter.match() && !BodyFilter.Result.empty()) {
        ...
        for (; i != BodyFilter.Result.end(); i++) {
            for (std::vector<Gui::SelectionObject>::iterator j = i->begin(); j != i->end(); j++) {
                if (j->getObject() != pcActiveBody) bodies.push_back(j->getObject());
            }
        }
        if (!bodies.empty()) {
            updateDocument = true;

...
bool ReferenceSelection::allow(App::Document *pDoc, App::DocumentObject *pObj, const char *sSubName)
{
    ...
    if (support && pDoc != support->getDocument()) { return false; }
    ...
    if (!sSubName || sSubName[0] == '\0') return type.testFlag(AllowSelection::WHOLE);
...
PartDesign::Body *ReferenceSelection::getBody() const
{
    ...
    if (support) { body = PartDesign::Body::findBodyOf(support); }
    else {
...
bool ReferenceSelection::allowOrigin(PartDesign::Body *body, App::OriginGroupExtension *originGroup,
                                     App::DocumentObject *pObj) const
{
    bool fits = false;
    if (type.testFlag(AllowSelection::FACE)
        && pObj->getTypeId().isDerivedFrom(App::Plane::getClassTypeId())) {
        fits = true;
    }
    else if (type.testFlag(AllowSelection::EDGE)
             && pObj->getTypeId().isDerivedFrom(App::Line::getClassTypeId())) {
        fits = true;
    }
    ...
    if (fits) { // check that it actually belongs to the chosen body or part
        try {   // here are some throwers
            if (body) {
                if (body->getOrigin()->hasObject(pObj)) { return true; }
            }
            else if (originGroup) {
                if (originGroup->getOrigin()->hasObject(pObj)) { return true; }
            }
        }
...
bool ReferenceSelection::allowDatum(PartDesign::Body *body, App::DocumentObject *pObj) const
{
    ...
    if (type.testFlag(AllowSelection::FACE)
        && (pObj->getTypeId().isDerivedFrom(PartDesign::Plane::getClassTypeId())))
        return true;
    if (type.testFlag(AllowSelection::EDGE)
        && (pObj->getTypeId().isDerivedFrom(PartDesign::Line::getClassTypeId())))
        return true;
    if (type.testFlag(AllowSelection::POINT)
        && (pObj->getTypeId().isDerivedFrom(PartDesign::Point::getClassTypeId())))
        return true;
...
bool ReferenceSelection::isEdge(App::DocumentObject *pObj, const char *sSubName) const
{
    ...
    if (!edgeShape.IsNull()) {
        if (type.testFlag(AllowSelection::PLANAR)) {
            ...
            if (adapt.GetType() == GeomAbs_Line) return true;
        }
        else {
            return true;
        }
...
bool ReferenceSelection::isFace(App::DocumentObject *pObj, const char *sSubName) const
{
    ...
    if (!face.IsNull()) {
        if (type.testFlag(AllowSelection::PLANAR)) {
            ...
            if (adapt.GetType() == GeomAbs_Plane) return true;
        }
        else {
            return true;
        }
...
bool ReferenceSelection::isCircle(App::DocumentObject *pObj, const char *sSubName) const
{
    ...
    const TopoDS_Edge &edgeShape = TopoDS::Edge(sh);
    BRepAdaptor_Curve adapt(edgeShape);
    if (adapt.GetType() == GeomAbs_Circle) { return true; }
...
bool NoDependentsSelection::allow(App::Document * /*pDoc*/, App::DocumentObject *pObj,
                                  const char * /*sSubName*/)
{
    if (support && support->testIfLinkDAGCompatible(pObj)) { return true; }
    else {
...
bool CombineSelectionFilterGates::allow(App::Document *pDoc, App::DocumentObject *pObj,
                                        const char *sSubName)
{
                            ...
                            App::DocumentObject *&selObj, std::vector<std::string> &selSub)
{
    ...
    if (strcmp(thisObj->getDocument()->getName(), msg.pDocName) != 0) return false;

    selObj = thisObj->getDocument()->getObject(msg.pObjectName);
    if (selObj == thisObj) return false;
        ...
        if (!selBody || body != selBody) {
            ...
            if (!dlg.radioXRef->isChecked()) {
                ...
                auto copy = PartDesignGui::TaskFeaturePick::makeCopy(
                    selObj, subname, dlg.radioIndependent->isChecked());
                body->addObject(copy);
    ...
    if (PartDesign::Feature::isDatum(selObj)) { subname = ""; }
...
{
    if (!obj) return QString::fromLatin1("");

    if (PartDesign::Feature::isDatum(obj)) return QString::fromLatin1(obj->getNameInDocument());
    else if (!sub.empty())
                                  ...
                                  const std::vector<std::string> &subs)
{
    ...
    for (const auto &sub : subs) result += "\"" + sub + "\",";
    result += "]";
                                        ...
                                        const std::vector<std::string> &subs)
{
    ...
    if (PartDesign::Feature::isDatum(obj)) return Gui::Command::getObjectCmd(obj, "(", ", [''])");
    else

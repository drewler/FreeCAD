...
ViewProviderDatumCoordinateSystem::ViewProviderDatumCoordinateSystem()
{
    ...
    if (hGrp->GetBool("CoordinateSystemSelectOnTop", true)) OnTopWhenSelected.setValue(1);
...
ViewProviderDatumCoordinateSystem::~ViewProviderDatumCoordinateSystem()
{
    ...
    if (labelSwitch) labelSwitch->unref();
    autoZoom->unref();
}
...
void ViewProviderDatumCoordinateSystem::setupLabels()
{
    ...
    if (!ShowLabel.getValue()) {
        if (labelSwitch) labelSwitch->whichChild = -1;
        return;
    }
    else if (labelSwitch) {
...
void ViewProviderDatumCoordinateSystem::setExtents(Base::BoundBox3d bbox)
{
    ...
    if (Zoom.getValue()) { axisLength = 6 * Zoom.getValue(); }
    else {

      ...
      ui(new Ui_TaskLoftParameters)
{
    ...
    connect(ui->buttonRefAdd, SIGNAL(toggled(bool)), this, SLOT(onRefButtonAdd(bool)));
    connect(ui->buttonRefRemove, SIGNAL(toggled(bool)), this, SLOT(onRefButtonRemove(bool)));
    ...
    for (QWidget *child : childs) child->blockSignals(true);
    ...
    for (QWidget *child : childs) child->blockSignals(false);
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection) {
        if (referenceSelected(msg)) {
            ...
            if (object) {
                ...
                if (selectionMode == refProfile) { ui->profileBaseEdit->setText(label); }
...
bool TaskLoftParameters::referenceSelected(const Gui::SelectionChanges &msg) const
{
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection && selectionMode != none) {
        ...
        if (selectionMode == refProfile) {
            static_cast<ViewProviderLoft *>(vp)->highlightReferences(ViewProviderLoft::Profile,
                                                                     false);
            ...
            if (selectionMode == refAdd) {
                if (f == refs.end()) loft->Sections.addValue(obj, {msg.pSubName});
                else
...
void TaskLoftParameters::removeFromListWidget(QListWidget *widget, QString name)
{
    ...
    if (!items.empty()) {
        for (QList<QListWidgetItem *>::const_iterator it = items.cbegin(); it != items.cend();
             ++it) {
...
{
    if (notThis != refProfile) ui->buttonProfileBase->setChecked(false);

...
{
    if (Base.getValue() && Base.getValue()->isTouched()) return 1;
...
void DressUp::getAddSubShape(Part::TopoShape &addShape, Part::TopoShape &subShape)
{
                    ...
                    if (!base)
                        FC_THROWM(Base::CADKernelError,
                                  "Cannot find additive or subtractive support for "
                                      << getFullName());
                    if (!base->isDerivedFrom(DressUp::getClassTypeId())) break;
                }
            }
    ...
    if (res.isNull()) throw Part::NullShapeException("Null AddSub shape");

    if (res.getShape().ShapeType() != TopAbs_COMPOUND) { addShape = res; }
    else {
        int count = res.countSubShapes(TopAbs_SHAPE);
        if (!count) throw Part::NullShapeException("Null AddSub shape");
        if (count) {
            Part::TopoShape s = res.getSubShape(TopAbs_SHAPE, 1);
            if (!s.isNull() && s.hasSubShape(TopAbs_SOLID)) addShape = s;
        }
        if (count > 1) {
            Part::TopoShape s = res.getSubShape(TopAbs_SHAPE, 2);
            if (!s.isNull() && s.hasSubShape(TopAbs_SOLID)) subShape = s;
        }
    }
}

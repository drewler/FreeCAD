...
public:
    ...
    static void getFilteredReferences(const App::PropertyLinkSubList *prop,
                                      App::GeoFeature *&object,
                                      std::vector<std::string> &subobjects);
    static Part::TopoShape buildShapeFromReferences(App::GeoFeature *obj,
...
public:
    ...
    void setLinks(std::map<App::DocumentObject *, std::vector<std::string>> &&values,
                  bool reset = false);
    ...
    int canLoadPartial() const override { return PartialLoad.getValue() ? 1 : 0; }

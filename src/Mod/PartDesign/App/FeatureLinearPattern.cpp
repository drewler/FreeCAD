...
{
    ADD_PROPERTY_TYPE(Direction, (nullptr), "LinearPattern", (App::PropertyType)(App::Prop_None),
                      "Direction");
...
{
    if (Direction.isTouched() || Reversed.isTouched() || Length.isTouched()
...
const std::list<gp_Trsf> LinearPattern::getTransformations(const std::vector<App::DocumentObject *>)
{
    ...
    if (distance < Precision::Confusion()) throw Base::ValueError("Pattern length too small");
    int occurrences = Occurrences.getValue();
    if (occurrences < 1) throw Base::ValueError("At least one occurrence required");
    bool reversed = Reversed.getValue();

    App::DocumentObject *refObject = Direction.getValue();
    if (!refObject) throw Base::ValueError("No direction reference specified");

    std::vector<std::string> subStrings = Direction.getSubValues();
    if (subStrings.empty()) throw Base::ValueError("No direction reference specified");
    ...
    if (refObject->getTypeId().isDerivedFrom(Part::Part2DObject::getClassTypeId())) {
        ...
        if (subStrings[0] == "H_Axis") axis = refSketch->getAxis(Part::Part2DObject::H_Axis);
            ...
            if (refEdge.IsNull()) throw Base::ValueError("Failed to extract direction edge");
        ...
        if (subStrings[0].empty()) throw Base::ValueError("No direction reference specified");
        ...
        if (ref.ShapeType() == TopAbs_FACE) {
            ...
            if (refFace.IsNull()) throw Base::ValueError("Failed to extract direction plane");
...
void LinearPattern::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                              App::Property *prop)

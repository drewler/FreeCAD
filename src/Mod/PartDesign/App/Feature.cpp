...
Feature::Feature()
{
    ...
    ADD_PROPERTY_TYPE(_Body, (nullptr), "Base",
...
const gp_Pnt Feature::getPointFromFace(const TopoDS_Face &f)
{
    if (!f.Infinite()) {
        ...
        if (exp.More()) return BRep_Tool::Pnt(TopoDS::Vertex(exp.Current()));
...
Part::Feature *Feature::getBaseObject(bool silent) const
{
    ...
    if (BaseLink) {
        ...
        if (!BaseObject) { err = "No base feature linked"; }
    }
    else {
...
const TopoDS_Shape &Feature::getBaseShape() const
{
    ...
    if (result.IsNull()) throw Base::ValueError("Base feature's shape is invalid");
    TopExp_Explorer xp(result, TopAbs_SOLID);
    if (!xp.More()) throw Base::ValueError("Base feature's shape is not a solid");
...
Part::TopoShape Feature::getBaseTopoShape(bool silent) const
{
    ...
    if (!silent) {
        if (result.isNull()) throw Base::ValueError("Base feature's TopoShape is invalid");
...
gp_Pln Feature::makePlnFromPlane(const App::DocumentObject *obj)
{
    ...
    if (!plane) throw Base::ValueError("Feature: Null object");

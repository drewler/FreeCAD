...
FeaturePrimitive::FeaturePrimitive() : primitiveType(Box)
{
...
App::DocumentObjectExecReturn *FeaturePrimitive::execute(const TopoDS_Shape &primitiveShape)
{
    try {
        ...
        try {
            ...
            BRepBuilderAPI_Transform trsf(getBaseShape(), getLocation().Transformation().Inverted(),
                                          true);
            base = trsf.Shape();
        }
        ...
        if (getAddSubType() == FeatureAddSub::Additive) {
            ...
            if (solidCount > 1) {
                return new App::DocumentObjectExecReturn(
                    "Additive: Result has multiple solids. This is not supported at this time.");
            }
            ...
            if (solidCount > 1) {
                return new App::DocumentObjectExecReturn(
                    "Subtractive: Result has multiple solids. This is not supported at this time.");
            }
...
void FeaturePrimitive::onChanged(const App::Property *prop) { FeatureAddSub::onChanged(prop); }

void FeaturePrimitive::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                                 const char *PropName)
{
...
{
    if (Radius.isTouched() || Height.isTouched() || Angle.isTouched()) return 1;
...
App::DocumentObjectExecReturn *Sphere::execute()
{
    ...
    try {
        BRepPrimAPI_MakeSphere mkSphere(
            Radius.getValue(), Base::toRadians<double>(Angle1.getValue()),
            Base::toRadians<double>(Angle2.getValue()), Base::toRadians<double>(Angle3.getValue()));
...
Ellipsoid::Ellipsoid()
{
    ...
    ADD_PROPERTY_TYPE(Radius3, (0.0), "Ellipsoid", App::Prop_None,
...
App::DocumentObjectExecReturn *Ellipsoid::execute()
{
    ...
    try {
        ...
        gp_Ax2 ax2(pnt, dir);
        BRepPrimAPI_MakeSphere mkSphere(
            ax2, Radius2.getValue(), Base::toRadians<double>(Angle1.getValue()),
            Base::toRadians<double>(Angle2.getValue()), Base::toRadians<double>(Angle3.getValue()));
...
App::DocumentObjectExecReturn *Torus::execute()
{
    ...
    try {
        ...
        return FeaturePrimitive::execute(shape.makeTorus(Radius1.getValue(), Radius2.getValue(),

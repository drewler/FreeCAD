...
Mirrored::Mirrored()
{
    ADD_PROPERTY_TYPE(MirrorPlane, (nullptr), "Mirrored", (App::PropertyType)(App::Prop_None),
                      "Mirror plane");
}

short Mirrored::mustExecute() const
{
    if (MirrorPlane.isTouched()) return 1;
...
const std::list<gp_Trsf> Mirrored::getTransformations(const std::vector<App::DocumentObject *>)
{
    ...
    if (!refObject) throw Base::ValueError("No mirror plane reference specified");
    std::vector<std::string> subStrings = MirrorPlane.getSubValues();
    if (subStrings.empty()) throw Base::ValueError("No mirror plane reference specified");
    ...
    if (refObject->getTypeId().isDerivedFrom(Part::Part2DObject::getClassTypeId())) {
        ...
        Base::Axis axis;
        if (subStrings[0] == "H_Axis") axis = refSketch->getAxis(Part::Part2DObject::V_Axis);
            ...
            if (AxId >= 0 && AxId < refSketch->getAxisCount()) {
                ...
                axis.setDirection(Base::Vector3d(-axis.getDirection().y, axis.getDirection().x,
                                                 axis.getDirection().z));
            }
        }
        ...
        if (face.IsNull()) throw Base::ValueError("Failed to extract mirror plane");
        BRepAdaptor_Surface adapt(face);
        if (adapt.GetType() != GeomAbs_Plane) throw Base::TypeError("Mirror face must be planar");

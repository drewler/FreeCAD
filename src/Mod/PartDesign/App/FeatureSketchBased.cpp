...
ProfileBased::ProfileBased()
{
    ...
    ADD_PROPERTY_TYPE(Midplane, (0), "SketchBased", App::Prop_None,
                      "Extrude symmetric to sketch face");
    ADD_PROPERTY_TYPE(Reversed, (0), "SketchBased", App::Prop_None, "Reverse extrusion direction");
    ADD_PROPERTY_TYPE(UpToFace, (nullptr), "SketchBased", (App::PropertyType)(App::Prop_None),
                      "Face where feature will end");
    ADD_PROPERTY_TYPE(AllowMultiFace, (false), "SketchBased", App::Prop_None,
...
void ProfileBased::setupObject() { AllowMultiFace.setValue(true); }

void ProfileBased::positionByPrevious()
{
    Part::Feature *feat = getBaseObject(/* silent = */ true);
    if (feat) { this->Placement.setValue(feat->Placement.getValue()); }
    else {
...
{
    Part::Feature *feat = getBaseObject(/* silent = */ true);
    if (feat) { feat->transformPlacement(transform); }
    else {
        Part::Part2DObject *sketch = getVerifiedSketch();
...
Part::Part2DObject *ProfileBased::getVerifiedSketch(bool silent) const
{
    ...
    if (!result) { err = "No profile linked at all"; }
    else {
    ...
    if (!silent && err) { throw Base::RuntimeError(err); }
...
Part::Feature *ProfileBased::getVerifiedObject(bool silent) const
{
    ...
    if (!result) { err = "No object linked"; }
    else {
...
TopoDS_Shape ProfileBased::getVerifiedFace(bool silent) const
{
    ...
    if (!result) { err = "No profile linked"; }
    else if (AllowMultiFace.getValue()) {
        try {
            auto shape = getProfileShape();
            if (shape.isNull()) err = "Linked shape object is empty";
            else {
                auto faces = shape.getSubTopoShapes(TopAbs_FACE);
                if (faces.empty()) {
                    if (!shape.hasSubShape(TopAbs_WIRE)) shape = shape.makeWires();
        ...
        if (result->getTypeId().isDerivedFrom(Part::Part2DObject::getClassTypeId())) {
            ...
            if (Profile.getSubValues().empty()) err = "Linked object has no subshape specified";
            else {

                const Part::TopoShape &shape =
                    Profile.getValue<Part::Feature *>()->Shape.getShape();
                TopoDS_Shape sub = shape.getSubShape(Profile.getSubValues()[0].c_str());
                if (sub.ShapeType() == TopAbs_FACE) return TopoDS::Face(sub);
                else if (sub.ShapeType() == TopAbs_WIRE) {

                    auto wire = TopoDS::Wire(sub);
                    if (!wire.Closed()) err = "Linked wire is not closed";
                    else {
...
std::vector<TopoDS_Wire> ProfileBased::getProfileWires() const
{
    ...
    if (shape.IsNull()) throw Base::ValueError("Linked shape object is empty");
...
const TopoDS_Face ProfileBased::getSupportFace() const
{
    ...
    if (sketch) { return getSupportFace(sketch); }
...
TopoDS_Face ProfileBased::getSupportFace(const Part::Part2DObject *sketch) const
{
    if (sketch && sketch->MapMode.getValue() == Attacher::mmFlatFace
        ...
        if (part) {
            ...
            if (shape.getShape().IsNull()) throw Base::ValueError("Sketch support shape is empty!");

            TopoDS_Shape sh = shape.getSubShape(sub[0].c_str());
            if (sh.IsNull()) throw Base::ValueError("Null shape in SketchBased::getSupportFace()!");
...
TopoDS_Face ProfileBased::getSupportFace(const App::PropertyLinkSub &link) const
{
    ...
    if (!result) { throw Base::RuntimeError("No support linked"); }
...
int ProfileBased::getSketchAxisCount() const
{
    ...
    if (!sketch) return -1; // the link to the sketch is lost
...
Part::Feature *ProfileBased::getBaseObject(bool silent) const
{
    ...
    if (!obj->isDerivedFrom(Part::Part2DObject::getClassTypeId())) return obj;
...
void ProfileBased::getFaceFromLinkSub(TopoDS_Face &upToFace, const App::PropertyLinkSub &refFace)
{
    ...
    if (!ref) throw Base::ValueError("SketchBased: No face selected");
    ...
    if (upToFace.IsNull()) throw Base::ValueError("SketchBased: Failed to extract face");
}

void ProfileBased::getUpToFace(TopoDS_Face &upToFace, const TopoDS_Shape &support,
                               const TopoDS_Face &supportface, const TopoDS_Shape &sketchshape,
                               const std::string &method, const gp_Dir &dir)
{
    if ((method == "UpToLast") || (method == "UpToFirst")) {
        ...
        for (it = cfaces.begin(); it != cfaces.end(); it++)
            if (it->distsq > it_far->distsq) it_far = it;
...
bool ProfileBased::checkLineCrossesFace(const gp_Lin &line, const TopoDS_Face &face)
{
    ...
    if (distss.IsDone()) {
        if (distss.Value() > Precision::Confusion()) return false;
        ...
        for (Standard_Integer i = 1; i <= distss.NbSolution(); i++) {
            ...
            if (type == BRepExtrema_IsOnEdge) {
                ...
                if (fabs(dist_p_eps1_pnt) > 5 * Precision::Confusion()
                    ...
                    if (dist_p_eps1_pnt * dist_p_eps2_pnt < 0) return true;
                }
            }
    ...
    BRepBuilderAPI_MakeEdge mkEdge(line);
    if (!mkEdge.IsDone()) throw Base::RuntimeError("Revolve: Unexpected OCE failure");
...
void ProfileBased::remapSupportShape(const TopoDS_Shape &newShape)
{
    ...
    for (std::vector<App::DocumentObject *>::iterator it = refs.begin(); it != refs.end(); ++it) {
        ...
        for (std::vector<App::Property *>::iterator jt = props.begin(); jt != props.end(); ++jt) {
            if (!(*jt)->isDerivedFrom(App::PropertyLinkSub::getClassTypeId())) continue;
            App::PropertyLinkSub *link = static_cast<App::PropertyLinkSub *>(*jt);
            if (link->getValue() != this) continue;
            std::vector<std::string> subValues = link->getSubValues();
            std::vector<std::string> newSubValues;

            for (std::vector<std::string>::iterator it = subValues.begin(); it != subValues.end();
                 ++it) {
                std::string shapetype;
                if (it->compare(0, 4, "Face") == 0) { shapetype = "Face"; }
                ...
                if (!success) newSubValues.push_back(*it);
            }
...
struct gp_Pnt_Less {
    ...
    {
        if (fabs(p1.X() - p2.X()) > Precision::Confusion()) return p1.X() < p2.X();
        if (fabs(p1.Y() - p2.Y()) > Precision::Confusion()) return p1.Y() < p2.Y();
        if (fabs(p1.Z() - p2.Z()) > Precision::Confusion()) return p1.Z() < p2.Z();
...
bool ProfileBased::isQuasiEqual(const TopoDS_Shape &s1, const TopoDS_Shape &s2) const
{
    ...
    for (; it != p1.end(); ++it, ++jt) {
        if (!(*it).IsEqual(*jt, Precision::Confusion())) return false;
    }
    ...
    if (s1.ShapeType() == TopAbs_FACE && s2.ShapeType() == TopAbs_FACE) {
        ...
        if (a1.GetType() == GeomAbs_Plane && a2.GetType() == GeomAbs_Plane) {
            ...
            if (p1.Distance(p2.Location()) < Precision::Confusion()) {
                ...
                if (d1.IsParallel(d2, Precision::Confusion())) return true;
            }
        }
    }
...
void ProfileBased::getAxis(const App::DocumentObject *pcReferenceAxis,
                           const std::vector<std::string> &subReferenceAxis, Base::Vector3d &base,
                           Base::Vector3d &dir, ProfileBased::ForbiddenAxis checkAxis) const
{
    auto verifyAxisFunc = [](ProfileBased::ForbiddenAxis checkAxis, const gp_Pln &sketchplane,
                             const gp_Dir &dir) {
        switch (checkAxis) {
    ...
    auto getAxisFromEdge = [](const TopoDS_Edge &refEdge, Base::Vector3d &base,
                              Base::Vector3d &dir) {
        if (refEdge.IsNull()) throw Base::ValueError("Failed to extract rotation edge");
...
Base::Vector3d ProfileBased::getProfileNormal() const
{
    ...
    if (obj->isDerivedFrom(Part::Part2DObject::getClassTypeId())) {
        ...
        if (shape.IsNull()) return SketchVector;
...
void ProfileBased::Restore(Base::XMLReader &reader) { PartDesign::FeatureAddSub::Restore(reader); }

void ProfileBased::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                             const char *PropName)
{

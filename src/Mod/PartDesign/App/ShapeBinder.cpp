...
{
    ADD_PROPERTY_TYPE(Support, (nullptr, nullptr), "", (App::PropertyType)(App::Prop_None),
                      "Support of the geometry");
    Placement.setStatus(App::Property::Hidden, true);
    ADD_PROPERTY_TYPE(TraceSupport, (false), "", App::Prop_None, "Trace support shape");
}

ShapeBinder::~ShapeBinder() { this->connectDocumentChangedObject.disconnect(); }

void ShapeBinder::onChanged(const App::Property *prop) { Feature::onChanged(prop); }

short int ShapeBinder::mustExecute() const
{

    if (Support.isTouched()) return 1;
    if (TraceSupport.isTouched()) return 1;
...
void ShapeBinder::getFilteredReferences(const App::PropertyLinkSubList *prop, App::GeoFeature *&obj,
                                                      ...
                                                      std::vector<std::string> subs)
{
    ...
    if (obj->getTypeId().isDerivedFrom(Part::Feature::getClassTypeId())) {
        ...
        if (subs.empty()) return part->Shape.getValue();
        ...
        if (shapes.size() == 1) {
            ...
            for (const TopoDS_Shape &sh : shapes) { builder.Add(cmp, sh); }
            return cmp;
        }
    }
...
void ShapeBinder::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                            App::Property *prop)
{
...
void ShapeBinder::onSettingDocument()
{
    ...
    if (document) {
        this->connectDocumentChangedObject = document->signalChangedObject.connect(
            boost::bind(&ShapeBinder::slotChangedObject, this, bp::_1, bp::_2));
    }
}
...
void ShapeBinder::slotChangedObject(const App::DocumentObject &Obj, const App::Property &Prop)
{
    ...
    if (!doc || doc->testStatus(App::Document::Restoring)) return;
    if (this == &Obj) return;
    if (!TraceSupport.getValue()) return;
    if (!Prop.getTypeId().isDerivedFrom(App::PropertyPlacement::getClassTypeId())) return;
    ...
    if (obj) {
        if (obj == &Obj) {
            ...
            if (hasPlacementChanged()) enforceRecompute();
        }
...
{
    ADD_PROPERTY_TYPE(Support, (nullptr), "", (App::PropertyType)(App::Prop_None),
                      "Support of the geometry");
    ...
    ADD_PROPERTY_TYPE(MakeFace, (true), "Base", App::Prop_None,
                      "Create face using wires from bound shapes");
    ADD_PROPERTY_TYPE(Offset, (0.0), "Offsetting", App::Prop_None,
                      "2D offset face or wires, 0.0 = no offset");
    ADD_PROPERTY_TYPE(OffsetJoinType, ((long)0), "Offsetting", App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(OffsetFill, (false), "Offsetting", App::Prop_None,
                      "True = make face between original wire and offset.");
    ADD_PROPERTY_TYPE(OffsetOpenResult, (false), "Offsetting", App::Prop_None,
                      "False = make closed offset from open wire.");
    ADD_PROPERTY_TYPE(OffsetIntersection, (false), "Offsetting", App::Prop_None,
                      "False = offset child wires independently.");
    ADD_PROPERTY_TYPE(ClaimChildren, (false), "Base", App::Prop_Output,
                      "Claim linked object as children");
    ADD_PROPERTY_TYPE(Relative, (true), "Base", App::Prop_None,
                      "Enable relative sub-object binding");
    ADD_PROPERTY_TYPE(
        BindMode, ((long)0), "Base", App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(
        Context, (nullptr), "Base", App::Prop_Hidden,
    ...
    ADD_PROPERTY_TYPE(
        BindCopyOnChange, ((long)0), "Base", App::Prop_None,
                                                  ...
                                                  int depth) const
{
    ...
    if (sobj) return sobj;
    if (Data::ComplexGeoData::findElementName(subname) == subname) return nullptr;
    ...
    for (auto &l : Support.getSubListValues()) {
        ...
        for (auto &sub : l.getSubValues()) {
            ...
            if (!sobj || !sobj->getNameInDocument()) continue;
            if (subname[0] == '$') {
                if (sobj->Label.getStrValue() != name.c_str() + 1) continue;
            }
            else if (!boost::equals(sobj->getNameInDocument(), name))
                continue;
            name = Data::ComplexGeoData::noElementName(sub.c_str());
            name += dot + 1;
            if (mat && transform) *mat *= Placement.getValue().toMatrix();
...
void SubShapeBinder::setupCopyOnChange()
{
    ...
    hasCopyOnChange = App::LinkBaseExtension::setupCopyOnChange(
        this, linked, BindCopyOnChange.getValue() == 1 ? &copyOnChangeConns : nullptr,
        hasCopyOnChange);
    if (hasCopyOnChange) {
...
void SubShapeBinder::update(SubShapeBinder::UpdateOption options)
{
    ...
    if (!Relative.getValue()) parent = nullptr;
    else {
    ...
    for (auto &l : Support.getSubListValues()) {
        ...
        if (BindCopyOnChange.getValue() == 2 && Support.getSubListValues().size() == 1) {
            if (!_CopiedObjs.empty()) copied = _CopiedObjs.front().getObject();
            ...
            if (!copied || !copied->isValid()) {
                ...
                if (!objs.empty()) {
                    ...
                    if (!copied->recomputeFeature(true)) copyerror = 1;
                }
            }
            ...
            if (copied) {
                if (!copyerror) {
                    ...
                    getPropertyList(props);
                    for (auto prop : props) {
                        if (!App::LinkBaseExtension::isCopyOnChangeProperty(this, *prop)) continue;
                    ...
                    if (recomputeCopy && !copied->recomputeFeature(true)) copyerror = 2;
                }
                obj = copied;
        ...
        if (subs.empty()) subs.insert(none);
        else if (subs.size() > 1)
            subs.erase(none);
    ...
    if (!init) {
        ...
        if (!errMsg.empty()) {
            if (!(options & UpdateInit)) FC_THROWM(Base::RuntimeError, errMsg);
            if (!Shape.getValue().IsNull()) return;
        }
        ...
        if (!fused && result.hasSubShape(TopAbs_WIRE) && Offset.getValue() != 0.0) {
            try {
                result = result.makeOffset2D(Offset.getValue(), OffsetJoinType.getValue(),
                                             OffsetFill.getValue(), OffsetOpenResult.getValue(),
        ...
        if (Refine.getValue()) result = result.makeRefine();
    ...
    for (const auto &name : getDynamicPropertyNames()) {
        if (boost::starts_with(name, "Cache_")) caches.emplace(name);
    }
    ...
    for (auto &v : mats) {
        ...
        if (!prop || !prop->isDerivedFrom(App::PropertyMatrix::getClassTypeId())) {
            if (prop) removeDynamicProperty(name);
            prop =
...
App::DocumentObjectExecReturn *SubShapeBinder::execute()
{
    ...
    if (BindMode.getValue() == 0) update(UpdateForced);
...
{
    if (_Version.getValue() < 2) update(UpdateInit);
...
void SubShapeBinder::onChanged(const App::Property *prop)
{
    if (prop == &Context || prop == &Relative) {
        if (!Context.getValue() || !Relative.getValue()) { connRecomputedObj.disconnect(); }
        ...
        if (prop == &Support) {
            ...
            if (!Support.getSubListValues().empty()) {
                update();
                if (BindMode.getValue() == 2) Support.setValue(nullptr);
            }
        }
    ...
    for (auto &v : values) {
        ...
        for (auto &whole : wholeSubs) {
            for (auto it = v.second.begin(); it != v.second.end();) {
                auto &sub = *it;
                if (!boost::starts_with(sub, whole) || sub.size() == whole.size()) ++it;
                else {
...
void SubShapeBinder::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                               App::Property *prop)
{
    if (prop == &Support) { Support.upgrade(reader, TypeName); }
    else {

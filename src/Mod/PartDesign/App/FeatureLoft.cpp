...
App::DocumentObjectExecReturn *Loft::execute()
{
            ...
            if (i >= wiresections.size()) {
                if (initialWireSectionsEmpty) wiresections.emplace_back(1, ex.Current());
                    ...
                    throw Base::ValueError(
                        "Loft: Sections need to have the same amount of inner wires (except "
    ...
    try {
        ...
        if (numWires == 0) {
            ...
            if (i > 1)
                return new App::DocumentObjectExecReturn(
                    "Loft: When using points for profile/sections, the sketch should have a single "
                    "point");
        }
            ...
            if (numWiresAdded == 0) {
                ...
                for (ex.Init(shape, TopAbs_VERTEX); ex.More(); ex.Next(), ++j) {
                    if (isLastSectionVertex)
                        return new App::DocumentObjectExecReturn(
                            "Loft: Only the profile and last section can be vertices");
                    isLastSectionVertex = true;
                    for (auto &wires : wiresections) wires.push_back(ex.Current());
                }
                if (j > 1)
                    return new App::DocumentObjectExecReturn(
                        "Loft: When using points for profile/sections, the sketch should have a "
                        "single point");
            }
            if (!isLastSectionVertex && numWiresAdded < wiresections.size())
                return new App::DocumentObjectExecReturn(
                    "Loft: Sections need to have the same amount of inner wires as the base "
                    "section");
        }
        ...
        for (auto &wires : wiresections) {
            ...
            if (!profilePoint.IsNull()) mkTS.AddVertex(TopoDS::Vertex(copyProfilePoint));

            for (auto &shape : wires) {
                shape.Move(invObjLoc);
                if (shape.ShapeType() == TopAbs_VERTEX) mkTS.AddVertex(TopoDS::Vertex(shape));
                else
                    mkTS.AddWire(TopoDS::Wire(shape));
            }

            mkTS.Build();
            if (!mkTS.IsDone()) return new App::DocumentObjectExecReturn("Loft could not be built");
        ...
        if (!isLastSectionVertex) {
            ...
            for (auto &wires : wiresections) backwires.push_back(TopoDS::Wire(wires.back()));
        ...
        if (base.IsNull()) {
            if (getAddSubType() == FeatureAddSub::Subtractive)
                return new App::DocumentObjectExecReturn(
                    "Loft: There is nothing to subtract from\n");
        ...
        if (getAddSubType() == FeatureAddSub::Additive) {
            ...
            if (solidCount > 1) {
                return new App::DocumentObjectExecReturn(
                    "Loft: Result has multiple solids. This is not supported at this time.");
            }
        ...
        return new App::DocumentObjectExecReturn(
            "Loft: A fatal error occurred when making the loft");
    }
}


PROPERTY_SOURCE(PartDesign::AdditiveLoft, PartDesign::Loft)
AdditiveLoft::AdditiveLoft() { addSubType = Additive; }

PROPERTY_SOURCE(PartDesign::SubtractiveLoft, PartDesign::Loft)
SubtractiveLoft::SubtractiveLoft() { addSubType = Subtractive; }

void Loft::handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                     App::Property *prop)
{

...
class Epitrochoid:
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLength", "Distance", "Epitrochoid", "Distance"
        ).Distance = 10.0
        obj.addProperty(
            "App::PropertyInteger",
            "Segments",
    ...
    def execute(self, fp):  # main part of script
        ...
        for i in range(steps):
            if i == 0:
                x1 = f1 * math.cos(ang) - d * math.cos(
                    f2 * ang
                )  # coords for line startpoint
                y1 = f1 * math.sin(ang) - d * math.sin(f2 * ang)
                ang = dang
                x2 = f1 * math.cos(ang) - d * math.cos(
                    f2 * ang
                )  # coords for line endpoint
...
def makeEpitrochoid():
    ...
    epitrochoid = doc.addObject(
        "Part::FeaturePython", "Epitrochoid"

...
bool Measurement::has3DReferences() { return (References3D.getSize() > 0); }
...
MeasureType Measurement::getType()
{
    ...
    for (; obj != objects.end(); ++obj, ++subEl) {
        ...
        if (strcmp((*subEl).c_str(), "") == 0) { vols++; }
        else {

            TopoDS_Shape refSubShape;
            try {
                refSubShape = Part::Feature::getShape(*obj, (*subEl).c_str(), true);
                if (refSubShape.IsNull()) return Invalid;
            }
    ...
    if (vols > 0) {
        if (verts > 0 || edges > 0 || faces > 0) { mode = Invalid; }
        else {
            mode = Volumes;
        }
    }
        ...
        if (verts > 0 || edges > 0) {
            if (faces > 1 && verts > 1 && edges > 0) { mode = Invalid; }
            else {
        ...
        if (verts > 0) {
            if (verts > 1 && edges > 0) { mode = Invalid; }
            else {
                mode = PointToEdge;
            }
        }
...
TopoDS_Shape Measurement::getShape(App::DocumentObject *obj, const char *subName) const
{
    try {
        ...
        if (shape.IsNull()) throw Part::NullShapeException("null shape");
        return shape;
    }
    catch (Standard_Failure &e) {
        throw Base::CADKernelError(e.GetMessageString());
...
double Measurement::length() const
{
    ...
    if (numRefs == 0) {
        ...
        if (measureType == Points || measureType == PointToEdge || measureType == PointToSurface) {
            ...
            for (; obj != objects.end(); ++obj, ++subEl) {
                ...
                switch (curve.GetType()) {
                    ...
                    default: {
                        Base::Console().Error(
                            "Measurement::length - curve type: %d not implemented\n");
        ...
        if (measureType == Edges) {
            ...
            if (numRefs == 2) {
                ...
                if (curve1.GetType() == GeomAbs_Line && curve2.GetType() == GeomAbs_Line) {
                    ...
                    Base::Console().Error(
                        "Measurement::angle - Need 2 lines to make angle measure\n");
                    //                    throw Base::ValueError("Objects must both be lines");
                }
            }
            else {
                Base::Console().Error(
                    "Measurement::angle - Can not compute angle. Too many lines referenced\n");
...
unsigned int Measurement::getMemSize() const { return 0; }

...
void ViewProviderGeomFillSurface::setupContextMenu(QMenu *menu, QObject *receiver,
                                                   const char *member)
{
    QAction *act;
...
public:
    EdgeSelection(bool appendEdges, Surface::GeomFillSurface *editedObject)
        : Gui::SelectionFilterGate(nullPointer()), appendEdges(appendEdges),
...
bool GeomFillSurface::EdgeSelection::allow(App::Document *, App::DocumentObject *pObj,
                                           const char *sSubName)
{
...
void GeomFillSurface::setEditedObject(Surface::GeomFillSurface *obj)
{
    ...
    switch (curtype) {
        case GeomFill_StretchStyle: ui->fillType_stretch->setChecked(true); break;
        case GeomFill_CoonsStyle: ui->fillType_coons->setChecked(true); break;
        case GeomFill_CurvedStyle: ui->fillType_curved->setChecked(true); break;
        default: break;
    }
    ...
    for (; it != objects.end() && jt != element.end(); ++it, ++jt, ++index) {
        ...
        if (index < boolean.size()) {
            if (boolean[index]) item->setIcon(rotateLeft);
            else
...
void GeomFillSurface::clearSelection() { Gui::Selection().clearSelection(); }
...
void GeomFillSurface::slotUndoDocument(const Gui::Document &) { checkCommand = true; }

void GeomFillSurface::slotRedoDocument(const Gui::Document &) { checkCommand = true; }
...
void GeomFillSurface::slotDeletedObject(const Gui::ViewProviderDocumentObject &Obj)
{
    ...
    if (this->vp == &Obj) { this->vp->highlightReferences(false); }
}
...
bool GeomFillSurface::accept()
{
    ...
    if (count > 4) {
        QMessageBox::warning(this, tr("Too many edges"),
                             tr("The tool requires two, three or four edges"));
        return false;
    }
    else if (count < 2) {
        QMessageBox::warning(this, tr("Too less edges"),
                             tr("The tool requires two, three or four edges"));
        return false;
    }
...
void GeomFillSurface::on_fillType_stretch_clicked() { changeFillType(GeomFill_StretchStyle); }

void GeomFillSurface::on_fillType_coons_clicked() { changeFillType(GeomFill_CoonsStyle); }

void GeomFillSurface::on_fillType_curved_clicked() { changeFillType(GeomFill_CurvedStyle); }
    ...
    if (msg.Type == Gui::SelectionChanges::AddSelection) {
        ...
        if (selectionMode == Append) {
            ...
            if (new_booleans.size() < old_booleans.size()) {
                for (int i = 0; i < row; i++) new_booleans[i] = old_booleans[i];
            }
...
void GeomFillSurface::flipOrientation(QListWidgetItem *item)
{
    ...
    if (row < editedObject->ReversedList.getSize()) {
        ...
        if (reversed) { item->setIcon(rotateLeft); }
        else {
            item->setIcon(rotateRight);
        }
...
void TaskGeomFillSurface::open() { widget->open(); }

bool TaskGeomFillSurface::accept() { return widget->accept(); }

bool TaskGeomFillSurface::reject() { return widget->reject(); }

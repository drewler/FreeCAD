...
Filling::Filling()
{
    ...
    ADD_PROPERTY_TYPE(
        UnboundEdges, (nullptr, ""), "Filling", App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(FreeFaces, (nullptr, ""), "Filling", App::Prop_None,
                      "Free constraint on a face");
    ADD_PROPERTY_TYPE(FreeOrder, (0), "Filling", App::Prop_None,
                      "Order of constraint on free faces");

    ADD_PROPERTY_TYPE(Points, (nullptr, ""), "Filling", App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(MaximumSegments, (9), "Filling", App::Prop_None,
...
void Filling::addConstraints(BRepFill_Filling &builder, const App::PropertyLinkSubList &edges,
                             const App::PropertyStringList &faces,
                             const App::PropertyIntegerList &orders, Standard_Boolean bnd)
{
                ...
                if (!edge.IsNull() && edge.ShapeType() == TopAbs_EDGE) {
                    ...
                    if (subFace.empty()) {
                        if (!bnd) {
                            ...
                            if (testWire.IsDone()) { builder.Add(TopoDS::Edge(edge), cont, bnd); }
                            else {
...
void Filling::addConstraints(BRepFill_Filling &builder, const App::PropertyLinkSubList &faces,
...
App::DocumentObjectExecReturn *Filling::execute()
{
    ...
    try {
        BRepFill_Filling builder(degree, ptsoncurve, numIter, anisotropy, tol2d, tol3d, tolG1,
                                 tolG2, maxdeg, maxseg);
        ...
        if (FreeFaces.getSize() > 0) { addConstraints(builder, FreeFaces, FreeOrder); }

        // App point constraints
        if (Points.getSize() > 0) { addConstraints(builder, Points); }

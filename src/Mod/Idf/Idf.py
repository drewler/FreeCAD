...
EmpDisplayMode = (
    2  # 0='Flat Lines', 1='Shaded', 2='Wireframe', 3='Points'; recommended 2 or 0
)
...
def process_emn(doc, filename):
    ...
    for emnline in emnlines:
        ...
        if (
            ...
            and section_counter == 3
            and emnrecords[1] == "THOU"
        ):
            emn_unit = 0.0254
        ...
        if current_section == ".BOARD_OUTLINE" and section_counter == 2:
            ...
            FreeCAD.Console.PrintMessage(
                "Found board thickness " + emnrecords[0] + "\n"
            )
        if current_section == ".BOARD_OUTLINE" and section_counter > 2:
...
def Process_board_outline(doc, board_outline, drills, board_thickness):
    ...
    for point in board_outline:
        ...
        if vertex_index == 0:
            ...
            if point[3] != 0 and point[3] != 360:
                out_shape.append(
                    Part.Arc(
                        prev_vertex, mid_point(prev_vertex, vertex, point[3]), vertex
                    )
                )
                ...
                out_shape.append(
                    Part.Arc(
                        per_point, mid_point(per_point, vertex, point[3] / 2), vertex
                    )
                )
                out_shape.append(
                    Part.Arc(
    ...
    if lines != -1:
        ...
        for drill in drills:
            ...
            out_shape = Part.makeCircle(
                drill[0] / 2, Base.Vector(drill[1], drill[2], 0)
            )
            out_shape = Part.Wire(out_shape.Edges)
...
def process_emp(doc, filename, placement, board_thickness):
    ...
    for place_item in placement:
        if place_item[1] in comps:
            ...
            FreeCAD.Console.PrintMessage(
                "Adding EMP model " + str(place_item[0]) + "\n"
            )
    ...
    for point in comp_outline:
        ...
        if vertex_index > 0:
            if point[2] != 0 and point[2] != 360:
                out_shape.append(
                    Part.Arc(
                        prev_vertex, mid_point(prev_vertex, vertex, point[2]), vertex
                    )
                )
                FreeCAD.Console.PrintMessage("mid point " + str(mid_point) + "\n")
            elif point[2] == 360:
                ...
                out_shape.append(
                    Part.Arc(
                        per_point, mid_point(per_point, vertex, point[2] / 2), vertex
                    )
                )
                out_shape.append(
                    Part.Arc(
...
def place_steps(doc, placement, board_thickness):
    ...
    for place_item in placement:
        if place_item[2] in step_dict:
            ...
            FreeCAD.Console.PrintMessage(
                "Adding STEP model " + str(place_item[0]) + "\n"
            )
...
def Per_point(prev_vertex, vertex):
    ...
    perpoint = Base.Vector(
        2 * prev_vertex.x - vertex.x, 2 * prev_vertex.y - vertex.y, 0
    )
    return perpoint

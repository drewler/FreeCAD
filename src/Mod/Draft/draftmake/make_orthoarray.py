...
def _make_ortho_array(
    ...
    utils.print_header(
        _name, translate("draft", "Internal orthogonal array"), debug=False
    )
...
def _are_vectors(v_x, v_y, v_z=None, name="Unknown"):
    ...
    try:
        if v_z:
            ...
            utils.type_check(
                [(v_x, (int, float, App.Vector)), (v_y, (int, float, App.Vector))],
                name=name,
            )
    except TypeError:
...
def _are_integers(n_x, n_y, n_z=None, name="Unknown"):
    ...
    if n_x < 1:
        _wrn(
            translate(
                "draft", "Input: number of elements must be at least 1. It is set to 1."
            )
        )
        n_x = 1
    if n_y < 1:
...
def _are_numbers(d_x, d_y, d_z=None, name="Unknown"):
    ...
    try:
        if d_z:
            utils.type_check(
                [(d_x, (int, float)), (d_y, (int, float)), (d_z, (int, float))],
                name=name,
            )
        else:
...
def make_ortho_array2d(
    ...
    new_obj = _make_ortho_array(
        base_object, v_x=v_x, v_y=v_y, n_x=n_x, n_y=n_y, use_link=use_link
    )
    return new_obj


def make_rect_array(
    base_object, d_x=10, d_y=10, d_z=10, n_x=2, n_y=2, n_z=1, use_link=True
):

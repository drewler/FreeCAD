    ...
    def GetResources(self):
        ...
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "Draft_PathTwistedArray", "Path twisted array"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(
                "Draft_PathTwistedArray",
                "Creates copies of the selected object along a selected path, and twists the copies.\nFirst select the object, and then select the path.\nThe path can be a polyline, B-spline, Bezier curve, or even edges from other objects.",
            ),
        }
    ...
    def proceed(self):
        ...
        if len(sel) != 2:
            _err(
                translate(
                    "draft",
                    "Please select exactly two objects, the base object and the path object, before calling this command.",
                )
            )
        else:
...
class PathTwistedLinkArray(PathTwistedArray):
    ...
    def GetResources(self):
        ...
        return {
            ...
            "MenuText": QT_TRANSLATE_NOOP(
                "Draft_PathTwistedLinkArray", "Path twisted Link array"
            ),
            "ToolTip": QT_TRANSLATE_NOOP(

...
class Polygon(gui_base_original.Creator):
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Draft_Polygon",
                "Creates a regular polygon (triangle, square, pentagon, ...), by defining the number of sides and the circumscribed radius.\nCTRL to snap, SHIFT to constrain",
            ),
        }
    ...
    def Activated(self):
        ...
        if self.ui:
            ...
            self.tanpoints = []
            self.ui.pointUi(
                title=translate("draft", self.featureName), icon="Draft_Polygon"
            )
            self.ui.extUi()
        ...
        if arg["Type"] == "SoKeyboardEvent":
            ...
            if self.center and DraftVecUtils.dist(self.point, self.center) > 0:
                viewdelta = DraftVecUtils.project(
                    self.point.sub(self.center), App.DraftWorkingPlane.axis
                )

...
class Offset(gui_base_original.Modifier):
    ...
    def Activated(self):
        ...
        if self.ui:
            if not Gui.Selection.getSelection():
                ...
                _wrn(
                    translate("draft", "Offset only works " "on one object at a time.")
                )
            else:
                self.proceed()
        ...
        if not self.sel.isDerivedFrom("Part::Feature"):
            ...
            if utils.getType(self.sel) in ("Circle", "Arc"):
                ...
                if self.sel.FirstAngle <= self.sel.LastAngle:
                    ...
                    self.ghost.setStartAngle(
                        math.radians(self.sel.FirstAngle) - 2 * math.pi
                    )
                ...
                _wrn(
                    translate(
                        "draft", "Offset of Bezier curves " "is currently not supported"
                    )
                )
                self.finish()
                return
    ...
    def action(self, arg):
        ...
        if arg["Type"] == "SoKeyboardEvent":
            ...
            if (
                gui_tool_utils.hasMod(arg, gui_tool_utils.MODCONSTRAIN)
            ...
            if dist:
                ...
                if self.mode == "Wire":
                    ...
                    v2 = DraftGeomUtils.getTangent(
                        self.shape.Edges[dist[1]], self.point
                    )
                    ...
                    _wire = DraftGeomUtils.offsetWire(
                        self.shape, self.dvec, occ=occmode
                    )

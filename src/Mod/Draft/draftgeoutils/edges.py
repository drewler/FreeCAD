    ...
    for e in range(len(aList)):
        if str(anEdge.Curve) == str(aList[e].Curve):
            if DraftVecUtils.equals(
                anEdge.Vertexes[0].Point, aList[e].Vertexes[0].Point
            ):
                if DraftVecUtils.equals(
                    anEdge.Vertexes[-1].Point, aList[e].Vertexes[-1].Point
                ):
                    return e
    return None
...
def isSameLine(e1, e2):
    ...
    if DraftVecUtils.equals(
        e1.Vertexes[0].Point, e2.Vertexes[0].Point
    ) and DraftVecUtils.equals(e1.Vertexes[-1].Point, e2.Vertexes[-1].Point):
        return True
    elif DraftVecUtils.equals(
        e1.Vertexes[-1].Point, e2.Vertexes[0].Point
    ) and DraftVecUtils.equals(e1.Vertexes[0].Point, e2.Vertexes[-1].Point):
        return True
    return False
    ...
    if shape.ShapeType == "Wire":
        ...
        if geomType(shape) == "Line":
            return Part.LineSegment(
                shape.Vertexes[-1].Point, shape.Vertexes[0].Point
            ).toShape()
        elif geomType(shape) == "Circle":
            mp = findMidpoint(shape)
            return Part.Arc(
                shape.Vertexes[-1].Point, mp, shape.Vertexes[0].Point
            ).toShape()
...
def findMidpoint(edge):
    ...
    if edge.Length == 0:
        ...
        return edge.valueAt(
            edge.Curve.parameterAtDistance(edge.Length / 2, edge.FirstParameter)
        )

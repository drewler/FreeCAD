...
enum PropertyType
{
    ...
    Prop_Transient =
        2, /*!< Property content won't be saved to file, but still saves name, type and status */
...
struct AppExport PropertyData {
    struct PropertySpec {
        ...
        inline PropertySpec(const char *name, const char *group, const char *doc, short offset,
                            short type)
    ...
    struct OffsetBase {
        OffsetBase(const App::PropertyContainer *container)
            : m_container(container) {}                                         //explicit bombs
        ...
        short int getOffsetTo(const App::Property *prop) const
        {
            ...
            if (pt < base || pt > base + SHRT_MAX) return -1;
    ...
    mutable bmi::multi_index_container<
        ...
        bmi::indexed_by<
            bmi::sequenced<>,
            bmi::hashed_unique<bmi::member<PropertySpec, const char *, &PropertySpec::Name>,
                               CStringHasher, CStringHasher>,
            bmi::hashed_unique<bmi::member<PropertySpec, short, &PropertySpec::Offset>>>>
        propertyData;
    ...
    void addProperty(OffsetBase offsetBase, const char *PropName, Property *Prop,
                     const char *PropertyGroup = nullptr, PropertyType = Prop_None,
...
public:
   ...
   */
    ...
    virtual App::Property *addDynamicProperty(const char *type, const char *name = nullptr,
                                              const char *group = nullptr,
                                              const char *doc = nullptr, short attr = 0,
                                              bool ro = false, bool hidden = false);
    ...
    const char *getPropertyPrefix() const { return _propertyPrefix.c_str(); }

    void setPropertyPrefix(const char *prefix) { _propertyPrefix = prefix; }
...
protected:
    ...
    virtual void handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                           const char *PropName);
    virtual void handleChangedPropertyType(Base::XMLReader &reader, const char *TypeName,
                                           Property *prop);
...
#define _ADD_PROPERTY(_name, _prop_, _defaultval_)                                                 \
    do {                                                                                           \
        ...
        propertyData.addProperty(static_cast<App::PropertyContainer *>(this), _name,               \
                                 &this->_prop_);                                                   \
    } while (0)

#define ADD_PROPERTY(_prop_, _defaultval_) _ADD_PROPERTY(#_prop_, _prop_, _defaultval_)
...
#define _ADD_PROPERTY_TYPE(_name, _prop_, _defaultval_, _group_, _type_, _Docu_)                   \
    do {                                                                                           \
        ...
        this->_prop_.setContainer(this);                                                           \
        propertyData.addProperty(static_cast<App::PropertyContainer *>(this), _name,               \
                                 &this->_prop_, (_group_), (_type_), (_Docu_));                    \
    } while (0)
...
#define PROPERTY_SOURCE(_class_, _parentclass_)                                                    \
    ...
    const App::PropertyData *_class_::getPropertyDataPtr(void)                                     \
    {                                                                                              \
        return &propertyData;                                                                      \
    }                                                                                              \
    const App::PropertyData &_class_::getPropertyData(void) const                                  \
    {                                                                                              \
        return propertyData;                                                                       \
    }                                                                                              \
...
#define PROPERTY_SOURCE_TEMPLATE(_class_, _parentclass_)                                           \
    ...
    template<> const App::PropertyData *_class_::getPropertyDataPtr(void)                          \
    {                                                                                              \
        return &propertyData;                                                                      \
    }                                                                                              \
    template<> const App::PropertyData &_class_::getPropertyData(void) const                       \
    {                                                                                              \
        return propertyData;                                                                       \
    }                                                                                              \

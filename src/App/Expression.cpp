...
template<typename T> void copy_vector(T &dst, const T &src)
{
    dst.clear();
    dst.reserve(src.size());
    for (auto &s : src) {
        if (s) dst.push_back(s->copy());
        else
            dst.emplace_back();
    }
}
...
template<class T> static inline const T &cast(const App::any &value)
{
#ifdef USE_FAST_ANY
    return *value.cast<T>();
#else
...
template<class T> static inline T &cast(App::any &value)
{
#ifdef USE_FAST_ANY
    return *value.cast<T>();
#else
...
void ExpressionVisitor::updateLabelReference(Expression &e, DocumentObject *obj,
                                             const std::string &ref, const char *newLabel)
{
    e._updateLabelReference(obj, ref, newLabel);
}

bool ExpressionVisitor::updateElementReference(Expression &e, App::DocumentObject *feature,
                                               bool reverse)
{
    return e._updateElementReference(feature, reverse, *this);
}

bool ExpressionVisitor::relabeledDocument(Expression &e, const std::string &oldName,
...
bool ExpressionVisitor::renameObjectIdentifier(
    Expression &e, const std::map<ObjectIdentifier, ObjectIdentifier> &paths,
...
void ExpressionVisitor::moveCells(Expression &e, const CellAddress &address, int rowCount,
                                  int colCount)
{
...
template<class T> static inline bool essentiallyEqual(T a, T b)
{
...
template<class T> inline bool essentiallyZero(T a) { return !a; }

template<> inline bool essentiallyZero(double a) { return essentiallyEqual(a, 0.0); }

template<> inline bool essentiallyZero(float a) { return essentiallyEqual(a, 0.0f); }

template<class T> static inline bool definitelyGreaterThan(T a, T b)
{
    static const T _epsilon = std::numeric_limits<T>::epsilon();
    return (a - b) > ((std::fabs(a) < std::fabs(b) ? std::fabs(b) : std::fabs(a)) * _epsilon);
}

template<class T> static inline bool definitelyLessThan(T a, T b)
{
    static const T _epsilon = std::numeric_limits<T>::epsilon();
...
public:
    ...
    explicit PyObjectWrapper(PyObject *obj) : pyobj(obj) { Py::_XINCREF(pyobj); }
    ~PyObjectWrapper()
    {
        if (pyobj) {
...
namespace App
{
Py::Object pyObjectFromAny(const App::any &value) { return _pyObjectFromAny(value, nullptr); }

App::any pyObjectToAny(Py::Object value, bool check)
{

    if (value.isNone()) return App::any();

    PyObject *pyvalue = value.ptr();

    if (!check) return App::any(pyObjectWrap(pyvalue));
    ...
    if (PyFloat_Check(pyvalue)) return App::any(PyFloat_AsDouble(pyvalue));
    if (PyLong_Check(pyvalue)) return App::any(PyLong_AsLong(pyvalue));
    else if (PyUnicode_Check(pyvalue)) {
        const char *utf8value = PyUnicode_AsUTF8(pyvalue);
        if (!utf8value) { FC_THROWM(Base::ValueError, "Invalid unicode string"); }
...
static inline Quantity pyToQuantity(const Py::Object &pyobj, const Expression *e,
                                    const char *msg = nullptr)
{
    Quantity q;
    if (!pyToQuantity(q, pyobj)) {
        if (!msg) msg = "Failed to convert to Quantity.";
...
{
    if (!quantity.getUnit().isEmpty()) return Py::asObject(new QuantityPy(new Quantity(quantity)));
    double v = quantity.getValue();
    long l;
    int i;
    switch (essentiallyInteger(v, l, i)) {
        case 1:
        case 2: return Py::Long(l);
        default: return Py::Float(v);
    }
}

Quantity anyToQuantity(const App::any &value, const char *msg)
{
    if (is_type(value, typeid(Quantity))) { return cast<Quantity>(value); }
    else if (is_type(value, typeid(bool))) {
...
{
    if (is_type(value, typeid(int))) { res = cast<int>(value); }
    else if (is_type(value, typeid(long))) {
        res = cast<long>(value);
    }
    else if (is_type(value, typeid(bool)))
...
{
    if (is_type(value, typeid(double))) res = cast<double>(value);
    else if (is_type(value, typeid(float)))
...
{
    if (v1.empty()) return v2.empty();
    else if (v2.empty())
        return false;

    if (!is_type(v1, v2.type())) {
        if (is_type(v1, typeid(Quantity))) return cast<Quantity>(v1) == anyToQuantity(v2);
        ...
        double d1, d2;
        if (anyToLong(l1, v1)) {
            if (anyToLong(l2, v2)) return l1 == l2;
    ...
    if (is_type(v1, typeid(int))) return cast<int>(v1) == cast<int>(v2);
    if (is_type(v1, typeid(long))) return cast<long>(v1) == cast<long>(v2);
    if (is_type(v1, typeid(std::string))) return cast<std::string>(v1) == cast<std::string>(v2);
    if (is_type(v1, typeid(const char *))) {
        auto c1 = cast<const char *>(v1);
        auto c2 = cast<const char *>(v2);
        return c1 == c2 || (c1 && c2 && strcmp(c1, c2) == 0);
    }
    if (is_type(v1, typeid(bool))) return cast<bool>(v1) == cast<bool>(v2);
    if (is_type(v1, typeid(double))) return essentiallyEqual(cast<double>(v1), cast<double>(v2));
    if (is_type(v1, typeid(float))) return essentiallyEqual(cast<float>(v1), cast<float>(v2));

    if (is_type(v1, typeid(Quantity))) return cast<Quantity>(v1) == cast<Quantity>(v2);

    if (!isAnyPyObject(v1)) throw Base::TypeError("Unknown type");

    Base::PyGILStateLocker lock;
    Py::Object o1 = __pyObjectFromAny(v1);
    Py::Object o2 = __pyObjectFromAny(v2);
    if (!o1.isType(o2.type())) return false;
...
{
    if (value.isNone()) return new PyObjectExpression(owner);
    if (value.isString()) { return new StringExpression(owner, value.as_string()); }
        ...
        if (value.isTrue()) return new ConstantExpression(owner, "True", Quantity(1.0));
        else
            return new ConstantExpression(owner, "False", Quantity(0.0));
    }
    else {
        Quantity q;
        if (pyToQuantity(q, value)) return new NumberExpression(owner, q);
    }
...
Expression::Component::Component(const std::string &n)
    : comp(ObjectIdentifier::SimpleComponent(n)), e1(nullptr), e2(nullptr), e3(nullptr)
{}

Expression::Component::Component(Expression *_e1, Expression *_e2, Expression *_e3, bool isRange)
    : e1(_e1), e2(_e2), e3(_e3)
{
    if (isRange || e2 || e3) comp = ObjectIdentifier::RangeComponent(0);
}

Expression::Component::Component(const ObjectIdentifier::Component &comp)
    : comp(comp), e1(nullptr), e2(nullptr), e3(nullptr)
{}

Expression::Component::Component(const Component &other)
    : comp(other.comp), e1(other.e1 ? other.e1->copy() : nullptr),
      e2(other.e2 ? other.e2->copy() : nullptr), e3(other.e3 ? other.e3->copy() : nullptr)
{}
...
Expression::Component *Expression::Component::copy() const { return new Component(*this); }
...
{
    try {
        if (!e1 && !e2 && !e3) return comp.get(pyobj);
        if (!comp.isRange() && !e2 && !e3) {
            auto index = e1->getPyValue();
            Py::Object res;
            if (pyobj.isMapping()) res = Py::Mapping(pyobj).getItem(index);
            else {
                Py_ssize_t i = PyNumber_AsSsize_t(index.ptr(), PyExc_IndexError);
                if (PyErr_Occurred()) throw Py::Exception();
                res = Py::Sequence(pyobj).getItem(i);
            }
            if (!res.ptr()) throw Py::Exception();
            return res;
        }
        else {
            Py::Object v1, v2, v3;
            ...
            PyObject *s = PySlice_New(e1 ? v1.ptr() : nullptr, e2 ? v2.ptr() : nullptr,
                                      e3 ? v3.ptr() : nullptr);
            if (!s) throw Py::Exception();
            Py::Object slice(s, true);
            PyObject *res = PyObject_GetItem(pyobj.ptr(), slice.ptr());
            if (!res) throw Py::Exception();
...
void Expression::Component::set(const Expression *owner, Py::Object &pyobj,
                                const Py::Object &value) const
{
    if (!e1 && !e2 && !e3) return comp.set(pyobj, value);
    try {
        if (!comp.isRange() && !e2 && !e3) {
            auto index = e1->getPyValue();
            if (pyobj.isMapping()) Py::Mapping(pyobj).setItem(index, value);
            else {
            ...
            if (!s) throw Py::Exception();
            Py::Object slice(s, true);
            if (PyObject_SetItem(pyobj.ptr(), slice.ptr(), value.ptr()) < 0) throw Py::Exception();
        }
    }
    catch (Py::Exception &) {
        EXPR_PY_THROW(owner);
...
{
    try {
        if (!e1 && !e2 && !e3) { comp.del(pyobj); }
        else if (!comp.isRange() && !e2 && !e3) {
            auto index = e1->getPyValue();
            if (pyobj.isMapping()) { Py::Mapping(pyobj).delItem(index); }
            else {
                Py_ssize_t i = PyNumber_AsSsize_t(pyobj.ptr(), PyExc_IndexError);
            ...
            if (!s) throw Py::Exception();
            Py::Object slice(s, true);
            if (PyObject_DelItem(pyobj.ptr(), slice.ptr()) < 0) throw Py::Exception();
        }
    }
    catch (Py::Exception &) {
        EXPR_PY_THROW(owner);
...
void Expression::Component::toString(std::ostream &ss, bool persistent) const
{
    ...
    ss << '[';
    if (e1) e1->toString(ss, persistent);
    if (e2 || comp.isRange()) ss << ':';
    if (e2) e2->toString(ss, persistent);
    if (e3) {
        ss << ':';
...
Expression::Component *Expression::createComponent(Expression *e1, Expression *e2, Expression *e3,
                                                   bool isRange)
{
...
int Expression::priority() const { return 20; }
...
void Expression::getDeps(ExpressionDeps &deps, int option) const
{
    for (auto &v : getIdentifiers()) {
        ...
        if ((hidden && option == DepNormal) || (!hidden && option == DepHidden)) continue;
        for (auto &dep : var.getDep(true)) {
            DocumentObject *obj = dep.first;
            for (auto &propName : dep.second) { deps[obj][propName].push_back(var); }
        }
    }
}
    ...
    for (auto &v : getIdentifiers()) {
        ...
        for (auto &dep : var.getDep(false, &strings)) {
            ...
            if (!obj->testStatus(ObjectStatus::Remove)) {
                ...
                if (!hidden || res.second) res.first->second = hidden;
            }
...
public:
    ...
    void visit(Expression &e) override { this->getIdentifiers(e, deps); }
...
public:
    ...
    {
        if (this->adjustLinks(e, inList)) res = true;
    }
...
public:
    ...
    void visit(Expression &e) override { this->importSubNames(e, subNameMap); }
    ...
    for (auto &dep : getDeps(DepAll)) {
        for (auto &info : dep.second) {
            for (auto &path : info.second) {
                ...
                if (key.second.empty() || subNameMap.count(key)) continue;
                std::string imported = PropertyLinkBase::tryImportSubName(
                    obj, key.second.c_str(), owner->getDocument(), nameMap);
                if (!imported.empty()) subNameMap.emplace(std::move(key), std::move(imported));
            }
        }
    }
    if (subNameMap.empty()) return nullptr;
...
{
public:
    UpdateLabelExpressionVisitor(App::DocumentObject *obj, const std::string &ref,
                                 const char *newLabel)
        : obj(obj), ref(ref), newLabel(newLabel)
    {}

    void visit(Expression &e) override { this->updateLabelReference(e, obj, ref, newLabel); }
...
ExpressionPtr Expression::updateLabelReference(App::DocumentObject *obj, const std::string &ref,
                                               const char *newLabel) const
{
    if (ref.size() <= 2) return ExpressionPtr();
    std::vector<std::string> labels;
    for (auto &v : getIdentifiers()) v.first.getDepLabels(labels);
...
{
public:
    ReplaceObjectExpressionVisitor(const DocumentObject *parent, DocumentObject *oldObj,
    ...
    void visit(Expression &e) override
    {
        if (collect) this->collectReplacement(e, paths, parent, oldObj, newObj);
        else
...
ExpressionPtr Expression::replaceObject(const DocumentObject *parent, DocumentObject *oldObj,
                                        DocumentObject *newObj) const
{
    ...
    if (v.paths.empty()) return ExpressionPtr();
...
Py::Object Expression::getPyValue() const
{
    try {
        ...
        if (!components.empty()) {
            for (auto &c : components) pyobj = c->get(this, pyobj);
        }
        return pyobj;
    }
    catch (Py::Exception &) {
...
{
    _visit(v);
    for (auto &c : components) c->visit(v);
    v.visit(*this);
}
...
void Expression::toString(std::ostream &ss, bool persistent, bool checkPriority, int indent) const
{
    ...
    if (!_isIndexable()) {
        ...
        _toString(ss, persistent, indent);
    for (auto &c : components) c->toString(ss, persistent);
}
...
UnitExpression::UnitExpression(const DocumentObject *_owner, const Base::Quantity &_quantity,
    ...
    : Expression(_owner), quantity(_quantity), unitStr(_unitStr)
{}
...
Expression *UnitExpression::simplify() const { return new NumberExpression(owner, quantity); }
...
void UnitExpression::_toString(std::ostream &ss, bool, int) const { ss << unitStr; }

/**
  * Return a copy of the expression.
  */

Expression *UnitExpression::_copy() const { return new UnitExpression(owner, quantity, unitStr); }

Py::Object UnitExpression::_getPyValue() const
{
    if (!cache) cache = Py::new_reference_to(pyFromQuantity(quantity));
...
Expression *NumberExpression::simplify() const { return copy(); }

/**
  * Create and return a copy of the expression.
  */

Expression *NumberExpression::_copy() const { return new NumberExpression(owner, getQuantity()); }

/**
  * Negate the stored value.
  */

void NumberExpression::negate() { setQuantity(-getQuantity()); }
...
OperatorExpression::OperatorExpression(const App::DocumentObject *_owner, Expression *_left,
                                       Operator _op, Expression *_right)
    : UnitExpression(_owner), op(_op), left(_left), right(_right)
{}
...
bool OperatorExpression::isTouched() const { return left->isTouched() || right->isTouched(); }

static Py::Object calc(const Expression *expr, int op, const Expression *left,
                       const Expression *right, bool inplace)
{
...
#define BINARY_OP(_op, _pyop)                                                                      \
    case OperatorExpression::_op: {                                                                \
        ...
        case OperatorExpression::POW: {
            PyObject *res;
            if (inplace) res = PyNumber_InPlacePower(l.ptr(), r.ptr(), Py::None().ptr());
            else
        ...
        default: __EXPR_THROW(RuntimeError, "Unsupported operator", expr);
    }
}

Py::Object OperatorExpression::_getPyValue() const { return calc(this, op, left, right, false); }
...
void OperatorExpression::_toString(std::ostream &s, bool persistent, int) const
{
    ...
    if (left->priority() < priority()) // Check on operator priority first
        ...
        if (!isLeftAssociative()) needsParens = true;
    ...
    if (needsParens) s << "(" << left->toString(persistent) << ")";
    else
    ...
    if (right->priority() < priority()) // Check on operator priority first
        ...
        if (!isRightAssociative()) needsParens = true;
        else if (!isCommutative())
            needsParens = true;
    }
    else if (right->priority() == priority()) { // Same priority ?
        if (!isRightAssociative() || rightOperator == MOD) needsParens = true;
    }
...
bool OperatorExpression::isLeftAssociative() const { return true; }
...
struct HiddenReference {
    explicit HiddenReference(bool cond) : cond(cond)
    {
        if (cond) ++_HiddenReference;
    }
    ~HiddenReference()
    {
        if (cond) --_HiddenReference;
    ...
    static bool isHidden() { return _HiddenReference != 0; }

    bool cond;
};

FunctionExpression::FunctionExpression(const DocumentObject *_owner, Function _f,
                                       std::string &&name, std::vector<Expression *> _args)
    : UnitExpression(_owner), f(_f), fname(std::move(name)), args(_args)
{
    switch (f) {
        case ACOS:
        case ASIN:
        ...
        case HIDDENREF:
        case HREF:
            if (args.size() != 1) EXPR_THROW("Invalid number of arguments: exactly one required.");
            break;
        case MOD:
        case ATAN2:
        case POW:
            if (args.size() != 2) EXPR_THROW("Invalid number of arguments: exactly two required.");
            break;
        case HYPOT:
        case CATH:
        ...
        case CREATE:
        case MSCALE:
            if (args.empty()) EXPR_THROW("Invalid number of arguments: at least one required.");
            break;
        case LIST:
        case TUPLE: break;
        case NONE:
        case AGGREGATES:
        case LAST:
        default: PARSER_THROW("Unknown function"); break;
    }
}
...
public:
    ...
    {
        if (first) q.setUnit(value.getUnit());
    }
    virtual Quantity getQuantity() const { return q; }
...
public:
    ...
    {
        if (n < 2) throw ExpressionError("Invalid number of entries: at least two required.");
        else
...
Py::Object FunctionExpression::evalAggregate(const Expression *owner, int f,
                                             const std::vector<Expression *> &args)
{
    std::unique_ptr<Collector> c;

    switch (f) {
        case SUM: c.reset(new SumCollector); break;
        case AVERAGE: c.reset(new AverageCollector); break;
        case STDDEV: c.reset(new StdDevCollector); break;
        case COUNT: c.reset(new CountCollector); break;
        case MIN: c.reset(new MinCollector); break;
        case MAX: c.reset(new MaxCollector); break;
    ...
    for (auto &arg : args) {
        if (arg->isDerivedFrom(RangeExpression::getClassTypeId())) {
            ...
            Quantity q;
            if (pyToQuantity(q, arg->getPyValue())) c->collect(q);
        }
    }
...
Py::Object FunctionExpression::evaluate(const Expression *expr, int f,
                                        const std::vector<Expression *> &args)
{
    if (!expr || !expr->getOwner()) _EXPR_THROW("Invalid owner.", expr);

    // Handle aggregate functions
    if (f > AGGREGATES) return evalAggregate(expr, f, args);
    ...
    if (f == LIST) {
        ...
        int i = 0;
        for (auto &arg : args) list.setItem(i++, arg->getPyValue());
        return list;
    }
    else if (f == TUPLE) {
        if (args.size() == 1 && args[0]->isDerivedFrom(RangeExpression::getClassTypeId()))
        ...
        int i = 0;
        for (auto &arg : args) tuple.setItem(i++, arg->getPyValue());
        return tuple;
    }
    else if (f == MSCALE) {
        if (args.size() < 2) _EXPR_THROW("Function requires at least two arguments.", expr);
        ...
        if (!pyscale.isNone()) {
            Base::Vector3d vec;
            if (!PyArg_ParseTuple(pyscale.ptr(), "ddd", &vec.x, &vec.y, &vec.z)) PyErr_Clear();
            else {
        ...
        _EXPR_THROW("Function requires arguments to be either "
                    "(matrix,vector) or (matrix,number,number,number).",
                    expr);
    }

    if (args.empty()) _EXPR_THROW("Function requires at least one argument.", expr);
    ...
    if (f == MINVERT) {
        ...
        if (args.size() > 1) {
            ...
            for (unsigned i = 1; i < args.size(); ++i) tuple.setItem(i - 1, args[i]->getPyValue());
            Py::Dict dict;
    ...
    switch (f) {
        ...
        case ASIN:
        case ATAN:
            if (!v1.getUnit().isEmpty()) _EXPR_THROW("Unit must be empty.", expr);
            unit = Unit::Angle;
            scaler = 180.0 / M_PI;
            break;
        case EXP:
        ...
        case CEIL:
        case FLOOR:
        case ABS: unit = v1.getUnit(); break;
        case SQRT: {
            unit = v1.getUnit();
            ...
            unit = Unit(s.Length / 2, s.Mass / 2, s.Time / 2, s.ElectricCurrent / 2,
                        s.ThermodynamicTemperature / 2, s.AmountOfSubstance / 2,
                        s.LuminousIntensity / 2, s.Angle);
            break;
        }
        case ATAN2:
            if (e2.isNone()) _EXPR_THROW("Invalid second argument.", expr);

            if (v1.getUnit() != v2.getUnit()) _EXPR_THROW("Units must be equal.", expr);
            unit = Unit::Angle;
            scaler = 180.0 / M_PI;
            break;
        case MOD:
        ...
        case CATH:
            ...
            if (args.size() > 2) {
                if (e3.isNone()) _EXPR_THROW("Invalid second argument.", expr);
                if (v2.getUnit() != v3.getUnit()) _EXPR_THROW("Units must be equal.", expr);
            }
            unit = v1.getUnit();
            break;
        default: _EXPR_THROW("Unknown function: " << f, 0);
    }

    /* Compute result */
    switch (f) {
        case ACOS: output = acos(value); break;
        case ASIN: output = asin(value); break;
        case ATAN: output = atan(value); break;
        case ABS: output = fabs(value); break;
        case EXP: output = exp(value); break;
        case LOG: output = log(value); break;
        case LOG10: output = log(value) / log(10.0); break;
        case SIN: output = sin(value); break;
        case SINH: output = sinh(value); break;
        case TAN: output = tan(value); break;
        case TANH: output = tanh(value); break;
        case SQRT: output = sqrt(value); break;
        case COS: output = cos(value); break;
        case COSH: output = cosh(value); break;
        case MOD: {
        ...
        case ROUND: output = boost::math::round(value); break;
        case TRUNC: output = boost::math::trunc(value); break;
        case CEIL: output = ceil(value); break;
        case FLOOR: output = floor(value); break;
        default: _EXPR_THROW("Unknown function: " << f, 0);
    }
...
Py::Object FunctionExpression::_getPyValue() const { return evaluate(this, f, args); }
...
Expression *FunctionExpression::simplify() const
{
    ...
    for (auto it = args.begin(); it != args.end(); ++it) {
        ...
        if (freecad_dynamic_cast<NumberExpression>(v)) ++numerics;
        a.push_back(v);
    }
...
VariableExpression::VariableExpression(const DocumentObject *_owner, const ObjectIdentifier &_var)
    : UnitExpression(_owner), var(_var)
{}
...
bool VariableExpression::isTouched() const { return var.isTouched(); }
...
void VariableExpression::addComponent(Component *c)
{
    do {
        ...
        if (c->e3) {
            ...
            if (!n3 || !essentiallyEqual(n3->getValue(), (double)l3)) break;
        }
        if (c->e1) {
            ...
            if (!essentiallyInteger(n1->getValue(), l1)) break;
            if (!c->comp.isRange()) {
...
bool VariableExpression::_isIndexable() const { return true; }

Py::Object VariableExpression::_getPyValue() const { return var.getPyValue(true); }

void VariableExpression::_toString(std::ostream &ss, bool persistent, int) const
{
    if (persistent) ss << var.toPersistentString();
    else
        ss << var.toString();
}
...
Expression *VariableExpression::simplify() const { return copy(); }

/**
  * Return a copy of the expression.
  */

Expression *VariableExpression::_copy() const { return new VariableExpression(owner, var); }
...
bool VariableExpression::_relabeledDocument(const std::string &oldName, const std::string &newName,
                                            ExpressionVisitor &v)
{
...
void VariableExpression::_updateLabelReference(App::DocumentObject *obj, const std::string &ref,
                                               const char *newLabel)
{
    var.updateLabelReference(obj, ref, newLabel);
}

bool VariableExpression::_updateElementReference(App::DocumentObject *feature, bool reverse,
                                                 ExpressionVisitor &v)
{
    ...
    ExpressionVisitor &v)
{
    ...
    if (it != paths.end()) {
        v.aboutToChange();
        if (path.getOwner()) var = it->second.relativeTo(path);
        else
            var = it->second;
        return true;
    }
...
void VariableExpression::_moveCells(const CellAddress &address, int rowCount, int colCount,
                                    ExpressionVisitor &v)
{
    if (var.hasDocumentObjectName(true)) return;
...
void VariableExpression::_offsetCells(int rowOffset, int colOffset, ExpressionVisitor &v)
{
    ...
    const auto &comp = var.getPropertyComponent(0, &idx);
    CellAddress addr = stringToAddress(comp.getName().c_str(), true);
    if (!addr.isValid() || (addr.isAbsoluteCol() && addr.isAbsoluteRow())) return;

    if (!addr.isAbsoluteCol()) addr.setCol(addr.col() + colOffset);
    if (!addr.isAbsoluteRow()) addr.setRow(addr.row() + rowOffset);
    if (!addr.isValid()) {
...
void VariableExpression::setPath(const ObjectIdentifier &path) { var = path; }
...
Expression *StringExpression::simplify() const { return copy(); }

void StringExpression::_toString(std::ostream &ss, bool, int) const { ss << quote(text); }

/**
  * Return a copy of the expression.
  */

Expression *StringExpression::_copy() const { return new StringExpression(owner, text); }

Py::Object StringExpression::_getPyValue() const { return Py::String(text); }

TYPESYSTEM_SOURCE(App::ConditionalExpression, App::Expression)

ConditionalExpression::ConditionalExpression(const DocumentObject *_owner, Expression *_condition,
                                             Expression *_trueExpr, Expression *_falseExpr)
    : Expression(_owner), condition(_condition), trueExpr(_trueExpr), falseExpr(_falseExpr)
{}
...
{
    if (condition->getPyValue().isTrue()) return trueExpr->getPyValue();
    else
...
Expression *ConditionalExpression::simplify() const
{
    ...
    if (!v)
        return new ConditionalExpression(owner, condition->simplify(), trueExpr->simplify(),
                                         falseExpr->simplify());
    else {
        if (fabs(v->getValue()) > 0.5) return trueExpr->simplify();
        else
...
int ConditionalExpression::priority() const { return 2; }
...
ConstantExpression::ConstantExpression(const DocumentObject *_owner, const char *_name,
                                       const Quantity &_quantity)
    : NumberExpression(_owner, _quantity), name(_name)
{}
...
void ConstantExpression::_toString(std::ostream &ss, bool, int) const { ss << name; }

Py::Object ConstantExpression::_getPyValue() const
{
    if (!cache) {
        if (strcmp(name, "None") == 0) cache = Py::new_reference_to(Py::None());
...
RangeExpression::RangeExpression(const DocumentObject *_owner, const std::string &begin,
...
bool RangeExpression::isTouched() const
{
    ...
    do {
        ...
        if (prop && prop->isTouched()) return true;
    } while (i.next());
...
Py::Object RangeExpression::_getPyValue() const
{
    ...
    do {
        ...
        if (p) list.append(Py::asObject(p->getPyObject()));
    } while (range.next());
    return list;
}

void RangeExpression::_toString(std::ostream &ss, bool, int) const { ss << begin << ":" << end; }

Expression *RangeExpression::_copy() const { return new RangeExpression(owner, begin, end); }

Expression *RangeExpression::simplify() const { return copy(); }
...
Range RangeExpression::getRange() const
{
    ...
    if (c1.isValid() && c2.isValid()) return Range(c1, c2);

    Base::PyGILStateLocker lock;
    static const std::string attr("getCellFromAlias");
    Py::Object pyobj(owner->getPyObject(), true);
    if (!pyobj.hasAttr(attr)) EXPR_THROW("Invalid cell range " << begin << ':' << end);
...
void RangeExpression::_moveCells(const CellAddress &address, int rowCount, int colCount,
                                 ExpressionVisitor &v)
{
...
Base::XMLReader *ExpressionParser::ExpressionImporter::reader() { return _Reader; }
...
void ExpressionParser_yyerror(const char *errorinfo) { (void)errorinfo; }
...
double num_change(char *yytext, char dez_delim, char grp_delim)
{
    ...
    for (char *c = yytext; *c != '\0'; c++) {
        ...
        if (*c == dez_delim && dez_delim != '.') temp[i++] = '.';
        else
            temp[i++] = *c;
    ...
    if (ret_val == 0 && errno == ERANGE) throw Base::UnderflowError("Number underflow.");
    if (ret_val == HUGE_VAL || ret_val == -HUGE_VAL) throw Base::OverflowError("Number overflow.");
...
Expression *App::ExpressionParser::parse(const App::DocumentObject *owner, const char *buffer)
{
    ...
    if (result != 0) throw ParserError("Failed to parse expression.");

    if (!ScanResult) throw ParserError("Unknown error in expression");

    if (valueExpression) return ScanResult;
    else {
        delete ScanResult;
...
UnitExpression *ExpressionParser::parseUnit(const App::DocumentObject *owner, const char *buffer)
{
    ...
    if (!unitExpression) {
        ...
        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            ...
            if (denom && nom && essentiallyEqual(nom->getValue(), 1.0)) unitExpression = true;
        }
    }
    delete ScanResult;
...
bool ExpressionParser::isTokenAnIndentifier(const std::string &str)
{
    ...
    if (status == 0 && (token == IDENTIFIER || token == CELLADDRESS)) return true;
    else
        return false;
}
...
bool ExpressionParser::isTokenAUnit(const std::string &str)
{
    ...
    if (status == 0 && token == UNIT) return true;
    else
        return false;
}

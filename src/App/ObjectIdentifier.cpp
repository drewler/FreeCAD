...
ObjectIdentifier::ObjectIdentifier(const App::PropertyContainer *_owner,
    ...
    : owner(nullptr), documentNameSet(false), documentObjectNameSet(false), localProperty(false),
      _hash(0)
{
    if (_owner) {
        const DocumentObject *docObj = freecad_dynamic_cast<const DocumentObject>(_owner);
        if (!docObj) FC_THROWM(Base::RuntimeError, "Property must be owned by a document object.");
        owner = const_cast<DocumentObject *>(docObj);

        if (!property.empty()) { setDocumentObjectName(docObj); }
    }
    if (!property.empty()) {
        addComponent(SimpleComponent(property));
        if (index != INT_MAX) addComponent(ArrayComponent(index));
    }
}

ObjectIdentifier::ObjectIdentifier(const App::PropertyContainer *_owner, bool localProperty)
    : owner(nullptr), documentNameSet(false), documentObjectNameSet(false),
      localProperty(localProperty), _hash(0)
{
    if (_owner) {
        const DocumentObject *docObj = freecad_dynamic_cast<const DocumentObject>(_owner);
        if (!docObj) FC_THROWM(Base::RuntimeError, "Property must be owned by a document object.");
      ...
      _hash(0)
{
    ...
    if (!docObj) FC_THROWM(Base::TypeError, "Property must be owned by a document object.");
    if (!prop.hasName()) FC_THROWM(Base::RuntimeError, "Property must have a name.");
...
{
    if (components.size() <= 1 || documentObjectName.getString().empty()) return components;
    ResolveResults result(*this);
    if (result.propertyIndex == 0) return components;
...
int ObjectIdentifier::numComponents() const { return components.size(); }
...
{
    if (!_cache.empty() || !owner) return _cache;

    std::ostringstream s;
    ResolveResults result(*this);

    if (result.propertyIndex >= (int)components.size()) return _cache;
    ...
    if (!subObjectName.getString().empty()) s << subObjectName.toString() << '.';
...
std::string ObjectIdentifier::toPersistentString() const
{
    ...
    if (result.propertyIndex >= (int)components.size()) return std::string();
    ...
    if (localProperty
        ...
        if (documentObjectName.isRealString()) s << '@';
        s << '.';
    }
    else if (documentNameSet && !documentName.getString().empty()) {
        if (documentObjectNameSet && !documentObjectName.getString().empty())
    ...
    if (!subObjectName.getString().empty()) {
        ...
        s << String(PropertyLinkBase::exportSubName(exportName, result.resolvedDocumentObject,
                                                    subname),
                    true)
                 .toString()
          << '.';
...
{
    if (_hash && !_cache.empty()) return _hash;
                                     ...
                                     App::DocumentObject *oldObj, App::DocumentObject *newObj) const
{
    ...
    if (!result.resolvedDocumentObject) return false;

    auto r = PropertyLinkBase::tryReplaceLink(owner, result.resolvedDocumentObject, parent, oldObj,
                                              newObj, subObjectName.getString().c_str());
...
bool ObjectIdentifier::updateLabelReference(App::DocumentObject *obj, const std::string &ref,
                                            const char *newLabel)
{
    ...
    if (result.resolvedDocument != obj->getDocument()) return false;

    if (!documentObjectName.getString().empty()) {
        if (documentObjectName.isForceIdentifier()) return false;

        if (!documentObjectName.isRealString()
            && documentObjectName.getString() == obj->getNameInDocument())
            return false;

        if (documentObjectName.getString() != obj->Label.getValue()) return false;
...
bool ObjectIdentifier::relabeledDocument(ExpressionVisitor &v, const std::string &oldLabel,
...
void ObjectIdentifier::getSubPathStr(std::ostream &s, const ResolveResults &result,
                                     bool toPython) const
{
...
ObjectIdentifier::Component::Component(const String &_name,
                                       ObjectIdentifier::Component::typeEnum _type, int _begin,
                                       int _end, int _step)
    : name(_name), type(_type), begin(_begin), end(_end), step(_step)
{}

ObjectIdentifier::Component::Component(String &&_name, ObjectIdentifier::Component::typeEnum _type,
                                       int _begin, int _end, int _step)
    : name(std::move(_name)), type(_type), begin(_begin), end(_end), step(_step)
{}
...
{
    if (begin >= 0) {
        if (begin < (int)count) return begin;
    }
    else {
...
Py::Object ObjectIdentifier::Component::get(const Py::Object &pyobj) const
{
    ...
    if (isSimple()) {
        ...
        if (pyobj.isMapping()) res = Py::Mapping(pyobj).getItem(Py::Int(begin));
        else
        ...
        if (!r) Base::PyException::ThrowException();
        res = Py::asObject(r);
    }
    if (!res.ptr()) Base::PyException::ThrowException();
...
void ObjectIdentifier::Component::set(Py::Object &pyobj, const Py::Object &value) const
{
    if (isSimple()) {
        ...
        if (pyobj.isMapping()) Py::Mapping(pyobj).setItem(Py::Int(begin), value);
        else
...
{
    if (isSimple()) pyobj.delAttr(getName());
    else if (isArray()) {
        if (pyobj.isMapping()) Py::Mapping(pyobj).delItem(Py::Int(begin));
        else
        ...
        Py::Object slice(PySlice_New(Py::Int(begin).ptr(),
                                     ...
                                     step != 1 ? Py::Int(step).ptr() : nullptr),
                         true);
        if (PyObject_DelItem(pyobj.ptr(), slice.ptr()) < 0) Base::PyException::ThrowException();
    }
}
...
ObjectIdentifier::Component ObjectIdentifier::Component::RangeComponent(int _begin, int _end,
                                                                        int _step)
{
...
bool ObjectIdentifier::Component::operator==(const ObjectIdentifier::Component &other) const
{
    ...
    switch (type) {
        case SIMPLE:
        case MAP: return name == other.name;
        case ARRAY: return begin == other.begin;
        case RANGE: return begin == other.begin && end == other.end && step == other.step;
...
{
    switch (type) {
        case Component::SIMPLE: ss << name.getString(); break;
        case Component::MAP: ss << "[" << name.toString(toPython) << "]"; break;
        case Component::ARRAY: ss << "[" << begin << "]"; break;
        case Component::RANGE:
            ss << '[';
            if (begin != INT_MAX) ss << begin;
            ss << ':';
            if (end != INT_MAX) ss << end;
            if (step != 1) ss << ':' << step;
            ss << ']';
            break;
        default: assert(0);
                                                         ...
                                                         const String &name, std::bitset<32> &flags)
{
    ...
    if (!name.isRealString()) {
        ...
        if (name.isForceIdentifier()) return nullptr;
    }

    std::vector<DocumentObject *> docObjects = doc->getObjects();
    for (std::vector<DocumentObject *>::iterator j = docObjects.begin(); j != docObjects.end();
         ++j) {
...
void ObjectIdentifier::resolve(ResolveResults &results) const
{
    ...
    if (!results.resolvedDocument) {
        if (!documentName.getString().empty()) {
            if (docAmbiguous) results.flags.set(ResolveAmbiguous);
            return;
        }

        results.resolvedDocument = owner->getDocument();
        if (!results.resolvedDocument) return;
    }
    ...
    if (!documentObjectName.getString().empty()) {
        ...
        if (!results.resolvedDocumentObject) return;
        ...
        if (components.size() == 1 || (components.size() > 1 && !components[0].isSimple())) {
            ...
            /* No --  */
            if (!components[0].isSimple()) return;
            ...
            if (results.resolvedDocumentObject) {
                ...
                results.resolvedDocumentObjectName = String {
                    components[0].name.getString(), false, results.flags.test(ResolveByIdentifier)};
...
Document *ObjectIdentifier::getDocument(String name, bool *ambiguous) const
{
    ...
    /* Not found on id? */
    if (!docById) return docByLabel; // Either not found at all, or on label
    else {
        /* Not found on label? */
        ...
        if (docByLabel == docById) return docById;
        if (ambiguous) *ambiguous = true;
        return nullptr;
    }
}
...
{
    if (!documentObjectName.getString().empty()) {
        if (documentObjectName.isRealString()) labels.push_back(documentObjectName.getString());
    }
...
void ObjectIdentifier::getDep(Dependencies &deps, bool needProps,
                              std::vector<std::string> *labels) const
{
    ResolveResults result(*this);
    if (labels) getDepLabels(result, *labels);

    if (!result.resolvedDocumentObject) return;
...
std::vector<std::string> ObjectIdentifier::getStringList() const
{
    ...
    if (!result.resolvedProperty || result.resolvedDocumentObject != owner) {
        if (documentNameSet) l.push_back(documentName.toString());

        if (documentObjectNameSet) l.push_back(documentObjectName.toString());
    }
    if (!subObjectName.getString().empty()) { l.back() += subObjectName.toString(); }
...
ObjectIdentifier ObjectIdentifier::relativeTo(const ObjectIdentifier &other) const
{
    ...
    if (otherresult.resolvedDocumentObject != thisresult.resolvedDocumentObject)
        result.setDocumentObjectName(std::move(thisresult.resolvedDocumentObjectName), true,
                                            ...
                                            int &ptype) const
{
    ...
    if (it == _props.end()) ptype = PseudoNone;
    else {
        ptype = it->second;
...
ObjectIdentifier ObjectIdentifier::canonicalPath() const
{
    ...
    if (!result.resolvedProperty || result.propertyType != PseudoNone) return res;
...
ObjectIdentifier::DocumentMapper::~DocumentMapper() { _DocumentMap = nullptr; }
...
{
    if (name.getString().empty()) force = false;
                                             ...
                                             ObjectIdentifier::String &&subname, bool checkImport)
{
    ...
    if (obj == owner) force = false;
    else
        localProperty = false;
    if (obj->getDocument() == owner->getDocument()) setDocumentName(String());
...
{
    if (isRealString()) return quote(str, toPython);
    else
        return str;
}
        ...
        if (obj || objName) {
            ...
            if (restoreLabel) {
                if (!obj) {
                    ...
                    if (!obj) { FC_ERR("Cannot find object " << objName->toString()); }
                }

                if (obj) { PropertyLinkBase::restoreLabelReference(obj, str); }
            }
        }
        else if (str.back() != '@') {
            ...
            if (!objForMapped) { FC_ERR("Cannot find object " << str); }
            else {
                isString = true;
...
Py::Object ObjectIdentifier::access(const ResolveResults &result, Py::Object *value,
...
#define GET_MODULE(_name)                                                                          \
    do {                                                                                           \
        ...
        if (!pymod) {                                                                              \
            ...
            if (!pymod) Base::PyException::ThrowException();                                       \
            else                                                                                   \
                Py_DECREF(pymod);                                                                  \
        }                                                                                          \
    ...
    switch (ptype) {
        case PseudoApp: GET_MODULE(FreeCAD); break;
        case PseudoGui: GET_MODULE(FreeCADGui); break;
        case PseudoPart: GET_MODULE(Part); break;
        case PseudoCadquery: GET_MODULE(freecad.fc_cadquery); break;
        case PseudoRegex: GET_MODULE(re); break;
        case PseudoBuiltins: GET_MODULE(builtins); break;
        case PseudoMath: GET_MODULE(math); break;
        case PseudoCollections: GET_MODULE(collections); break;
        case PseudoShape: {
            GET_MODULE(Part);
            Py::Callable func(pyobj.getAttr("getShape"));
            Py::Tuple tuple(1);
            tuple.setItem(0, Py::Object(result.resolvedDocumentObject->getPyObject(), true));
            if (result.subObjectName.getString().empty()) pyobj = func.apply(tuple);
            else {
                Py::Dict dict;
        ...
        default: {
            ...
            if (result.resolvedSubObject) obj = result.resolvedSubObject;
            switch (ptype) {
                case PseudoPlacement: pyobj = Py::Placement(Base::Placement(mat)); break;
                case PseudoMatrix: pyobj = Py::Matrix(mat); break;
                case PseudoLinkPlacement:
                case PseudoLinkMatrix: {
                    auto linked = obj->getLinkedObject(true, &mat, false);
                    if (!linked || linked == obj) {
                        auto ext = obj->getExtensionByType<App::LinkBaseExtension>(true);
                        if (ext) ext->getTrueLinkedObject(true, &mat);
                    }
                    if (ptype == PseudoLinkPlacement) pyobj = Py::Placement(Base::Placement(mat));
                    else
                        pyobj = Py::Matrix(mat);
                    break;
                }
                case PseudoSelf: pyobj = Py::Object(obj->getPyObject(), true); break;
                default: {
    ...
    if (components.empty()) return pyobj;
    ...
    for (; idx < count; ++idx) {
        if (PyObject_TypeCheck(*pyobj, &DocumentObjectPy::Type))
            ...
            if (!prop && pyobj.hasAttr(attr)) attr = nullptr;
...
Py::Object ObjectIdentifier::getPyValue(bool pathValue, bool *isPseudoProperty) const
{
    ...
    if (rs.resolvedProperty && rs.propertyType == PseudoNone && pathValue) {
        Py::Object res;
        if (rs.resolvedProperty->getPyPathValue(*this, res)) return res;
    }
...
void ObjectIdentifier::setValue(const App::any &value) const
{
    std::stringstream ss;
    ResolveResults rs(*this);
    if (rs.propertyType) FC_THROWM(Base::RuntimeError, "Cannot set pseudo property");
...
{
    if (newStyle && !shadowSub.first.empty()) return shadowSub.first;
    if (!shadowSub.second.empty()) return shadowSub.second;
    return subObjectName.getString();
}

const std::string &ObjectIdentifier::getSubObjectName() const { return subObjectName.getString(); }
...
void ObjectIdentifier::importSubNames(const ObjectIdentifier::SubNameMap &subNameMap)
{
    ...
    if (it != subNameMap.end()) {
        ...
        if (documentObjectName.isRealString()) documentObjectName.str = obj->Label.getValue();
        else
            documentObjectName.str = obj->getNameInDocument();
        _cache.clear();
    }
    if (subObjectName.getString().empty()) return;
    it = subNameMap.find(std::make_pair(result.resolvedDocumentObject, subObjectName.str));
    if (it == subNameMap.end()) return;
...
bool ObjectIdentifier::updateElementReference(ExpressionVisitor &v, App::DocumentObject *feature,
                                              bool reverse)
{
    assert(v.getPropertyLink());
    if (subObjectName.getString().empty()) return false;

    ResolveResults result(*this);
    if (!result.resolvedSubObject) return false;
    if (v.getPropertyLink()->_updateElementReference(feature, result.resolvedDocumentObject,
                                                     subObjectName.str, shadowSub, reverse)) {
        _cache.clear();
        v.aboutToChange();
        return true;
    }
...
bool ObjectIdentifier::isTouched() const
{
    try {
        ResolveResults result(*this);
        if (result.resolvedProperty) {
            if (result.propertyType == PseudoNone) return result.resolvedProperty->isTouched();
            else
...
void ObjectIdentifier::resolveAmbiguity(ResolveResults &result)
{
    ...
    if (result.propertyIndex == 1) components.erase(components.begin());
    ...
    if (result.resolvedDocumentObject == owner) {
        ...
        setDocumentObjectName(
            String(result.resolvedDocumentObject->Label.getStrValue(), true, false), true,
            std::move(subname));
...
ObjectIdentifier::ResolveResults::ResolveResults(const ObjectIdentifier &oi)
    : propertyIndex(0), resolvedDocument(nullptr), resolvedDocumentName(),
      resolvedDocumentObject(nullptr), resolvedDocumentObjectName(), resolvedSubObject(nullptr),
      resolvedProperty(nullptr), propertyName(), propertyType(PseudoNone)
{
    oi.resolve(*this);
}
...
{
    resolvedProperty = oi.resolveProperty(resolvedDocumentObject, propertyName.c_str(),
                                          resolvedSubObject, propertyType);
}

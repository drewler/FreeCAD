...
PropertyInteger::PropertyInteger() { _lValue = 0; }
...
long PropertyInteger::getValue() const { return _lValue; }

PyObject *PropertyInteger::getPyObject() { return Py_BuildValue("l", _lValue); }
...
void PropertyInteger::setPathValue(const ObjectIdentifier &path, const boost::any &value)
{
    ...
    if (value.type() == typeid(long)) setValue(boost::any_cast<long>(value));
...
const boost::filesystem::path &PropertyPath::getValue() const { return _cValue; }
...
{
    std::string path;
    if (PyUnicode_Check(value)) { path = PyUnicode_AsUTF8(value); }
    else {
...
PropertyEnumeration::PropertyEnumeration(const App::Enumeration &e) { _enum = e; }
...
void PropertyEnumeration::setEnums(const char **plEnums)
{
    ...
    if (notify) aboutToSetValue();
    _enum.setEnums(plEnums);
    if (notify) hasSetValue();
}

void PropertyEnumeration::setEnums(const std::vector<std::string> &Enums) { setEnumVector(Enums); }
...
long PropertyEnumeration::getValue() const { return _enum.getInt(); }

bool PropertyEnumeration::isValue(const char *value) const { return _enum.isValue(value); }

bool PropertyEnumeration::isPartOf(const char *value) const { return _enum.contains(value); }

const char *PropertyEnumeration::getValueAsString() const
{
    if (!_enum.isValid()) throw Base::RuntimeError("Cannot get value from invalid enumeration");
    return _enum.getCStr();
}

const Enumeration &PropertyEnumeration::getEnum() const { return _enum; }
...
bool PropertyEnumeration::hasEnums() const { return _enum.hasEnums(); }

bool PropertyEnumeration::isValid() const { return _enum.isValid(); }

void PropertyEnumeration::Save(Base::Writer &writer) const
{
    writer.Stream() << writer.ind() << "<Integer value=\"" << _enum.getInt() << "\"";
    if (_enum.isCustom()) writer.Stream() << " CustomEnum=\"true\"";
...
void PropertyEnumeration::setPyObject(PyObject *value)
{
    if (PyLong_Check(value)) {
        ...
        try {
            ...
            for (int i = 0; i < seq.size(); ++i) values[i] = Py::Object(seq[i].ptr()).as_string();

            aboutToSetValue();
            _enum.setEnums(values);
            if (idx >= 0) _enum.setValue(idx, true);
            hasSetValue();
            return;
        }
        catch (Py::Exception &) {
...
Property *PropertyEnumeration::Copy() const { return new PropertyEnumeration(_enum); }
...
{
    if (value.type() == typeid(int)) setValue(boost::any_cast<int>(value));
...
PropertyIntegerConstraint::PropertyIntegerConstraint() : _ConstStruct(nullptr) {}


PropertyIntegerConstraint::~PropertyIntegerConstraint()
{
    if (_ConstStruct && _ConstStruct->isDeletable()) delete _ConstStruct;
}
...
{
    if (_ConstStruct) return _ConstStruct->LowerBound;
...
{
    if (_ConstStruct) return _ConstStruct->UpperBound;
...
{
    if (_ConstStruct) return _ConstStruct->StepSize;
    return 1;
}
...
void PropertyIntegerConstraint::setPyObject(PyObject *value)
{
    if (PyLong_Check(value)) {
        ...
        if (_ConstStruct) {
            if (temp > _ConstStruct->UpperBound) temp = _ConstStruct->UpperBound;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (PyLong_Check(item)) values[i] = PyLong_AsLong(item);
            else
        ...
        if (values[0] > c->UpperBound) values[0] = c->UpperBound;
...
PropertyPercent::PropertyPercent() { _ConstStruct = &percent; }
...
PyObject *PropertyIntegerList::getPyObject()
{
    ...
    for (int i = 0; i < getSize(); i++) PyList_SetItem(list, i, PyLong_FromLong(_lValueList[i]));
    return list;
}
...
PropertyFloat::PropertyFloat() { _dValue = 0.0; }
...
double PropertyFloat::getValue() const { return _dValue; }

PyObject *PropertyFloat::getPyObject() { return Py_BuildValue("d", _dValue); }
...
PropertyFloatConstraint::PropertyFloatConstraint() : _ConstStruct(nullptr) {}
...
void PropertyFloatConstraint::setPyObject(PyObject *value)
{
    if (PyFloat_Check(value)) {
        ...
        for (int i = 0; i < 4; i++) {
            PyObject *item;
            item = PyTuple_GetItem(value, i);
            if (PyFloat_Check(item)) values[i] = PyFloat_AsDouble(item);
        ...
        // need a value > 0
        if (stepSize < DBL_EPSILON) throw Base::ValueError("Step size must be greater than zero");
...
PropertyPrecision::PropertyPrecision() { setConstraints(&PrecisionStandard); }
...
{
    PyObject *list = PyList_New(getSize());
    for (int i = 0; i < getSize(); i++) PyList_SetItem(list, i, PyFloat_FromDouble(_lValueList[i]));
    return list;
}
...
void PropertyFloatList::SaveDocFile(Base::Writer &writer) const
{
    ...
    if (!isSinglePrecision()) {
        for (std::vector<double>::const_iterator it = _lValueList.begin(); it != _lValueList.end();
             ++it) {
            str << *it;
        }
    }
...
void PropertyString::setValue(const char *newLabel)
{
    ...
    if (obj && obj->getNameInDocument() && this == &obj->Label
        ...
        if (doc && !_hPGrp->GetBool("DuplicateLabels") && !obj->allowDuplicateLabel()) {
            ...
            for (it = objs.begin(); it != objs.end(); ++it) {
                if (*it == obj) continue; // don't compare object with itself
                std::string objLabel = (*it)->Label.getValue();
                if (!match && objLabel == newLabel) match = true;
            ...
            if (match && *newLabel) {
                ...
                if (label != obj->getNameInDocument()
                    ...
                    if (*c == 0
                        && std::find(objectLabels.begin(), objectLabels.end(),
                ...
                if (!changed) label = Base::Tools::getUniqueName(label, objectLabels, 3);
            }
        }
    ...
    for (auto &change : propChanges) change.first->Paste(*change.second.get());

    if (commit) GetApplication().closeActiveTransaction();
}

void PropertyString::setValue(const std::string &sString) { setValue(sString.c_str()); }

const char *PropertyString::getValue() const { return _cValue.c_str(); }
...
void PropertyString::Save(Base::Writer &writer) const
{
    ...
    bool exported = false;
    if (obj && obj->getNameInDocument() && obj->isExporting() && &obj->Label == this) {
        if (obj->allowDuplicateLabel()) writer.Stream() << "restore=\"1\" ";
    ...
    if (!exported) val = encodeAttribute(_cValue);
...
{
    verifyPath(path);
    if (value.type() == typeid(bool)) setValue(boost::any_cast<bool>(value) ? "True" : "False");
    else if (value.type() == typeid(int))
...
const std::string &PropertyUUID::getValueStr() const { return _uuid.getValue(); }

const Base::Uuid &PropertyUUID::getValue() const { return _uuid; }
...
unsigned int PropertyUUID::getMemSize() const { return static_cast<unsigned int>(sizeof(_uuid)); }
...
void PropertyStringList::setValues(const std::list<std::string> &lValue)
{
    ...
    for (const auto &v : lValue) vals.push_back(v);
    setValues(vals);
}
...
{
    std::string ret;
    if (PyUnicode_Check(item)) { ret = PyUnicode_AsUTF8(item); }
...
{
    size_t size = 0;
    for (int i = 0; i < getSize(); i++) size += _lValueList[i].size();
...
int PropertyMap::getSize() const { return static_cast<int>(_lValueList.size()); }
...
const std::string &PropertyMap::operator[](const std::string &key) const
{
    ...
    if (it != _lValueList.end()) return it->second;
    else
        return empty;
}
...
void PropertyMap::setPyObject(PyObject *value)
{
    if (PyDict_Check(value)) {
        ...
        for (Py_ssize_t i = 0; i < nSize; ++i) {
            ...
            if (PyUnicode_Check(key)) { keyStr = PyUnicode_AsUTF8(key); }
            else {
                std::string error =
            ...
            if (PyUnicode_Check(item)) { values[keyStr] = PyUnicode_AsUTF8(item); }
            else {
...
PropertyBool::PropertyBool() { _lValue = false; }
...
bool PropertyBool::getValue() const { return _lValue; }

PyObject *PropertyBool::getPyObject() { return PyBool_FromLong(_lValue ? 1 : 0); }

void PropertyBool::setPyObject(PyObject *value)
{
    if (PyBool_Check(value) || PyLong_Check(value)) { setValue(Base::asBoolean(value)); }
    else {
...
void PropertyBool::setPathValue(const ObjectIdentifier &path, const boost::any &value)
{
    ...
    if (value.type() == typeid(bool)) setValue(boost::any_cast<bool>(value));
    else if (value.type() == typeid(int))
...
PyObject *PropertyBoolList::getPyObject()
{
    ...
    for (int i = 0; i < getSize(); i++) {
        ...
        if (v) { PyTuple_SetItem(tuple, i, PyBool_FromLong(1)); }
        else {
...
const Color &PropertyColor::getValue() const { return _cCol; }
...
void PropertyColor::setPyObject(PyObject *value)
{
    ...
    if (PyTuple_Check(value) && PyTuple_Size(value) == 3) {
        ...
        if (PyFloat_Check(item)) cCol.r = (float)PyFloat_AsDouble(item);
        else
            throw Base::TypeError("Type in tuple must be float");
        item = PyTuple_GetItem(value, 1);
        if (PyFloat_Check(item)) cCol.g = (float)PyFloat_AsDouble(item);
        else
            throw Base::TypeError("Type in tuple must be float");
        item = PyTuple_GetItem(value, 2);
        if (PyFloat_Check(item)) cCol.b = (float)PyFloat_AsDouble(item);
        else
            throw Base::TypeError("Type in tuple must be float");
    }
    else if (PyTuple_Check(value) && PyTuple_Size(value) == 4) {
        ...
        if (PyFloat_Check(item)) cCol.a = (float)PyFloat_AsDouble(item);
        else
            throw Base::TypeError("Type in tuple must be float");
    }
    else if (PyLong_Check(value)) {
...
void PropertyColorList::SaveDocFile(Base::Writer &writer) const
{
    ...
    uint32_t uCt = (uint32_t)getSize();
    str << uCt;
    for (std::vector<App::Color>::const_iterator it = _lValueList.begin(); it != _lValueList.end();
         ++it) {
...
const Material &PropertyMaterial::getValue() const { return _cMat; }
...
PyObject *PropertyMaterial::getPyObject() { return new MaterialPy(new Material(_cMat)); }
...
void PropertyMaterial::Save(Base::Writer &writer) const
{
    writer.Stream() << writer.ind() << "<PropertyMaterial ambientColor=\""
                    ...
                    << "\" transparency=\"" << _cMat.transparency << "\"/>" << endl;
}
...
{
    if (testStatus(MaterialEdit)) return "Gui::PropertyEditor::PropertyMaterialItem";
    return "";
}
...
void PropertyMaterialList::SaveDocFile(Base::Writer &writer) const
{
    ...
    uint32_t uCt = (uint32_t)getSize();
    str << uCt;
    for (std::vector<App::Material>::const_iterator it = _lValueList.begin();
         it != _lValueList.end(); ++it) {
...
{
    if (testStatus(NoMaterialListEdit)) return "";
...
{
    if (_pObject) return _pObject->getPyObject();
...
void PropertyPersistentObject::Restore(Base::XMLReader &reader)
{
    ...
    if (_pObject) _pObject->Restore(reader);
...
unsigned int PropertyPersistentObject::getMemSize() const
{
    ...
    if (_pObject) size += _pObject->getMemSize();
    return size;
}

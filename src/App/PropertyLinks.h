...
public:
    ...
    virtual bool referenceChanged() const { return false; }
     ...
     */
    virtual void getLinks(std::vector<App::DocumentObject *> &objs, bool all = false,
                          std::vector<std::string> *subs = nullptr, bool newStyle = true) const = 0;
     ...
     */
    virtual Property *CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
    ...
    template<class T> void getLinkedObjects(T &inserter, bool all = false) const
    {
        std::vector<App::DocumentObject *> ret;
        getLinks(ret, all);
                           ...
                           bool newStyle = true, bool all = true) const
    {
        ...
        int i = 0;
        for (auto obj : ret) elements[obj].push_back(subs[i++]);
    }
    ...
    tryReplaceLink(const App::PropertyContainer *owner, App::DocumentObject *obj,
    ...
    tryReplaceLinkSubs(const App::PropertyContainer *owner, App::DocumentObject *obj,
                       const App::DocumentObject *parent, App::DocumentObject *oldObj,
                       App::DocumentObject *newObj, const std::vector<std::string> &subs);
     ...
     */
    bool _updateElementReference(App::DocumentObject *feature, App::DocumentObject *obj,
     ...
     * during import.
     */
    static const char *exportSubName(std::string &output, const App::DocumentObject *obj,
                                     const char *subname, bool first_obj = false);
     ...
     * parsing.
     */
    static void restoreLabelReference(const App::DocumentObject *obj, std::string &sub,
    ...
    static std::string updateLabelReference(const App::DocumentObject *linked, const char *subname,
                                            App::DocumentObject *obj, const std::string &ref,
                                            const char *newLabel);
    //@}
    ...
    inline bool testFlag(int flag) const { return _Flags.test((std::size_t)flag); }
...
protected:
    ...
    inline void setFlag(int flag, bool value = true) { _Flags.set((std::size_t)flag, value); }
...
public:
    ...
    /** Returns the link type checked
     */
    template<typename _type> inline _type getValue() const
    {
    ...
    unsigned int getMemSize() const override { return sizeof(App::DocumentObject *); }
    const char *getEditorName() const override { return "Gui::PropertyEditor::PropertyLinkItem"; }

    void getLinks(std::vector<App::DocumentObject *> &objs, bool all = false,
                  std::vector<std::string> *subs = nullptr, bool newStyle = true) const override;
    ...
    Property *CopyOnLinkReplace(const App::DocumentObject *parent, App::DocumentObject *oldObj,
...
public:
    ...
    const std::vector<ShadowSub> &getShadowSubs() const { return _ShadowSubList; }
    ...
    Property *CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
...
public:
    ...
    void addValue(App::DocumentObject *obj, const std::vector<std::string> &SubList = {},
                  bool reset = false);

    const std::vector<DocumentObject *> &getValues() const { return _lValueList; }
    ...
    const std::vector<std::string> &getSubValues() const { return _lSubList; }
...
public:
    ...
    void setValue(std::string &&filePath, std::string &&objectName,
                  std::vector<std::string> &&SubList, std::vector<ShadowSub> &&ShadowSubList = {});
    ...
    const std::vector<std::string> &getSubValues() const { return _SubList; }
    const std::vector<ShadowSub> &getShadowSubs() const { return _ShadowSubList; }
    std::vector<std::string> getSubValues(bool newStyle) const;
    std::vector<std::string> getSubValuesStartsWith(const char *, bool newStyle = false) const;
...
protected:
    ...
    void copyTo(PropertyXLink &other, App::DocumentObject *linked = nullptr,
...
public:
    ...
    void addValue(App::DocumentObject *obj, const std::vector<std::string> &SubList = {},
                  bool reset = false);
    void addValue(App::DocumentObject *obj, std::vector<std::string> &&SubList = {},
                  bool reset = false);
    ...
    const std::list<PropertyXLinkSub> &getSubListValues() const { return _Links; }

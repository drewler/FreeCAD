...
template<class T> inline const T &any_cast(const boost::any &value)
{
    return boost::any_cast<const T &>(value);
}

template<class T> inline T &any_cast(boost::any &value) { return boost::any_cast<T &>(value); }
...
public:
    ...
    public:
        ...
        void checkImport(const App::DocumentObject *owner, const App::DocumentObject *obj = nullptr,
    ...
    public:
        ...
        Component(const String &_name = String(), typeEnum _type = SIMPLE, int begin = INT_MAX,
                  int end = INT_MAX, int step = 1); //explicit bombs
        Component(String &&_name, typeEnum _type = SIMPLE, int begin = INT_MAX, int end = INT_MAX,
    ...
    static Component ArrayComponent(int _index) { return Component::ArrayComponent(_index); }
    ...
    static Component MapComponent(const String &_key) { return Component::MapComponent(_key); }

    static Component MapComponent(String &&_key) { return Component::MapComponent(_key); }
    ...
    template<typename C> void addComponents(const C &cs)
    {
    ...
    void setDocumentObjectName(String &&name, bool force = false, String &&subname = String(),
    ...
    bool relabeledDocument(ExpressionVisitor &v, const std::string &oldLabel,
     ...
     * dependencies will be returned.
     */
    void getDep(Dependencies &deps, bool needProps,
    ...
    bool updateElementReference(ExpressionVisitor &v, App::DocumentObject *feature = nullptr,
                                bool reverse = false);
...
protected:
    ...
    App::Property *resolveProperty(const App::DocumentObject *obj, const char *propertyName,
                                   App::DocumentObject *&sobj, int &ptype) const;

    void getSubPathStr(std::ostream &ss, const ResolveResults &result, bool toPython = false) const;

    Py::Object access(const ResolveResults &rs, Py::Object *value = nullptr,
                      Dependencies *deps = nullptr) const;

    void resolve(ResolveResults &results) const;
    void resolveAmbiguity(ResolveResults &results);

    static App::DocumentObject *getDocumentObject(const App::Document *doc, const String &name,
                                                  std::bitset<32> &flags);
...
inline std::size_t hash_value(const App::ObjectIdentifier &path) { return path.hash(); }
...
template<> struct hash<App::ObjectIdentifier> {
    using argument_type = App::ObjectIdentifier;
    using result_type = std::size_t;
    inline result_type operator()(argument_type const &s) const { return s.hash(); }
};
} // namespace std

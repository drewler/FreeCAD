...
#define LINK_THROW(_type, _msg)                                                                    \
    do {                                                                                           \
        if (FC_LOG_INSTANCE.isEnabled(FC_LOGLEVEL_LOG)) FC_ERR(_msg);                              \
        throw _type(_msg);                                                                         \
    } while (0)
...
#define LINK_PARAM_LINK_PLACEMENT(...)                                                             \
    (LinkPlacement, Base::Placement, App::PropertyPlacement, Base::Placement(), "Link placement",  \
     ##__VA_ARGS__)
...
#define LINK_PARAM_CLAIM_CHILD(...)                                                                \
    (LinkClaimChild, bool, App::PropertyBool, false, "Claim the linked object as a child",         \
     ##__VA_ARGS__)
...
#define LINK_PARAM_COPY_ON_CHANGE_SOURCE(...)                                                      \
    (LinkCopyOnChangeSource, App::DocumentObject *, App::PropertyLink, 0,                          \
     "The copy on change source object", ##__VA_ARGS__)
...
#define LINK_PARAM_COPY_ON_CHANGE_TOUCHED(...)                                                     \
    (LinkCopyOnChangeTouched, bool, App::PropertyBool, 0,                                          \
     "Indicating the copy on change source object has been changed", ##__VA_ARGS__)

#define LINK_PARAM_SCALE(...)                                                                      \
    (Scale, double, App::PropertyFloat, 1.0, "Scale factor", ##__VA_ARGS__)

#define LINK_PARAM_SCALE_VECTOR(...)                                                               \
    (ScaleVector, Base::Vector3d, App::PropertyVector, Base::Vector3d(1, 1, 1), "Scale factors",   \
     ##__VA_ARGS__)

#define LINK_PARAM_PLACEMENTS(...)                                                                 \
    (PlacementList, std::vector<Base::Placement>, App::PropertyPlacementList,                      \
     std::vector<Base::Placement>(), "The placement for each link element", ##__VA_ARGS__)

#define LINK_PARAM_SCALES(...)                                                                     \
    (ScaleList, std::vector<Base::Vector3d>, App::PropertyVectorList,                              \
     std::vector<Base::Vector3d>(), "The scale factors for each link element", ##__VA_ARGS__)
...
#define LINK_PARAM_ELEMENTS(...)                                                                   \
    (ElementList, std::vector<App::DocumentObject *>, App::PropertyLinkList,                       \
     std::vector<App::DocumentObject *>(), "The link element object list", ##__VA_ARGS__)
...
#define LINK_PARAM_COLORED_ELEMENTS(...)                                                           \
    (ColoredElements, App::DocumentObject *, App::PropertyLinkSubHidden, 0,                        \
     "Link colored elements", ##__VA_ARGS__)
...
#define LINK_PROP_GET(_1, _2, _param)                                                              \
    ...
    bool extensionGetSubObject(DocumentObject *&ret, const char *subname,
                               PyObject **pyObj = nullptr, Base::Matrix4D *mat = nullptr,
                               bool transform = false, int depth = 0) const override;

    bool extensionGetSubObjects(std::vector<std::string> &ret, int reason) const override;

    bool extensionGetLinkedObject(DocumentObject *&ret, bool recurse, Base::Matrix4D *mat,
    ...
    DocumentObject *getTrueLinkedObject(bool recurse, Base::Matrix4D *mat = nullptr, int depth = 0,
                                        bool noElement = false) const;

    using LinkPropMap = std::map<const Property *, std::pair<LinkBaseExtension *, int>>;

    bool hasPlacement() const { return getLinkPlacementProperty() || getPlacementProperty(); }

    void cacheChildLabel(int enable = -1) const;

    static bool
    setupCopyOnChange(App::DocumentObject *obj, App::DocumentObject *linked,
...
protected:
    void _handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                    const char *PropName);
    ...
    void detachElements();
    void checkGeoElementMap(const App::DocumentObject *obj, const App::DocumentObject *linked,
                            PyObject **pyObj, const char *postfix) const;
    void updateGroup();
...
#define _LINK_PROP_ADD(_add_property, _param)                                                      \
    _add_property(BOOST_PP_STRINGIZE(LINK_ENAME(_param)), LINK_ENAME(_param), (LINK_PDEF(_param)), \
                                     LINK_EGROUP(_param), LINK_EPTYPE(_param), LINK_PDOC(_param)); \
...
#define LINK_PARAMS_LINK                                                                           \
    ...
    const char *getViewProviderName() const override { return "Gui::ViewProviderLink"; }

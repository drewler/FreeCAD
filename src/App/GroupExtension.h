...
private:
    ...
    bool recursiveHasObject(const DocumentObject *obj, const GroupExtension *group,
...
template<typename ExtensionT> class GroupExtensionPythonT: public ExtensionT
{
...
public:
    ...
    bool allowObject(DocumentObject *obj) override
    {
        ...
        if (result.isNone()) return ExtensionT::allowObject(obj);

        if (result.isBoolean()) return result.isTrue();

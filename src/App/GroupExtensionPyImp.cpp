...
PyObject *GroupExtensionPy::newObject(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|s", &sType, &sName)) return nullptr;

    DocumentObject *object = getGroupExtensionPtr()->addObject(sType, sName);
    if (object) { return object->getPyObject(); }
    else {
...
{
    PyObject *object;
    if (!PyArg_ParseTuple(args, "O!", &(DocumentObjectPy::Type), &object)) return nullptr;
    ...
    Py::List list;
    for (App::DocumentObject *obj : vec) list.append(Py::asObject(obj->getPyObject()));
...
PyObject *GroupExtensionPy::addObjects(PyObject *args)
{
    ...
    if (PyTuple_Check(object) || PyList_Check(object)) {
        ...
        Py::List result;
        for (App::DocumentObject *obj : vec) result.append(Py::asObject(obj->getPyObject()));
...
{
    char *pcName;
    if (!PyArg_ParseTuple(args, "s", &pcName)) return nullptr;

    DocumentObject *obj = getGroupExtensionPtr()->getObject(pcName);
    if (obj) { return obj->getPyObject(); }
    else {
        Py_Return;
    }
}
...
PyObject *GroupExtensionPy::hasObject(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O!|O!", &(DocumentObjectPy::Type), &object, &PyBool_Type,
                          &recursivePy))
        return nullptr;
...
PyObject *GroupExtensionPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

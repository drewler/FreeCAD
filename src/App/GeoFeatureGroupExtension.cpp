...
DocumentObject *GeoFeatureGroupExtension::getGroupOfObject(const DocumentObject *obj)
{
    ...
    for (auto inObj : list) {
        ...
        if (group && group->hasObject(obj)) return inObj;
    }
...
GeoFeatureGroupExtension::addObjects(std::vector<App::DocumentObject *> objects)
{
    ...
    for (auto object : objects) {
        ...
        if (!allowObject(object)) continue;
...
GeoFeatureGroupExtension::removeObjects(std::vector<App::DocumentObject *> objects)
{
    ...
    if (!removed.empty()) Group.setValues(grp);
    ...
    if (p == &Group && !Group.testStatus(Property::User3)) {
        ...
        if ((!getExtendedObject()->isRestoring()
            ...
            for (auto obj : Group.getValues()) {
                ...
                auto list = obj->getInList();
                for (auto in : list) {
                    if (in == getExtendedObject()) continue;
...
GeoFeatureGroupExtension::getScopedObjectsFromLinks(const DocumentObject *obj, LinkScope scope)
{
    ...
    if (!obj) return std::vector<DocumentObject *>();
...
std::vector<DocumentObject *>
GeoFeatureGroupExtension::getScopedObjectsFromLink(App::Property *prop, LinkScope scope)
{

    if (!prop) return std::vector<DocumentObject *>();

    std::vector<App::DocumentObject *> result;
    auto link = Base::freecad_dynamic_cast<PropertyLinkBase>(prop);
    if (link && link->getScope() == scope) link->getLinks(result);
                                           ...
                                           std::vector<DocumentObject *> &vec)
{
    ...
    for (App::DocumentObject *parent : obj->getInList()) {
        ...
        if (parent->hasExtension(App::GroupExtension::getExtensionClassTypeId())) continue;
        ...
        if (std::find(res.begin(), res.end(), obj) != res.end()) vec.push_back(parent);
    }
...
std::vector<DocumentObject *>
GeoFeatureGroupExtension::getCSRelevantLinks(const DocumentObject *obj)
{
                                                        ...
                                                        std::vector<DocumentObject *> &vec)
{
    ...
    for (auto o : links) {
        if (!o || o == obj || std::find(vec.begin(), vec.end(), o) != vec.end()) continue;
...
bool GeoFeatureGroupExtension::isLinkValid(App::Property *prop)
{
    ...
    for (auto link : result) {
        if (getGroupOfObject(link) != group) return false;
    }
    ...
    if (obj->hasExtension(App::GeoFeatureGroupExtension::getExtensionClassTypeId())) {
        ...
        for (auto link : result) {
            if (!groupExt->hasObject(link, true)) return false;
        }
    }
                                                     ...
                                                     std::vector<DocumentObject *> &vec)
{
    ...
    for (auto link : result) {
        if (getGroupOfObject(link) != group) vec.push_back(link);
    }
    ...
    if (group) {
        ...
        for (auto link : result) {
            if (!groupExt->hasObject(link, true)) vec.push_back(link);
        }
    }
}

...
void PropertyLinkBase::setAllowExternal(bool allow) { setFlag(LinkAllowExternal, allow); }

void PropertyLinkBase::hasSetValue()
{
    auto owner = dynamic_cast<DocumentObject *>(getContainer());
    if (owner) owner->clearOutListCache();
...
void PropertyLinkBase::unregisterLabelReferences()
{
    for (auto &label : _LabelRefs) {
        ...
        if (it != _LabelMap.end()) {
            ...
            if (it->second.empty()) _LabelMap.erase(it);
        }
    }
    _LabelRefs.clear();
}
...
{
    if (reset) unregisterLabelReferences();
    for (auto &label : labels) {
        auto res = _LabelRefs.insert(std::move(label));
        if (res.second) _LabelMap[*res.first].insert(this);
    }
}
...
std::string PropertyLinkBase::updateLabelReference(const App::DocumentObject *parent,
                                                   const char *subname, App::DocumentObject *obj,
...
PropertyLinkBase::updateLabelReferences(App::DocumentObject *obj, const char *newLabel)
{
    ...
    if (!obj || !obj->getNameInDocument()) return ret;
    auto it = _LabelMap.find(obj->Label.getStrValue());
    if (it == _LabelMap.end()) return ret;
    std::string ref("$");
    ...
    for (auto prop : props) {
        if (!prop->getContainer()) continue;
        std::unique_ptr<Property> copy(prop->CopyOnLabelChange(obj, ref, newLabel));
        if (copy) ret.emplace_back(prop, std::move(copy));
    }
    return ret;
}
...
static std::string propertyName(const Property *prop)
{
    ...
    if (!prop->getContainer() || !prop->hasName()) {
        ...
        if (xlink) return propertyName(xlink->parent());
    }
...
void PropertyLinkBase::_registerElementReference(App::DocumentObject *obj, std::string &sub,
                                                 ShadowSub &shadow)
{
    (void)obj;
    (void)sub;
...
{
public:
    explicit StringGuard(char *c) : c(c)
    {
        v1 = c[0];
        v2 = c[1];
        c[0] = '.';
...
void PropertyLinkBase::restoreLabelReference(const DocumentObject *obj, std::string &subname,
                                             ShadowSub *shadow)
{
...
bool PropertyLinkBase::_updateElementReference(DocumentObject *feature, App::DocumentObject *obj,
                                               std::string &sub, ShadowSub &shadow, bool reverse,
                                               bool notify)
{
    (void)feature;
    (void)obj;
...
PropertyLinkBase::tryReplaceLink(const PropertyContainer *owner, DocumentObject *obj,
                                 const DocumentObject *parent, DocumentObject *oldObj,
                                 DocumentObject *newObj, const char *subname)
{
...
PropertyLinkBase::tryReplaceLinkSubs(const PropertyContainer *owner, DocumentObject *obj,
                                     const DocumentObject *parent, DocumentObject *oldObj,
...
PropertyLink::PropertyLink() : _pcLink(nullptr) {}


PropertyLink::~PropertyLink() { resetLink(); }
...
void PropertyLink::resetLink()
{
    ...
    if (_pcScope != LinkScope::Hidden && _pcLink && getContainer()
        ...
        if (!parent->testStatus(ObjectStatus::Destroy)) {
            if (_pcLink) _pcLink->_removeBackLink(parent);
        }
    }
#endif
    _pcLink = nullptr;
...
void PropertyLink::setValue(App::DocumentObject *lValue)
{
    ...
    if (_pcScope != LinkScope::Hidden && parent) {
        ...
        if (!parent->testStatus(ObjectStatus::Destroy)) {
            if (_pcLink) _pcLink->_removeBackLink(parent);
            if (lValue) lValue->_addBackLink(parent);
        }
    }
#endif
    _pcLink = lValue;
...
App::DocumentObject *PropertyLink::getValue() const { return _pcLink; }
...
{
    if (_pcLink) return _pcLink->getPyObject();
    else
        Py_Return;
}
...
void PropertyLink::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                            std::vector<std::string> *subs, bool newStyle) const
{
    (void)newStyle;
    (void)subs;
...
{
    if (_pcLink == obj || (clear && getContainer() == obj)) setValue(nullptr);
}
    ...
    if (_pcScope != LinkScope::Hidden && !_lValueList.empty() && getContainer()
        ...
        if (!parent->testStatus(ObjectStatus::Destroy)) {
            for (auto *obj : _lValueList) {
                if (obj) obj->_removeBackLink(parent);
            }
        }
    }
#endif
...
void PropertyLinkList::setSize(int newSize, const_reference def)
{
    ...
    setSize(newSize);
    for (auto i = oldSize; i < newSize; ++i) _lValueList[i] = def;
}
...
void PropertyLinkList::set1Value(int idx, DocumentObject *const &value)
{
    ...
    if (!value || !value->getNameInDocument()) throw Base::ValueError("invalid document object");
    ...
    if (getContainer() && getContainer()->isDerivedFrom(App::DocumentObject::getClassTypeId())) {
        ...
        if (!parent->testStatus(ObjectStatus::Destroy) && _pcScope != LinkScope::Hidden) {
            ...
            if (value) value->_addBackLink(static_cast<DocumentObject *>(getContainer()));
        }
    }
#endif
...
void PropertyLinkList::setValues(const std::vector<DocumentObject *> &lValue)
{
    ...
    if (parent) {
        ...
        if (!parent->testStatus(ObjectStatus::Destroy) && _pcScope != LinkScope::Hidden) {
            ...
            for (auto *obj : lValue) {
                if (obj) obj->_addBackLink(parent);
            }
        }
    }
#endif
...
void PropertyLinkList::Restore(Base::XMLReader &reader)
{
    ...
    if (!container) throw Base::RuntimeError("Property is not part of a container");
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (child) values.push_back(child);
...
DocumentObject *PropertyLinkList::find(const std::string &name, int *pindex) const
{
    if (_nameMap.empty() || _nameMap.size() > _lValueList.size()) {
        ...
        for (int i = 0; i < (int)_lValueList.size(); ++i) {
            auto obj = _lValueList[i];
            if (obj && obj->getNameInDocument()) _nameMap[obj->getNameInDocument()] = i;
        }
    }
    auto it = _nameMap.find(name);
    if (it == _nameMap.end()) return nullptr;
    if (pindex) *pindex = it->second;
    return _lValueList[it->second];
}

void PropertyLinkList::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                                std::vector<std::string> *subs, bool newStyle) const
{
    (void)subs;
    (void)newStyle;
    if (all || _pcScope != LinkScope::Hidden) {
        ...
        for (auto obj : _lValueList) {
            if (obj && obj->getNameInDocument()) objs.push_back(obj);
        }
    }
}
...
void PropertyLinkList::breakLink(App::DocumentObject *obj, bool clear)
{
    ...
    for (auto o : _lValueList) {
        if (o != obj) values.push_back(o);
    }
    if (values.size() != _lValueList.size()) setValues(values);
}
...
PropertyLinkSub::PropertyLinkSub() : _pcLinkSub(nullptr), _restoreLabel(false) {}
...
PropertyLinkSub::~PropertyLinkSub()
{
    ...
    if (_pcLinkSub && getContainer()
        ...
        // otherwise the backlink contains dangling pointers
        if (!parent->testStatus(ObjectStatus::Destroy) && _pcScope != LinkScope::Hidden) {
            if (_pcLinkSub) _pcLinkSub->_removeBackLink(parent);
        }
    }
#endif
}
...
void PropertyLinkSub::setValue(App::DocumentObject *lValue, std::vector<std::string> &&subs,
                               std::vector<ShadowSub> &&shadows)
{
    auto parent = Base::freecad_dynamic_cast<App::DocumentObject>(getContainer());
    if (lValue) {
    ...
    if (shadows.size() == _cSubList.size()) _ShadowSubList = std::move(shadows);
    else
...
App::DocumentObject *PropertyLinkSub::getValue() const { return _pcLinkSub; }

const std::vector<std::string> &PropertyLinkSub::getSubValues() const { return _cSubList; }
...
{
    if (!newStyle) {
        if (!shadow.second.empty()) return shadow.second;
    }
                                                                 ...
                                                                 bool newStyle) const
{
    ...
    for (std::vector<std::string>::const_iterator it = _cSubList.begin(); it != _cSubList.end();
         ++it)
        if (strncmp(starter, it->c_str(), strlen(starter)) == 0) temp.push_back(*it);
    return temp;
}
...
PyObject *PropertyLinkSub::getPyObject()
{
    ...
    if (_pcLinkSub) {
        ...
        for (unsigned int i = 0; i < _cSubList.size(); i++) list[i] = Py::String(_cSubList[i]);
        tup[1] = list;
...
void PropertyLinkSub::setPyObject(PyObject *value)
{
    if (PyObject_TypeCheck(value, &(DocumentObjectPy::Type))) {
        ...
        if (seq.size() == 0) setValue(nullptr);
        else if (seq.size() != 2)
            ...
            if (seq[1].isString()) {
                ...
                for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it, ++i) {
                    if (!(*it).isString()) throw Base::TypeError(errMsg);
...
static bool updateLinkReference(App::PropertyLinkBase *prop, App::DocumentObject *feature,
                                bool reverse, bool notify, App::DocumentObject *link,
                                std::vector<std::string> &subs, std::vector<int> &mapped,
                                std::vector<PropertyLinkBase::ShadowSub> &shadows)
{
    ...
    if (!link || !link->getNameInDocument()) return false;
    auto owner = dynamic_cast<DocumentObject *>(prop->getContainer());
    if (owner && owner->isRestoring()) return false;
    int i = 0;
    bool touched = false;
    for (auto &sub : subs) {
    ...
    for (int idx : mapped) {
        if (idx < (int)subs.size() && !shadows[idx].first.empty()) subs[idx] = shadows[idx].first;
    }
    mapped.clear();
    if (owner && feature) owner->onUpdateElementReference(prop);
    return true;
}
...
void PropertyLinkSub::afterRestore()
{
    ...
    if (!testFlag(LinkRestoreLabel) || !_pcLinkSub || !_pcLinkSub->getNameInDocument()) return;
...
{
    unregisterElementReference();
    if (!_pcLinkSub || !_pcLinkSub->getNameInDocument()) return;
    for (std::size_t i = 0; i < _cSubList.size(); ++i)
...
{
    if (!updateLinkReference(this, feature, reverse, notify, _pcLinkSub, _cSubList, _mapped,
                             _ShadowSubList))
        return;
    if (notify) hasSetValue();
}

bool PropertyLinkSub::referenceChanged() const { return !_mapped.empty(); }

std::string PropertyLinkBase::importSubName(Base::XMLReader &reader, const char *sub,
                                            bool &restoreLabel)
{
    if (!reader.doNameMapping()) return sub;
...
const char *PropertyLinkBase::exportSubName(std::string &output, const App::DocumentObject *obj,
                                            const char *sub, bool first_obj)
{
    std::ostringstream str;
    const char *res = sub;

    if (!sub || !sub[0]) return res;
    ...
    if (first_obj) {
        ...
        if (*hash == '#') doc = GetApplication().getDocument(std::string(sub, hash - sub).c_str());
        else {
            hash = nullptr;
            if (obj && obj->getNameInDocument()) doc = obj->getDocument();
        }
        if (!doc) {
        ...
        if (!obj || !obj->getNameInDocument()) return res;
        if (hash) {
            if (!obj->isExporting()) str << doc->getName() << '#';
            sub = hash + 1;
        }
    }
                                                 ...
                                                 const std::map<std::string, std::string> &nameMap)
{
    if (doc && obj && obj->getNameInDocument()) {
        ...
        if (it != nameMap.end()) {
            ...
            if (!obj) FC_THROWM(Base::RuntimeError, "Cannot find import object " << it->second);
        }
    }
...
std::string PropertyLinkBase::tryImportSubName(const App::DocumentObject *obj, const char *_subname,
                                               ...
                                               const std::map<std::string, std::string> &nameMap)
{
    if (!doc || !obj || !obj->getNameInDocument()) return std::string();
    ...
    for (char *dot = strchr(next, '.'); dot; next = dot + 1, dot = strchr(next, '.')) {
        ...
        dot[0] = 0;
        if (next[0] == '$') {
            if (strcmp(next + 1, sobj->Label.getValue()) != 0) continue;
        }
        ...
        if (!imported) FC_THROWM(RuntimeError, "Failed to find imported object " << it->second);
        ss.write(sub, next - sub);
        if (next[0] == '$') ss << '$' << imported->Label.getStrValue() << '.';
        else
            ss << it->second << '.';
        sub = dot + 1;
    }
    if (sub != subname.c_str()) return ss.str();
    return std::string();
}
...
void PropertyLinkSub::Save(Base::Writer &writer) const
{
    ...
    // returns 0
    if (_pcLinkSub && _pcLinkSub->getNameInDocument()) internal_name = _pcLinkSub->getExportName();
...
void PropertyLinkSub::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (reader.hasAttribute(ATTR_MAPPED)) mapped.push_back(i);
    }
...
std::vector<std::string> updateLinkSubs(const App::DocumentObject *obj,
                                        const std::vector<std::string> &subs, Func *f,
                                        Args &&...args)
{
...
PropertyLinkSub::CopyOnImportExternal(const std::map<std::string, std::string> &nameMap) const
{
    ...
    if (!owner || !owner->getDocument()) return nullptr;
    if (!_pcLinkSub || !_pcLinkSub->getNameInDocument()) return nullptr;
    ...
    if (subs.empty() && linked == _pcLinkSub) return nullptr;

    PropertyLinkSub *p = new PropertyLinkSub();
    p->_pcLinkSub = linked;
    if (subs.empty()) p->_cSubList = _cSubList;
    else
        p->_cSubList = std::move(subs);
    return p;
}

Property *PropertyLinkSub::CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
...
void PropertyLinkSub::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                               std::vector<std::string> *subs, bool newStyle) const
{
    if (all || _pcScope != LinkScope::Hidden) {
        if (_pcLinkSub && _pcLinkSub->getNameInDocument()) {
            objs.push_back(_pcLinkSub);
            if (subs) *subs = getSubValues(newStyle);
        }
    }
}
...
{
    if (obj == _pcLinkSub || (clear && getContainer() == obj)) setValue(nullptr);
}

static App::DocumentObject *
adjustLinkSubs(App::PropertyLinkBase *prop, const std::set<App::DocumentObject *> &inList,
               ...
               std::map<App::DocumentObject *, std::vector<std::string>> *links = nullptr)
{
    ...
    for (auto &sub : subs) {
        ...
        for (; pos != std::string::npos; pos = sub.find('.', pos + 1)) {
            ...
            if (!newLink) {
                if (inList.count(sobj)) continue;
                newLink = sobj;
                if (links) (*links)[sobj].push_back(sub.substr(pos + 1));
                else
...
{
    if (_pcScope == LinkScope::Hidden) return false;
    if (!_pcLinkSub || !_pcLinkSub->getNameInDocument() || !inList.count(_pcLinkSub)) return false;
    auto subs = _cSubList;
...
int PropertyLinkSubList::getSize() const { return static_cast<int>(_lValueList.size()); }
...
{
    auto parent = Base::freecad_dynamic_cast<App::DocumentObject>(getContainer());
    for (auto obj : lValue) { verifyObject(obj, parent); }
...
{
    setValues(std::vector<DocumentObject *>(lValue), std::vector<std::string>(lSubNames),
                                    ...
                                    std::vector<ShadowSub> &&ShadowSubList)
{
    ...
    if (ShadowSubList.size() == _lSubList.size()) _ShadowSubList = std::move(ShadowSubList);
    else
        updateElementReference(nullptr);
    checkLabelReferences(_lSubList);
    hasSetValue();
...
void PropertyLinkSubList::addValue(App::DocumentObject *obj, const std::vector<std::string> &subs,
                                   bool reset)
{
    auto parent = Base::freecad_dynamic_cast<App::DocumentObject>(getContainer());
        ...
        if (!parent->testStatus(ObjectStatus::Destroy) && _pcScope != LinkScope::Hidden) {
            ...
            if (reset) {
                for (auto *value : _lValueList) {
                    if (value && value == obj) value->_removeBackLink(parent);
                }
            }
...
const string PropertyLinkSubList::getPyReprString() const
{
    ...
    if (this->_lValueList.empty()) return std::string("None");
...
DocumentObject *PropertyLinkSubList::getValue() const
{
    ...
    for (std::size_t i = 0; i < this->_lValueList.size(); i++) {
        if (!ret) ret = this->_lValueList[i];
        if (ret != this->_lValueList[i]) return nullptr;
    }
    return ret;
}
...
std::vector<PropertyLinkSubList::SubSet> PropertyLinkSubList::getSubListValues(bool newStyle) const
{
    ...
    for (std::size_t i = 0; i < _lValueList.size(); i++) {
        ...
        std::string sub;
        if (newStyle && !_ShadowSubList[i].first.empty()) sub = _ShadowSubList[i].first;
...
PyObject *PropertyLinkSubList::getPyObject()
{
    ...
    for (std::size_t i = 0; i < count; i++) {
        ...
        for (std::size_t j = 0; j < sub.size(); j++) { items[j] = Py::String(sub[j]); }
...
void PropertyLinkSubList::setPyObject(PyObject *value)
{
    ...
    if (!PyTuple_Check(value) && !PyList_Check(value)) throw Base::TypeError(errMsg);
...
void PropertyLinkSubList::afterRestore()
{
    ...
    if (!testFlag(LinkRestoreLabel)) return;
    setFlag(LinkRestoreLabel, false);
...
void PropertyLinkSubList::updateElementReference(DocumentObject *feature, bool reverse, bool notify)
{
    ...
    if (owner && owner->isRestoring()) return;
    int i = 0;
    bool touched = false;
    ...
    for (int idx : _mapped) {
        if (idx < (int)_lSubList.size()) {
            if (!_ShadowSubList[idx].first.empty()) _lSubList[idx] = _ShadowSubList[idx].first;
            else
                mapped.push_back(idx);
        }
    }
    _mapped.swap(mapped);
    if (owner && feature) owner->onUpdateElementReference(this);
    if (notify) hasSetValue();
}

bool PropertyLinkSubList::referenceChanged() const { return !_mapped.empty(); }
...
void PropertyLinkSubList::Save(Base::Writer &writer) const
{
    ...
    int count = 0;
    for (auto obj : _lValueList) {
        if (obj && obj->getNameInDocument()) ++count;
    }
...
{
    auto owner = dynamic_cast<const DocumentObject *>(getContainer());
    if (!owner || !owner->getDocument() || _lValueList.size() != _lSubList.size()) return nullptr;
    std::vector<App::DocumentObject *> values;
    std::vector<std::string> subs;
...
Property *PropertyLinkSubList::CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
                                                 const char *newLabel) const
{
    auto owner = dynamic_cast<const DocumentObject *>(getContainer());
    if (!owner || !owner->getDocument()) return nullptr;
...
unsigned int PropertyLinkSubList::getMemSize() const
{
    ...
    for (int i = 0; i < getSize(); i++) size += _lSubList[i].size();
    return size;
}
...
void PropertyLinkSubList::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                                   std::vector<std::string> *subs, bool newStyle) const
{
    if (all || _pcScope != LinkScope::Hidden) {
        objs.reserve(objs.size() + _lValueList.size());
...
void PropertyLinkSubList::breakLink(App::DocumentObject *obj, bool clear)
{
    ...
    if (values.size() != _lValueList.size()) setValues(values, subs);
}
...
bool PropertyLinkSubList::adjustLink(const std::set<App::DocumentObject *> &inList)
{
    ...
    for (std::string &sub : subs) {
        ++idx;
        auto &link = links[idx];
        if (!link || !link->getNameInDocument() || !inList.count(link)) continue;
        touched = true;
        size_t pos = sub.find('.');
        for (; pos != std::string::npos; pos = sub.find('.', pos + 1)) {
            auto sobj = link->getSubObject(sub.substr(0, pos + 1).c_str());
    ...
    if (touched) setValues(links, subs);
    return touched;
}
...
public:
    ...
    static std::string getDocPath(const char *filename, App::Document *pDoc, bool relative,
                                  QString *fullPath = nullptr)
    {
        ...
        if ((absolute = QFileInfo(path).isAbsolute())) {
            if (fullPath) *fullPath = path;
            if (!relative) return std::string(path.toUtf8().constData());
        }

        const char *docPath = pDoc->getFileName();
        if (!docPath || *docPath == 0) throw Base::RuntimeError("Owner document not saved");
        ...
        if (relative) return std::string(docDir.relativeFilePath(path).toUtf8().constData());
        else
            return std::string(path.toUtf8().constData());
    }

    static DocInfoPtr get(const char *filename, App::Document *pDoc, PropertyXLink *l,
                          const char *objName)
    {
        QString path;
        ...
        if (info->pcDoc) {
            ...
            if (owner && owner->getDocument() == info->pcDoc) return info;
        }
    ...
    static QString getFullPath(const char *p)
    {
        ...
        if (path.startsWith(QLatin1String("https://"))) return path;
        else {
    ...
    const char *filePath() const { return myPath.c_str(); }

    DocInfo() : pcDoc(nullptr) {}
    ...
    void init(DocInfoMap::iterator pos, const char *objName, PropertyXLink *l)
    {
        ...
        connSaveDocument =
            app.signalSaveDocument.connect(boost::bind(&DocInfo::slotSaveDocument, this, bp::_1));

        QString fullpath(getFullPath());
        if (fullpath.isEmpty()) FC_ERR("document not found " << filePath());
        else {
            for (App::Document *doc : App::GetApplication().getDocuments()) {
                if (getFullPath(doc->getFileName()) == fullpath) {
    ...
    void slotDeleteDocument(const App::Document &doc)
    {
        ...
        for (auto link : links) {
            ...
            if (link->parentProp) parentLinks[link->parentProp].push_back(link);
            else
    ...
    bool hasXLink(const App::Document *doc) const
    {
        for (auto link : links) {
            auto obj = dynamic_cast<DocumentObject *>(link->getContainer());
            if (obj && obj->getDocument() == doc) return true;
        }
        return false;
    }
    ...
    static void breakLinks(App::DocumentObject *obj, bool clear)
    {
        ...
        for (auto itD = _DocInfoMap.begin(), itDNext = itD; itD != _DocInfoMap.end();
            ...
            if (docInfo->pcDoc != doc) continue;
            auto &links = docInfo->links;
            std::set<PropertyLinkBase *> parentLinks;
            for (auto it = links.begin(), itNext = it; it != links.end(); it = itNext) {
                ++itNext;
                auto link = *it;
                if (link->_pcLink != obj && !(clear && link->getContainer() == obj)) continue;
                if (link->parentProp) parentLinks.insert(link->parentProp);
                else
                    link->breakLink(obj, clear);
            }
            for (auto link : parentLinks) link->breakLink(obj, clear);
        }
    }
};
                                  ...
                                  const std::vector<App::DocumentObject *> &objs, bool clear)
{
    ...
    for (auto obj : objs) {
        ...
        for (auto prop : props) {
            ...
            if (linkProp) linkProp->breakLink(link, clear);
        }
    }
...
PropertyXLink::PropertyXLink(bool _allowPartial, PropertyLinkBase *parent) : parentProp(parent)
{
    setAllowPartial(_allowPartial);
    setAllowExternal(true);
    setSyncSubObject(true);
    if (parent) setContainer(parent->getContainer());
}

PropertyXLink::~PropertyXLink() { unlink(); }
...
{
    if (parentProp) parentProp->aboutToSetChildValue(*this);
    else
...
{
    if (parentProp) parentProp->hasSetChildValue(*this);
    else
...
{
    std::vector<std::string> subs;
    if (subname && subname[0]) subs.emplace_back(subname);
    aboutToSetValue();
...
void PropertyXLink::setSubValues(std::vector<std::string> &&subs, std::vector<ShadowSub> &&shadows)
{
    ...
    _ShadowSubList.clear();
    if (shadows.size() == _SubList.size()) _ShadowSubList = std::move(shadows);
    else
        updateElementReference(nullptr);
    checkLabelReferences(_SubList);
}

void PropertyXLink::setValue(App::DocumentObject *lValue) { setValue(lValue, nullptr); }
...
void PropertyXLink::restoreLink(App::DocumentObject *lValue)
{
    ...
    auto owner = dynamic_cast<DocumentObject *>(getContainer());
    if (!owner || !owner->getNameInDocument()) throw Base::RuntimeError("invalid container");
...
void PropertyXLink::setValue(App::DocumentObject *lValue, std::vector<std::string> &&subs,
                             std::vector<ShadowSub> &&shadows)
{
    if (_pcLink == lValue && _SubList == subs) return;
    ...
    if (lValue == owner) throw Base::ValueError("self linking");
...
#ifndef USE_OLD_DAG
    if (!owner->testStatus(ObjectStatus::Destroy) && _pcScope != LinkScope::Hidden) {
        if (_pcLink) _pcLink->_removeBackLink(owner);
        if (lValue) lValue->_addBackLink(owner);
    }
#endif
    if (docInfo != info) {
        unlink();
    ...
    if (!docInfo) filePath.clear();
    _pcLink = lValue;
    if (docInfo && docInfo->pcDoc) stamp = docInfo->pcDoc->LastModifiedDate.getValue();
    objectName = name;
                             ...
                             std::vector<std::string> &&subs, std::vector<ShadowSub> &&shadows)
{
    ...
    if (!filename.empty()) {
        ...
        if (info->pcDoc) pObject = info->pcDoc->getObject(name.c_str());
    }
    else
...
void PropertyXLink::setValue(App::DocumentObject *link, const std::vector<std::string> &subs,
                             std::vector<ShadowSub> &&shadows)
{
    setValue(link, std::vector<std::string>(subs), std::move(shadows));
}

App::Document *PropertyXLink::getDocument() const { return docInfo ? docInfo->pcDoc : nullptr; }
...
const char *PropertyXLink::getObjectName() const { return objectName.c_str(); }
...
int PropertyXLink::checkRestore(std::string *msg) const
{
    if (!docInfo) {
        if (!_pcLink && !objectName.empty()) {
            ...
            if (msg) {
                ...
                if (!filePath.empty()) ss << std::endl << "File: " << filePath;
                *msg = ss.str();
            }
            return 2;
        }
    ...
    if (!docInfo->pcDoc || stamp == docInfo->pcDoc->LastModifiedDate.getValue()) return 0;
...
void PropertyXLink::afterRestore()
{
    ...
    if (!testFlag(LinkRestoreLabel) || !_pcLink || !_pcLink->getNameInDocument()) return;
    setFlag(LinkRestoreLabel, false);
...
{
    if (!_pcLink || !_pcLink->getNameInDocument()) return;
    for (size_t i = 0; i < _SubList.size(); ++i)
...
{
    if (!updateLinkReference(this, feature, reverse, notify, _pcLink, _SubList, _mapped,
                             _ShadowSubList))
        return;
    if (notify) hasSetValue();
}

bool PropertyXLink::referenceChanged() const { return !_mapped.empty(); }

void PropertyXLink::Save(Base::Writer &writer) const
{
    auto owner = dynamic_cast<const DocumentObject *>(getContainer());
    if (!owner || !owner->getDocument()) return;
    ...
    if (_pcLink && exporting && _pcLink->isExporting()) {
        ...
        if (exporting) {
            ...
            if (docInfo) _path = docInfo->filePath();
            else {
            ...
            if (!_path.empty()) path = _path.c_str();
        }
    ...
    if (testFlag(LinkAllowPartial)) writer.Stream() << "\" partial=\"1";

    if (_SubList.empty()) { writer.Stream() << "\"/>" << std::endl; }
...
void PropertyXLink::Restore(Base::XMLReader &reader)
{
    ...
    if (reader.hasAttribute("stamp")) stampAttr = reader.getAttribute("stamp");
    if (reader.hasAttribute("file")) file = reader.getAttribute("file");
    setFlag(LinkAllowPartial,
            reader.hasAttribute("partial") && reader.getAttributeAsInteger("partial"));
    std::string name;
    if (file.empty()) name = reader.getName(reader.getAttribute("name"));
    else
    ...
    if (reader.hasAttribute("sub")) {
        if (reader.hasAttribute(ATTR_MAPPED)) mapped.push_back(0);
        subs.emplace_back();
...
PropertyXLink::CopyOnImportExternal(const std::map<std::string, std::string> &nameMap) const
{
    ...
    if (subs.empty() && linked == _pcLink) return nullptr;
...
Property *PropertyXLink::CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
                                           const char *newLabel) const
{
    auto owner = dynamic_cast<const DocumentObject *>(getContainer());
    if (!owner || !owner->getDocument() || !_pcLink || !_pcLink->getNameInDocument())
...
void PropertyXLink::copyTo(PropertyXLink &other, DocumentObject *linked,
                           std::vector<std::string> *subs) const
{
    if (!linked) linked = _pcLink;
    ...
    if (subs) other._SubList = std::move(*subs);
    else
...
bool PropertyXLink::hasXLink(const App::Document *doc)
{
    for (auto &v : _DocInfoMap) {
        if (v.second->hasXLink(doc)) return true;
    }
    return false;
}
                             ...
                             std::vector<App::Document *> *unsaved)
{
    ...
    for (auto o : objs) {
        if (o && o->getNameInDocument() && docs.insert(o->getDocument()).second) {
            if (!hasXLink(o->getDocument())) continue;
            if (!unsaved) return true;
            ret = true;
            if (!o->getDocument()->isSaved()) unsaved->push_back(o->getDocument());
        }
    }
    return ret;
}

void PropertyXLink::restoreDocument(const App::Document &doc) { DocInfo::restoreDocument(doc); }
...
PropertyXLink::getDocumentOutList(App::Document *doc)
{
    ...
    for (auto &v : _DocInfoMap) {
        for (auto link : v.second->links) {
            ...
            if (!obj || !obj->getNameInDocument() || !obj->getDocument()) continue;
            if (doc && obj->getDocument() != doc) continue;
...
PropertyXLink::getDocumentInList(App::Document *doc)
{
    std::map<App::Document *, std::set<App::Document *>> ret;
    for (auto &v : _DocInfoMap) {
        if (!v.second->pcDoc || (doc && doc != v.second->pcDoc)) continue;
...
{
    if (!_pcLink) Py_Return;
    const auto &subs = getSubValues(false);
    if (subs.empty()) return _pcLink->getPyObject();
    Py::Tuple ret(2);
...
void PropertyXLink::setPyObject(PyObject *value)
{
    if (PySequence_Check(value)) {
        Py::Sequence seq(value);
        if (seq.size() != 2) throw Base::ValueError("Expect input sequence of size 2");
        std::vector<std::string> subs;
...
{
    if (_SubList.empty() || _ShadowSubList.empty()) return "";
    return getSubNameWithStyle(_SubList[0], _ShadowSubList[0], newStyle).c_str();
}

void PropertyXLink::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                             std::vector<std::string> *subs, bool newStyle) const
{
    if ((all || _pcScope != LinkScope::Hidden) && _pcLink && _pcLink->getNameInDocument()) {
        objs.push_back(_pcLink);
        if (subs && _SubList.size() == _ShadowSubList.size()) *subs = getSubValues(newStyle);
    }
}
...
{
    if (_pcScope == LinkScope::Hidden) return false;
    if (!_pcLink || !_pcLink->getNameInDocument() || !inList.count(_pcLink)) return false;
    auto subs = _SubList;
...
void PropertyXLink::setAllowPartial(bool enable)
{
    ...
    if (!App::GetApplication().isRestoring() && !owner->getDocument()->isPerformingTransaction()
        ...
        if (!path.empty()) App::GetApplication().openDocument(path.c_str());
    }
}
...
int PropertyXLinkSubList::getSize() const { return static_cast<int>(_Links.size()); }
...
void PropertyXLinkSubList::setValue(DocumentObject *lValue, const char *SubName)
{
    ...
    if (lValue) {
        ...
        if (SubName) subs.emplace_back(SubName);
    }
...
#define CHECK_SUB_SIZE(_l, _r)                                                                     \
    ...
    for (auto &obj : lValue) {
        ...
        if (sub) values[obj].emplace_back(sub);
    }
    setValues(std::move(values));
}
                                     ...
                                     const std::vector<std::string> &lSubNames)
{
    ...
    std::map<DocumentObject *, std::vector<std::string>> values;
    int i = 0;
    for (auto &obj : lValue) values[obj].push_back(lSubNames[i++]);
    setValues(std::move(values));
}
...
void PropertyXLinkSubList::addValue(App::DocumentObject *obj, const std::vector<std::string> &subs,
                                    bool reset)
{
    addValue(obj, std::vector<std::string>(subs), reset);
}

void PropertyXLinkSubList::addValue(App::DocumentObject *obj, std::vector<std::string> &&subs,
                                    bool reset)
{

    if (!obj || !obj->getNameInDocument()) FC_THROWM(Base::ValueError, "invalid document object");

    for (auto &l : _Links) {
        if (l.getValue() == obj) {
            auto s = l.getSubValues();
            if (s.empty() || reset) l.setSubValues(std::move(subs));
            else {
...
{
    std::map<DocumentObject *, std::vector<std::string>> values;
    if (lValue) values[lValue] = SubList;
    setValues(std::move(values));
}
...
void PropertyXLinkSubList::set1Value(int idx, DocumentObject *value,
                                     const std::vector<std::string> &SubList)
{
    if (idx < -1 || idx > getSize()) throw Base::RuntimeError("index out of bound");
...
{
    if (_Links.empty()) return std::string("None");
    std::ostringstream ss;
    ss << '[';
...
{
    if (!_Links.empty()) return _Links.begin()->getValue();
    return nullptr;
}
...
PyObject *PropertyXLinkSubList::getPyObject()
{
    ...
    for (auto &link : _Links) {
        ...
        for (std::size_t j = 0; j < subs.size(); j++) { items[j] = Py::String(subs[j]); }
        tup[1] = items;
        list.append(tup);
    }
...
void PropertyXLinkSubList::Save(Base::Writer &writer) const
{
    ...
    writer.Stream() << "\">" << endl;
    writer.incInd();
    for (auto &l : _Links) l.Save(writer);
    writer.decInd();
...
PropertyXLinkSubList::CopyOnImportExternal(const std::map<std::string, std::string> &nameMap) const
{
    ...
    for (++it; it != _Links.end(); ++it) {
        p->_Links.emplace_back();
        copy.reset(it->CopyOnImportExternal(nameMap));
        if (copy) static_cast<PropertyXLinkSub &>(*copy).copyTo(p->_Links.back());
        else
...
Property *PropertyXLinkSubList::CopyOnLabelChange(App::DocumentObject *obj, const std::string &ref,
                                                  const char *newLabel) const
{
    std::unique_ptr<Property> copy;
    auto it = _Links.begin();
    ...
    for (; it != _Links.end(); ++it) {
        ...
        if (copy) {
            ...
            if (copied->getValue() == newObj) {
                for (auto &sub : copied->getSubValues()) subs.insert(sub);
            }
            break;
        }
    }
    ...
    for (auto iter = _Links.begin(); iter != it; ++iter) {
        if (iter->getValue() == newObj && copied->getValue() == newObj) {
            ...
            for (auto &sub : iter->getSubValues()) {
                if (subs.insert(sub).second) copied->_SubList.push_back(sub);
            }
        }
        else {
            p->_Links.emplace_back();
...
{
    unsigned int size = 0;
    for (auto &l : _Links) size += l.getMemSize();
    return size;
}
...
{
    for (auto &l : _Links) {
        if (l.getValue() == obj) return l.getSubValues();
    }
...
{
    for (auto &l : _Links) {
        if (l.getValue() == obj) return l.getSubValues(newStyle);
    }
    return {};
}

void PropertyXLinkSubList::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                                    std::vector<std::string> *subs, bool newStyle) const
{
    if (all || _pcScope != LinkScope::Hidden) {
        if (!subs) {
        ...
        for (auto &l : _Links) {
            auto obj = l.getValue();
            if (obj && obj->getNameInDocument()) count += std::max((int)l.getSubValues().size(), 1);
        }
        if (!count) {
            objs.reserve(objs.size() + _Links.size());
            for (auto &l : _Links) {
        ...
        for (auto &l : _Links) {
            ...
            if (obj && obj->getNameInDocument()) {
                ...
                if (subnames.empty()) subnames.emplace_back("");
...
bool PropertyXLinkSubList::adjustLink(const std::set<App::DocumentObject *> &inList)
{
    ...
    if (touched) {
        ...
        if (count) {
            ...
            for (auto it = _Links.begin(), itNext = it; it != _Links.end(); it = itNext) {
                ++itNext;
                if (!it->getValue()) tmp.splice(tmp.end(), _Links, it);
            }
        }
        setValues(std::move(values));
...
int PropertyXLinkSubList::checkRestore(std::string *msg) const
{
    for (auto &l : _Links) {
        int res;
        if ((res = l.checkRestore(msg))) return res;
    }
    return 0;
}
...
bool PropertyXLinkSubList::upgrade(Base::XMLReader &reader, const char *typeName)
{
    if (strcmp(typeName, PropertyLinkListGlobal::getClassTypeId().getName()) == 0
        ...
        for (size_t i = 0; i < objs.size(); ++i) values[objs[i]].push_back(subs[i]);
        setValues(std::move(values));
        return true;
    }
    _Links.clear();
...
{
    setFlag(LinkAllowPartial, enable);
    for (auto &l : _Links) l.setAllowPartial(enable);
}

void PropertyXLinkSubList::hasSetChildValue(Property &)
{
    if (!signalCounter) hasSetValue();
}
...
void PropertyXLinkSubList::aboutToSetChildValue(Property &)
{
    if (!signalCounter || !hasChanged) {
        aboutToSetValue();
        if (signalCounter) hasChanged = true;
    }
}
...
PyObject *PropertyXLinkList::getPyObject()
{
    for (auto &link : _Links) {
        auto obj = link.getValue();
        if (!obj || !obj->getNameInDocument()) continue;
        if (link.hasSubName()) return PropertyXLinkSubList::getPyObject();
    }
...
void PropertyXLinkContainer::breakLink(App::DocumentObject *obj, bool clear)
{
    ...
    if (!clear || obj != owner) {
        ...
        if (it == _Deps.end()) return;
        aboutToSetValue();
        onBreakLink(obj);
        if (obj->getDocument() != owner->getDocument()) _XLinks.erase(obj->getFullName());
        else if (!it->second)
    ...
    for (auto &v : _Deps) {
        auto key = v.first;
        if (!key || !key->getNameInDocument()) continue;
        onBreakLink(key);
        if (!v.second && key->getDocument() == owner->getDocument()) key->_removeBackLink(owner);
    }
    _XLinks.clear();
    _Deps.clear();
}
...
void PropertyXLinkContainer::Save(Base::Writer &writer) const
{
    ...
    if (owner && !owner->isExporting()) {
        ...
        for (auto &v : _XLinks) {
            ...
            if (obj && obj->getDocument()) docSet.insert(std::make_pair(obj->getDocument(), i));
        }

        if (!docSet.empty()) writer.Stream() << "\" docs=\"" << docSet.size();
    }
    ...
    if (hidden) writer.Stream() << "\" hidden=\"" << ss.str();
    ...
    for (auto &v : _XLinks) v.second->Save(writer);
    writer.decInd();
...
void PropertyXLinkContainer::Restore(Base::XMLReader &reader)
{
    ...
    for (auto &info : *_XLinkRestores) {
        ...
        if (info.hidden) info.xlink->setScope(LinkScope::Hidden);
...
PropertyXLink *PropertyXLinkContainer::createXLink() { return new PropertyXLink(false, this); }
...
void PropertyXLinkContainer::updateDeps(std::map<DocumentObject *, bool> &&newDeps)
{
        ...
        if (obj && obj->getNameInDocument()) {
            ...
            if (it != _Deps.end()) {
                if (v.second != it->second) {
                    if (v.second) obj->_removeBackLink(owner);
                    else
    ...
    if (!_LinkRestored && !testFlag(LinkDetached)) {
        for (auto it = _XLinks.begin(), itNext = it; it != _XLinks.end(); it = itNext) {
            ++itNext;
            if (!it->second->getValue()) _XLinks.erase(it);
        }
    }
}
...
void PropertyXLinkContainer::clearDeps()
{
    ...
    if (!owner->testStatus(ObjectStatus::Destroy)) {
        for (auto &v : _Deps) {
            auto obj = v.first;
            if (!v.second && obj && obj->getNameInDocument()
...
void PropertyXLinkContainer::getLinks(std::vector<App::DocumentObject *> &objs, bool all,
                                      std::vector<std::string> * /*subs*/, bool /*newStyle*/) const
{
    for (auto &v : _Deps) {
        if (all || !v.second) objs.push_back(v.first);
    }
}

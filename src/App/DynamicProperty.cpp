...
Property *DynamicProperty::getDynamicPropertyByName(const char *name) const
{
    ...
    if (it != index.end()) return it->property;
    return nullptr;
}
...
std::vector<std::string> DynamicProperty::getDynamicPropertyNames() const
{
    ...
    for (auto &v : index) names.push_back(v.name);
    return names;
}
...
short DynamicProperty::getPropertyType(const char *name) const
{
    ...
    if (it != index.end()) {
        short attr = it->attr;
        if (it->hidden) attr |= Prop_Hidden;
        if (it->readonly) attr |= Prop_ReadOnly;
        return attr;
    }
    return 0;
}
...
const char *DynamicProperty::getPropertyGroup(const Property *prop) const
{
    ...
    if (it != index.end()) return it->group.c_str();
    return nullptr;
}
...
const char *DynamicProperty::getPropertyDocumentation(const Property *prop) const
{
    auto &index = props.get<1>();
    auto it = index.find(const_cast<Property *>(prop));
    if (it != index.end()) return it->doc.c_str();
    return nullptr;
}
...
Property *DynamicProperty::addDynamicProperty(PropertyContainer &pc, const char *type,
                                              const char *name, const char *group, const char *doc,
...
{
    if (!prop || !prop->hasName()) return false;
    auto &index = props.get<0>();
    if (index.count(prop->getName())) return false;
    index.emplace(prop, std::string(), prop->getName(), prop->getGroup(), prop->getDocumentation(),
                  prop->getType(), false, false);
    return true;
}
...
Property *DynamicProperty::restore(PropertyContainer &pc, const char *PropName,
                                   const char *TypeName, Base::XMLReader &reader)
{
    if (!reader.hasAttribute("group")) return nullptr;
    ...
    if (reader.hasAttribute("doc")) doc = reader.getAttribute("doc");
...
bool DynamicProperty::changeDynamicProperty(const Property *prop, const char *group,
                                            const char *doc)
{
    auto &index = props.get<1>();
    auto it = index.find(const_cast<Property *>(prop));
...
const char *DynamicProperty::getPropertyName(const Property *prop) const
{
    auto &index = props.get<1>();
    auto it = index.find(const_cast<Property *>(prop));
    if (it != index.end()) return it->getName();
    return nullptr;
}

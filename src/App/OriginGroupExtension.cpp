...
OriginGroupExtension::OriginGroupExtension()
{
    ...
    EXTENSION_ADD_PROPERTY_TYPE(Origin, (nullptr), 0, App::Prop_Hidden,
                                                 ...
                                                 bool transform, int depth) const
{
    App::DocumentObject *originObj = Origin.getValue();
    const char *dot;
    if (originObj && originObj->getNameInDocument() && subname && (dot = strchr(subname, '.'))) {
        bool found;
        if (subname[0] == '$') found = std::string(subname + 1, dot) == originObj->Label.getValue();
        else
    ...
    return GeoFeatureGroupExtension::extensionGetSubObject(ret, subname, pyObj, mat, transform,
                                                           depth);
}
...
App::DocumentObject *OriginGroupExtension::getGroupOfObject(const DocumentObject *obj)
{
    ...
    for (auto o : list) {
        if (o->hasExtension(App::OriginGroupExtension::getExtensionClassTypeId())) return o;
...
void OriginGroupExtension::relinkToOrigin(App::DocumentObject *obj)
{
    ...
    for (App::Property *prop : list) {
        if (prop->getTypeId().isDerivedFrom(App::PropertyLink::getClassTypeId())) {
            ...
            if (changed) static_cast<App::PropertyLinkList *>(prop)->setValues(result);
        }
            ...
            if (changed) p->setSubListValues(vec);
        }
    }
}
...
std::vector<DocumentObject *> OriginGroupExtension::addObjects(std::vector<DocumentObject *> objs)
{
    ...
    for (auto obj : objs) relinkToOrigin(obj);
...
bool OriginGroupExtension::hasObject(const DocumentObject *obj, bool recursive) const
{
    ...
    if (Origin.getValue() && (obj == getOrigin() || getOrigin()->hasObject(obj))) return true;

...
PyObject *PropertyContainerPy::getPropertyByName(PyObject *args)
{
    char *pstr;
    int checkOwner = 0;
    if (!PyArg_ParseTuple(args, "s|i", &pstr, &checkOwner)) return nullptr;
...
PyObject *PropertyContainerPy::getPropertyTouchList(PyObject *args)
{
    ...
    if (prop && prop->isDerivedFrom(PropertyLists::getClassTypeId())) {
        ...
        int i = 0;
        for (int idx : touched) ret.setItem(i++, Py::Long(idx));
...
PyObject *PropertyContainerPy::getTypeOfProperty(PyObject *args)
{
    ...
    if (Type & Prop_ReadOnly) ret.append(Py::String("ReadOnly"));
    if (Type & Prop_Transient) ret.append(Py::String("Transient"));
    if (Type & Prop_Hidden) ret.append(Py::String("Hidden"));
    if (Type & Prop_Output) ret.append(Py::String("Output"));
    if (Type & Prop_NoRecompute) ret.append(Py::String("NoRecompute"));
    if (Type & Prop_NoPersist) ret.append(Py::String("NoPersist"));
    ...
    if (PyArg_ParseTuple(args, "sO", &name, &iter)) {
        if (PyTuple_Check(iter) || PyList_Check(iter)) {
            ...
            for (Py::Sequence::iterator it = seq.begin(); it != seq.end(); ++it) {
                ...
                if (str == "ReadOnly") status.set(Property::ReadOnly);
                else if (str == "Hidden")
...
PyObject *PropertyContainerPy::setPropertyStatus(PyObject *args)
{
    char *name;
    PyObject *pyValue;
    if (!PyArg_ParseTuple(args, "sO", &name, &pyValue)) return nullptr;
    ...
    for (size_t i = 0; i < count; ++i) {
        Py::Object item;
        if (isSeq) item = Py::Object(PySequence_GetItem(pyValue, i));
        else
...
PyObject *PropertyContainerPy::getPropertyStatus(PyObject *args)
{
    ...
    const auto &statusMap = getStatusMap();
    if (!name[0]) {
        for (auto &v : statusMap) ret.append(Py::String(v.first.c_str()));
    }
    else {
        App::Property *prop = getPropertyContainerPtr()->getPropertyByName(name);
        if (!prop) {
        ...
        for (size_t i = 1; i < bits.size(); ++i) {
            ...
            if (!found) ret.append(Py::Int(static_cast<long>(i)));
        }
    }
    return Py::new_reference_to(ret);
}
...
PyObject *PropertyContainerPy::getGroupOfProperty(PyObject *args)
{
    ...
    if (Group) return Py::new_reference_to(Py::String(Group));
    else
...
PyObject *PropertyContainerPy::setGroupOfProperty(PyObject *args)
{
    char *pstr;
    char *group;
    if (!PyArg_ParseTuple(args, "ss", &pstr, &group)) return nullptr;

    PY_TRY
    {
        Property *prop = getPropertyContainerPtr()->getDynamicPropertyByName(pstr);
        if (!prop) {
            PyErr_Format(PyExc_AttributeError, "Property container has no dynamic property '%s'",
                         pstr);
            return nullptr;
        }
...
PyObject *PropertyContainerPy::getDocumentationOfProperty(PyObject *args)
{
    ...
    if (docstr) return Py::new_reference_to(Py::String(docstr));
    else
        return Py::new_reference_to(Py::String(""));
}
...
PyObject *PropertyContainerPy::setDocumentationOfProperty(PyObject *args)
{
    char *pstr;
    char *doc;
    if (!PyArg_ParseTuple(args, "ss", &pstr, &doc)) return nullptr;
...
PyObject *PropertyContainerPy::getEnumerationsOfProperty(PyObject *args)
{
    ...
    if (!enumProp) Py_Return;

    std::vector<std::string> enumerations = enumProp->getEnumVector();
    Py::List ret;
    for (std::vector<std::string>::const_iterator it = enumerations.begin();
         it != enumerations.end(); ++it) {
...
PyObject *PropertyContainerPy::restorePropertyContent(PyObject *args)
{
    PyObject *buffer;
    char *property;
    if (!PyArg_ParseTuple(args, "sO", &property, &buffer)) return nullptr;
    ...
    Py_buffer buf;
    if (PyObject_GetBuffer(buffer, &buf, PyBUF_SIMPLE) < 0) return nullptr;
...
PyObject *PropertyContainerPy::getCustomAttributes(const char *attr) const
{
    ...
    if (FC_LOG_INSTANCE.level() > FC_LOGLEVEL_TRACE) { FC_TRACE("Get property " << attr); }
        ...
        if (_getShape != Py_None) {
            ...
            if (!res) PyErr_Clear();
                ...
                if (pyres.hasAttr("isNull")) {
                    ...
                    if (!func.apply().isTrue()) return Py::new_reference_to(res);
                }
            }
        }
    }

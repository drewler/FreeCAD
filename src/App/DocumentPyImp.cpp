...
{
    char *fn;
    if (!PyArg_ParseTuple(args, "et", "utf-8", &fn)) return nullptr;
...
PyObject *DocumentPy::addObject(PyObject *args, PyObject *kwd)
{
    ...
    static char *kwlist[] = {"type",   "name",     "objProxy", "viewProxy",
                             "attach", "viewType", nullptr};
    if (!PyArg_ParseTupleAndKeywords(args, kwd, "s|sOOO!s", kwlist, &sType, &sName, &obj, &view,
                                     &PyBool_Type, &attach, &sViewType))
        return nullptr;
    ...
    if (obj) {
        try {
            ...
            Py::Object pyobj(obj);
            if (pyobj.hasAttr("__object__")) { pyobj.setAttr("__object__", pyftr); }
            ...
            Py::Object pyvp;
            if (view) pyvp = Py::Object(view);
            if (pyvp.isNone()) pyvp = Py::Int(1);
...
PyObject *DocumentPy::copyObject(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        if (ret.size() == 1 && single) return ret[0]->getPyObject();
...
PyObject *DocumentPy::importLinks(PyObject *args)
{
    ...
    if (objs.empty()) objs = getDocumentPtr()->getObjects();
...
PyObject *DocumentPy::moveObject(PyObject *args)
{
    ...
    if (move) { return move->getPyObject(); }
    else {
...
{
    if (!PyArg_ParseTuple(args, "")) return nullptr;
    if (getDocumentPtr()->getAvailableUndos()) getDocumentPtr()->undo();
    Py_Return;
}
...
{
    if (!PyArg_ParseTuple(args, "")) return nullptr;
    if (getDocumentPtr()->getAvailableRedos()) getDocumentPtr()->redo();
    Py_Return;
}
...
{
    PyObject *close;
    if (!PyArg_ParseTuple(args, "O!", &PyBool_Type, &close)) return nullptr;
...
PyObject *DocumentPy::recompute(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        int options = 0;
        if (Base::asBoolean(checkCycle)) options = Document::DepNoCycle;
...
PyObject *DocumentPy::getObjectsByLabel(PyObject *args)
{
    ...
    for (std::vector<DocumentObject *>::iterator it = objs.begin(); it != objs.end(); ++it) {
        if (name == (*it)->Label.getValue()) list.append(Py::asObject((*it)->getPyObject()));
    }
...
PyObject *DocumentPy::findObjects(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|sss", kwlist, &sType, &sName, &sLabel))
        return nullptr;
...
Py::Object DocumentPy::getActiveObject() const
{
    ...
    if (pcFtr) return Py::Object(pcFtr->getPyObject(), true);
    return Py::None();
}
...
void DocumentPy::setUndoMode(Py::Int arg) { getDocumentPtr()->setUndoMode(arg); }
...
Py::String DocumentPy::getName() const { return Py::String(getDocumentPtr()->getName()); }
...
Py::List DocumentPy::getInList() const
{
    ...
    if (lists.size() == 1) {
        for (auto doc : lists.begin()->second) ret.append(Py::Object(doc->getPyObject(), true));
    }
    return ret;
}
...
PyObject *DocumentPy::getDependentDocuments(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        Py::List ret;
        for (auto doc : docs) ret.append(Py::Object(doc->getPyObject(), true));
        return Py::new_reference_to(ret);
    }
    PY_CATCH;
}
...
Py::String DocumentPy::getOldLabel() const { return Py::String(getDocumentPtr()->getOldLabel()); }

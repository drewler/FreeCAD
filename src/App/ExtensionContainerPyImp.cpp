...
std::string ExtensionContainerPy::representation() const { return std::string("<extension>"); }
        ...
        if (meth->ml_name) {
            ...
            if (!item) {
                ...
                while (meth->ml_name) {
                    ...
                    if (!func) break;
                    if (PyDict_SetItemString(dict, meth->ml_name, func) < 0) break;
                    Py_DECREF(func);
                    ++meth;
                }
...
PyObject *ExtensionContainerPy::PyMake(struct _typeobject *, PyObject *,
...
// constructor method
int ExtensionContainerPy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }
...
PyObject *ExtensionContainerPy::hasExtension(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|O!", &type, &PyBool_Type, &deriv)) return nullptr;
    ...
    bool val = false;
    if (getExtensionContainerPtr()->hasExtension(extension, derived)) { val = true; }
...
PyObject *ExtensionContainerPy::addExtension(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|O", &typeId, &proxy)) return nullptr;

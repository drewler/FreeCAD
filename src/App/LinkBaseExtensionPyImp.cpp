...
PyObject *LinkBaseExtensionPy::configLinkProperty(PyObject *args, PyObject *keywds)
{
    ...
    if (args && PyTuple_Check(args)) {
        for (Py_ssize_t pos = 0; pos < PyTuple_GET_SIZE(args); ++pos) {
            ...
            if (!getProperty(props, info, propMap, key, key)) return nullptr;
        }
    }
    if (keywds && PyDict_Check(keywds)) {
        ...
        while (PyDict_Next(keywds, &pos, &key, &value)) {
            if (!getProperty(props, info, propMap, key, value)) return nullptr;
        }
    }
    for (auto &v : props) ext->setProperty(v.second.first, v.second.second);
    Py_Return;
}
...
PyObject *LinkBaseExtensionPy::getLinkPropertyInfo(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "")) {
        ...
        for (const auto &info : infos) {
            ret.setItem(i++,
                        Py::TupleN(Py::String(info.name), Py::String(info.type.getName()),
                                   Py::String(info.doc)));
        }
    ...
    if (PyArg_ParseTuple(args, "h", &index)) {
        ...
        Py::TupleN ret(Py::String(infos[index].name), Py::String(infos[index].type.getName()),
    ...
    if (PyArg_ParseTuple(args, "s", &name)) {
        for (int i = 0; i < (int)infos.size(); ++i) {
            if (strcmp(infos[i].name, name) == 0) {
                Py::TupleN ret(Py::String(infos[i].type.getName()), Py::String(infos[i].doc));
                return Py::new_reference_to(ret);
            }
        }
        ...
        if (PyObject_TypeCheck(value, &DocumentObjectPy::Type)) {
            ...
            if (seq[0].ptr() != Py_None) {
                ...
                if (seq.size() > 1) {
                    ...
                    if (seq.size() > 2) subs.setPyObject(seq[2].ptr());
                }
            }
        }
    }
...
PyObject *LinkBaseExtensionPy::setLink(PyObject *_args)
{
    ...
    PY_TRY
    {
        ...
        if (pcObj == Py_None) { ext->setLink(-1, nullptr); }
        else if (PyDict_Check(pcObj)) {
            PyObject *key, *value;
            Py_ssize_t pos = 0;
            while (PyDict_Next(pcObj, &pos, &key, &value)) parseLink(ext, Py::Int(key), value);
        }
        else if (PySequence_Check(pcObj)) {
            ext->setLink(-1, nullptr);
            Py::Sequence seq(pcObj);
            for (Py_ssize_t i = 0; i < seq.size(); ++i) parseLink(ext, i, seq[i].ptr());
        }
        else
...
PyObject *LinkBaseExtensionPy::cacheChildLabel(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O", &enable)) return nullptr;
    PY_TRY
    {
...
PyObject *LinkBaseExtensionPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

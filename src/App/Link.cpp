...
public:
    ...
    // Auto generated code. See class document of LinkParams.
    std::unordered_map<const char *, void (*)(LinkParamsP *), App::CStringHasher,
                       App::CStringHasher>
        funcs;
...
// Auto generated code. See class document of LinkParams.
ParameterGrp::handle LinkParams::getHandle() { return instance()->handle; }
...
// Auto generated code. See class document of LinkParams.
const bool &LinkParams::getCopyOnChangeApplyToAll() { return instance()->CopyOnChangeApplyToAll; }
    ...
    : enableLabelCache(false), hasOldSubElement(false), hasCopyOnChange(true)
{
    ...
    EXTENSION_ADD_PROPERTY_TYPE(_LinkOwner, (0), " Link", PropertyType(Prop_Hidden | Prop_Output),
                                0);
...
const std::vector<LinkBaseExtension::PropInfo> &LinkBaseExtension::getPropertyInfo() const
{
    ...
    if (PropsInfo.empty()) { BOOST_PP_SEQ_FOR_EACH(LINK_PROP_INFO, PropsInfo, LINK_PARAMS); }
    return PropsInfo;
}
...
const LinkBaseExtension::PropInfoMap &LinkBaseExtension::getPropertyInfoMap() const
{
    ...
    if (PropsMap.empty()) {
        ...
        for (const auto &info : infos) PropsMap[info.name] = info;
    }
    return PropsMap;
}
...
{
    if (idx >= 0 && idx < (int)props.size()) return props[idx];
    return nullptr;
}
...
void LinkBaseExtension::setProperty(int idx, Property *prop)
{
    ...
    switch (idx) {
        case PropLinkMode: {
            static const char *linkModeEnums[] = {"None", "Auto Delete", "Auto Link", "Auto Unlink",
                                                  nullptr};
            auto propLinkMode = static_cast<PropertyEnumeration *>(prop);
            if (!propLinkMode->hasEnums()) propLinkMode->setEnums(linkModeEnums);
            break;
        }
        case PropLinkCopyOnChange: {
            ...
            if (!propEnum->hasEnums()) propEnum->setEnums(enums);
            break;
        }
        case PropLinkCopyOnChangeTouched:
        case PropLinkCopyOnChangeSource:
        case PropLinkCopyOnChangeGroup: prop->setStatus(Property::Hidden, true); break;
    ...
    if (FC_LOG_INSTANCE.isEnabled(FC_LOGLEVEL_TRACE)) {
        ...
        if (!Property::isValidName(propName)) propName = "?";
    ...
    if (getLinkedObjectProperty()) {
        ...
        if (!linked) {
            ...
            if (xlink) {
                ...
                if (objname && objname[0]) ss << "\nObject: " << objname;
                const char *filename = xlink->getFilePath();
                if (filename && filename[0]) ss << "\nFile: " << filename;
            }
        ...
        if (proxy) {
            ...
            try {
                ...
                if (!method || !method[0]) method = "appLinkExecute";
                if (proxyValue.hasAttr(method)) {
                    ...
                    if (attr.ptr() && attr.isCallable()) {
                        ...
                        if (!_getElementCountValue()) { Py::Callable(attr).apply(args); }
                        else {
...
{
    auto parent = getContainer();
    if (!src) src = getLinkCopyOnChangeSourceValue();
    if (!src || getLinkCopyOnChangeValue() == CopyOnChangeDisabled) return {};
    ...
    for (auto it = res.begin(); it != res.end();) {
        ...
        const char *v = "";
        if (src->getDocument() != obj->getDocument()) v = "-";
        auto iter = map.find("*");
        if (iter != map.end()) v = iter->second.c_str();
        else if ((iter = map.find(parent->getNameInDocument())) != map.end())
            v = iter->second.c_str();
        if (boost::equals(v, "-")) {
            if (excludes) excludes->push_back(obj);
            else {
                it = res.erase(it);
                continue;
            }
...
void LinkBaseExtension::setOnChangeCopyObject(App::DocumentObject *obj, OnChangeCopyOptions options)
{
    ...
    const char *key =
        flags.testFlag(OnChangeCopyOptions::ApplyAll) ? "*" : parent->getNameInDocument();
    if (external) prop->setValue(key, exclude ? "" : "+");
    else
...
{
    if (!getLinkCopyOnChangeValue()) return;
    ...
    if (auto prop = getLinkCopyOnChangeGroupProperty()) {
        ...
        if (!copyOnChangeGroup) {
            ...
            for (auto obj : objs) {
                ...
                if (prop && prop->getContainer() == obj) oldObjs.emplace_back(prop);
                else
    ...
    for (auto prop : propList) {
        if (!prop->testStatus(Property::CopyOnChange) || prop->getContainer() != linked) continue;
    ...
    for (auto obj : copiedObjs) {
        ...
        if (prop) newObjs.insert(std::make_pair(prop->getValue(), obj));
    }
    ...
    for (const auto &objT : oldObjs) {
        ...
        if (it == newObjs.end()) continue;
        auto oldObj = objT.getObject();
        auto newObj = it->second.getObject();
        if (oldObj && newObj) replacements.emplace_back(oldObj, newObj);
    }
    ...
    if (!replacements.empty()) {
        ...
        for (auto doc : App::GetApplication().getDocuments()) {
            for (auto o : doc->getObjects()) {
                ...
                for (auto prop : propList) {
                    if (prop->getContainer() != o) continue;
    ...
    for (const auto &v : propChanges) {
        ...
        if (prop) prop->Paste(*v.second.get());
    }
...
{
    if (obj != prop.getContainer() || !prop.testStatus(App::Property::PropDynamic)) return false;
...
void LinkBaseExtension::setupCopyOnChange(DocumentObject *parent, bool checkSource)
{
    ...
    if (!linked || getLinkCopyOnChangeValue() == CopyOnChangeDisabled) return;

    if (checkSource && !pauseCopyOnChange) {
        PropertyLink *source = getLinkCopyOnChangeSourceProperty();
        if (source) {
            source->setValue(linked);
            if (auto touched = getLinkCopyOnChangeTouchedProperty()) touched->setValue(false);
        }
    }
...
bool LinkBaseExtension::setupCopyOnChange(
    DocumentObject *parent, DocumentObject *linked,
    std::vector<boost::signals2::scoped_connection> *copyOnChangeConns, bool checkExisting)
{
    ...
    for (auto prop : props) {
        ...
        if (!linkedGroupName || !linkedGroupName[0]) linkedGroupName = "Base";
        ...
        if (p) {
            if (p->getContainer() != parent) p = nullptr;
            else {
        ...
        if (!p) {
            p = parent->addDynamicProperty(prop->getTypeId().getName(), prop->getName(),
                                           groupName.c_str(), prop->getDocumentation());
            std::unique_ptr<Property> pcopy(prop->Copy());
            Base::ObjectStatusLocker<Property::Status, Property> guard(Property::User3, p);
            if (pcopy) { p->Paste(*pcopy); }
    ...
    if (checkExisting) {
        ...
        parent->getPropertyList(props);
        for (auto prop : props) {
            if (prop->getContainer() != parent) continue;
            auto gname = prop->getGroup();
            if (!gname || !boost::starts_with(gname, _GroupPrefix)) continue;
            if (!newProps.count(prop)) parent->removeDynamicProperty(prop->getName());
        }
    }

    if (!copyOnChangeConns) return res;
    ...
    for (const auto &v : newProps) {
        ...
        copyOnChangeConns->push_back(
            v.second->signalChanged.connect([parent](const Property &prop) {
                if (!prop.testStatus(Property::CopyOnChange)) return;
                auto p = parent->getPropertyByName(prop.getName());
                if (p && p->getTypeId() == prop.getTypeId()) {
                    std::unique_ptr<Property> pcopy(prop.Copy());
...
void LinkBaseExtension::checkCopyOnChange(App::DocumentObject *parent, const App::Property &prop)
{
    ...
    if (copied) {
        ...
        if (linkedProp && linkedProp->getTypeId() == prop.getTypeId()) {
            std::unique_ptr<Property> pcopy(prop.Copy());
            if (pcopy) linkedProp->Paste(*pcopy);
        }
    }
}
...
{
    auto linked = getLinkedObjectValue();
    if (pauseCopyOnChange || !linked) return nullptr;
    auto parent = getContainer();
...
{
    copyOnChangeSrcConns.clear();
    if (getLinkCopyOnChangeValue() == CopyOnChangeDisabled) return;
    for (auto obj : objs) {
...
{
    if (!mySubElements.empty() && !mySubElements[0].empty()) return nullptr;
    auto linked = getTrueLinkedObject(false);
...
{
    if (_ChildCache.getSize()) return _ChildCache.getValues();
    if (getElementListProperty()) return getElementListProperty()->getValues();
...
App::PropertyBool *LinkBaseExtension::_getShowElementProperty() const
{
    ...
    if (prop && !linkedPlainGroup()) return const_cast<App::PropertyBool *>(prop);
    return nullptr;
}
...
App::PropertyInteger *LinkBaseExtension::_getElementCountProperty() const
{
    ...
    if (prop && !linkedPlainGroup()) return const_cast<App::PropertyInteger *>(prop);
    return nullptr;
}
...
bool LinkBaseExtension::extensionHasChildElement() const
{
    if (!_getElementListValue().empty() || (_getElementCountValue() && _getShowElementValue()))
        return true;
    if (getLinkClaimChildValue()) return false;
    DocumentObject *linked = getTrueLinkedObject(false);
    if (linked) {
        if (linked->hasChildElement()) return true;
    }
    return false;
}
...
int LinkBaseExtension::extensionSetElementVisible(const char *element, bool visible)
{
    ...
    if (index >= 0) {
        ...
        if (!propElementVis || !element || !element[0]) return -1;
        ...
        if (index < (int)elements.size()) {
            if (!visible) myHiddenElements.insert(elements[index]);
            else
    ...
    DocumentObject *linked = getTrueLinkedObject(false);
    if (linked) return linked->setElementVisible(element, visible);
    return -1;
}
...
int LinkBaseExtension::extensionIsElementVisible(const char *element)
{
    ...
    if (index >= 0) {
        auto propElementVis = getVisibilityListProperty();
        if (propElementVis) {
            if (propElementVis->getSize() <= index || propElementVis->getValues()[index]) return 1;
            return 0;
        }
        return -1;
    }
    DocumentObject *linked = getTrueLinkedObject(false);
    if (linked) return linked->isElementVisible(element);
    return -1;
}
...
{
    if (!GetApplication().checkLinkDepth(depth, true)) return nullptr;
    if (getLinkedObjectProperty()) return getLinkedObjectValue();
    return nullptr;
}

int LinkBaseExtension::getArrayIndex(const char *subname, const char **psubname)
{
    if (!subname || Data::ComplexGeoData::isMappedElement(subname)) return -1;
...
int LinkBaseExtension::getElementIndex(const char *subname, const char **psubname) const
{
    ...
    if (isdigit(subname[0])) {
        ...
        if (_getElementCountProperty()) {
            if (idx >= _getElementCountValue()) return -1;
        }
            ...
            if (boost::algorithm::starts_with(name, ownerName.c_str())) {
                for (const char *txt = dot - 1; txt >= name + ownerName.size(); --txt) {
                    if (*txt == 'i') {
                        ...
                        if (idx < 0 || idx >= _getElementCountValue()) idx = -1;
                        break;
                    }
                    if (!isdigit(*txt)) break;
                }
            }
        }
        if (idx < 0) {
            ...
            if (subname[0] == '$') {
                ...
                if (boost::equals(sub, linked->Label.getValue())) idx = 0;
            }
            else {
                CharRange sub(subname, dot);
                if (boost::equals(sub, linked->getNameInDocument())) idx = 0;
            }
            if (idx < 0) {
        ...
        const auto &elements = _getElementListValue();
        if (enableLabelCache) {
            if (myLabelCache.empty()) cacheChildLabel(1);
            auto it = myLabelCache.find(name);
            if (it == myLabelCache.end()) return -1;
            idx = it->second;
        }
        else {
            idx = 0;
            for (auto element : elements) {
                if (element->Label.getStrValue() == name) break;
                ++idx;
            }
        }
        if (idx < 0 || idx >= (int)elements.size()) return -1;
    ...
    if (psubname) *psubname = dot[0] ? dot + 1 : dot;
    return idx;
}
...
{
    if (getScaleVectorProperty()) return getScaleVectorValue();
...
Base::Matrix4D LinkBaseExtension::getTransform(bool transform) const
{
    Base::Matrix4D mat;
    if (transform) {
        if (getLinkPlacementProperty()) mat = getLinkPlacementValue().toMatrix();
...
bool LinkBaseExtension::extensionGetSubObjects(std::vector<std::string> &ret, int reason) const
{
    ...
    if (mySubElements.empty() || mySubElements[0].empty()) {
        DocumentObject *linked = getTrueLinkedObject(true);
        if (linked) {
            if (!_getElementCountValue()) ret = linked->getSubObjects(reason);
            else {
                char index[30];
...
bool LinkBaseExtension::extensionGetSubObject(DocumentObject *&ret, const char *subname,
                                              PyObject **pyObj, Base::Matrix4D *mat, bool transform,
                                              int depth) const
{
    ret = nullptr;
    ...
    if (idx >= 0) {
        ...
        if (!elements.empty()) {
            if (idx >= (int)elements.size() || !elements[idx]
                || !elements[idx]->getNameInDocument())
                return true;
        ...
        if (idx >= elementCount) return true;
        isElement = true;
        if (mat) {
    ...
    if (!linked || linked == obj) return true;
    ...
    if (const char *dot = strchr(subname, '.')) {
        ...
        if (subname[0] == '$') {
            CharRange sub(subname + 1, dot);
            if (group && boost::equals(sub, group->Label.getValue())) linked = group;
            else if (!boost::equals(sub, linked->Label.getValue()))
                dot = nullptr;
        }
        else {
            CharRange sub(subname, dot);
            if (group && boost::equals(sub, group->getNameInDocument())) linked = group;
...
void LinkBaseExtension::checkGeoElementMap(const App::DocumentObject *obj,
                                           const App::DocumentObject *linked, PyObject **pyObj,
...
{
    if (!getElementListProperty()) return;
    detachElements();
...
DocumentObject *LinkBaseExtension::getTrueLinkedObject(bool recurse, Base::Matrix4D *mat, int depth,
                                                       bool noElement) const
{
    ...
    if (ret && recurse) ret = ret->getLinkedObject(recurse, mat, transform, depth + 1);
    if (ret && !ret->getNameInDocument()) return nullptr;
    return ret;
}

bool LinkBaseExtension::extensionGetLinkedObject(DocumentObject *&ret, bool recurse,
                                                 Base::Matrix4D *mat, bool transform,
                                                 int depth) const
{
    if (mat) *mat *= getTransform(transform);
    ret = nullptr;
    if (!_getElementCountValue()) ret = getTrueLinkedObject(recurse, mat, depth);
    if (!ret) ret = const_cast<DocumentObject *>(getContainer());
...
void LinkBaseExtension::parseSubName() const
{
    ...
    if (!xlink || xlink->getSubValues().empty()) {
        if (hasSubElement) mySubElements.emplace_back("");
        return;
    }
...
{
    auto group = obj.getExtensionByType<GroupExtension>(true, false);
    if (group && &prop == &group->Group) updateGroup();
}
...
void LinkBaseExtension::updateGroup()
{
    ...
    if (group) {
        ...
        for (auto o : getElementListProperty()->getValues()) {
            if (!o || !o->getNameInDocument()) continue;
    ...
    if (!groups.empty()) {
        ...
        for (auto ext : groups) {
            ...
            for (; count < children.size(); ++count) {
                ...
                if (!child->getExtensionByType<GroupExtension>(true, false)) continue;
                groupSet.insert(child);
                auto &conn = plainGroupConns[child];
                if (!conn.connected()) {
                    FC_LOG("new group connection " << getExtendedObject()->getFullName() << " -> "
                                                   << child->getFullName());
                    conn = child->signalChanged.connect(boost::bind(
                        &LinkBaseExtension::slotChangedPlainGroup, this, bp::_1, bp::_2));
                }
            }
        }
    }
    for (auto it = plainGroupConns.begin(); it != plainGroupConns.end();) {
        if (!groupSet.count(it->first)) it = plainGroupConns.erase(it);
        else
            ++it;
    }
    if (children != _ChildCache.getValues()) _ChildCache.setValue(children);
}
...
void LinkBaseExtension::update(App::DocumentObject *parent, const Property *prop)
{
    ...
    if (prop == getLinkPlacementProperty() || prop == getPlacementProperty()) {
        ...
        if (_getShowElementValue()) update(parent, _getElementCountProperty());
        else {
            ...
            if (getScaleListProperty()) getScaleListProperty()->setValue(scales);
        ...
        if (!_getShowElementValue()) {
            ...
            if (elementCount > objs.size()) {
                ...
                for (size_t i = objs.size(); i < elementCount; ++i) {
                    ...
                    if (vis.size() > i && !vis[i]) myHiddenElements.insert(obj);
                ...
                if (getPlacementListProperty()) getPlacementListProperty()->setSize(0);
                if (getScaleListProperty()) getScaleListProperty()->setSize(0);
        ...
        if (_getShowElementValue()) {
            ...
            for (size_t i = 0; i < vis.size(); ++i) {
                ...
                if (!vis[i]) myHiddenElements.insert(elements[i]);
            }
        }
    }
        ...
        if (enableLabelCache) myLabelCache.clear();
        ...
        if (auto source = getLinkCopyOnChangeSourceValue()) {
            this->connCopyOnChangeSource = source->signalChanged.connect(
                [this](const DocumentObject &obj, const Property &prop) {
                    ...
                    if (src != &obj || getLinkCopyOnChangeValue() == CopyOnChangeDisabled) return;
...
void LinkBaseExtension::cacheChildLabel(int enable) const
{
    ...
    for (auto child : _getElementListValue()) {
        if (child && child->getNameInDocument()) myLabelCache[child->Label.getStrValue()] = idx;
        ++idx;
    }
}
...
void LinkBaseExtension::syncElementList()
{
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
        ...
        if (element->LinkCopyOnChange.getValue() == 2) continue;
        if (xlink) {
...
void LinkBaseExtension::onExtendedDocumentRestored()
{
    ...
    if (hasOldSubElement) {
        ...
        if (!xlink) FC_ERR("Failed to restore SubElements for " << parent->getFullName());
            ...
            for (const auto &s : subset) subs.push_back(sub + s);
            xlink->setSubValues(std::move(subs));
        }
    }
    if (getScaleVectorProperty() && getScaleProperty()) {
        ...
        const auto &v = getScaleVectorValue();
        double s = getScaleValue();
        if (v.x == v.y && v.x == v.z && v.x != s) getScaleVectorProperty()->setValue(s, s, s);
    }
    update(parent, getVisibilityListProperty());
...
void LinkBaseExtension::_handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                                   const char *PropName)
{
...
void LinkBaseExtension::setLink(int index, DocumentObject *obj, const char *subname,
                                const std::vector<std::string> &subElements)
{
    auto parent = getContainer();
    if (!parent) LINK_THROW(Base::RuntimeError, "No parent container");

    if (obj && !App::Document::isAnyRestoring()) {
        auto inSet = parent->getInListEx(true);
        inSet.insert(parent);
        if (inSet.find(obj) != inSet.end()) LINK_THROW(Base::RuntimeError, "Cyclic dependency");
    }
    ...
    if (index >= 0) {
        ...
        if (!obj) {
            ...
            for (int i = 0; i < (int)elements.size(); ++i) {
                if (i != index) objs.push_back(elements[i]);
            }
            getElementListProperty()->setValue(objs);
        }
            ...
            if (index < (int)elements.size()) old = elements[index];
            ...
            if (getLinkModeValue() >= LinkModeAutoLink || (subname && subname[0])
                ...
                if (linked) link->Label.setValue(linked->Label.getValue());
                auto pla =
                    freecad_dynamic_cast<PropertyPlacement>(obj->getPropertyByName("Placement"));
                if (pla) link->Placement.setValue(pla->getValue());
    ...
    if (obj) {
        if (!obj->getNameInDocument()) LINK_THROW(Base::ValueError, "Invalid document object");
        if (!xlink) {
...
void LinkBaseExtension::detachElements()
{
    ...
    for (auto obj : getElementListValue()) objs.emplace_back(obj);
    getElementListProperty()->setValue();
    for (const auto &objT : objs) detachElement(objT.getObject());
}

void LinkBaseExtension::detachElement(DocumentObject *obj)
{
    if (!obj || !obj->getNameInDocument() || obj->isRemoving()) return;
    auto ext = obj->getExtensionByType<LinkBaseExtension>(true);
    auto owner = getContainer();
    long ownerID = owner ? owner->getID() : 0;
    if (getLinkModeValue() == LinkModeAutoUnlink) {
        if (!ext || ext->_LinkOwner.getValue() != ownerID) return;
    }
    else if (getLinkModeValue() != LinkModeAutoDelete) {
        if (ext && ext->_LinkOwner.getValue() == ownerID) ext->_LinkOwner.setValue(0);
        return;
    }
    obj->getDocument()->removeObject(obj->getNameInDocument());
}
...
{
    if (!filter) return _getElementListValue();
    std::vector<App::DocumentObject *> ret;
    for (auto o : _getElementListValue()) {
        if (!o->hasExtension(GroupExtension::getExtensionClassTypeId(), false)) ret.push_back(o);
    }
    return ret;
}
...
const char *LinkBaseExtension::flattenSubname(const char *subname) const
{
    if (subname && _ChildCache.getSize()) {
        ...
        for (const char *dot = strchr(sub, '.'); dot; sub = dot + 1, dot = strchr(sub, '.')) {
            ...
            if (!obj) break;
            if (!obj->hasExtension(GroupExtension::getExtensionClassTypeId(), false)) return sub;
        }
    }
    return subname;
}
...
{
    if (checkingProperty) return inherited::extensionGetPropertyByName(name);
    Base::StateLocker guard(checkingProperty);
    if (isExcludedProperties(name)) return nullptr;
    auto owner = getContainer();
    if (owner) {
        ...
        if (owner->canLinkProperties()) {
            ...
            if (linked) return linked->getPropertyByName(name);
        }
    }
    return nullptr;
}
...
bool Link::canLinkProperties() const { return true; }
...
{
    if (!_LinkOwner.getValue()) return true;

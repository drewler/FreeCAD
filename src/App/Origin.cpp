...
App::OriginFeature *Origin::getOriginFeature(const char *role) const
{
    ...
    if (featIt != features.end()) { return static_cast<App::OriginFeature *>(*featIt); }
    else {
...
App::Line *Origin::getAxis(const char *role) const
{
    ...
    if (feat->isDerivedFrom(App::Line::getClassTypeId())) { return static_cast<App::Line *>(feat); }
    else {
        std::stringstream err;
...
{
    if (OriginFeatures.isTouched()) { return 1; }
    else {
...
void Origin::setupObject()
{
    const static struct {
        ...
        {App::Line::getClassTypeId(), "Y_Axis",
         Base::Rotation(Base::Vector3d(1, 1, 1), M_PI * 2 / 3)},
        {App::Line::getClassTypeId(), "Z_Axis",
         Base::Rotation(Base::Vector3d(1, 1, 1), M_PI * 4 / 3)},
        ...
        {App::Plane::getClassTypeId(), "YZ_Plane",
         Base::Rotation(Base::Vector3d(1, 1, 1), M_PI * 2 / 3)},
    };
...
void Origin::unsetupObject()
{
    ...
    for (auto obj : objs) {
        ...
        if (std::find(objsLnk.begin(), objsLnk.end(), obj) != objsLnk.end()) {
            if (!obj->isRemoving()) { obj->getDocument()->removeObject(obj->getNameInDocument()); }
        }
    }
}
...
Origin::OriginExtension::OriginExtension(Origin *obj) : obj(obj)
{
...
bool Origin::OriginExtension::extensionGetSubObject(DocumentObject *&ret, const char *subname,
                                                    PyObject **pyobj, Base::Matrix4D *mat, bool,
                                                    int depth) const
{
    if (!subname || subname[0] == '\0') { return false; }

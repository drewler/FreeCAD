...
Property::Property() : father(nullptr), myName(nullptr), _id(++_PropID) {}

Property::~Property() = default;

const char *Property::getName() const { return myName ? myName : ""; }

bool Property::hasName() const { return isValidName(myName); }

bool Property::isValidName(const char *name) { return name && name[0] != '\0'; }

std::string Property::getFullName() const
{
    std::string name;
    if (myName) {
        if (father) name = father->getFullName() + ".";
        else
            name = "?.";
        name += myName;
    }
...
const char *Property::getGroup() const { return father->getPropertyGroup(this); }

const char *Property::getDocumentation() const { return father->getPropertyDocumentation(this); }

void Property::setContainer(PropertyContainer *Father) { father = Father; }
...
ObjectIdentifier Property::canonicalPath(const ObjectIdentifier &p) const { return p; }
...
struct PropertyCleaner {
    explicit PropertyCleaner(Property *p) : prop(p) { ++_PropCleanerCounter; }
    ~PropertyCleaner()
    {
        if (--_PropCleanerCounter) return;
        bool found = false;
        ...
        if (found) _RemovedProps.push_back(prop);
    }
    static void add(Property *prop) { _RemovedProps.push_back(prop); }
...
void Property::touch()
{
    ...
    if (father) father->onChanged(this);
    StatusBits.set(Touched);
}

void Property::setReadOnly(bool readOnly) { this->setStatus(App::Property::ReadOnly, readOnly); }
...
{
    if (father) father->onBeforeChange(this);
}

void Property::verifyPath(const ObjectIdentifier &p) const { p.verify(*this); }
...
{
    static const unsigned long mask = (1 << PropDynamic) | (1 << PropNoRecompute)
        | (1 << PropReadOnly) | (1 << PropTransient) | (1 << PropOutput) | (1 << PropHidden)
        | (1 << PropNoPersist) | (1 << Busy);
...
bool Property::isSame(const Property &other) const
{
    ...
    if (other.getTypeId() != getTypeId() || getMemSize() != other.getMemSize()) return false;
...
void PropertyListsBase::_setPyObject(PyObject *value)
{
    ...
    if (PyDict_Check(value)) {
        ...
        for (auto it = dict.begin(); it != dict.end(); ++it) {
            ...
            if (!PyLong_Check(key)) throw Base::TypeError("expect key type to be integer");
            long idx = PyLong_AsLong(key);
            if (idx < -1 || idx > listSize) throw Base::ValueError("index out of bound");
        ...
        if (PySequence_Check(value)) pySeq = value;
        else {
        ...
        if (!pySeq.isNone()) {
            ...
            for (auto it = seq.begin(); it != seq.end(); ++it) vals.push_back((*it).ptr());
        }
    }

...
public:
    ...
    bool getLinkedObject(App::DocumentObject *&ret, bool recurse, Base::Matrix4D *mat,
    ...
    ValueT redirectSubName(std::ostringstream &ss, App::DocumentObject *topParent,
...
public:
    ...
    virtual ~FeaturePythonT() { delete imp; }

    /** @name methods override DocumentObject */
    //@{
    short mustExecute() const override
    {
        if (this->isTouched()) return 1;
    ...
    DocumentObjectExecReturn *execute() override
    {
        try {
            ...
            if (!handled) return FeatureT::execute();
        }
    ...
    const char *getViewProviderNameOverride() const override
    {
        ...
        if (!viewProviderName.empty()) return viewProviderName.c_str();
                                      ...
                                      bool transform, int depth) const override
    {
        ...
        if (imp->getSubObject(ret, subname, pyObj, mat, transform, depth)) return ret;
    ...
    std::vector<std::string> getSubObjects(int reason = 0) const override
    {
        ...
        if (imp->getSubObjects(ret, reason)) return ret;
        return FeatureT::getSubObjects(reason);
    }

    App::DocumentObject *getLinkedObject(bool recurse, Base::Matrix4D *mat, bool transform,
                                         int depth) const override
    {
        App::DocumentObject *ret = nullptr;
        if (imp->getLinkedObject(ret, recurse, mat, transform, depth)) return ret;
    ...
    bool hasChildElement() const override
    {
        switch (imp->hasChildElement()) {
            case FeaturePythonImp::Accepted: return true;
            case FeaturePythonImp::Rejected: return false;
            default: return FeatureT::hasChildElement();
        }
    }
    /// Get sub-element visibility
    int isElementVisible(const char *element) const override
    {
        ...
        if (ret == -2) return FeatureT::isElementVisible(element);
        return ret;
    }
    /// Set sub-element visibility
    int setElementVisible(const char *element, bool visible) override
    {
        ...
        if (ret == -2) return FeatureT::setElementVisible(element, visible);
        return ret;
    }
    ...
    bool canLinkProperties() const override
    {
        switch (imp->canLinkProperties()) {
            case FeaturePythonImp::Accepted: return true;
            case FeaturePythonImp::Rejected: return false;
            default: return FeatureT::canLinkProperties();
        }
    }
    ...
    bool allowDuplicateLabel() const override
    {
        switch (imp->allowDuplicateLabel()) {
            case FeaturePythonImp::Accepted: return true;
            case FeaturePythonImp::Rejected: return false;
            default: return FeatureT::allowDuplicateLabel();
        }
    }

    bool redirectSubName(std::ostringstream &ss, App::DocumentObject *topParent,
                         App::DocumentObject *child) const override
    {
        switch (imp->redirectSubName(ss, topParent, child)) {
            case FeaturePythonImp::Accepted: return true;
            case FeaturePythonImp::Rejected: return false;
            default: return FeatureT::redirectSubName(ss, topParent, child);
        }
    }
    ...
    {
        if (!imp->editProperty(propName)) FeatureT::editProperty(propName);
    }
    ...
    {
        if (obj) FeatureT::PythonObject = obj;
        else
...
protected:
    ...
    {
        if (!imp->onBeforeChangeLabel(newLabel)) FeatureT::onBeforeChangeLabel(newLabel);
    }
    void onChanged(const Property *prop) override
    {
        if (prop == &Proxy) imp->init(Proxy.getValue().ptr());
        imp->onChanged(prop);

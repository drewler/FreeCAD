...
AutoTransaction::AutoTransaction(const char *name, bool tmpName)
{
    ...
    if (app._activeTransactionGuard < 0) --app._activeTransactionGuard;
...
{
    auto &app = GetApplication();
    if (!app._activeTransactionGuard) return;
    if ((enable && app._activeTransactionGuard > 0) || (!enable && app._activeTransactionGuard < 0))
        return;
...
int Application::setActiveTransaction(const char *name, bool persist)
{
    ...
    if (_activeTransactionGuard > 0 && getActiveTransaction()) {
        ...
        for (auto &v : DocMap) v.second->_commitTransaction();
...
{
    int tid = 0;
    if (Transaction::getLastID() == _activeTransactionID) tid = _activeTransactionID;
    if (id) *id = tid;
...
void Application::closeActiveTransaction(bool abort, int id)
{
    ...
    if (_TransactionLock) {
        if (_TransactionClosed >= 0) _TransactionLock = abort ? -1 : 1;
    ...
    for (auto &v : DocMap) {
        if (v.second->getTransactionID(true) != id) continue;
        if (abort) v.second->_abortTransaction();
        else
...
TransactionLocker::TransactionLocker(bool lock) : active(lock)
{
    if (lock) ++_TransactionLock;
}
...
bool TransactionLocker::isLocked() { return _TransactionLock > 0; }

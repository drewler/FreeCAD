...
PyObject *DocumentObjectPy::touch(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|s", &propName)) return nullptr;
    if (propName) {
        if (!propName[0]) {
        ...
        if (!prop) throw Py::RuntimeError("Property not found");
        prop->touch();
        Py_Return;
    }
...
Py::List DocumentObjectPy::getState() const
{
    ...
    if (object->testStatus(App::Recompute2)) { list.append(Py::String("Recompute2")); }
    if (object->isRestoring()) {
        uptodate = false;
        list.append(Py::String("Restore"));
    }
    if (object->testStatus(App::Expand)) { list.append(Py::String("Expanded")); }
    if (object->testStatus(App::PartialObject)) { list.append(Py::String("Partial")); }
    if (object->testStatus(App::ObjImporting)) { list.append(Py::String("Importing")); }
...
Py::Object DocumentObjectPy::getViewObject() const
{
    try {
        ...
        if (!internalName) { throw Py::RuntimeError("Object has been removed from document"); }
...
PyObject *DocumentObjectPy::setExpression(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sO|s", &path, &expr, &comment)) return nullptr;
    ...
    if (Py::Object(expr).isNone()) {
        ...
        if (shared_expr && comment) shared_expr->comment = comment;
...
{
    const char *expr;
    if (!PyArg_ParseTuple(args, "s", &expr)) return nullptr;
    ...
    PY_TRY
    {
        ...
        if (shared_expr) return Py::new_reference_to(shared_expr->getPyValue());
        Py_Return;
    }
    PY_CATCH
}
...
PyObject *DocumentObjectPy::recompute(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &recursive)) return nullptr;
...
PyObject *DocumentObjectPy::getSubObject(PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|hO!O!h", kwlist, &obj, &retType,
                                     &Base::MatrixPy::Type, &pyMat, &PyBool_Type, &doTransform,
                                     &depth))
        return nullptr;
    ...
    if (PyUnicode_Check(obj)) { subs.emplace_back(PyUnicode_AsUTF8(obj)); }
    else if (PySequence_Check(obj)) {
        single = false;
        Py::Sequence shapeSeq(obj);
        for (Py::Sequence::iterator it = shapeSeq.begin(); it != shapeSeq.end(); ++it) {
            PyObject *item = (*it).ptr();
            if (PyUnicode_Check(item)) { subs.emplace_back(PyUnicode_AsUTF8(item)); }
            else {
    ...
    PY_TRY
    {
        ...
        for (const auto &sub : subs) {
            ...
            if (pyObj) info.pyObj = Py::asObject(pyObj);
            if (info.sobj) info.obj = Py::asObject(info.sobj->getPyObject());
        }

        if (ret.empty()) Py_Return;

        auto getReturnValue = [retEnum, pyMat](SubInfo &ret) -> Py::Object {
            if (retEnum == ReturnType::PyObject) return ret.pyObj;
                ...
                if (retEnum != ReturnType::DocObject) rret.setItem(2, ret.pyObj);
                return rret;
            }
        };

        if (single) { return Py::new_reference_to(getReturnValue(ret[0])); }

        Py::Tuple tuple(ret.size());
        for (size_t i = 0; i < ret.size(); ++i) { tuple.setItem(i, getReturnValue(ret[i])); }
...
{
    const char *subname;
    if (!PyArg_ParseTuple(args, "s", &subname)) return nullptr;
    Py::List res;
    PY_TRY
    {
...
{
    int reason = 0;
    if (!PyArg_ParseTuple(args, "|i", &reason)) return nullptr;

    PY_TRY
    {
        auto names = getDocumentObjectPtr()->getSubObjects(reason);
        Py::Tuple pyObjs(names.size());
        for (size_t i = 0; i < names.size(); ++i) pyObjs.setItem(i, Py::String(names[i]));
...
PyObject *DocumentObjectPy::getLinkedObject(PyObject *args, PyObject *keywds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "|O!OO!h", kwlist, &PyBool_Type, &recursive,
                                     &pyMat, &PyBool_Type, &transform, &depth))
        return nullptr;
    ...
    PY_TRY
    {
        auto linked = getDocumentObjectPtr()->getLinkedObject(Base::asBoolean(recursive), mat,
                                                              Base::asBoolean(transform), depth);
        if (!linked) linked = getDocumentObjectPtr();
...
PyObject *DocumentObjectPy::isElementVisible(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s", &element)) return nullptr;
    PY_TRY { return Py_BuildValue("h", getDocumentObjectPtr()->isElementVisible(element)); }
    PY_CATCH;
}
...
PyObject *DocumentObjectPy::setElementVisible(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|O!", &element, &PyBool_Type, &visible)) return nullptr;
    PY_TRY
    {
        return Py_BuildValue(
            "h", getDocumentObjectPtr()->setElementVisible(element, Base::asBoolean(visible)));
    }
    PY_CATCH;
}
...
{
    PyObject *o;
    if (!PyArg_ParseTuple(args, "O!", &DocumentObjectPy::Type, &o)) return nullptr;
    ...
    try {
        ...
        for (const auto &it : array) {
            Py::List path;
            for (auto jt : it) { path.append(Py::asObject(jt->getPyObject())); }
            list.append(path);
        }
...
PyObject *DocumentObjectPy::getCustomAttributes(const char *) const { return nullptr; }
//remove
int DocumentObjectPy::setCustomAttributes(const char *, PyObject *) { return 0; }

Py::Int DocumentObjectPy::getID() const { return Py::Int(getDocumentObjectPtr()->getID()); }
...
PyObject *DocumentObjectPy::resolveSubElement(PyObject *args)
{
    ...
    int type = 0;
    if (!PyArg_ParseTuple(args, "s|O!i", &subname, &PyBool_Type, &append, &type)) return nullptr;

    PY_TRY
    {
        std::pair<std::string, std::string> elementName;
        auto obj =
            GeoFeature::resolveElement(getDocumentObjectPtr(), subname, elementName,
...
PyObject *DocumentObjectPy::adjustRelativeLinks(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        return Py::new_reference_to(Py::Boolean(getDocumentObjectPtr()->adjustRelativeLinks(
            inList, Base::asBoolean(recursive) ? &visited : nullptr)));
    }
    PY_CATCH
}

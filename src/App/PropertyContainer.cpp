...
PropertyContainer::PropertyContainer() { propertyData.parentPropertyData = nullptr; }
...
unsigned int PropertyContainer::getMemSize() const
{
    ...
    unsigned int size = 0;
    for (It = Map.begin(); It != Map.end(); ++It) size += It->second->getMemSize();
    return size;
}


App::Property *PropertyContainer::addDynamicProperty(const char *type, const char *name,
...
const char *PropertyContainer::getPropertyName(const Property *prop) const
{
    ...
    if (!res) res = getPropertyData().getName(this, prop);
    return res;
}
...
void PropertyContainer::handleChangedPropertyName(Base::XMLReader &reader, const char *TypeName,
                                                  const char *PropName)
{
    (void)reader;
    (void)TypeName;
...
void PropertyContainer::handleChangedPropertyType(XMLReader &reader, const char *TypeName,
                                                  Property *prop)
{
    (void)reader;
    (void)TypeName;
...
void PropertyContainer::Save(Base::Writer &writer) const
{
    ...
    for (auto it = Map.begin(); it != Map.end(); ++it) {
        ...
        if (status) writer.Stream() << "\" status=\"" << status;
        writer.Stream() << "\">";
...
void PropertyContainer::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < transientCount; ++i) {
        ...
        if (prop) FC_TRACE("restore transient '" << prop->getName() << "'");
...
void PropertyData::addProperty(OffsetBase offsetBase, const char *PropName, Property *Prop,
                               const char *PropertyGroup, PropertyType Type,
                               const char *PropertyDocu)
{
...
#endif
    {
        ...
        if (offset < 0) throw Base::RuntimeError("Invalid static property");
        auto &index = propertyData.get<1>();
        auto it = index.find(PropName);
        if (it == index.end()) {
            if (parentMerged) throw Base::RuntimeError("Cannot add static property");
            index.emplace(PropName, PropertyGroup, PropertyDocu, offset, Type);
        }
        else {
#ifdef FC_DEBUG
            if (it->Offset != offset) { FC_ERR("Duplicate property '" << PropName << "'"); }
#endif
        }
    }
...
{
    if (!other) other = const_cast<PropertyData *>(parentPropertyData);
    ...
    if (other) {
        ...
        for (const auto &spec : other->propertyData.get<0>()) index.push_back(spec);
    }
}
...
void PropertyData::split(PropertyData *other)
{
    ...
    if (other) {
        ...
        for (const auto &spec : other->propertyData.get<0>()) index.erase(spec.Offset);
    }
}
...
Property *PropertyData::getPropertyByName(OffsetBase offsetBase, const char *name) const
{
    ...
    if (Spec) return (Property *)(Spec->Offset + offsetBase.getOffset());
    else
        return nullptr;
}
...
void PropertyData::getPropertyNamedList(
    OffsetBase offsetBase, std::vector<std::pair<const char *, Property *>> &List) const
{
    merge();
    size_t base = List.size();
    List.reserve(base + propertyData.size());

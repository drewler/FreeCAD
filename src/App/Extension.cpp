...
void Extension::initExtensionType(Base::Type type)
{
    ...
    if (m_extensionType.isBad()) throw Base::RuntimeError("Extension: Extension type not set");
}
...
void Extension::initExtension(ExtensionContainer *obj)
{
    ...
    for (Property *prop : list) prop->setContainer(obj);
...
std::string Extension::name() const
{
    ...
    if (pos != std::string::npos) return temp.substr(pos + 1);
    else
        return std::string();
}
...
void Extension::initExtensionSubclass(Base::Type &toInit, const char *ClassName,

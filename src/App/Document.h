...
public:
    enum Status
    {
        ...
        RestoreError = 10,
        LinkStampChanged =
            11, // Indicates during restore time if any linked document's time stamp has changed
    ...
    boost::signals2::signal<void(const App::DocumentObject &, Transaction *)>
    ...
    boost::signals2::signal<void(const std::vector<App::DocumentObject *> &, Base::Writer &)>
        signalExportObjects;
    boost::signals2::signal<void(const std::vector<App::DocumentObject *> &, Base::Writer &)>
        signalExportViewObjects;
    boost::signals2::signal<void(const std::vector<App::DocumentObject *> &, Base::XMLReader &)>
        signalImportObjects;
    ...
    boost::signals2::signal<void(const App::Document &, const std::vector<App::DocumentObject *> &)>
        signalRecomputed;
    ...
    void restore(const char *filename = nullptr, bool delaySignal = false,
    ...
    std::vector<DocumentObject *> findObjects(const Base::Type &typeId, const char *objname,
    ...
    int recompute(const std::vector<App::DocumentObject *> &objs = {}, bool force = false,
                  bool *hasError = nullptr, int options = 0);
     ...
     */
    void getLinksTo(std::set<DocumentObject *> &links, const DocumentObject *obj, int options,
                    int maxCount = 0, const std::vector<DocumentObject *> &objs = {}) const;
    ...
    void renameObjectIdentifiers(
        ...
        const std::function<bool(const App::DocumentObject *)> &selector =
            [](const App::DocumentObject *) { return true; });
...
template<typename T> inline std::vector<T *> Document::getObjectsOfType() const
{
    std::vector<T *> type;

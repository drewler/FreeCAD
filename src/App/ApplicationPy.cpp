...
PyMethodDef Application::Methods[] = {
    ...
    {"open",
     ...
     METH_VARARGS | METH_KEYWORDS, "See openDocument(string)"},
    {"openDocument",
     reinterpret_cast<PyCFunction>(reinterpret_cast<void (*)()>(Application::sOpenDocument)),
...
PyObject *Application::sLoadFile(PyObject * /*self*/, PyObject *args)
{
    ...
    char *doc = "";
    char *mod = "";
    if (!PyArg_ParseTuple(args, "s|ss", &path, &doc, &mod)) return nullptr;
    try {
...
PyObject *Application::sOpenDocument(PyObject * /*self*/, PyObject *args, PyObject *kwd)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwd, "et|O!", kwlist, "utf-8", &Name, &PyBool_Type,
                                     &hidden))
        return nullptr;
...
PyObject *Application::sNewDocument(PyObject * /*self*/, PyObject *args, PyObject *kwd)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwd, "|etetO!O!", kwlist, "utf-8", &docName, "utf-8",
                                     &usrName, &PyBool_Type, &hidden, &PyBool_Type, &temp))
        return nullptr;

    PY_TRY
    {
        App::Document *doc = GetApplication().newDocument(
            docName, usrName, !Base::asBoolean(hidden), Base::asBoolean(temp));
        PyMem_Free(docName);
        PyMem_Free(usrName);
...
{
    char *pDoc;
    if (!PyArg_ParseTuple(args, "s", &pDoc)) return nullptr;
...
PyObject *Application::sGetParam(PyObject * /*self*/, PyObject *args)
{
    ...
    PY_TRY { return GetPyObject(GetApplication().GetParameterGroupByPath(pstr)); }
    PY_CATCH;
}
...
PyObject *Application::sGetConfig(PyObject * /*self*/, PyObject *args)
{
    ...
    if (it != Map.end()) { return Py_BuildValue("s", it->second.c_str()); }
    else {
...
PyObject *Application::sSetConfig(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss", &pstr, &pstr2)) return nullptr;
...
PyObject *Application::sGetVersion(PyObject * /*self*/, PyObject *args)
{
    ...
    if (it != cfg.end()) list.append(Py::String(it->second));
...
PyObject *Application::sAddImportType(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss", &psKey, &psMod)) return nullptr;
...
PyObject *Application::sChangeImportModule(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sss", &key, &oldMod, &newMod)) return nullptr;
...
PyObject *Application::sGetImportType(PyObject * /*self*/, PyObject *args)
{
    ...
    if (psKey) {
        ...
        for (std::vector<std::string>::iterator it = types.begin(); it != types.end(); ++it) {
            ...
            if (modules.empty()) { dict.setItem(it->c_str(), Py::None()); }
            else if (modules.size() == 1) {
                dict.setItem(it->c_str(), Py::String(modules.front()));
            }
            else {
                Py::List list;
                for (std::vector<std::string>::iterator jt = modules.begin(); jt != modules.end();
                     ++jt) {
...
PyObject *Application::sGetUserMacroPath(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &actual)) return nullptr;
...
PyObject *Application::sListDocuments(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &sort)) return nullptr;
    PY_TRY
    {
        ...
        ;
        if (Base::asBoolean(sort)) docs = Document::getDependentDocuments(docs, true);
...
PyObject *Application::sSetLogLevel(PyObject * /*self*/, PyObject *args)
{
    char *tag;
    PyObject *pcObj;
    if (!PyArg_ParseTuple(args, "sO", &tag, &pcObj)) return nullptr;
    PY_TRY
    {
        int l;
        if (PyUnicode_Check(pcObj)) {
            const char *pstr = PyUnicode_AsUTF8(pcObj);
            if (strcmp(pstr, "Log") == 0) l = FC_LOGLEVEL_LOG;
...
{
    char *tag;
    if (!PyArg_ParseTuple(args, "s", &tag)) return nullptr;
...
{
    short depth = 0;
    if (!PyArg_ParseTuple(args, "h", &depth)) return nullptr;

    PY_TRY { return Py::new_reference_to(Py::Int(GetApplication().checkLinkDepth(depth, false))); }
    PY_CATCH;
}
...
PyObject *Application::sGetLinksTo(PyObject * /*self*/, PyObject *args)
{
    ...
    int options = 0;
    short count = 0;
    if (!PyArg_ParseTuple(args, "|Oih", &pyobj, &options, &count)) return nullptr;
    ...
    PY_TRY
    {
        ...
        int i = 0;
        for (auto o : links) ret.setItem(i++, Py::Object(o->getPyObject(), true));
...
PyObject *Application::sGetDependentObjects(PyObject * /*self*/, PyObject *args)
{
    PyObject *obj;
    int options = 0;
    if (!PyArg_ParseTuple(args, "O|i", &obj, &options)) return nullptr;
...
PyObject *Application::sSetActiveTransaction(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|O!", &name, &PyBool_Type, &persist)) return nullptr;
...
PyObject *Application::sCloseActiveTransaction(PyObject * /*self*/, PyObject *args)
{
    ...
    int id = 0;
    if (!PyArg_ParseTuple(args, "|O!i", &PyBool_Type, &abort, &id)) return nullptr;

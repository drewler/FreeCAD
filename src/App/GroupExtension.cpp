...
GroupExtension::GroupExtension()
{
    ...
    EXTENSION_ADD_PROPERTY_TYPE(Group, (nullptr), "Base", (App::PropertyType)(Prop_None),
...
std::vector<DocumentObject *> GroupExtension::addObjects(std::vector<DocumentObject *> objs)
{
    ...
    for (auto obj : objs) {
        ...
        if (!allowObject(obj)) continue;

        if (hasObject(obj)) continue;
        ...
        if (geogrp != objgrp) {
            ...
            if (geogrp) geogrp->getExtensionByType<GeoFeatureGroupExtension>()->addObject(obj);
            else
...
std::vector<DocumentObject *> GroupExtension::removeObjects(std::vector<DocumentObject *> objs)
{
    ...
    if (grp.size() != newGrp.size()) { Group.setValues(newGrp); }
...
DocumentObject *GroupExtension::getObject(const char *Name) const
{
    ...
    if (obj && hasObject(obj)) return obj;
    return nullptr;
}
...
bool GroupExtension::hasObject(const DocumentObject *obj, bool recursive) const
{
    ...
    if (obj == getExtendedObject()) return false;
    ...
    for (auto child : grp) {
        ...
        if (child == obj) { return true; }
            ...
            if (subGroup->recursiveHasObject(obj, subGroup, history)) { return true; }
        }
    }
                                        ...
                                        std::vector<const GroupExtension *> history) const
{
    ...
    if (group->hasObject(obj, false)) return true;
    ...
    for (auto child : group->Group.getValues()) {
        ...
        if (child->hasExtension(GroupExtension::getExtensionClassTypeId())) {
            ...
            if (recursiveHasObject(obj, ext, history)) { return true; }
        }
    }
    return false;
}
...
std::vector<DocumentObject *> GroupExtension::getObjectsOfType(const Base::Type &typeId) const
{
    ...
    for (std::vector<DocumentObject *>::const_iterator it = grp.begin(); it != grp.end(); ++it) {
        if ((*it)->getTypeId().isDerivedFrom(typeId)) type.push_back(*it);
    }
...
int GroupExtension::countObjectsOfType(const Base::Type &typeId) const
{
    ...
    const std::vector<DocumentObject *> &grp = Group.getValues();
    for (std::vector<DocumentObject *>::const_iterator it = grp.begin(); it != grp.end(); ++it) {
        if ((*it)->getTypeId().isDerivedFrom(typeId)) type++;
    }
...
DocumentObject *GroupExtension::getGroupOfObject(const DocumentObject *obj)
{
    ...
    for (auto o : obj->getInList()) {
        if (o->hasExtension(App::GroupExtension::getExtensionClassTypeId(), false)) return o;
        if (o->hasExtension(App::GroupExtensionPython::getExtensionClassTypeId(), false)) return o;
    }
        ...
        for (auto obj : Group.getValue()) {
            if (obj && obj->getNameInDocument()) {
                _Conns[obj] = obj->signalChanged.connect(
                    boost::bind(&GroupExtension::slotChildChanged, this, bp::_1, bp::_2));
            }
        }
    }
                                           ...
                                           bool /*transform*/, int depth) const
{
    ...
    dot = strchr(subname, '.');
    if (!dot) return false;
    if (subname[0] != '$') ret = Group.find(std::string(subname, dot));
    else {
                                    ...
                                    std::set<App::DocumentObject *> &rset) const
{
    for (auto obj : Group.getValues()) {
        ...
        if (!rset.insert(obj).second) continue;
        res.push_back(obj);
        auto ext = obj->getExtensionByType<GroupExtension>(true, false);
        if (ext) ext->getAllChildren(res, rset);
    }
}

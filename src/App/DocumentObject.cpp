...
App::DocumentObjectExecReturn *DocumentObject::recompute()
{
    ...
    if (ret == StdReturn) {
        ...
        if (this->testStatus(App::RecomputeExtension)) { ret = executeExtensions(); }
    }

    return ret;
}

DocumentObjectExecReturn *DocumentObject::execute() { return executeExtensions(); }
...
bool DocumentObject::recomputeFeature(bool recursive)
{
    ...
    if (doc) return doc->recomputeFeature(this, recursive);
    return isValid();
}
...
{
    if (!noRecompute) StatusBits.set(ObjectStatus::Enforce);
    StatusBits.set(ObjectStatus::Touch);
    if (_pDoc) _pDoc->signalTouchedObject(*this);
}
 ...
 */
void DocumentObject::enforceRecompute() { touch(false); }
...
{
    if (StatusBits.test(ObjectStatus::Enforce)) return true;
...
{
    if (ExpressionEngine.isTouched()) return 1;

    //ask all extensions
    auto vector = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : vector) {
        if (ext->extensionMustExecute()) return 1;
    }
...
{
    if (!getDocument() || !pcNameInDocument) return "?";
...
{
    if (!getDocument() || !getNameInDocument()) return 0;
    return getDocument()->isExporting(this);
}

std::string DocumentObject::getExportName(bool forced) const
{
    if (!pcNameInDocument) return std::string();

    if (!forced && !isExporting()) return *pcNameInDocument;
...
bool DocumentObject::isAttachedToDocument() const { return (pcNameInDocument != nullptr); }
...
void DocumentObject::getOutList(int options, std::vector<DocumentObject *> &res) const
{
    ...
    for (auto prop : props) {
        ...
        if (link) link->getLinks(res, noHidden);
    }
    if (!(options & OutListNoExpression)) ExpressionEngine.getLinks(res);

    if (options & OutListNoXLinked) {
        for (auto it = res.begin() + size; it != res.end();) {
            auto obj = *it;
            if (obj && obj->getDocument() != getDocument()) it = res.erase(it);
            else
                ++it;
        }
    }
...
std::vector<App::DocumentObject *> DocumentObject::getOutListOfProperty(App::Property *prop) const
{
    ...
    if (!prop || prop->getContainer() != this) return ret;

    auto link = dynamic_cast<PropertyLinkBase *>(prop);
    if (link) link->getLinks(ret);
    return ret;
}
...
{
    if (_pDoc) return _pDoc->getInList(this);
    else
...
const std::vector<App::DocumentObject *> &DocumentObject::getInList() const { return _inList; }
...
void DocumentObject::getInListEx(std::set<App::DocumentObject *> &inSet, bool recursive,
                                 std::vector<App::DocumentObject *> *inList) const
{
    ...
    for (auto doc : GetApplication().getDocuments()) {
        for (auto obj : doc->getObjects()) {
            if (!obj || !obj->getNameInDocument() || obj == this) continue;
...
void _getOutListRecursive(std::set<DocumentObject *> &objSet, const DocumentObject *obj,
...
bool _isInInListRecursive(const DocumentObject *act, const DocumentObject *checkObj, int depth)
{
    ...
    for (auto obj : act->getInList()) {
        ...
        if (_isInInListRecursive(obj, checkObj, depth - 1)) return true;
    }
#else
    (void)act;
    (void)checkObj;
...
{
#ifndef USE_OLD_DAG
    if (std::find(_inList.begin(), _inList.end(), linkTo) != _inList.end()) return true;
    else
        return false;
#else
    (void)linkTo;
...
bool _isInOutListRecursive(const DocumentObject *act, const DocumentObject *checkObj, int depth)
{
    ...
    for (auto obj : act->getOutList()) {
        ...
        if (_isInOutListRecursive(obj, checkObj, depth - 1)) return true;
    }
#else
    (void)act;
    (void)checkObj;
...
bool DocumentObject::testIfLinkDAGCompatible(const std::vector<DocumentObject *> &linksTo) const
{
    ...
    for (auto obj : linksTo)
        if (inLists.count(obj)) return false;
    return true;
}
...
App::Document *DocumentObject::getDocument() const { return _pDoc; }
...
{
    if (!_pDoc || testStatus(ObjectStatus::Destroy)) return false;

    Property *prop = getDynamicPropertyByName(name);
    if (!prop || prop->testStatus(App::Property::LockDynamic)) return false;

    if (prop->isDerivedFrom(PropertyLinkBase::getClassTypeId())) clearOutListCache();
    ...
    for (const auto &it : expressions) {
        if (it.first.getProperty() == prop) { removeExpr.push_back(it.first); }
    }
...
App::Property *DocumentObject::addDynamicProperty(const char *type, const char *name,
                                                  const char *group, const char *doc, short attr,
                                                  bool ro, bool hidden)
{
    auto prop = TransactionalObject::addDynamicProperty(type, name, group, doc, attr, ro, hidden);
    if (prop && _pDoc) _pDoc->addOrRemovePropertyOfObject(this, prop, true);
    return prop;
}
...
void DocumentObject::onBeforeChange(const Property *prop)
{
    ...
    if (prop == &Label) oldLabel = Label.getStrValue();

    if (_pDoc) onBeforeChangeProperty(_pDoc, prop);
...
{
    if (GetApplication().isClosingAll()) return;
    ...
    if (!testStatus(ObjectStatus::NoTouch) && !(prop->getType() & Prop_Output)
        ...
        if (!(prop->getType() & Prop_NoRecompute)) StatusBits.set(ObjectStatus::Enforce);
    }
    ...
    if (_pDoc) _pDoc->onChangedProperty(this, prop);
...
DocumentObject *DocumentObject::getSubObject(const char *subname, PyObject **pyObj,
                                             Base::Matrix4D *mat, bool transform, int depth) const
{
    DocumentObject *ret = nullptr;
    auto exts = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : exts) {
        if (ext->extensionGetSubObject(ret, subname, pyObj, mat, transform, depth)) return ret;
    }

    std::string name;
    const char *dot = nullptr;
    if (!subname || !(dot = strchr(subname, '.'))) { ret = const_cast<DocumentObject *>(this); }
    else if (subname[0] == '$') {
        ...
        if (outList.size() != _outListMap.size()) {
            _outListMap.clear();
            for (auto obj : outList) _outListMap[obj->getNameInDocument()] = obj;
        }
        auto it = _outListMap.find(name.c_str());
        if (it != _outListMap.end()) ret = it->second;
    }
    ...
    if (transform && mat) {
        ...
        if (pla) *mat *= pla->getValue().toMatrix();
    }

    if (ret && dot) return ret->getSubObject(dot + 1, pyObj, mat, true, depth + 1);
    return ret;
}
...
std::vector<DocumentObject *> DocumentObject::getSubObjectList(const char *subname) const
{
    ...
    for (auto pos = sub.find('.'); pos != std::string::npos; pos = sub.find('.', pos + 1)) {
        ...
        if (!sobj || !sobj->getNameInDocument()) break;
        res.push_back(sobj);
        sub[pos + 1] = c;
    }
    return res;
...
std::vector<std::string> DocumentObject::getSubObjects(int reason) const
{
    ...
    auto exts = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : exts) {
        if (ext->extensionGetSubObjects(ret, reason)) return ret;
    }
    return ret;
}
...
DocumentObject::getParents(int depth) const
{
    ...
    if (!getNameInDocument() || !GetApplication().checkLinkDepth(depth)) return ret;
    std::string name(getNameInDocument());
    name += ".";
    for (auto parent : getInList()) {
        if (!parent || !parent->getNameInDocument()) continue;
        if (!parent->hasChildElement()
            && !parent->hasExtension(GeoFeatureGroupExtension::getExtensionClassTypeId()))
            continue;
        if (!parent->getSubObject(name.c_str())) continue;
        ...
        for (auto parent : links) {
            ...
            if (parents.empty()) parents.emplace_back(parent, std::string());
            for (auto &v : parents) ret.emplace_back(v.first, v.second + name);
        }
    }
    return ret;
}

DocumentObject *DocumentObject::getLinkedObject(bool recursive, Base::Matrix4D *mat, bool transform,
                                                int depth) const
{
    DocumentObject *ret = nullptr;
    auto exts = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : exts) {
        if (ext->extensionGetLinkedObject(ret, recursive, mat, transform, depth)) return ret;
    }
    if (transform && mat) {
...
{
    if (this->getNameInDocument()) writer.ObjectName = this->getNameInDocument();
...
void DocumentObject::onDocumentRestored()
{
    //call all extensions
    auto vector = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : vector) ext->onExtendedDocumentRestored();
    if (Visibility.testStatus(Property::Output)) Visibility.setStatus(Property::NoModify, true);
}
...
void DocumentObject::onSettingDocument()
{
    //call all extensions
    auto vector = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : vector) ext->onExtendedSettingDocument();
}
...
void DocumentObject::setupObject()
{
    //call all extensions
    auto vector = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : vector) ext->onExtendedSetupObject();
}
...
void DocumentObject::unsetupObject()
{
    //call all extensions
    auto vector = getExtensionsDerivedFromType<App::DocumentObjectExtension>();
    for (auto ext : vector) ext->onExtendedUnsetupObject();
}
...
void App::DocumentObject::_removeBackLink(DocumentObject *rmvObj)
{
    ...
    if (it != _inList.end()) _inList.erase(it);
#else
    (void)rmvObj;
#endif
}
...
{
    for (auto ext : getExtensionsDerivedFromType<DocumentObjectExtension>()) {
        if (ext->extensionHasChildElement()) return true;
    }
    return false;
}

DocumentObject *DocumentObject::resolve(const char *subname, App::DocumentObject **parent,
                                        std::string *childName, const char **subElement,
                                        PyObject **pyObj, Base::Matrix4D *pmat, bool transform,
                                        int depth) const
{
    ...
    if (!obj || !subname || *subname == 0) return self;

    if (!parent && !subElement) return obj;
    ...
    // object name.
    const char *dot = nullptr;
    if (Data::ComplexGeoData::isMappedElement(subname) || !(dot = strrchr(subname, '.'))
        || dot == subname) {
        if (subElement) *subElement = dot ? dot + 1 : subname;
    ...
    if (subElement) *subElement = *lastDot == '.' ? lastDot + 1 : lastDot;
    return obj;
}

DocumentObject *DocumentObject::resolveRelativeLink(std::string &subname, DocumentObject *&link,
                                                    std::string &linkSub) const
{
    if (!link || !link->getNameInDocument() || !getNameInDocument()) return nullptr;
                                         ...
                                         std::set<App::DocumentObject *> *visited)
{
    ...
    for (auto prop : props) {
        ...
        if (linkProp && linkProp->adjustLink(inList)) touched = true;
    }
    if (visited) {
        for (auto obj : getOutList()) {
            if (!visited->count(obj)) {
                if (obj->adjustRelativeLinks(inList, visited)) touched = true;
            }
        }
    }
    return touched;

...
inline Color ColorField::getColor(float fVal) const
{
    ...
    if (fVal <= fMin) return colorModel.colors[0];
    else if (fVal >= fMax)
...
public:
    ColorGradient();
    ColorGradient(float fMin, float fMax, std::size_t usCtColors, ColorBarStyle tS,
    ...
    const ColorGradientProfile &getProfile() const { return profile; }
    ...
    bool isOutOfRange(float fVal) const { return ((fVal < profile.fMin) || (fVal > profile.fMax)); }
    std::size_t getCountColors() const { return profile.ctColors; }
    ...
    ColorBarStyle getStyle() const { return profile.tStyle; }
    void setOutsideGrayed(bool value) { profile.visibility.setFlag(Visibility::Grayed, value); }
    bool isOutsideGrayed() const { return profile.visibility.testFlag(Visibility::Grayed); }
    void setOutsideInvisible(bool value)
    {
        profile.visibility.setFlag(Visibility::Invisible, value);
    }
    bool isOutsideInvisible() const { return profile.visibility.testFlag(Visibility::Invisible); }
    void setColorModel(std::size_t tModel);
    std::size_t getColorModelType() const { return profile.tColorModel; }
    inline const ColorModel &getColorModel() const;
    std::vector<std::string> getColorModelNames() const;
    float getMinValue() const { return profile.fMin; }
    float getMaxValue() const { return profile.fMax; }
...
inline Color ColorLegend::getColor(float fVal) const
{
    ...
    if (outsideGrayed) {
        if ((pI == values.begin()) || (pI == values.end())) return Color(0.5f, 0.5f, 0.5f);
        else
            return colorFields[pI - values.begin() - 1];
    }

    if (pI == values.begin()) return *colorFields.begin();
...
inline float ColorLegend::getMinValue() const { return values.front(); }

inline float ColorLegend::getMaxValue() const { return values.back(); }

inline Color ColorGradient::getColor(float fVal) const
{
    Color color = _getColor(fVal);
    if (isOutsideInvisible()) {
        if (isOutOfRange(fVal)) color.a = 0.8f;
    }
...
{
    if (isOutsideGrayed()) {
        if (isOutOfRange(fVal)) return Color(0.5f, 0.5f, 0.5f);
    }

    switch (profile.tStyle) {
        case ColorBarStyle::ZERO_BASED: {
            if ((profile.fMin < 0.0f) && (profile.fMax > 0.0f)) {
                if (fVal < 0.0f) return colorField1.getColor(fVal);
                else
...
inline std::size_t ColorGradient::getColorIndex(float fVal) const
{
    switch (profile.tStyle) {
        case ColorBarStyle::ZERO_BASED: {
            if ((profile.fMin < 0.0f) && (profile.fMax > 0.0f)) {
                if (fVal < 0.0f) return colorField1.getColorIndex(fVal);
                else
...
inline const ColorModel &ColorGradient::getColorModel() const
{
    if (profile.tStyle == ColorBarStyle::ZERO_BASED) {
        if (profile.fMax <= 0.0f) return currentModelPack.bottomModel;

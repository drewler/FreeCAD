...
DocumentT::DocumentT(Document *doc) { document = doc->getName(); }
...
void DocumentT::operator=(const Document *doc) { document = doc->getName(); }

void DocumentT::operator=(const std::string &name) { document = name; }

Document *DocumentT::getDocument() const { return GetApplication().getDocument(document.c_str()); }

const std::string &DocumentT::getDocumentName() const { return document; }
...
DocumentObjectT::DocumentObjectT(const DocumentObjectT &other) { *this = other; }

DocumentObjectT::DocumentObjectT(DocumentObjectT &&other) { *this = std::move(other); }

DocumentObjectT::DocumentObjectT(const DocumentObject *obj) { *this = obj; }

DocumentObjectT::DocumentObjectT(const Property *prop) { *this = prop; }

DocumentObjectT::DocumentObjectT(const Document *doc, const std::string &objName)
{
    if (doc && doc->getName()) document = doc->getName();
    object = objName;
}

DocumentObjectT::DocumentObjectT(const char *docName, const char *objName)
{
    if (docName) document = docName;
...
const std::string &DocumentObjectT::getDocumentName() const { return document; }
...
DocumentObject *DocumentObjectT::getObject() const
{
    ...
    if (doc) { obj = doc->getObject(object.c_str()); }
    return obj;
}

const std::string &DocumentObjectT::getObjectName() const { return object; }

const std::string &DocumentObjectT::getObjectLabel() const { return label; }
...
const std::string &DocumentObjectT::getPropertyName() const { return property; }

std::string DocumentObjectT::getPropertyPython() const
{
    std::stringstream str;
    str << getObjectPython();
    if (!property.empty()) str << '.' << property;
    return str.str();
}
...
{
    auto obj = getObject();
    if (obj) return obj->getPropertyByName(property.c_str());
    return nullptr;
}
...
SubObjectT::SubObjectT(const SubObjectT &other) : DocumentObjectT(other), subname(other.subname) {}
...
SubObjectT::SubObjectT(const DocumentObject *obj) : DocumentObjectT(obj) {}
...
{
    if (getDocumentName() < other.getDocumentName()) return true;
    if (getDocumentName() > other.getDocumentName()) return false;
    if (getObjectName() < other.getObjectName()) return true;
    if (getObjectName() > other.getObjectName()) return false;
    if (getSubName() < other.getSubName()) return true;
    if (getSubName() > other.getSubName()) return false;
...
void SubObjectT::setSubName(const char *s) { subname = s ? s : ""; }

const std::string &SubObjectT::getSubName() const { return subname; }
...
std::string SubObjectT::getOldElementName(int *index) const
{
    ...
    if (!obj) return std::string();
    GeoFeature::resolveElement(obj, subname.c_str(), element);
    if (!index) return std::move(element.second);
...
{
    auto obj = getObject();
    if (obj) return obj->getSubObject(subname.c_str());
    return nullptr;
}

std::string SubObjectT::getSubObjectPython(bool force) const
{
    if (!force && subname.empty()) return getObjectPython();
    std::stringstream str;
...
{
    auto obj = getObject();
    if (obj) return obj->getSubObjectList(subname.c_str());
    return {};
}
...
{
    if (subname.empty()) return getObjectFullName(docName);
    std::ostringstream ss;
    if (!docName || getDocumentName() != docName) {
        ss << getDocumentName();
        if (auto doc = getDocument()) {
...
PropertyLinkT::PropertyLinkT() : toPython("None") {}

PropertyLinkT::PropertyLinkT(DocumentObject *obj) : PropertyLinkT()
{
    if (obj) {
...
PropertyLinkT::PropertyLinkT(const std::vector<DocumentObject *> &objs) : PropertyLinkT()
{
    if (!objs.empty()) {
        std::stringstream str;
        str << "[";
...
std::string PropertyLinkT::getPropertyPython() const { return toPython; }
...
DocumentWeakPtrT::DocumentWeakPtrT(App::Document *doc) noexcept : d(new Private(doc)) {}
...
App::Document *DocumentWeakPtrT::operator*() const noexcept { return d->_document; }
...
{
public:
    explicit Private(App::DocumentObject *obj) : object(obj), indocument(false) { set(obj); }
    void deletedDocument(const App::Document &doc)
    {
    ...
    App::DocumentObject *get() const noexcept { return indocument ? object : nullptr; }
...
DocumentObjectWeakPtrT::DocumentObjectWeakPtrT(App::DocumentObject *obj) : d(new Private(obj)) {}

DocumentObjectWeakPtrT::~DocumentObjectWeakPtrT() = default;

App::DocumentObject *DocumentObjectWeakPtrT::_get() const noexcept { return d->get(); }

void DocumentObjectWeakPtrT::reset() { d->reset(); }

bool DocumentObjectWeakPtrT::expired() const noexcept { return !d->indocument; }
...
App::DocumentObject *DocumentObjectWeakPtrT::operator*() const noexcept { return d->get(); }
...
Document *DocumentObserver::getDocument() const { return this->_document; }
...
void DocumentObjectObserver::addToObservation(App::DocumentObject *obj) { _objects.insert(obj); }
...
void DocumentObjectObserver::slotDeletedObject(const App::DocumentObject &Obj)
{
    ...
    if (_objects.empty()) cancelObservation();
}

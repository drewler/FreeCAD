...
template<size_t const __K, typename _Val, typename _Acc = _Bracket_accessor<_Val>,
         typename _Dist =
             squared_difference<typename _Acc::result_type, typename _Acc::result_type>,
...
public:
    ...
    KDTree(_Acc const &__acc = _Acc(), _Dist const &__dist = _Dist(), _Cmp const &__cmp = _Cmp(),
        ...
        : _Base(__a), _M_header(), _M_count(0), _M_acc(__acc), _M_cmp(__cmp), _M_dist(__dist)
    {
        _M_empty_initialise();
    }

    KDTree(const KDTree &__x)
        : _Base(__x.get_allocator()), _M_header(), _M_count(0), _M_acc(__x._M_acc),
          _M_cmp(__x._M_cmp), _M_dist(__x._M_dist)
    {
        _M_empty_initialise();
        // this is slow:
    ...
    KDTree(_InputIterator __first, _InputIterator __last, _Acc const &acc = _Acc(),
           _Dist const &__dist = _Dist(), _Cmp const &__cmp = _Cmp(),
           const allocator_type &__a = allocator_type())
        : _Base(__a), _M_header(), _M_count(0), _M_acc(acc), _M_cmp(__cmp), _M_dist(__dist)
    {
        _M_empty_initialise();
        // this is slow:
    ...
    {
        for (; __n > 0; --__n) this->insert(__pos, __x);
    }

    template<typename _InputIterator>
    void insert(iterator __pos, _InputIterator __first, _InputIterator __last)
    {
        for (; __first != __last; ++__first) this->insert(__pos, *__first);
    }
    ...
    void erase(const_iterator const &__IT)
    {
        ...
        size_type level = 0;
        while ((n = _S_parent(n)) != &_M_header) ++level;
...
*/
    ...
    template<class SearchVal> const_iterator find(SearchVal const &__V) const
    {
    ...
    //
    template<class SearchVal> const_iterator find_exact(SearchVal const &__V) const
    {
        if (!_M_get_root()) return this->end();
    ...
    _OutputIterator find_within_range(SearchVal const &val, subvalue_type const range,
                                                          ...
                                                          distance_type __max) const
    {
        if (_M_get_root()) {
            ...
            std::pair<const _Node<_Val> *, std::pair<size_type, typename _Acc::result_type>> best =
                _S_node_nearest(__K, 0, __val, _M_get_root(), &_M_header, node, __max, _M_cmp,
                                _M_acc, _M_dist, always_true<value_type>());
...
protected:
    void _M_check_children(_Link_const_type child, _Link_const_type parent, size_type const level,
                           bool to_the_left)
    {
        assert(parent);
        if (child) {
    ...
    _Link_type _M_get_erase_replacement(_Link_type node, size_type const level)
    {
        ...
        if (!_S_left(node))
            candidate =
                _M_get_j_min(std::pair<_Link_type, size_type>(_S_right(node), level), level + 1);
        // ditto for the right
        else if ((!_S_right(node)))
            candidate =
                _M_get_j_max(std::pair<_Link_type, size_type>(_S_left(node), level), level + 1);
        // we have both children ...
            ...
            if (compare(_S_right(node)->_M_value, _S_left(node)->_M_value))
                ...
                candidate = _M_get_j_min(std::pair<_Link_type, size_type>(_S_right(node), level),
                                         level + 1);
            else
                candidate =
                    _M_get_j_max(std::pair<_Link_type, size_type>(_S_left(node), level), level + 1);
    ...
    const_iterator _M_find(_Link_const_type node, const_reference value,
    ...
    const_iterator _M_find_exact(_Link_const_type node, const_reference value,
                                 size_type const level) const
    {
        // be aware! This is very different to normal binary searches, because of the <=
        // relationship used. See top for notes.
    ...
    bool _M_matches_node_in_other_ds(_Link_const_type __N, const_reference __V,
    ...
    size_type _M_count_within_range(_Link_const_type __N, _Region_ const &__REGION,
    ...
    template<class Visitor>
    Visitor _M_visit_within_range(Visitor visitor, _Link_const_type N, _Region_ const &REGION,
                                  _Region_ const &BOUNDS, size_type const L) const
    {
        ...
        if (_S_left(N)) {
            ...
            if (REGION.intersects_with(bounds))
                visitor = _M_visit_within_range(visitor, _S_left(N), REGION, bounds, L + 1);
        }
        if (_S_right(N)) {
            _Region_ bounds(BOUNDS);
    ...
    template<typename _OutputIterator>
    _OutputIterator _M_find_within_range(_OutputIterator out, _Link_const_type __N,
                                         _Region_ const &__REGION, _Region_ const &__BOUNDS,
    ...
    _Link_type _M_new_node(const_reference __V, //  = value_type(),
                           _Base_ptr const __PARENT = nullptr, _Base_ptr const __LEFT = nullptr,

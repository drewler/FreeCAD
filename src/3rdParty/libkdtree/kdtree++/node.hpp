...
struct _Node_base {
    ...
    _Node_base(_Base_ptr const __PARENT = nullptr, _Base_ptr const __LEFT = nullptr,
...
template<typename _Val> struct _Node: public _Node_base {
    using _Node_base::_Base_ptr;
    typedef _Node *_Link_type;

    _Val _M_value;

    _Node(_Val const &__VALUE = _Val(), _Base_ptr const __PARENT = nullptr,
          _Base_ptr const __LEFT = nullptr, _Base_ptr const __RIGHT = nullptr)
    ...
    operator<<(typename std::basic_ostream<Char, Traits> &out, _Node_base const &node)
    {
        out << &node;
    ...
    template<typename Char, typename Traits>
    friend std::basic_ostream<Char, Traits> &
    operator<<(typename std::basic_ostream<Char, Traits> &out, _Node<_Val> const &node)
    {
        out << &node;
        out << ' ' << node._M_value;
        out << "; parent: " << node._M_parent;
...
template<typename _Val, typename _Acc, typename _Cmp> class _Node_compare
{
public:
...
inline bool _S_node_compare(const size_t __dim, const _Cmp &__cmp, const _Acc &__acc,
...
inline typename _Dist::distance_type _S_node_distance(const size_t __dim, const _Dist &__dist,
...
_S_accumulate_node_distance(const size_t __dim, const _Dist &__dist, const _Acc &__acc,
                            const _ValA &__a, const _ValB &__b)
{
    typename _Dist::distance_type d = 0;
    for (size_t i = 0; i < __dim; ++i) d += __dist(__acc(__a, i), __acc(__b, i));
    return d;
}
...
inline _Node_base *_S_node_descend(const size_t __dim, const _Cmp &__cmp, const _Acc &__acc,
                                   const _Val &__val, const _Node_base *__node)
{
    if (_S_node_compare(__dim, __cmp, __acc, __val,
...
/*! Find the nearest node to __val from __node
    ...
    given arguments.
   */
template<class SearchVal, typename _Val, typename _Cmp, typename _Acc, typename _Dist,
         typename _Predicate>
...
_S_node_nearest(const size_t __k, size_t __dim, SearchVal const &__val, const _Node<_Val> *__node,
                const _Node_base *__end, const _Node<_Val> *__best,
                typename _Dist::distance_type __max, const _Cmp &__cmp, const _Acc &__acc,
                const _Dist &__dist, _Predicate __p)
{
    ...
    while (cur != __end) {
        ...
        if (cur != __end) {
            ...
            if (near_node
                // only visit node's children if node's plane intersect hypersphere
                && (sqrt(_S_node_distance(cur_dim % __k, __dist, __acc, __val,
                                          static_cast<const _Node<_Val> *>(cur)->_M_value))
                    <= __max)) {
                probe = near_node;
                ++probe_dim;

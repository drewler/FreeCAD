...
typedef KDTree::KDTree<2, duplet, std::pointer_to_binary_function<duplet, int, double>>
    duplet_tree_type;
...
int main()
{
    ...
    for (int i = 0; i < 700; i++) {
        ...
        duplet_tree_type::iterator pItr =
            dupl_tree_test.find_nearest(super_dupre, std::numeric_limits<double>::max()).first;
        if (*pItr != super_dupre) {
    ...
    while (vDuplets.size() > 0) //delete all duplets from tree which are in the vector
    {
        ...
        if (vDuplets.size() == 147) cout << "THIS IS THE BUG TRIGGER" << endl;

        cout << vDuplets.size() << " : Deleting " << element_to_erase << endl;

        assert(find(dupl_tree_test.begin(), dupl_tree_test.end(), element_to_erase)
               != dupl_tree_test.end());
        ...
        dupl_tree_test.erase(
            element_to_erase); //erase() : will probably erase wrong element sooner or later

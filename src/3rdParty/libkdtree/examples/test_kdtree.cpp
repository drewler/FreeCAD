...
struct triplet {
    ...
    {
        double dist = 0;
        for (int i = 0; i != 3; ++i) dist += (d[i] - x.d[i]) * (d[i] - x.d[i]);
...
typedef KDTree::KDTree<3, triplet, std::pointer_to_binary_function<triplet, size_t, double>>
    tree_type;
...
int main()
{
    // check that it'll find nodes exactly MAX away
    {
        ...
        std::cout << "Test find_nearest(), found at exact distance away from " << target
                  << ", found " << *found.first << std::endl;
    }
    ...
    // do the same test, except use alternate_triplet as the search key
    {
        ...
        std::cout << "Test with alternate search type, found: " << *found.first << ", wanted "
                  << actual_target << std::endl;
...

    {
        ...
        assert(found.first != exact_dist.end());
        std::cout << "Test find_nearest(), found at exact distance away from " << target
                  << ", found " << *found.first << " @ " << found.second << " should be "
                  << std::sqrt(8) << std::endl;
    ...
    for (int loop = 0; loop != 4; ++loop) {
...

        {
            ...
            std::cout << "Test find_nearest(), nearest to " << s << " within " << small_dist
                      << " should not be found" << std::endl;
...

        {
            ...
            std::cout << "Test find_nearest_if(), nearest to " << s << " @ " << nif.second << ": "
                      << *nif.first << std::endl;

            std::pair<tree_type::const_iterator, double> cantfind =
                t.find_nearest_if(s, std::numeric_limits<double>::max(), FalsePredicate());
...

        {
            ...
            std::cout << "Nearest to " << s << " @ " << found.second << " " << *found.first
                      << std::endl;
...

        {
            ...
            std::cout << "Nearest to " << s2 << " @ " << found.second << " " << *found.first
                      << std::endl;
    ...
    // the specified range... this is the test.
    {
        ...
        std::cout << std::endl
                  << "Test find_with_range( " << sv << ", 10.0f) found " << vectors.size()
                  << " candidates." << std::endl;
        ...
        for (std::deque<triplet>::iterator v = vectors.begin(); v != vectors.end(); ++v) {
            ...
            if (dist > 10.0f)
                std::cout << "    This point is too far! But that is by design, its within a 'box' "
                             "with a 'radius' of 10, not a sphere with a radius of 10"
                          << std::endl;

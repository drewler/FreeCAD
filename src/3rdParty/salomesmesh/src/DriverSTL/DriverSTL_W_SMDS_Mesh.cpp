...
DriverSTL_W_SMDS_Mesh::DriverSTL_W_SMDS_Mesh() { myIsAscii = false; }

void DriverSTL_W_SMDS_Mesh::SetIsAscii(const bool theIsAscii) { myIsAscii = theIsAscii; }
...
Driver_Mesh::Status DriverSTL_W_SMDS_Mesh::Perform()
{
    ...
    findVolumeTriangles();
    if (myIsAscii) aResult = writeAscii();
    else
        aResult = writeBinary();
...
{
    for (unsigned i = 0; i < myVolumeFacets.size(); ++i) delete myVolumeFacets[i];
}
        ...
        for (int iF = 0; iF < theVolume.NbFaces(); ++iF)
            if (theVolume.IsFreeFace(iF)) {
                ...
                if (!myMesh->FindElement(nodes, SMDSAbs_Face, /*noMedium=*/false)) {
                    if ((nbN == 9 || nbN == 7) && (!theVolume.IsPoly())) // facet is bi-quaratic
                    {
                        int nbTria = nbN - 1;
                        for (int iT = 0; iT < nbTria; ++iT)
                            myVolumeFacets.push_back(
                                new SMDS_FaceOfNodes(n[8], n[0 + iT], n[1 + iT]));
...
SMDS_ElemIteratorPtr DriverSTL_W_SMDS_Mesh::getFaces() const
{
    ...
    SMDS_ElemIteratorPtr tmpTriaIter(
        new SMDS_ElementVectorIterator(myVolumeFacets.begin(), myVolumeFacets.end()));
...
static gp_XYZ getNormale(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const SMDS_MeshNode *n3)
{
    ...
    if (len > std::numeric_limits<double>::min()) n /= len;
...
struct PolyVertex {
    ...
    bool IsInsideTria(const PolyVertex *v)
    {
        ...
        return ((p ^ t) >= tol && (t ^ n) >= tol && (n ^ p) >= tol);
...
bool triangulate(std::vector<const SMDS_MeshNode *> &nodes, const size_t nbNodes)
{
    ...
    for (size_t i = 1; i < nbNodes; ++i) pv[i - 1].SetNodeAndNext(nodes[i - 1], pv[i]);
    ...
    while (nbBadTria < nbVertices) {
        if ((isGoodTria = v->TriaArea() > minArea)) {
            for (vi = v->_next->_next; vi != v->_prev; vi = vi->_next) {
                if (v->IsInsideTria(vi)) break;
            }
...
static int getNbTriangles(const SMDS_MeshElement *face)
{
    ...
    switch (face->GetEntityType()) {
        ...
        case SMDSEntity_BiQuad_Quadrangle: return face->NbNodes() - 1;
...
static int getTriangles(const SMDS_MeshElement *face, std::vector<const SMDS_MeshNode *> &nodes)
{
    ...
    switch (type) {
        ...
        default:
            ...
            // case SMDSEntity_Quad_Polygon:
            nbTria = nbNodes - 2;
            while (nIt->more()) nodes[i++] = nIt->next();
...
Driver_Mesh::Status DriverSTL_W_SMDS_Mesh::writeAscii() const
{
    ...
    while (itFaces->more()) {
        ...
        for (int iT = 0, iN = 0; iT < nbTria; ++iT) {
            gp_XYZ normale = getNormale(triaNodes[iN], triaNodes[iN + 1], triaNodes[iN + 2]);
            sprintf(sval,
            ...
            for (int jN = 0; jN < 3; ++jN, ++iN) {
                ...
                sprintf(sval, "     vertex % 12e % 12e % 12e\n", node.X(), node.Y(), node.Z());

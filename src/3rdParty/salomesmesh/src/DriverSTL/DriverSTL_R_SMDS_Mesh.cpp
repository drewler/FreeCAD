...
struct Hasher {
    ...
    //purpose  :
    //=======================================================================
    inline static Standard_Integer HashCode(const gp_Pnt &point, Standard_Integer Upper)
    {
        union
        {
            Standard_Real R[3];
        ...
        return ::HashCode(U.I[0] / 23 + U.I[1] / 19 + U.I[2] / 17 + U.I[3] / 13 + U.I[4] / 11
                              + U.I[5] / 7,
                          Upper);
    }
    //=======================================================================
    ...
    //purpose  :
    //=======================================================================
    inline static Standard_Boolean IsEqual(const gp_Pnt &point1, const gp_Pnt &point2)
    {
...
Driver_Mesh::Status DriverSTL_R_SMDS_Mesh::Perform()
{
    ...
    if (!file.open()) {
        ...
        if (file.error().empty()) fprintf(stderr, ">> ERROR : %s \n", file.error().c_str());
        return DRS_FAIL;
    }
    ...
    if (myIsAscii) aResult = readAscii(file);
    else
...
static SMDS_MeshNode *addNode(const gp_Pnt &P, TDataMapOfPntNodePtr &uniqnodes, SMDS_Mesh *theMesh)
{
    ...
    if (uniqnodes.IsBound(P)) { node = uniqnodes.Find(P); }
    else {
...
static SMDS_MeshNode *readNode(SMESH_File &theFile, TDataMapOfPntNodePtr &uniqnodes,
                               SMDS_Mesh *theMesh)
{
    gp_Pnt coord;
...
Driver_Mesh::Status DriverSTL_R_SMDS_Mesh::readAscii(SMESH_File &theFile) const
{
    ...
    for (Standard_Integer iTri = 0; iTri < nbTri; ++iTri) {
        ...
        if (myIsCreateFaces) myMesh->AddFace(node1, node2, node3);

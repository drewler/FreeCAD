...
bool SMESH_File::open()
{
    ...
    if (!_map && length > 0) {
#ifdef WIN32
        _file = CreateFileA(_name.data(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
{
    if (pos > (const char *)_map && pos < _end) _pos = (char *)pos;
}
...
void SMESH_File::rewind() { _pos = (char *)_map; }
...
bool SMESH_File::openForWriting()
{
    ...
    _file = ::open(_name.c_str(), O_WRONLY | O_CREAT,
...
bool SMESH_File::writeRaw(const void *data, size_t size)
{
    ...
    return ((err == FALSE) && (nbWritten == (DWORD)size));

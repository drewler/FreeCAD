...
DriverMED_W_SMESHDS_Mesh::DriverMED_W_SMESHDS_Mesh()
    : myMedVersion(MED::eV2_2), myAllSubMeshes(false), myDoGroupOfNodes(false),
      myDoGroupOfEdges(false), myDoGroupOfFaces(false), myDoGroupOfVolumes(false),
      myDoGroupOf0DElems(false), myDoGroupOfBalls(false), myAutoDimension(true),
...
string DriverMED_W_SMESHDS_Mesh::GetVersionString(const MED::EVersion theVersion, int theNbDigits)
{
    ...
    release = 1;
    ostringstream name;
    if (theNbDigits > 0) name << majeur;
    if (theNbDigits > 1) name << "." << mineur;
    if (theNbDigits > 2) name << "." << release;
    return name.str();
}
...
void DriverMED_W_SMESHDS_Mesh::AddAllSubMeshes() { myAllSubMeshes = true; }
...
void DriverMED_W_SMESHDS_Mesh::AddGroupOfNodes() { myDoGroupOfNodes = true; }

void DriverMED_W_SMESHDS_Mesh::AddGroupOfEdges() { myDoGroupOfEdges = true; }

void DriverMED_W_SMESHDS_Mesh::AddGroupOfFaces() { myDoGroupOfFaces = true; }

void DriverMED_W_SMESHDS_Mesh::AddGroupOfVolumes() { myDoGroupOfVolumes = true; }
...
// 3 dim
TGetCoord aXYZGetCoord[3] = {&SMDS_MeshNode::X, &SMDS_MeshNode::Y, &SMDS_MeshNode::Z};
TName aXYZName[3] = {X, Y, Z};

// 2 dim
TGetCoord aXYGetCoord[2] = {&SMDS_MeshNode::X, &SMDS_MeshNode::Y};
TName aXYName[2] = {X, Y};

TGetCoord aYZGetCoord[2] = {&SMDS_MeshNode::Y, &SMDS_MeshNode::Z};
TName aYZName[2] = {Y, Z};

TGetCoord aXZGetCoord[2] = {&SMDS_MeshNode::X, &SMDS_MeshNode::Z};
TName aXZName[2] = {X, Z};

// 1 dim
TGetCoord aXGetCoord[1] = {&SMDS_MeshNode::X};
TName aXName[1] = {X};

TGetCoord aYGetCoord[1] = {&SMDS_MeshNode::Y};
TName aYName[1] = {Y};

TGetCoord aZGetCoord[1] = {&SMDS_MeshNode::Z};
TName aZName[1] = {Z};
...
public:
    TCoordHelper(const SMDS_NodeIteratorPtr &theNodeIter, TGetCoord *theGetCoord, TName *theName,
                 TUnit *theUnit = aUnit)
        : myNodeIter(theNodeIter), myGetCoord(theGetCoord), myName(theName), myUnit(theUnit)
    {}
    virtual ~TCoordHelper() {}
    bool Next() { return myNodeIter->more() && (myCurrentNode = myNodeIter->next()); }
    const SMDS_MeshNode *GetNode() { return myCurrentNode; }
    MED::TIntVector::value_type GetID() { return myCurrentNode->GetID(); }
    MED::TFloatVector::value_type GetCoord(TInt theCoodId)
    {
        return (myCurrentNode->*myGetCoord[theCoodId])();
    }
    MED::TStringVector::value_type GetName(TInt theDimId) { return myName[theDimId]; }
    MED::TStringVector::value_type GetUnit(TInt theDimId) { return myUnit[theDimId]; }
};
...
void fillElemFamilyMap(TElemFamilyMap &anElemFamMap, list<DriverMED_FamilyPtr> &aFamilies,
                       const SMDSAbs_ElementType anElemType)
{
    anElemFamMap.Clear();
    list<DriverMED_FamilyPtr>::iterator aFamsIter = aFamilies.begin();
    while (aFamsIter != aFamilies.end()) {
        if ((*aFamsIter)->GetType() != anElemType) { aFamsIter++; }
        else {
...
int getFamilyId(const TElemFamilyMap &anElemFamMap, const SMDS_MeshElement *anElement,
...
SMESHDS_SubMeshIteratorPtr getIterator(std::vector<SMESHDS_SubMesh *> &mySubMeshes)
{
    return SMESHDS_SubMeshIteratorPtr(
        new SMDS_SetIterator<const SMESHDS_SubMesh *, std::vector<SMESHDS_SubMesh *>::iterator>(
            mySubMeshes.begin(), mySubMeshes.end()));
}
} // namespace
...
Driver_Mesh::Status DriverMED_W_SMESHDS_Mesh::Perform()
{
    ...
    try {
        ...
        TCoordHelperPtr aCoordHelperPtr;
        {
            ...
            if (myAutoDimension) {
                ...
                if (!mainShape.IsNull() && hasShapeToMesh) {
                    ...
                    for (subShape.Init(mainShape, TopAbs_VERTEX); subShape.More();
                         subShape.Next()) {
                ...
                anIsXDimension =
                    (aBounds[1] - aBounds[0]) + abs(aBounds[1]) + abs(aBounds[0]) > EPS;
                anIsYDimension =
                    (aBounds[3] - aBounds[2]) + abs(aBounds[3]) + abs(aBounds[2]) > EPS;
                anIsZDimension =
                    (aBounds[5] - aBounds[4]) + abs(aBounds[5]) + abs(aBounds[4]) > EPS;
                aSpaceDimension = anIsXDimension + anIsYDimension + anIsZDimension;
                if (!aSpaceDimension) aSpaceDimension = 3;
        ...
        if (myMesh->NbEdges() > 0) aMeshDimension = 1;
        if (myMesh->NbFaces() > 0) aMeshDimension = 2;
        if (myMesh->NbVolumes() > 0) aMeshDimension = 3;
        ...
        if (myAllSubMeshes) {
            aFamilies = DriverMED_Family::MakeFamilies(
                myMesh->SubMeshes(), myGroups, myDoGroupOfNodes && nbNodes,
                myDoGroupOfEdges && nbEdges, myDoGroupOfFaces && nbFaces,
                myDoGroupOfVolumes && nbVolumes, myDoGroupOf0DElems && nb0DElements,
                myDoGroupOfBalls && nbBalls);
        }
        else {
            aFamilies = DriverMED_Family::MakeFamilies(
                getIterator(mySubMeshes), myGroups, myDoGroupOfNodes && nbNodes,
                myDoGroupOfEdges && nbEdges, myDoGroupOfFaces && nbFaces,
                myDoGroupOfVolumes && nbVolumes, myDoGroupOf0DElems && nb0DElements,
                myDoGroupOfBalls && nbBalls);
        }
        ...
        PNodeInfo aNodeInfo = myMed->CrNodeInfo(aMeshInfo, aNbNodes, theMode, theSystem,
                                                theIsElemNum, theIsElemNames);
        ...
        bool polyTypesSupported =
            (myMed->CrPolygoneInfo(aMeshInfo, eMAILLE, ePOLYGONE, 0, 0).get() != 0);
...
#endif
        aTElemTypeDatas.push_back(TElemTypeData(
            anEntity, ePOINT1, nbElemInfo.Nb0DElements() + nodesOf0D.size(), SMDSAbs_0DElement));
#ifdef _ELEMENTS_BY_DIM_
        anEntity = eSTRUCT_ELEMENT;
#endif
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eBALL, nbElemInfo.NbBalls(), SMDSAbs_Ball));
#ifdef _ELEMENTS_BY_DIM_
        anEntity = eARETE;
#endif
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eSEG2, nbElemInfo.NbEdges(ORDER_LINEAR), SMDSAbs_Edge));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eSEG3, nbElemInfo.NbEdges(ORDER_QUADRATIC), SMDSAbs_Edge));
#ifdef _ELEMENTS_BY_DIM_
        anEntity = eFACE;
#endif
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eTRIA3, nbElemInfo.NbTriangles(ORDER_LINEAR), SMDSAbs_Face));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eTRIA6,
                          nbElemInfo.NbTriangles(ORDER_QUADRATIC) - nbElemInfo.NbBiQuadTriangles(),
                          SMDSAbs_Face));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eTRIA7, nbElemInfo.NbBiQuadTriangles(), SMDSAbs_Face));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eQUAD4, nbElemInfo.NbQuadrangles(ORDER_LINEAR), SMDSAbs_Face));
        aTElemTypeDatas.push_back(TElemTypeData(anEntity, eQUAD8,
                                                nbElemInfo.NbQuadrangles(ORDER_QUADRATIC)
                                                    - nbElemInfo.NbBiQuadQuadrangles(),
                                                SMDSAbs_Face));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eQUAD9, nbElemInfo.NbBiQuadQuadrangles(), SMDSAbs_Face));
        if (polyTypesSupported) {
            aTElemTypeDatas.push_back(TElemTypeData(
                anEntity, ePOLYGONE, nbElemInfo.NbPolygons(ORDER_LINEAR), SMDSAbs_Face));
            // we need one more loop on poly elements to count nb of their nodes
            aTElemTypeDatas.push_back(TElemTypeData(
                anEntity, ePOLYGONE, nbElemInfo.NbPolygons(ORDER_LINEAR), SMDSAbs_Face));
            aTElemTypeDatas.push_back(TElemTypeData(
                anEntity, ePOLYGON2, nbElemInfo.NbPolygons(ORDER_QUADRATIC), SMDSAbs_Face));
...
#endif
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eTETRA4, nbElemInfo.NbTetras(ORDER_LINEAR), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(TElemTypeData(
            anEntity, eTETRA10, nbElemInfo.NbTetras(ORDER_QUADRATIC), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, ePYRA5, nbElemInfo.NbPyramids(ORDER_LINEAR), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(TElemTypeData(
            anEntity, ePYRA13, nbElemInfo.NbPyramids(ORDER_QUADRATIC), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, ePENTA6, nbElemInfo.NbPrisms(ORDER_LINEAR), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(TElemTypeData(
            anEntity, ePENTA15, nbElemInfo.NbPrisms(ORDER_QUADRATIC), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eHEXA8, nbElemInfo.NbHexas(ORDER_LINEAR), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(TElemTypeData(
            anEntity, eHEXA20, nbElemInfo.NbHexas(ORDER_QUADRATIC) - nbElemInfo.NbTriQuadHexas(),
            SMDSAbs_Volume));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eHEXA27, nbElemInfo.NbTriQuadHexas(), SMDSAbs_Volume));
        aTElemTypeDatas.push_back(
            TElemTypeData(anEntity, eOCTA12, nbElemInfo.NbHexPrisms(), SMDSAbs_Volume));
        if (polyTypesSupported) {
            aTElemTypeDatas.push_back(
                TElemTypeData(anEntity, ePOLYEDRE, nbElemInfo.NbPolyhedrons(), SMDSAbs_Volume));
            // we need one more loop on poly elements to count nb of their nodes
            aTElemTypeDatas.push_back(
                TElemTypeData(anEntity, ePOLYEDRE, nbElemInfo.NbPolyhedrons(), SMDSAbs_Volume));
        }
        ...
        for (; aElemTypeData != aTElemTypeDatas.end(); ++aElemTypeData) {
            if (aElemTypeData->_nbElems == 0) continue;
            ...
            // Treat POLYGONs
            // ---------------
            if (aElemTypeData->_geomType == ePOLYGONE || aElemTypeData->_geomType == ePOLYGON2) {
                ...
                if (nbPolygonNodes == 0) {
                    ...
                    while (elemIterator->more()) {
                        ...
                        if (++iElem == aElemTypeData->_nbElems) break;
                    }
                }
                else {
                    // Store in med file
                    PPolygoneInfo aPolygoneInfo = myMed->CrPolygoneInfo(
                        aMeshInfo, aElemTypeData->_entity, aElemTypeData->_geomType,
                        aElemTypeData->_nbElems, nbPolygonNodes, theConnMode, theIsElemNum,
                        theIsElemNames);
                    ...
                    while (elemIterator->more()) {
                        ...
                        if (++iElem == aPolygoneInfo->GetNbElem()) break;
                    }
            ...
            // ----------------
                ...
                if (nbPolyhedronNodes == 0) {
                    ...
                    // Store in med file
                    PPolyedreInfo aPolyhInfo = myMed->CrPolyedreInfo(
                        aMeshInfo, aElemTypeData->_entity, aElemTypeData->_geomType,
                        aElemTypeData->_nbElems, nbPolyhedronFaces + 1, nbPolyhedronNodes,
                        theConnMode, theIsElemNum, theIsElemNames);
                    ...
                    while (elemIterator->more()) {
                        ...
                        if (++iElem == aPolyhInfo->GetNbElem()) break;
                    }
            ...
            // ----------------
                ...
                // allocate data arrays
                PCellInfo aCellInfo = myMed->CrCellInfo(
                    aMeshInfo, aElemTypeData->_entity, aElemTypeData->_geomType,
                    aElemTypeData->_nbElems, theConnMode, theIsElemNum, theIsElemNames);
                // build map of family numbers for this type
                if (!isElemFamMapBuilt[aElemTypeData->_smdsType]) {
                ...
                while (elemIterator->more()) {
                    ...
                    if (++iElem == aCellInfo->GetNbElem()) break;
                }
                // store data in a file
...
bool DriverMED_W_SMESHDS_Mesh::getNodesOfMissing0DOnVert(
    SMESHDS_Mesh *meshDS, std::vector<const SMDS_MeshElement *> &nodes)
{
    nodes.clear();
    for (int i = 1; i <= meshDS->MaxShapeIndex(); ++i) {
        if (meshDS->IndexToShape(i).ShapeType() != TopAbs_VERTEX) continue;
        if (SMESHDS_SubMesh *sm = meshDS->MeshElements(i)) {
            SMDS_NodeIteratorPtr nIt = sm->GetNodes();
            while (nIt->more()) {
                const SMDS_MeshNode *n = nIt->next();
                if (n->NbInverseElements(SMDSAbs_0DElement) == 0) nodes.push_back(n);
            }
        }
    }
    return !nodes.empty();

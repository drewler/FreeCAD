...
SMESH_Mesh *SMESH_Gen::CreateMesh(int theStudyId, bool theIsEmbeddedMode)
{
    ...
    SMESH_Mesh *aMesh =
        new SMESH_Mesh(_localId++, theStudyId, this, theIsEmbeddedMode, aStudyContext->myDocument);
...
bool SMESH_Gen::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                        const bool aShapeOnly /*=false*/, const bool anUpward /*=false*/,
                        const ::MeshDimension aDim /*=::MeshDim_3D*/, TSetOfInt *aShapesId /*=0*/)
{
    ...
    if (anUpward) // is called from the below code in this method
    {
        ...
        while (smIt->more()) {
            ...
            if (!aMesh.HasShapeToMesh() && shapeType == TopAbs_VERTEX) continue;
            ...
            if (smToCompute->GetComputeState() == SMESH_subMesh::READY_TO_COMPUTE) {
                if (_compute_canceled) return false;
        ...
        while (smIt->more()) {
            SMESH_subMesh *smToCompute = smIt->next();
            if (smToCompute->GetComputeState() != SMESH_subMesh::READY_TO_COMPUTE) continue;
            ...
            // check for preview dimension limitations
            if (aShapesId && aShapeDim > (int)aDim) continue;
            ...
            if (algo && !algo->NeedDiscreteBoundary()) {
                if (algo->SupportSubmeshes()) {
                    ...
                    if (prevShapeDim != aShapeDim) {
                        ...
                        for (shDim2smIt = shDim2sm.rbegin(); shDim2smIt != shDim2sm.rend();
                             ++shDim2smIt)
                            if (shDim2smIt->first == globalAlgoDim)
                                smWithAlgoSupportingSubmeshes[aShapeDim].push_back(
                                    shDim2smIt->second);
                            else
                                smWithAlgoSupportingSubmeshes[aShapeDim].push_front(
                                    shDim2smIt->second);
                        shDim2sm.clear();
                    }
                ...
                else // Compute w/o support of sub-meshes
                {
                    ...
                    smToCompute->ComputeStateEngine(computeEvent);
                    setCurrentSubMesh(NULL);
                    if (aShapesId) aShapesId->insert(smToCompute->GetId());
                }
            }
        }
        // reload sub-meshes from shDim2sm into smWithAlgoSupportingSubmeshes
        ...
        for (aShapeDim = 0; aShapeDim < 4; ++aShapeDim) {
            ...
            for (size_t i = 0; i < smVec.size(); ++i) {
                ...
                if (!GetAlgo(sm, &algoShape)) continue; // strange...
                ...
                while (smIt->more()) {
                    ...
                    // check for preview dimension limitations
                    if (aShapesId && GetShapeDim(aSubShape.ShapeType()) > (int)aDim) continue;

                    SMESH_HypoFilter filter(SMESH_HypoFilter::IsAlgo());
                    filter.And(SMESH_HypoFilter::IsApplicableTo(aSubShape))
                        .And(SMESH_HypoFilter::IsMoreLocalThan(algoShape, aMesh));

                    if (SMESH_Algo *subAlgo =
                            (SMESH_Algo *)aMesh.GetHypothesis(smToCompute, filter, true)) {
                        if (!subAlgo->NeedDiscreteBoundary()) continue;
                        SMESH_Hypothesis::Hypothesis_Status status;
                        if (subAlgo->CheckHypothesis(aMesh, aSubShape, status))
                            // mesh a lower smToCompute starting from vertices
                            Compute(aMesh, aSubShape, aShapeOnly, /*anUpward=*/true, aDim,
                                    aShapesId);
                    }
                }
            }
            ...
            for (size_t i = 0; i < smVec.size(); ++i) {
                ...
                if (sm->GetComputeState() == SMESH_subMesh::READY_TO_COMPUTE) {
                    ...
                    // check for preview dimension limitations
                    if (aShapesId && GetShapeDim(shapeType) > (int)aDim) continue;

                    if (_compute_canceled) return false;
                    setCurrentSubMesh(sm);
                    sm->ComputeStateEngine(computeEvent);
                    setCurrentSubMesh(NULL);
                    if (aShapesId) aShapesId->insert(sm->GetId());
                }
            }
    ...
    if (aShape.IsSame(aMesh.GetShapeToMesh()) && !aShapesId && // not preview
 ...
 */
//=============================================================================
void SMESH_Gen::PrepareCompute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    _compute_canceled = false;
    resetCurrentSubMesh();
}
 ...
 */
//=============================================================================
void SMESH_Gen::CancelCompute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
...
{
    if (sm) _sm_current.push_back(sm);

    else if (!_sm_current.empty())
        _sm_current.pop_back();
}

void SMESH_Gen::resetCurrentSubMesh() { _sm_current.clear(); }
...
bool SMESH_Gen::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape, MapShapeNbElems &aResMap,
                         const bool anUpward, TSetOfInt *aShapesId)
{
...
static bool checkConformIgnoredAlgos(SMESH_Mesh &aMesh, SMESH_subMesh *aSubMesh,
                                     const SMESH_Algo *aGlobIgnoAlgo,
                                     const SMESH_Algo *aLocIgnoAlgo, bool &checkConform,
                                     set<SMESH_subMesh *> &aCheckedMap,
                                     list<SMESH_Gen::TAlgoStateError> &theErrors)
{
    ASSERT(aSubMesh);
    if (aSubMesh->GetSubShape().ShapeType() == TopAbs_VERTEX) return true;
    ...
    for (; it != listHyp.end(); it++) {
        ...
        if (aHyp->GetType() == SMESHDS_Hypothesis::PARAM_ALGO) continue;
        ...
        if (aLocIgnoAlgo) // algo is hidden by a local algo of upper dim
        {
            ...
            if ((dim < aMaxGlobIgnoDim && !isNeededDim)
                ...
                if (algo->SupportSubmeshes()) algo = 0;
                SMESH_subMeshIteratorPtr revItSub = aSubMesh->getDependsOnIterator(
                    /*includeSelf=*/false, /*complexShapeFirst=*/true);
                bool checkConform2 = false;
                while (revItSub->more()) {
                    SMESH_subMesh *sm = revItSub->next();
                    checkConformIgnoredAlgos(aMesh, sm, aGlobIgnoAlgo, algo, checkConform2,
                                             aCheckedMap, theErrors);
...
static bool checkMissing(SMESH_Gen *aGen, SMESH_Mesh &aMesh, SMESH_subMesh *aSubMesh,
                         const int aTopAlgoDim, bool *globalChecked, const bool checkNoAlgo,
                         set<SMESH_subMesh *> &aCheckedMap,
                         list<SMESH_Gen::TAlgoStateError> &theErrors)
{
    switch (aSubMesh->GetSubShape().ShapeType()) {
        ...
        default: return true;     // not meshable sub-mesh
    }
    if (aCheckedMap.count(aSubMesh)) return true;
    ...
    switch (aSubMesh->GetAlgoState()) {
        ...
        case SMESH_subMesh::MISSING_HYP: {
            ...
            if (!IsGlobalHypothesis || !globalChecked[algo->GetDim()]) {
                ...
                if (IsGlobalHypothesis) globalChecked[algo->GetDim()] = true;
                theErrors.push_back(SMESH_Gen::TAlgoStateError());
        ...
        case SMESH_subMesh::HYP_OK:
            ...
            if (!algo->NeedDiscreteBoundary()) {
                SMESH_subMeshIteratorPtr itsub =
                    aSubMesh->getDependsOnIterator(/*includeSelf=*/false,
                                                   /*complexShapeFirst=*/false);
                while (itsub->more()) aCheckedMap.insert(itsub->next());
            }
            break;
        default: ASSERT(0);
    }
    ...
    ASSERT(algo);
    bool isTopLocalAlgo = (aTopAlgoDim <= algo->GetDim() && !aGen->IsGlobalHypothesis(algo, aMesh));
    if (!algo->NeedDiscreteBoundary() || isTopLocalAlgo) {
        ...
        while (itsub->more()) {
            ...
            if (isTopLocalAlgo) {
                ...
                if (!checkMissing(aGen, aMesh, sm, aTopAlgoDim2, globalChecked, checkNoAlgo2,
                                  aCheckedMap, theErrors)) {
                    ret = false;
                    if (sm->GetAlgoState() == SMESH_subMesh::NO_ALGO) checkNoAlgo2 = false;
                }
            }
            aCheckedMap.insert(sm);
        }
...
bool SMESH_Gen::GetAlgoState(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                             list<TAlgoStateError> &theErrors)
{
    ...
    while (revItSub->more()) {
        ...
        if (smToCheck->GetSubShape().ShapeType() == TopAbs_VERTEX) break;

        if (aCheckedSubs.insert(smToCheck).second) // not yet checked
            if (!checkConformIgnoredAlgos(theMesh, smToCheck, aGlobIgnoAlgo, 0, checkConform,
                                          aCheckedSubs, theErrors))
                ret = false;

        if (smToCheck->GetAlgoState() != SMESH_subMesh::NO_ALGO) hasAlgo = true;
    }
    ...
    while (revItSub->more()) {
        ...
        if (!checkMissing(this, theMesh, smToCheck, aTopAlgoDim, globalChecked, checkNoAlgo,
                          aCheckedSubs, theErrors)) {
            ret = false;
...
std::vector<std::string> SMESH_Gen::GetPluginXMLPaths()
{
    ...
    if (const char *meshersList = getenv("SMESH_MeshersList")) {
        ...
        while (from < meshers.size()) {
            ...
            if (pos != string::npos) plugin = meshers.substr(from, pos - from);
            else
            ...
            if (plugin == "StdMeshers") rootDirVar = "SMESH", pluginSubDir = "smesh";
            else
                for (pos = 0; pos < plugin.size(); ++pos) rootDirVar += toupper(plugin[pos]);
            ...
            if (xmlPath[xmlPath.size() - 1] != sep[0]) xmlPath += sep;
            xmlPath += "share" + sep + "salome" + sep + "resources" + sep;
            for (pos = 0; pos < pluginSubDir.size(); ++pos) xmlPath += tolower(pluginSubDir[pos]);
...
#endif
            if (fileOK) xmlPaths.push_back(xmlPath);
        }
    }
...
SMESH_Algo *SMESH_Gen::GetAlgo(SMESH_subMesh *aSubMesh, TopoDS_Shape *assignedTo)
{
    ...
    SMESH_Algo *algo = (SMESH_Algo *)aMesh.GetHypothesis(aSubMesh, filter, true, &assignedToShape);

    if (algo && aShape.ShapeType() == TopAbs_FACE && !aShape.IsSame(assignedToShape)
        ...
        if (algo2 &&                                               // algo found
            ...
            PShapeIteratorPtr solidIt =
                SMESH_MesherHelper::GetAncestors(aShape, aMesh, TopAbs_SOLID);
            ...
            if (algos3D.size() > 1) {
                ...
                if ((algoData2.IsCompatible(algoData3d0) && algoData2.IsCompatible(algoData3d1))
                    && !(algoData.IsCompatible(algoData3d0) && algoData.IsCompatible(algoData3d1)))
                    algo = algo2;
            }
        }
    }

    if (assignedTo && algo) *assignedTo = assignedToShape;
...
int SMESH_Gen::GetANewId() { return _hypId++; }

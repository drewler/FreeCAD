...
SMESH_OctreeNode::SMESH_OctreeNode(const TIDSortedNodeSet &theNodes, const int maxLevel,
    ...
    : SMESH_Octree(new Limit(maxLevel, minBoxSize, maxNbNodes)), myNodes(theNodes)
{
    compute();
}
...
int SMESH_OctreeNode::getMaxNbNodes() const { return ((Limit *)myLimit)->myMaxNbNodes; }
...
SMESH_Octree *SMESH_OctreeNode::newChild() const { return new SMESH_OctreeNode(); }
...
Bnd_B3d *SMESH_OctreeNode::buildRootBox()
{
    ...
    if (myNodes.size() <= getMaxNbNodes()) myIsLeaf = true;
...
{
    if (precision <= 0.) return !(getBox()->IsOut(p));
...
void SMESH_OctreeNode::buildChildrenData()
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (myChild->myNodes.size() <= getMaxNbNodes()) myChild->myIsLeaf = true;
    }
}
 ...
 */
//====================================================================
void SMESH_OctreeNode::NodesAround(const SMDS_MeshNode *Node, list<const SMDS_MeshNode *> *Result,
                                   const double precision)
{
    SMESH_TNodeXYZ p(Node);
    if (isInside(p, precision)) {
        if (isLeaf()) { Result->insert(Result->end(), myNodes.begin(), myNodes.end()); }
        else {
            for (int i = 0; i < 8; i++) {
                SMESH_OctreeNode *myChild = dynamic_cast<SMESH_OctreeNode *>(myChildren[i]);
...
bool SMESH_OctreeNode::NodesAround(const gp_XYZ &node,
                                   map<double, const SMDS_MeshNode *> &dist2Nodes, double precision)
{
    if (!dist2Nodes.empty()) precision = min(precision, sqrt(dist2Nodes.begin()->first));
    else if (precision == 0.)
        precision = maxSize() / 2;
    ...
    if (isInside(node, precision)) {
        if (!isLeaf()) {
            ...
            if (((SMESH_OctreeNode *)myChildren[nodeChild])
                    ->NodesAround(node, dist2Nodes, precision))
                return true;

            for (int i = 0; i < 8; i++)
                if (i != nodeChild)
                    if (((SMESH_OctreeNode *)myChildren[i])
                            ->NodesAround(node, dist2Nodes, precision))
                        return true;
        }
        else if (NbNodes() > 0) {
            ...
            for (; nIt != myNodes.end(); ++nIt) {
                ...
                if (dist2 < minDist) dist2Nodes.insert(make_pair(minDist = dist2, p2._node));
            }
...
void SMESH_OctreeNode::NodesAround(const gp_XYZ &point, std::vector<const SMDS_MeshNode *> &nodes,
                                   double precision)
{
    if (isInside(point, precision)) {
        if (isLeaf() && NbNodes()) {
            ...
            for (; nIt != myNodes.end(); ++nIt) {
                ...
                if (dist2 <= minDist2) nodes.push_back(p2._node);
            }
        }
        else if (myChildren) {
            for (int i = 0; i < 8; i++) {
                                           ...
                                           const int maxNbNodes)
{
    ...
    SMESH_OctreeNode theOctreeNode(theSetOfNodes, maxLevel < 0 ? MAX_LEVEL : maxLevel, maxNbNodes,
                                   theTolerance);
    ...
    while (it1 != theSetOfNodes->end()) {
        ...
        if (!ListOfCoincidentNodes.empty()) {
            ...
            if (idLess(n1, ListOfCoincidentNodes.front())) ListOfCoincidentNodes.push_front(n1);
                ...
                ListOfCoincidentNodes.push_back(n1);
 ...
 */
//======================================================================================
void SMESH_OctreeNode::FindCoincidentNodes(const SMDS_MeshNode *Node, TIDSortedNodeSet *SetOfNodes,
            ...
            while (it != myNodes.end()) {
                ...
                if (Node->GetID() != n2->GetID()) // JFA: for bug 0020185
                {
                    ...
                    if (squareBool) {
                        ...
                        SetOfNodes->erase(n2);
                        myNodes.erase(
                            *it++); // it++ goes forward and returns it's previous position
                    }
                }
                if (!squareBool) it++;
            }
            if (!Result->empty()) myNodes.erase(Node); // JFA: for bug 0020185
        }
        else {
...
void SMESH_OctreeNode::UpdateByMoveNode(const SMDS_MeshNode *node, const gp_Pnt &toPnt)
{
    if (isLeaf()) {
        ...
        if (pointInMe != nodeInMe) {
            if (pointInMe) myNodes.insert(node);
            else
                myNodes.erase(node);
        }
    }
...
{
    return SMESH_OctreeNodeIteratorPtr(new SMDS_SetIterator<SMESH_OctreeNode *, TBaseTree **>(
...
{
    return SMDS_NodeIteratorPtr(new SMDS_SetIterator<SMDS_pNode, TIDSortedNodeSet::const_iterator>(

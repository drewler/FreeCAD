...
bool buildMeshGrille(const MED::PWrapper &theWrapper, const MED::PMeshInfo &theMeshInfo,
                     SMESHDS_Mesh *theMesh, const TID2FamilyMap &myFamilies);
/*!
...
const SMDS_MeshNode *FindNode(const SMDS_Mesh *theMesh, TInt theId)
{
    ...
    EXCEPTION(runtime_error,
              "SMDS_Mesh::FindNode - cannot find a SMDS_MeshNode for ID = " << theId);
}
...
void DriverMED_R_SMESHDS_Mesh::SetMeshName(string theMeshName) { myMeshName = theMeshName; }
...
Driver_Mesh::Status DriverMED_R_SMESHDS_Mesh::Perform()
{
    ...
    try {
        ...
        PWrapper aMed = CrWrapper(
            myFile,
            false); // We are using the internal MED file version checker instead of an external reader
        ...
        for (int iMesh = 0; iMesh < aNbMeshes; iMesh++) {
            ...
            for (TInt iFam = 0; iFam < aNbFams; iFam++) {
                ...
                if (anErr >= 0) {
                    ...
                    bool isAttrOk = false;
                    if (aFamilyInfo->GetNbAttr() == aNbGrp) isAttrOk = true;
            ...
            aMeshInfo->myDim =
                aMeshInfo->mySpaceDim; // ignore meshdim in MEDFile because it can be false
            ...
            for (TInt iElem = 0; iElem < aNbElems; iElem++) {
                ...
                if (anIsNodeNum) {
                    aNode = myMesh->AddNodeWithID(aCoords[0], aCoords[1], aCoords[2],
                                                  aNodeInfo->GetElemNum(iElem));
                }
                else {
                    aNode = myMesh->AddNodeWithID(aCoords[0], aCoords[1], aCoords[2], iElem + 1);
                }
            ...
            for (; anEntityIter != aEntityInfo.end(); anEntityIter++) {
                ...
                for (; aGeom2SizeIter != aGeom2Size.end(); aGeom2SizeIter++) {
                    ...
                    if (anEntity == eSTRUCT_ELEMENT) // MED_BALL (issue 0021459)
                    {
                        ...
                        if (anIsElemNum && aBallInfo->myElemNum->empty()) anIsElemNum = eFAUX;
                        ...
                        if (anIsNodeNum) {
                            ...
                            for (TInt iBall = 0; iBall < aNbBalls && anIsNodeNum; iBall++) {
                                aNodeIds[iBall] =
                                    aNodeInfo->GetElemNum((*aBallInfo->myConn)[iBall] - 1);
                                anIsNodeNum = myMesh->FindNode(aNodeIds[iBall]) ? eVRAI : eFAUX;
                            }
                        }
#endif
                        if (!anIsNodeNum) aNodeIds.swap(*(aBallInfo->myConn));
                        ...
                        for (TInt iBall = 0; iBall < aNbBalls; iBall++) {
                            anElement = 0;
                            if (anIsElemNum) {
                                if (!(anElement = myMesh->AddBallWithID(
                                          aNodeIds[iBall], aBallInfo->myDiameters[iBall],
                        ...
                        if (!anIsElemNum
                            && (takeNumbers && aBallInfo->IsElemNum()
                                && !aBallInfo->myElemNum->empty()))
                            if (aResult < DRS_WARN_RENUMBER) aResult = DRS_WARN_RENUMBER;
                    ...
                    switch (aGeom) {
                        ...
                        case ePOLYGONE:
                        case ePOLYGON2: {
                            PPolygoneInfo aPolygoneInfo =
                                aMed->GetPPolygoneInfo(aMeshInfo, anEntity, aGeom);
                            ...
                            for (TInt iElem = 0; iElem < aNbElem; iElem++) {
                                ...
                                if (anIsNodeNum)
                                    for (TInt iConn = 0; iConn < aNbConn; iConn++)
                                        aNodeIds[iConn] =
                                            aNodeInfo->GetElemNum(aConnSlice[iConn] - 1);
                                else
                                    for (TInt iConn = 0; iConn < aNbConn; iConn++)
...
#endif
                                if (!anElement) { aResult = DRS_WARN_SKIP_ELEM; }
                                else {
                                    if (isRenum) {
                                        anIsElemNum = eFAUX;
                                        takeNumbers = false;
                        ...
                        case ePOLYEDRE: {
                            PPolyedreInfo aPolyedreInfo =
                                aMed->GetPPolyedreInfo(aMeshInfo, anEntity, aGeom);
                            EBooleen anIsElemNum = takeNumbers ? aPolyedreInfo->IsElemNum() : eFAUX;

                            TInt aNbElem = aPolyedreInfo->GetNbElem();
                            for (TInt iElem = 0; iElem < aNbElem; iElem++) {
                                MED::TCConnSliceArr aConnSliceArr =
                                    aPolyedreInfo->GetConnSliceArr(iElem);
                                ...
                                try {
                                    ...
                                    if (!anElement) {
                                        ...
                                        for (TInt iConn = 0; iConn < aNbNodes; iConn++)
                                            aNodes[iConn] = FindNode(myMesh, aNodeIds[iConn]);
                                        anElement =
                                            myMesh->AddPolyhedralVolume(aNodes, aQuantities);
                                        isRenum = anIsElemNum;
                                    }
#ifndef _DEXCEPT_
                                }
                        ...
                        default: {
                            ...
                            if (MYDEBUG)
                                MESSAGE("Perform - anEntity = " << anEntity << "; anIsElemNum = "
                                                                << anIsElemNum);
                            if (MYDEBUG)
                                MESSAGE("Perform - aGeom = " << aGeom
                                                             << "; aNbElems = " << aNbElems);
                                ...
                                try {
                                    ...
                                    if (anIsNodeNum)
                                        for (int iNode = 0; iNode < aNbNodes; iNode++)
                                            aNodeIds[iNode] =
                                                aNodeInfo->GetElemNum(aConnSlice[iNode] - 1);
                                    else
                                        for (int iNode = 0; iNode < aNbNodes; iNode++)
                                ...
                                try {
                                    ...
                                    switch (aGeom) {
                                        case ePOINT1:
                                            ...
                                            if (!anElement) {
                                                anElement = myMesh->Add0DElement(
                                                    FindNode(myMesh, aNodeIds[0]));
                                                isRenum = anIsElemNum;
                                            }
                                            break;
                                        case eSEG2:
                                            if (anIsElemNum)
                                                anElement = myMesh->AddEdgeWithID(
                                                    aNodeIds[0], aNodeIds[1],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement =
                                                    myMesh->AddEdge(FindNode(myMesh, aNodeIds[0]),
                                        ...
                                        case eSEG3:
                                            if (anIsElemNum)
                                                anElement = myMesh->AddEdgeWithID(
                                                    aNodeIds[0], aNodeIds[1], aNodeIds[2],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement =
                                                    myMesh->AddEdge(FindNode(myMesh, aNodeIds[0]),
                                        ...
                                        case eTRIA3:
                                            aNbNodes = 3;
                                            if (anIsElemNum)
                                                anElement = myMesh->AddFaceWithID(
                                                    aNodeIds[0], aNodeIds[1], aNodeIds[2],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement =
                                                    myMesh->AddFace(FindNode(myMesh, aNodeIds[0]),
                                                                    FindNode(myMesh, aNodeIds[1]),
                                                                    FindNode(myMesh, aNodeIds[2]));
                                                isRenum = anIsElemNum;
                                            }
                                        ...
                                        case eTRIA6:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddFaceWithID(
                                                    aNodeIds[0], aNodeIds[1], aNodeIds[2],
                                                    aNodeIds[3], aNodeIds[4], aNodeIds[5],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement =
                                                    myMesh->AddFace(FindNode(myMesh, aNodeIds[0]),
                                        ...
                                        case eTRIA7:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddFaceWithID(
                                                    aNodeIds[0], aNodeIds[1], aNodeIds[2],
                                                    aNodeIds[3], aNodeIds[4], aNodeIds[5],
                                                    aNodeIds[6], aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement =
                                                    myMesh->AddFace(FindNode(myMesh, aNodeIds[0]),
                                                                    FindNode(myMesh, aNodeIds[1]),
                                        ...
                                        case eTETRA4:
                                            aNbNodes = 4;
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    aNodeIds[0], aNodeIds[1], aNodeIds[2],
                                                    aNodeIds[3], aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement = myMesh->AddVolume(
                                                    FindNode(myMesh, aNodeIds[0]),
                                                    FindNode(myMesh, aNodeIds[1]),
                                                    FindNode(myMesh, aNodeIds[2]),
                                                    FindNode(myMesh, aNodeIds[3]));
                                        ...
                                        case eTETRA10:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    ...
                                                    aNodeIds[3], aNodeIds[4], aNodeIds[5],
                                                    aNodeIds[6], aNodeIds[7], aNodeIds[8],
                                                    aNodeIds[9], aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement = myMesh->AddVolume(
                                                    FindNode(myMesh, aNodeIds[0]),
                                                    FindNode(myMesh, aNodeIds[1]),
                                        ...
                                        case ePYRA13:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    ...
                                                    aNodeIds[3], aNodeIds[4], aNodeIds[5],
                                                    aNodeIds[6], aNodeIds[7], aNodeIds[8],
                                                    aNodeIds[9], aNodeIds[10], aNodeIds[11],
                                                    aNodeIds[12], aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement = myMesh->AddVolume(
                                                    FindNode(myMesh, aNodeIds[0]),
                                                    FindNode(myMesh, aNodeIds[1]),
                                        ...
                                        case ePENTA15:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    ...
                                                    aNodeIds[6], aNodeIds[7], aNodeIds[8],
                                                    aNodeIds[9], aNodeIds[10], aNodeIds[11],
                                                    aNodeIds[12], aNodeIds[13], aNodeIds[14],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement = myMesh->AddVolume(
                                                    FindNode(myMesh, aNodeIds[0]),
                                        ...
                                        case eHEXA20:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    ...
                                                    aNodeIds[9], aNodeIds[10], aNodeIds[11],
                                                    aNodeIds[12], aNodeIds[13], aNodeIds[14],
                                                    aNodeIds[15], aNodeIds[16], aNodeIds[17],
                                        ...
                                        case eHEXA27:
                                            ...
                                            if (anIsElemNum)
                                                anElement = myMesh->AddVolumeWithID(
                                                    ...
                                                    aNodeIds[12], aNodeIds[13], aNodeIds[14],
                                                    aNodeIds[15], aNodeIds[16], aNodeIds[17],
                                                    aNodeIds[18], aNodeIds[19], aNodeIds[20],
                                                    aNodeIds[21], aNodeIds[22], aNodeIds[23],
                                                    aNodeIds[24], aNodeIds[25], aNodeIds[26],
                                                    aCellInfo->GetElemNum(iElem));
                                            if (!anElement) {
                                                anElement = myMesh->AddVolume(
                                                    FindNode(myMesh, aNodeIds[0]),
    ...
    for (; aFamsIter != myFamilies.end(); aFamsIter++) {
        ...
        for (; aGrNamesIter != aGroupNames.end(); aGrNamesIter++) {
            ...
            for (; type != types.end(); ++type) {
                ...
                if (aResGroupNames.insert(aNameAndType).second) { aResult.push_back(aNameAndType); }
            }
        }
    }
...
void DriverMED_R_SMESHDS_Mesh::GetGroup(SMESHDS_Group *theGroup)
{
    ...
    for (; aFamsIter != myFamilies.end(); aFamsIter++) {
        ...
        if (aFamily->GetTypes().count(theGroup->GetType()) && aFamily->MemberOf(aGroupName)) {
            ...
            if (aGroupAttrVal != 0) theGroup->SetColorGroup(aGroupAttrVal);
...
void DriverMED_R_SMESHDS_Mesh::GetSubMesh(SMESHDS_SubMesh *theSubMesh, const int theId)
{
            ...
            if (aName.substr(0, 7) == string("SubMesh")) {
                ...
                if (aFamily->GetType() == SMDSAbs_Node) {
                    for (; anElemsIter != anElements.end(); anElemsIter++) {
                        ...
                        switch (aShapeType) {
                            case TopAbs_FACE: myMesh->SetNodeOnFace(node, Id); break;
                            case TopAbs_EDGE: myMesh->SetNodeOnEdge(node, Id); break;
                            case TopAbs_VERTEX: myMesh->SetNodeOnVertex(node, Id); break;
                            default: myMesh->SetNodeInVolume(node, Id);
                        }
                    }
                }
                else {
 ...
 * \retval bool  - true if successful
 */
bool DriverMED::checkFamilyID(DriverMED_FamilyPtr &aFamily, int anID,
                              const TID2FamilyMap &myFamilies)
{
    if (!aFamily || aFamily->GetId() != anID) {
        ...
        if (i_fam == myFamilies.end()) return false;
 ...
 */
bool DriverMED::buildMeshGrille(const MED::PWrapper &theWrapper, const MED::PMeshInfo &theMeshInfo,
                                SMESHDS_Mesh *myMesh, const TID2FamilyMap &myFamilies)
{
    bool res = true;
        ...
        switch (aGrilleInfo->GetGeom()) {
            case MED::eSEG2:
                if (aNodeIds.size() != 2) {
                    ...
                    EXCEPTION(
                        runtime_error,
                        "buildMeshGrille Error. Incorrect size of ids 2!=" << aNodeIds.size());
                }
                anElement = myMesh->AddEdgeWithID(aNodeIds[0] + 1, aNodeIds[1] + 1, iCell + 1);
                break;
            case MED::eQUAD4:
                if (aNodeIds.size() != 4) {
                    res = false;
                    EXCEPTION(
                        runtime_error,
                        "buildMeshGrille Error. Incorrect size of ids 4!=" << aNodeIds.size());
                }
                anElement = myMesh->AddFaceWithID(aNodeIds[0] + 1, aNodeIds[2] + 1, aNodeIds[3] + 1,
                                                  aNodeIds[1] + 1, iCell + 1);
                break;
            case MED::eHEXA8:
                if (aNodeIds.size() != 8) {
                    res = false;
                    EXCEPTION(
                        runtime_error,
                        "buildMeshGrille Error. Incorrect size of ids 8!=" << aNodeIds.size());
                }
                anElement = myMesh->AddVolumeWithID(
                    aNodeIds[0] + 1, aNodeIds[2] + 1, aNodeIds[3] + 1, aNodeIds[1] + 1,
                    aNodeIds[4] + 1, aNodeIds[6] + 1, aNodeIds[7] + 1, aNodeIds[5] + 1, iCell + 1);
                break;
            default: break;
        }
        if (!anElement) {

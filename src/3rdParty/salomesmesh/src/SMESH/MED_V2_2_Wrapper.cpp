...
namespace MED
{
template<> TInt GetDESCLength<eV2_2>() { return 200; }

template<> TInt GetIDENTLength<eV2_2>() { return 8; }

template<> TInt GetNOMLength<eV2_2>() { return 64; }

template<> TInt GetLNOMLength<eV2_2>() { return 80; }

template<> TInt GetPNOMLength<eV2_2>() { return 16; }

template<> void GetVersionRelease<eV2_2>(TInt &majeur, TInt &mineur, TInt &release)
{
...
template<> TInt GetNbConn<eV2_2>(EGeometrieElement typmai, EEntiteMaillage typent, TInt mdim)
{
    return typmai % 100;
}
...
public:
    TFile(const std::string &theFileName) : myCount(0), myFid(0), myFileName(theFileName) {}
    ...
    void Open(EModeAcces theMode, TErr *theErr = NULL)
    {
        ...
        if (theErr) *theErr = TErr(myFid);
        else if (myFid < 0)
            EXCEPTION(std::runtime_error,
                      "TFile - MEDfileOpen('" << myFileName << "'," << theMode << ")");
    }

    const TIdt &Id() const
    {
        if (myFid < 0) EXCEPTION(std::runtime_error, "TFile - GetFid() < 0");
        return myFid;
    }

    void Close()
    {
        if (--myCount == 0) MEDfileClose(myFid);
    }
...
public:
    TFileWrapper(const PFile &theFile, EModeAcces theMode, TErr *theErr = NULL) : myFile(theFile)
    {
        myFile->Open(theMode, theErr);
    }

    ~TFileWrapper() { myFile->Close(); }
};


//---------------------------------------------------------------
TVWrapper::TVWrapper(const std::string &theFileName) : myFile(new TFile(theFileName))
{
    TErr aRet;
...
//----------------------------------------------------------------------------
void TVWrapper ::GetMeshInfo(TInt theMeshId, MED::TMeshInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) return;
    ...
    TErr aRet = MEDmeshInfo(myFile->Id(), theMeshId, &aMeshName, &aSpaceDim, &aDim, &aType,
                            &theInfo.myDesc[0], dtunit, &sorttype, &nstep, &at, axisname, axisunit);
    delete[] axisname;
    delete[] axisunit;
    if (aRet < 0) EXCEPTION(std::runtime_error, "GetMeshInfo - MEDmeshInfo(...)");
}


//----------------------------------------------------------------------------
void TVWrapper ::SetMeshInfo(const MED::TMeshInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    ...
    TErr aRet = MEDmeshCr(myFile->Id(), &aMeshName, aSpaceDim, aDim, aType, &aDesc, "",
                          MED_SORT_DTIT, MED_CARTESIAN, nam, unit);
    delete[] nam;
    delete[] unit;
    ...
    INITMSG(MYDEBUG,
            "TVWrapper::SetMeshInfo - MED_MODE_ACCES = " << theMode << "; aRet = " << aRet
                                                         << std::endl);

    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetMeshInfo(const MED::TMeshInfo &theInfo, TErr *theErr)
{
    TErr aRet;
    SetMeshInfo(theInfo, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetMeshInfo(theInfo, eLECTURE_AJOUT, &aRet);

    if (aRet < 0) SetMeshInfo(theInfo, eCREATION, &aRet);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbFamilies(const MED::TMeshInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbFamAttr(TInt theFamId, const MED::TMeshInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbFamGroup(TInt theFamId, const MED::TMeshInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
//----------------------------------------------------------------------------
void TVWrapper ::GetFamilyInfo(TInt theFamId, MED::TFamilyInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet = MEDfamily23Info(myFile->Id(), &aMeshName, theFamId, &aFamilyName, &anAttrId,
                                &anAttrVal, &anAttrDesc, &aFamilyId, &aGroupNames);

    if (theErr) *theErr = aRet;
    else if (aRet < 0)
        EXCEPTION(std::runtime_error,
                  "GetFamilyInfo - MEDfamily23Info(...) - "
                      << " aMeshInfo.myName = '" << &aMeshName << "'; theFamId = " << theFamId
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFamilyInfo(const MED::TFamilyInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet =
        MEDfamilyCr(myFile->Id(), &aMeshName, &aFamilyName, aFamilyId, aNbGroup, &aGroupNames);

    INITMSG(MYDEBUG,
            "TVWrapper::SetFamilyInfo - MED_MODE_ACCES = " << theMode << "; aRet = " << aRet
                                                           << std::endl);
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFamilyInfo(const MED::TFamilyInfo &theInfo, TErr *theErr)
{
    TErr aRet;
    SetFamilyInfo(theInfo, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetFamilyInfo(theInfo, eLECTURE_AJOUT, &aRet);

    if (theErr) *theErr = aRet;
}

//----------------------------------------------------------------------------
void TVWrapper ::GetNames(TElemInfo &theInfo, TInt theNb, EEntiteMaillage theEntity,
                          EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) return;

    if (theGeom == eBALL) theGeom = GetBallGeom(theInfo.myMeshInfo);
    ...
    TErr aRet = MEDmeshEntityNameRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                                    &anElemNames);
...
//----------------------------------------------------------------------------
void TVWrapper ::GetNumeration(TElemInfo &theInfo, TInt theNb, EEntiteMaillage theEntity,
                               EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet = MEDmeshEntityNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity,
                                      aGeom, &anElemNum);
...
//----------------------------------------------------------------------------
void TVWrapper ::GetFamilies(TElemInfo &theInfo, TInt theNb, EEntiteMaillage theEntity,
                             EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet = MEDmeshEntityFamilyNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                            anEntity, aGeom, &aFamNum);
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNames(const TElemInfo &theInfo, EEntiteMaillage theEntity,
                          EGeometrieElement theGeom, TErr *theErr)
{
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNames(const TElemInfo &theInfo, EModeAcces theMode, EEntiteMaillage theEntity,
                          EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    if (theInfo.myIsElemNames) {
        ...
        aRet = MEDmeshEntityNameWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                                   (TInt)anInfo.myElemNames->size(), &anElemNames);
        if (theErr) *theErr = aRet;
        else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNumeration(const TElemInfo &theInfo, EEntiteMaillage theEntity,
                               EGeometrieElement theGeom, TErr *theErr)
{
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNumeration(const TElemInfo &theInfo, EModeAcces theMode,
                               EEntiteMaillage theEntity, EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    if (theInfo.myIsElemNum) {
        ...
        aRet = MEDmeshEntityNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity,
                                     aGeom, (TInt)anInfo.myElemNum->size(), &anElemNum);
        if (theErr) *theErr = aRet;
        else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFamilies(const TElemInfo &theInfo, EEntiteMaillage theEntity,
                             EGeometrieElement theGeom, TErr *theErr)
{
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFamilies(const TElemInfo &theInfo, EModeAcces theMode,
                             EEntiteMaillage theEntity, EGeometrieElement theGeom, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet =
        MEDmeshEntityFamilyNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbNodes(const MED::TMeshInfo &theMeshInfo, ETable theTable, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    med_bool chgt, trsf;
    return MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE, MED_NO_GEOTYPE,
                          aTable, MED_NO_CMODE, &chgt, &trsf);
}


//----------------------------------------------------------------------------
void TVWrapper ::GetNodeInfo(MED::TNodeInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet = MEDmeshNodeCoordinateRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, aModeSwitch,
                                        &aCoord);

    TErr aRet2 = MEDmeshEntityFamilyNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                             MED_NODE, MED_NO_GEOTYPE, &aFamNum);
    if (aRet2 < 0) {
    ...
    if (MEDmeshEntityNameRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                            MED_NO_GEOTYPE, &anElemNames)
        < 0)
        theInfo.myIsElemNames = eFAUX;

    if (MEDmeshEntityNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                              MED_NO_GEOTYPE, &anElemNum)
        < 0)
        theInfo.myIsElemNum = eFAUX;
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNodeInfo(const MED::TNodeInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet = MEDmeshNodeCoordinateWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NO_DT,
                                        aModeSwitch, aNbElem, &aCoord);

    MEDmeshEntityFamilyNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                                MED_NO_GEOTYPE, aNbElem, &aFamNum);
    if (anIsElemNames)
        MEDmeshEntityNameWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                            MED_NO_GEOTYPE, aNbElem, &anElemNames);
    if (anIsElemNum)
        MEDmeshEntityNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                              MED_NO_GEOTYPE, aNbElem, &anElemNum);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetNodeInfo(const MED::TNodeInfo &theInfo, TErr *theErr)
{
    TErr aRet;
    SetNodeInfo(theInfo, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetNodeInfo(theInfo, eLECTURE_AJOUT, &aRet);
...
//-----------------------------------------------------------------
void TVWrapper ::GetPolygoneInfo(MED::TPolygoneInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    aRet = -1;
#else
    aRet = MEDmeshPolygon2Rd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
...
//----------------------------------------------------------------------------
void TVWrapper ::SetPolygoneInfo(const MED::TPolygoneInfo &theInfo, TErr *theErr)
{
    SetPolygoneInfo(theInfo, eLECTURE_ECRITURE, theErr);
}

//----------------------------------------------------------------------------
void TVWrapper ::SetPolygoneInfo(const MED::TPolygoneInfo &theInfo, EModeAcces theMode,
                                 TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
...
#else
    TErr aRet =
        MEDmeshPolygon2Wr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT, anEntity,
                          aGeom, aConnMode, anInfo.myNbElem + 1, &anIndex, &aConn);
#endif
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbPolygones(const MED::TMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                EGeometrieElement theGeom, EConnectivite theConnMode, TErr *theErr)
{
    return GetNbCells(theMeshInfo, theEntity, theGeom, theConnMode, theErr);
}

//----------------------------------------------------------------------------
TInt TVWrapper ::GetPolygoneConnSize(const MED::TMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                     EGeometrieElement theGeom, EConnectivite theConnMode,
                                     TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) return 0;
    ...
    med_int aTaille = 0;
    med_bool chgt, trsf;
    aTaille = MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                             med_entity_type(theEntity), med_geometry_type(theGeom),
                             MED_CONNECTIVITY, med_connectivity_mode(theConnMode), &chgt, &trsf);


    if (aTaille < 0) EXCEPTION(std::runtime_error, "GetPolygoneInfo - MEDmeshnEntity(...)");

    return TInt(aTaille);
}

//-----------------------------------------------------------------
void TVWrapper ::GetPolyedreInfo(TPolyedreInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet;
    aRet = MEDmeshPolyhedronRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_CELL, aConnMode,
                               &anIndex, &aFaces, &aConn);
...
//----------------------------------------------------------------------------
void TVWrapper ::SetPolyedreInfo(const TPolyedreInfo &theInfo, TErr *theErr)
{
    SetPolyedreInfo(theInfo, eLECTURE_ECRITURE, theErr);
}

//----------------------------------------------------------------------------
void TVWrapper ::SetPolyedreInfo(const MED::TPolyedreInfo &theInfo, EModeAcces theMode,
                                 TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet;
    aRet = MEDmeshPolyhedronWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT,
                               MED_CELL, aConnMode, anInfo.myNbElem + 1, &anIndex,
                               (TInt)anInfo.myFaces->size(), &aFaces, &aConn);
    ...
    if (theInfo.myIsElemNames) {
        ...
        aRet = MEDmeshEntityNameWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity,
                                   MED_POLYHEDRON, (TInt)anInfo.myElemNames->size(), &anElemNames);
        if (theErr) *theErr = aRet;
        else if (aRet < 0)
    ...
    if (theInfo.myIsElemNum) {
        TValueHolder<TElemNum, med_int> anElemNum(anInfo.myElemNum);
        aRet = MEDmeshEntityNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity,
                                     MED_POLYHEDRON, (TInt)anInfo.myElemNum->size(), &anElemNum);
        if (theErr) *theErr = aRet;
        else if (aRet < 0)
    ...
    TValueHolder<TElemNum, med_int> aFamNum(anInfo.myFamNum);
    aRet = MEDmeshEntityFamilyNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity,
                                       MED_POLYHEDRON, (TInt)anInfo.myFamNum->size(), &aFamNum);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbPolyedres(const MED::TMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                EGeometrieElement theGeom, EConnectivite theConnMode, TErr *theErr)
{
    return GetNbCells(theMeshInfo, theEntity, theGeom, theConnMode, theErr);
}
...
//----------------------------------------------------------------------------
void TVWrapper ::GetPolyedreConnSize(const TMeshInfo &theMeshInfo, TInt &theNbFaces,
                                     TInt &theConnSize, EConnectivite theConnMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) EXCEPTION(std::runtime_error, "GetPolyedreConnSize - (...)");
    ...
    med_bool chgt, trsf;
    theNbFaces = MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_CELL,
                                MED_POLYHEDRON, MED_INDEX_NODE, aConnMode, &chgt, &trsf);

    theConnSize = MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_CELL,
                                 MED_POLYHEDRON, MED_CONNECTIVITY, aConnMode, &chgt, &trsf);
...
//-----------------------------------------------------------------
TEntityInfo TVWrapper ::GetEntityInfo(const MED::TMeshInfo &theMeshInfo, EConnectivite theConnMode,
                                      TErr *theErr)
{
    TEntityInfo anInfo;
    ...
    if (theErr && *theErr < 0) return anInfo;
    ...
    if (theMeshInfo.GetType() == eNON_STRUCTURE) {
                    ...
                    if (aNbElem > 0) {
                        if (anEntity == eSTRUCT_ELEMENT) {
                            ...
                            for (TInt structType = 0; structType < nbStructTypes; ++structType) {
                                ...
                                TValueHolder<TString, char> aMeshName(
                                    (TString &)theMeshInfo.myName);
                                ...
                                if (strcmp(geotypename, MED_BALL_NAME) == 0) {
                                    aNbElem =
                                        GetNbCells(theMeshInfo, anEntity,
                                                   EGeometrieElement(geotype), theConnMode, theErr);
                                    if (aNbElem > 0)
        ...
        if (aGrilleType == eGRILLE_STANDARD) { GetGrilleStruct(theMeshInfo, aStruct, theErr); }
        ...
        switch (aDim) {
            case 1: aGeom = eSEG2; break;
            case 2:
                aGeom = eQUAD4;
                aSubGeom = eSEG2;
                aSubEntity = eARETE;
                aNbSub = (aStruct[0]) * (aStruct[1] - 1) + (aStruct[0] - 1) * (aStruct[1]);
                break;
            case 3:
                aGeom = eHEXA8;
                aSubGeom = eQUAD4;
                aSubEntity = eFACE;
                aNbSub = (aStruct[0]) * (aStruct[1] - 1) * (aStruct[2] - 1)
        ...
        if (aDim > 1) anInfo[aSubEntity][aSubGeom] = aNbSub;
    }
    return anInfo;
}
...
//-----------------------------------------------------------------
TInt TVWrapper::GetNbCells(const MED::TMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                           EGeometrieElement theGeom, EConnectivite theConnMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    switch (theGeom) {
        case MED::ePOLYGONE:
        case MED::ePOLYGON2: {
            return MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                  med_entity_type(theEntity), med_geometry_type(theGeom),
                                  MED_INDEX_NODE, med_connectivity_mode(theConnMode), &chgt, &trsf)
                - 1;
        }
        case MED::ePOLYEDRE: {
            return MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                  med_entity_type(theEntity), MED_POLYHEDRON, MED_INDEX_FACE,
        ...
        default: {
            return MEDmeshnEntity(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                  med_entity_type(theEntity), med_geometry_type(theGeom),
                                  MED_CONNECTIVITY, med_connectivity_mode(theConnMode), &chgt,
                                  &trsf);
        }
    }
    return 0;
...
void TVWrapper::GetCellInfo(MED::TCellInfo &theInfo, TErr *theErr)
{
    ...
    TErr aRet;
    aRet = MEDmeshElementRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                            aConnMode, aModeSwitch, &aConn, &anIsElemNames, &anElemNames,
                            &anIsElemNum, &anElemNum, &anIsFamNum, &aFamNum);
...
//----------------------------------------------------------------------------
void TVWrapper ::SetCellInfo(const MED::TCellInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet;
    aRet =
        MEDmeshElementConnectivityWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT,
                                     anEntity, aGeom, aConnMode, aModeSwitch, aNbElem, &aConn);

    MEDmeshEntityFamilyNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                                aNbElem, &aFamNum);
    if (anIsElemNames)
        MEDmeshEntityNameWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                            aNbElem, &anElemNames);
    if (anIsElemNum)
        MEDmeshEntityNumberWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, anEntity, aGeom,
                              aNbElem, &anElemNum);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetCellInfo(const MED::TCellInfo &theInfo, TErr *theErr)
{
...
void TVWrapper::GetBallInfo(TBallInfo &theInfo, TErr *theErr)
{
    ...
    if (theInfo.myGeom == eBALL) {
        ...
        if (theInfo.myGeom < 0) {
            if (!theErr) EXCEPTION(std::runtime_error, "GetBallInfo - no balls in the mesh");
    ...
    TErr aRet = MEDmeshStructElementVarAttRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, aGeom,
                                             varattname, &aDiam);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
void TVWrapper::SetBallInfo(const TBallInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    ...
    if (ballGeom < 0) {
        ...
        if ((ret = MEDsupportMeshCr(myFile->Id(), ballsupportname,
                                    theInfo.myMeshInfo->GetSpaceDim(), theInfo.myMeshInfo->GetDim(),
                                    "Support mesh for a ball model", MED_CARTESIAN,
                                    /*axisname=*/dummyname, /*unitname=*/dummyname))
            < 0) {
            if (!theErr) EXCEPTION(std::runtime_error, "SetBallInfo - MEDsupportMeshCr");
            *theErr = ret;
            return;
        }
        // write coordinates of 1 node
        med_float coord[3] = {0, 0, 0};
        if ((ret = MEDmeshNodeCoordinateWr(myFile->Id(), ballsupportname, MED_NO_DT, MED_NO_IT, 0.0,
                                           MED_FULL_INTERLACE, /*nnode=*/1, coord))
            < 0) {
            if (!theErr) EXCEPTION(std::runtime_error, "SetBallInfo - MEDmeshNodeCoordinateWr");
            *theErr = ret;
            return;
        }
        // ball model creation
        char geotypename[MED_NAME_SIZE + 1] = MED_BALL_NAME;
        if ((ballGeom = (EGeometrieElement)MEDstructElementCr(myFile->Id(), geotypename,
                                                              theInfo.myMeshInfo->GetSpaceDim(),
                                                              ballsupportname, MED_NODE, MED_NONE))
            < 0) {
            if (!theErr) EXCEPTION(std::runtime_error, "SetBallInfo - MEDstructElementCr");
            *theErr = ret;
            return;
        }
        // create diameter attribute
        if ((ret = MEDstructElementVarAttCr(myFile->Id(), geotypename, MED_BALL_DIAMETER,
                                            MED_ATT_FLOAT64, /*ncomp=*/1))
            < 0) {
            if (!theErr) EXCEPTION(std::runtime_error, "SetBallInfo - MEDstructElementVarAttCr");
            *theErr = ret;
            return;
        }
    } // ballGeom < 0
    ...
    ret = MEDmeshStructElementVarAttWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, aGeom,
                                       MED_BALL_DIAMETER, theInfo.myNbElem, &aDiam);
    if (theErr) *theErr = ret;
    else if (ret < 0)
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbComp(TInt theFieldId, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
//----------------------------------------------------------------------------
void TVWrapper ::GetFieldInfo(TInt theFieldId, MED::TFieldInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    aRet = MEDfieldInfo(myFile->Id(), theFieldId, &aFieldName[0], local_mesh_name, &local, &aType,
                        &aCompNames, &anUnitNames, dtunit, &nbofstp);
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFieldInfo(const MED::TFieldInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    aRet = MEDfieldCr(myFile->Id(), &aFieldName, aType, anInfo.myNbComp, &aCompNames, &anUnitNames,
                      dtunit, &aMeshInfo.myName[0]);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
//----------------------------------------------------------------------------
void TVWrapper ::SetFieldInfo(const MED::TFieldInfo &theInfo, TErr *theErr)
{
    TErr aRet;
    SetFieldInfo(theInfo, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetFieldInfo(theInfo, eLECTURE_AJOUT, &aRet);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbGauss(TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
//----------------------------------------------------------------------------
TGaussInfo::TInfo TVWrapper ::GetGaussPreInfo(TInt theId, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) return TGaussInfo::TInfo(TGaussInfo::TKey(ePOINT1, ""), 0);
    ...
    aRet = MEDlocalizationInfo(myFile->Id(), theId, &aName[0], &aGeom, &dim, &aNbGaussPoints,
...
//----------------------------------------------------------------------------
void TVWrapper ::GetGaussInfo(TInt theId, TGaussInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet;
    aRet = MEDlocalizationRd(myFile->Id(), &aGaussName, aModeSwitch, &aRefCoord, &aGaussCoord,
                             &aWeight);
...
//----------------------------------------------------------------------------
TInt TVWrapper ::GetNbProfiles(TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
...
TProfileInfo::TInfo TVWrapper ::GetProfilePreInfo(TInt theId, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) return TProfileInfo::TInfo();

    med_int aSize = -1;
    TVector<char> aName(GetNOMLength<eV2_2>() + 1);

    TErr aRet;
    aRet = MEDprofileInfo(myFile->Id(), theId, &aName[0], &aSize);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
...
void TVWrapper ::GetProfileInfo(TInt theId, TProfileInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    TErr aRet;
    aRet = MEDprofileRd(myFile->Id(), &aProfileName, &anElemNum);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
        EXCEPTION(std::runtime_error, "GetProfileInfo - MEDprofileRd(...)");
}

void TVWrapper ::SetProfileInfo(const TProfileInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
...
void TVWrapper ::SetProfileInfo(const TProfileInfo &theInfo, TErr *theErr)
{
    TErr aRet;
    SetProfileInfo(theInfo, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetProfileInfo(theInfo, eLECTURE_AJOUT, &aRet);

    if (aRet < 0) SetProfileInfo(theInfo, eCREATION, &aRet);
...
TInt TVWrapper ::GetNbTimeStamps(const MED::TFieldInfo &theInfo,
                                 const MED::TEntityInfo &theEntityInfo, EEntiteMaillage &theEntity,
                                 TGeom2Size &theGeom2Size, TErr *theErr)
{
    theEntity = EEntiteMaillage(-1);
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr) {
        if (theEntityInfo.empty()) *theErr = -1;
        if (*theErr < 0) return -1;
    }
    ...
    for (; anIter != localEntityInfo.end(); anIter++) {
        ...
        for (; anGeomIter != aGeom2Size.end(); anGeomIter++) {
            ...
            med_int aNbStamps;
            MEDfieldInfoByName(anId, &aFieldName, aMeshName, &islocal, &ft, cname, unitname, dtunit,
                               &aNbStamps);
            delete[] cname;
            delete[] unitname;
            med_int nval = 0;
            ...
            med_float aDt;
            if (aNbStamps > 0) {
                MEDfieldComputingStepInfo(anId, &aFieldName, 1, &aNumDt, &aNumOrd, &aDt);
                ...
                if (anEntity == MED_NODE_ELEMENT && aGeom % 100 == 0) continue;

                nval = MEDfieldnValueWithProfile(anId, &aFieldName, aNumDt, aNumOrd, anEntity,
                                                 med_geometry_type(aGeom), 1, MED_COMPACT_STMODE,
                                                 profilename, &profilsize, locname, &aNbGauss);
            }
            bool anIsSatisfied = (nval > 0);
            if (anIsSatisfied) {
                INITMSG(MYDEBUG,
                        "GetNbTimeStamps aNbTimeStamps = " << aNbStamps << "; aGeom = " << aGeom
                                                           << "; anEntity = " << anEntity << "\n");
...
//----------------------------------------------------------------------------
void TVWrapper ::GetTimeStampInfo(TInt theTimeStampId, MED::TTimeStampInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    med_int aNbStamps;
    MEDfieldInfoByName(myFile->Id(), &aFieldName, &aMeshName, &anIsLocal, &aFieldType, aCompName,
                       aCompUnit, &anUnitDt, &aNbStamps);
    delete[] aCompName;
    delete[] aCompUnit;
    ...
    for (; anIter != aGeom2Size.end(); anIter++) {
        ...
        med_int profilsize;
        char locname[MED_NAME_SIZE + 1];
        MEDfieldnValueWithProfile(myFile->Id(), &aFieldName, aNumDt, aNumOrd, anEntity,
                                  med_geometry_type(aGeom), 1, MED_COMPACT_STMODE, profilename,
                                  &profilsize, locname, &aNbGauss);

        static TInt MAX_NB_GAUSS_POINTS = 32;
        if (aNbGauss <= 0 || aNbGauss > MAX_NB_GAUSS_POINTS) aNbGauss = 1;
...
//----------------------------------------------------------------------------
void TVWrapper ::GetTimeStampValue(const PTimeStampValueBase &theTimeStampValue,
                                   const TMKey2Profile &theMKey2Profile,
                                   const TKey2Gauss &theKey2Gauss, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    for (; anIter != aGeom2Size.end(); anIter++) {
        ...
        TInt aNbVal = MEDfieldnValueWithProfile(
            anId, &aFieldName, aNumDt, aNumOrd, anEntity, med_geometry_type(aGeom), 1, aProfileMode,
            &aProfileName[0], &profilesize, &aGaussName[0], &aNbGauss);
        ...
        if (aNbVal <= 0) {
            ...
            EXCEPTION(std::runtime_error,
                      "GetTimeStampValue - MEDfieldnValueWithProfile(...) - aNbVal == " << aNbVal
                                                                                        << " <= 0");
        }

        TInt aNbComp = aFieldInfo->myNbComp;
        TInt aNbValue = aNbVal; // / aNbGauss; rules in MED changed
        theTimeStampValue->AllocateValue(aGeom, aNbValue, aNbGauss, aNbComp);
        TInt aValueSize = theTimeStampValue->GetValueSize(aGeom);

        INITMSG(MYDEBUG,
                "TVWrapper::GetTimeStampValue - aGeom = "
                    << aGeom << "; aNbVal = " << aNbVal << "; aNbValue = " << aNbValue
                    << "; aNbGauss = " << aNbGauss << "; aNbComp = " << aNbComp << std::endl);

        TErr aRet = MEDfieldValueWithProfileRd(anId, &aFieldName, aNumDt, aNumOrd, anEntity,
                                               med_geometry_type(aGeom), aProfileMode,
                                               &aProfileName[0], aModeSwitch, MED_ALL_CONSTITUENT,
        ...
        if (aGaussInfo && aNbGauss != aGaussInfo->GetNbGauss()) {
            ...
            EXCEPTION(std::runtime_error,
                      "GetTimeStampValue - aNbGauss != aGaussInfo->GetNbGauss()");
        }
        ...
        if (aProfileInfo && aProfileInfo->IsPresent()) {
            ...
            if (aProfileSize != aValueSize) {
                ...
                EXCEPTION(std::runtime_error,
                          "GetTimeStampValue - aProfileSize("
                              << aProfileSize << ") != aValueSize(" << aValueSize << "); aNbVal = "
                              << aNbVal << "; anEntity = " << anEntity << "; aGeom = " << aGeom
                              << "; aNbElem = " << aNbElem << "; aNbSubElem = " << aNbSubElem
                              << "; aNbComp = " << aNbComp << "; aNbGauss = " << aNbGauss << "");
            }
        }
        else {
            if ((aProfileMode == MED_GLOBAL_STMODE) && (aNbElem != aNbValue)) {
                ...
                EXCEPTION(std::runtime_error,
                          "GetTimeStampValue - aNbElem("
                              << aNbElem << ") != aNbValue(" << aNbValue << "); aNbVal = " << aNbVal
                              << "; anEntity = " << anEntity << "; aGeom = " << aGeom
                              << "; aNbElem = " << aNbElem << "; aNbComp = " << aNbComp
                              << "; aNbGauss = " << aNbGauss << "");
            }
        }
    }
...
//----------------------------------------------------------------------------
void TVWrapper ::SetTimeStampValue(const MED::PTimeStampValueBase &theTimeStampValue,
                                   EModeAcces theMode, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    for (; anIter != aGeomSet.end(); anIter++) {
        ...
        aRet = MEDfieldValueWithProfileWr(anId, &aFieldName, aNumDt, aNumOrd, aDt, anEntity,
                                          med_geometry_type(aGeom), aProfileMode, &aProfileName[0],
                                          &aGaussName[0], aModeSwitch, MED_ALL_CONSTITUENT, aNbVal,
                                          theTimeStampValue->GetValuePtr(aGeom));
        if (aRet < 0) {
            if (theErr) {
                *theErr = MED_FALSE;
    ...
    INITMSG(MYDEBUG,
            "TVWrapper::SetTimeStampValue - MED_MODE_ACCES = " << theMode << "; aRet = " << aRet
                                                               << std::endl);
}


//----------------------------------------------------------------------------
void TVWrapper ::SetTimeStampValue(const PTimeStampValueBase &theTimeStampValue, TErr *theErr)
{
    TErr aRet;
    SetTimeStampValue(theTimeStampValue, eLECTURE_ECRITURE, &aRet);

    if (aRet < 0) SetTimeStampValue(theTimeStampValue, eLECTURE_AJOUT, &aRet);

    if (theErr) *theErr = aRet;
}

//----------------------------------------------------------------------------
void TVWrapper ::SetGrilleInfo(const MED::TGrilleInfo &theInfo, TErr *theErr)
{
    SetGrilleInfo(theInfo, eLECTURE_ECRITURE, theErr);
}

//----------------------------------------------------------------------------
void TVWrapper ::SetGrilleInfo(const MED::TGrilleInfo &theInfo, EModeAcces theMode, TErr *theErr)
{
    if (theInfo.myMeshInfo->myType != eSTRUCTURE) return;
    TFileWrapper aFileWrapper(myFile, theMode, theErr);
    ...
    TErr aRet = 0;
    aRet = MEDmeshGridTypeRd(myFile->Id(), &aMeshName, &aGrilleType);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)
    ...
    if (anInfo.myGrilleType == eGRILLE_STANDARD) {
        ...
        aRet = MEDmeshNodeCoordinateWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT,
                                       aModeSwitch, aNbNoeuds, &aCoord);

        if (aRet < 0) EXCEPTION(std::runtime_error, "SetGrilleInfo - MEDmeshNodeCoordinateWr(...)");

        TValueHolder<TIntVector, med_int> aGrilleStructure(anInfo.myGrilleStructure);
        aRet = MEDmeshGridStructWr(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT,
                                   &aGrilleStructure);
        if (aRet < 0) EXCEPTION(std::runtime_error, "SetGrilleInfo - MEDmeshGridStructWr(...)");
    }
    else {
        for (med_int aAxis = 0; aAxis < aMeshInfo.myDim; aAxis++) {
            aRet = MEDmeshGridIndexCoordinateWr(
                myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_UNDEF_DT, aAxis + 1,
                anInfo.GetIndexes(aAxis).size(), &anInfo.GetIndexes(aAxis)[0]);
...
//----------------------------------------------------------------------------
void TVWrapper ::GetGrilleInfo(TGrilleInfo &theInfo, TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    if (aMaillageType == eSTRUCTURE && aGrilleType == eGRILLE_STANDARD) {
        ...
        aRet = MEDmeshNodeCoordinateRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, aModeSwitch,
                                       &aCoord);
        ...
        aRet = MEDmeshEntityFamilyNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, MED_NODE,
                                           MED_NO_GEOTYPE, &aFamNumNode);
    ...
    if (aMaillageType == eSTRUCTURE && aGrilleType != eGRILLE_STANDARD) {
        ...
        for (med_int anAxis = 1; anAxis <= aMeshInfo.myDim; anAxis++) {
            switch (anAxis) {
                case 1: aTable = eCOOR_IND1; break;
                case 2: aTable = eCOOR_IND2; break;
                case 3: aTable = eCOOR_IND3; break;
                default: aRet = -1;
            }
            ...
            if (aNbIndexes < 0)
                EXCEPTION(std::runtime_error,
                          "GetGrilleInfo - Erreur a la lecture de la taille de l'indice");
            ...
            aRet = MEDmeshGridIndexCoordinateRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                                anAxis, &anIndexes);
    ...
    aRet =
        MEDmeshEntityFamilyNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                    med_entity_type(aEntity), med_geometry_type(aGeom), &aFamNum);
    ...
    if (aMeshInfo.myDim == 3) {
        ...
        aRet = MEDmeshEntityFamilyNumberRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT,
                                           med_entity_type(aEntity), med_geometry_type(aGeom),
                                           &aFamNum);
    }
    if (aRet < 0) {
        //          if (aRet == MED_ERR_DOESNTEXIST) // --- only valid with MED3.x files
...
void TVWrapper ::GetGrilleType(const MED::TMeshInfo &theMeshInfo, EGrilleType &theGridType,
                               TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);

    if (theErr && *theErr < 0) EXCEPTION(std::runtime_error, " GetGrilleType - aFileWrapper (...)");
    ...
    if (aMeshInfo.myType == eSTRUCTURE) {
        ...
        TErr aRet = MEDmeshGridTypeRd(myFile->Id(), &aMeshName, &aGridType);

        if (aRet < 0) EXCEPTION(std::runtime_error, "GetGrilleInfo - MEDmeshGridTypeRd(...)");
    }
}

void TVWrapper ::GetGrilleStruct(const MED::TMeshInfo &theMeshInfo, TIntVector &theStruct,
                                 TErr *theErr)
{
    TFileWrapper aFileWrapper(myFile, eLECTURE, theErr);
    ...
    aRet = MEDmeshGridStructRd(myFile->Id(), &aMeshName, MED_NO_DT, MED_NO_IT, &aGridStructure);
    if (theErr) *theErr = aRet;
    else if (aRet < 0)

...
bool SMESH_Algo::Features::IsCompatible(const SMESH_Algo::Features &algo2) const
{
    ...
    if (_outElemTypes.empty() || algo2._inElemTypes.empty()) return false;
        ...
        for (size_t iXML = 0; iXML < xmlPaths.size(); ++iXML) {
            ...
            for (int i = 0; i < algoNodeList.getLength(); ++i) {
                ...
                for (int isInput = 0; isInput < 2; ++isInput) {
                    ...
                    set<SMDSAbs_GeometryType> &typeSet =
                        isInput ? data._inElemTypes : data._outElemTypes;
                    int beg = 1, end;
                    while (beg <= typeStr.Length()) {
                        while (beg < typeStr.Length() && !isalpha(typeStr.Value(beg))) ++beg;
                        end = beg;
                        while (end < typeStr.Length() && isalpha(typeStr.Value(end + 1))) ++end;
                        if (end > beg) {
                            TCollection_AsciiString typeName = typeStr.SubString(beg, end);
                            if (typeName == "EDGE") typeSet.insert(SMDSGeom_EDGE);
                            else if (typeName == "TRIA")
                                typeSet.insert(SMDSGeom_TRIANGLE);
                            else if (typeName == "QUAD")
                                typeSet.insert(SMDSGeom_QUADRANGLE);
                        }
                        beg = end + 1;
                    }
                }
    ...
    : SMESH_Hypothesis(hypId, studyId, gen)
{
    ...
    _error = COMPERR_OK;
    for (int i = 0; i < 4; ++i) _neededLowerHyps[i] = false;
}
...
const vector<string> &SMESH_Algo::GetCompatibleHypothesis() { return _compatibleHypothesis; }
...
SMESH_Algo::GetUsedHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
const list<const SMESHDS_Hypothesis *> &
SMESH_Algo::GetAppliedHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                 const bool ignoreAuxiliary) const
{
    SMESH_Algo *me = const_cast<SMESH_Algo *>(this);
...
bool SMESH_Algo::SetParametersByDefaults(const TDefaults &, const SMESH_Mesh *) { return false; }
//================================================================================
/*!
...
bool SMESH_Algo::GetNodeParamOnEdge(const SMESHDS_Mesh *theMesh, const TopoDS_Edge &theEdge,
                                    vector<double> &theParams)
{
    theParams.clear();

    if (!theMesh || theEdge.IsNull()) return false;

    SMESHDS_SubMesh *eSubMesh = theMesh->MeshElements(theEdge);
    if (!eSubMesh || !eSubMesh->GetElements()->more()) return false; // edge is not meshed
    ...
    if (eSubMesh) {
        ...
        while (nIt->more()) {
            ...
            if (pos->GetTypeOfPosition() != SMDS_TOP_EDGE) return false;
            const SMDS_EdgePosition *epos =
                static_cast<const SMDS_EdgePosition *>(node->GetPosition());
            if (!paramSet.insert(epos->GetUParameter()).second) return false; // equal parameters
        }
    }
    // add vertex nodes params
    TopoDS_Vertex V1, V2;
    TopExp::Vertices(theEdge, V1, V2);
    if (VertexNode(V1, theMesh) && !paramSet.insert(BRep_Tool::Parameter(V1, theEdge)).second)
        return false; // there are equal parameters
    if (VertexNode(V2, theMesh) && !paramSet.insert(BRep_Tool::Parameter(V2, theEdge)).second)
        return false; // there are equal parameters
    ...
    for (; par != paramSet.end(); ++par, ++vecPar) *vecPar = *par;
...
bool SMESH_Algo::GetSortedNodesOnEdge(const SMESHDS_Mesh *theMesh, const TopoDS_Edge &theEdge,
                                      ...
                                      const SMDSAbs_ElementType typeToCheck)
{
    ...
    if (eSubMesh) {
        ...
        while (nIt->more()) {
            const SMDS_MeshNode *node = nIt->next();
            if (ignoreMediumNodes && SMESH_MesherHelper::IsMedium(node, typeToCheck)) continue;
            const SMDS_PositionPtr &pos = node->GetPosition();
            if (pos->GetTypeOfPosition() != SMDS_TOP_EDGE) return false;
            const SMDS_EdgePosition *epos =
                static_cast<const SMDS_EdgePosition *>(node->GetPosition());
    ...
    if (v1.Orientation() != TopAbs_FORWARD) std::swap(f, l);
    if (n1 && ++nbNodes) theNodes.insert(make_pair(f, n1));
    if (n2 && ++nbNodes) theNodes.insert(make_pair(l, n2));
...
GeomAbs_Shape SMESH_Algo::Continuity(TopoDS_Edge E1, TopoDS_Edge E2)
{
    ...
    else if (VV1[0].IsSame(VV2[1])) {
        V = VV1[0];
    }
    else if (VV1[1].IsSame(VV2[1])) {
        V = VV1[1];
        E1.Reverse();
    }
    else if (VV1[0].IsSame(VV2[0])) {
        V = VV1[0];
        E1.Reverse();
    }
    else {
...
bool SMESH_Algo::IsStraight(const TopoDS_Edge &E, const bool degenResult)
{
    {
        double f, l;
        if (BRep_Tool::Curve(E, f, l).IsNull()) return degenResult;
    }
    ...
    switch (curve.GetType()) {
        ...
        case GeomAbs_Parabola: return false;
    ...
    if (v1Len < std::numeric_limits<double>::min()) return false; // E seems closed
...
const SMDS_MeshNode *SMESH_Algo::VertexNode(const TopoDS_Vertex &V, const SMESHDS_Mesh *meshDS)
{
    if (SMESHDS_SubMesh *sm = meshDS->MeshElements(V)) {
        SMDS_NodeIteratorPtr nIt = sm->GetNodes();
        if (nIt->more()) return nIt->next();
    }
    return 0;
}
...
const SMDS_MeshNode *SMESH_Algo::VertexNode(const TopoDS_Vertex &V, const SMESH_Mesh *mesh)
{
    ...
    if (!node && mesh->HasModificationsToDiscard()) {
        ...
        while (const TopoDS_Shape *edge = edgeIt->next())
            if (SMESHDS_SubMesh *edgeSM = mesh->GetMeshDS()->MeshElements(*edge))
                if (edgeSM->NbElements() > 0) return VertexNode(V, edgeSM, mesh, /*checkV=*/false);
    }
    return node;
}
...
const SMDS_MeshNode *SMESH_Algo::VertexNode(const TopoDS_Vertex &V, const SMESHDS_SubMesh *edgeSM,
                                            const SMESH_Mesh *mesh, const bool checkV)
{
    ...
    if (!node && edgeSM) {
        ...
        while (segIt->more()) {
            ...
            if (seg->GetType() != SMDSAbs_Edge) return node;
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (n1->getshapeId() != edgeSM->GetID()) otherShapeNodes.insert(n1);
            }
        }
        ...
        if (notSharedNodes.size() == 2) // two end nodes found
        {
            ...
            if (E.IsNull() || E.ShapeType() != TopAbs_EDGE) return node;
            ...
            if (S1.ShapeType() == TopAbs_VERTEX && SMESH_MesherHelper::IsSubShape(S1, E)) return n2;
            TopoDS_Shape S2 = SMESH_MesherHelper::GetSubShapeByNode(n2, meshDS);
            if (S2.ShapeType() == TopAbs_VERTEX && SMESH_MesherHelper::IsSubShape(S2, E)) return n1;
...
SMESH_Algo::EMeshError SMESH_Algo::GetMeshError(SMESH_subMesh *subMesh)
{
    ...
    if (!smDS) return MEr_EMPTY;

    switch (subMesh->GetSubShape().ShapeType()) {
        case TopAbs_FACE: { // ====================== 2D =====================

            SMDS_ElemIteratorPtr fIt = smDS->GetElements();
            if (!fIt->more()) return MEr_EMPTY;
        ...
        case TopAbs_SOLID: { // ====================== 3D =====================
            ...
            if (!vIt->more()) return MEr_EMPTY;

            SMDS_VolumeTool vTool;
            while (!vIt->more()) {
                if (!vTool.Set(vIt->next())) continue; // strange
...
double SMESH_Algo::GetProgress() const { return _progress; }
...
{
    if (elem) _badInputElements.push_back(elem);
}
...
void SMESH_Algo::addBadInputElements(const SMESHDS_SubMesh *sm, const bool addNodes)
{
    if (sm) {
        if (addNodes) {
            SMDS_NodeIteratorPtr nIt = sm->GetNodes();
...
bool SMESH_2D_Algo::FixInternalNodes(const SMESH_ProxyMesh &mesh, const TopoDS_Face &face)
{
    const SMESHDS_SubMesh *smDS = mesh.GetSubMesh(face);
    if (!smDS || smDS->NbElements() < 1) return false;
    ...
    const SMDS_MeshElement *quad;
    {
        ...
        while (!cornerQuad && fIt->more()) {
            cornerQuad = fIt->next();
            if (cornerQuad->NbCornerNodes() != 4) return false;
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                while (fIt->more()) nbInverseQuads += allFaces.count(fIt->next());
                if (nbInverseQuads == 1) cornerNodeInd = i, i = 4;
            }
            if (cornerNodeInd < 0) cornerQuad = 0;
        }
        if (!cornerQuad || cornerNodeInd < 0) return false;
        ...
        while (quad) {
            ...
            if ((quad =
                     SMESH_MeshAlgos::FindFaceInSet(nodeRows[iRow1].back(), nodeRows[iRow2].back(),
                ...
                if (quad->NbCornerNodes() != 4) return false;
            }
        }
        if (nodeRows[iRow1].size() < 3) return true; // there is nothing to fix
    }
    ...
    while (true) {
        ...
        if ((quad = SMESH_MeshAlgos::FindFaceInSet(nodeRows[iRow1][0], nodeRows[iRow1][1], allFaces,
                                                   /*avoid=*/firstRowQuads, &iN1, &iN2))) {
            if (quad->NbCornerNodes() != 4) return false;
        ...
        while (quad) {
            ...
            if ((quad = SMESH_MeshAlgos::FindFaceInSet(nodeRows[iRow1][nodeRows[iRow2].size() - 1],
                                                       nodeRows[iRow2].back(), allFaces, avoidSet,
                                                       &iN1, &iN2))) {
                if (quad->NbCornerNodes() != 4) return false;
                nodeRows[iRow2].push_back(quad->GetNode(helper.WrapIndex(iN1 + 2, 4)));
            }
        }
        if (nodeRows[iRow1].size() != nodeRows[iRow2].size()) return false;
    }
    if (nodeRows.size() < 3) return true; // there is nothing to fix
    ...
    for (int isBot = 0; isBot < 2; ++isBot) {
        ...
        for (size_t i = 1; i < nodes.size(); ++i) uvps[i].x /= uvps.back().x;
    }
    ...
    for (int isLeft = 0; isLeft < 2; ++isLeft) {
        ...
        for (size_t i = 1; i < nodeRows.size(); ++i) uvps[i].y /= uvps.back().y;
    }

...
bool sortNodes(const SMDS_MeshElement *theTool, const int *idNodes, int theNodeCount, int *myResult)
{
    ...
    for (int i = 1; i <= theNodeCount; i++) BaryCenter += myNodeList.Value(i);
    ...
    for (it = myMap.begin(); it != myMap.end(); ++it) myResult[resID++] = it->second;
...
// Purpose  :
//================================================================
Standard_Boolean SMESH_MeshVSLink::GetGeom(const Standard_Integer ID,
                                           ...
                                           MeshVS_EntityType &Type) const
{
    if (IsElement) {
        ...
        if (myElem->GetType() == SMDSAbs_Edge) Type = MeshVS_ET_Link;
        ...
        if (myNode->GetType() == SMDSAbs_Node) Type = MeshVS_ET_Node;
        else
            Type = MeshVS_ET_0D;
        NbNodes = 1;
        Coords(1) = myNode->X();
                                             ...
                                             Handle(MeshVS_HArray1OfSequenceOfInteger) & Data) const
{
    ...
    if (Data.IsNull()) Data = new MeshVS_HArray1OfSequenceOfInteger(1, NbFaces);
    ...
    for (int itr = 0; itr < NbFaces; itr++) {
        ...
        if (sortNodes(myVolume, FaceIndices, NbThisFaceNodeCount, sortedFaceIndices)) {
            ...
            for (int itrX = 0; itrX < NbThisFaceNodeCount; itrX++) aSeq.Append(FaceIndices[itrX]);
        }
...
// Purpose  :
//================================================================
Standard_Boolean SMESH_MeshVSLink::GetGeomType(const Standard_Integer ID,
                                               const Standard_Boolean IsElement,
                                               MeshVS_EntityType &Type) const
{
    if (IsElement) {
...
// Purpose  :
//================================================================
Standard_Boolean SMESH_MeshVSLink::GetNodesByElement(const Standard_Integer ID,
...
// Purpose  :
//================================================================
const TColStd_PackedMapOfInteger &SMESH_MeshVSLink::GetAllNodes() const { return myNodes; }

//================================================================
// Function : GetAllElements
// Purpose  :
//================================================================
const TColStd_PackedMapOfInteger &SMESH_MeshVSLink::GetAllElements() const { return myElements; }

//================================================================
// Function : GetAllElements
// Purpose  :
//================================================================
void SMESH_MeshVSLink::GetAllGroups(TColStd_PackedMapOfInteger &Ids) const { Ids = myGroups; }
                                             ...
                                             Standard_Real &nz) const
{
    ...
    for (int itr = 0; itr < 3; itr++)
        nodes[itr] =
            gp_XYZ(myElem->GetNode(itr)->X(), myElem->GetNode(itr)->Y(), myElem->GetNode(itr)->Z());

...
// --------------------------------------------------------------------------------
DriverGMF_Read::DriverGMF_Read() : Driver_SMESHDS_Mesh(), _makeRequiredGroups(true) {}
// --------------------------------------------------------------------------------
...
Driver_Mesh::Status DriverGMF_Read::Perform()
{
    ...
    if (nbNodes < 1) return addMessage("No nodes in the mesh", /*fatal=*/true);
    ...
    if (int nbEdges = GmfStatKwd(meshID, GmfEdges)) {
        ...
        if (int nbQuadEdges = GmfStatKwd(meshID, GmfExtraVerticesAtEdges)) {
            ...
            for (int i = 1; i <= nbQuadEdges; ++i) {
                ...
                if (iN[1] >= 1) quadNodesAtEdges[iN[0]] = iN[2];
            }
        }
        // create edges
        ...
        for (int i = 1; i <= nbEdges; ++i) {
            ...
            if (midN > 0) {
                if (!myMesh->AddEdgeWithID(iN[0], iN[1], midN, edgeIDShift + i))
                    status = storeBadNodeIds("GmfEdges + GmfExtraVerticesAtEdges", i, 3, iN[0],
                                             iN[1], midN);
            }
            else {
    ...
    if (int nbTria = GmfStatKwd(meshID, GmfTriangles)) {
        ...
        if (int nbQuadTria = GmfStatKwd(meshID, GmfExtraVerticesAtTriangles)) {
            ...
            for (int i = 1; i <= nbQuadTria; ++i) {
                GmfGetLin(meshID, GmfExtraVerticesAtTriangles, &iN[0], &iN[1], &iN[2], &iN[3],
                          &iN[4],
    ...
    if (int nbQuad = GmfStatKwd(meshID, GmfQuadrilaterals)) {
        ...
        if (int nbQuadQuad = GmfStatKwd(meshID, GmfExtraVerticesAtQuadrilaterals)) {
            ...
            for (int i = 1; i <= nbQuadQuad; ++i) {
                GmfGetLin(meshID, GmfExtraVerticesAtQuadrilaterals, &iN[0], &iN[1], &iN[2], &iN[3],
                          &iN[4], &iN[5], &iN[6]);
                if (iN[0] <= nbQuad) {
        ...
        for (int i = 1; i <= nbQuad; ++i) {
            ...
            {
                if (!myMesh->AddFaceWithID(iN[0], iN[1], iN[2], iN[3], midN[0], midN[1], midN[2],
                                           midN[3], quadIDShift + i))
                    status = storeBadNodeIds("GmfQuadrilaterals + GmfExtraVerticesAtQuadrilaterals",
                                             i, 8, iN[0], iN[1], iN[2], iN[3], midN[0], midN[1],
                                             midN[2], midN[3]);
            }
            else if (midN.size() > 8 - 4) // QUAD9
            {
                if (!myMesh->AddFaceWithID(iN[0], iN[1], iN[2], iN[3], midN[0], midN[1], midN[2],
                                           midN[3], midN[4], quadIDShift + i))
                    status = storeBadNodeIds("GmfQuadrilaterals + GmfExtraVerticesAtQuadrilaterals",
                                             i, 9, iN[0], iN[1], iN[2], iN[3], midN[0], midN[1],
                                             midN[2], midN[3], midN[4]);
            }
            else // QUAD4
            {
    ...
    if (int nbTet = GmfStatKwd(meshID, GmfTetrahedra)) {
        ...
        if (int nbQuadTetra = GmfStatKwd(meshID, GmfExtraVerticesAtTetrahedra)) {
            ...
            for (int i = 1; i <= nbQuadTetra; ++i) {
                GmfGetLin(meshID, GmfExtraVerticesAtTetrahedra, &iN[0], &iN[1], &iN[2], &iN[3],
                          &iN[4], &iN[5], &iN[6], &iN[7]);
                if (iN[0] <= nbTet) {
        ...
        for (int i = 1; i <= nbTet; ++i) {
            ...
            {
                if (!myMesh->AddVolumeWithID(iN[0], iN[2], iN[1], iN[3], midN[2], midN[1], midN[0],
                                             midN[3], midN[5], midN[4], tetIDShift + i))
                    status = storeBadNodeIds("GmfTetrahedra + GmfExtraVerticesAtTetrahedra", i, 10,
                                             iN[0], iN[2], iN[1], iN[3], midN[2], midN[1], midN[0],
                                             midN[3], midN[5], midN[4]);
            }
            else // TETRA4
            {
    ...
    if (int nbHex = GmfStatKwd(meshID, GmfHexahedra)) {
        ...
        for (int i = 1; i <= nbHex; ++i) {
            GmfGetLin(meshID, GmfHexahedra, &iN[0], &iN[1], &iN[2], &iN[3], &iN[4], &iN[5], &iN[6],
                      &iN[7], &ref);
            ...
            if (midN.size() == 20 - 8) // HEXA20
            {
                if (!myMesh->AddVolumeWithID(iN[0], iN[3], iN[2], iN[1], iN[4], iN[7], iN[6], iN[5],
                                             midN[3], midN[2], midN[1], midN[0], midN[7], midN[6],
                                             midN[5], midN[4], midN[8], midN[11], midN[10], midN[9],
                                             hexIDShift + i))
                    status = storeBadNodeIds(
                        "GmfHexahedra + GmfExtraVerticesAtHexahedra", i, 20, iN[0], iN[3], iN[2],
                        iN[1], iN[4], iN[7], iN[6], iN[5], midN[3], midN[2], midN[1], midN[0],
                        midN[7], midN[6], midN[5], midN[4], midN[8], midN[11], midN[10], midN[9]);
            }
            else if (midN.size() >= 27 - 8) // HEXA27
            {
                if (!myMesh->AddVolumeWithID(iN[0], iN[3], iN[2], iN[1], iN[4], iN[7], iN[6], iN[5],
                                             midN[3], midN[2], midN[1], midN[0], midN[7], midN[6],
                                             midN[5], midN[4], midN[8], midN[11], midN[10], midN[9],
                                             midN[12], midN[16], midN[15], midN[14], midN[13],
                                             midN[17], midN[18], hexIDShift + i))
                    status = storeBadNodeIds(
                        "GmfHexahedra + GmfExtraVerticesAtHexahedra", i, 27, iN[0], iN[3], iN[2],
                        iN[1], iN[4], iN[7], iN[6], iN[5], midN[3], midN[2], midN[1], midN[0],
                        midN[7], midN[6], midN[5], midN[4], midN[8], midN[11], midN[10], midN[9],
                        midN[12], midN[16], midN[15], midN[14], midN[13], midN[17], midN[18]);
            }
            else // HEXA8
            {
                if (!myMesh->AddVolumeWithID(iN[0], iN[3], iN[2], iN[1], iN[4], iN[7], iN[6], iN[5],
    ...
    if (int nbPrism = GmfStatKwd(meshID, GmfPrisms)) {
        ...
        for (int i = 1; i <= nbPrism; ++i) {
            ...
            if (!myMesh->AddVolumeWithID(iN[0], iN[2], iN[1], iN[3], iN[5], iN[4],
                                         prismIDShift + i))
                status =
                    storeBadNodeIds("GmfPrisms", i, 6, iN[0], iN[1], iN[2], iN[3], iN[4], iN[5]);
        }
    }
    ...
    if (_makeRequiredGroups) {
        ...
        for (; grIter != groups.end(); ++grIter) groupIDs.insert((*grIter)->GetID());
        ...
        const char *names[4] = {"_required_Vertices", "_required_Edges", "_required_Triangles",
...
{
    if (myStatus != DRS_OK) return myStatus;
    ...
    if (!msg.empty()) {
        ...
        if (elemNb < 3) nbStr = nbNames[elemNb - 1];
            ...
            nbStr = SMESH_Comment(elemNb) << "-th ";

        return addMessage(SMESH_Comment("Wrong node IDs of ") << nbStr << gmfKwd << ":" << msg,
                          /*fatal=*/false);
    }
    return DRS_OK;
}

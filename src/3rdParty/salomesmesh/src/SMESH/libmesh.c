...
static const char *GmfKwdFmt[GmfMaxKwd + 1][4] = {
    ...
    {"QuadrilateralsOnGeometricQuadrilaterals", "QuadrilateralOnGeometricQuadrilateral", "i",
     "iir"},
...
int GmfOpenMesh(const char *FilNam, int mod, ...)
{
    ...
    if (!GmfIniFlg) {
        for (i = 0; i <= MaxMsh; i++) GmfMshTab[i] = NULL;
    ...
    if (!MshIdx || !(msh = (GmfMshSct *)calloc(1, sizeof(GmfMshSct)))) return (0);
    ...
    if (k < 0) k = 0;
    ptr = msh->FilNam + k;
    if (strstr(ptr, ".meshb")) msh->typ |= (Bin | MshFil);
...
int GmfSetKwd(int MshIdx, int KwdCod, ...)
{
    ...
    if (msh->typ & Asc) {
        ...
        if (kwd->typ != InfKwd) fprintf(msh->hdl, "%d\n", kwd->NmbLin);
        ...
        if (kwd->typ != InfKwd) RecWrd(msh, (unsigned char *)&kwd->NmbLin);
        ...
        if (msh->ver == 1) {
            if (msh->typ & Asc) {
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') fscanf(msh->hdl, "%f", va_arg(VarArg, float *));
                    else if (kwd->fmt[i] == 'n') {
                ...
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') ScaWrd(msh, (unsigned char *)va_arg(VarArg, float *));
                    else if (kwd->fmt[i] == 'n') {
            ...
            if (msh->typ & Asc) {
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') fscanf(msh->hdl, "%lf", va_arg(VarArg, double *));
                    else if (kwd->fmt[i] == 'n') {
                        fscanf(msh->hdl, "%d", &nb_repeat);
                        *(va_arg(VarArg, int *)) = nb_repeat;
                        for (k = 0; k < nb_repeat; k++)
...
void GmfSetLin(int MshIdx, int KwdCod, ...)
{
    ...
    if (kwd->typ != SolKwd) {
        ...
        if (msh->ver == 1) {
            if (msh->typ & Asc) {
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') fprintf(msh->hdl, "%g ", (float)va_arg(VarArg, double));
                    else if (kwd->fmt[i] == 'n') {
                ...
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') msh->FltBuf[i] = va_arg(VarArg, double);
                    else if (kwd->fmt[i] == 'n') {
                        nb_repeat = va_arg(VarArg, int);
            ...
            if (msh->typ & Asc) {
                for (i = 0; i < kwd->SolSiz; i++)
                    if (kwd->fmt[i] == 'r') fprintf(msh->hdl, "%.15lg ", va_arg(VarArg, double));
                    else if (kwd->fmt[i] == 'n') {
                        nb_repeat = va_arg(VarArg, int);
                        fprintf(msh->hdl, "%d ", nb_repeat);
                        for (k = 0; k < nb_repeat; k++)
...
void GmfCpyLin(int InpIdx, int OutIdx, int KwdCod)
{
    ...
    for (i = 0; i < kwd->SolSiz; i++) {
        if (kwd->fmt[i] == 'r') {
            if (InpMsh->ver == 1) {
                if (InpMsh->typ & Asc) fscanf(InpMsh->hdl, "%f", &f);
                else
                ...
                if (InpMsh->typ & Asc) fscanf(InpMsh->hdl, "%lf", &d);
                else
            ...
            if (OutMsh->ver == 1)
                if (OutMsh->typ & Asc) fprintf(OutMsh->hdl, "%g ", f);
                else
            ...
            if (InpMsh->typ & Asc) fscanf(InpMsh->hdl, "%d", &a);
            else
                ScaWrd(InpMsh, (unsigned char *)&a);

            nb_repeat = a;

            if (OutMsh->typ & Asc) fprintf(OutMsh->hdl, "%d ", a);
            else
...
static int ScaKwdTab(GmfMshSct *msh)
{
    ...
    if (msh->typ & Asc) {
        ...
        do {
            ...
            if (NexPos > EndPos) return (0);

            /* Check if this kwd belongs to this mesh version */

            if ((KwdCod >= 1) && (KwdCod <= GmfMaxKwd)) ScaKwdHdr(msh, KwdCod);

            /* Go to the next kwd */

            if (NexPos) fseek(msh->hdl, NexPos, SEEK_SET);
...
static void ScaKwdHdr(GmfMshSct *msh, int KwdCod)
{
    ...
    if (!strcmp("i", GmfKwdFmt[KwdCod][2])) {
        if (msh->typ & Asc) fscanf(msh->hdl, "%d", &kwd->NmbLin);
        else
...
static void ExpFmt(GmfMshSct *msh, int KwdCod)
{
    ...
    if (!strlen(GmfKwdFmt[KwdCod][2])) kwd->typ = InfKwd;
...
static long GetPos(GmfMshSct *msh)
{
    ...
    if (msh->ver >= 3) ScaDblWrd(msh, (unsigned char *)&pos);
    else {
...
static void RecWrd(GmfMshSct *msh, unsigned char *wrd) { fwrite(wrd, WrdSiz, 1, msh->hdl); }
...
static void RecDblWrd(GmfMshSct *msh, unsigned char *wrd) { fwrite(wrd, WrdSiz, 2, msh->hdl); }
...
static void SetPos(GmfMshSct *msh, long pos)
{
    ...
    if (msh->ver >= 3) RecDblWrd(msh, (unsigned char *)&pos);
    else {
        IntVal = pos;

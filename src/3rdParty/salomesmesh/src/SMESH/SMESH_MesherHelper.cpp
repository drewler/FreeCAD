...
SMESH_MesherHelper::SMESH_MesherHelper(SMESH_Mesh &theMesh)
    : myParIndex(0), myMesh(&theMesh), myShapeID(0), myCreateQuadratic(false),
      myCreateBiQuadratic(false), myFixNodeParameters(false)
{
...
{
    {
        ...
        for (; i_proj != myFace2Projector.end(); ++i_proj) delete i_proj->second;
    }
    {
        TID2ProjectorOnCurve::iterator i_proj = myEdge2Projector.begin();
        for (; i_proj != myEdge2Projector.end(); ++i_proj) delete i_proj->second;
    }
}
...
bool SMESH_MesherHelper::IsQuadraticSubMesh(const TopoDS_Shape &aSh)
{
    ...
    if (!myMesh->HasShapeToMesh()) {
        if ((myCreateQuadratic = myMesh->NbFaces(ORDER_QUADRATIC))) {
            ...
            while (fIt->more()) AddTLinks(static_cast<const SMDS_MeshFace *>(fIt->next()));
        }
    }
    else {
                    ...
                    while (it->more()) {
                        ...
                        if (e->GetType() != elemType || !e->IsQuadratic()) {
                            ...
                            switch (e->NbCornerNodes()) {
                                case 2:
                                    AddTLinkNode(e->GetNode(0), e->GetNode(1), e->GetNode(2));
                                    break;
                                case 3:
                                    AddTLinkNode(e->GetNode(0), e->GetNode(1), e->GetNode(3));
                                    AddTLinkNode(e->GetNode(1), e->GetNode(2), e->GetNode(4));
                                    AddTLinkNode(e->GetNode(2), e->GetNode(0), e->GetNode(5));
                                    break;
                                case 4:
                                    ...
                                    break;
                                default: myCreateQuadratic = false; break;
                            }
                        }
                    }
                }
    ...
    if (myTLinkNodeMap.empty()) myCreateQuadratic = false;

    if (!myCreateQuadratic) myTLinkNodeMap.clear();
...
{
    if (aShID == myShapeID) return;
    if (aShID > 0) SetSubShape(GetMeshDS()->IndexToShape(aShID));
    else
...
bool SMESH_MesherHelper::GetNodeUVneedInFaceNode(const TopoDS_Face &F) const
{
    ...
    if (!F.IsNull() && !myShape.IsNull() && myShape.IsSame(F)) return !mySeamShapeIds.empty();

    TopLoc_Location loc;
    Handle(Geom_Surface) aSurface = BRep_Tool::Surface(F, loc);
    if (!aSurface.IsNull()) return (aSurface->IsUPeriodic() || aSurface->IsVPeriodic());
...
bool SMESH_MesherHelper::IsMedium(const SMDS_MeshNode *node, const SMDSAbs_ElementType typeToCheck)
{
                                                   ...
                                                   const SMESHDS_Mesh *meshDS)
{
    ...
    if (0 < shapeID && shapeID <= meshDS->MaxShapeIndex()) return meshDS->IndexToShape(shapeID);
    else
        return TopoDS_Shape();
}
...
void SMESH_MesherHelper::AddTLinkNode(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
...
bool SMESH_MesherHelper::AddTLinks(const SMDS_MeshFace *f)
{
    ...
    if (!f->IsPoly()) switch (f->NbNodes()) {
            ...
            case 6:
                ...
                AddTLinkNode(f->GetNode(2), f->GetNode(0), f->GetNode(5));
                break;
    ...
    if (volume->IsQuadratic()) {
        ...
        for (int iF = 1; iF < vTool.NbFaces(); ++iF) {
            ...
            for (int i = 0; i < nbN;) {
                ...
                if (it_isNew.second) AddTLinkNode(nodes[iN1], nodes[iN2], nodes[iN12]);
                else
            ...
            if (vTool.NbNodes() == 27) {
                ...
                if (nFCenter->GetPosition()->GetTypeOfPosition() == SMDS_TOP_3DSPACE)
                    myMapWithCentralNode.insert(
                        make_pair(TBiQuad(nodes[iNodes[0]], nodes[iNodes[1]], nodes[iNodes[2]],
                                          nodes[iNodes[3]]),
                                  nFCenter));
            }
        }
...
void SMESH_MesherHelper::ToFixNodeParameters(bool toFix) { myFixNodeParameters = toFix; }
...
gp_Pnt2d SMESH_MesherHelper::getUVOnSeam(const gp_Pnt2d &uv1, const gp_Pnt2d &uv2) const
{
    ...
    for (int i = U_periodic; i <= V_periodic; ++i) {
        if (myParIndex & i) {
            ...
            if (myParIndex == i || dp1 < (myPar2[i - 1] - myPar1[i - 1]) / 100.
                || dp2 < (myPar2[i - 1] - myPar1[i - 1]) / 100.) {
                double p2 = uv2.Coord(i);
                double p1Alt = (dp1 < dp2) ? myPar2[i - 1] : myPar1[i - 1];
                if (Abs(p2 - p1) > Abs(p2 - p1Alt)) result.SetCoord(i, p1Alt);
            }
        }
    }
    return result;
...
gp_XY SMESH_MesherHelper::GetNodeUV(const TopoDS_Face &F, const SMDS_MeshNode *n,
                                    const SMDS_MeshNode *n2, bool *check) const
{
    ...
    if (Pos->GetTypeOfPosition() == SMDS_TOP_FACE) {
        ...
        if (n2 && IsSeamShape(edgeID)) { uv = getUVOnSeam(uv, GetNodeUV(F, n2, 0, check)); }
        else { // adjust uv to period
            TopLoc_Location loc;
        ...
        if (int vertexID = n->getshapeId()) {
            ...
            if (!uvOK) {
                if (!IsSubShape(V, F)) {
                    MESSAGE("SMESH_MesherHelper::GetNodeUV(); Vertex "
                            << vertexID << " not in face " << GetMeshDS()->ShapeToIndex(F));
                    // get UV of a vertex closest to the node
                    double dist = 1e100;
                    gp_Pnt pn = XYZ(n);
                    for (TopExp_Explorer vert(F, TopAbs_VERTEX); !uvOK && vert.More();
                         vert.Next()) {
                    ...
                    if (!uvOK && V.Orientation() == TopAbs_INTERNAL) {
                        ...
                        if (Precision::IsInfinite(uv.X()))
                            uv = projector->NextValueOfUV(uv, BRep_Tool::Pnt(V),
            ...
            if (n2 && IsSeamShape(vertexID)) {
                ...
                if (isSeam) uv = getUVOnSeam(uv, GetNodeUV(F, n2, 0));
            }
        }
    }
    else {
...
bool SMESH_MesherHelper::CheckNodeUV(const TopoDS_Face &F, const SMDS_MeshNode *n, gp_XY &uv,
                                     const double tol, const bool force, double distXYZ[4]) const
{
    int shapeID = n->getshapeId();
    bool infinit;
    if ((infinit = (Precision::IsInfinite(uv.X()) || Precision::IsInfinite(uv.Y()))) || (force)
        || (uv.X() == 0. && uv.Y() == 0.) || (toCheckPosOnShape(shapeID))) {
        ...
        if (infinit || (dist = nodePnt.Distance(surfPnt = surface->Value(uv.X(), uv.Y()))) > tol) {
...
GeomAPI_ProjectPointOnSurf &SMESH_MesherHelper::GetProjector(const TopoDS_Face &F,
                                                             TopLoc_Location &loc, double tol) const
{
    Handle(Geom_Surface) surface = BRep_Tool::Surface(F, loc);
...
gp_XY SMESH_MesherHelper::ApplyIn2D(Handle(Geom_Surface) surface, const gp_XY &uv1,
                                    const gp_XY &uv2, xyFunPtr fun, const bool resultInPeriod)
{
    ...
    if (!isUPeriodic && !isVPeriodic) return fun(uv1, uv2);

    // move uv2 not far than half-period from uv1
    double u2 = uv2.X()
        + (isUPeriodic ? ShapeAnalysis::AdjustByPeriod(uv2.X(), uv1.X(), surface->UPeriod()) : 0);
    double v2 = uv2.Y()
        + (isVPeriodic ? ShapeAnalysis::AdjustByPeriod(uv2.Y(), uv1.Y(), surface->VPeriod()) : 0);
    ...
    if (resultInPeriod) {
        ...
        if (isUPeriodic) res.SetX(res.X() + ShapeAnalysis::AdjustToPeriod(res.X(), UF, UL));
        if (isVPeriodic) res.SetY(res.Y() + ShapeAnalysis::AdjustToPeriod(res.Y(), VF, VL));
    }
...
gp_XY SMESH_MesherHelper::GetMiddleUV(const Handle(Geom_Surface) & surface, const gp_XY &p1,
                                      const gp_XY &p2)
{
    // NOTE:
...
gp_XY SMESH_MesherHelper::GetCenterUV(const gp_XY &uv1, const gp_XY &uv2, const gp_XY &uv3,
                                      const gp_XY &uv12, const gp_XY &uv23, const gp_XY &uv31,
                                      bool *isBadTria /*=0*/)
{
    bool badTria;
    gp_XY uvAvg = (uv12 + uv23 + uv31) / 3.;

    if ((badTria = ((uvAvg - uv1) * (uvAvg - uv23) > 0))) uvAvg = (uv1 + uv23) / 2.;
    else if ((badTria = ((uvAvg - uv2) * (uvAvg - uv31) > 0)))
        uvAvg = (uv2 + uv31) / 2.;
    else if ((badTria = ((uvAvg - uv3) * (uvAvg - uv12) > 0)))
        uvAvg = (uv3 + uv12) / 2.;

    if (isBadTria) *isBadTria = badTria;
    return uvAvg;
}
...
double SMESH_MesherHelper::GetNodeU(const TopoDS_Edge &E, const SMDS_MeshNode *n,
                                    const SMDS_MeshNode *inEdgeNode, bool *check) const
{
...
bool SMESH_MesherHelper::CheckNodeU(const TopoDS_Edge &E, const SMDS_MeshNode *n, double &u,
                                    const double tol, const bool force, double distXYZ[4]) const
{
    int shapeID = n->getshapeId();
    bool infinit;
    if ((infinit = Precision::IsInfinite(u)) || (force) || (u == 0.)
...
{
    if (useCurSubShape && !myShape.IsNull()) return std::make_pair(myShapeID, myShape.ShapeType());
    ...
    if (!shape.IsNull()) {
        if (shapeID < 1) shapeID = GetMeshDS()->ShapeToIndex(shape);
        shapeType = shape.ShapeType(); // EDGE or FACE

        if (expectedSupport < shapeType && expectedSupport != TopAbs_SHAPE && !myShape.IsNull()
            ...
            for (int iN = 0; iN < 2; ++iN) {
                ...
                while (it->more()) {
                    ...
                    if (elem->getshapeId() == shapeID && elem->IsQuadratic()) {
                        ++nbConvertedElems;
                        break;
                    }
                }
...
const SMDS_MeshNode *
SMESH_MesherHelper::GetCentralNode(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                   const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                   const SMDS_MeshNode *n12, const SMDS_MeshNode *n23,
                                   const SMDS_MeshNode *n34, const SMDS_MeshNode *n41, bool force3d)
{
    ...
    if (itMapCentralNode != myMapWithCentralNode.end()) { return (*itMapCentralNode).second; }
    ...
    if (solidID < 1 && !faceId2nbNodes.empty()) // SOLID not found
    {
        ...
        if (itMapWithIdFace != faceId2nbNodes.end() && itMapWithIdFace->second == 4) {
            shapeType = TopAbs_FACE;
            faceID = myShapeID;
        }
        else {
    ...
    TopoDS_Face F;
    if (shapeType == TopAbs_FACE) { F = TopoDS::Face(meshDS->IndexToShape(faceID)); }
    ...
    if (!F.IsNull() && !force3d) {
        ...
        if (HasDegeneratedEdges() || surface->HasSingularities(1e-7)) {
            gp_Pnt center = calcTFI(0.5, 0.5, // IPAL0052863
                                    SMESH_TNodeXYZ(n1), SMESH_TNodeXYZ(n2), SMESH_TNodeXYZ(n3),
                                    SMESH_TNodeXYZ(n4), SMESH_TNodeXYZ(n12), SMESH_TNodeXYZ(n23),
            ...
            gp_XY uv[8] = {GetNodeUV(F, n1, n3, &toCheck), GetNodeUV(F, n2, n4, &toCheck),
    ...
    {
        P = calcTFI(0.5, 0.5, SMESH_TNodeXYZ(n1), SMESH_TNodeXYZ(n2), SMESH_TNodeXYZ(n3),
                    SMESH_TNodeXYZ(n4), SMESH_TNodeXYZ(n12), SMESH_TNodeXYZ(n23),
                    SMESH_TNodeXYZ(n34), SMESH_TNodeXYZ(n41));
        centralNode = meshDS->AddNode(P.X(), P.Y(), P.Z());
        ...
        if (!F.IsNull()) // force3d
        {
            uvAvg = (GetNodeUV(F, n1, n3, &toCheck) + GetNodeUV(F, n2, n4, &toCheck)
                     + GetNodeUV(F, n3, n1, &toCheck) + GetNodeUV(F, n4, n2, &toCheck))
                / 4;
...
const SMDS_MeshNode *
SMESH_MesherHelper::GetCentralNode(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                   const SMDS_MeshNode *n3, const SMDS_MeshNode *n12,
                                   const SMDS_MeshNode *n23, const SMDS_MeshNode *n31, bool force3d)
{
    SMDS_MeshNode *centralNode = 0; // central node to return
    ...
    if (shapeType == TopAbs_FACE) {
        ...
        gp_XY uv[6] = {GetNodeUV(F, n1, n23, &checkOK), GetNodeUV(F, n2, n31, &checkOK),
                       GetNodeUV(F, n3, n12, &checkOK), GetNodeUV(F, n12, n3, &checkOK),
                       GetNodeUV(F, n23, n1, &checkOK), GetNodeUV(F, n31, n2, &checkOK)};
        AdjustByPeriod(F, uv, 6); // put uv[] within a period (IPAL52698)

        uvAvg = GetCenterUV(uv[0], uv[1], uv[2], uv[3], uv[4], uv[5], &badTria);

        if (badTria || !checkOK) force3d = true;
    }
    ...
    {
        P = (SMESH_TNodeXYZ(n12) + SMESH_TNodeXYZ(n23) + SMESH_TNodeXYZ(n31)) / 3;
        centralNode = meshDS->AddNode(P.X(), P.Y(), P.Z());
...
const SMDS_MeshNode *SMESH_MesherHelper::GetMediumNode(const SMDS_MeshNode *n1,
                                                       const SMDS_MeshNode *n2, bool force3d,
                                                       TopAbs_ShapeEnum expectedSupport)
{
    // Find existing node
    ...
    if (itLN != myTLinkNodeMap.end()) { return (*itLN).second; }
    ...
    if (pos.second == TopAbs_FACE) {
        ...
        if ((!force3d) && (HasDegeneratedEdges() || GetSurface(F)->HasSingularities(1e-7))) {
            ...
            Handle(ShapeAnalysis_Surface) projector = GetSurface(F);
            gp_Pnt2d uvMid;
            if (uvOK[0]) uvMid = projector->NextValueOfUV(uv[0], pMid, BRep_Tool::Tolerance(F));
            else
        ...
        if (Pos1->GetTypeOfPosition() == SMDS_TOP_EDGE && Pos2->GetTypeOfPosition() == SMDS_TOP_EDGE
        ...
        if (!F.IsNull()) {
            ...
            if (!C.IsNull()) {
                ...
                if (isPeriodic) {
                    ...
                    U = pmid
                        + ShapeAnalysis::AdjustToPeriod(pmid, C->FirstParameter(),
                                                        C->LastParameter());
                }
                else
                    U = (u[0] + u[1]) / 2.;
                                                                     ...
                                                                     bool force3d)
{
        ...
        switch (shape[iS].ShapeType()) {
            ...
            case TopAbs_VERTEX: {
                ...
                if (edge.IsNull()) {
                    ...
                    while (const TopoDS_Shape *e = eIt->next()) edges.push_back(*e);
                }
                break;
            }
            case TopAbs_FACE: {
    ...
    if (!bestEdge.IsNull()) {
        ...
        //if ( mySetElemOnShape ) node is not elem!
        {
            ...
            if (edgeID != n12->getshapeId()) GetMeshDS()->UnSetNodeOnShape(n12);
...
SMDS_MeshNode *SMESH_MesherHelper::AddNode(double x, double y, double z, int ID, double u, double v)
{
    SMESHDS_Mesh *meshDS = GetMeshDS();
    SMDS_MeshNode *node = 0;
    if (ID) node = meshDS->AddNodeWithID(x, y, z, ID);
    else
...
SMDS_MeshEdge *SMESH_MesherHelper::AddEdge(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           const int id, const bool force3d)
{
    SMESHDS_Mesh *meshDS = GetMeshDS();
    ...
    if (myCreateQuadratic) {
        ...
        if (id) edge = meshDS->AddEdgeWithID(n1, n2, n12, id);
        else
            edge = meshDS->AddEdge(n1, n2, n12);
    }
    else {
        if (id) edge = meshDS->AddEdgeWithID(n1, n2, id);
        else
            edge = meshDS->AddEdge(n1, n2);
    }

    if (mySetElemOnShape && myShapeID > 0) meshDS->SetMeshElementOnShape(edge, myShapeID);
...
SMDS_MeshFace *SMESH_MesherHelper::AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           const SMDS_MeshNode *n3, const int id,
                                           const bool force3d)
{
    SMESHDS_Mesh *meshDS = GetMeshDS();
    SMDS_MeshFace *elem = 0;

    if (n1 == n2 || n2 == n3 || n3 == n1) return elem;

    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, id);
        else
        ...
        if (myCreateBiQuadratic) {
            ...
            if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, n12, n23, n31, nCenter, id);
            else
                elem = meshDS->AddFace(n1, n2, n3, n12, n23, n31, nCenter);
        }
        else {
            if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, n12, n23, n31, id);
            else
                elem = meshDS->AddFace(n1, n2, n3, n12, n23, n31);
        }
    }
    if (mySetElemOnShape && myShapeID > 0) meshDS->SetMeshElementOnShape(elem, myShapeID);
                                           ...
                                           const int id, const bool force3d)
{
    ...
    if (n1 == n2) { return AddFace(n1, n3, n4, id, force3d); }
    if (n1 == n3) { return AddFace(n1, n2, n4, id, force3d); }
    if (n1 == n4) { return AddFace(n1, n2, n3, id, force3d); }
    if (n2 == n3) { return AddFace(n1, n2, n4, id, force3d); }
    if (n2 == n4) { return AddFace(n1, n2, n3, id, force3d); }
    if (n3 == n4) { return AddFace(n1, n2, n3, id, force3d); }

    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, n4, id);
        else
        ...
        if (myCreateBiQuadratic) {
            const SMDS_MeshNode *nCenter =
                GetCentralNode(n1, n2, n3, n4, n12, n23, n34, n41, force3d);
            if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, n4, n12, n23, n34, n41, nCenter, id);
            else
                elem = meshDS->AddFace(n1, n2, n3, n4, n12, n23, n34, n41, nCenter);
        }
        else {
            if (id) elem = meshDS->AddFaceWithID(n1, n2, n3, n4, n12, n23, n34, n41, id);
            else
                                                    ...
                                                    const int id, const bool force3d)
{
    ...
    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddPolygonalFaceWithID(nodes, id);
        else
        ...
        if (id) elem = meshDS->AddQuadPolygonalFaceWithID(newNodes, id);
        else
...
SMDS_MeshVolume *SMESH_MesherHelper::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                               ...
                                               const int id, const bool force3d)
{
    ...
    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, id);
        else
        ...
        if (id)
            elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, n12, n23, n31, n45, n56, n64,
                                           n14, n25, n36, id);
        else
            elem = meshDS->AddVolume(n1, n2, n3, n4, n5, n6, n12, n23, n31, n45, n56, n64, n14, n25,
                                     n36);
    }
    if (mySetElemOnShape && myShapeID > 0) meshDS->SetMeshElementOnShape(elem, myShapeID);
                                               ...
                                               const int id, const bool force3d)
{
    ...
    SMDS_MeshVolume *elem = 0;
    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, id);
        else
        ...
        if (id) elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n12, n23, n31, n14, n24, n34, id);
        else
...
SMDS_MeshVolume *SMESH_MesherHelper::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                               const SMDS_MeshNode *n5, const int id,
                                               const bool force3d)
{
    SMDS_MeshVolume *elem = 0;
    if (!myCreateQuadratic) {
        if (id) elem = GetMeshDS()->AddVolumeWithID(n1, n2, n3, n4, n5, id);
        else
        ...
        if (id)
            elem = GetMeshDS()->AddVolumeWithID(n1, n2, n3, n4, n5, n12, n23, n34, n41, n15, n25,
                                                n35, n45, id);
        else
            elem =
                GetMeshDS()->AddVolume(n1, n2, n3, n4, n5, n12, n23, n34, n41, n15, n25, n35, n45);
    }
    if (mySetElemOnShape && myShapeID > 0) GetMeshDS()->SetMeshElementOnShape(elem, myShapeID);
                                               ...
                                               const int id, const bool force3d)
{
    ...
    SMDS_MeshVolume *elem = 0;
    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, id);
        else
        ...
        if (myCreateBiQuadratic) {
            ...
            const SMDS_MeshNode *n1256 =
                GetCentralNode(n1, n2, n5, n6, n12, n26, n56, n15, force3d);
            const SMDS_MeshNode *n2367 =
                GetCentralNode(n2, n3, n6, n7, n23, n37, n67, n26, force3d);
            const SMDS_MeshNode *n3478 =
                GetCentralNode(n3, n4, n7, n8, n34, n48, n78, n37, force3d);
            const SMDS_MeshNode *n1458 =
                GetCentralNode(n1, n4, n5, n8, n41, n48, n15, n85, force3d);
            const SMDS_MeshNode *n5678 =
                GetCentralNode(n5, n6, n7, n8, n56, n67, n78, n85, force3d);
            ...
            if (id)
                elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41,
                                               n56, n67, n78, n85, n15, n26, n37, n48, n1234, n1256,
                                               n2367, n3478, n1458, n5678, nCenter, id);
            else
                elem = meshDS->AddVolume(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56,
                                         n67, n78, n85, n15, n26, n37, n48, n1234, n1256, n2367,
                                         n3478, n1458, n5678, nCenter);
        }
        else {
            if (id)
                elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41,
...
SMDS_MeshVolume *SMESH_MesherHelper::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               ...
                                               const SMDS_MeshNode *n5, const SMDS_MeshNode *n6,
                                               const SMDS_MeshNode *n7, const SMDS_MeshNode *n8,
                                               const SMDS_MeshNode *n9, const SMDS_MeshNode *n10,
                                               const SMDS_MeshNode *n11, const SMDS_MeshNode *n12,
                                               const int id, bool force3d)
{
    SMESHDS_Mesh *meshDS = GetMeshDS();
    SMDS_MeshVolume *elem = 0;
    if (id) elem = meshDS->AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, id);
    else
...
SMESH_MesherHelper::AddPolyhedralVolume(const std::vector<const SMDS_MeshNode *> &nodes,
                                        const std::vector<int> &quantities, const int id,
                                        const bool force3d)
{
    SMESHDS_Mesh *meshDS = GetMeshDS();
    SMDS_MeshVolume *elem = 0;
    if (!myCreateQuadratic) {
        if (id) elem = meshDS->AddPolyhedralVolumeWithID(nodes, quantities, id);
        else
        ...
        if (id) elem = meshDS->AddPolyhedralVolumeWithID(newNodes, newQuantities, id);
        else
...
bool isNodeInSubMesh(const SMDS_MeshNode *n, const SMESHDS_SubMesh *sm)
{
    ...
    while (fIt->more())
        if (sm->Contains(fIt->next())) return true;
    return false;
}
} // namespace
...
bool SMESH_MesherHelper::IsSameElemGeometry(const SMESHDS_SubMesh *smDS, SMDSAbs_GeometryType shape,
                                            const bool nullSubMeshRes)
{
    ...
    while (elemIt->more()) {
        ...
        if (e->GetGeomType() != shape) return false;
    }
    return true;
}
...
bool SMESH_MesherHelper::LoadNodeColumns(TParam2ColumnMap &theParam2ColumnMap,
                                         const TopoDS_Face &theFace, const TopoDS_Edge &theBaseEdge,
                                         SMESHDS_Mesh *theMesh, SMESH_ProxyMesh *theProxyMesh)
{
    return LoadNodeColumns(theParam2ColumnMap, theFace, std::list<TopoDS_Edge>(1, theBaseEdge),
                           theMesh, theProxyMesh);
}
                                         ...
                                         SMESHDS_Mesh *theMesh, SMESH_ProxyMesh *theProxyMesh)
{
    ...
    if (!faceSubMesh) faceSubMesh = theMesh->MeshElements(theFace);
    ...
    if (theParam2ColumnMap.empty()) {
        ...
        for (int iE = 0; edge != theBaseSide.end(); ++edge, ++iE) {
            ...
            if (theProxyMesh) // from sortedBaseNN remove nodes not shared by faces of faceSubMesh
            {
                ...
                bool allNodesAreProxy =
                    (n1 != theProxyMesh->GetProxyNode(n1) && n2 != theProxyMesh->GetProxyNode(n2));
                if (allNodesAreProxy)
                ...
                if (u_n = sortedBaseNN.begin(), !isNodeInSubMesh(u_n->second, faceSubMesh)) {
                    while (++u_n != sortedBaseNN.end()
                           && !isNodeInSubMesh(u_n->second, faceSubMesh))
                        ;
            ...
            const double prevPar =
                theParam2ColumnMap.empty() ? 0 : theParam2ColumnMap.rbegin()->first;
            for (u_n = sortedBaseNN.begin(); u_n != sortedBaseNN.end(); u_n++) {
                double par = prevPar + coeff * (u_n->first - f);
                TParam2ColumnMap::iterator u2nn = theParam2ColumnMap.insert(
                    theParam2ColumnMap.end(), make_pair(par, TNodeColumn()));
                u2nn->second.push_back(u_n->second);
            }
        }
        if (theParam2ColumnMap.size() < 2) return false;
    }
    ...
    for (; par_nVec_2 != theParam2ColumnMap.end(); ++par_nVec_1, ++par_nVec_2) {
        ...
        while (const SMDS_MeshElement *face =
            ...
            if (faceSubMesh->Contains(face)) {
                ...
                if (nbNodes != 4) return false;
                if (foundNbRows + 1 > expectedNbRows) return false;
        ...
        if (foundNbRows != expectedNbRows) return false;
        avoidSet.clear();
    }
...
bool isCornerOfStructure(const SMDS_MeshNode *n, const SMESHDS_SubMesh *faceSM,
                         SMESH_MesherHelper &faceAnalyser)
{
    int nbFacesInSM = 0;
    if (n) {
        SMDS_ElemIteratorPtr fIt = n->GetInverseElementIterator(SMDSAbs_Face);
        while (fIt->more()) nbFacesInSM += faceSM->Contains(fIt->next());
    }
    if (nbFacesInSM == 1) return true;
...
bool SMESH_MesherHelper::IsStructured(SMESH_subMesh *faceSM)
{
    ...
    if (!fSM || fSM->NbElements() == 0) return false;

    list<TopoDS_Edge> edges;
    list<int> nbEdgesInWires;
    int nbWires =
        SMESH_Block::GetOrderedEdges(TopoDS::Face(faceSM->GetSubShape()), edges, nbEdgesInWires);
    ...
    do {
        ...
        isCorner = isCornerOfStructure(SMESH_Algo::VertexNode(V, meshDS), fSM, faceAnalyser);
        if (!isCorner) {
    ...
    for (; edge != edges.end(); ++edge) {
        ...
        for (; u2n != u2Nodes.end(); ++u2n) edgeNodes.push_back(u2n->second);
        if (edge->Orientation() == TopAbs_REVERSED) edgeNodes.reverse();

        if (!nodes.empty() && nodes.back() == edgeNodes.front()) edgeNodes.pop_front();
    ...
    // checks
    if (nbEdgesInSide.size() != 4) return false;
    if (nbEdgesInSide[0] != nbEdgesInSide[2]) return false;
    if (nbEdgesInSide[1] != nbEdgesInSide[3]) return false;
    if (nbEdgesInSide[0] * nbEdgesInSide[1] != fSM->NbElements()) return false;
...
bool SMESH_MesherHelper::IsDistorted2D(SMESH_subMesh *faceSM, bool checkUV)
{
    if (!faceSM || faceSM->GetSubShape().ShapeType() != TopAbs_FACE) return false;
    ...
    while (faceIt->more() && !haveBadFaces) {
        ...
        for (size_t i = 0; i < nodes.size(); ++n, ++i) nodes[i] = *n;
        ...
        if (helper.HasDegeneratedEdges()) {
            ...
            if (isOnDegen) continue;
        }
        // prepare to getting UVs
        const SMDS_MeshNode *inFaceNode = 0;
        if (helper.HasSeam()) {
            for (size_t i = 0; (i < nodes.size() && !inFaceNode); ++i)
                if (!helper.IsSeamShape(nodes[i]->getshapeId())) inFaceNode = nodes[i];
            if (!inFaceNode) continue;
        }
        // get UVs
...
bool SMESH_MesherHelper::IsReversedSubMesh(const TopoDS_Face &theFace)
{
    ...
    if (!aSubMeshDSFace) return isReversed;
    ...
    while (iteratorElem->more()) // loop on elements on theFace
    {
        ...
        for (int iN = 0; iN < nbNodes; ++iN) {
            ...
            if (nn[0]->GetPosition()->GetDim() < 2 && nn[1]->GetPosition()->GetDim() < 2) {
                ...
                if (!E.IsNull() && !s0.IsSame(s1)) {
                    ...
                    if (nb == 1) {
                        ...
                        if (ok) {
                            isReversed = (u0 > u1);
                            if (E.Orientation() == TopAbs_REVERSED) isReversed = !isReversed;
                            return isReversed;
                        }
                    }
                }
    ...
    if (surf.IsNull() || surf->Continuity() < GeomAbs_C0) return isReversed;
    ...
    if (theFace.Orientation() == TopAbs_REVERSED) Nf.Reverse();
...
int SMESH_MesherHelper::Count(const TopoDS_Shape &shape, const TopAbs_ShapeEnum type,
                              const bool ignoreSame)
{
    if (ignoreSame) {
        ...
        int nb = 0;
        for (TopExp_Explorer exp(shape, type); exp.More(); exp.Next()) ++nb;
        return nb;
    }
}
...
int SMESH_MesherHelper::NbAncestors(const TopoDS_Shape &shape, const SMESH_Mesh &mesh,
                                                      ...
                                                      const TopoDS_Shape &subShape)
{
    ...
    if (!shape.IsNull() && !subShape.IsNull()) {
        ...
        for (; e.More(); e.Next())
            if (subShape.IsSame(e.Current())) break;
        if (e.More()) ori = e.Current().Orientation();
    }
    return ori;
}
...
bool SMESH_MesherHelper::IsSubShape(const TopoDS_Shape &shape, const TopoDS_Shape &mainShape)
{
    if (!shape.IsNull() && !mainShape.IsNull()) {
        for (TopExp_Explorer exp(mainShape, shape.ShapeType()); exp.More(); exp.Next())
            if (shape.IsSame(exp.Current())) return true;
    }
    SCRUTE((shape.IsNull()));
...
{
    if (shape.IsNull() || !aMesh) return false;
...
double SMESH_MesherHelper::getFaceMaxTol(const TopoDS_Shape &face) const
{
    ...
    if (tol < 0) tol = MaxTolerance(face);
...
double SMESH_MesherHelper::GetAngle(const TopoDS_Edge &theE1, const TopoDS_Edge &theE2,
                                    const TopoDS_Face &theFace, const TopoDS_Vertex &theCommonV,
                                    gp_Vec *theFaceNormal)
{
    ...
    try {
        ...
        if (c1.IsNull() || c2.IsNull()) return angle;
        ...
        if (theFace.Orientation() == TopAbs_REVERSED) vecRef.Reverse();
        ...
        if (theE1.Orientation() /*GetSubShapeOri( F, theE1 )*/ == TopAbs_REVERSED) vec1.Reverse();
        if (theE2.Orientation() /*GetSubShapeOri( F, theE2 )*/ == TopAbs_REVERSED) vec2.Reverse();
        ...
        if (Abs(angle) >= 0.99 * M_PI) {
            ...
            c1->D1(p1, p, vec1);
            if (theE1.Orientation() == TopAbs_REVERSED) vec1.Reverse();
            BRep_Tool::Range(theE2, f, l);
            p2 += 1e-7 * (p2 - f < l - p2 ? +1. : -1.);
            c2->D1(p2, p, vec2);
            if (theE2.Orientation() == TopAbs_REVERSED) vec2.Reverse();
            angle = vec1.AngleWithRef(vec2, vecRef);
        }
    }
    catch (...) {
...
TopoDS_Vertex SMESH_MesherHelper::IthVertex(const bool is2nd, TopoDS_Edge anEdge, const bool CumOri)
{
    if (anEdge.Orientation() >= TopAbs_INTERNAL) anEdge.Orientation(TopAbs_FORWARD);

    const TopAbs_Orientation tgtOri = is2nd ? TopAbs_REVERSED : TopAbs_FORWARD;
    TopoDS_Iterator vIt(anEdge, CumOri);
    while (vIt.More() && vIt.Value().Orientation() != tgtOri) vIt.Next();
...
{
    if (!group.IsNull()) {
        if (group.ShapeType() != TopAbs_COMPOUND) return group.ShapeType();

        // iterate on a compound
        TopoDS_Iterator it(group);
        if (it.More()) return avoidCompound ? GetGroupType(it.Value()) : it.Value().ShapeType();
    }
    return TopAbs_SHAPE;
}
...
TopoDS_Shape SMESH_MesherHelper::GetShapeOfHypothesis(const SMESHDS_Hypothesis *hyp,
                                                      const TopoDS_Shape &shape, SMESH_Mesh *mesh)
{
...
SMESH_MesherHelper::MType SMESH_MesherHelper::IsQuadraticMesh()
{
    ...
    if (NbAllEdgsAndFaces == 0) return SMESH_MesherHelper::LINEAR;
        ...
        : _ancIter(ancestors), _type(type)
    {
        if (_ancIter.More()) {
            if (_ancIter.Value().ShapeType() != _type) next();
                ...
                _encountered.Add(_ancIter.Value());
        }
    }
    virtual bool more() { return _ancIter.More(); }
                                                   ...
                                                   TopAbs_ShapeEnum ancestorType)
{
    ...
    if (!shape1.IsNull() && !shape2.IsNull()) {
        if (shape1.ShapeType() == ancestorType && IsSubShape(shape2, shape1)) return shape1;
        if (shape2.ShapeType() == ancestorType && IsSubShape(shape1, shape2)) return shape2;
...
//=======================================================================
namespace
{ // Structures used by FixQuadraticElements()
//=======================================================================

#define __DMP__(txt) // cout << txt
...
struct QLink: public SMESH_TLink {
    ...
    {
        _nodeMove += move;
        _nbMoves += sum ? (_nbMoves == 0) : 1;
        _is2dFixed |= is2dFixed;
    }
...
struct TChainLink {
    ...
    TChainLink(const QLink *qlink = 0) : _qlink(qlink) { _qfaces[0] = _qfaces[1] = 0; }
    void SetFace(const QFace *face) const
    {
        int iF = _qfaces[0] ? 1 : 0;
        _qfaces[iF] = face;
    }
    ...
    {
        _qfaces[(face == _qfaces[1])] = 0;
        if (!_qfaces[0]) std::swap(_qfaces[0], _qfaces[1]);
    }
...
struct QFace: public TIDSortedNodeSet {
    ...
    {
        for (int i = 0; i < _sides.size(); ++i) _sides[i]->_faces.push_back(this);
    }
    int LinkIndex(const QLink *side) const
    {
        for (int i = 0; i < _sides.size(); ++i)
            if (_sides[i] == side) return i;
        return -1;
    }
    ...
    TLinkInSet GetBoundaryLink(const TLinkSet &links, const TChainLink &avoidLink,
                               TLinkInSet *notBoundaryLink = 0,
                               const SMDS_MeshNode *nodeToContain = 0, bool *isAdjacentUsed = 0,
                               int nbRecursionsLeft = -1) const;

    TLinkInSet GetLinkByNode(const TLinkSet &links, const TChainLink &avoidLink,
    ...
    double MoveByBoundary(const TChainLink &theLink, const gp_Vec &theRefVec,
                          const TLinkSet &theLinks, SMESH_MesherHelper *theFaceHelper = 0,
                          const double thePrevLen = 0, const int theStep = theFirstStep,
                          gp_Vec *theLinkNorm = 0, double theSign = 1.0) const;
};
...
{
    out << "QLink nodes: " << l.node1()->GetID() << " - " << l._mediumNode->GetID() << " - "
        << l.node2()->GetID() << endl;
    return out;
}
inline ostream &operator<<(ostream &out, const QFace &f)
{
    ...
    for (TIDSortedNodeSet::const_iterator n = f.begin(); n != f.end(); ++n)
        out << (*n)->GetID() << " ";
    out << " \tvolumes: " << (f._volumes[0] ? f._volumes[0]->GetID() : 0) << " "
        << (f._volumes[1] ? f._volumes[1]->GetID() : 0);
    out << "  \tNormal: " << f._normal.X() << ", " << f._normal.Y() << ", " << f._normal.Z()
        << endl;
    return out;
}
...
QFace::QFace(const vector<const QLink *> &links, const SMDS_MeshElement *face)
{
    ...
    for (int i = 1; i < _sides.size(); ++i) {
        ...
        if (l1->node1() != l2->node1() && l1->node2() != l2->node2()) v1.Reverse();
        _normal += v1 ^ v2;
    }
    double normSqSize = _normal.SquareMagnitude();
    if (normSqSize > numeric_limits<double>::min()) _normal /= sqrt(normSqSize);
    else
                ...
                if (!face->_sideIsAdded[i] && face->_sides[i]) {
                    ...
                    if (face->_sides[i]->MediumPos() == pos)
                        if (const QFace *contFace = face->_sides[i]->GetContinuesFace(face))
                            if (contFace->_sides.size() == 3) faces.push_back(contFace);
                }
            }
            faces.pop_front();
        }
        if (error < ERR_TRI) error = ERR_TRI;
    ...
    if (link->MediumPos() >= pos) {
        ...
        if (nbLinkFaces == 4 || (/*nbLinkFaces < 4 && */ link->OnBoundary())) {
            ...
            if (const QFace *f = link->GetContinuesFace(this))
                if (f->_sides.size() == 4) return f->GetLinkChain(*chLink, chain, pos, error);
        }
        else {
            TChainLink chLink(link); // side face of prismatic mesh - visit all faces of iSide
            for (int i = 0; i < nbLinkFaces; ++i)
                if (link->_faces[i]) link->_faces[i]->GetLinkChain(chLink, chain, pos, error);
            if (error < ERR_PRISM) error = ERR_PRISM;
            return false;
        }
    }
    return true;
...
TLinkInSet QFace::GetBoundaryLink(const TLinkSet &links, const TChainLink &avoidLink,
                                  TLinkInSet *notBoundaryLink, const SMDS_MeshNode *nodeToContain,
                                  bool *isAdjacentUsed, int nbRecursionsLeft) const
{
    ...
    for (int iL = 0; iL < _sides.size(); ++iL) {
        if (avoidLink._qlink == _sides[iL]) continue;
        ...
        // check link
        if (link->IsBoundary()) {
            if (!nodeToContain || (*link)->node1() == nodeToContain
        ...
        if (boundaryLink == linksEnd && nodeToContain) // collect adjacent faces
            if (const QFace *adj = link->NextFace(this))
                if (adj->Contains(nodeToContain)) adjacentFaces.push_back(make_pair(adj, link));
    }

    if (isAdjacentUsed) *isAdjacentUsed = false;
    if (boundaryLink == linksEnd && nodeToContain && nbRecursionsLeft) // check adjacent faces
    {
        if (nbRecursionsLeft < 0) nbRecursionsLeft = nodeToContain->NbInverseElements();
...
TLinkInSet QFace::GetLinkByNode(const TLinkSet &links, const TChainLink &avoidLink,
                                const SMDS_MeshNode *nodeToContain) const
{
    for (int i = 0; i < _sides.size(); ++i)
...
gp_Vec QFace::LinkNorm(const int i, SMESH_MesherHelper * /*uvHelper*/) const
{
    ...
    gp_XYZ pIn = (_sides[(i + 1) % 3]->MiddlePnt() + _sides[(i + 2) % 3]->MiddlePnt()) / 2.;
    gp_Vec vecOut = (_sides[i]->MiddlePnt() - pIn);

    if (norm * vecOut < 0) norm.Reverse();
    double mag2 = norm.SquareMagnitude();
    if (mag2 > numeric_limits<double>::min()) norm /= sqrt(mag2);
    return norm;
}
//================================================================================
/*!
...
double QFace::MoveByBoundary(const TChainLink &theLink, const gp_Vec &theRefVec,
                             const TLinkSet &theLinks, SMESH_MesherHelper *theFaceHelper,
                             const double thePrevLen, const int theStep, gp_Vec *theLinkNorm,
                             double theSign) const
{
    if (!theStep) return thePrevLen; // propagation limit reached

    int iL; // index of theLink
    for (iL = 0; iL < _sides.size(); ++iL)
        if (theLink._qlink == _sides[iL]) break;

    MSG(string(theStep, '.') << " Ref( " << theRefVec.X() << "," << theRefVec.Y() << ","
                             << theRefVec.Z() << " )"
    ...
    if (theStep == theFirstStep) theSign = refProj < 0. ? -1. : 1.;
    ...
    if (link1 != theLinks.end() && link2 != theLinks.end()) {
        ...
        {
            if (!isBndLink1 && !f1) f1 = (*link1)->GetContinuesFace(this); // get a quadrangle face
            if (!isBndLink2 && !f2) f2 = (*link2)->GetContinuesFace(this);
        }
    }
    ...
    try {
        OCC_CATCH_SIGNALS;
        if (f1 && !isBndLink1)
            len1 = f1->MoveByBoundary(*link1, theRefVec, theLinks, theFaceHelper, len1, theStep - 1,
                                      &linkDir1, theSign);
        else
    ...
    try {
        OCC_CATCH_SIGNALS;
        if (f2 && !isBndLink2)
            len2 = f2->MoveByBoundary(*link2, theRefVec, theLinks, theFaceHelper, len2, theStep - 1,
                                      &linkDir2, theSign);
        else
    ...
    if (theStep != theFirstStep) {
        ...
        MSG(string(theStep, '.') << " Move " << theLink->_mediumNode->GetID() << " by "
                                 << refProj * (1 - r) << " following "
                                 << (choose1 ? *link1->_qlink
                                             : *link2->_qlink)); // warning: link1 can be invalid
...
void QLink::SetContinuesFaces() const
{
    ...
    if (_faces[0]->IsBoundary()) iBoundary[nbBoundary++] = 0;
    for (int iF = 1; iFaceCont < 0 && iF < _faces.size(); ++iF) {
        ...
        if (!sameVol) iFaceCont = iF;
        if (_faces[iF]->IsBoundary()) iBoundary[nbBoundary++] = iF;
    }
    ...
    {
        if (iFaceCont != 1) std::swap(_faces[1], _faces[iFaceCont]);
    }
...
void fixPrism(TChain &allLinks)
{
    ...
    for (TChain::iterator lnk = allLinks.begin(); lnk != allLinks.end(); ++lnk) {
        if ((*lnk)->OnBoundary()) bndLinks1.insert(lnk->_qlink);
        else
            interLinks.insert(lnk->_qlink);
        isCurved = isCurved || !lnk->IsStraight();
    }
    if (!isCurved) return; // no need to move
...
void fixTriaNearBoundary(TChain &allLinks, SMESH_MesherHelper & /*helper*/)
{
    ...
    for (linkIt = linkSet.begin(); linkIt != linksEnd; ++linkIt) {
        if (linkIt->IsBoundary() && !linkIt->IsStraight() && linkIt->_qfaces[0]) {
            ...
            gp_XYZ pIn = (face->_sides[0]->MiddlePnt() + face->_sides[1]->MiddlePnt()
                          + face->_sides[2]->MiddlePnt())
                / 3.;
            gp_XYZ insideDir(pIn - (*linkIt)->MiddlePnt());
            bool linkBentInside = ((*linkIt)->_nodeMove.Dot(insideDir) > 0);
            //if ( face->IsSpoiled( linkIt->_qlink ))
            if (linkBentInside) face->MoveByBoundary(*linkIt, (*linkIt)->_nodeMove, linkSet);
        }
    }
}
...
TSplitTriaResult splitTrianglesIntoChains(TChain &allLinks, vector<TChain> &resultChains,
                                          SMDS_TypeOfPosition pos)
{
    ...
    for (linkIt = linkSet.begin(); linkIt != linksEnd; ++linkIt)
        if (linkIt->IsBoundary() && (corner = (*linkIt)->EndPosNode(cornerPos))) break;
    if (!corner) return _NO_CORNERS;
    ...
    while (startLink != linksEnd) // loop on columns
    {
        ...
        if (resultChains.size() == nbBndLinks / 2) return _NOT_RECT;
        ...
        while (botLink != linksEnd) // loop on rows
        {
            ...
            if (!botTria) {                                      // the column ends
                if (botLink == startLink) return _TWISTED_CHAIN; // issue 0020951
                linkSet.erase(botLink);
                if (iRow != rowChains.size()) return _FEW_ROWS; // different nb of rows in columns
                break;
            }
            ...
            TLinkInSet sideLink =
                botTria->GetBoundaryLink(linkSet, *botLink, &midQuadLink, corner, &isCase2);
            if (isCase2) { // find midQuadLink among links of botTria
                midQuadLink = botTria->GetLinkByNode(linkSet, *botLink, corner);
                if (midQuadLink->IsBoundary()) return _BAD_MIDQUAD;
            }
            ...
            if (iRow >= rowChains.size()) {
                if (iCol > 0) return _MANY_ROWS; // different nb of rows in columns
                if (resultChains.size() == nbBndLinks / 2) return _NOT_RECT;
                resultChains.push_back(TChain());
            ...
            if (iRow == 0) {
                ...
                if (startLink != linksEnd) {
                    ...
                    else if (startLink == botLink || startLink == midQuadLink
                             || startLink == sideLink)
                        return _BAD_START;
                }
            }
            ...
            if (startLink != linksEnd) {
                ...
                for (int iL = 0; iL < 3; ++iL) {
                    ...
                    if (linkIt != linksEnd) linkIt->RemoveFace(tria);
                }
            }
    ...
    if (linkSet.size() != rowChains.size()) return _BAD_SET_SIZE;
    for (int iRow = 0; iRow < rowChains.size(); ++iRow) {
        ...
        for (startLink = linkSet.begin(); startLink != linksEnd; ++startLink) {
            if ((*startLink)->node1() == startCorner) {
                corner = (*startLink)->node2();
                break;
            }
            else if ((*startLink)->node2() == startCorner) {
                corner = (*startLink)->node1();
                break;
            }
        }
        if (startLink == linksEnd) return _BAD_CORNER;
...
void force3DOutOfBoundary(SMESH_MesherHelper &theHelper, SMESH_ComputeErrorPtr &theError)
{
    ...
    if (shape.ShapeType() == TopAbs_FACE) // 2D
    {
        ...
        for (; edgeIt.More(); edgeIt.Next()) // loop on EDGEs of a FACE
        {
            ...
            if (SMESH_Algo::isDegenerated(edge)) continue;
            if (theHelper.IsRealSeam(edge) && edge.Orientation() == TopAbs_REVERSED) continue;
            ...
            switch (curve3D.GetType()) {
                ...
                case GeomAbs_Parabola:
                    try {
                        ...
                        if (curvNorm * D2 > 0) continue; // convex edge
                    }
            ...
            if (edgeSM->NbNodes() > 0) { nodeIt = edgeSM->GetNodes(); }
            else {
                SMESHDS_SubMesh *vertexSM = meshDS->MeshElements(theHelper.IthVertex(0, edge));
                if (!vertexSM) vertexSM = meshDS->MeshElements(theHelper.IthVertex(1, edge));
            ...
            while (nodeIt->more()) {
                ...
                while (faceIt->more()) {
                    ...
                    if (!faceSM->Contains(f) || f->NbNodes() < 6
                        || // check quadratic triangles only
                    ...
                    for (int nbN = 0; nbN < 3; ++triNode, ++nbN) {
                        n = *triNode;
                        if (n->GetPosition()->GetDim() == 2) nOnFace = n;
                        else
                            nOnEdge.push_back(n);
                    }
                    ...
                    if (nOnFace && nOnEdge.size() == 2) {
                        ...
                        n = theHelper.GetMediumNode(nOnEdge[0], nOnEdge[1],
                                                    true); // find n, not create
        ...
        if (!theError->myBadElements.empty()) theError->myName = EDITERR_NO_MEDIUM_ON_GEOM;
        return;
    ...
    if (shape.ShapeType() == TopAbs_SOLID) // 3D
    {
        if (theHelper.GetMesh()->NbTetras(ORDER_QUADRATIC) < 1
            && theHelper.GetMesh()->NbPyramids(ORDER_QUADRATIC) < 1)
            return;
            ...
            switch (surface.GetType()) {
                ...
                case GeomAbs_Sphere:
                    try {
                        ...
                        if (concaveU || concaveV) concaveFaces.push_back(face);
                    }
                    catch (Standard_Failure &) {
        ...
        typedef SMDS_IteratorOnIterators<const SMDS_MeshElement *, vector<SMDS_ElemIteratorPtr>>
            TIterOnIter;
        ...
        for (size_t iF = 0; iF < concaveFaces.size(); ++iF) // loop on concave FACEs
        {
            ...
            SMDS_NodeIteratorPtr nodeIt;
            if (faceSM->NbNodes() > 0) { nodeIt = faceSM->GetNodes(); }
            else {
            ...
            while (smIt->more()) subIDs.insert(smIt->next()->GetId());
            ...
            while (nodeIt->more()) {
                ...
                while (volIt->more()) {
                    ...
                    if ((nbN != 4 && nbN != 5) || !solidSM->Contains(vol)
                    ...
                    for (int nb = nbN; nb > 0; ++volNode, --nb) {
                        n = *volNode;
                        if (n->GetPosition()->GetDim() == 3) nInSolid = n;
                        else if (subIDs.count(n->getshapeId()))
                            nOnFace.push_back(n);
                        else
                            nInSolid = n;
                    }
                    if (!nInSolid || nOnFace.size() != nbN - 1) continue;
                    ...
                    if (isDistorted) {
                        for (size_t i = 0; i < links.size(); ++i) {
                            ...
                            if (movedNodes.insert(nMedium).second) {
                                gp_Pnt pMid3D = 0.5
                                    * (SMESH_TNodeXYZ(links[i].first.node1())
...
void SMESH_MesherHelper::FixQuadraticElements(SMESH_ComputeErrorPtr &compError, bool volumeOnly)
{
    // setenv NO_FixQuadraticElements to know if FixQuadraticElements() is guilty of bad conversion
    if (getenv("NO_FixQuadraticElements")) return;
...
#ifdef _DEBUG_
        int nbfaces = faces.Extent(); /*avoid "unused varianbles": */
        nbfaces++, nbfaces--;
#endif
    ...
    if (!elemIt || !elemIt->more() || elemType < SMDSAbs_Face) return;
    ...
    if (elemType == SMDSAbs_Volume) {
        while (elemIt->more()) // loop on volumes
        {
            ...
            if (!vol->IsQuadratic() || !volTool.Set(vol)) return;
            ...
            for (int iF = 0; iF < volTool.NbFaces(); ++iF) // loop on faces of volume
            {
                ...
                for (int iN = 0; iN < nbN; iN += 2) // loop on links of a face
                {
                    ...
                    if (link.MediumPos() == SMDS_TOP_3DSPACE) {
                        if (!link.IsStraight()) return; // already fixed
                    }
                    else if (!isCurved) {
                ...
                if (pFace->NbVolumes() == 0) pFace->AddSelfToLinks();
                pFace->SetVolume(vol);
...
//             volTool.GetVolumeType() == SMDS_VolumeTool::QUAD_PENTA );
            ...
            if (vol->NbCornerNodes() == 5) apexOfPyramid.insert(vol->GetNode(apexIndex));
        }
        set<QLink>::iterator pLink = links.begin();
        for (; pLink != links.end(); ++pLink) pLink->SetContinuesFaces();
    }
    else {
        while (elemIt->more()) // loop on faces
        {
            const SMDS_MeshElement *face = elemIt->next();
            if (!face->IsQuadratic()) continue;
            ...
            {
                // store QLink
                QLink link(face->GetNode(iN), face->GetNode((iN + 1) % nbN),
                           face->GetNode(iN + nbN));
                pLink = links.insert(link).first;
                faceLinks[iN] = &*pLink;
                if (!isCurved && link.node1()->GetPosition()->GetTypeOfPosition() < 2
    ...
    if (!isCurved) return; // no curved edges of faces
    ...
    for (; isInside < 2; ++isInside) {
        ...
        for (pFace = faces.begin(); pFace != faces.end(); ++pFace) {
            if (bool(isInside) == pFace->IsBoundary()) continue;
            for (int dir = 0; dir < 2; ++dir) // 2 directions of propagation from the quadrangle
            {
                ...
                int error = ERR_OK;
                TChain rawChain;
                if (!pFace->GetLinkChain(dir, rawChain, pos, error) && error == ERR_UNKNOWN)
                    continue;
                rawChain.reverse();
                if (!pFace->GetLinkChain(dir + 2, rawChain, pos, error) && error == ERR_UNKNOWN)
                    continue;
                ...
                for (int iC = 0; iC < chains.size(); ++iC) {
                    ...
                    if (chain.front()->MediumPos() > bndPos || chain.back()->MediumPos() > bndPos) {
                    ...
                    for (++link1; link1 != chain.end(); ++link1, ++link0) {
                        MSGBEG((*link0)->_mediumNode->GetID()
                               << "-" << (*link1)->_mediumNode->GetID() << " ");
                    ...
                    if (!isInside) {
                        ...
                        if (!f.IsNull() && f.ShapeType() == TopAbs_FACE) {
                            ...
                            for (int is1 = 0; is1 < 2; ++is1) // move0 or move1
                            {
                                ...
                                gp_XY uvm = faceHlp.GetNodeUV(face, link->_mediumNode, nodeOnFace,
                                                              &checkUV);
                                gp_XY uv1 =
                                    faceHlp.GetNodeUV(face, link->node1(), nodeOnFace, &checkUV);
                                gp_XY uv2 =
                                    faceHlp.GetNodeUV(face, link->node2(), nodeOnFace, &checkUV);
                                gp_XY uv12 = faceHlp.GetMiddleUV(surf, uv1, uv2);
                                // uvMove = uvm - uv12
                                gp_XY uvMove = ApplyIn2D(surf, uvm, uv12, gp_XY_Subtracted,
                                                         /*inPeriod=*/false);
                            ...
                            gp_XY uvm =
                                faceHlp.GetNodeUV(face, linkOnFace->_mediumNode, 0, &checkUV);
                            gp_XY uv1 =
                                faceHlp.GetNodeUV(face, linkOnFace->node1(), nodeOnFace, &checkUV);
                            gp_XY uv2 =
                                faceHlp.GetNodeUV(face, linkOnFace->node2(), nodeOnFace, &checkUV);
                            gp_XY uv12 = faceHlp.GetMiddleUV(surf, uv1, uv2);
                    ...
                    for (int i = 0; link2 != chain.end(); ++link0, ++link1, ++link2, ++i) {
                        ...
                        if (isInside || face.IsNull()) {
                            ...
                            try {
                                ...
                                trsf.SetTransformation(gp_Ax3(gp::Origin(), link1->Normal(), x),
                                                       gp_Ax3());
                            }
                            catch (Standard_Failure &) {
                                trsf.Invert();
                            ...
                            gp_XY oldUV =
                                faceHlp.GetNodeUV(face, (*link1)->_mediumNode, 0, &checkUV);
                            gp_XY newUV =
                                ApplyIn2D(s, oldUV, gp_XY(move.X(), move.Y()), gp_XY_Added);
                            gp_Pnt newPnt = s->Value(newUV.X(), newUV.Y());
                            move = gp_Vec(XYZ((*link1)->_mediumNode), newPnt.Transformed(loc));
                            if (SMDS_FacePosition *nPos = dynamic_cast<SMDS_FacePosition *>(
                                    (*link1)->_mediumNode->GetPosition()))
                                nPos->SetParameters(newUV.X(), newUV.Y());
#ifdef _DEBUG_
                            if ((XYZ((*link1)->node1()) - XYZ((*link1)->node2())).SquareModulus()
                                < move.SquareMagnitude()) {
                                gp_XY uv0 =
                                    faceHlp.GetNodeUV(face, (*link0)->_mediumNode, 0, &checkUV);
                                gp_XY uv2 =
                                    faceHlp.GetNodeUV(face, (*link2)->_mediumNode, 0, &checkUV);
                                MSG("TOO LONG MOVE \t"
                                    << "uv0: " << uv0.X() << ", " << uv0.Y() << " \t"
                                    << "uv2: " << uv2.X() << ", " << uv2.Y() << " \t"
                        ...
                        MSG("Move " << (*link1)->_mediumNode->GetID() << " following "
                                    ...
                                    << chain.back()->_mediumNode->GetID() << " by "
                                    << move.Magnitude());
                    }
    ...
    const bool toFixCentralNodes = (myMesh->NbBiQuadQuadrangles() + myMesh->NbBiQuadTriangles()
            ...
            if (!pLink->IsFixedOnSurface()) {
                ...
                if (faceHlp.GetSubShape().ShapeType() == TopAbs_FACE) {
                    ...
                    if (faceHlp.CheckNodeUV(TopoDS::Face(faceHlp.GetSubShape()), pLink->_mediumNode,
                                            uv, /*tol=*/pLink->Move().Modulus(), /*force=*/true,
                                            distXYZ))
    ...
    // treat bi-quad quadrangles
    {
        ...
        for (; quadIt != biQuadQuas.end(); ++quadIt) {
            ...
            gp_XY uvCent =
                calcTFI(0.5, 0.5, uv[0], uv[1], uv[2], uv[3], uv[4], uv[5], uv[6], uv[7]);
    ...
    // treat bi-quad triangles
    {
        ...
        for (; triIt != biQuadTris.end(); ++triIt) {
            ...
            gp_Pnt p;
            if (!uvOK || badTria) {
                p = (SMESH_TNodeXYZ(nodes[3]) + SMESH_TNodeXYZ(nodes[4]) + SMESH_TNodeXYZ(nodes[5]))
                    / 3;
            }
            else {
                AdjustByPeriod(F, uv, 6); // put uv[] within a period (IPAL52698)
        ...
        for (; hexIt != triQuadHexa.end(); ++hexIt) {
            ...
            for (int iQuad = 0; iQuad < volExp.NbFaces(); ++iQuad) {
                ...
                if (quadNodes[8]->GetPosition()->GetTypeOfPosition() == SMDS_TOP_3DSPACE) {
                    gp_XYZ p = calcTFI(0.5, 0.5, SMESH_TNodeXYZ(quadNodes[0]),
                                       SMESH_TNodeXYZ(quadNodes[2]), SMESH_TNodeXYZ(quadNodes[4]),
                                       SMESH_TNodeXYZ(quadNodes[6]), SMESH_TNodeXYZ(quadNodes[1]),
                                       SMESH_TNodeXYZ(quadNodes[3]), SMESH_TNodeXYZ(quadNodes[5]),
                                       SMESH_TNodeXYZ(quadNodes[7]));
            ...
            GetMeshDS()->MoveNode(hexNodes[26], nCenterCoords.X(), nCenterCoords.Y(),
                                  nCenterCoords.Z());
        }
    }
}

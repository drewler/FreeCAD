                                                ...
                                                const MED::TEntityInfo &theEntityInfo)
{
    ...
    for (; anIter != theEntityInfo.end(); anIter++) {
        ...
        for (; anIter2 != aGeom2Size.end(); anIter2++) {
            ...
            aGeom2ElemInfo[aGeom] =
                theWrapper->GetPElemInfo(theMeshInfo, anEntity, aGeom, MED::eNOD, &anErr);
        }
    }
    ADDMSG(MYDEBUG, "\n");
...
TFamilyInfoSet GetFamilyInfoSet(const PWrapper &theWrapper, const PMeshInfo &theMeshInfo)
{
    ...
    for (TInt iFam = 1; iFam <= aNbFam; iFam++) {
        ...
        if (anErr >= 0) aFamilyInfoSet.insert(aFamilyInfo);
    }
    ADDMSG(MYDEBUG, "\n");
                                                          ...
                                                          const MED::TEntityInfo &theEntityInfo)
{
    ...
    for (TInt iField = 1; iField <= aNbFields; iField++) {
        ...
        TInt aNbTimeStamps =
            theWrapper->GetNbTimeStamps(aFieldInfo, theEntityInfo, anEntity, aGeom2Size);
        ADDMSG(MYDEBUG,
               "anEntity = " << anEntity << "; GetNbTimeStamps = " << aNbTimeStamps << "\n");
        for (TInt iTimeStamp = 1; iTimeStamp <= aNbTimeStamps; iTimeStamp++) {
            ...
            INITMSG(MYDEBUG,
                    "aDt = " << aTimeStamp->GetDt() << ", Unit = \'" << aTimeStamp->GetUnitDt()
                             << "\n");
        }
    }
    ADDMSG(MYDEBUG, "\n");
...
//---------------------------------------------------------------
TEntite2TFieldInfo2TimeStampInfoSet GetEntite2TFieldInfo2TimeStampInfoSet(
    const TFieldInfo2TimeStampInfoSet &theFieldInfo2TimeStampInfoSet)
{
    ...
    for (; anIter != theFieldInfo2TimeStampInfoSet.end(); anIter++) {
        ...
        if (aTimeStampInfoSet.empty()) continue;
        const PTimeStampInfo &aTimeStampInfo = *aTimeStampInfoSet.begin();
        anEntite2TFieldInfo2TimeStampInfoSet[ConvertEntity(aTimeStampInfo->GetEntity())].insert(
            *anIter);
    }
...
//---------------------------------------------------------------
TEntity2FamilySet GetEntity2FamilySet(const PWrapper &theWrapper,
...
TKey2Gauss GetKey2Gauss(const PWrapper &theWrapper, TErr *theErr, EModeSwitch theMode)
{
    ...
    for (TInt anId = 1; anId <= aNbGauss; anId++) {
        ...
        INITMSG(MYDEBUG, "- aGeom = " << aGeom << "; aName = '" << aName << "'" << std::endl);
#endif
    }
    return aKey2Gauss;
}
...
//---------------------------------------------------------------
PProfileInfo GetProfileInfo(const PWrapper &theWrapper, const std::string &theProfileName,
                            TErr *theErr, EModeProfil theMode)
{
    PProfileInfo anInfo;
    ...
    for (TInt anId = 1; anId <= aNbProfiles; anId++) {
        ...
        if (aName == theProfileName) return theWrapper->GetPProfileInfo(anId, theMode, theErr);
    }
    return anInfo;
}

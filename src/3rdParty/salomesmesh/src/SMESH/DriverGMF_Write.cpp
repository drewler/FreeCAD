...
Control_Pnt::Control_Pnt() : gp_Pnt() { size = 0; }
Control_Pnt::Control_Pnt(const gp_Pnt &aPnt, double theSize) : gp_Pnt(aPnt) { size = theSize; }
Control_Pnt::Control_Pnt(double theX, double theY, double theZ) : gp_Pnt(theX, theY, theZ)
{
    size = 0;
}
Control_Pnt::Control_Pnt(double theX, double theY, double theZ, double theSize)
    : gp_Pnt(theX, theY, theZ)
{
    size = theSize;
}

DriverGMF_Write::DriverGMF_Write() : Driver_SMESHDS_Mesh(), _exportRequiredGroups(true) {}
...
Driver_Mesh::Status DriverGMF_Write::Perform()
{
    ...
    node2IdMap[edge->GetNode(0)], node2IdMap[edge->GetNode(1)],
        END_ELEM_WRITE_ADD_TO_MAP(edge, edge2IDMap);

    // nodes of quadratic edges
    BEGIN_EXTRA_VERTICES_WRITE(SMDSGeom_EDGE, SMDSEntity_Edge, GmfExtraVerticesAtEdges, edge)
    node2IdMap[edge->GetNode(2)] END_EXTRA_VERTICES_WRITE();

    // triangles
    TElem2IDMap tria2IDMap;
    BEGIN_ELEM_WRITE(SMDSGeom_TRIANGLE, GmfTriangles, tria)
    node2IdMap[tria->GetNode(0)], node2IdMap[tria->GetNode(1)], node2IdMap[tria->GetNode(2)],
        END_ELEM_WRITE_ADD_TO_MAP(tria, tria2IDMap);

    // nodes of quadratic triangles
    BEGIN_EXTRA_VERTICES_WRITE(SMDSGeom_TRIANGLE, SMDSEntity_Triangle, GmfExtraVerticesAtTriangles,
                               tria)
    node2IdMap[tria->GetNode(3)], node2IdMap[tria->GetNode(4)], node2IdMap[tria->GetNode(5)],
    ...
    node2IdMap[quad->GetNode(0)], node2IdMap[quad->GetNode(1)], node2IdMap[quad->GetNode(2)],
        node2IdMap[quad->GetNode(3)], END_ELEM_WRITE_ADD_TO_MAP(quad, quad2IDMap);
    ...
    node2IdMap[quad->GetNode(4)], node2IdMap[quad->GetNode(5)], node2IdMap[quad->GetNode(6)],
    ...
    node2IdMap[tetra->GetNode(0)], node2IdMap[tetra->GetNode(2)], node2IdMap[tetra->GetNode(1)],
        node2IdMap[tetra->GetNode(3)], END_ELEM_WRITE(tetra);

    // nodes of quadratic terahedra
    BEGIN_EXTRA_VERTICES_WRITE(SMDSGeom_TETRA, SMDSEntity_Tetra, GmfExtraVerticesAtTetrahedra,
                               tetra)
    node2IdMap[tetra->GetNode(6)], node2IdMap[tetra->GetNode(5)], node2IdMap[tetra->GetNode(4)],
        node2IdMap[tetra->GetNode(7)], node2IdMap[tetra->GetNode(9)],
    ...
    node2IdMap[pyra->GetNode(0)], node2IdMap[pyra->GetNode(2)], node2IdMap[pyra->GetNode(1)],
        node2IdMap[pyra->GetNode(3)], node2IdMap[pyra->GetNode(4)], END_ELEM_WRITE(pyra);

    // hexahedra
    BEGIN_ELEM_WRITE(SMDSGeom_HEXA, GmfHexahedra, hexa)
    node2IdMap[hexa->GetNode(0)], node2IdMap[hexa->GetNode(3)], node2IdMap[hexa->GetNode(2)],
        node2IdMap[hexa->GetNode(1)], node2IdMap[hexa->GetNode(4)], node2IdMap[hexa->GetNode(7)],
        node2IdMap[hexa->GetNode(6)], node2IdMap[hexa->GetNode(5)], END_ELEM_WRITE(hexa);

    // nodes of quadratic hexahedra
    BEGIN_EXTRA_VERTICES_WRITE(SMDSGeom_HEXA, SMDSEntity_Hexa, GmfExtraVerticesAtHexahedra, hexa)
    node2IdMap[hexa->GetNode(11)], // HEXA20
        node2IdMap[hexa->GetNode(10)], node2IdMap[hexa->GetNode(9)], node2IdMap[hexa->GetNode(8)],
        node2IdMap[hexa->GetNode(15)], node2IdMap[hexa->GetNode(14)], node2IdMap[hexa->GetNode(13)],
        node2IdMap[hexa->GetNode(12)], node2IdMap[hexa->GetNode(16)], node2IdMap[hexa->GetNode(19)],
        node2IdMap[hexa->GetNodeWrap(18)], // + HEXA27
        node2IdMap[hexa->GetNodeWrap(17)], node2IdMap[hexa->GetNodeWrap(20)],
        node2IdMap[hexa->GetNodeWrap(24)], node2IdMap[hexa->GetNodeWrap(23)],
        node2IdMap[hexa->GetNodeWrap(22)], node2IdMap[hexa->GetNodeWrap(21)],
        node2IdMap[hexa->GetNodeWrap(25)],
        node2IdMap[hexa->GetNodeWrap(26)] END_EXTRA_VERTICES_WRITE();

    // prism
    BEGIN_ELEM_WRITE(SMDSEntity_Penta, GmfPrisms, prism)
    node2IdMap[prism->GetNode(0)], node2IdMap[prism->GetNode(2)], node2IdMap[prism->GetNode(1)],
        node2IdMap[prism->GetNode(3)], node2IdMap[prism->GetNode(5)], node2IdMap[prism->GetNode(4)],
    ...
    if (_exportRequiredGroups) {
        ...
        for (; grIt != groupSet.end(); ++grIt) {
            ...
            while (elemIt->more()) nbOkElems += (elemIt->next()->GetEntityType() == smdsEntity);
            ...
            if (elem2IDMap)
                ...
                for (int gmfID = 1; elemIt->more(); ++gmfID) {
                    const SMDS_MeshElement *elem = elemIt->next();
                    if (elem->GetEntityType() == smdsEntity) GmfSetLin(meshID, gmfKwd, gmfID);
                }

        } // loop on groups

        if (!badGroups.empty())
            addMessage(SMESH_Comment("Groups of elements of inappropriate geometry:") << badGroups,
                       /*fatal=*/false);
    }

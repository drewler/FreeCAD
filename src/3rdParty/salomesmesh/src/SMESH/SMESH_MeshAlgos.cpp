...
struct SMESH_NodeSearcherImpl: public SMESH_NodeSearcher {
   ...
   */
    SMESH_NodeSearcherImpl(const SMDS_Mesh *theMesh)
    {
        ...
        if (theMesh) {
            ...
            while (nIt->more()) nodes.insert(nodes.end(), nIt->next());
        }
   ...
   */
    const SMDS_MeshNode *FindClosestTo(const gp_Pnt &thePnt)
    {
        ...
        if (!dist2Nodes.empty()) return dist2Nodes.begin()->second;
        ...
        if (nodes.empty()) // get all nodes of OctreeNode's closest to thePnt
        {
            ...
            for (trIt = treeList.begin(); trIt != treeList.end(); ++trIt) {
                ...
                else if (tree->NbNodes()) // put a tree to the treeMap
                {
                    ...
                    double sqDist =
                        thePnt.SquareDistance(0.5 * (box.CornerMin() + box.CornerMax()));
                    pair<TDistTreeMap::iterator, bool> it_in =
                        treeMap.insert(make_pair(sqDist, tree));
                    if (!it_in.second) // not unique distance to box center
                        treeMap.insert(it_in.first,
                                       make_pair(sqDist + 1e-13 * treeMap.size(), tree));
                }
            }
            ...
            for (; sqDist_tree != treeMap.end(); ++sqDist_tree) {
                if (sqDist_tree->first > sqLimit) break;
   ...
   */
    int FindNearPoint(const gp_Pnt &point, const double tolerance,
...
public:
    ...
    void getElementsInSphere(const gp_XYZ &center, const double radius,
...
ElementBndBoxTree::ElementBndBoxTree(const SMDS_Mesh &mesh, SMDSAbs_ElementType elemType,
                                     SMDS_ElemIteratorPtr theElemIt, double tolerance)
    : SMESH_Octree(new SMESH_TreeLimit(MaxLevel, /*minSize=*/0.))
{
    ...
    while (elemIt->more()) _elements.push_back(new ElementBox(elemIt->next(), tolerance));
...
ElementBndBoxTree::~ElementBndBoxTree()
{
    for (int i = 0; i < _elements.size(); ++i)
        if (--_elements[i]->_refCount <= 0) delete _elements[i];
}
...
Bnd_B3d *ElementBndBoxTree::buildRootBox()
{
    ...
    for (int i = 0; i < _elements.size(); ++i) box->Add(*_elements[i]);
    return box;
}
...
void ElementBndBoxTree::buildChildrenData()
{
    ...
    for (int j = 0; j < 8; j++) {
        ...
        if (child->_elements.size() <= MaxNbElemsInLeaf) child->myIsLeaf = true;
...
void ElementBndBoxTree::getElementsNearPoint(const gp_Pnt &point, TIDSortedElemSet &foundElems)
{
    if (getBox()->IsOut(point.XYZ())) return;

    if (isLeaf()) {
        for (int i = 0; i < _elements.size(); ++i)
            if (!_elements[i]->IsOut(point.XYZ())) foundElems.insert(_elements[i]->_element);
    }
    else {
        for (int i = 0; i < 8; i++)
...
void ElementBndBoxTree::getElementsNearLine(const gp_Ax1 &line, TIDSortedElemSet &foundElems)
{
    if (getBox()->IsOut(line)) return;

    if (isLeaf()) {
        for (int i = 0; i < _elements.size(); ++i)
            if (!_elements[i]->IsOut(line)) foundElems.insert(_elements[i]->_element);
    }
    else {
        for (int i = 0; i < 8; i++)
...
void ElementBndBoxTree::getElementsInSphere(const gp_XYZ &center, const double radius,
                                            TIDSortedElemSet &foundElems)
{
    if (getBox()->IsOut(center, radius)) return;

    if (isLeaf()) {
        for (int i = 0; i < _elements.size(); ++i)
            if (!_elements[i]->IsOut(center, radius)) foundElems.insert(_elements[i]->_element);
    }
    else {
        for (int i = 0; i < 8; i++)
...
ElementBndBoxTree::ElementBox::ElementBox(const SMDS_MeshElement *elem, double tolerance)
{
    ...
    while (nIt->more()) Add(SMESH_TNodeXYZ(nIt->next()));
    Enlarge(tolerance);
}
...
struct SMESH_ElementSearcherImpl: public SMESH_ElementSearcher {
    ...
    SMESH_ElementSearcherImpl(SMDS_Mesh &mesh, double tol = -1,
                              SMDS_ElemIteratorPtr elemIt = SMDS_ElemIteratorPtr())
        : _mesh(&mesh), _meshPartIt(elemIt), _ebbTree(0), _nodeSearcher(0), _tolerance(tol),
          _outerFacesFound(false)
    {}
    ...
    void GetElementsNearLine(const gp_Ax1 &line, SMDSAbs_ElementType type,
...
bool SMESH_ElementSearcherImpl::getIntersParamOnLine(const gp_Lin &line,
                                                     const SMDS_MeshElement *face, const double tol,
                                                     double &param)
{
    int nbInts = 0;
    param = 0;
...
void SMESH_ElementSearcherImpl::findOuterBoundary(const SMDS_MeshElement *outerFace)
{
    ...
    while (!startLinks.empty()) {
        ...
        if (faces.size() == 2) {
            ...
            gp_Vec n1n2(SMESH_TNodeXYZ(link.node1()), SMESH_TNodeXYZ(link.node2()));
            ...
            if (SMESH_MeshAlgos::FaceNormal(outerFace, ofNorm, /*normalized=*/false)) {
                ...
                if (!angle2Face.empty()) outerFace2 = angle2Face.begin()->second;
            }
        }
...
int SMESH_ElementSearcherImpl::FindElementsByPoint(const gp_Pnt &point, SMDSAbs_ElementType type,
                                                   vector<const SMDS_MeshElement *> &foundElements)
{
    foundElements.clear();
    ...
    if (type == SMDSAbs_Node || type == SMDSAbs_0DElement || type == SMDSAbs_Ball) {
        if (!_nodeSearcher) _nodeSearcher = new SMESH_NodeSearcherImpl(_mesh);

        std::vector<const SMDS_MeshNode *> foundNodes;
        _nodeSearcher->FindNearPoint(point, tolerance, foundNodes);

        if (type == SMDSAbs_Node) { foundElements.assign(foundNodes.begin(), foundNodes.end()); }
        else {
            for (size_t i = 0; i < foundNodes.size(); ++i) {
                SMDS_ElemIteratorPtr elemIt = foundNodes[i]->GetInverseElementIterator(type);
                while (elemIt->more()) foundElements.push_back(elemIt->next());
            }
        }
    }
    // =================================================================================
    else // elements more complex than 0D
    {
        ...
        for (; elem != suspectElems.end(); ++elem)
            if (!SMESH_MeshAlgos::IsOut(*elem, point, tolerance)) foundElements.push_back(*elem);
    }
                                                                 ...
                                                                 SMDSAbs_ElementType type)
{
    ...
    if (type == SMDSAbs_Face || type == SMDSAbs_Volume) {
        ...
        if (suspectElems.empty() && _ebbTree->maxSize() > 0) {
            gp_Pnt boxCenter =
                0.5 * (_ebbTree->getBox()->CornerMin() + _ebbTree->getBox()->CornerMax());
            double radius = -1;
            if (_ebbTree->getBox()->IsOut(point.XYZ()))
                radius = point.Distance(boxCenter) - 0.5 * _ebbTree->maxSize();
            if (radius < 0) radius = _ebbTree->maxSize() / pow(2., _ebbTree->getHeight()) / 2;
        ...
        if (!dist2face.empty()) {
            ...
            for (++d2f; d2f != dist2face.end() && fabs(d2f->first - minDist) < 1e-10; ++d2f) {
                if (minDistToGC == 0) {
                    gp_XYZ gc(0, 0, 0);
                    gc = accumulate(TXyzIterator(closestElem->nodesIterator()), TXyzIterator(), gc)
                        / closestElem->NbNodes();
                    minDistToGC = point.SquareDistance(gc);
                }
                gp_XYZ gc(0, 0, 0);
                gc = accumulate(TXyzIterator(d2f->second->nodesIterator()), TXyzIterator(), gc)
                    / d2f->second->NbNodes();
...
TopAbs_State SMESH_ElementSearcherImpl::GetPointState(const gp_Pnt &point)
{
    ...
    for (int axis = 0; axis < nbAxes; ++axis) {
        ...
        for (; face != suspectFaces.end(); ++face) {
            ...
            if (!intersection.IsDone()) continue;
        ...
        if (nbInter == 0) return TopAbs_OUT;
        ...
        if (fabs(f) < tolerance || fabs(l) < tolerance) return TopAbs_ON;

        if ((f < 0) == (l < 0)) return TopAbs_OUT;

        int nbIntBeforePoint = std::distance(u2inters.begin(), u2inters.lower_bound(0));
        int nbIntAfterPoint = nbInter - nbIntBeforePoint;
        if (nbIntBeforePoint == 1 || nbIntAfterPoint == 1) return TopAbs_IN;
    ...
    for (int hasPositionInfo = _outerFacesFound; hasPositionInfo < 2; ++hasPositionInfo) {
        ...
        for (; nb_axis != nbInt2Axis.end(); ++nb_axis) {
            ...
            while (ok && u_int1 != u2inters.end()) {
                ...
                if (++u_int2 != u2inters.end()) {
                    ...
                    if (nbSamePnt + nbTgt > 0) {
                        double minDot = numeric_limits<double>::max(),
                               maxDot = -numeric_limits<double>::max();
            ...
            if (ok) {
                ...
                if (nbIntBeforePoint == 0 || nbIntAfterPoint == 0) return TopAbs_OUT;
                ...
                if (hasPositionInfo) return nbIntBeforePoint % 2 ? TopAbs_IN : TopAbs_OUT;
            }
...
void SMESH_ElementSearcherImpl::GetElementsNearLine(const gp_Ax1 &line, SMDSAbs_ElementType type,
                                                    vector<const SMDS_MeshElement *> &foundElems)
{
    if (!_ebbTree || _elementType != type) {
        if (_ebbTree) delete _ebbTree;
...
bool SMESH_MeshAlgos::IsOut(const SMDS_MeshElement *element, const gp_Pnt &point, double tol)
{
    ...
    if (element->GetType() == SMDSAbs_Face) // --------------------------------------------------
    {
        ...
        if (normSize <= tol) {
            ...
            for (i = 0; i < nbNodes; ++i) {
                ...
                if (!IsOut(&edge, point, tol)) return false;
            }
            return true;
        }
        faceNorm /= normSize;
        ...
        if (fabs(n2p * faceNorm) > tol) return true; // not on face plane
        ...
        if (iClosest < 0) return true; // no intesections - out
    ...
    if (element->GetType() == SMDSAbs_Edge) // --------------------------------------------------
    {
        ...
        for (i = 1; i < nbNodes; ++i) {
            ...
            if (u <= 0.) {
                if (n1p.SquareMagnitude() < tol * tol) return false;
                continue;
            }
            if (u >= 1.) {
                if (point.SquareDistance(xyz[i]) < tol * tol) return false;
                continue;
            }
...
PointPos getPointPosition(const gp_XY &point2D, const gp_XY *segEnds, const int index0 = 0,
                          const int posToFindOut = POS_ALL)
{
    ...
    if (posToFindOut & POS_VERTEX) {
        ...
        gp_XY pp1[2] = {p1, gp_XY(p1.X() - grad.Y(), p1.Y() + grad.X())};
        ...
        gp_XY pp2[2] = {p2, gp_XY(p2.X() - grad.Y(), p2.Y() + grad.X())};
        if (getPointPosition(point2D, pp2, 0, POS_LEFT | POS_RIGHT)._name == POS_RIGHT)
            return PointPos(POS_VERTEX, index0 + 1);
    }
    double edgeEquation = (point2D.X() - p1.X()) * grad.Y() - (point2D.Y() - p1.Y()) * grad.X();
...
{
    switch (elem->GetType()) {
        case SMDSAbs_Volume: return GetDistance(dynamic_cast<const SMDS_MeshVolume *>(elem), point);
        case SMDSAbs_Face: return GetDistance(dynamic_cast<const SMDS_MeshFace *>(elem), point);
        case SMDSAbs_Edge: return GetDistance(dynamic_cast<const SMDS_MeshEdge *>(elem), point);
        case SMDSAbs_Node: return point.Distance(SMESH_TNodeXYZ(elem));
    }
    return -1;
}
...
double SMESH_MeshAlgos::GetDistance(const SMDS_MeshVolume *volume, const gp_Pnt &point)
{
    ...
    for (int iF = 0; iF < vTool.NbFaces(); ++iF) {
        ...
        if (gp_XYZ(n[0], n[1], n[2]) * bcp < 1e-6) continue;
...
void SMESH_MeshAlgos::GetBarycentricCoords(const gp_XY &p, const gp_XY &t0, const gp_XY &t1,
                                           const gp_XY &t2, double &bc0, double &bc1)
{
    const double // matrix 2x2
        T11 = t0.X() - t2.X(),
        T12 = t1.X() - t2.X(), T21 = t0.Y() - t2.Y(), T22 = t1.Y() - t2.Y();
...
const SMDS_MeshElement *SMESH_MeshAlgos::FindFaceInSet(const SMDS_MeshNode *n1,
                                                       ...
                                                       const TIDSortedElemSet &avoidSet, int *n1ind,
                                                       int *n2ind)

{
    ...
    while (invElemIt->more() && !face) // loop on inverse faces of n1
    {
        ...
        if (avoidSet.count(elem)) continue;
        if (!elemSet.empty() && !elemSet.count(elem)) continue;
        // index of n1
        ...
        for (int di = -1; di < 2 && !face; di += 2) {
            i2 = (i1 + di + nbN) % nbN;
            if (elem->GetNode(i2) == n2) face = elem;
        }
...
{
    if (!F || F->GetType() != SMDSAbs_Face) return false;
    ...
    if (normalized && ok) normal /= sqrt(size2);
                                                              ...
                                                              const SMDS_MeshElement *e2)
{
    ...
    for (int i = 0; i < e1->NbNodes(); ++i)
        if (e2->GetNodeIndex(e1->GetNode(i)) >= 0) common.push_back(e1->GetNode(i));
    return common;
}
...
SMESH_ElementSearcher *SMESH_MeshAlgos::GetElementSearcher(SMDS_Mesh &mesh, double tolerance)
{
...
SMESH_MeshAlgos::GetElementSearcher(SMDS_Mesh &mesh, SMDS_ElemIteratorPtr elemIt, double tolerance)
{

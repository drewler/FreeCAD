...
SMESH_subMesh::~SMESH_subMesh() { deleteOwnListeners(); }
...
SMESHDS_SubMesh *SMESH_subMesh::GetSubMeshDS()
{
    ...
    return _subMeshDS ? _subMeshDS
                      : _subMeshDS = _father->GetMeshDS()->MeshElements(_subShape); // may be null
}
...
SMESH_subMesh *SMESH_subMesh::GetFirstToCompute()
{
    ...
    while (smIt->more()) {
        ...
        if (sm->GetComputeState() == READY_TO_COMPUTE) return sm;
    }
...
void SMESH_subMesh::SetIsAlwaysComputed(bool isAlCo)
{
    ...
    if (_alwaysComputed) _computeState = COMPUTE_OK;
    else
...
{
    if (_alwaysComputed) return true;
...
bool SMESH_subMesh::SubMeshesComputed(bool *isFailedToCompute /*=0*/) const
{
    ...
    while (smIt->more()) {
        SMESH_subMesh *sm = smIt->next();
        if (sm->_alwaysComputed) continue;
...
int SMESH_subMesh::GetComputeCost() const { return _realComputeCost; }
...
int SMESH_subMesh::computeCost() const
{
    if (!_computeCost) {
        ...
        while (childIt->more()) computeCost += childIt->next()->computeCost();
...
{
    if (_dependenceAnalysed || !_father->HasShapeToMesh()) return _mapDepend;
...
void SMESH_subMesh::insertDependence(const TopoDS_Shape aShape, TopAbs_ShapeEnum aSubType)
{
    TopExp_Explorer sub(aShape, aSubType);
    for (; sub.More(); sub.Next()) {
        SMESH_subMesh *aSubMesh = _father->GetSubMesh(sub.Current());
        if (aSubMesh->GetId() == 0) continue; // not a sub-shape of the shape to mesh
...
const TopoDS_Shape &SMESH_subMesh::GetSubShape() const { return _subShape; }
...
bool SMESH_subMesh::CanAddHypothesis(const SMESH_Hypothesis *theHypothesis) const
{
    ...
    //     return false;
    // }
    if (aHypDim <= aShapeDim) return true;
                                          ...
                                          const TopAbs_ShapeEnum theShapeType)
{
    ...
    switch (theShapeType) {
        ...
        case TopAbs_EDGE:
        case TopAbs_FACE:
        case TopAbs_SOLID: return SMESH_Gen::GetShapeDim(theShapeType) == theHypothesis->GetDim();
                                                                   ...
                                                                   SMESH_Hypothesis *anHyp)
{
    ...
    if (_subShape.ShapeType() == TopAbs_VERTEX) {
        if (anHyp->GetDim() != 0) {
            if (event == ADD_HYP || event == ADD_ALGO) return SMESH_Hypothesis::HYP_BAD_DIM;
            else
    ...
    if (event == ADD_ALGO || event == ADD_FATHER_ALGO) {
        ...
        if (event == ADD_ALGO) {
            ...
            if (SMESH_Algo *curAlgo = (SMESH_Algo *)_father->GetHypothesis(this, filter, true))
                if (!curAlgo->NeedDiscreteBoundary()) algoRequiringCleaning = curAlgo;
        }
    }
    ...
    if (event == ADD_HYP || event == ADD_ALGO) {
        ...
        if (!meshDS->AddHypothesis(_subShape, anHyp)) return SMESH_Hypothesis::HYP_ALREADY_EXIST;
    }
    ...
    if (event == REMOVE_HYP || event == REMOVE_ALGO) {
        ...
        if (event == REMOVE_ALGO) {
            ...
            if (!algo->NeedDiscreteBoundary()) algoRequiringCleaning = algo;
        }
    }
    ...
    // analyse algo state
    // ------------------
    if (!isApplicableHyp) return ret; // not applicable hypotheses do not change algo state

    if ((algo = GetAlgo())) algo->InitComputeError();
        ...
        case NO_ALGO:
            switch (event) {
                ...
                case ADD_ALGO: {
                    algo = GetAlgo();
                    ASSERT(algo);
                    if (algo->CheckHypothesis((*_father), _subShape, aux_ret)) setAlgoState(HYP_OK);
        ...
        case MISSING_HYP:
            switch (event) {
                case ADD_HYP: {
                    algo = GetAlgo();
                    ASSERT(algo);
                    if (algo->CheckHypothesis((*_father), _subShape, ret)) setAlgoState(HYP_OK);
            ...
            switch (event) {
                ...
                case ADD_ALGO: { //already existing algo : on father ?
                    ...
                    if (algo->CheckHypothesis((*_father), _subShape, aux_ret)) {
                        ...
                        if (prevAlgo && string(algo->GetName()) != prevAlgo->GetName())
                            modifiedHyp = true;
                    }
                    else
                        setAlgoState(MISSING_HYP);
                ...
                case ADD_FATHER_ALGO: {
                    ...
                    if (algo == anHyp) { // a new algo on father
                        if (algo->CheckHypothesis((*_father), _subShape, aux_ret)) {
                            ...
                            f.And(SMESH_HypoFilter::IsApplicableTo(_subShape));
                            f.AndNot(SMESH_HypoFilter::Is(algo));
                            const SMESH_Hypothesis *prevAlgo =
                                _father->GetHypothesis(this, f, true);
                            if (prevAlgo && string(algo->GetName()) != string(prevAlgo->GetName()))
                                modifiedHyp = true;
                        }
                        else
                            setAlgoState(MISSING_HYP);
                ...
                case REMOVE_FATHER_HYP: {
                    ...
                    if (algo->CheckHypothesis((*_father), _subShape, aux_ret)) {
                        ...
                        if (!getSimilarAttached(_subShape, anHyp)) modifiedHyp = true;
                    }
                    else
                        setAlgoState(MISSING_HYP);
                    break;
    ...
    //
    if (ret == SMESH_Hypothesis::HYP_OK && (event == ADD_ALGO || event == ADD_FATHER_ALGO) && algo
        ...
        // is algo hiding?
        if (ret == SMESH_Hypothesis::HYP_OK && !algo->NeedDiscreteBoundary()
            ...
            for (; (ret == SMESH_Hypothesis::HYP_OK && i_sm != _mapDepend.rend()); ++i_sm)
                if (gen->GetAlgo(i_sm->second, &otherAssignedTo)
                    && SMESH_MesherHelper::IsSubShape(/*sub=*/otherAssignedTo,
                                                      /*main=*/algoAssignedTo))
    ...
    if (event == REMOVE_ALGO || event == REMOVE_FATHER_ALGO) _algo = 0;
    ...
    if (stateChange || modifiedHyp) ComputeStateEngine(MODIF_ALGO_STATE);
...
bool SMESH_subMesh::IsConform(const SMESH_Algo *theAlgo)
{
    ...
    if (_subShape.ShapeType() == _father->GetMeshDS()->ShapeToMesh().ShapeType()) return true;
    ...
    for (; itsub.More(); itsub.Next()) {
        ...
        for (size_t iA = 0; iA < ancestors.size(); ++iA) {
            ...
            if (adjacent.ShapeType() != _subShape.ShapeType()) break;

            // check algo attached to smAdjacent
            SMESH_Algo *algo = ancestors[iA]->GetAlgo();
            if (algo && !algo->NeedDiscreteBoundary() && algo->OnlyUnaryInput())
...
void SMESH_subMesh::setAlgoState(algo_state state) { _algoState = state; }
...
SMESH_subMesh::SubMeshesAlgoStateEngine(int event, SMESH_Hypothesis *anHyp, bool exitOnFatal)
{
    ...
    //if (_subShape.ShapeType() < TopAbs_EDGE ) // wire,face etc
    {
        ...
        while (smIt->more()) {
            ...
            if (ret2 > ret) {
                ...
                if (exitOnFatal && SMESH_Hypothesis::IsStatusFatal(ret)) break;
            }
        }
    }
    return ret;
...
void SMESH_subMesh::cleanDependsOn(SMESH_Algo *algoRequiringCleaning /*=0*/)
{
    ...
    if (_father->NbNodes() == 0) {
        while (smIt->more()) smIt->next()->ComputeStateEngine(CHECK_COMPUTE_STATE);
    }
    else if (!algoRequiringCleaning || !algoRequiringCleaning->SupportSubmeshes()) {
        while (smIt->more()) smIt->next()->ComputeStateEngine(CLEAN);
    }
    else if (algoRequiringCleaning && algoRequiringCleaning->SupportSubmeshes()) {
        SMESHDS_Mesh *meshDS = _father->GetMeshDS();
            ...
            if (!sm->IsEmpty()) {
                ...
                if (!keepSubMeshes) {
                    ...
                    const list<const SMESHDS_Hypothesis *> &hyps =
                        meshDS->GetHypothesis(sm->_subShape);
                ...
                if (keepSubMeshes) {
                    ...
                    while (smIt2->more()) smToKeep.insert(smIt2->next());
                }
            }
        }
        // remove elements
        ...
        while (smIt->more()) {
            SMESH_subMesh *sm = smIt->next();
            if (!smToKeep.count(sm)) sm->ComputeStateEngine(CLEAN);
        }
    }
}
...
void SMESH_subMesh::DumpAlgoState(bool isMain)
{
    ...
    MESSAGE("dim = " << SMESH_Gen::GetShapeDim(_subShape) << " type of shape "
                     << _subShape.ShapeType());
    switch (_algoState) {
    ...
    if (subMesh) {
        if (SMESHDS_SubMesh *subMeshDS = subMesh->GetSubMeshDS()) {
            ...
            while (itn->more()) {
                ...
                if (node->NbInverseElements() == 0) meshDS->RemoveFreeNode(node, 0);
...
bool SMESH_subMesh::ComputeStateEngine(int event)
{
    ...
    if (event == CLEAN) _alwaysComputed = false; // Unset 'true' set by MergeNodes() (issue 0022182)
    ...
    if (_subShape.ShapeType() == TopAbs_VERTEX) {
        ...
        else if ((event == COMPUTE || event == COMPUTE_SUBMESH) && !_alwaysComputed) {
        ...
        if (event == MODIF_ALGO_STATE) cleanDependants();
        return true;
    }
    SMESH_Gen *gen = _father->GetGen();
    SMESH_Algo *algo = 0;
    ...
    switch (_computeState) {
        ...
        case NOT_READY:
            switch (event) {
                case MODIF_ALGO_STATE:
                    ...
                    if (_algoState == HYP_OK) _computeState = READY_TO_COMPUTE;
                    break;
                case COMPUTE: // nothing to do
                case COMPUTE_SUBMESH: break;
                ...
                case SUBMESH_COMPUTED: // nothing to do
                    break;
                case SUBMESH_RESTORED: ComputeSubMeshStateEngine(SUBMESH_RESTORED); break;
                case MESH_ENTITY_REMOVED: break;
                case SUBMESH_LOADED:
                    loadDependentMeshes();
                    ...
                    //break;
                case CHECK_COMPUTE_STATE:
                    if (IsMeshComputed()) _computeState = COMPUTE_OK;
                    break;
                default: ASSERT(0); break;
            }
            break;
                ...
                case COMPUTE_SUBMESH: {
                 ...
                 ( algo->NeedDiscreteBoundary() || algo->SupportSubmeshes() )*/)
                        ...
                        if (!ret) {
                            ...
                            if (!algo->NeedDiscreteBoundary() && !subFailed)
                                _computeError = SMESH_ComputeError::New(
                                    COMPERR_BAD_INPUT_MESH, "Unexpected computed sub-mesh", algo);
                            break; // goto exit
                        }
                    }
                    // Compute
                    ...
                    try {
                        ...
                        _computeError =
                            SMESH_ComputeError::Worst(_computeError, algo->GetComputeError());
                    }
                        ...
                        *_computeError = comperr;
                        ...
                        if (_computeError) _computeError->myName = COMPERR_EXCEPTION;
                        else
                            ret = false;
                    }
                    ...
                    if (isComputeErrorSet) ret = false;
                        ...
                        if (!algo->NeedDiscreteBoundary())
                            ...
                            for (subS.ReInit(); subS.More(); subS.Next()) {
                                ...
                                SMESH_subMeshIteratorPtr smIt =
                                    sm->getDependsOnIterator(false, false);
                                while (smIt->more()) {
                                    sm = smIt->next();
        ...
        case FAILED_TO_COMPUTE:
            switch (event) {
                case MODIF_ALGO_STATE:
                    if (!IsEmpty()) ComputeStateEngine(CLEAN);
                    algo = GetAlgo();
                    if (algo && !algo->NeedDiscreteBoundary())
                        cleanDependsOn(algo); // clean sub-meshes with event CLEAN
                    if (_algoState == HYP_OK) _computeState = READY_TO_COMPUTE;
...
bool SMESH_subMesh::Evaluate(MapShapeNbElems &aResMap)
{
    ...
    if (algo && !aResMap.count(this)) {
        ...
        if (_father->HasShapeToMesh() && algo->NeedDiscreteBoundary()) {
            ...
            if (!subMeshEvaluated) return false;
        }
        _computeError = SMESH_ComputeError::New(COMPERR_OK, "", algo);
        ...
        if (IsMeshComputed()) {
            ...
            if (SMESHDS_SubMesh *sm = GetSubMeshDS()) {
                ...
                while (elemIt->more()) nbEntities[elemIt->next()->GetEntityType()]++;
            }
        }
        else {
...
bool SMESH_subMesh::checkComputeError(SMESH_Algo *theAlgo, const bool theComputeOK,
                                      const TopoDS_Shape &theShape)
{
    ...
    if (!theShape.IsNull()) {
        ...
        if (!theAlgo->NeedDiscreteBoundary()) {
            SMESH_subMeshIteratorPtr smIt = getDependsOnIterator(false, false);
            while (smIt->more())
                if (!smIt->next()->checkComputeError(theAlgo, theComputeOK)) noErrors = false;
        }

        // Check state of neighbours
        if (!theAlgo->OnlyUnaryInput() && theShape.ShapeType() == TopAbs_COMPOUND
    ...
    }
    {
        ...
        if (_computeError && !_computeError->IsOK()) {
            if (!_computeError->myAlgo) _computeError->myAlgo = theAlgo;

            // Show error
            SMESH_Comment text;
            text << theAlgo->GetName() << " failed on sub-shape #" << _Id << " with error ";
            if (_computeError->IsCommon()) text << _computeError->CommonName();
            else
...
{
    SMESH_subMeshIteratorPtr smIt = getDependsOnIterator(false, false);
    while (smIt->more()) smIt->next()->_computeState = theState;
}
...
void SMESH_subMesh::ComputeSubMeshStateEngine(int event, const bool includeSelf)
{
    ...
    while (smIt->more()) smIt->next()->ComputeStateEngine(event);
}
...
TopoDS_Shape SMESH_subMesh::getCollection(SMESH_Gen *theGen, SMESH_Algo *theAlgo,
                                          bool &theSubComputed, bool &theSubFailed,
                                          std::vector<SMESH_subMesh *> &theSubs)
{
    ...
    if (mainShape.IsSame(_subShape)) return _subShape;
    ...
    while (smIt->more()) {
        ...
        if (S.ShapeType() != this->_subShape.ShapeType()) continue;
        theSubs.push_back(subMesh);
        if (subMesh == this) { aBuilder.Add(aCompound, S); }
                ...
                (anAlgo->GetUsedHypothesis(*_father, S, skipAuxHyps) == aUsedHyp)) // same hyps
            {
                ...
                if (!subMesh->SubMeshesComputed()) theSubComputed = false;
            }
        }
    }
                                                          ...
                                                          const int theHypType)
{
    ...
    if (theHyp) {
        ...
        if (theHyp->IsAuxiliary()) hypoKind.And(hypoKind.HasName(theHyp->GetName()));
        else
...
SMESH_Hypothesis::Hypothesis_Status SMESH_subMesh::CheckConcurentHypothesis(const int theHypType)
{
    ...
    if (getSimilarAttached(_subShape, 0, theHypType)) return SMESH_Hypothesis::HYP_OK;
...
SMESH_subMesh::OwnListenerData::OwnListenerData(SMESH_subMesh *sm, EventListener *el)
    : mySubMesh(sm), myMeshID(sm ? sm->GetFather()->GetId() : -1),
      mySubMeshID(sm ? sm->GetId() : -1), myListener(el)
{}
...
void SMESH_subMesh::setEventListener(EventListener *listener, EventListenerData *data)
{
    map<EventListener *, EventListenerData *>::iterator l_d = _eventListeners.find(listener);
    if (l_d != _eventListeners.end()) {
        EventListenerData *curData = l_d->second;
        if (curData && curData != data && curData->IsDeletable()) delete curData;
        l_d->second = data;
    }
    else {
        for (l_d = _eventListeners.begin(); l_d != _eventListeners.end(); ++l_d)
            if (listener->GetName() == l_d->first->GetName()) {
                EventListenerData *curData = l_d->second;
                if (curData && curData != data && curData->IsDeletable()) delete curData;
                if (l_d->first != listener && l_d->first->IsDeletable()) delete l_d->first;
                                                       ...
                                                       const bool myOwn) const
{
    if (myOwn) {
        ...
        if (l_d != _eventListeners.end()) return l_d->second;
    }
    return 0;
}
                                                       ...
                                                       const bool myOwn) const
{
    if (myOwn) {
        ...
        for (; l_d != _eventListeners.end(); ++l_d)
            if (listenerName == l_d->first->GetName()) return l_d->second;
    }
    return 0;
}
...
void SMESH_subMesh::notifyListenersOnEvent(const int event, const event_type eventType,
...
void SMESH_subMesh::deleteOwnListeners()
{
    ...
    for (d = _ownListeners.begin(); d != _ownListeners.end(); ++d) {
        ...
        if (!mesh || !mesh->GetSubMeshContaining(d->mySubMeshID)) continue;
...
void SMESH_subMesh::loadDependentMeshes()
{
    list<OwnListenerData>::iterator d;
    for (d = _ownListeners.begin(); d != _ownListeners.end(); ++d)
        if (_father != d->mySubMesh->_father) d->mySubMesh->_father->Load();
...
void SMESH_subMeshEventListener::ProcessEvent(const int event, const int eventType,
                                              SMESH_subMesh *subMesh, EventListenerData *data,
                                              const SMESH_Hypothesis * /*hyp*/)
{
    if (data && !data->mySubMeshes.empty() && eventType == SMESH_subMesh::COMPUTE_EVENT) {
        ...
        switch (event) {
            case SMESH_subMesh::CLEAN:
                for (; smIt != smEnd; ++smIt) (*smIt)->ComputeStateEngine(event);
                break;
...
struct _Iterator: public SMDS_Iterator<SMESH_subMesh *> {
    _Iterator(SMDS_Iterator<SMESH_subMesh *> *subIt, SMESH_subMesh *prepend, SMESH_subMesh *append)
        : myIt(subIt), myAppend(append)
    {
        myCur = prepend ? prepend : myIt->more() ? myIt->next() : append;
        if (myCur == append) append = 0;
    }
    /// Return true if and only if there are other object in this iterator
    virtual bool more() { return myCur; }
...
{
    if (_ancestors.empty() && !_subShape.IsSame(_father->GetShapeToMesh())) {
        ...
        for (TopTools_ListIteratorOfListOfShape it(ancShapes); it.More(); it.Next())
            if (SMESH_subMesh *sm = _father->GetSubMeshContaining(it.Value()))
                if (map.Add(it.Value())) me->_ancestors.push_back(sm);
    }
...
void SMESH_subMesh::ClearAncestors() { _ancestors.clear(); }
                                     ...
                                     std::set<const SMESH_subMesh *> &theSetOfCommon) const
{
    ...
    if (theOther->_mapDepend.find(this->GetId()) != otherEnd) theSetOfCommon.insert(this);
    if (_mapDepend.find(theOther->GetId()) != _mapDepend.end()) theSetOfCommon.insert(theOther);

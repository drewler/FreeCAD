...
/**
   ...
   * \internal
   */
Standard_Boolean ModifyShape(const TopoDS_Shape &theShape, TopoDS_Shape &theModifiedShape,
...
{
    treeStr.append("{");
    for (GEOMUtils::LevelsList::const_iterator j = theLevelList.begin(); j != theLevelList.end();
         ++j) {
        if (j != theLevelList.begin()) { treeStr.append(";"); }
        GEOMUtils::LevelInfo level = (*j);
        GEOMUtils::LevelInfo::iterator upIter;
        for (upIter = level.begin(); upIter != level.end(); ++upIter) {
            if (upIter != level.begin()) { treeStr.append(","); }
...
GEOMUtils::LevelsList parseWard(const std::string &theData, std::size_t &theCursor)
{
    ...
    while (std::getline(ss, substr, ';')) {
        if (!substr.empty()) levelsListStr.push_back(substr);
    }
    ...
    for (int level = 0; level < levelsListStr.size(); level++) {
        ...
        while (std::getline(ss1, substr, ',')) {
            if (!substr.empty()) namesListStr.push_back(substr);
        }
        ...
        for (int node = 0; node < namesListStr.size(); node++) {
            ...
            while (std::getline(ss2, substr, '_')) {
                if (!substr.empty()) linksListStr.push_back(substr);
            }
...
std::pair<double, double> GEOMUtils::ShapeToDouble(const TopoDS_Shape &S, bool isOldSorting)
{
    ...
    if (S.ShapeType() == TopAbs_VERTEX) {
        ...
        if (isOldSorting) { BRepGProp::LinearProperties(S, GPr); }
        else {
...
gp_Ax3 GEOMUtils::GetPosition(const TopoDS_Shape &theShape)
{
    ...
    if (theShape.IsNull()) return aResult;
    ...
    if (aShType == TopAbs_VERTEX) { aPnt = BRep_Tool::Pnt(TopoDS::Vertex(theShape)); }
    else {
        if (aShType == TopAbs_COMPOUND) { aShType = GetTypeOfSimplePart(theShape); }
...
//purpose  :
//=======================================================================
gp_Vec GEOMUtils::GetVector(const TopoDS_Shape &theShape, Standard_Boolean doConsiderOrientation)
{
    if (theShape.IsNull()) Standard_NullObject::Raise("Null shape is given for a vector");
                                          ...
                                          const TopoDS_Shape &theShape2)
{
    ...
    if (dMidXYZ >= tol) { exchange = Standard_True; }
    else if (Abs(dMidXYZ) < tol) {
        double dLength = val1.second - val2.second;
        if (dLength >= tol) { exchange = Standard_True; }
            ...
            if (!box1.IsVoid()) {
                ...
                if (dSquareExtent >= tol) { exchange = Standard_True; }
                    ...
                    if ((val1 - val2) >= tol) { exchange = Standard_True; }
                }
            }
        }
    }
...
//purpose  :
//=======================================================================
void GEOMUtils::SortShapes(TopTools_ListOfShape &SL, const Standard_Boolean isOldSorting)
{
#ifdef STD_SORT_ALGO
    ...
    for (; it.More(); it.Next()) { aShapesVec.push_back(it.Value()); }
    SL.Clear();
    ...
    for (; anIter != aShapesVec.end(); ++anIter) { SL.Append(*anIter); }
#else
    // old implementation
    ...
    for (Index = 1; Index <= MaxShapes; Index++) SL.Append(aShapes(OrderInd(Index)));
#endif
}
...
{
    if (theCompsolid.ShapeType() != TopAbs_COMPSOLID) { return theCompsolid; }
    ...
    for (; It.More(); It.Next()) {
        ...
        if (mapShape.Add(aShape_i)) { B.Add(aCompound, aShape_i); }
    }
...
bool GEOMUtils::CheckTriangulation(const TopoDS_Shape &aShape)
{
    ...
    if (exp.More()) {
        ...
        if (aTRF.IsNull()) { isTriangulation = false; }
    }
    else // no faces, try edges
    {
        TopExp_Explorer expe(aShape, TopAbs_EDGE);
        if (!expe.More()) { return false; }
        TopLoc_Location aLoc;
        Handle(Poly_Polygon3D) aPE = BRep_Tool::Polygon3D(TopoDS::Edge(expe.Current()), aLoc);
        if (aPE.IsNull()) { isTriangulation = false; }
    }
...
TopAbs_ShapeEnum GEOMUtils::GetTypeOfSimplePart(const TopoDS_Shape &theShape)
{
    ...
    else if (aType == TopAbs_EDGE || aType == TopAbs_WIRE)
        return TopAbs_EDGE;
    else if (aType == TopAbs_FACE || aType == TopAbs_SHELL)
        return TopAbs_FACE;
    else if (aType == TopAbs_SOLID || aType == TopAbs_COMPSOLID)
        return TopAbs_SOLID;
    else if (aType == TopAbs_COMPOUND) {
        // Only the iType of the first shape in the compound is taken into account
        TopoDS_Iterator It(theShape, Standard_False, Standard_False);
        if (It.More()) { return GetTypeOfSimplePart(It.Value()); }
    }
    return TopAbs_SHAPE;
}
                                         ...
                                         const TopoDS_Vertex &thePoint)
{
    ...
    for (; exp.More(); exp.Next()) {
        if (mapShape.Add(exp.Current())) { nbEdges++; }
    }

    if (nbEdges == 0) Standard_NullObject::Raise("Given shape contains no edges");
        ...
        if (mapShape.Add(exp.Current())) {
            ...
            if (!aDistTool.IsDone())
                Standard_ConstructionError::Raise(
                    "Cannot find a distance from the given point to one of edges");
    ...
    for (ind = 1; ind <= nbEdges; ind++) {
        if (Abs(aDistances(ind) - nearest) < prec) { nbFound++; }
...
//purpose  :
//=======================================================================
Standard_Boolean GEOMUtils::PreciseBoundingBox(const TopoDS_Shape &theShape, Bnd_Box &theBox)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        BRepBuilderAPI_MakeFace aMkFace(aPln, -aPlnSize[iHalf], aPlnSize[iHalf], -aPlnSize[iHalf],
                                        aPlnSize[iHalf]);
        ...
        Standard_Real aMinDist = GEOMUtils::GetMinDistance(aFace, theShape, aPMin[0], aPMin[1]);

        if (aMinDist < 0.) { return Standard_False; }
...
//purpose  :
//=======================================================================
double GEOMUtils::GetMinDistanceSingular(const TopoDS_Shape &aSh1, const TopoDS_Shape &aSh2,
                                         gp_Pnt &Ptmp1, gp_Pnt &Ptmp2)
{
    ...
    if (!IsChange1 && !IsChange2) return -2.0;
    ...
    if (dst.IsDone()) {
        ...
        if (MinDist < 1.e-7) {
            ...
            if (MinDist > (AddDist1 + AddDist2)) {
                Ptmp1 = gp_Pnt(PMin1.X() + aDir.X() * AddDist1, PMin1.Y() + aDir.Y() * AddDist1,
                               PMin1.Z() + aDir.Z() * AddDist1);
                Ptmp2 = gp_Pnt(PMin2.X() - aDir.X() * AddDist2, PMin2.Y() - aDir.Y() * AddDist2,
                                        ...
                                        gp_Pnt &thePnt2)
{
    ...
    double dist = GEOMUtils::GetMinDistanceSingular(theShape1, theShape2, thePnt1, thePnt2);
...
//=======================================================================
void GEOMUtils::ConvertTreeToString(const TreeModel &tree, std::string &dependencyStr)
{
...
//=======================================================================
void GEOMUtils::ConvertStringToTree(const std::string &dependencyStr, TreeModel &tree)
{
    std::size_t cursor = 0;
...
bool GEOMUtils::CheckShape(TopoDS_Shape &shape, bool checkGeometry)
{
    BRepCheck_Analyzer analyzer(shape, checkGeometry);
    return analyzer.IsValid();
}

bool GEOMUtils::FixShapeTolerance(TopoDS_Shape &shape, TopAbs_ShapeEnum type,
                                  Standard_Real tolerance, bool checkGeometry)
{
...
bool GEOMUtils::FixShapeTolerance(TopoDS_Shape &shape, Standard_Real tolerance, bool checkGeometry)
{
    return FixShapeTolerance(shape, TopAbs_SHAPE, tolerance, checkGeometry);
}

bool GEOMUtils::FixShapeTolerance(TopoDS_Shape &shape, bool checkGeometry)
{
...
bool GEOMUtils::FixShapeCurves(TopoDS_Shape &shape)
{
    ...
    NCollection_DataMap<TopoDS_Edge, Standard_Real, TopTools_ShapeMapHasher>::Iterator aDMETolIt(
        aDMETol);
...
TopoDS_Shape GEOMUtils::ReduceCompound(const TopoDS_Shape &shape)
{
    ...
    if (shape.ShapeType() == TopAbs_COMPOUND || shape.ShapeType() == TopAbs_COMPSOLID) {
        ...
        for (; it.More(); it.Next()) l.Append(it.Value());
        if (l.Extent() == 1 && l.First() != shape) result = ReduceCompound(l.First());
    }

    return result;
}

void GEOMUtils::MeshShape(const TopoDS_Shape shape, double deflection, bool theForced)
{
...
double GEOMUtils::DefaultDeflection() { return 0.001; }
    ...
    if (theShape.IsNull() == Standard_False) {
        if (theShape.Closed()) {
            ...
            if (aType == TopAbs_EDGE || aType == TopAbs_WIRE) {
                ...
                if (aV[0].IsNull() == Standard_False && aV[1].IsNull() == Standard_False) {
                    if (aV[0].IsSame(aV[1])) {
                        // The shape is closed
                        isOpen = false;
                    }

...
TGaussCoord ::TGaussCoord()
    : TModeSwitchInfo(eFULL_INTERLACE), myNbElem(0), myNbGauss(0), myDim(0), myGaussStep(0)
{}

void TGaussCoord ::Init(TInt theNbElem, TInt theNbGauss, TInt theDim, EModeSwitch theMode)
{
...
TInt TGaussCoord ::GetNbElem() const { return myNbElem; }

TInt TGaussCoord ::GetNbGauss() const { return myNbGauss; }

TInt TGaussCoord ::GetDim() const { return myDim; }

unsigned char *TGaussCoord ::GetValuePtr() { return (unsigned char *)&(myGaussCoord[0]); }


TCCoordSliceArr TGaussCoord ::GetCoordSliceArr(TInt theElemId) const
{
    ...
    if (GetModeSwitch() == eFULL_INTERLACE) {
        ...
        for (TInt anGaussId = 0; anGaussId < myNbGauss; anGaussId++) {
            aCoordSliceArr[anGaussId] = TCCoordSlice(myGaussCoord, std::slice(anId, myDim, 1));
            anId += myDim;
        }
    }
    else {
...
TCoordSliceArr TGaussCoord ::GetCoordSliceArr(TInt theElemId)
{
    TCoordSliceArr aCoordSliceArr(myNbGauss);
    if (GetModeSwitch() == eFULL_INTERLACE) {
        TInt anId = theElemId * myGaussStep;
        for (TInt anGaussId = 0; anGaussId < myNbGauss; anGaussId++) {
            aCoordSliceArr[anGaussId] = TCoordSlice(myGaussCoord, std::slice(anId, myDim, 1));
            anId += myDim;
        }
    }
    else {
...
//---------------------------------------------------------------
inline bool IsEqual(TFloat theLeft, TFloat theRight)
{
...
TShapeFun::TShapeFun(TInt theDim, TInt theNbRef)
    : myRefCoord(theNbRef * theDim), myDim(theDim), myNbRef(theNbRef)
{}

TCCoordSlice TShapeFun::GetCoord(TInt theRefId) const
{
    return TCCoordSlice(myRefCoord, std::slice(theRefId * myDim, myDim, 1));
}

TCoordSlice TShapeFun::GetCoord(TInt theRefId)
{
    return TCoordSlice(myRefCoord, std::slice(theRefId * myDim, myDim, 1));
}

void TShapeFun::GetFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
...
bool TShapeFun::IsSatisfy(const TCCoordSliceArr &theRefCoord) const
{
    ...
    if (anIsSatisfy) {
        for (TInt aRefId = 0; aRefId < aNbRef; aRefId++) {
            ...
            if (!anIsEqual) {
                ...
                TInt aDim = aCoord.size();
                for (TInt anId = 0; anId < aDim; anId++) ADDMSG(MYDEBUG, "\t" << aCoord[anId]);
                const TCCoordSlice &aCoord2 = theRefCoord[aRefId];
                ADDMSG(MYDEBUG, "}\t!=\taCoord2 = {");
                for (TInt anId = 0; anId < aDim; anId++) ADDMSG(MYDEBUG, "\t" << aCoord2[anId]);
                ADDMSG(MYDEBUG, "}\n");
#endif
#ifndef _DEBUG_
...
bool TShapeFun::Eval(const TCellInfo &theCellInfo, const TNodeInfo &theNodeInfo,
                     const TElemNum &theElemNum, const TCCoordSliceArr &theRef,
                     const TCCoordSliceArr &theGauss, TGaussCoord &theGaussCoord,
                     EModeSwitch theMode)
{
    ...
    if (IsSatisfy(theRef)) {
        ...
        TInt aNbElem;
        if (anIsSubMesh) aNbElem = theElemNum.size();
        else
        ...
        INITMSG(MYDEBUG,
                "aDim = " << aDim << "; aNbGauss = " << aNbGauss << "; aNbElem = " << aNbElem
                          << "; aNbNodes = " << theNodeInfo.GetNbElem() << std::endl);
...
void TSeg2a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                     TFun &theFun) const
{
...
void TSeg3a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                     TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTria3a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTria6a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTria3b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTria6b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad4a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad8a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad9a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad4b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad8b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TQuad9b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTetra4a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTetra10a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                        TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        aSlice[2] = (1.0 - aCoord[0] - aCoord[1] - aCoord[2])
            * (1.0 - 2.0 * aCoord[0] - 2.0 * aCoord[1] - 2.0 * aCoord[2]);
...
void TTetra4b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TTetra10b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                        TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void THexa8a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void THexa20a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void THexa27a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        aSlice[0] = 0.125 * aCoord[0] * (aCoord[0] - 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[1] = 0.125 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[2] = 0.125 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[3] = 0.125 * aCoord[0] * (aCoord[0] - 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[4] = 0.125 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[5] = 0.125 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[6] = 0.125 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[7] = 0.125 * aCoord[0] * (aCoord[0] - 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);

        aSlice[8] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * aCoord[1] * (aCoord[1] - 1.0) * aCoord[2]
            * (aCoord[2] - 1.0);
        aSlice[9] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * (1.0 - aCoord[1] * aCoord[1]) * aCoord[2]
            * (aCoord[2] - 1.0);
        aSlice[10] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[11] = 0.25 * aCoord[0] * (aCoord[0] - 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[12] = 0.25 * aCoord[0] * (aCoord[0] - 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[13] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] - 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[14] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[15] = 0.25 * aCoord[0] * (aCoord[0] - 1.0) * aCoord[1] * (aCoord[1] + 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[16] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * aCoord[1] * (aCoord[1] - 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[17] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[18] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * aCoord[1] * (aCoord[1] + 1.0)
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[19] = 0.25 * aCoord[0] * (aCoord[0] - 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[20] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] - 1.0);
        aSlice[21] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * aCoord[1] * (aCoord[1] - 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[22] = 0.25 * aCoord[0] * (aCoord[0] + 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[23] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * aCoord[1] * (aCoord[1] + 1.0)
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[24] = 0.25 * aCoord[0] * (aCoord[0] - 1.0) * (1.0 - aCoord[1] * aCoord[1])
            * (1.0 - aCoord[2] * aCoord[2]);
        aSlice[25] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * (1.0 - aCoord[1] * aCoord[1])
            * aCoord[2] * (aCoord[2] + 1.0);
        aSlice[26] = 0.25 * (1.0 - aCoord[0] * aCoord[0]) * (1.0 - aCoord[1] * aCoord[1])
            * (1.0 - aCoord[1] * aCoord[1]);
    }
}
...
void THexa8b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void THexa20b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TPenta6a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TPenta6b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TPenta15a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                        TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        aSlice[2] = 0.5 * (aCoord[0] - 1.0) * (1.0 - aCoord[1] - aCoord[2])
            * (aCoord[0] + 2.0 * aCoord[1] + 2.0 * aCoord[2]);

        aSlice[3] = 0.5 * aCoord[1] * (1.0 + aCoord[0]) * (2.0 * aCoord[1] - 2.0 + aCoord[0]);
        aSlice[4] = 0.5 * aCoord[2] * (1.0 + aCoord[0]) * (2.0 * aCoord[2] - 2.0 + aCoord[0]);
        aSlice[5] = 0.5 * (-aCoord[0] - 1.0) * (1.0 - aCoord[1] - aCoord[2])
            * (-aCoord[0] + 2.0 * aCoord[1] + 2.0 * aCoord[2]);
...
void TPenta15b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                        TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        aSlice[1] = 0.5 * (aCoord[0] - 1.0) * (1.0 - aCoord[1] - aCoord[2])
            * (aCoord[0] + 2.0 * aCoord[1] + 2.0 * aCoord[2]);

        aSlice[3] = 0.5 * aCoord[1] * (1.0 + aCoord[0]) * (2.0 * aCoord[1] - 2.0 + aCoord[0]);
        aSlice[5] = 0.5 * aCoord[2] * (1.0 + aCoord[0]) * (2.0 * aCoord[2] - 2.0 + aCoord[0]);
        aSlice[4] = 0.5 * (-aCoord[0] - 1.0) * (1.0 - aCoord[1] - aCoord[2])
            * (-aCoord[0] + 2.0 * aCoord[1] + 2.0 * aCoord[2]);
...
void TPyra5a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        // w4 = +Z;
        aSlice[0] = 0.25 * (-aCoord[0] + aCoord[1] - 1.0) * (-aCoord[0] - aCoord[1] - 1.0)
            * (1.0 - aCoord[2]);
        aSlice[1] = 0.25 * (-aCoord[0] - aCoord[1] - 1.0) * (+aCoord[0] - aCoord[1] - 1.0)
            * (1.0 - aCoord[2]);
        aSlice[2] = 0.25 * (+aCoord[0] + aCoord[1] - 1.0) * (+aCoord[0] - aCoord[1] - 1.0)
            * (1.0 - aCoord[2]);
        aSlice[3] = 0.25 * (+aCoord[0] + aCoord[1] - 1.0) * (-aCoord[0] + aCoord[1] - 1.0)
            * (1.0 - aCoord[2]);
        aSlice[4] = aCoord[2];
    }
}
...
void TPyra5b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                      TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
void TPyra13a::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
    ...
    for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
        ...
        aSlice[0] = 0.5 * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0) * (aCoord[0] - 0.5) / (1.0 - aCoord[2]);
        aSlice[1] = 0.5 * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (+aCoord[0] - aCoord[1] + aCoord[2] - 1.0) * (aCoord[1] - 0.5) / (1.0 - aCoord[2]);
        aSlice[2] = 0.5 * (+aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (+aCoord[0] + aCoord[1] + aCoord[2] - 1.0) * (-aCoord[0] - 0.5) / (1.0 - aCoord[2]);
        aSlice[3] = 0.5 * (+aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0) * (-aCoord[1] - 0.5) / (1.0 - aCoord[2]);

        aSlice[4] = 2.0 * aCoord[2] * (aCoord[2] - 0.5);

        aSlice[5] = 0.5 * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0) * (aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            / (1.0 - aCoord[2]);
        aSlice[6] = 0.5 * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (aCoord[0] - aCoord[1] + aCoord[2] - 1.0) * (aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            / (1.0 - aCoord[2]);
        aSlice[7] = 0.5 * (aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (aCoord[0] + aCoord[1] + aCoord[2] - 1.0) * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            / (1.0 - aCoord[2]);
        aSlice[8] = 0.5 * (aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0) / (1.0 - aCoord[2]);

        aSlice[9] = 0.5 * aCoord[2] * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0) / (1.0 - aCoord[2]);
        aSlice[10] = 0.5 * aCoord[2] * (-aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (aCoord[0] - aCoord[1] + aCoord[2] - 1.0) / (1.0 - aCoord[2]);
        aSlice[11] = 0.5 * aCoord[2] * (aCoord[0] - aCoord[1] + aCoord[2] - 1.0)
            * (aCoord[0] + aCoord[1] + aCoord[2] - 1.0) / (1.0 - aCoord[2]);
        aSlice[12] = 0.5 * aCoord[2] * (aCoord[0] + aCoord[1] + aCoord[2] - 1.0)
            * (-aCoord[0] + aCoord[1] + aCoord[2] - 1.0) / (1.0 - aCoord[2]);
    }
}
...
void TPyra13b::InitFun(const TCCoordSliceArr &theRef, const TCCoordSliceArr &theGauss,
                       TFun &theFun) const
{
    GetFun(theRef, theGauss, theFun);
...
//---------------------------------------------------------------
bool GetGaussCoord3D(const TGaussInfo &theGaussInfo, const TCellInfo &theCellInfo,
                     const TNodeInfo &theNodeInfo, TGaussCoord &theGaussCoord,
                     const TElemNum &theElemNum, EModeSwitch theMode)
{
    ...
    if (theGaussInfo.myGeom == theCellInfo.myGeom) {
        ...
        switch (aGeom) {
            case eSEG2: {
                ...
                if (TSeg2a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                  theGaussCoord, theMode))
                    return true;
            ...
            case eSEG3: {
                ...
                if (TSeg3a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                  theGaussCoord, theMode))
                    return true;
            ...
            case eTRIA3: {
                ...
                if (TTria3a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TTria3b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eTRIA6: {
                ...
                if (TTria6a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TTria6b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eQUAD4: {
                ...
                if (TQuad4a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TQuad4b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eQUAD8: {
                ...
                if (TQuad8a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TQuad8b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eQUAD9: {
                ...
                if (TQuad9a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TQuad9b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eTETRA4: {
                ...
                if (TTetra4a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;

                if (TTetra4b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;
            ...
            case ePYRA5: {
                ...
                if (TPyra5a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (TPyra5b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case ePENTA6: {
                ...
                if (TPenta6a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;

                if (TPenta6b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;
            ...
            case eHEXA8: {
                ...
                if (THexa8a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;

                if (THexa8b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                   theGaussCoord, theMode))
                    return true;
            ...
            case eTETRA10: {
                ...
                if (TTetra10a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                     theGaussCoord, theMode))
                    return true;

                if (TTetra10b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                     theGaussCoord, theMode))
                    return true;
            ...
            case ePYRA13: {
                ...
                if (TPyra13a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;

                if (TPyra13b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;
            ...
            case ePENTA15: {
                ...
                if (TPenta15a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                     theGaussCoord, theMode))
                    return true;

                if (TPenta15b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                     theGaussCoord, theMode))
                    return true;
            ...
            case eHEXA20: {
                ...
                if (THexa20a().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;

                if (THexa20b().Eval(theCellInfo, theNodeInfo, theElemNum, aRefSlice, aGaussSlice,
                                    theGaussCoord, theMode))
                    return true;

                break;
            }
            default: INITMSG(MYDEBUG, "eNONE" << std::endl); return false;
        }
    }
...
//---------------------------------------------------------------
bool GetBaryCenter(const TCellInfo &theCellInfo, const TNodeInfo &theNodeInfo,
                   TGaussCoord &theGaussCoord, const TElemNum &theElemNum, EModeSwitch theMode)
{
    ...
    for (TInt anElemId = 0; anElemId < aNbElem; anElemId++) {
        ...
        for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
            ...
            for (TInt aDimId = 0; aDimId < aDim; aDimId++) { aGaussCoordSlice[aDimId] /= aConnDim; }
        }
    }
...
//---------------------------------------------------------------
bool GetBaryCenter(const TPolygoneInfo &thePolygoneInfo, const TNodeInfo &theNodeInfo,
                   TGaussCoord &theGaussCoord, const TElemNum &theElemNum, EModeSwitch theMode)
{
    INITMSG(MYDEBUG, "GetBaryCenter\n");
    ...
    for (TInt anElemId = 0; anElemId < aNbElem; anElemId++) {
        ...
        for (TInt aGaussId = 0; aGaussId < aNbGauss; aGaussId++) {
            ...
            for (TInt aDimId = 0; aDimId < aDim; aDimId++) { aGaussCoordSlice[aDimId] /= aNbNodes; }
        }
    }
...
//---------------------------------------------------------------
bool GetBaryCenter(const TPolyedreInfo &thePolyedreInfo, const TNodeInfo &theNodeInfo,
                   TGaussCoord &theGaussCoord, const TElemNum &theElemNum, EModeSwitch theMode)
{
    INITMSG(MYDEBUG, "GetBaryCenter\n");

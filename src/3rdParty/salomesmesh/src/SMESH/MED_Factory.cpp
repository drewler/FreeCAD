...
EVersion GetVersionId(const std::string &theFileName, bool theDoPreCheckInSeparateProcess)
{
...
#ifndef WIN32
    if (access(theFileName.c_str(), F_OK)) return aVersion;
    ...
    MSG(MYDEBUG,
        "GetVersionId - theFileName = '" << theFileName << "'; aFid = " << aFid << std::endl);
    if (aFid >= 0) {
        med_int aMajor, aMinor, aRelease;
        med_err aRet = MEDfileNumVersionRd(aFid, &aMajor, &aMinor, &aRelease);
        INITMSG(MYDEBUG,
                "GetVersionId - theFileName = '" << theFileName << "'; aRet = " << aRet
                                                 << std::endl);
        if (aRet >= 0) {
            if (aMajor == 2 && aMinor == 1) aVersion = eV2_1;
            else
                aVersion = eV2_2;
        }
        else {
    ...
    BEGMSG(MYDEBUG,
           "GetVersionId - theFileName = '" << theFileName << "'; aVersion = " << aVersion
                                            << std::endl);
    return aVersion;
}
...
PWrapper CrWrapper(const std::string &theFileName, bool theDoPreCheckInSeparateProcess)
{
    ...
    switch (aVersion) {
        case eV2_2: aWrapper.reset(new MED::V2_2::TVWrapper(theFileName)); break;
        case eV2_1:
            EXCEPTION(std::runtime_error,
                      "Cannot open file '" << theFileName
                                           << "'. Med version 2.1 is not supported any more.");
...
PWrapper CrWrapper(const std::string &theFileName, EVersion theId)
{
    ...
    if (aVersion != theId) remove(theFileName.c_str());
    ...
    switch (theId) {
        ...
        case eV2_1:
            ...
            //aWrapper.reset(new MED::V2_1::TVWrapper(theFileName));
            break;
        default: aWrapper.reset(new MED::V2_2::TVWrapper(theFileName));
    }
    return aWrapper;
}

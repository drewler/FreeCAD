...
{
    if (*theSring < '0' || *theSring > '9') return -1;
...
int readLine(list<const char *> &theFields, const char *&theLineBeg, const bool theClearFields)
{
    if (theClearFields) theFields.clear();
...
int loadVE(const list<TopoDS_Edge> &eList, TopTools_IndexedMapOfOrientedShape &map)
{
    ...
    // edges
    for (eIt = eList.begin(); eIt != eList.end(); eIt++) map.Add(*eIt);
...
bool SMESH_Pattern::Load(const char *theFileContents)
{
    ...
    for (iPoint = 0; iPoint < nbPoints; iPoint++) {
        ...
        for (int iCoord = 1; iCoord <= dim; iCoord++, fIt++) {
            ...
            if (myIs2D) p.myInitUV.SetCoord(iCoord, coord);
        }
    }
...
bool SMESH_Pattern::Save(ostream &theFile)
{
    ...
    if (myIs2D) {
        ...
        for (; kpIt != myKeyPointIDs.end(); kpIt++) theFile << " " << *kpIt;
        if (!myKeyPointIDs.empty()) theFile << endl;
    }
    // elements
    ...
    for (; epIt != myElemPointIDs.end(); epIt++) {
        ...
        for (; iIt != elemPoints.end(); iIt++) theFile << " " << *iIt;
        theFile << endl;
    }
...
template<typename T> struct TSizeCmp {
    bool operator()(const list<T> &l1, const list<T> &l2) const { return l1.size() < l2.size(); }
};
...
template<class TFaceIterator> bool areNodesBound(TFaceIterator &faceItr)
{
    while (faceItr->more()) {
        ...
        while (nIt->more()) {
            ...
            if (node->getshapeId() < 1) { return false; }
        }
    }
    return true;
}
...
static bool isMeshBoundToShape(SMESHDS_Mesh *aMeshDS, SMESHDS_SubMesh *aFaceSubmesh,
                               const bool isMainShape)
{
    if (isMainShape && aFaceSubmesh) {
        // check that all faces are bound to aFaceSubmesh
        if (aMeshDS->NbFaces() != aFaceSubmesh->NbElements()) return false;
    }
...
bool SMESH_Pattern::Load(SMESH_Mesh *theMesh, const TopoDS_Face &theFace, bool theProject,
                         TopoDS_Vertex the1stVertex)
{
    ...
    if (isClosed) canProject = false; // so far

    if ((theProject || needProject) && !canProject) return setErrorCode(ERR_LOADF_CANT_PROJECT);

    Extrema_GenExtPS projector;
    GeomAdaptor_Surface aSurface(BRep_Tool::Surface(face));
    if (theProject || needProject) projector.Initialize(aSurface, 20, 20, 1e-5, 1e-5);
    ...
    if (needProject) {
        ...
        if (nbElems > 0) fIt = fSubMesh->GetElements();
        else
        ...
        while (fIt->more()) {
            ...
            for (int i = 0; i < nbNodes; ++i) {
                ...
                if (nIdIt->second == -1) nIdIt->second = iPoint++;
                ...
                if (helper.IsSeamShape(edge)) {
                    ...
                    for (int is2 = 0; is2 < 2; ++is2) {
                        ...
                        if (helper.IsRealSeam(v)) {
                            ...
                            if (eVec[iSeam].Orientation() == TopAbs_REVERSED) v.Reverse();
                        }
                    }
                }
            }
            ...
            if (eSubMesh && eSubMesh->NbNodes()) {
                ...
                if (paramNodeMap.size() != eSubMesh->NbNodes()) {
                    ...
                    for (int iNode = 0; nIt->more(); ++iNode) {
                        const SMDS_MeshNode *node = nIt->next();
                        if (isQuadMesh && helper.IsMedium(node, SMDSAbs_Face)) continue;
                ...
                while (unIt != paramNodeMap.end()) {
                    ...
                    if (isSeam && !isForward) closeNodePointIDMap.insert(make_pair(node, iPoint));
                    else
                        nodePointIDMap.insert(make_pair(node, iPoint));

                    if (theProject) p->myInitUV = project(node, projector);
                    else {
        ...
        if (fSubMesh && fSubMesh->NbElements()) {
            ...
            while (elemIt->more()) {
                ...
                while (nIt->more()) {
                    ...
                    if (n_id == nodePointIDMap.end()) continue; // medium node
                    ...
                    if (helper.IsRealSeam(node->getshapeId())
                        ...
                        while (nIt2->more() && !notSeamNode) {
                            ...
                            if (!helper.IsSeamShape(n->getshapeId())) notSeamNode = n;
                        }
                        ...
                        if (dist2 < dist1) iPoint = n_id->second;
                    }
    ...
    for (; pVecIt != myPoints.end(); pVecIt++) bndBox.Add(gp_Pnt2d((*pVecIt).myInitUV));
...
void SMESH_Pattern::computeUVOnEdge(const TopoDS_Edge &theEdge, const list<TPoint *> &ePoints)
{
    bool isForward = (theEdge.Orientation() == TopAbs_FORWARD);
    double f, l;
    Handle(Geom2d_Curve) C2d = BRep_Tool::CurveOnSurface(theEdge, TopoDS::Face(myShape), f, l);
...
static bool intersectIsolines(const gp_XY &uv11, const gp_XY &uv12, const double r1,
                              const gp_XY &uv21, const gp_XY &uv22, const double r2, gp_XY &resUV,
                              bool &isDeformed)
{
...
bool SMESH_Pattern::compUVByIsoIntersection(const list<list<TPoint *>> &theBndPoints,
                                            const gp_XY &theInitUV, gp_XY &theUV,
                                            bool &theIsDeformed)
{
    ...
    for (int iIso = 0; iIso < 2; iIso++) {
            ...
            for (; pIt != bndPoints.end(); pIt++) {
                ...
                if (!coincPrev &&       // ignore if initParam coincides with prev point param
                    ...
                    if (i >= 2) {
                        ...
                        bool isBetween =
                            (vec0 * vec1 < 0); // is theInitUV between initUV[0] and initUV[1]
        ...
        if (nbUV < 2 || (UV[0] - UV[1]).SquareModulus() <= DBL_MIN * DBL_MIN) {
            MESSAGE(" consequent edge-points not found, nb UV found: "
                    << nbUV << ", for point: " << theInitUV.X() << " " << theInitUV.Y());
        ...
        double r = (initUV[0] - theInitUV).Modulus() / (initUV[0] - initUV[1]).Modulus();
    ...
    if (!intersectIsolines(uv1[0], uv2[0], ratio[0], uv1[1], uv2[1], ratio[1], theUV,
                           theIsDeformed)) {
...
struct TIsoNode {
    ...
    bool IsUVComputed() const { return myUV.X() != 1e100; }
    bool IsMovable() const
    {
        return myIsMovable && myNext[0] && myNext[1] && myNext[2] && myNext[3];
    }
    void SetNotMovable() { myIsMovable = false; }
    void SetBoundaryNode(TIsoNode *node, int iDir, int i) { myBndNodes[iDir + i * 2] = node; }
    TIsoNode *GetBoundaryNode(int iDir, int i) { return myBndNodes[iDir + i * 2]; }
    void SetNext(TIsoNode *node, int iDir, int isForward) { myNext[iDir + isForward * 2] = node; }
    TIsoNode *GetNext(int iDir, int isForward) { return myNext[iDir + isForward * 2]; }
};
...
static bool checkQuads(const TIsoNode *node, gp_XY &newUV, const bool reversed,
                       const int crit = FIX_OLD, double fixSize = 0.)
{
    ...
    for (; dir1 < 4; dir1++, dir2++) // loop on 4 quadrangles around <node>
    {
        ...
        if (!isTriangle
            ...
            isTriangle = true;
        if (isTriangle && (n[0]->myUV - n[2]->myUV).SquareModulus() <= DBL_MIN) continue;
        ...
        for (i = isTriangle ? 2 : 0; i < 3; i++) {
            bool isDiag = (i == 2);
            if (isDiag && newOk[0] && newOk[1] && !isTriangle) break;
            gp_Vec2d sideDir;
            if (isDiag) sideDir = gp_Vec2d(n[0]->myUV, n[2]->myUV);
            else
            ...
            if (crit == FIX_OLD) {
                ...
                if (!newOk[i]) newBadRate += outDir * newDir;
                if (!wasOk[i]) oldBadRate += outDir * oldDir;
                // push node inside
                if (!wasOk[i]) {
        ...
        bool convex = true;
        if (!isTriangle) convex = (outVec[0] * gp_Vec2d(n[1]->myUV, n[2]->myUV) < 0);
        ...
        if (!isOldIn) { // node is outside a quadrangle
            ...
            if (convex || isTriangle) {
                ...
                if (outSize > DBL_MIN) out /= outSize;
                else
            ...
            for (i = isTriangle ? 2 : 0; i < 3; i++)         // mark not computed vectors
                if (wasOk[i]) moveVec[i].SetCoord(1, 2e100); // not use this vector
            while (!isOldOk) {
                ...
                if (isOldOk) oldUVImpr[nbOldImpr++] = uv1;
                else {
                    // check if uv2 is ok
                    for (i = isTriangle ? 2 : 0; i < 3; i++)
                        wasOk[i] = (outVec[i] * gp_Vec2d(n[i]->myUV, uv2) < -minDiag);
                    isOldOk = (wasOk[0] && wasOk[1]) || (wasOk[2] && convex);
                    if (isOldOk) oldUVImpr[nbOldImpr++] = uv2;
                }
            }
        }
    ...
    if (crit == CHECK_NEW_OK) return newIsOk;
    if (crit == CHECK_NEW_IN) return newIsIn;

    if (newIsOk) return true;

    if (oldIsOk) newUV = oldUV;
    else {
        if (oldIsIn && nbOldImpr) {
            //       MESSAGE(" Try to improve UV, init: "<<node->myInitUV.X()<<" "<<node->myInitUV.Y()<<
            //               " uv: "<<oldUV.X()<<" "<<oldUV.Y() );
            gp_XY uv = oldUVImpr[0];
            for (int i = 1; i < nbOldImpr; i++) uv += oldUVImpr[i];
            uv /= nbOldImpr;
        ...
        if (!oldIsIn && nbOldFix) {
            ...
            for (int i = 1; i < nbOldFix; i++) uv += oldUVFixed[i];
            uv /= nbOldFix;
        ...
        if (newIsIn && oldIsIn) newUV = (newBadRate < oldBadRate) ? newUV : oldUV;
        else if (!newIsIn)
            newUV = oldUV;
    }
...
bool SMESH_Pattern::compUVByElasticIsolines(const list<list<TPoint *>> &theBndPoints,
    ...
    for (; bndIt != theBndPoints.end(); bndIt++) {
        ...
        for (; pIt != bndPoints.end(); pIt++) {
            ...
            for (iDir = 0; iDir < 2; iDir++) {
                ...
                if (Abs(parDif) <= DBL_MIN) continue;
                ...
                for (; isoIt != isos.end(); isoIt++) {
                    ...
                    if (isoParam < minPar || isoParam > maxPar) continue;
                    ...
                    if (::IsEqual(r, 1.)) node->myDir[0] = tgt;
                    else if (::IsEqual(r, 0.))
                        node->myDir[1] = tgt;
                    else
                    ...
                    if (bndIt == theBndPoints.begin() && ::IsEqual(r, 1.))
                        if (bndNodes.empty() || bndNodes.back() != node) bndNodes.push_back(node);
                } // loop on isolines
    ...
    for (iDir = 0; iDir < 2; iDir++) {
        ...
        for (; isoIt != isos.end(); isoIt++) {
            ...
            while (nIt != isoLine.end()) {
                ...
                if (node->IsUVComputed()) lastCompNodePos = nIt;
                ...
                // 4. remove nodes out of the boundary
                if (!firstCompNodeFound) isoLine.pop_front();
...
*/
            ...
            for (nIt = bndNodes.begin(); nIt != bndNodes.end(); nIt++) {
                ...
                for (; !nClose && it != testNodes.end(); it++) {
                    for (int i = 0; i < 4; i++) {
                        nClose = (*it)->myNext[i];
                        if (nClose) {
                            if (!nClose->IsUVComputed() && nClose->IsMovable()) break;
                            else {
                                testNodes.push_back(nClose);
                                nClose = 0;
                            }
    ...
    maxNbNodeMove++;
    int nbNodeMove = 0;
    if (!useNbMoveNode) maxNbIter = (maxNbIter < 0) ? 100 : -1;
#endif
    double maxMove;
    int nbIter = 0;
    do {
...
double SMESH_Pattern::setFirstEdge(list<TopoDS_Edge> &theWire, int theFirstEdgeID)
{
    ...
    for (iE = 0; iE < nbEdges; iE++) maxNbPnt = Max(maxNbPnt, getShapePoints(eID++).size());
    ...
    for (eIt = theWire.begin(); eIt != theWire.end(); eIt++) {
        ...
        for (int i = 0; i < maxNbPnt; i++) eBndBox.Add(C2d->Value(f + i * dU));
    }
    ...
    MESSAGE("EDGES: X: " << eMinPar[0] << " - " << eMaxPar[0] << " Y: " << eMinPar[1] << " - "
                         << eMaxPar[1]);
#endif
...
bool SMESH_Pattern::Apply(const TopoDS_Face &theFace, const TopoDS_Vertex &theVertexOnKeyPoint1,
                          const bool theReverse)
{
    MESSAGE(" ::Apply(face) ");
    TopoDS_Face face = theReverse ? TopoDS::Face(theFace.Reversed()) : theFace;
    if (!setShapeToMesh(face)) return false;

    // find points on edges, it fills myNbKeyPntInBoundary
    if (!findBoundaryPoints()) return false;
    ...
    list<TopoDS_Edge>::iterator elIt;
    for (iE = 0, elIt = eList.begin(); iE < nbEdgesInOuterWire && elIt != eList.end();
         iE++, elIt++) {
    ...
    if (nbWires > 1) {
        ...
        while (wlIt != wireList.end()) {
            ...
            if (nbSameSize > 0)
                if (!sortSameSizeWires(wireList, wlIt, wlIt2, id1, edgesPointsList)) return false;
            wlIt = wlIt2;
    ...
    for (pIt = fPoints.begin(); !isDeformed && pIt != fPoints.end(); pIt++)
        if (!compUVByIsoIntersection(edgesPointsList, (*pIt)->myInitUV, (*pIt)->myUV, isDeformed)) {
            MESSAGE("can't Apply(face)");
            return false;
        }
    ...
    if (isDeformed && !compUVByElasticIsolines(edgesPointsList, fPoints)) {
        for (; pIt != fPoints.end(); pIt++) // continue with the simple algo
            if (!compUVByIsoIntersection(edgesPointsList, (*pIt)->myInitUV, (*pIt)->myUV,
                                         isDeformed)) {
                MESSAGE("can't Apply(face)");
                return false;
            }
    ...
    for (pIt = fPoints.begin(); pIt != fPoints.end(); pIt++) {
        ...
        if (!loc.IsIdentity()) aTrsf.Transforms(point->myXYZ.ChangeCoord());
    }
...
bool SMESH_Pattern::Apply(const SMDS_MeshFace *theFace, const int theNodeIndexOnKeyPoint1,
                          const bool theReverse)
{
    ...
    if (myNbKeyPntInBoundary.size() > 1) { return setErrorCode(ERR_APPL_BAD_NB_VERTICES); }
    ...
    while (noIt->more() && iSub < nbFaceNodes) {
        ...
        if (iSub++ == theNodeIndexOnKeyPoint1) n = --nodes.end();
    }
    if (n != nodes.end()) {
        if (theReverse) {
            if (n != --nodes.end()) nodes.splice(nodes.begin(), nodes, ++n, nodes.end());
            nodes.reverse();
        }
    ...
    if (N.SquareMagnitude() <= DBL_MIN) return setErrorCode(ERR_APPLF_BAD_FACE_GEOM);
    gp_Ax2 pos(P, N, Vx);
...
bool SMESH_Pattern::Apply(SMESH_Mesh *theMesh, const SMDS_MeshFace *theFace,
                          const TopoDS_Shape &theSurface, const int theNodeIndexOnKeyPoint1,
                          const bool theReverse)
{
    //  MESSAGE(" ::Apply(MeshFace) " );
    ...
    if (helper.GetNodeUVneedInFaceNode()) {
        ...
        for (n = nodes.begin(); (!inFaceNode && n != nodes.end()); ++n) {
            ...
            if (!shapeID) return Apply(theFace, theNodeIndexOnKeyPoint1, theReverse);
            if (!helper.IsSeamShape(shapeID)) inFaceNode = *n;
        }
    }
    ...
    for (int i = 0; i < myOrderedNodes.size(); ++i, ++iSub) {
        ...
        while (*pIt != ePoints.back()) {
            ...
            if (!loc.IsIdentity()) aTrsf.Transforms(p->myXYZ.ChangeCoord());
        }
        // collect on-edge points (excluding the last one)
        edgesPoints->insert(edgesPoints->end(), ePoints.begin(), --ePoints.end());
    }
...
inline static bool isDefined(const gp_XYZ &theXYZ) { return theXYZ.X() < 1.e100; }
...
bool SMESH_Pattern::Apply(SMESH_Mesh *theMesh, std::set<const SMDS_MeshFace *> &theFaces,
                          const int theNodeIndexOnKeyPoint1, const bool theReverse)
{
    ...
    for (; face != theFaces.end(); ++face) {
        ...
        //     }
        bool ok;
        if (shape.IsNull()) ok = Apply(*face, theNodeIndexOnKeyPoint1, theReverse);
        else
        ...
        for (int i = 0; i < nbNodes; i++) {
            ...
            for (p++; *p != linkPoints.back(); p++) indList.push_back((*p - &myPoints[0]) + ind1);
        }
        ind1 += myPoints.size();
    }
...
bool SMESH_Pattern::Apply(std::set<const SMDS_MeshVolume *> &theVolumes, const int theNode000Index,
                          const int theNode001Index)
{
    ...
    for (int i = 0; i < myPoints.size(); i++) pointIndex.insert(make_pair(&myPoints[i], i));
    ...
    for (; vol != theVolumes.end(); ++vol) {
        ...
        for (int Id = SMESH_Block::ID_V000; Id <= SMESH_Block::ID_F1yz; Id++) {
            ...
            TNodeSet subNodes;
            vector<int> subIDs;
            if (SMESH_Block::IsVertexID(Id)) { subNodes.insert(myOrderedNodes[Id - 1]); }
            ...
            groups.push_back(list<int>());
            list<int> &indList = groups.back();
            for (; p != points.end(); p++) indList.push_back(pointIndex[*p] + ind1);
...
bool SMESH_Pattern::Load(SMESH_Mesh *theMesh, const TopoDS_Shell &theBlock)
{
    MESSAGE(" ::Load(volume) ");
    Clear();
    myIs2D = false;
    ...
    for (shapeID = 1; shapeID <= myShapeIDMap.Extent(); shapeID++) {
        ...
        if (aSubMesh) nbNodes += aSubMesh->NbNodes();
    }
    myPoints.resize(nbNodes);
    ...
    for (shapeID = 1; shapeID <= myShapeIDMap.Extent(); shapeID++) {
        ...
        while (nIt->more()) {
            const SMDS_MeshNode *node = smdsNode(nIt->next());
            if (isQuadMesh && SMESH_MeshEditor::IsMedium(node, SMDSAbs_Volume)) continue;
            nodePointIDMap.insert(make_pair(node, iPoint));
            if (block.IsVertexID(shapeID)) myKeyPointIDs.push_back(iPoint);
            TPoint *p = &myPoints[iPoint++];
            ...
            case TopAbs_EDGE: {
                ...
                for (; pIt != shapePoints.end(); pIt++) {
                    ...
                    for (int iCoord = 1; iCoord <= 3; iCoord++)
                        if (coef[iCoord - 1] > 0) (*pIt)->myInitXYZ.SetCoord(iCoord, 1.);
                }
                if (S.ShapeType() == TopAbs_VERTEX) break;
                ...
                for (; nIt->more(); pIt++) {
                    const SMDS_MeshNode *node = nIt->next();
                    if (isQuadMesh && SMESH_MeshEditor::IsMedium(node, SMDSAbs_Edge)) continue;
                    const SMDS_EdgePosition *epos =
                        static_cast<const SMDS_EdgePosition *>(node->GetPosition());
...
bool SMESH_Pattern::Apply(const TopoDS_Shell &theBlock, const TopoDS_Vertex &theVertex000,
...
bool SMESH_Pattern::Apply(const SMDS_MeshVolume *theVolume, const int theNode000Index,
                          const int theNode001Index)
{
...
void SMESH_Pattern::mergePoints(const bool uniteGroups)
{
    ...
    for (; idListIt != myIdsOnBoundary.end(); idListIt++) {
        ...
        if (nodes.size() > 1) {
            ...
            for (; n != nodes.end(); ++n) box.Add(gp_Pnt(SMESH_TNodeXYZ(*n)));
            double x, y, z, X, Y, Z;
            box.Get(x, y, z, X, Y, Z);
                ...
                for (ind1 = indices1.begin(); ind1 != indices1.end(); ind1++) {
                    ...
                    while (ind2 != indices2.end()) {
                        ...
                        if ((p1 - p2).SquareModulus() <= tol2) {
                            ASSERT(myReverseConnectivity.find(*ind2)
                                   != myReverseConnectivity.end());
        ...
        if (unite) { // put all sorted indices into the first group
            ...
            for (; dist_ind != distIndMap.end(); dist_ind++) g.push_back(dist_ind->second);
        }
...
void SMESH_Pattern::makePolyElements(const vector<const SMDS_MeshNode *> &theNodes,
                                     const bool toCreatePolygons, const bool toCreatePolyedrs)
{
    ...
    if (toCreatePolygons) {
        ...
        for (; indListIt != myIdsOnBoundary.end(); indListIt++) {
            ...
            if (linkNodes.size() != 2) continue; // skip face
            ...
            if (idGroups.empty() || idGroups.front().empty()) continue;
            ...
            while (true) {
                ...
                if (face) {
                    ...
                    while (nIt->more()) nodes[i++] = smdsNode(nIt->next());
                    nodes[i] = nodes[0];
                    for (i = 0; i < nbNodes; ++i) {
                        ...
                        if (faceLinkNodes == linkNodes) nn_IdList = indListIt;
                        else
                        ...
                        if (nn_IdList != myIdsOnBoundary.end()) {
                            ...
                            if (isReversed(nodes[i], mappedIds))
                                faceNodeIds.insert(faceNodeIds.end(), mappedIds.rbegin(),
                                                   mappedIds.rend());
                            else
                                faceNodeIds.insert(faceNodeIds.end(), mappedIds.begin(),
                                                   mappedIds.end());
                        }
                    } // loop on links of a <face>
                }     // if ( face )
            ...
            if (myIs2D && idGroups.size() > 1) {
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    if (append) idsOnLink.push_back(nodeId);
                    else
                    ...
                    for (; pElemDef != elemDefs.end(); pElemDef++) // loop on elements sharing id
                    {
                        ...
                        for (int prev = 0; prev < 2; ++prev) {
                            ...
                            if (id2 != idsOnLink.end() && id != --id2) { // found not next to id
                                ...
                                if (prev)
                                    for (; id2 != id; --id2) pIdList->insert(idDef, *id2);
                                else {
            ...
            while (nIt->more()) {
                ...
                while (eIt->more()) {
                    ...
                    for (int iF = 0; iF < volTool.NbFaces(); ++iF) {
                        if (getFacesDefinition(volTool.GetFaceNodes(iF), volTool.NbFaceNodes(iF),
                                               theNodes, elemDef, quantity))
                            makePoly = true;
                    }
                    if (makePoly) myPolyElems.push_back(elem);
                    else {
...
bool SMESH_Pattern::getFacesDefinition(const SMDS_MeshNode **theBndNodes, const int theNbBndNodes,
                                       const vector<const SMDS_MeshNode *> &theNodes,
                                       list<int> &theFaceDefs, vector<int> &theQuantity)
{
    bool makePoly = false;
    ...
    if (!myIs2D) { // for 2D, merge only edges
        ...
        for (; id != ids.end(); ++id) {
            ...
            for (; pIdList != defList.end(); ++pIdList) {
                ...
                if (volType == SMDS_VolumeTool::UNKNOWN) continue;
                ...
                for (int iF = 0; iF < nbFaces; ++iF) {
                    ...
                    if (all) {
                        ...
                        for (iN = 0; iN < nbN; ++iN) { theFaceDefs.push_back(idVec[nodeInds[iN]]); }
...
static bool clearSubMesh(SMESH_Mesh *theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (SMESH_subMesh *aSubMesh = theMesh->GetSubMeshContaining(theShape)) {
        ...
        if (removed) aSubMesh->ComputeStateEngine(SMESH_subMesh::CLEAN);
    }
    else {
        SMESHDS_Mesh *aMeshDS = theMesh->GetMeshDS();
        if (SMESHDS_SubMesh *aSubMeshDS = aMeshDS->MeshElements(theShape)) {
            ...
            removed = eIt->more();
            while (eIt->more()) aMeshDS->RemoveElement(eIt->next());
            SMDS_NodeIteratorPtr nIt = aSubMeshDS->GetNodes();
            removed = removed || nIt->more();
            while (nIt->more()) aMeshDS->RemoveNode(smdsNode(nIt->next()));
        }
    }
    return removed;
}
...
void SMESH_Pattern::clearMesh(SMESH_Mesh *theMesh) const
{
    ...
    if (!myShape.IsNull()) {
        if (!clearSubMesh(theMesh, myShape)
            && !myIs2D) { // myShape is SHELL but volumes may be bound to SOLID
...
bool SMESH_Pattern::findExistingNodes(SMESH_Mesh *mesh, const TopoDS_Shape &S,
                                      const std::list<TPoint *> &points,
                                      vector<const SMDS_MeshNode *> &nodesVector)
{
    if (S.IsNull() || points.empty()) return false;
    ...
    switch (S.ShapeType()) {
        ...
        case TopAbs_EDGE: {
            ...
            if (!SMESH_Algo::GetSortedNodesOnEdge(aMeshDS, edge,
                                                  /*ignoreMediumNodes=*/false, paramsOfNodes)
            ...
            if (paramsOfNodes.size() == points.size()) {
                for (; u2n != u2nEnd; ++u2n) {
                    ...
                    if (!nodesVector[pIndex]) nodesVector[pIndex] = u2n->second;
                    ++pItF;
                    ++pItR;
                }
                return true;
                ...
                while (u2n != u2nEnd && pItF != points.end()) {
                    ...
                    do {
                        ...
                        if (Abs(u - p->myU) < tol) {
                            int pIndex = p - &myPoints[0];
                            if (!nodesVector[pIndex]) nodesVector[pIndex] = n;
                            ++pItF;
                            ++pItR;
                            break;
                        }
...
bool SMESH_Pattern::MakeMesh(SMESH_Mesh *theMesh, const bool toCreatePolygons,
                             const bool toCreatePolyedrs)
{
    MESSAGE(" ::MakeMesh() ");
    if (!myIsComputed) return setErrorCode(ERR_MAKEM_NOT_COMPUTED);
    ...
    if (onMeshElements) {
        ...
        for (int i = 0; i < myXYZ.size(); ++i) {
            if (!nodesVector[i] && isDefined(myXYZ[i]))
                nodesVector[i] = aMeshDS->AddNode(myXYZ[i].X(), myXYZ[i].Y(), myXYZ[i].Z());
        }
        if (theMesh->HasShapeToMesh()) {
            ...
            for (; idListIt != myIdsOnBoundary.end(); idListIt++) {
                list<list<int>> &groups = idListIt->second;
                const TNodeSet &nodes = idListIt->first;
                if (nodes.size() != 2) continue; // not a link
                ...
                if (S1.IsNull() || S1.ShapeType() < TopAbs_EDGE || S2.IsNull()
                    || S2.ShapeType() < TopAbs_EDGE)
                    continue;
                TopoDS_Shape S;
                if (S1.ShapeType() == TopAbs_EDGE) {
                    if (S1 == S2 || helper.IsSubShape(S2, S1)) S = S1;
                }
                else if (S2.ShapeType() == TopAbs_EDGE) {
                    if (helper.IsSubShape(S1, S2)) S = S2;
                }
                else {
        ...
        for (; idPointIt != myShapeIDToPointsMap.end(); idPointIt++) {
            ...
            TopoDS_Shape S;
            if (!myShapeIDMap.IsEmpty()) S = myShapeIDMap(idPointIt->first);

            // find existing nodes on EDGEs and VERTEXes
            if (findExistingNodes(theMesh, S, points, nodesVector)) continue;

            list<TPoint *>::iterator pIt = points.begin();
            for (; pIt != points.end(); pIt++) {
                TPoint *point = *pIt;
                int pIndex = point - &myPoints[0];
                if (nodesVector[pIndex]) continue;
                SMDS_MeshNode *node =
                    aMeshDS->AddNode(point->myXYZ.X(), point->myXYZ.Y(), point->myXYZ.Z());
                ...
                if (!S.IsNull()) {
                    ...
                    switch (S.ShapeType()) {
                        ...
                        case TopAbs_EDGE: {
                            aMeshDS->SetNodeOnEdge(node, TopoDS::Edge(S), point->myU);
                            break;
                        }
                        case TopAbs_FACE: {
                            aMeshDS->SetNodeOnFace(node, TopoDS::Face(S), point->myUV.X(),
                                                   point->myUV.Y());
                            break;
                        }
                        default: aMeshDS->SetNodeInVolume(node, TopoDS::Shell(S));
                    }
                }
            }
        }
                                   ...
                                   const vector<const SMDS_MeshElement *> &theElements)
{
        ...
        if (!aMainShape.IsNull()) {
            ...
            for (; shellExp.More(); shellExp.Next()) {
                ...
                if (sm) {
                    ...
                    while (nIt->more()) shellNodes.insert(nIt->next());
                }
            }
        }
    }
    ...
    if (onMeshElements) nbNewElemsPerOld = theElemNodeIDs.size() / theElements.size();
    ...
    for (int iElem = 0; enIt != theElemNodeIDs.end(); enIt++, iElem++) {
        ...
        for (nbNodes = 0; id != elemNodeInd.end(); id++) {
            if (*id < theNodesVector.size()) nodes[nbNodes++] = theNodesVector[*id];
            else
                nodes[nbNodes++] = myXYZIdToNodeMap[*id];
        }
        // dim of refined elem
        int elemIndex = iElem / nbNewElemsPerOld; // refined element index
        if (onMeshElements) { is2d = (theElements[elemIndex]->GetType() == SMDSAbs_Face); }
        // add an element
        const SMDS_MeshElement *elem = 0;
        if (is2d) {
            switch (nbNodes) {
                case 3: elem = aMeshDS->AddFace(nodes[0], nodes[1], nodes[2]); break;
                case 4: elem = aMeshDS->AddFace(nodes[0], nodes[1], nodes[2], nodes[3]); break;
                case 6:
                    if (!onMeshElements) { // create a quadratic face
                        elem = aMeshDS->AddFace(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                                nodes[5]);
                        break;
                ...
                case 8:
                    if (!onMeshElements) { // create a quadratic face
                      ...
                      /* FALLTHRU */
                default: elem = aMeshDS->AddPolygonalFace(nodes);
            }
        }
        else {
            switch (nbNodes) {
                case 4: elem = aMeshDS->AddVolume(nodes[0], nodes[1], nodes[2], nodes[3]); break;
                case 5:
                    elem = aMeshDS->AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4]);
                    break;
                case 6:
                    elem = aMeshDS->AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                              nodes[5]);
                ...
                case 8:
                    elem = aMeshDS->AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                              nodes[5], nodes[6], nodes[7]);
                    break;
                default: elem = aMeshDS->AddPolyhedralVolume(nodes, *quantity++);
            }
        }
        ...
        if (elem && onMeshElements) // applied to mesh elements
        {
            ...
            if (shapeID > 0) {
                ...
                if (S.ShapeType() == TopAbs_SOLID) {
                    ...
                    if (shellIt.More()) shapeID = aMeshDS->ShapeToIndex(shellIt.Value());
                }
                SMDS_ElemIteratorPtr noIt = elem->nodesIterator();
                while (noIt->more()) {
                    SMDS_MeshNode *node = const_cast<SMDS_MeshNode *>(smdsNode(noIt->next()));
                    if (node->getshapeId() < 1 && shellNodes.find(node) == shellNodes.end()) {
                        if (S.ShapeType() == TopAbs_FACE)
                            aMeshDS->SetNodeOnFace(
                                node, shapeID,
            ...
            for (; g != groups[elemIndex].end(); ++g) (*g)->SMDSGroup().Add(elem);
        }
    ...
    if (!myShape.IsNull()) {
        ...
        if (subMesh) subMesh->ComputeStateEngine(SMESH_subMesh::CHECK_COMPUTE_STATE);
    }
    if (onMeshElements) {
        list<int> elemIDs;
        for (int i = 0; i < theElements.size(); i++) {
...
bool SMESH_Pattern::isReversed(const SMDS_MeshNode *theFirstNode, const list<int> &theIdsList) const
{
    if (theIdsList.size() < 2) return false;

    gp_Pnt Pf(theFirstNode->X(), theFirstNode->Y(), theFirstNode->Z());
    gp_Pnt P[2];
    list<int>::const_iterator id = theIdsList.begin();
    for (int i = 0; i < 2; ++i, ++id) {
        if (*id < myXYZ.size()) P[i] = myXYZ[*id];
        else {
...
void SMESH_Pattern::arrangeBoundaries(list<list<TPoint *>> &boundaryList)
{
    ...
    if (nbBoundaries > 1) {
        ...
        if (nbBoundaries > 2) {
            ...
            for (; nbKpIt != myNbKeyPntInBoundary.end(); nbKpIt++, bndIt++) {
                ...
                while (nbKpBndPosMap.find(nb) != nbKpBndPosMap.end()) nb++;
    ...
    list<int>::iterator kpIt = myKeyPointIDs.begin();
    for (; kpIt != myKeyPointIDs.end(); kpIt++) keyPointSet.insert(&myPoints[*kpIt]);
    myKeyPointIDs.clear();
    ...
    for (bndIt = boundaryList.begin(); bndIt != boundaryList.end(); bndIt++, nbKpIt++) {
        ...
        if (p == boundary.front()) pPrev = *(++boundary.rbegin());
        else {
            xpIt--;
            pPrev = *xpIt;
            xpIt++;
        }
        if (p == boundary.back()) pNext = *(++boundary.begin());
        else {
            xpIt++;
            pNext = *xpIt;
        }
        ...
        for (; pIt != boundary.end(); pIt++) {
            TPoint *point = *pIt;
            if (keyPointSet.find(point) == keyPointSet.end()) continue;
            // find an index of a keypoint
            int index = 0;
            vector<TPoint>::const_iterator pVecIt = myPoints.begin();
            for (; pVecIt != myPoints.end(); pVecIt++, index++)
                if (&(*pVecIt) == point) break;
            myKeyPointIDs.push_back(index);
            (*nbKpIt)++;
        }
...
bool SMESH_Pattern::findBoundaryPoints()
{
    ...
    if (myIs2D) {
        ...
        for (; epIt != myElemPointIDs.end(); epIt++) {
            ...
            for (; pIt != elemPoints.end(); pIt++) {
                ...
                if (!itUniq.second) linkSet.erase(itUniq.first);
                prevP = *pIt;
        ...
        while (lIt != linkSet.end()) {
            if ((*lIt).first == point1) point2 = (*lIt).second;
            ...
            for (pIt++; pIt != boundary->end(); pIt++) {
                ...
                else // a key-point
                {
                    ...
                    if (edgePoints.size() > 2) {
                        ...
                        for (; epIt != edgePoints.end(); epIt++) (*epIt)->myInitU /= edgeLength;
                    }
    ...
    else // 3D case
    {
        ...
        for (int i = 0; pVecIt != myPoints.end(); pVecIt++, i++) {
            ...
            // detect key-points
            if (SMESH_Block::IsVertexID(shapeID)) myKeyPointIDs.push_back(i);
        }
    }
...
{
    thePoints.clear();
    if (!myIsComputed) return false;

    if (myElements.empty()) { // applied to shape
        vector<TPoint>::const_iterator pVecIt = myPoints.begin();
        for (; pVecIt != myPoints.end(); pVecIt++) thePoints.push_back(&(*pVecIt).myXYZ.XYZ());
    }
        ...
        for (; xyz != myXYZ.end(); ++xyz)
            if (!isDefined(*xyz)) thePoints.push_back(definedXYZ);
            else
...
bool SMESH_Pattern::GetPoints(list<const gp_XYZ *> &thePoints) const
{
    ...
    vector<TPoint>::const_iterator pVecIt = myPoints.begin();
    for (; pVecIt != myPoints.end(); pVecIt++) thePoints.push_back(&(*pVecIt).myInitXYZ);
...
{
    int aShapeID;
    if (!myShapeIDMap.Contains(theShape)) aShapeID = myShapeIDMap.Add(theShape);
    else

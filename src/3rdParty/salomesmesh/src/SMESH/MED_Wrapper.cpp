...
namespace MED
{
TLockProxy ::TLockProxy(TWrapper *theWrapper) : myWrapper(theWrapper)
{
...
{
    INITMSG(MYDEBUG,
            "~TLockProxy() - this -" << this << "; myWrapper = " << myWrapper << std::endl);
#if BOOST_VERSION >= 103500
...
TWrapper *TLockProxy ::operator->() const // never throws
{
    return myWrapper;
}
...
//----------------------------------------------------------------------------
PMeshInfo TWrapper ::GetPMeshInfo(TInt theId, TErr *theErr)
{
...
//----------------------------------------------------------------------------
PFamilyInfo TWrapper ::GetPFamilyInfo(const PMeshInfo &theMeshInfo, TInt theId, TErr *theErr)
{
    ...
    INITMSG(MYDEBUG,
            "GetPFamilyInfo - aFamilyName = '" << aName << "'; andId = " << anInfo->GetId()
                                               << "; aNbAttr = " << aNbAttr
                                               << "; aNbGroup = " << aNbGroup << "\n");
...
//----------------------------------------------------------------------------
PNodeInfo TWrapper ::GetPNodeInfo(const PMeshInfo &theMeshInfo, TErr *theErr)
{
    TInt aNbElems = GetNbNodes(*theMeshInfo);
    if (aNbElems == 0) { return PNodeInfo(); }
...
//----------------------------------------------------------------------------
PPolygoneInfo TWrapper ::GetPPolygoneInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                          EGeometrieElement theGeom, EConnectivite theConnMode)
{
    if (theMeshInfo->GetType() != eNON_STRUCTURE) return PPolygoneInfo();
    ...
    PPolygoneInfo anInfo =
        CrPolygoneInfo(theMeshInfo, theEntity, theGeom, aNbElem, aConnSize, theConnMode);
    GetPolygoneInfo(anInfo);

#ifdef _DEBUG_
    INITMSG(MYDEBUG,
            "GetPPolygoneInfo"
                << " - theGeom = " << theGeom << "; aNbElem = " << aNbElem << ": ");
...
//----------------------------------------------------------------------------
PPolyedreInfo TWrapper ::GetPPolyedreInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                          EGeometrieElement theGeom, EConnectivite theConnMode)
{
    if (theMeshInfo->GetType() != eNON_STRUCTURE) return PPolyedreInfo();
    ...
    PPolyedreInfo anInfo =
        CrPolyedreInfo(theMeshInfo, theEntity, theGeom, aNbElem, aNbFaces, aConnSize, theConnMode);
...
//----------------------------------------------------------------------------
PElemInfo TWrapper ::GetPElemInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                  EGeometrieElement theGeom, EConnectivite theConnMode,
                                  TErr *theErr)
{
    ...
    if (aType == eNON_STRUCTURE) {
        switch (theGeom) {
            case ePOINT1:
                if (theEntity == eNOEUD) return GetPNodeInfo(theMeshInfo, theErr);
            ...
            case ePOLYEDRE:
                ...
                break;
            default: return GetPCellInfo(theMeshInfo, theEntity, theGeom, theConnMode, theErr);
        }
    }
    else {
        ...
        switch (theGeom) {
            ...
            case eSEG2:
            case eQUAD4:
            case eHEXA8: nbElems = aGrille->GetNbCells(); break;
            default: nbElems = 0;
        }
        ...
        if (theGeom == ePOINT1) {
            aElemInfo = CrElemInfo(theMeshInfo, nbElems, theIsElemNum);
            ...
            GetFamilies(aTElemInfo, nbElems, theEntity, theGeom, theErr);
            ...
            aElemInfo = CrElemInfo(theMeshInfo, nbElems, aFamNum, aElemNum, aElemNames);
        }
...
//----------------------------------------------------------------------------
PCellInfo TWrapper ::GetPCellInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                                  EGeometrieElement theGeom, EConnectivite theConnMode,
                                  TErr *theErr)
{
    if (theMeshInfo->GetType() != eNON_STRUCTURE) return PCellInfo();
    ...
    INITMSG(MYDEBUG,
            "GetPCellInfo - theEntity = " << theEntity << "; theGeom = " << theGeom
                                          << "; aConnDim: " << aConnDim << "\n");
...
PBallInfo TWrapper ::GetPBallInfo(const PMeshInfo &theMeshInfo)
{
...
//----------------------------------------------------------------------------
PFieldInfo TWrapper ::GetPFieldInfo(const PMeshInfo &theMeshInfo, TInt theId, TErr *theErr)
{
    ...
    INITMSG(MYDEBUG,
            "GetPFieldInfo "
                ...
                << "; aType = " << anInfo->GetType() << "; aNbComp = " << aNbComp << std::endl);
#endif
...
//----------------------------------------------------------------------------
PProfileInfo TWrapper ::GetPProfileInfo(TInt theId, EModeProfil theMode, TErr *theErr)
{
...
//----------------------------------------------------------------------------
PTimeStampValueBase TWrapper ::CrTimeStampValue(const PTimeStampInfo &theTimeStampInfo,
                                                const TGeom2Profile &theGeom2Profile,
                                                EModeSwitch theMode)
{
    PFieldInfo aFieldInfo = theTimeStampInfo->GetFieldInfo();
    return CrTimeStampValue(theTimeStampInfo, aFieldInfo->GetType(), theGeom2Profile, theMode);
}
...
{
    PFieldInfo aFieldInfo = theTimeStampInfo->GetFieldInfo();
    return CrTimeStampValue(theTimeStampInfo, theInfo, aFieldInfo->GetType());
}

//----------------------------------------------------------------------------
template<class TimeStampValueType> void Print(SharedPtr<TimeStampValueType> theTimeStampValue)
{
...
//----------------------------------------------------------------------------
PTimeStampValueBase TWrapper ::GetPTimeStampValue(const PTimeStampInfo &theTimeStampInfo,
                                                  const TMKey2Profile &theMKey2Profile,
                                                  const TKey2Gauss &theKey2Gauss, TErr *theErr)
{
    PFieldInfo aFieldInfo = theTimeStampInfo->GetFieldInfo();
    PTimeStampValueBase anInfo = CrTimeStampValue(theTimeStampInfo, aFieldInfo->GetType());
    GetTimeStampValue(anInfo, theMKey2Profile, theKey2Gauss, theErr);
#ifdef _DEBUG_
    if (aFieldInfo->GetType() == eFLOAT64) Print<TFloatTimeStampValue>(anInfo);
    else
...
//----------------------------------------------------------------------------
void TWrapper ::GetTimeStampVal(const PTimeStampVal &theVal, const TMKey2Profile &theMKey2Profile,
                                const TKey2Gauss &theKey2Gauss, TErr *theErr)
{
    ...
    if (aFieldInfo->GetType() == eFLOAT64)
        GetTimeStampValue(theVal, theMKey2Profile, theKey2Gauss, theErr);
    else {
        PTimeStampValueBase aVal = CrTimeStampValue(aTimeStampInfo, theVal, eINT);
        GetTimeStampValue(aVal, theMKey2Profile, theKey2Gauss, theErr);
        CopyTimeStampValueBase(aVal, theVal);
    }
}

//----------------------------------------------------------------------------
void TWrapper ::SetTimeStamp(const PTimeStampVal &theVal, TErr *theErr)
{
    PTimeStampInfo aTimeStampInfo = theVal->GetTimeStampInfo();
    PFieldInfo aFieldInfo = aTimeStampInfo->GetFieldInfo();
    if (aFieldInfo->GetType() == eFLOAT64) SetTimeStampValue(theVal, theErr);
    else {
        PTimeStampValueBase aVal = CrTimeStampValue(aTimeStampInfo, eINT, theVal->GetGeom2Profile(),
...
{
    return CrTimeStampValue(theTimeStampInfo, eFLOAT64, theGeom2Profile, theMode);
}
...
{
    return CrTimeStampValue(theTimeStampInfo, theInfo, eFLOAT64);
}
...
//----------------------------------------------------------------------------
PGrilleInfo TWrapper ::GetPGrilleInfo(const PMeshInfo &theMeshInfo)
{
    if (theMeshInfo->GetType() != eSTRUCTURE) return PGrilleInfo();
    ...
    if (type == eGRILLE_STANDARD) {
        ...
        for (int aAxe = 0; aAxe < theMeshInfo->GetDim(); aAxe++) {
            ETable aATable;
            switch (aAxe) {
                case 0: aATable = eCOOR_IND1; break;
                case 1: aATable = eCOOR_IND2; break;
                case 2: aATable = eCOOR_IND3; break;
            }
...
//----------------------------------------------------------------------------
PGrilleInfo TWrapper ::GetPGrilleInfo(const PMeshInfo &theMeshInfo, const PGrilleInfo &theInfo)
{

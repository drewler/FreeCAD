...
void SMESH_Block::TEdge::Set(const int edgeID, Adaptor3d_Curve *curve, const bool isForward)
{
    ...
    if (!isForward) std::swap(myFirst, myLast);
}
...
void SMESH_Block::TFace::Set(const int faceID, Adaptor3d_Surface *S, Adaptor2d_Curve2d *c2D[4],
                             const bool isForward[4])
{
    ...
    for (int iE = 0; iE < edgeIdVec.size(); iE++) // loop on 4 edges
    {
        ...
        if (!isForward[iE]) std::swap(myFirst[iE], myLast[iE]);
    }
    // 2d corners
...
void SMESH_Block::TFace::GetCoefs(int iE, const gp_XYZ &theParams, double &Ecoef,
                                  double &Vcoef) const
{
...
namespace
{
inline bool isPntInTria(const gp_XY &p, const gp_XY &t0, const gp_XY &t1, const gp_XY &t2)
{
    double bc0, bc1;
...
inline bool isPntInQuad(const gp_XY &p, const gp_XY &q0, const gp_XY &q1, const gp_XY &q2,
                        const gp_XY &q3)
{
...
bool SMESH_Block::TFace::IsUVInQuad(const gp_XY &uv, const gp_XYZ &param0, const gp_XYZ &param1,
...
double *SMESH_Block::GetShapeCoef(const int theShapeID)
{
    static double shapeCoef[][3] = {
        ...
        // ID_Shell
        {0, 0, 0}};
    if (theShapeID < ID_V000 || theShapeID > ID_F1yz) return shapeCoef[ID_Shell - 1];
...
bool SMESH_Block::ShellPoint(const gp_XYZ &theParams, gp_XYZ &thePoint) const
{
    ...
    for (int shapeID = ID_V000; shapeID < ID_Shell; shapeID++) {
        ...
        for (int iCoef = 0; iCoef < 3; iCoef++) {
            if (coefs[iCoef] != 0) {
                if (coefs[iCoef] < 0) k *= (1. - theParams.Coord(iCoef + 1));
                else
...
bool SMESH_Block::ShellPoint(const gp_XYZ &theParams, const vector<gp_XYZ> &thePointOnShape,
                             gp_XYZ &thePoint)
{
    if (thePointOnShape.size() < ID_F1yz) return false;
    ...
    thePoint = x1 * p[ID_F0yz] + x * p[ID_F1yz] + y1 * p[ID_Fx0z] + y * p[ID_Fx1z] + z1 * p[ID_Fxy0]
        + z * p[ID_Fxy1]
        + x1 * (y1 * (z1 * p[ID_V000] + z * p[ID_V001]) + y * (z1 * p[ID_V010] + z * p[ID_V011]))
        + x * (y1 * (z1 * p[ID_V100] + z * p[ID_V101]) + y * (z1 * p[ID_V110] + z * p[ID_V111]));
    thePoint -= x1 * (y1 * p[ID_E00z] + y * p[ID_E01z]) + x * (y1 * p[ID_E10z] + y * p[ID_E11z])
        + y1 * (z1 * p[ID_Ex00] + z * p[ID_Ex01]) + y * (z1 * p[ID_Ex10] + z * p[ID_Ex11])
        + z1 * (x1 * p[ID_E0y0] + x * p[ID_E1y0]) + z * (x1 * p[ID_E0y1] + x * p[ID_E1y1]);
...
SMESH_Block::SMESH_Block()
    : myNbIterations(0), mySumDist(0.), myTolerance(-1.) // to be re-initialized
{}
...
Standard_Integer SMESH_Block::NbVariables() const { return 3; }
...
Standard_Integer SMESH_Block::NbEquations() const { return 1; }
...
Standard_Boolean SMESH_Block::Values(const math_Vector &theXYZ, math_Vector &theFxyz,
                                     math_Matrix &theDf)
{
    gp_XYZ P, params(theXYZ(1), theXYZ(2), theXYZ(3));
    if (params.IsEqual(myParam, DBL_MIN)) { // same param
    ...
    if (sqDist < myValues[SQUARE_DIST]) // a better guess
    {
        ...
        for (int iP = 1; iP <= 3; iP++) {
            ...
            if (onEdge) params.SetCoord(iP, theXYZ(iP) - 0.001);
            else
        ...
        for (int iP = 0; iP < 3; iP++) {
            ...
            if (Abs(Direc) <= DBL_MIN) theDf(1, iP + 1) = dP * drv[iP];
            else {
...
#ifdef DEBUG_PARAM_COMPUTE
        MESSAGE("F = " << theFxyz(1) << " DRV: " << theDf(1, 1) << " " << theDf(1, 2) << " "
                       << theDf(1, 3));
...
bool SMESH_Block::computeParameters(const gp_Pnt &thePoint, gp_XYZ &theParams,
                                    const gp_XYZ &theParamsHint, int theShapeID)
{
    ...
    while (distance() > loopTol && nbLoops <= 3) {
        paramSearch.Perform(*static_cast<math_FunctionSetWithDerivatives *>(this), start, low, up);
        start(1) = myParam.X();
        start(2) = myParam.Y();
        start(3) = myParam.Z();
    ...
    MESSAGE(" ------ SOLUTION: ( "
            ...
            << " ------ DIST : " << distance() << "\t Tol=" << myTolerance
            << "\t Nb LOOPS=" << nbLoops << endl
    ...
    if (myFaceIndex > 0) {
        ...
        if (distance() > loopTol) refineParametersOnFace(thePoint, theParams, theShapeID);
    }
    return true;
}
...
bool SMESH_Block::ComputeParameters(const gp_Pnt &thePoint, gp_XYZ &theParams, const int theShapeID,
                                    const gp_XYZ &theParamsHint)
{
    if (VertexParameters(theShapeID, theParams)) return true;
    ...
    bool hasHint = (0 <= theParamsHint.X() && theParamsHint.X() <= 1 && 0 <= theParamsHint.Y()
                    && theParamsHint.Y() <= 1 && 0 <= theParamsHint.Z() && theParamsHint.Z() <= 1);
    if (!hasHint && !myGridComputed) {
        ...
        if (needGrid) {
            ...
            myGridComputed = true;
            if (myTolerance < 0) myTolerance = sqrt(box.SquareExtent()) * 1e-5;
        }
    }

    if (hasHint) { start = theParamsHint; }
    if (myGridComputed) {
        double minDist = DBL_MAX;
        if (hasHint) {
            gp_XYZ p;
            if (ShellPoint(start, p)) minDist = thePoint.SquareDistance(p);
        }
        gp_XYZ *bestParam = 0;
    ...
    while (nbLoops <= 100) {
        ...
        for (int iP = 1; iP <= 3; iP++) {
            ...
            if (onEdge) nearParams.SetCoord(iP, params.Coord(iP) - parDelta);
            else
    ...
    MESSAGE(" ------ SOLUTION: ( "
            << solution.X() << " " << solution.Y() << " " << solution.Z() << " )" << std::endl
            << " ------ DIST : " << sqrt(sqDistance) << "\t Tol=" << myTolerance
            << "\t Nb LOOPS=" << nbLoops << std::endl
            << " ------ NB IT: " << myNbIterations << ",  SUM DIST: " << mySumDist);
#endif

    if (myFaceIndex > 0) theParams.SetCoord(myFaceIndex, myFaceParam);
    ...
    if (reachedDist > 10 * myTolerance
        && computeParameters(thePoint, theParams, solution, theShapeID) && reachedDist > distance())
        return true;
...
void SMESH_Block::refineParametersOnFace(const gp_Pnt &thePoint, gp_XYZ &theParams, int theFaceID)
{
    ...
    Extrema_ExtPS extPS(thePoint, *tface.Surface(), tface.Surface()->UResolution(myTolerance),
                        tface.Surface()->VResolution(myTolerance));
    if (!extPS.IsDone() || extPS.NbExt() < 1) return;
    ...
    if (minDist > 100 * myTolerance * myTolerance) return;

    gp_XY uv(U, V);
    if (findUVByHalfDivision(thePoint, uv, tface, theParams)) return;

    int nbGetWorstLimit = 20;
    if (findUVAround(thePoint, uv, tface, theParams, nbGetWorstLimit)) return;
        ...
        for (double y = delta; y < 1.; y += delta) {
            ...
            for (double x = delta; x < 1.; x += delta) {
                ...
                if (dist2 < prevSolDist * prevSolDist) {
                    if (findUVAround(thePoint, uv, tface, theParams, nbGetWorstLimit)) return;
                    if (distance() < 1000 * myTolerance) return;
...
bool SMESH_Block::findUVByHalfDivision(const gp_Pnt &thePoint, const gp_XY &theUV,
                                       const SMESH_Block::TFace &tface, gp_XYZ &theParams)
{
    ...
    gp_XYZ xXYZ(0, 0, 0);
    xXYZ.SetCoord(tface.GetUInd(), 1);
    gp_XYZ yXYZ(0, 0, 0);
    yXYZ.SetCoord(tface.GetVInd(), 1);
    gp_XYZ xy0, xy1, xy2, xy3;
    bool isInQuad = false;
    while (!isInQuad) {
        xMin = Max(0., xSol - 0.5 * dx);
        xMax = Min(1.0, xSol + 0.5 * dx);
        yMin = Max(0., ySol - 0.5 * dy);
        yMax = Min(1.0, ySol + 0.5 * dy);
...
bool SMESH_Block::findUVAround(const gp_Pnt &thePoint, const gp_XY &theUV,
                               const SMESH_Block::TFace &tface, gp_XYZ &theParams,
                               int nbGetWorstLimit)
{
#ifdef _DEBUG_REFINE_
    cout << "SMESH_Block::refineParametersOnFace(): walk around Starts at dist " << distance()
         << endl;
    ...
    //nbGetUV++;
    if (saveBetterSolution(sol, theParams, thePoint.SquareDistance(tface.Point(sol)))) return true;
    ...
    while (xMax - xMin < 1 || yMax - yMin < 1) {
        ...
        if (yMin > 0.) {
            ...
            for (x = Max(0., xMin); x <= xMax + paramTol; x += dx) {
                ...
                if (saveBetterSolution(sol, theParams, dist)) return true;
            ...
            if (yMinNbGetWorst > nbGetWorstLimit) yMin = 0;
        }
        if (yMax < 1.) {
            bestDist = 1e100;
            for (x = Max(0., xMin); x <= xMax + paramTol; x += dx) {
            ...
            if (yMaxNbGetWorst > nbGetWorstLimit) yMax = 1;
        }
        // walk along Y
        if (xMin > 0.) {
            bestDist = 1e100;
            ...
            if (xMinNbGetWorst > nbGetWorstLimit) xMin = 0;
        }
        if (xMax < 1.) {
            bestDist = 1e100;
            for (y = Max(0., yMin); y <= yMax + paramTol; y += dy) {
            ...
            if (xMaxNbGetWorst > nbGetWorstLimit) xMax = 1;
        }
    }
#ifdef _DEBUG_REFINE_
    cout << "SMESH_Block::refineParametersOnFace(): walk around failed at dist " << distance()
         << endl;
    //cout << " nbGetUV = " << nbGetUV << endl;
#endif
...
bool SMESH_Block::saveBetterSolution(const gp_XYZ &theNewParams, gp_XYZ &theParams,
                                     double sqDistance)
{
    if (myValues[SQUARE_DIST] > sqDistance) {
        myValues[SQUARE_DIST] = sqDistance;
        theParams = theNewParams;
        if (distance() <= myTolerance) return true;
    }
    return false;
}
...
bool SMESH_Block::IsToleranceReached() const { return distance() < myTolerance; }
...
#define CASEDUMP(id, strm)                                                                         \
    case id: strm << #id; break;
...
int SMESH_Block::GetShapeIDByParams(const gp_XYZ &theCoord)
{
    ...
    if (id > 26 || id < 0) {
        MESSAGE("GetShapeIDByParams() = " << id << " " << theCoord.X() << " " << theCoord.Y() << " "
                                          << theCoord.Z());
    }
...
int SMESH_Block::GetOrderedEdges(const TopoDS_Face &theFace, list<TopoDS_Edge> &theEdges,
                                 list<int> &theNbEdgesInWires, TopoDS_Vertex theFirstVertex,
                                 const bool theShapeAnalysisAlgo)
{
    ...
    for (TopoDS_Iterator wIt(theFace); wIt.More(); wIt.Next())
        ...
        {
            if (!anOuterWire.IsSame(wIt.Value())) aWireList.push_back(TopoDS::Wire(wIt.Value()));
            else
        ...
        if (wlIt == aWireList.begin() && theEdges.size() > 1) { // the outer wire
            ...
            for (eIt = theEdges.begin(); eIt != theEdges.end(); eIt++) {
                ...
                if (TopExp::FirstVertex(edge).IsSame(TopExp::LastVertex(edge))) {
                    ...
                    if (isNext ? isFirst : !isFirst) edge.Reverse();
                    // to make a seam go first
                    if (theFirstVertex.IsNull()) theFirstVertex = TopExp::FirstVertex(edge, true);
                }
            }
            ...
            if (!theFirstVertex.IsNull()) {
                ...
                while (!vv[0].IsSame(theFirstVertex) || vv[0].IsSame(vv[1])) {
                    theEdges.splice(theEdges.end(), theEdges, theEdges.begin(), ++theEdges.begin());
                    TopExp::Vertices(theEdges.front(), vv[0], vv[1], true);
                    if (iE++ > theNbEdgesInWires.back()) {
#ifdef _DEBUG_
                        gp_Pnt p = BRep_Tool::Pnt(theFirstVertex);
                        MESSAGE(" : Warning : vertex "
                                << theFirstVertex.TShape().operator->() << " ( " << p.X() << " "
                                << p.Y() << " " << p.Z() << " )"
                                << " not found in outer wire of face "
                                << theFace.TShape().operator->() << " with vertices: ");
                        ...
                        for (int i = 0; wExp.More(); wExp.Next(), i++) {
                            ...
                            MESSAGE_ADD(i << " " << v.TShape().operator->() << " " << p.X() << " "
                                          << p.Y() << " " << p.Z() << " " << std::endl);
                        }
#endif
...
bool SMESH_Block::LoadMeshBlock(const SMDS_MeshVolume *theVolume, const int theNode000Index,
                                ...
                                vector<const SMDS_MeshNode *> &theOrderedNodes)
{
    ...
    for (iF = 0; iF < vTool.NbFaces(); ++iF) {
        ...
        for (iN = 0; iN < 4; ++iN)
            if (nid[iN] == V000) { fV000.push_back(iF); }
            else if (nid[iN] == V001) {
                fV001.push_back(iF);
            }
    }
    ...
    for (iE = 0; iE < NbEdges(); ++iE) {
        ...
        myEdge[iE].Set((iE + ID_FirstE), myPnt[vertexVec[0] - 1], myPnt[vertexVec[1] - 1]);
    }
...
bool SMESH_Block::LoadBlockShapes(const TopoDS_Shell &theShell, const TopoDS_Vertex &theVertex000,
...
bool SMESH_Block::FindBlockShapes(const TopoDS_Shell &theShell, const TopoDS_Vertex &theVertex000,
                                  const TopoDS_Vertex &theVertex001,
                                  TopTools_IndexedMapOfOrientedShape &theShapeIDMap)
{
    ...
    if (f000List.Extent() != NB_FACES_BY_VERTEX || f001List.Extent() != NB_FACES_BY_VERTEX) {
    ...
    for (j = 0; f000It.More(); f000It.Next(), j++) {
        ...
        for (i = 0, f001It.Initialize(f001List); f001It.More(); f001It.Next(), i++) {
            ...
            if (F.IsSame(f001It.Value())) break;
        }
    ...
    for (i = 0; elIt != eList.end(); elIt++, i++) switch (i) {
            case 0:
                E0y0 = *elIt;
                V010 = TopExp::LastVertex(*elIt, true);
                break;
            case 1:
                Ex10 = *elIt;
                V110 = TopExp::LastVertex(*elIt, true);
                break;
            case 2:
                E1y0 = *elIt;
                V100 = TopExp::LastVertex(*elIt, true);
                break;
    ...
    for (i = 0, elIt = eList.begin(); elIt != eList.end(); elIt++, i++) switch (i) {
            case 0:
                Ex01 = *elIt;
                V101 = TopExp::LastVertex(*elIt, true);
                break;
            case 1:
                E1y1 = *elIt;
                V111 = TopExp::LastVertex(*elIt, true);
                break;
            case 2:
                Ex11 = *elIt;
                V011 = TopExp::LastVertex(*elIt, true);
                break;
    ...
    for (; exp.More(); exp.Next()) // Fx0z shares V101 and V100
        if (V101.IsSame(exp.Current()) || V100.IsSame(exp.Current())) break; // V101 or V100 found
    ...
    if (f111List.Extent() != NB_FACES_BY_VERTEX || f110List.Extent() != NB_FACES_BY_VERTEX) {
    ...
    for (exp.Init(Fx1z, TopAbs_VERTEX); exp.More(); exp.Next())
        if (V010.IsSame(exp.Current()) || V011.IsSame(exp.Current())) break;
    if (!exp.More()) { std::swap(Fx1z, F1yz); }

    // find vertical edges
    for (exp.Init(Fx0z, TopAbs_EDGE); exp.More(); exp.Next()) {
        const TopoDS_Edge &edge = TopoDS::Edge(exp.Current());
        const TopoDS_Shape &vFirst = TopExp::FirstVertex(edge, true);
        if (vFirst.IsSame(V001)) E00z = edge;
    ...
    for (exp.Init(Fx1z, TopAbs_EDGE); exp.More(); exp.Next()) {
        const TopoDS_Edge &edge = TopoDS::Edge(exp.Current());
        const TopoDS_Shape &vFirst = TopExp::FirstVertex(edge, true);
        if (vFirst.IsSame(V111)) E11z = edge;
    ...
    for (int shapeID = 1; shapeID < theShapeIDMap.Extent(); shapeID++) {
        ...
        switch (S.ShapeType()) {
            ...
            case TopAbs_EDGE: {
                ...
                tEdge.Set(shapeID, new BRepAdaptor_Curve(edge), IsForwardEdge(edge, theShapeIDMap));
                break;
            }
            case TopAbs_FACE: {

                if (!LoadFace(TopoDS::Face(S), shapeID, theShapeIDMap)) return false;
                break;
            }
            default: break;
        }
...
bool SMESH_Block::LoadFace(const TopoDS_Face &theFace, const int theFaceID,
                           const TopTools_IndexedMapOfOrientedShape &theShapeIDMap)
{
    ...
    for (int iE = 0; iE < edgeIdVec.size(); iE++) // loop on 4 edges
    {
        if (edgeIdVec[iE] > theShapeIDMap.Extent()) return false;
...
bool SMESH_Block::Insert(const TopoDS_Shape &theShape, const int theShapeID,
                         TopTools_IndexedMapOfOrientedShape &theShapeIDMap)
{
...
void SMESH_Block::GetFaceEdgesIDs(const int faceID, vector<int> &edgeVec)
{
    ...
    switch (faceID) {
        ...
        case ID_F1yz:
            ...
            edgeVec[3] = ID_E11z;
            break;
        default: MESSAGE(" GetFaceEdgesIDs(), wrong face ID: " << faceID);
    }
}
...
void SMESH_Block::GetEdgeVertexIDs(const int edgeID, vector<int> &vertexVec)
{
    ...
    switch (edgeID) {
        ...
        case ID_E11z:
            ...
            vertexVec[1] = ID_V111;
            break;
        default: vertexVec.resize(0); MESSAGE(" GetEdgeVertexIDs(), wrong edge ID: " << edgeID);
    }
}

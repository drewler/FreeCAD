...
void SmeshException() { throw SMESH_Exception("Smesh Exception"); }

SMESH_Exception::SMESH_Exception(void) : /*exception() ,*/ _text(0)
{
    MESSAGE(
        "You must use the standard builder: SMESH_Exception::SMESH_Exception( const char *text )");
    INTERRUPTION(1);
}
...
SMESH_Exception::SMESH_Exception(const char *text, const char *fileName,
...
const char *SMESH_Exception::what(void) const throw() { return _text; }

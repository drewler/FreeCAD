...
DriverMED_W_Field::DriverMED_W_Field()
    : //_medFileID( -1 ),
      _elemType(SMDSAbs_All), _dt(-1), _it(-1)
{}
...
bool DriverMED_W_Field::Set(SMESHDS_Mesh *mesh, const std::string &fieldName,
                            SMDSAbs_ElementType type, const int nbComps, const bool isIntData)
{
    _fieldName = fieldName;
    _compNames.resize(nbComps, "");

    if (type == SMDSAbs_All) {
        if (mesh->NbVolumes() > 0) type = SMDSAbs_Volume;
    ...
    if (myMesh != mesh) {
        ...
        for (int iG = 0; iG < SMDSEntity_Last; ++iG) _elemsByGeom[iG].clear();
        SetMesh(mesh);
    }
    ...
    if (_nbElemsByGeom.empty() || _elemType != type) {
        ...
        if (_addODOnVertices && _elemType == SMDSAbs_0DElement) {
            ...
            if (nodes.empty()) DriverMED_W_SMESHDS_Mesh::getNodesOfMissing0DOnVert(myMesh, nodes);
            if (!nodes.empty()) {
                if (_nbElemsByGeom.size() == 1)
                    _nbElemsByGeom.push_back(std::make_pair(SMDSEntity_0D, 0));
                _nbElemsByGeom.push_back(
                    std::make_pair(SMDSEntity_Node, nodes.size() + _nbElemsByGeom.back().second));
            }
        }
    ...
    if (isIntData) _intValues.reserve(nbElems * nbComps);
    else
...
{
    if ((int)_compNames.size() <= iComp) _compNames.resize(iComp + 1);
...
void DriverMED_W_Field::AddValue(double val) { _dblValues.push_back(val); }
...
void DriverMED_W_Field::AddValue(int val) { _intValues.push_back(val); }
...
{
    if (_nbElemsByGeom.size() < 2) return SMDS_ElemIteratorPtr();
    ...
    for (size_t iG = 1; iG < _nbElemsByGeom.size(); ++iG) {
        ...
        iterVec[iG - 1] = SMDS_ElemIteratorPtr(new SMDS_ElementVectorIterator(
            _elemsByGeom[iGeoType].begin(), _elemsByGeom[iGeoType].end()));
    }
    typedef SMDS_IteratorOnIterators<const SMDS_MeshElement *, std::vector<SMDS_ElemIteratorPtr>>
        TItIterator;
...
Driver_Mesh::Status DriverMED_W_Field::Perform()
{
    ...
    if (_nbElemsByGeom.size() < 2) return addMessage("No values to write", /*isFatal=*/false);
    if (!myMesh) return addMessage("Supporting mesh not set", /*isFatal=*/true);

    MED::PWrapper medFile = MED::CrWrapper(myFile, MED::eV2_2);
    MED::PMeshInfo meshInfo;
    if (myMeshId > 0) { meshInfo = medFile->GetPMeshInfo(myMeshId); }
    else {
        ...
        for (int iMesh = aNbMeshes; iMesh > 0 && myMeshId < 1; --iMesh) {
            ...
            if (!meshInfo || meshInfo->GetName() != myMeshName) meshInfo.reset();
            else
                myMeshId = iMesh;
        }
    }
    if ((!meshInfo) || (!myMeshName.empty() && meshInfo->GetName() != myMeshName)) {
        myMeshId = -1;
    ...
    MED::PFieldInfo fieldInfo = medFile->CrFieldInfo(meshInfo, _compNames.size(), dataType);
    ...
    if (_compNames.size() > 1) {
        for (size_t i = 0; i < fieldInfo->myCompNames.size() - 1; ++i)
            if (!fieldInfo->myCompNames[i]) fieldInfo->myCompNames[i] = ' ';
    }
    medFile->SetFieldInfo(fieldInfo);

    // specific treatment of added 0D elements
    if (_nbElemsByGeom.size() == 3 && _nbElemsByGeom[1].first == SMDSEntity_0D) {
    ...
    for (size_t iG = 1; iG < _nbElemsByGeom.size(); ++iG) {
        ...
        MED::EGeometrieElement medType = (MED::EGeometrieElement)DriverMED::GetMedGeoType(smdsType);
        int nbElems =
            (_nbElemsByGeom[iG].second - _nbElemsByGeom[iG - 1].second) * _compNames.size();
        if (dataType == MED::eFLOAT64) {
            ptrDbl = timeStampFltVal->GetMeshValue(medType).GetPointer();
            for (int i = 0; i < nbElems; ++i, ++iVal) ptrDbl[i] = _dblValues[iVal];
        }
        else {
            ptrInt = timeStampIntVal->GetMeshValue(medType).GetPointer();
            for (int i = 0; i < nbElems; ++i, ++iVal) ptrInt[i] = _intValues[iVal];
        }
    }
...
int GetMedGeoType(SMDSAbs_EntityType smdsType) { return getMedTypesVec()[smdsType]; }

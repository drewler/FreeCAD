...
struct _ImportData {
    ...
    void removeImportedMesh(SMESHDS_Mesh *meshDS)
    {
        ...
        while (eIt->more()) meshDS->RemoveFreeElement(eIt->next(), 0, /*fromGroups=*/false);
        SMDS_NodeIteratorPtr nIt = _importMeshSubDS->GetNodes();
        while (nIt->more()) meshDS->RemoveFreeNode(nIt->next(), 0, /*fromGroups=*/false);
    ...
    void removeGroups(SMESH_subMesh *subM, const StdMeshers_ImportSource1D *srcHyp)
    {
        ...
        vector<SMESH_Group *> *groups =
            const_cast<StdMeshers_ImportSource1D *>(srcHyp)->GetResultGroups(*srcMeshDS,
                                                                             *tgtMeshDS);
        if (groups) {
    ...
    void trackHypParams(SMESH_subMesh *sm, const StdMeshers_ImportSource1D *srcHyp)
    {
        ...
        if (toCopyMesh) _copyMeshSubM.insert(sm);
            ...
            _copyMeshSubM.erase(sm);

        if (toCopyGroups) _copyGroupSubM.insert(sm);
        else
            _copyGroupSubM.erase(sm);
    }
    void addComputed(SMESH_subMesh *sm)
    {
        ...
        while (smIt->more()) {
            ...
            switch (sm->GetSubShape().ShapeType()) {
                case TopAbs_EDGE:
                    if (SMESH_Algo::isDegenerated(TopoDS::Edge(sm->GetSubShape()))) continue;
                    /* FALLTHRU */
                case TopAbs_FACE:
                    _subM.insert(sm);
                    if (!sm->IsEmpty()) _computedSubM.insert(sm);
...
class _Listener: public SMESH_subMeshEventListener
{
    ...
    _Listener()
        : SMESH_subMeshEventListener(/*isDeletable=*/false, "StdMeshers_Import_1D::_Listener")
    {}

public:
    // return poiter to a static listener
    static _Listener *get()
    {
        static _Listener theListener;
        return &theListener;
    }
    ...
    {
        sm->SetEventListener(get(), SMESH_subMeshEventListenerData::MakeData(sm, WAIT_HYP_MODIF),
                             sm);
    }
};
//--------------------------------------------------------------------------------
/*!
   ...
   */
_ImportData *_Listener::getImportData(const SMESH_Mesh *srcMesh, SMESH_Mesh *tgtMesh)
{
    list<_ImportData> &dList = get()->_tgtMesh2ImportData[tgtMesh];
    list<_ImportData>::iterator d = dList.begin();
    for (; d != dList.end(); ++d)
        if (d->_srcMesh == srcMesh) return &*d;
...
/*!
   ...
   */
void _Listener::storeImportSubmesh(SMESH_subMesh *importSub, const SMESH_Mesh *srcMesh,
                                   const StdMeshers_ImportSource1D *srcHyp)
{
...
void _Listener::removeSubmesh(SMESH_subMesh *sm, _ListenerData *data)
{
    ...
    for (; d != dList.end(); ++d)
        if ((*d)._subM.erase(sm)) {
            ...
            if (rmMesh) d->removeImportedMesh(sm->GetFather()->GetMeshDS());
            if (rmGroups && data && data->myType == SRC_HYP) d->removeGroups(sm, data->_srcHyp);
        }
}
//--------------------------------------------------------------------------------
/*!
...
void _Listener::clearN2N(SMESH_Mesh *tgtMesh)
{
    list<_ImportData> &dList = get()->_tgtMesh2ImportData[tgtMesh];
    list<_ImportData>::iterator d = dList.begin();
    for (; d != dList.end(); ++d) d->_n2n.clear();
}
//--------------------------------------------------------------------------------
/*!
    ...
    for (; d != dList.end(); ++d) {
        ...
        if ((*d)._computedSubM.erase(sm)) {
            ...
            if (copyMesh || clearAllSub) {
                ...
                if (data && data->myType == SRC_HYP) d->removeGroups(sm, data->_srcHyp);
        ...
        if (data && data->myType == SRC_HYP) d->trackHypParams(sm, data->_srcHyp);
        d->_n2n.clear();
        d->_e2e.clear();
    }
}
...
/*!
   ...
   */
void _Listener::ProcessEvent(const int event, const int eventType, SMESH_subMesh *subMesh,
                             SMESH_subMeshEventListenerData *data, const SMESH_Hypothesis * /*hyp*/)
{
    if (data && data->myType == WAIT_HYP_MODIF) {
        // event of Import submesh
        if (SMESH_subMesh::MODIF_HYP == event && SMESH_subMesh::ALGO_EVENT == eventType) {
        ...
        if (!data->mySubMeshes.empty()) clearN2N(data->mySubMeshes.front()->GetFather());
    }
    else // event of Import submesh
    {
        // find out what happens: import hyp modified or removed
        bool removeImport = false, modifHyp = false;
        if (SMESH_subMesh::ALGO_EVENT == eventType) modifHyp = true;
        if (subMesh->GetAlgoState() != SMESH_subMesh::HYP_OK) { removeImport = true; }
        else if ((SMESH_subMesh::REMOVE_ALGO == event || SMESH_subMesh::REMOVE_FATHER_ALGO == event)
        ...
        if (removeImport) {
            ...
            removeSubmesh(subMesh, (_ListenerData *)data);
                 ...
                 || (SMESH_subMesh::CLEAN == event && SMESH_subMesh::COMPUTE_EVENT == eventType)) {
            ...
            for (; d != dList.end(); ++d)
                if (d->_subM.count(subMesh)) {
                    ...
                    for (; smIt != d->_subM.end(); ++smIt)
                        if ((*smIt)->IsMeshComputed()) d->_computedSubM.insert(*smIt);
                }
        }
        // Clear _ImportData::_n2n if it's no more useful, i.e. when
        // the event is not within mesh.Compute()
        if (SMESH_subMesh::ALGO_EVENT == eventType) clearN2N(subMesh->GetFather());
    }
}
...
int getSubmeshIDForCopiedMesh(const SMESHDS_Mesh *srcMeshDS, SMESH_Mesh *tgtMesh)
{
    ...
    if (!smDS) smDS = tgtMeshDS->NewSubMesh(sm->GetId());
    ...
    if (smDS->IsComplexSubmesh()) {
        ...
        for (; sub != subSM.end(); ++sub) smDS->RemoveSubMesh(*sub);
    }
    return sm->GetId();
}
...
SMESHDS_SubMesh *getSubmeshForCopiedMesh(const SMESH_Mesh *srcMesh, SMESH_Mesh *tgtMesh,
...
bool StdMeshers_Import_1D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                           SMESH_Hypothesis::Hypothesis_Status &aStatus)
{
    ...
    if (hypName == _compatibleHypothesis.front()) {
        ...
        if (aStatus == HYP_BAD_PARAMETER) _Listener::waitHypModification(aMesh.GetSubMesh(aShape));
...
bool StdMeshers_Import_1D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (srcGroups.empty()) return error("Invalid source groups");
    ...
    for (; vExp.More(); vExp.Next()) {
        ...
        if (!subShapeIDs.insert(tgtMesh->ShapeToIndex(v)).second) continue; // closed edge
    ...
    for (int iG = 0; iG < srcGroups.size(); ++iG) {
        ...
        double u =
            0.314159; // "random" value between 0 and 1, avoid 0 and 1, false detection possible on edge restrictions
        while (srcElems->more()) // loop on group contents
        {
            ...
            for (unsigned i = 0; i < newNodes.size(); ++i, ++node) {
                TNodeNodeMap::iterator n2nIt =
                    n2n->insert(make_pair(*node, (SMDS_MeshNode *)0)).first;
                if (n2nIt->second) {
                    if (!subShapeIDs.count(n2nIt->second->getshapeId())) break;
                }
                else {
                    ...
                    for (vNIt = vertexNodes.begin(); vNIt != vertexNodes.end(); ++vNIt)
                        if (vNIt->SquareDistance(*node) < checktol) {
                            ...
                            MESSAGE("SquareDistance missed"
                                    << vNIt->SquareDistance(*node) << " checktol " << checktol
                                    << " " << vNIt->X() << " " << vNIt->Y() << " " << vNIt->Z());
                }
                if (!n2nIt->second) {
                    // find out if node lies on theShape
                    //double dxyz[4];
                    tmpNode->setXYZ((*node)->X(), (*node)->Y(), (*node)->Z());
                    if (helper.CheckNodeU(
                            geomEdge, tmpNode, u, mytol,
                            /*force=*/true)) // , dxyz )) // dxyz used for debug purposes
                    {
                        SMDS_MeshNode *newNode =
                            tgtMesh->AddNode((*node)->X(), (*node)->Y(), (*node)->Z());
                ...
                if (!(newNodes[i] = n2nIt->second)) break;
            }
            if (!newNodes.back()) {
    ...
    if (n2n->empty()) return error("Empty source groups");
    ...
    if (!isEdgeMeshed) return error("Source elements don't cover totally the geometrical edge");
...
void StdMeshers_Import_1D::importMesh(const SMESH_Mesh *srcMesh, SMESH_Mesh &tgtMesh,
                                      ...
                                      const TopoDS_Shape &tgtShape)
{
    ...
    while (eIt->more()) {
        ...
        if (toCopyGroups) (*e2eIt).second = newElem;
    }
    // copy free nodes
        ...
        if (srcMesh) {
            ...
            while (groupIt->more()) {
                ...
                if (srcGroupDS->GetType() == SMDSAbs_Node)
                    while (eIt->more()) {
                        TNodeNodeMap::iterator n2nIt =
                            n2n->find((const SMDS_MeshNode *)eIt->next());
                                            ...
                                            StdMeshers_ImportSource1D *sourceHyp)
{
    if (sourceHyp) {
        ...
        if (srcMeshes.empty()) _Listener::waitHypModification(subMesh);
        for (unsigned i = 0; i < srcMeshes.size(); ++i)
...
void StdMeshers_Import_1D::SubmeshRestored(SMESH_subMesh *subMesh) { SetEventListener(subMesh); }
...
bool StdMeshers_Import_1D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                    MapShapeNbElems &aResMap)
{
    ...
    if (toCopyMesh) // the whole mesh is copied
    {
        ...
        TopExp_Explorer vExp(theShape, TopAbs_VERTEX);
        for (; vExp.More(); vExp.Next()) theMesh.GetSubMesh(vExp.Current())->Evaluate(aResMap);
...
void StdMeshers_Import_1D::getMaps(const SMESH_Mesh *srcMesh, SMESH_Mesh *tgtMesh,
                                   TNodeNodeMap *&n2n, TElemElemMap *&e2e)
{
    _ImportData *iData = _Listener::getImportData(srcMesh, tgtMesh);
    n2n = &iData->_n2n;
    e2e = &iData->_e2e;

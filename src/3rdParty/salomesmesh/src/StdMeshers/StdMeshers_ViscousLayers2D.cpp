...
struct _ProxyMeshHolder: public SMESH_subMeshEventListener {
    ...
    void ProcessEvent(const int event, const int eventType, SMESH_subMesh *subMesh,
                      EventListenerData *data, const SMESH_Hypothesis * /*hyp*/)
    {
...
struct _Segment {
    ...
    _Segment() {}
    _Segment(const gp_XY &p1, const gp_XY &p2) : _indexInLine(-1)
    {
        _uv[0] = &p1;
        _uv[1] = &p2;
    }
...
struct _PolyLine {
    ...
    size_t FirstLEdge() const { return (_leftLine->_advancable && _lEdges.size() > 2) ? 1 : 0; }
    bool IsAdjacent(const _Segment &seg, const _LayerEdge *LE = 0) const
    {
        if (LE /*&& seg._indexInLine < _lEdges.size()*/)
            return (seg._uv[0] == &LE->_uvIn || seg._uv[1] == &LE->_uvIn);
        return (&seg == &_leftLine->_segments.back() || &seg == &_rightLine->_segments[0]);
    }
...
struct _SegmentIntersection {
    ...
    bool Compute(const _Segment &seg1, const _Segment &seg2, bool seg2IsRay = false)
    {
        ...
        if (fabs(_D) < std::numeric_limits<double>::min()) return false;
        _param1 = _vec2.Crossed(_vec21) / _D;
        if (_param1 < -eps || _param1 > 1 + eps) return false;
...
{
public:
    _ViscousBuilder2D(SMESH_Mesh &theMesh, const TopoDS_Face &theFace,
                      vector<const THypVL *> &theHyp, vector<TopoDS_Shape> &theHypShapes);
...
private:
    ...
    bool shrink();
    bool improve();
    bool toShrinkForAdjacent(const TopoDS_Face &adjFace, const TopoDS_Edge &E,
                             const TopoDS_Vertex &V);
    void setLenRatio(_LayerEdge &LE, const gp_Pnt &pOut);
    void setLayerEdgeData(_LayerEdge &lEdge, const double u, Handle(Geom2d_Curve) & pcurve,
                          Handle(Geom_Curve) & curve, const gp_Pnt pOut, const bool reverse,
...
/*!
   ...
   */
bool findHyps(SMESH_Mesh &theMesh, const TopoDS_Face &theFace,
              vector<const StdMeshers_ViscousLayers2D *> &theHyps,
              vector<TopoDS_Shape> &theAssignedTo)
{
    theHyps.clear();
    theAssignedTo.clear();
    SMESH_HypoFilter hypFilter(SMESH_HypoFilter::HasName(StdMeshers_ViscousLayers2D::GetHypType()));
    list<const SMESHDS_Hypothesis *> hypList;
    list<TopoDS_Shape> hypShapes;
    int nbHyps = theMesh.GetHypotheses(theFace, hypFilter, hypList, /*ancestors=*/true, &hypShapes);
    if (nbHyps) {
...
int getEdgesToIgnore(const StdMeshers_ViscousLayers2D *theHyp, const TopoDS_Shape &theFace,
                     const SMESHDS_Mesh *theMesh, set<int> &theEdgeIds)
{
    ...
    else // EDGEs to make the Viscous Layers on are given
    {
        ...
        for (size_t i = 0; i < ids.size(); ++i) nbEdgesToIgnore -= theEdgeIds.erase(ids[i]);
    }
                                                         ...
                                                         const TopoDS_Face &theFace)
{
    ...
    if (VISCOUS_2D::findHyps(theMesh, theFace, hyps, hypShapes)) {
        ...
        if (error && !error->IsOK()) theMesh.GetSubMesh(theFace)->GetComputeError() = error;
        else if (!pm)
            pm.reset(new SMESH_ProxyMesh(theMesh));
        if (getenv("__ONLY__VL2D__")) pm.reset();
    }
    else {
        pm.reset(new SMESH_ProxyMesh(theMesh));
    }
...
StdMeshers_ViscousLayers2D::CheckHypothesis(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                            SMESH_Hypothesis::Hypothesis_Status &theStatus)
{
    ...
    for (; exp.More() && theStatus == SMESH_Hypothesis::HYP_OK; exp.Next()) {
        ...
        if (VISCOUS_2D::findHyps(theMesh, face, hyps, hypShapes)) {
            ...
            builder._faceSideVec = StdMeshers_FaceSide::GetFaceWires(face, theMesh, true, error,
                                                                     SMESH_ProxyMesh::Ptr(),
                                                                     /*theCheckVertexNodes=*/false);
            if (error->IsOK() && !builder.findEdgesWithLayers()) {
                error = builder.GetError();
                if (error && !error->IsOK()) theStatus = SMESH_Hypothesis::HYP_INCOMPAT_HYPS;
            }
        }
    }
    return error;
...
void StdMeshers_ViscousLayers2D::RestoreListeners() const
{
    ...
    for (; i_smesh != sc->mapMesh.end(); ++i_smesh) {
        ...
        if (!smesh || !smesh->HasShapeToMesh() || !smesh->GetMeshDS()
            ...
            if (SMESH_Algo *algo = _gen->GetAlgo(*smesh, face.Current())) {
                ...
                if (std::find(usedHyps.begin(), usedHyps.end(), this) != usedHyps.end())
                    for (TopExp_Explorer edge(face.Current(), TopAbs_EDGE); edge.More();
                         edge.Next())
                        VISCOUS_3D::ToClearSubWithMain(smesh->GetSubMesh(edge.Current()),
                                                       face.Current());
            }
    }
}
...
_ViscousBuilder2D::_ViscousBuilder2D(SMESH_Mesh &theMesh, const TopoDS_Face &theFace,
...
SMESH_ProxyMesh::Ptr _ViscousBuilder2D::Compute()
{
    ...
    if (!_error->IsOK()) return _proxyMesh;
...
bool _ViscousBuilder2D::findEdgesWithLayers()
{
    ...
    for (int iE = 1; iE <= facesOfEdgeMap.Extent(); ++iE)
        if (facesOfEdgeMap(iE).Extent() > 1) sharedEdges.Add(facesOfEdgeMap.FindKey(iE));
    ...
    if (_hyps.size() > 1) {
        ...
        for (size_t iWire = 0; iWire < _faceSideVec.size(); ++iWire) {
                        ...
                        if (!ignoreEdgesOfHyp[i].first.count(edgeID)) {
                            if (hyp)
                                return error(SMESH_Comment("Several hypotheses define "
                                                           "Viscous Layers on the edge #")
                                             << edgeID);
                            hyp = ignoreEdgesOfHyp[i].second;
                        }
                }
                _hypOfEdge.push_back(hyp);
                if (!hyp) _ignoreShapeIds.insert(edgeID);
            }
            ...
            for (; iH < _hypOfEdge.size(); ++iH) {
                hyp = _hypOfEdge[iH];
                if (hyp && prevHyp && hyp->GetNumberLayers() != prevHyp->GetNumberLayers()) {
            ...
            if (sharedEdges.Contains(wire->Edge(iE))) {
                ...
                for (; faceIt.More(); faceIt.Next()) {
                    ...
                    for (int iV = 0; iV < 2; ++iV) {
                        ...
                        if (!viscHyp) _noShrinkVert.insert(getMeshDS()->ShapeToIndex(vertex));
                        else {
                            PShapeIteratorPtr edgeIt =
                                _helper.GetAncestors(vertex, *_mesh, TopAbs_EDGE);
                            while (const TopoDS_Shape *edge = edgeIt->next())
                                if (!edge->IsSame(wire->Edge(iE))
                                    ...
                                    if (hasVL) {
                                        hasVL = false;
                                        for (hyp = allHyps.begin(); hyp != allHyps.end() && !hasVL;
                                             ++hyp)
...
bool _ViscousBuilder2D::makePolyLines()
{
    ...
    TopLoc_Location loc;
    {
        ...
        if (Abs(r2 - r1) / maxR > 0.2 * maxR) faceProj = &_helper.GetProjector(_face, loc);
    }
    ...
    for (size_t iWire = 0; iWire < _faceSideVec.size(); ++iWire) {
        ...
        for (int iE = 0; iE < wire->NbEdges(); ++iE) {
            ...
            while (points[iPnt].normParam < lastNormPar) ++iPnt;
            L._lastPntInd = iPnt;
            ...
            if (L._lastPntInd - L._firstPntInd + 1 < 3) // add 3-d _LayerEdge in the middle
            {
                ...
                if (!curve.IsNull()) p = curve->Value(u);
                else
                    p = 0.5
                        * (SMESH_TNodeXYZ(points[L._firstPntInd].node)
    ...
    if (_maxThickness * maxLen2dTo3dRatio > sqrt(faceBndBox2D.SquareExtent()) / 10) {
                ...
                for (size_t iLE = 1; iLE < L1._lEdges.size(); ++iLE) {
                    ...
                    for (size_t i = 0; i < foundSegs.size(); ++i)
                        if (intersection.Compute(*foundSegs[i], L1._lEdges[iLE]._ray)) {
                            double distToL2 =
                                intersection._param2 / L1._lEdges[iLE]._len2dTo3dRatio;
        ...
        if (maxPossibleThick > 0.) _maxThickness = Min(_maxThickness, maxPossibleThick);
    }
    ...
    for (iPoLine = 0; iPoLine < _polyLineVec.size(); ++iPoLine) {
        _PolyLine &L = _polyLineVec[iPoLine];
        if (L._leftLine->_advancable) L._segments[0]._uv[0] = &L._leftLine->_lEdges.back()._uvIn;
    }
    ...
    for (iPoLine = 0; iPoLine < _polyLineVec.size(); ++iPoLine) {
        ...
        for (size_t iL2 = 0; iL2 < _polyLineVec.size(); ++iL2) {
            _PolyLine &L2 = _polyLineVec[iL2];
            if (iPoLine == iL2 || lineBoxes[iPoLine].IsOut(lineBoxes[iL2])) continue;
            if (!L1._advancable && (L1._leftLine == &L2 || L1._rightLine == &L2)) continue;
...
void _ViscousBuilder2D::adjustCommonEdge(_PolyLine &LL, _PolyLine &LR)
{
    ...
    gp_XY normCommon = (normL * int(LL._advancable) + normR * int(LR._advancable)).Normalized();
    ...
    if (largeAngle) // not 180 degrees
    {
        ...
        if (dotNormTang < 0.) // ---------------------------- CONVEX ANGLE
        {
            ...
            for (size_t iL1 = 0; iL1 < _polyLineVec.size(); ++iL1) {
                ...
                if (boxL1->IsOut(pCommOut, pCommIn)) continue;
                for (size_t iLE = 1; iLE < L1._lEdges.size(); ++iLE) {
                    foundSegs.clear();
                    ...
                    for (size_t i = 0; i < foundSegs.size(); ++i)
                        if (intersection.Compute(*foundSegs[i], segCommon)
                            ...
                            if (len2D < maxLen2D) {
                                maxLen2D = len2D;
                                pCommIn = pCommOut
                                    + normAvg * maxLen2D; // here length of segCommon changes
                            }
                        }
                }
            }
            ...
            for (int isR = 0; isR < 2; ++isR) // loop on [ LL, LR ]
            {
                ...
                for (; iLE < L._lEdges.size(); ++iLE, eIt += dIt) {
                    gp_XY uvIn =
                        eIt->_uvOut + eIt->_normal2D * _maxThickness * eIt->_len2dTo3dRatio;
                    _Segment segOfEdge(eIt->_uvOut, uvIn);
                    if (!intersection.Compute(segCommon, segOfEdge)) break;
                ...
                if (iLE >= L._lEdges.size() - 1) {
                    ...
                    if (isR) LR._lEdges.erase(LR._lEdges.begin() + 1, eIt);
                    else
...
void _ViscousBuilder2D::setLayerEdgeData(_LayerEdge &lEdge, const double u,
                                         Handle(Geom2d_Curve) & pcurve, Handle(Geom_Curve) & curve,
                                         const gp_Pnt pOut, const bool reverse,
                                         GeomAPI_ProjectPointOnSurf *faceProj)
{
    gp_Pnt2d uv;
    if (faceProj && !curve.IsNull()) {
        ...
        if (reverse) tangent.Reverse();
        _surface->D1(uv.X(), uv.Y(), p, du, dv);
        gp_Vec faceNorm = du ^ dv;
        gp_Vec normal = faceNorm ^ tangent;
        normal.Normalize();
        p = pOut.XYZ()
            + normal.XYZ() * /*1e-2 * */ _hyps[0]->GetTotalThickness()
                / _hyps[0]->GetNumberLayers();
        faceProj->Perform(p);
...
bool _ViscousBuilder2D::inflate()
{
            ...
            for (size_t iLE = 1; iLE < L1._lEdges.size(); ++iLE) {
                ...
                for (size_t i = 0; i < foundSegs.size(); ++i)
                    if (!L1.IsAdjacent(*foundSegs[i], &L1._lEdges[iLE])
                        ...
                        if (1e-10 < size && size < minSize) minSize = size;
    ...
    int nbSteps = 0;
    if (maxSize > _maxThickness) maxSize = _maxThickness;
    while (curThick < maxSize) {
        curThick += stepSize * 1.25;
        if (curThick > _maxThickness) curThick = _maxThickness;
        ...
        for (size_t iL = 0; iL < _polyLineVec.size(); ++iL) {
            ...
            //        << endl;
            if (lenChange) L._segTree.reset(new _SegmentTree(L._segments));
        }
    ...
    for (size_t iL = 0; iL < _polyLineVec.size(); ++iL) {
        ...
        for (int isR = 0; (isR < 2 && L._lEdges.size() > 2); ++isR) {
            ...
            for (eIt += deltaIt; nbRemove < L._lEdges.size() - 1; eIt += deltaIt) {
                ...
                if (!intersection.Compute(seg1, seg2)) break;
                ++nbRemove;
            }
            if (nbRemove > 0) {
                if (nbRemove == L._lEdges.size() - 1) // 1st and last _LayerEdge's intersect
                {
                    ...
                    if (L.IsCommonEdgeShared(*L._leftLine)) L._leftLine->_lEdges.back() = L0;
                }
                if (isR)
                    L._lEdges.erase(L._lEdges.end() - nbRemove - 1, L._lEdges.end() - nbRemove);
                else
                    L._lEdges.erase(L._lEdges.begin() + 1, L._lEdges.begin() + 1 + nbRemove);
            }
        }
    }
    return true;
...
bool _ViscousBuilder2D::fixCollisions()
{
                        ...
                        if (newLen2D < 1.1 * LE1._length2D) // collision!
                        {
                            if (newLen2D > 0 || !L1._advancable) {
                                ...
                                if (L1._advancable && newLen2D > 0) {
                                    ...
                                    blockedEdgesList.push_back(
                                        &L2._lEdges[foundSegs[i]->_indexInLine]);
                                    blockedEdgesList.push_back(
    ...
    for (; edge != blockedEdgesList.end(); ++edge) (*edge)->_isBlocked = true;
    ...
    for (size_t iL = 0; iL < _polyLineVec.size(); ++iL) {
        ...
        if (!L._advancable) continue;
        for (size_t iLE = L.FirstLEdge(); iLE < L._lEdges.size(); ++iLE)
            if (!L._lEdges[iLE]._isBlocked) return false;
    }
...
bool _ViscousBuilder2D::shrink()
{
    ...
    for (size_t iL1 = 0; iL1 < _polyLineVec.size(); ++iL1) {
        ...
        if (!isShrinkableL && !isShrinkableR) continue;
        ...
        while (const TopoDS_Shape *f = faceIt->next())
            if (!_face.IsSame(*f)) {
                ...
                if (!pm || pm->NbProxySubMeshes() == 0 /*|| !pm->GetProxySubMesh( E )*/) {
                    ...
                    _ProxyMeshOfFace::_EdgeSubMesh *myEdgeSM =
                        getProxyMesh()->GetEdgeSubMesh(edgeID);
        ...
        if (existingNodesFound) continue; // nothing more to do in this case
        ...
        const vector<UVPtStruct> &points = L._wire->GetUVPtStruct();
        UVPtStructVec nodeDataVec(&points[L._firstPntInd], &points[L._lastPntInd + 1]);
        ...
        for (int isR = 0; isR < 2; ++isR) {
            ...
            if (!L2->_advancable
                ...
                continue;
            if (isR ? !isShrinkableR : !isShrinkableL) continue;
            ...
            if (L2->_advancable) {
                ...
                const uvPtStruct &tang2P2 =
                    points[isR ? L2->_firstPntInd + 1 : L2->_lastPntInd - 1];
                gp_XY seg2Dir(tang2P2.u - tang2P1.u, tang2P2.v - tang2P1.v);
           ...
           *                                          x------x---      */
            ...
            else // L2 is advancable but in the face adjacent by L
            {
                ...
                if (length2D == 0) {
                    ...
                    if (length2D == 0) length2D = _maxThickness * nearLE._len2dTo3dRatio;
                }
            }
            ...
            if (Abs(length2D) > maxLen2D) length2D = maxLen2D;
            ...
            for (size_t i = 0; i < params.size(); ++i) params[i] = u0 + heights[i];
        ...
        for (size_t iP = 1; iP < nodeDataVec.size() - 1; ++iP) {
            ...
            if (!discret.IsDone()) throw SALOME_Exception(LOCALIZED("GCPnts_AbscissaPoint failed"));

            nodeDataVec[iP].param = discret.Parameter();
            if (oldNode->GetPosition()->GetTypeOfPosition() != SMDS_TOP_EDGE)
                throw SALOME_Exception(
                    SMESH_Comment("ViscousBuilder2D: not SMDS_TOP_EDGE node position: ")
                    << oldNode->GetPosition()->GetTypeOfPosition() << " of node "
                    << oldNode->GetID());
     ...
     *     /    */
        for (int isR = 0; isR < 2; ++isR) {
            _PolyLine &L2 = *(isR ? L._rightLine : L._leftLine); // line with layers
            if (!L2._advancable || L.IsCommonEdgeShared(L2)) continue;
            ...
            newLength = GCPnts_AbscissaPoint::Length(curve, nodeDataVec.front().param,
                                                     nodeDataVec.back().param);
            for (size_t iP = 1; iP < nodeDataVec.size(); ++iP) {
                const double len = GCPnts_AbscissaPoint::Length(curve, nodeDataVec.front().param,
...
bool _ViscousBuilder2D::toShrinkForAdjacent(const TopoDS_Face &adjFace, const TopoDS_Edge &E,
                                            const TopoDS_Vertex &V)
{
    if (_noShrinkVert.count(getMeshDS()->ShapeToIndex(V)) || adjFace.IsNull()) return false;
    ...
    if (VISCOUS_2D::findHyps(*_mesh, adjFace, hyps, hypShapes)) {
        ...
        while (const TopoDS_Shape *edgeAtV = edgeIt->next()) {
            if (!edgeAtV->IsSame(E) && _helper.IsSubShape(*edgeAtV, adjFace)
...
bool _ViscousBuilder2D::refine()
{
    ...
    bool isReverse = (_helper.GetSubShapeOri(_mesh->GetShapeToMesh(), _face) == TopAbs_REVERSED);
    ...
    for (size_t iL = 0; iL < _polyLineVec.size(); ++iL) {
        ...
        if (lenLimited) {
            ...
            for (size_t iS = 1; iS < segLen.size(); ++iS) segLen[iS] /= totSegLen;
        ...
        const size_t nbN = L._lastPntInd - L._firstPntInd + 1,
        ...
        if (hasOwnLeftNode) nL = L._leftNodes.back();
            ...
            nL = L._leftLine->_rightNodes.back();
        if (hasOwnRightNode) nR = L._rightNodes.back();
        else if (hasRightNode)
            nR = L._rightLine->_leftNodes.back();
        if (nL) L._lEdges[0]._uvIn = _helper.GetNodeUV(_face, nL, points[L._firstPntInd + 1].node);
        if (nR)
        ...
        const double normF = L._wire->FirstParameter(L._edgeInd),
                     normL = L._wire->LastParameter(L._edgeInd), normDist = normL - normF;
        for (int i = L._firstPntInd; i <= L._lastPntInd; ++i)
        ...
        {
            for (size_t i = 0; i < nbLE; ++i) innerUV[i] = L._lEdges[i]._uvIn;
        }
        ...
        for (int iF = 0; iF < hyp->GetNumberLayers(); ++iF) // loop on layers of faces
        {
            ...
            if (hasOwnLeftNode) innerNodes.front() = L._leftNodes[iF];
                ...
                innerNodes.front() = L._leftLine->_rightNodes[iF];
            if (hasOwnRightNode) innerNodes.back() = L._rightNodes[iF];
            else if (hasRightNode)
                innerNodes.back() = L._rightLine->_leftNodes[iF];
            ...
            for (size_t i = 1; i < innerNodes.size(); ++i)
                if (SMDS_MeshElement *f =
                        _helper.AddFace(outerNodes[i + prev], outerNodes[i + cur],
        ...
        for (int isR = 0; isR < 2; ++isR) {
            if (isR ? rightEdgeShared : leftEdgeShared) continue;
            vector<const SMDS_MeshNode *> &lNodes =
                                              (isR ? L._rightNodes : L._leftLine->_rightNodes),
                                          rNodes = (isR ? L._rightLine->_leftNodes : L._leftNodes);
            if (lNodes.empty() || rNodes.empty() || lNodes.size() != rNodes.size()) continue;

            for (size_t i = 1; i < lNodes.size(); ++i)
                _helper.AddFace(lNodes[i + prev], rNodes[i + prev], rNodes[i + cur],
                                lNodes[i + cur]);

            const UVPtStruct &ptOnVertex = points[isR ? L._lastPntInd : L._firstPntInd];
            if (isReverse) _helper.AddFace(ptOnVertex.node, lNodes[0], rNodes[0]);
            else
...
{
    if (_proxyMesh.get()) return (_ProxyMeshOfFace *)_proxyMesh.get();
...
void _ViscousBuilder2D::calcLayersHeight(const double totalThick, vector<double> &heights,
                                         const THypVL *hyp)
{
    const double fPowN = pow(hyp->GetStretchFactor(), hyp->GetNumberLayers());
    heights.resize(hyp->GetNumberLayers());
    double h0;
    if (fPowN - 1 <= numeric_limits<double>::min()) h0 = totalThick / hyp->GetNumberLayers();
    else
...
{
    if (_lEdges.size() < 2) return false;
...
_SegmentTree::_SegmentTree(const vector<_Segment> &segments) : SMESH_Quadtree()
{
    _segments.resize(segments.size());
    for (size_t i = 0; i < segments.size(); ++i) _segments[i].Set(segments[i]);
...
void _SegmentTree::GetSegmentsNear(const _Segment &seg, vector<const _Segment *> &found)
{
    if (getBox()->IsOut(*seg._uv[0], *seg._uv[1])) return;

    if (isLeaf()) {
        for (int i = 0; i < _segments.size(); ++i)
            if (!_segments[i].IsOut(seg)) found.push_back(_segments[i]._seg);
    }
    else {
...
void _SegmentTree::GetSegmentsNear(const gp_Ax2d &ray, vector<const _Segment *> &found)
{
    if (getBox()->IsOut(ray)) return;

    if (isLeaf()) {
        for (int i = 0; i < _segments.size(); ++i)
            if (!_segments[i].IsOut(ray)) found.push_back(_segments[i]._seg);
    }
    else {
        for (int i = 0; i < nbChildren(); i++)

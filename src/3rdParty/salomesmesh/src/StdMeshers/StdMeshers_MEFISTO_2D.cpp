...
bool StdMeshers_MEFISTO_2D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                            SMESH_Hypothesis::Hypothesis_Status &aStatus)
{
    ...
    if (!error(StdMeshers_ViscousLayers2D::CheckHypothesis(aMesh, aShape, aStatus))) return false;
    ...
    if (isOk) {
        ...
        if (!isOk) aStatus = SMESH_Hypothesis::HYP_BAD_PARAMETER;
    }
...
bool StdMeshers_MEFISTO_2D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (!_hypMaxElementArea) {
        ...
        if (nbSegments) _edgeLength /= nbSegments;
    }

    if (/*_hypLengthFromEdges &&*/ _edgeLength < DBL_MIN) _edgeLength = 100;
    ...
    if (LoadPoints(wires, uvslf, mefistoToDS, scalex, scaley)) {
        // Compute
        aptrte(nutysu, aretmx, nblf, nudslf, uvslf, nbpti, uvpti, nbst, uvst, nbt, nust, ierr);
...
bool StdMeshers_MEFISTO_2D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                     MapShapeNbElems &aResMap)
{
    ...
    int nbFaces = Precision::IsInfinite(tmpLength)
        ? 0
...
static bool fixOverlappedLinkUV(R2 &uv0, const R2 &uv1, const R2 &uv2)
{
    ...
    if (dot > 0 && 1 - dot * dot / (sqMod1 * sqMod2) < minSin * minSin) {
        ...
        if (v1.X() < 0) uv0.x -= delta;
        else
            uv0.x += delta;
        if (v1.Y() < 0) uv0.y -= delta;
        else
            uv0.y += delta;
        // #ifdef _DEBUG_
        //     MESSAGE(" -> " << uv0.x << " " << uv0.y << " ");
...
static bool fixCommonVertexUV(R2 &theUV, const TopoDS_Vertex &theV, const TopoDS_Face &theF,
                              const TopTools_IndexedDataMapOfShapeListOfShape &theVWMap,
                              SMESH_Mesh &theMesh, const double theScaleX, const double theScaleY,
                              const bool theCreateQuadratic)
{
    ...
    for (aWIt.Initialize(WList); aWIt.More(); aWIt.Next()) aWires.Add(aWIt.Value());
    ...
    for (aWIt.Initialize(WList); aWIt.More() && anInnerWire.IsNull(); aWIt.Next())
        if (!anOuterWire.IsSame(aWIt.Value())) anInnerWire = aWIt.Value();
    ...
    for (; itE.More(); itE.Next()) {
        ...
        for (; itV.More(); itV.Next()) {
            ...
            if (E.Orientation() == TopAbs_REVERSED) n.Reverse();
            N += n.Normalized();
        }
    }
    ...
    if (fixOverlappedLinkUV(uv0, uv1, uv2)) {
        ...
        MESSAGE("--fixCommonVertexUV move(" << theUV.x << " " << theUV.x << ") by (" << N.X() << " "
                                            << N.Y() << ")" << endl
                                            << "--- MAX DIST " << maxDist);
...
bool StdMeshers_MEFISTO_2D::LoadPoints(TWireVector &wires, R2 *uvslf,
                                       vector<const SMDS_MeshNode *> &mefistoToDS, double scalex,
                                       double scaley)
{
    ...
    if (wires.size() > 1) {
        ...
        int nbVertices = 0;
        for (int iW = 0; iW < wires.size(); ++iW) nbVertices += wires[iW]->NbEdges();
        if (nbVertices == VWMap.Extent()) VWMap.Clear(); // wires have no common vertices
    }
    ...
    for (int iW = 0; iW < wires.size(); ++iW) {
        ...
        if (m + uvPtVec.size() - 1 > mefistoToDS.size()) {
            MESSAGE("Wrong mefistoToDS.size: " << mefistoToDS.size() << " < "
                                               << m + uvPtVec.size() - 1);
        ...
        for (++uvPt; uvPt != uvPtVec.end(); ++uvPt) {
            ...
            switch (uvPt->node->GetPosition()->GetTypeOfPosition()) {
                case SMDS_TOP_VERTEX: mOnVertex.push_back(m); break;
                case SMDS_TOP_EDGE:
        ...
        for (; mIt != mOnVertex.end(); ++mIt) {
            ...
            if (iW && !VWMap.IsEmpty()) { // except outer wire
                ...
                if (fixCommonVertexUV(uvslf[m], V, F, VWMap, *_helper->GetMesh(), scalex, scaley,
                                      _quadraticMesh)) {
    ...
    for (int i = 0; i < m; ++i)
        cout << i << ": \t" << uvslf[i].x << ", " << uvslf[i].y << " Node "
             << mefistoToDS[i]->GetID() << endl;
#endif
...
void StdMeshers_MEFISTO_2D::ComputeScaleOnFace(SMESH_Mesh &aMesh, const TopoDS_Face &aFace,
                                               double &scalex, double &scaley)
{
    ...
    if (xyratio > maxratio) { scaley *= xyratio / maxratio; }
                                        ...
                                        double scaley)
{
    ...
    int i1 = 1, i2 = 2;
    if (F.Orientation() != TopAbs_FORWARD) std::swap(i1, i2);
    ...
    for (n = 1; n <= nbt; n++) {
        ...
        if (!isDegen) _helper->AddFace(nn[0], nn[i1], nn[i2]);
    }

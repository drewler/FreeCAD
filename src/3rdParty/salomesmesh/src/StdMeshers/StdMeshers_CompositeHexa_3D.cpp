...
public: //** Methods to find and orient faces of 6 sides of the box **//
    ...
    typedef SMDS_SetIterator<const _QuadFaceGrid &, TChildren::const_iterator,
...
private:
    ...
    {
        myError = SMESH_ComputeError::New(code, text);
        return false;
    }
    ...
    {
        myError = err;
        return (!myError || myError->IsOK());
    }

    bool loadCompositeGrid(SMESH_Mesh &mesh);

    bool fillGrid(SMESH_Mesh &theMesh, vector<const SMDS_MeshNode *> &theGrid,
                  const _Indexer &theIndexer, int theX, int theY);
...
bool StdMeshers_CompositeHexa_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
bool isContinuousMesh(TopoDS_Edge E1, TopoDS_Edge E2, const TopoDS_Face &F, const SMESH_Mesh &mesh)
{
...
void getBlockCorners(SMESH_Mesh &mesh, const TopoDS_Shape &shape, TopTools_MapOfShape &cornerVV)
{
...
bool getInternalEdges(SMESH_Mesh &mesh, const TopoDS_Shape &shape,
                      const TopTools_MapOfShape &cornerVV, TopTools_MapOfShape &internEE)
{
    ...
    for (; vIt.More(); vIt.Next()) {
        ...
        while (const TopoDS_Shape *riE = riIt->next()) {
            if (!subEE.Contains(*riE) || !subChecked.Add(*riE)) continue;
            TopoDS_Edge ridgeE = TopoDS::Edge(*riE);
            while (!ridgeE.IsNull()) {
                TopExp::Vertices(ridgeE, VV[0], VV[1]);
                TopoDS_Shape V1 = VV[V0.IsSame(VV[0])];
                if (cornerVV.Contains(V1)) break; // ridgeE reached a corner VERTEX
...
bool StdMeshers_CompositeHexa_3D::findBoxFaces(const TopoDS_Shape &shape,
                                               list<_QuadFaceGrid> &boxFaces, SMESH_Mesh &mesh,
                                               _QuadFaceGrid *&fBottom, _QuadFaceGrid *&fTop,
                                               _QuadFaceGrid *&fFront, _QuadFaceGrid *&fBack,
                                               _QuadFaceGrid *&fLeft, _QuadFaceGrid *&fRight)
{
    ...
    for (exp.Init(shape, TopAbs_FACE); exp.More(); exp.Next(), ++nbFaces) {
        _QuadFaceGrid f;
        if (!f.Init(TopoDS::Face(exp.Current()), mesh)) return error(COMPERR_BAD_SHAPE);
        ...
        if (!prevContinuous) boxFaces.push_back(f);
    }
    // Check what we have
    if (boxFaces.size() != 6 && nbFaces != 6)
        return error(COMPERR_BAD_SHAPE,
                     SMESH_Comment("Can't find 6 sides of a box. Number of found sides - ")
                         << boxFaces.size());
    ...
    // check the found
    if (!fFront || !fBack || !fLeft || !fRight) return error(COMPERR_BAD_SHAPE);
    // find a top face
    fTop = 0;
    for (boxFace = ++boxFaces.begin(); boxFace != boxFaces.end() && !fTop; ++boxFace) {
        fTop = &(*boxFace);
        fTop->SetID(B_TOP);
        if (fTop == fFront || fTop == fLeft || fTop == fBack || fTop == fRight) fTop = 0;
    }
    ...
    if (!fTop->SetBottomSide(fFront->GetSide(Q_TOP))) {
        if (!fFront->IsComplex())
            ...
            while (chIt.more()) {
                ...
                if (fTop->SetBottomSide(frontChild.GetSide(Q_TOP))) break;
            }
        }
    }
    if (!fTop) return error(COMPERR_BAD_SHAPE);
...
bool StdMeshers_CompositeHexa_3D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (!findBoxFaces(theShape, boxFaceContainer, theMesh, fBottom, fTop, fFront, fBack, fLeft,
                      fRight))
        return false;
    ...
    for (x = 0; x < xSize - 1; ++x) {
        for (y = 0; y < ySize - 1; ++y) {
            ...
            for (z = 0; z < zSize - 1; ++z) {
                ...
                helper.AddVolume(col00[z], col01[z], col11[z], col10[z], col00[z + 1], col01[z + 1],
                                 col11[z + 1], col10[z + 1]);
            }
        }
    }
...
bool StdMeshers_CompositeHexa_3D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                           MapShapeNbElems &aResMap)
{
    ...
    for (++face; face != boxFaceContainer.end() && lessComplexSide->IsComplex(); ++face)
        if (face->NbChildren() < lessComplexSide->NbChildren()) lessComplexSide = &*face;

    // Get an 1D size of lessComplexSide
    int nbSeg1 = 0;
    vector<TopoDS_Edge> edges;
    if (!lessComplexSide->GetHoriEdges(edges)) return false;
...
bool _QuadFaceGrid::Init(const TopoDS_Face &f, SMESH_Mesh &mesh)
{
    ...
    {
        for (; edgeIt != edges.end(); ++edgeIt) mySides.AppendSide(_FaceSide(*edgeIt));
    }
        ...
        while (!edges.empty()) {
            ...
            sideEdges.splice(sideEdges.end(), edges,
                             edges.begin()); // edges.front()->sideEdges.back()
            while (!edges.empty()) {
    ...
    if (mySides.size() != 4) return false;
            ...
            if (internalEdges.Contains(otherSide.Edge(0))) {
                ...
                if (other.IsComplex())
                    for (TChildIterator children = other.GetChildren(); children.more();) {
                        ...
                        myChildren.back().SetBottomSide(
                            myChildren.back().GetSide(otherBottomIndex));
                    }
                else {
                ...
                if (other.IsComplex())
                    for (TChildIterator children = other.GetChildren(); children.more();) {
                        ...
                        for (int i = 0; i < 4; ++i) mySides.AppendSide(child.GetSide(i));
                    }
                else
                    for (int i = 0; i < 4; ++i) mySides.AppendSide(other.GetSide(i));
...
bool _QuadFaceGrid::SetBottomSide(const _FaceSide &bottom, int *sideIndex)
{
    ...
    if (myChildren.empty()) {
        if (mySides.Contain(bottom, &myBottomIndex)) {
            ...
            if (sideIndex) *sideIndex = myBottomIndex;
            return true;
        }
    }
    else {
...
_QuadFaceGrid *_QuadFaceGrid::FindAdjacentForSide(int i, list<_QuadFaceGrid> &faces,
                                                  EBoxSides id) const
{
    ...
    for (; boxFace != faces.end(); ++boxFace) {
        ...
        if (f != this && f->SetBottomSide(iSide)) return f->SetID(id), f;
    }
...
{
    if (myChildren.empty()) return *mySides.GetSide(i);

    _QuadFaceGrid *me = const_cast<_QuadFaceGrid *>(this);
    if (!me->locateChildren() || !myLeftBottomChild) return *mySides.GetSide(i);
    ...
    switch (i) {
        ...
        case Q_LEFT: break;
        case Q_RIGHT:
            while (child->myRightBrother) child = child->myRightBrother;
            break;
        case Q_TOP:
            while (child->myUpBrother) child = child->myUpBrother;
            break;
        default:;
    }
...
void _QuadFaceGrid::ReverseEdges(/*int e1, int e2*/)
{
    ...
    if (myChildren.empty()) {
        ...
        for (; child != childEnd; ++child) child->ReverseEdges(/*e1, e2*/);
    }
}
...
bool _QuadFaceGrid::LoadGrid(SMESH_Mesh &mesh)
{
    if (!myChildren.empty()) {
        ...
        for (; child != childEnd; ++child) {
            child->SetID(myID);
            if (!child->LoadGrid(mesh)) return error(child->GetError());
        }
    ...
    while (fIt->more())
        if (fIt->next()->NbNodes() % 4 > 0)
            return error(
                "Non-quadrangular mesh faces are not allowed on sides of a composite block");
...
{
    if (myLeftBottomChild) return true;
    ...
    for (; !myLeftBottomChild && child != childEnd; ++child) {
        ...
        for (; otherChild != childEnd && !sharedVertex; ++otherChild)
            if (otherChild != child) sharedVertex = otherChild->mySides.Contain(leftVertex);
        if (!sharedVertex) {
            myLeftBottomChild = &(*child);
            DUMP_VERT("0 left bottom Vertex: ", leftVertex);
        }
    }
    if (!myLeftBottomChild) return error(ERR_LI("Error in locateChildren()"));
    ...
    if (!notLocatedChilren.empty()) return error(ERR_LI("Error in locateChildren()"));
...
bool _QuadFaceGrid::loadCompositeGrid(SMESH_Mesh &mesh)
{
    ...
    if (!locateChildren()) return false;
...
void _QuadFaceGrid::setBrothers(set<_QuadFaceGrid *> &notLocatedBrothers)
{
    if (!notLocatedBrothers.empty()) {
        ...
        // recursive call
        if (myRightBrother) myRightBrother->setBrothers(notLocatedBrothers);
        if (myUpBrother) myUpBrother->setBrothers(notLocatedBrothers);
    }
}
...
bool _QuadFaceGrid::fillGrid(SMESH_Mesh &theMesh, vector<const SMDS_MeshNode *> &theGrid,
                             const _Indexer &theIndexer, int theX, int theY)
{
    if (myGrid.empty() && !LoadGrid(theMesh)) return false;
    ...
    if (myUpBrother) {
        if (!myUpBrother->fillGrid(theMesh, theGrid, theIndexer, theX, theY + myIndexer._ySize - 1))
...
{
    int nbSegs = 0;
    if (myLeftBottomChild) { nbSegs += myLeftBottomChild->GetNbHoriSegments(mesh, true); }
    else {
...
{
    int nbSegs = 0;
    if (myLeftBottomChild) { nbSegs += myLeftBottomChild->GetNbVertSegments(mesh, true); }
    else {
...
{
    if (myLeftBottomChild) { return myLeftBottomChild->GetHoriEdges(edges); }
    else {
        ...
        if (myRightBrother) myRightBrother->GetHoriEdges(edges);
    }
    return edges.size();
}
...
bool _QuadFaceGrid::GetNormal(const TopoDS_Vertex &v, gp_Vec &n) const
{
    if (myChildren.empty()) {
        ...
        for (; child != childEnd; ++child)
            if (child->GetNormal(v, n)) return true;
    }
    return false;
}
...
void _QuadFaceGrid::DumpVertices() const
{
    ...
    cout << "****** Face ";
    const char *names[] = {"B_BOTTOM", "B_RIGHT", "B_TOP", "B_LEFT", "B_FRONT", "B_BACK"};
    if (myID >= B_BOTTOM && myID < B_BACK) cout << names[myID] << endl;
    else
        cout << "UNDEFINED" << endl;
...
_FaceSide::_FaceSide(const TopoDS_Edge &edge) : myEdge(edge), myNbChildren(0)
{
    if (!edge.IsNull())
...
_FaceSide::_FaceSide(const list<TopoDS_Edge> &edges) : myNbChildren(0)
{
...
{
    if (i >= myNbChildren) return 0;

    list<_FaceSide>::iterator side = myChildren.begin();
    if (i) std::advance(side, i);
    return &(*side);
}
...
{
    if (myChildren.empty()) return myVertices.Extent();
...
int _FaceSide::NbCommonVertices(const TopTools_MapOfShape &VV) const
{
    ...
    for (; vIt.More(); vIt.Next()) nbCommon += (VV.Contains(vIt.Key()));
...
{
    if (myChildren.empty()) return TopExp::FirstVertex(myEdge, Standard_True);
...
{
    if (myChildren.empty()) return TopExp::LastVertex(myEdge, Standard_True);
...
{
    if (myChildren.empty()) return i ? LastVertex() : FirstVertex();

    if (i >= myNbChildren) return myChildren.back().LastVertex();
...
{
    if (i == 0 && !myEdge.IsNull()) return myEdge;

    if (const _FaceSide *iSide = GetSide(i)) return iSide->myEdge;
...
bool _FaceSide::Contain(const _FaceSide &side, int *which) const
{
    if (!which || myChildren.empty()) {
        ...
        for (; vIt.More(); vIt.Next()) nbCommon += (myVertices.Contains(vIt.Key()));
        return (nbCommon > 1);
    }
...
bool _FaceSide::Contain(const TopoDS_Vertex &vertex) const { return myVertices.Contains(vertex); }
...
void _FaceSide::AppendSide(const _FaceSide &side)
{
    ...
    myNbChildren++;
    TopTools_MapIteratorOfMapOfShape vIt(side.myVertices);
    for (; vIt.More(); vIt.Next()) myVertices.Add(vIt.Key());
...
{
    int nb = 0;
    if (myChildren.empty()) { nb = mesh.GetSubMesh(myEdge)->GetSubMeshDS()->NbElements(); }
    else {
        list<_FaceSide>::const_iterator side = myChildren.begin(), sideEnd = myChildren.end();
        for (; side != sideEnd; ++side) nb += side->GetNbSegments(mesh);
    }
    return nb;
}
...
bool _FaceSide::StoreNodes(SMESH_Mesh &mesh, vector<const SMDS_MeshNode *> &myGrid, bool reverse)
{
    list<TopoDS_Edge> edges;
    if (myChildren.empty()) { edges.push_back(myEdge); }
    else {
        list<_FaceSide>::const_iterator side = myChildren.begin(), sideEnd = myChildren.end();
        for (; side != sideEnd; ++side)
            if (reverse) edges.push_front(side->myEdge);
            else
    ...
    for (; edge != eEnd; ++edge) {
        ...
        bool ok = SMESH_Algo::GetSortedNodesOnEdge(mesh.GetMeshDS(), *edge,
                                                   /*ignoreMediumNodes=*/true, nodes);
        if (!ok) return false;
...
void _FaceSide::Dump() const
{
    if (myChildren.empty()) {
        ...
        if (myID >= Q_BOTTOM && myID < Q_PARENT) cout << sideNames[myID] << endl;
        else

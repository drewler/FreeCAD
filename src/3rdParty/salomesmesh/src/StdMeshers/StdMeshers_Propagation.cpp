...
#define DBGMSG(txt) //  cout << txt << endl;
...
public:
     ...
     */
    static TopoDS_Edge GetSource(SMESH_subMesh *submesh, bool &isPropagOfDistribution);
    /*!
...
StdMeshers_PropagOfDistribution::StdMeshers_PropagOfDistribution(int hypId, int studyId,
                                                                 SMESH_Gen *gen)
    : StdMeshers_Propagation(hypId, studyId, gen)
{
    _name = GetName();
}
...
bool StdMeshers_Propagation::SetParametersByMesh(const SMESH_Mesh *, const TopoDS_Shape &)
{
    return false;
}
bool StdMeshers_Propagation::SetParametersByDefaults(const TDefaults &, const SMESH_Mesh *)
{
    return false;
}
void StdMeshers_Propagation::SetPropagationMgr(SMESH_subMesh *subMesh)
{
    PropagationMgr::Set(subMesh);
}
/*!
...
{
    return PropagationMgr::GetSource(theMesh.GetSubMeshContaining(theEdge), isPropagOfDistribution);
}
const SMESH_HypoFilter &StdMeshers_Propagation::GetFilter()
{
    static SMESH_HypoFilter propagHypFilter;
    if (propagHypFilter.IsEmpty()) {
        propagHypFilter.Init(SMESH_HypoFilter::HasName(StdMeshers_Propagation::GetName()))
...
struct PropagationMgrData: public EventListenerData {
    ...
    void Init()
    {
        myType = WAIT_PROPAG_HYP;
        mySubMeshes.clear();
        myForward = true;
        myIsPropagOfDistribution = false;
    }
    SubMeshState State() const { return (SubMeshState)myType; }
    void SetState(SubMeshState state) { myType = state; }
    ...
    {
        mySubMeshes.clear();
        mySubMeshes.splice(mySubMeshes.end(), chain);
    }
...
PropagationMgr *getListener() { return PropagationMgr::GetListener(); }
//=============================================================================
/*!
...
{
    if (sm) return static_cast<PropagationMgrData *>(sm->GetEventListenerData(getListener()));
    return 0;
}
//=============================================================================
/*!
...
{
    if (theEdge.ShapeType() == TopAbs_EDGE) return findData(theMesh.GetSubMeshContaining(theEdge));
    return 0;
}
//=============================================================================
/*!
...
{
    return theSubMesh->GetFather()->GetHypothesis(theSubMesh, StdMeshers_Propagation::GetFilter(),
                                                  true);
}
//================================================================================
/*!
    ...
    for (; smIt != chain.end(); ++smIt) {
        ...
        for (; itA.More(); itA.Next()) {
            ...
            for (; aWE.More(); aWE.Next()) {
                ...
                if (edge.IsSame(anE)) edgeIndex = edges.size();
            ...
            if (edges.size() < 4) {
                ...
                for (int i = 0; i < edges.size(); ++i, ++eIndex) {
                    if (eIndex == edges.size()) eIndex = 0;
                    if (!SMESH_Algo::IsContinuous(prevEdge, edges[eIndex])) { nbSide++; }
                    else {
                    ...
                    if (nbSide == 5) {
                        anOppE.Nullify();
                        break; // too many sides
                    }
                    prevEdge = edges[eIndex];
                }
                if (anOppE.IsNull()) continue;
                if (nbSide != 4) {
                    DBGMSG(nbSide << " sides in wire #"
                                  << mesh->GetMeshDS()->ShapeToIndex(itA.Value()) << " - SKIP");
                    continue;
                }
            }
            if (anOppE.IsNull() || !checkedShapes.Add(anOppE)) continue;
...
bool clearPropagationChain(SMESH_subMesh *subMesh)
{
    ...
    if (PropagationMgrData *data = findData(subMesh)) {
        switch (data->State()) {
            case IN_CHAIN: return clearPropagationChain(data->GetSource());
            ...
            case LAST_IN_CHAIN: {
                SMESH_subMeshIteratorPtr smIt =
                    iterate(data->mySubMeshes.begin(), data->mySubMeshes.end());
                while (smIt->more()) clearPropagationChain(smIt->next());
                data->Init();
                break;
            }
            default:;
...
{
    switch (State()) {
        case HAS_PROPAG_HYP: return iterate(mySubMeshes.begin(), mySubMeshes.end());
        case IN_CHAIN:
...
{
    if (myType == IN_CHAIN)
        if (!mySubMeshes.empty()) return mySubMeshes.front();
    return 0;
}
...
void PropagationMgr::Set(SMESH_subMesh *submesh)
{
    ...
    const SMESH_Hypothesis *propagHyp = getProagationHyp(submesh);
    if (propagHyp) {
        data->myIsPropagOfDistribution =
            (StdMeshers_PropagOfDistribution::GetName() == propagHyp->GetName());
        getListener()->ProcessEvent(SMESH_subMesh::ADD_HYP, SMESH_subMesh::ALGO_EVENT, submesh,
                                    data, propagHyp);
    }
}
//================================================================================
...
TopoDS_Edge PropagationMgr::GetSource(SMESH_subMesh *submesh, bool &isPropagOfDistribution)
{
    if (PropagationMgrData *data = findData(submesh)) {
        if (data->State() == IN_CHAIN) {
            if (SMESH_subMesh *sm = data->GetSource()) {
                ...
                if (edge.ShapeType() == TopAbs_EDGE) return TopoDS::Edge(edge);
            }
        }
    }
    return TopoDS_Edge();
...
void PropagationMgr::ProcessEvent(const int event, const int eventType, SMESH_subMesh *subMesh,
                                  SMESH_subMeshEventListenerData *listenerData,
                                  const SMESH_Hypothesis *hyp)
{
    if (!listenerData) return;
    if (!hyp || hyp->GetType() != SMESHDS_Hypothesis::PARAM_ALGO || hyp->GetDim() != 1) return;
    if (eventType != SMESH_subMesh::ALGO_EVENT) return;
    ...
    switch (data->State()) {
        ...
        case WAIT_PROPAG_HYP: { // propagation hyp or local 1D hyp is missing
            ...
            if (!hasPropagHyp) return;
            bool hasLocal1DHyp = getLocal1DHyp(subMesh);
            if (!hasLocal1DHyp) return;
        ...
        case LAST_IN_CHAIN: { // submesh with local 1D hyp, breaking a chain
            ...
            if (event == SMESH_subMesh::REMOVE_HYP) { // remove local hyp
                ...
                while (smIt->more()) buildPropagationChain(smIt->next());
            }
            return;
        }

...
struct TQuadrangleAlgo: public StdMeshers_Quadrangle_2D {
    ...
    {
        static TQuadrangleAlgo *algo =
            new TQuadrangleAlgo(fatherAlgo->GetStudyId(), fatherAlgo->GetGen());
        if (helper && algo->myProxyMesh && algo->myProxyMesh->GetMesh() != helper->GetMesh())
            algo->myProxyMesh.reset(new SMESH_ProxyMesh(*helper->GetMesh()));

        algo->myQuadList.clear();

        if (helper) algo->_quadraticMesh = helper->GetIsQuadratic();
...
struct TProjction1dAlgo: public StdMeshers_Projection_1D {
    ...
    {
        static TProjction1dAlgo *algo =
            new TProjction1dAlgo(fatherAlgo->GetStudyId(), fatherAlgo->GetGen());
        return algo;
    }
};
//=======================================================================
...
struct TProjction2dAlgo: public StdMeshers_Projection_1D2D {
    ...
    {
        static TProjction2dAlgo *algo =
            new TProjction2dAlgo(fatherAlgo->GetStudyId(), fatherAlgo->GetGen());
        return algo;
    }
    const NSProjUtils::TNodeNodeMap &GetNodesMap() { return _src2tgtNodes; }
};
//=======================================================================
/*!
   * \brief Returns already computed EDGEs
   */
void getPrecomputedEdges(SMESH_MesherHelper &theHelper, const TopoDS_Shape &theShape,
                         vector<TopoDS_Edge> &theEdges)
{
    ...
    for (int iE = 1; iE <= edges.Extent(); ++iE) {
        ...
        if ((!(sm = meshDS->MeshElements(edge))) || (sm->NbElements() == 0)) continue;
        ...
        PShapeIteratorPtr faceIt = theHelper.GetAncestors(edge, *theHelper.GetMesh(), TopAbs_FACE);
        while (const TopoDS_Shape *face = faceIt->next())

            if ((sm = meshDS->MeshElements(*face)) && (sm->NbElements() > 0)
                && (!theHelper.IsSameElemGeometry(sm, SMDSGeom_QUADRANGLE))) {
                faceFound = true;
                break;
            }
        if (!faceFound) theEdges.push_back(TopoDS::Edge(edge));
    }
}
...
bool setBottomEdge(const TopoDS_Edge &botE, FaceQuadStruct::Ptr &quad, const TopoDS_Shape &face)
{
    ...
    for (size_t i = 0; i < quad->side.size(); ++i) {
        ...
        for (int iE = 0; iE < quadSide->NbEdges(); ++iE)
            if (botE.IsSame(quadSide->Edge(iE))) {
                if (quadSide->NbEdges() > 1) isComposite = true; //return false;
                edgeIndex = i;
...
TParam2ColumnIt getColumn(const TParam2ColumnMap *columnsMap, const double parameter)
{
    TParam2ColumnIt u_col = columnsMap->upper_bound(parameter);
    if (u_col != columnsMap->begin()) --u_col;
...
double getRAndNodes(const TNodeColumn *column, const double param, const SMDS_MeshNode *&node1,
...
void splitParams(const int nbParts, const TParam2ColumnMap *columnsMap, vector<double> &params)
{
    params.clear();
    ...
    for (int i = 0; i < nbParts - 1; ++i) {
        ...
        if (par_col->first == par) {
            ...
            if (par_col == last_par_col) {
                while (i < nbParts - 1) params.push_back(par + partSize * i++);
                break;
            }
        }
        par = par_col->first;
...
gp_Ax2 getLayerCoordSys(const int z, const vector<const TNodeColumn *> &columns, int &xColumn)
{
    ...
    if (vertexCol >= 0) { O = gpXYZ((*columns[vertexCol])[z]); }
...
int removeQuasiQuads(list<SMESH_subMesh *> &notQuadSubMesh, SMESH_MesherHelper *helper,
                     StdMeshers_Quadrangle_2D *quadAlgo)
{
    ...
    while (smIt != notQuadSubMesh.end()) {
        ...
        bool toRemove;
        if (nbQuads > 0) toRemove = helper->IsStructured(faceSm);
        else
            toRemove = (bool)quadAlgo->CheckNbEdges(*helper->GetMesh(), faceSm->GetSubShape());
        nbRemoved += toRemove;
        if (toRemove) smIt = notQuadSubMesh.erase(smIt);
        else
            ++smIt;
    }
...
int countNbSides(const Prism_3D::TPrismTopo &thePrism, vector<int> &nbUnitePerEdge,
...
bool StdMeshers_Prism_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
bool StdMeshers_Prism_3D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (nbSolids == 1) {
        ...
        if (!meshedFaces.empty()) prism.myBottom = meshedFaces.front();
        return (initPrism(prism, solid, selectBottom) && compute(prism));
    }
    ...
    for (size_t i = 0, nb = 0; i < computedEdges.size(); ++i) {
        StdMeshers_ProjectionUtils::GetPropagationEdge(&theMesh, TopoDS_Edge(), computedEdges[i],
                                                       myPropagChains + nb);
    ...
    while (meshedSolids.Extent() < nbSolids) {
        if (_computeCanceled) return toSM(error(SMESH_ComputeError::New(COMPERR_CANCELED)));
        ...
        while (!meshedFaces.empty()) {
            ...
            while (!solidList.IsEmpty()) {
                ...
                if (meshedSolids.Add(solid)) {
                    prism.Clear();
                    prism.myBottom = face;
                    if (!initPrism(prism, solid, selectBottom) || !compute(prism)) return false;
        ...
        if (meshedSolids.Extent() == nbSolids) break;
                ...
                for (; wQuad != prismIt->myWallQuads[iW].end(); ++wQuad) {
                    ...
                    while (solidIt.More()) {
                        ...
                        PShapeIteratorPtr faceIt =
                            myHelper->GetAncestors(wEdge, *myHelper->GetMesh(), TopAbs_FACE);
                        while (const TopoDS_Shape *f = faceIt->next()) {
                            ...
                            for (TopExp_Explorer v(prismIt->myTop, TopAbs_VERTEX); v.More();
                                 v.Next())
                            ...
                            if (!myHelper->IsSubShape(candidateF, prismIt->myShape3D)
                                ...
                                && !myHelper->GetMesh()
                                        ->GetSubMesh(prism.myBottom)
                                        ->IsMeshComputed()
                                && project2dMesh(sourceF, prism.myBottom)) {
                                mySetErrorToSM = true;
                                if (!compute(prism)) return false;
                                SMESHDS_SubMesh *smDS =
                                    theMesh.GetMeshDS()->MeshElements(prism.myTop);
                                if (!myHelper->IsSameElemGeometry(smDS, SMDSGeom_QUADRANGLE)) {
                                    meshedFaces.push_front(prism.myTop);
                        ...
                        mySetErrorToSM = true;
                        InitComputeError();
                        if (meshedSolids.Contains(solid)) solidList.Remove(solidIt);
                        else
                            solidIt.Next();
                    }
                }
            }
            if (!meshedFaces.empty()) break; // to compute prisms with avident sources
        }
        ...
        if (meshedFaces.empty()) {
            for (TopExp_Explorer solid(theShape, TopAbs_SOLID); solid.More(); solid.Next()) {
                mySetErrorToSM = false;
                prism.Clear();
                if (!meshedSolids.Contains(solid.Current()) && initPrism(prism, solid.Current())) {
                    mySetErrorToSM = true;
                    if (!compute(prism)) return false;
                    meshedFaces.push_front(prism.myTop);
                    meshedFaces.push_front(prism.myBottom);
...
bool StdMeshers_Prism_3D::getWallFaces(Prism_3D::TPrismTopo &thePrism, const int totalNbFaces)
{
    ...
    TopExp::MapShapesAndAncestors(thePrism.myShape3D, TopAbs_EDGE, TopAbs_FACE, edgeToFaces);
        ...
        if (BRep_Tool::Curve(*edge, f, l).IsNull()) {
            ...
            for (; faceIt.More(); faceIt.Next()) {
                ...
                if (!thePrism.myBottom.IsSame(face)) {
                    ...
                    if (!quadList.back())
                        return toSM(error(TCom("Side face #")
                                          << shapeID(face) << " not meshable with quadrangles"));
                    ...
                    if (isCompositeBase) {
                        ...
                        for (int iE = 0; iE < botSide->NbEdges(); ++iE)
                            if (!myHelper->IsSubShape(botSide->Edge(iE), thePrism.myBottom))
                                return toSM(
                                    error(TCom("Composite 'horizontal' edges are not supported")));
                    }
                    if (faceMap.Add(face)) thePrism.myWallQuads.push_back(quadList);
                    break;
                }
            }
            ++edge;
    ...
    while (totalNbFaces - faceMap.Extent() > 2) {
                ...
                for (int iE = 0; iE < rightSide->NbEdges(); ++iE) // rightSide can be composite
                {
                    ...
                    for (; face.More(); face.Next())
                        if (faceMap.Add(face.Value())) {
                            ...
                            thePrism.myWallQuads[iRight].push_back(
                                quadAlgo->CheckNbEdges(*mesh, newWallF));
                            if (!thePrism.myWallQuads[iRight].back())
                                return toSM(error(TCom("Side face #")
                                                  << shapeID(newWallF)
                                                  << " not meshable with quadrangles"));
                            if (!setBottomEdge(newBotE, thePrism.myWallQuads[iRight].back(),
                                               newWallF))
                                return toSM(
                                    error(TCom("Composite 'horizontal' edges are not supported")));
                        }
        ...
        if (totalNbFaces - faceMap.Extent() > 2) {
            ...
            for (size_t i = 0; i < thePrism.myWallQuads.size(); ++i) {
                StdMeshers_FaceSidePtr topSide =
                    thePrism.myWallQuads[i].back()->side[QUAD_TOP_SIDE];
                ...
                for (; faceIt.More(); faceIt.Next())
                    if (faceMap.Add(faceIt.Value())) {
                        ...
                        thePrism.myWallQuads[i].push_back(
                            quadAlgo->CheckNbEdges(*mesh, faceIt.Value()));
                        if (!thePrism.myWallQuads[i].back())
                            return toSM(error(TCom("Side face #")
                                              << shapeID(faceIt.Value())
                                              << " not meshable with quadrangles"));
            ...
            if (nbFoundWalls == faceMap.Extent()) return toSM(error("Failed to find wall faces"));
        }
    } // while ( totalNbFaces - faceMap.Extent() > 2 )
    ...
    if (thePrism.myTop.IsNull()) {
        ...
        if (thePrism.myTop.IsNull()) return toSM(error("Top face not found"));
    }
...
bool StdMeshers_Prism_3D::compute(const Prism_3D::TPrismTopo &thePrism)
{
    ...
    if ((botSM->IsEmpty())
        ...
        return error(COMPERR_BAD_INPUT_MESH,
                     TCom("No mesher defined to compute the face #") << shapeID(thePrism.myBottom));

    // Make all side FACEs of thePrism meshed with quads
    if (!computeWalls(thePrism)) return false;
    ...
    if (!myBlock.Init(myHelper, thePrism)) return toSM(error(myBlock.GetError()));
    ...
    if (!myUseBlock) {
        ...
        for (; edge != thePrism.myBottomEdges.end(); ++edge) {
            ...
            TParam2ColumnMap *u2col =
                const_cast<TParam2ColumnMap *>(myBlock.GetParam2ColumnMap(edgeID, dummy));
    ...
    else // use block approach
    {
        ...
        for (; bot_column != myBotToColumnMap.end(); ++bot_column) {
            ...
            if (!tBotNode.HasParams()) {
                ...
                if (prevBNode.IsNeighbor(tBotNode)) paramHint = prevBNode.GetParams();
            ...
            for (int z = 0; columnNodes != column.end(); ++columnNodes, ++z) {
                ...
                int sideFaceIDs[nbSideFaces] = {SMESH_Block::ID_Fx0z, SMESH_Block::ID_Fx1z,
                                                SMESH_Block::ID_F0yz, SMESH_Block::ID_F1yz};
                for (int iF = 0; iF < nbSideFaces; ++iF)
                    if (!setFaceAndEdgesXYZ(sideFaceIDs[iF], params, z)) return false;
    ...
    while (faceIt->more()) {
        ...
        if (!face || face->GetType() != SMDSAbs_Face) continue;
...
bool StdMeshers_Prism_3D::computeWalls(const Prism_3D::TPrismTopo &thePrism)
{
    ...
    for (size_t iW = 0; iW != nbWalls; ++iW) wgt2quad.insert(make_pair(wgt[iW], iW));
    ...
    for (; w2q != wgt2quad.rend(); ++w2q) {
        ...
        for (; quad != quads.end(); ++quad) {
            ...
            if (swapLeftRight) std::swap(lftSide, rgtSide);
            ...
            for (int i = 0; i < lftSide->NbEdges(); ++i) {
                ...
                if (!srcSM->IsMeshComputed()) {
                    ...
                    if (!srcSM->IsMeshComputed()) return toSM(error("Can't compute 1D mesh"));
                }
            ...
            if (rgtSide->NbEdges() == nbTgtMeshed) // all tgt EDGEs meshed
            {
                if (nbTgtSegments != nbSrcSegments) {
                    ...
                    if (!badMeshRemoved) {
                        ...
                        return toSM(error(
                            TCom("Different nb of segment on logically vertical edges #")
                            << shapeID(lftSide->Edge(0)) << " and #" << shapeID(rgtSide->Edge(0))
            ...
            if (nbTgtMeshed == 0) {
                ...
                for (size_t iN = 1; iN < srcNodeStr.size(); ++iN) // add segments
                {
                    ...
                    if (id2type.second != TopAbs_EDGE) {
                        ...
                        if (vn) {
                            SMESH_MeshEditor::TListOfListOfNodes lln(1,
                                                                     list<const SMDS_MeshNode *>());
                ...
                if (!swapLeftRight && rgtSide->NbEdges() > 1 && w2q->second == iW) {
                    std::pair<int, int> wgt2quadKeyVal(w2q->first + 1,
                                                       thePrism.myRightQuadIndex[iW]);
    ...
    for (size_t iW = 0; iW != thePrism.myWallQuads.size(); ++iW) {
        ...
        for (; quad != thePrism.myWallQuads[iW].end(); ++quad) {
            ...
            if (!fSM->IsMeshComputed()) {
                ...
                srcSM->ComputeStateEngine(SMESH_subMesh::CHECK_COMPUTE_STATE);
                tgtSM->ComputeStateEngine(SMESH_subMesh::CHECK_COMPUTE_STATE);
                if (!srcSM->IsMeshComputed() && tgtSM->IsMeshComputed()) std::swap(srcSM, tgtSM);
                ...
                if (tgtSM->IsMeshComputed()
                    ...
                    PShapeIteratorPtr fIt =
                        myHelper->GetAncestors(tgtSM->GetSubShape(), *mesh, TopAbs_FACE);
                    while (const TopoDS_Shape *f = fIt->next())
                        if ((isAdjFaceMeshed = mesh->GetSubMesh(*f)->IsMeshComputed())) break;
                    if (isAdjFaceMeshed)
                        return toSM(
                            error(TCom("Different nb of segment on logically horizontal edges #")
                                  << shapeID(botE) << " and #" << shapeID(topE) << ": "
                ...
                if (!tgtSM->IsMeshComputed()) {
                    ...
                    SMESH_subMeshIteratorPtr smIt =
                        tgtSM->getDependsOnIterator(/*includeSelf=*/false);
                    while (smIt->more()) smIt->next()->ComputeStateEngine(SMESH_subMesh::COMPUTE);
                    // project segments
...
TopoDS_Edge StdMeshers_Prism_3D::findPropagationSource(const TopoDS_Edge &E)
{
    if (myPropagChains)
        for (size_t i = 0; !myPropagChains[i].IsEmpty(); ++i)
            if (myPropagChains[i].Contains(E)) return TopoDS::Edge(myPropagChains[i].FindKey(1));
...
bool StdMeshers_Prism_3D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                   MapShapeNbElems &aResMap)
{
    ...
    for (TopExp_Explorer exp(theShape, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        if (anIt == aResMap.end()) return toSM(error("Submesh can not be evaluated"));

        std::vector<int> aVec = (*anIt).second;
        int nbtri = Max(aVec[SMDSEntity_Triangle], aVec[SMDSEntity_Quad_Triangle]);
        int nbqua = Max(aVec[SMDSEntity_Quadrangle], aVec[SMDSEntity_Quad_Quadrangle]);
        if (nbtri == 0 && nbqua > 0) { NbQFs++; }
        if (nbtri > 0) { NumBase = i; }
    }
    ...
    if (IsQuadratic) {
        ...
        aResVec[SMDSEntity_Node] =
            nb0d_face0 * (2 * nb2d / nb1d - 1) - nb1d_face0_int * nb2d / nb1d;
    }
    else {
                                    ...
                                    SMESH_MesherHelper *helper)
{
    ...
    switch (nbNodes) {
        case 3: {
            SMDS_VolumeOfNodes tmpPenta((*columns[0])[z - 1], // bottom
                                        (*columns[1])[z - 1], (*columns[2])[z - 1],
                                        (*columns[0])[z], // top
                                        (*columns[1])[z], (*columns[2])[z]);
            vTool.Set(&tmpPenta);
        ...
        case 4: {
            SMDS_VolumeOfNodes tmpHex((*columns[0])[z - 1], (*columns[1])[z - 1], // bottom
                                      (*columns[2])[z - 1], (*columns[3])[z - 1], (*columns[0])[z],
                                      (*columns[1])[z], // top
        ...
        default:
            ...
            SMDS_VolumeOfNodes tmpVol((*columns[0])[z - 1], (*columns[di])[z - 1],
                                      (*columns[2 * di])[z - 1], (*columns[0])[z],
                                      (*columns[di])[z], (*columns[2 * di])[z]);
            vTool.Set(&tmpVol);
            isForward = vTool.IsForward();
    }
    ...
    switch (nbNodes) {
        ...
        case 3: { // ---------- pentahedra
            ...
            for (z = 1; z < nbZ; ++z)
                helper->AddVolume((*columns[0])[z - 1], // bottom
                                  (*columns[i1])[z - 1], (*columns[i2])[z - 1],
                                  (*columns[0])[z], // top
                                  (*columns[i1])[z], (*columns[i2])[z]);
            break;
        }
        case 4: { // ---------- hexahedra
            ...
            for (z = 1; z < nbZ; ++z)
                helper->AddVolume((*columns[0])[z - 1], (*columns[i1])[z - 1], // bottom
                                  (*columns[2])[z - 1], (*columns[i3])[z - 1], (*columns[0])[z],
                                  (*columns[i1])[z], // top
        ...
        case 6: { // ---------- octahedra
            ...
            for (z = 1; z < nbZ; ++z)
                helper->AddVolume(
                    (*columns[0])[z + iBase1], (*columns[1])[z + iBase1], // bottom or top
                    (*columns[2])[z + iBase1], (*columns[3])[z + iBase1], (*columns[4])[z + iBase1],
                    (*columns[5])[z + iBase1], (*columns[0])[z + iBase2],
                    (*columns[1])[z + iBase2], // top or bottom
                    (*columns[2])[z + iBase2], (*columns[3])[z + iBase2], (*columns[4])[z + iBase2],
                    (*columns[5])[z + iBase2]);
            break;
        }
        default: // ---------- polyhedra
                                                ...
                                                const Prism_3D::TPrismTopo &thePrism)
{
    ...
    if (!needProject
        ...
        MESSAGE("nb elem bot " << botSMDS->NbElements() << " top "
                               << (topSMDS ? topSMDS->NbElements() : 0));
        MESSAGE("nb node bot " << botSMDS->NbNodes() << " top "
                               << (topSMDS ? topSMDS->NbNodes() : 0));
        return toSM(error(TCom("Mesh on faces #")
                          << botSM->GetId() << " and #" << topSM->GetId() << " seems different"));
    }
    ...
    if (needProject) {
        if (!projectBottomToTop(bottomToTopTrsf, thePrism)) return false;
    ...
    if (!n2nMapPtr || n2nMapPtr->size() < botSMDS->NbNodes()) {
        ...
        const bool sameTopo = NSProjUtils::FindSubShapeAssociation(
            thePrism.myBottom, myHelper->GetMesh(), thePrism.myTop, myHelper->GetMesh(),
            shape2ShapeMap);
        if (!sameTopo)
            for (size_t iQ = 0; iQ < thePrism.myWallQuads.size(); ++iQ) {
                ...
                if (botSide->NbEdges() == topSide->NbEdges()) {
                    for (int iE = 0; iE < botSide->NbEdges(); ++iE) {
                        NSProjUtils::InsertAssociation(botSide->Edge(iE), topSide->Edge(iE),
                                                       shape2ShapeMap);
                    ...
                    if (vb.IsSame(sideB->FirstVertex()) && vt.IsSame(sideT->LastVertex())) {
                        NSProjUtils::InsertAssociation(botSide->Edge(0), topSide->Edge(0),
                                                       shape2ShapeMap);
        ...
        if (!NSProjUtils::FindMatchingNodesOnFaces(thePrism.myBottom, myHelper->GetMesh(),
            ...
            if (sameTopo)
                return toSM(error(TCom("Mesh on faces #") << botSM->GetId() << " and #"
                                                          << topSM->GetId() << " seems different"));
            else
                return toSM(error(TCom("Topology of faces #")
                                  << botSM->GetId() << " and #" << topSM->GetId()
                                  << " seems different"));
        }
    }
...
bool StdMeshers_Prism_3D::projectBottomToTop(const gp_Trsf &bottomToTopTrsf,
                                             const Prism_3D::TPrismTopo &thePrism)
{
    if (project2dMesh(thePrism.myBottom, thePrism.myTop)) { return true; }
    NSProjUtils::TNodeNodeMap &n2nMap =
    ...
    while (nIt->more()) {
        ...
        if (botNode->GetPosition()->GetTypeOfPosition() != SMDS_TOP_FACE) continue; // strange
        ...
        if (bottomToTopTrsf.Form() == gp_Identity) {
            ...
            if (!myBlock.FacePoint(ID_TOP_FACE, bN.GetParams(), topXYZ)
                ...
                return toSM(error(TCom("Can't compute coordinates "
                                       "by normalized parameters on the face #")
                                  << topSM->GetId()));
        ...
        else // use bottomToTopTrsf
        {
            ...
            if (topHelper.CheckNodeUV(topFace, topNode, topUV, fixTol, /*force=*/false, distXYZ)
                ...
                meshDS->MoveNode(topNode, distXYZ[1], distXYZ[2],
                                 distXYZ[3]); // transform can be inaccurate
        }
        // create node column
        TNode2ColumnMap::iterator bN_col =
            myBotToColumnMap.insert(make_pair(bN, TNodeColumn())).first;
    ...
    while (faceIt->more()) {
        ...
        switch (nbNodes) {
            ...
            default: newFace = meshDS->AddPolygonalFace(nodes);
        }
        if (newFace) meshDS->SetMeshElementOnShape(newFace, topFaceID);
    }
    ...
    if (thePrism.myNbEdgesInWires.size() > 1 && // there are holes
        ...
        if (!isFixed)
            return toSM(error(TCom("Projection from face #")
                              << botSM->GetId() << " to face #" << topSM->GetId()
...
double StdMeshers_Prism_3D::getSweepTolerance(const Prism_3D::TPrismTopo &thePrism)
{
    ...
    for (int iSM = 0; iSM < 2; ++iSM) {
        ...
        while (fIt->more()) {
            ...
            for (int iN = 0; iN < nbNodes; ++iN) nodes[iN] = nIt->next();
            nodes.back() = nodes[0];
            ...
            for (int iN = 0; iN < nbNodes; ++iN) {
                if (nodes[iN]._node->GetPosition()->GetDim() < 2
                    ...
                    {
                        if (nodes[iN2] == nodes[iN] || nodes[iN2] == nodes[iN + 1]) continue;
                        if (isDegen) { dist2 = (nodes[iN] - nodes[iN2]).SquareModulus(); }
                        else {
                            dist2 = linkDir.CrossSquareMagnitude(nodes[iN] - nodes[iN2]);
                        }
                        if (dist2 > numeric_limits<double>::min()) minDist = Min(minDist, dist2);
                    }
                }
                // measure length link
...
{
    if (mySetErrorToSM && !isOK && myHelper && !myHelper->GetSubShape().IsNull()
...
struct EdgeWithNeighbors {
    ...
    EdgeWithNeighbors(const TopoDS_Edge &E, int iE, int nbE, int shift = 0)
        : _edge(E), _iL(SMESH_MesherHelper::WrapIndex(iE - 1, nbE) + shift),
...
struct PrismSide {
    ...
    const TopoDS_Edge &Edge(int i) const { return (*_edges)[i]._edge; }
...
/*!
   ...
   */
bool getEdges(const TopoDS_Face &face, vector<EdgeWithNeighbors> &edges, const bool noHolesAllowed)
{
    list<TopoDS_Edge> ee;
    list<int> nbEdgesInWires;
    int nbW = SMESH_Block::GetOrderedEdges(face, ee, nbEdgesInWires);
    if (nbW > 1 && noHolesAllowed) return false;
    ...
    for (nbE = nbEdgesInWires.begin(); nbE != nbEdgesInWires.end(); ++nbE) {
        for (iE = 0; iE < *nbE; ++e, ++iE) edges.push_back(EdgeWithNeighbors(*e, iE, *nbE, nbTot));
        nbTot += *nbE;
    }
    return edges.size();
}
...
/*!
   ...
   */
const TopoDS_Shape &getAnotherFace(const TopoDS_Face &face, const TopoDS_Edge &edge,
                                   TopTools_IndexedDataMapOfShapeListOfShape &facesOfEdge)
{
    ...
    for (; faceIt.More(); faceIt.Next())
        if (!face.IsSame(faceIt.Value())) return faceIt.Value();
    return face;
}
} // namespace
...
bool StdMeshers_Prism_3D::IsApplicable(const TopoDS_Shape &shape, bool toCheckAll)
{
    ...
    for (; sExp.More(); sExp.Next()) {
        ...
        if (allFaces.Extent() == 6) {
            ...
            bool isBox = SMESH_Block::FindBlockShapes(TopoDS::Shell(shell), TopoDS_Vertex(),
                                                      TopoDS_Vertex(), map);
            if (isBox) {
        ...
        TopTools_IndexedMapOfShape *facesOfSide =
            new TopTools_IndexedMapOfShape[faceEdgesVec.size()];
        ...
        for (int iF = 1; iF < allFaces.Extent() && !prismDetected; ++iF) {
            ...
            if (botEdges.empty())
                if (!getEdges(botF, botEdges, /*noHoles=*/false)) break;
            ...
            for (int iLoop = 0; isOK && isAdvanced; ++iLoop) {
                ...
                for (size_t iS = 0; iS < sides.size() && isOK; ++iS) {
                    ...
                    if (side._topEdge.IsNull()) {
                        ...
                        for (int is2nd = 0; is2nd < 2 && isOK; ++is2nd) // 2 adjacent neighbors
                        {
                            ...
                            for (size_t i = 1; i < side._edges->size(); ++i) {
                                int iE = SMESH_MesherHelper::WrapIndex(i * di + side._iBotEdge,
                                                                       side._edges->size());
                                if (side._isCheckedEdge[iE]) continue;
                                const TopoDS_Edge &vertE = side.Edge(iE);
                                const TopoDS_Shape &neighborF =
                                    getAnotherFace(side._face, vertE, facesOfEdge);
                    ...
                    else //if ( !side._topEdge.IsNull() )
                    {
                        ...
                        const TopoDS_Shape &f =
                            getAnotherFace(side._face, side._topEdge, facesOfEdge);
                        side._faces->Add(f);
                        bool stop = false;
                        ...
                        if (side._edges->empty())
                            if (!getEdges(side._face, *side._edges, /*noHoles=*/true)) break;
                ...
                if (nbFoundSideFaces > allFaces.Extent()) { isOK = false; }
            ...
            if (isOK && sides[0]._faces->Extent() > 1) {
                ...
                if (botEdges.size() == 1) // cylinder
                {
                    ...
                    for (iS = 1; iS < sides.size(); ++iS)
                        if (!sides[iS]._faces->Contains(topFace)) break;
...
bool Prism_3D::TNode::IsNeighbor(const Prism_3D::TNode &other) const
{
    ...
    while (fIt->more())
        if (fIt->next()->GetNodeIndex(myNode) >= 0) return true;
    return false;
}
...
void TPrismTopo::SetUpsideDown()
{
    ...
    for (size_t i = 0; i < myWallQuads.size(); ++i) {
        ...
        for (; q != myWallQuads[i].end(); ++q) { (*q)->shift(2, /*keepUnitOri=*/true); }
...
StdMeshers_PrismAsBlock::StdMeshers_PrismAsBlock() { mySide = 0; }

StdMeshers_PrismAsBlock::~StdMeshers_PrismAsBlock() { Clear(); }
...
bool StdMeshers_Prism_3D::initPrism(Prism_3D::TPrismTopo &thePrism, const TopoDS_Shape &theShape3D,
                                    const bool selectBottom)
{
    ...
    while (smIt->more()) {
        ...
        else if (face.ShapeType() < TopAbs_FACE)
            continue;
        nbFaces++;
        anyFaceSM = sm;
        ...
        list<int> nbEdgesInWires;
        int nbWires =
            SMESH_Block::GetOrderedEdges(TopoDS::Face(face), orderedEdges, nbEdgesInWires);
        if (nbWires != 1 || nbEdgesInWires.front() != 4) notQuadGeomSubMesh.push_back(sm);
    ...
    if (nbNotQuadMeshed > 2) {
        return toSM(
            error(COMPERR_BAD_INPUT_MESH,
                  TCom("More than 2 faces with not quadrangle elements: ") << nbNotQuadMeshed));
    }
    ...
    if (hasNotQuad) // can choose a bottom FACE
    {
        ...
        if (nbNotQuad == 2 && nbNotQuadMeshed > 0) {
            ...
            if (nbNotQuadMeshed == 1)
                ok = (find(notQuadGeomSubMesh.begin(), notQuadGeomSubMesh.end(), botSM)
                      != notQuadGeomSubMesh.end());
            else
                ok = (notQuadGeomSubMesh == notQuadElemSubMesh);
            if (!ok)
                return toSM(
                    error(COMPERR_BAD_INPUT_MESH, "Side face meshed with not quadrangle elements"));
        }
    }
    ...
    if (!botSM) // find a proper bottom
    {
        ...
        for (; !botSM && sm != meshedSubMesh.end(); ++sm) {
            ...
            botSM = *sm;
            thePrism.myBottom = TopoDS::Face(botSM->GetSubShape());
            if (!initPrism(thePrism, theShape3D, /*selectBottom=*/false)) botSM = NULL;
        }
    ...
    for (TopExp_Explorer exp(botSM->GetSubShape(), TopAbs_VERTEX); exp.More(); exp.Next()) {
    ...
    if (thePrism.myBottom.IsNull()) thePrism.myBottom = TopoDS::Face(botSM->GetSubShape());
    ...
    SMESH_Block::GetOrderedEdges(reverseBottom, thePrism.myBottomEdges, thePrism.myNbEdgesInWires,
                                 V000);
    ...
    if (topSM) {
        if (!thePrism.myTop.IsSame(topSM->GetSubShape()))
            return toSM(
                error(notQuadGeomSubMesh.empty() ? COMPERR_BAD_INPUT_MESH : COMPERR_BAD_SHAPE,
...
bool StdMeshers_PrismAsBlock::Init(SMESH_MesherHelper *helper, const Prism_3D::TPrismTopo &thePrism)
{
    myHelper = helper;
    SMESHDS_Mesh *meshDS = myHelper->GetMeshDS();
    SMESH_Mesh *mesh = myHelper->GetMesh();
    ...
    for (iE = 0; iE < nbEdges; ++iE, ++edgeIt) {
        ...
        for (; quad != thePrism.myWallQuads[iE].end(); ++quad) {
            ...
            if (!myHelper->LoadNodeColumns(faceColumns, (*quad)->face, quadBot, meshDS))
                return error(COMPERR_BAD_INPUT_MESH,
                             TCom("Can't find regular quadrangle mesh ")
        ...
        if (nbSides != NB_WALL_FACES) // define how to split
        {
            ...
            switch (nbEdges) {
                ...
                case 2: // either the longest edge is split into 3 parts, or both edges into halves
                    if (maxLen / 3 > midLen / 2) { nbSplitPerEdge[maxLen_i->second] = 3; }
                    else {
    ...
    else // **************************** Unite faces
    {
        ...
        for (iE = 0; iE < nbEdges; ++iE) {
            if (nbUnitePerEdge[iE] < 0) continue;
    ...
    for (iE = 0; iE < nbEdges; ++iE, ++botE) {
        ...
        if (nbSplit > 0) // split
        {
            ...
            const bool isForward = StdMeshers_PrismAsBlock::IsForwardEdge(
                myHelper->GetMeshDS(), myParam2ColumnMaps[iE], *botE, SMESH_Block::ID_Fx0z);
            for (int i = 0; i < nbSplit; ++i) {
                double f = (isForward ? params[i] : params[nbSplit - i - 1]);
                double l = (isForward ? params[i + 1] : params[nbSplit - i]);
                TSideFace *comp = new TSideFace(*mesh, wallFaceIds[iSide], thePrism.myWallQuads[iE],
                                                *botE, &myParam2ColumnMaps[iE], f, l);
        ...
        else if (nbExraFaces > 1) // unite
        {
            ...
            for (int i = iE; i < iE + nbExraFaces; ++i) sumLen += edgeLength[i];
            ...
            for (int i = 0; i < nbExraFaces; ++i, ++botE, ++iE) {
                ...
                components[i] = new TSideFace(*mesh, wallFaceIds[iSide], thePrism.myWallQuads[iE],
                                              *botE, &myParam2ColumnMaps[iE]);
    ...
    for (int iF = 0; iF < NB_WALL_FACES; ++iF) {
        ...
        if (!sideFace) RETURN_BAD_RESULT("NULL TSideFace");
        int fID = sideFace->FaceID(); // in-block ID

        // fill myShapeIDMap
        if (sideFace->InsertSubShapes(myShapeIDMap) != 8 && !sideFace->IsComplex())
            MESSAGE(": Warning : InsertSubShapes() < 8 on side " << iF);

        // side faces geometry
        Adaptor2d_Curve2d *pcurves[NB_WALL_FACES];
        if (!sideFace->GetPCurves(pcurves)) RETURN_BAD_RESULT("TSideFace::GetPCurves() failed");
    ...
    for (fListIt = fList.begin(); fListIt != fList.end(); ++fListIt) {
        ...
        for (int i = 0; i < nb; ++i) {
            if (TSideFace *comp = (*fListIt)->GetComponent(i)) fList.push_back(comp);
        }
...
const TNodeColumn *StdMeshers_PrismAsBlock::GetNodeColumn(const SMDS_MeshNode *node) const
{
    ...
    if (col_frw != myShapeIndex2ColumnMap.end()) {
        ...
        for (; u_col != cols->end(); ++u_col)
            if (u_col->second[0] == node) return &u_col->second;
    }
    return 0;
}
                                                      ...
                                                      const Prism_3D::TPrismTopo &prism) const
{
    ...
    vector<const TNodeColumn *> columns;
    {
        ...
        for (int iE = 0; iE < prism.myNbEdgesInWires.front(); ++iE, ++edgeIt) {
            ...
            for (int i = 0; i < nbCol; ++i) {
                ...
                if (columns.empty() || col != columns.back()) columns.push_back(col);
            }
        }
    }
    ...
    {
        Bnd_B3d bndBox;
        for (int i = 0; i < columns.size(); ++i) bndBox.Add(gpXYZ(columns[i]->front()));
...
bool StdMeshers_PrismAsBlock::IsForwardEdge(SMESHDS_Mesh *meshDS,
                                            ...
                                            const TopoDS_Edge &bottomEdge, const int sideFaceID)
{
    ...
    if (sideFaceID == ID_Fx1z || sideFaceID == ID_F0yz) isForward = !isForward;
    return isForward;
}
...
void StdMeshers_PrismAsBlock::faceGridToPythonDump(const SMESH_Block::TShapeID face, const int nb)
{
#ifdef _DEBUG_
    gp_XYZ pOnF[6] = {gp_XYZ(0, 0, 0), gp_XYZ(0, 0, 1), gp_XYZ(0, 0, 0),
                      gp_XYZ(0, 1, 0), gp_XYZ(0, 0, 0), gp_XYZ(1, 0, 0)};
    gp_XYZ p2;
    ...
    for (int j = 0; j <= nb; ++j) {
        ...
        for (int i = 0; i <= nb; ++i) {
            ...
            if (dist > 1e-4)
                cout << "#### dist from ShellPoint " << dist << " (" << p2.X() << ", " << p2.Y()
                     << ", " << p2.Z() << " ) " << endl;
        }
    }
    for (int j = 0; j < nb; ++j)
        for (int i = 0; i < nb; ++i) {
            int n = 1 + i + j * (nb + 1);
            cout << "mesh.AddFace([ " << n << ", " << n + 1 << ", " << n + nb + 2 << ", "
                 << n + nb + 1 << "]) " << endl;
        }
...
StdMeshers_PrismAsBlock::TSideFace::TSideFace(SMESH_Mesh &mesh, const int faceID,
                                              const Prism_3D::TQuadList &quadList,
                                              const TopoDS_Edge &baseEdge,
                                              TParam2ColumnMap *columnsMap, const double first,
                                              const double last)
    : myID(faceID), myParamToColumnMap(columnsMap), myHelper(mesh)
{
    myParams.resize(1);
    myParams[0] = make_pair(first, last);
    mySurface = PSurface(new BRepAdaptor_Surface(quadList.front()->face));
    myBaseEdge = baseEdge;
    myIsForward = StdMeshers_PrismAsBlock::IsForwardEdge(myHelper.GetMeshDS(), *myParamToColumnMap,
                                                         myBaseEdge, myID);
    ...
    if (quadList.size() > 1) // side is vertically composite
    {
        ...
        for (; quad != quadList.end(); ++quad) {
            ...
            myShapeID2Surf.insert(
                make_pair(meshDS->ShapeToIndex(face), PSurface(new BRepAdaptor_Surface(face))));
        }
...
StdMeshers_PrismAsBlock::TSideFace::TSideFace(SMESH_Mesh &mesh,
    ...
    : myID(components[0] ? components[0]->myID : 0), myParamToColumnMap(0), myParams(params),
      myIsForward(true), myComponents(components), myHelper(mesh)
{
...
StdMeshers_PrismAsBlock::TSideFace::TSideFace(const TSideFace &other)
    : myID(other.myID), myParamToColumnMap(other.myParamToColumnMap), mySurface(other.mySurface),
      myBaseEdge(other.myBaseEdge), myShapeID2Surf(other.myShapeID2Surf), myParams(other.myParams),
      myIsForward(other.myIsForward), myComponents(other.myComponents.size()),
...
{
    for (int i = 0; i < myComponents.size(); ++i)
        if (myComponents[i]) delete myComponents[i];
}
...
Adaptor3d_Curve *StdMeshers_PrismAsBlock::TSideFace::VertiCurve(const bool isMax) const
{
    if (!myComponents.empty()) {
        if (isMax) return myComponents.back()->VertiCurve(isMax);
        else
...
bool StdMeshers_PrismAsBlock::TSideFace::GetPCurves(Adaptor2d_Curve2d *pcurv[4]) const
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        switch (iEdge[i]) {
            case TOP_EDGE: line = new Geom2d_Line(gp_Pnt2d(0, 1), gp::DX2d()); break;
            case BOTTOM_EDGE: line = new Geom2d_Line(gp::Origin2d(), gp::DX2d()); break;
            case V0_EDGE: line = new Geom2d_Line(gp::Origin2d(), gp::DY2d()); break;
            case V1_EDGE: line = new Geom2d_Line(gp_Pnt2d(1, 0), gp::DY2d()); break;
        }
...
{
    localU = U;
    if (myComponents.empty()) return const_cast<TSideFace *>(this);

    int i;
    for (i = 0; i < myComponents.size(); ++i)
        if (U < myParams[i].second) break;
    if (i >= myComponents.size()) i = myComponents.size() - 1;
...
double StdMeshers_PrismAsBlock::TSideFace::GetColumns(const double U, TParam2ColumnIt &col1,
...
void StdMeshers_PrismAsBlock::TSideFace::GetNodesAtZ(
    const int Z, map<double, const SMDS_MeshNode *> &nodes) const
{
    if (!myComponents.empty()) {
        double u0 = 0.;
        for (size_t i = 0; i < myComponents.size(); ++i) {
            ...
            if (!nodes.empty() && nodes.rbegin()->second == u2n->second) ++u2n;
        ...
        if (Abs(uRange) < std::numeric_limits<double>::min()) return;
        TParam2ColumnIt u2col = getColumn(myParamToColumnMap, myParams[0].first + 1e-3);
        for (; u2col != myParamToColumnMap->end(); ++u2col)
            if (u2col->first > myParams[0].second + 1e-9) break;
            else
                nodes.insert(nodes.end(), make_pair((u2col->first - f) / uRange, u2col->second[Z]));
    }
}
...
gp_Pnt StdMeshers_PrismAsBlock::TSideFace::Value(const Standard_Real U, const Standard_Real V) const
{
    if (!myComponents.empty()) {
        double u;
        TSideFace *comp = GetComponent(U, u);
    ...
    if (V < tol || V + tol >= 1.) {
        ...
        if (V < tol) { edge = myBaseEdge; }
            ...
            if (!s.IsNull() && s.ShapeType() == TopAbs_EDGE) edge = TopoDS::Edge(s);
        }
        if (!edge.IsNull()) {
...
TopoDS_Edge StdMeshers_PrismAsBlock::TSideFace::GetEdge(const int iEdge) const
{
    ...
    switch (iEdge) {
        ...
        case V1_EDGE: {
            ...
            if (back) column = &(myParamToColumnMap->rbegin()->second);
            else
                column = &(myParamToColumnMap->begin()->second);
            if (column->size() > 0) edge = myHelper.GetSubShapeByNode((*column)[1], meshDS);
            if (edge.IsNull() || edge.ShapeType() == TopAbs_VERTEX) node = column->front();
            break;
        }
        default:;
    }
    if (!edge.IsNull() && edge.ShapeType() == TopAbs_EDGE) return TopoDS::Edge(edge);
    ...
    if (!V2.IsNull() && V2.ShapeType() == TopAbs_VERTEX && !V2.IsSame(V1)) {
        TopoDS_Shape ancestor =
            myHelper.GetCommonAncestor(V1, V2, *myHelper.GetMesh(), TopAbs_EDGE);
        if (!ancestor.IsNull()) return TopoDS::Edge(ancestor);
    }
    return TopoDS_Edge();
}
...
int StdMeshers_PrismAsBlock::TSideFace::InsertSubShapes(TBlockShapes &shapeMap) const
{
    ...
    for (int i = BOTTOM_EDGE; i <= V1_EDGE; ++i) {
        ...
        if (!e.IsNull()) { nbInserted += SMESH_Block::Insert(e, edgeIdVec[i], shapeMap); }
    }
...
StdMeshers_PrismAsBlock::TVerticalEdgeAdaptor::TVerticalEdgeAdaptor(
    const TParam2ColumnMap *columnsMap, const double parameter)
{
...
void StdMeshers_PrismAsBlock::TVerticalEdgeAdaptor::dumpNodes(int nbNodes) const
{
    ...
    if (nbNodes < myNodeColumn->size()) cout << myNodeColumn->back()->GetID();
#endif
}
...
void StdMeshers_PrismAsBlock::THorizontalEdgeAdaptor::dumpNodes(int nbNodes) const
{
    ...
    double u = 0;
    if (mySide->IsComplex()) side = mySide->GetComponent(0, u);
    ...
    // last node
    u = 1;
    if (mySide->IsComplex()) side = mySide->GetComponent(1, u);

    side->GetColumns(u, col, col2);
    if (n != col->second[i]) cout << col->second[i]->GetID();
#endif
}
...
StdMeshers_PrismAsBlock::TPCurveOnHorFaceAdaptor::TPCurveOnHorFaceAdaptor(
    const TSideFace *sideFace, const bool isTop, const TopoDS_Face &horFace)
{
    if (sideFace && !horFace.IsNull()) {
        ...
        for (; u2n != u2nodes.end(); ++u2n) {
            ...
            if (n->GetPosition()->GetTypeOfPosition() == SMDS_TOP_EDGE) {
                ...
                if (!C2d.IsNull()) {
                    double u =
                        static_cast<const SMDS_EdgePosition *>(n->GetPosition())->GetUParameter();
                    if (f <= u && u <= l) {
                        uv = C2d->Value(u).XY();
            ...
            if (!okUV) uv = helper.GetNodeUV(horFace, n, prevNode, &okUV);
...
gp_Pnt2d StdMeshers_PrismAsBlock::TPCurveOnHorFaceAdaptor::Value(const Standard_Real U) const
{
    ...
    if (i1 == myUVmap.end()) return myUVmap.empty() ? gp_XY(0, 0) : myUVmap.rbegin()->second;

    if (i1 == myUVmap.begin()) return (*i1).second;
...
bool StdMeshers_Sweeper::projectIntPoints(const vector<gp_XYZ> &fromBndPoints,
                                          ...
                                          NSProjUtils::TrsfFinder3D &trsf, vector<gp_XYZ> *bndError)
{
    // find transformation
    if (trsf.IsIdentity() && !trsf.Solve(fromBndPoints, toBndPoints)) return false;
...
void StdMeshers_Sweeper::applyBoundaryError(const vector<gp_XYZ> &bndPoints,
                                            ...
                                            const vector<gp_XYZ> &bndError2, const double r,
                                            vector<gp_XYZ> &intPoints, vector<double> &int2BndDist)
{
...
bool StdMeshers_Sweeper::ComputeNodes(SMESH_MesherHelper &helper, const double tol,
                                      const bool allowHighBndError)
{
    ...
    for (; zS < zT; ++zS, --zT) // vertical loop on layers
    {
        ...
        if (!projectIntPoints(fromSrcBndPnts, toSrcBndPnts, intPntsOfLayer[zS - 1],
                              intPntsOfLayer[zS], trsfOfLayer[zS - 1], &bndError[zS - 1]))
            return false;
        if (!projectIntPoints(fromTgtBndPnts, toTgtBndPnts, intPntsOfLayer[zT + 1],
                              intPntsOfLayer[zT], trsfOfLayer[zT + 1], &bndError[zT + 1]))
            return false;
    ...
    if (!projectIntPoints(fromSrcBndPnts, toSrcBndPnts, intPntsOfLayer[zS - 1], centerSrcIntPnts,
                          trsfOfLayer[zS - 1], &bndError[zS - 1]))
        return false;
    if (!projectIntPoints(fromTgtBndPnts, toTgtBndPnts, intPntsOfLayer[zT + 1], centerTgtIntPnts,
    ...
    for (size_t iP = 0; (iP < myBndColumns.size() && bndErrorIsSmall); ++iP) {
        ...
        for (size_t z = 1; z < zS; ++z) // loop on layers
            sumError += (bndError[z - 1][iP].Modulus() + bndError[zSize - z][iP].Modulus());

        bndErrorIsSmall = (sumError < tol);
    }

    if (!bndErrorIsSmall && !allowHighBndError) return false;
    ...
    if (zS == zT) {
        for (size_t iP = 0; iP < myIntColumns.size(); ++iP) {
            intPntsOfLayer[zS][iP] = (1 - r) * centerSrcIntPnts[iP] + r * centerTgtIntPnts[iP];
        }
        if (!bndErrorIsSmall) {
        ...
        for (size_t iP = 0; iP < myIntColumns.size(); ++iP) {
            intPntsOfLayer[zS][iP] = r * intPntsOfLayer[zS][iP] + (1 - r) * centerSrcIntPnts[iP];
            intPntsOfLayer[zT][iP] = r * intPntsOfLayer[zT][iP] + (1 - r) * centerTgtIntPnts[iP];
        }
        if (!bndErrorIsSmall) {
            applyBoundaryError(toSrcBndPnts, bndError[zS - 1], bndError[zS + 1], r,
                               intPntsOfLayer[zS], int2BndDist);
    ...
    if (!centerIntErrorIsSmall) {
        ...
        for (++zS, --zT; zS < zTgt; ++zS, --zT) // vertical loop on layers
        {
            ...
            if (!trsfOfLayer[zT - 1].Invert()) trsfOfLayer[zT - 1] = NSProjUtils::TrsfFinder3D();
            ...
            projectIntPoints(fromSrcBndPnts, toSrcBndPnts, fromSrcIntPnts, toSrcIntPnts,
                             trsfOfLayer[zS + 1], &srcBndError);
            projectIntPoints(fromTgtBndPnts, toTgtBndPnts, fromTgtIntPnts, toTgtIntPnts,
    ...
    for (size_t iP = 0; iP < myIntColumns.size(); ++iP) {
        ...
        for (size_t z = zSrc + 1; z < zTgt; ++z) // vertical loop on layers
        {
            ...
            if (!(nodeCol[z] = helper.AddNode(xyz.X(), xyz.Y(), xyz.Z()))) return false;
        }
    }

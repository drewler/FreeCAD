...
bool StdMeshers_Cartesian_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
struct LineIndexer {
    ...
    LineIndexer() {}
    LineIndexer(size_t sz1, size_t sz2, size_t sz3, size_t iv1, size_t iv2, size_t iConst,
    ...
    void operator++()
    {
        if (++_curInd[_iVar1] == _size[_iVar1]) _curInd[_iVar1] = 0, ++_curInd[_iVar2];
    }
    ...
    {
        return (_curInd[_iVar1] + 1) + (_curInd[_iVar2] + 1) * _size[_iVar1];
    }
...
struct Grid {
    ...
    {
        return i + j * (_coords[0].size() - 1)
            + k * (_coords[0].size() - 1) * (_coords[1].size() - 1);
    }
    ...
    void SetCoordinates(const vector<double> &xCoords, const vector<double> &yCoords,
                        const vector<double> &zCoords, const double *axesDirs,
...
struct FaceGridIntersector {
    ...
    IntCurvesFace_Intersector *GetCurveFaceIntersector()
    {
        if (!_surfaceInt) {
            ...
            if (_bndBox.IsVoid()) BRepBndLib::Add(_face, _bndBox);
        }
        return _surfaceInt;
    }
#ifdef WITH_TBB
...
struct FaceLineIntersector {
    ...
    bool isParamOnLineOK(const double linLength) { return -_tol < _w && _w < linLength + _tol; }
    FaceLineIntersector() : _surfaceInt(0) {}
    ~FaceLineIntersector()
    {
        if (_surfaceInt) delete _surfaceInt;
        _surfaceInt = 0;
    }
};
// --------------------------------------------------------------------------
...
class Hexahedron
{
    ...
    struct _Node //!< node either at a hexahedron corner or at intersection
    {
        ...
        gp_Pnt Point() const
        {
            if (const SMDS_MeshNode *n = Node()) return SMESH_TNodeXYZ(n);
    ...
    struct _OrientedLink {
        ...
        vector<TGeomID>
        GetNotUsedFace(const set<TGeomID> &usedIDs) const // returns supporting FACEs
        {
            vector<TGeomID> faces;
            const B_IntersectPoint *ip0, *ip1;
            if ((ip0 = _link->_nodes[0]->_intPoint) && (ip1 = _link->_nodes[1]->_intPoint)) {
                for (size_t i = 0; i < ip0->_faceIDs.size(); ++i)
                    if (ip1->IsOnFace(ip0->_faceIDs[i]) && !usedIDs.count(ip0->_faceIDs[i]))
        ...
        int NbFaces() const { return !_link->_faces[0] ? 0 : 1 + bool(_link->_faces[1]); }
        void AddFace(_Face *f)
        {
            if (_link->_faces[0]) { _link->_faces[1] = f; }
            else {
                _link->_faces[0] = f;
                _link->_faces[1] = 0;
            }
        ...
        void RemoveFace(_Face *f)
        {
            ...
            if (_link->_faces[1] == f) { _link->_faces[1] = 0; }
    ...
    struct _volumeDef // holder of nodes of a volume mesh element
    {
        ...
        void set(_Node **nodes, int nb) { _nodes.assign(nodes, nodes + nb); }
    };
...
private:
    ...
    void init(size_t i);
    void addEdges(SMESH_MesherHelper &helper, vector<Hexahedron *> &intersectedHex,
                  const map<TGeomID, vector<TGeomID>> &edge2faceIDsMap);
    gp_Pnt findIntPoint(double u1, double proj1, double u2, double proj2, double proj,
                        BRepAdaptor_Curve &curve, const gp_XYZ &axis, const gp_XYZ &origin);
    int getEntity(const E_IntersectPoint *ip, int *facets, int &sub);
    bool addIntersection(const E_IntersectPoint &ip, vector<Hexahedron *> &hexes, int ijk[],
                         int dIJK[]);
    ...
    bool findChainOnEdge(const vector<_OrientedLink> &splits, const _OrientedLink &prevSplit,
                         const _OrientedLink &avoidSplit, size_t &iS, _Face &quad,
                         vector<_Node *> &chn);
    ...
    _Node *findEqualNode(vector<_Node *> &nodes, const E_IntersectPoint *ip, const double tol2)
    {
...
struct ParallelHexahedron {
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const
    {
        for (size_t i = r.begin(); i != r.end(); ++i)
            if (Hexahedron *hex = _hexVec[i]) hex->ComputeElements();
    }
};
// --------------------------------------------------------------------------
/*!
...
struct ParallelIntersector {
    ...
    void operator()(const tbb::blocked_range<size_t> &r) const
    {
        for (size_t i = r.begin(); i != r.end(); ++i) _faceVec[i].Intersect();
    }
};
#endif
...
/*!
   ...
   */
inline void locateValue(int &i, double val, const vector<double> &values, int &di, double tol)
{
    //val += values[0]; // input \a val is measured from 0.
    if (i > values.size() - 2) i = values.size() - 2;
    else
        while (i + 2 < values.size() && val > values[i + 1]) ++i;
    while (i > 0 && val < values[i]) --i;

    if (i > 0 && val - values[i] < tol) di = -1;
...
void GridLine::RemoveExcessIntPoints(const double tol)
{
    ...
    while (ip2 != _intPoints.end()) {
        ...
        if (tranSet.size() > 1) // points with different transition coincide
        {
            ...
            if (isIN && isOUT) (*ip1)._transition = Trans_TANGENT;
            else
...
{
    if (ip->_transition == Trans_IN) return true;
    if (ip->_transition == Trans_OUT) return false;
    if (ip->_transition == Trans_APEX) {
        // singularity point (apex of a cone)
        if (_intPoints.size() == 1 || ip == _intPoints.begin()) return true;
        multiset<F_IntersectPoint>::iterator ipBef = ip, ipAft = ++ip;
        if (ipAft == _intPoints.end()) return false;
        --ipBef;
        if (ipBef->_transition != ipAft->_transition) return (ipBef->_transition == Trans_OUT);
...
/*
   * Adds face IDs
   */
void B_IntersectPoint::Add(const vector<TGeomID> &fIDs, const SMDS_MeshNode *n) const
{
    if (_faceIDs.empty()) _faceIDs = fIDs;
    else
        for (size_t i = 0; i < fIDs.size(); ++i) {
            vector<TGeomID>::iterator it = std::find(_faceIDs.begin(), _faceIDs.end(), fIDs[i]);
            if (it == _faceIDs.end()) _faceIDs.push_back(fIDs[i]);
        }
    if (!_node) _node = n;
}
//================================================================================
...
int B_IntersectPoint::HasCommonFace(const B_IntersectPoint *other, int avoidFace) const
{
    if (other)
        for (size_t i = 0; i < other->_faceIDs.size(); ++i)
            if (avoidFace != other->_faceIDs[i] && IsOnFace(other->_faceIDs[i]))
...
{
    vector<TGeomID>::const_iterator it = std::find(_faceIDs.begin(), _faceIDs.end(), faceID);
...
LineIndexer Grid::GetLineIndexer(size_t iDir) const
{
    ...
    LineIndexer li(_coords[0].size(), _coords[1].size(), _coords[2].size(), indices[iDir * 3],
                   indices[iDir * 3 + 1], indices[iDir * 3 + 2], s[indices[iDir * 3]],
                   s[indices[iDir * 3 + 1]], s[indices[iDir * 3 + 2]]);
    return li;
}
//=============================================================================
/*
   ...
   */
void Grid::SetCoordinates(const vector<double> &xCoords, const vector<double> &yCoords,
                          const vector<double> &zCoords, const double *axesDirs,
                          const Bnd_Box &shapeBox)
{
    ...
    _axes[0].SetCoord(axesDirs[0], axesDirs[1], axesDirs[2]);
    _axes[1].SetCoord(axesDirs[3], axesDirs[4], axesDirs[5]);
    _axes[2].SetCoord(axesDirs[6], axesDirs[7], axesDirs[8]);
    _axes[0].Normalize();
    _axes[1].Normalize();
    _axes[2].Normalize();
    ...
    for (int iDir = 0; iDir < 3; ++iDir) // loop on 3 line directions
    {
        for (size_t i = 1; i < _coords[iDir].size(); ++i) {
            ...
            if (cellLen < _minCellSize) _minCellSize = cellLen;
        }
    }
    ...
    for (int i = 0; i < 6; ++i)
        if (fabs(sP[i] - *cP[i]) < _tol) *cP[i] = sP[i]; // + _tol/1000. * ( i < 3 ? +1 : -1 );
    ...
    _origin = (_coords[0][0] * _axes[0] + _coords[1][0] * _axes[1] + _coords[2][0] * _axes[2]);
    ...
    for (int iDir = 0; iDir < 3; ++iDir) // loop on 3 line directions
    {
        ...
        for (; li.More(); ++li) {
            ...
            gl._line.SetLocation(_coords[0][li.I()] * _axes[0] + _coords[1][li.J()] * _axes[1]
            ...
            for (; ip != intPnts.end(); ++ip) {
                ...
                if (nodeParam < ip->_paramOnLine - _tol) {
                    ...
                    while (nodeParam < ip->_paramOnLine - _tol) {
                        if (isOut) isNodeOut[nIndex0 + nShift * (nodeCoord - coord0)] = isOut;
                        if (++nodeCoord < coordEnd) nodeParam = *nodeCoord - *coord0;
                        else
                            break;
                    }
                ...
                // create a mesh node at ip concident with a grid node
                    ...
                    if (_gridIntP[nodeIndex]) _gridIntP[nodeIndex]->Add(ip->_faceIDs);
                    else
                        _gridIntP[nodeIndex] = &*ip;
        ...
        for (size_t y = 0; y < _coords[1].size(); ++y)
            for (size_t x = 0; x < _coords[0].size(); ++x) {
                ...
                if (!isNodeOut[nodeIndex] && !_nodes[nodeIndex]) {
                    ...
                    gp_XYZ xyz = (_coords[0][x] * _axes[0] + _coords[1][y] * _axes[1]
                                  + _coords[2][z] * _axes[2]);
                    _nodes[nodeIndex] = helper.AddNode(xyz.X(), xyz.Y(), xyz.Z());
                }
            }
        ...
        for (; li.More(); ++li) {
            ...
            if (intPnts.size() == 1) {
                if (intPnts.begin()->_transition != Trans_TANGENT
                    ...
                    throw SMESH_ComputeError(
                        COMPERR_ALGO_FAILED,
                        SMESH_Comment("Wrong SOLE transition of GridLine (")
                            << li._curInd[li._iVar1] << ", " << li._curInd[li._iVar2] << ") along "
                            << li._nameConst << ": " << trName[intPnts.begin()->_transition]);
            }
            else {
                ...
                if (intPnts.rbegin()->_transition == Trans_IN)
                    throw SMESH_ComputeError(
                        ...
                        SMESH_Comment("Wrong END transition of GridLine (")
                            << li._curInd[li._iVar1] << ", " << li._curInd[li._iVar2] << ") along "
                            << li._nameConst << ": " << trName[intPnts.rbegin()->_transition]);
            }
        }
    }
#endif
...
void FaceGridIntersector::Intersect()
{
    ...
    switch (surf.GetType()) {
        ...
        case GeomAbs_Torus:
            ...
            break;
        default: interFunction = &FaceLineIntersector::IntersectWithSurface;
    }
    if (!isDirect) std::swap(intersector._transOut, intersector._transIn);
...
void FaceLineIntersector::IntersectWithCylinder(const GridLine &gridLine)
{
    ...
    if (linCylinder.IsDone() && linCylinder.NbPoints() > 0) {
        ...
        if (linCylinder.NbPoints() == 1) _transition = Trans_TANGENT;
        else
    ...
    for (int i = 1; i <= linCone.NbPoints(); ++i) {
        ...
        if (UVIsOnFace()) {
            ...
            if (normSize2 > Precision::Angular() * Precision::Angular()) {
                ...
                cos /= sqrt(normSize2);
                if (cos < -Precision::Angular()) _transition = _transIn;
...
void FaceLineIntersector::IntersectWithSphere(const GridLine &gridLine)
{
    ...
    if (linSphere.IsDone() && linSphere.NbPoints() > 0) {
        ...
        if (linSphere.NbPoints() == 1) _transition = Trans_TANGENT;
        else
    ...
    : _grid(grid), _sizeThreshold(sizeThreshold), _nbFaceIntNodes(0)
{
    ...
    for (int faceID = SMESH_Block::ID_Fxy0; faceID <= SMESH_Block::ID_F1yz; ++faceID) {
        ...
        bool revFace = (faceID == SMESH_Block::ID_Fxy0 || faceID == SMESH_Block::ID_Fx1z
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            link = _OrientedLink(&_hexLinks[SMESH_Block::ShapeIndex(idVec[interlace[i]])], revLink);
        }
    }
}
//================================================================================
    ...
    : _grid(other._grid), _sizeThreshold(other._sizeThreshold), _nbFaceIntNodes(0)
{
    ...
    for (int i = 0; i < 8; ++i) _nodeShift[i] = other._nodeShift[i];
                ...
                if (checkTransition) {
                    if (link._fIntPoints[i]->_faceIDs.size() > 1 || _eIntPoints.size() > 0)
                        ...
                        switch (link._fIntPoints[i]->_transition) {
                            ...
                            default: isOut = isOutPoint(link, i, helper);
                        }
                }
            }
        ...
        for (size_t iP = 0; iP < _eIntPoints.size(); ++iP) {
            ...
            switch (nbFacets) {
                case 1: // in a _Face
                {
                    ...
                    if (equalNode) { equalNode->Add(_eIntPoints[iP]); }
                    else {
...
void Hexahedron::ComputeElements()
{
    ...
    if (_nbCornerNodes + nbIntersections < 4) return;

    if (_nbBndNodes == _nbCornerNodes && nbIntersections == 0 && isInHole()) return;
    ...
    for (int iF = 0; iF < 6; ++iF) // loop on 6 sides of a hexahedron
    {
        ...
        for (size_t iP = 0; iP < quad._eIntNodes.size(); ++iP)
            if (quad._eIntNodes[iP]->IsUsedInFace(polygon)) quad._eIntNodes[iP]->_usedInFace = 0;
#endif
        ...
        while (nbSplits > 0) {
            ...
            for (; nFirst != n2 && iS < splits.size(); ++iS) {
                ...
                n1 = split.FirstNode();
                if (n1 == n2 && n1->_intPoint && n1->_intPoint->_faceIDs.size() > 1) {
                    // n1 is at intersection with EDGE
                    if (findChainOnEdge(splits, polygon->_links.back(), split, iS, quad,
                                        chainNodes)) {
                    ...
                    // try to connect to a split ending on the same FACE
                        ...
                        if (foundSplit) {
                            ...
                            if (n2->IsLinked(nFirst->_intPoint)) break;
            ...
            if (nFirst != n2) // close a polygon
            {
                if (!findChain(n2, nFirst, quad, chainNodes)) {
                    if (!closePolygon(polygon, chainNodes))
                        if (!isImplementEdges()) chainNodes.push_back(nFirst);
                }
                for (size_t i = 1; i < chainNodes.size(); ++i) {
                    polygon->AddPolyLink(chainNodes[i - 1], chainNodes[i], prevPolyg);
        ...
        if (polygon->_links.size() < 3) { _polygons.pop_back(); }
    ...
    // clear _usedInFace
    for (size_t iN = 0; iN < _intNodes.size(); ++iN) _intNodes[iN]._usedInFace = 0;
    ...
    for (size_t iP = 0; iP < _polygons.size(); ++iP) {
        _Face &polygon = _polygons[iP];
        for (size_t iL = 0; iL < polygon._links.size(); ++iL)
            if (polygon._links[iL].NbFaces() < 2) freeLinks.push_back(&polygon._links[iL]);
    }
    ...
    while (nbFreeLinks > 0) {
        ...
        _Node *curNode;
        if ((!hasEdgeIntersections) || (nbFreeLinks < 4 && nbVertexNodes == 0)) {
            // get a remaining link to start from
            for (size_t iL = 0; iL < freeLinks.size() && !curLink; ++iL)
                if ((curLink = freeLinks[iL])) freeLinks[iL] = 0;
        ...
        else // there are intersections with EDGEs
        {
            // get a remaining link to start from, one lying on minimal nb of FACEs
            {
                ...
                for (size_t iL = 0; iL < freeLinks.size(); ++iL)
                    if (freeLinks[iL]) {
                        ...
                        if (faces.size() == 1) {
                            ...
                            if (!freeLinks[iL]->HasEdgeNodes()) break;
                ...
                for (int i = 0; faceOfLink.first < 0 && i < 3; ++i) faceOfLink = facesOfLink[i];
            ...
            do {
                ...
                curLink = 0;
                for (size_t iL = 0; iL < freeLinks.size() && !curLink; ++iL)
                    if (freeLinks[iL] && freeLinks[iL]->FirstNode() == curNode
            ...
            if (polygon._links[0].LastNode() != curNode) {
                if (nbVertexNodes > 0) {
                    ...
                    chainNodes.clear();
                    for (size_t iN = 0; iN < _vIntNodes.size(); ++iN)
                        if (!_vIntNodes[iN]->IsUsedInFace() && _vIntNodes[iN]->IsOnFace(curFace)) {
                            _vIntNodes[iN]->_usedInFace = &polygon;
                            chainNodes.push_back(_vIntNodes[iN]);
                        }
                    if (chainNodes.size() > 1) { sortVertexNodes(chainNodes, curNode, curFace); }
        ...
        if (polygon._links.size() == 2) {
            ...
            if (iPolygon == _polygons.size() - 1) _polygons.pop_back();
        }
        else // polygon._links.size() >= 2
        {
            ...
            if (/*hasEdgeIntersections &&*/ iPolygon == _polygons.size() - 1) {
                ...
                for (size_t iL = 0; iL < polygon._links.size() && !coplanarPolyg; ++iL) {
                    if (polygon._links[iL].NbFaces() < 2) continue; // it's a just added free link
                    ...
                    for (iL2 = iL + 1; iL2 < polygon._links.size(); ++iL2)
                        if (polygon._links[iL2]._link->_faces[0] == coplanarPolyg
                            ...
                            break;
                    if (iL2 == polygon._links.size()) coplanarPolyg = 0;
                }
                if (coplanarPolyg) // coplanar polygon found
                {
                    ...
                    for (size_t iL = 0; iL < coplanarPolyg->_links.size(); ++iL)
                        if (coplanarPolyg->_links[iL]._link->_faces[1]
                            ...
                            if (p == coplanarPolyg) p = coplanarPolyg->_links[iL]._link->_faces[1];
                            for (size_t iL2 = 0; iL2 < p->_links.size(); ++iL2)
                                if (p->_links[iL2]._link == coplanarPolyg->_links[iL]._link) {
                                    ...
                                    for (; iL3 < freeLinks.size() && freeLinks[iL3]; ++iL3)
                                        ;
                                    if (iL3 < freeLinks.size()) freeLinks[iL3] = (&p->_links[iL2]);
                                    else
                                        freeLinks.push_back(&p->_links[iL2]);
                                    ++nbFreeLinks;
    ...
    if (!checkPolyhedronSize()) { return; }

    for (size_t i = 0; i < 8; ++i)
        if (_hexNodes[i]._intPoint == &ipTmp) _hexNodes[i]._intPoint = 0;
    ...
    int nbNodes = 0;
    for (size_t i = 0; i < 8; ++i) nbNodes += _hexNodes[i].IsUsedInFace();
    for (size_t i = 0; i < _intNodes.size(); ++i) nbNodes += _intNodes[i].IsUsedInFace();

    bool isClassicElem = false;
    if (nbNodes == 8 && nbPolygons == 6) isClassicElem = addHexa();
    else if (nbNodes == 4 && nbPolygons == 4)
        isClassicElem = addTetra();
    else if (nbNodes == 6 && nbPolygons == 5)
        isClassicElem = addPenta();
    else if (nbNodes == 5 && nbPolygons == 5)
        isClassicElem = addPyra();
    if (!isClassicElem) {
                             ...
                             const map<TGeomID, vector<TGeomID>> &edge2faceIDsMap)
{
    ...
    size_t nbCells[3] = {_grid->_coords[0].size() - 1, _grid->_coords[1].size() - 1,
    ...
    for (int iDir = 0; iDir < 3; ++iDir) {
        ...
        dInd[3][iDirOther[iDir][0]] = -1;
        dInd[3][iDirOther[iDir][1]] = -1;
        ...
        for (; lineInd.More(); ++lineInd) {
            ...
            for (; ip != line._intPoints.end(); ++ip) {
                ...
                for (int iL = 0; iL < 4; ++iL) // loop on 4 cells sharing a link
                {
                    ...
                    if (i < 0 || i >= nbCells[0] || j < 0 || j >= nbCells[1] || k < 0
                        || k >= nbCells[2])
                        continue;
    ...
    for (size_t i = 0; i < allHexa.size(); ++i) {
        ...
        if ((_nbCornerNodes == 8) && (_nbBndNodes < _nbCornerNodes || !isInHole())) {
            ...
            SMDS_MeshElement *el = mesh->AddVolume(
                _hexNodes[0].Node(), _hexNodes[2].Node(), _hexNodes[3].Node(), _hexNodes[1].Node(),
                _hexNodes[4].Node(), _hexNodes[6].Node(), _hexNodes[7].Node(), _hexNodes[5].Node());
...
#ifdef WITH_TBB
    tbb::parallel_for(tbb::blocked_range<size_t>(0, intHexa.size()), ParallelHexahedron(intHexa),
                      tbb::simple_partitioner()); // ComputeElements() is called here
    for (size_t i = 0; i < intHexa.size(); ++i)
        if (Hexahedron *hex = intHexa[i]) nbAdded += hex->addElements(helper);
#else
    for (size_t i = 0; i < intHexa.size(); ++i)
...
/*!
   ...
   */
void Hexahedron::addEdges(SMESH_MesherHelper &helper, vector<Hexahedron *> &hexes,
                          const map<TGeomID, vector<TGeomID>> &edge2faceIDsMap)
{
    ...
    for (; e2fIt != edge2faceIDsMap.end(); ++e2fIt) {
        ...
        if (!discret.IsDone() || discret.NbPoints() < 2) continue;
        ...
        for (int iDirZ = 0; iDirZ < 3; ++iDirZ) {
            ...
            double o[3] = {_grid->_coords[0][0], _grid->_coords[1][0], _grid->_coords[2][0]};
            ...
            for (int iP = 2; iP <= discret.NbPoints(); ++iP) {
                ...
                if (Abs(zProj2 - zProj1) > std::numeric_limits<double>::min()) {
                    ...
                    for (int i = 0, nb = Abs(iZ1 - iZ2); i < nb; ++i, iZ += dZ) {
                        ip._point = findIntPoint(u1, zProj1, u2, zProj2, planes._zProjs[iZ], curve,
                                                 planes._zNorm, _grid->_origin);
                        _grid->ComputeUVW(ip._point.XYZ(), ip._uvw);
                        locateValue(ijk[iDirX], ip._uvw[iDirX], _grid->_coords[iDirX], dIJK[iDirX],
                                    tol);
                        locateValue(ijk[iDirY], ip._uvw[iDirY], _grid->_coords[iDirY], dIJK[iDirY],
                                    tol);
                        ijk[iDirZ] = iZ;
                        ...
                        ijk[iDirZ] = iZ - 1;
                        if (!addIntersection(_grid->_edgeIntP.back(), hexes, ijk, dIJK) && !added)
                            _grid->_edgeIntP.pop_back();
                    }
                }
                iZ1 = iZ2;
...
/*!
   ...
   */
gp_Pnt Hexahedron::findIntPoint(double u1, double proj1, double u2, double proj2, double proj,
                                BRepAdaptor_Curve &curve, const gp_XYZ &axis, const gp_XYZ &origin)
{
    ...
    if (Abs(proj - newProj) > _grid->_tol / 10.) {
        if (r > 0.5) return findIntPoint(u2, proj2, u, newProj, proj, curve, axis, origin);
        else
...
int Hexahedron::getEntity(const E_IntersectPoint *ip, int *facets, int &sub)
{
    ...
    switch (nbFacets) {
        ...
        case 2: {
            const int edge[3][8] = {{SMESH_Block::ID_E00z, SMESH_Block::ID_E10z,
        ...
        //case 3:
        default: sub = vertex + SMESH_Block::ID_FirstV;
    }
...
/*!
   ...
   */
bool Hexahedron::addIntersection(const E_IntersectPoint &ip, vector<Hexahedron *> &hexes, int ijk[],
                                 int dIJK[])
{
    bool added = false;

    size_t hexIndex[4] = {_grid->CellIndex(ijk[0], ijk[1], ijk[2]),
...
#ifdef _DEBUG_
            if (h->isOutParam(ip._uvw)) throw SALOME_Exception("ip outside a hex");
#endif
...
/*!
   ...
   */
bool Hexahedron::findChain(_Node *n1, _Node *n2, _Face &quad, vector<_Node *> &chn)
{
    chn.clear();
    chn.push_back(n1);
    for (size_t iP = 0; iP < quad._eIntNodes.size(); ++iP)
    ...
    if (chn.back() != n2 && chn.back()->IsLinked(n2->_intPoint)) chn.push_back(n2);
...
bool Hexahedron::closePolygon(_Face *polygon, vector<_Node *> &chainNodes) const
{
    ...
    for (i = nbLinks - 2; i >= 0; --i)
        if (node->IsLinked(polygon->_links[i].FirstNode()->_intPoint, avoidFace)) break;
    if (i >= 0) {
        for (; i < nbLinks; ++i) newLinks.push_back(polygon->_links[i]);
    }
    else {
        ...
        for (i = 1; i < nbLinks; ++i)
            if (node->IsLinked(polygon->_links[i].LastNode()->_intPoint, avoidFace)) break;
        if (i < nbLinks)
            for (nbLinks = i + 1, i = 0; i < nbLinks; ++i) newLinks.push_back(polygon->_links[i]);
    }
    if (newLinks.size() > 1) {
...
bool Hexahedron::findChainOnEdge(const vector<_OrientedLink> &splits,
                                 const _OrientedLink &prevSplit, const _OrientedLink &avoidSplit,
                                 size_t &iS, _Face &quad, vector<_Node *> &chn)
{
    if (!isImplementEdges()) return false;

    _Node *pn1 = prevSplit.FirstNode();
    _Node *pn2 = prevSplit.LastNode();
    int avoidFace = pn1->IsLinked(pn2->_intPoint); // FACE under the quad
    if (avoidFace < 1 && pn1->_intPoint) return false;
    ...
    for (i = splits.size() - 1; i >= 0; --i) {
        ...
        n = splits[i].LastNode();
        if (n == stopNode) break;
        if ((n != pn1) && (n->IsLinked(pn2->_intPoint, avoidFace))
            && (!avoidFace || n->IsOnFace(avoidFace)))
            break;

        n = splits[i].FirstNode();
        if (n == stopNode) break;
        if ((n->IsLinked(pn2->_intPoint, avoidFace)) && (!avoidFace || n->IsOnFace(avoidFace)))
            break;
        n = 0;
    }
    if (n && n != stopNode) {
        if (chn.empty()) chn.push_back(pn2);
        chn.push_back(n);
        iS = i - 1;
        return true;
    }
...
bool Hexahedron::isOutPoint(_Link &link, int iP, SMESH_MesherHelper &helper) const
{
    ...
    if (!n1->Node()) n1 = link._nodes[0];
    _Node *n2 = moreIntPoints ? link._fIntNodes[iP + 1] : 0;
    if (!n2 || !n2->Node()) n2 = link._nodes[1];
    if (!n2->Node()) return true;

    // get all FACEs under n1 and n2
    set<TGeomID> faceIDs;
    if (moreIntPoints)
        faceIDs.insert(link._fIntPoints[iP + 1]->_faceIDs.begin(),
                       link._fIntPoints[iP + 1]->_faceIDs.end());
    if (n2->_intPoint)
        faceIDs.insert(n2->_intPoint->_faceIDs.begin(), n2->_intPoint->_faceIDs.end());
    if (faceIDs.empty()) return false; // n2 is inside
    if (n1->_intPoint)
        faceIDs.insert(n1->_intPoint->_faceIDs.begin(), n1->_intPoint->_faceIDs.end());
    faceIDs.insert(link._fIntPoints[iP]->_faceIDs.begin(), link._fIntPoints[iP]->_faceIDs.end());
    ...
    for (; faceID != faceIDs.end(); ++faceID) {
        ...
        if (proj.IsDone() && proj.NbPoints() > 0) {
            ...
            if (proj.LowerDistance() <= 0.1 * _grid->_tol) { isOut = false; }
            else {
                // find isOut by normals
                gp_Dir normal;
                if (GeomLib::NormEstim(BRep_Tool::Surface(face, loc), gp_Pnt2d(u, v),
                                       0.1 * _grid->_tol, normal)
                    < 3) {
...
void Hexahedron::sortVertexNodes(vector<_Node *> &nodes, _Node *curNode, TGeomID faceID)
{
    ...
    for (size_t i = 0; i < nodes.size(); ++i)
        if (!(nShapeIds[i] = nodes[i]->ShapeID())) return;
        ...
        for (; nbW > 0; ++nE, --nbW) {
            ...
            for (; e != eEnd; ++e)
                for (int i = 0; i < 2; ++i) {
                    ...
                    if ((id > 0) && (std::find(&nShapeIds[0], nShapeIdsEnd, id) != nShapeIdsEnd)) {
            ...
            if (nbW > 0) edges.erase(edges.begin(), eEnd); // remove a current irrelevant wire
        }
    }
    ...
    for (; e != edges.end(); ++e) {
        if (!_grid->_shapes.FindIndex(*e)) continue;
        bool isOut = false;
        gp_Pnt p;
        double uvw[3], f, l;
    ...
    if (e != edges.end()) edges.splice(edges.end(), edges, edges.begin(), e);
    else if (eMidOut != edges.end())
            ...
            if (!(nodes[iN] = _volumeDefs._nodes[iN]->Node())) {
                if (const E_IntersectPoint *eip = _volumeDefs._nodes[iN]->EdgeIntPnt())
                    nodes[iN] = _volumeDefs._nodes[iN]->_intPoint->_node =
                        helper.AddNode(eip->_point.X(), eip->_point.Y(), eip->_point.Z());
                else
            ...
            switch (nodes.size()) {
                case 8:
                    helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], nodes[5],
                                     nodes[6], nodes[7]);
                    break;
                case 4: helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3]); break;
                case 6:
                    helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], nodes[5]);
                    break;
                case 5: helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4]); break;
            }
        }
...
{
    if (!_vIntNodes.empty()) return false;
    ...
    for (int iDir = 0; iDir < 3; ++iDir) {
        ...
        li.SetIJK(_i, _j, _k);
        size_t lineIndex[4] = {li.LineIndex(), li.LineIndex10(), li.LineIndex01(),
                               li.LineIndex11()};
        ...
        if (hasLinks && allLinksOut) return true;
    }
    return false;
}
...
bool Hexahedron::checkPolyhedronSize() const
{
    ...
    for (size_t iP = 0; iP < _polygons.size(); ++iP) {
        ...
        if (polygon._links.empty()) continue;
        gp_XYZ area(0, 0, 0);
...
bool Hexahedron::addHexa()
{
    ...
    for (size_t i = 0; i < _polygons.size(); ++i) {
        if (_polygons[i]._links.empty()) continue;
        if (_polygons[i]._links.size() != 4) return false;
        ++nbQuad;
        if (iQuad < 0) iQuad = i;
    }
    if (nbQuad != 6) return false;
    ...
    for (int iL = 0; iL < 4; ++iL) {
        ...
        if (!link->_faces[0] || !link->_faces[1]) return debugDumpLink(link);
    ...
    if (nbN == 8) _volumeDefs.set(&nodes[0], 8);
...
bool Hexahedron::addTetra()
{
    ...
    for (size_t i = 0; i < _polygons.size() && iTria < 0; ++i)
        if (_polygons[i]._links.size() == 3) iTria = i;
    if (iTria < 0) return false;
...
bool Hexahedron::addPenta()
{
    ...
    for (int iF = 0; iF < 5 && iTri < 0; ++iF)
        if (_polygons[iF]._links.size() == 3) iTri = iF;
    ...
    if (nbN == 6) _volumeDefs.set(&nodes[0], 6);
...
bool Hexahedron::addPyra()
{
    ...
    for (int iF = 0; iF < 5 && iQuad < 0; ++iF)
        if (_polygons[iF]._links.size() == 4) iQuad = iF;
...
void getExactBndBox(const vector<TopoDS_Shape> &faceVec, const double *axesDirs, Bnd_Box &shapeBox)
{
    BRep_Builder b;
    TopoDS_Compound allFacesComp;
    b.MakeCompound(allFacesComp);
    for (size_t iF = 0; iF < faceVec.size(); ++iF) b.Add(allFacesComp, faceVec[iF]);

    double sP[6]; // aXmin, aYmin, aZmin, aXmax, aYmax, aZmax
    shapeBox.Get(sP[0], sP[1], sP[2], sP[3], sP[4], sP[5]);
    double farDist = 0;
    for (int i = 0; i < 6; ++i) farDist = Max(farDist, 10 * sP[i]);
...
bool StdMeshers_Cartesian_3D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    try {
        ...
        vector<TopoDS_Shape> faceVec;
        {
            ...
            for (fExp.ReInit(); fExp.More(); fExp.Next())
                if (faceMap.Contains(fExp.Current())) faceVec.push_back(fExp.Current());
        }
            ...
            if (_hyp->GetToAddEdges()) {
                ...
                for (eExp.Init(faceVec[i], TopAbs_EDGE); eExp.More(); eExp.Next()) {
                    ...
                    if (!SMESH_Algo::isDegenerated(edge) && !helper.IsRealSeam(edge))
                        edge2faceIDsMap[grid._shapes.Add(edge)].push_back(
                            facesItersectors[i]._faceID);
                }
            }
        }
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, facesItersectors.size()),
                          ParallelIntersector(facesItersectors), tbb::simple_partitioner());
#else
        for (size_t i = 0; i < facesItersectors.size(); ++i) facesItersectors[i].Intersect();
#endif
...
bool StdMeshers_Cartesian_3D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
...
struct _EventListener: public SMESH_subMeshEventListener {
    ...
    _EventListener(const string &algoName)
        : SMESH_subMeshEventListener(/*isDeletable=*/true,
                                     "StdMeshers_Cartesian_3D::_EventListener"),
          _algoName(algoName)
    {}
    // --------------------------------------------------------------------------------
    // setting/unsetting _alwaysComputed flag to submeshes of inferior levels
    //
    static void setAlwaysComputed(const bool isComputed, SMESH_subMesh *subMeshOfSolid)
    {
        SMESH_subMeshIteratorPtr smIt = subMeshOfSolid->getDependsOnIterator(
            /*includeSelf=*/false, /*complexShapeFirst=*/false);
        while (smIt->more()) {
    ...
    //
    virtual void ProcessEvent(const int event, const int eventType, SMESH_subMesh *subMeshOfSolid,
                              SMESH_subMeshEventListenerData *data, const SMESH_Hypothesis *hyp = 0)
    {
        if (eventType == SMESH_subMesh::COMPUTE_EVENT) {
            ...
            if (!algo3D || _algoName != algo3D->GetName()) setAlwaysComputed(false, subMeshOfSolid);
        }
    }

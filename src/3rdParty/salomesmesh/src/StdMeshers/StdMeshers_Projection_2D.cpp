...
bool StdMeshers_Projection_2D::CheckHypothesis(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                               SMESH_Hypothesis::Hypothesis_Status &theStatus)
{
    ...
    if (hypName == "ProjectionSource2D") {
        ...
        if (_sourceHypo->HasVertexAssociation()) {
            ...
            if (edge.IsNull() || !SMESH_MesherHelper::IsSubShape(edge, srcMesh)
                ...
                // target vertices
                edge = TAssocTool::GetEdgeByVertices(tgtMesh, _sourceHypo->GetTargetVertex(1),
                                                     _sourceHypo->GetTargetVertex(2));
...
public:
    ...
    static void Clean(SMESH_subMesh *sm, bool withSub = true)
    {
        ...
        while (nIt->more()) {
            ...
            if (!isOldNode(node)) mesh->RemoveNode(node);
        }
        ...
        while (smIt->more()) Clean(smIt->next(), false);
        //       }
    }
};
...
bool getBoundaryNodes(SMESH_subMesh *sm, const TopoDS_Face &face,
                      map<double, const SMDS_MeshNode *> &u2nodes,
                      set<const SMDS_MeshNode *> &seamNodes)
{
    u2nodes.clear();
    seamNodes.clear();
    if (!sm || !sm->GetSubMeshDS()) RETURN_BAD_RESULT("Null submesh");
        ...
        case TopAbs_EDGE: {
            ...
            while (nIt->more() && !nE) {
                ...
                while (vElems->more() && !nE) {
                    ...
                    if (elem->IsQuadratic()) nbNodes /= 2;
                    int iV1 = elem->GetNodeIndex(nV1);
                    // try next after nV1
                    int iE = SMESH_MesherHelper::WrapIndex(iV1 + 1, nbNodes);
                    if (smDS->Contains(elem->GetNode(iE))) nE = elem->GetNode(iE);
                    if (!nE) {
                        // try node before nV1
                    ...
                    if (nE && elem->IsQuadratic()) { // find medium node between nV1 and nE
                        if (Abs(iV1 - iE) == 1) nE = elem->GetNode(Min(iV1, iE) + nbNodes);
                        else
                            nE = elem->GetNode(elem->NbNodes() - 1);
                    }
                }
            }
            if (!nV1) RETURN_BAD_RESULT("No new node found on V1");
            if (!nE) RETURN_BAD_RESULT("new node on edge not found");
            ...
            if (!SMESH_MeshEditor::FindFreeBorder(nV1, nE, nV1, bordNodes, bordFaces))
                RETURN_BAD_RESULT("free border of a face not found by nodes " << nV1->GetID() << " "
                                                                              << nE->GetID());
            ...
            for (; bordIt != bordNodes.end(); ++bordIt) {
                ...
                if (v2smDS->Contains(node)) break;
                if (node->GetPosition()->GetTypeOfPosition() != SMDS_TOP_EDGE)
                    RETURN_BAD_RESULT("Bad node position type: node "
                                      << node->GetID() << " pos type "
...
bool are2dConnected(const TopoDS_Edge &E1, const TopoDS_Edge &E2, const TopoDS_Face &F)
{
    double f, l;
...
TError getWires(const TopoDS_Face &tgtFace, const TopoDS_Face &srcFace, SMESH_Mesh *tgtMesh,
                SMESH_Mesh *srcMesh, const TAssocTool::TShapeShapeMap &shape2ShapeMap,
                TSideVector &srcWires, TSideVector &tgtWires,
                TAssocTool::TNodeNodeMap &src2tgtNodes, bool &is1DComputed)
{
    ...
    if ((err && !err->IsOK()) || srcWires.empty()) return err;
    ...
    for (size_t iW = 0; iW < srcWires.size(); ++iW) {
        ...
        for (int iE = 0; iE < srcWire->NbEdges(); ++iE) {
            ...
            if (srcE.Orientation() != srcEbis.Orientation()) tgtE.Reverse();
            ...
            if (index < edgeMap.Extent()) // E is a seam
            {
                ...
                if (are2dConnected(tgtEdges.back(), tgtE, tgtFace)) {
                    ...
                    if (are2dConnected(tgtEdges.back(), *eIt, tgtFace)) eIt->Reverse();
                }
                else {
                    tgtE.Reverse();
                }
            ...
            if (srcWire->NbEdges() == 1 && tgtMesh == srcMesh) // circle
            {
                ...
                if (!nE.second.IsNull()) tgtE = nE.second;
            }
        ...
        for (int iE = 0; iE < srcWire->NbEdges(); ++iE) {
            if (srcMesh->GetSubMesh(srcWire->Edge(iE))->IsEmpty()
                ...
                if (srcN && tgtN) src2tgtNodes.insert(make_pair(srcN, tgtN));
            }
            else {
...
bool projectPartner(const TopoDS_Face &tgtFace, const TopoDS_Face &srcFace,
                    const TSideVector &tgtWires, const TSideVector &srcWires,
                    const TAssocTool::TShapeShapeMap &shape2ShapeMap,
                    TAssocTool::TNodeNodeMap &src2tgtNodes, const bool is1DComputed)
{
    ...
    if (!trsfIsOK) {
        ...
        if (!trsf.Solve(srcPnts, tgtPnts)) return false;
        ...
        if (trsfIsOK) {
            ...
            if (!loc.IsIdentity()) tgtTrsfP.Transform(loc.Transformation().Inverted());
            proj.Perform(tgtTrsfP);
            trsfIsOK = (proj.IsDone() && proj.NbPoints() > 0 && proj.LowerDistance() < tol);
        }
        if (!trsfIsOK) return false;
    }
    ...
    if (isReverse) std::swap(tri1, tri2), std::swap(quad1, quad3);
    ...
    while (elemIt->more()) // loop on all mesh faces on srcFace
    {
        ...
        switch (nbN) {
            ...
            case 4:
                helper.AddFace(tgtNodes[0], tgtNodes[quad1], tgtNodes[2], tgtNodes[quad3]);
                break;
            default:
    ...
    if (!tgtFace.IsPartner(srcFace)) {
        ...
        for (; srcN_tgtN != src2tgtNodes.end(); ++srcN_tgtN) {
            ...
            switch (n->GetPosition()->GetTypeOfPosition()) {
                ...
                case SMDS_TOP_EDGE: {
                    const TopoDS_Edge &tgtE =
                        TopoDS::Edge(tgtMeshDS->IndexToShape(n->getshapeId()));
...
bool projectBy2DSimilarity(const TopoDS_Face &tgtFace, const TopoDS_Face &srcFace,
                           const TSideVector &tgtWires, const TSideVector &srcWires,
                           const TAssocTool::TShapeShapeMap &shape2ShapeMap,
                           TAssocTool::TNodeNodeMap &src2tgtNodes, const bool is1DComputed)
{
    ...
    SMESH_MesherHelper helper(*tgtMesh);
    helper.SetSubShape(tgtFace);
    if (is1DComputed) helper.IsQuadraticSubMesh(tgtFace);
    else
        helper.SetIsQuadratic(srcSubDS->GetElements()->next()->IsQuadratic());
    helper.SetElementsOnShape(true);
    ...
    while (elemIt->more()) // loop on all mesh faces on srcFace
    {
        ...
        for (int i = 0; i < nbN; ++i) // loop on nodes of the source element
        {
            ...
            if (srcN_tgtN->second == nullNode) {
                // create a new node
                gp_Pnt2d srcUV = srcHelper.GetNodeUV(
                    srcFace, srcNode, elem->GetNode(helper.WrapIndex(i + 1, nbN)), &uvOK);
                ...
                switch (srcNode->GetPosition()->GetTypeOfPosition()) {
                    ...
                    case SMDS_TOP_EDGE: {
                        TopoDS_Shape srcEdge =
                            srcHelper.GetSubShapeByNode(srcNode, srcHelper.GetMeshDS());
...
inline bool projectQuads(const TopoDS_Face &tgtFace, const TopoDS_Face &srcFace,
                         const TSideVector &tgtWires, const TSideVector &srcWires,
                         const TAssocTool::TShapeShapeMap &shape2ShapeMap,
                         TAssocTool::TNodeNodeMap &src2tgtNodes, const bool is1DComputed)
{
    ...
    if (srcWires[0]->NbEdges() != 4) return false;
    if (!is1DComputed) return false;
    for (int iE = 0; iE < 4; ++iE) {
        SMESHDS_SubMesh *sm = srcMeshDS->MeshElements(srcWires[0]->Edge(iE));
        if (!sm) return false;
        if (sm->NbNodes() + sm->NbElements() == 0) return false;
    }
    if (BRepAdaptor_Surface(tgtFace).GetType() != GeomAbs_Plane) return false;
        ...
        for (int i = 0; i < nbN; ++i) // loop on nodes of the source element
        {
            ...
            if (n2n_isNew.second) // new src node encounters
            {
                ...
                if (srcN_tgtN != src2tgtNodes.end()) {
                    ...
                    if (!block.ComputeParameters(SMESH_TNodeXYZ(srcNode), tgtNodeOrXY.second,
                                                 srcFaceBID))
                        return false;
                }
            }
    ...
    for (size_t iFaceTgt = 0; iFaceTgt < newFacesVec.size(); ++iFaceTgt) {
        ...
        for (size_t iN = 0; iN < tgtConn.size(); ++iN) {
            ...
            {
                if (!block.FaceUV(tgtFaceBID, tgtConn[iN]->second, uv)) return false;
        ...
        switch (tgtNodes.size()) {
            ...
            default:
                if (tgtNodes.size() > 4) helper.AddPolygonalFace(tgtNodes);
        }
    }
    return true;
...
bool fixDistortedFaces(SMESH_MesherHelper &helper, TSideVector &tgtWires)
{
    ...
    if (helper.IsDistorted2D(faceSM, /*checkUV=*/false)) {
        ...
        for (size_t iW = 0; iW < tgtWires.size() && !isConcaveBoundary; ++iW) {
            ...
            for (int iE = 0; iE < tgtWires[iW]->NbEdges() && !isConcaveBoundary; ++iE) {
                double angle = helper.GetAngle(prevEdge, tgtWires[iW]->Edge(iE), F,
                                               tgtWires[iW]->FirstVertex(iE));
...
bool StdMeshers_Projection_2D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (tgtMesh == srcMesh) {
        ...
        if (!srcSubMesh->IsMeshComputed()) srcMeshError = TAssocTool::SourceNotComputedError();
    }
    if (!srcMeshError.empty()) return error(COMPERR_BAD_INPUT_MESH, srcMeshError);
    ...
    TError err = getWires(tgtFace, srcFace, tgtMesh, srcMesh, shape2ShapeMap, srcWires, tgtWires,
                          _src2tgtNodes, is1DComputed);
    if (err && !err->IsOK()) return error(err);

    bool projDone = false;

    if (!projDone) {
        // try to project from the same face with different location
        projDone = projectPartner(tgtFace, srcFace, tgtWires, srcWires, shape2ShapeMap,
                                  _src2tgtNodes, is1DComputed);
    }
    if (!projDone) {
        // projection in case if the faces are similar in 2D space
        projDone = projectBy2DSimilarity(tgtFace, srcFace, tgtWires, srcWires, shape2ShapeMap,
                                         _src2tgtNodes, is1DComputed);
    }
    if (!projDone) {
    ...
    if (!projDone) {
        ...
        if (nbEdgesInWires.front() > 1) // possible to find out orientation
        {
            ...
            if (reverse &&
                ...
                nbEdgesInWires.front() > 2 && helper.IsRealSeam(tgtEdges.front())) {
            ...
            if (!_sourceHypo->HasVertexAssociation() && nbEdgesInWires.front() > 2
                ...
                if (srcEdge2 == srcEdges.end()
                    || srcEdge3 == srcEdges.end()) // srcEdge1 is not a seam
                {
                    ...
                    if (srcEdge2 != srcEdges.end()) {
                        ...
                        if (SMESHDS_SubMesh *sm = srcMesh->GetMeshDS()->MeshElements(*srcEdge2))
                            nbN2 = sm->NbNodes();
                        if (nbN1 != nbN2) srcV1 = helper.IthVertex(1, srcEdges.front());
                    }
                }
            }
        }
        ...
        SMESH_subMeshIteratorPtr smIt =
            tgtSubMesh->getDependsOnIterator(/*includeSelf=*/false,
                                             /*complexShapeFirst=*/false);
        while (smIt->more()) {
            SMESH_subMesh *sm = smIt->next();
            SMESHDS_SubMesh *smDS = sm->GetSubMeshDS();
            if (!smDS || smDS->NbNodes() == 0) continue;
                ...
                if (sm->GetSubShape().ShapeType() == TopAbs_EDGE) {
                    ...
                    while (smDegenIt->more())
                        if ((smDS = smDegenIt->next()->GetSubMeshDS())) {
                            ...
                            while (nIt->more()) groupsOfNodes.back().push_back(nIt->next());
                        }
                }
            ...
            while (nIt->more()) {
                ...
                if (!isOld && isSeam) {                  // new node on a seam edge
                    if (seamNodes.count(node)) continue; // node is already in the map
                }

                // sort nodes on edges by their position
                map<double, const SMDS_MeshNode *> &pos2nodes =
                    u2nodesMaps[isOld ? OLD_NODES : NEW_NODES];
            ...
            const bool mergeSeamToNew = (u2nodesMaps[NEW_NODES].size() == u2nodesOnSeam.size());

            if (!mergeNewToOld)
                if (u2nodesMaps[NEW_NODES].size() > 0 && u2nodesMaps[OLD_NODES].size() > 0) {
            ...
            if (isSeam && !mergeSeamToNew) {
                ...
                if (u2nodesMaps[NEW_NODES].size() > 0 && u2nodesOnSeam.size() > 0
                    ...
                    if (nbE1 != nbE2) // 2 EDGEs are mapped to a seam EDGE
                    {
                        ...
                        TopTools_DataMapIteratorOfDataMapOfShapeShape src2tgtIt(
                            shape2ShapeMap._map2to1);
            ...
            if (mergeNewToOld)
                for (; u_newNode != newEnd; ++u_newNode, ++u_oldNode) {
                    ...
                    if (mergeSeamToNew) groupsOfNodes.back().push_back((u_newOnSeam++)->second);
                }
        ...
        if (srcIsQuad && !tgtIsQuad) {
            ...
            faceIt = tgtSubMesh->GetSubMeshDS()->GetElements();
            while (faceIt->more()) tgtFaces.insert(tgtFaces.end(), faceIt->next());
    ...
    if (!projDone || is1DComputed)
        ...
        // ----------------------------------------------------------------
        if (!fixDistortedFaces(helper, tgtWires)) return error("Invalid mesh generated");
...
bool StdMeshers_Projection_2D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                        MapShapeNbElems &aResMap)
{
    ...
    if (srcSubMesh->IsMeshComputed()) {
        ...
        while (elemIt->more()) aVec[elemIt->next()->GetEntityType()]++;
    }
    else {
        ...
        if (aVec.empty()) return error(COMPERR_BAD_INPUT_MESH, "Source mesh is wrongly evaluated");
    }
...
{
    TAssocTool::SetEventListener(subMesh, _sourceHypo->GetSourceFace(),

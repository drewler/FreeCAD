...
{
#ifdef _DEBUG_
    const char *type[] = {"COMPOUND", "COMPSOLID", "SOLID", "SHELL",
                          "FACE",     "WIRE",      "EDGE",  "VERTEX"};
    BRepTools::Write(shape,
                     SMESH_Comment("/tmp/") << type[shape.ShapeType()] << "_"
...
void reverseEdges(list<TopoDS_Edge> &edges, const int nbEdges, const int firstEdge = 0)
{
    ...
    for (int i = 0; i < nbEdges; ++i, ++eBackIt) eBackIt->Reverse(); // reverse edge
    // reverse list
    --eBackIt;
    while (eIt != eBackIt) {
        std::swap(*eIt, *eBackIt);
        SHOW_LIST("# AFTER SWAP", edges)
        if ((++eIt) != eBackIt) --eBackIt;
    }
...
bool fixAssocByPropagation(const int nbEdges, list<TopoDS_Edge> &edges1, list<TopoDS_Edge> &edges2,
                           SMESH_Mesh *theMesh1, SMESH_Mesh *theMesh2)
{
...
bool assocFewEdgesFaces(const TopoDS_Face &face1, SMESH_Mesh *mesh1, const TopoDS_Face &face2,
                        SMESH_Mesh *mesh2, HERE::TShapeShapeMap &theMap)
{
...
TopoDS_Shape findGroupContaining(const TopoDS_Shape &tgtShape, const SMESH_Mesh *tgtMesh1,
...
bool assocGroupsByPropagation(const TopoDS_Shape &theGroup1, const TopoDS_Shape &theGroup2,
                              SMESH_Mesh &theMesh, HERE::TShapeShapeMap &theMap)
{
    ...
    for (; vExp1.More(); vExp1.Next()) {
        ...
        for (; verticEdge.IsNull() && ancestIt.More(); ancestIt.Next()) {
            ...
            if (subshapes2.Contains(v2)) verticEdge = ancestIt.Value();
        }
        if (verticEdge.IsNull()) return false;
        ...
        for (; ancestIt.More(); ancestIt.Next()) {
            ...
            if (nbEdgesInWire.front() != 4) return storeShapeForDebug(face);
    ...
    if (gr1It.Value().ShapeType() == TopAbs_FACE) {
        ...
        if (bndEdge.IsNull()) return false;
        ...
        for (; edge1 != edges.end(); ++edge1) {
            ...
            for (; ancestIt.More() && face1.IsNull(); ancestIt.Next()) {
                if (ancestIt.Value().ShapeType() == TopAbs_FACE && !theMap.IsBound(ancestIt.Value())
                ...
                for (TopExp_Explorer e(face1, TopAbs_EDGE); e.More(); e.Next())
                    if (!edge1->IsSame(e.Current())) edges.push_back(e.Current());
            }
            if (!face1.IsNull()) {
...
bool sameVertexUV(const TopoDS_Edge &edge, const TopoDS_Face &face, const int &vIndex,
                  const gp_Pnt2d &uv, const double &tol2d)
{
    TopoDS_Vertex VV[2];
...
bool getOuterEdges(const TopoDS_Shape shape, SMESH_Mesh &mesh, std::list<TopoDS_Edge> &allBndEdges)
{
    if (shape.ShapeType() == TopAbs_COMPOUND) {
        ...
        if (it.More() && it.Value().ShapeType() == TopAbs_FACE) // group of FACEs
        {
            ...
            if (!allBndEdges.empty()) return true;
        }
    }
    ...
    if (expF.More()) {
        ...
        if (!helper.IsClosedEdge(TopoDS::Edge(shape))) allBndEdges.push_back(TopoDS::Edge(shape));
    }
                                                         ...
                                                         TShapeShapeMap &theMap)
{
    ...
    if (theShape1.ShapeType() != theShape2.ShapeType()) {
        ...
        if (group1.IsNull() || group2.IsNull()) RETURN_BAD_RESULT("Different shape types");
        // Associate compounds
    ...
    for (; partner && vvIt.More(); vvIt.Next()) partner = vvIt.Key().IsPartner(vvIt.Value());
    ...
    if (!theMap.IsEmpty()) {
        ...
        switch (theShape1.ShapeType()) {
            ...
            case TopAbs_EDGE: { // TopAbs_EDGE
                // ----------------------------------------------------------------------
                if (theMap.Extent() != 1) RETURN_BAD_RESULT("Wrong map extent " << theMap.Extent());
            ...
            case TopAbs_FACE: { // TopAbs_FACE
                ...
                if (VV2[1].IsNull()) { // 2 bound vertices not found
                    if (nbEdges > 1) { RETURN_BAD_RESULT("2 bound vertices not found"); }
                    else {
                        VV2[1] = VV2[0];
                    }
                }
            ...
            case TopAbs_SOLID: {
                ...
                if (edge2.IsNull()) RETURN_BAD_RESULT("GetEdgeByVertices() failed");
                ...
                if (F1.IsNull()) RETURN_BAD_RESULT(" Face1 not found");
                ...
                for (int i = 0; FF2[1].IsNull() && ancestIt2.More(); ancestIt2.Next())
                    if (ancestIt2.Value().ShapeType() == TopAbs_FACE)
                        FF2[i++] =
                            ancestIt2.Value().Oriented // ( TopAbs_FORWARD );
                ...
                for (fe1 = FE1.begin(), fe2 = FE2.begin(); fe1 != FE1.end(); ++fe1, ++fe2) {
                    ...
                    list<TopoDS_Edge> edges1, edges2;
                    int nbE =
                        FindFaceAssociation(face1, VV1, face2, VV2, edges1, edges2, isVCloseness);
                    if (!nbE) RETURN_BAD_RESULT("FindFaceAssociation() failed");
                    InsertAssociation(face1, face2, theMap); // assoc faces
                    MESSAGE("Assoc FACE " << theMesh1->GetMeshDS()->ShapeToIndex(face1) << " to "
                                          << theMesh2->GetMeshDS()->ShapeToIndex(face2));
                    if (nbE == 2
                        && (edge1.IsSame(edges1.front())) != (edge2.IsSame(edges2.front()))) {
                    ...
                    for (; eIt1 != edges1.end(); ++eIt1, ++eIt2) {
                        ...
                        if (!nextFace1.IsNull() && !nextFace2.IsNull()) {
                            if (SMESH_MesherHelper::GetSubShapeOri(nextFace1, *eIt1)
                                == eIt1->Orientation())
                                nextFace1.Reverse();
                            if (SMESH_MesherHelper::GetSubShapeOri(nextFace2, *eIt2)
                                == eIt2->Orientation())
                                nextFace2.Reverse();
            ...
            case TopAbs_COMPOUND: { // GROUP
                ...
                if (nbMembers == 1) {
                    return FindSubShapeAssociation(it1.Value(), theMesh1, it2.Value(), theMesh2,
                                                   theMap);
                }
                ...
                if (memberType == TopAbs_FACE) {
                    ...
                    for (; it1.More(); it1.Next(), it2.Next()) {
                        ...
                        if (!initAssocOK) {
                            ...
                            TopoDS_Vertex v1, v2;
                            TopExp::Vertices(TopoDS::Edge(it1.Value().Oriented(TopAbs_FORWARD)), v1,
                                             v2);
                                ...
                                if (!face.IsNull()) {
                                    ...
                                    for (TopExp_Explorer f(face, TopAbs_EDGE); f.More(); f.Next())
                                        if (groupEdges[is2ndGroup].Contains(f.Current()))
                                            if (++nbGroupEdges > 1) break;
                                    bool add =
                                        (nbGroupEdges > 1
                                         || SMESH_MesherHelper::Count(face, TopAbs_EDGE, true)
                                             == 1);
                                    if (!add) {
                                        add = true;
                    ...
                    if (ok) {
                        ...
                        if (boundMembers[0].Extent() != nbMembers) {
                            ...
                            for (int is2ndGroup = 0; is2ndGroup < 2; ++is2ndGroup) {
                                ...
                                for (mIt.Initialize(is2ndGroup ? theShape2 : theShape1); mIt.More();
                                     mIt.Next())
                            ...
                            if (hasInitialAssoc == bool(!theMap.IsEmpty()))
                                ok = FindSubShapeAssociation(comp[0], theMesh1, comp[1], theMesh2,
                                                             theMap);
                            else {
                ...
                while (!v2e[0].IsEmpty()) {
                    ...
                    if (V[0].IsNull()) RETURN_BAD_RESULT("No more bound vertices");
                    ...
                    while (!V[0].IsNull() && v2e[0].IsBound(V[0])) {
                        ...
                        if (nbE0 == 1) {
                            ...
                            MESSAGE("Assoc edge " << theMesh1->GetMeshDS()->ShapeToIndex(e0)
                                                  << " to "
                                                  << theMesh2->GetMeshDS()->ShapeToIndex(e1));
                            V[0] = GetNextVertex(e0, V[0]);
                            V[1] = GetNextVertex(e1, V[1]);
                            if (!V[0].IsNull()) {
                                InsertAssociation(V[0], V[1], theMap);
                                MESSAGE("Assoc vertex "
                                        << theMesh1->GetMeshDS()->ShapeToIndex(V[0]) << " to "
                                        << theMesh2->GetMeshDS()->ShapeToIndex(V[1]));
                            }
                        }
                        else if (nbE0 == 2) {
            ...
            default: RETURN_BAD_RESULT("Unexpected shape type");
    ...
    switch (theShape1.ShapeType()) {
        ...
        case TopAbs_EDGE: {
            ...
            if (isPropagationPossible(theMesh1, theMesh2)) {
                ...
                if (!prpEdge.IsNull()) {
                    ...
                    if (VV1[0].IsSame(VV1[1]) || // one of edges is closed
                        VV2[0].IsSame(VV2[1])) {
                        InsertAssociation(edge1, prpEdge,
                                          theMap); // insert with a proper orientation
                    }
                    InsertAssociation(theShape1, theShape2, theMap);
            ...
            if (SMESH_MesherHelper::IsClosedEdge(edge1)
                ...
                InsertAssociation(TopExp::FirstVertex(edge1), TopExp::FirstVertex(edge2), theMap);
                InsertAssociation(theShape1, theShape2, theMap);
                return true; // done
            }
            ...
            if (isPropagationPossible(theMesh1, theMesh2)) // try by propagation in one mesh
            {
                ...
                for (TopoDS_Iterator edgeIt(wire); edgeIt.More(); edgeIt.Next()) {
                    ...
                    for (TopExp_Explorer exp(face2, TopAbs_EDGE); exp.More(); exp.Next()) {
                        edge2 = TopoDS::Edge(exp.Current());
                        pair<int, TopoDS_Edge> step_edge =
                            GetPropagationEdge(theMesh1, edge2, edge1);
                        if (!step_edge.second.IsNull()) { // propagation found
                            propag_edges.insert(
                                make_pair(step_edge.first, (make_pair(edge1, step_edge.second))));
        ...
        case TopAbs_COMPOUND: {
            ...
            if (isPropagationPossible(theMesh1, theMesh2)) {
                ...
                if (assocGroupsByPropagation(theShape1, theShape2, *theMesh1, theMap)) return true;

                // find a boundary edge of theShape1
                TopoDS_Edge E = GetBoundaryEdge(theShape1, *theMesh1);
                if (E.IsNull()) break; // try by vertex closeness
                ...
                for (TopExp_Explorer eexp(E, TopAbs_VERTEX); eexp.More(); eexp.Next()) {
                    ...
                    for (; ita.More(); ita.Next()) {
                        ...
                        for (TopExp_Explorer expe(theShape1, TopAbs_EDGE); expe.More();
                             expe.Next()) {
                        ...
                        if (!FromShape1) {
                            ...
                            if (FromShape2) {
                                if (VV1[0].IsNull()) VV1[0] = V1, VV2[0] = V2;
                                else
                                    VV1[1] = V1, VV2[1] = V2;
    ...
    if (vMap1.Extent() == 1 || vMap2.Extent() == 1) {
        ...
        if (theShape1.ShapeType() == TopAbs_EDGE) {
            if (vMap1.Extent() == 2) InsertAssociation(vMap1(2), vMap2(1), theMap);
    ...
    Bnd_Box box[2];
    for (int i = 1; i <= vMap1.Extent(); ++i) box[0].Add(BRep_Tool::Pnt(TopoDS::Vertex(vMap1(i))));
    for (int i = 1; i <= vMap2.Extent(); ++i) box[1].Add(BRep_Tool::Pnt(TopoDS::Vertex(vMap2(i))));
    ...
    if (!getOuterEdges(theShape1, *theMesh1, allBndEdges1)) {
        if (theShape1.ShapeType() != TopAbs_FACE) RETURN_BAD_RESULT("Edge not found");
        return assocFewEdgesFaces(TopoDS::Face(theShape1), theMesh1, TopoDS::Face(theShape2),
                                  theMesh2, theMap);
    }
    ...
    for (int nbChecked = 0; edge1 != allBndEdges1.end() && nbChecked++ < 10; ++edge1) {
        ...
        if (edge1VV[0].IsSame(edge1VV[1])) continue; //RETURN_BAD_RESULT("Only closed edges");
        ...
        for (int i1 = 0; i1 < 2; ++i1) {
            ...
            if (!i1) {
                ...
                PShapeIteratorPtr edgeIt =
                    SMESH_MesherHelper::GetAncestors(edge2VV[0], *theMesh2, TopAbs_EDGE);
    ...
    InsertAssociation(VV1[0], VV2[0], theMap);
    InsertAssociation(VV1[1], VV2[1], theMap);
    MESSAGE("Initial assoc VERT " << theMesh1->GetMeshDS()->ShapeToIndex(VV1[0]) << " to "
                                  << theMesh2->GetMeshDS()->ShapeToIndex(VV2[0])
                                  << "\nand         VERT "
                                  << theMesh1->GetMeshDS()->ShapeToIndex(VV1[1]) << " to "
                                  << theMesh2->GetMeshDS()->ShapeToIndex(VV2[1]));
    if (theShape1.ShapeType() == TopAbs_EDGE) {
        InsertAssociation(theShape1, theShape2, theMap);
        return true;
    }
...
int StdMeshers_ProjectionUtils::FindFaceAssociation(const TopoDS_Face &face1, TopoDS_Vertex VV1[2],
                                                    const TopoDS_Face &face2, TopoDS_Vertex VV2[2],
                                                    ...
                                                    const bool isClosenessAssoc)
{
        ...
        if (revSeam2 != edges2.end()) // two seams detected
        {
            ...
            if (!reverse && isClosenessAssoc) {
                ...
                for (; edge1 != edges1.end(); ++edge1, ++edge2) {
                    if ((edge1 == revSeam1) || (SMESH_Algo::isDegenerated(*edge1))
                        || (std::find(++edges1.begin(), edges1.end(), edge1->Reversed())
                            != edges1.end()))
                        continue;
                    ...
                    if (edge1->Orientation() == TopAbs_REVERSED) std::swap(pp1[0], pp1[1]);
                    ...
                    if (edge2->Orientation() == TopAbs_REVERSED) std::swap(pp2[0], pp2[1]);
            ...
            for (int iW1 = OK; nbE1 != nbEInW1.end(); ++nbE1, ++iW1) // loop on wires of face1
            {
                ...
                for (int iW2 = OK; nbE2 != nbEInW2.end(); ++nbE2, ++iW2) // loop on wires of face2
                {
                    ...
                    if (*nbE1 == *nbE2 && iW2 >= iW1) {
                        ...
                        int i = *nbE2;
                        bool sameUV = false;
                        while (!(sameUV = sameVertexUV(*edge2Beg, face2, 0, v0f1UV, vTolUV))
                               && --i > 0)
                        ...
                        if (sameUV) {
                            ...
                            if (SMESH_MesherHelper::IsClosedEdge(*edge1Beg)) {
                                ...
                                if (!sameVertexUV(*edge2Beg, face2, 1, v1f1UV, vTolUV))
                                    reverseEdges(edges2, *nbE2,
                                                 std::distance(edges2.begin(), edge2Beg));
                            }
    ...
    if (OK && nbEdges == 2) {
        ...
        if (Abs(l1[0] - l1[1]) > 0.1 * Max(l1[0], l1[1])) {
            ...
            if ((l1[0] < l1[1]) != (l2[0] < l2[1])) { reverseEdges(edges2, nbEdges); }
        }
    }
    ...
    if (theMesh && !theV1.IsNull() && !theV2.IsNull()) {
        ...
        for (; ancestorIt.More(); ancestorIt.Next())
            if (ancestorIt.Value().ShapeType() == TopAbs_EDGE)
                for (TopExp_Explorer expV(ancestorIt.Value(), TopAbs_VERTEX); expV.More();
                     expV.Next())
                    if (theV2.IsSame(expV.Current())) return TopoDS::Edge(ancestorIt.Value());
    }
    return TopoDS_Edge();
}
                                                    ...
                                                    const TopoDS_Face &face)
{
    ...
    if (!edge.IsNull() && edgeToFaces.Contains(edge)) // PAL16202
    {
        ...
        for (; ancestorIt.More(); ancestorIt.Next())
            if (ancestorIt.Value().ShapeType() == TopAbs_FACE && !face.IsSame(ancestorIt.Value()))
                                                        ...
                                                        const TopoDS_Vertex &vertex)
{
    ...
    if (vF.IsSame(vL)) return TopoDS_Vertex();
...
StdMeshers_ProjectionUtils::GetPropagationEdge(SMESH_Mesh *aMesh, const TopoDS_Edge &anEdge,
                ...
                if (aW.ShapeType() == TopAbs_WIRE /*&& checkedWires.Add( aW )*/) {
                    ...
                    if (nb == 4 && found >= 0) {
                        ...
                        if (aChain.Add(anOppE) > prevChainSize) { // ... anOppE is not in aChain
                            ...
                            if (anOppE.IsSame(anEdge)) return make_pair(step, TopoDS::Edge(anOppE));
...
bool StdMeshers_ProjectionUtils::FindMatchingNodesOnFaces(
    const TopoDS_Face &face1, SMESH_Mesh *mesh1, const TopoDS_Face &face2, SMESH_Mesh *mesh2,
    const TShapeShapeMap &assocMap, TNodeNodeMap &node1To2Map)
{
    ...
    if (!SM2 || !SM1) RETURN_BAD_RESULT("Empty submeshes");
    if (SM2->NbNodes() != SM1->NbNodes() || SM2->NbElements() != SM1->NbElements())
        RETURN_BAD_RESULT("Different meshes on corresponding faces " << meshDS1->ShapeToIndex(
                              face1) << " and " << meshDS2->ShapeToIndex(face2));
    if (SM2->NbElements() == 0) RETURN_BAD_RESULT("Empty submeshes");

    helper1.SetSubShape(face1);
    helper2.SetSubShape(face2);
    if (helper1.HasSeam() != helper2.HasSeam()) RETURN_BAD_RESULT("Different faces' geometry");
    ...
    do {
        ...
        eE.Next();
        // edge 1
        if (!assocMap.IsBound(e2, /*is2nd=*/true)) continue;
        ...
        if (!helper1.IsSubShape(e1, face1))
            RETURN_BAD_RESULT("Wrong association, edge " << meshDS1->ShapeToIndex(e1)
                                                         << " isn't a sub-shape of face "
                                                         << meshDS1->ShapeToIndex(face1));
    ...
    if (hasNodesOnEdge) {
        int nbNodeToGet = 1;
        if (helper1.IsClosedEdge(edge1) || helper2.IsClosedEdge(edge2)) nbNodeToGet = 2;
        ...
        for (int is2 = 0; is2 < 2; ++is2) {
            ...
            if (!helper->IsRealSeam(is2 ? edge2 : edge1)) {
                ...
                for (int i = 0; i < nbNodes; ++i) {
                    ...
                    if (f1) elems.insert(f1);
                }
            } // case on a sphere
        }     // loop on 2 faces

        node1To2Map.clear();
        assocRes = SMESH_MeshEditor::FindMatchingNodes(Elems1, Elems2, vNode1, vNode2, eNode1[0],
                                                       eNode2[0], node1To2Map);
    ...
    if (helper1.IsRealSeam(edge1)) {
        ...
        for (int is2 = 0; is2 < 2; ++is2) {
            ...
            if (pos2nodes.size() != edgeSM->NbNodes())
                RETURN_BAD_RESULT("Equal params of nodes on edge " << smDS->ShapeToIndex(edge)
                                                                   << " of face " << is2);
        }
...
TopoDS_Shape StdMeshers_ProjectionUtils::OuterShape(const TopoDS_Face &face, TopAbs_ShapeEnum type)
{
    ...
    if (exp.More()) return exp.Current();
    return TopoDS_Shape();
}
...
{
    if (iterationNb > 10) RETURN_BAD_RESULT("Infinite recursive projection");
    if (!sm) RETURN_BAD_RESULT("NULL submesh");
    if (sm->IsMeshComputed()) return true;
    ...
    if (!algo) {
        if (shape.ShapeType() != TopAbs_COMPOUND) {
            ...
            for (++dim; (dim <= 3 && !algo); ++dim) {
                ...
                int nbAlgos = mesh->GetHypotheses(shape, hypoFilter, hyps, true, &assignedTo);
            ...
            for (TopoDS_Iterator grMember(shape); grMember.More(); grMember.Next())
                if (SMESH_subMesh *grSub = mesh->GetSubMesh(grMember.Value()))
                    if (!MakeComputed(grSub, iterationNb + 1)) computed = false;
            return computed;
        }
    }
    ...
    const list<const SMESHDS_Hypothesis *> &hyps = algo->GetUsedHypothesis(*mesh, shape);
    ...
    if (srcShape.IsSame(shape)) RETURN_BAD_RESULT("Projection from self");
                                                               ...
                                                               SMESH_Algo *projAlgo)
{
    ...
    if (!projAlgo) return usualMessage;
    ...
    for (int dimIncrement = 1; shapeDim + dimIncrement < 4; ++dimIncrement) {
        ...
        if (algo && !algo->NeedDiscreteBoundary())
            return SMESH_Comment("\"") << algo->GetFeatures()._label << "\""
        ...
        for (TopExp_Explorer exp(edgeContainer, TopAbs_EDGE); exp.More(); exp.Next()) {
            ...
            while (const TopoDS_Shape *face = faceIt->next())
                if (facesOfEdgeContainer.Contains(*face))
                    if (facesNearEdge.Add(*face) && facesNearEdge.Extent() > 1) break;
            if (facesNearEdge.Extent() == 1) {
                if (allBndEdges) allBndEdges->push_back(edge);
                else
                    return edge;
            }
        }
...
struct HypModifWaiter: SMESH_subMeshEventListener {
    ...
    {
        if (event == SMESH_subMesh::MODIF_HYP && eventType == SMESH_subMesh::ALGO_EVENT) {
            // delete current source listener
            subMesh->DeleteEventListener(getSrcSubMeshListener());
            // let algo set a new one
            if (SMESH_Algo *algo = subMesh->GetAlgo()) algo->SetEventListener(subMesh);
        }
    }
};
//================================================================================
...
void StdMeshers_ProjectionUtils::SetEventListener(SMESH_subMesh *subMesh, TopoDS_Shape srcShape,
                                                  SMESH_Mesh *srcMesh)
{
    ...
    if (!srcShape.IsNull()) {
        if (!srcMesh) srcMesh = subMesh->GetFather();

        SMESH_subMesh *srcShapeSM = srcMesh->GetSubMesh(srcShape);

        if (srcShapeSM != subMesh) {
            if (srcShapeSM->GetSubMeshDS()
                && srcShapeSM->GetSubMeshDS()->IsComplexSubmesh()) { // source shape is a group
                TopExp_Explorer it(
                    srcShapeSM->GetSubShape(),           // explore the group into sub-shapes...
                ...
                for (; it.More(); it.Next()) {
                    ...
                    if (srcSM != subMesh) {
                        ...
                        if (data) data->mySubMeshes.push_back(subMesh);
                        else
                ...
                if (SMESH_subMeshEventListenerData *data =
                    ...
                    bool alreadyIn =
                        (std::find(data->mySubMeshes.begin(), data->mySubMeshes.end(), subMesh)
                         != data->mySubMeshes.end());
                    if (!alreadyIn) data->mySubMeshes.push_back(subMesh);
                }
                else {
...
bool TrsfFinder2D::Solve(const vector<gp_XY> &srcPnts, const vector<gp_XY> &tgtPnts)
{
    ...
    solver.Solve(vec);
    if (vec.Norm2() < gp::Resolution()) return false;
...
bool TrsfFinder3D::Solve(const vector<gp_XYZ> &srcPnts, const vector<gp_XYZ> &tgtPnts)
{
    // find gravity center
    ...
    M.SetRows(gp_XYZ(vec(1), vec(2), vec(3)), gp_XYZ(vec(4), vec(5), vec(6)),

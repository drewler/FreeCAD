...
{
public:
    Algo1D(int studyId, SMESH_Gen *gen) : StdMeshers_Regular_1D(gen->GetANewId(), studyId, gen) {}
    ...
    void ComputeDistribution(SMESH_MesherHelper &theHelper, const gp_Pnt &thePnt1,
                             const gp_Pnt &thePnt2, list<double> &theParams)
    {
        ...
        if (!StdMeshers_Regular_1D::computeInternalParameters(mesh, C3D, len, f, l, theParams,
                                                              false)) {
            for (size_t i = 1; i < 15; ++i) theParams.push_back(i / 15);
        }
        else {
...
StdMeshers_QuadFromMedialAxis_1D2D::StdMeshers_QuadFromMedialAxis_1D2D(int hypId, int studyId,
                                                                       SMESH_Gen *gen)
    : StdMeshers_Quadrangle_2D(hypId, studyId, gen), _regular1D(0)
{
...
struct TmpMesh: public SMESH_Mesh {
    TmpMesh() { _myMeshDS = new SMESHDS_Mesh(/*id=*/0, /*isEmbeddedMode=*/true); }
};
...
struct EdgeCleaner: public SMESH_subMeshEventListener {
    ...
    virtual void ProcessEvent(const int event, const int eventType, SMESH_subMesh *faceSubMesh,
                              SMESH_subMeshEventListenerData *data, const SMESH_Hypothesis *hyp)
    {
        ...
        if (_prevAlgoEvent == SMESH_subMesh::REMOVE_HYP
            ...
            SMESH_subMeshIteratorPtr smIt =
                faceSubMesh->getDependsOnIterator(/*includeSelf=*/false);
            while (smIt->more()) smIt->next()->ComputeStateEngine(SMESH_subMesh::CLEAN);
        }
        _prevAlgoEvent = -1;
    }
};
...
template<typename T> T &get(std::pair<T, T> &thePair, bool is2nd)
{
...
double getMinSegLen(SMESH_MesherHelper &theHelper, const vector<TopoDS_Edge> &theEdges)
{
    TmpMesh tmpMesh;
    SMESH_Mesh *mesh = theHelper.GetMesh();
    ...
    for (size_t i = 0; i < theEdges.size(); ++i) {
        SMESH_subMesh *sm = mesh->GetSubMesh(theEdges[i]);
        if (SMESH_Algo::IsStraight(theEdges[i], /*degenResult=*/true)) continue;
        // get algo
        ...
        if (status != SMESH_Hypothesis::HYP_OK) {
            ...
            try {
                ...
                if (!algo->Compute(tmpMesh, theEdges[i])) continue;
            }
            catch (...) {
                continue;
            }
    ...
    if (Precision::IsInfinite(minSegLen)) minSegLen = mesh->GetShapeDiagonalSize() / nbSegDflt;
...
bool getConnectedEdges(const SMESH_MAT2d::Branch *br1, const SMESH_MAT2d::Branch *br2,
                       const vector<TopoDS_Edge> &allEdges, vector<TopoDS_Edge> &shortEdges)
{
    ...
    size_t iEnd[4] = {vAtStart1 ? edgeIDs[0].back() : edgeIDs[0][0],
    ...
    if (connectedIDs.count(0) && connectedIDs.count(allEdges.size() - 1)) {
        size_t iE = allEdges.size() - 1;
        while (connectedIDs.count(iE - 1)) --iE;
        ...
        for (; i != connectedIDs.end(); ++i) shortEdges.push_back(allEdges[*i]);
    }
    return true;
}
...
bool getSinuousEdges(SMESH_MesherHelper &theHelper, SinuousFace &theSinuFace)
{
    ...
    if (nbEdges < 4 && theSinuFace._nbWires == 1) return false;
    ...
    if (theSinuFace._nbWires > 2) return false;
    ...
    if (braPoints.size() < 2) return false;
    ...
    for (size_t i = 0; i < braPoints.size(); ++i) {
        ...
        for (size_t ib = 0; ib < braPoints[i]->_branches.size(); ++ib) {
            ...
            if (branch->hasEndOfType(SMESH_MAT2d::BE_ON_VERTEX)) vertBranches.push_back(branch);
        }
        if (vertBranches.size() != 2 || braPoints[i]->_branches.size() != 3) continue;

        // get common EDGEs of two branches
        if (!getConnectedEdges(vertBranches[0], vertBranches[1], allEdges,
                               theShortEdges[nbBranchPoints > 0]))
            return false;
    ...
    if (nbBranchPoints != 2) return false;
    ...
    theSinuFace._sinuEdges.insert(theSinuFace._sinuEdges.end(), theSinuEdges[1].begin(),
                                  theSinuEdges[1].end());

    return (theShortEdges[0].size() > 0 && theShortEdges[1].size() > 0 && theSinuEdges[0].size() > 0
            && theSinuEdges[1].size() > 0);
...
TopoDS_Edge makeEdgeFromMA(SMESH_MesherHelper &theHelper, const SMESH_MAT2d::MedialAxis &theMA,
                           const double theMinSegLen)
{
    if (theMA.nbBranches() != 1) return TopoDS_Edge();

    vector<gp_XY> uv;
    theMA.getPoints(theMA.getBranch(0), uv);
    if (uv.size() < 2) return TopoDS_Edge();
    ...
    interpol.Perform();
    if (!interpol.IsDone()) return TopoDS_Edge();
...
TopAbs_ShapeEnum getHypShape(SMESH_Mesh *mesh, const TopoDS_Shape &edge)
{
    ...
    TopoDS_Shape shapeOfHyp = SMESH_MesherHelper::GetShapeOfHypothesis(hyps.front(), edge, mesh);
...
bool divideMA(SMESH_MesherHelper &theHelper, const SMESH_MAT2d::MedialAxis &theMA,
              const SinuousFace &theSinuFace, SMESH_Algo *the1dAlgo, const double theMinSegLen,
              vector<double> &theMAParams)
{
    ...
    if (branchEdge.IsNull()) return false;
    ...
    if (mostSimpleShape > -1) {
        ...
        if (!algo->CheckHypothesis(*mesh, edge, status)) algo = the1dAlgo;
    }
    ...
    try {
        ...
        if (!algo->Compute(tmpMesh, branchEdge)) return false;
    }
    catch (...) {
        return false;
    }
...
bool getParamsForEdgePair(const size_t theEdgePairInd,
                          ...
                          const vector<double> &theMAParams, vector<double> &theSelectedMAParams)
{
    if (theDivPoints.empty()) {
        theSelectedMAParams = theMAParams;
        return true;
    }
    if (theEdgePairInd > theDivPoints.size() || theMAParams.empty()) return false;
    ...
    if (theEdgePairInd > 0) {
        ...
        if (par1 - theMAParams[iPar1 - 1] < theMAParams[iPar1] - par1) --iPar1;
    }
    ...
    if (theEdgePairInd < theDivPoints.size()) {
        ...
        if (par2 - theMAParams[iPar2 - 1] < theMAParams[iPar2] - par2) --iPar2;
    }

    theSelectedMAParams.assign(theMAParams.begin() + iPar1, theMAParams.begin() + iPar2 + 1);
...
bool findVertexAndNode(NodePoint &theNodePnt, const vector<TopoDS_Edge> &theSinuEdges,
                       SMESHDS_Mesh *theMeshDS = 0, size_t theEdgeIndPrev = 0,
                       size_t theEdgeIndNext = 0)
{
    if (theNodePnt._edgeInd >= theSinuEdges.size()) return false;
...
bool projectVertices(SMESH_MesherHelper &theHelper, const SMESH_MAT2d::MedialAxis &theMA,
                     vector<SMESH_MAT2d::BranchPoint> &theDivPoints,
                     const vector<std::size_t> &theEdgeIDs1, const vector<std::size_t> &theEdgeIDs2,
                     const vector<bool> &theIsEdgeComputed, TMAPar2NPoints &thePointsOnE,
                     SinuousFace &theSinuFace)
{
    if (theDivPoints.empty()) return true;
    ...
    const SMESH_MAT2d::Branch &branch = *theMA.getBranch(0);
    {
        ...
        if (!branch.getBoundaryPoints(0., bp[0], bp[1])
            ...
            return false;
        if (!theSinuFace.IsRing() && !theMA.getBoundary().moveToClosestEdgeEnd(bp[1])) return false;
    ...
    if (!theSinuFace.IsRing()) {
        if (!branch.getBoundaryPoints(1., bp[0], bp[1])
            || !theMA.getBoundary().moveToClosestEdgeEnd(bp[0])
            || !theMA.getBoundary().moveToClosestEdgeEnd(bp[1]))
            return false;
        NodePoint np0(bp[0]), np1(bp[1]);
        findVertexAndNode(np0, theSinuEdges, meshDS);
        findVertexAndNode(np1, theSinuEdges, meshDS);
        ...
        if (!(theDivPoints[0]._iEdge == 0 && theDivPoints[0]._edgeParam == 0.)) // recursive call
        {
            ...
            projectVertices(theHelper, theMA, divPoint, edgeIDs1, edgeIDs2, theIsEdgeComputed,
                            thePointsOnE, theSinuFace);
        }
    }
    ...
    for (size_t i = 0; i < theDivPoints.size(); ++i) {
        if (!branch.getParameter(theDivPoints[i], uMA)) return false;
        if (!branch.getBoundaryPoints(theDivPoints[i], bp[0], bp[1])) return false;

        NodePoint np[2] = {NodePoint(bp[0]), NodePoint(bp[1])};
        bool isVertex[2] = {
        ...
        if (isVertex[0] != isVertex[1]) // try to find an opposite VERTEX
        {
            ...
            if (np2[iVert]._node == np[iVert]._node && np2[iNode]._node) {
                np[iNode] = np2[iNode];
                isVertex[iNode] = true;
            }
        }
        ...
        if (isVertex[0] && isVertex[1]) continue;

        bool isOppComputed = theIsEdgeComputed[np[iNode]._edgeInd];
        if (!isOppComputed) continue;
    ...
    for (u2NP = thePointsOnE.begin(); u2NP->first < 0;) thePointsOnE.erase(u2NP++);
    thePointsOnE.erase(1.1);

    return true;
}

double getUOnEdgeByPoint(const size_t iEdge, const NodePoint *point, SinuousFace &sinuFace)
{
    if (point->_edgeInd == iEdge) return point->_u;
...
void separateNodes(SMESH_MesherHelper &theHelper, const SMESH_MAT2d::MedialAxis &theMA,
                   TMAPar2NPoints &thePointsOnE, SinuousFace &theSinuFace,
                   const vector<bool> &theIsComputedEdge)
{
    if (thePointsOnE.size() < 2) return;
                ...
                for (; edgeID != edgeInds.end(); ++edgeID) {
                    ...
                    for (size_t i = 0; i < sameU2NP.size(); ++i) {
                        ...
                        if (!np->_node && *edgeID == np->_edgeInd) {
                            ...
                            if (theIsComputedEdge[*edgeID]) mergeNodes.push_back(np->_node);
                        }
                    }
                }
...
bool setQuadSides(SMESH_MesherHelper &theHelper, const TMAPar2NPoints &thePointsOnEdges,
                  SinuousFace &theFace, SMESH_Algo *the1dAlgo)
{
    SMESH_Mesh *mesh = theHelper.GetMesh();
    ...
    if (theFace.IsRing()) {
        ...
        if (thePointsOnEdges.size() < 4) return false;
        ...
        static_cast<StdMeshers_QuadFromMedialAxis_1D2D::Algo1D *>(the1dAlgo)->ComputeDistribution(
            theHelper, SMESH_TNodeXYZ(u2NPdist->second.first._node),
            SMESH_TNodeXYZ(u2NPdist->second.second._node), params);
        ...
        if (theFace._sinuSide[0].size() > 1) {
            ...
            for (i = 0; i < uvsOut.size(); ++i)
                if (nOut == uvsOut[i].node) break;
            if (i == uvsOut.size()) return false;
        ...
        if (nIn0 != nIn) {
            ...
            for (i = 0; i < uvsIn.size(); ++i)
                if (nIn == uvsIn[i].node) break;
            if (i == uvsIn.size()) return false;
...
bool computeSinuEdges(SMESH_MesherHelper &theHelper, double /*theMinSegLen*/,
                      SMESH_MAT2d::MedialAxis &theMA, vector<double> &theMAParams,
                      SinuousFace &theSinuFace, SMESH_Algo *the1dAlgo)
{
    if (theMA.nbBranches() != 1) return false;

    // normalize theMAParams
    for (size_t i = 0; i < theMAParams.size(); ++i) theMAParams[i] /= theMAParams.back();
    ...
    if (!allComputed) branch.getOppositeGeomEdges(edgeIDs1, edgeIDs2, divPoints);

    for (size_t i = 0; i < edgeIDs1.size(); ++i)
        if (isComputed[edgeIDs1[i]] && isComputed[edgeIDs2[i]]) {
            int nbNodes1 = meshDS->MeshElements(edgeIDs[edgeIDs1[i]])->NbNodes();
            int nbNodes2 = meshDS->MeshElements(edgeIDs[edgeIDs2[i]])->NbNodes();
            if (nbNodes1 != nbNodes2) return false;
            if ((i >= 1) && (edgeIDs1[i - 1] == edgeIDs1[i] || edgeIDs2[i - 1] == edgeIDs2[i]))
                return false;
            if ((i + 1 < edgeIDs1.size())
                && (edgeIDs1[i + 1] == edgeIDs1[i] || edgeIDs2[i + 1] == edgeIDs2[i]))
                return false;
        }
    ...
    {
        if (projectedEdges.count(edgeIDs1[iEdgePair]) || projectedEdges.count(edgeIDs2[iEdgePair]))
            continue;
        ...
        if (isComputed[edgeIDs1[iEdgePair]] != // one EDGE is meshed
            ...
            if (!isComputed[iEdgeComputed]) ++iSideComputed, iEdgeComputed = edgeIDs2[iEdgePair];

            map<double, const SMDS_MeshNode *> nodeParams; // params of existing nodes
            if (!SMESH_Algo::GetSortedNodesOnEdge(meshDS, theSinuEdges[iEdgeComputed],
                                                  /*skipMedium=*/true, nodeParams))
                return false;
            ...
            map<double, const SMDS_MeshNode *>::iterator u2n = nodeParams.begin(),
                                                         u2nEnd = nodeParams.end();
        ...
        // --------------------------------------------------------------------------------
            ...
            // "projection" from MA
            maParams.clear();
            if (!getParamsForEdgePair(iEdgePair, divPoints, theMAParams, maParams)) return false;

            for (size_t i = 1; i < maParams.size() - 1; ++i) {
                if (!branch.getBoundaryPoints(maParams[i], bp[0], bp[1])) return false;

                pointsOnE.insert(
                    pointsOnE.end(),
                    make_pair(maParams[i], make_pair(NodePoint(bp[0]), NodePoint(bp[1]))));
            }
        }
        // --------------------------------------------------------------------------------
            ...
            if (nodeParams[0].size() <= 2) continue; // nodes on VERTEXes only
            ...
            map<double, const SMDS_MeshNode *>::iterator u2n0F = ++nodeParams[0].begin(),
                                                         u2n1F = ++nodeParams[1].begin();
            map<double, const SMDS_MeshNode *>::reverse_iterator u2n1R = ++nodeParams[1].rbegin();
    ...
    if (!projectVertices(theHelper, theMA, divPoints, edgeIDs1, edgeIDs2, isComputed, pointsOnE,
                         theSinuFace))
        return false;
    ...
    for (size_t i = 0; i < theSinuEdges.size(); ++i) {
        SMESH_subMesh *sm = mesh->GetSubMesh(theSinuEdges[i]);
        if (sm->GetSubMeshDS() && sm->GetSubMeshDS()->NbElements() > 0) continue;
    ...
    if (!setQuadSides(theHelper, pointsOnE, theSinuFace, the1dAlgo)) return false;
...
bool computeShortEdges(SMESH_MesherHelper &theHelper, const vector<TopoDS_Edge> &theShortEdges,
                       SMESH_Algo *the1dAlgo, const bool theHasRadialHyp, const bool theIs2nd)
{
    SMESH_Hypothesis::Hypothesis_Status aStatus;
    for (size_t i = 0; i < theShortEdges.size(); ++i) {
        ...
        if (sm->IsEmpty()) {
            ...
            try {
                ...
                if (!the1dAlgo->Compute(*theHelper.GetMesh(), theShortEdges[i])) return false;
            }
            catch (...) {
                return false;
            }
...
bool ellipticSmooth(FaceQuadStruct::Ptr quad, int nbLoops)
{
    ...
    for (int iLoop = 0; iLoop < nbLoops; ++iLoop) {
        ...
        for (int i = 1; i < nbhoriz - 1; i++)
            for (int j = 1; j < nbvertic - 1; j++) {
                g11 = ((q.U(i, j + 1) - q.U(i, j - 1)) * (q.U(i, j + 1) - q.U(i, j - 1)) / dksi2
                       + (q.V(i, j + 1) - q.V(i, j - 1)) * (q.V(i, j + 1) - q.V(i, j - 1)) / deta2)
                    / 4;

                g22 = ((q.U(i + 1, j) - q.U(i - 1, j)) * (q.U(i + 1, j) - q.U(i - 1, j)) / dksi2
                       + (q.V(i + 1, j) - q.V(i - 1, j)) * (q.V(i + 1, j) - q.V(i - 1, j)) / deta2)
                    / 4;

                g12 = ((q.U(i + 1, j) - q.U(i - 1, j)) * (q.U(i, j + 1) - q.U(i, j - 1)) / dksi2
                       + (q.V(i + 1, j) - q.V(i - 1, j)) * (q.V(i, j + 1) - q.V(i, j - 1)) / deta2)
                    / (4 * dksi * deta);

                pNew.u = dksi2 / (2 * (g11 + g22))
                    * (g11 * (q.U(i + 1, j) + q.U(i - 1, j)) / dksi2
                       + g22 * (q.U(i, j + 1) + q.U(i, j - 1)) / dksi2
                       - 0.5 * g12 * q.U(i + 1, j + 1) + 0.5 * g12 * q.U(i - 1, j + 1)
                       + -0.5 * g12 * q.U(i - 1, j - 1) + 0.5 * g12 * q.U(i + 1, j - 1));

                pNew.v = deta2 / (2 * (g11 + g22))
                    * (g11 * (q.V(i + 1, j) + q.V(i - 1, j)) / deta2
        ...
        if (err / (nbhoriz - 2) / (nbvertic - 2) < 1e-6) break;
    }
...
void mergeNodes(SMESH_MesherHelper &theHelper, SinuousFace &theSinuFace)
{
                                                      ...
                                                      FaceQuadStruct::Ptr theQuad)
{
    ...
    if (!StdMeshers_Quadrangle_2D::setNormalizedGrid(theQuad)) return false;
    ...
    if (!isRing) ellipticSmooth(theQuad, 1);

    // create quadrangles
    bool ok;
    theHelper.SetElementsOnShape(true);
    if (nbNodesShort0 == nbNodesShort1)
        ok = StdMeshers_Quadrangle_2D::computeQuadDominant(*theHelper.GetMesh(), theQuad->face,
                                                           theQuad);
    else
        ok = StdMeshers_Quadrangle_2D::computeTriangles(*theHelper.GetMesh(), theQuad->face,
                                                        theQuad);
...
bool StdMeshers_QuadFromMedialAxis_1D2D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (getSinuousEdges(helper, sinuFace)) {
        ...
        if (!_regular1D) _regular1D = new Algo1D(_studyId, _gen);
        ...
        _progress = 0.8;
        if (_hyp2D) _regular1D->SetRadialDistribution(_hyp2D);
        ...
        if (ok) mergeNodes(helper, sinuFace);
...
bool StdMeshers_QuadFromMedialAxis_1D2D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
...
bool StdMeshers_QuadFromMedialAxis_1D2D::IsApplicable(const TopoDS_Shape &aShape, bool toCheckAll)
{
    TmpMesh tmpMesh;

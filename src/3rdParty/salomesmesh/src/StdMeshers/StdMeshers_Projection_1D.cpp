...
bool StdMeshers_Projection_1D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
        ...
        if (_sourceHypo->HasVertexAssociation()) {
            ...
            // PAL16202
                ...
                if (!_sourceHypo->IsCompoundSource()) {
                    if (!isSub) {
                        aStatus = HYP_BAD_PARAMETER;
                        SCRUTE((SMESH_MesherHelper::IsSubShape(_sourceHypo->GetTargetVertex(),
                                                               aShape)));
                    }
                }
                else if (isSub) {
                    ...
                    for (; ancestIt.More(); ancestIt.Next()) {
                        ...
                        if (ancestor.ShapeType() == TopAbs_EDGE
                            && SMESH_MesherHelper::IsSubShape(ancestor,
                                                              _sourceHypo->GetSourceEdge())) {
...
bool StdMeshers_Projection_1D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (!TAssocTool::FindSubShapeAssociation(tgtEdge, tgtMesh, srcShape, srcMesh, shape2ShapeMap)
    ...
    {
        for (i = 1; i < nbNodes; ++i) lengths[i - 1] = params[i] - params[i - 1];
    ...
    if (!nodes.front() || !nodes.back()) return error(COMPERR_BAD_INPUT_MESH, "No node on vertex");
    ...
    if (tgtLength <= 0) tgtLength = tgtParams.back() - tgtParams.front();
    ...
    if (tgtLength > 0) {
        ...
        for (i = 1; i < nbNodes - 1; ++i) {
            ...
            if (!Discret.IsDone()) return error("GCPnts_AbscissaPoint failed");
    ...
    if (elemIt->more()) quadratic = elemIt->next()->IsQuadratic();
    else {
    ...
    for (i = di; i < nbNodes; i += di) {
        if (quadratic) edge = meshDS->AddEdge(nodes[i - 2], nodes[i], nodes[i - 1]);
        else
...
bool StdMeshers_Projection_1D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                        MapShapeNbElems &aResMap)
{
    ...
    SMDS_ElemIteratorPtr elemIt = srcSubMesh->GetSubMeshDS()->GetElements();
    if (elemIt->more()) quadratic = elemIt->next()->IsQuadratic();
    if (quadratic) aVec[SMDSEntity_Quad_Edge] = (nbNodes - 1) / 2;
    else
...
{
    TAssocTool::SetEventListener(subMesh, _sourceHypo->GetSourceEdge(),

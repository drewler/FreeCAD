...
StdMeshers_Quadrangle_2D::StdMeshers_Quadrangle_2D(int hypId, int studyId, SMESH_Gen *gen)
    : SMESH_2D_Algo(hypId, studyId, gen), myQuadranglePreference(false),
      myTrianglePreference(false), myTriaVertexID(-1), myNeedSmooth(false), myCheckOri(false),
      myParams(NULL), myQuadType(QUAD_STANDARD), myHelper(NULL)
{
...
bool StdMeshers_Quadrangle_2D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                               SMESH_Hypothesis::Hypothesis_Status &aStatus)
{
    ...
    if (hyps.size() > 0) {
        ...
        if (strcmp("QuadrangleParams", aHyp->GetName()) == 0) {
            ...
            if (myQuadType == QUAD_QUADRANGLE_PREF || myQuadType == QUAD_QUADRANGLE_PREF_REVERSED)
    ...
    if (hyps.size() > 1) {
        ...
        if (isFirstParams) {
            ...
            const StdMeshers_QuadrangleParams *aHyp2 = (const StdMeshers_QuadrangleParams *)aHyp;
...
bool StdMeshers_Quadrangle_2D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (myQuadranglePreference) {
        ...
        int n24tmp = n24 / 2;
        n24tmp = n24tmp * 2;
        if ((n1 == n3 && n2 != n4 && n24tmp == n24) || (n2 == n4 && n1 != n3 && n13tmp == n13)) {
    ...
    if (res == NOT_COMPUTED) {
        if (n1 != n3 || n2 != n4) res = computeTriangles(aMesh, F, quad);
        else
            res = computeQuadDominant(aMesh, F);
    }

    if (res == COMPUTE_OK && myNeedSmooth) smooth(quad);

    if (res == COMPUTE_OK) res = check();
...
bool StdMeshers_Quadrangle_2D::computeTriangles(SMESH_Mesh &aMesh, const TopoDS_Face &aFace,
                                                FaceQuadStruct::Ptr quad)
{
    ...
    if (nb > nt) quad->shift(nl > nr ? 3 : 2, true);
    else if (nr > nl)
        quad->shift(1, true);
    else if (nl > nr)
        quad->shift(nt > nb ? 0 : 3, true);

    if (!setNormalizedGrid(quad)) return false;

    if (quad->nbNodeOut(QUAD_TOP_SIDE)) { splitQuad(quad, 0, quad->jSize - 2); }
    ...
    if (quad->nbNodeOut(QUAD_RIGHT_SIDE)) { splitQuad(quad, quad->iSize - 2, 0); }
    ...
    if (!computeQuadDominant(aMesh, aFace)) return false;
...
bool StdMeshers_Quadrangle_2D::computeQuadDominant(SMESH_Mesh &aMesh, const TopoDS_Face &aFace)
{
    if (!addEnforcedNodes()) return false;

    std::list<FaceQuadStruct::Ptr>::iterator quad = myQuadList.begin();
    for (; quad != myQuadList.end(); ++quad)
        if (!computeQuadDominant(aMesh, aFace, *quad)) return false;
                                                   ...
                                                   FaceQuadStruct::Ptr quad)
{
    ...
    int ilow = 0;
    int iup = nbhoriz - 1;
    if (quad->nbNodeOut(3)) { ilow++; }
    else {
        if (quad->nbNodeOut(1)) iup--;
    }

    int jlow = 0;
    int jup = nbvertic - 1;
    if (quad->nbNodeOut(0)) { jlow++; }
    else {
        if (quad->nbNodeOut(2)) jup--;
    }
    ...
    if (quad->nbNodeOut(0) && nbvertic == 2) // this should not occure
    {
        ...
        if (quad->nbNodeOut(QUAD_RIGHT_SIDE)) quad->UVPt(nbhoriz - 1, 1).node = uv_e1[1].node;
        if (quad->nbNodeOut(QUAD_LEFT_SIDE)) quad->UVPt(0, 1).node = uv_e3[1].node;
        ...
        for (i = 0; i < stop; i++) {
            ...
            if (near == g) { // make triangle
                ...
                if (near - 1 < ilow) d = uv_e3[1].node;
                else
                    d = quad->uv_grid[nbhoriz + near - 1].node;
                //SMDS_MeshFace* face = meshDS->AddFace(a, b, c, d);

                if (!myTrianglePreference) { myHelper->AddFace(a, b, c, d); }
                else {
        ...
        if (quad->nbNodeOut(2) && nbvertic == 2) {
            ...
            if (quad->side[3].grid->Edge(0).IsNull()) // left side is simulated one
            {
                ...
                if (quad->nbNodeOut(QUAD_LEFT_SIDE)) quad->UVPt(0, 0).node = uv_e3[nbleft - 2].node;
            ...
            // avoid creating zero-area triangles near a straight-angle corner
            {
                ...
                if (Abs(area) < 1e-20) {
                    ...
                    if (myTrianglePreference) { myHelper->AddFace(a, d, c); }
                    else {
            ...
            for (; i > stop; i--) {
                ...
                if (i == stop + 1) { // left bound reached, link with the leftmost node
                    ...
                    for (int k = g; k >= ilow; k--) {
                        const SMDS_MeshNode *nk;
                        if (k > iup) nk = uv_e1[nbright - 2].node;
                        else
                ...
                if (near == g) { // make triangle
                    ...
                    if (near + 1 > iup) d = uv_e1[nbright - 2].node;
                    else
                    ...
                    if (near + 1 < g) { // if d is not at g - make additional triangles
                        for (int k = near + 1; k < g; k++) {
                            ...
                            if (k + 1 > iup) d = uv_e1[nbright - 2].node;
                            else
    ...
    if (quad->nbNodeOut(QUAD_RIGHT_SIDE) && nbhoriz == 2) // this should not occure
    {
        ...
        for (; i < stop; i++) {
            ...
            if (i == stop - 1) { // up bondary reached
                ...
                for (int k = g; k <= jup; k++) {
                    const SMDS_MeshNode *nk;
                    if (k < jlow) nk = uv_e0[nbdown - 2].node;
                    else
            ...
            if (near == g) { // make triangle
                ...
                if (near - 1 < jlow) d = uv_e0[nbdown - 2].node;
                else
                ...
                if (near - 1 > g) { // if d not is at g - make additional triangles
                    for (int k = near - 1; k > g; k--) {
                        ...
                        if (k - 1 < jlow) d = uv_e0[nbdown - 2].node;
                        else
            ...
            for (; i > stop; i--) // loop on nodes on the left side
            {
                ...
                if (i == stop + 1) { // down bondary reached
                    ...
                    for (int k = g; k >= jlow; k--) {
                        const SMDS_MeshNode *nk;
                        if (k > jup) nk = quad->uv_grid[nbhoriz * jup + 1].node; //uv_e2[1].node;
                        else
                ...
                if (near == g) { // make triangle
                    ...
                    if (near + 1 > jup) d = quad->uv_grid[nbhoriz * jup + 1].node; //uv_e2[1].node;
                    else
...
bool StdMeshers_Quadrangle_2D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aFace,
...

{
    ...
    if (!checkNbEdgesForEvaluate(aMesh, aFace, aResMap, aNbNodes, IsQuadratic)) {
        ...
        smError.reset(
            new SMESH_ComputeError(COMPERR_ALGO_FAILED, "Submesh can not be evaluated", this));
        return false;
    }
...
bool StdMeshers_Quadrangle_2D::IsApplicable(const TopoDS_Shape &aShape, bool toCheckAll)
{
    ...
    for (TopExp_Explorer exp(aShape, TopAbs_FACE); exp.More(); exp.Next(), ++nbFoundFaces) {
        ...
        for (; eExp.More() && nbNoDegenEdges < 3; eExp.Next()) {
            if (!SMESH_Algo::isDegenerated(TopoDS::Edge(eExp.Current()))) ++nbNoDegenEdges;
        }
...
static bool twoEdgesMeatAtVertex(const TopoDS_Edge &e1, const TopoDS_Edge &e2, SMESH_Mesh &mesh)
{
    ...
    for (; ancestIt.More(); ancestIt.Next())
        if (ancestIt.Value().ShapeType() == TopAbs_EDGE)
            if (!e1.IsSame(ancestIt.Value()) && !e2.IsSame(ancestIt.Value())) return false;
    return true;
}
...
{
    if (!myQuadList.empty() && myQuadList.front()->face.IsSame(aShape)) return myQuadList.front();
    ...
    if (nbSides == 0) { return FaceQuadStruct::Ptr(); }
    ...
    if (nbSides == 3) // 3 sides and corners[0] is a vertex with myTriaVertexID
    {
        for (int iSide = 0; iSide < 3; ++iSide) {
            ...
            while (edgeIt != edges.end()
                ...
                if (SMESH_Algo::isDegenerated(*edgeIt)) ++edgeIt;
                else
                    sideEdges.push_back(*edgeIt++);
            if (!sideEdges.empty())
                quad->side.push_back(StdMeshers_FaceSide::New(
                    F, sideEdges, &aMesh, iSide < QUAD_TOP_SIDE, ignoreMediumNodes, myProxyMesh));
            else
                --iSide;
        }
...

{
    ...
    if (nbEdgesInWire.front() == 3) { // exactly 3 edges
        if (myTriaVertexID > 0) {
            ...
            if (!V.IsNull()) {
                ...
                if (anIt == aResMap.end()) return false;
                std::vector<int> aVec = (*anIt).second;
                if (IsQuadratic) aNbNodes[0] = (aVec[SMDSEntity_Node] - 1) / 2 + 2;
                else
                ...
                if (anIt == aResMap.end()) return false;
                aVec = (*anIt).second;
                if (IsQuadratic) aNbNodes[1] = (aVec[SMDSEntity_Node] - 1) / 2 + 2;
                else
                ...
                if (anIt == aResMap.end()) return false;
                aVec = (*anIt).second;
                if (IsQuadratic) aNbNodes[2] = (aVec[SMDSEntity_Node] - 1) / 2 + 2;
                else
    ...
    if (nbEdgesInWire.front() == 4) { // exactly 4 edges
        for (; edgeIt != edges.end(); edgeIt++) {
            ...
            if (anIt == aResMap.end()) { return false; }
            std::vector<int> aVec = (*anIt).second;
            if (IsQuadratic) aNbNodes[nbSides] = (aVec[SMDSEntity_Node] - 1) / 2 + 2;
            else
        ...
        while (!edges.empty()) {
            sideEdges.clear();
            sideEdges.splice(sideEdges.end(), edges,
                             edges.begin()); // edges.front() -> sideEdges.end()
            bool sameSide = true;
            while (!edges.empty() && sameSide) {
                sameSide = SMESH_Algo::IsContinuous(sideEdges.back(), edges.front());
                if (sameSide) sideEdges.splice(sideEdges.end(), edges, edges.begin());
            }
            if (nbSides == 0) { // go backward from the first edge
                sameSide = true;
                while (!edges.empty() && sameSide) {
                    sameSide = SMESH_Algo::IsContinuous(sideEdges.front(), edges.back());
                    if (sameSide) sideEdges.splice(sideEdges.begin(), edges, --edges.end());
                }
            }
            ...
            for (; ite != sideEdges.end(); ite++) {
                ...
                if (anIt == aResMap.end()) { return false; }
                std::vector<int> aVec = (*anIt).second;
                if (IsQuadratic) aNbNodes[nbSides] += (aVec[SMDSEntity_Node] - 1) / 2 + 1;
                else
    ...
    if (nbSides != 4) {
        if (!nbSides) nbSides = nbEdgesInWire.front();
                                                                ...
                                                                const bool CreateQuadratic)
{
    ...
    if (quad) {
        // set normalized grid on unit square in parametric domain
        if (!setNormalizedGrid(quad)) quad.reset();
    }
    return quad;
}
...
inline const vector<UVPtStruct> &getUVPtStructIn(FaceQuadStruct::Ptr &quad, int i, int nbSeg)
{
    ...
    return quad->nbNodeOut(i) ? quad->side[i].grid->SimulateUVPtStruct(nbSeg, isXConst, constValue)
                              : quad->side[i].grid->GetUVPtStruct(isXConst, constValue);
}
inline gp_UV calcUV(double x, double y, const gp_UV &a0, const gp_UV &a1, const gp_UV &a2,
                    const gp_UV &a3, const gp_UV &p0, const gp_UV &p1, const gp_UV &p2,
                    const gp_UV &p3)
{
    return ((1 - y) * p0 + x * p1 + y * p2 + (1 - x) * p3)
...
{
    if (!quad->uv_grid.empty()) return true;
    ...
    if (nbhoriz < 1 || nbvertic < 1) return error("Algo error: empty quad");
    ...
    { // LEFT
        ...
        int j = 0, nb = nbvertic;
        sideIter.Init(lSide);
        if (quad->UVPt(i, j).node) ++j, sideIter.Next(); // avoid copying from a split emulated side
...
void FaceQuadStruct::shift(size_t nb, bool ori, bool keepGrid)
{
    ...
    for (int i = QUAD_BOTTOM_SIDE; i < NB_QUAD_SIDES; ++i) {
        ...
        if (ori) {
            ...
            if (wasForward != newForward) side[i].Reverse(keepGrid);
        }
        newSides[id] = side[i];
        sidePtrs[i] = &side[i];
    }
    ...
    for (size_t i = 0; i < newSides.size(); ++i) {
        ...
        for (size_t iC = 0; iC < ns.contacts.size(); ++iC) {
            ...
            if (sIt != sidePtrs.end()) ns.contacts[iC].other_side = &newSides[*sIt - sidePtrs[0]];
        }
    }
    newSides.swap(side);
...
static gp_UV calcUV(double x0, double x1, double y0, double y1, FaceQuadStruct::Ptr &quad,
...
static gp_UV calcUV2(double x, double y, FaceQuadStruct::Ptr &quad, const gp_UV &a0,
                     const gp_UV &a1, const gp_UV &a2, const gp_UV &a3)
{
    gp_UV p0 = quad->side[QUAD_BOTTOM_SIDE].grid->Value2d(x).XY();
    gp_UV p1 = quad->side[QUAD_RIGHT_SIDE].grid->Value2d(y).XY();
    gp_UV p2 = quad->side[QUAD_TOP_SIDE].grid->Value2d(x).XY();
...
bool StdMeshers_Quadrangle_2D::computeQuadPref(SMESH_Mesh &aMesh, const TopoDS_Face &aFace,
                                               FaceQuadStruct::Ptr quad)
{
    ...
    if (myForcedPnts.empty()) {
        ...
        // dh >= dv and nt > nb
        if (dh >= dv) shiftQuad(quad, (nt > nb) ? 0 : 2);
        else
            shiftQuad(quad, (nr > nl) ? 1 : 3);
    }
    else {
        // rotate the quad to have nt > nb [and nr > nl]
        if (nb > nt) shiftQuad(quad, nr > nl ? 1 : 2);
        else if (nr > nl)
    ...
    const int lfrom = quad->side[3].from;
    {
        ...
        if (uv_eb_vec.empty() || uv_er_vec.empty() || uv_et_vec.empty() || uv_el_vec.empty())
            return error(COMPERR_BAD_INPUT_MESH);
    }
    ...
    if (!myForcedPnts.empty()) {
        ...
        double yCbL, yCbR;
        {
            ...
            double lLwL = quad->side[2].Length(tfrom, tfrom + (lw)*dt);
            yCbL = lLwL / (lLwL + lL);

            double lR = quad->side[1].Length();
            double lLwR =
                quad->side[2].Length(tfrom + (lw + nb - 1) * dt, tfrom + (lw + nb - 1 + lw) * dt);
            yCbR = lLwR / (lLwR + lR);
        }
    ...
    for (i = 0; i < nb; i++) { npb.Append(uv_eb[i].normParam); }
    for (i = 0; i < nr; i++) { npr.Append(uv_er[i].normParam); }
    for (i = 0; i < nt; i++) { npt.Append(uv_et[i].normParam); }
    for (i = 0; i < nl; i++) { npl.Append(uv_el[i].normParam); }
    ...
    if (OldVersion) {
        ...
        for (i = 1; i <= dr; i++) { npr.InsertAfter(1, npr.Value(2) - dpr); }
        // insert to left
        dl = nbv - nl;
        dpr = (npl.Value(2) - npl.Value(1)) / (dl + 1);
        for (i = 1; i <= dl; i++) { npl.InsertAfter(1, npl.Value(2) - dpr); }
    }
    ...
    if (OldVersion) {
        ...
        // add left nodes
        for (j = 1; j <= nl; j++) NodesL.SetValue(1, j, uv_el[j - 1].node);
        if (dl > 0) {
            // add top nodes
            for (i = 1; i <= dl; i++) NodesL.SetValue(i + 1, nl, uv_et[i].node);
        ...
        // add right nodes
        for (j = 1; j <= nr; j++) NodesR.SetValue(1, j, uv_er[nr - j].node);
        if (dr > 0) {
            // add top nodes
            for (i = 1; i <= dr; i++) NodesR.SetValue(i + 1, 1, uv_et[nt - 1 - i].node);
            // create and add needed nodes
            TColgp_SequenceOfXY UVtmp;
            for (i = 1; i <= dr; i++) {
        ...
        for (i = 1; i <= dl + 1; i++) NodesC.SetValue(1, i, NodesL(i, 1));
        for (i = 2; i <= nl; i++) NodesC.SetValue(1, dl + i, NodesL(dl + 1, i));
        // add last line using NodesR
        for (i = 1; i <= dr + 1; i++) NodesC.SetValue(nb, i, NodesR(i, nr));
        for (i = 1; i < nr; i++) NodesC.SetValue(nb, dr + i + 1, NodesR(dr + 1, nr - i));
        // add top nodes (last columns)
        for (i = dl + 2; i < nbh - dr; i++) NodesC.SetValue(i - dl, nbv, uv_et[i - 1].node);
        // add bottom nodes (first columns)
        for (i = 2; i < nb; i++) NodesC.SetValue(i, 1, uv_eb[i - 1].node);
        ...
        for (j = 0; j < nb; j++) { NodesBRD.SetValue(j + 1, 1, uv_eb[j].node); }
        for (i = 1; i < nnn - 1; i++) {
        ...
        for (j = 1; j <= nb; j++) { NodesC.SetValue(j, 1, NodesBRD.Value(j, nnn - 1)); }
        if ((drl + addv) > 0) {
            int n1, n2;
            if (nr > nl) {
                n1 = 1;
            ...
            for (i = 1; i <= nt; i++) { NodesLast.SetValue(i, 2, uv_et[i - 1].node); }
            int nnn = 0;
...
bool StdMeshers_Quadrangle_2D::evaluateQuadPref(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                                std::vector<int> &aNbNodes,
                                                MapShapeNbElems &aResMap, bool IsQuadratic)
{
    // Auxilary key in order to keep old variant
    // of meshing after implementation new variant
    // for bug 0016220 from Mantis.
    bool OldVersion = false;
    if (myQuadType == QUAD_QUADRANGLE_PREF_REVERSED) OldVersion = true;
...
void StdMeshers_Quadrangle_2D::splitQuadFace(SMESHDS_Mesh *theMeshDS, int theFaceID,
...
inline SMDS_MeshNode *makeNode(UVPtStruct &uvPt, const double y, FaceQuadStruct::Ptr &quad,
                               const gp_UV *UVs, SMESH_MesherHelper *helper, Handle(Geom_Surface) S)
{
    ...
    int iTop = int(rTop);
    double xBot =
        uv_eb[iBot].normParam + (rBot - iBot) * (uv_eb[iBot + 1].normParam - uv_eb[iBot].normParam);
    double xTop =
        uv_et[iTop].normParam + (rTop - iTop) * (uv_et[iTop + 1].normParam - uv_et[iTop].normParam);
...
void reduce42(const vector<UVPtStruct> &curr_base, vector<UVPtStruct> &next_base, const int j,
              int &next_base_len, FaceQuadStruct::Ptr &quad, gp_UV *UVs, const double y,
              SMESH_MesherHelper *helper, Handle(Geom_Surface) & S)
{
    ...
    if (!Na) Na = makeNode(next_base[next_base_len], y, quad, UVs, helper, S);

    // b (i + 1, j + 4)
    const SMDS_MeshNode *&Nb = next_base[++next_base_len].node;
    if (!Nb) Nb = makeNode(next_base[next_base_len], y, quad, UVs, helper, S);
    ...
    // Faces
    helper->AddFace(curr_base[j + 0].node, curr_base[j + 1].node, Nc,
                    next_base[next_base_len - 2].node);

    helper->AddFace(curr_base[j + 1].node, curr_base[j + 2].node, Nd, Nc);

    helper->AddFace(curr_base[j + 2].node, curr_base[j + 3].node, Ne, Nd);

    helper->AddFace(curr_base[j + 3].node, curr_base[j + 4].node, Nb, Ne);
...
void reduce31(const vector<UVPtStruct> &curr_base, vector<UVPtStruct> &next_base, const int j,
              int &next_base_len, FaceQuadStruct::Ptr &quad, gp_UV *UVs, const double y,
              SMESH_MesherHelper *helper, Handle(Geom_Surface) & S)
{
    ...
    // 2
    helper->AddFace(curr_base[j + 1].node, curr_base[j + 2].node, Ne, Nc);
    // 4
    helper->AddFace(curr_base[j + 2].node, curr_base[j + 3].node, Nb, Ne);
    // 5
    helper->AddFace(Nc, Ne, Nb, next_base[next_base_len - 1].node);
}

typedef void (*PReduceFunction)(const vector<UVPtStruct> &curr_base, vector<UVPtStruct> &next_base,
                                const int j, int &next_base_len, FaceQuadStruct::Ptr &quad,
                                gp_UV *UVs, const double y, SMESH_MesherHelper *helper,
...
bool StdMeshers_Quadrangle_2D::computeReduced(SMESH_Mesh &aMesh, const TopoDS_Face &aFace,
                                              FaceQuadStruct::Ptr quad)
{
    SMESHDS_Mesh *meshDS = aMesh.GetMeshDS();
    Handle(Geom_Surface) S = BRep_Tool::Surface(aFace);
    ...
    bool MultipleReduce = false;
    {
        ...
        int nrows_tree31 = int(ceil(log(double(ncol_bot) / ncol_top) / log(3.))); // = log x base 3
    ...
    if (MultipleReduce) { // == computeQuadPref QUAD_QUADRANGLE_PREF_REVERSED
        ...
        // arrays for normalized params
        TColStd_SequenceOfReal npb, npr, npt, npl;
        for (j = 0; j < nb; j++) { npb.Append(uv_eb[j].normParam); }
        for (i = 0; i < nr; i++) { npr.Append(uv_er[i].normParam); }
        for (j = 0; j < nt; j++) { npt.Append(uv_et[j].normParam); }
        for (i = 0; i < nl; i++) { npl.Append(uv_el[i].normParam); }
        ...
        int nrows_tree42 =
            int(log((double)(ncol_bot / ncol_top))
                / log((double)2)); // needed to avoid overflow at pow(2) while computing max_tree42
        if (nrows_tree42 < nrows) {
            ...
            if (ncol_top > npair_top * 2) {
                ...
                if (delta <= (ncol_top - npair_top * 2)) max_tree42 = ncol_bot;
            }
        }
        ...
        if (true) { // ------------------------------------------------------------------
            ...
            if (ncol_bot > max_tree42) { is_lin_31 = true; }
            else {
                if ((ncol_top / 3) * 3 == ncol_top) { is_lin_31 = true; }
                else {
                    is_lin_42 = true;
                }
            }
            ...
            if (nb_col * col_top_size >= nt) // == "tree" reducing situation
            {
                ...
                while (remainder > 0) {
                    ...
                    int nb_col_free =
                        cur_top_size / col_top_size - nb_col_by_row[nbrows_not_full - 1];
                    if (nb_col_add > nb_col_free) nb_col_add = nb_col_free;
            ...
            else // == "linear" reducing situation
            {
                nb_col_by_row.resize(nrows, nb_col);
                if (remainder > 0)
                    for (int irow = remainder / 2; irow < nrows; ++irow) nb_col_by_row[irow]--;
            }
            ...
            for (i = 1; i < nr; i++) // layer by layer
            {
                ...
                for (j = 0; j < free_left; j++) {
                    ...
                    if (!Nf) Nf = makeNode(next_base[next_base_len], y, quad, uv, myHelper, S);

                    myHelper->AddFace(curr_base[j].node, curr_base[j + 1].node, Nf,
                                      next_base[next_base_len - 1].node);
                }

                for (int icol = 1; icol <= nb_col; icol++) {
                    // add "H"
                    reduceFunction(curr_base, next_base, j, next_base_len, quad, uv, y, myHelper,
                                   S);
            ...
            for (i = 1; i < nr; i++) // layer by layer
            {
                ...
                int nb_next = nb_reduce_groups * (reduce_grp_size - 2)
                    + (curr_base_len - nb_reduce_groups * reduce_grp_size);
                if (nb_next < nt) nb_next = nt;
                ...
                for (j = 0; j + reduce_grp_size < curr_base_len && delta > 0;
                     j += reduce_grp_size, delta -= 2) {
                    reduceFunction(curr_base, next_base, j, next_base_len, quad, uv, y, myHelper,
                                   S);
                }
...
void StdMeshers_Quadrangle_2D::updateDegenUV(FaceQuadStruct::Ptr quad)
{
    if (myNeedSmooth)
        ...
        for (unsigned i = 0; i < quad->side.size(); ++i) {
            ...
            int degenInd = -1;
            if (myHelper->IsDegenShape(uvVec[0].node->getshapeId())) degenInd = 0;
            ...
            if (i >= QUAD_TOP_SIDE) isPrev = !isPrev;
            int i2 = (isPrev ? (i + 3) : (i + 1)) % 4;
            const vector<UVPtStruct> &uvVec2 = quad->side[i2].GetUVPtStruct();
            int degenInd2 = -1;
            if (uvVec[degenInd].node == uvVec2.front().node) degenInd2 = 0;
        ...
        for (size_t i = 0; i < quad->side.size(); ++i) {
            ...
            if (!myHelper->IsDegenShape(degSide->EdgeID(0))) continue;
            StdMeshers_FaceSidePtr oppSide = quad->side[(i + 2) % quad->side.size()];
            if (degSide->NbSegments() == oppSide->NbSegments()) continue;
...
void StdMeshers_Quadrangle_2D::smooth(FaceQuadStruct::Ptr quad)
{
    ...
    while (nIt->more()) // loop on nodes bound to a FACE
    {
        ...
        if (fixedNodes.count(node)) continue; // fixed - no triangles
        ...
        while (fIt->more()) {
            ...
            sNode._triangles.push_back(TTriangle(&smooNoMap[prevNode], &smooNoMap[nextNode]));
        }
    }
    ...
    for (; n2sn != smooNoMap.end(); ++n2sn)
        if (!n2sn->second._triangles.empty()) break;
    ...
    for (int iLoop = 0; iLoop < 5; ++iLoop) {
        for (n2sn = smooNoMap.begin(); n2sn != smooNoMap.end(); ++n2sn) {
            ...
            if (sNode._triangles.empty()) continue; // not movable node
...
bool StdMeshers_Quadrangle_2D::check()
{
    ...
    if (!myCheckOri || myQuadList.empty() || !myQuadList.front() || !myHelper) return isOK;
    ...
    double okSign;
    {
        ...
        for (int i = 0; i < wire->NbEdges(); ++i) {
            ...
            if ((maxAngle < angle) && (5. * M_PI / 180 < angle && angle < 175. * M_PI / 180)) {
                maxAngle = angle;
                iVertex = i;
            }
        }
    ...
    while (fIt->more()) // loop on faces bound to a FACE
    {
        ...
        for (int i = 0; i < nbN; ++i) nn[i] = f->GetNode(i);

        const SMDS_MeshNode *nInFace = 0;
        if (myHelper->HasSeam())
            for (int i = 0; i < nbN && !nInFace; ++i)
                if (!myHelper->IsSeamShape(nn[i]->getshapeId())) nInFace = nn[i];
        ...
        switch (nbN) {
            case 4: {
                ...
                if (sign1 * sign2 < 0) {
                    ...
                    if (sign1 * sign2 < 0) continue; // this should not happen
                }
                if (sign1 * okSign < 0) badFaces.push_back(f);
                break;
            }
            case 3: {
                double sign = getArea(uv[0], uv[1], uv[2]);
                if (sign * okSign < 0) badFaces.push_back(f);
                break;
            }
            default:;
        }
    ...
    if (!badFaces.empty()) {
        ...
        err.reset(new SMESH_ComputeError(COMPERR_ALGO_FAILED, "Inverted elements generated"));
...
int StdMeshers_Quadrangle_2D::getCorners(const TopoDS_Face &theFace, SMESH_Mesh &theMesh,
                                         std::list<TopoDS_Edge> &theWire,
                                         std::vector<TopoDS_Vertex> &theVertices,
                                         int &theNbDegenEdges, const bool theConsiderMesh)
{
    theNbDegenEdges = 0;
    ...
    if (SMESH_Algo::isDegenerated(prevE)) {
        ...
        while (SMESH_Algo::isDegenerated(*edge)) ++edge;
        if (edge == theWire.rend()) return false;
        prevE = *edge;
    }
    ...
    if (!triaVertex.IsNull() && triaVertex.ShapeType() == TopAbs_VERTEX
    ...
    if (nbCorners == 3) vMap.Add(triaVertex);
    ...
    myCheckOri = (vertexByAngle.size() > nbCorners || vertexByAngle.begin()->first < angleTol);
    ...
    if (haveVariants) {
        if (nbCorners == 3) angles[0] = 2 * M_PI; // not to move the base triangle VERTEX
        ...
        if (nbSegHalf) {
            ...
            for (TGeoIndex iV1 = 0; iV1 < TGeoIndex(angles.size()); ++iV1) {
                ...
                if (nbSegs == nbSegHalf && angles[iV1] + angleTol >= minAngle
        ...
        for (size_t iC = 0; iC < cornerInd.size(); ++iC) {
            ...
            if (!treatedCorners.insert(iV).second) continue;
            ...
            for (int isFwd = 0; isFwd < 2; ++isFwd) {
                ...
                while (iVNext != iV) {
                    ...
                    if (equal) equVerts.insert(isFwd ? equVerts.end() : equVerts.begin(), iVNext);
                    if (iVNext == cornerInd[iCNext]) {
                        if (!equal) {
                            if (angles[iV] < angles[iVNext]) refinedCorners.insert(iVNext);
                            break;
                        }
                        nbC[isFwd]++;
                ...
                if (iVNext == iV) break; // all angles equal
            }
            ...
            if (nbExcessV > 0) // there is nbExcessV vertices that can become corners
            {
                ...
                for (size_t iS = 0; iS < accuLength.size(); ++iS) accuLength[iS] /= totalLen;
                ...
                for (int iS = 1; iS < nbSides; ++iS, ++iBestEV) {
                    ...
                    if (iBestEV > iS - 1 + nbExcessV) iBestEV = iS - 1 + nbExcessV;
...
bool StdMeshers_Quadrangle_2D::getEnforcedUV()
{
    ...
    for (size_t i = 0; i < shapes.size(); ++i)
        if (!shapes[i].IsNull()) TopExp::MapShapes(shapes[i], TopAbs_VERTEX, vMap);
    ...
    bool isStrictCheck = false;
    {
        ...
        if (myHelper->GetMesh()->GetHypothesis(myHelper->GetSubShape(), paramFilter,
                                               /*ancestors=*/true, &assignedTo))
    ...
    for (size_t iP = 0; iP < points.size(); ++iP) {
        ...
        if (project->Gap() > farTol) {
            if (isStrictCheck && iP < nbPoints)
                return error(COMPERR_BAD_PARMETERS,
                             TComm("An enforced point is too far from the face, dist = ")
                                 << points[iP].Distance(project->Value(uv)) << " - ("
                                 << points[iP].X() << ", " << points[iP].Y() << ", "
                                 << points[iP].Z() << " )");
            continue;
        }
        ...
        switch (clsf.State()) {
            case TopAbs_IN: {
                ...
                if (iP >= nbPoints) fp.vertex = TopoDS::Vertex(vMap(iP - nbPoints + 1));

                TUV2VMap::iterator uv2v =
                    uv2intV.lower_bound(make_pair(uv.X() - tol, uv.Y() - tol));
                for (; uv2v != uv2intV.end() && uv2v->first.first <= uv.X() + tol; ++uv2v)
                    if (uv.SquareDistance(gp_Pnt2d(uv2v->first.first, uv2v->first.second))
                        < tol * tol) {
                ...
                if (myHelper->IsSubShape(fp.vertex, myHelper->GetMesh())) {
                    ...
                    fp.node = myHelper->AddNode(fp.xyz.X(), fp.xyz.Y(), fp.xyz.Z(), 0, fp.uv.X(),
                                                fp.uv.Y());
                }
            ...
            case TopAbs_OUT: {
                if (isStrictCheck && iP < nbPoints)
                    return error(COMPERR_BAD_PARMETERS,
                                 TComm("An enforced point is out of the face boundary - ")
                                     << points[iP].X() << ", " << points[iP].Y() << ", "
                                     << points[iP].Z() << " )");
                break;
            }
            case TopAbs_ON: {
                if (isStrictCheck && iP < nbPoints)
                    return error(COMPERR_BAD_PARMETERS,
                                 TComm("An enforced point is on the face boundary - ")
                                     << points[iP].X() << ", " << points[iP].Y() << ", "
                                     << points[iP].Z() << " )");
                break;
            }
            default: {
                if (isStrictCheck && iP < nbPoints)
                    return error(TComm("Classification of an enforced point ralative to the face "
                                       "boundary failed - ")
                                 << points[iP].X() << ", " << points[iP].Y() << ", "
                                 << points[iP].Z() << " )");
            }
    ...
    for (; d2uv != sortedFP.end(); ++d2uv) myForcedPnts.push_back((*d2uv).second);
...
bool StdMeshers_Quadrangle_2D::addEnforcedNodes()
{
    ...
    for (; quadIt != myQuadList.end(); ++quadIt)
        for (size_t iSide = 0; iSide < (*quadIt)->side.size(); ++iSide) {
            if (!setNormalizedGrid(*quadIt)) return false;
    ...
    for (size_t iFP = 0; iFP < myForcedPnts.size(); ++iFP) {
        ...
        for (quadIt = myQuadList.begin(); quadIt != myQuadList.end(); ++quadIt) {
            ...
            if (!setNormalizedGrid(*quadIt)) return false;
            int i, j;
            if (!quad->findCell(myForcedPnts[iFP], i, j)) continue;
            ...
            for (int di = 0; di < 2; ++di)
                for (int dj = 0; dj < 2; ++dj) {
                    double dist2 =
                        (myForcedPnts[iFP].uv - quad->UVPt(i + di, j + dj).UV()).SquareModulus();
            ...
            for (; !isNodeEnforced && d2ij != ijByDist.end(); ++d2ij) {
                ...
                int iSide = -1;
                if (dj == 0 && j == 0) iSide = QUAD_BOTTOM_SIDE;
                        ...
                        if (side.contacts[i].point == sideNodeIndex) {
                            ...
                            if (adjQuads.size() > 1
                                ...
                                updateSideUV(*side.contacts[i].other_side,
                                             side.contacts[i].other_point, quadsBySide);
                            }
            ...
            for (; qIt != changedQuads.end(); ++qIt) (*qIt)->uv_grid.clear();

            if (isNodeEnforced) break;
        ...
        if (!isNodeEnforced) {
            if (!myForcedPnts[iFP].vertex.IsNull())
                ...
                return error(TComm("Unable to move any node to point ( ")
                             << myForcedPnts[iFP].xyz.X() << ", " << myForcedPnts[iFP].xyz.Y()
    ...
    for (quadIt = myQuadList.begin(); quadIt != myQuadList.end(); ++quadIt) {
        ...
        for (int iSide = 0; iSide < 4; ++iSide) {
            FaceQuadStruct::Side &side = quad->side[iSide];
            if (side.nbNodeOut > 0) continue; // emulated side
            vector<FaceQuadStruct::Ptr> &quadVec = quadsBySide[side];
            if (quadVec.size() <= 1) continue; // outer side

            const vector<UVPtStruct> &points = side.grid->GetUVPtStruct();
            for (size_t iC = 0; iC < side.contacts.size(); ++iC) {
                if (side.contacts[iC].point < side.from || side.contacts[iC].point >= side.to)
                    continue;
                if (side.contacts[iC].other_point < side.contacts[iC].other_side->from
                    ...
                    continue;
                const vector<UVPtStruct> &oGrid =
                    side.contacts[iC].other_side->grid->GetUVPtStruct();
                ...
                if (side.contacts[iC].other_point >= oGrid.size()
                    ...
                    throw SALOME_Exception(
                        "StdMeshers_Quadrangle_2D::addEnforcedNodes(): wrong contact");
            ...
            if (missedNodesOnSide) {
                ...
                for (size_t iQ = 0; iQ < quadVec.size(); ++iQ) quadVec[iQ]->uv_grid.clear();
            }
        }
    }
...
int StdMeshers_Quadrangle_2D::splitQuad(FaceQuadStruct::Ptr quad, int I, int J)
{
    ...
    if (I > 0 && I <= quad->iSize - 2) {
        ...
        for (int jP = 0; jP < quad->jSize; ++jP) points.push_back(quad->UVPt(I, jP));
    ...
    else if (J > 0 && J <= quad->jSize - 2) //// split horizontally, a new quad is below an old one
    {
        ...
        for (int iP = 0; iP < quad->iSize; ++iP) points.push_back(quad->UVPt(iP, J));
...
void StdMeshers_Quadrangle_2D::updateSideUV(FaceQuadStruct::Side &side, int iForced,
                                            const TQuadsBySide &quadsBySide, int *iNext)
{
    if (!iNext) {
        ...
        if (iForced + 1 < iEnd) updateSideUV(side, iForced, quadsBySide, &iEnd);

        iIt = side.forced_nodes.lower_bound(iForced);
        int iBeg = Max(0, (iIt == side.forced_nodes.begin()) ? 0 : *--iIt);
        if (iForced - 1 > iBeg) updateSideUV(side, iForced, quadsBySide, &iBeg);
        ...
        while (points[is2nd].size() < sideSize) {
            ...
            for (size_t iQ = 0; iQ < quads.size(); ++iQ) {
                ...
                for (iS = 0; iS < q->side.size(); ++iS)
                    if (side.grid == q->side[iS].grid) break;
                bool isOut;
                ...
                if (isOut) continue;
                if (!setNormalizedGrid(q)) continue;
                ...
                for (; nb > 0; --nb) {
                    ...
                    if (points[is2nd].size() >= sideSize) break;
                    i += di;
                    j += dj;
                }
            ...
            if (nbLoops++ > quads.size())
                throw SALOME_Exception(
                    "StdMeshers_Quadrangle_2D::updateSideUV() bug: infinite loop");
...
bool FaceQuadStruct::findCell(const gp_XY &UV, int &I, int &J)
{
    ...
    if (uv_box.IsOut(UV)) return false;
        ...
        for (int isU = 0; isU < 2; ++isU) {
            ...
            for (i = 0; i < nb; ++i, ind1 += di1, ind2 += di2)
                if ((dist2 = (UV - UVPt(ind1, ind2).UV()).SquareModulus()) < minDist2) {
                    I = ind1;
                    J = ind2;
                    if (isNear(UV, I, J)) return true;
                    minDist2 = (UV - UVPt(I, J).UV()).SquareModulus();
                }
        }
        if (!isNear(UV, I, J, Max(iSize, jSize) / 2)) return false;
    }
    return true;
}
...
void FaceQuadStruct::normPa2IJ(double X, double Y, int &I, int &J)
{
    ...
    do {
        oldI = I, oldJ = J;
        while (X <= UVPt(I, J).x && I != 0) --I;
        while (X > UVPt(I + 1, J).x && I + 2 < iSize) ++I;
        while (Y <= UVPt(I, J).y && J != 0) --J;
        while (Y > UVPt(I, J + 1).y && J + 2 < jSize) ++J;
    } while (oldI != I || oldJ != J);
}
...
bool FaceQuadStruct::isNear(const gp_XY &UV, int &I, int &J, int nbLoops)
{
    ...
    for (int iLoop = 0; iLoop < nbLoops; ++iLoop) {
        ...
        if (bcI >= 0. && bcJ >= 0. && bcI + bcJ <= 1.) return true;
...
bool FaceQuadStruct::isEqual(const gp_XY &UV, int I, int J)
{
    ...
    for (int di = -1; di < 2; di += 2) {
        ...
        for (int dj = -1; dj < 2; dj += 2) {
            ...
            dist2 = Min(dist2, p2.SquareDistance(surf->Value(UVPt(i, j).u, UVPt(i, j).v)));
        }
    }
...
int FaceQuadStruct::Side::ToSideIndex(int quadNodeIndex) const { return from + di * quadNodeIndex; }
...
bool FaceQuadStruct::Side::IsForced(int nodeIndex) const
{
    ...
    if (forced_nodes.count(nodeIndex)) return true;
...
{
    if (ip >= GetUVPtStruct().size() || iop >= side->GetUVPtStruct().size())
        throw SALOME_Exception("FaceQuadStruct::Side::AddContact(): wrong point");
    if (ip < from || ip >= to) return;
    {
...
{
    const vector<UVPtStruct> &points = GetUVPtStruct();
    double u = (points[from].normParam + x * (points[to - di].normParam - points[from].normParam));
...
{
    if (IsReversed() != (theTo < theFrom)) std::swap(theTo, theFrom);

    const vector<UVPtStruct> &points = GetUVPtStruct();
    double r;
    if (theFrom == theTo && theTo == -1) r = Abs(First().normParam - Last().normParam);
    else if (IsReversed())
        r = Abs(points[Max(to, theTo + 1)].normParam - points[Min(from, theFrom)].normParam);
    else
        r = Abs(points[Min(to, theTo - 1)].normParam - points[Max(from, theFrom)].normParam);

...
StdMeshers_Regular_1D::StdMeshers_Regular_1D(int hypId, int studyId, SMESH_Gen *gen)
...
bool StdMeshers_Regular_1D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                            Hypothesis_Status &aStatus)
{
    ...
    for (; h != hyps.end(); ++h) {
        if (static_cast<const SMESH_Hypothesis *>(*h)->IsAuxiliary()) {
            if (strcmp("QuadraticMesh", (*h)->GetName()) == 0) _quadraticMesh = true;
            if (propagFilter.IsOk(static_cast<const SMESH_Hypothesis *>(*h), aShape))
                propagTypes.insert((*h)->GetName());
        }
        else {
            if (!theHyp) theHyp = *h; // use only the first non-auxiliary hypothesis
        }
    }
    ...
    if (hypName == "LocalLength") {
        const StdMeshers_LocalLength *hyp = dynamic_cast<const StdMeshers_LocalLength *>(theHyp);
        ASSERT(hyp);
        ...
        const StdMeshers_MaxLength *hyp = dynamic_cast<const StdMeshers_MaxLength *>(theHyp);
        ASSERT(hyp);
        _value[BEG_LENGTH_IND] = hyp->GetLength();
        ...
        switch (_ivalue[DISTR_TYPE_IND]) {
            ...
            case StdMeshers_NumberOfSegments::DT_ExprFunc:
                ...
                _revEdgesIDs = hyp->GetReversedEdges();
                break;
            case StdMeshers_NumberOfSegments::DT_Regular: break;
        ...
        const StdMeshers_Arithmetic1D *hyp = dynamic_cast<const StdMeshers_Arithmetic1D *>(theHyp);
        ASSERT(hyp);
        ...
        const StdMeshers_Geometric1D *hyp = dynamic_cast<const StdMeshers_Geometric1D *>(theHyp);
        ASSERT(hyp);
        ...
        const StdMeshers_Deflection1D *hyp = dynamic_cast<const StdMeshers_Deflection1D *>(theHyp);
        ASSERT(hyp);
    ...
    if (propagTypes.size() > 1 && aStatus == HYP_OK) {
        ...
        if (aMesh.GetHypotheses(aShape, propagFilter, _usedHypList, true, &assignedTo) > 1) {
            ...
            for (; hyp != _usedHypList.end(); ++hyp, ++shape)
                if (shape->ShapeType() == simpleShape && !localHyp->IsSameName(**hyp)) {
...
static bool computeParamByFunc(Adaptor3d_Curve &C3d, double first, double last, double length,
                               bool theReverse, int nbSeg, Function &func, list<double> &theParams)
{
    // never do this way
    //OSD::SetSignal( true );

    if (nbSeg <= 0) return false;
    ...
    if (!buildDistribution(func, 0.0, 1.0, nbSeg, x, 1E-4)) return false;
    ...
    for (int i = 1; i < nbSeg; i++) {
        ...
        if (U > first && U < last) theParams.push_back(U);
        else
            return false;
        prevU = U;
    }
    if (theReverse) theParams.reverse();
    return true;
}
...
static void compensateError(double a1, double an, double U1, double Un, double length,
                            Adaptor3d_Curve &C3d, list<double> &theParams,
...
StdMeshers_Regular_1D::getVertexHyp(SMESH_Mesh &theMesh, const TopoDS_Vertex &theV)
{
...
void StdMeshers_Regular_1D::redistributeNearVertices(SMESH_Mesh &theMesh, Adaptor3d_Curve &theC3d,
                                                     double theLength,
                                                     ...
                                                     const TopoDS_Vertex &theVl)
{
    ...
    for (int isEnd1 = 0; isEnd1 < 2; ++isEnd1) {
        ...
        if (hyp) {
            ...
            if (vertexLength > theLength / 2.0) continue;
            ...
            if (_hypType == NB_SEGMENTS) {
                ...
                if (!isEnd1) vertexLength = -vertexLength;
                GCPnts_AbscissaPoint Discret(theC3d, vertexLength, l);
                if (Discret.IsDone()) {
                    if (nPar == 0) theParameters.push_back(Discret.Parameter());
                    else {
                        double L = GCPnts_AbscissaPoint::Length(theC3d, theParameters.back(), l);
                        if (vertexLength < L / 2.0) theParameters.push_back(Discret.Parameter());
                        else
                            compensateError(0, vertexLength, f, l, theLength, theC3d, theParameters,
                                            true);
                    }
                }
            }
                ...
                list<double> params;
                if (auxAlgo->computeInternalParameters(theMesh, theC3d, L, from, to, params,
                                                       false)) {
                    if (isEnd1) params.reverse();
                    while (1 + nHalf--) theParameters.pop_back();
            ...
            if (isEnd1) theParameters.reverse();
        }
    }
}
 ...
 */
//=============================================================================
bool StdMeshers_Regular_1D::computeInternalParameters(SMESH_Mesh &theMesh, Adaptor3d_Curve &theC3d,
                                                      double theLength, double theFirstU,
                                                      double theLastU, list<double> &theParams,
                                                      ...
                                                      bool theConsiderPropagation)
{
    ...
    if (!_mainEdge.IsNull() && _isPropagOfDistribution) {
        ...
        if (!smDS) return error("No mesh on the source edge of Propagation Of Distribution");
        if (smDS->NbNodes() < 1) return true; // 1 segment
        ...
        map<double, const SMDS_MeshNode *>::iterator u_n2 = mainEdgeParamsOfNodes.begin(),
                                                     u_n1 = u_n2++;
        for (size_t i = 1; i < mainEdgeParamsOfNodes.size(); ++i, ++u_n1, ++u_n2) {
            segLen[i - 1] = GCPnts_AbscissaPoint::Length(mainEdgeCurve, u_n1->first, u_n2->first);
            totalLen += segLen[i - 1];
        }
        for (size_t i = 0; i < segLen.size(); ++i) segLen[i] *= theLength / totalLen;
        ...
        compensateError(segLen[theReverse ? segLen.size() - 1 : 0],
                        segLen[theReverse ? 0 : segLen.size() - 1], f, l, theLength, theC3d,
                        theParams, true);
        return true;
    }
    ...
    switch (_hypType) {
        ...
        case NB_SEGMENTS: {
            ...
            if (_hypType == MAX_LENGTH) {
                ...
                // NPAL17873:
                bool isFound = false;
                if (theConsiderPropagation
                    && !_mainEdge.IsNull()) // propagated from some other edge
                {
                ...
                switch (_ivalue[DISTR_TYPE_IND]) {
                    case StdMeshers_NumberOfSegments::DT_Scale: {
                        ...
                        for (; u != uEnd; ++u) {
                            ...
                            if (Discret.IsDone()) *u = Discret.Parameter();
                        }
                        return true;
                    } break;
                    case StdMeshers_NumberOfSegments::DT_TabFunc: {
                        ...
                        return computeParamByFunc(theC3d, f, l, theLength, theReverse,
                                                  _ivalue[NB_SEGMENTS_IND], func, theParams);
                    } break;
                    case StdMeshers_NumberOfSegments::DT_ExprFunc: {
            ...
            if (!Discret.IsDone()) return error("GCPnts_UniformAbscissa failed");
            ...
            compensateError(eltSize, eltSize, f, l, theLength, theC3d, theParams,
                            true); // for PAL9899
            return true;
        }
        ...
        case BEG_END_LENGTH: {
            ...
            if (q < theLength / 1e6 || 1.01 * theLength < a1 + an)
                return error(SMESH_Comment("Invalid segment lengths (")
                             << a1 << " and " << an << ") "
            ...
            while (1) {
                ...
                if (!Discret.IsDone()) break;
                param = Discret.Parameter();
                if (f < param && param < l) theParams.push_back(param);
                else
                    break;
                eltSize *= q;
            }
        ...
        case ARITHMETIC_1D: {
            ...
            int n = int(fabs(q) > numeric_limits<double>::min() ? (1 + (an - a1) / q)
                                                                : (1 + theLength / a1));
            ...
            while (n-- > 0 && eltSize * (Un - U1) > 0) {
                ...
                if (!Discret.IsDone()) break;
                param = Discret.Parameter();
                if (param > f && param < l) theParams.push_back(param);
                else
                    break;
                eltSize += q;
            }
        ...
        case FIXED_POINTS_1D: {
            ...
            for (i = 0; i < Params.Length(); i++) {
                ...
                if (!APnt.IsDone()) return error("GCPnts_AbscissaPoint failed");
                par2 = APnt.Parameter();
                eltSize = segmentSize / nbseg;
                GCPnts_UniformAbscissa Discret(theC3d, eltSize, par1, par2);
                if (theReverse) Discret.Initialize(theC3d, eltSize, par2, par1);
                else
                ...
                for (; itP != tmpParams.end(); itP++) { theParams.push_back(*(itP)); }
            ...
            if (theReverse) Discret.Initialize(theC3d, eltSize, par1, lp);
            else
...
bool StdMeshers_Regular_1D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (!idFirst || !idLast) return error(COMPERR_BAD_INPUT_MESH, "No node on vertex");
    ...
    if (SMESHDS_SubMesh *subMeshDS = meshDS->MeshElements(theShape)) {
        ...
        while (ite->more()) meshDS->RemoveFreeElement(ite->next(), subMeshDS);
        SMDS_NodeIteratorPtr itn = subMeshDS->GetNodes();
        while (itn->more()) {
            const SMDS_MeshNode *node = itn->next();
            if (node->NbInverseElements() == 0) meshDS->RemoveFreeNode(node, subMeshDS);
            else
    ...
    if (!Curve.IsNull()) {
        ...
        if (!_mainEdge.IsNull()) {
            ...
            if (!_isPropagOfDistribution) {
                ...
                if (std::find(_revEdgesIDs.begin(), _revEdgesIDs.end(), mainID)
                    != _revEdgesIDs.end())
                    reversed = !reversed;
            }
        }
...
bool StdMeshers_Regular_1D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
...
StdMeshers_Regular_1D::GetUsedHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                         const bool ignoreAuxiliary)
{
    ...
    if (nbHyp == 0 && aShape.ShapeType() == TopAbs_EDGE) {
        ...
        _mainEdge =
            StdMeshers_Propagation::GetPropagationSource(aMesh, aShape, _isPropagOfDistribution);
        if (!_mainEdge.IsNull()) {
...
void StdMeshers_Regular_1D::CancelCompute()
{
    ...
    if (_hypType == ADAPTIVE) _adaptiveHyp->GetAlgo()->CancelCompute();
}

...
Function::Function(const int conv) : myConv(conv) {}
...
FunctionIntegral::FunctionIntegral(const Function *f, const double st)
    : Function(-1), myFunc(const_cast<Function *>(f)), myStart(st)
{}
...
double FunctionIntegral::integral(const double, const double) const { return 0; }

FunctionTable::FunctionTable(const std::vector<double> &data, const int conv) : Function(conv)
{
    myData = data;
}
...
{
    int i1, i2;
    if (!findBounds(t, i1, i2)) return false;
    ...
    double x1 = myData[2 * i1], y1 = myData[2 * i1 + 1], x2 = myData[2 * i2],
           y2 = myData[2 * i2 + 1];
...
{
    if (i >= 0 && i < myData.size() - 1) return integral(i, myData[2 * (i + 1)] - myData[2 * i]);
    else
        return 0;
}
...
double FunctionTable::integral(const double a, const double b) const
{
    ...
    double J = 0;
    for (int i = x1s; i < x2s; i++) J += integral(i);
...
FunctionExpr::FunctionExpr(const char *str, const int conv)
    : Function(conv), myVars(1, 1), myValues(1, 1)
{
    bool ok = true;
    try {
#ifdef NO_CAS_CATCH
    ...
    if (!ok || !myExpr->IsDone()) myExpr.Nullify();
...
double dihotomySolve(Function &f, const double val, const double _start, const double _fin,
                     const double eps, bool &ok)
{
    double start = _start, fin = _fin, start_val, fin_val;
    bool ok1, ok2;
...
bool buildDistribution(const TCollection_AsciiString &f, const int conv, const double start,
                       const double end, const int nbSeg, vector<double> &data, const double eps)
{
    FunctionExpr F(f.ToCString(), conv);
    return buildDistribution(F, start, end, nbSeg, data, eps);
}

bool buildDistribution(const std::vector<double> &f, const int conv, const double start,
                       const double end, const int nbSeg, vector<double> &data, const double eps)
{

...
struct EventProparatorToEdges: public SMESH_subMeshEventListener {
    EventProparatorToEdges()
        : SMESH_subMeshEventListener(/*isDeletable=*/false,
                                     "Projection_1D2D::EventProparatorToEdges")
    {}
    static EventProparatorToEdges *Instance()
    {
        static EventProparatorToEdges E;
        return &E;
    }
...
struct UnsetterOfEventProparatorToEdges {
    ...
    ~UnsetterOfEventProparatorToEdges() { EventProparatorToEdges::Set(_faceSubMesh); }
};
} // namespace
...
bool StdMeshers_Projection_1D2D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (!StdMeshers_Projection_2D::Compute(theMesh, theShape)) return false;
    ...
    if (_quadraticMesh) {
        ...
        while (fIt->more()) // loop on mesh faces created by StdMeshers_Projection_2D
        {
            ...
            for (size_t i = 2; i < nodes.size(); i += 2) {
                ...
                if (idType.second == TopAbs_EDGE && idType.first != nodes[i - 1]->getshapeId()) {
                    ...
                    posFixer.CheckNodeU(TopoDS::Edge(posFixer.GetSubShape()), nodes[i - 1],
                                        dummyU = 0., tol, /*force=*/true);
                }
            }
        }
    }
    ...
    for (size_t iWire = 0; iWire < wires.size(); ++iWire) {
        ...
        if (nodes.empty()) return error("Wrong nodes on a wire");
        ...
        for (size_t i = 0; i < nodes.size(); ++i) {
            ...
            if (!faceFound) return error("The existing 1D mesh mismatches the generated 2D mesh");
        }
        ...
        if (_quadraticMesh) {
            ...
            for (size_t i = 1; i < nodes.size(); ++i) {
                if (checkExisting && meshDS->FindEdge(nodes[i - 1], nodes[i])) continue;
                SMDS_MeshElement *e = meshDS->AddEdge(nodes[i - 1], nodes[i]);
                if (nodes[i - 1]->getshapeId() != edgeID && nodes[i]->getshapeId() != edgeID) {
...
bool StdMeshers_Projection_1D2D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                          MapShapeNbElems &aResMap)
{
    if (!StdMeshers_Projection_2D::Evaluate(theMesh, theShape, aResMap)) return false;
    ...
    if (!srcFaceSM->IsMeshComputed()) _gen->Evaluate(*srcMesh, srcFace, srcResMap);
    ...
    while (smIt->more()) {
        ...
        if (shapeType == TopAbs_EDGE) {
            ...
            if (srcSubMeshDS && srcSubMeshDS->NbElements()) {
                ...
                aVec[_quadraticMesh ? SMDSEntity_Quad_Edge : SMDSEntity_Edge] =
                    srcSubMeshDS->NbElements();
            }
            else {
                if (srcResMap.empty())

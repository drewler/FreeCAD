...
bool StdMeshers_Import_1D2D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
struct TLink: public SMESH_OrientedLink {
    ...
    TLink(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const SMDS_MeshNode *medium = 0)
...
bool StdMeshers_Import_1D2D::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    if (allGroupsEmpty) return error("No faces in source groups");
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (!subShapeIDs.insert(tgtMesh->ShapeToIndex(v)).second) continue;
    ...
    for (size_t iG = 0; iG < srcGroups.size(); ++iG) {
        ...
        while (srcElems->more()) // loop on group contents
        {
            ...
            if (bndBox3d.IsOut(SMESH_TNodeXYZ(*node))) continue;
            ...
            bool isOut = false,
                 isIn = false; // if at least one node isIn - do not classify other nodes
            for (size_t i = 0; i < newNodes.size(); ++i, ++node) {
                ...
                if (newNode) {
                    ...
                    if (newNode->GetID() < (int)isNodeIn.size() && isNodeIn[newNode->GetID()])
                        isIn = true;
                    if (!isIn && bndNodes.count(*node)) nodeState[i] = TopAbs_ON;
                }
                else {
                ...
                if (!newNode) {
                    ...
                    if (!isOut) // create a new node
                    {
                        ...
                        if (nodeState[i] == TopAbs_ON) bndNodes.insert(*node);
                        else
                            isNodeIn[newNode->GetID()] = isIn = true;
                    }
                }
                if (!(newNodes[i] = newNode) || isOut) break;
            }

            if (!newNodes.back()) continue; // not all nodes of the face lie on theShape
            ...
            if (!isIn) // if all nodes are on FACE boundary, a mesh face can be OUT
            {
                ...
                {
                    gp_XYZ gc(0., 0., 0);
                    for (size_t i = 0; i < newNodes.size(); ++i) gc += SMESH_TNodeXYZ(newNodes[i]);
                    gc /= newNodes.size();

                    TopLoc_Location loc;
                    GeomAPI_ProjectPointOnSurf &proj =
                        helper.GetProjector(geomFace, loc, helper.MaxTolerance(geomFace));
            ...
            if (toCheckOri) {
                ...
                if (geomNorm * meshNorm < 0)
                    SMDS_MeshCell::applyInterlace(
                        SMDS_MeshCell::reverseSmdsOrder(face->GetEntityType(), newNodes.size()),
                        newNodes);
            }

            // make a new face
            if (face->IsPoly()) newFace = tgtMesh->AddPolygonalFace(newNodes);
            else
                switch (newNodes.size()) {
                    ...
                    case 4:
                        newFace =
                            tgtMesh->AddFace(newNodes[0], newNodes[1], newNodes[2], newNodes[3]);
                        break;
                    case 6:
                        newFace = tgtMesh->AddFace(newNodes[0], newNodes[1], newNodes[2],
        ...
        for (n2nIt = n2n->begin(); n2nIt != n2n->end();)
            if (!n2nIt->second) n2n->erase(n2nIt++);
            else
                ++n2nIt;
    }
    ...
    if (tgtFaceSM) {
            ...
            if (nbFaces == 1) {
                ...
                for (int is1stN = 0; is1stN < 2 && nodesOnBoundary; ++is1stN) {
                    ...
                    if (nodesOnBoundary) {
                        ...
                        if (s.ShapeType() == TopAbs_VERTEX) bndShapes.push_front(s); // vertex first
                        else
                ...
                if (bndShapes.back().ShapeType() != TopAbs_EDGE) // all link nodes are on VERTEXes
                {
                    ...
                    TopoDS_Shape geomEdge = helper.GetCommonAncestor(
                        bndShapes.back(), bndShapes.front(), theMesh, TopAbs_EDGE);
                    if (geomEdge.IsNull()) {
        ...
        if (isFaceMeshed) {
            ...
            for (; subID != subShapeIDs.end(); ++subID) {
                ...
                if (nbEdges < 2 && !helper.IsRealSeam(s)) return false; // weird
                if (nbEdges > 2)
    ...
    if (helper.HasSeam()) {
        ...
        for (size_t iE = 0; iE < edges.size(); ++iE) {
            ...
            set<const SMDS_MeshNode *> checkedNodes;
            checkedNodes.insert(vertNode);
            ...
            // create edges
            {
                ...
                for (n2 = u2nodeMap.begin(), n1 = n2++; n2 != u2nEnd; ++n1, ++n2) {
                    ...
                    if (node2->getshapeId() == helper.GetSubShapeID()) {
                        ...
                        tgtMesh->SetNodeOnEdge(const_cast<SMDS_MeshNode *>(node2), seamEdge,
                                               n2->first);
                    }
                }
            }
...
void StdMeshers_Import_1D2D::SubmeshRestored(SMESH_subMesh *subMesh) { SetEventListener(subMesh); }
...
bool StdMeshers_Import_1D2D::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                      MapShapeNbElems &aResMap)
{
    ...
    if (toCopyMesh) // the whole mesh is copied
    {
        ...
        for (unsigned i = 0; i < srcMeshes.size(); ++i) {
            SMESH_subMesh *sm =
                StdMeshers_Import_1D::getSubMeshOfCopiedMesh(theMesh, *srcMeshes[i]);
        ...
        TopExp_Explorer exp(theShape, TopAbs_VERTEX);
        for (; exp.More(); exp.Next()) theMesh.GetSubMesh(exp.Current())->Evaluate(aResMap);
        ...
        for (int iG = 0; iG < srcGroups.size(); ++iG) {
            ...
            while (srcElems->more()) // loop on group contents
            {
                ...
                gp_XYZ gc(0, 0, 0);
                gc = accumulate(TXyzIterator(face->nodesIterator()), TXyzIterator(), gc)
                    / face->NbNodes();
        ...
        for (exp.Init(theShape, TopAbs_EDGE); exp.More(); exp.Next()) {
            ...
            if (edgeVec.empty()) {
                ...
                for (link2Nb = linkCount.begin(); link2Nb != linkCount.end();) {
                    ...
                    if (nbFacesOfLink == 1) {
                        if (helper.CheckNodeU(geomEdge, link.node1(), u, minGroupTol,
                                              /*force=*/true)
                            && helper.CheckNodeU(geomEdge, link.node2(), u, minGroupTol,
                                                 /*force=*/true)) {
                    ...
                    if (eraseLink) linkCount.erase(link2Nb++);
                    else
                        link2Nb++;
                }
            ...
            else if (!helper.IsSeamShape(geomEdge) || geomEdge.Orientation() == TopAbs_FORWARD) {

...
struct InPoint {
    ...
    bool operator==(const TVDVertex *v) const
    {
        return (Abs(_a - v->x()) < 1. && Abs(_b - v->y()) < 1.);
    }
};
// -------------------------------------------------------------------------------------
...
struct InSegment {
    ...
    InSegment(InPoint *p0, InPoint *p1, size_t iE) : _p0(p0), _p1(p1), _geomEdgeInd(iE) {}
...
inline bool InSegment::isConnected(const TVDEdge *edge)
{
    return (
        ...
        && ((Abs(edge->vertex0()->x() - _p1->_a) < 1. && Abs(edge->vertex0()->y() - _p1->_b) < 1.)
...
{
    if (inSegments.size() > 1000) return;
    ...
    for (size_t i = 0; i < inSegments.size(); ++i) {
        text << inSegments[i]._p0->_a << " " << inSegments[i]._p0->_b << " "
             << inSegments[i]._p1->_a << " " << inSegments[i]._p1->_b << "\n";
    }
    text << "\n";
...
{
    cout << "*Edge_" << edge;
    if (!edge->vertex0()) cout << " ( INF, INF";
    else
        cout << " ( " << edge->vertex0()->x() << ", " << edge->vertex0()->y();
    if (!edge->vertex1()) cout << ") -> ( INF, INF";
    else
        cout << ") -> ( " << edge->vertex1()->x() << ", " << edge->vertex1()->y();
    cout << ")\t cell=" << edge->cell() << " iBnd=" << edge->color() << " twin=" << edge->twin()
         << " twin_cell=" << edge->twin()->cell() << " prev=" << edge->prev()
         << " next=" << edge->next() << (edge->is_primary() ? " MA " : " SCND")
         << (edge->is_linear() ? " LIN " : " CURV") << endl;
}
void dumpCell(const TVDCell *cell)
{
    cout << "**Cell_" << cell << " GEOM=" << cell->color() << " ";
    cout << (cell->contains_segment() ? " SEG " : " PNT ");
    if (cell->is_degenerate()) cout << " degen ";
    else {
        cout << endl;
...
template<> struct geometry_concept<InPoint> {
    typedef point_concept type;
};
template<> struct point_traits<InPoint> {
...
template<> struct geometry_concept<InSegment> {
    typedef segment_concept type;
};

template<> struct segment_traits<InSegment> {
    typedef int coordinate_type;
...
struct BndSeg {
    ...
    void setIndexToEdge(size_t id) { SMESH_MAT2d::Branch::setBndSegment(id, _edge); }

    int branchID() const { return Abs(_branchID); }

    size_t geomEdge() const { return _inSeg->_geomEdgeInd; }

    static BndSeg *getBndSegOfEdge(const TVDEdge *edge, vector<vector<BndSeg>> &bndSegsPerEdge)
    {
        BndSeg *seg = 0;
        if (edge) {
    ...
    void setBranch(int branchID, vector<vector<BndSeg>> &bndSegsPerEdge)
    {
        ...
        if (_edge)
            if (BndSeg *oppSeg = getBndSegOfEdge(_edge->twin(), bndSegsPerEdge)) {
                if (oppSeg->_branchID == theNoBrachID) oppSeg->_branchID = -branchID;
            }
    }
    ...
    {
        if (!_edge || !seg2._edge) return true;

        if (_edge->twin() == seg2._edge) return true;
        ...
        if (edgeMedium1->is_secondary() && edgeMedium2->is_secondary()) {
            if (edgeMedium1->twin() == edgeMedium2) return true;
...
struct BranchIterator {
    ...
    BranchIterator &operator=(const BranchIterator &other)
    {
        _i = other._i;
        return *this;
    }
    ...
    const TVDEdge *edgePrev()
    {
        --_i;
        const TVDEdge *e = edge();
        ++_i;
        return e;
    }
    const TVDEdge *edgeNext()
    {
        ++_i;
        const TVDEdge *e = edge();
        --_i;
...
void bndSegsToMesh(const vector<vector<BndSeg>> &bndSegsPerEdge)
{
    ...
    for (size_t iE = 0; iE < bndSegsPerEdge.size(); ++iE) {
        ...
        for (size_t i = 0; i < bndSegs.size(); ++i) {
            if (!bndSegs[i]._edge) text << "# E=" << iE << " i=" << i << " NULL edge\n";
            else if (!bndSegs[i]._edge->vertex0() || !bndSegs[i]._edge->vertex1())
                text << "# E=" << iE << " i=" << i << " INFINITE edge\n";
            else if (addedEdges.insert(bndSegs[i]._edge).second
                     && addedEdges.insert(bndSegs[i]._edge->twin()).second) {
                v2n =
                    v2Node.insert(make_pair(bndSegs[i]._edge->vertex0(), v2Node.size() + 1)).first;
                int n0 = v2n->second;
                if (n0 == v2Node.size())
                    text << "n" << n0 << " = m.AddNode( "
                ...
                v2n =
                    v2Node.insert(make_pair(bndSegs[i]._edge->vertex1(), v2Node.size() + 1)).first;
                int n1 = v2n->second;
                if (n1 == v2Node.size())
                    text << "n" << n1 << " = m.AddNode( "
...
void computeProportionScale(const TopoDS_Face &face, const Bnd_B2d &uvBox, double scale[2])
{
    scale[0] = scale[1] = 1.;
...
bool makeInputData(const TopoDS_Face &face, const std::vector<TopoDS_Edge> &edges,
                   const double minSegLen, vector<InPoint> &inPoints, vector<InSegment> &inSegments,
                   double scale[2])
{
    ...
    for (size_t iE = 0; iE < edges.size(); ++iE) {
        ...
        // }
        gp_Pnt p, pPrev;
        if (!c3d.IsNull()) pPrev = c3d->Value(f);
        if (discret.NbPoints() > 2)
        ...
        if (edges[iE].Orientation() == TopAbs_REVERSED) std::reverse(points.begin(), points.end());
    }
    ...
    for (size_t iE = 0; iE < edges.size(); ++iE) {
        ...
        if (!TopExp::CommonVertex(edges[iE], edges[iE2], vShared)) continue;
        if (!vShared.IsSame(TopExp::LastVertex(edges[iE], true))) return false;
    ...
    double vMax[2] = {Max(Abs(uvMin.X()), Abs(uvMax.X())), Max(Abs(uvMin.Y()), Abs(uvMax.Y()))};
    int iMax = (vMax[0] > vMax[1]) ? 0 : 1;
    const double precision = 1e-5;
    double preciScale = Min(vMax[iMax] / precision, std::numeric_limits<int>::max() / vMax[iMax]);
    preciScale /= scale[iMax];
    double roundedScale = 10; // to ease debug
    while (roundedScale * 10 < preciScale) roundedScale *= 10.;
    ...
    inSegments.clear();
    size_t nbPnt = 0;
    for (size_t iE = 0; iE < uvuVec.size(); ++iE) nbPnt += uvuVec[iE].size();
...
void updateJoinedBranch(vector<const TVDEdge *> &branchEdges, const size_t newID,
                        vector<vector<BndSeg>> &bndSegs, const bool reverse)
{
    BndSeg *seg1, *seg2;
    if (reverse) {
...
void makeMA(const TVD &vd, const bool ignoreCorners, vector<InPoint> &inPoints,
            vector<InSegment> &inSegments, vector<SMESH_MAT2d::Branch> &branch,
            vector<const SMESH_MAT2d::BranchEnd *> &branchPnt, SMESH_MAT2d::Boundary &boundary)
{
    ...
    for (size_t i = 0; i < inSegments.size(); ++i) {
        ...
        for (int is2nd = 0; is2nd < 2; ++is2nd) // loop on two ends of inSeg
        {
            ...
            if (inPntChecked[pInd]) continue;
            if (pInd > 0 && inPntChecked[pInd - 1] && inPoints[pInd - 1] == inPnt) continue;
            inPntChecked[pInd] = true;

            const TVDEdge *maE = is2nd ? maEdges.front() : maEdges.back();
            if (inPnt == (is2nd ? maE->vertex0() : maE->vertex1())) continue;
            ...
            while (inSeg.isConnected(edge)) {
                ...
                const TVDEdge *edge2 =
                    edge->twin(); // we are in a neighbor cell, add MA edges to inPnt
                if (inSeg.getGeomEdge(edge2->cell()) != theNoEdgeID) break; // cell of an InSegment
                bool hasInfinite = false;
                list<const TVDEdge *> pointEdges;
                edge = edge2;
                do {
                    edge = edge->next(); // Returns the CCW next edge within the cell.
                    if (edge->is_infinite()) hasInfinite = true;
                    else if (edge->is_primary() && !inSeg.isExternal(edge))
                        pointEdges.push_back(edge);
                } while (edge != edge2 && !hasInfinite);

                if (hasInfinite || pointEdges.empty()) break;
    ...
    vector<vector<BndSeg>> bndSegsPerEdge(boundary.nbEdges()); // all BndSeg's
    {
        ...
        for (size_t i = 0; i < inSegments.size(); ++i) {
            ...
            if (inSeg._geomEdgeInd != prevGeomEdge) {
                if (!bndSegs.empty()) bndSegsPerEdge[prevGeomEdge].swap(bndSegs);
            ...
            switch (nbMaEdges) {
                case 0: // "around" circle center
                    bndSegs.push_back(BndSeg(&inSeg, 0, inSeg._p1->_param));
                    break;
                case 1:
                    bndSegs.push_back(BndSeg(&inSeg, inSeg._edges.back(), inSeg._p1->_param));
                    break;
                default:
                    gp_XY inSegDir(inSeg._p1->_a - inSeg._p0->_a, inSeg._p1->_b - inSeg._p0->_b);
    ...
    for (size_t iE = 0; iE < bndSegsPerEdge.size(); ++iE) {
        ...
        for (size_t i = 0; i < bndSegs.size(); ++i) {
            if (bndSegs[i].branchID()) {
                if (bndSegs[i]._prev && bndSegs[i]._branchID == -bndSegs[i]._prev->_branchID
                    && bndSegs[i]._edge) {
            ...
            if (!bndSegs[i]._prev && !bndSegs[i].hasOppositeEdge()) continue;

            if (!bndSegs[i]._prev || !bndSegs[i]._prev->isSameBranch(bndSegs[i])) {
                branchEdges.resize((branchID = branchEdges.size()) + 1);
                if (bndSegs[i]._edge && bndSegs[i]._prev)
                    endType.insert(
                        make_pair(bndSegs[i]._edge->vertex1(), SMESH_MAT2d::BE_BRANCH_POINT));
            }
            ...
            else if (bndSegs[i]._edge) // 1st bndSeg of a WIRE
            {
                ...
                if (bndSegs[i]._inSeg->isConnected(bndSegs[i]._edge)) {
                    if (bndSegs[i]._inSeg->point0() == bndSegs[i]._edge->vertex1())
                        endType.insert(
                            make_pair(bndSegs[i]._edge->vertex1(), SMESH_MAT2d::BE_ON_VERTEX));
                    else
                        endType.insert(
                            make_pair(bndSegs[i]._edge->vertex0(), SMESH_MAT2d::BE_ON_VERTEX));
                }
            }

            bndSegs[i].setBranch(branchID,
                                 bndSegsPerEdge); // set to i-th and to the opposite bndSeg
    ...
    if (ignoreCorners && branchEdges.size() > 2 && !branchEdges[2].empty()) {
        ...
        for (size_t iB = 1; iB < branchEdges.size(); ++iB) {
            if (branchEdges[iB].empty()) continue;
        ...
        for (size_t iB = 1; iB < branchEdges.size(); ++iB) {
            if (branchEdges[iB].empty() || isBranchRemoved[iB]) continue;
            const TVDVertex *v0 = branchEdges[iB][0]->vertex1();
            const TVDVertex *v1 = branchEdges[iB].back()->vertex0();
            v2et = endType.find(v0);
            if (v2et == endType.end() || v2et->second != SMESH_MAT2d::BE_BRANCH_POINT) v0 = 0;
            v2et = endType.find(v1);
            if (v2et == endType.end() || v2et->second != SMESH_MAT2d::BE_BRANCH_POINT) v1 = 0;
            if (!v0 && !v1) continue;
            ...
            for (int isV0 = 0; isV0 < 2; ++isV0) {
                ...
                for (size_t iB2 = 1; iB2 < branchEdges.size(); ++iB2) {
                    if (branchEdges[iB2].empty() || isBranchRemoved[iB2] || iB == iB2) continue;
                ...
                if (iBrToJoin > 0) {
                    ...
                    updateJoinedBranch(branch, iB, bndSegsPerEdge,
                                       /*reverse=*/(v0 == v02 || v1 == v12));
                    if (v0 == v02 || v0 == v12)
                        branchEdges[iB].insert(branchEdges[iB].begin(), branch.begin(),
                                               branch.end());
                    else
    ...
    int nbBranches = 0;
    for (size_t i = 0; i < branchEdges.size(); ++i) { nbBranches += (!branchEdges[i].empty()); }
    branch.resize(nbBranches);
    size_t iBr = 0;
    for (size_t brID = 1; brID < branchEdges.size(); ++brID) // 1st - not removed
    {
        if (!branchEdges[brID].empty() && !isBranchRemoved[brID]) branchByID[brID] = &branch[iBr++];
    }
    ...
    for (size_t iE = 0; iE < bndSegsPerEdge.size(); ++iE) {
        ...
        for (size_t i = 0; i < bndSegs.size(); ++i)
            if (bndSegs[i]._edge) SMESH_MAT2d::Branch::setBndSegment(i, bndSegs[i]._edge);
        ...
        for (size_t i = 0; i < bndSegs.size(); ++i) bndPoints._params.push_back(bndSegs[i]._uLast);
        ...
        for (size_t i = 0; i < bndSegs.size(); ++i) {
            ...
            if (bndSegs[i]._edge && !branchEdges[brID].empty()) {
                ...
                if ((edgeInd < 0 || edgeInd >= (int)branchEdges[brID].size())
                    ...
                    if (bndSegs[i]._branchID < 0) {
                        ...
                        for (edgeInd = branchEdges[brID].size() - 1; edgeInd > 0; --edgeInd)
                            if (branchEdges[brID][edgeInd]->twin() == bndSegs[i]._edge) break;
                    }
                    else // bndSegs[ i ]._branchID > 0
                    {
                        dInd = +1;
                        for (edgeInd = 0; edgeInd < branchEdges[brID].size(); ++edgeInd)
                            if (branchEdges[brID][edgeInd] == bndSegs[i]._edge) break;
                    }
                }
            }
            else {
                // no MA edge, bndSeg corresponds to an end point of a branch
                if (bndPoints._maEdges.empty()) edgeInd = 0;
                else
    ...
    for (size_t i = 0; i < branch.size(); ++i)
        if (!branch[i].isRemoved()) branch[i].setBranchesToEnds(branch);
    ...
    for (size_t i = 0; v2e != v2end.end(); ++v2e, ++i) branchPnt[i] = v2e->second;
...
SMESH_MAT2d::MedialAxis::MedialAxis(const TopoDS_Face &face, const std::vector<TopoDS_Edge> &edges,
                                    const double minSegLen, const bool ignoreCorners)
    : _face(face), _boundary(edges.size())
{
    // input to construct_voronoi()
    vector<InPoint> inPoints;
    vector<InSegment> inSegments;
    if (!makeInputData(face, edges, minSegLen, inPoints, inSegments, _scale)) return;
    ...
    for (size_t i = 0; i < _branch.size(); ++i)
        if (_branch[i].isRemoved()) --_nbBranches;
}
...
void SMESH_MAT2d::MedialAxis::getPoints(const Branch *branch, std::vector<gp_XY> &points) const
{
...
bool SMESH_MAT2d::Boundary::getBranchPoint(const std::size_t iEdge, double u, BranchPoint &p) const
{
    if (iEdge >= _pointsPerEdge.size() || _pointsPerEdge[iEdge]._params.empty()) return false;
    ...
    if (points._params[i] == points._params[i + 1]) // coincident points at some end
    {
        ...
        while (points._params[i] == points._params[i + 1]) i += di;
        if (i < 0 || i + 1 >= points._params.size()) i = 0;
    }
    ...
    if (!points._maEdges[i].second) // no branch at the EDGE end, look for a closest branch
    {
        ...
        {
            while (i < points._maEdges.size() - 1 && !points._maEdges[i].second) ++i;
            edgeParam = edgeReverse;
        }
        else // near last point
        {
            while (i > 0 && !points._maEdges[i].second) --i;
            edgeParam = !edgeReverse;
        }
    }
...
bool SMESH_MAT2d::Boundary::getBranchPoint(const BoundaryPoint &bp, BranchPoint &p) const
{
...
bool SMESH_MAT2d::Boundary::isConcaveSegment(std::size_t iEdge, std::size_t iSeg) const
{
    ...
    const BndPoints &points = _pointsPerEdge[iEdge];
    if (points._params.size() <= iSeg + 1) return false;
...
{
    if (bp._edgeIndex >= _pointsPerEdge.size()) return false;
...
void SMESH_MAT2d::Branch::init(vector<const TVDEdge *> &maEdges, const Boundary *boundary,
                               map<const TVDVertex *, BranchEndType> endType)
{
    ...
    for (size_t i = 1; i < _params.size(); ++i) _params[i] /= _params.back();


    _endPoint1._vertex = _maEdges.front()->vertex1();
    _endPoint2._vertex = _maEdges.back()->vertex0();

    if (endType.count(_endPoint1._vertex)) _endPoint1._type = endType[_endPoint1._vertex];
    if (endType.count(_endPoint2._vertex)) _endPoint2._type = endType[_endPoint2._vertex];
}
...
SMESH_MAT2d::BranchPoint SMESH_MAT2d::Branch::getPoint(const TVDVertex *vertex) const
{
    ...
    if (vertex == _maEdges[0]->vertex1()) { p._edgeParam = 0; }
    else {
...
void SMESH_MAT2d::Branch::setRemoved(const BranchPoint &proxyPoint) { _proxyPoint = proxyPoint; }
...
bool SMESH_MAT2d::Branch::getBoundaryPoints(double param, BoundaryPoint &bp1,
                                            BoundaryPoint &bp2) const
{
    if (param < _params[0] || param > _params.back()) return false;
...
bool SMESH_MAT2d::Branch::getBoundaryPoints(std::size_t iMAEdge, double maEdgeParam,
                                            BoundaryPoint &bp1, BoundaryPoint &bp2) const
{
    if (isRemoved()) return _proxyPoint._branch->getBoundaryPoints(_proxyPoint, bp1, bp2);

    if (iMAEdge > _maEdges.size()) return false;
    if (iMAEdge == _maEdges.size()) iMAEdge = _maEdges.size() - 1;
...
bool SMESH_MAT2d::Branch::getBoundaryPoints(const BranchPoint &p, BoundaryPoint &bp1,
                                            BoundaryPoint &bp2) const
{
...
{
    if (this != p._branch && p._branch) return p._branch->getParameter(p, u);

    if (isRemoved()) return _proxyPoint._branch->getParameter(_proxyPoint, u);

    if (p._iEdge > _params.size() - 1) return false;
    if (p._iEdge == _params.size() - 1) return (u = 1.);

    u = (_params[p._iEdge] * (1 - p._edgeParam) + _params[p._iEdge + 1] * p._edgeParam);
...
void SMESH_MAT2d::Branch::getPoints(std::vector<gp_XY> &points, const double scale[2]) const
{
                                                  ...
                                                  int &i) const
{
    ...
    if (!isConcaNext && !isConcaPrev) return false;
    ...
    if (isConcaNext) // all null-length segments follow
    {
        ...
        while ((maE = ++iNext)) {
            ...
            if (!_boundary->isConcaveSegment(ie1, iSeg2)) break;
        }
        ...
        while ((maE = iPrev.edgePrev())) {
            ...
            if (_boundary->isConcaveSegment(edgeIDs1.back(), iSeg1)) --iPrev;
            else
                break;
        }
    }
    ...
    if (iPrev.index() < i - 1 || iNext.index() > i) {
        ...
        for (; _params[iPrev.indexMod()] < midPar; ++iPrev) divisionPnt._iEdge = iPrev.indexMod();
        divisionPnt._edgeParam = (_params[iPrev.indexMod()] - midPar)
                                               ...
                                               std::vector<BranchPoint> &divPoints) const
{
    ...
    for (size_t i = 0; i < _maEdges.size(); ++i) twins[i] = _maEdges[i]->twin();
    ...
    for (++maIter, ++twIter; maIter.index() < _maEdges.size(); ++maIter, ++twIter) {
        ...
        if (otherE1 || otherE2) {
            bool isConcaveV = false;
            if (otherE1 && !otherE2) {
                isConcaveV = addDivPntForConcaVertex(edgeIDs1, edgeIDs2, divPoints, _maEdges, twins,
                                                     maIter._i);
            }
            if (!otherE1 && otherE2) {
                isConcaveV = addDivPntForConcaVertex(edgeIDs2, edgeIDs1, divPoints, twins, _maEdges,
                                                     maIter._i);
            }
...
bool SMESH_MAT2d::Boundary::getPoint(std::size_t iEdge, std::size_t iSeg, double u,
                                     BoundaryPoint &bp) const
{
    if (iEdge >= _pointsPerEdge.size()) return false;
    if (iSeg + 1 >= _pointsPerEdge[iEdge]._params.size()) return false;

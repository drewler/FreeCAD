...
StdMeshers_Hexa_3D::~StdMeshers_Hexa_3D() { MESSAGE("StdMeshers_Hexa_3D::~StdMeshers_Hexa_3D"); }
...
bool StdMeshers_Hexa_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
*/
    ...
    aStatus = HYP_OK;
    for (; h != hyps.end(); ++h) {
        if (!(_viscousLayersHyp = dynamic_cast<const StdMeshers_ViscousLayers *>(*h))) break;
    }
    if (!_viscousLayersHyp) aStatus = HYP_INCOMPATIBLE;
    else
...
struct _FaceGrid {
    ...
    const SMDS_MeshNode *GetNode(int iCol, int iRow) const { return _columns[iCol][iRow]; }
    gp_XYZ GetXYZ(int iCol, int iRow) const { return SMESH_TNodeXYZ(GetNode(iCol, iRow)); }
};
...
/*!
   ...
   */
FaceQuadStructPtr getQuadWithBottom(StdMeshers_FaceSidePtr side, FaceQuadStructPtr quad[6])
{
        ...
        for (unsigned iS = 0; iS < quad[i]->side.size(); ++iS) {
            ...
            if ((side->FirstVertex().IsSame(side2->FirstVertex())
                ...
                if (iS != Q_BOTTOM) {
                    ...
                    for (unsigned j = 0; j < iS; ++j) newSides.push_back(quad[i]->side[j]);
...
bool StdMeshers_Hexa_3D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (!exp.More()) return error(COMPERR_BAD_SHAPE, "No SHELL in the geometry");
    ...
    for (int i = 1; i < 6; ++i)
        if (!aCubeSide[i]._quad) return error(COMPERR_BAD_SHAPE);
    ...
    for (int i = 0; i < 6; ++i) aCubeSide[i]._columns.clear();
...
bool StdMeshers_Hexa_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                  MapShapeNbElems &aResMap)
{
    ...
    for (; i < 6; i++) {
        ...
        if (algoName == "Quadrangle_2D") {
            ...
            if (nbtri == 0) isAllQuad = true;
        }
        if (!isAllQuad) { return EvaluatePentahedralMesh(aMesh, aShape, aResMap); }
    }
...
SMESH_ComputeErrorPtr ComputePentahedralMesh(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
bool EvaluatePentahedralMesh(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                             MapShapeNbElems &aResMap)
{
    StdMeshers_Penta_3D anAlgo;

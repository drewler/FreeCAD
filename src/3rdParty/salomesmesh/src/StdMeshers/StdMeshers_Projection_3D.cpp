...
bool StdMeshers_Projection_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
bool StdMeshers_Projection_3D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    int nbShell;
    for (nbShell = 0; exp.More(); exp.Next(), ++nbShell) srcShell = TopoDS::Shell(exp.Current());
    if (nbShell != 1)
        return error(COMPERR_BAD_SHAPE,
                     SMESH_Comment("Source shape must have 1 shell but not ") << nbShell);

    exp.Init(aShape, TopAbs_SHELL);
    for (nbShell = 0; exp.More(); exp.Next(), ++nbShell) tgtShell = TopoDS::Shell(exp.Current());
    if (nbShell != 1)
        return error(COMPERR_BAD_SHAPE,
    ...
    while (volIt->more()) // loop on source volumes
    {
        ...
        if (!srcVol || srcVol->GetType() != SMDSAbs_Volume) continue;
        int nbNodes = srcVol->NbNodes();
        SMDS_VolumeTool::VolumeType volType = volTool.GetType(nbNodes);
        if (srcVol->IsQuadratic()) nbNodes = volTool.NbCornerNodes(volType);
        ...
        switch (volType) {
            ...
            case SMDS_VolumeTool::QUAD_TETRA:
                tgtVol = helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], id, force3d);
                break;
            case SMDS_VolumeTool::PYRAM:
            case SMDS_VolumeTool::QUAD_PYRAM:
                tgtVol =
                    helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], id, force3d);
                break;
            case SMDS_VolumeTool::PENTA:
            case SMDS_VolumeTool::QUAD_PENTA:
                tgtVol = helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                          nodes[5], id, force3d);
                break;
            case SMDS_VolumeTool::HEXA:
            case SMDS_VolumeTool::QUAD_HEXA:
                tgtVol = helper.AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                          nodes[5], nodes[6], nodes[7], id, force3d);
                break;
            default: // polyhedron
                const SMDS_VtkVolume *poly = dynamic_cast<const SMDS_VtkVolume *>(srcVol);
                if (!poly) RETURN_BAD_RESULT("Unexpected volume type");
                if (!poly->IsPoly()) RETURN_BAD_RESULT("Unexpected volume type");
                tgtVol = tgtMeshDS->AddPolyhedralVolume(nodes, poly->GetQuantities());
        }
        if (tgtVol) { tgtMeshDS->SetMeshElementOnShape(tgtVol, helper.GetSubShapeID()); }
...
bool StdMeshers_Projection_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                        MapShapeNbElems &aResMap)
{
    ...
    while (elemIt->more()) {
        ...
        if (E->NbNodes() == 4) { aVec[SMDSEntity_Tetra]++; }
        else if (E->NbNodes() == 5) {
...
{
    TAssocTool::SetEventListener(subMesh, _sourceHypo->GetSource3DShape(),

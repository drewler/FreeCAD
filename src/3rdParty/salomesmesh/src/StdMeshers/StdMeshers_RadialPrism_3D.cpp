...
bool StdMeshers_RadialPrism_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
...
bool StdMeshers_RadialPrism_3D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    for (TopoDS_Iterator It(solid); It.More(); It.Next(), ++nbShells)
        if (!outerShell.IsSame(It.Value())) innerShell = It.Value();
    if (nbShells != 2)
    ...
    bool mapOk1 = ProjectionUtils::FindSubShapeAssociation(innerShell, &aMesh, outerShell, &aMesh,
                                                           shape2ShapeMaps[0]);
    bool mapOk2 = ProjectionUtils::FindSubShapeAssociation(innerShell.Reversed(), &aMesh,
                                                           outerShell, &aMesh, shape2ShapeMaps[1]);
    if (!mapOk1 && !mapOk2)
    ...
    for (exp.Init(outerShell, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        while (faceIt->more()) // loop on faces on inFace
        {
            ...
            for (int i = 0; i < nbNodes; ++i) {
                ...
                if (n_col != node2columnMap.end()) { columns[i] = &n_col->second; }
                else {
                    TNodeNodeMap::iterator nInOut = nodeIn2OutMap.find(nIn);
                    if (nInOut == nodeIn2OutMap.end())
                        RETURN_BAD_RESULT("No matching node for " << nIn->GetID() << " in face "
                                                                  << face->GetID());
                                                       ...
                                                       const SMDS_MeshNode *inNode)
{
    ...
    TNode2ColumnMap::iterator n_col = n2ColMap.insert(make_pair(outNode, TNodeColumn())).first;
...
bool StdMeshers_RadialPrism_3D::computeLayerPositions(const gp_Pnt &pIn, const gp_Pnt &pOut)
{
    if (myNbLayerHypo) {
    ...
    if (myDistributionHypo) {
        ...
        if (!TNodeDistributor::GetDistributor(*mesh)->Compute(myLayerPositions, pIn, pOut, *mesh,
                                                              myDistributionHypo)) {
...
bool StdMeshers_RadialPrism_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                         MapShapeNbElems &aResMap)
{
    ...
    if (!ProjectionUtils::FindSubShapeAssociation(outerShell, &aMesh, innerShell, &aMesh,
                                                  shape2ShapeMap)) {
        std::vector<int> aResVec(SMDSEntity_Last);
        for (int i = SMDSEntity_Node; i < SMDSEntity_Last; i++) aResVec[i] = 0;
        SMESH_subMesh *sm = aMesh.GetSubMesh(aShape);
    ...
    for (TopExp_Explorer exp(outerShell, TopAbs_EDGE); exp.More(); exp.Next()) {
        if (tmpMap.Contains(exp.Current())) continue;
    ...
    bool IsOK = (nb0d_Out == nb0d_In) && (nb1d_Out == nb1d_In) && (nb2d_3_Out == nb2d_3_In)
        && (nb2d_4_Out == nb2d_4_In);
    if (!IsOK) {
        std::vector<int> aResVec(SMDSEntity_Last);
        for (int i = SMDSEntity_Node; i < SMDSEntity_Last; i++) aResVec[i] = 0;
        SMESH_subMesh *sm = aMesh.GetSubMesh(aShape);
    ...
    int nbLayers = 0;
    if (myNbLayerHypo) { nbLayers = myNbLayerHypo->GetNumberOfLayers(); }
    if (myDistributionHypo) {

...
TopoDS_Edge nextC1Edge(TopoDS_Edge edge, SMESH_Mesh &aMesh, const bool forward)
{
...
struct VertexNodesRestoringListener: public SMESH_subMeshEventListener {
                      ...
                      EventListenerData *data, const SMESH_Hypothesis * /*hyp*/)
    {
        if (data && eventType == SMESH_subMesh::ALGO_EVENT) {
            bool hypRemoved;
            if (subMesh->GetAlgoState() != SMESH_subMesh::HYP_OK) hypRemoved = true;
            else {
                SMESH_Algo *algo = subMesh->GetAlgo();
                hypRemoved =
                    (string(algo->GetName()) != StdMeshers_CompositeSegment_1D::AlgoName());
            }
            if (hypRemoved) {
                ...
                if (meshDS->NbNodes() > 0) {
                    ...
                    if (!hasNodesOnVerext) {
                        ...
                        unique_ptr<StdMeshers_FaceSide> side(
                            StdMeshers_CompositeSegment_1D::GetFaceSide(*subMesh->GetFather(), edge,
                                                                        face, false));
                        if (side->NbEdges() > 1 && side->NbSegments()) careOfSubMeshes(*side);
                    }
                }
            }
        }
        // clean all EDGEs of a complex side if one EDGE is cleaned
        else if (event == SMESH_subMesh::CLEAN && eventType == SMESH_subMesh::COMPUTE_EVENT) {
...
void careOfSubMeshes(StdMeshers_FaceSide &side)
{
    ...
    for (int iE = 0; iE < side.NbEdges(); ++iE) {
        ...
        if (iE) {
            ...
            sm->ComputeStateEngine(SMESH_subMesh::CHECK_COMPUTE_STATE);
            if (sm->GetComputeState() != SMESH_subMesh::COMPUTE_OK) sm->SetIsAlwaysComputed(true);
            listenerData->mySubMeshes.push_back(sm);
        }
    }
}
...
StdMeshers_CompositeSegment_1D::StdMeshers_CompositeSegment_1D(int hypId, int studyId,
                                                               SMESH_Gen *gen)
...
std::string StdMeshers_CompositeSegment_1D::AlgoName() { return "CompositeSegment_1D"; }
...
void StdMeshers_CompositeSegment_1D::SetEventListener(SMESH_subMesh *subMesh)
{
    ...
    SMESH_subMeshIteratorPtr smIt = subMesh->getDependsOnIterator(false, false);
    while (!isAlwaysComputed && smIt->more()) isAlwaysComputed = smIt->next()->IsAlwaysComputed();
                                                                 ...
                                                                 const bool ignoreMeshed)
{
    ...
    if (anEdge.Orientation() <= TopAbs_REVERSED) edges.push_back(anEdge);
    else
        ...
        while (!eNext.IsNull()) {
            if (ignoreMeshed) {
                ...
                if (SMESHDS_SubMesh *sm = aMesh.GetMeshDS()->MeshElements(eNext))
                    if (sm->NbNodes() || sm->NbElements()) break;
            }
            // eNext must have same hypotheses
            SMESH_Algo *algo = aMesh.GetGen()->GetAlgo(aMesh, eNext);
            if (!algo || string(theAlgo->GetName()) != algo->GetName()
                || hypList != algo->GetUsedHypothesis(aMesh, eNext, false))
                break;
            if (std::find(edges.begin(), edges.end(), eNext) != edges.end()) break;
            if (forward) edges.push_back(eNext);
            else
...
bool StdMeshers_CompositeSegment_1D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (side->NbEdges() < 2) return StdMeshers_Regular_1D::Compute(aMesh, aShape);
    ...
    if (!hyps.empty()) {
        ...
        if (autoLenHyp) _value[BEG_LENGTH_IND] = autoLenHyp->GetLength(&aMesh, side->Length());
    }
    ...
    list<double> params;
    if (!computeInternalParameters(aMesh, *C3d, side->Length(), f, l, params, false)) return false;
    ...
    if (!nFirst)
        return error(COMPERR_BAD_INPUT_MESH,
                     TComm("No node on vertex ") << meshDS->ShapeToIndex(VFirst));
    if (!nLast)
        return error(COMPERR_BAD_INPUT_MESH,
                     TComm("No node on vertex ") << meshDS->ShapeToIndex(VLast));
    ...
    for (int iN = 0; parIt != params.end(); ++iN, ++parIt) {
        ...
        if (iN) {
            ...
            if (_quadraticMesh) {
                ...
                if (ruler.IsDone()) mPar = ruler.Parameter();
    ...
    for (int iE = 1; iE < side->NbEdges(); ++iE) {
        TopoDS_Vertex V = side->FirstVertex(iE);
        while (const SMDS_MeshNode *n = SMESH_Algo::VertexNode(V, meshDS)) meshDS->RemoveNode(n);
    }

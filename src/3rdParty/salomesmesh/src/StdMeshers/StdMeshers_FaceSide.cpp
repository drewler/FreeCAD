...
StdMeshers_FaceSide::StdMeshers_FaceSide(const TopoDS_Face &theFace, const TopoDS_Edge &theEdge,
                                         SMESH_Mesh *theMesh, const bool theIsForward,
...
StdMeshers_FaceSide::StdMeshers_FaceSide(const TopoDS_Face &theFace, list<TopoDS_Edge> &theEdges,
                                         SMESH_Mesh *theMesh, const bool theIsForward,
                                         const bool theIgnoreMediumNodes,
                                         SMESH_ProxyMesh::Ptr theProxyMesh)
{
    ...
    for (int index = 0; edge != theEdges.end(); ++index, ++edge) {
        ...
        if (theFace.IsNull()) BRep_Tool::Range(*edge, myFirst[i], myLast[i]);
        else
            myC2d[i] = BRep_Tool::CurveOnSurface(*edge, theFace, myFirst[i], myLast[i]);
        if (myEdge[i].Orientation() == TopAbs_REVERSED) std::swap(myFirst[i], myLast[i]);

        // check if the edge has a non-uniform parametrization (issue 0020705)
        if (!myC2d[i].IsNull()) {
            if (myEdgeLength[i] > DBL_MIN) {
                Geom2dAdaptor_Curve A2dC(myC2d[i], std::min(myFirst[i], myLast[i]),
                ...
                myIsUniform[i] = !(fabs(2 * d2 / myEdgeLength[i] - 1.0) > 0.01
                                   || fabs(2 * d4 / d2 - 1.0) > 0.01);
        ...
        if (!theIsForward) reverseProxySubmesh(myEdge[i]);
    ...
    if (nbEdges > 1 && myLength > DBL_MIN) {
        ...
        if (nbDegen) totLength += myLength * degenNormLen * nbDegen;
        double prevNormPar = 0;
        for (int i = 0; i < nbEdges; ++i) {
            if (myEdgeLength[i] < DBL_MIN) myEdgeLength[i] = myLength * degenNormLen;
...
StdMeshers_FaceSide::StdMeshers_FaceSide(const StdMeshers_FaceSide *theSide,
                                         const SMDS_MeshNode *theNode, const gp_Pnt2d *thePnt2d1,
                                         const gp_Pnt2d *thePnt2d2,
                                         const Handle(Geom2d_Curve) & theC2d,
                                         const double theUFirst, const double theULast)
{
...
StdMeshers_FaceSide::StdMeshers_FaceSide(UVPtStructVec &theSideNodes, const TopoDS_Face &theFace)
{
    myEdge.resize(1);
    myEdgeID.resize(1, -1);
    myC2d.resize(1);
    ...
    if (!myPoints.empty()) {
        ...
        if (myPoints[0].node && myPoints.back().node && myPoints[myNbPonits / 2].node) {
        ...
        if (myLength > std::numeric_limits<double>::min())
            for (size_t i = 1; i < myPoints.size(); ++i) myPoints[i].normParam /= myLength;
    }
...
const vector<UVPtStruct> &StdMeshers_FaceSide::GetUVPtStruct(bool isXConst, double constValue) const
{
    if (myPoints.empty()) {
        ...
        for (iE = 0; iE < myEdge.size(); ++iE) {
            ...
            if (proxySubMesh[iE]) {
                if (proxySubMesh[iE]->GetUVPtStructVec().empty()) { proxySubMesh[iE] = 0; }
                else {
                    nbProxyNodes += proxySubMesh[iE]->GetUVPtStructVec().size() - 1;
                    if (iE + 1 == myEdge.size()) ++nbProxyNodes;
                    continue;
                }
            }
            ...
            {
                if (vertexNodes.insert(node).second || fHelper.IsRealSeam(node->getshapeId())
                ...
                if (!node) return myPoints;
                vertexNodes.insert(node);
            }

            // Add internal nodes
            nodes.clear();
            if (!GetEdgeNodes(iE, nodes, /*v0=*/false, /*v1=*/false)) return myPoints;
            if (!nodes.empty()) {
                u2nodeVec.clear();
                ...
                if (!myIsUniform[iE])
                    for (size_t i = 0; i < nodes.size(); ++i) {
                        ...
                        double aLenU =
                            GCPnts_AbscissaPoint::Length(me->myC3dAdaptor[iE], myFirst[iE], u);
        ...
        if (!proxySubMesh.back()) {
            ...
            if (IsClosed() && !proxySubMesh[0]) node = u2node.begin()->second;
            else {
                node = VertexNode(iE);
                while (!node && iE > 0) node = VertexNode(--iE);
                if (!node) return myPoints;
            }
            if (u2node.rbegin()->second == node && !fHelper.IsRealSeam(node->getshapeId())
                && !fHelper.IsDegenShape(node->getshapeId()))
...
const vector<UVPtStruct> &StdMeshers_FaceSide::SimulateUVPtStruct(int nbSeg, bool isXConst,
    ...
    if (myPoints.empty() || (theEdgeInd >= 0 && NbEdges() > 0)) {
        ...
        for (iE = 0; iE < iEnd; ++iE) {
            ...
            if (node) { // nodes on internal vertices may be missing
                ...
                if (!node) return resultNodes;
                vertexNodes.insert(node);
            }

            // Add internal nodes
            nodes.clear();
            if (!GetEdgeNodes(iE, nodes, /*v0=*/false, /*v1=*/false)) return resultNodes;
            if (!nodes.empty()) {
                double paramSize = myLast[iE] - myFirst[iE];
                double r = myNormPar[iE] - prevNormPar;
                eHelper.SetSubShape(myEdge[iE]);
        ...
        {
            const SMDS_MeshNode *node;
            if (IsClosed() && theEdgeInd < 0) node = u2node.begin()->second;
            else {
                node = VertexNode(iE);
                while (!node && iE > 0) node = VertexNode(--iE);
                if (!node) return resultNodes;
        ...
        if (theEdgeInd < 0 && u2node.size() != myNbPonits
            && u2node.size() != NbPoints(/*update=*/true)) {
            u2node.clear();
        }
        resultNodes.reserve(u2node.size());
        map<double, const SMDS_MeshNode *>::iterator u2n = u2node.begin();
        for (; u2n != u2node.end(); ++u2n) resultNodes.push_back(u2n->second);
    }
    else {
        resultNodes.resize(myPoints.size());
        for (size_t i = 0; i < myPoints.size(); ++i) resultNodes[i] = myPoints[i].node;
    }
...
bool StdMeshers_FaceSide::GetEdgeNodes(size_t i, vector<const SMDS_MeshNode *> &nodes,
                                       bool inlude1stVertex, bool inludeLastVertex) const
{
    if (i >= myEdge.size()) return false;
    ...
    if (inlude1stVertex) {
        if (const SMDS_MeshNode *n0 = VertexNode(i)) nodes.push_back(n0);
    }
    ...
    if (sm && (sm->NbElements() > 0 || sm->NbNodes() > 0)) {
        if (mesh->HasModificationsToDiscard()) // check nb of nodes on the EDGE sub-mesh
        {
            ...
            if (nbExpect != sm->NbNodes()) // some nodes are moved from the EDGE by MergeNodes()
            {
                ...
                while (segIt->more()) {
                    ...
                    if (seg->GetType() != SMDSAbs_Edge) continue;
        ...
        if (nodes.size() < 2) // add nodes assigned to the EDGE
        {
            ...
            while (nItr->more()) {
                ...
                if (myIgnoreMediumNodes && SMESH_MeshEditor::IsMedium(n, SMDSAbs_Edge)) continue;
                nodes.push_back(n);
            }
        }
    ...
    if (inludeLastVertex) {
        if (const SMDS_MeshNode *n1 = VertexNode(i + 1)) nodes.push_back(n1);
    }
    return true;
}
...
template<typename T> void reverse(vector<T> &vec) { std::reverse(vec.begin(), vec.end()); }
...
void StdMeshers_FaceSide::Reverse()
{
    ...
    for (int i = nbEdges - 1; i >= 0; --i) {
        ...
        if (!myEdge[i].IsNull()) myEdge[i].Reverse();
    ...
    if (nbEdges > 0) {
        myNormPar[nbEdges - 1] = 1.;
        if (!myEdge[0].IsNull()) {
            for (size_t i = 0; i < myEdge.size(); ++i) reverseProxySubmesh(myEdge[i]);
            myPoints.clear();
            myFalsePoints.clear();
        }
        else {
    ...
    for (size_t i = 0; i < myEdge.size(); ++i) {
        ...
        if (!C3d.IsNull()) myC3dAdaptor[i].Load(C3d, fp, lp);
    }
}
...
void StdMeshers_FaceSide::SetIgnoreMediumNodes(bool toIgnore)
{
    if (myIgnoreMediumNodes != toIgnore) {
        ...
        if (!myPoints.empty()) {
            ...
            for (size_t i = 0; i < myPoints.size(); i += 2) newPoints.push_back(myPoints[i]);
...
{
    if (!myPoints.empty()) return myPoints.size();
    ...
    if (update && myEdge.size() > 0) {
        ...
        for (int i = 0; i < nbV; ++i)
            if (const SMDS_MeshNode *n = VertexNode(i)) {
                if (!vNodes.insert(n).second
                    && (helper.IsRealSeam(n->getshapeId()) || helper.IsDegenShape(n->getshapeId())))
                    me->myNbPonits++;
            }
            else {
        ...
        if (IsClosed()) me->myNbPonits++; // closing node is repeated
    }
    return myNbPonits;
}
...
void StdMeshers_FaceSide::dump(const char *msg) const
{
    ...
    MESSAGE_ADD("nbPoints: " << myNbPonits << " vecSize: " << myPoints.size() << " "
                             << myFalsePoints.size());
    for (int i = 0; i < myEdge.size(); ++i) {
        MESSAGE_ADD("\t" << i + 1);
        MESSAGE_ADD("\tEDGE: ");
        if (myEdge[i].IsNull()) { MESSAGE_ADD("NULL"); }
        else {
            TopAbs::Print(myEdge[i].Orientation(), cout)
                << " " << myEdge[i].TShape().operator->() << endl;
            MESSAGE_ADD("\tV1: " << TopExp::FirstVertex(myEdge[i], 1).TShape().operator->()
                                 << "  V2: "
                                 << TopExp::LastVertex(myEdge[i], 1).TShape().operator->());
        }
        MESSAGE_ADD("\tC2d: ");

        if (myC2d[i].IsNull()) { MESSAGE_ADD("NULL"); }
        else {
...
Adaptor2d_Curve2d *StdMeshers_FaceSide::GetCurve2d() const { return new Adaptor2dCurve2d(this); }
...
{
    if (myEdge.empty()) return 0;

    TopoDS_Wire aWire;
    BRep_Builder aBuilder;
    aBuilder.MakeWire(aWire);
    for (int i = 0; i < myEdge.size(); ++i) aBuilder.Add(aWire, myEdge[i]);

    if (myEdge.size() == 2 && IsClosed()) aWire.Closed(true); // issue 0021141
...
gp_Pnt2d StdMeshers_FaceSide::Value2d(double U) const
{
    if (!myC2d[0].IsNull()) {
        ...
        if (!myIsUniform[i]) {
            ...
            GCPnts_AbscissaPoint AbPnt(const_cast<GeomAdaptor_Curve &>(myC3dAdaptor[i]), aLen3dU,
                                       myFirst[i]);
            if (AbPnt.IsDone()) { par = AbPnt.Parameter(); }
        }
        return myC2d[i]->Value(par);
    }
    else if (!myPoints.empty()) {
        int i = U * double(myPoints.size() - 1);
        while (i > 0 && myPoints[i].normParam > U) --i;
        while (i + 1 < myPoints.size() && myPoints[i + 1].normParam < U) ++i;
        double r =
            ((U - myPoints[i].normParam) / (myPoints[i + 1].normParam - myPoints[i].normParam));
        return (myPoints[i].UV() * (1 - r) + myPoints[i + 1].UV() * r);
    }
...
TSideVector StdMeshers_FaceSide::GetFaceWires(const TopoDS_Face &theFace, SMESH_Mesh &theMesh,
                                              const bool theIgnoreMediumNodes, TError &theError,
                                              SMESH_ProxyMesh::Ptr theProxyMesh,
                                              const bool theCheckVertexNodes)
{
    ...
    for (int iW = 0; iW < nbWires; ++iW, ++nbE) {
        ...
        if (wireEdges.front().Orientation() != TopAbs_INTERNAL) // Issue 0020676
        {
            if (theCheckVertexNodes)
                while (!SMESH_Algo::VertexNode(TopExp::FirstVertex(wireEdges.front(), true),
                                               theMesh.GetMeshDS())) {
                    wireEdges.splice(wireEdges.end(), wireEdges, wireEdges.begin(),
                                     ++wireEdges.begin());
        ...
        {
            internalEdges.splice(internalEdges.end(), wireEdges, ++wireEdges.begin(),
                                 wireEdges.end());
        }

        StdMeshers_FaceSide *wire =
            new StdMeshers_FaceSide(theFace, wireEdges, &theMesh,
                                    /*isForward=*/true, theIgnoreMediumNodes, theProxyMesh);
        wires[iW] = StdMeshers_FaceSidePtr(wire);
        from = to;
    }
    while (!internalEdges.empty()) {
        StdMeshers_FaceSide *wire =
            new StdMeshers_FaceSide(theFace, internalEdges.back(), &theMesh,
                                    /*isForward=*/true, theIgnoreMediumNodes, theProxyMesh);
...
TopoDS_Vertex StdMeshers_FaceSide::FirstVertex(int i) const
{
    TopoDS_Vertex v;
    if (i < NbEdges()) {
        v = myEdge[i].Orientation() <= TopAbs_REVERSED
            ? // FORWARD || REVERSED
...
TopoDS_Vertex StdMeshers_FaceSide::LastVertex(int i) const
{
    ...
    if (i < NbEdges()) {
        ...
        if (edge.Orientation() <= TopAbs_REVERSED) // FORWARD || REVERSED
            ...
            for (TopoDS_Iterator vIt(edge); vIt.More(); vIt.Next()) v = TopoDS::Vertex(vIt.Value());
    }
    return v;
}

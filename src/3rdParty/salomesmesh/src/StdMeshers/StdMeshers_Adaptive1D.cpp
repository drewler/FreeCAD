...
class SegSizeTree: public SMESH_Octree
{
    ...
    SegSizeTree(double size) : SMESH_Octree(), mySegSize(size) { allocateChildren(); }
    void allocateChildren()
    {
        myChildren = new SMESH_Octree::TBaseTree *[nbChildren()];
        for (int i = 0; i < nbChildren(); ++i) myChildren[i] = NULL;
    }
...
public:
    ...
    virtual bool Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
...
struct Segment {
    ...
    void Init(const gp_Pnt &p1, const gp_Pnt &p2)
    {
        ...
        if (myLength > std::numeric_limits<double>::min()) myDir /= myLength;
    }
...
struct TriaTreeData: public ElemTreeData {
    ...
    double GetMinDistInSphere(const gp_Pnt &p, const double radius, const bool projectedOnly,
...
TriaTreeData::TriaTreeData(const TopoDS_Face &face, ElementBndBoxTree *triaTree)
    : myTriasDeflection(0), mySurface(face), myTree(NULL), myPolyTrias(NULL), myNodes(NULL),
      myOwnNodes(false)
{
    TopLoc_Location loc;
    ...
    if (!tr.IsNull()) {
        ...
        {
            TColgp_Array1OfPnt *trsfNodes =
                new TColgp_Array1OfPnt(myNodes->Lower(), myNodes->Upper());
...
void TriaTreeData::PrepareToTriaSearch()
{
    ...
    if (!tr.IsNull()) {
        ...
        for (l2s = linkToSegMap.begin(); l2s != linkToSegMap.end(); ++l2s, ++iS) {
            ...
            mySegments[iS].Init(myNodes->Value(link.N1()), myNodes->Value(link.N2()));
        }
    }
    ...
    for (int i = 1; i <= myPolyTrias->Upper(); ++i) {
        ...
        t.Init(myNodes->Value(n1), myNodes->Value(n2), myNodes->Value(n3));
        int nbSeg = 0;
        ...
        if ((l2s = linkToSegMap.find(NLink(n3, n1))) != linkToSegMap.end())
            t.mySegments[nbSeg++] = l2s->second;
        while (nbSeg < 3) t.mySegments[nbSeg++] = NULL;

        t.myIsChecked = false;
        t.myHasNodeOnVertex =
            (vertexNodes.count(n1) || vertexNodes.count(n2) || vertexNodes.count(n3));
    }
...
{
    if (mySurface.GetType() == GeomAbs_Plane || myTriasDeflection <= 1e-100) return;
    ...
    switch (mySurface.GetType()) {
        ...
        case GeomAbs_Sphere:
        case GeomAbs_Torus: isConstSize = true; break;
        default: isConstSize = false;
    }
    ...
    for (int i = 1; i <= myPolyTrias->Upper(); ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (isDone[j]) lenOfDoneLink.erase(link2len);
            if (a[j] > maxLinkLen) {
                maxLinkLen = a[j];
                jLongest = j;
            }
        ...
        if (!isDone[jLongest] || !isConstSize) {
            //++nbLinks;
            if (size < numeric_limits<double>::min()) continue;
...
double TriaTreeData::GetMinDistInSphere(const gp_Pnt &p, const double radius,
                                        const bool projectedOnly, const gp_Pnt *avoidPnt) const
{
    ...
    for (size_t i = 0; i < myFoundTriaIDs.size(); ++i) {
        ...
        if (avoidPnt && t.myHasNodeOnVertex) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                if ((avoidTria = (pn.SquareDistance(*avoidPnt) <= tol2))) break;
                if (!projectedOnly) minD2 = Min(minD2, pn.SquareDistance(p));
            }
            if (avoidTria) continue;
            ...
            if (t.DistToProjection(p, d) && d * d > dMin2) minDist2 = Min(minDist2, d * d);
        }
        else {
            for (int i = 0; i < 3; ++i) minD2 = Min(minD2, p.SquareDistance(myNodes->Value(n[i])));
...
void Triangle::Init(const gp_Pnt &p1, const gp_Pnt &p2, const gp_Pnt &p3)
{
    ...
    myMaxSize2 = Max(p2.SquareDistance(p3), Max(myEdge2.SquareModulus(), myEdge1.SquareModulus()));
}
//================================================================================
/*!
...
{
    if (myInvDet == 0) return false; // degenerated triangle
    ...
    if (u < 0.0 || u > 1.0) return false; // projected outside the triangle

    /* calculate V parameter and test bounds */
    gp_XYZ qvec = tvec ^ myEdge1;
    double v = (myNorm * qvec) * myInvDet;
    if (v < 0.0 || u + v > 1.0) return false; // projected outside the triangle
...
ElementBndBoxTree::ElementBndBoxTree(const TopoDS_Face &face) : SMESH_Octree()
{
...
void ElementBndBoxTree::FillIn()
{
    ...
    if (!data->myTrias.empty()) {
        for (size_t i = 0; i < data->myTrias.size(); ++i) _elementIDs.push_back(i);
...
void ElementBndBoxTree::buildChildrenData()
{
    ...
    for (int j = 0; j < 8; j++) {
        ...
        if (child->_elementIDs.size() <= theMaxNbElemsInLeaf) child->myIsLeaf = true;
                                            ...
                                            vector<int> &foundElemIDs) const
{
    if (const box_type *box = getBox()) {
        if (box->IsOut(center, radius)) return;
        ...
        if (isLeaf()) {
            ...
            for (int i = 0; i < 8; i++)
                ((ElementBndBoxTree *)myChildren[i])
                    ->GetElementsInSphere(center, radius, foundElemIDs);
        }
    }
}
    ...
    : SMESH_Octree(new _CommonData())
{
    ...
    double maxBoxHSize =
        0.5 * Max(pmax.X() - pmin.X(), Max(pmax.Y() - pmin.Y(), pmax.Z() - pmin.Z()));
    maxBoxHSize *= 1.01;
...
void SegSizeTree::SetSize(const gp_Pnt &p, double size)
{
    ...
    while (root->myFather) root = (SegSizeTree *)root->myFather;
    if (root->getBox()->IsOut(p.XYZ())) return;
    ...
    while (true) {
        ...
        if (leaf->myChildren[iChild]) leaf = (SegSizeTree *)leaf->myChildren[iChild];
        else
            break;
    }
    // don't increase the current size
    if (leaf->mySegSize <= 1.1 * size) return;
...
double deflection2(const BRepAdaptor_Curve &theCurve, double theU1, double theU2)
{
...
//purpose  : Constructor
StdMeshers_Adaptive1D::StdMeshers_Adaptive1D(int hypId, int studyId, SMESH_Gen *gen)
                                                ...
                                                const TopoDS_Shape &theShape)
{
    ...
    if (nbEdges) {
        ...
        if (maxDefl2 > 0) myDeflection = maxDefl2;
    }
    return nbEdges;
}
...
{
    if (!myAlgo) {
        AdaptiveAlgo *newAlgo = new AdaptiveAlgo(_gen->GetANewId(), _studyId, _gen);
...
AdaptiveAlgo::AdaptiveAlgo(int hypId, int studyId, SMESH_Gen *gen)
    : StdMeshers_Regular_1D(hypId, studyId, gen), myHyp(NULL)
{
...
void AdaptiveAlgo::SetHypothesis(const StdMeshers_Adaptive1D *hyp) { myHyp = hyp; }
...
bool AdaptiveAlgo::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    // *theProgress = 0.01;
    ...
    {
        BRepMesh_IncrementalMesh im(theMesh.GetShapeToMesh(), myHyp->GetDeflection(),
                                    /*isRelatif=*/0);
    }
    ...
    for (int iE = 0; iE < myEdges.size(); ++iE) {
        ...
        if (!computeInternalParameters(theMesh, eData.myC3d, eData.myLength, f, l, params, false,
                                       false))
            continue;
        if (params.size() <= 1 && helper.IsClosedEdge(eData.Edge())) // 2 segments on a circle
        {
            params.clear();
            for (int i = 1; i < 6; ++i) params.push_back((l - f) * i / 6.);
        }
        EdgeData::TPntIter where = --eData.myPoints.end();
        list<double>::const_iterator param = params.begin();
        for (; param != params.end(); ++param) eData.AddPoint(where, *param);
    ...
    for (int iF = 1; iF <= faceMap.Extent(); ++iF) {
        ...
        for (int iE = 0; iE < myEdges.size(); ++iE) {
            ...
            if (isAdjFace && triaSearcher->mySurface.GetType() == GeomAbs_Plane) continue;

            bool sizeDecreased = true;
            for (int iLoop = 0; sizeDecreased;
                 ++iLoop) //repeat until segment size along the edge becomes stable
            {
                double maxSegSize = 0;
                ...
                if (iLoop == 0 && eData.IsTooDistant(triaSearcher->myBBox, maxSegSize)) break;
                ...
                for (pIt1 = eData.myPoints.begin(); pIt1 != eData.myPoints.end();) {
                    double distToFace = triaSearcher->GetMinDistInSphere(pIt1->myP, pIt1->mySegSize,
                                                                         isAdjFace, avoidPnt);
                    ...
                    if (allowedSize < pIt1->mySegSize) {
                        ...
                        if (1.1 * allowedSize < pIt1->mySegSize) {
                            ...
                            if (--pIt2 != eData.myPoints.end() && pIt2->mySegSize > allowedSize)
                                sizeTree.SetSize(
                                    eData.myC3d.Value(0.6 * pIt2->myU + 0.4 * pIt1->myU),
                                    allowedSize);
                            pIt2 = pIt1;
                            if (++pIt2 != eData.myPoints.end() && pIt2->mySegSize > allowedSize)
                                sizeTree.SetSize(
                    ...
                    ++pIt1;
                    if (pIt1 == pItLast) avoidPnt = &eData.Last().myP;
                    else
                        avoidPnt = NULL;
...
bool AdaptiveAlgo::makeSegments()
{
    ...
    for (int iE = 0; iE < myEdges.size(); ++iE) {
        ...
        for (; pIt1 != eData.myPoints.end(); ++pIt1)
            edgeMinSize = Min(edgeMinSize, Min(pIt1->mySegSize, mySizeTree->GetSize(pIt1->myP)));
        ...
        //cout << "E " << theMesh.GetMeshDS()->ShapeToIndex( eData.Edge() ) << endl;
        while (
            toRecompute) // recompute if segment size at some point is less than edgeMinSize/nbDivSeg
        {
            ...
            for (++pIt1; pIt1 != eData.myPoints.end(); ++pIt1) {
                ...
                if (i < nbSegs.size()) nbSegs[i] = size2;
                pp1 = pp2;
            }
        ...
        for (i = 0, segCount = 1; segCount < nbSegFinal; ++segCount) {
            while (nbSegs[i] * fact < segCount) ++i;
            if (i < nbDiv) {
        ...
        if (!nf || !nl) return error("No node on vertex");
...
bool AdaptiveAlgo::Evaluate(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                            MapShapeNbElems &theResMap)
{

...
/*!
   ...
   */
struct _MeshOfSolid: public SMESH_ProxyMesh, public SMESH_subMeshEventListenerData {
    ...
    void setNode2Node(const SMDS_MeshNode *srcNode, const SMDS_MeshNode *proxyNode,
...
public:
    ...
    {
        static _ShrinkShapeListener l;
        return &l;
    }
    virtual void ProcessEvent(const int event, const int eventType, SMESH_subMesh *solidSM,
...
class _ViscousListener: SMESH_subMeshEventListener
{
    ...
    static SMESH_subMeshEventListener *Get()
    {
        static _ViscousListener l;
        return &l;
    }

public:
    virtual void ProcessEvent(const int event, const int eventType, SMESH_subMesh *subMesh,
                              SMESH_subMeshEventListenerData *data, const SMESH_Hypothesis *hyp)
    {
    ...
    static _MeshOfSolid *GetSolidMesh(SMESH_Mesh *mesh, const TopoDS_Shape &solid,
                                      bool toCreate = false)
    {
        if (!mesh) return 0;
...
struct _Simplex {
    ...
    bool IsForward(const SMDS_MeshNode *nSrc, const gp_XYZ *pntTgt, double &vol) const
    {
        ...
        vol = (+M[0][0] * M[1][1] * M[2][2] + M[0][1] * M[1][2] * M[2][0]
               + M[0][2] * M[1][0] * M[2][1] - M[0][0] * M[1][2] * M[2][1]
               - M[0][1] * M[1][0] * M[2][2] - M[0][2] * M[1][1] * M[2][0]);
        return vol > 1e-100;
    }
    bool IsForward(const gp_XY &tgtUV, const SMDS_MeshNode *smoothedNode, const TopoDS_Face &face,
                   SMESH_MesherHelper &helper, const double refSign) const
    {
    ...
    static void GetSimplices(const SMDS_MeshNode *node, vector<_Simplex> &simplices,
                             const set<TGeomID> &ingnoreShapes,
                             const _SolidData *dataToCheckOri = 0, const bool toSort = false);
...
struct _LayerEdge {
    ...
    bool SetNewLength2d(Handle(Geom_Surface) & surface, const TopoDS_Face &F, _EdgesOnShape &eos,
                        SMESH_MesherHelper &helper);
    void SetDataByNeighbors(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                            const _EdgesOnShape &eos, SMESH_MesherHelper &helper);
    ...
    bool SmoothOnEdge(Handle(Geom_Surface) & surface, const TopoDS_Face &F,
                      SMESH_MesherHelper &helper);
    bool FindIntersection(SMESH_ElementSearcher &searcher, double &distance, const double &epsilon,
                          _EdgesOnShape &eos, const SMDS_MeshElement **face = 0);
    bool SegTriaInter(const gp_Ax1 &lastSegment, const SMDS_MeshNode *n0, const SMDS_MeshNode *n1,
                      const SMDS_MeshNode *n2, double &dist, const double &epsilon) const;
...
_LayerEdge::PSmooFun _LayerEdge::_funs[theNbSmooFuns] = {
    &_LayerEdge::smoothLaplacian, &_LayerEdge::smoothLengthWeighted, &_LayerEdge::smoothCentroidal,
    &_LayerEdge::smoothNefPolygon, &_LayerEdge::smoothAngular};
const char *_LayerEdge::_funNames[theNbSmooFuns + 1] = {
    "Laplacian", "LengthWeighted", "Centroidal", "NefPolygon", "Angular", "None"};
...
struct _halfPlane {
    ...
    bool IsOut(const gp_XY p, const double tol) const { return _inNorm * (p - _pos) < -tol; }
    bool FindInterestion(const _halfPlane &hp, gp_XY &intPnt)
    {
        const double eps = 1e-10;
        double D = _dir.Crossed(hp._dir);
        if (fabs(D) < std::numeric_limits<double>::min()) return false;
...
struct AverageHyp {
    ...
    bool ToSmooth() const { return _method == StdMeshers_ViscousLayers::SURF_OFFSET_SMOOTH; }
    bool IsOffsetMethod() const { return _method == StdMeshers_ViscousLayers::FACE_OFFSET; }
...
struct _EdgesOnShape {
    ...
    TopAbs_ShapeEnum SWOLType() const { return _sWOL.IsNull() ? TopAbs_SHAPE : _sWOL.ShapeType(); }
...
struct _ConvexFace {
    ...
    bool GetCenterOfCurvature(_LayerEdge *ledge, BRepLProp_SLProps &surfProp,
                              SMESH_MesherHelper &helper, gp_Pnt &center) const;
...
struct _SolidData {
    ...
    Handle(Geom_Curve)
        CurveForSmooth(const TopoDS_Edge &E, _EdgesOnShape &eos, SMESH_MesherHelper &helper);
...
struct _CentralCurveOnEdge {
    ...
    void SetShapes(const TopoDS_Edge &edge, const _ConvexFace &convFace, _SolidData &data,
                   SMESH_MesherHelper &helper);
};
//--------------------------------------------------------------------------------
/*!
...
struct _SmoothNode {
    ...
    bool Smooth(int &badNb, Handle(Geom_Surface) & surface, SMESH_MesherHelper &helper,
                const double refSign, SmoothType how, bool set3D);
...
private:
    ...
    void getIgnoreFaces(const TopoDS_Shape &solid, const StdMeshers_ViscousLayers *hyp,
                        const TopoDS_Shape &hypShape, set<TGeomID> &ignoreFaces);
    ...
    gp_XYZ getFaceNormal(const SMDS_MeshNode *n, const TopoDS_Face &face,
                         SMESH_MesherHelper &helper, bool &isOK, bool shiftInside = false);
    bool getFaceNormalAtSingularity(const gp_XY &uv, const TopoDS_Face &face,
                                    SMESH_MesherHelper &helper, gp_Dir &normal);
    gp_XYZ getWeigthedNormal(const SMDS_MeshNode *n, std::pair<TopoDS_Face, gp_XYZ> fId2Normal[],
                             int nbFaces);
    bool findNeiborsOnEdge(const _LayerEdge *edge, const SMDS_MeshNode *&n1,
                           const SMDS_MeshNode *&n2, _EdgesOnShape &eos, _SolidData &data);
    ...
    void limitStepSize(_SolidData &data, const SMDS_MeshElement *face,
    ...
    bool smoothAnalyticEdge(_SolidData &data, _EdgesOnShape &eos, Handle(Geom_Surface) & surface,
                            const TopoDS_Face &F, SMESH_MesherHelper &helper);
    ...
    bool shrink();
    bool prepareEdgeToShrink(_LayerEdge &edge, _EdgesOnShape &eos, SMESH_MesherHelper &helper,
                             const SMESHDS_SubMesh *faceSubMesh);
    void restoreNoShrink(_LayerEdge &edge) const;
    void fixBadFaces(const TopoDS_Face &F, SMESH_MesherHelper &helper, const bool is2D,
                     const int step, set<const SMDS_MeshNode *> *involvedNodes = NULL);
...
struct _NodeCoordHelper {
        ...
        : _helper(helper), _face(F)
    {
        ...
        if (_surface.IsNull()) _fun = &_NodeCoordHelper::direct;
        else
...
private:
    gp_XYZ direct(const SMDS_MeshNode *n) const { return SMESH_TNodeXYZ(n); }
...
StdMeshers_ViscousLayers::StdMeshers_ViscousLayers(int hypId, int studyId, SMESH_Gen *gen)
    : SMESH_Hypothesis(hypId, studyId, gen), _isToIgnoreShapes(1), _nbLayers(1), _thickness(1),
      _stretchFactor(1), _method(SURF_OFFSET_SMOOTH)
{
...
{
    if (thickness != _thickness) _thickness = thickness, NotifySubMeshesHypothesisModification();
} // --------------------------------------------------------------------------------
void StdMeshers_ViscousLayers::SetNumberLayers(int nb)
{
    if (_nbLayers != nb) _nbLayers = nb, NotifySubMeshesHypothesisModification();
} // --------------------------------------------------------------------------------
void StdMeshers_ViscousLayers::SetStretchFactor(double factor)
{
    if (_stretchFactor != factor) _stretchFactor = factor, NotifySubMeshesHypothesisModification();
} // --------------------------------------------------------------------------------
void StdMeshers_ViscousLayers::SetMethod(ExtrusionMethod method)
{
    if (_method != method) _method = method, NotifySubMeshesHypothesisModification();
} // --------------------------------------------------------------------------------
                                                       ...
                                                       const bool toMakeN2NMap) const
{
    ...
    if (err && !err->IsOK()) return SMESH_ProxyMesh::Ptr();
    ...
    for (; exp.More(); exp.Next()) {
        if (_MeshOfSolid *pm =
            ...
            if (toMakeN2NMap && !pm->_n2nMapComputed)
                if (!bulder.MakeN2NMap(pm)) return SMESH_ProxyMesh::Ptr();
            ...
            if (pm->_warning && !pm->_warning->IsOK()) {
                ...
                if (!smError || smError->IsOK()) smError = pm->_warning;
            }
        }
...
{
    save << " " << _nbLayers << " " << _thickness << " " << _stretchFactor << " "
         << _shapeIds.size();
    for (size_t i = 0; i < _shapeIds.size(); ++i) save << " " << _shapeIds[i];
...
std::istream &StdMeshers_ViscousLayers::LoadFrom(std::istream &load)
{
    ...
    while (_shapeIds.size() < nbFaces && load >> faceID) _shapeIds.push_back(faceID);
    if (load >> shapeToTreat) {
        _isToIgnoreShapes = !shapeToTreat;
        if (load >> method) _method = (ExtrusionMethod)method;
    }
    else {
...
StdMeshers_ViscousLayers::CheckHypothesis(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape,
                                          SMESH_Hypothesis::Hypothesis_Status &theStatus)
{
    VISCOUS_3D::_ViscousBuilder bulder;
    SMESH_ComputeErrorPtr err = bulder.CheckHypotheses(theMesh, theShape);
    if (err && !err->IsOK()) theStatus = SMESH_Hypothesis::HYP_INCOMPAT_HYPS;
    else
...
{
    bool isIn = (std::find(_shapeIds.begin(), _shapeIds.end(), shapeIndex) != _shapeIds.end());
...
//--------------------------------------------------------------------------------
gp_XYZ getFaceDir(const TopoDS_Face &F, const TopoDS_Vertex &fromV, const SMDS_MeshNode *node,
                  SMESH_MesherHelper &helper, bool &ok, double *cosin = 0);
//--------------------------------------------------------------------------------
gp_XYZ getFaceDir(const TopoDS_Face &F, const TopoDS_Edge &fromE, const SMDS_MeshNode *node,
                  SMESH_MesherHelper &helper, bool &ok)
{
    double f, l;
    ...
    if (o == TopAbs_REVERSED) du.Reverse();

    gp_Vec dir = norm ^ du;

    if (node->GetPosition()->GetTypeOfPosition() == SMDS_TOP_VERTEX && helper.IsClosedEdge(fromE)) {
        if (fabs(u - f) < fabs(u - l)) c->D1(l, p, dv);
        else
            c->D1(f, p, dv);
        if (o == TopAbs_REVERSED) dv.Reverse();
        gp_Vec dir2 = norm ^ dv;
...
//--------------------------------------------------------------------------------
gp_XYZ getFaceDir(const TopoDS_Face &F, const TopoDS_Vertex &fromV, const SMDS_MeshNode *node,
                  SMESH_MesherHelper &helper, bool &ok, double *cosin)
{
    ...
    if (nbEdges == 2) {
        ...
        for (size_t i = 0; i < nbEdges && ok; ++i) {
            ...
            if ((ok = size2 > numeric_limits<double>::min())) edgeDir[i] /= sqrt(size2);
        }
        if (!ok) return dir;
...
bool getConcaveVertices(const TopoDS_Face &F, SMESH_MesherHelper &helper,
                        set<TGeomID> *vertices = 0)
{
    ...
    for (size_t iW = 0; iW < wires.size(); ++iW) {
        ...
        if (nbEdges < 2 && SMESH_Algo::isDegenerated(wires[iW]->Edge(0))) continue;
        for (int iE1 = 0; iE1 < nbEdges; ++iE1) {
            if (SMESH_Algo::isDegenerated(wires[iW]->Edge(iE1))) continue;
            int iE2 = (iE1 + 1) % nbEdges;
            while (SMESH_Algo::isDegenerated(wires[iW]->Edge(iE2))) iE2 = (iE2 + 1) % nbEdges;
            TopoDS_Vertex V = wires[iW]->FirstVertex(iE2);
            double angle = helper.GetAngle(wires[iW]->Edge(iE1), wires[iW]->Edge(iE2), F, V);
            if (angle < -5. * M_PI / 180.) {
...
bool isConcave(const TopoDS_Face &F, SMESH_MesherHelper &helper, set<TGeomID> *vertices = 0)
{
    ...
    for (; eExp.More(); eExp.Next()) {
        ...
        for (int i = 1; i <= nbIntervals && isConvex; ++i) {
            ...
            if (E.Orientation() == TopAbs_REVERSED) cross = -cross;
    ...
    // check angles at VERTEXes
    if (getConcaveVertices(F, helper, vertices)) isConcv = true;
...
bool getDistFromEdge(const SMDS_MeshElement *face, const SMDS_MeshNode *nodeOnEdge,
                     double &faceSize)
{
    ...
    const SMDS_MeshNode *nNext[2] = {face->GetNode(iNext[0]), face->GetNode(iNext[1])};
    gp_XYZ segVec, segEnd = SMESH_TNodeXYZ(nodeOnEdge); // segment on EDGE
    double segLen = -1.;
    // look for two neighbor not in-FACE nodes of face
    for (int i = 0; i < 2; ++i) {
        if (nNext[i]->GetPosition()->GetDim() != 2 && nNext[i]->GetID() < nodeOnEdge->GetID()) {
            // look for an in-FACE node
            for (int iN = 0; iN < nbN; ++iN) {
                if (iN == iOnE || iN == iNext[i]) continue;
...
struct PyDump {
    ...
    void Finish()
    {
        if (py) {
            *py << "mesh.GroupOnFilter(SMESH.VOLUME,'Viscous Prisms',"
                   "smesh.GetFilter(SMESH.VOLUME,SMESH.FT_ElemGeomType,'=',SMESH.Geom_PENTA))"
                << endl;
            *py << "mesh.GroupOnFilter(SMESH.VOLUME,'Neg Volumes',"
                   "smesh.GetFilter(SMESH.VOLUME,SMESH.FT_Volume3D,'<',0))"
                << endl;
        }
        delete py;
        py = 0;
    ...
    {
        Finish();
        cout << "NB FUNCTIONS: " << theNbPyFunc << endl;
    }
};
#define dumpFunction(f)                                                                            \
    {                                                                                              \
        _dumpFunction(f, __LINE__);                                                                \
    }
#define dumpMove(n)                                                                                \
    {                                                                                              \
        _dumpMove(n, __LINE__);                                                                    \
    }
#define dumpMoveComm(n, txt)                                                                       \
    {                                                                                              \
        _dumpMove(n, __LINE__, txt);                                                               \
    }
#define dumpCmd(txt)                                                                               \
    {                                                                                              \
        _dumpCmd(txt, __LINE__);                                                                   \
    }
void _dumpFunction(const string &fun, int ln)
{
    if (py) *py << "def " << fun << "(): # " << ln << endl;
    cout << fun << "()" << endl;
    ++theNbPyFunc;
}
...
{
    if (py)
        *py << "  mesh.MoveNode( " << n->GetID() << ", " << n->X() << ", " << n->Y() << ", "
            << n->Z() << ")\t\t # " << ln << " " << txt << endl;
}
...
{
    if (py) {
        *py << "  mesh.ChangeElemNodes( " << f->GetID() << ", [";
...
bool _ViscousBuilder::error(const string &text, int solidId)
{
    ...
    if (_mesh) {
        ...
        if (!sm && !_sdVec.empty()) sm = _mesh->GetSubMeshContaining(solidId = _sdVec[0]._index);
        if (sm && sm->GetSubShape().ShapeType() == TopAbs_SOLID) {
            SMESH_ComputeErrorPtr &smError = sm->GetComputeError();
            if (smError && smError->myAlgo) _error->myAlgo = smError->myAlgo;
            smError = _error;
            sm->ComputeStateEngine(SMESH_subMesh::CHECK_COMPUTE_STATE);
        }
        // set KO to all solids
        for (size_t i = 0; i < _sdVec.size(); ++i) {
            if (_sdVec[i]._index == solidId) continue;
...
bool _ViscousBuilder::MakeN2NMap(_MeshOfSolid *pm)
{
    ...
    for (; fExp.More(); fExp.Next()) {
        ...
        if (!srcSmDS || !prxSmDS || !srcSmDS->NbElements() || !prxSmDS->NbElements()) continue;
        if (srcSmDS->GetElements()->next() == prxSmDS->GetElements()->next()) continue;

        if (srcSmDS->NbElements() != prxSmDS->NbElements())
            return error("Different nb elements in a source and a proxy sub-mesh",
                         solidSM->GetId());
        ...
        while (prxIt->more()) {
            ...
            if (fSrc->NbNodes() != fPrx->NbNodes())
                return error("Different elements in a source and a proxy sub-mesh",
                             solidSM->GetId());
...
SMESH_ComputeErrorPtr _ViscousBuilder::Compute(SMESH_Mesh &theMesh, const TopoDS_Shape &theShape)
{
    ...
    TopExp_Explorer exp(theShape, TopAbs_SOLID);
    if (!exp.More()) return error("No SOLID's in theShape"), _error;
    ...
    if (!findSolidsWithLayers()) return _error;

    if (!findFacesWithLayers()) return _error;

    for (size_t i = 0; i < _sdVec.size(); ++i) {
        if (!makeLayer(_sdVec[i])) return _error;

        if (_sdVec[i]._n2eMap.size() == 0) continue;

        if (!inflate(_sdVec[i])) return _error;

        if (!refine(_sdVec[i])) return _error;
    }
    if (!shrink()) return _error;
...
SMESH_ComputeErrorPtr _ViscousBuilder::CheckHypotheses(SMESH_Mesh &mesh, const TopoDS_Shape &shape)
{
    _mesh = &mesh;
    ...
    for (int i = 1; i <= allSolids.Extent(); ++i) {
        ...
        for (; hyp != allHyps.end(); ++hyp)
            if ((viscHyp = dynamic_cast<const StdMeshers_ViscousLayers *>(*hyp))) {
                ...
                if (!soData) {
                    _MeshOfSolid *proxyMesh = _ViscousListener::GetSolidMesh(_mesh, allSolids(i),
                                                                             /*toCreate=*/true);
...
bool _ViscousBuilder::findFacesWithLayers(const bool onlyWith)
{
        ...
        if (nbHyps > 1) {
                    ...
                    if (!igFacesOfHyp->first.count(faceID)) {
                        if (hyp)
                            return error(SMESH_Comment("Several hypotheses define "
                                                       "Viscous Layers on the face #")
                                         << faceID);
                        hyp = igFacesOfHyp->second;
                    }
                if (hyp) _sdVec[i]._face2hyp.insert(make_pair(faceID, hyp));
                else
            ...
            if (nbLayersSet.size() > 1) {
                for (exp.Init(_sdVec[i]._solid, TopAbs_EDGE); exp.More(); exp.Next()) {
                    ...
                    if (hyp1 && hyp2 && hyp1->GetNumberLayers() != hyp2->GetNumberLayers()) {
    ...
    set<string> notSupportAlgos;
    notSupportAlgos.insert("Hexa_3D");
    for (size_t i = 0; i < _sdVec.size(); ++i) {
            ...
            while (soIt->more()) {
                ...
                for (; iSolid < _sdVec.size(); ++iSolid) {
                    if (_sdVec[iSolid]._solid.IsSame(*solid)) {
                        if (_sdVec[iSolid]._shrinkShape2Shape.count(edgeID)) notShrinkFace = false;
                        break;
                    }
                }
                if (notShrinkFace) {
                    ...
                    if (iSolid == _sdVec.size()) continue; // no VL in the solid
                    shapes.Clear();
                    ...
                    for (int iE = 1; iE <= shapes.Extent(); ++iE) {
                        ...
                        if (eID == edgeID || !_sdVec[iSolid]._shrinkShape2Shape.count(eID)
                        ...
                        for (int is1st = 0; is1st < 2; ++is1st) {
                            ...
                            if (_sdVec[i]._noShrinkShapes.count(getMeshDS()->ShapeToIndex(V))) {
                                ...
                                return error("No way to make a conformal mesh with "
                                             "the given set of faces with layers",
                                             _sdVec[i]._index);
                            }
                        }
                    }
                ...
                case 1: {
                    ...
                    if (helper.IsRealSeam(vInd)) // inflate along a seam edge?
                    {
                        ...
                        while (eIt->more() && seamEdge.IsNull()) {
                            ...
                            if (helper.IsRealSeam(*e)) seamEdge = *e;
                        }
                        if (!seamEdge.IsNull()) {
                ...
                case 2: {
                    ...
                    while (eIt->more()) {
                        ...
                        if (helper.IsSubShape(*e, facesWOL[0])
                            ...
                            _sdVec[i]._shrinkShape2Shape.insert(make_pair(vInd, *e));
                            break;
                        }
                    }
                    break;
                }
                default: return error("Not yet supported case", _sdVec[i]._index);
            }
        }
    }
...
void _ViscousBuilder::getIgnoreFaces(const TopoDS_Shape &solid, const StdMeshers_ViscousLayers *hyp,
                                     const TopoDS_Shape &hypShape, set<TGeomID> &ignoreFaceIds)
{
    TopExp_Explorer exp;
    ...
    if (hyp->IsToIgnoreShapes()) // FACEs to ignore are given
    {
        for (size_t ii = 0; ii < ids.size(); ++ii) {
            ...
            if (!s.IsNull() && s.ShapeType() == TopAbs_FACE) ignoreFaceIds.insert(ids[ii]);
        }
    }
    else // FACEs with layers are given
    {
        ...
        for (; exp.More(); exp.Next()) {
            ...
            if (find(ids.begin(), ids.end(), faceInd) == ids.end()) ignoreFaceIds.insert(faceInd);
        }
    }
    ...
    if (hyp->IsToIgnoreShapes()) {
        ...
        TopExp::MapShapesAndAncestors(hypShape, TopAbs_FACE, TopAbs_SOLID, solidsOfFace);

        for (exp.Init(solid, TopAbs_FACE); exp.More(); exp.Next()) {
            const TopoDS_Face &face = TopoDS::Face(exp.Current());
            if (SMESH_MesherHelper::NbAncestors(face, *_mesh, TopAbs_SOLID) < 2) continue;

            int nbSolids = solidsOfFace.FindFromKey(face).Extent();
            if (nbSolids > 1) ignoreFaceIds.insert(getMeshDS()->ShapeToIndex(face));
        }
    }
}
...
bool _ViscousBuilder::makeLayer(_SolidData &data)
{
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (!data._ignoreFaceIds.count(fSubM->GetId())) {
            ...
            while (subIt->more()) subIds.insert(subIt->next()->GetId());
        }
    }
    ...
    for (; s2s != data._shrinkShape2Shape.end(); ++s2s) {
        ...
        for (size_t i = 0; i < _sdVec.size(); ++i) {
            ...
            if (s2s2 != _sdVec[i]._shrinkShape2Shape.end() && *s2s == *s2s2
                && !_sdVec[i]._n2eMap.empty()) {
    ...
    if (SMESH_subMesh *sm = _mesh->GetSubMesh(data._solid)) {
        ...
        while (smIt->more()) {
            sm = smIt->next();
            if (sm->GetSubShape().ShapeType() == TopAbs_FACE && !faceIds.count(sm->GetId()))
                continue;
    ...
    for (set<TGeomID>::iterator id = faceIds.begin(); id != faceIds.end(); ++id) {
        ...
        SMESH_ProxyMesh::SubMesh *proxySub = data._proxyMesh->getFaceSubM(F, /*create=*/true);
        ...
        while (eIt->more()) {
            ...
            for (size_t i = 0; i < newNodes.size(); ++i) {
                ...
                if (!(*n2e).second) {
                    ...
                    if ((!noShrink) && (n->GetPosition()->GetTypeOfPosition() != SMDS_TOP_FACE)
                        ...
                        const_cast<SMDS_MeshNode *>(edge->_nodes.back())
                ...
                if (onDegenEdge) data._n2eMap.insert(make_pair(face->GetNode(i), n2e->second));
            }
            if (newNodes.size() - nbDegenNodes < 2) continue;
            ...
            if (faceMaxCosin > theMinSmoothCosin) limitStepSize(data, face, maxCosinEdge);

        } // loop on 2D elements on a FACE
    }     // loop on FACEs of a SOLID

    data._epsilon = 1e-7;
    if (data._stepSize < 1.) data._epsilon *= data._stepSize;

    if (!findShapesToSmooth(data)) return false;
    ...
    for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        ...
        for (size_t i = 0; i < localEdges.size(); ++i) {
            ...
            if (edge->IsOnEdge()) {
                ...
                if (!hasData) edge->SetDataByNeighbors(nn[0], nn[1], eos, helper);
            }
            ...
            if (helper.IsDegenShape(edge->_nodes[0]->getshapeId())) {
                ...
                if ((n2e = data._n2eMap.find(vN)) == data._n2eMap.end()) continue;
...
void _ViscousBuilder::limitStepSize(_SolidData &data, const SMDS_MeshElement *face,
                                    const _LayerEdge *maxCosinEdge)
{
    int iN = 0;
    ...
    for (int i = 0; i < nbNodes; ++i) {
        ...
        if (nextN->GetPosition()->GetTypeOfPosition() == SMDS_TOP_FACE
            ...
            if (dist < minSize) minSize = dist, iN = i;
        }
    }
...
void _ViscousBuilder::limitStepSize(_SolidData &data, const double minSize)
{
    if (minSize < data._stepSize) {
        ...
        if (data._stepSizeNodes[0]) {
            double dist = SMESH_TNodeXYZ(data._stepSizeNodes[0]).Distance(data._stepSizeNodes[1]);
...
void _ViscousBuilder::limitStepSizeByCurvature(_SolidData &data)
{
    ...
    for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        ...
        if (eof.ShapeType() != TopAbs_FACE || data._ignoreFaceIds.count(eof._shapeID)) continue;
        ...
        while (smIt->more()) {
            ...
            for (size_t i = 0; i < eos->_edges.size(); i += iStep) {
                ...
                if (!surfProp.IsCurvatureDefined()) continue;
        ...
        _ConvexFace &convFace = data._convexFaces.insert(make_pair(faceID, cnvFace)).first->second;
...
bool _ViscousBuilder::findShapesToSmooth(_SolidData &data)
{
    ...
    for (size_t iS = 0; iS < edgesByGeom.size(); ++iS) // check FACEs
    {
        ...
        eos._toSmooth = false;
        if (eos._edges.empty() || eos.ShapeType() != TopAbs_FACE) continue;
                ...
                for (size_t i = 0; i < eE.size() && !eos._toSmooth; ++i)
                    if (eE[i]->_cosin > theMinSmoothCosin) {
                        SMDS_ElemIteratorPtr fIt =
                            eE[i]->_nodes[0]->GetInverseElementIterator(SMDSAbs_Face);
        ...
        if (eos._toSmooth) {
            for (eExp.ReInit(); eExp.More(); eExp.Next()) edgesOfSmooFaces.Add(eExp.Current());
    ...
    for (size_t iS = 0; iS < edgesByGeom.size(); ++iS) // check EDGEs
    {
        ...
        if (SMESH_Algo::isDegenerated(E) || !edgesOfSmooFaces.Contains(E)) continue;
        ...
        for (TopoDS_Iterator vIt(E); vIt.More() && !eos._toSmooth; vIt.Next()) {
            ...
            if (cosinAbs > theMinSmoothCosin) {
                ...
                SMDS_ElemIteratorPtr eIt =
                    eV[0]->_nodes[0]->GetInverseElementIterator(SMDSAbs_Edge);
    ...
    for (size_t iS = 0; iS < edgesByGeom.size(); ++iS) {
        ...
        if (!eos._hyp.ToSmooth())
            for (size_t i = 0; i < eos._edges.size(); ++i) eos._edges[i]->SetCosin(0);
    }
...
void _ViscousBuilder::setShapeData(_EdgesOnShape &eos, SMESH_subMesh *sm, _SolidData &data)
{
    if (!eos._shape.IsNull() || sm->GetSubShape().ShapeType() == TopAbs_WIRE) return;
    ...
    // set _SWOL
    map<TGeomID, TopoDS_Shape>::const_iterator s2s = data._shrinkShape2Shape.find(eos._shapeID);
    if (s2s != data._shrinkShape2Shape.end()) eos._sWOL = s2s->second;

    // set _hyp
    if (data._hyps.size() == 1) { eos._hyp = data._hyps.back(); }
    else {
...
bool _EdgesOnShape::GetNormal(const SMDS_MeshElement *face, gp_Vec &norm)
{
    ...
    if (face->getshapeId() == _shapeID) { eos = this; }
    else {
        for (size_t iF = 0; iF < _faceEOS.size() && !eos; ++iF)
            if (face->getshapeId() == _faceEOS[iF]->_shapeID) eos = _faceEOS[iF];
    }

    if ((eos) && (ok = (face->getIdInShape() < eos->_faceNormals.size()))) {
        norm = eos->_faceNormals[face->getIdInShape()];
    }
    else if (!eos) {
        debugMsg("_EdgesOnShape::Normal() failed for face " << face->GetID() << " on _shape #"
                                                            << _shapeID);
    }
    return ok;
}
...
bool _ViscousBuilder::setEdgeData(_LayerEdge &edge, _EdgesOnShape &eos, const set<TGeomID> &subIds,
                                  SMESH_MesherHelper &helper, _SolidData &data)
{
    ...
    const bool useGeometry =
        ((eos._hyp.UseSurfaceNormal()) || (eos.ShapeType() != TopAbs_FACE && !onShrinkShape));

    // get geom FACEs the node lies on
    //if ( useGeometry )
    {
        set<TGeomID> faceIds;
        if (eos.ShapeType() == TopAbs_FACE) { faceIds.insert(eos._shapeID); }
        else {
            SMDS_ElemIteratorPtr fIt = node->GetInverseElementIterator(SMDSAbs_Face);
            while (fIt->more()) faceIds.insert(fIt->next()->getshapeId());
        }
        set<TGeomID>::iterator id = faceIds.begin();
        for (; id != faceIds.end(); ++id) {
            const TopoDS_Shape &s = getMeshDS()->IndexToShape(*id);
            if (s.IsNull() || s.ShapeType() != TopAbs_FACE || !subIds.count(*id)) continue;
            F = TopoDS::Face(s);
    ...
    if (useGeometry) {
        if (onShrinkShape) // one of faces the node is on has no layers
        {
            if (eos.SWOLType() == TopAbs_EDGE) {
                ...
                edge._normal = getFaceDir(TopoDS::Face(eos._sWOL), TopoDS::Vertex(eos._shape), node,
                                          helper, normOK, &edge._cosin);
            }
            else {
                // inflate from EDGE along FACE
                edge._normal = getFaceDir(TopoDS::Face(eos._sWOL), TopoDS::Edge(eos._shape), node,
                                          helper, normOK);
            }
        }
        ...
        // layers are on all faces of SOLID the node is on
            ...
            for (int iF = 0; iF < totalNbFaces; ++iF) {
                ...
                if (helper.GetSubShapeOri(data._solid, F) != TopAbs_REVERSED) geomNorm.Reverse();
                face2Norm[iF].second = geomNorm.XYZ();
                edge._normal += geomNorm.XYZ();
            }
            if (nbOkNorms == 0)
                return error(SMESH_Comment("Can't get normal to node ") << node->GetID(),
                             data._index);
            ...
            if (edge._normal.Modulus() < 1e-3 && nbOkNorms > 1) {
                ...
                for (int iF = 0; iF < totalNbFaces; ++iF) {
                    ...
                    if (helper.GetSubShapeOri(data._solid, F) != TopAbs_REVERSED)
                        geomNorm.Reverse();
                    if (normOK) face2Norm[iF].second = geomNorm.XYZ();
...
gp_XYZ _ViscousBuilder::getFaceNormal(const SMDS_MeshNode *node, const TopoDS_Face &face,
                                      SMESH_MesherHelper &helper, bool &isOK, bool shiftInside)
{
    gp_XY uv;
    if (shiftInside) {
    ...
    if (!shiftInside && helper.IsDegenShape(node->getshapeId())
    ...
    if (pointKind == IMPOSSIBLE && node->GetPosition()->GetDim() == 2) // node inside the FACE
    {
    ...
    if (pointKind < IMPOSSIBLE) {
            ...
            && node->GetPosition()->GetDim() < 2) // FACE boundary
        {
            ...
            if (normShift * normal.XYZ() < 0.) normal = normShift;
        }
        isOK = true;
    }
        ...
        while (fIt->more()) {
            ...
            if (f->getshapeId() == faceID) {
                ...
                if (isOK) {
                    ...
                    if (helper.IsReversedSubMesh(ff)) normal.Reverse();
                    break;
                }
            }
        }
...
bool _ViscousBuilder::getFaceNormalAtSingularity(const gp_XY &uv, const TopoDS_Face &face,
                                                 SMESH_MesherHelper &helper, gp_Dir &normal)
{
    BRepAdaptor_Surface surface(face);
    gp_Dir axis;
    if (!getRovolutionAxis(surface, axis)) return false;
    ...
    for (int iLoop = 0; true; ++iLoop) {
        ...
        if (GeomLib::NormEstim(geomsurf, testUV, tol, refDir) == REGULAR) break;
        if (iLoop > 20) return false;
        tol /= 10.;
    }

    if (axis * refDir < 0.) axis.Reverse();
...
bool _ViscousBuilder::findNeiborsOnEdge(const _LayerEdge *edge, const SMDS_MeshNode *&n1,
                                        const SMDS_MeshNode *&n2, _EdgesOnShape &eos,
                                        _SolidData &data)
{
    ...
    if (!n2) return error(SMESH_Comment("Wrongly meshed EDGE ") << shapeInd, data._index);
    return true;
}
...
void _LayerEdge::SetDataByNeighbors(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                    const _EdgesOnShape &eos, SMESH_MesherHelper &helper)
{
    if (eos.ShapeType() != TopAbs_EDGE) return;
...
gp_XYZ _LayerEdge::Copy(_LayerEdge &other, _EdgesOnShape &eos, SMESH_MesherHelper &helper)
{
    _nodes = other._nodes;
    ...
    _curvature = 0;
    std::swap(_curvature, other._curvature);
    _2neibors = 0;
    std::swap(_2neibors, other._2neibors);
...
void _Simplex::GetSimplices(const SMDS_MeshNode *node, vector<_Simplex> &simplices,
                            const set<TGeomID> &ingnoreShapes, const _SolidData *dataToCheckOri,
                            const bool toSort)
{
    simplices.clear();
    SMDS_ElemIteratorPtr fIt = node->GetInverseElementIterator(SMDSAbs_Face);
    ...
    if (toSort) SortSimplices(simplices);
}
...
void _Simplex::SortSimplices(vector<_Simplex> &simplices)
{
    ...
    if (nbFound == simplices.size() - 1) simplices.swap(sortedSimplices);
}
    ...
    for (size_t i = 0; i < _sdVec.size(); ++i) {
        ...
        for (n2e = _sdVec[i]._n2eMap.begin(); n2e != _sdVec[i]._n2eMap.end(); ++n2e) {
            ...
            for (size_t iN = 1; iN < le->_nodes.size(); ++iN)
                dumpCmd(SMESH_Comment("mesh.AddEdge([ ")
                        << le->_nodes[iN - 1]->GetID() << ", " << le->_nodes[iN]->GetID() << "])");
        }
        dumpFunctionEnd();
        ...
        for (n2e = _sdVec[i]._n2eMap.begin(); n2e != _sdVec[i]._n2eMap.end(); ++n2e) {
            ...
            dumpCmd(SMESH_Comment("mesh.AddEdge([ ")
                    << edge->_nodes[0]->GetID() << ", mesh.AddNode( " << nXYZ.X() << "," << nXYZ.Y()
                    << "," << nXYZ.Z() << ")])");
        }
        dumpFunctionEnd();
        ...
        dumpCmd(SMESH_Comment("mesh.MakeGroup( 'tmpFaces_")
                << i << "',"
...
void _ViscousBuilder::computeGeomSize(_SolidData &data)
{
    ...
    unique_ptr<SMESH_ElementSearcher> searcher(
        SMESH_MeshAlgos::GetElementSearcher(*getMeshDS(), data._proxyMesh->GetFaces(data._solid)));
    ...
    for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        ...
        for (size_t i = 0; i < eos._edges.size(); ++i) {
            ...
            if (data._geomSize > intersecDist && intersecDist > 0) data._geomSize = intersecDist;
        }
    }
}
...
bool _ViscousBuilder::inflate(_SolidData &data)
{
    ...
    if (data._stepSize > 0.3 * data._geomSize) limitStepSize(data, 0.3 * data._geomSize);

    const double tgtThick = data._maxThickness;
    if (data._stepSize > data._minThickness) limitStepSize(data, data._minThickness);

    if (data._stepSize < 1.) data._epsilon = data._stepSize * 1e-7;

    debugMsg("-- geomSize = " << data._geomSize << ", stepSize = " << data._stepSize);

    const double safeFactor =
        (2 * data._maxThickness < data._geomSize) ? 1 : theThickToIntersection;
    ...
    while (avgThick < 0.99) {
        ...
        if (!updateNormals(data, helper, nbSteps)) return false;
...
#endif
                dumpFunction(SMESH_Comment("invalidate")
                             << data._index << "_step" << nbSteps); // debug
                for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
                    _EdgesOnShape &eos = data._edgesOnShape[iS];
                    for (size_t i = 0; i < eos._edges.size(); ++i)
    ...
    if (nbSteps == 0) return error("failed at the very first inflation step", data._index);

    if (avgThick < 0.99) {
        if (!data._proxyMesh->_warning || data._proxyMesh->_warning->IsOK()) {
            data._proxyMesh->_warning.reset(new SMESH_ComputeError(
                COMPERR_WARNING,
                SMESH_Comment("Thickness ") << tgtThick
                                            << " of viscous layers not reached,"
                                               " average reached thickness is "
                                            << avgThick * tgtThick));
        }
    }
    ...
    for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        ...
        if (!eos._edges.empty() && eos._edges[0]->_nodes.size() == 1)
            for (size_t i = 0; i < eos._edges.size(); ++i) { restoreNoShrink(*eos._edges[i]); }
    }
    dumpFunctionEnd();
...
bool _ViscousBuilder::smoothAndCheck(_SolidData &data, const int nbSteps,
                                     double &distToIntersection)
{
    if (data._nbShapesToSmooth == 0) return true; // no shapes needing smoothing
    ...
    for (int isFace = 0; isFace < 2; ++isFace) // smooth on [ EDGEs, FACEs ]
    {
        ...
        for (int iS = 0; iS < data._edgesOnShape.size(); ++iS) {
            _EdgesOnShape &eos = data._edgesOnShape[iS];
            if (!eos._toSmooth || eos.ShapeType() != shapeType || eos._edges.empty()) continue;
            ...
            if (!eos._hyp.ToSmooth()) {
                ...
                for (size_t i = 0; i < eos._edges.size(); ++i) {
                    ...
                    for (size_t iF = 0; iF < edge->_simplices.size(); ++iF)
                        if (!edge->_simplices[iF].IsForward(edge->_nodes[0], &edge->_pos.back(),
                                                            vol))
                            return false;
                }
            ...
            if (eos.ShapeType() == TopAbs_EDGE) {
                dumpFunction(SMESH_Comment("smooth")
                             << data._index << "_Ed" << sInd << "_InfStep" << nbSteps);
                ...
                while ((++step <= stepLimit) || improved) {
                    dumpFunction(SMESH_Comment("smooth")
                                 << data._index << "_Fa" << sInd << "_InfStep" << nbSteps << "_"
                                 << step); // debug
                    int oldBadNb = badNb;
                    badSmooEdges.clear();
                    ...
                    if (!badSmooEdges.empty() && step >= stepLimit / 2) {
                        ...
                        for (size_t i = 0; i < badSmooEdges.size(); ++i) {
                            ...
                            _Simplex::GetSimplices(ledge->_nodes[0], simplices,
                                                   data._ignoreFaceIds);
                    ...
                    for (int i = 0; i < eos._edges.size(); ++i) {
                        ...
                        for (size_t j = 0; j < edge->_simplices.size(); ++j)
                            if (!edge->_simplices[j].IsForward(edge->_nodes[0], &tgtXYZ, vol)) {
                                cout << "Bad simplex ( " << edge->_nodes[0]->GetID() << " "
                                     << tgtXYZ._node->GetID() << " "
    ...
    for (; id2face != data._convexFaces.end(); ++id2face) {
        ...
        if (!convFace.CheckPrisms()) return false;
    }
    ...
    for (int iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        _EdgesOnShape &eos = data._edgesOnShape[iS];
        if (eos._edges.empty() || !eos._sWOL.IsNull()) continue;
        for (size_t i = 0; i < eos._edges.size(); ++i) {
            ...
            if (distToIntersection > dist) {
                ...
                if (_ConvexFace *convFace = data.GetConvexFace(intFace->getshapeId()))
                    if (convFace->_subIdToEOS.count(eos._shapeID)) continue;
                ...
                if (intFace->getshapeId() == eos._shapeID) continue;
    ...
    if (closestFace) {
        ...
        cout << "Shortest distance: _LayerEdge nodes: tgt " << le->_nodes.back()->GetID() << " src "
             << le->_nodes[0]->GetID() << ", intersection with face (" << (*nIt++)->GetID() << " "
             << (*nIt++)->GetID() << " " << (*nIt++)->GetID()
...
Handle(Geom_Curve) _SolidData::CurveForSmooth(const TopoDS_Edge &E, _EdgesOnShape &eos,
                                              SMESH_MesherHelper &helper)
{
    ...
    if (i2curve == _edge2curve.end()) {
        ...
        if (eos._sWOL.IsNull()) /////////////////////////////////////////// 3D case
        {
            ...
            if (!isLine && !isCirc) // Check if the EDGE is close to a line
            {
                ...
                if (isLine) line = new Geom_Line(gp::OX()); // only type does matter
            }
            if (!isLine && !isCirc
                && eos._edges.size() > 2) // Check if the EDGE is close to a circle
            {
                // TODO
            }
        }
        else //////////////////////////////////////////////////////////////////////// 2D case
        {
            ...
            if (!isLine && !isCirc) // Check if the EDGE is close to a line
            {
                Bnd_B2d bndBox;
                SMDS_NodeIteratorPtr nIt = smDS->GetNodes();
                while (nIt->more()) bndBox.Add(helper.GetNodeUV(F, nIt->next()));
                gp_XY size = bndBox.CornerMax() - bndBox.CornerMin();

                const double lineTol = 1e-2 * sqrt(bndBox.SquareExtent());
                for (int i = 0; i < 2 && !isLine; ++i) isLine = (size.Coord(i + 1) <= lineTol);
            }
            if (!isLine && !isCirc
                && eos._edges.size() > 2) // Check if the EDGE is close to a circle
            {
...
void _SolidData::SortOnEdge(const TopoDS_Edge &E, vector<_LayerEdge *> &edges,
                            SMESH_MesherHelper &helper)
{
    ...
    for (int i = 0; i < edges.size(); ++i, ++u2e) edges[i] = u2e->second;
...
void _SolidData::Sort2NeiborsOnEdge(vector<_LayerEdge *> &edges)
{
    ...
    if (edges.size() > 1 && edges[iLast]->_2neibors->tgtNode(0) != edges[iLast - 1]->_nodes.back())
...
{
    if (shapeID < _edgesOnShape.size() && _edgesOnShape[shapeID]._shapeID == shapeID)
        return &_edgesOnShape[shapeID];

    for (size_t i = 0; i < _edgesOnShape.size(); ++i)
        if (_edgesOnShape[i]._shapeID == shapeID) return &_edgesOnShape[i];
...
void _SolidData::PrepareEdgesToSmoothOnFace(_EdgesOnShape *eof, bool substituteSrcNodes)
{
    ...
    for (size_t i = 0; i < eof->_edges.size(); ++i) eof->_edges[i]->_smooFunction = 0;

    for (size_t i = 0; i < eof->_edges.size(); ++i) {
        _LayerEdge *edge = eof->_edges[i];
        _Simplex::GetSimplices(edge->_nodes[0], edge->_simplices, _ignoreFaceIds, this,
                               /*sort=*/true);
...
bool _ViscousBuilder::smoothAnalyticEdge(_SolidData &data, _EdgesOnShape &eos,
                                         Handle(Geom_Surface) & surface, const TopoDS_Face &F,
                                         SMESH_MesherHelper &helper)
{
    ...
    vector<double> len(iTo - iFrom + 1);
    {
        ...
        for (int i = iFrom; i < iTo; ++i) {
            curLen =
                prevLen * eos._edges[i]->_2neibors->_wgt[0] / eos._edges[i]->_2neibors->_wgt[1];
        ...
        if (F.IsNull()) // 3D
        {
                ...
                == eos._edges[iTo - 1]->_2neibors->tgtNode(1)) // closed edge
            {
                ...
                if (iPeriodic == 1 || iPeriodic == 2) {
                    ...
                    if (uv0.Coord(iPeriodic) > uv1.Coord(iPeriodic)) std::swap(uv0, uv1);
                }
            }
    ...
    if (curve->IsKind(STANDARD_TYPE(Geom_Circle))) {
        ...
        else // 2D
        {
            ...
            if (uLast * uMidl <= 0.) uLast += (uMidl > 0 ? +2. : -2.) * M_PI;
...
bool _ViscousBuilder::updateNormals(_SolidData &data, SMESH_MesherHelper &helper, int stepNb)
{
    if (stepNb > 0) return updateNormalsOfConvexFaces(data, helper, stepNb);
    ...
    vector<const SMDS_MeshElement *> tmpFaces;
    {
        ...
        for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
            _EdgesOnShape &eos = data._edgesOnShape[iS];
            if (eos.ShapeType() != TopAbs_EDGE || !eos._sWOL.IsNull()) continue;
            for (size_t i = 0; i < eos._edges.size(); ++i) {
                _LayerEdge *edge = eos._edges[i];
    ...
    SMDS_ElemIteratorPtr fIt(new SMDS_ElementVectorIterator(tmpFaces.begin(), tmpFaces.end()));
    unique_ptr<SMESH_ElementSearcher> searcher(
    ...
    if (!edge2CloseEdge.empty()) {
        ...
        for (size_t iE = 0; e2ee != edge2CloseEdge.end(); ++e2ee, ++iE) {
            ...
            for (; !edge2 && eIt != ee.end(); ++eIt) {
                if (eos1->_sWOL == data.GetShapeEdges(*eIt)->_sWOL) edge2 = *eIt;
            }
            if (!edge2) continue;
            ...
            // get new cosin
            if (cos1 < theMinSmoothCosin) { newEdge._cosin = edge2->_cosin; }
            ...
            if (edge1->_cosin < theMinSmoothCosin && newEdge._cosin > theMinSmoothCosin) {
                if (eos1->_sWOL.IsNull()) {
                    SMDS_ElemIteratorPtr fIt =
                        edge1->_nodes[0]->GetInverseElementIterator(SMDSAbs_Face);
                    while (fIt->more())
                ...
                else // edge1 inflates along a FACE
                {
                    ...
                    while (const TopoDS_Shape *E = eIt->next()) {
                        if (!helper.IsSubShape(*E, /*FACE=*/eos1->_sWOL)) continue;
                        gp_Vec edgeDir = getEdgeDir(TopoDS::Edge(*E), TopoDS::Vertex(V));
                        double angle = edgeDir.Angle(newEdge._normal); // [0,PI]
                        if (angle < M_PI / 2) shapesToSmooth.insert(data.GetShapeEdges(*E));
                    }
                }
            }
        }
        ...
        for (size_t iE = 0; iE < edge2newEdge.size(); ++iE) {
            ...
            if (!edge1->_2neibors) continue;
            for (int j = 0; j < 2; ++j) // loop on 2 neighbors
            {
                _LayerEdge *neighbor = edge1->_2neibors->_edges[j];
                if (edge2CloseEdge.count(neighbor)) continue; // j-th neighbor is also intersected
                ...
                {
                    if (!neighbor->_2neibors) break; // neighbor is on VERTEX
                    int iNext = 0;
                    _LayerEdge *nextEdge = neighbor->_2neibors->_edges[iNext];
                    if (nextEdge == prevEdge) nextEdge = neighbor->_2neibors->_edges[++iNext];
                    double r = double(step - 1) / nbSteps;
                    if (!nextEdge->_2neibors) r = 0.5;
                    ...
                    neighbor->SetDataByNeighbors(prevEdge->_nodes[0], nextEdge->_nodes[0], *eos,
                                                 helper);
    ...
    for (size_t i = 0; i < tmpFaces.size(); ++i) delete tmpFaces[i];
...
bool _ViscousBuilder::updateNormalsOfConvexFaces(_SolidData &data, SMESH_MesherHelper &helper,
                                                 int stepNb)
{
    ...
    for (; id2face != data._convexFaces.end(); ++id2face) {
        _ConvexFace &convFace = (*id2face).second;
        if (convFace._normalsFixed) continue; // already fixed
        if (convFace.CheckPrisms()) continue; // nothing to fix
        ...
        const bool isSpherical = (centersBox.SquareExtent() < 1e-6 * nodesBox.SquareExtent());
        ...
        if (isSpherical) {
            ...
            for (; id2eos != convFace._subIdToEOS.end(); ++id2eos) {
                ...
                if (eos.ShapeType() == TopAbs_EDGE) {
                    ...
                    if (!eos._sWOL.IsNull()) ceCurve._adjFace.Nullify();
                    else
                        ceCurve._ledges.insert(ceCurve._ledges.end(), eos._edges.begin(),
                                               eos._edges.end());
                }
                // summarize normals
                for (size_t i = 0; i < eos._edges.size(); ++i) avgNormal += eos._edges[i]->_normal;
            }
            ...
            for (size_t iE = 0; iE < centerCurves.size(); ++iE) {
                ...
                if (ceCurve._adjFace.IsNull()) continue;
            ...
            for (; id2eos != convFace._subIdToEOS.end(); ++id2eos) {
                _EdgesOnShape &eos = *(id2eos->second);
                if (eos.ShapeType() != TopAbs_EDGE)
                    for (size_t i = 0; i < eos._edges.size(); ++i) eos._edges[i]->_cosin = avgCosin;

                for (size_t i = 0; i < eos._edges.size(); ++i) eos._edges[i]->_normal = avgNormal;
            }
        }
        else // if ( isSpherical )
        {
            ...
            for (int iE = 0; edgeExp.More(); edgeExp.Next(), ++iE) {
                ...
                if (!eos || eos->_edges.empty()) {
                    ...
                    if (!eos->_toSmooth) data.SortOnEdge(edge, eos->_edges, helper);
                    ...
                    if (!eos->_sWOL.IsNull()) centerCurves[iE]._adjFace.Nullify();
                }
                ...
                if (edgeLEdge[0]->IsOnEdge()
                    && convFace.GetCenterOfCurvature(vertexLEdges[0], surfProp, helper,
                                                     center)) { // 1st VERTEX
                    centerCurves[iE].Append(center, vertexLEdges[0]);
                    centersBox.Add(center);
                }
                for (; edgeLEdge < edgeLEdgeEnd; ++edgeLEdge)
                    if (convFace.GetCenterOfCurvature(*edgeLEdge, surfProp, helper,
                                                      center)) { // EDGE or VERTEXes
                        centerCurves[iE].Append(center, *edgeLEdge);
                        centersBox.Add(center);
                    }
                if (edgeLEdge[-1]->IsOnEdge()
                    && convFace.GetCenterOfCurvature(vertexLEdges[1], surfProp, helper,
                                                     center)) { // 2nd VERTEX
            ...
            for (size_t iE1 = 0; iE1 < centerCurves.size(); ++iE1) {
                ...
                if (ceCurve._isDegenerated) continue;
            ...
            for (size_t iE = 0; iE < centerCurves.size(); ++iE) {
                if (centerCurves[iE]._isDegenerated) continue;
            ...
            for (size_t iE = 0; iE < centerCurves.size(); ++iE) {
                if (!centerCurves[iE]._isDegenerated || centerCurves[iE]._ledges.size() < 3)
                    continue;
            ...
            if (id2eos != convFace._subIdToEOS.end()) {
                ...
                for (size_t i = 0; i < eos._edges.size(); ++i) {
                    _LayerEdge *ledge = eos._edges[i];
                    if (!convFace.GetCenterOfCurvature(ledge, surfProp, helper, center)) continue;
        ...
        dumpFunction(SMESH_Comment("updateNormalsOfConvexFaces")
                     << data._index << "_F" << meshDS->ShapeToIndex(convFace._face));
        ...
        for (size_t iE = 0; iE < centerCurves.size(); ++iE) {
            if (centerCurves[iE]._adjFace.IsNull() || centerCurves[iE]._adjFaceToSmooth) continue;
            for (size_t iLE = 0; iLE < centerCurves[iE]._ledges.size(); ++iLE) {
...
bool _ConvexFace::GetCenterOfCurvature(_LayerEdge *ledge, BRepLProp_SLProps &surfProp,
                                       SMESH_MesherHelper &helper, gp_Pnt &center) const
{
    gp_XY uv = helper.GetNodeUV(_face, ledge->_nodes[0]);
    surfProp.SetParameters(uv.X(), uv.Y());
    if (!surfProp.IsCurvatureDefined()) return false;
    ...
    if (surfCurvatureMin > surfCurvatureMax)
        center =
            surfProp.Value().Translated(surfProp.Normal().XYZ() / surfCurvatureMin * oriFactor);
    else
        center =
            surfProp.Value().Translated(surfProp.Normal().XYZ() / surfCurvatureMax * oriFactor);
...
{
    if (this->_isDegenerated) return false;
    ...
    for (size_t i = 0, nb = _curvaCenters.size() - 1; i < nb; ++i) {
        ...
        if (d2 > sl2 || d2 + d1 < 1e-100) continue;

        d1 = Sqrt(d1);
        d2 = Sqrt(d2);
        double r = d1 / (d1 + d2);
        gp_XYZ norm = ((1. - r) * _ledges[i]->_normal + (r)*_ledges[i + 1]->_normal);
        norm.Normalize();
...
void _CentralCurveOnEdge::SetShapes(const TopoDS_Edge &edge, const _ConvexFace &convFace,
                                    _SolidData &data, SMESH_MesherHelper &helper)
{
    _edge = edge;
...
bool _LayerEdge::FindIntersection(SMESH_ElementSearcher &searcher, double &distance,
                                  const double &epsilon, _EdgesOnShape &eos,
                                  const SMDS_MeshElement **face)
{
    ...
    for (size_t j = 0; j < suspectFaces.size() /*&& !segmentIntersected*/; ++j) {
        ...
        if (face->GetNodeIndex(_nodes.back()) >= 0 || face->GetNodeIndex(_nodes[0]) >= 0)
        ...
        if (intFound) {
            ...
            if (distance > dist) distance = dist, iFace = j;
        }
    }
    ...
    if (segmentIntersected) {
        ...
        cout << "nodes: tgt " << _nodes.back()->GetID() << " src " << _nodes[0]->GetID()
             << ", intersection with face (" << (*nIt++)->GetID() << " " << (*nIt++)->GetID() << " "
             << (*nIt++)->GetID() << ") at point (" << intP.X() << ", " << intP.Y() << ", "
             << intP.Z() << ") distance = " << distance - segLen << endl;
#endif
    }
...
gp_Ax1 _LayerEdge::LastSegment(double &segLen, _EdgesOnShape &eos) const
{
    ...
    while (iPrev >= 0) {
        dir = orig - _pos[iPrev];
        if (dir.SquareModulus() > tol * tol) break;
        else
            iPrev--;
    }
...
gp_XY _LayerEdge::LastUV(const TopoDS_Face &F, _EdgesOnShape &eos) const
{
    ...
    if (!C2d.IsNull() && f <= u && u <= l) return C2d->Value(u).XY();
...
bool _LayerEdge::SegTriaInter(const gp_Ax1 &lastSegment, const SMDS_MeshNode *n0,
                              const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, double &t,
...
bool _LayerEdge::SmoothOnEdge(Handle(Geom_Surface) & surface, const TopoDS_Face &F,
                              SMESH_MesherHelper &helper)
{
    ASSERT(IsOnEdge());
...
{
    if (_simplices.size() < 2) return 0; // _LayerEdge inflated along EDGE or FACE
    ...
    for (int iFun = -1; iFun < theNbSmooFuns; ++iFun) {
        if (iFun < 0) newPos = (this->*_smooFunction)(); // fun chosen by ChooseSmooFunction()
        ...
        if (_curvature) {
            ...
            if (delta > 0) newPos += _normal * delta;
            else {
                double segLen = _normal * (newPos - prevPos);
                if (segLen + delta > 0) newPos += _normal * delta;
            }
        ...
        // get worse?
        if (nbOkAfter < nbOkBefore) continue;
        if ((isConcaveFace || findBest) && (nbOkAfter == nbOkBefore) &&
...
void _LayerEdge::ChooseSmooFunction(const set<TGeomID> &concaveVertices, const TNode2Edge &n2eMap)
{
...
{
    if (!fun) fun = _smooFunction;
    for (int i = 0; i < theNbSmooFuns; ++i)
        if (fun == _funs[i]) return i;
...
{
    gp_XYZ newPos(0, 0, 0);
    for (size_t i = 0; i < _simplices.size(); ++i) newPos += SMESH_TNodeXYZ(_simplices[i]._nPrev);
...
{
    vector<gp_Vec> edgeDir;
    edgeDir.reserve(_simplices.size() + 1);
    vector<double> edgeSize;
    edgeSize.reserve(_simplices.size());
    vector<gp_XYZ> points;
    points.reserve(_simplices.size());
...
{
    vector<double> edgeSize;
    edgeSize.reserve(_simplices.size() + 1);
    vector<gp_XYZ> points;
    points.reserve(_simplices.size());
    ...
    for (size_t i = 0; i < _simplices.size(); ++i) {
        ...
        if (edgeSize.back() < numeric_limits<double>::min()) { edgeSize.pop_back(); }
        else {
            points.push_back(p);
        }
        pPrev = p;
...
gp_XYZ _LayerEdge::smoothNefPolygon()
{
    ...
    gp_XYZ center(0, 0, 0);
    for (i = 0; i < _simplices.size(); ++i) {
        vecs[i] = (SMESH_TNodeXYZ(_simplices[i]._nNext) - SMESH_TNodeXYZ(_simplices[i]._nPrev));
    ...
    gp_XYZ zAxis(0, 0, 0);
    for (i = 0; i < _simplices.size(); ++i) zAxis += vecs[i] ^ vecs[i + 1];

    gp_XYZ yAxis;
    for (i = 0; i < _simplices.size(); ++i) {
        yAxis = vecs[i];
        if (yAxis.SquareModulus() > tol) break;
    }
    ...
    for (int iHP1 = 0; iHP1 < nbHP; ++iHP1) {
            ...
            if (ips1.second == UNDEF) {
                ...
                if (ips1.second != NO_INT) {
                    ...
                    for (int i = 0; i < nbHP && ips1.second == NOT_OUT; ++i)
                        if (i != iHP1 && i != iHP2 && halfPlns[i].IsOut(ips1.first, tol))
                            ips1.second = IS_OUT;
                }
        ...
        if (nbNotOut < 2) continue; // no such a segment
        ...
        if (nbNotOut > 2) {
            ...
            for (; u2ips != ipsByParam.end(); ++u2ips) {
                ...
                if (ips1.second == NOT_OUT) segEnds[bool(nbNotOut++)] = &ips1.first;
                else if (nbNotOut >= 2)
                    break;
                else
                    nbNotOut = 0;
...
void _LayerEdge::SetNewLength(double len, _EdgesOnShape &eos, SMESH_MesherHelper &helper)
{
    ...
    if (eos._hyp.IsOffsetMethod()) {
        ...
        while (faceIt->more()) {
            ...
            if (!eos.GetNormal(face, faceNorm)) continue;
            ...
            if (dot < std::numeric_limits<double>::min()) dot = (len - _len) * 1e-3;
...
void _LayerEdge::InvalidateStep(int curStep, const _EdgesOnShape &eos, bool restoreLength)
{
    if (_pos.size() > curStep) {
        if (restoreLength) _len -= (_pos[curStep - 1] - _pos.back()).Modulus();
...
bool _ViscousBuilder::refine(_SolidData &data)
{
    ...
    for (size_t iS = 0; iS < data._edgesOnShape.size(); ++iS) {
        ...
        if (eos._edges[0]->_nodes.size() < 2) continue; // on _noShrinkShapes
        ...
        for (size_t i = 0; i < eos._edges.size(); ++i) {
            ...
            if (baseShapeId != prevBaseId) {
                ...
                if (s2ne == data._s2neMap.end()) n2eMap = 0;
                else
                    n2eMap = s2ne->second;
            ...
            if (fPowN - 1 <= numeric_limits<double>::min()) h0 = T / N;
            else
                h0 = T * (f - 1) / (fPowN - 1);
            ...
            for (size_t iStep = 1; iStep < edge._nodes.size(); ++iStep) {
                ...
                hi *= f;
                hSum += hi;
                while (hSum > segLen[iSeg] && iSeg < segLen.size() - 1) ++iSeg;
                int iPrevSeg = iSeg - 1;
                while (fabs(segLen[iPrevSeg] - segLen[iSeg]) <= zeroLen && iPrevSeg > 0) --iPrevSeg;
                ...
                if (!eos._sWOL.IsNull()) {
                    ...
                    if (isOnEdge) {
                        u = pos.X();
                        if (!node) pos = curve->Value(u).Transformed(loc);
                    }
                    else {
                        uv.SetCoord(pos.X(), pos.Y());
                        if (!node) pos = surface->Value(pos.X(), pos.Y()).Transformed(loc);
                    }
                }
                ...
                if (!node) {
                    ...
                    if (!eos._sWOL.IsNull()) {
                        if (isOnEdge) getMeshDS()->SetNodeOnEdge(node, geomEdge, u);
                        else
                    ...
                    if (!eos._sWOL.IsNull()) {
                        ...
                        if (isOnEdge) {
                            ...
                            SMDS_EdgePosition *epos =
                                static_cast<SMDS_EdgePosition *>(node->GetPosition());
            ...
            {
                if (isOnEdge) edge._pos.back().SetCoord(u, 0, 0);
                else
                    edge._pos.back().SetCoord(uv.X(), uv.Y(), 0);

                if (edgeOnSameNode) edgeOnSameNode->_pos.back() = edge._pos.back();
            }
    ...
    for (; exp.More(); exp.Next()) {
        ...
        while (fIt->more()) {
            ...
            for (int iN = 0; iN < nbNodes; ++iN) {
                ...
                if (nnVec[i]->size() < 2) degenEdgeInd.insert(iN);
                else
                    nbZ = nnVec[i]->size();

                if (helper.HasDegeneratedEdges()) nnSet.insert(nnVec[i]);
            }
            if (nbZ == 0) continue;
            if (0 < nnSet.size() && nnSet.size() < 3) continue;
            ...
            switch (nbNodes) {
                        ...
                        case 0: // PENTA
                        {
                            for (int iZ = 1; iZ < nbZ; ++iZ)
                                helper.AddVolume((*nnVec[0])[iZ - 1], (*nnVec[1])[iZ - 1],
                                                 (*nnVec[2])[iZ - 1], (*nnVec[0])[iZ],
                                                 (*nnVec[1])[iZ], (*nnVec[2])[iZ]);
                            break;
                        }
                        case 1: // PYRAM
                        {
                ...
                case 4:
                    switch (degenEdgeInd.size()) {
                        ...
                        case 2: // PENTA?
                        {
                            ...
                            for (int iZ = 1; iZ < nbZ; ++iZ) {
                                const SMDS_MeshElement *vol = helper.AddVolume(
                                    (*nnVec[i3])[0], (*nnVec[i0])[iZ], (*nnVec[i0])[iZ - 1],
                                    (*nnVec[i2])[0], (*nnVec[i1])[iZ], (*nnVec[i1])[iZ - 1]);
                                if (!ok && vol) degenVols.push_back(vol);
                            }
                            break;
                        }
                        case 3: // degen HEX
                        {
                            ...
                            for (int iZ = 1; iZ < nbZ; ++iZ) {
                                const SMDS_MeshElement *vol = helper.AddVolume(
                                    nnVec[0]->size() > 1 ? (*nnVec[0])[iZ - 1] : (*nnVec[0])[0],
                ...
                default: return error("Not supported type of element", data._index);
    ...
    if (!degenVols.empty()) {
        ...
        if (!err || err->IsOK()) {
            err.reset(new SMESH_ComputeError(COMPERR_WARNING, "Degenerated volumes created"));
            err->myBadElements.insert(err->myBadElements.end(), degenVols.begin(), degenVols.end());
        }
    }
...
bool _ViscousBuilder::shrink()
{
            ...
            if (s2s->second.ShapeType() == TopAbs_FACE) {
                ...
                if (FFMap.Add((*s2s).second))
                    ...
                    if (SMESHDS_SubMesh *smDS = getMeshDS()->MeshElements(s2s->second)) {
                        SMESH_ProxyMesh::SubMesh *proxySub = data._proxyMesh->getFaceSubM(
                            TopoDS::Face(s2s->second), /*create=*/true);
                        SMDS_ElemIteratorPtr fIt = smDS->GetElements();
                        while (fIt->more()) proxySub->AddElement(fIt->next());
    ...
    for (; f2sd != f2sdMap.end(); ++f2sd) {
        ...
        vector<const SMDS_MeshNode *> smoothNodes;
        {
            ...
            while (nIt->more()) {
                const SMDS_MeshNode *n = nIt->next();
                if (n->NbInverseElements(SMDSAbs_Face) > 0) smoothNodes.push_back(n);
            }
        }
        ...
        if (!smoothNodes.empty()) {
            ...
            if (!simplices[0].IsForward(uv, smoothNodes[0], F, helper, refSign)) refSign = -1;
        }
        ...
        lEdges.clear();
        {
            ...
            while (subIt->more()) {
                ...
                if (data._noShrinkShapes.count(subID)) continue;
        ...
        SMDS_ElemIteratorPtr fIt = smDS->GetElements();
        while (fIt->more()) {
            if (const SMDS_MeshElement *f = fIt->next()) dumpChangeNodes(f);
        }
        dumpFunctionEnd();
        ...
        vector<_SmoothNode> nodesToSmooth(smoothNodes.size());
        {
            ...
            for (size_t i = 0; i < smoothNodes.size(); ++i) {
                ...
                _Simplex::GetSimplices(n, nodesToSmooth[i]._simplices, ignoreShapes, 0,
                                       sortSimplices);
        ...
        set<_Shrinker1D *> eShri1D;
        {
            for (size_t iS = 0; iS < subEOS.size(); ++iS) {
                ...
                for (size_t i = 0; i < eos._edges.size(); ++i) {
                    _LayerEdge &edge = *eos._edges[i];
                    _Simplex::GetSimplices(/*tgtNode=*/edge._nodes.back(), edge._simplices,
                                           ignoreShapes);
                }
            }
        }
        ...
        if (isConcaveFace) {
            ...
            if (hasTria != hasQuad) { toFixTria = hasTria; }
            else {
                set<int> nbNodesSet;
                SMDS_ElemIteratorPtr fIt = smDS->GetElements();
        ...
        while (shrinked) {
            ...
            // -----------------------------------------------
            dumpFunction(SMESH_Comment("moveBoundaryOnF")
                         << f2sd->first << "_st" << shriStep); // debug
            shrinked = false;
            for (size_t iS = 0; iS < subEOS.size(); ++iS) {
                _EdgesOnShape &eos = *subEOS[iS];
                for (size_t i = 0; i < eos._edges.size(); ++i) {
            ...
            for (; shr != eShri1D.end(); ++shr) (*shr)->Compute(/*set3D=*/false, helper);
            ...
            while ((nbNoImpSteps < 5 && badNb > 0) && moved) {
                dumpFunction(SMESH_Comment("shrinkFace")
                             << f2sd->first << "_st" << ++smooStep); // debug

                int oldBadNb = badNb;
                badNb = 0;
                moved = false;
                // '% 5' minimizes NB FUNCTIONS on viscous_layers_00/B2 case
                _SmoothNode::SmoothType smooTy =
                    (smooStep % 5) ? smoothType : _SmoothNode::LAPLACIAN;
                for (size_t i = 0; i < nodesToSmooth.size(); ++i) {
                    moved |= nodesToSmooth[i].Smooth(badNb, surface, helper, refSign, smooTy,
                                                     /*set3D=*/isConcaveFace);
                }
                if (badNb < oldBadNb) nbNoImpSteps = 0;
                else
                    nbNoImpSteps++;
            ...
            if (shriStep > 200)
                return error(SMESH_Comment("Infinite loop at shrinking 2D mesh on face ")
                             << f2sd->first);
            ...
            if (toFixTria) {
                ...
                for (size_t i = 0; i < nodesToSmooth.size() && !usedNodes.empty(); ++i) {
                    ...
                    if (n != usedNodes.end()) {
                        _Simplex::GetSimplices(
                            nodesToSmooth[i]._node, nodesToSmooth[i]._simplices, ignoreShapes, NULL,
                ...
                for (size_t i = 0; i < lEdges.size() && !usedNodes.empty(); ++i) {
                    ...
                    if (n != usedNodes.end()) {
                        _Simplex::GetSimplices(lEdges[i]->_nodes.back(), lEdges[i]->_simplices,
                                               ignoreShapes);
                        usedNodes.erase(n);
                    }
                }
        ...
        if (!isStructuredFixed) {
            ...
            for (int st = 3; st; --st) {
                ...
                dumpFunction(SMESH_Comment("shrinkFace")
                             << f2sd->first << "_st" << ++smooStep); // debug
                for (size_t i = 0; i < nodesToSmooth.size(); ++i) {
                    nodesToSmooth[i].Smooth(badNb, surface, helper, refSign, smoothType,
                                            /*set3D=*/st == 1);
                }
                dumpFunctionEnd();
            }
    ...
    for (; e2shr != e2shrMap.end(); ++e2shr) e2shr->second.SwapSrcTgtNodes(getMeshDS());
...
bool _ViscousBuilder::prepareEdgeToShrink(_LayerEdge &edge, _EdgesOnShape &eos,
                                          SMESH_MesherHelper &helper,
                                          const SMESHDS_SubMesh *faceSubMesh)
{
    const SMDS_MeshNode *srcNode = edge._nodes[0];
...
void _ViscousBuilder::fixBadFaces(const TopoDS_Face &F, SMESH_MesherHelper &helper, const bool is2D,
                                  const int step, set<const SMDS_MeshNode *> *involvedNodes)
{
    ...
    if (step == 1) {
        ...
        while (fIt->more()) {
            const SMDS_MeshElement *f = fIt->next();
            if (f->NbCornerNodes() == 3) dumpChangeNodes(f);
        }
        dumpFunctionEnd();
    }
    if (badTrias.empty()) return;
    ...
    for (size_t iTia = 0; iTia < badTrias.size(); ++iTia) {
        ...
        if (!involvedFaces.insert(badTrias[iTia]).second) continue;
        for (int iP = 0; iP < 3; ++iP) points(iP + 1) = xyz(badTrias[iTia]->GetNode(iP));
        ...
        for (int iSide = 0; iSide < 3; ++iSide) {
            ...
            if (aspRatio[iSide] > aspectInit + badAspects[iTia]) continue;
            ...
            if (norm1 * norm2 < 0. && norm1.Angle(norm2) > 70. / 180. * M_PI) continue;

            if (bestCouple < 0 || aspRatio[bestCouple] > aspRatio[iSide]) bestCouple = iSide;
        }
...
bool _LayerEdge::SetNewLength2d(Handle(Geom_Surface) & surface, const TopoDS_Face &F,
                                _EdgesOnShape &eos, SMESH_MesherHelper &helper)
{
    if (_pos.empty()) return false; // already at the target position
    ...
    if (eos.SWOLType() == TopAbs_FACE) {
        ...
        for (size_t i = 0; i < _simplices.size(); ++i) {
            ...
            if (step > 0) stepSize = Min(step, stepSize);
        }
        gp_Pnt2d newUV;
        if (uvLen <= stepSize) {
            newUV = tgtUV;
...
bool _SmoothNode::Smooth(int &badNb, Handle(Geom_Surface) & surface, SMESH_MesherHelper &helper,
                         const double refSign, SmoothType how, bool set3D)
{
    ...
    if (how == TFI && _simplices.size() == 4) {
        ...
        newPos = helper.calcTFI(0.5, 0.5, corners[0], corners[1], corners[2], corners[3], uv[1],
                                uv[2], uv[3], uv[0]);
    }
    else if (how == ANGULAR) {
        ...
        // Laplacian smooth
        for (size_t i = 0; i < _simplices.size(); ++i) newPos += uv[i];
        newPos /= _simplices.size();
    }
...
gp_XY _SmoothNode::computeAngularPos(vector<gp_XY> &uv, const gp_XY &uvToFix, const double refSign)
{
    ...
    for (size_t i = 1; i < edgeDir.size(); ++i) {
        ...
        if (edgeSize[i - 1] < numeric_limits<double>::min()) edgeDir[i - 1].SetX(100);
        else
    ...
    for (size_t i = 1; i < edgeDir.size(); ++i) {
        ...
        if (bisec * dirToN < 0) distToN = -distToN;
...
_SolidData::~_SolidData()
{
    ...
    for (; n2e != _n2eMap.end(); ++n2e) {
        ...
        if (e && e->_2neibors) delete e->_2neibors;
        delete e;
        e = NULL;
    }
    _n2eMap.clear();
...
void _Shrinker1D::AddEdge(const _LayerEdge *e, _EdgesOnShape &eos, SMESH_MesherHelper &helper)
{
    // init
    if (_nodes.empty()) {
        _edges[0] = _edges[1] = 0;
    ...
    // check _LayerEdge
    if (e == _edges[0] || e == _edges[1]) return;
    if (eos.SWOLType() != TopAbs_EDGE)
    ...
    if (_nodes.empty()) {
        ...
        if (!eSubMesh || eSubMesh->NbNodes() < 1) return;
        TopLoc_Location loc;
        ...
        while (nIt->more()) {
            ...
            if (node->NbInverseElements(SMDSAbs_Edge) == 0 || node == tgtNode0 || node == tgtNode1)
        ...
        for (size_t i = 0; i < _nodes.size(); ++i)
            if (!_nodes[i] || _nodes[i] == tgtNode0 || _nodes[i] == tgtNode1)
                _nodes[i] = 0, nbFound++;
        if (nbFound == _nodes.size()) _nodes.clear();
    }
}
...
{
    if (_done || _nodes.empty()) return;
    const _LayerEdge *e = _edges[0];
    if (!e) e = _edges[1];
    if (!e) return;

    _done = ((!_edges[0] || _edges[0]->_pos.empty()) && (!_edges[1] || _edges[1]->_pos.empty()));
    ...
    if (set3D || _done) {
        ...
        if (_edges[0]) f = helper.GetNodeU(_geomEdge, _edges[0]->_nodes.back(), _nodes[0]);
        if (_edges[1]) l = helper.GetNodeU(_geomEdge, _edges[1]->_nodes.back(), _nodes.back());
...
void _Shrinker1D::SwapSrcTgtNodes(SMESHDS_Mesh *mesh)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        while (eIt->more()) {
            const SMDS_MeshElement *e = eIt->next();
            if (!eSubMesh->Contains(e)) continue;
...
bool _ViscousBuilder::addBoundaryElements()
{
    ...
    for (size_t i = 0; i < _sdVec.size(); ++i) {
        ...
        for (int iE = 1; iE <= geomEdges.Extent(); ++iE) {
            ...
            if (data._noShrinkShapes.count(getMeshDS()->ShapeToIndex(E))) continue;
            ...
            map<double, const SMDS_MeshNode *>::iterator u2n = u2nodes.begin();
            {
                ...
                u2n++;
                if ((n2e = n2eMap.find(u2n->second)) == n2eMap.end()) continue; // no layers on E
                ...
                if (nbSharedPyram > 1) continue; // not free border of the pyramid
            ...
            for (++u2n; u2n != u2nodes.end(); ++u2n) ledges.push_back(n2eMap[u2n->second]);
            ...
            if ((isOnFace = (e2f != data._shrinkShape2Shape.end()))) {
                ...
                if (helper.IsReversedSubMesh(TopoDS::Face(F))) reverse = !reverse;
            }
            else {
            ...
            if (isOnFace) sm = getMeshDS()->MeshElements(F);
            else
                sm = data._proxyMesh->getFaceSubM(TopoDS::Face(F), /*create=*/true);
            if (!sm) return error("error in addBoundaryElements()", data._index);
                ...
                if (nn1.size() == nn2.size()) {
                    if (isOnFace)
                        for (size_t z = 1; z < nn1.size(); ++z)
                            sm->AddElement(
                                getMeshDS()->AddFace(nn1[z - 1], nn2[z - 1], nn2[z], nn1[z]));
                    else
                        for (size_t z = 1; z < nn1.size(); ++z)
                            sm->AddElement(
                                new SMDS_FaceOfNodes(nn1[z - 1], nn2[z - 1], nn2[z], nn1[z]));
                }
                else if (nn1.size() == 1) {
                    if (isOnFace)
            ...
            for (int isFirst = 0; isFirst < 2; ++isFirst) {
                ...
                if (eos && eos->SWOLType() == TopAbs_EDGE) {
                    ...
                    helper.SetElementsOnShape(true);
                    for (size_t z = 1; z < nn.size(); ++z) helper.AddEdge(nn[z - 1], nn[z]);
                }
            }

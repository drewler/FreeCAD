...
//purpose  :
//=======================================================================
StdMeshers_Penta_3D::StdMeshers_Penta_3D() : myErrorStatus(SMESH_ComputeError::New())
{
    myTol3D = 0.1;
...
//purpose  :
//=======================================================================
bool StdMeshers_Penta_3D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    //
    CheckData();
    if (!myErrorStatus->IsOK()) { return bOK; }
...
void StdMeshers_Penta_3D::MakeNodes()
{
    ...
    double aX, aY, aZ;
    SMESH_Block::TShapeID aSID,
        aSIDs[aNbSIDs] = {SMESH_Block::ID_V000, SMESH_Block::ID_V100, SMESH_Block::ID_V110,
                          SMESH_Block::ID_V010, SMESH_Block::ID_Ex00, SMESH_Block::ID_E1y0,
                          SMESH_Block::ID_Ex10, SMESH_Block::ID_E0y0, SMESH_Block::ID_Fxy0};
    //
    ...
    SMESH_Block::TShapeID wallFaceID[NB_WALL_FACES] = {SMESH_Block::ID_Fx0z, SMESH_Block::ID_Fx1z,
                                                       SMESH_Block::ID_F0yz, SMESH_Block::ID_F1yz};
    SMESH_Block::TShapeID baseEdgeID[NB_WALL_FACES] = {SMESH_Block::ID_Ex00, SMESH_Block::ID_Ex10,
                                                       SMESH_Block::ID_E0y0, SMESH_Block::ID_E1y0};
    for (i = 0; i < NB_WALL_FACES; ++i) {
        int fIndex = SMESH_Block::ShapeIndex(wallFaceID[i]);
        bool ok = LoadIJNodes(myWallNodesMaps[fIndex], TopoDS::Face(myBlock.Shape(wallFaceID[i])),
                              TopoDS::Edge(myBlock.Shape(baseEdgeID[i])), pMesh->GetMeshDS());
        if (!ok) {
    ...
    for (j = 0; j < myJSize; ++j) { // loop on all nodes of the base face (ID_Fxy0)
        ...
        if (createNode) {
            for (k = 0; k < NB_WALL_FACES; ++k) {
                ratio[k] =
                    SetHorizEdgeXYZ(aBNXYZ, wallFaceID[k], nColumns[k * 2], nColumns[k * 2 + 1]);
            }
        }
        //
...
void StdMeshers_Penta_3D::FindNodeOnShape(const TopoDS_Shape &aS, const gp_XYZ &aParams,
                                          const int z, StdMeshers_TNode &aTN)
{
    ...
    //
    if (aS.ShapeType() == TopAbs_FACE || aS.ShapeType() == TopAbs_EDGE) {
        // find a face ID to which aTN belongs to
        int faceID;
        if (aS.ShapeType() == TopAbs_FACE) faceID = myBlock.ShapeID(aS);
        else { // edge maybe vertical or top horizontal
            gp_XYZ aCoord = aParams;
            if (aCoord.Z() == 1.) aCoord.SetZ(0.5); // move from top down
            else
...
double StdMeshers_Penta_3D::SetHorizEdgeXYZ(const gp_XYZ &aBaseNodeParams, const int aFaceID,
                                            ...
                                            vector<const SMDS_MeshNode *> *&aCol2)
{
    ...
    if (!isForward) param = 1. - param;
    //
    // look for columns around param
    StdMeshers_IJNodeMap &ijNodes = myWallNodesMaps[SMESH_Block::ShapeIndex(aFaceID)];
    StdMeshers_IJNodeMap::iterator par_nVec_1 = ijNodes.begin();
    while (par_nVec_1->first < param) par_nVec_1++;
    ...
    // base edge
    myBlock.Block().EdgePoint(edgeVec[BASE], aBaseNodeParams, myShapeXYZ[edgeVec[BASE]]);
    return r;
}
...
void StdMeshers_Penta_3D::MakeVolumeMesh()
{
    ...
    //
    const TopoDS_Face &aFxy0 = TopoDS::Face(myBlock.Shape(SMESH_Block::ID_Fxy0));
    ...
    while (itf->more()) {
        ...
        if (myCreateQuadratic) nbFaceNodes = nbFaceNodes / 2;
        if (aN.size() < nbFaceNodes * 2) aN.resize(nbFaceNodes * 2);
        //
        ...
        for (i = 0; i < ik; ++i) {
            ...
            if (i == 0) {
                ...
                switch (nbFaceNodes) {
                    case 3: {
                        SMDS_VolumeOfNodes tmpVol(aN[0], aN[1], aN[2], aN[3], aN[4], aN[5]);
                        vTool.Set(&tmpVol);
                        break;
                    }
                    case 4: {
                        SMDS_VolumeOfNodes tmpVol(aN[0], aN[1], aN[2], aN[3], aN[4], aN[5], aN[6],
                                                  aN[7]);
                        vTool.Set(&tmpVol);
                        break;
                    }
            ...
            switch (nbFaceNodes) {
                ...
                case 4:
                    if (forward) {
                        ...
                        //                       aN[4], aN[5], aN[6], aN[7]);
                        aV = myTool->AddVolume(aN[0], aN[1], aN[2], aN[3], aN[4], aN[5], aN[6],
                                               aN[7]);
                    }
                    else {
                        //aV = meshDS->AddVolume(aN[0], aN[3], aN[2], aN[1],
                        //                       aN[4], aN[7], aN[6], aN[5]);
                        aV = myTool->AddVolume(aN[0], aN[3], aN[2], aN[1], aN[4], aN[7], aN[6],
                                               aN[5]);
                    }
                    break;
                default: continue;
...
void StdMeshers_Penta_3D::MakeMeshOnFxy1()
{
    ...
    //
    const TopoDS_Face &aFxy0 = TopoDS::Face(myBlock.Shape(SMESH_Block::ID_Fxy0));
    const TopoDS_Face &aFxy1 = TopoDS::Face(myBlock.Shape(SMESH_Block::ID_Fxy1));
    //
    SMESH_Mesh *pMesh = GetMesh();
    SMESHDS_Mesh *meshDS = pMesh->GetMeshDS();
    //
    ...
    while (itf->more()) {
        ...
        if (!(aElementType == SMDSAbs_Face)) { continue; }
        aNbNodes = pE0->NbNodes();
        if (myCreateQuadratic) aNbNodes = aNbNodes / 2;
        if (aNodes1.size() < aNbNodes) aNodes1.resize(aNbNodes);
        //
        k = aNbNodes - 1; // reverse a face
        aItNodes = pE0->nodesIterator();
        while (aItNodes->more()) {
            const SMDS_MeshNode *pNode = static_cast<const SMDS_MeshNode *>(aItNodes->next());
            if (myTool->IsMedium(pNode)) continue;
            aID0 = pNode->GetID();
        ...
        switch (aNbNodes) {
            case 3: face = myTool->AddFace(aNodes1[0], aNodes1[1], aNodes1[2]); break;
            case 4: face = myTool->AddFace(aNodes1[0], aNodes1[1], aNodes1[2], aNodes1[3]); break;
            default: continue;
        }
...
void StdMeshers_Penta_3D::ClearMeshOnFxy1()
{
    ...
    if (aSubMesh) aSubMesh->ComputeStateEngine(SMESH_subMesh::CLEAN);
}
...
//purpose  :
//=======================================================================
void StdMeshers_Penta_3D::CreateNode(const bool bIsUpperLayer, const gp_XYZ &aParams,
                                     StdMeshers_TNode &aTN)
{
    double aX, aY, aZ;
    //
        ...
        if (iNbF == 6) {
            ...
            if (iNbE == 12) {
                ...
                if ((nb_e01 == nb_e03) && (nb_e03 == nb_e05) && (nb_e05 == nb_e07)) {
                    if (has_only_quad_f1 && has_only_quad_f2 && has_only_quad_f3
                        && has_only_quad_f4) {
                        if ((nb_e09 == nb_e10) && (nb_e08 == nb_e06) && (nb_e11 == nb_e12)
                            && (nb_e04 == nb_e02)) {
                            if (nb_f5 == nb_f6) {
                                nb_ok += 1;
                                aFTr = aM(5);
                ...
                if ((nb_e02 == nb_e04) && (nb_e04 == nb_e06) && (nb_e06 == nb_e08)) {
                    if (has_only_quad_f1 && has_only_quad_f2 && has_only_quad_f5
                        && has_only_quad_f6) {
                        if ((nb_e01 == nb_e03) && (nb_e10 == nb_e12) && (nb_e05 == nb_e07)
                            && (nb_e09 == nb_e11)) {
                            if (nb_f3 == nb_f4) {
                                nb_ok += 1;
                                aFTr = aM(3);
                ...
                if ((nb_e09 == nb_e10) && (nb_e10 == nb_e11) && (nb_e11 == nb_e12)) {
                    if (has_only_quad_f3 && has_only_quad_f4 && has_only_quad_f5
                        && has_only_quad_f6) {
                        if ((nb_e01 == nb_e05) && (nb_e02 == nb_e06) && (nb_e03 == nb_e07)
                            && (nb_e04 == nb_e08)) {
                            if (nb_f1 == nb_f2) {
                                nb_ok += 1;
                                aFTr = aM(1);
...
void StdMeshers_Penta_3D::CheckData()
{
    ...
    TopAbs_ShapeEnum aSTEx[] = {TopAbs_VERTEX, TopAbs_EDGE, TopAbs_FACE};
...
bool StdMeshers_Penta_3D::LoadIJNodes(StdMeshers_IJNodeMap &theIJNodes, const TopoDS_Face &theFace,
                                      const TopoDS_Edge &theBaseEdge, SMESHDS_Mesh *theMesh)
{
    ...
    for (; exp.More(); exp.Next()) {
        ...
        if (theBaseEdge.IsSame(e)) continue;
        TopoDS_Vertex vCommon;
        if (!TopExp::CommonVertex(theBaseEdge, e, vCommon)) eTop = e;
        else if (vCommon.IsSame(vfb)) {
            e1 = e;
            vft = TopExp::LastVertex(e1, CumOri);
            rev1 = vfb.IsSame(vft);
            if (rev1) vft = TopExp::FirstVertex(e1, CumOri);
        }
        else
            e2 = e;
    }
    ...
    if (!smFace || !smb || !smt || !sm1 || !sm2 || !smVfb || !smVlb || !smVft) {
        MESSAGE("NULL submesh " << smFace << " " << smb << " " << smt << " " << sm1 << " " << sm2
                                << " " << smVfb << " " << smVlb << " " << smVft);
        return false;
    }
    ...
    if (sm1->NbNodes() * smb->NbNodes() != smFace->NbNodes()) {
        ...
        if (myCreateQuadratic) {
            ...
            if (nf != smFace->NbNodes()) {
                MESSAGE("Wrong nb face nodes: " << sm1->NbNodes() << " " << smb->NbNodes() << " "
                                                << smFace->NbNodes());
                return false;
            }
        }
    ...
    while (nIt->more()) {
        node = nIt->next();
        if (myTool->IsMedium(node)) continue;
        const SMDS_EdgePosition *pos = dynamic_cast<const SMDS_EdgePosition *>(node->GetPosition());
        if (!pos) { return false; }
    ...
    while (eIt->more()) {
        ...
        if (e->GetType() == SMDSAbs_Face) allFaces.insert(e);
    }
        ...
        do {
            ...
            if (face) {
                ...
                while (!found && eIt->more()) {
                    ...
                    if (!found && node != n1 && node != n2) n3 = node;
                }
                if (found) {
                    if (++row > vsize - 1) {
...
bool StdMeshers_SMESHBlock::IsForwadEdge(const int theEdgeID)
{
    ...
    if (!myTBlock.IsEdgeID(theEdgeID)) return false;
...
//purpose  :
//=======================================================================
int StdMeshers_SMESHBlock::ErrorStatus() const { return myErrorStatus; }
...
SMESH_ComputeErrorPtr StdMeshers_SMESHBlock::GetError() const
{
    ...
    if (!text.empty()) err->myName = myErrorStatus;
    return err;
}
...
//purpose  :
//=======================================================================
void StdMeshers_SMESHBlock::Load(const TopoDS_Shell &theShell, const TopoDS_Vertex &theV000,
...
//purpose  :
//=======================================================================
void StdMeshers_SMESHBlock::ComputeParameters(const gp_Pnt &thePnt, gp_XYZ &theXYZ)
{
...
//purpose  :
//=======================================================================
void StdMeshers_SMESHBlock::ComputeParameters(const gp_Pnt &thePnt, const TopoDS_Shape &theShape,
                                              gp_XYZ &theXYZ)
{
    myErrorStatus = 0;
    //
...
void StdMeshers_SMESHBlock::ComputeParameters(const double &theU, const TopoDS_Shape &theShape,
                                              gp_XYZ &theXYZ)
{
    myErrorStatus = 0;
    //
    ...
    aID = ShapeID(theShape);
    if (myErrorStatus) { return; }
    if (SMESH_Block::IsEdgeID(aID)) bOk = myTBlock.EdgeParameters(aID, theU, theXYZ);
    if (!bOk) {
        myErrorStatus = 4; // problems with computation Parameters
        return;
    }
...
//purpose  :
//=======================================================================
void StdMeshers_SMESHBlock::Point(const gp_XYZ &theParams, const TopoDS_Shape &theShape,
                                  gp_Pnt &aP3D)
{
    myErrorStatus = 0;
    //
    ...
    aP3D.SetXYZ(aXYZ);
    //
    if (theShape.IsNull()) { bOk = myTBlock.ShellPoint(theParams, aXYZ); }
    //
    else {
        aID = ShapeID(theShape);
        if (myErrorStatus) { return; }
        //
        if (SMESH_Block::IsVertexID(aID)) { bOk = myTBlock.VertexPoint(aID, aXYZ); }
...
//purpose  :
//=======================================================================
bool StdMeshers_Penta_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                   MapShapeNbElems &aResMap)
{
    ...
    for (i = SMDSEntity_Node; i < SMDSEntity_Last; i++) (*anIt).second[i] = aVec[i];

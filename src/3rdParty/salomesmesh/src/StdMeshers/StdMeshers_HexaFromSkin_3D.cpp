...
bool isQuadrangle(const SMDS_MeshElement *e) { return (e && e->NbCornerNodes() == 4); }
...
public:
    ...
    _OrientedIndexer(const _Indexer &indexer, const int oriFlags)
        : _Indexer(indexer._xSize, indexer._ySize), _xSize(indexer._xSize), _ySize(indexer._ySize),
...
#define _grid_access_(pobj, i) pobj->_grid[i]
    ...
    gp_XYZ getGC() const
    {
        gp_XYZ xyz = getXYZ(0, 0) + getXYZ(_index._xSize - 1, 0) + getXYZ(0, _index._ySize - 1)
...
struct _OrientedBlockSide {
    ...
    gp_XYZ xyz(int x, int y) const { return SMESH_TNodeXYZ(_grid_access_(_side, _index(x, y))); }
...
struct _Block {
    ...
    bool hasSide(const _OrientedBlockSide &s) const
    {
        if (s)
            for (int i = 0; i < 6; ++i)
                if (_side[i] && _side[i]._side == s._side) return true;
        return false;
    }
    int nbSides() const
    {
...
private:
    ...
    bool fillRowsUntilCorner(const SMDS_MeshElement *quad, const SMDS_MeshNode *n1,
                             const SMDS_MeshNode *n2, vector<const SMDS_MeshNode *> &verRow1,
                             vector<const SMDS_MeshNode *> &verRow2, bool alongN1N2);
    _OrientedBlockSide findBlockSide(EBoxSides startBlockSide, EQuadEdge sharedSideEdge1,
                                     EQuadEdge sharedSideEdge2, bool withGeometricAnalysis,
    ...
    int error(const SMESH_Comment &reason)
    {
        _error = reason;
        return 0;
    }
...
int _Skin::findBlocks(SMESH_Mesh &mesh)
{
    ...
    while (nIt->more()) {
        nCorner = nIt->next();
        if (isCornerNode(nCorner)) break;
        else
            nCorner = 0;
    }
    if (!nCorner) return BAD_MESH_ERR;
    ...
    while (corner != corners.end()) {
        ...
        while (faceIt->more()) {
            ...
            if (!cornerFaces.insert(face).second) continue; // already loaded block side

            if (!isQuadrangle(face)) return error("Non-quadrangle elements in the input mesh");
        ...
        if (corner == corners.end() && nbFacesOnSides < mesh.NbQuadrangles()) {
            while (nIt->more()) {
                nCorner = nIt->next();
                if (isCornerNode(nCorner)) corner = corners.insert(corner, nCorner);
            }
            nbFacesOnSides = mesh.NbQuadrangles();
        }
    }

    if (_allSides.empty()) return BAD_MESH_ERR;
    if (_allSides.back()._grid.empty()) _allSides.pop_back();
    ...
    while (nbBlockSides >= 6) {
        ...
        while (i_side != sortedSides.end() && i_side->second->isBound()) ++i_side;

        // start searching for block sides from the got side
        bool ok = true;
        if (_blocks.empty() || _blocks.back()._side[B_FRONT]) _blocks.resize(_blocks.size() + 1);
        ...
        for (int advAnalys = 0; advAnalys < 2; ++advAnalys) {
            ...
            for (int i = 0; (ok || !advAnalys) && i < NB_QUAD_SIDES; ++i)
                if (!block._side[i])
                    ok = block.setSide(i,
                                       findBlockSide(B_FRONT, edgeOfFront[i], edgeOfAdj[i],
                                                     advAnalys, sidesAround));
            // try to find a BACK side by a TOP one
            if (ok || !advAnalys)
                if (!block._side[B_BACK] && block._side[B_TOP])
                    ok = block.setSide(B_BACK,
                                       findBlockSide(B_TOP, Q_TOP, Q_TOP, advAnalys, sidesAround));
        ...
        for (int i = 0; i < NB_BLOCK_SIDES; ++i) {
            ...
            if (block._side[i]) {
                ...
                _DUMP_("\t corners " << block._side[i].cornerNode(0, 0)->GetID() << ", "
    ...
    if (nbBlocks == 0 && _error.empty()) return BAD_MESH_ERR;
                     ...
                     const SMDS_MeshNode *nCorner)
{
    ...
    while (++y < nbY) {
        ...
        firstQuad =
            SMESH_MeshAlgos::FindFaceInSet(n1down, n2down, emptySet, avoidSet, &i1down, &i2down);
        if (!isQuadrangle(firstQuad)) return BAD_MESH_ERR;
        ...
        while (++x < nbX) {
            const SMDS_MeshElement *quad =
                SMESH_MeshAlgos::FindFaceInSet(n2up, n2down, emptySet, avoidSet, &i2up, &i2down);
            if (!isQuadrangle(quad)) return BAD_MESH_ERR;
    ...
    // check side validity
    bool ok = side.getCornerFace(side.getCornerNode(0, 0))
                          ...
                          map<const SMDS_MeshNode *, list<_BlockSide *>> &corner2Sides)
{
    ...
    map<const SMDS_MeshNode *, list<_BlockSide *>>::iterator c2sides = corner2Sides.find(n1);
    ...
    while (n != n2) {
        ...
        for (int y = 0; y < 2 && !n; ++y)
            for (int x = 0; x < 2; ++x) {
                ...
                if (n == n1 || c2sides == corner2Sides.end()) n = 0;
                else
                    break;
            }
        if (!n) return false;
...
_OrientedBlockSide _Skin::findBlockSide(EBoxSides startBlockSide, EQuadEdge sharedSideEdge1,
                                        EQuadEdge sharedSideEdge2, bool withGeometricAnalysis,
                                        set<_BlockSide *> &sidesAround)
{
    _Block &block = _blocks.back();
    ...
    for (int i = 0; i < NB_BLOCK_SIDES; ++i)
        if (block._side[i]) sidesOnEdge.erase(block._side[i]._side);

    int nbSidesOnEdge = sidesOnEdge.size();
    _DUMP_("nbSidesOnEdge " << nbSidesOnEdge << " " << n1->GetID() << "-" << n2->GetID());
    if (nbSidesOnEdge == 0) return 0;

    _BlockSide *foundSide = 0;
    if (nbSidesOnEdge == 1) { foundSide = *sidesOnEdge.begin(); }
    else {
        ...
        if (nbLoadedSides > 1) {
            ...
            for (; !foundSide && sideIt != sidesOnEdge.end(); ++sideIt) {
                ...
                int nbCommonCorners = block._corners.count(sideI->getCornerNode(0, 0))
                ...
                if (nbCommonCorners > 2) foundSide = sideI;
            }
        }
        ...
        if (!foundSide) {
            ...
            if (nbLoadedSides == 1) {
                ...
                for (; nIt != block._corners.end(); ++nIt) corner2Sides.erase(*nIt);
                ...
                _DUMP_("  Select adjacent for " << side1._side << " - side dir (" << side1Dir.X()
                                                << ", " << side1Dir.Y() << ", " << side1Dir.Z()
                                                << ")");
                ...
                for (sideIt = sidesOnEdge.begin(); sideIt != sidesOnEdge.end(); ++sideIt) {
                    ...
                    _DUMP_("  " << sideI << " - side dir (" << sideIDir.X() << ", " << sideIDir.Y()
                                << ", " << sideIDir.Z() << ")"
                                << " angle " << angle);
                }
                ...
                for (int i = 0; i < NB_BLOCK_SIDES; ++i)
                    if (block._side[i]) gc += block._side[i]._side->getGC();
                gc /= nbLoadedSides;
                ...
                foundSide =
                    gcAngle < 0 ? angleOfSide.rbegin()->second : angleOfSide.begin()->second;
            }
        }
    ...
    for (int ori = 0; ori < _OrientedIndexer::MAX_ORI + 1; ++ori) {
        ...
        if (n1 == n12 && n2 == n22) return orientedSide;
    }
    error(SMESH_Comment("Failed to orient a block side found by edge ")
          << sharedSideEdge1 << " of side " << startBlockSide << " of block " << _blocks.size());
    return 0;
}
...
bool _Skin::fillRowsUntilCorner(const SMDS_MeshElement *quad, const SMDS_MeshNode *n1,
                                const SMDS_MeshNode *n2, vector<const SMDS_MeshNode *> &row1,
                                vector<const SMDS_MeshNode *> &row2, const bool alongN1N2)
{
    ...
    if (isCornerNode(row1[1])) return true;
...
bool StdMeshers_HexaFromSkin_3D::Compute(SMESH_Mesh &aMesh, SMESH_MesherHelper *aHelper)
{
    ...
    if (nbBlocks == 0) return error(skin.error());
    ...
    for (int i = 0; i < nbBlocks; ++i) {
        ...
        for (int xMax = 0; xMax < 2; ++xMax)
            for (int yMax = 0; yMax < 2; ++yMax)
                for (int zMax = 0; zMax < 2; ++zMax) {
                    ...
                    SMDS_VolumeOfNodes probeVolume(n000, n010, n110, n100, n001, n011, n111, n101);
        ...
        for (x = 0; x < xSize - 1; ++x) {
            for (y = 0; y < ySize - 1; ++y) {
                ...
                if (isForw)
                    for (z = 0; z < zSize - 1; ++z)
                        aHelper->AddVolume(col00[z], col01[z], col11[z], col10[z], col00[z + 1],
                                           col01[z + 1], col11[z + 1], col10[z + 1]);
                else
                    for (z = 0; z < zSize - 1; ++z)
                        aHelper->AddVolume(col00[z], col10[z], col11[z], col01[z], col00[z + 1],
                                           col10[z + 1], col11[z + 1], col01[z + 1]);
            }
        }
    } // loop on blocks
...
bool StdMeshers_HexaFromSkin_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                          MapShapeNbElems &aResMap)
{
    ...
    if (entity >= nbByType.size()) nbByType.resize(SMDSEntity_Last, 0);
    ...
    for (int i = 0; i < nbBlocks; ++i) {
        ...
        if (secondOrder)
            nbNodes += (nbX - 2) * (nbY - 2) * (nbZ - 1) + (nbX - 2) * (nbY - 1) * (nbZ - 2)
                + (nbX - 1) * (nbY - 2) * (nbZ - 2);

...
{
    return (F1->GetNode(1) == F2->GetNode(2) && F1->GetNode(2) == F2->GetNode(1))
...
bool TooCloseAdjacent(const SMDS_MeshElement *PrmI, const SMDS_MeshElement *PrmJ,
                      const bool hasShape)
{
    const SMDS_MeshNode *nApexI = PrmI->GetNode(4);
    const SMDS_MeshNode *nApexJ = PrmJ->GetNode(4);
    if (nApexI == nApexJ || nApexI->getshapeId() != nApexJ->getshapeId()) return false;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (j >= 0) {
            ...
            if (baseNodes[ind]) return false; // pyramids with a common base face
    ...
    if (tooClose && !hasShape) {
        ...
        if (isOutJ == isOutI) return false; // other domain
        ...
        int i1, i2;
        while (const SMDS_MeshElement *f = SMESH_MeshAlgos::FindFaceInSet(
                   baseNodes[0], baseNodes[1], emptySet, avoidSet, &i1, &i2)) {
            avoidSet.insert(f);
...
void UpdateQuadraticPyramids(const set<const SMDS_MeshNode *> &commonApex, SMESHDS_Mesh *meshDS)
{
    ...
    for (; nIt != commonApex.end(); ++nIt) {
        ...
        for (unsigned i = 0; i < pyrams.size(); ++i)
            for (int baseIndex = 0; baseIndex < PYRAM_APEX; ++baseIndex) {
                ...
                if (b2m->second != medium) { nodesToRemove.push_back(medium); }
                else {
        ...
        for (unsigned i = 0; i < pyrams.size(); ++i) {
            vector<const SMDS_MeshNode *> nodes(pyrams[i]->begin_nodes(), pyrams[i]->end_nodes());
            for (int baseIndex = 0; baseIndex < PYRAM_APEX; ++baseIndex) {
                                                 ...
                                                 set<const SMDS_MeshNode *> &nodesToMove)
{
    ...
    for (unsigned i = 0; i < inverseElems.size(); ++i) {
        ...
        while (!FJEqual && triItJ->more()) {
            ...
            if (EqualTriangles(FJ, FI)) FJEqual = FJ;
        }
        if (FJEqual) {
            removeTmpElement(FI);
    ...
    GetMeshDS()->RemoveFreeNode(Nrem, GetMeshDS()->MeshElements(Nrem->getshapeId()),
                                /*fromGroups=*/false);
}
                                                 ...
                                                 set<const SMDS_MeshNode *> &nodesToMove)
{
    ...
    for (int k = 0; k < 4; k++) // loop on 4 base nodes of PrmI
    {
        ...
        while (vIt->more()) {
            ...
            if (PrmJ->NbCornerNodes() != 5 || !adjacentPyrams.insert(PrmJ).second) continue;
...
StdMeshers_QuadToTriaAdaptor::StdMeshers_QuadToTriaAdaptor() : myElemSearcher(0) {}
...
static gp_Pnt FindBestPoint(const gp_Pnt &P1, const gp_Pnt &P2, const gp_Pnt &PC, const gp_Vec &V)
{
    ...
    if (a < (b + c) / 2) return Pbest;
        ...
        if (fabs(Vsize) > std::numeric_limits<double>::min()) {
            const double shift =
                sqrt(a * a + (b * b - c * c) * (b * b - c * c) / 16 / a / a - (b * b + c * c) / 2);
...
static bool HasIntersection3(const gp_Pnt &P, const gp_Pnt &PC, gp_Pnt &Pint, const gp_Pnt &P1,
                             const gp_Pnt &P2, const gp_Pnt &P3)
{
    ...
    if (IAICQ.IsDone()) {
        ...
        if (IAICQ.NbPoints() == 1) {
            ...
            gp_Vec V2(PIn, P2);
            gp_Vec V3(PIn, P3);
            if (V1.Magnitude() < preci || V2.Magnitude() < preci || V3.Magnitude() < preci) {
                Pint = PIn;
                return true;
            }
            ...
            if (VC1.Magnitude() < gp::Resolution()) {
                if (VC2.IsOpposite(VC3, angularTol)) { return false; }
            }
            else if (VC2.Magnitude() < gp::Resolution()) {
                if (VC1.IsOpposite(VC3, angularTol)) { return false; }
            }
            else if (VC3.Magnitude() < gp::Resolution()) {
                if (VC1.IsOpposite(VC2, angularTol)) { return false; }
            }
            else {
                            ...
                            Handle(TColgp_HSequenceOfPnt) & aContour)
{
    if (aContour->Length() == 3) {
        return HasIntersection3(P, PC, Pint, aContour->Value(1), aContour->Value(2),
                                aContour->Value(3));
    }
    else {
        bool check = false;
        if ((aContour->Value(1).Distance(aContour->Value(2)) > 1.e-6)
            ...
            check = HasIntersection3(P, PC, Pint, aContour->Value(1), aContour->Value(2),
                                     aContour->Value(3));
        }
        ...
        if ((aContour->Value(1).Distance(aContour->Value(4)) > 1.e-6)
            ...
            check = HasIntersection3(P, PC, Pint, aContour->Value(1), aContour->Value(3),
                                     aContour->Value(4));
        }
        if (check) return true;
    }
...
bool StdMeshers_QuadToTriaAdaptor::CheckIntersection(const gp_Pnt &P, const gp_Pnt &PC,
                                                     gp_Pnt &Pint, SMESH_Mesh &aMesh,
                                                     const TopoDS_Shape &aShape,
                                                     const SMDS_MeshElement *NotCheckedFace)
{
    if (!myElemSearcher) myElemSearcher = SMESH_MeshAlgos::GetElementSearcher(*aMesh.GetMeshDS());
...
int StdMeshers_QuadToTriaAdaptor::Preparation(const SMDS_MeshElement *face,
                                              ...
                                              vector<const SMDS_MeshNode *> &FNodes, gp_Pnt &PC,
                                              gp_Vec &VNorm, const SMDS_MeshElement **volumes)
{
    if (face->NbCornerNodes() != 4) { return NOT_QUAD; }
    ...
    for (i = 1; i < 4; i++) {
        j = i + 1;
        for (; j <= 4; j++) {
            if (PN->Value(i).Distance(PN->Value(j)) < 1.e-6) break;
        }
        if (j <= 4) break;
    }
    ...
    if (volumes) {
        ...
        while (vIt->more()) {
            ...
            if (volSharesAllNodes) volumes[volumes[0] ? 1 : 0] = vol;
        ...
        if (volumes[0]) {
            ...
            if (VNorm * gp_Vec(PC, volGC) < 0) swap(volumes[0], volumes[1]);
        }
    }
                                           ...
                                           SMESH_ProxyMesh *aProxyMesh)
{
    ...
    if (aShape.ShapeType() != TopAbs_SOLID && aShape.ShapeType() != TopAbs_SHELL) return false;
    ...
    for (TopExp_Explorer exp(aShape, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        if (aProxyMesh) aSubMeshDSFace = aProxyMesh->GetSubMesh(aShapeFace);
        else
        ...
        if (aSubMeshDSFace) {
            ...
            while (iteratorElem->more()) // loop on elements on a geometrical face
            {
                ...
                switch (stat) {
                    case NOT_QUAD: trias.push_back(face); break;

                    case DEGEN_QUAD: {
                        // degenerate face
                        // add triangles to result map
                        SMDS_MeshFace *NewFace;
                        if (!isRev) NewFace = meshDS->AddFace(FNodes[0], FNodes[1], FNodes[2]);
                        else
                    ...
                    case QUAD: {
                        ...
                        for (; i <= 4; i++) {
                            gp_Pnt Pbest;
                            if (!isRev)
                                Pbest = FindBestPoint(PN->Value(i), PN->Value(i + 1), PC,
                                                      VN->Value(i).Reversed());
                            else
                                Pbest =
                                    FindBestPoint(PN->Value(i), PN->Value(i + 1), PC, VN->Value(i));
                            xc += Pbest.X();
                            yc += Pbest.Y();
                            zc += Pbest.Z();
                        }
            ...
            if (hasNewTrias) {
                ...
                if (sourceSubMeshIsProxy) {
                    for (unsigned i = 0; i < quads.size(); ++i) removeTmpElement(quads[i]);
...
bool StdMeshers_QuadToTriaAdaptor::Compute(SMESH_Mesh &aMesh)
{
    ...
    if (aMesh.NbQuadrangles() < 1) return false;
    ...
    if (!myElemSearcher) myElemSearcher = SMESH_MeshAlgos::GetElementSearcher(*meshDS);
    SMESH_ElementSearcher *searcher = const_cast<SMESH_ElementSearcher *>(myElemSearcher);
    ...
    while (fIt->more()) {
        ...
        if (what == DEGEN_QUAD) {
            ...
            for (int iF = 0; iF < suspectElems.size(); ++iF) {
                ...
                if (F == face) continue;
                Handle(TColgp_HSequenceOfPnt) aContour = new TColgp_HSequenceOfPnt;
                for (int i = 0; i < 4; ++i) aContour->Append(SMESH_TNodeXYZ(F->GetNode(i)));
                gp_Pnt PPP;
        ...
        if (height < 1.e-6) {
            ...
            if (height < std::numeric_limits<double>::min()) return false; // batterfly element
        }

        // Restrict pyramid height by intersection with other faces
        gp_Vec tmpDir(PC, PCbest);
        tmpDir.Normalize();
        ...
        for (int iF = 0; iF < suspectElems.size(); ++iF) {
            ...
            for (i = 0; i < nbN; ++i) aContour->Append(SMESH_TNodeXYZ(F->GetNode(i)));
            gp_Pnt intP;
        ...
        if (groupDS && groupDS->Contains(face)) { intersected[0] = false; }
        else if (intersected[0] && intersected[1]) // check if one of pyramids is in a hole
        {
            ...
            if (searcher->GetPointState(P) == TopAbs_OUT) intersected[0] = false;
            else {
                P = (PC.XYZ() - tmpDir.XYZ() * 0.5 * PC.Distance(intPnt[1]));
                if (searcher->GetPointState(P) == TopAbs_OUT) intersected[1] = false;
            }
        }
        ...
        for (int isRev = 0; isRev < 2; ++isRev) {
            ...
            for (i = 0; i < 4; i++) {
                SMDS_MeshFace *NewFace;
                if (isRev) NewFace = meshDS->AddFace(NewNode, FNodes[i], FNodes[i + 1]);
                else
...
bool StdMeshers_QuadToTriaAdaptor::Compute2ndPart(
    SMESH_Mesh &aMesh, const vector<const SMDS_MeshElement *> &myPyramids)
{
    if (myPyramids.empty()) return true;
    ...
    for (i = 0; i < myPyramids.size(); ++i) {
        ...
        for (k = 0; k < 4; k++) // loop on 4 base nodes of PrmI
        {
            ...
            for (j = 0; j < suspectPyrams.size(); ++j) {
                ...
                if (!checkedPyrams.insert(PrmJ).second) continue; // already checked
                ...
                for (k = 0; k < 4 && !hasInt; k++) {
                    gp_Vec Vtmp(PsI[k], PsI[4]);
                    gp_Pnt Pshift =
                        PsI[k].XYZ() + Vtmp.XYZ() * 0.01; // base node moved a bit to apex
                    hasInt = (HasIntersection3(Pshift, PsI[4], Pint, PsJ[0], PsJ[1], PsJ[4])
                ...
                for (k = 0; k < 4 && !hasInt; k++) {
                    ...
                    hasInt = (HasIntersection3(Pshift, PsJ[4], Pint, PsI[0], PsI[1], PsI[4])
                ...
                if (hasInt) {
                    ...
                    int nbc = 0;
                    for (k = 0; k < 4; k++) nbc += int(PrmI->GetNodeIndex(PrmJ->GetNode(k)) >= 0);

                    if (nbc == 4) continue; // pyrams have a common base face
                    ...
                    if (nbc > 0) {
                        ...
                        double coef2 =
                            0.5 - ((ang2 < M_PI / 3.) ? cos(ang2) * 0.25 : 0); // cos(ang2) ?
                        //             double coef2 = 0.5;
                        //             if(ang2<PI/3)
                        //               coef2 -= cos(ang1)*0.25;
    ...
    if (!nodesToMove.empty() && !meshDS->IsEmbeddedMode()) {
        ...
        for (; n != nodesToMove.end(); ++n) meshDS->MoveNode(*n, (*n)->X(), (*n)->Y(), (*n)->Z());
    }

    // move medium nodes of merged quadratic pyramids
    if (myPyramids[0]->IsQuadratic()) UpdateQuadraticPyramids(nodesToMove, GetMeshDS());
        ...
        for (int i = 0; i <= meshDS->MaxShapeIndex(); ++i)
            if (SMESH_ProxyMesh::SubMesh *sm = findProxySubMesh(i)) {
                ...
                while (fIt->more()) {
                    ...
                    if (rmTria != myRemovedTrias.end()) myRemovedTrias.erase(rmTria);
                    else

...
StdMeshers_CartesianParameters3D::StdMeshers_CartesianParameters3D(int hypId, int studyId,
                                                                   SMESH_Gen *gen)
...
{
    if (axis < 0 || axis > 2)
        throw SALOME_Exception(SMESH_Comment("Invalid axis index ")
                               << axis << ". Valid axis indices are 0, 1 and 2");
}
...
void checkGridSpacing(std::vector<std::string> &spaceFunctions, std::vector<double> &internalPoints,
                      const std::string &axis)
{
    ...
    if (internalPoints.size() < 2 || internalPoints.back() < 1 - tol) internalPoints.push_back(1.);
...
void StdMeshers_CartesianParameters3D::SetGrid(std::vector<double> &coords, int axis)
{
    ...
    if (coords.size() < 2) throw SALOME_Exception(LOCALIZED("Wrong number of grid coordinates"));
                                                      ...
                                                      const int axis)
{
    ...
    bool changed =
        (xSpaceFuns != _spaceFunctions[axis] || xInternalPoints != _internalPoints[axis]);

    _spaceFunctions[axis] = xSpaceFuns;
    _internalPoints[axis] = xInternalPoints;
    _coords[axis].clear();

    if (changed) NotifySubMeshesHypothesisModification();
}
...
{
    if (toUnset != Precision::IsInfinite(_fixedPoint[0])) NotifySubMeshesHypothesisModification();

    if (toUnset) _fixedPoint[0] = Precision::Infinite();
    else
...
{
    if (Precision::IsInfinite(_fixedPoint[0])) return false;
...
{
    if (threshold <= 1.0) throw SALOME_Exception(LOCALIZED("threshold must be > 1.0"));
...
void StdMeshers_CartesianParameters3D::ComputeCoordinates(const double x0, const double x1,
                                                          vector<string> &theSpaceFuns,
                                                          vector<double> &thePoints,
                                                          vector<double> &coords,
                                                          const string &axis, const double *xForced)
{
    ...
    for (size_t i = 0; i < spaceFuns.size(); ++i) {
        ...
        for (size_t iCell = 1, i = 1; i <= nbSections; ++i) {
            if (nbSegments[i] * corr >= iCell) {
                t = (i - (nbSegments[i] - iCell / corr) / (nbSegments[i] - nbSegments[i - 1]))
                    / nbSections;
        ...
        if (fabs(coords.back() - p1) > 0.5 * lastCellLen) coords.push_back(p1);
    }
    ...
    if (forced) {
        ...
        for (size_t i = 1; i < coords.size(); ++i) {
            if (!iF && Abs(coords[i] - *xForced) < 1e-20) iF = i++; // xForced found
            else
                minLen = Min(minLen, coords[i] - coords[i - 1]);
        }
        const double tol = minLen * 1e-3;
        int iRem = -1;
        if ((iF > 1) && (coords[iF] - coords[iF - 1] < tol)) iRem = iF - 1;
        else if ((iF < coords.size() - 2) && (coords[iF + 1] - coords[iF] < tol))
            iRem = iF + 1;
        if (iRem > 0) coords.erase(coords.begin() + iRem);
    }
}
                                                      ...
                                                      const Bnd_Box &bndBox) const
{
    ...
    if (IsGridBySpacing(0) || IsGridBySpacing(1) || IsGridBySpacing(2)) {
        if (bndBox.IsVoid() || bndBox.IsXThin(Precision::Confusion())
            || bndBox.IsYThin(Precision::Confusion()) || bndBox.IsZThin(Precision::Confusion()))
                                                              ...
                                                              double dirCoords[9])
{
    ...
    for (exp.Init(shape, TopAbs_FACE); exp.More(); exp.Next())
        if (!faceMap.Add(exp.Current())) faceMap.Remove(exp.Current());
    ...
    for (int iF = 1; norm2a != areasByNormal.end(); ++norm2a, ++iF) {
        ...
        if (iF == areasByNormal.size()) normsByArea.insert(make_pair(sumArea, norms));
    }
    ...
    if (!normsByArea.empty()) {
        ...
        if (normsByArea.size() == 1) {
            ...
            if (Abs(normDirs[0].Y()) < 1e-100 && Abs(normDirs[0].Z()) < 1e-100) // normDirs[0] || OX
            ...
            for (int is2nd = 0; is2nd < 2; ++is2nd) {
                ...
                if (is2nd) { normDirs[2] = minDir; }
                else {
                    normDirs[1] = maxDir;
                    normDirs[2] = normDirs[0] ^ normDirs[1];
                    if (isOrthogonal || normsByArea.size() < 3) break;
                    testDir = normDirs[2];
                }
            }
        }
...
{
    gp_Vec x(the9DirComps[0], the9DirComps[1], the9DirComps[2]);
    gp_Vec y(the9DirComps[3], the9DirComps[4], the9DirComps[5]);
    gp_Vec z(the9DirComps[6], the9DirComps[7], the9DirComps[8]);
    if (x.Magnitude() < RealSmall() || y.Magnitude() < RealSmall() || z.Magnitude() < RealSmall())
        throw SALOME_Exception("Zero magnitude of axis direction");

    if (x.IsParallel(y, M_PI / 180.) || x.IsParallel(z, M_PI / 180.)
        || y.IsParallel(z, M_PI / 180.))
        throw SALOME_Exception("Parallel axis directions");

    gp_Vec normXY = x ^ y, normYZ = y ^ z;
    if (normXY.IsParallel(normYZ, M_PI / 180.)) throw SALOME_Exception("Axes lie in one plane");

    bool isChanged = false;
    for (int i = 0; i < 9; ++i) {
        if (Abs(_axisDirs[i] - the9DirComps[i]) > 1e-7) isChanged = true;
        _axisDirs[i] = the9DirComps[i];
    }
    if (isChanged) NotifySubMeshesHypothesisModification();
}
...
double StdMeshers_CartesianParameters3D::GetSizeThreshold() const { return _sizeThreshold; }
...
bool StdMeshers_CartesianParameters3D::GetToAddEdges() const { return _toAddEdges; }
...
std::ostream &StdMeshers_CartesianParameters3D::SaveTo(std::ostream &save)
{
    ...
    for (int i = 0; i < 3; ++i) {
        save << _coords[i].size() << " ";
        for (size_t j = 0; j < _coords[i].size(); ++j) save << _coords[i][j] << " ";

        save << _internalPoints[i].size() << " ";
        for (size_t j = 0; j < _internalPoints[i].size(); ++j) save << _internalPoints[i][j] << " ";

        save << _spaceFunctions[i].size() << " ";
        for (size_t j = 0; j < _spaceFunctions[i].size(); ++j) save << _spaceFunctions[i][j] << " ";
    }
    save << _toAddEdges << " ";

    save.setf(save.scientific);
    save.precision(12);
    for (int i = 0; i < 9; ++i) save << _axisDirs[i] << " ";

    for (int i = 0; i < 3; ++i) save << _fixedPoint[i] << " ";
...
std::istream &StdMeshers_CartesianParameters3D::LoadFrom(std::istream &load)
{
    ...
    for (int ax = 0; ax < 3; ++ax) {
        if (ok) {
            ...
            if (i > 0 && ok) {
                _coords[ax].resize(i);
                for (i = 0; i < _coords[ax].size() && ok; ++i) ok = (bool)(load >> _coords[ax][i]);
            }
        }
        if (ok) {
            size_t i = 0;
    ...
    for (int i = 0; i < 9 && ok; ++i) ok = (bool)(load >> _axisDirs[i]);

    for (int i = 0; i < 3 && ok; ++i) ok = (bool)(load >> _fixedPoint[i]);
...
bool StdMeshers_CartesianParameters3D::SetParametersByMesh(const SMESH_Mesh *, const TopoDS_Shape &)
{
    return false;
}

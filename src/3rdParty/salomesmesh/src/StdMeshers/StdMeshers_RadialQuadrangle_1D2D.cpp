...
StdMeshers_RadialQuadrangle_1D2D::StdMeshers_RadialQuadrangle_1D2D(int hypId, int studyId,
                                                                   SMESH_Gen *gen)
...
public:
    ...
    void ProcessEvent(const int event, const int eventType, SMESH_subMesh *edgeSubMesh,
...
int analyseFace(const TopoDS_Shape &face, TopoDS_Edge &CircEdge, TopoDS_Edge &LinEdge1,
...
bool isCornerInsideCircle(const TopoDS_Edge &CircEdge, const TopoDS_Edge &LinEdge1,
                          const TopoDS_Edge &LinEdge2)
{
    if (!CircEdge.IsNull() && !LinEdge1.IsNull() && !LinEdge2.IsNull()) {
        Handle(Geom_Circle) aCirc = Handle(Geom_Circle)::DownCast(getCurve(CircEdge));
        TopoDS_Vertex aCommonV;
        if (!aCirc.IsNull() && TopExp::CommonVertex(LinEdge1, LinEdge2, aCommonV)) {
...
public:
    ...
    static TNodeDistributor *GetDistributor(SMESH_Mesh &aMesh)
    {
        ...
        if (!myHyp) myHyp = new TNodeDistributor(myID, 0, aMesh.GetGen());
        return myHyp;
    }
    // -----------------------------------------------------------------------------
    //! Computes distribution of nodes on a straight line ending at pIn and pOut
    bool Compute(vector<double> &positions, gp_Pnt pIn, gp_Pnt pOut, SMESH_Mesh &aMesh,
    ...
    bool ComputeCircularEdge(SMESH_Mesh &aMesh, const TopoDS_Edge &anEdge)
    {
    ...
    // -----------------------------------------------------------------------------
    //! Make mesh on an adge using assigned 1d hyp or defaut nb of segments
    bool EvaluateCircularEdge(SMESH_Mesh &aMesh, const TopoDS_Edge &anEdge,
                              MapShapeNbElems &aResMap)
    {
        _gen->Evaluate(aMesh, anEdge, aResMap);
        if (aResMap.count(aMesh.GetSubMesh(anEdge))) return true;
...
bool StdMeshers_RadialQuadrangle_1D2D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (nbe == 1) {
        if (!algo1d->ComputeCircularEdge(aMesh, CircEdge)) return error(algo1d->GetComputeError());
        ...
        if (!computeLayerPositions(P0, P1)) return false;
        ...
        if (!computeLayerPositions(P0, P1, LinEdge1, &linEdgeComputed)) return false;
        ...
        if (linEdgeComputed) {
            ...
            Crv->D0(fp, Ptmp);
            bool ori = true;
            if (P1.Distance(Ptmp) > Precision::Confusion()) ori = false;
            ...
            for (; i < myLayerPositions.size(); i++) {
                ...
                Nodes1[i] = node;
                double param;
                if (ori) param = fp + dp2 * (1 - myLayerPositions[i]);
                else
            ...
            for (i = 0; i < Nodes2.size(); i++) tmpNodes[Nodes2.size() - i - 1] = Nodes2[i];
            tmpNodes[Nodes2.size()] = NC;
            for (i = 0; i < Nodes1.size(); i++) tmpNodes[Nodes2.size() + 1 + i] = Nodes1[i];
    ...
    {
        if (nbe == 2 && LinEdge1.Orientation() == TopAbs_INTERNAL) LinEdge2 = LinEdge1;
        ...
        if (aCirc.IsNull() || aLine1.IsNull() || aLine2.IsNull()) return error(COMPERR_BAD_SHAPE);
        if (!isCornerInsideCircle(CircEdge, LinEdge1, LinEdge2)) return error(COMPERR_BAD_SHAPE);
        ...
        if (!computeLayerPositions(P0, P1, LinEdge1, &linEdge1Computed)) return false;

        Nodes1.resize(myLayerPositions.size() + 1);
        Nodes2.resize(myLayerPositions.size() + 1);

        // check that both linear edges have same hypotheses
        if (!computeLayerPositions(P0, P2, LinEdge2, &linEdge2Computed)) return false;
        ...
        if (linEdge1Computed) {
            ...
            bool nodesFromP0ToP1 = (theNodes.rbegin()->second == NF);
            NC = const_cast<SMDS_MeshNode *>(nodesFromP0ToP1 ? theNodes.begin()->second
                                                             : theNodes.rbegin()->second);
            int i = 0, ir = Nodes1.size() - 1;
            int *pi = nodesFromP0ToP1 ? &i : &ir;
            itn = theNodes.begin();
            if (nodesFromP0ToP1) ++itn;
            for (; i < Nodes1.size(); ++i, --ir, ++itn) { Nodes1[*pi] = itn->second; }
            ...
            bool ori = false;
            if (P1.Distance(Ptmp) > Precision::Confusion()) ori = true;
            // get UV points for edge
            gp_Pnt2d PF, PL;
            BRep_Tool::UVPoints(LinEdge1, TopoDS::Face(aShape), PF, PL);
            gp_Vec2d V2d;
            ...
            for (; i < myLayerPositions.size(); i++) {
                ...
                Nodes1[i] = node;
                double param;
                if (!ori) param = fp + dp * (1 - myLayerPositions[i]);
                else
            ...
            if (nbe == 2 && LinEdge1.Orientation() == TopAbs_INTERNAL) Nodes2 = Nodes1;
        }
        markEdgeAsComputedByMe(LinEdge1, aMesh.GetSubMesh(F));
        ...
        if (linEdge2Computed) {
            ...
            for (; i < Nodes2.size(); ++i, --ir, ++itn) Nodes2[*pi] = itn->second;
        }
        else {
            ...
            gp_Pnt Ptmp = Crv->Value(fp);
            bool ori = false;
            if (P2.Distance(Ptmp) > Precision::Confusion()) ori = true;
            // get UV points for edge
            gp_Pnt2d PF, PL;
    ...
    for (; i < Angles.Length(); i++) {
        ...
        for (j = 0; j < Nodes1.size() - 1; j++) {
            SMDS_MeshFace *MF;
            if (IsForward)
                MF = myHelper->AddFace(tmpNodes[j], Nodes1[j], Nodes1[j + 1], tmpNodes[j + 1]);
            else
                MF = myHelper->AddFace(tmpNodes[j], tmpNodes[j + 1], Nodes1[j + 1], Nodes1[j]);
            if (MF) meshDS->SetMeshElementOnShape(MF, faceID);
        }
        // tria
        SMDS_MeshFace *MF;
        if (IsForward) MF = myHelper->AddFace(NC, Nodes1[0], tmpNodes[0]);
        else
            MF = myHelper->AddFace(NC, tmpNodes[0], Nodes1[0]);
        if (MF) meshDS->SetMeshElementOnShape(MF, faceID);
        for (j = 0; j < Nodes1.size(); j++) { Nodes1[j] = tmpNodes[j]; }
    }
    // create last faces
    // quad
    for (i = 0; i < Nodes1.size() - 1; i++) {
        SMDS_MeshFace *MF;
        if (IsForward) MF = myHelper->AddFace(Nodes2[i], Nodes1[i], Nodes1[i + 1], Nodes2[i + 1]);
        else
            MF = myHelper->AddFace(Nodes2[i], Nodes2[i + 1], Nodes1[i + 1], Nodes1[i]);
        if (MF) meshDS->SetMeshElementOnShape(MF, faceID);
    }
    // tria
    SMDS_MeshFace *MF;
    if (IsForward) MF = myHelper->AddFace(NC, Nodes1[0], Nodes2[0]);
    else
...
bool StdMeshers_RadialQuadrangle_1D2D::computeLayerPositions(const gp_Pnt &p1, const gp_Pnt &p2,
                                                             ...
                                                             bool *linEdgeComputed)
{
    ...
    const SMESH_Hypothesis *hyp1D =
        myDistributionHypo ? myDistributionHypo->GetLayerDistribution() : 0;
    ...
    if (!hyp1D && !nbLayers) {
        ...
        if (!edge.IsNull()) {
            ...
            const SMESH_HypoFilter *hypKind =
                TNodeDistributor::GetDistributor(*mesh)->GetCompatibleHypoFilter(
                    /*ignoreAux=*/true);
    ...
    {
        if (!TNodeDistributor::GetDistributor(*mesh)->Compute(myLayerPositions, p1, p2, *mesh,
                                                              hyp1D)) {
    ...
    {
        if (!nbLayers) nbLayers = _gen->GetDefaultNbSegments();
    ...
    if (meshComputed) {
        ...
        if (!pointsAreOnVertices) nbNodes = (nodeParams.size() - 3) / 2; // 1 straight edge

        if (myLayerPositions.empty()) { myLayerPositions.resize(nbNodes); }
...
bool StdMeshers_RadialQuadrangle_1D2D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                                MapShapeNbElems &aResMap)
{
    if (aShape.ShapeType() != TopAbs_FACE) { return false; }
    SMESH_subMesh *sm = aMesh.GetSubMesh(aShape);
    if (aResMap.count(sm)) return false;
    ...
    TopoDS_Edge CircEdge, LinEdge1, LinEdge2;
    int nbe = analyseFace(aShape, CircEdge, LinEdge1, LinEdge2);
    if (nbe > 3 || nbe < 1 || CircEdge.IsNull()) return false;

    Handle(Geom_Circle) aCirc = Handle(Geom_Circle)::DownCast(getCurve(CircEdge));
    if (aCirc.IsNull()) return error(COMPERR_BAD_SHAPE);
    ...
    if (nbe == 1) {
        ...
        if (ok) { ok = algo1d->EvaluateCircularEdge(aMesh, CircEdge, aResMap); }
        if (ok) {
            const vector<int> &aVec = aResMap[aMesh.GetSubMesh(CircEdge)];
            isQuadratic = aVec[SMDSEntity_Quad_Edge] > aVec[SMDSEntity_Edge];
            if (isQuadratic) {
    ...
    else // nbe==3 or ( nbe==2 && linEdge is INTERNAL )
    {
        ...
        if (!ok) {
            if (myDistributionHypo || myNbLayerHypo) ok = true; // override other 1d hyps
            else {
                const vector<int> &aVec = aResMap[aMesh.GetSubMesh(LinEdge1)];
                ok = (aVec[SMDSEntity_Node] == myLayerPositions.size());
            }

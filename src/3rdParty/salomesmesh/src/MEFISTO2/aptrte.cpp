...
NEWDEPART:
    ...
    for (n = 1; n <= nblf; n++) {
        ...
        d = pow(uvslf[ns0 + 1].x - uvslf[ns0].x, 2) + pow(uvslf[ns0 + 1].y - uvslf[ns0].y, 2);
        aremin = Min(aremin, d);
        aremax = Max(aremax, d);
        ...
        fasoar(ns1, ns2, moins1, moins1, n, mosoar, mxsoar, n1soar, mnsoar, mnarst, noar0, ierr);
        ...
        if (noar > 0) mnsoar[mosoar * noar - mosoar + 5] = noar0;
        ...
        for (i = 2; i <= nbarli; i++) {
            ...
            fasoar(ns1, ns2, moins1, moins1, n, mosoar, mxsoar, n1soar, mnsoar, mnarst, noar, ierr);
            //pas de test sur ierr car pas de saturation possible a ce niveau
    ...
    if (aretmx < aremin && aremin > 0) aretmx = aremin;
    ...
    for (i = 0; i <= nuds; i++) mnpxyd[i].z = Min(mnpxyd[i].z, aretmx);
    //MESSAGE("Numero du dernier sommet frontalier=" << nuds+1);
    //fin  ajout 9/11/2006  .................................................


    MESSAGE("Sur  le  bord: arete min=" << aremin << " arete max=" << aremax);
    MESSAGE("Triangulation: arete mx=" << aretmx << " triangle aire mx=" << airemx);
...
NEWTREE: //en cas de saturation de l'un des tableaux, on boucle
    ...
    tehote(nutysu, nbarpi, mxsomm, nbsomm, mnpxyd, comxmi, aretmx, mntree, mxqueu, mnqueu, ierr);

    deltacpu_(d);
    tcpu += d;
    MESSAGE("Temps de l'adaptation et l'homogeneisation de l'arbre-4 des TE=" << d << " secondes");
    if (ierr != 0) {
    ...
    // ==========================================================================
    tetrte(comxmi, aretmx, nbarpi, mxsomm, mnpxyd, mxqueu, mnqueu, mntree, mosoar, mxsoar, n1soar,
           mnsoar, moartr, mxartr, n1artr, mnartr, mnarst, ierr);
    ...
    qualitetrte(mnpxyd, mosoar, mxsoar, mnsoar, moartr, mxartr, mnartr, nbt, quamoy, quamin);
    ...
    tedela(mnpxyd, mnarst, mosoar, mxsoar, n1soar, mnsoar, na, moartr, mxartr, n1artr, mnartr, n);

    MESSAGE("Nombre d'echanges des diagonales de 2 triangles=" << n);
    deltacpu_(d);
    tcpu += d;
    MESSAGE("Temps de la triangulation Delaunay par echange des diagonales=" << d << " secondes");
    ...
    terefr(nbarpi, mnpxyd, mosoar, mxsoar, n1soar, mnsoar, moartr, mxartr, n1artr, mnartr, mnarst,
           mxarcf, mn1arcf, mnarcf, mnarcf1, mnarcf2, n, ierr);

    MESSAGE("Restauration de " << n << " aretes perdues de la frontiere  ierr=" << ierr);
    deltacpu_(d);
    tcpu += d;
    MESSAGE("Temps de la recuperation des aretes perdues de la frontiere=" << d << " secondes");
    ...
    tesuex(nblf, mnlftr, ndtri0, nbsomm, mnpxyd, mnslig, mosoar, mxsoar, mnsoar, moartr, mxartr,
           n1artr, mnartr, mnarst, nbt, mntrsu, ierr);
    ...
    teamqt(nutysu, aretmx, airemx, mnarst, mosoar, mxsoar, n1soar, mnsoar, moartr, mxartr, n1artr,
           mnartr, mxarcf, mnarcf2, mnarcf3, mn1arcf, mnarcf, mnarcf1, nbarpi, nbsomm, mxsomm,
           mnpxyd, mnslig, ierr);
    if (mnarcf3 != NULL) {
        delete[] mnarcf3;
        mnarcf3 = NULL;
    ...
    MESSAGE("APTRTE: Fin de la triangulation plane avec " << nbst << " sommets et " << nbt
                                                          << " triangles");
    deltacpu_(d);
    tcpu += d;
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
{
    ...
    for (nt = 1; nt <= mxartr; nt++) {
        ...
        if (mnartr[mn] != 0) {
            ...
            qutr2d(mnpxyd[nosotr[0] - 1], mnpxyd[nosotr[1] - 1], mnpxyd[nosotr[2] - 1], qualite);
            ...
            if (d < 0) {
                ...
                nbtrianeg++;
                MESSAGE("ATTENTION: le triangle " << nt << " de sommets:" << nosotr[0] << " "
                                                  << nosotr[1] << " " << nosotr[2] << " a une aire "
                                                  << d << "<=0");
            }
    ...
    //les affichages
    quamoy /= nbtria;
    MESSAGE("Qualite moyenne=" << quamoy << "  Qualite minimale=" << quamin << " des " << nbtria
                               << " triangles de surface plane totale=" << aire);

    if (quamin < 0.3) {
        //le numero des 3 sommets du triangle ntqmin de qualite minimale
        nusotr(ntqmin, mosoar, mnsoar, moartr, mnartr, nosotr);
        MESSAGE("Triangle de qualite minimale " << quamin << " de sommets:" << nosotr[0] << " "
                                                << nosotr[1] << " " << nosotr[2] << " ");
        for (int i = 0; i < 3; i++)
    ...
    if (nbtrianeg > 0) MESSAGE("ATTENTION: " << nbtrianeg << " TRIANGLES d'AIRE NEGATIVE");

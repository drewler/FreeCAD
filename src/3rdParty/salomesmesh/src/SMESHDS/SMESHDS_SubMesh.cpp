...
{
    if (!IsComplexSubmesh()) return myElements.size() - myUnusedIdElements;

    int nbElems = 0;
    set<const SMESHDS_SubMesh *>::const_iterator it = mySubMeshes.begin();
    for (; it != mySubMeshes.end(); it++) nbElems += (*it)->NbElements();
...
{
    if (!IsComplexSubmesh()) return myNodes.size() - myUnusedIdNodes;
...
{
    if (IsComplexSubmesh()) return SMDS_ElemIteratorPtr(new MyElemIterator(mySubMeshes));
    return SMDS_ElemIteratorPtr(
        new MySetIterator<const SMDS_MeshElement *, std::vector<const SMDS_MeshElement *>>(
            myElements));
}
...
{
    if (IsComplexSubmesh()) return SMDS_NodeIteratorPtr(new MyNodeIterator(mySubMeshes));
...
bool SMESHDS_SubMesh::Contains(const SMDS_MeshElement *ME) const
{
    ...
    if (IsComplexSubmesh()) {
        ...
        for (; aSubIt != mySubMeshes.end(); aSubIt++)
            if ((*aSubIt)->Contains(ME)) return true;
        return false;
    }
...
bool SMESHDS_SubMesh::IsQuadratic() const
{
    if (IsComplexSubmesh()) {
        set<const SMESHDS_SubMesh *>::const_iterator aSubIt = mySubMeshes.begin();
        for (; aSubIt != mySubMeshes.end(); aSubIt++)
            if ((*aSubIt)->IsQuadratic()) return true;
        return false;
    }

    for (size_t i = 0; i < myElements.size(); ++i)
        if (myElements[i]) return myElements[i]->IsQuadratic();
...
void SMESHDS_SubMesh::RemoveAllSubmeshes() { mySubMeshes.clear(); }
...
SMESHDS_SubMeshIteratorPtr SMESHDS_SubMesh::GetSubMeshIterator() const
{
    ...
    return SMESHDS_SubMeshIteratorPtr(new SMDS_SetIterator<const SMESHDS_SubMesh *, TIterator>(
...
void SMESHDS_SubMesh::Clear()
{
    ...
    if (NbSubMeshes() > 0) {
        ...
        while (sub->more()) {
            if (SMESHDS_SubMesh *sm = (SMESHDS_SubMesh *)sub->next()) sm->Clear();
        }
    }
}

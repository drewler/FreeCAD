...
SMESHDS_Mesh::SMESHDS_Mesh(int theMeshID, bool theIsEmbeddedMode)
    : myMeshID(theMeshID), mySubMeshHolder(new SubMeshHolder), myIsEmbeddedMode(theIsEmbeddedMode)
{
...
//=======================================================================
bool SMESHDS_Mesh::IsEmbeddedMode() { return myIsEmbeddedMode; }
...
{
    if (NbNodes() == 0) myPersistentID = id;
}
//================================================================================
/*!
...
int SMESHDS_Mesh::GetPersistentId() const { return myPersistentID; }
    ...
    if (!myShape.IsNull() && S.IsNull()) {
        ...
        while (SMESHDS_SubMesh *sm = const_cast<SMESHDS_SubMesh *>(smIt->next())) {
            if (!sm->IsComplexSubmesh()) {
                ...
                while (nIt->more()) sm->RemoveNode(nIt->next(), false);
            }
        }
        // - sub-meshes
        ...
        while (gr != myGroups.end()) {
            if (dynamic_cast<SMESHDS_GroupOnGeom *>(*gr)) myGroups.erase(gr++);
            else
                gr++;
        }
    }
        ...
        myShape = S;
        if (!S.IsNull()) TopExp::MapShapes(myShape, myIndexToShape);
    }
}
...
bool SMESHDS_Mesh::AddHypothesis(const TopoDS_Shape &SS, const SMESHDS_Hypothesis *H)
{
...
bool SMESHDS_Mesh::RemoveHypothesis(const TopoDS_Shape &S, const SMESHDS_Hypothesis *H)
{
    if (myShapeToHypothesis.IsBound(S /*.Oriented(TopAbs_FORWARD)*/)) {
        list<const SMESHDS_Hypothesis *> &alist =
            myShapeToHypothesis.ChangeFind(S /*.Oriented(TopAbs_FORWARD)*/);
        list<const SMESHDS_Hypothesis *>::iterator ith = find(alist.begin(), alist.end(), H);
        if (ith != alist.end()) {
            alist.erase(ith);
            return true;
...
bool SMESHDS_Mesh::ChangeElementNodes(const SMDS_MeshElement *elem, const SMDS_MeshNode *nodes[],
                                      const int nbnodes)
{
    //MESSAGE("SMESHDS_Mesh::ChangeElementNodes");
    if (!SMDS_Mesh::ChangeElementNodes(elem, nodes, nbnodes)) return false;

    vector<int> IDs(nbnodes);
    for (int i = 0; i < nbnodes; i++) IDs[i] = nodes[i]->GetID();
                                         ...
                                         std::vector<int> quantities)
{
    ...
    if (!SMDS_Mesh::ChangePolyhedronNodes(elem, nodes, quantities)) return false;

    int i, len = nodes.size();
    std::vector<int> nodes_ids(len);
    for (i = 0; i < len; i++) { nodes_ids[i] = nodes[i]->GetID(); }
...
void SMESHDS_Mesh::Renumber(const bool isNodes, const int startID, const int deltaID)
{
    ...
    if (!this->isCompacted()) this->compactMesh();
...
SMDS_Mesh0DElement *SMESHDS_Mesh::Add0DElementWithID(const SMDS_MeshNode *node, int ID)
{
...
SMDS_BallElement *SMESHDS_Mesh::AddBallWithID(const SMDS_MeshNode *node, double diameter, int ID)
{
    SMDS_BallElement *anElem = SMDS_Mesh::AddBallWithID(node, diameter, ID);
...
SMDS_BallElement *SMESHDS_Mesh::AddBall(const SMDS_MeshNode *node, double diameter)
{
...
SMDS_MeshEdge *SMESHDS_Mesh::AddEdgeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, int ID)
{
    return AddEdgeWithID(n1->GetID(), n2->GetID(), ID);
}

SMDS_MeshEdge *SMESHDS_Mesh::AddEdge(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2)
{
    SMDS_MeshEdge *anElem = SMDS_Mesh::AddEdge(n1, n2);
    if (anElem) myScript->AddEdge(anElem->GetID(), n1->GetID(), n2->GetID());
    return anElem;
}
...
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           const SMDS_MeshNode *n3, int ID)
{
    return AddFaceWithID(n1->GetID(), n2->GetID(), n3->GetID(), ID);
}

SMDS_MeshFace *SMESHDS_Mesh::AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                     const SMDS_MeshNode *n3)
{
    SMDS_MeshFace *anElem = SMDS_Mesh::AddFace(n1, n2, n3);
    if (anElem) myScript->AddFace(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID());
    return anElem;
}
...
{
    return AddFaceWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), ID);
}
                                     ...
                                     const SMDS_MeshNode *n3, const SMDS_MeshNode *n4)
{
    ...
    if (anElem)
        myScript->AddFace(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID());
    return anElem;
}
...
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                               int ID)
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), ID);
}

SMDS_MeshVolume *SMESHDS_Mesh::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                         const SMDS_MeshNode *n3, const SMDS_MeshNode *n4)
{
    SMDS_MeshVolume *anElem = SMDS_Mesh::AddVolume(n1, n2, n3, n4);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID());
    return anElem;
}
...
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(), ID);
}
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6,
                                               int ID)
{
...
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               ...
                                               int ID)
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(),
                           n6->GetID(), ID);
}
...
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(),
                           n6->GetID(), n7->GetID(), n8->GetID(), n9->GetID(), n10->GetID(),
                           n11->GetID(), n12->GetID(), ID);
}
...
{
    SMDS_MeshVolume *anElem =
        SMDS_Mesh::AddVolume(n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n5->GetID(), n6->GetID(), n7->GetID(), n8->GetID(), n9->GetID(),
                            n10->GetID(), n11->GetID(), n12->GetID());
    return anElem;
}
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMESHDS_Mesh::AddPolygonalFaceWithID(const std::vector<int> &nodes_ids, const int ID)
{
    SMDS_MeshFace *anElem = SMDS_Mesh::AddPolygonalFaceWithID(nodes_ids, ID);
    if (anElem) { myScript->AddPolygonalFace(ID, nodes_ids); }
    return anElem;
}
...
SMDS_MeshFace *SMESHDS_Mesh::AddPolygonalFace(const std::vector<const SMDS_MeshNode *> &nodes)
{
                                                        ...
                                                        const int ID)
{
    ...
    if (anElem) { myScript->AddQuadPolygonalFace(ID, nodes_ids); }
    return anElem;
}
                                                         ...
                                                         const int ID)
{
    ...
    if (anElem) { myScript->AddPolyhedralVolume(ID, nodes_ids, quantities); }
    return anElem;
}

SMDS_MeshVolume *
SMESHDS_Mesh::AddPolyhedralVolumeWithID(const std::vector<const SMDS_MeshNode *> &nodes,
...
static void removeFromContainers(SMESHDS_Mesh *theMesh, set<SMESHDS_GroupBase *> &theGroups,
                                 list<const SMDS_MeshElement *> &theElems, const bool isNode)
{
    if (theElems.empty()) return;
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::RemoveFreeNode(const SMDS_MeshNode *n, SMESHDS_SubMesh *subMesh, bool fromGroups)
{
    myScript->RemoveNode(n->GetID());
    ...
    if (fromGroups && !myGroups.empty()) {
        ...
        for (; GrIt != myGroups.end(); GrIt++) {
            SMESHDS_Group *group = dynamic_cast<SMESHDS_Group *>(*GrIt);
            if (group && !group->IsEmpty()) group->SMDSGroup().Remove(n);
        }
    }
    ...
    if (!subMesh || !subMesh->RemoveNode(n, /*deleted=*/false))
        if ((subMesh = MeshElements(n->getshapeId()))) subMesh->RemoveNode(n, /*deleted=*/false);
...
void SMESHDS_Mesh::RemoveElement(const SMDS_MeshElement *elt)
{
    ...
    if (!hasConstructionEdges() && !hasConstructionFaces()) {
        ...
        if (elt->getshapeId() > 0) subMesh = MeshElements(elt->getshapeId());
                                     ...
                                     bool fromGroups)
{
    ...
    if (fromGroups && !myGroups.empty()) {
        ...
        for (; GrIt != myGroups.end(); GrIt++) {
            SMESHDS_Group *group = dynamic_cast<SMESHDS_Group *>(*GrIt);
            if (group && !group->IsEmpty()) group->SMDSGroup().Remove(elt);
        }
    }

    // Rm from sub-mesh
    // Element should belong to only one sub-mesh
    if (!subMesh && elt->getshapeId() > 0) subMesh = MeshElements(elt->getshapeId());
    if (subMesh) subMesh->RemoveElement(elt, /*deleted=*/false);
...
void SMESHDS_Mesh::ClearMesh()
{
    ...
    // clear submeshes
    SMESHDS_SubMeshIteratorPtr smIt = SubMeshes();
    while (SMESHDS_SubMesh *sm = const_cast<SMESHDS_SubMesh *>(smIt->next())) sm->Clear();
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeInVolume(const SMDS_MeshNode *aNode, const TopoDS_Shell &S)
{
    if (add(aNode, getSubmesh(S)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(SMDS_SpacePosition::originSpacePosition());
}
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeInVolume(const SMDS_MeshNode *aNode, const TopoDS_Solid &S)
{
    if (add(aNode, getSubmesh(S)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(SMDS_SpacePosition::originSpacePosition());
}
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeOnFace(const SMDS_MeshNode *aNode, const TopoDS_Face &S, double u,
                                 double v)
{
    if (add(aNode, getSubmesh(S)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeOnEdge(const SMDS_MeshNode *aNode, const TopoDS_Edge &S, double u)
{
    if (add(aNode, getSubmesh(S)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(SMDS_PositionPtr(new SMDS_EdgePosition(u)));
}
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeOnVertex(const SMDS_MeshNode *aNode, const TopoDS_Vertex &S)
{
    if (add(aNode, getSubmesh(S)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(
            SMDS_PositionPtr(new SMDS_VertexPosition()));
}
...
void SMESHDS_Mesh::UnSetNodeOnShape(const SMDS_MeshNode *aNode)
{
    ...
    if (shapeId > 0)
        if (SMESHDS_SubMesh *sm = MeshElements(shapeId)) sm->RemoveNode(aNode, /*deleted=*/false);
}
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetMeshElementOnShape(const SMDS_MeshElement *anElement, const TopoDS_Shape &S)
{
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::UnSetMeshElementOnShape(const SMDS_MeshElement *elem, const TopoDS_Shape &S)
{
...
//purpose  :
//=======================================================================
TopoDS_Shape SMESHDS_Mesh::ShapeToMesh() const { return myShape; }
...
{
    if (myIndexToShape.Contains(theShape)) return true;

    for (TopoDS_Iterator it(theShape); it.More(); it.Next())
        if (IsGroupOfSubShapes(it.Value())) return true;
...
list<int> SMESHDS_Mesh::SubMeshIndices() const
{
    list<int> anIndices;
    SMESHDS_SubMeshIteratorPtr smIt = SubMeshes();
    while (const SMESHDS_SubMesh *sm = smIt->next()) anIndices.push_back(sm->GetID());
...
bool SMESHDS_Mesh::IsUsedHypothesis(const SMESHDS_Hypothesis *H) const
{
    ...
    for (; s2h.More(); s2h.Next())
        if (std::find(s2h.Value().begin(), s2h.Value().end(), H) != s2h.Value().end()) return true;
    return false;
}
...
//purpose  :
//=======================================================================
SMESHDS_Script *SMESHDS_Mesh::GetScript() { return myScript; }

//=======================================================================
//function : ClearScript
//purpose  :
//=======================================================================
void SMESHDS_Mesh::ClearScript() { myScript->Clear(); }
...
int SMESHDS_Mesh::AddCompoundSubmesh(const TopoDS_Shape &S, TopAbs_ShapeEnum type)
{
    int aMainIndex = 0;
        ...
        if (!aNewSub->IsComplexSubmesh()) // is empty
        {
            ...
            for (; shapeType <= typeLimit; shapeType++) {
                ...
                for (; exp.More(); exp.Next()) {
                    ...
                    if (index) aNewSub->AddSubMesh(NewSubMesh(index));
                }
            }
        }
    }
...
{
    try {
        if (ShapeIndex > 0) return myIndexToShape.FindKey(ShapeIndex);
    }
...
int SMESHDS_Mesh::MaxSubMeshIndex() const { return mySubMeshHolder->GetMaxID(); }
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetNodeOnEdge(const SMDS_MeshNode *aNode, int Index, double u)
{
    //Set Position on Node
    if (add(aNode, NewSubMesh(Index)))
        const_cast<SMDS_MeshNode *>(aNode)->SetPosition(SMDS_PositionPtr(new SMDS_EdgePosition(u)));
...
//purpose  :
//=======================================================================
void SMESHDS_Mesh::SetMeshElementOnShape(const SMDS_MeshElement *anElement, int Index)
{
                                     ...
                                     const SMDS_MeshNode *n12)
{
    ...
    if (anElem) myScript->AddEdge(anElem->GetID(), n1->GetID(), n2->GetID(), n12->GetID());
    return anElem;
}
...
//purpose  :
//=======================================================================
SMDS_MeshEdge *SMESHDS_Mesh::AddEdgeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           const SMDS_MeshNode *n12, int ID)
{
    return AddEdgeWithID(n1->GetID(), n2->GetID(), n12->GetID(), ID);
}
                                     ...
                                     const SMDS_MeshNode *n23, const SMDS_MeshNode *n31)
{
    ...
    if (anElem)
        myScript->AddFace(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n12->GetID(),
                          n23->GetID(), n31->GetID());
    return anElem;
}
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n12, int n23, int n31,
                                           int ID)
{
...
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           ...
                                           int ID)
{
    return AddFaceWithID(n1->GetID(), n2->GetID(), n3->GetID(), n12->GetID(), n23->GetID(),
                         n31->GetID(), ID);
}
                                     ...
                                     const SMDS_MeshNode *nCenter)
{
    ...
    if (anElem)
        myScript->AddFace(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n12->GetID(),
                          n23->GetID(), n31->GetID(), nCenter->GetID());
    return anElem;
}
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n4, int n12, int n23,
                                           int n34, int n41, int ID)
{
...
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                           ...
                                           int ID)
{
    return AddFaceWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n12->GetID(),
                         n23->GetID(), n34->GetID(), n41->GetID(), ID);
}
                                     ...
                                     const SMDS_MeshNode *nCenter)
{
    ...
    if (anElem)
        myScript->AddFace(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                          n12->GetID(), n23->GetID(), n34->GetID(), n41->GetID(), nCenter->GetID());
    return anElem;
}
...
//=======================================================================
SMDS_MeshFace *SMESHDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n4, int n12, int n23,
                                           int n34, int n41, int nCenter, int ID)
{
    SMDS_MeshFace *anElem =
        SMDS_Mesh::AddFaceWithID(n1, n2, n3, n4, n12, n23, n34, n41, nCenter, ID);
...
{
    return AddFaceWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n12->GetID(),
                         n23->GetID(), n34->GetID(), n41->GetID(), nCenter->GetID(), ID);
}
                                         ...
                                         const SMDS_MeshNode *n24, const SMDS_MeshNode *n34)
{
    ...
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n12->GetID(), n23->GetID(), n31->GetID(), n14->GetID(), n24->GetID(),
                            n34->GetID());
    return anElem;
}
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n12, int n23,
...
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                               ...
                                               int ID)
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n12->GetID(),
                           n23->GetID(), n31->GetID(), n14->GetID(), n24->GetID(), n34->GetID(),
                           ID);
}
...
{
    SMDS_MeshVolume *anElem =
        SMDS_Mesh::AddVolume(n1, n2, n3, n4, n5, n12, n23, n34, n41, n15, n25, n35, n45);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n5->GetID(), n12->GetID(), n23->GetID(), n34->GetID(), n41->GetID(),
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n12,
                                               ...
                                               int n45, int ID)
{
    ...
    if (anElem) myScript->AddVolume(ID, n1, n2, n3, n4, n5, n12, n23, n34, n41, n15, n25, n35, n45);
    return anElem;
}
...
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(),
                           n12->GetID(), n23->GetID(), n34->GetID(), n41->GetID(), n15->GetID(),
                           n25->GetID(), n35->GetID(), n45->GetID(), ID);
}
...
{
    SMDS_MeshVolume *anElem =
        SMDS_Mesh::AddVolume(n1, n2, n3, n4, n5, n6, n12, n23, n31, n45, n56, n64, n14, n25, n36);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n5->GetID(), n6->GetID(), n12->GetID(), n23->GetID(), n31->GetID(),
                            n45->GetID(), n56->GetID(), n64->GetID(), n14->GetID(), n25->GetID(),
                            n36->GetID());
    return anElem;
}
...
{
    SMDS_MeshVolume *anElem = SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n6, n12, n23, n31, n45,
                                                         n56, n64, n14, n25, n36, ID);
    if (anElem)
        myScript->AddVolume(ID, n1, n2, n3, n4, n5, n6, n12, n23, n31, n45, n56, n64, n14, n25,
                            n36);
    return anElem;
}
...
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(),
                           n6->GetID(), n12->GetID(), n23->GetID(), n31->GetID(), n45->GetID(),
                           n56->GetID(), n64->GetID(), n14->GetID(), n25->GetID(), n36->GetID(),
                           ID);
}
...
{
    SMDS_MeshVolume *anElem = SMDS_Mesh::AddVolume(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34,
                                                   n41, n56, n67, n78, n85, n15, n26, n37, n48);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n5->GetID(), n6->GetID(), n7->GetID(), n8->GetID(), n12->GetID(),
                            n23->GetID(), n34->GetID(), n41->GetID(), n56->GetID(), n67->GetID(),
                            n78->GetID(), n85->GetID(), n15->GetID(), n26->GetID(), n37->GetID(),
                            n48->GetID());
    return anElem;
}
...
//=======================================================================
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6,
                                               int n7, int n8, int n12, int n23, int n34, int n41,
                                               int n56, int n67, int n78, int n85, int n15, int n26,
                                               int n37, int n48, int ID)
{
    SMDS_MeshVolume *anElem =
        SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67,
                                   n78, n85, n15, n26, n37, n48, ID);
    if (anElem)
        myScript->AddVolume(ID, n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67, n78,
                            n85, n15, n26, n37, n48);
    return anElem;
}
...
//=======================================================================
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(
...
{
    return AddVolumeWithID(n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(), n5->GetID(),
                           n6->GetID(), n7->GetID(), n8->GetID(), n12->GetID(), n23->GetID(),
                           n34->GetID(), n41->GetID(), n56->GetID(), n67->GetID(), n78->GetID(),
                           n85->GetID(), n15->GetID(), n26->GetID(), n37->GetID(), n48->GetID(),
                           ID);
}
...
SMDS_MeshVolume *SMESHDS_Mesh::AddVolume(
    ...
    const SMDS_MeshNode *n1458, const SMDS_MeshNode *n5678, const SMDS_MeshNode *nCenter)
{
    SMDS_MeshVolume *anElem =
        SMDS_Mesh::AddVolume(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67, n78, n85,
                             n15, n26, n37, n48, n1234, n1256, n2367, n3478, n1458, n5678, nCenter);
    if (anElem)
        myScript->AddVolume(anElem->GetID(), n1->GetID(), n2->GetID(), n3->GetID(), n4->GetID(),
                            n5->GetID(), n6->GetID(), n7->GetID(), n8->GetID(), n12->GetID(),
                            n23->GetID(), n34->GetID(), n41->GetID(), n56->GetID(), n67->GetID(),
                            n78->GetID(), n85->GetID(), n15->GetID(), n26->GetID(), n37->GetID(),
                            n48->GetID(), n1234->GetID(), n1256->GetID(), n2367->GetID(),
                            n3478->GetID(), n1458->GetID(), n5678->GetID(), nCenter->GetID());
    return anElem;
}
...
SMDS_MeshVolume *SMESHDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6,
                                               int n7, int n8, int n12, int n23, int n34, int n41,
                                               int n56, int n67, int n78, int n85, int n15, int n26,
                                               int n37, int n48, int n1234, int n1256, int n2367,
                                               int n3478, int n1458, int n5678, int nCenter, int ID)
{
    SMDS_MeshVolume *anElem = SMDS_Mesh::AddVolumeWithID(
        n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67, n78, n85, n15, n26, n37, n48,
        n1234, n1256, n2367, n3478, n1458, n5678, nCenter, ID);
    if (anElem)
        myScript->AddVolume(ID, n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67, n78,
                            n85, n15, n26, n37, n48, n1234, n1256, n2367, n3478, n1458, n5678,
                            nCenter);
    return anElem;
}
...
void SMESHDS_Mesh::compactMesh()
{
    ...
    SMESHDS_SubMeshIteratorPtr smIt = SubMeshes();
    while (SMESHDS_SubMesh *sm = const_cast<SMESHDS_SubMesh *>(smIt->next())) sm->compactList();
}

void SMESHDS_Mesh::CleanDownWardConnectivity() { myGrid->CleanDownwardConnectivity(); }

    ...
    : _grid(grid), _nbDownCells(nbDownCells)
{
    ...
    if (_cellDimension.empty()) getCellDimension(VTK_LINE);
}
...
{
    int localId = -1;
    if (vtkId >= 0) localId = _grid->CellIdToDownId(vtkId);
 ...
 * @return
 */
int SMDS_Downward::getNumberOfDownCells(int cellId) { return _nbDownCells; }
 ...
 */
const unsigned char *SMDS_Downward::getDownTypes(int cellId) { return &_cellTypes[0]; }
...
int SMDS_Downward::getNodeSet(int cellId, int *nodeSet) { return 0; }
...
void SMDS_Down1D::compactStorage()
{
    ...
    int sizeUpCells = 0;
    for (int i = 0; i < _maxId; i++) sizeUpCells += _upCellIdsVector[i].size();
    _upCellIds.resize(sizeUpCells, -1);
    _upCellTypes.resize(sizeUpCells);
    _upCellIndex.resize(
        _maxId + 1,
        -1); // id and types of rank i correspond to [ _upCellIndex[i], _upCellIndex[i+1] [
    int current = 0;
...
{
    for (int i = 0; i < _nbDownCells; i++) nodeSet.insert(_cellIds[_nbDownCells * cellId + i]);
}

int SMDS_Down1D::getNodeSet(int cellId, int *nodeSet)
{
    for (int i = 0; i < _nbDownCells; i++) nodeSet[i] = _cellIds[_nbDownCells * cellId + i];
    return _nbDownCells;
}
...
void SMDS_Down1D::setNodes(int cellId, int vtkId)
{
    ...
    for (int i = 0; i < npts; i++) { _cellIds[_nbDownCells * cellId + i] = pts[i]; }
}

void SMDS_Down1D::setNodes(int cellId, const int *nodeIds)
{
    //ASSERT(nodeIds.size() == _nbDownCells);
    for (int i = 0; i < _nbDownCells; i++) { _cellIds[_nbDownCells * cellId + i] = nodeIds[i]; }
}
 ...
 */
int SMDS_Down1D::computeFaces(int cellId, int *vtkIds, int nbcells, int *downFaces,
 ...
 * @return number of downward faces
 */
int SMDS_Down1D::computeFaces(int *pts, int *vtkIds, int nbcells, int *downFaces,
                              unsigned char *downTypes)
{
    int cnt = 0;
    for (int i = 0; i < nbcells; i++) {
        ...
        if (SMDS_Downward::getCellDimension(vtkType) == 2) {
            ...
            for (int n = 0; n < nbFaces; n++) {
                SMDS_Down2D *downFace =
                    static_cast<SMDS_Down2D *>(_grid->getDownArray(downTypesVol[n]));
...
{
    int nbup = 0;
    if (_upCellIds[2 * cellId] >= 0) nbup++;
    if (_upCellIds[2 * cellId + 1] >= 0) nbup++;
    return nbup;
}
...
int SMDS_Down2D::computeVolumeIds(int cellId, int *ids)
{
    ...
    vector<int> nodes;
    for (int i = 0; i < npts; i++) nodes.push_back(pts[i]);
...
{
    int nvol =
        this->computeVolumeIdsFromNodesFace(&faceByNodes.nodeIds[0], faceByNodes.nbNodes, ids);
    return nvol;
}
...
void SMDS_Down2D::setTempNodes(int cellId, int vtkId)
{
    ...
    for (int i = 0; i < npts; i++) { _tempNodes[_nbNodes * cellId + i] = pts[i]; }
}
...
{
    for (int i = 0; i < _nbNodes; i++) nodeSet[i] = _tempNodes[_nbNodes * cellId + i];
    return _nbNodes;
}
...
int SMDS_Down2D::FindEdgeByNodes(int cellId, ElemByNodesType &edgeByNodes)
{
    ...
    for (int i = 0; i < _nbDownCells; i++) {
        if ((edges[i] >= 0) && (edgeByNodes.vtkType == _cellTypes[i])) {
            int nodeSet[3];
            int npts =
                this->_grid->getDownArray(edgeByNodes.vtkType)->getNodeSet(edges[i], nodeSet);
            bool found = false;
            for (int j = 0; j < npts; j++) {
...
int SMDS_Down3D::getNumberOfUpCells(int cellId) { return 0; }
...
void SMDS_Down3D::getNodeIds(int cellId, std::set<int> &nodeSet)
{
    ...
    for (int i = 0; i < npts; i++) nodeSet.insert(nodes[i]);
}
...
int SMDS_Down3D::FindFaceByNodes(int cellId, ElemByNodesType &faceByNodes)
{
    ...
    for (int i = 0; i < _nbDownCells; i++) {
        if ((faces[i] >= 0) && (faceByNodes.vtkType == _cellTypes[i])) {
            if (npoints == 0) npoints = faceByNodes.nbNodes;

            int nodeSet[10];
            int npts =
                this->_grid->getDownArray(faceByNodes.vtkType)->getNodeSet(faces[i], nodeSet);
            if (npts != npoints) continue; // skip this face
            bool found = false;
            for (int j = 0; j < npts; j++) {
                ...
                if (!found) break; // point j is not in the 2 faces, skip the remaining tests
            }
            if (found) return faces[i];
        }
    }
    return -1;
}
...
SMDS_DownEdge::SMDS_DownEdge(SMDS_UnstructuredGrid *grid) : SMDS_Down1D(grid, 2)
{
...
SMDS_DownQuadEdge::SMDS_DownQuadEdge(SMDS_UnstructuredGrid *grid) : SMDS_Down1D(grid, 3)
{
    _cellTypes.push_back(VTK_VERTEX);
    _cellTypes.push_back(VTK_VERTEX);
    _cellTypes.push_back(VTK_VERTEX);
...
SMDS_DownTriangle::SMDS_DownTriangle(SMDS_UnstructuredGrid *grid) : SMDS_Down2D(grid, 3)
{
...
SMDS_DownQuadTriangle::SMDS_DownQuadTriangle(SMDS_UnstructuredGrid *grid) : SMDS_Down2D(grid, 3)
{
...
SMDS_DownQuadrangle::SMDS_DownQuadrangle(SMDS_UnstructuredGrid *grid) : SMDS_Down2D(grid, 4)
{
    _cellTypes.push_back(VTK_LINE);
    _cellTypes.push_back(VTK_LINE);
    _cellTypes.push_back(VTK_LINE);
...
SMDS_DownQuadQuadrangle::SMDS_DownQuadQuadrangle(SMDS_UnstructuredGrid *grid) : SMDS_Down2D(grid, 4)
{
    _cellTypes.push_back(VTK_QUADRATIC_EDGE);
    _cellTypes.push_back(VTK_QUADRATIC_EDGE);
    _cellTypes.push_back(VTK_QUADRATIC_EDGE);
...
SMDS_DownTetra::SMDS_DownTetra(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 4)
{
...
void SMDS_DownTetra::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    set<int> setNodes;
    setNodes.clear();
    for (int i = 0; i < orderedNodes.size(); i++) setNodes.insert(orderedNodes[i]);
    ...
    for (int k = 0; k < 4; k++) {
        tofind.clear();
        for (int i = 0; i < 3; i++) tofind.insert(nodes[ids[3 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 3; i++) orderedNodes[i] = nodes[ids[3 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
                                  << _grid->_mesh->fromVtkToSmds(_vtkCellIds[cellId]));
...
SMDS_DownQuadTetra::SMDS_DownQuadTetra(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 4)
{
...
void SMDS_DownQuadTetra::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    for (int k = 0; k < 4; k++) {
        tofind.clear();
        for (int i = 0; i < 6; i++) tofind.insert(nodes[ids[6 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 6; i++) orderedNodes[i] = nodes[ids[6 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownPyramid::SMDS_DownPyramid(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 5)
{
...
void SMDS_DownPyramid::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    // Quadrangular face
    tofind.clear();
    for (int i = 0; i < 4; i++) tofind.insert(nodes[ids[i]]);
    if (setNodes == tofind) {
        for (int i = 0; i < 4; i++) orderedNodes[i] = nodes[ids[i]];
        return;
    }
    // Triangular faces
    for (int k = 0; k < 4; k++) {
        tofind.clear();
        for (int i = 0; i < 3; i++) tofind.insert(nodes[ids[4 + 3 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 3; i++) orderedNodes[i] = nodes[ids[4 + 3 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownQuadPyramid::SMDS_DownQuadPyramid(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 5)
{
...
void SMDS_DownQuadPyramid::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    set<int> tofind;
    int ids[32] = {0, 1, 2,  3,  5, 6, 7, 8, 0,  3,  4, 8, 12, 9, 3, 2,
                   4, 7, 11, 12, 2, 1, 4, 6, 10, 11, 1, 0, 4,  5, 9, 10};

    // Quadrangular face
    tofind.clear();
    for (int i = 0; i < 8; i++) tofind.insert(nodes[ids[i]]);
    if (setNodes == tofind) {
        for (int i = 0; i < 8; i++) orderedNodes[i] = nodes[ids[i]];
        return;
    }
    // Triangular faces
    for (int k = 0; k < 4; k++) {
        tofind.clear();
        for (int i = 0; i < 6; i++) tofind.insert(nodes[ids[8 + 6 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 6; i++) orderedNodes[i] = nodes[ids[8 + 6 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownPenta::SMDS_DownPenta(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 5)
{
    _cellTypes.push_back(VTK_QUAD);
    _cellTypes.push_back(VTK_QUAD);
    _cellTypes.push_back(VTK_QUAD);
...
void SMDS_DownPenta::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    for (int k = 0; k < 3; k++) {
        tofind.clear();
        for (int i = 0; i < 4; i++) tofind.insert(nodes[ids[6 + 4 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 4; i++) orderedNodes[i] = nodes[ids[6 + 4 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownQuadPenta::SMDS_DownQuadPenta(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 5)
{
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
...
void SMDS_DownQuadPenta::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    set<int> tofind;
    //int ids[18] = { 0, 2, 1,  3, 4, 5,   0, 1, 4, 3,   1, 2, 5, 4,   2, 0, 3, 5 };
    int ids[36] = {0,  1, 2, 6, 7, 8, 3,  5,  4,  11, 10, 9, 0, 3, 4,  1,  12, 9,
                   13, 6, 1, 4, 5, 2, 13, 10, 14, 7,  2,  5, 3, 0, 14, 11, 12, 8};
    ...
    for (int k = 0; k < 3; k++) {
        tofind.clear();
        for (int i = 0; i < 8; i++) tofind.insert(nodes[ids[12 + 8 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 8; i++) orderedNodes[i] = nodes[ids[12 + 8 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownHexa::SMDS_DownHexa(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 6)
{
    _cellTypes.push_back(VTK_QUAD);
    _cellTypes.push_back(VTK_QUAD);
    _cellTypes.push_back(VTK_QUAD);
...
void SMDS_DownHexa::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    {
        tofind.clear();
        for (int i = 0; i < 4; i++) tofind.insert(nodes[ids[4 * k + i]]); // node ids of the face i
        if (setNodes == tofind) {
            for (int i = 0; i < 4; i++) orderedNodes[i] = nodes[ids[4 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "
...
SMDS_DownQuadHexa::SMDS_DownQuadHexa(SMDS_UnstructuredGrid *grid) : SMDS_Down3D(grid, 6)
{
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
    _cellTypes.push_back(VTK_QUADRATIC_QUAD);
...
void SMDS_DownQuadHexa::getOrderedNodesOfFace(int cellId, std::vector<vtkIdType> &orderedNodes)
{
    ...
    set<int> tofind;
    //int ids[24] = { 3, 2, 1, 0,  4, 5, 6, 7,  7, 3, 0, 4,  4, 0, 1, 5,  5, 1, 2, 6,  6, 2, 3, 7};
    int ids[48] = {3, 2, 1, 0, 10, 9,  8,  11, 4, 5, 6, 7, 12, 13, 14, 15,
                   7, 3, 0, 4, 19, 11, 16, 15, 4, 0, 1, 5, 16, 8,  17, 12,
                   5, 1, 2, 6, 17, 9,  18, 13, 6, 2, 3, 7, 18, 10, 19, 14};
    for (int k = 0; k < 6; k++) {
        tofind.clear();
        for (int i = 0; i < 8; i++) tofind.insert(nodes[ids[8 * k + i]]);
        if (setNodes == tofind) {
            for (int i = 0; i < 8; i++) orderedNodes[i] = nodes[ids[8 * k + i]];
            return;
        }
    }
    MESSAGE("=== Problem volume " << _vtkCellIds[cellId] << " "

...
void SMDS_VtkFace::init(const std::vector<vtkIdType> &nodeIds, SMDS_Mesh *mesh)
{
    ...
    switch (nodeIds.size()) {
        ...
        default: aType = VTK_POLYGON; break;
    }
...
void SMDS_VtkFace::initQuadPoly(const std::vector<vtkIdType> &nodeIds, SMDS_Mesh *mesh)
{
    ...
    myMeshId = mesh->getMeshId();
    myVtkID =
        grid->InsertNextLinkedCell(VTK_QUADRATIC_POLYGON, nodeIds.size(), (vtkIdType *)&nodeIds[0]);
    mesh->setMyModified();
#else
...
void SMDS_VtkFace::Print(std::ostream &OS) const { OS << "face <" << GetID() << "> : "; }
...
int SMDS_VtkFace::NbEdges() const
{
    ...
    switch (aVtkType) {
        ...
        case VTK_BIQUADRATIC_TRIANGLE: nbEdges = 3; break;
        case VTK_QUAD:
        case VTK_QUADRATIC_QUAD:
        case VTK_BIQUADRATIC_QUAD: nbEdges = 4; break;
#ifndef VTK_NO_QUAD_POLY
...
#endif
        case VTK_POLYGON:
        default: nbEdges = grid->GetCell(myVtkID)->GetNumberOfPoints(); break;
    }
    return nbEdges;
}

int SMDS_VtkFace::NbFaces() const { return 1; }
 ...
 */
const SMDS_MeshNode *SMDS_VtkFace::GetNode(const int ind) const
{
    vtkUnstructuredGrid *grid = SMDS_Mesh::_meshList[myMeshId]->getGrid();
    vtkIdType npts;
    vtkIdTypePtr pts;
...
int SMDS_VtkFace::GetNodeIndex(const SMDS_MeshNode *node) const
{
    ...
    for (vtkIdType i = 0; i < npts; ++i)
        if (pts[i] == node->getVtkId()) return i;
    return -1;
}
...
bool SMDS_VtkFace::IsQuadratic() const
{
    ...
    switch (aVtkType) {
        ...
        case VTK_BIQUADRATIC_TRIANGLE: return true; break;
...
#ifndef VTK_NO_QUAD_POLY
        case VTK_QUADRATIC_POLYGON: nbPoints /= 2; break;
#endif
        default:
            if (nbPoints > 4) nbPoints /= 2;
    }
    return nbPoints;
}
...
SMDS_NodeIteratorPtr SMDS_VtkFace::interlacedNodesIterator() const { return nodesIteratorToUNV(); }
...
void SMDS_VtkFace::ChangeApex(SMDS_MeshNode *node)
{
    ...
    pts[0] = node->getVtkId();
#endif
    node->AddInverseElement(this), SMDS_Mesh::_meshList[myMeshId]->setMyModified();
}

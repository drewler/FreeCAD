...
SMDS_LinearEdge::SMDS_LinearEdge(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2)
{
...
{
    OS << "edge <" << GetID() << "> : (" << myNodes[0] << " , " << myNodes[1] << ") " << endl;
}

int SMDS_LinearEdge::NbNodes() const { return 2; }

int SMDS_LinearEdge::NbEdges() const { return 1; }
...
public:
    SMDS_LinearEdge_MyNodeIterator(const SMDS_MeshNode *const *nodes) : myNodes(nodes), myIndex(0)
    {}

    bool more() { return myIndex < 2; }
...
SMDS_ElemIteratorPtr SMDS_LinearEdge::elementsIterator(SMDSAbs_ElementType type) const
{
    switch (type) {
        ...
        case SMDSAbs_Node: return SMDS_ElemIteratorPtr(new SMDS_LinearEdge_MyNodeIterator(myNodes));
        default:
 ...
 */
const SMDS_MeshNode *SMDS_LinearEdge::GetNode(const int ind) const { return myNodes[ind]; }
...
bool SMDS_LinearEdge::ChangeNodes(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2)
{
    myNodes[0] = node1;
    myNodes[1] = node2;
    return true;

...
///////////////////////////////////////////////////////////////////////////////
SMDS_Mesh::SMDS_Mesh()
    : myParent(NULL), myNodeIDFactory(new SMDS_MeshNodeIDFactory()),
      myElementIDFactory(new SMDS_MeshElementIDFactory()), myHasConstructionEdges(false),
      myHasConstructionFaces(false), myHasInverseElements(true), myNodeMin(0), myNodeMax(0),
      myNodePool(0), myEdgePool(0), myFacePool(0), myVolumePool(0), myBallPool(0),
      myModified(false), myModifTime(0), myCompactTime(0), xmin(0), xmax(0), ymin(0), ymax(0),
      zmin(0), zmax(0)
{
...
SMDS_Mesh::SMDS_Mesh(SMDS_Mesh *parent)
    : myParent(parent), myNodeIDFactory(parent->myNodeIDFactory),
      myElementIDFactory(parent->myElementIDFactory), myHasConstructionEdges(false),
      myHasConstructionFaces(false), myHasInverseElements(true), myNodePool(parent->myNodePool),
      myEdgePool(parent->myEdgePool), myFacePool(parent->myFacePool),
      myVolumePool(parent->myVolumePool), myBallPool(parent->myBallPool)
{}
...
SMDS_MeshEdge *SMDS_Mesh::AddEdge(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2)
{
...
SMDS_MeshEdge *SMDS_Mesh::AddEdgeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, int ID)
{
    if (!n1 || !n2) return 0;
...
SMDS_MeshFace *SMDS_Mesh::AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(int idnode1, int idnode2, int idnode3, int idnode4, int ID)
{
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n3, const SMDS_MeshNode *n4, int ID)
{
    //MESSAGE("AddFaceWithID " << ID);
...
SMDS_MeshFace *SMDS_Mesh::AddFace(const SMDS_MeshEdge *e1, const SMDS_MeshEdge *e2,
                                  const SMDS_MeshEdge *e3)
{
    if (!hasConstructionEdges()) return NULL;
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshEdge *e1, const SMDS_MeshEdge *e2,
...
SMDS_MeshFace *SMDS_Mesh::AddFace(const SMDS_MeshEdge *e1, const SMDS_MeshEdge *e2,
                                  const SMDS_MeshEdge *e3, const SMDS_MeshEdge *e4)
{
    if (!hasConstructionEdges()) return NULL;
    //MESSAGE("AddFaceWithID" );
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshEdge *e1, const SMDS_MeshEdge *e2,
                                        const SMDS_MeshEdge *e3, const SMDS_MeshEdge *e4, int ID)
{
    if (!hasConstructionEdges()) return NULL;
    MESSAGE("AddFaceWithID" << ID);
...
SMDS_MeshVolume *SMDS_Mesh::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                      const SMDS_MeshNode *n3, const SMDS_MeshNode *n4)
{
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int idnode1, int idnode2, int idnode3, int idnode4,
                                            int ID)
{
    //MESSAGE("AddVolumeWithID" << ID);
    SMDS_MeshNode *node1, *node2, *node3, *node4;
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                            const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                            int ID)
{
    //MESSAGE("AddVolumeWithID " << ID);
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int idnode1, int idnode2, int idnode3, int idnode4,
                                            int idnode5, int idnode6, int ID)
{
    //MESSAGE("AddVolumeWithID " << ID);
                                      ...
                                      const SMDS_MeshNode *n11, const SMDS_MeshNode *n12)
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v =
        SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, ID);
    if (v == NULL) myElementIDFactory->ReleaseID(ID);
    return v;
}
                                            ...
                                            int ID)
{
    ...
    return SMDS_Mesh::AddVolumeWithID(node1, node2, node3, node4, node5, node6, node7, node8, node9,
                                      node10, node11, node12, ID);
}
                                            ...
                                            int ID)
{
    ...
    if (!node1 || !node2 || !node3 || !node4 || !node5 || !node6 || !node7 || !node8) return NULL;
    return SMDS_Mesh::AddVolumeWithID(node1, node2, node3, node4, node5, node6, node7, node8, ID);
}
...
SMDS_MeshVolume *SMDS_Mesh::AddVolume(const SMDS_MeshFace *f1, const SMDS_MeshFace *f2,
                                      const SMDS_MeshFace *f3, const SMDS_MeshFace *f4)
{
    //MESSAGE("AddVolumeWithID");
    if (!hasConstructionFaces()) return NULL;
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(const SMDS_MeshFace *f1, const SMDS_MeshFace *f2,
                                            const SMDS_MeshFace *f3, const SMDS_MeshFace *f4,
                                            int ID)
{
    MESSAGE("AddVolumeWithID" << ID);
...
SMDS_MeshVolume *SMDS_Mesh::AddVolume(const SMDS_MeshFace *f1, const SMDS_MeshFace *f2,
                                      const SMDS_MeshFace *f3, const SMDS_MeshFace *f4,
                                      const SMDS_MeshFace *f5, const SMDS_MeshFace *f6)
{
    //MESSAGE("AddVolumeWithID" );
    if (!hasConstructionFaces()) return NULL;
...
SMDS_MeshFace *SMDS_Mesh::AddPolygonalFaceWithID(const vector<int> &nodes_ids, const int ID)
{
...
SMDS_MeshFace *SMDS_Mesh::AddPolygonalFaceWithID(const vector<const SMDS_MeshNode *> &nodes,
                                                 const int ID)
{
    SMDS_MeshFace *face;
    ...
    if (hasConstructionEdges()) {
        ...
        for (size_t i = 0; i < nodes.size(); ++i) myNodeIds[i] = nodes[i]->getVtkId();
...
SMDS_MeshFace *SMDS_Mesh::AddQuadPolygonalFaceWithID(const vector<int> &nodes_ids, const int ID)
{
...
SMDS_MeshFace *SMDS_Mesh::AddQuadPolygonalFaceWithID(const vector<const SMDS_MeshNode *> &nodes,
                                                     const int ID)
{
    SMDS_MeshFace *face;
...
SMDS_MeshVolume *SMDS_Mesh::AddPolyhedralVolumeWithID(const vector<int> &nodes_ids,
                                                      const vector<int> &quantities, const int ID)
{
    int nbNodes = nodes_ids.size();
    vector<const SMDS_MeshNode *> nodes(nbNodes);
    for (int i = 0; i < nbNodes; i++) {
...
SMDS_MeshVolume *SMDS_Mesh::AddPolyhedralVolumeWithID(const vector<const SMDS_MeshNode *> &nodes,
                                                      const vector<int> &quantities, const int ID)
{
    SMDS_MeshVolume *volume = 0;
    if (nodes.empty() || quantities.empty()) return NULL;
    if (NbVolumes() % CHECKMEMORY_INTERVAL == 0) CheckMemory();
    if (hasConstructionFaces()) {
        MESSAGE("Error : Not implemented");
        return NULL;
...
SMDS_MeshVolume *SMDS_Mesh::AddPolyhedralVolume(const vector<const SMDS_MeshNode *> &nodes,
                                                      ...
                                                      const int ID)
{
    ...
    switch (aVtkType) {
        case VTK_TETRA: myInfo.myNbTetras++; break;
        case VTK_PYRAMID: myInfo.myNbPyramids++; break;
        case VTK_WEDGE: myInfo.myNbPrisms++; break;
        case VTK_HEXAHEDRON: myInfo.myNbHexas++; break;
        case VTK_QUADRATIC_TETRA: myInfo.myNbQuadTetras++; break;
        case VTK_QUADRATIC_PYRAMID: myInfo.myNbQuadPyramids++; break;
        case VTK_QUADRATIC_WEDGE: myInfo.myNbQuadPrisms++; break;
        ...
        case VTK_POLYHEDRON:
            ...
            break;
            //#endif
        default: myInfo.myNbPolyhedrons++; break;
    }
    return volvtk;
}
                                                  ...
                                                  const int ID)
{
    ...
    switch (aVtkType) {
        case VTK_TRIANGLE: myInfo.myNbTriangles++; break;
        case VTK_QUAD: myInfo.myNbQuadrangles++; break;
        case VTK_QUADRATIC_TRIANGLE: myInfo.myNbQuadTriangles++; break;
        case VTK_QUADRATIC_QUAD: myInfo.myNbQuadQuadrangles++; break;
        case VTK_BIQUADRATIC_QUAD: myInfo.myNbBiQuadQuadrangles++; break;
        case VTK_BIQUADRATIC_TRIANGLE: myInfo.myNbBiQuadTriangles++; break;
        case VTK_POLYGON: myInfo.myNbPolygons++; break;
        default: myInfo.myNbPolygons++;
    }
    return facevtk;
}
...
bool SMDS_Mesh::registerElement(int ID, SMDS_MeshElement *element)
{
    ...
    if (vtkId == -1) vtkId = myElementIDFactory->SetInVtkGrid(element);
...
///////////////////////////////////////////////////////////////////////////////
SMDS_MeshFace *SMDS_Mesh::createTriangle(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2,
                                         const SMDS_MeshNode *node3, int ID)
{
...
///////////////////////////////////////////////////////////////////////////////
SMDS_MeshFace *SMDS_Mesh::createQuadrangle(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2,
...
bool SMDS_Mesh::ChangeElementNodes(const SMDS_MeshElement *element, const SMDS_MeshNode *nodes[],
                                   const int nbnodes)
{
                                      ...
                                      const vector<int> &quantities)
{
    ...
    while (itn->more()) { oldNodes.insert(itn->next()); }
    ...
    // RemoveInverseElement from the nodes removed from elem
    for (it = oldNodes.begin(); it != oldNodes.end(); it++) {
        SMDS_MeshNode *n = static_cast<SMDS_MeshNode *>(const_cast<SMDS_MeshElement *>(*it));
...
const SMDS_Mesh0DElement *SMDS_Mesh::Find0DElement(const SMDS_MeshNode *node)
{
    ...
    while (it1->more() && (toReturn == NULL)) {
        ...
        if (e->NbNodes() == 1) { toReturn = static_cast<const SMDS_Mesh0DElement *>(e); }
    }
    return toReturn;
}
...
const SMDS_BallElement *SMDS_Mesh::FindBall(const SMDS_MeshNode *node)
{
    ...
    while (it1->more() && (toReturn == NULL)) {
        const SMDS_MeshElement *e = it1->next();
        if (e->GetGeomType() == SMDSGeom_BALL) toReturn = static_cast<const SMDS_BallElement *>(e);
    }
    return toReturn;
}
...
//#include "Profiler.h"
const SMDS_MeshEdge *SMDS_Mesh::FindEdge(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2)
{
    if (!node1) return 0;
...
SMDS_MeshEdge *SMDS_Mesh::FindEdgeOrCreate(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2)
{
...
const SMDS_MeshEdge *SMDS_Mesh::FindEdge(int idnode1, int idnode2, int idnode3) const
{
    const SMDS_MeshNode *node1 = FindNode(idnode1);
    const SMDS_MeshNode *node2 = FindNode(idnode2);
                                         ...
                                         const SMDS_MeshNode *node3)
{
    ...
    while (it1->more()) {
        ...
        if (e->NbNodes() == 3) {
            ...
            if (e) return static_cast<const SMDS_MeshEdge *>(e);
        }
    }
    return 0;
}
...
const SMDS_MeshFace *SMDS_Mesh::FindFace(int idnode1, int idnode2, int idnode3) const
{
    const SMDS_MeshNode *node1 = FindNode(idnode1);
    const SMDS_MeshNode *node2 = FindNode(idnode2);
    const SMDS_MeshNode *node3 = FindNode(idnode3);
...
const SMDS_MeshFace *SMDS_Mesh::FindFace(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2,
                                         const SMDS_MeshNode *node3)
{
    if (!node1) return 0;
    ...
    while (it1->more()) {
        ...
        if (e->NbNodes() == 3) {
            ...
            if (e) return static_cast<const SMDS_MeshFace *>(e);
        }
    }
    return 0;
}

SMDS_MeshFace *SMDS_Mesh::FindFaceOrCreate(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2,
                                           const SMDS_MeshNode *node3)
{
...
const SMDS_MeshFace *SMDS_Mesh::FindFace(int idnode1, int idnode2, int idnode3, int idnode4) const
{
    const SMDS_MeshNode *node1 = FindNode(idnode1);
    const SMDS_MeshNode *node2 = FindNode(idnode2);
    const SMDS_MeshNode *node3 = FindNode(idnode3);
                                         ...
                                         const SMDS_MeshNode *node3, const SMDS_MeshNode *node4)
{
    ...
    while (it1->more()) {
        ...
        if (e->NbNodes() == 4) {
            ...
            while (it2->more()) {
                const SMDS_MeshElement *n = it2->next();
                if (n != node1 && n != node2 && n != node3 && n != node4) {
                    e = 0;
                    break;
                }
            }
...
const SMDS_MeshFace *SMDS_Mesh::FindFace(int idnode1, int idnode2, int idnode3, int idnode4,
                                         ...
                                         const SMDS_MeshNode *node5, const SMDS_MeshNode *node6)
{
    ...
    while (it1->more()) {
        ...
        if (e->NbNodes() == 6) {
            ...
            while (it2->more()) {
                const SMDS_MeshElement *n = it2->next();
                if (n != node1 && n != node2 && n != node3 && n != node4 && n != node5
                    && n != node6) {
                    e = 0;
                    break;
                }
                                         ...
                                         const SMDS_MeshNode *node7, const SMDS_MeshNode *node8)
{
    ...
    while (it1->more()) {
        ...
        if (e->NbNodes() == 8) {
            ...
            while (it2->more()) {
                const SMDS_MeshElement *n = it2->next();
                if (n != node1 && n != node2 && n != node3 && n != node4 && n != node5 && n != node6
                    && n != node7 && n != node8) {
                    e = 0;
                    break;
                }
...
const SMDS_MeshElement *SMDS_Mesh::FindElement(const vector<const SMDS_MeshNode *> &nodes,
                                               const SMDSAbs_ElementType type, const bool noMedium)
{
        ...
        while (itF->more()) {
            ...
            if (nbNodesToCheck == nodes.size()) {
                for (size_t i = 1; e && i < nodes.size(); ++i) {
                    ...
                    if (nodeIndex < 0 || nodeIndex >= nbNodesToCheck) e = 0;
                }
                if (e) return e;
            }
        }
...
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::Nb0DElements() const { return myInfo.Nb0DElements(); }

///////////////////////////////////////////////////////////////////////////////
/// Return the number of 0D elements
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::NbBalls() const { return myInfo.NbBalls(); }

///////////////////////////////////////////////////////////////////////////////
/// Return the number of edges (including construction edges)
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::NbEdges() const { return myInfo.NbEdges(); }

///////////////////////////////////////////////////////////////////////////////
/// Return the number of faces (including construction faces)
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::NbFaces() const { return myInfo.NbFaces(); }

///////////////////////////////////////////////////////////////////////////////
/// Return the number of volumes
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::NbVolumes() const { return myInfo.NbVolumes(); }

///////////////////////////////////////////////////////////////////////////////
/// Return the number of child mesh of this mesh.
/// Note that the tree structure of SMDS_Mesh is unused in SMESH
///////////////////////////////////////////////////////////////////////////////
int SMDS_Mesh::NbSubMesh() const { return myChildren.size(); }
...
public:
    IdSortedIterator(SMDS_MeshElementIDFactory &fact,
                     ...
                     const int totalNb)
        : myIDFact(fact), myID(1), myMaxID(myIDFact.GetMaxID()), myNbFound(0), myTotalNb(totalNb),
          myType(type), myElem(0)
    {
        next();
    }
    bool more() { return myElem; }
    ELEM next()
    {
        ELEM current = myElem;

        for (myElem = 0; !myElem && myNbFound < myTotalNb && myID <= myMaxID; ++myID)
            if ((myElem = (ELEM)myIDFact.MeshElement(myID)) && myElem->GetType() != myType)
                myElem = 0;
...
template<typename RETURN_VALUE, typename VECTOR_VALUE = SMDS_MeshCell *,
...
public:
    ...
    {
        if (_more && !_filter(_vector[_index])) next();
    }
    virtual bool more() { return _more; }
    virtual RETURN_VALUE next()
    {
        if (!_more) return NULL;
        VECTOR_VALUE current = _vector[_index];
        _more = 0;
        while (!_more && ++_index < _vector.size()) _more = _filter(_vector[_index]);
...
{
    // naturally always sorted by ID
    typedef ElemVecIterator<const SMDS_MeshElement *, SMDS_MeshCell *, SMDS_MeshElement::GeomFilter>
        TIterator;
    return SMDS_ElemIteratorPtr(new TIterator(myCells, SMDS_MeshElement::GeomFilter(type)));
}
...
SMDS_ElemIteratorPtr SMDS_Mesh::elementEntityIterator(SMDSAbs_EntityType type) const
{
    ...
    // naturally always sorted by ID
    typedef ElemVecIterator<const SMDS_MeshElement *, SMDS_MeshCell *,
                            SMDS_MeshElement::EntityFilter>
        TIterator;
    return SMDS_ElemIteratorPtr(new TIterator(myCells, SMDS_MeshElement::EntityFilter(type)));
}
...
{
    // naturally always sorted by ID
    typedef ElemVecIterator<const SMDS_MeshEdge *, SMDS_MeshCell *, SMDS_MeshElement::TypeFilter>
        TIterator;
    return SMDS_EdgeIteratorPtr(new TIterator(myCells, SMDS_MeshElement::TypeFilter(SMDSAbs_Edge)));
}
...
{
    // naturally always sorted by ID
    typedef ElemVecIterator<const SMDS_MeshFace *, SMDS_MeshCell *, SMDS_MeshElement::TypeFilter>
        TIterator;
    return SMDS_FaceIteratorPtr(new TIterator(myCells, SMDS_MeshElement::TypeFilter(SMDSAbs_Face)));
}
...
{
    // naturally always sorted by ID
    typedef ElemVecIterator<const SMDS_MeshVolume *, SMDS_MeshCell *, SMDS_MeshElement::TypeFilter>
        TIterator;
                                                         ...
                                                         int numberOfSets)
{
    ...
    for (int i = 0; i < numberOfSets - 1; i++) {
        ...
        set_intersection(rsetA->begin(), rsetA->end(), vs[i + 1].begin(), vs[i + 1].end(),
                                     ...
                                     set<const SMDS_MeshElement *> &nodes)
{
    switch (element->GetType()) {
        case SMDSAbs_Node: MESSAGE("Internal Error: This should not happen"); break;
        case SMDSAbs_0DElement: {
        } break;
        case SMDSAbs_Edge: {
        ...
        case SMDSAbs_Face: {
            ...
            if (hasConstructionEdges()) {
                ...
                while (ite->more()) addChildrenWithNodes(setOfChildren, ite->next(), nodes);
            }
        } break;
        case SMDSAbs_Volume: {
            if (hasConstructionFaces()) {
...
///////////////////////////////////////////////////////////////////////////////
void SMDS_Mesh::RemoveElement(const SMDS_MeshElement *elem, const bool removenodes)
{
...
void SMDS_Mesh::RemoveElement(const SMDS_MeshElement *elem,
                              list<const SMDS_MeshElement *> &removedElems,
                              list<const SMDS_MeshElement *> &removedNodes, bool removenodes)
{
    ...
    while (it != s3.end()) {
        ...
        while (itn->more()) {
            SMDS_MeshNode *n =
                static_cast<SMDS_MeshNode *>(const_cast<SMDS_MeshElement *>(itn->next()));
        ...
        switch ((*it)->GetType()) {
            case SMDSAbs_Node: MYASSERT("Internal Error: This should not happen"); break;
            case SMDSAbs_0DElement:
                if (IdToRemove >= 0) {
...
bool SMDS_Mesh::Contains(const SMDS_MeshElement *elem) const
{
    ...
    SMDS_NodeIteratorPtr itn = nodesIterator();
    while (itn->more())
        if (elem == itn->next()) return true;
    SMDS_ElemIteratorPtr ite = elementsIterator();
    while (ite->more())
        if (elem == ite->next()) return true;
    return false;
}
...
int SMDS_Mesh::MaxNodeID() const { return myNodeMax; }
...
int SMDS_Mesh::MinNodeID() const { return myNodeMin; }
...
int SMDS_Mesh::MaxElementID() const { return myElementIDFactory->GetMaxID(); }
...
int SMDS_Mesh::MinElementID() const { return myElementIDFactory->GetMinID(); }
...
void SMDS_Mesh::Renumber(const bool isNodes, const int startID, const int deltaID)
{
    ...
    SMDS_MeshNodeIDFactory *idFactory = isNodes ? myNodeIDFactory : myElementIDFactory;
...
SMDSAbs_ElementType SMDS_Mesh::GetElementType(const int id, const bool iselem) const
{
    ...
    if (iselem) elem = myElementIDFactory->MeshElement(id);
    else
...
{
    return SMDS_Mesh::AddEdgeWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n12), ID);
}
...
//purpose  :
//=======================================================================
SMDS_MeshEdge *SMDS_Mesh::AddEdge(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
...
//purpose  :
//=======================================================================
SMDS_MeshEdge *SMDS_Mesh::AddEdgeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n12, int ID)
{
...
SMDS_MeshFace *SMDS_Mesh::AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                  ...
                                  const SMDS_MeshNode *n23, const SMDS_MeshNode *n31)
{
    return SMDS_Mesh::AddFaceWithID(n1, n2, n3, n12, n23, n31, myElementIDFactory->GetFreeID());
}
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n12, int n23, int n31, int ID)
{
    return SMDS_Mesh::AddFaceWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                    ...
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n31), ID);
}
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n3, const SMDS_MeshNode *n12,
                                        const SMDS_MeshNode *n23, const SMDS_MeshNode *n31, int ID)
{
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n12, int n23, int n31,
                                        int nCenter, int ID)
{
    return SMDS_Mesh::AddFaceWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                    ...
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(nCenter), ID);
}
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n3, const SMDS_MeshNode *n12,
                                        const SMDS_MeshNode *n23, const SMDS_MeshNode *n31,
                                        const SMDS_MeshNode *nCenter, int ID)
{
...
//purpose  :
//=======================================================================
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(int n1, int n2, int n3, int n4, int n12, int n23, int n34,
                                        int n41, int ID)
{
    return SMDS_Mesh::AddFaceWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                    ...
                                    (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n41), ID);
}
...
SMDS_MeshFace *SMDS_Mesh::AddFaceWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                                        const SMDS_MeshNode *n12, const SMDS_MeshNode *n23,
                                        const SMDS_MeshNode *n34, const SMDS_MeshNode *n41, int ID)
{
                                      ...
                                      const SMDS_MeshNode *n24, const SMDS_MeshNode *n34)
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v =
        SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n12, n23, n31, n14, n24, n34, ID);
    if (v == NULL) myElementIDFactory->ReleaseID(ID);
    return v;
}
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n12, int n23,
                                            int n31, int n14, int n24, int n34, int ID)
{
    return SMDS_Mesh::AddVolumeWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n3),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n4),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n12),
                                      ...
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n34), ID);
}
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                            ...
                                            int ID)
{
    if (!n1 || !n2 || !n3 || !n4 || !n12 || !n23 || !n31 || !n14 || !n24 || !n34) return 0;
    if (hasConstructionFaces()) {
                                      ...
                                      const SMDS_MeshNode *n45)
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v =
        SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n12, n23, n34, n41, n15, n25, n35, n45, ID);
    if (v == NULL) myElementIDFactory->ReleaseID(ID);
    return v;
}
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n12,
                                            int n23, int n34, int n41, int n15, int n25, int n35,
                                            int n45, int ID)
{
    return SMDS_Mesh::AddVolumeWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                      ...
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n45), ID);
}
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                            ...
                                            const SMDS_MeshNode *n41, const SMDS_MeshNode *n15,
                                            const SMDS_MeshNode *n25, const SMDS_MeshNode *n35,
                                            const SMDS_MeshNode *n45, int ID)
{
    if (!n1 || !n2 || !n3 || !n4 || !n5 || !n12 || !n23 || !n34 || !n41 || !n15 || !n25 || !n35
        || !n45)
        return 0;
    if (hasConstructionFaces()) {
        // creation quadratic faces - not implemented
...
//=======================================================================
SMDS_MeshVolume *
SMDS_Mesh::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const SMDS_MeshNode *n3,
...
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v = SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n6, n12, n23, n31, n45, n56,
                                                    n64, n14, n25, n36, ID);
    if (v == NULL) myElementIDFactory->ReleaseID(ID);
    return v;
}
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6, int n12,
                                            int n23, int n31, int n45, int n56, int n64, int n14,
                                            int n25, int n36, int ID)
{
    return SMDS_Mesh::AddVolumeWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n3),
                                      ...
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n14),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n25),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n36), ID);
}
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                            ...
                                            const SMDS_MeshNode *n36, int ID)
{
    if (!n1 || !n2 || !n3 || !n4 || !n5 || !n6 || !n12 || !n23 || !n31 || !n45 || !n56 || !n64
        || !n14 || !n25 || !n36)
        return 0;
    if (hasConstructionFaces()) {
        // creation quadratic faces - not implemented
                     ...
                     const SMDS_MeshNode *n37, const SMDS_MeshNode *n48)
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v =
        SMDS_Mesh::AddVolumeWithID(n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67,
...
//purpose  :
//=======================================================================
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6, int n7,
                                            int n8, int n12, int n23, int n34, int n41, int n56,
                                            int n67, int n78, int n85, int n15, int n26, int n37,
                                            int n48, int ID)
{
    return SMDS_Mesh::AddVolumeWithID((SMDS_MeshNode *)myNodeIDFactory->MeshElement(n1),
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n2),
                                      ...
                                      (SMDS_MeshNode *)myNodeIDFactory->MeshElement(n48), ID);
}
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(
    ...
    const SMDS_MeshNode *n56, const SMDS_MeshNode *n67, const SMDS_MeshNode *n78,
    const SMDS_MeshNode *n85, const SMDS_MeshNode *n15, const SMDS_MeshNode *n26,
    const SMDS_MeshNode *n37, const SMDS_MeshNode *n48, int ID)
{
    if (!n1 || !n2 || !n3 || !n4 || !n5 || !n6 || !n7 || !n8 || !n12 || !n23 || !n34 || !n41 || !n56
...
SMDS_Mesh::AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const SMDS_MeshNode *n3,
                     ...
                     const SMDS_MeshNode *n56, const SMDS_MeshNode *n67, const SMDS_MeshNode *n78,
                     const SMDS_MeshNode *n85, const SMDS_MeshNode *n15, const SMDS_MeshNode *n26,
                     const SMDS_MeshNode *n37, const SMDS_MeshNode *n48, const SMDS_MeshNode *n1234,
                     const SMDS_MeshNode *n1256, const SMDS_MeshNode *n2367,
                     const SMDS_MeshNode *n3478, const SMDS_MeshNode *n1458,
                     const SMDS_MeshNode *n5678, const SMDS_MeshNode *nCenter)
{
    int ID = myElementIDFactory->GetFreeID();
    SMDS_MeshVolume *v = SMDS_Mesh::AddVolumeWithID(
        n1, n2, n3, n4, n5, n6, n7, n8, n12, n23, n34, n41, n56, n67, n78, n85, n15, n26, n37, n48,
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(int n1, int n2, int n3, int n4, int n5, int n6, int n7,
                                            int n8, int n12, int n23, int n34, int n41, int n56,
                                            int n67, int n78, int n85, int n15, int n26, int n37,
                                            int n48, int n1234, int n1256, int n2367, int n3478,
...
SMDS_MeshVolume *SMDS_Mesh::AddVolumeWithID(
    ...
    const SMDS_MeshNode *n85, const SMDS_MeshNode *n15, const SMDS_MeshNode *n26,
    const SMDS_MeshNode *n37, const SMDS_MeshNode *n48, const SMDS_MeshNode *n1234,
    const SMDS_MeshNode *n1256, const SMDS_MeshNode *n2367, const SMDS_MeshNode *n3478,
    const SMDS_MeshNode *n1458, const SMDS_MeshNode *n5678, const SMDS_MeshNode *nCenter, int ID)
{
    if (!n1 || !n2 || !n3 || !n4 || !n5 || !n6 || !n7 || !n8 || !n12 || !n23 || !n34 || !n41 || !n56
...
void SMDS_Mesh::updateNodeMinMax()
{
    ...
    while (!myNodes[myNodeMin] && (myNodeMin < myNodes.size())) myNodeMin++;
    myNodeMax = myNodes.size() - 1;
    while (!myNodes[myNodeMax] && (myNodeMin >= 0)) myNodeMin--;
}
...
void SMDS_Mesh::dumpGrid(string ficdump)
{
    ...
    for (int i = 0; i < nbPoints; i++) {
        ficcon << i << " " << *(myGrid->GetPoint(i)) << " " << *(myGrid->GetPoint(i) + 1) << " "
               << " " << *(myGrid->GetPoint(i) + 2) << endl;
    }
    ...
    for (int i = 0; i < nbCells; i++) {
        ...
        for (int j = 0; j < nbptcell; j++) { ficcon << " " << listid->GetId(j); }
        ficcon << endl;
    }
    ...
    for (int i = 0; i < nbPoints; i++) {
        ...
        ficcon << i << " - " << ncells << " -";
        for (int j = 0; j < ncells; j++) { ficcon << " " << cells[j]; }
        ficcon << endl;
    }
    ficcon.close();
}

void SMDS_Mesh::compactMesh() { MESSAGE("SMDS_Mesh::compactMesh do nothing!"); }

int SMDS_Mesh::fromVtkToSmds(int vtkid)
{
    if (vtkid >= 0 && vtkid < myCellIdVtkToSmds.size()) return myCellIdVtkToSmds[vtkid];
...
VTK_MTIME_TYPE SMDS_Mesh::GetMTime() const { return this->myModifTime; }

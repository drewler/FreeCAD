...
static int Hexa_F[6][5] = {           // FORWARD
    {0, 1, 2, 3, 0}, {4, 7, 6, 5, 4}, // all face normals are external
    {0, 4, 5, 1, 0}, {1, 5, 6, 2, 1}, {3, 2, 6, 7, 3}, {0, 3, 7, 4, 0}};
static int Hexa_RE[6][5] = {          // REVERSED -> EXTERNAL
    {0, 3, 2, 1, 0}, {4, 5, 6, 7, 4}, // all face normals are external
    {0, 1, 5, 4, 0}, {1, 2, 6, 5, 1}, {3, 7, 6, 2, 3}, {0, 4, 7, 3, 0}};
...
static int HexPrism_F[8][7] = { // FORWARD
    {0, 1, 2, 3, 4, 5, 0},   {6, 11, 10, 9, 8, 7, 6}, {0, 6, 7, 1, 0, 0, 0},
    {1, 7, 8, 2, 1, 1, 1},   {2, 8, 9, 3, 2, 2, 2},   {3, 9, 10, 4, 3, 3, 3},
    {4, 10, 11, 5, 4, 4, 4}, {5, 11, 6, 0, 5, 5, 5}};
static int HexPrism_RE[8][7] = { // REVERSED -> EXTERNAL
    {0, 5, 4, 3, 2, 1, 0},   {6, 11, 10, 9, 8, 7, 6}, {0, 6, 7, 1, 0, 0, 0},
    {1, 7, 8, 2, 1, 1, 1},   {2, 8, 9, 3, 2, 2, 2},   {3, 9, 10, 4, 3, 3, 3},
    {4, 10, 11, 5, 4, 4, 4}, {5, 11, 6, 0, 5, 5, 5}};
...
static int QuadHexa_F[6][9] = {    // FORWARD
    ...
    {4, 15, 7, 14, 6, 13, 5, 12, 4}, {0, 16, 4, 12, 5, 17, 1, 8, 0}, {1, 17, 5, 13, 6, 18, 2, 9, 1},
    {3, 10, 2, 18, 6, 14, 7, 19, 3}, {0, 11, 3, 19, 7, 15, 4, 16, 0}};
static int QuadHexa_RE[6][9] = {   // REVERSED -> EXTERNAL
    {0, 11, 3, 10, 2, 9, 1, 8, 0}, // all face normals are external
    {4, 12, 5, 13, 6, 14, 7, 15, 4}, {0, 8, 1, 17, 5, 12, 4, 16, 0}, {1, 9, 2, 18, 6, 13, 5, 17, 1},
    {3, 19, 7, 14, 6, 18, 2, 10, 3}, {0, 16, 4, 15, 7, 19, 3, 11, 0}};
...
static int TriQuadHexa_F[6][9] = {  // FORWARD
    ...
    {4, 15, 7, 14, 6, 13, 5, 12, 25}, {0, 16, 4, 12, 5, 17, 1, 8, 21},
    {1, 17, 5, 13, 6, 18, 2, 9, 22},  {3, 10, 2, 18, 6, 14, 7, 19, 23},
...
static int TriQuadHexa_RE[6][9] = {   // REVERSED -> EXTERNAL
    ...
    {0, 8, 1, 17, 5, 12, 4, 16, 21},  {1, 9, 2, 18, 6, 13, 5, 17, 22},
    {3, 19, 7, 14, 6, 18, 2, 10, 23}, {0, 16, 4, 15, 7, 19, 3, 11, 24}};
...
inline XYZ XYZ::operator-(const XYZ &Right) { return XYZ(x - Right.x, y - Right.y, z - Right.z); }
inline XYZ XYZ::operator+(const XYZ &Right) { return XYZ(x + Right.x, y + Right.y, z + Right.z); }
inline XYZ XYZ::Crossed(const XYZ &Right)
{
    return XYZ(y * Right.z - z * Right.y, z * Right.x - x * Right.z, x * Right.y - y * Right.x);
}
inline double XYZ::Dot(const XYZ &Other) { return (x * Other.x + y * Other.y + z * Other.z); }
inline double XYZ::Magnitude() { return sqrt(x * x + y * y + z * z); }
inline double XYZ::SquareMagnitude() { return (x * x + y * y + z * z); }
...
SMDS_VolumeTool::VolumeType quadToLinear(SMDS_VolumeTool::VolumeType quadType)
{
    ...
    if (SMDS_VolumeTool::NbCornerNodes(linType) == nbCornersByQuad) return linType;
...
struct SMDS_VolumeTool::SaveFacet {
    ...
    SaveFacet(SMDS_VolumeTool::Facet &facet) : myToRestore(facet) { mySaved = facet; }
    ~SaveFacet()
    {
        if (myToRestore.myIndex != mySaved.myIndex) myToRestore = mySaved;
    }
};
...
SMDS_VolumeTool::SMDS_VolumeTool() { Set(0); }
...
SMDS_VolumeTool::SMDS_VolumeTool(const SMDS_MeshElement *theVolume, const bool ignoreCentralNodes)
{
...
SMDS_VolumeTool::~SMDS_VolumeTool() { myCurFace.myNodeIndices = NULL; }
...
bool SMDS_VolumeTool::Set(const SMDS_MeshElement *theVolume, const bool ignoreCentralNodes)
{
    // reset fields
    myVolume = 0;
    myPolyedre = 0;
    ...
    // set volume data
    if (!theVolume || theVolume->GetType() != SMDSAbs_Volume) return false;
    ...
    // check validity
    if (!setFace(0)) return (myVolume = 0);
    ...
    if (!myPolyedre) {
        ...
        if (GetFaceNormal(0, botNormal.x, botNormal.y, botNormal.z)) {
            ...
            XYZ upDir(topNode->X() - botNode->X(), topNode->Y() - botNode->Y(),
                      topNode->Z() - botNode->Z());
...
void SMDS_VolumeTool::Inverse()
{
    ...
    switch (myVolumeNodes.size()) {
        case 4: SWAP_NODES(myVolumeNodes, 1, 2); break;
        case 5: SWAP_NODES(myVolumeNodes, 1, 3); break;
        case 6:
...
static double getTetraVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
...
{
    double V = 0.;
    if (!myVolume) return 0.;

    if (myVolume->IsPoly()) {
        if (!myPolyedre) return 0.;
        ...
        const static int ind[] = {0, 1, 3, 6, 11, 23, 31, 44, 58, 78};
        const static int vtab[][4] = {
            // decomposition into tetra in the order of enum VolumeType
            // tetrahedron
            {0, 1, 2, 3},
            // pyramid
            {0, 1, 3, 4},
        ...
        for (int i = n1; i < n2; i++) {
            V -= getTetraVolume(myVolumeNodes[vtab[i][0]], myVolumeNodes[vtab[i][1]],
                                myVolumeNodes[vtab[i][2]], myVolumeNodes[vtab[i][3]]);
        }
    }
    return V;
}
...
bool SMDS_VolumeTool::IsOut(double X, double Y, double Z, double tol) const
{
    ...
    for (int iF = 0; iF < myNbFaces; ++iF) {
        XYZ faceNormal;
        if (!GetFaceNormal(iF, faceNormal.x, faceNormal.y, faceNormal.z)) continue;
        if (!IsFaceExternal(iF)) faceNormal = XYZ() - faceNormal; // reverse

        XYZ face2p(p - XYZ(myCurFace.myNodes[0]));
        if (face2p.Dot(faceNormal) > tol) return true;
    }
    return false;
}
...
bool SMDS_VolumeTool::GetFaceNodes(int faceIndex, set<const SMDS_MeshNode *> &theFaceNodes) const
{
    if (!setFace(faceIndex)) return false;
...
{
    if (myExternalFaces || !myVolume) return true;
    ...
    if (myPolyFacetOri[faceIndex]) return myPolyFacetOri[faceIndex] > 0;
    ...
    {
        for (size_t i = 0; i < myPolyFacetOri.size() && !ori; ++i) ori = myPolyFacetOri[i];
    ...
    for (int i = 0; i < myCurFace.myNbNodes && !ori; ++i) {
        ...
        if (l2o != myFwdLinks.end()) ori = link.myOri * l2o->second * -1;
        links[i] = link;
    }
    while (!ori) // the facet has no common links with already oriented facets
    {
        ...
        if (!ori) return false; // error in algorithm: infinite loop
        ...
        for (size_t i = 0; i < links.size() && !ori; ++i) {
            ...
            if (l2o != myFwdLinks.end()) ori = links[i].myOri * l2o->second * -1;
        }
        me->myPolyFacetOri[faceIndex] = ori;
    }
...
bool SMDS_VolumeTool::projectNodesToNormal(int faceIndex, double &minProj, double &maxProj) const
{
    minProj = std::numeric_limits<double>::max();
    maxProj = std::numeric_limits<double>::min();

    XYZ normal;
    if (!GetFaceNormal(faceIndex, normal.x, normal.y, normal.z)) return false;
    XYZ p0(myCurFace.myNodes[0]);
    for (size_t i = 0; i < myVolumeNodes.size(); ++i) {
        if (std::find(myCurFace.myNodes.begin() + 1, myCurFace.myNodes.end(), myVolumeNodes[i])
...
bool SMDS_VolumeTool::GetFaceNormal(int faceIndex, double &X, double &Y, double &Z) const
{
    ...
    if (size <= numeric_limits<double>::min()) return false;
...
{
    double area = 0;
    if (!setFace(faceIndex)) return area;
...
int SMDS_VolumeTool::GetOppFaceIndex(int faceIndex) const
{
    ...
    if (faceIndex >= 0 && faceIndex < NbFaces()) {
        switch (myVolumeNodes.size()) {
            case 6:
            case 15:
                if (faceIndex == 0 || faceIndex == 1) ind = 1 - faceIndex;
                break;
            case 8:
            case 12:
                ...
                break;
            case 20:
            case 27: ind = GetOppFaceIndexOfHex(faceIndex); break;
            default:;
        }
    }
    return ind;
...
int SMDS_VolumeTool::GetOppFaceIndexOfHex(int faceIndex) { return Hexa_oppF[faceIndex]; }
...
bool SMDS_VolumeTool::IsLinked(const SMDS_MeshNode *theNode1, const SMDS_MeshNode *theNode2,
                               const bool theIgnoreMediumNodes) const
{
    ...
    if (myVolume->IsPoly()) {
        ...
        for (int iface = 0; iface < myNbFaces; iface++) {
            ...
            if (i != myVolumeNodes.end()) {
                if ((theNode2 == *(i - d1) || theNode2 == *(i + d1))) return true;
                if ((d2) && ((theNode2 == *(i - d2) || theNode2 == *(i + d2)))) return true;
            }
        }
        return false;
    }
    ...
    for (int i = 0; i < myVolumeNodes.size() && nbFound < 2; i++) {
        if (myVolumeNodes[i] == theNode1) i1 = i, ++nbFound;
...
bool SMDS_VolumeTool::IsLinked(const int theNode1Index, const int theNode2Index,
                               bool theIgnoreMediumNodes) const
{
    ...
    if (minInd < 0 || maxInd > myVolumeNodes.size() - 1 || maxInd == minInd) return false;

    VolumeType type = GetVolumeType();
    if (myVolume->IsQuadratic()) {
        int firstMediumInd = myVolume->NbCornerNodes();
        if (minInd >= firstMediumInd) return false; // both nodes are medium - not linked
    ...
    switch (type) {
        ...
        case QUAD_TETRA: {
            switch (minInd) {
                case 0:
                    if (maxInd == 4 || maxInd == 6 || maxInd == 7) return true;
                    /* FALLTHRU */
                case 1:
                    if (maxInd == 4 || maxInd == 5 || maxInd == 8) return true;
                    /* FALLTHRU */
                case 2:
                    if (maxInd == 5 || maxInd == 6 || maxInd == 9) return true;
                    /* FALLTHRU */
                case 3:
                    if (maxInd == 7 || maxInd == 8 || maxInd == 9) return true;
                    /* FALLTHRU */
                default:;
            }
            break;
        ...
        case QUAD_HEXA: {
            switch (minInd) {
                case 0:
                    if (maxInd == 8 || maxInd == 11 || maxInd == 16) return true;
                    /* FALLTHRU */
                case 1:
                    if (maxInd == 8 || maxInd == 9 || maxInd == 17) return true;
                    /* FALLTHRU */
                case 2:
                    if (maxInd == 9 || maxInd == 10 || maxInd == 18) return true;
                    /* FALLTHRU */
                case 3:
                    if (maxInd == 10 || maxInd == 11 || maxInd == 19) return true;
                    /* FALLTHRU */
                case 4:
                    if (maxInd == 12 || maxInd == 15 || maxInd == 16) return true;
                    /* FALLTHRU */
                case 5:
                    if (maxInd == 12 || maxInd == 13 || maxInd == 17) return true;
                    /* FALLTHRU */
                case 6:
                    if (maxInd == 13 || maxInd == 14 || maxInd == 18) return true;
                    /* FALLTHRU */
                case 7:
                    if (maxInd == 14 || maxInd == 15 || maxInd == 19) return true;
                    /* FALLTHRU */
                default:;
            }
            break;
        ...
        case QUAD_PYRAM: {
            switch (minInd) {
                case 0:
                    if (maxInd == 5 || maxInd == 8 || maxInd == 9) return true;
                    /* FALLTHRU */
                case 1:
                    if (maxInd == 5 || maxInd == 6 || maxInd == 10) return true;
                    /* FALLTHRU */
                case 2:
                    if (maxInd == 6 || maxInd == 7 || maxInd == 11) return true;
                    /* FALLTHRU */
                case 3:
                    if (maxInd == 7 || maxInd == 8 || maxInd == 12) return true;
                    /* FALLTHRU */
                case 4:
                    if (maxInd == 9 || maxInd == 10 || maxInd == 11 || maxInd == 12) return true;
                default:;
            }
            break;
        }
        case QUAD_PENTA: {
            switch (minInd) {
                case 0:
                    if (maxInd == 6 || maxInd == 8 || maxInd == 12) return true;
                    /* FALLTHRU */
                case 1:
                    if (maxInd == 6 || maxInd == 7 || maxInd == 13) return true;
                    /* FALLTHRU */
                case 2:
                    if (maxInd == 7 || maxInd == 8 || maxInd == 14) return true;
                    /* FALLTHRU */
                case 3:
                    if (maxInd == 9 || maxInd == 11 || maxInd == 12) return true;
                    /* FALLTHRU */
                case 4:
                    if (maxInd == 9 || maxInd == 10 || maxInd == 13) return true;
                    /* FALLTHRU */
                case 5:
                    if (maxInd == 10 || maxInd == 11 || maxInd == 14) return true;
                    /* FALLTHRU */
                default:;
            }
            break;
...
int SMDS_VolumeTool::GetNodeIndex(const SMDS_MeshNode *theNode) const
{
    if (myVolume) {
        for (int i = 0; i < myVolumeNodes.size(); i++) {
            if (myVolumeNodes[i] == theNode) return i;
        }
    }
    return -1;
}
    ...
    if (IsPoly())
        ...
        for (int iF = 0; iF < NbFaces(); ++iF) {
            ...
            switch (NbFaceNodes(iF)) {
                case 3: face = SMDS_Mesh::FindFace(nodes[0], nodes[1], nodes[2]); break;
                case 4: face = SMDS_Mesh::FindFace(nodes[0], nodes[1], nodes[2], nodes[3]); break;
                case 6:
                    face = SMDS_Mesh::FindFace(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
                                               nodes[5]);
                    break;
                case 8:
                    face = SMDS_Mesh::FindFace(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4],
...
bool SMDS_VolumeTool::IsFreeFace(int faceIndex, const SMDS_MeshElement **otherVol /*=0*/) const
{
    ...
    if (!setFace(faceIndex)) return !isFree;

    const SMDS_MeshNode **nodes = GetFaceNodes(faceIndex);

    const int di = myVolume->IsQuadratic() ? 2 : 1;
    const int nbN = (myCurFace.myNbNodes / di <= 4 && !IsPoly())
        ? 3
        : myCurFace.myNbNodes / di; // nb nodes to check

    SMDS_ElemIteratorPtr eIt = nodes[0]->GetInverseElementIterator(SMDSAbs_Volume);
    while (eIt->more()) {
        const SMDS_MeshElement *vol = eIt->next();
        if (vol == myVolume) continue;
        int iN;
        for (iN = 1; iN < nbN; ++iN)
            if (vol->GetNodeIndex(nodes[iN * di]) < 0) break;
...
bool SMDS_VolumeTool::IsFreeFaceAdv(int faceIndex, const SMDS_MeshElement **otherVol /*=0*/) const
{
    ...
    for (int iNode = 0; iNode < nbFaceNodes; iNode++) {
        ...
        while (eIt->more()) {
            ...
            if (elem != myVolume) {
                ...
                (*vNbIt).second++;
                if (vNbIt->second > maxNbShared) maxNbShared = vNbIt->second;
            }
        }
    }
    if (maxNbShared < 3) return isFree; // is free
    ...
    if (IsFaceExternal(faceIndex)) intNormal = XYZ(-intNormal.x, -intNormal.y, -intNormal.z);
    ...
    for (int iNode = 0; iNode < nbFaceNodes; iNode++) {
        ...
        bool isShared = false;
        SMDS_ElemIteratorPtr eIt = n->GetInverseElementIterator(SMDSAbs_Volume);
        while (eIt->more() && !isShared) isShared = volNbShared.count(eIt->next());
        if (!isShared) return isFree;
    }
                                  ...
                                  const int theFaceIndexHint) const
{
    if (theFaceIndexHint >= 0) {
        ...
        if (nbNodes == (int)theFaceNodes.size()) {
            ...
            while (nbNodes)
                if (theFaceNodes.count(nodes[nbNodes - 1])) --nbNodes;
                else
                    break;
            if (nbNodes == 0) return theFaceIndexHint;
        }
    }
    for (int iFace = 0; iFace < myNbFaces; iFace++) {
        if (iFace == theFaceIndexHint) continue;
...
bool SMDS_VolumeTool::setFace(int faceIndex) const
{
    ...
    if (myCurFace.myIndex == faceIndex) return true;

    myCurFace.myIndex = -1;

    if (faceIndex < 0 || faceIndex >= NbFaces()) return false;
    ...
    if (myVolume->IsPoly()) {
        ...
        myCurFace.myNodeIndices =
            (int *)(myVolForward ? myAllFacesNodeIndices_F + faceIndex * myMaxFaceNbNodes
                                 : myAllFacesNodeIndices_RE + faceIndex * myMaxFaceNbNodes);
...
const int *SMDS_VolumeTool::GetFaceNodesIndices(VolumeType type, int faceIndex, bool external)
{
    switch (type) {
        ...
        case QUAD_PENTA:
            return external ? QuadPenta_F[faceIndex] : QuadPenta_F[faceIndex];
...
int SMDS_VolumeTool::NbFaceNodes(VolumeType type, int faceIndex)
{
    switch (type) {
        ...
        case QUAD_PENTA:
            return QuadPenta_nbN[faceIndex];
            // what about SMDSEntity_TriQuad_Hexa?
...
int SMDS_VolumeTool::ID() const { return myVolume ? myVolume->GetID() : 0; }

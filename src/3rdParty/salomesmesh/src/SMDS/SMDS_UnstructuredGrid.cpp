...
void SMDS_CellLinks::ResizeForPoint(vtkIdType vtkID)
{
    if (vtkID > this->MaxId) {
        this->MaxId = vtkID;
        if (vtkID >= this->Size) vtkCellLinks::Resize(vtkID + SMDS_Mesh::chunkSize);
    }
}
...
{
    if (type != VTK_POLYHEDRON) return vtkUnstructuredGrid::InsertNextLinkedCell(type, npts, pts);
...
void SMDS_UnstructuredGrid::setSMDS_mesh(SMDS_Mesh *mesh) { _mesh = mesh; }

void SMDS_UnstructuredGrid::compactGrid(std::vector<int> &idNodesOldToNew, int newNodeSize,
                                        std::vector<int> &idCellsOldToNew, int newCellSize)
{
    MESSAGE("------------------------- SMDS_UnstructuredGrid::compactGrid "
            << newNodeSize << " " << newCellSize); //CHRONO(1);
    int alreadyCopied = 0;
    ...
    if (newNodeSize) {
        ...
        while (i < oldNodeSize) {
            // skip a hole if any
            while (i < oldNodeSize && idNodesOldToNew[i] < 0) ++i;
            int startBloc = i;
    ...
    while (i < oldCellSize) {
        // skip a hole if any
        while (i < oldCellSize && this->Types->GetValue(i) == VTK_EMPTY_CELL) ++i;
        int startBloc = i;
        // look for a block end
        while (i < oldCellSize && this->Types->GetValue(i) != VTK_EMPTY_CELL) ++i;
        int endBloc = i;
        if (endBloc > startBloc)
            copyBloc(newTypes, idCellsOldToNew, idNodesOldToNew, newConnectivity, newLocations,
                     pointsCell, alreadyCopied, startBloc, endBloc);
    }
            ...
            vtkDoubleArray::SafeDownCast(vtkDataSet::CellData->GetScalars())) // Balls
    {
        for (int oldCellID = 0; oldCellID < oldCellSize; oldCellID++) {
            if (this->Types->GetValue(oldCellID) == VTK_EMPTY_CELL) continue;
    ...
    if (this->FaceLocations) {
        ...
        for (int i = 0; i < oldCellSize; i++) {
            if (this->Types->GetValue(i) == VTK_EMPTY_CELL) continue;
...
void SMDS_UnstructuredGrid::copyNodes(vtkPoints *newPoints, std::vector<int> &idNodesOldToNew,
                                      int &alreadyCopied, int start, int end)
{
    MESSAGE("copyNodes " << alreadyCopied << " " << start << " " << end << " size: " << end - start
                         << " total: " << alreadyCopied + end - start);
...
void SMDS_UnstructuredGrid::copyBloc(vtkUnsignedCharArray *newTypes,
                                     ...
                                     vtkCellArray *newConnectivity, vtkIdTypeArray *newLocations,
                                     vtkIdType *pointsCell, int &alreadyCopied, int start, int end)
{
    ...
    for (int j = start; j < end; j++) {
        ...
        vtkIdType oldLoc =
            ((vtkIdTypeArray *)(this->Connectivity->GetOffsetsArray()))->GetValue(j) + j;
#else
...
void SMDS_UnstructuredGrid::CleanDownwardConnectivity()
{
    for (int i = 0; i < _downArray.size(); i++) {
        if (_downArray[i]) delete _downArray[i];
        _downArray[i] = 0;
    }
    _cellIdToDownId.clear();
}
...
{
    MESSAGE("SMDS_UnstructuredGrid::BuildDownwardConnectivity");
    CHRONO(2);
    ...
    int nbLineGuess =
        int((4.0 / 3.0) * nbLinTetra + 2 * nbLinPrism + 2.5 * nbLinPyra + 3 * nbLinHexa);
    int nbQuadEdgeGuess =
        int((4.0 / 3.0) * nbQuadTetra + 2 * nbQuadPrism + 2.5 * nbQuadPyra + 3 * nbQuadHexa);
    ...
    int nbQuadQuadGuess =
        int((2.0 / 3.0) * nbQuadPrism + (1.0 / 2.0) * nbQuadPyra + 3 * nbQuadHexa);
    ...
    MESSAGE("--- iteration on vtkUnstructuredGrid cells, only faces");
    CHRONO(20);
    ...
    CHRONOSTOP(20);
    MESSAGE("--- iteration on vtkUnstructuredGrid cells, only volumes");
    CHRONO(21);
        ...
        if (SMDS_Downward::getCellDimension(vtkType) == 3) {
            ...
            for (int iface = 0; iface < facesWithNodes.nbElems; iface++) {
                ...
                for (int ivol = 0; ivol < nbVolumes; ivol++) {
                    ...
                    if (connFaceId >= 0) break; // --- face already created
                }                               //CHRONOSTOP(33);
    ...
    CHRONOSTOP(21);
    MESSAGE("--- iteration on vtkUnstructuredGrid cells, only edges");
    CHRONO(22);
    ...
    for (int i = 0; i < cellSize; i++) {
        ...
        if (SMDS_Downward::getCellDimension(vtkEdgeType) == 1) {
            ...
            int nbDownFaces =
                downEdge->computeFaces(connEdgeId, &vtkIds[0], nbVtkCells, downFaces, downTypes);
    ...
    for (int vtkFaceType = 0; vtkFaceType < VTK_QUADRATIC_PYRAMID; vtkFaceType++) {
        if (SMDS_Downward::getCellDimension(vtkFaceType) != 2) continue;
        ...
        for (int faceId = 0; faceId < maxId; faceId++) {
            ...
            for (int iedge = 0; iedge < edgesWithNodes.nbElems; iedge++) {
                ...
                unsigned char downTypes[1000];
                int nbDownFaces =
                    downEdge->computeFaces(pts, &vtkIds[0], nbVtkCells, downFaces, downTypes);
                //CHRONOSTOP(42);
                ...
                for (int idf = 0; idf < nbDownFaces; idf++) {
                    ...
                    if (connEdgeId >= 0) break; // --- edge already created
                }                               //CHRONOSTOP(43);
    ...
    for (int vtkType = VTK_QUADRATIC_PYRAMID; vtkType >= 0; vtkType--) {
        if (SMDS_Downward *down = _downArray[vtkType]) { down->compactStorage(); }
    }
 ...
 */
int SMDS_UnstructuredGrid::GetNeighbors(int *neighborsVtkIds, int *downIds,
                                        unsigned char *downTypes, int vtkId, bool getSkin)
{
    int vtkType = this->GetCellType(vtkId);
    int cellDim = SMDS_Downward::getCellDimension(vtkType);
    if (cellDim < 2) return 0; // TODO voisins des edges = edges connectees
    ...
    for (int i = 0; i < nbCells; i++) {
        ...
        for (int j = 0; j < nbUp; j++) {
            if ((upCells[j] == cellId) && (upTypes[j] == vtkType)) continue;
            ...
            if (nb >= NBMAXNEIGHBORS) {
                INFOS("SMDS_UnstructuredGrid::GetNeighbors problem: NBMAXNEIGHBORS="
                      << NBMAXNEIGHBORS << " not enough");
                return nb;
            }
        }
        if (getSkin) {
            ...
            {
                neighborsVtkIds[nb] =
                    _downArray[cellType]->getVtkCellId(downId); // OK if skin present
                downIds[nb] = downId;
                downTypes[nb] = cellType;
                nb++;
                if (nb >= NBMAXNEIGHBORS) {
...
int SMDS_UnstructuredGrid::GetParentVolumes(int *volVtkIds, int vtkId)
{
    ...
    for (int i = 0; i < nbFaces; i++) {
        ...
        for (int j = 0; j < nv; j++) {
            ...
            if (vtkVolId >= 0) volVtkIds[nbvol++] = vtkVolId;
        }
    }
    return nbvol;
}
 ...
 */
int SMDS_UnstructuredGrid::getOrderedNodesOfFace(int vtkVolId, int &dim,
...
void SMDS_UnstructuredGrid::BuildLinks()
{
    ...
    if (this->Links) { this->Links->UnRegister(this); }
 ...
 */
SMDS_MeshCell *
SMDS_UnstructuredGrid::extrudeVolumeFromFace(int vtkVolId, int domain1, int domain2,
                                             ...
                                             std::map<int, std::map<long, int>> &nodeQuadDomains)
{
    ...
    for (; it != originalNodes.end(); ++it) orderedOriginals.push_back(*it);
    ...
    switch (orderedOriginals.size()) {
        case 3:
            if (dim == 2) isQuadratic = true;
            break;
        case 6:
        case 8: isQuadratic = true; break;
        default: isQuadratic = false; break;
    }
    ...
    if (isQuadratic) {
        ...
        if (domain1 < domain2) dom1_2 = dom1 + INT_MAX * dom2;
        else
...
void SMDS_UnstructuredGrid::AllocateDiameters(vtkIdType MaxVtkID) { SetBallDiameter(MaxVtkID, 0); }

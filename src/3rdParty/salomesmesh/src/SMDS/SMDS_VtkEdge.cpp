...
void SMDS_VtkEdge::init(std::vector<vtkIdType> &nodeIds, SMDS_Mesh *mesh)
{
    ...
    if (nodeIds.size() == 3) aType = VTK_QUADRATIC_EDGE;
...
void SMDS_VtkEdge::Print(std::ostream &OS) const { OS << "edge <" << GetID() << "> : "; }
...
int SMDS_VtkEdge::NbEdges() const { return 1; }

SMDSAbs_EntityType SMDS_VtkEdge::GetEntityType() const
{
    if (NbNodes() == 2) return SMDSEntity_Edge;
    else
 ...
 */
const SMDS_MeshNode *SMDS_VtkEdge::GetNode(const int ind) const
{
    vtkUnstructuredGrid *grid = SMDS_Mesh::_meshList[myMeshId]->getGrid();
    vtkIdType npts;
    vtkIdTypePtr pts;
...
SMDS_NodeIteratorPtr SMDS_VtkEdge::interlacedNodesIterator() const { return nodesIteratorToUNV(); }

...
///////////////////////////////////////////////////////////////////////////////
SMDS_VolumeOfNodes::SMDS_VolumeOfNodes(const SMDS_MeshNode *node1, const SMDS_MeshNode *node2,
...
bool SMDS_VolumeOfNodes::ChangeNodes(const SMDS_MeshNode *nodes[], const int nbNodes)
{
    if (nbNodes < 4 || nbNodes > 8 || nbNodes == 7) return false;

    delete[] myNodes;
    myNbNodes = nbNodes;
    myNodes = new const SMDS_MeshNode *[myNbNodes];
    for (int i = 0; i < nbNodes; i++) myNodes[i] = nodes[i];
...
int SMDS_VolumeOfNodes::NbNodes() const { return myNbNodes; }
...
SMDSAbs_ElementType SMDS_VolumeOfNodes::GetType() const { return SMDSAbs_Volume; }
 ...
 */
const SMDS_MeshNode *SMDS_VolumeOfNodes::GetNode(const int ind) const { return myNodes[ind]; }

...
bool SMDS_VtkVolume::ChangeNodes(const SMDS_MeshNode *nodes[], const int nbNodes)
{
    ...
    if (nbNodes != cellPoints->GetNumberOfIds()) {
        MESSAGE("ChangeNodes problem: not the same number of nodes " << cellPoints->GetNumberOfIds()
                                                                     << " -> " << nbNodes);
        return false;
    }
    for (int i = 0; i < nbNodes; i++) { cellPoints->SetId(i, nodes[i]->getVtkId()); }
#else
    vtkIdType npts = 0;
    vtkIdType *pts = 0;
    ...
    for (int i = 0; i < nbNodes; i++) { pts[i] = nodes[i]->getVtkId(); }
#endif
...
bool SMDS_VtkVolume::vtkOrder(const SMDS_MeshNode *nodes[], const int nbNodes)
{
    ...
    if (!interlace.empty()) {
        ...
        for (size_t i = 0; i < interlace.size(); ++i) nodes[i] = initNodes[interlace[i]];
    }
    return true;
}
...
void SMDS_VtkVolume::Print(ostream &OS) const { OS << "volume <" << GetID() << "> : "; }
...
int SMDS_VtkVolume::NbFaces() const
{
    ...
    switch (aVtkType) {
        case VTK_TETRA:
        case VTK_QUADRATIC_TETRA: nbFaces = 4; break;
        case VTK_PYRAMID:
        case VTK_WEDGE:
        case VTK_QUADRATIC_PYRAMID:
        case VTK_QUADRATIC_WEDGE: nbFaces = 5; break;
        case VTK_HEXAHEDRON:
        case VTK_QUADRATIC_HEXAHEDRON:
        case VTK_TRIQUADRATIC_HEXAHEDRON: nbFaces = 6; break;
        case VTK_POLYHEDRON: {
            vtkIdType nFaces = 0;
        ...
        case VTK_HEXAGONAL_PRISM: nbFaces = 8; break;
        default:
...
int SMDS_VtkVolume::NbNodes() const
{
    ...
    vtkIdType aVtkType = grid->GetCellType(this->myVtkID);
    int nbPoints = 0;
    if (aVtkType != VTK_POLYHEDRON) { nbPoints = grid->GetCell(myVtkID)->GetNumberOfPoints(); }
    else {
        vtkIdType nFaces = 0;
        vtkIdTypePtr ptIds = 0;
        grid->GetFaceStream(this->myVtkID, nFaces, ptIds);
...
int SMDS_VtkVolume::NbEdges() const
{
    ...
    switch (aVtkType) {
        case VTK_TETRA:
        case VTK_QUADRATIC_TETRA: nbEdges = 6; break;
        case VTK_PYRAMID:
        case VTK_QUADRATIC_PYRAMID: nbEdges = 8; break;
        case VTK_WEDGE:
        case VTK_QUADRATIC_WEDGE: nbEdges = 9; break;
        case VTK_HEXAHEDRON:
        case VTK_QUADRATIC_HEXAHEDRON:
        case VTK_TRIQUADRATIC_HEXAHEDRON: nbEdges = 12; break;
        case VTK_POLYHEDRON: {
            vtkIdType nFaces = 0;
            vtkIdTypePtr ptIds = 0;
            grid->GetFaceStream(this->myVtkID, nFaces, ptIds);
        ...
        case VTK_HEXAGONAL_PRISM: nbEdges = 18; break;
        default:
            MESSAGE("invalid volume type");
            nbEdges = 0;
            break;
        ...
        case SMDSAbs_Node: {
            ...
            if (aType == SMDSEntity_Polyhedra)
                return SMDS_ElemIteratorPtr(
                    new SMDS_VtkCellIteratorPolyH(SMDS_Mesh::_meshList[myMeshId], myVtkID, aType));
            else
                return SMDS_ElemIteratorPtr(
                    new SMDS_VtkCellIterator(SMDS_Mesh::_meshList[myMeshId], myVtkID, aType));
        }
        default:
...
SMDS_NodeIteratorPtr SMDS_VtkVolume::nodesIteratorToUNV() const
{
    return SMDS_NodeIteratorPtr(
        new SMDS_VtkCellIteratorToUNV(SMDS_Mesh::_meshList[myMeshId], myVtkID, GetEntityType()));
}
...
SMDSAbs_ElementType SMDS_VtkVolume::GetType() const { return SMDSAbs_Volume; }
...
const SMDS_MeshNode *SMDS_VtkVolume::GetNode(const int ind) const
{
    ...
    return SMDS_Mesh::_meshList[myMeshId]->FindNodeVtk(
        pts[interlace.empty() ? ind : interlace[ind]]);
}
/*!
    ...
    if (aVtkType == VTK_POLYHEDRON) {
        ...
        for (int iF = 0; iF < nFaces; iF++) {
            ...
            for (vtkIdType i = 0; i < nodesInFace; ++i)
                if (ptIds[id + i + 1] == node->getVtkId()) return id + i - iF;
            id += (nodesInFace + 1);
        }
        return -1;
    }
...
bool SMDS_VtkVolume::IsQuadratic() const
{
    ...
    switch (aVtkType) {
        ...
        case VTK_QUADRATIC_HEXAHEDRON:
        case VTK_TRIQUADRATIC_HEXAHEDRON: return true; break;
...
bool SMDS_VtkVolume::IsMediumNode(const SMDS_MeshNode *node) const
{
    ...
    for (int rank = 0; rank < npts; rank++) {
        if (pts[rank] == nodeId) {
            if (rank < rankFirstMedium) return false;
            else
                return true;
        }
    }
...
SMDSAbs_EntityType SMDS_VtkVolume::GetEntityType() const
{
    ...
    switch (aVtkType) {
        case VTK_TETRA: aType = SMDSEntity_Tetra; break;
        case VTK_PYRAMID: aType = SMDSEntity_Pyramid; break;
        case VTK_WEDGE: aType = SMDSEntity_Penta; break;
        case VTK_HEXAHEDRON: aType = SMDSEntity_Hexa; break;
        case VTK_QUADRATIC_TETRA: aType = SMDSEntity_Quad_Tetra; break;
        case VTK_QUADRATIC_PYRAMID: aType = SMDSEntity_Quad_Pyramid; break;
        case VTK_QUADRATIC_WEDGE: aType = SMDSEntity_Quad_Penta; break;
        case VTK_QUADRATIC_HEXAHEDRON: aType = SMDSEntity_Quad_Hexa; break;
        case VTK_TRIQUADRATIC_HEXAHEDRON: aType = SMDSEntity_TriQuad_Hexa; break;
        ...
        case VTK_POLYHEDRON:
            ...
            break;
            //#endif
        default: aType = SMDSEntity_Polyhedra; break;
    }
    return aType;
}
...
SMDSAbs_GeometryType SMDS_VtkVolume::GetGeomType() const
{
    ...
    switch (aVtkType) {
        case VTK_TETRA:
        case VTK_QUADRATIC_TETRA: aType = SMDSGeom_TETRA; break;
        case VTK_PYRAMID:
        case VTK_QUADRATIC_PYRAMID: aType = SMDSGeom_PYRAMID; break;
        case VTK_WEDGE:
        case VTK_QUADRATIC_WEDGE: aType = SMDSGeom_PENTA; break;
        case VTK_HEXAHEDRON:
        case VTK_QUADRATIC_HEXAHEDRON:
        case VTK_TRIQUADRATIC_HEXAHEDRON: aType = SMDSGeom_HEXA; break;
        case VTK_HEXAGONAL_PRISM:
        ...
        case VTK_POLYHEDRON:
            ...
            break;
            //#endif
        default: aType = SMDSGeom_POLYHEDRA; break;
    }
    return aType;
}
...
void SMDS_VtkVolume::gravityCenter(SMDS_UnstructuredGrid *grid, const vtkIdType *nodeIds,
                                   int nbNodes, double *result)
{
    for (int j = 0; j < 3; j++) result[j] = 0;
    if (nbNodes <= 0) return;
    for (int i = 0; i < nbNodes; i++) {
        double *coords = grid->GetPoint(nodeIds[i]);
        for (int j = 0; j < 3; j++) result[j] += coords[j];
    }
    for (int j = 0; j < 3; j++) result[j] = result[j] / nbNodes;
...
bool SMDS_VtkVolume::isForward(double *a, double *b, double *c, double *d)
{
    ...
    double prodmixte = (u[1] * v[2] - u[2] * v[1]) * w[0] + (u[2] * v[0] - u[0] * v[2]) * w[1]
        + (u[0] * v[1] - u[1] * v[0]) * w[2];
    return (prodmixte < 0);
}
...
SMDS_ElemIteratorPtr SMDS_VtkVolume::uniqueNodesIterator() const
{
    ...
    return SMDS_ElemIteratorPtr(
        new SMDS_VtkCellIterator(SMDS_Mesh::_meshList[myMeshId], myVtkID, GetEntityType()));
}

...
SMDS_MeshElement::SMDS_MeshElement(int ID) { init(ID); }
...
void SMDS_MeshElement::Print(ostream &OS) const { OS << "dump of mesh element" << endl; }
...
public:
    SMDS_MeshElement_MyIterator(const SMDS_MeshElement *element) : myElement(element), myMore(true)
    {}

    bool more() { return myMore; }
...
SMDS_ElemIteratorPtr SMDS_MeshElement::elementsIterator(SMDSAbs_ElementType type) const
{
...
SMDS_NodeIteratorPtr SMDS_MeshElement::nodesIteratorToUNV() const { return nodeIterator(); }

//! virtual, redefined in vtkEdge, vtkFace and vtkVolume classes
SMDS_NodeIteratorPtr SMDS_MeshElement::interlacedNodesIterator() const { return nodeIterator(); }
...
{
    return SMDS_ElemIteratorPtr(new _MyElemIteratorFromNodeIterator(interlacedNodesIterator()));
}

SMDS_NodeIteratorPtr SMDS_MeshElement::nodeIterator() const
{
    return SMDS_NodeIteratorPtr(new _MyNodeIteratorFromElemIterator(nodesIterator()));
}
...
bool operator<(const SMDS_MeshElement &e1, const SMDS_MeshElement &e2)
{
    ...
    switch (e1.GetType()) {
        case SMDSAbs_Node:
            return static_cast<const SMDS_MeshNode &>(e1) < static_cast<const SMDS_MeshNode &>(e2);

        case SMDSAbs_Edge:
            return static_cast<const SMDS_MeshEdge &>(e1) < static_cast<const SMDS_MeshEdge &>(e2);

        case SMDSAbs_Face:
            return static_cast<const SMDS_MeshFace &>(e1) < static_cast<const SMDS_MeshFace &>(e2);
...
bool SMDS_MeshElement::IsValidIndex(const int ind) const { return (ind > -1 && ind < NbNodes()); }

const SMDS_MeshNode *SMDS_MeshElement::GetNode(const int ind) const
{
    if (ind >= 0) {
        SMDS_ElemIteratorPtr it = nodesIterator();
        for (int i = 0; i < ind; ++i) it->next();
        if (it->more()) return static_cast<const SMDS_MeshNode *>(it->next());
    }
    return 0;
}

bool SMDS_MeshElement::IsQuadratic() const { return false; }

bool SMDS_MeshElement::IsMediumNode(const SMDS_MeshNode *node) const { return false; }

    ...
    : _mesh(mesh), _cellId(vtkCellId), _index(0), _type(aType)
{
    ...
    if (interlace.empty()) {
        ...
        for (int i = 0; i < _nbNodes; i++) _vtkIdList->SetId(i, pts[interlace[i]]);
    }
}

SMDS_VtkCellIterator::~SMDS_VtkCellIterator() { _vtkIdList->Delete(); }

bool SMDS_VtkCellIterator::more() { return (_index < _nbNodes); }
...
SMDS_VtkCellIteratorToUNV::SMDS_VtkCellIteratorToUNV(SMDS_Mesh *mesh, int vtkCellId,
    ...
    : SMDS_VtkCellIterator()
{
    ...
    switch (_type) {
        ...
        case SMDSEntity_TriQuad_Hexa: {
            static int id[] = {0,  8,  1, 9,  2, 10, 3, 11, 16, 17,
    ...
    if (ids)
        for (int i = 0; i < _nbNodes; i++) _vtkIdList->SetId(i, pts[ids[i]]);
    else
        for (int i = 0; i < _nbNodes; i++) _vtkIdList->SetId(i, pts[i]);
}

bool SMDS_VtkCellIteratorToUNV::more() { return SMDS_VtkCellIterator::more(); }
...
SMDS_VtkCellIteratorPolyH::SMDS_VtkCellIteratorPolyH(SMDS_Mesh *mesh, int vtkCellId,
                                                     SMDSAbs_EntityType aType)
    : SMDS_VtkCellIterator()
{
    _mesh = mesh;
    ...
    switch (_type) {
        case SMDSEntity_Polyhedra: {
            ...
            for (int i = 0; i < nFaces; i++) {
                int nodesInFace = ptIds[id]; // nodeIds in ptIds[id+1 .. id+nodesInFace]
                for (int k = 1; k <= nodesInFace; k++) _vtkIdList->SetId(n++, ptIds[id + k]);
                id += (nodesInFace + 1);
            }
            break;
        }
...
bool SMDS_VtkCellIteratorPolyH::more() { return (_index < _nbNodesInFaces); }

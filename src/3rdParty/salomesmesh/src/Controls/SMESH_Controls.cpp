...
inline double getAngle(const gp_XYZ &P1, const gp_XYZ &P2, const gp_XYZ &P3)
{
    ...
    return v1.Magnitude() < gp::Resolution() || v2.Magnitude() < gp::Resolution() ? 0
                                                                                  : v1.Angle(v2);
}
...
{
    if (theMesh == 0) return 0;

    const SMDS_MeshElement *anEdge = theMesh->FindElement(theId);
    if (anEdge == 0 || anEdge->GetType() != SMDSAbs_Edge /* || anEdge->NbNodes() != 2 */) return 0;
...
NumericalFunctor::NumericalFunctor() : myMesh(NULL) { myPrecision = -1; }

void NumericalFunctor::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }

bool NumericalFunctor::GetPoints(const int theId, TSequenceOfXYZ &theRes) const
{
    theRes.clear();

    if (myMesh == 0) return false;

    const SMDS_MeshElement *anElem = myMesh->FindElement(theId);
    if (!anElem || anElem->GetType() != this->GetType()) return false;

    return GetPoints(anElem, theRes);
}

bool NumericalFunctor::GetPoints(const SMDS_MeshElement *anElem, TSequenceOfXYZ &theRes)
{
    theRes.clear();

    if (anElem == 0) return false;
    ...
    if (anElem->IsQuadratic()) {
        switch (anElem->GetType()) {
            case SMDSAbs_Edge:
                anIter = dynamic_cast<const SMDS_VtkEdge *>(anElem)->interlacedNodesElemIterator();
                break;
            case SMDSAbs_Face:
                anIter = dynamic_cast<const SMDS_VtkFace *>(anElem)->interlacedNodesElemIterator();
                break;
            default: anIter = anElem->nodesIterator();
        }
    }
    else {
...
long NumericalFunctor::GetPrecision() const { return myPrecision; }
...
void NumericalFunctor::GetHistogram(int nbIntervals, std::vector<int> &nbEvents,
                                    std::vector<double> &funValues, const vector<int> &elements,
                                    const double *minmax, const bool isLogarithmic)
{
    if (nbIntervals < 1 || !myMesh || !myMesh->GetMeshInfo().NbElements(GetType())) return;
    ...
    if (elements.empty()) {
        ...
        while (elemIt->more()) values.insert(GetValue(elemIt->next()->GetID()));
    }
    else {
        vector<int>::const_iterator id = elements.begin();
        for (; id != elements.end(); ++id) values.insert(GetValue(*id));
    }
    ...
    for (int i = 0; i < nbIntervals; ++i) {
        ...
        if (min != values.end() && *min <= funValues[i + 1]) {
            ...
            max = values.upper_bound(
                funValues[i + 1]); // max is greater than funValues[i+1], or end()
...
double Volume::GetValue(long theElementId)
{
    if (theElementId && myMesh) {
        ...
        if (aVolumeTool.Set(myMesh->FindElement(theElementId))) return aVolumeTool.GetSize();
    }
    return 0;
}

double Volume::GetBadRate(double Value, int /*nbNodes*/) const { return Value; }

SMDSAbs_ElementType Volume::GetType() const { return SMDSAbs_Volume; }
...
double MaxElementLength2D::GetBadRate(double Value, int /*nbNodes*/) const { return Value; }

SMDSAbs_ElementType MaxElementLength2D::GetType() const { return SMDSAbs_Face; }
...
double MaxElementLength3D::GetBadRate(double Value, int /*nbNodes*/) const { return Value; }

SMDSAbs_ElementType MaxElementLength3D::GetType() const { return SMDSAbs_Volume; }
...
SMDSAbs_ElementType MinimumAngle::GetType() const { return SMDSAbs_Face; }
...
double AspectRatio::GetValue(long theId)
{
    ...
    if (myCurrElement && myCurrElement->GetVtkType() == VTK_QUAD) {
        ...
        TSequenceOfXYZ P;
        if (GetPoints(myCurrElement, P)) aVal = Round(GetValue(P));
    }
    return aVal;
}
...
double AspectRatio::GetValue(const TSequenceOfXYZ &P)
{
    ...
    if (nbNodes == 3) {
        ...
        for (int i = 0; i < nbNodes - 1; i++) aLen[i] = getDistance(P(i + 1), P(i + 2));
        ...
        if (anArea <= theEps) return theInf;
        ...
        double L = Max(aLen[0], Max(aLen[1], Max(aLen[2], Max(aLen[3], Max(aDia[0], aDia[1])))));
        double C1 = sqrt(
            (aLen[0] * aLen[0] + aLen[1] * aLen[1] + aLen[2] * aLen[2] + aLen[3] * aLen[3]) / 4.);
        double C2 = Min(anArea[0], Min(anArea[1], Min(anArea[2], anArea[3])));
        if (C2 <= theEps) return theInf;
...
SMDSAbs_ElementType AspectRatio::GetType() const { return SMDSAbs_Face; }
...
{
    return sqrt(theHalfPerim * (theHalfPerim - theTria[0]) * (theHalfPerim - theTria[1])
...
double AspectRatio3D::GetValue(const TSequenceOfXYZ &P)
{
    ...
    if (myCurrElement->IsQuadratic()) {
        if (nbNodes == 10) nbNodes = 4; // quadratic tetrahedron
            ...
            nbNodes = 5; // quadratic pyramid
        else if (nbNodes == 15)
            nbNodes = 6; // quadratic pentahedron
        else if (nbNodes == 20)
            nbNodes = 8; // quadratic hexahedron
        else if (nbNodes == 27)
            nbNodes = 8; // quadratic hexahedron
        else
            return aQuality;
    ...
    if (nbNodes > 4) {
        ...
        for (int i = 0; i < nbFaces; ++i) { // loop on faces of a volume
            if (SMDS_VolumeTool::NbFaceNodes(type, i) != 4) continue;
...
SMDSAbs_ElementType AspectRatio3D::GetType() const { return SMDSAbs_Volume; }
...
double Warping::ComputeA(const gp_XYZ &thePnt1, const gp_XYZ &thePnt2, const gp_XYZ &thePnt3,
                         const gp_XYZ &theG) const
{
    ...
    if (L < theEps) return theInf;

    gp_XYZ GI = (thePnt2 + thePnt1) / 2. - theG;
    gp_XYZ GJ = (thePnt3 + thePnt2) / 2. - theG;
    gp_XYZ N = GI.Crossed(GJ);

    if (N.Modulus() < gp::Resolution()) return M_PI / 2;
...
SMDSAbs_ElementType Warping::GetType() const { return SMDSAbs_Face; }
...
SMDSAbs_ElementType Taper::GetType() const { return SMDSAbs_Face; }
...
SMDSAbs_ElementType Skew::GetType() const { return SMDSAbs_Face; }
...
SMDSAbs_ElementType Area::GetType() const { return SMDSAbs_Face; }
    ...
    if (GetPoints(theElementId, P)) {
        ...
        switch (aType) {
            case SMDSEntity_Edge:
                if (len == 2) aVal = getDistance(P(1), P(2));
                break;
            ...
            case SMDSEntity_Quad_Polygon:
                if (len > 2) {
                    aVal =
                        getDistance(P(1), P(P.size())) + getDistance(P(P.size()), P(P.size() - 1));
                    for (size_t i = 1; i < P.size() - 1; i += 2)
                        aVal = Min(aVal,
                                   getDistance(P(i), P(i + 1)) + getDistance(P(i + 1), P(i + 2)));
                }
                break;
#endif
...
SMDSAbs_ElementType Length2D::GetType() const { return SMDSAbs_Face; }

Length2D::Value::Value(double theLength, long thePntId1, long thePntId2) : myLength(theLength)
{
    myPntId[0] = thePntId1;
    myPntId[1] = thePntId2;
...
void Length2D::GetValues(TValues &theValues)
{
    ...
    for (; anIter->more();) {
        ...
        if (anElem->IsQuadratic()) {
            const SMDS_VtkFace *F = dynamic_cast<const SMDS_VtkFace *>(anElem);
...
double MultiConnection::GetValue(const TSequenceOfXYZ &P) { return 0; }
double MultiConnection::GetValue(long theId) { return getNbMultiConnection(myMesh, theId); }
...
double MultiConnection2D::GetValue(const TSequenceOfXYZ &P) { return 0; }
    ...
    switch (aType) {
        case SMDSAbs_Face: {
            ...
            for (i = 0; i <= len; i++) {
                ...
                int aNb = 0;
                if (anIter->more()) { aNode = (SMDS_MeshNode *)anIter->next(); }
                else {
                    if (i == len) aNode = aNode0;
                    else
                        break;
                }
                if (!aNode) break;
...
SMDSAbs_ElementType MultiConnection2D::GetType() const { return SMDSAbs_Face; }
...
void MultiConnection2D::GetValues(MValues &theValues)
{
    ...
    for (; anIter->more();) {
        ...
        if (anElem->IsQuadratic())
            aNodesIter = dynamic_cast<const SMDS_VtkFace *>(anElem)->interlacedNodesElemIterator();
        else
...
SMDSAbs_ElementType BallDiameter::GetType() const { return SMDSAbs_Ball; }
...
BadOrientedVolume::BadOrientedVolume() { myMesh = 0; }

void BadOrientedVolume::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
SMDSAbs_ElementType BadOrientedVolume::GetType() const { return SMDSAbs_Volume; }
    ...
    if (myTool.Set(myMesh->FindElement(theElementId))) {
        for (int iF = 0; iF < myTool.NbFaces(); ++iF)
            if (myTool.IsFreeFace(iF)) {
                ...
                if (!myMesh->FindElement(nodes, SMDSAbs_Face, /*Nomedium=*/false)) return true;
            }
    }
    return false;
}
        ...
        if (face->GetType() == SMDSAbs_Face) {
            ...
            for (int i = 0; i < nbN && !ok; ++i) {
                ...
                if (!isShared) {
                    ...
                    myLinkNodes[0] = n1;
                    myLinkNodes[1] = n2;
                    if (iQuad) myLinkNodes[2] = face->GetNode(i + nbN);
...
bool OverConstrainedVolume::IsSatisfy(long theElementId)
{
    ...
    if (myTool.Set(myMesh->FindElement(theElementId))) {
        int nbSharedFaces = 0;
        for (int iF = 0; iF < myTool.NbFaces(); ++iF)
            if (!myTool.IsFreeFace(iF) && ++nbSharedFaces > 1) break;
    ...
    if (const SMDS_MeshElement *face = myMesh->FindElement(theElementId))
        if (face->GetType() == SMDSAbs_Face) {
            ...
            for (int i = 0; i < nbN; ++i) {
                ...
                if (isShared && ++nbSharedBorders > 1) break;
            }
...
CoincidentNodes::CoincidentNodes() { myToler = 1e-5; }
...
SMDSAbs_ElementType CoincidentNodes::GetType() const { return SMDSAbs_Node; }
...
void CoincidentNodes::SetMesh(const SMDS_Mesh *theMesh)
{
    ...
    if (myMeshModifTracer.IsMeshModified()) {
        ...
        while (nIt->more()) nodesToCheck.insert(nodesToCheck.end(), nIt->next());
        ...
        for (; groupIt != nodeGroups.end(); ++groupIt) {
            ...
            for (; n != coincNodes.end(); ++n) myCoincidentIDs.Add((*n)->GetID());
        }
    }
}
...
CoincidentElements::CoincidentElements() { myMesh = 0; }

void CoincidentElements::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
SMDSAbs_ElementType CoincidentElements1D::GetType() const { return SMDSAbs_Edge; }
SMDSAbs_ElementType CoincidentElements2D::GetType() const { return SMDSAbs_Face; }
SMDSAbs_ElementType CoincidentElements3D::GetType() const { return SMDSAbs_Volume; }
...
FreeBorders::FreeBorders() { myMesh = 0; }

void FreeBorders::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }

bool FreeBorders::IsSatisfy(long theId) { return getNbMultiConnection(myMesh, theId) == 1; }

SMDSAbs_ElementType FreeBorders::GetType() const { return SMDSAbs_Edge; }
...
FreeEdges::FreeEdges() { myMesh = 0; }

void FreeEdges::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (anElemIter->more()) {
            if (const SMDS_MeshElement *anElem = anElemIter->next()) {
                ...
                if (anId != theFaceId && !aMap.Add(anId)) return false;
            }
        }
    }
    return true;
...
bool FreeEdges::IsSatisfy(long theId)
{
    ...
    if (aFace == 0 || aFace->GetType() != SMDSAbs_Face || aFace->NbNodes() < 3) return false;

    SMDS_NodeIteratorPtr anIter = aFace->interlacedNodesIterator();
    if (!anIter) return false;

    int i = 0, nbNodes = aFace->NbNodes();
    std::vector<const SMDS_MeshNode *> aNodes(nbNodes + 1);
    while (anIter->more())
        if (!(aNodes[i++] = anIter->next())) return false;
    aNodes[nbNodes] = aNodes[0];

    for (i = 0; i < nbNodes; i++)
        if (IsFreeEdge(&aNodes[i], theId)) return true;

    return false;
}

SMDSAbs_ElementType FreeEdges::GetType() const { return SMDSAbs_Face; }

FreeEdges::Border::Border(long theElemId, long thePntId1, long thePntId2) : myElemId(theElemId)
{
    myPntId[0] = thePntId1;
    myPntId[1] = thePntId2;
    if (thePntId1 > thePntId2) {
...
inline void UpdateBorders(const FreeEdges::Border &theBorder, FreeEdges::TBorders &theRegistry,
...
void FreeEdges::GetBoreders(TBorders &theBorders)
{
    ...
    for (; anIter->more();) {
        ...
        SMDS_ElemIteratorPtr aNodesIter;
        if (anElem->IsQuadratic())
            aNodesIter = static_cast<const SMDS_VtkFace *>(anElem)->interlacedNodesElemIterator();
        else
            aNodesIter = anElem->nodesIterator();
        long aNodeId[2];
        const SMDS_MeshElement *aNode;
...
FreeNodes::FreeNodes() { myMesh = 0; }

void FreeNodes::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
SMDSAbs_ElementType FreeNodes::GetType() const { return SMDSAbs_Node; }
...
FreeFaces::FreeFaces() { myMesh = 0; }

void FreeFaces::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
bool FreeFaces::IsSatisfy(long theId)
{
    ...
    const SMDS_MeshElement *aFace = myMesh->FindElement(theId);
    if (!aFace || aFace->GetType() != SMDSAbs_Face) return false;
    ...
    for (; volItr != volEnd; ++volItr)
        if ((*volItr).second >= nbNode) nbVol++;
    // face is not free if number of volumes constructed on thier nodes more than one
    return (nbVol < 2);
}

SMDSAbs_ElementType FreeFaces::GetType() const { return SMDSAbs_Face; }
...
LinearOrQuadratic::LinearOrQuadratic() { myMesh = 0; }

void LinearOrQuadratic::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }

bool LinearOrQuadratic::IsSatisfy(long theId)
{
    if (!myMesh) return false;
    const SMDS_MeshElement *anElem = myMesh->FindElement(theId);
    if (!anElem || (myType != SMDSAbs_All && anElem->GetType() != myType)) return false;
    return (!anElem->IsQuadratic());
}

void LinearOrQuadratic::SetType(SMDSAbs_ElementType theType) { myType = theType; }

SMDSAbs_ElementType LinearOrQuadratic::GetType() const { return myType; }
...
bool GroupColor::IsSatisfy(long theId) { return myIDs.count(theId); }

void GroupColor::SetType(SMDSAbs_ElementType theType) { myType = theType; }

SMDSAbs_ElementType GroupColor::GetType() const { return myType; }
...
void GroupColor::SetMesh(const SMDS_Mesh *theMesh)
{
    ...
    for (; GrIt != aGroups.end(); GrIt++) {
        ...
        if (!isEqual(myColor, aGrp->GetColor())) continue;
        ...
        if (SMESHDS_GroupOnFilter *gof = dynamic_cast<SMESHDS_GroupOnFilter *>(aGrp))
            if (gof->GetPredicate().get() == this) continue;
        ...
        if (myType == aGrpElType || (myType == SMDSAbs_All && aGrpElType != SMDSAbs_Node)) {
            ...
            for (int i = 0; i < aSize; i++) myIDs.insert(aGrp->GetID(i + 1));
        }
    }
}
...
void GroupColor::SetColorStr(const TCollection_AsciiString &theStr)
{
    ...
    aStr.RemoveAll(' ');
    aStr.RemoveAll('\t');
    for (int aPos = aStr.Search(";;"); aPos != -1; aPos = aStr.Search(";;")) aStr.Remove(aPos, 2);
    Standard_Real clr[3];
    clr[0] = clr[1] = clr[2] = 0.;
    for (int i = 0; i < 3; i++) {
        TCollection_AsciiString tmpStr = aStr.Token(";", i + 1);
        if (!tmpStr.IsEmpty() && tmpStr.IsRealValue()) clr[i] = tmpStr.RealValue();
    }
...
void ElemGeomType::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
bool ElemGeomType::IsSatisfy(long theId)
{
    ...
    if (myType != SMDSAbs_All && anElemType != myType) return false;
    bool isOk = (anElem->GetGeomType() == myGeomType);
    return isOk;
}

void ElemGeomType::SetType(SMDSAbs_ElementType theType) { myType = theType; }

SMDSAbs_ElementType ElemGeomType::GetType() const { return myType; }

void ElemGeomType::SetGeomType(SMDSAbs_GeometryType theType) { myGeomType = theType; }

SMDSAbs_GeometryType ElemGeomType::GetGeomType() const { return myGeomType; }
...
ElemEntityType::ElemEntityType() : myMesh(0), myType(SMDSAbs_All), myEntityType(SMDSEntity_0D) {}

void ElemEntityType::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }

bool ElemEntityType::IsSatisfy(long theId)
{
    if (!myMesh) return false;
    if (myType == SMDSAbs_Node) return myMesh->FindNode(theId);
    const SMDS_MeshElement *anElem = myMesh->FindElement(theId);
    return (anElem && myEntityType == anElem->GetEntityType());
}

void ElemEntityType::SetType(SMDSAbs_ElementType theType) { myType = theType; }

SMDSAbs_ElementType ElemEntityType::GetType() const { return myType; }
...
SMDSAbs_EntityType ElemEntityType::GetElemEntityType() const { return myEntityType; }
...
ConnectedElements::ConnectedElements() : myNodeID(0), myType(SMDSAbs_All), myOkIDsReady(false) {}

SMDSAbs_ElementType ConnectedElements::GetType() const { return myType; }

int ConnectedElements::GetNode() const
{
    return myXYZ.empty() ? myNodeID : 0;
} // myNodeID can be found by myXYZ

std::vector<double> ConnectedElements::GetPoint() const { return myXYZ; }
...
{
    if (myType != theType || myMeshModifTracer.IsMeshModified()) clearOkIDs();
    myType = theType;
}
...
void ConnectedElements::SetNode(int nodeID)
{
    ...
    if (myOkIDsReady && myMeshModifTracer.GetMesh() && !myMeshModifTracer.IsMeshModified())
        if (const SMDS_MeshNode *n = myMeshModifTracer.GetMesh()->FindNode(myNodeID)) {
            ...
            while (!isSameDomain && eIt->more()) isSameDomain = IsSatisfy(eIt->next()->GetID());
        }
    if (!isSameDomain) clearOkIDs();
}
...
bool ConnectedElements::IsSatisfy(long theElementId)
{
    ...
    if (!myOkIDsReady) {
        if (!myMeshModifTracer.GetMesh()) return false;
        ...
        while (!nodeQueue.empty()) {
            ...
            while (eIt->more()) {
                ...
                if (element->GetType() == myType) myOkIDs.insert(myOkIDs.end(), element->GetID());

                // enqueue nodes of the element
                SMDS_ElemIteratorPtr nIt = element->nodesIterator();
                while (nIt->more()) {
                    const SMDS_MeshNode *n = static_cast<const SMDS_MeshNode *>(nIt->next());
                    if (checkedNodeIDs.insert(n->GetID()).second) nodeQueue.push_back(n);
                }
            }
        }
        if (myType == SMDSAbs_Node) std::swap(myOkIDs, checkedNodeIDs);

        size_t totalNbElems = myMeshModifTracer.GetMesh()->GetMeshInfo().NbElements(myType);
        if (myOkIDs.size() == totalNbElems) myOkIDs.clear();
...
CoplanarFaces::CoplanarFaces() : myFaceID(0), myToler(0) {}
void CoplanarFaces::SetMesh(const SMDS_Mesh *theMesh)
{
    ...
    if (myMeshModifTracer.IsMeshModified()) {
        ...
        if (!myMeshModifTracer.GetMesh() || !myFaceID || !myToler) return;

        const SMDS_MeshElement *face = myMeshModifTracer.GetMesh()->FindElement(myFaceID);
        if (!face || face->GetType() != SMDSAbs_Face) return;
        ...
        while (!faceQueue.empty()) {
            ...
            for (int i = 0, nbN = face->NbCornerNodes(); i < nbN; ++i) {
                const SMDS_MeshNode *n1 = face->GetNode(i);
                const SMDS_MeshNode *n2 = face->GetNode((i + 1) % nbN);
                if (!checkedLinks.insert(SMESH_TLink(n1, n2)).second) continue;
                SMDS_ElemIteratorPtr fIt = n1->GetInverseElementIterator(SMDSAbs_Face);
                while (fIt->more()) {
                    const SMDS_MeshElement *f = fIt->next();
...
bool CoplanarFaces::IsSatisfy(long theElementId) { return myCoplanarIDs.count(theElementId); }
...
// Purpose : Set mesh
//=======================================================================
void RangeOfIds::SetMesh(const SMDS_Mesh *theMesh) { myMesh = theMesh; }
...
void RangeOfIds::GetRangeStr(TCollection_AsciiString &theResStr)
{
    ...
    for (int i = 1, n = myMin.Length(); i <= n; i++) {
        ...
        if (aMinId != IntegerFirst()) aStr += aMinId;

        aStr += "-";

        if (aMaxId != IntegerLast()) aStr += aMaxId;
...
bool RangeOfIds::SetRangeStr(const TCollection_AsciiString &theStr)
{
    ...
    for (int i = 1; i <= aStr.Length(); ++i)
        if (isspace(aStr.Value(i))) aStr.SetValue(i, ',');

    for (int aPos = aStr.Search(",,"); aPos != -1; aPos = aStr.Search(",,")) aStr.Remove(aPos, 1);
    ...
    while (tmpStr != "") {
        ...
        if (aPos == -1) {
            if (tmpStr.IsIntegerValue()) myIds.Add(tmpStr.IntegerValue());
            else
                return false;
        }
        else {
...
//=======================================================================
SMDSAbs_ElementType RangeOfIds::GetType() const { return myType; }

//=======================================================================
// name    : SetType
// Purpose : Set type of supported entities
//=======================================================================
void RangeOfIds::SetType(SMDSAbs_ElementType theType) { myType = theType; }
...
bool RangeOfIds::IsSatisfy(long theId)
{
    ...
    if (myType == SMDSAbs_Node) {
        if (myMesh->FindNode(theId) == 0) return false;
    }
    else {
        const SMDS_MeshElement *anElem = myMesh->FindElement(theId);
        if (anElem == 0 || (myType != anElem->GetType() && myType != SMDSAbs_All)) return false;
    }

    if (myIds.Contains(theId)) return true;

    for (int i = 1, n = myMin.Length(); i <= n; i++)
        if (theId >= myMin(i) && theId <= myMax(i)) return true;
...
*/
Comparator::Comparator() : myMargin(0) {}

Comparator::~Comparator() {}

void Comparator::SetMesh(const SMDS_Mesh *theMesh)
{
    if (myFunctor) myFunctor->SetMesh(theMesh);
}

void Comparator::SetMargin(double theValue) { myMargin = theValue; }

void Comparator::SetNumFunctor(NumericalFunctorPtr theFunct) { myFunctor = theFunct; }
...
double Comparator::GetMargin() { return myMargin; }
...
*/
bool LessThan::IsSatisfy(long theId) { return myFunctor && myFunctor->GetValue(theId) < myMargin; }
...
/*
  ...
  Description : Comparator ">"
*/
bool MoreThan::IsSatisfy(long theId) { return myFunctor && myFunctor->GetValue(theId) > myMargin; }
...
/*
  ...
  Description : Comparator "="
*/
EqualTo::EqualTo() : myToler(Precision::Confusion()) {}
...
void EqualTo::SetTolerance(double theToler) { myToler = theToler; }

double EqualTo::GetTolerance() { return myToler; }
...
bool LogicalNOT::IsSatisfy(long theId) { return myPredicate && !myPredicate->IsSatisfy(theId); }

void LogicalNOT::SetMesh(const SMDS_Mesh *theMesh)
{
    if (myPredicate) myPredicate->SetMesh(theMesh);
}

void LogicalNOT::SetPredicate(PredicatePtr thePred) { myPredicate = thePred; }
...
{
    if (myPredicate1) myPredicate1->SetMesh(theMesh);

    if (myPredicate2) myPredicate2->SetMesh(theMesh);
}

void LogicalBinary::SetPredicate1(PredicatePtr thePredicate) { myPredicate1 = thePredicate; }

void LogicalBinary::SetPredicate2(PredicatePtr thePredicate) { myPredicate2 = thePredicate; }

SMDSAbs_ElementType LogicalBinary::GetType() const
{
    if (!myPredicate1 || !myPredicate2) return SMDSAbs_All;
...
{
    return myPredicate1 && myPredicate2 && myPredicate1->IsSatisfy(theId)
...
{
    return myPredicate1 && myPredicate2
...
void Filter::SetPredicate(PredicatePtr thePredicate) { myPredicate = thePredicate; }

void Filter::GetElementsId(const SMDS_Mesh *theMesh, PredicatePtr thePredicate,
                           TIdSequence &theSequence)
{
    theSequence.clear();

    if (!theMesh || !thePredicate) return;
    ...
    if (elemIt) {
        while (elemIt->more()) {
            ...
            if (thePredicate->IsSatisfy(anId)) theSequence.push_back(anId);
        }
    }
}

void Filter::GetElementsId(const SMDS_Mesh *theMesh, Filter::TIdSequence &theSequence)
{
...
ManifoldPart::Link::Link(SMDS_MeshNode *theNode1, SMDS_MeshNode *theNode2)
{
    myNode1 = theNode1;
    myNode2 = theNode2;
}
...
{
    if (myNode1 == theLink.myNode1 && myNode2 == theLink.myNode2) return true;
    else if (myNode1 == theLink.myNode2 && myNode2 == theLink.myNode1)
        return true;
    else
        return false;
}
...
bool ManifoldPart::IsEqual(const ManifoldPart::Link &theLink1, const ManifoldPart::Link &theLink2)
{
...
ManifoldPart::~ManifoldPart() { myMesh = 0; }
...
SMDSAbs_ElementType ManifoldPart::GetType() const { return SMDSAbs_Face; }

bool ManifoldPart::IsSatisfy(long theElementId) { return myMapIds.Contains(theElementId); }

void ManifoldPart::SetAngleTolerance(const double theAngToler) { myAngToler = theAngToler; }

double ManifoldPart::GetAngleTolerance() const { return myAngToler; }

void ManifoldPart::SetIsOnlyManifold(const bool theIsOnly) { myIsOnlyManifold = theIsOnly; }

void ManifoldPart::SetStartElem(const long theStartId) { myStartElemId = theStartId; }
...
bool ManifoldPart::process()
{
    ...
    for (int fi = aStartIndx; !isStartTreat || fi != aStartIndx; fi++) {
        if (fi == aStartIndx) isStartTreat = true;
        // as result next time when fi will be equal to aStartIndx

        SMDS_MeshFace *aFacePtr = myAllFacePtr[fi];
        if (aMapOfTreated.Contains(aFacePtr->GetID())) continue;

        aMapOfTreated.Add(aFacePtr->GetID());
        TColStd_MapOfInteger aResFaces;
        if (!findConnected(myAllFacePtrIntDMap, aFacePtr, aMapOfNonManifold, aResFaces)) continue;
        ...
        if (fi == (myAllFacePtr.size() - 1)) fi = 0;
...
bool ManifoldPart::findConnected(const ManifoldPart::TDataMapFacePtrInt &theAllFacePtrInt,
                                 SMDS_MeshFace *theStartFace,
                                 ManifoldPart::TMapOfLink &theNonManifold,
                                 TColStd_MapOfInteger &theResFaces)
{
    theResFaces.Clear();
    if (!theAllFacePtrInt.size()) return false;
    ...
    expandBoundary(aMapOfBoundary, aSeqOfBoundary, aDMapLinkFace, theNonManifold, theStartFace);
    ...
    while (!isDone && aMapOfBoundary.size() != 0) {
        ...
        for (; !isToReset && pLink != aSeqOfBoundary.end(); ++pLink) {
            ...
            if (aMapToSkip.find(aLink) != aMapToSkip.end()) continue;
            // each link could be treated only once
            aMapToSkip.insert(aLink);

            ManifoldPart::TVectorOfFacePtr aFaces;
            // find next
            if (myIsOnlyManifold && (theNonManifold.find(aLink) != theNonManifold.end())) continue;
            else {
            ...
            for (; pFace != aFaces.end(); ++pFace) {
                ...
                if (aPrevFace == aNextFace) continue;
                ...
                if (myMapBadGeomIds.Contains(anNextFaceID) || !isInPlane(aPrevFace, aNextFace))
                    continue;
                // add new element to connected and extend the boundaries.
                theResFaces.Add(anNextFaceID);
                expandBoundary(aMapOfBoundary, aSeqOfBoundary, aDMapLinkFace, theNonManifold,
                               aNextFace);
                isToReset = true;
            }
        }
...
bool ManifoldPart::isInPlane(const SMDS_MeshFace *theFace1, const SMDS_MeshFace *theFace2)
{
    ...
    if (aNorm1.IsParallel(gp_Dir(aNorm2XYZ), myAngToler)) return true;
                                  ...
                                  ManifoldPart::TVectorOfFacePtr &theFaces) const
{
    ...
    for (; anItr->more();) {
        SMDS_MeshFace *aFace = (SMDS_MeshFace *)anItr->next();
        if (aSetOfFaces.count(aFace)) theFaces.push_back(aFace);
    }
}
...
void BelongToMeshGroup::SetGroup(SMESHDS_GroupBase *g) { myGroup = g; }

void BelongToMeshGroup::SetStoreName(const std::string &sn) { myStoreName = sn; }

void BelongToMeshGroup::SetMesh(const SMDS_Mesh *theMesh)
{
    if (myGroup && myGroup->GetMesh() != theMesh) { myGroup = 0; }
    if (!myGroup && !myStoreName.empty()) {
        if (const SMESHDS_Mesh *aMesh = dynamic_cast<const SMESHDS_Mesh *>(theMesh)) {
            ...
            for (; g != grps.end() && !myGroup; ++g)
                if (*g && myStoreName == (*g)->GetStoreName()) myGroup = *g;
        }
    }
    if (myGroup) {
...
{
    myMeshModifTracer.SetMesh(theMesh);
    if (myMeshModifTracer.IsMeshModified()) process();
}

bool ElementsOnSurface::IsSatisfy(long theElementId) { return myIds.Contains(theElementId); }

SMDSAbs_ElementType ElementsOnSurface::GetType() const { return myType; }

void ElementsOnSurface::SetTolerance(const double theToler)
{
    if (myToler != theToler) myIds.Clear();
    myToler = theToler;
}

double ElementsOnSurface::GetTolerance() const { return myToler; }
...
void ElementsOnSurface::SetSurface(const TopoDS_Shape &theShape, const SMDSAbs_ElementType theType)
{
    myIds.Clear();
    myType = theType;
    mySurf.Nullify();
    if (theShape.IsNull() || theShape.ShapeType() != TopAbs_FACE) return;
    mySurf = TopoDS::Face(theShape);
    BRepAdaptor_Surface SA(mySurf, myUseBoundaries);
    Standard_Real u1 = SA.FirstUParameter(), u2 = SA.LastUParameter(), v1 = SA.FirstVParameter(),
...
void ElementsOnSurface::process()
{
    ...
    if (!myMeshModifTracer.GetMesh()) return;
    ...
    for (; anIter->more();) process(anIter->next());
}
...
void ElementsOnSurface::process(const SMDS_MeshElement *theElemPtr)
{
    ...
    if (isSatisfy) myIds.Add(theElemPtr->GetID());
}

bool ElementsOnSurface::isOnSurface(const SMDS_MeshNode *theNode)
{
    if (mySurf.IsNull()) return false;
...
ElementsOnShape::ElementsOnShape()
    : //myMesh(0),
      myType(SMDSAbs_All), myToler(Precision::Confusion()), myAllNodesFlag(false)
{}

ElementsOnShape::~ElementsOnShape() { clearClassifiers(); }

SMDSAbs_ElementType ElementsOnShape::GetType() const { return myType; }
...
double ElementsOnShape::GetTolerance() const { return myToler; }

void ElementsOnShape::SetAllNodes(bool theAllNodes) { myAllNodesFlag = theAllNodes; }
...
{
    if (n->GetID() >= (int)myNodeIsChecked.size() || !myNodeIsChecked[n->GetID()]) return false;
...
void ElementsOnShape::SetShape(const TopoDS_Shape &theShape, const SMDSAbs_ElementType theType)
{
    myType = theType;
    myShape = theShape;
...
{
    for (size_t i = 0; i < myClassifiers.size(); ++i) delete myClassifiers[i];
    myClassifiers.clear();
}
...
bool ElementsOnShape::IsSatisfy(long elemId)
{
    ...
    if (!elem || myClassifiers.empty()) return false;
    ...
    if (isSatisfy && myAllNodesFlag && myClassifiers[0]->ShapeType() == TopAbs_SOLID) {
...
TopAbs_ShapeEnum ElementsOnShape::TClassifier::ShapeType() const { return myShape.ShapeType(); }

bool ElementsOnShape::TClassifier::IsOut(const gp_Pnt &p) { return (this->*myIsOutFun)(p); }
...
void ElementsOnShape::TClassifier::Init(const TopoDS_Shape &theShape, double theTol)
{
    ...
    switch (myShape.ShapeType()) {
        case TopAbs_SOLID: {
            if (isBox(theShape)) { myIsOutFun = &ElementsOnShape::TClassifier::isOutOfBox; }
            else {
...
bool ElementsOnShape::TClassifier::isOutOfBox(const gp_Pnt &p) { return myBox.IsOut(p.XYZ()); }
...
bool ElementsOnShape::TClassifier::isOutOfFace(const gp_Pnt &p)
{
    ...
    if (myProjFace.IsDone() && myProjFace.LowerDistance() <= myTol) {
        ...
        if (aClsf.State() == TopAbs_IN || aClsf.State() == TopAbs_ON) return false;
    }
    return true;
}
...
bool ElementsOnShape::TClassifier::isBox(const TopoDS_Shape &theShape)
{
    ...
    if (vMap.Extent() != 8) return false;

    myBox.Clear();
    for (int i = 1; i <= 8; ++i) myBox.Add(BRep_Tool::Pnt(TopoDS::Vertex(vMap(i))).XYZ());
    ...
    for (int i = 1; i <= 8; ++i) {
        ...
        for (int iC = 1; iC <= 3; ++iC) {
            ...
            if (Min(d1, d2) > myTol) return false;
        }
    }
    myBox.Enlarge(myTol);
    return true;
...
static bool IsSubShape(const TopTools_IndexedMapOfShape &theMap, const TopoDS_Shape &theShape)
{
    ...
    if (theShape.ShapeType() == TopAbs_COMPOUND || theShape.ShapeType() == TopAbs_COMPSOLID) {
        ...
        for (; anIt.More(); anIt.Next()) {
            if (!IsSubShape(theMap, anIt.Value())) { return false; }
        }
        return true;
    }
...
void BelongToGeom::init()
{
    ...
    if (aMainShape.IsNull()) { myIsSubshape = false; }
    else {
...
static bool IsContains(const SMESHDS_Mesh *theMeshDS, const TopoDS_Shape &theShape,
                       const SMDS_MeshElement *theElem, TopAbs_ShapeEnum theFindShapeEnum,
                       TopAbs_ShapeEnum theAvoidShapeEnum = TopAbs_SHAPE)
{
    ...
    while (anExp.More()) {
        ...
        if (SMESHDS_SubMesh *aSubMesh = theMeshDS->MeshElements(aShape)) {
            if (aSubMesh->Contains(theElem)) return true;
        }
        anExp.Next();
    }
    return false;
...
{
    if (myMeshDS == 0 || myShape.IsNull()) return false;

    if (!myIsSubshape) { return myElementsOnShapePtr->IsSatisfy(theId); }

    // Case of submesh
    if (myType == SMDSAbs_Node) {
        if (const SMDS_MeshNode *aNode = myMeshDS->FindNode(theId)) {
            if (aNode->getshapeId() < 1) return myElementsOnShapePtr->IsSatisfy(theId);
            ...
            switch (aTypeOfPosition) {
                ...
                case SMDS_TOP_3DSPACE:
                    return (IsContains(myMeshDS, myShape, aNode, TopAbs_SOLID)
        ...
        if (const SMDS_MeshElement *anElem = myMeshDS->FindElement(theId)) {
            if (anElem->getshapeId() < 1) return myElementsOnShapePtr->IsSatisfy(theId);
            ...
            if (myType == SMDSAbs_All) {
                ...
                switch (myType) {
                    ...
                    case SMDSAbs_Volume:
                        return (IsContains(myMeshDS, myShape, anElem, TopAbs_SOLID)
                                || IsContains(myMeshDS, myShape, anElem, TopAbs_SHELL));
                }
            }
        }
...
SMDSAbs_ElementType BelongToGeom::GetType() const { return myType; }

TopoDS_Shape BelongToGeom::GetShape() { return myShape; }

const SMESHDS_Mesh *BelongToGeom::GetMeshDS() const { return myMeshDS; }
...
double BelongToGeom::GetTolerance() { return myTolerance; }
...
void LyingOnGeom::init()
{
    ...
    if (myIsSubshape) {
        ...
        for (int i = 1; i <= shapes.Extent(); ++i) {
            ...
            if (subID > 0) mySubShapesIDs.Add(subID);
        }
    }
    else {
        myElementsOnShapePtr.reset(new ElementsOnShape());
...
bool LyingOnGeom::IsSatisfy(long theId)
{
    ...
    if (mySubShapesIDs.Contains(elem->getshapeId())) return true;

    if (elem->GetType() != SMDSAbs_Node) {
        SMDS_ElemIteratorPtr nodeItr = elem->nodesIterator();
        while (nodeItr->more()) {
            const SMDS_MeshElement *aNode = nodeItr->next();
            if (mySubShapesIDs.Contains(aNode->getshapeId())) return true;
        }
    }
...
SMDSAbs_ElementType LyingOnGeom::GetType() const { return myType; }

TopoDS_Shape LyingOnGeom::GetShape() { return myShape; }

const SMESHDS_Mesh *LyingOnGeom::GetMeshDS() const { return myMeshDS; }
...
double LyingOnGeom::GetTolerance() { return myTolerance; }

bool LyingOnGeom::Contains(const SMESHDS_Mesh *theMeshDS, const TopoDS_Shape &theShape,
                           const SMDS_MeshElement *theElem, TopAbs_ShapeEnum theFindShapeEnum,
...
TSequenceOfXYZ::TSequenceOfXYZ(const TSequenceOfXYZ &theSequenceOfXYZ)
    : myArray(theSequenceOfXYZ.myArray), myElem(theSequenceOfXYZ.myElem)
{}

template<class InputIterator>
TSequenceOfXYZ::TSequenceOfXYZ(InputIterator theBegin, InputIterator theEnd)
    : myArray(theBegin, theEnd), myElem(0)
{}
...
gp_XYZ &TSequenceOfXYZ::operator()(size_type n) { return myArray[n - 1]; }

const gp_XYZ &TSequenceOfXYZ::operator()(size_type n) const { return myArray[n - 1]; }

void TSequenceOfXYZ::clear() { myArray.clear(); }

void TSequenceOfXYZ::reserve(size_type n) { myArray.reserve(n); }

void TSequenceOfXYZ::push_back(const gp_XYZ &v) { myArray.push_back(v); }

TSequenceOfXYZ::size_type TSequenceOfXYZ::size() const { return myArray.size(); }
...
{
    if (theMesh != myMesh) myMeshModifTime = 0;
    myMesh = theMesh;
}

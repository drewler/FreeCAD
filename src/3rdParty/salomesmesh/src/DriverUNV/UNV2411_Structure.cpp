...
UNV2411::TRecord::TRecord()
    : exp_coord_sys_num(1), disp_coord_sys_num(1),
   ...
   */
    ...
    if (!in_stream.eof()) {
        ...
        for (size_t i = 0; i < num_buf.size();) {
            // skip spaces
            while (i < num_buf.size() && num_buf[i] == ' ') ++i;
...
void UNV2411::Write(std::ofstream &out_stream, const TDataSet &theDataSet)
{
   ...
   */
    ...
    for (; anIter != theDataSet.end(); anIter++) {
        ...
        char buf[78];
        sprintf(buf, "%10d%10d%10d%10d\n", aRec.label, aRec.exp_coord_sys_num,
                aRec.disp_coord_sys_num, aRec.color);
        out_stream << buf;

        // the coordinates
        sprintf(buf, "%25.16E%25.16E%25.16E\n", aRec.coord[0], aRec.coord[1], aRec.coord[2]);
        out_stream << buf;
    }

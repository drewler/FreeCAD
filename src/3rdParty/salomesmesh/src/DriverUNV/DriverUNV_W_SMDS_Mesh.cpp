...
inline int GetConnect(const SMDS_ElemIteratorPtr &theNodesIter, TConnect &theConnect)
{
    theConnect.clear();
...
Driver_Mesh::Status DriverUNV_W_SMDS_Mesh::Perform()
{
    ...
    try {
        ...
        }
        {
            ...
            if (myMesh->NbEdges()) {
                ...
                while (anIter->more()) {
                    ...
                    if (anElem->IsQuadratic()) { aRec.fe_descriptor_id = 22; }
                    else {
      ...
      UNV2417::Write(out_stream,aDataSet2417);
        ...
        out_stream.flush();
        out_stream.close();
        if (!check_file(myFile)) EXCEPTION(runtime_error, "ERROR: Output file not good.");
    }

...
static string _group_labels[] = {"2417", "2429", "2430", "2432", "2435", "2452", "2467", "2477"};
#define NBGROUP 8
...
void UNV2417::Read(std::ifstream &in_stream, TDataSet &theDataSet)
{
     ...
     */
        ...
        if (in_stream.eof()) return;
        for (int i = 0; i < NBGROUP; i++) {
            if (news == _group_labels[i]) { ReadGroup(news, in_stream, theDataSet); }
        }
    }
}


void UNV2417::ReadGroup(const std::string &myGroupLabel, std::ifstream &in_stream,
                        TDataSet &theDataSet)
{
    TGroupId aId;
    for (; !in_stream.eof();) {
        ...
        for (int j = 0; j < n_nodes; j++) {
            ...
            if ((myGroupLabel.compare("2435") == 0) || (myGroupLabel.compare("2452") == 0)
                || (myGroupLabel.compare("2467") == 0) || (myGroupLabel.compare("2477") == 0)) {
                in_stream >> aTmp;
                in_stream >> aTmp;
            }
            switch (aElType) {

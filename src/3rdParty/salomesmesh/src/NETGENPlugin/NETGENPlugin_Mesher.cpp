...
DLL_HEADER extern int OCCGenerateMesh(OCCGeometry &, shared_ptr<Mesh> &, MeshingParameters &, int,
                                      int);
...
NETGENPlugin_Mesher::NETGENPlugin_Mesher(SMESH_Mesh *mesh, const TopoDS_Shape &aShape,
                                         const bool isVolume)
    : _mesh(mesh), _shape(aShape), _isVolume(isVolume), _optimize(true),
      _fineness(NETGENPlugin_Hypothesis::GetDefaultFineness()), _isViscousLayers2D(false),
      _ngMesh(NULL), _occgeom(NULL), _curShapeIndex(-1), _progressTic(1), _totalTime(1.0),
      _simpleHyp(NULL), _ptrToMe(NULL)
{
...
void SetLocalSize(TopoDS_Shape GeomShape, double LocalSize)
{
    ...
    int key;
    if (!ShapesWithLocalSize.Contains(GeomShape)) key = ShapesWithLocalSize.Add(GeomShape);
    else
        key = ShapesWithLocalSize.FindIndex(GeomShape);
    if (GeomType == TopAbs_VERTEX) { VertexId2LocalSize[key] = LocalSize; }
...
{
    _simpleHyp = hyp;
    if (_simpleHyp) SetDefaultParameters();
}
...
int HashCode(const Link &aLink, int aLimit) { return HashCode(aLink.n1 + aLink.n2, aLimit); }
...
int ngNodeId(const SMDS_MeshNode *node, netgen::Mesh &ngMesh, TNode2IdMap &nodeNgIdMap)
{
...
list<TopoDS_Edge> getConnectedEdges(const TopoDS_Edge &edge, const TopoDS_Face &face,
                                    ...
                                    map<SMESH_subMesh *, set<int>> &addedEdgeSM2Faces)
{
    ...
    while (edges.begin() != eItFwd) edges.splice(edges.end(), edges, edges.begin());
    ...
    while (++eItFwd != edges.end()) {
        ...
        bool orientOK =
            ((ePrev->Orientation() < TopAbs_INTERNAL) == (eItFwd->Orientation() < TopAbs_INTERNAL));
        if (!connected || !computed || !orientOK || added || doubled) {
            // stop advancement; move edges from tail to head
            while (edges.back() != *ePrev) edges.splice(edges.begin(), edges, --edges.end());
            break;
        }
        ePrev = eItFwd;
    }
    ...
    if (edges.front() != edges.back()) {
        ...
        while (!SMESH_Algo::VertexNode(SMESH_MesherHelper::IthVertex(0, edges.front()),
                                       helper.GetMeshDS())
               && edges.front() != eLast)
...
const SMDS_MeshNode *mediumNode(const SMDS_MeshNode *corner1, const SMDS_MeshNode *corner2,
                                ...
                                const SMESH_MesherHelper *helper)
{
    if (helper) {
        ...
        if (l2n != helper->GetTLinkNodeMap().end()) defaultMedium = l2n->second;
    }
...
inline void makeQuadratic(const TopTools_IndexedMapOfShape &shapes, SMESH_Mesh *mesh)
{
    for (int i = 1; i <= shapes.Extent(); ++i) {
        ...
        if (!e || e->IsQuadratic()) continue;

        TIDSortedElemSet elems;
        elems.insert(e);
        while (elemIt->more()) elems.insert(elems.end(), elemIt->next());
...
void NETGENPlugin_Mesher::PrepareOCCgeometry(netgen::OCCGeometry &occgeo, const TopoDS_Shape &shape,
                                             SMESH_Mesh &mesh, list<SMESH_subMesh *> *meshedSM,
                                             NETGENPlugin_Internals *intern)
{
    ...
    for (; rootIt != rootEnd; ++rootIt) {
        ...
        while (smIt->more()) {
            ...
            if (intern && intern->isShapeToPrecompute(shape)) continue;
            if (!meshedSM || sm->IsEmpty()) {
                if (shape.ShapeType() != TopAbs_VERTEX)
                    shape =
                        subShapes(subShapes.FindIndex(shape)); // shape -> index -> oriented shape
...
double NETGENPlugin_Mesher::GetDefaultMinSize(const TopoDS_Shape &geom, const double maxSize)
{
    ...
    for (; fExp.More(); fExp.Next()) {
        ...
        for (int iT = trias.Lower(); iT <= trias.Upper(); ++iT) {
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                if (dist2 < minh && fTol * fTol < dist2) minh = dist2;
                bb.Add(points(*pi[j]));
            }
        }
    }
    ...
    if (minh > 0.5 * maxSize) minh = maxSize / 3.;
...
void NETGENPlugin_Mesher::RestrictLocalSize(netgen::Mesh &ngMesh, const gp_XYZ &p, double size,
                                            const bool overrideMinH)
{
    if (size <= std::numeric_limits<double>::min()) return;
...
bool NETGENPlugin_Mesher::FillNgMesh(netgen::OCCGeometry &occgeom, netgen::Mesh &ngMesh,
                                     vector<const SMDS_MeshNode *> &nodeVec,
                                     const list<SMESH_subMesh *> &meshedSM,
                                     SMESH_MesherHelper *quadHelper, SMESH_ProxyMesh::Ptr proxyMesh)
{
    ...
    for (smIt = meshedSM.begin(); smIt != smEnd; ++smIt) {
        ...
        if (!visitedShapes.Add(sm->GetSubShape())) continue;
        ...
        switch (sm->GetSubShape().ShapeType()) {
            case TopAbs_EDGE: { // EDGE
                ...
                PShapeIteratorPtr fIt =
                    helper.GetAncestors(geomEdge, *sm->GetFather(), TopAbs_FACE);
                while (const TopoDS_Shape *anc = fIt->next()) {
                    faceNgID = occgeom.fmap.FindIndex(*anc);
                    if (faceNgID < 1) continue; // meshed face

                    int faceSMDSId = helper.GetMeshDS()->ShapeToIndex(*anc);
                    if (visitedEdgeSM2Faces[sm].count(faceSMDSId)) continue; // already treated EDGE
                    ...
                    list<TopoDS_Edge> edges =
                        getConnectedEdges(geomEdge, face, computedSM, helper, visitedEdgeSM2Faces);
                    if (edges.empty()) continue; // wrong ancestor?
                    ...
                    for (int iE = 0; iE < fSide.NbEdges(); ++iE)
                        visitedEdgeSM2Faces[helper.GetMesh()->GetSubMesh(fSide.Edge(iE))].insert(
                            faceSMDSId);
                    ...
                    for (i = 0; i < nbSeg; ++i) {
                        ...
                        if (p1.node->GetPosition()->GetTypeOfPosition()
                            == SMDS_TOP_VERTEX) //an EDGE begins
                        {
                            isSeam = false;
                            if (helper.IsRealSeam(p1.node->getshapeId())) {
                                TopoDS_Edge e = fSide.Edge(
                                    fSide.EdgeIndex(0.5 * (p1.normParam + p2.normParam)));
                                isSeam = helper.IsRealSeam(e);
                                if (isSeam) {
                                    otherSeamParam = helper.GetOtherParam(
...
#ifdef DUMP_SEGMENTS
                        cout << "Segment: " << seg.edgenr << " on SMESH face "
                             << helper.GetMeshDS()->ShapeToIndex(face) << std::endl
                             << "\tface index: " << seg.si << std::endl
                             << "\tp1: " << seg[0] << std::endl
                             << "\tp2: " << seg[1] << std::endl
                             << "\tp0 param: " << seg.epgeominfo[0].dist << std::endl
                             << "\tp0 uv: " << seg.epgeominfo[0].u << ", " << seg.epgeominfo[0].v
                             << std::endl
                             //<< "\tp0 edge: " << seg.epgeominfo[ 0 ].edgenr << std::endl
                             << "\tp1 param: " << seg.epgeominfo[1].dist << std::endl
                             << "\tp1 uv: " << seg.epgeominfo[1].u << ", " << seg.epgeominfo[1].v
                             << std::endl;
...
#ifdef DUMP_SEGMENTS
                            cout << "Segment: " << seg.edgenr << std::endl
                                 << "\t is REVERSE of the previous" << std::endl;
#endif
                        }
                    }
                ...
                if (quadHelper) // remember medium nodes of sub-meshes
                {
                    ...
                    while (edges->more()) {
                        ...
                        if (!quadHelper->AddTLinks(static_cast<const SMDS_MeshEdge *>(e))) break;
                    }
                }
            ...
            case TopAbs_FACE: { // FACE
                ...
                if (quadAdaptor) { solidID1 = occgeom.somap.FindIndex(quadAdaptor->GetShape()); }
                else {
                    PShapeIteratorPtr solidIt =
                        helper.GetAncestors(geomFace, *sm->GetFather(), TopAbs_SOLID);
                ...
                if (solidID1) {
                    ...
                    if (faceOriInSolid >= 0)
                        reverse = helper.IsReversedSubMesh(
                            TopoDS::Face(geomFace.Oriented(faceOriInSolid)));
                }
...
#endif
                if (proxyMesh) smDS = proxyMesh->GetSubMesh(geomFace);
                ...
                while (faces->more()) {
                    ...
                    if (f->NbNodes() % 3 != 0) // not triangle
                    {
                        ...
                        smError.reset(new SMESH_ComputeError(COMPERR_BAD_INPUT_MESH,
                    ...
                    double size = ((xyz[0] - xyz[1]).Modulus() + (xyz[1] - xyz[2]).Modulus()
                                   + (xyz[2] - xyz[0]).Modulus())
                        / 3;
                ...
                if (quadHelper) // remember medium nodes of sub-meshes
                {
                    ...
                    while (faces->more()) {
                        const SMDS_MeshElement *f = faces->next();
                        if (!quadHelper->AddTLinks(static_cast<const SMDS_MeshFace *>(f))) break;
                    }
                }
            ...
            case TopAbs_VERTEX: { // VERTEX
                ...
                PShapeIteratorPtr ansIt =
                    helper.GetAncestors(sm->GetSubShape(), *sm->GetFather(), TopAbs_EDGE);
                bool toAdd = false;
                ...
                if (toAdd) {
                    ...
                    if (nodeIt->more()) ngNodeId(nodeIt->next(), ngMesh, nodeNgIdMap);
                }
                break;
            }
            default:;
...
void NETGENPlugin_Mesher::FixIntFaces(const netgen::OCCGeometry &occgeom, netgen::Mesh &ngMesh,
                                      NETGENPlugin_Internals &internalShapes)
{
    ...
    for (int ngFaceID = 1; ngFaceID <= occgeom.fmap.Extent(); ++ngFaceID) {
        ...
        if (internalShapes.isInternalShape(smeshID)) ngFaceIds.insert(ngFaceID);
    }
    if (!ngFaceIds.empty()) {
        // duplicate faces
...
bool NETGENPlugin_Mesher::FixFaceMesh(const netgen::OCCGeometry &occgeom, netgen::Mesh &ngMesh,
                                      const int faceID)
{
    ...
    if (occgeom.fmap.Extent() < faceID) return false;
    ...
    for (int iF = 1; iF <= ngMesh.GetNSE(); ++iF) {
        ...
        if (faceID != elem.GetIndex()) continue;
        int n0 = elem[elem.GetNP() - 1];
        for (int i = 0; i < elem.GetNP(); ++i) {
            int n1 = elem[i];
            Link link(n0, n1);
            if (!linkMap.Add(link)) linkMap.Remove(link);
            n0 = n1;
        }
    }
    ...
    if (linkMap.IsEmpty()) return true;
    if (linkMap.Extent() < 3) return false;
    ...
    for (linkIt2.Next(); linkIt2.More(); linkIt2.Next()) {
        ...
        if (link2.IsConnected(link1)) {
            ...
            for (linkIt3.Next(); linkIt3.More(); linkIt3.Next()) {
                ...
                if (link3.IsConnected(link1) && link3.IsConnected(link2)) {
                    // add a triangle
                    tri[0] = link1.n2;
                    tri[1] = link1.n1;
                    tri[2] = (link2.Contains(link1.n1) ? link2.n1 : link3.n1);
                    if (tri[0] == tri[2] || tri[1] == tri[2]) return false;
                    ngMesh.AddSurfaceElement(tri);

                    // prepare for the next tria search
                    if (linkMap.Extent() == 3) return true;
...
double evalDist(const gp_XY &uv1, const gp_XY &uv2, const Handle(Geom_Surface) & surf,
                const int stopHandler = -1)
{
    if (stopHandler > 0) // continue recursion
    {
        gp_XY mid = SMESH_MesherHelper::GetMiddleUV(surf, uv1, uv2);
        return evalDist(uv1, mid, surf, stopHandler - 1)
            + evalDist(mid, uv2, surf, stopHandler - 1);
    }
...
void NETGENPlugin_Mesher::AddIntVerticesInFaces(const netgen::OCCGeometry &occgeom,
                                                ...
                                                NETGENPlugin_Internals &internalShapes)
{
    if (nodeVec.size() < ngMesh.GetNP()) nodeVec.resize(ngMesh.GetNP(), 0);
    ...
    for (; f2v != face2Vert.end(); ++f2v) {
        ...
        for (int nbV = 0; iv != iVertices.end(); ++iv, nbV++) {
            ...
            for (i = 1; i <= ngMesh.GetNSeg(); ++i) {
                ...
                for (int iEnd = 0; iEnd < 2; ++iEnd) {
                    ...
                    dist2 = helper.ApplyIn2D(surf, uv[iEnd], vData.uv, gp_XY_Subtracted, 0)
                                .SquareModulus();
                    if (dist2 < closeDist2)
                ...
                if (!nbV) {
                    totSegLen2D +=
                        helper.ApplyIn2D(surf, uv[0], uv[1], gp_XY_Subtracted, false).Modulus();
                    totNbSeg++;
                }
            }
        ...
        for (; dist_vData != dist2VData.end(); ++dist_vData) {
            ...
            if (hintLenOK || avgLenOK) {
                ...
                // how far from V
                double r =
                    min(0.5, (hintLenOK ? segLenHint / nodeDist3D : avgSegLen2d / nodeDist2D));
                                                 ...
                                                 NETGENPlugin_Internals &internalShapes)
{
    ...
    for (; s2v != so2Vert.end(); ++s2v) {
        ...
        if (ngFaceIds.size() == 1 && *ngFaceIds.begin() == 0) ngFaceIds.insert(1);
            ...
            for (i = 1; i <= ngMesh.GetNSE(); ++i) {
                ...
                for (int j = 0; j < elem.GetNP(); ++j) {
                    ...
                    if (!nbV)
                        totNbSeg++,
                            totSegLen +=
                            sqrt(dist2(mp, ngMesh.Point(elem[(j + 1) % elem.GetNP()])));
                }
                double dist = dist2nID.begin()->first; //avgDist2;
                if (dist < closeDist2)
                    vData.ngIdClose = i, vData.ngIdCloseN = dist2nID.begin()->second,
                    closeDist2 = dist;
            }
            dist2VData.insert(make_pair(closeDist2, vData));
        }
...
#ifdef DUMP_TRIANGLES_SCRIPT
            py << "n1 = m.AddNode( " << mpV(0) << ", " << mpV(1) << ", " << mpV(2) << ") "
               << std::endl
               << "n2 = m.AddNode( " << mp[0](0) << ", " << mp[0](1) << ", " << mp[0](2) << ") "
               << std::endl
               << "n3 = m.AddNode( " << mp[1](0) << ", " << mp[1](1) << ", " << mp[1](2) << " )"
               << std::endl
...
SMESH_ComputeErrorPtr NETGENPlugin_Mesher::AddSegmentsToMesh(
    netgen::Mesh &ngMesh, netgen::OCCGeometry &geom, const TSideVector &wires,
    SMESH_MesherHelper &helper, vector<const SMDS_MeshNode *> &nodeVec, const bool overrideMinH)
{
    // ----------------------------
    ...
    for (int iW = 0; iW < wires.size(); ++iW) {
        ...
        if (uvPtVec.size() != wire->NbPoints())
            return SMESH_ComputeError::New(COMPERR_BAD_INPUT_MESH,
                                           SMESH_Comment("Unexpected nb of points on wire ")
                                               << iW << ": " << uvPtVec.size()
                                               << " != " << wire->NbPoints());
        nbNodes += wire->NbPoints();
    }
    nodeVec.reserve(nodeVec.size() + nbNodes + 1);
    if (nodeVec.empty()) nodeVec.push_back(0);
    ...
    for (int iW = 0; iW < wires.size(); ++iW) {
        ...
        for (int i = 0; i < nbSegments; ++i) // loop on segments
        {
            ...
            ngMesh.AddSegment(seg);
            {
                ...
                int nbSeg = (int(segLen[iPrev] > sumH / 100.) + int(segLen[i] > sumH / 100.)
        ...
        for (int i = prevNbNGSeg + 1; i <= ngMesh.GetNSeg(); ++i) {
            ...
            if (i > 1) {
                ...
                if (seg[0] == prevSeg[1] && seg[1] == prevSeg[0]) {
                    cout << "Segment: " << seg.edgenr << std::endl
                         << "\tis REVERSE of the previous one" << std::endl;
                    continue;
                }
            }
            cout << "Segment: " << seg.edgenr << std::endl
...
int NETGENPlugin_Mesher::FillSMesh(const netgen::OCCGeometry &occgeo, netgen::Mesh &ngMesh,
                                   const NETGENPlugin_ngMeshInfo &initState, SMESH_Mesh &sMesh,
                                   std::vector<const SMDS_MeshNode *> &nodeVec,
                                   SMESH_Comment &comment, SMESH_MesherHelper *quadHelper)
{
    ...
    for (i = nbInitNod + 1; i <= nbNod; ++i) {
        ...
        if (i - nbInitNod <= occgeo.vmap.Extent()) {
            ...
            for (int iV = i - nbInitNod; aVert.IsNull() && iV <= occgeo.vmap.Extent(); ++iV) {
                ...
                if (p.SquareDistance(pV) > 1e-20) aVert.Nullify();
                else
        ...
        if (!node) // node not found on vertex
        {
            ...
            if (!aVert.IsNull()) meshDS->SetNodeOnVertex(node, aVert);
        }
        nodeVec[i] = node;
    }
    ...
    for (i = nbInitSeg + 1; i <= nbSeg; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            int pind = pinds[j];
            if (pind <= 0 || !nodeVec_ACCESS(pind)) break;
            ++nbp;
            double param;
            if (j < 2) {
                if (aEdge.IsNull()) {
        ...
        if (nbp > 1) {
            ...
            {
                if (meshDS->FindEdge(nodeVec_ACCESS(pinds[0]), nodeVec_ACCESS(pinds[1]))) continue;
    ...
    for (i = nbInitFac + 1; i <= nbFac; ++i) {
        ...
        for (int j = 1; j <= elem.GetNP(); ++j) {
            ...
            if (pind < 1 || pind >= nodeVec.size()) break;
        ...
        if (nodes.size() != elem.GetNP()) {
            if (comment.empty()) comment << "Invalid netgen 2d element #" << i;
        ...
        switch (elem.GetType()) {
            ...
            case netgen::QUAD8:
                ...
                face = meshDS->AddFace(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], nodes[7],
                                       nodes[5], nodes[6]);
                // exclude qudrangle elements from computation of 3D mesh
                const_cast<netgen::Element2d &>(elem).SetIndex(quadFaceID);
                break;
            default: MESSAGE("NETGEN created a face of unexpected type, ignoring"); continue;
        }
        if (!face) {
        ...
        if (!aFace.IsNull()) meshDS->SetMeshElementOnShape(face, aFace);
    }
    ...
    for (i = 1; i <= nbVol; ++i) {
        ...
        if (nodes.size() != elem.GetNP()) {
            if (comment.empty()) comment << "Invalid netgen 3d element #" << i;
            continue;
        }
        ...
        switch (elem.GetType()) {
            ...
            case netgen::TET10:
                ...
                vol = meshDS->AddVolume(nodes[0], nodes[1], nodes[2], nodes[3], nodes[4], nodes[7],
                                        nodes[5], nodes[6], nodes[8], nodes[9]);
                break;
            default: MESSAGE("NETGEN created a volume of unexpected type, ignoring"); continue;
        }
        if (!vol) {
        ...
        if (!aSolid.IsNull()) meshDS->SetMeshElementOnShape(vol, aSolid);
    }
...
std::string text(int err)
{
    ...
    return SMESH_Comment("Error in netgen::OCCGenerateMesh() at ") << netgen::multithread.task;
}
...
std::string text(Standard_Failure &ex)
{
    ...
    str << " at " << netgen::multithread.task << ": " << ex.DynamicType()->Name();
...
std::string text(netgen::NgException &ex)
{
    ...
    if (strlen(netgen::multithread.task) > 0) str << " at " << netgen::multithread.task;
    str << ": " << ex.What();
    return std::move(str);
}
...
bool hasBadElemOnSolid(const list<const SMDS_MeshElement *> &elems, SMESH_subMesh *solidSM)
{
    ...
    for (; e != elems.end(); ++e) {
        ...
        while (nIt->more()) {
            ...
            if (nbNodesOnSolid > 2 || nbNodesOnSolid == nbNodes) return true;
        }
    }
    return false;
}
...
bool NETGENPlugin_Mesher::Compute()
{
    ...
    if (_optimize) _totalTime += faceOptimizTime;
    if (_isVolume) _totalTime += voluMeshingTime + (_optimize ? volOptimizeTime : 0);
    double doneTime = 0;
    _ticTime = -1;
    _progressTic = 1;
    _curShapeIndex = -1;
...

    {
        ...
        if (mparams.maxh == 0.0) mparams.maxh = occgeo.boundingbox.Diam();
        if (_simpleHyp
            || (mparams.minh == 0.0 && _fineness != NETGENPlugin_Hypothesis::UserDefined))
...
#endif
            if (netgen::multithread.terminate) return false;
        ...
        if (_simpleHyp) {
            ...
            for (int iE = 1; iE <= occgeo.emap.Extent(); ++iE) {
                ...
                if (nbSeg) segSize = SMESH_Algo::EdgeLength(e) / (nbSeg - 0.4);
        ...
        else // if ( ! _simpleHyp )
        {
            ...
            // --------------------------------
            for (std::map<int, double>::const_iterator it = EdgeId2LocalSize.begin();
                 it != EdgeId2LocalSize.end(); it++) {
                int key = (*it).first;
                double hi = (*it).second;
            ...
            for (std::map<int, double>::const_iterator it = VertexId2LocalSize.begin();
                 it != VertexId2LocalSize.end(); it++) {
                int key = (*it).first;
                double hi = (*it).second;
                const TopoDS_Shape &shape = ShapesWithLocalSize.FindKey(key);
        ...
        if (_isVolume) _ticTime = (doneTime += edgeMeshingTime) / _totalTime / _progressTic;
        ...
        if (!err) {
            ...
            if (_isViscousLayers2D) {
                ...
                for (int faceID = 1; faceID <= occgeo.fmap.Extent(); ++faceID) {
                    ...
                    for (int i = 1; i <= _ngMesh->GetNSeg(); i++) {
                        ...
                        if (seg.si == faceID) seg.si = 0;
                    }
                    // add new segments to _ngMesh instead of excluded ones
                    helper.SetSubShape(F);
                    TSideVector wires = StdMeshers_FaceSide::GetFaceWires(
                        F, *_mesh, /*skipMediumNodes=*/true, error, viscousMesh);
            ...
            mparams.perfstepsend =
                _optimize ? netgen::MESHCONST_OPTSURFACE : netgen::MESHCONST_MESHSURFACE;
#endif
            try {
                OCC_CATCH_SIGNALS;
#if NETGEN_VERSION >= NETGEN_VERSION_STRING(6, 2, 0)
            ...
            if (_mesh->NbQuadrangles() > 0)
                for (int iS = 1; iS <= occgeo.somap.Extent(); ++iS) {
                    ...
                    if (nbPyrams != _mesh->NbPyramids()) {
                        ...
                        for (TopExp_Explorer face(occgeo.somap(iS), TopAbs_FACE); face.More();
                             face.Next())
            ...
            try {
                ...
                if (!meshedSM[MeshDim_1D].empty()) {
                    ...
                    for (int i = 1; i <= _ngMesh->GetNSeg(); ++i) {
                        ...
                        if (seg.epgeominfo[0].edgenr == 0) _ngMesh->DeleteSegment(i);
                    }
                    _ngMesh->Compress();
                }
    ...
    MESSAGE((err ? "Mesh Generation failure" : "End of Mesh Generation")
            << ", nb nodes: " << nbNod << ", nb segments: " << nbSeg << ", nb faces: " << nbFac
            << ", nb volumes: " << nbVol);
    ...
    if (error->IsOK() && (!isOK || comment.size() > 0)) error->myName = COMPERR_ALGO_FAILED;
    if (!comment.empty()) error->myComment = comment;
    ...
    const int nbMaps = 2;
    const TopTools_IndexedMapOfShape *geoMaps[nbMaps] = {&occgeo.vmap,
                                                         &occgeo.emap /*, & occgeo.fmap*/};
    for (int iMap = 0; iMap < nbMaps; ++iMap)
        for (int i = 1; i <= geoMaps[iMap]->Extent(); i++)
            if (SMESH_subMesh *sm = _mesh->GetSubMeshContaining(geoMaps[iMap]->FindKey(i)))
                if (!sm->IsMeshComputed()) sm->SetIsAlwaysComputed(true);
        ...
        for (int i = 1; i <= occgeo.fmap.Extent(); i++) {
            ...
            if (SMESH_subMesh *sm = _mesh->GetSubMeshContaining(occgeo.fmap(i))) {
                ...
                if (!smError || smError->IsOK()) {
                    if (status == -1) smError.reset(new SMESH_ComputeError(*error));
                    else
            ...
            for (int i = 1; i <= occgeo.somap.Extent(); i++)
                if (SMESH_subMesh *sm = _mesh->GetSubMeshContaining(occgeo.somap(i))) {
                    ...
                    if (smComputed && internals.hasInternalVertexInSolid(sm->GetId())) {
                        int nbIntV =
                            internals.getSolidsWithVertices().find(sm->GetId())->second.size();
                    ...
                    if (!smComputed && (!smError || smError->IsOK())) {
                        ...
                        {
                            if (!hasBadElemOnSolid(smError->myBadElements, sm)) smError.reset();
                        }
                    }
                    pb3D = pb3D || (smError && smError->IsKO());
                }
        if (!pb2D && !pb3D) err = 0; // no fatal errors, only warnings
    }
...
bool NETGENPlugin_Mesher::Evaluate(MapShapeNbElems &aResMap)
{
    ...
    for (TopExp_Explorer exp(_shape, TopAbs_EDGE); exp.More(); exp.Next()) {
        ...
        if (!Edge2NbSeg.Bind(E, 0)) continue;

        double aLen = SMESH_Algo::EdgeLength(E);
        fullLen += aLen;

        vector<int> &aVec = aResMap[_mesh->GetSubMesh(E)];
        if (aVec.empty()) aVec.resize(SMDSEntity_Last, 0);
        else
    ...
    for (TopExp_Explorer exp(_shape, TopAbs_FACE); exp.More(); exp.Next()) {
        ...
        if (!tooManyElems) {
            ...
            for (TopExp_Explorer exp1(F, TopAbs_EDGE); exp1.More(); exp1.Next())
                if (edges.Add(exp1.Current())) nb1d += Edge2NbSeg.Find(exp1.Current());
        }
        int nbFaces =
            tooManyElems ? hugeNb : int(4 * anArea / (mparams.maxh * mparams.maxh * sqrt(3.)));
...
double NETGENPlugin_Mesher::GetProgress(const SMESH_Algo *holder, const int *algoProgressTic,
                                        const double *algoProgress) const
{
    ...
    if (_ticTime > 0) progress = Max(*algoProgressTic * _ticTime, *algoProgress);
    if (progress > 0) {
        ((int &)*algoProgressTic)++;
...
SMESH_ComputeErrorPtr NETGENPlugin_Mesher::ReadErrors(const vector<const SMDS_MeshNode *> &nodeVec)
{
    ...
    while (!file.eof()) {
        if (strncmp(file, "Edge ", 5) == 0 && file.getInts(two)
            && strncmp(file, badEdgeStr, badEdgeStrLen) == 0 && two[0] < nodeVec.size()
            && two[1] < nodeVec.size()) {
            ...
            if (ok) {
                err->myBadElements.push_back(new SMDS_FaceOfNodes(
                    nodeVec[three1[0]], nodeVec[three1[1]], nodeVec[three1[2]]));
                err->myBadElements.push_back(new SMDS_FaceOfNodes(
                    nodeVec[three2[0]], nodeVec[three2[1]], nodeVec[three2[2]]));
...
void NETGENPlugin_Mesher::toPython(const netgen::Mesh *ngMesh, const std::string &pyFile)
{
    ...
    outfile << "import SMESH" << std::endl
            << "from salome.smesh import smeshBuilder" << std::endl
            << "smesh = smeshBuilder.New(salome.myStudy)" << std::endl
            << "mesh = smesh.Mesh()" << std::endl
            << std::endl;

    using namespace netgen;
    PointIndex pi;
    for (pi = PointIndex::BASE; pi < ngMesh->GetNP() + PointIndex::BASE; pi++) {
    ...
    for (int i = 0; i < nbDom; ++i)
        outfile << "grp" << i + 1 << " = mesh.CreateEmptyGroup( SMESH.FACE, 'domain" << i + 1
                << "')" << std::endl;
    ...
    for (int i = 1; i <= ngMesh->GetNSeg(); i++) {
        ...
        outfile << "mesh.AddEdge([ " << seg[0] << ", " << seg[1] << " ])" << std::endl;
    }
...
NETGENPlugin_ngMeshInfo::NETGENPlugin_ngMeshInfo(netgen::Mesh *ngMesh) : _copyOfLocalH(0)
{
    if (ngMesh) {
...
NETGENPlugin_Internals::NETGENPlugin_Internals(SMESH_Mesh &mesh, const TopoDS_Shape &shape,
                                               bool is3D)
    ...
    for (f.Init(shape, TopAbs_FACE); f.More(); f.Next()) {
        ...
        for (TopoDS_Iterator fSub(f.Current()); fSub.More(); fSub.Next())
            if (fSub.Value().ShapeType() == TopAbs_VERTEX) {
                ...
                if (intVV.insert(vID).second) _f2v[faceID].push_back(vID);
            }
                ...
                for (; edge != edges.end(); ++edge)
                    for (TopoDS_Iterator v(*edge); v.More(); v.Next()) {
                        ...
                        while (const TopoDS_Shape *pEdge = eIt->next()) {
                            ...
                            if (isInternalShape(edgeID)) internalEdges.insert(edgeID);
                        }
...
void NETGENPlugin_Internals::findBorderElements(TIDSortedElemSet &borderElems)
{
    ...
    for (; intShapeId != _intShapes.end(); ++intShapeId) {
        ...
        while (const TopoDS_Shape *pFace = faces->next()) {
            ...
            if (isInternalShape(faceID)) intFace = faceID;
            else
                ...
                while (fIt->more()) {
                    ...
                    if (intFaceSM->Contains(f)) {
                        for (int i = 0; i < nbNodes; ++i)
                            links.insert(
                                SMESH_OrientedLink(f->GetNode(i), f->GetNode((i + 1) % nbNodes)));
                    }
                    else {
                        int nbDblNodes = 0;
                        for (int i = 0; i < nbNodes; ++i)
                            nbDblNodes += isInternalShape(f->GetNode(i)->getshapeId());
                        if (nbDblNodes) suspectFaces[nbDblNodes < 2].push_back(f);
                        nbSuspectFaces++;
                    }
                }
            }
        ...
        for (int isPostponed = 0; isPostponed < 2; ++isPostponed) {
            ...
            for (int nbF = 0; fIt != suspectFaces[isPostponed].end(); ++fIt, ++nbF) {
                ...
                if (isBorder) {
                    ...
                    if (nbF > 2 * nbSuspectFaces) break; // dead loop protection
                }
            }
        }
    }
                                              ...
                                              list<SMESH_subMesh *> &boundarySM)
{
    ...
    if (!_borderFaces.empty()) shapeIDs.insert(_borderFaces.begin(), _borderFaces.end());
    ...
    for (; intS != shapeIDs.end(); ++intS) {
        ...
        while (smIt->more()) {
            ...
            if (sm->IsEmpty()) {
                ...
                if (s.ShapeType() != TopAbs_FACE) boundarySM.push_back(sm);
            }
        }
    }
}
...
SMESH_Mesh &NETGENPlugin_Internals::getMesh() const { return const_cast<SMESH_Mesh &>(_mesh); }

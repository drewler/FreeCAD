...
bool NETGENPlugin_NETGEN_3D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                             Hypothesis_Status &aStatus)
{
    ...
    for (; h != hyps.end(); ++h) {
        ...
        if (!_hypParameters) _hypParameters = dynamic_cast<const NETGENPlugin_Hypothesis *>(*h);

        if (*h != _hypMaxElementVolume && *h != _viscousLayersHyp && *h != _hypParameters
            && !dynamic_cast<const StdMeshers_ViscousLayers *>(*h)) // several VL hyps allowed
            aStatus = HYP_INCOMPATIBLE;
    }
    if (_hypMaxElementVolume && _hypParameters) aStatus = HYP_INCOMPATIBLE;
    else if (aStatus == HYP_OK && _viscousLayersHyp)
        error(_viscousLayersHyp->CheckHypothesis(aMesh, aShape, aStatus));

    if (_hypMaxElementVolume) _maxElementVolume = _hypMaxElementVolume->GetMaxVolume();
...
bool NETGENPlugin_NETGEN_3D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    vector<const SMDS_MeshNode *> nodeVec;
    {
        ...
        for (TopExp_Explorer exFa(aShape, TopAbs_FACE); exFa.More(); exFa.Next()) {
            ...
            while (iteratorElem->more()) // loop on elements on a geom face
            {
                ...
                if (!elem) return error(COMPERR_BAD_INPUT_MESH, "Null element encounters");
                ...
                for (int iN = 0; iN < 3; ++iN) {
                    ...
                    if (node->GetPosition()->GetTypeOfPosition() == SMDS_TOP_EDGE
                        ...
                        TopoDS_Shape vertex =
                            TopoDS_Iterator(meshDS->IndexToShape(shapeID)).Value();
        ...
        for (; n_id != nodeToNetgenID.end(); ++n_id) nodeVec[n_id->second] = n_id->first;
        nodeToNetgenID.clear();

        if (internals.hasInternalVertexInSolid()) {
            netgen::OCCGeometry occgeo;
            NETGENPlugin_Mesher::AddIntVerticesInSolids(occgeo, (netgen::Mesh &)*Netgen_mesh,
                                                        nodeVec, internals);
        }
    }
...
bool NETGENPlugin_NETGEN_3D::compute(SMESH_Mesh &aMesh, SMESH_MesherHelper &helper,
                                     vector<const SMDS_MeshNode *> &nodeVec, Ng_Mesh *Netgen_mesh)
{
    netgen::multithread.terminate = 0;
    ...
    try {
        ...
        if (err)
            error(SMESH_Comment("Error in netgen::OCCGenerateMesh() at ")
                  << netgen::multithread.task);
    }
    ...
    MESSAGE("End of Volume Mesh Generation. err=" << err << ", nb new nodes: "
    ...
    if (err) {
        ...
        if (ce && !ce->myBadElements.empty()) error(ce);
    }
    ...
    if (isOK) {
        ...
        for (; nodeIndex <= Netgen_NbOfNodesNew; ++nodeIndex) {
            ...
            nodeVec.at(nodeIndex) =
                helper.AddNode(Netgen_point[0], Netgen_point[1], Netgen_point[2]);
        }
        ...
        for (int elemIndex = 1; elemIndex <= Netgen_NbOfTetra; ++elemIndex) {
            ...
            try {
                helper.AddVolume(
                    nodeVec.at(Netgen_tetrahedron[0]), nodeVec.at(Netgen_tetrahedron[1]),
                    nodeVec.at(Netgen_tetrahedron[2]), nodeVec.at(Netgen_tetrahedron[3]));
            }
            catch (...) {
            }
        }
...
bool NETGENPlugin_NETGEN_3D::Compute(SMESH_Mesh &aMesh, SMESH_MesherHelper *aHelper)
{
    const int invalid_ID = -1;
    ...
    // insert old nodes into nodeVec
    TNodeToIDMap::iterator n_id = nodeToNetgenID.begin();
    for (; n_id != nodeToNetgenID.end(); ++n_id) nodeVec.at(n_id->second) = n_id->first;
    nodeToNetgenID.clear();
...
bool NETGENPlugin_NETGEN_3D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,

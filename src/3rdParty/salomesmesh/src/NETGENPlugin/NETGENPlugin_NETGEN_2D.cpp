...
NETGENPlugin_NETGEN_2D::NETGENPlugin_NETGEN_2D(int hypId, int studyId, SMESH_Gen *gen)
...
bool NETGENPlugin_NETGEN_2D::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                             Hypothesis_Status &aStatus)
{
    ...
    const list<const SMESHDS_Hypothesis *> &hyps =
        GetUsedHypothesis(aMesh, aShape, /*skipAux=*/false);
    list<const SMESHDS_Hypothesis *>::const_iterator h = hyps.begin();
    for (; h != hyps.end(); ++h) {
        const SMESHDS_Hypothesis *theHyp = *h;
        string hypName = theHyp->GetName();
        if (hypName == StdMeshers_ViscousLayers2D::GetHypType()) _isViscousLayers2D = true;
        else if (_hypothesis)
...
bool NETGENPlugin_NETGEN_2D::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
...
bool NETGENPlugin_NETGEN_2D::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,

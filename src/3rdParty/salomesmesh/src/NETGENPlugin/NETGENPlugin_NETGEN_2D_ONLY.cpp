...
NETGENPlugin_NETGEN_2D_ONLY::NETGENPlugin_NETGEN_2D_ONLY(int hypId, int studyId, SMESH_Gen *gen)
...
bool NETGENPlugin_NETGEN_2D_ONLY::CheckHypothesis(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                                  Hypothesis_Status &aStatus)
{
    ...
    if (nbHyps > 1) aStatus = HYP_CONCURENT;
    else if (hasVL)
    ...
    if (aStatus == HYP_OK && _hypParameters && _hypQuadranglePreference) {
        ...
        return error(SMESH_Comment("\"")
                     << _hypQuadranglePreference->GetName() << "\" and \""
                     << _hypParameters->GetName() << "\" are incompatible hypotheses");
    }
...
bool NETGENPlugin_NETGEN_2D_ONLY::Compute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape)
{
    ...
    if (_hypQuadranglePreference) netgen::mparam.quad = true;

    // local size is common for all FACEs in aShape?
    const bool isCommonLocalSize =
        (!_hypLengthFromEdges && !_hypMaxElementArea && netgen::mparam.uselocalh);
    ...
    if (isCommonLocalSize) // compute common local size in ngMeshes[0]
    {
        ...
        if (!_hypParameters || netgen::mparam.minh < DBL_MIN) {
            if (!_hypParameters) netgen::mparam.maxh = occgeoComm.GetBoundingBox().Diam() / 3.;
    ...
    for (int iF = 0; fExp.More(); fExp.Next(), ++iF) {
        ...
        if (faceErr && !faceErr->IsOK()) continue;
        int nbWires = wires.size();
        if (nbWires == 0) {
            faceErr.reset(new SMESH_ComputeError(COMPERR_ALGO_FAILED,
                                                 "Problem in StdMeshers_FaceSide::GetFaceWires()"));
            continue;
        }
        if (wires[0]->NbSegments() < 3) // ex: a circle with 2 segments
        {
            faceErr.reset(new SMESH_ComputeError(COMPERR_BAD_INPUT_MESH,
                                                 SMESH_Comment("Too few segments: ")
                                                     << wires[0]->NbSegments()));
            continue;
        }
        ...
        if (!_hypParameters) {
            ...
            if (_hypLengthFromEdges) {
                ...
                if (nbSegments) edgeLength /= nbSegments;
                ...
                for (int iW = 0; iW < nbWires; ++iW) {
                    ...
                    if (points.empty()) return error(COMPERR_BAD_INPUT_MESH);
        ...
        for (; iLoop < 2; iLoop++) {
            ...
            try {
...
#else
                err =
                    netgen::OCCGenerateMesh(occgeom, mesh_ptr, netgen::mparam, startWith, endWith);
#endif
#elif NETGEN_VERSION >= NETGEN_VERSION_STRING(5, 0, 0)
                ...
                str << "Exception in  netgen::OCCGenerateMesh()"
                    << " at " << netgen::multithread.task << ": " << ex.DynamicType()->Name();
            ...
            if (err) {
                if (aMesher.FixFaceMesh(occgeom, *ngMesh, 1)) break;
                if (iLoop == LOC_SIZE) {
            ...
            for (i = 1; i <= nbFaces; ++i) {
                ...
                if (j > elem.GetNP()) {
                    ...
                    if (elem.GetType() == TRIG) face = helper.AddFace(nodes[0], nodes[1], nodes[2]);
                    else
...
bool NETGENPlugin_NETGEN_2D_ONLY::Evaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                           MapShapeNbElems &aResMap)
{
    ...
    if (anArea / hugeNb > ELen * ELen) {
        SMESH_subMesh *sm = aMesh.GetSubMesh(F);
        SMESH_ComputeErrorPtr &smError = sm->GetComputeError();
        smError.reset(new SMESH_ComputeError(
            COMPERR_ALGO_FAILED, "Submesh can not be evaluated.\nToo small element length", this));
        return false;
    }

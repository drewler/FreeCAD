...
public:
   ...
   * \brief Wrap one edge
   */
    StdMeshers_FaceSide(const TopoDS_Face &theFace, const TopoDS_Edge &theEdge, SMESH_Mesh *theMesh,
                        const bool theIsForward, const bool theIgnoreMediumNodes,
                        SMESH_ProxyMesh::Ptr theProxyMesh = SMESH_ProxyMesh::Ptr());
    /*!
   * \brief Wrap several edges. Edges must be properly ordered and oriented.
   */
    StdMeshers_FaceSide(const TopoDS_Face &theFace, std::list<TopoDS_Edge> &theEdges,
   ...
   */
    StdMeshers_FaceSide(const StdMeshers_FaceSide *theSide, const SMDS_MeshNode *theNode,
                        const gp_Pnt2d *thePnt2d1, const gp_Pnt2d *thePnt2d2 = NULL,
                        const Handle(Geom2d_Curve) &theC2d = NULL, const double theUFirst = 0.,
   ...
   */
    ...
    // static "constructors"
    static StdMeshers_FaceSidePtr New(const TopoDS_Face &Face, const TopoDS_Edge &Edge,
                                      SMESH_Mesh *Mesh, const bool IsForward,
    ...
    static StdMeshers_FaceSidePtr New(const TopoDS_Face &Face, std::list<TopoDS_Edge> &Edges,
                                      SMESH_Mesh *Mesh, const bool IsForward,
                                      const bool IgnoreMediumNodes,
                                      SMESH_ProxyMesh::Ptr ProxyMesh = SMESH_ProxyMesh::Ptr())
    {
    ...
    static StdMeshers_FaceSidePtr New(const StdMeshers_FaceSide *Side, const SMDS_MeshNode *Node,
                                      const gp_Pnt2d *Pnt2d1, const gp_Pnt2d *Pnt2d2 = NULL,
                                      const Handle(Geom2d_Curve) &C2d = NULL,
                                      const double UFirst = 0., const double ULast = 1.)
    {
        return StdMeshers_FaceSidePtr(
   ...
   */
    static TSideVector GetFaceWires(const TopoDS_Face &theFace, SMESH_Mesh &theMesh,
   ...
   * \brief Simulates detailed data on nodes
    ...
    * \param constValue - constant parameter value
   */
    const UVPtStructVec &SimulateUVPtStruct(int nbSeg, bool isXConst = 0,
   ...
   * \retval bool - is OK
   */
    bool GetEdgeNodes(const size_t i, std::vector<const SMDS_MeshNode *> &nodes,
                      bool inlude1stVertex = true, bool inludeLastVertex = true) const;
...
inline double StdMeshers_FaceSide::FirstU(int i) const { return myFirst[i % myFirst.size()]; }
...
inline double StdMeshers_FaceSide::LastU(int i) const { return myLast[i % myLast.size()]; }
...
inline int StdMeshers_FaceSide::EdgeID(int i) const { return myEdgeID[i % myEdgeID.size()]; }
...
inline bool StdMeshers_FaceSide::IsReversed(int i) const { return myFirst[i] > myLast[i]; }

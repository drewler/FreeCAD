...
struct MEDWRAPPER_EXPORT TModeSwitchInfo: virtual TBase {
    ...
    TModeSwitchInfo() : myModeSwitch(eFULL_INTERLACE) {}

    //! To construct instance of the class
    TModeSwitchInfo(EModeSwitch theModeSwitch) : myModeSwitch(theModeSwitch) {}
...
struct MEDWRAPPER_EXPORT TNodeInfo: virtual TElemInfo, virtual TModeSwitchInfo {
...
struct MEDWRAPPER_EXPORT TCellInfo: virtual TElemInfo, virtual TModeSwitchInfo {
...
struct MEDWRAPPER_EXPORT TPolygoneInfo: virtual TElemInfo {
...
//
struct MEDWRAPPER_EXPORT TBallInfo: virtual TCellInfo {
...
struct MEDWRAPPER_EXPORT TPolyedreInfo: virtual TElemInfo {
...
struct MEDWRAPPER_EXPORT TFieldInfo: virtual TNameInfo {
    PMeshInfo myMeshInfo; //!< A reference to corresponding MED Mesh
    //! Get a reference to corresponding MED Mesh
    const PMeshInfo &GetMeshInfo() const { return myMeshInfo; }
...
struct MEDWRAPPER_EXPORT TGaussInfo: virtual TNameInfo, virtual TModeSwitchInfo {
    ...
    struct MEDWRAPPER_EXPORT TLess {
        ...
        bool operator()(const TGaussInfo &theLeft, const TGaussInfo &theRight) const;
    };
...
struct MEDWRAPPER_EXPORT TTimeStampInfo: virtual TBase {
    ...
    TInt GetNbGauss(EGeometrieElement theGeom)
        const; //!< Gives number of the Gauss Points for the MED TimeStamp
...
struct MEDWRAPPER_EXPORT TProfileInfo: virtual TNameInfo {
...
struct MEDWRAPPER_EXPORT TMeshValueBase: virtual TModeSwitchInfo {
    TInt myNbElem;
    TInt myNbComp;
    TInt myNbGauss;
    TInt myStep;
    ...
    void Allocate(TInt theNbElem, TInt theNbGauss, TInt theNbComp,
...
//---------------------------------------------------------------
//! The class is a helper one. It provide safe and flexible way to get access to values for a MED TimeStamp
template<class TValueType> struct TTMeshValue: virtual TMeshValueBase {
    ...
    //! Returns bare pointer on the internal value representation
    virtual unsigned char *GetValuePtr() { return (unsigned char *)&myValue[0]; }

    //! Returns bare pointer on the internal value representation
    virtual TElement *GetPointer() { return &myValue[0]; }

    //! Returns bare pointer on the internal value representation
    virtual const TElement *GetPointer() const { return &myValue[0]; }
    ...
    TCValueSliceArr GetGaussValueSliceArr(TInt theElemId) const
    {
        ...
        if (GetModeSwitch() == eFULL_INTERLACE) {
            ...
            for (TInt aGaussId = 0; aGaussId < myNbGauss; aGaussId++) {
                aValueSliceArr[aGaussId] = TCValueSlice(myValue, std::slice(anId, myNbComp, 1));
                anId += myNbComp;
            }
        }
        else {
    ...
    TValueSliceArr GetGaussValueSliceArr(TInt theElemId)
    {
        ...
        if (GetModeSwitch() == eFULL_INTERLACE) {
            TInt anId = theElemId * myStep;
            for (TInt aGaussId = 0; aGaussId < myNbGauss; aGaussId++) {
                aValueSliceArr[aGaussId] = TValueSlice(myValue, std::slice(anId, myNbComp, 1));
                anId += myNbComp;
            }
        }
        else {
...
struct MEDWRAPPER_EXPORT TTimeStampValueBase: virtual TModeSwitchInfo {
    ...
    virtual void AllocateValue(EGeometrieElement theGeom, TInt theNbElem, TInt theNbGauss,
                               TInt theNbComp, EModeSwitch theMode = eFULL_INTERLACE) = 0;
...
template<class TMeshValueType> struct TTimeStampValue: virtual TTimeStampValueBase {
    ...
    //! Gets type of the champ
    virtual ETypeChamp GetTypeChamp() const { return myTypeChamp; }

    //! Keeps map of MED TimeStamp values per geometric type (const version)
    TTGeom2Value myGeom2Value;

    const TTGeom2Value &GetGeom2Value() const { return myGeom2Value; }
...
void CopyTimeStampValue(SharedPtr<TTimeStampValue<TMeshValueTypeFrom>> theTimeStampValueFrom,
                        SharedPtr<TTimeStampValue<TMeshValueTypeTo>> theTimeStampValueTo)
{
    ...
    typename TimeStampValueTypeFrom::TTGeom2Value &aGeom2Value =
        theTimeStampValueFrom->myGeom2Value;
    ...
    for (; anIter != aGeom2Value.end(); anIter++) {
        ...
        typename TimeStampValueTypeTo::TTMeshValue &aMeshValue2 =
            theTimeStampValueTo->GetMeshValue(aGeom);
        aMeshValue2.Allocate(aMeshValue.myNbElem, aMeshValue.myNbGauss, aMeshValue.myNbComp,
        ...
        for (TInt anId = 0; anId < aSize; anId++) aValue2[anId] = TElementTo(aValue[anId]);
    }
}

template<class TMeshValueType>
void CopyTimeStampValue(SharedPtr<TTimeStampValue<TMeshValueType>> theTimeStampValueFrom,
                        SharedPtr<TTimeStampValue<TMeshValueType>> theTimeStampValueTo)
{
    ...
    for (; anIter != aGeom2Value.end(); anIter++) {
        ...
        typename TimeStampValueType::TTMeshValue &aMeshValue2 =
            theTimeStampValueTo->GetMeshValue(aGeom);
...
//---------------------------------------------------------------
inline void CopyTimeStampValueBase(const PTimeStampValueBase &theValueFrom,
...
struct MEDWRAPPER_EXPORT TGrilleInfo: virtual TModeSwitchInfo {
    ...
    EGrilleType
        myGrilleType; //!< Defines grille type (eGRILLE_CARTESIENNE,eGRILLE_POLAIRE,eGRILLE_STANDARD)

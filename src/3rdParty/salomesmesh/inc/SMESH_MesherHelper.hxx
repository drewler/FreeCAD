...
public:
   ...
   */
    static bool IsSameElemGeometry(const SMESHDS_SubMesh *smDS, SMDSAbs_GeometryType shape,
   ...
   */
    static bool LoadNodeColumns(TParam2ColumnMap &theParam2ColumnMap, const TopoDS_Face &theFace,
                                const std::list<TopoDS_Edge> &theBaseSide, SMESHDS_Mesh *theMesh,
   ...
   *   a0   p0    a1
   */
    inline static gp_XY calcTFI(double x, double y, const gp_XY &a0, const gp_XY &a1,
                                const gp_XY &a2, const gp_XY &a3, const gp_XY &p0, const gp_XY &p1,
                                const gp_XY &p2, const gp_XY &p3);

    /*!
   * \brief Same as "gp_XY calcTFI(...)" but in 3D
   */
    inline static gp_XYZ calcTFI(double x, double y, const gp_XYZ &a0, const gp_XYZ &a1,
                                 const gp_XYZ &a2, const gp_XYZ &a3, const gp_XYZ &p0,
   ...
   */
    static int NbAncestors(const TopoDS_Shape &shape, const SMESH_Mesh &mesh,
                           TopAbs_ShapeEnum ancestorType = TopAbs_SHAPE);
    /*!
   * \brief Return iterator on ancestors of the given type
   */
    static PShapeIteratorPtr GetAncestors(const TopoDS_Shape &shape, const SMESH_Mesh &mesh,
                                          TopAbs_ShapeEnum ancestorType);
    /*!
   * \brief Find a common ancestor, of the given type, of two shapes
   */
    static TopoDS_Shape GetCommonAncestor(const TopoDS_Shape &shape1, const TopoDS_Shape &shape2,
                                          const SMESH_Mesh &mesh, TopAbs_ShapeEnum ancestorType);
    /*!
   ...
   */
    ...
    static double GetAngle(const TopoDS_Edge &E1, const TopoDS_Edge &E2, const TopoDS_Face &F,
                           const TopoDS_Vertex &V, gp_Vec *faceNormal = 0);
...
public:
   ...
   */
    void SetIsQuadratic(const bool theBuildQuadratic) { myCreateQuadratic = theBuildQuadratic; }
   ...
   */
    SMDS_MeshEdge *AddEdge(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const int id = 0,
                           const bool force3d = true);
    /*!
   * Creates quadratic or linear triangle
   */
    SMDS_MeshFace *AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                           const SMDS_MeshNode *n3, const int id = 0, const bool force3d = false);
    /*!
   * Creates bi-quadratic, quadratic or linear quadrangle
   */
    SMDS_MeshFace *AddFace(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                           const SMDS_MeshNode *n3, const SMDS_MeshNode *n4, const int id = 0,
   ...
   */
    SMDS_MeshFace *AddPolygonalFace(const std::vector<const SMDS_MeshNode *> &nodes,
                                    const int id = 0, const bool force3d = false);
    /*!
   * Creates quadratic or linear tetrahedron
   */
    SMDS_MeshVolume *AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                               const SMDS_MeshNode *n3, const SMDS_MeshNode *n4, const int id = 0,
                               const bool force3d = true);
    /*!
   ...
   */
    SMDS_MeshVolume *AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                               ...
                               const SMDS_MeshNode *n5, const int id = 0,
                               const bool force3d = true);
    /*!
   ...
   */
    SMDS_MeshVolume *AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                               const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
                               const SMDS_MeshNode *n5, const SMDS_MeshNode *n6, const int id = 0,
                               const bool force3d = true);
    /*!
   ...
   */
    SMDS_MeshVolume *AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                               ...
                               const SMDS_MeshNode *n7, const SMDS_MeshNode *n8, const int id = 0,
                               bool force3d = true);
   ...
   */
    SMDS_MeshVolume *AddVolume(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                               ...
                               const SMDS_MeshNode *n11, const SMDS_MeshNode *n12, const int id = 0,
                               bool force3d = true);
   ...
   */
    SMDS_MeshVolume *AddPolyhedralVolume(const std::vector<const SMDS_MeshNode *> &nodes,
                                         const std::vector<int> &quantities, const int ID = 0,
                                         const bool force3d = true);
    /*!
   ...
   */
    double GetNodeU(const TopoDS_Edge &theEdge, const SMDS_MeshNode *theNode,
                    const SMDS_MeshNode *inEdgeNode = 0, bool *check = 0) const;
    /*!
   * \brief Return node UV on face
   *  \param inFaceNode - a node of element being created located inside a face
   *  \param check - if provided, returns result of UV check that it enforces
   */
    gp_XY GetNodeUV(const TopoDS_Face &F, const SMDS_MeshNode *n,
                    const SMDS_MeshNode *inFaceNode = 0, bool *check = 0) const;
    /*!
   ...
   */
    bool CheckNodeUV(const TopoDS_Face &F, const SMDS_MeshNode *n, gp_XY &uv, const double tol,
                     const bool force = false, double distXYZ[4] = 0) const;
    /*!
   ...
   */
    bool CheckNodeU(const TopoDS_Edge &E, const SMDS_MeshNode *n, double &u, const double tol,
                    const bool force = false, double distXYZ[4] = 0) const;
    /*!
   * \brief Return middle UV taking in account surface period
   */
    static gp_XY GetMiddleUV(const Handle(Geom_Surface) & surface, const gp_XY &uv1,
                             const gp_XY &uv2);
    /*!
   * \brief Return UV for the central node of a biquadratic triangle
   */
    static gp_XY GetCenterUV(const gp_XY &uv1, const gp_XY &uv2, const gp_XY &uv3,
...
#define gp_XY_FunPtr(meth)                                                                         \
    static gp_XY __gpXY_##meth(const gp_XY &uv1, const gp_XY &uv2)                                 \
    {                                                                                              \
        return uv1.meth(uv2);                                                                      \
    }                                                                                              \
   ...
   */
    static gp_XY ApplyIn2D(Handle(Geom_Surface) surface, const gp_XY &uv1, const gp_XY &uv2,
                           xyFunPtr fun, const bool resultInPeriod = true);
   ...
   */
    GeomAPI_ProjectPointOnSurf &GetProjector(const TopoDS_Face &F, TopLoc_Location &loc,
                                             double tol = 0) const;
    /*!
   ...
   */
    const SMDS_MeshNode *GetMediumNode(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                       const bool force3d,
   ...
   */
    const SMDS_MeshNode *GetCentralNode(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2,
                                        const SMDS_MeshNode *n3, const SMDS_MeshNode *n4,
...
private:
    ...
    struct TBiQuad: public std::pair<int, std::pair<int, int>> {
        TBiQuad(const SMDS_MeshNode *n1, const SMDS_MeshNode *n2, const SMDS_MeshNode *n3,
...
//=======================================================================
inline gp_XY SMESH_MesherHelper::calcTFI(double x, double y, const gp_XY &a0, const gp_XY &a1,
                                         const gp_XY &a2, const gp_XY &a3, const gp_XY &p0,
                                         const gp_XY &p1, const gp_XY &p2, const gp_XY &p3)
{
    return ((1 - y) * p0 + x * p1 + y * p2 + (1 - x) * p3)
        - ((1 - x) * (1 - y) * a0 + x * (1 - y) * a1 + x * y * a2 + (1 - x) * y * a3);
}
//=======================================================================
inline gp_XYZ SMESH_MesherHelper::calcTFI(double x, double y, const gp_XYZ &a0, const gp_XYZ &a1,
                                          const gp_XYZ &a2, const gp_XYZ &a3, const gp_XYZ &p0,
                                          const gp_XYZ &p1, const gp_XYZ &p2, const gp_XYZ &p3)
{
    return ((1 - y) * p0 + x * p1 + y * p2 + (1 - x) * p3)

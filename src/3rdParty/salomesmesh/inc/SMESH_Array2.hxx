...
*/
template<class TheItemType> class SMESH_Array2: public NCollection_BaseCollection<TheItemType>
{
public:
    ...
    public:
        ...
        Iterator(const SMESH_Array2 &theArray)
            : myCurrent(0), mySize(theArray.Length()),
        ...
        //! Check end
        virtual Standard_Boolean More(void) const { return (myCurrent < mySize); }
        //! Make step
        virtual void Next(void) { myCurrent++; }
        //! Constant value access
        virtual const TheItemType &Value(void) const { return myArray->myStart[myCurrent]; }
        //! Variable value access
        virtual TheItemType &ChangeValue(void) const { return myArray->myStart[myCurrent]; }
...
public:
    ...
    //! Constructor
    SMESH_Array2(const Standard_Integer theRowLower, const Standard_Integer theRowUpper,
                 const Standard_Integer theColLower, const Standard_Integer theColUpper)
        : NCollection_BaseCollection<TheItemType>(), myLowerRow(theRowLower),
          myUpperRow(theRowUpper), myLowerCol(theColLower), myUpperCol(theColUpper),
    ...
    SMESH_Array2(const SMESH_Array2 &theOther)
        : NCollection_BaseCollection<TheItemType>(), myLowerRow(theOther.LowerRow()),
          myUpperRow(theOther.UpperRow()), myLowerCol(theOther.LowerCol()),
    ...
    SMESH_Array2(const TheItemType &theBegin, const Standard_Integer theRowLower,
    ...
    void Init(const TheItemType &theValue)
    {
        ...
        for (pCur = myStart; pCur < pEnd; pCur++) *pCur = theValue;
    }

    //! Size (number of items)
    virtual Standard_Integer Size(void) const { return Length(); }
    //! Length (number of items)
    Standard_Integer Length(void) const { return RowLength() * ColLength(); }

    //! RowLength
    Standard_Integer RowLength(void) const { return (myUpperCol - myLowerCol + 1); }
    //! ColLength
    Standard_Integer ColLength(void) const { return (myUpperRow - myLowerRow + 1); }

    //! LowerRow
    Standard_Integer LowerRow(void) const { return myLowerRow; }
    //! UpperRow
    Standard_Integer UpperRow(void) const { return myUpperRow; }
    //! LowerCol
    Standard_Integer LowerCol(void) const { return myLowerCol; }
    //! UpperCol
    Standard_Integer UpperCol(void) const { return myUpperCol; }

    //! myDeletable flag
    Standard_Boolean IsDeletable(void) const { return myDeletable; }
    ...
    SMESH_Array2 &operator=(const SMESH_Array2 &theOther)
    {
        ...
        for (Standard_Integer i = 0; i < iSize; i++, pItem++, pMyItem++) *pMyItem = *pItem;
        return *this;
    }
    ...
    const TheItemType &Value(const Standard_Integer theRow, const Standard_Integer theCol) const
    {
        ...
        if (theRow < myLowerRow || theRow > myUpperRow || theCol < myLowerCol
            || theCol > myUpperCol)
...
private:
    ...
    void Allocate(void)
    {
        ...
        if (iRowSize <= 0 || iColSize <= 0) Standard_RangeError::Raise("SMESH_Array2::Allocate");
#endif
        if (myDeletable) {
            ...
            if (!myStart) Standard_OutOfMemory::Raise("SMESH_Array2 : Allocation failed");
#endif
        }
        // else myStart is set to the beginning of the given array
        TheItemType **pTable = new TheItemType *[iColSize];
#if !defined No_Exception && !defined No_Standard_OutOfMemory
        if (!pTable) Standard_OutOfMemory::Raise("SMESH_Array2 : Allocation failed");
#endif

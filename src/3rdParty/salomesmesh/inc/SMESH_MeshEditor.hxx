...
public:
    ...
    struct ElemFeatures //!< Features of element to create
    {
        ...
        SMESH_EXPORT ElemFeatures(SMDSAbs_ElementType type = SMDSAbs_All, bool isPoly = false,
                                  bool isQuad = false)
            : myType(type), myIsPoly(isPoly), myIsQuad(isQuad), myID(-1), myBallDiameter(0)
        {}

        SMESH_EXPORT ElemFeatures &Init(SMDSAbs_ElementType type, bool isPoly = false,
                                        bool isQuad = false)
        {
            myType = type;
            myIsPoly = isPoly;
        ...
        {
            myType = SMDSAbs_Ball;
            myBallDiameter = diameter;
            return *this;
        }

        SMESH_EXPORT ElemFeatures &Init(std::vector<int> &quanities, bool isQuad = false)
        {
            myType = SMDSAbs_Volume;
            myIsPoly = 1;
            myIsQuad = isQuad;
            myPolyhedQuantities.swap(quanities);
            return *this;
        }
   ...
   */
    ...
    int Reorient2D(TIDSortedElemSet &theFaces, const gp_Dir &theDirection,
    ...
    int Reorient2DBy3D(TIDSortedElemSet &theFaces, TIDSortedElemSet &theVolumes,
   ...
   */
    bool TriToQuad(TIDSortedElemSet &theElems, SMESH::Controls::NumericalFunctorPtr theCriterion,
   ...
   */
    ...
    {
        HEXA_TO_5 = 1, // split into tetrahedra
        HEXA_TO_6,
        HEXA_TO_24,
   ...
   */
    void GetHexaFacetsToSplit(TIDSortedElemSet &theHexas, const gp_Ax1 &theFacetNormal,
   ...
   */
    ...
    void Smooth(TIDSortedElemSet &theElements, std::set<const SMDS_MeshNode *> &theFixedNodes,
                const SmoothMethod theSmoothMethod, const int theNbIterations,
                double theTgtAspectRatio = 1.0, const bool the2D = true);
    ...
    typedef std::map<const SMDS_MeshElement *, std::list<const SMDS_MeshElement *>, TElemSort>
        TTElemOfElemListMap;
    ...
    PGroupIDs RotationSweep(TIDSortedElemSet theElements[2], const gp_Ax1 &theAxis,
                            const double theAngle, const int theNbSteps, const double theToler,
                            const bool theMakeGroups, const bool theMakeWalls = true);
   ...
   */
    class SMESH_EXPORT ExtrusParam
    {
        ...
        int (ExtrusParam::*myMakeNodesFun)(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                                           std::list<const SMDS_MeshNode *> &newNodes,
                                           const bool makeMediumNodes);

    public:
        ExtrusParam(const gp_Vec &theStep, const int theNbSteps, const int theFlags = 0,
                    const double theTolerance = 1e-6);
        ExtrusParam(const gp_Dir &theDir, Handle(TColStd_HSequenceOfReal) theSteps,
                    const int theFlags = 0, const double theTolerance = 1e-6);
        ExtrusParam(const double theStep, const int theNbSteps, const int theFlags,
        ...
        int MakeNodes(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                      std::list<const SMDS_MeshNode *> &newNodes, const bool makeMediumNodes)
        {
            return (this->*myMakeNodesFun)(mesh, srcNode, newNodes, makeMediumNodes);
        }

    private:
        int makeNodesByDir(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                           std::list<const SMDS_MeshNode *> &newNodes, const bool makeMediumNodes);
        int makeNodesByDirAndSew(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                                 std::list<const SMDS_MeshNode *> &newNodes,
                                 const bool makeMediumNodes);
        int makeNodesByNormal2D(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                                std::list<const SMDS_MeshNode *> &newNodes,
                                const bool makeMediumNodes);
        int makeNodesByNormal1D(SMESHDS_Mesh *mesh, const SMDS_MeshNode *srcNode,
                                std::list<const SMDS_MeshNode *> &newNodes,
                                const bool makeMediumNodes);
        // step iteration
   ...
   */
    PGroupIDs ExtrusionSweep(TIDSortedElemSet theElems[2], const gp_Vec &theStep,
                             const int theNbSteps, TTElemOfElemListMap &newElemsMap,
                             const int theFlags, const double theTolerance = 1.e-6);
   ...
   */
    PGroupIDs ExtrusionSweep(TIDSortedElemSet theElems[2], ExtrusParam &theParams,
    ...
    Extrusion_Error ExtrusionAlongTrack(TIDSortedElemSet theElements[2],
                                        ...
                                        const SMDS_MeshNode *theNodeStart, const bool theHasAngles,
                                        std::list<double> &theAngles, const bool theLinearVariation,
                                        const bool theHasRefPoint, const gp_Pnt &theRefPoint,
    ...
    PGroupIDs Transform(TIDSortedElemSet &theElements, const gp_Trsf &theTrsf, const bool theCopy,
                        const bool theMakeGroups, SMESH_Mesh *theTargetMesh = 0);
    // Move or copy theElements applying theTrsf to their nodes

    typedef std::list<std::list<const SMDS_MeshNode *>> TListOfListOfNodes;

    void FindCoincidentNodes(TIDSortedNodeSet &theNodes, const double theTolerance,
    ...
    static bool CheckFreeBorderNodes(const SMDS_MeshNode *theNode1, const SMDS_MeshNode *theNode2,
    ...
    static bool FindFreeBorder(const SMDS_MeshNode *theFirstNode,
                               const SMDS_MeshNode *theSecondNode, const SMDS_MeshNode *theLastNode,
    ...
    Sew_Error
    SewFreeBorder(const SMDS_MeshNode *theBorderFirstNode, const SMDS_MeshNode *theBorderSecondNode,
                  const SMDS_MeshNode *theBorderLastNode, const SMDS_MeshNode *theSide2FirstNode,
                  const SMDS_MeshNode *theSide2SecondNode,
                  const SMDS_MeshNode *theSide2ThirdNode = 0,
                  const bool theSide2IsFreeBorder = true, const bool toCreatePolygons = false,
    ...
    Sew_Error SewSideElements(TIDSortedElemSet &theSide1, TIDSortedElemSet &theSide2,
    ...
    void InsertNodesIntoLink(const SMDS_MeshElement *theFace, const SMDS_MeshNode *theBetweenNode1,
    ...
    void UpdateVolumes(const SMDS_MeshNode *theBetweenNode1, const SMDS_MeshNode *theBetweenNode2,
                       std::list<const SMDS_MeshNode *> &theNodesToInsert);
   ...
   */
    static void GetLinkedNodes(const SMDS_MeshNode *node, TIDSortedElemSet &linkedNodes,
   ...
   */
    ...
    bool DoubleNodes(const TIDSortedElemSet &theElems, const TIDSortedElemSet &theNodesNot,
    ...
    bool DoubleNodesInRegion(const TIDSortedElemSet &theElems, const TIDSortedElemSet &theNodesNot,
                             const TopoDS_Shape &theShape);
    ...
    void CreateHoleSkin(double radius, const TopoDS_Shape &theShape,
   ...
   */
    ...
    int MakeBoundaryMesh(const TIDSortedElemSet &elements, Bnd_Dimension dimension,
                         SMESH_Group *group = 0, SMESH_Mesh *targetMesh = 0,
                         bool toCopyElements = false, bool toCopyExistingBondary = false,
                         bool toAddExistingBondary = false, bool aroundElements = false);
...
private:
   ...
   */
    int convertElemToQuadratic(SMESHDS_SubMesh *theSm, SMESH_MesherHelper &theHelper,
   ...
   */
    PGroupIDs generateGroups(const SMESH_SequenceOfElemPtr &nodeGens,
                             const SMESH_SequenceOfElemPtr &elemGens, const std::string &postfix,
                             SMESH_Mesh *targetMesh = 0, const bool topPresent = true);
    /*!
   ...
   */
    void sweepElement(const SMDS_MeshElement *elem,
                      ...
                      std::list<const SMDS_MeshElement *> &newElems, const int nbSteps,
   ...
   * \param srcElements - to append elem for each generated element
   */
    void makeWalls(TNodeOfNodeListMap &mapNewNodes, TTElemOfElemListMap &newElemsMap,
    ...
    Extrusion_Error MakeEdgePathPoints(std::list<double> &aPrms, const TopoDS_Edge &aTrackEdge,
                                       bool aFirstIsStart,
    ...
    Extrusion_Error MakeExtrElements(TIDSortedElemSet theElements[2],
                                     ...
                                     const bool theHasAngles, std::list<double> &theAngles,
                                     const bool theLinearVariation, const bool theHasRefPoint,
                                     const gp_Pnt &theRefPoint, const bool theMakeGroups);
    void LinearAngleVariation(const int NbSteps, std::list<double> &theAngles);

    bool doubleNodes(SMESHDS_Mesh *theMeshDS, const TIDSortedElemSet &theElems,
                     const TIDSortedElemSet &theNodesNot, TNodeNodeMap &theNodeNodeMap,

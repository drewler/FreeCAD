...
public:
    ...
    bool getBoundaryPoints(std::size_t iMAEdge, double maEdgeParam, BoundaryPoint &bp1,
...
public: // internal: construction
    void init(std::vector<const TVDEdge *> &maEdges, const Boundary *boundary,
...
private:
    bool addDivPntForConcaVertex(std::vector<std::size_t> &edgeIDs1,
                                 ...
                                 const std::vector<const TVDEdge *> &maEdgesTwin, int &i) const;
...
{
public:
    MedialAxis(const TopoDS_Face &face, const std::vector<TopoDS_Edge> &edges,
               const double minSegLen, const bool ignoreCorners = false);

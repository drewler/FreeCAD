...
public:
   ...
   */
    SMESH_subMesh *SubMesh(const int shapeID) const { return Mesh()->GetSubMesh(Shape(shapeID)); }
   ...
   */
    SMESHDS_SubMesh *SubMeshDS(const int shapeID) const { return SubMesh(shapeID)->GetSubMeshDS(); }
   ...
   */
    const TopoDS_Shape &Shape(const int shapeID) const { return myShapeIDMap(shapeID); }
   ...
   */
    int ShapeID(const TopoDS_Shape &shape) const { return myShapeIDMap.FindIndex(shape); }
   ...
   */
    static bool IsForwardEdge(SMESHDS_Mesh *meshDS, const TParam2ColumnMap &columnsMap,
...
private:
   ...
   */
    ...
    public:
        TSideFace(SMESH_Mesh &mesh, const int faceID, const Prism_3D::TQuadList &quadList,
                  const TopoDS_Edge &baseEdge, TParam2ColumnMap *columnsMap,
                  const double first = 0.0, const double last = 1.0);
        TSideFace(SMESH_Mesh &mesh, const std::vector<TSideFace *> &components,
        ...
        gp_XY GetNodeUV(const TopoDS_Face &F, const SMDS_MeshNode *n,
                        const SMDS_MeshNode *n2 = 0) const
        {
        ...
        int ColumnHeight() const
        {
            if (NbComponents()) return GetComponent(0)->GetColumns()->begin()->second.size();
                ...
                return GetColumns()->begin()->second.size();
        }
   ...
   */
    ...
    public:
        TPCurveOnHorFaceAdaptor(const TSideFace *sideFace, const bool isTop,
...
private:
    gp_XYZ bndPoint(int iP, int z) const { return SMESH_TNodeXYZ((*myBndColumns[iP])[z]); }

    gp_XYZ intPoint(int iP, int z) const { return SMESH_TNodeXYZ((*myIntColumns[iP])[z]); }
    ...
    static void applyBoundaryError(const std::vector<gp_XYZ> &bndPoints,
                                   ...
                                   const std::vector<gp_XYZ> &bndError2, const double r,
                                   std::vector<gp_XYZ> &toIntPoints,

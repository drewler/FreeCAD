...
template<class TValue, class TRepresentation> struct TValueHolder {
    TValue &myValue;
    TRepresentation myRepresentation;

    TValueHolder(TValue &theValue) : myValue(theValue), myRepresentation(TRepresentation(theValue))
    {}

    ~TValueHolder() { myValue = TValue(myRepresentation); }

    TRepresentation *operator&() { return &myRepresentation; }

    operator TRepresentation() const { return myRepresentation; }

    const TValue &operator()() const { return myValue; }
};

//! To customize TValueHolder common template definition for TVector
template<class TVal, class TRepresentation> struct TValueHolder<TVector<TVal>, TRepresentation> {
    typedef TVector<TVal> TValue;
    TValue &myValue;
    TRepresentation *myRepresentation;

    TValueHolder(TValue &theValue) : myValue(theValue)
    {
        if (theValue.empty()) myRepresentation = (TRepresentation *)NULL;
        else
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTNameInfo: virtual TNameInfo {
    ...
    virtual std::string GetName() const { return GetString(0, GetNOMLength<eVersion>(), myName); }
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTMeshInfo: virtual TMeshInfo, virtual TTNameInfo<eVersion> {
    typedef TTNameInfo<eVersion> TNameInfoBase;

    TTMeshInfo(const PMeshInfo &theInfo) : TNameInfoBase(theInfo->GetName())
    {
    ...
    TTMeshInfo(TInt theDim, TInt theSpaceDim, const std::string &theValue, EMaillage theType,
    ...
    virtual std::string GetDesc() const { return GetString(0, GetDESCLength<eVersion>(), myDesc); }
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTFamilyInfo: virtual TFamilyInfo, virtual TTNameInfo<eVersion> {
    typedef TTNameInfo<eVersion> TNameInfoBase;
    ...
    TTFamilyInfo(const PMeshInfo &theMeshInfo, TInt theNbGroup, TInt theNbAttr, TInt theId,
    ...
    TTFamilyInfo(const PMeshInfo &theMeshInfo, const std::string &theValue, TInt theId,
                 const TStringSet &theGroupNames, const TStringVector &theAttrDescs,
                 const TIntVector &theAttrIds, const TIntVector &theAttrVals)
        : TNameInfoBase(theValue)
    {
        myMeshInfo = theMeshInfo;
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTElemInfo: virtual TElemInfo {
    TTElemInfo(const PMeshInfo &theMeshInfo, const PElemInfo &theInfo)
    {
        ...
        if (theInfo->IsElemNum()) myElemNum.reset(new TElemNum(myNbElem));
        else
    ...
    TTElemInfo(const PMeshInfo &theMeshInfo, TInt theNbElem, EBooleen theIsElemNum,
               EBooleen theIsElemNames)
    {
        ...
        if (theIsElemNum) myElemNum.reset(new TElemNum(theNbElem));
        else
            myElemNum.reset(new TElemNum());
    ...
    TTElemInfo(const PMeshInfo &theMeshInfo, TInt theNbElem, const TIntVector &theFamilyNums,
               const TIntVector &theElemNums, const TStringVector &theElemNames)
    {
        myMeshInfo = theMeshInfo;
        ...
        if (myIsElemNum) myElemNum.reset(new TElemNum(theNbElem));
        else
            myElemNum.reset(new TElemNum());
        ...
        if (theNbElem) {
            ...
            if (theFamilyNums.size()) *myFamNum = theFamilyNums;

            if (myIsElemNum) *myElemNum = theElemNums;
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTNodeInfo: virtual TNodeInfo, virtual TTElemInfo<eVersion> {
    typedef TTElemInfo<eVersion> TElemInfoBase;

    TTNodeInfo(const PMeshInfo &theMeshInfo, const PNodeInfo &theInfo)
        : TNodeInfo(theInfo), TElemInfoBase(theMeshInfo, theInfo)
    {
    ...
    TTNodeInfo(const PMeshInfo &theMeshInfo, TInt theNbElem, EModeSwitch theMode, ERepere theSystem,
               EBooleen theIsElemNum, EBooleen theIsElemNames)
        : TModeSwitchInfo(theMode),
          TElemInfoBase(theMeshInfo, theNbElem, theIsElemNum, theIsElemNames)
    {
        mySystem = theSystem;
    ...
    TTNodeInfo(const PMeshInfo &theMeshInfo, const TFloatVector &theNodeCoords, EModeSwitch theMode,
               ERepere theSystem, const TStringVector &theCoordNames,
               const TStringVector &theCoordUnits, const TIntVector &theFamilyNums,
               const TIntVector &theElemNums, const TStringVector &theElemNames)
        : TModeSwitchInfo(theMode),
          TElemInfoBase(theMeshInfo, (TInt)theNodeCoords.size() / theMeshInfo->GetDim(),
                        theFamilyNums, theElemNums, theElemNames)
    {
        mySystem = theSystem;
        ...
        if (!theCoordNames.empty())
            for (TInt anId = 0; anId < aSpaceDim; anId++) SetCoordName(anId, theCoordNames[anId]);

        myCoordUnits.resize(aSpaceDim * GetPNOMLength<eVersion>() + 1);
        if (!theCoordUnits.empty())
            for (TInt anId = 0; anId < aSpaceDim; anId++) SetCoordUnit(anId, theCoordUnits[anId]);
    }
...
struct TTPolygoneInfo: virtual TPolygoneInfo, virtual TTElemInfo<eVersion> {
    typedef TTElemInfo<eVersion> TElemInfoBase;
    ...
    TTPolygoneInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                   EGeometrieElement theGeom, TInt theNbElem, TInt theConnSize,
                   EConnectivite theConnMode, EBooleen theIsElemNum, EBooleen theIsElemNames)
        : TElemInfoBase(theMeshInfo, theNbElem, theIsElemNum, theIsElemNames)
    {
        myEntity = theEntity;
        myGeom = theGeom;
    ...
    TTPolygoneInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                   EGeometrieElement theGeom, const TIntVector &theIndexes,
                   const TIntVector &theConnectivities, EConnectivite theConnMode,
                   const TIntVector &theFamilyNums, const TIntVector &theElemNums,
                   const TStringVector &theElemNames)
        : TElemInfoBase(theMeshInfo, (TInt)theIndexes.size() - 1, theFamilyNums, theElemNums,
                        theElemNames)
    {
        myEntity = theEntity;
        myGeom = theGeom;
...
//---------------------------------------------------------------
template<EVersion eVersion>
struct TTPolyedreInfo: virtual TPolyedreInfo, virtual TTElemInfo<eVersion> {
    typedef TTElemInfo<eVersion> TElemInfoBase;
    ...
    TTPolyedreInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                   EGeometrieElement theGeom, TInt theNbElem, TInt theNbFaces, TInt theConnSize,
                   EConnectivite theConnMode, EBooleen theIsElemNum, EBooleen theIsElemNames)
        : TElemInfoBase(theMeshInfo, theNbElem, theIsElemNum, theIsElemNames)
    {
        myEntity = theEntity;
    ...
    TTPolyedreInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity,
                   EGeometrieElement theGeom, const TIntVector &theIndexes,
                   const TIntVector &theFaces, const TIntVector &theConnectivities,
                   EConnectivite theConnMode, const TIntVector &theFamilyNums,
                   const TIntVector &theElemNums, const TStringVector &theElemNames)
        : TElemInfoBase(theMeshInfo, (TInt)theIndexes.size() - 1, theFamilyNums, theElemNums,
                        theElemNames)
    {
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTCellInfo: virtual TCellInfo, virtual TTElemInfo<eVersion> {
    typedef TTElemInfo<eVersion> TElemInfoBase;
    ...
    TTCellInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity, EGeometrieElement theGeom,
               TInt theNbElem, EConnectivite theConnMode, EBooleen theIsElemNum,
               EBooleen theIsElemNames, EModeSwitch theMode)
        : TModeSwitchInfo(theMode),
          TElemInfoBase(theMeshInfo, theNbElem, theIsElemNum, theIsElemNames)
    {
        myEntity = theEntity;
    ...
    TTCellInfo(const PMeshInfo &theMeshInfo, EEntiteMaillage theEntity, EGeometrieElement theGeom,
               const TIntVector &theConnectivities, EConnectivite theConnMode,
               const TIntVector &theFamilyNums, const TIntVector &theElemNums,
               const TStringVector &theElemNames, EModeSwitch theMode)
        : TModeSwitchInfo(theMode),
          TElemInfoBase(theMeshInfo, (TInt)theConnectivities.size() / GetNbNodes(theGeom),
                        theFamilyNums, theElemNums, theElemNames)
    {
        myEntity = theEntity;
        myGeom = theGeom;
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTBallInfo: virtual TBallInfo, virtual TTCellInfo<eVersion> {
    typedef TTCellInfo<eVersion> TCellInfoBase;

    TTBallInfo(const PMeshInfo &theMeshInfo, const PBallInfo &theInfo)
        : TCellInfoBase::TElemInfoBase(theMeshInfo, theInfo), TCellInfoBase(theMeshInfo, theInfo)
    {
        myDiameters = theInfo->myDiameters;
    }

    TTBallInfo(const PMeshInfo &theMeshInfo, TInt theNbElem, EBooleen theIsElemNum)
        : TCellInfoBase::TElemInfoBase(theMeshInfo, theNbElem, theIsElemNum,
                                       /*theIsElemNames=*/eFAUX),
          TCellInfoBase(theMeshInfo, eSTRUCT_ELEMENT, eBALL, theNbElem,
                        /*EConnectivite=*/eNOD, theIsElemNum,
                        /*theIsElemNames=*/eFAUX, eFULL_INTERLACE)
    {
        myDiameters.resize(theNbElem);
    }

    TTBallInfo(const PMeshInfo &theMeshInfo, const TIntVector &theNodes, TFloatVector &theDiameters,
               const TIntVector &theFamilyNums, const TIntVector &theElemNums)
        : TCellInfoBase::TElemInfoBase(theMeshInfo,
                                       (TInt)std::max(theNodes.size(), theDiameters.size()),
                                       theFamilyNums, theElemNums, TStringVector()),
          TCellInfoBase(theMeshInfo, eSTRUCT_ELEMENT, eBALL, theNodes,
                        /*EConnectivite=*/eNOD, theFamilyNums, theElemNums, TStringVector(),
                        eFULL_INTERLACE)
    {
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTFieldInfo: virtual TFieldInfo, virtual TTNameInfo<eVersion> {
    typedef TTNameInfo<eVersion> TNameInfoBase;
    ...
    TTFieldInfo(const PMeshInfo &theMeshInfo, TInt theNbComp, ETypeChamp theType,
                const std::string &theValue, EBooleen theIsLocal, TInt theNbRef)
        : TNameInfoBase(theValue)
    {
        myMeshInfo = theMeshInfo;
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTGaussInfo: virtual TGaussInfo, virtual TTNameInfo<eVersion> {
    typedef TTNameInfo<eVersion> TNameInfoBase;

    TTGaussInfo(const TGaussInfo::TInfo &theInfo, EModeSwitch theMode)
        : TModeSwitchInfo(theMode), TNameInfoBase(boost::get<1>(boost::get<0>(theInfo)))
    {
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTTimeStampInfo: virtual TTimeStampInfo {
    ...
    TTTimeStampInfo(const PFieldInfo &theFieldInfo, EEntiteMaillage theEntity,
                    const TGeom2Size &theGeom2Size, const TGeom2NbGauss &theGeom2NbGauss,
                    TInt theNumDt, TInt theNumOrd, TFloat theDt, const std::string &theUnitDt,
...
//---------------------------------------------------------------
template<EVersion eVersion>
struct TTProfileInfo: virtual TProfileInfo, virtual TTNameInfo<eVersion> {
    typedef TTNameInfo<eVersion> TNameInfoBase;
...
struct TTTimeStampValue: virtual TTimeStampValue<TMeshValueType> {
    TTTimeStampValue(const PTimeStampInfo &theTimeStampInfo, const PTimeStampValueBase &theInfo,
    ...
    TTTimeStampValue(const PTimeStampInfo &theTimeStampInfo, ETypeChamp theTypeChamp,
                     const TGeom2Profile &theGeom2Profile, EModeSwitch theMode)
        : TModeSwitchInfo(theMode)
    {
        this->myTimeStampInfo = theTimeStampInfo;
        ...
        for (; anIter != aGeom2Size.end(); anIter++) {
            ...
            if (anIter != theGeom2Profile.end()) aProfileInfo = anIter->second;

            if (aProfileInfo && aProfileInfo->IsPresent()) aNbElem = aProfileInfo->GetSize();
    ...
    virtual void AllocateValue(EGeometrieElement theGeom, TInt theNbElem, TInt theNbGauss,
                               TInt theNbComp, EModeSwitch theMode = eFULL_INTERLACE)
    {
...
//---------------------------------------------------------------
template<EVersion eVersion> struct TTGrilleInfo: virtual TGrilleInfo {

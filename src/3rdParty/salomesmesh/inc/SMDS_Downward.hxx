...
typedef struct {
    ElemByNodesType
        elems[6]; //!< max number of faces in a volume or edges in a face : hexahedron = 6
    int nbElems;
...
{
public:
    DownIdType(int a, unsigned char b) : cellId(a), cellType(b) {}
    int cellId;
...
struct DownIdCompare {
    ...
    {
        if (e1.cellId == e2.cellId) return (e1.cellType < e2.cellType);
        else
...
public:
    ...
    int getVtkCellId(int cellId) { return _vtkCellIds[cellId]; }
    int getMaxId() { return _maxId; }
...
protected:
    ...
    virtual void addDownCell(int cellId, int lowCellId,
                             unsigned char aType); //!< Id's are downward connectivity id's
    virtual void addUpCell(int cellId, int upCellId,
                           unsigned char aType); //!< Id's are downward connectivity id's
    ...
    std::vector<int>
        _cellIds; //!< growing size: all the down cell id's, size = _maxId * _nbDownCells
    std::vector<int> _vtkCellIds; //!< growing size: size = _maxId, either vtkId or -1
    std::vector<unsigned char>
        _cellTypes; //!< fixed size: the same vector for all cells of a derived class
...
protected:
    ...
    int computeFaces(int cellId, int *vtkIds, int nbcells, int *downFaces,
                     unsigned char *downTypes);
    int computeFaces(int *pts, int *vtkIds, int nbcells, int *downFaces, unsigned char *downTypes);

    std::vector<std::vector<int>>
        _upCellIdsVector; //!< the number of faces sharing an edge is not known
    std::vector<std::vector<unsigned char>>
        _upCellTypesVector;                  //!< the number of faces sharing an edge is not known
...
protected:
    ...
    std::vector<int>
        _tempNodes; //!< temporary storage of nodes, until downward connectivity completion

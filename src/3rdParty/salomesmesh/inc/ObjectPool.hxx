...
private:
    ...
    int getNextFree()
    {
        ...
        if (_nbHoles == 0) return std::min(_maxOccupied + 1, _maxAvail);
...
public:
    ...
    virtual ~ObjectPool()
    {
        for (size_t i = 0; i < _chunkList.size(); i++) delete[] _chunkList[i];
    }
    ...
    X *getNew()
    {
        ...
        if (_nextFree < _maxOccupied) { _nbHoles -= 1; }
        else {
    ...
    void destroy(X *obj)
    {
        ...
        for (size_t i = 0; i < _chunkList.size(); i++) {
            ...
            if (toFree < _maxOccupied) _nbHoles += 1;
            //obj->clean();

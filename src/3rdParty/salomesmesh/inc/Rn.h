...
template<class T> inline void Echange(T &a, T &b)
{
    T c = a;
    a = b;
    b = c;
...
public:
    ...
    {
        return xyzMin.x <= x && x <= xyzMax.x && xyzMin.y <= y && y <= xyzMax.y && xyzMin.z <= z
            && z <= xyzMax.z;
    }
};
...
public:
    ...
    {
        return x * P.x + y * P.y + z * P.z + omega * P.omega;
    } // produit scalaire
...
{
    return R3(P.x < Q.x ? P.x : Q.x, P.y < Q.y ? P.y : Q.y, P.z < Q.z ? P.z : Q.z);
} //Pt de xyz Min

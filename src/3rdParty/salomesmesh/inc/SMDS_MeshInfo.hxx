...
inline SMDS_MeshInfo::SMDS_MeshInfo()
    : myNbNodes(0), myNb0DElements(0), myNbBalls(0), myNbEdges(0), myNbQuadEdges(0),
      myNbTriangles(0), myNbQuadTriangles(0), myNbBiQuadTriangles(0), myNbQuadrangles(0),
      myNbQuadQuadrangles(0), myNbBiQuadQuadrangles(0), myNbPolygons(0), myNbQuadPolygons(0),
      myNbTetras(0), myNbQuadTetras(0), myNbHexas(0), myNbQuadHexas(0), myNbTriQuadHexas(0),
      myNbPyramids(0), myNbQuadPyramids(0), myNbPrisms(0), myNbQuadPrisms(0), myNbHexPrism(0),
      myNbPolyhedrons(0)
{
...
{
    for (std::size_t i = 0; i < myNb.size(); ++i)
        if (myNb[i]) (*myNb[i]) = (*other.myNb[i]);
...
{
    if (el->IsQuadratic()) --myNbQuadEdges;
    else
        --myNbEdges;
}
...
{
    return order == ORDER_ANY   ? myNbEdges + myNbQuadEdges
        : order == ORDER_LINEAR ? myNbEdges
                                : myNbQuadEdges;
}
...
{
    return NbTriangles(order) + NbQuadrangles(order)
        + (order == ORDER_ANY          ? myNbPolygons + myNbQuadPolygons
               : order == ORDER_LINEAR ? myNbPolygons
                                       : myNbQuadPolygons);
}
...
{
    return order == ORDER_ANY   ? myNbTriangles + myNbQuadTriangles + myNbBiQuadTriangles
        : order == ORDER_LINEAR ? myNbTriangles
...
{
    return order == ORDER_ANY   ? myNbQuadrangles + myNbQuadQuadrangles + myNbBiQuadQuadrangles
        : order == ORDER_LINEAR ? myNbQuadrangles
                                : myNbQuadQuadrangles + myNbBiQuadQuadrangles;
}

inline int // NbPolygons
SMDS_MeshInfo::NbPolygons(SMDSAbs_ElementOrder order) const
{
    return order == ORDER_ANY   ? myNbPolygons + myNbQuadPolygons
        : order == ORDER_LINEAR ? myNbPolygons
                                : myNbQuadPolygons;
}
...
{
    return NbTetras(order) + NbHexas(order) + NbPyramids(order) + NbPrisms(order)
        + NbHexPrisms(order) + (order == ORDER_QUADRATIC ? 0 : myNbPolyhedrons);
}

inline int // NbTetras
SMDS_MeshInfo::NbTetras(SMDSAbs_ElementOrder order) const
{
    return order == ORDER_ANY   ? myNbTetras + myNbQuadTetras
        : order == ORDER_LINEAR ? myNbTetras
                                : myNbQuadTetras;
}
...
{
    return order == ORDER_ANY   ? myNbHexas + myNbQuadHexas + myNbTriQuadHexas
        : order == ORDER_LINEAR ? myNbHexas
                                : myNbQuadHexas + myNbTriQuadHexas;
}

inline int // NbPyramids
SMDS_MeshInfo::NbPyramids(SMDSAbs_ElementOrder order) const
{
    return order == ORDER_ANY   ? myNbPyramids + myNbQuadPyramids
        : order == ORDER_LINEAR ? myNbPyramids
                                : myNbQuadPyramids;
}
...
{
    return order == ORDER_ANY   ? myNbPrisms + myNbQuadPrisms
        : order == ORDER_LINEAR ? myNbPrisms
                                : myNbQuadPrisms;
}
...
SMDS_MeshInfo::NbElements(SMDSAbs_ElementType type) const
{
    ...
    switch (type) {
        case SMDSAbs_All:
            for (std::size_t i = 1 + index(SMDSAbs_Node, 1); i < myNb.size(); ++i)
                if (myNb[i]) nb += *myNb[i];
            nb += myNbPolygons + myNbQuadPolygons + myNbPolyhedrons;
            break;
        case SMDSAbs_Volume:
            nb = (myNbTetras + myNbPyramids + myNbPrisms + myNbHexas + myNbHexPrism + myNbQuadTetras
                  + myNbQuadPyramids + myNbQuadPrisms + myNbQuadHexas + myNbTriQuadHexas
                  + myNbPolyhedrons);
            break;
        case SMDSAbs_Face:
            nb = (myNbTriangles + myNbQuadrangles + myNbQuadTriangles + myNbBiQuadTriangles
                  + myNbQuadQuadrangles + myNbBiQuadQuadrangles + myNbPolygons + myNbQuadPolygons);
            break;
        case SMDSAbs_Edge: nb = myNbEdges + myNbQuadEdges; break;
        case SMDSAbs_Node: nb = myNbNodes; break;
        case SMDSAbs_0DElement: nb = myNb0DElements; break;
        case SMDSAbs_Ball: nb = myNbBalls; break;
        default:;
    }
    return nb;
}
...
#endif
        case SMDSEntity_Quad_Polyhedra: break;
        default: break;
    }
    return 0;
}
...
SMDS_MeshInfo::NbElementsOfGeom(SMDSAbs_GeometryType geom) const
{
    switch (geom) {
        ...
        case SMDSGeom_POINT:
            ...
            // 1D:
        case SMDSGeom_EDGE:
            return (myNbEdges + myNbQuadEdges);
            // 2D:
        case SMDSGeom_TRIANGLE: return (myNbTriangles + myNbQuadTriangles + myNbBiQuadTriangles);
        case SMDSGeom_QUADRANGLE:
            return (myNbQuadrangles + myNbQuadQuadrangles + myNbBiQuadQuadrangles);
        case SMDSGeom_POLYGON:
            return (myNbPolygons + myNbQuadPolygons);
            // 3D:
        case SMDSGeom_TETRA: return (myNbTetras + myNbQuadTetras);
        case SMDSGeom_PYRAMID: return (myNbPyramids + myNbQuadPyramids);
        case SMDSGeom_HEXA: return (myNbHexas + myNbQuadHexas + myNbTriQuadHexas);
        case SMDSGeom_PENTA: return (myNbPrisms + myNbQuadPrisms);
        case SMDSGeom_HEXAGONAL_PRISM: return myNbHexPrism;
        case SMDSGeom_POLYHEDRA:

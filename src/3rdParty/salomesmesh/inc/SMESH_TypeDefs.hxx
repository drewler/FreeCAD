...
typedef std::map<const SMDS_MeshElement *, std::list<const SMDS_MeshElement *>, TIDCompare>
    TElemOfElemListMap;
typedef std::map<const SMDS_MeshElement *, std::list<const SMDS_MeshNode *>, TIDCompare>
    TElemOfNodeListMap;
...
/*!
   ...
   */
template<class TVECTOR> void FreeVector(TVECTOR &vec)
{
    TVECTOR v2;
    vec.swap(v2);
}
template<class TVECTOR> void CompactVector(TVECTOR &vec)
{
    TVECTOR v2(vec);
    vec.swap(v2);
}
...
/*!
   ...
   */
template<typename TOBJ> struct ArrayDeleter {
    TOBJ *_obj;
...
struct SMESH_TNodeXYZ: public gp_XYZ {
    ...
    double _xyz[3];
    SMESH_TNodeXYZ(const SMDS_MeshElement *e = 0) : gp_XYZ(0, 0, 0), _node(0) { Set(e); }
...
DEFINE_SEQUENCE(SMESH_SequenceOfNode, SMESH_BaseCollectionNodePtr, SMDS_MeshNodePtr)

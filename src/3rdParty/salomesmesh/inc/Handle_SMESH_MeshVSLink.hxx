...
public:
    ...
    Handle(SMESH_MeshVSLink)(const Handle(SMESH_MeshVSLink) & aHandle)
        : Handle(MeshVS_DataSource3D)(aHandle)
    {}

    Handle(SMESH_MeshVSLink)(const SMESH_MeshVSLink *anItem)
        : Handle(MeshVS_DataSource3D)((MeshVS_DataSource3D *)anItem)
    {}
    ...
    SMESH_MeshVSLink *operator->() const { return (SMESH_MeshVSLink *)ControlAccess(); }

    Standard_EXPORT static const Handle(SMESH_MeshVSLink)
        DownCast(const Handle(Standard_Transient) & AnObject);
};

...
public:
    ...
    bool Apply(const TopoDS_Face &theFace, const TopoDS_Vertex &theVertexOnKeyPoint1,
    ...
    bool Apply(const TopoDS_Shell &theBlock, const TopoDS_Vertex &theVertex000,
    ...
    bool Apply(const SMDS_MeshFace *theFace, const int theNodeIndexOnKeyPoint1,
               const bool theReverse);
    // Compute nodes coordinates applying
    // the loaded pattern to <theFace>. The first key-point
    // will be mapped into <theNodeIndexOnKeyPoint1>-th node

    bool Apply(SMESH_Mesh *theMesh, const SMDS_MeshFace *theFace, const TopoDS_Shape &theSurface,
               const int theNodeIndexOnKeyPoint1, const bool theReverse);
    // Compute nodes coordinates applying
    // the loaded pattern to <theFace>. The first key-point
    // will be mapped into <theNodeIndexOnKeyPoint1>-th node

    bool Apply(SMESH_Mesh *theMesh, std::set<const SMDS_MeshFace *> &theFaces,
               const int theNodeIndexOnKeyPoint1, const bool theReverse);
    // Compute nodes coordinates applying
    // the loaded pattern to <theFaces>. The first key-point
    // will be mapped into <theNodeIndexOnKeyPoint1>-th node

    bool Apply(const SMDS_MeshVolume *theVolume, const int theNode000Index,
    ...
    bool Apply(std::set<const SMDS_MeshVolume *> &theVolumes, const int theNode000Index,
               const int theNode001Index);
    // Compute nodes coordinates applying
    ...
    bool MakeMesh(SMESH_Mesh *theMesh, const bool toCreatePolygons = false,
...
private:
    ...
    bool compUVByIsoIntersection(const std::list<std::list<TPoint *>> &boundaryPoints,
                                 const gp_XY &theInitUV, gp_XY &theUV, bool &theIsDeformed);
    ...
    bool sortSameSizeWires(TListOfEdgesList &theWireList,
                           ...
                           const TListOfEdgesList::iterator &theToWire, const int theFirstEdgeID,
    ...
    bool getFacesDefinition(const SMDS_MeshNode **theBndNodes, const int theNbBndNodes,
                            const std::vector<const SMDS_MeshNode *> &theNodes,
                            std::list<int> &theFaceDefs, std::vector<int> &theQuantity);
    ...
    bool findExistingNodes(SMESH_Mesh *mesh, const TopoDS_Shape &S,
...
private:
    ...
    std::map<int, const SMDS_MeshNode *>
        myXYZIdToNodeMap;                             // map XYZ id to node of a refined element

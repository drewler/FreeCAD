...
public:
    ...
    FaceQuadStruct::Ptr CheckAnd2Dcompute(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                          const bool CreateQuadratic);

    FaceQuadStruct::Ptr CheckNbEdges(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                     const bool considerMesh = false);

    static bool IsApplicable(const TopoDS_Shape &aShape, bool toCheckAll);

protected:
    bool checkNbEdgesForEvaluate(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape,
                                 MapShapeNbElems &aResMap, std::vector<int> &aNbNodes,
                                 bool &IsQuadratic);

    bool setNormalizedGrid(FaceQuadStruct::Ptr quad);

    void splitQuadFace(SMESHDS_Mesh *theMeshDS, const int theFaceID, const SMDS_MeshNode *theNode1,
                       const SMDS_MeshNode *theNode2, const SMDS_MeshNode *theNode3,
    ...
    bool evaluateQuadPref(SMESH_Mesh &aMesh, const TopoDS_Shape &aShape, std::vector<int> &aNbNodes,
    ...
    int getCorners(const TopoDS_Face &theFace, SMESH_Mesh &theMesh, std::list<TopoDS_Edge> &theWire,
                   std::vector<TopoDS_Vertex> &theVertices, int &theNbDegenEdges,
    ...
    void updateSideUV(FaceQuadStruct::Side &side, int iForced, const TQuadsBySide &quads,
                      int *iNext = NULL);

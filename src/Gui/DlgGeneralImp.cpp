...
DlgGeneralImp::DlgGeneralImp(QWidget *parent)
    : PreferencePage(parent), localeIndex(0), ui(new Ui_DlgGeneral)
{
    ui->setupUi(this);
    ...
    { // add special workbench to selection
        ...
        if (px.isNull()) ui->AutoloadModuleCombo->addItem(key, QVariant(value));
        else
    ...
    for (QMap<QString, QString>::Iterator it = menuText.begin(); it != menuText.end(); ++it) {
        ...
        if (px.isNull()) ui->AutoloadModuleCombo->addItem(it.key(), QVariant(it.value()));
        else
            ui->AutoloadModuleCombo->addItem(px, it.key(), QVariant(it.value()));
    }

    recreatePreferencePackMenu();
    connect(ui->SaveNewPreferencePack, &QPushButton::clicked, this,
            &DlgGeneralImp::saveAsNewPreferencePack);

    ui->ManagePreferencePacks->setToolTip(tr("Manage preference packs"));
    connect(ui->ManagePreferencePacks, &QPushButton::clicked, this,
            &DlgGeneralImp::onManagePreferencePacksClicked);
    ...
    if (backups.empty()) ui->RevertToSavedConfig->setEnabled(false);
    else
        ui->RevertToSavedConfig->setEnabled(true);
    connect(ui->RevertToSavedConfig, &QPushButton::clicked, this,
            &DlgGeneralImp::revertToSavedConfig);
}
...
void DlgGeneralImp::setNumberLocale(bool force /* = false*/)
{
    ...
    if (localeIndex == localeFormat && (!force || localeFormat == 0)) return;
...
void DlgGeneralImp::loadSettings()
{
    ...
    for (auto it = list.begin(); it != list.end(); ++it, index++) {
        ...
        if (!native.isEmpty()) {
            if (native[0].isLetter()) native[0] = native[0].toUpper();
            langname = native;
        }

        ui->Languages->addItem(langname, lang);
        if (language == lang) { ui->Languages->setCurrentIndex(index); }
    }

    QAbstractItemModel *model = ui->Languages->model();
    if (model) model->sort(0);
    ...
    index = 0;
    if (propertyView || treeView) { index = comboView ? 2 : 1; }
    ...
    if (index < 0 && !selectedStyleSheet.isEmpty()) {
        ...
        if (fi.isAbsolute()) {
            ...
            if (qssPaths.indexOf(path) >= 0) { selectedStyleSheet = fi.fileName(); }
            else {
...
void DlgGeneralImp::recreatePreferencePackMenu()
{
    ...
    ui->PreferencePacks->horizontalHeader()->setSectionResizeMode(0,
                                                                  QHeaderView::ResizeMode::Stretch);
    ui->PreferencePacks->horizontalHeader()->setSectionResizeMode(1,
                                                                  QHeaderView::ResizeMode::Stretch);
    ...
    for (const auto &pack : packs) {
        ...
        for (const auto &tag : tags) {
            if (tagString.isEmpty()) tagString.append(QString::fromStdString(tag));
            else
        ...
        button->setToolTip(
            tr("Apply the %1 preference pack").arg(QString::fromStdString(pack.first)));
        connect(button, &QPushButton::clicked, this,
                [this, pack]() { onLoadPreferencePackClicked(pack.first); });
...
void DlgGeneralImp::saveAsNewPreferencePack()
{
    ...
    connect(newPreferencePackDialog.get(), &DlgCreateNewPreferencePackImp::accepted, this,
            &DlgGeneralImp::newPreferencePackDialogAccepted);
...
void DlgGeneralImp::revertToSavedConfig()
{
    ...
    connect(revertToBackupConfigDialog.get(), &DlgRevertToBackupConfigImp::accepted, [this]() {
        ...
        if (parentDialog) parentDialog->reload();
    });
...
void DlgGeneralImp::newPreferencePackDialogAccepted()
{
    ...
    std::copy_if(preferencePackTemplates.begin(), preferencePackTemplates.end(),
                 std::back_inserter(selectedTemplates),
                 [selection](PreferencePackManager::TemplateFile &t) {
                     for (const auto &item : selection)
                         if (item.group == t.group && item.name == t.name) return true;
                     return false;
                 });
...
void DlgGeneralImp::onManagePreferencePacksClicked()
{
    if (!this->preferencePackManagementDialog) {
        this->preferencePackManagementDialog =
            std::make_unique<DlgPreferencePackManagementImp>(this);
        connect(this->preferencePackManagementDialog.get(),
                &DlgPreferencePackManagementImp::packVisibilityChanged, this,

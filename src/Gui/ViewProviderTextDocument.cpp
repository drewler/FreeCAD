...
ViewProviderTextDocument::ViewProviderTextDocument()
{
    ...
    ADD_PROPERTY_TYPE(ReadOnly, (false), "Editor", App::Prop_None,
    ...
    ADD_PROPERTY_TYPE(FontName, ((const char *)font.family().toLatin1()), "Editor", App::Prop_None,
                      "Font name");

    ADD_PROPERTY_TYPE(SyntaxHighlighter, (static_cast<long>(0)), "Editor", App::Prop_None,
                      "Syntax highlighting");
...
bool ViewProviderTextDocument::doubleClicked()
{
    if (!activateView()) {
        ...
        getMainWindow()->addWindow(new TextDocumentEditorView {
            static_cast<App::TextDocument *>(getObject()), editorWidget, getMainWindow()});
    }
    return true;
}
...
{
    if (editorWidget) {
        if (prop == &ReadOnly) { editorWidget->setReadOnly(ReadOnly.getValue()); }
...
MDIView *ViewProviderTextDocument::getMDIView() const
{
    ...
    for (auto v : views) {
        ...
        if (textView->getTextObject() == getObject()) { return textView; }
    }
    return nullptr;
}

...
{
    auto *group =
        getExtendedViewProvider()->getObject()->getExtensionByType<App::OriginGroupExtension>();
...
ViewProviderOriginGroupExtension::extensionClaimChildren3D() const
{
...
void ViewProviderOriginGroupExtension::extensionUpdateData(const App::Property *prop)
{
    ...
    auto *group =
        getExtendedViewProvider()->getObject()->getExtensionByType<App::OriginGroupExtension>();
    if (group && prop == &group->Group) { updateOriginSize(); }
...
void ViewProviderOriginGroupExtension::slotChangedObjectApp(const App::DocumentObject &obj)
{
    auto *group =
        getExtendedViewProvider()->getObject()->getExtensionByType<App::OriginGroupExtension>();
    if (group && group->hasObject(&obj, /*recursive=*/true)) { updateOriginSize(); }
}
    ...
    const Gui::ViewProviderDocumentObject &vp)
{
    if (!vp.isDerivedFrom(Gui::ViewProviderOriginFeature::getClassTypeId())) {
        ...
        if (group && obj && group->hasObject(obj, /*recursive=*/true)) { updateOriginSize(); }
    }
}
...
void ViewProviderOriginGroupExtension::updateOriginSize()
{
    ...
    if (!owner->getNameInDocument() || owner->isRemoving()

...
DlgPropertyLink::DlgPropertyLink(QWidget *parent)
    : QDialog(parent), SelectionObserver(false, ResolveMode::NoResolve), ui(new Ui_DlgPropertyLink)
{
    ui->setupUi(this);
    ui->typeTree->hide();
    ...
    connect(ui->treeWidget, SIGNAL(itemEntered(QTreeWidgetItem *, int)), this,
...
QList<App::SubObjectT> DlgPropertyLink::getLinksFromProperty(const App::PropertyLinkBase *prop)
{
    ...
    if (subs.empty()) {
        for (auto obj : objs) res.push_back(App::SubObjectT(obj, nullptr));
    }
    else if (objs.size() == 1) {
        for (auto &sub : subs) res.push_back(App::SubObjectT(objs.front(), sub.c_str()));
    }
    else {
        int i = 0;
        for (auto obj : objs) res.push_back(App::SubObjectT(obj, subs[i++].c_str()));
    }
    return res;
}

QString DlgPropertyLink::formatObject(App::Document *ownerDoc, App::DocumentObject *obj,
                                      const char *sub)
{
    if (!obj || !obj->getNameInDocument()) return QLatin1String("?");
    ...
    if (!sub || !sub[0]) {
        if (obj->Label.getStrValue() == obj->getNameInDocument()) return QLatin1String(objName);
    ...
    return QString::fromLatin1("%1.%2 (%3)")
        .arg(QLatin1String(objName), QString::fromUtf8(sub),
...
{
    if (!ownerDoc || links.empty()) return QString();

    auto obj = links.front().getObject();
    if (!obj) return QLatin1String("?");
    ...
    if (isLinkSub(links)) {
        ...
        return QString::fromLatin1("%1 [%2%3]")
            .arg(formatObject(ownerDoc, obj, nullptr), list.join(QLatin1String(", ")),
...
void DlgPropertyLink::init(const App::DocumentObjectT &prop, bool tryFilter)
{
    ...
    if (!allowSubObject) { ui->treeWidget->setColumnCount(1); }
    else {
    ...
    if (oldLinks.empty()) { expandDocs.insert(owner->getDocument()); }
    else {
        for (auto &link : oldLinks) {
            auto doc = link.getDocument();
            if (doc) expandDocs.insert(doc);
        }
    }
    ...
    for (auto d : docs) {
        ...
        if (expandDocs.count(d)) item->setExpanded(true);
        docItems[d] = item;
    }
    ...
    for (auto &link : oldLinks) {
        onSelectionChanged(Gui::SelectionChanges(SelectionChanges::AddSelection,
                                                 link.getDocumentName(), link.getObjectName(),
                                                 link.getSubName()));
    }
    ui->treeWidget->blockSignals(false);
    ...
    if (tryFilter && isLinkList) {
        ...
        for (const auto &link : qAsConst(oldLinks)) {
            ...
            for (; objType != App::DocumentObject::getClassTypeId();
                ...
                if (obj->isDerivedFrom(objType)) break;
            }
        }
        ...
        if (!baseType.isBad()) {
            ...
            if (it != typeItems.end()) it->second->setSelected(true);
...
void DlgPropertyLink::onClicked(QAbstractButton *button)
{
    if (button == resetButton) {
        ...
        for (auto item : subSelections) item->setText(1, QString());
        ui->treeWidget->blockSignals(false);
        subSelections.clear();
...
void DlgPropertyLink::attachObserver()
{
    ...
    if (view) view->blockSelection(true);
}
...
{
    if (isSelectionAttached()) detachSelection();
    ...
    if (view) view->blockSelection(false);
...
void DlgPropertyLink::onItemSelectionChanged()
{
    ...
    if (newSelections.isEmpty() || selections.contains(newSelections.back())) {
        ...
        if (newSelections.isEmpty()) currentObj = nullptr;
        return;
    }
    ...
    if (!TreeParams::Instance()->SyncView()) {
        ...
        if (doc) {
            ...
            if (vp) { doc->setActiveView(vp, Gui::View3DInventor::getClassTypeId()); }
        }
    }
    ...
    Gui::Selection().clearSelection();
    for (auto &sobj : sobjs)
        Gui::Selection().addSelection(sobj.getDocumentName().c_str(), sobj.getObjectName().c_str(),
    ...
    if (singleParent && currentObj && currentObj != obj) {
        ...
        for (auto item : items) {
            if (item != selections.back()) item->setSelected(false);
        }
...
QTreeWidgetItem *DlgPropertyLink::findItem(App::DocumentObject *obj, const char *subname,
                                           bool *pfound)
{
    ...
    if (itDoc == docItems.end()) return nullptr;
    onItemExpanded(itDoc->second);

    auto it = itemMap.find(obj);
    if (it == itemMap.end() || it->second->isHidden()) return nullptr;
...
{
    if (msg.Type != SelectionChanges::AddSelection) return;
    ...
    if (allowSubObject) {
        ...
        if (element.size()) {
            ...
            if (text.size()) list = text.split(QLatin1Char(','));
...
void DlgPropertyLink::accept() { QDialog::accept(); }

static QTreeWidgetItem *_getLinkFromItem(std::ostringstream &ss, QTreeWidgetItem *item,
                                         const char *objName)
{
    auto parent = item->parent();
    assert(parent);
    QByteArray nextName = parent->data(0, Qt::UserRole).toByteArray();
    if (nextName.isEmpty()) return item;
                                                        ...
                                                        bool needSubName) const
{
    ...
    QString elements;
    if (needSubName && allowSubObject) elements = item->text(1);
    ...
    for (const QString &element : split) {
        res.append(App::SubObjectT());
        res.last() = App::SubObjectT(sobj.getDocumentName().c_str(), sobj.getObjectName().c_str(),
...
void DlgPropertyLink::onTimer()
{
    ...
    if (sobjs.isEmpty()) return;
    const auto &sobj = sobjs.front();
    Gui::Selection().setPreselect(sobj.getDocumentName().c_str(), sobj.getObjectName().c_str(),
                                  sobj.getSubName().c_str(), 0, 0, 0,
...
QList<App::SubObjectT> DlgPropertyLink::currentLinks() const
{
    ...
    QList<App::SubObjectT> res;
    for (auto item : items) res.append(getLinkFromItem(item));
    return res;
}

QList<App::SubObjectT> DlgPropertyLink::originalLinks() const { return oldLinks; }

QString DlgPropertyLink::linksToPython(const QList<App::SubObjectT> &links)
{
    if (links.isEmpty()) return QLatin1String("None");
    ...
    if (isLinkSub(links)) {
        ...
        ss << '[';
        for (const auto &link : links) ss << link.getSubObjectPython(false) << ',';
        ss << ']';
    }
...
void DlgPropertyLink::filterObjects()
{
    for (int i = 0, count = ui->treeWidget->topLevelItemCount(); i < count; ++i) {
        ...
        for (int j = 0, c = item->childCount(); j < c; ++j) filterItem(item->child(j));
    }
}
...
void DlgPropertyLink::filterItem(QTreeWidgetItem *item)
{
    ...
    for (int i = 0, count = item->childCount(); i < count; ++i) filterItem(item->child(i));
}
...
void DlgPropertyLink::onItemSearch() { itemSearch(ui->searchBox->text(), true); }

void DlgPropertyLink::keyPressEvent(QKeyEvent *ev)
{
    if (ev->key() == Qt::Key_Enter || ev->key() == Qt::Key_Return) {
        if (ui->searchBox->hasFocus()) return;
    }
...
{
    if (searchItem) searchItem->setBackground(0, bgBrush);
    ...
    try {
        ...
        if (path.getPropertyName() != "_self") return;
        ...
        if (select) {
            ...
            Selection().setPreselect(obj->getDocument()->getName(), obj->getNameInDocument(),
                                     subname, 0, 0, 0, Gui::SelectionChanges::MsgSource::TreeView);
            searchItem = item;
...
QTreeWidgetItem *DlgPropertyLink::createItem(App::DocumentObject *obj, QTreeWidgetItem *parent)
{
    ...
    if (inList.find(obj) != inList.end()) return nullptr;
    ...
    if (prop) {
        ...
        if (!proxy.isNone() && !proxy.isString()) {
            ...
            if (it != typeItems.end()) proxyType = it->first;
            else if (name)
...
QTreeWidgetItem *DlgPropertyLink::createTypeItem(Base::Type type)
{
    ...
    if (!item) item = new QTreeWidgetItem(ui->typeTree);
    else
        item = new QTreeWidgetItem(item);
    item->setExpanded(true);
    item->setText(0, QString::fromLatin1(type.getName()));
    if (type == App::DocumentObject::getClassTypeId()) item->setFlags(Qt::ItemIsEnabled);
    return item;
}
...
bool DlgPropertyLink::filterType(QTreeWidgetItem *item)
{
    ...
    if (!proxyType.size()) {
        ...
        if (icon.isNull()) typeItem->setIcon(0, item->icon(0));
    }

    if (!ui->checkObjectType->isChecked() || selectedTypes.empty()) return false;

    if (proxyItem && selectedTypes.count(proxyType)) return false;

    for (auto t = type; !t.isBad() && t != App::DocumentObject::getClassTypeId();
         t = t.getParent()) {
        const char *name = t.getName();
        if (selectedTypes.count(QByteArray::fromRawData(name, strlen(name) + 1))) return false;
    }
...
void DlgPropertyLink::onItemExpanded(QTreeWidgetItem *item)
{
    ...
    if (objName.isEmpty()) {
        for (auto obj : doc->getObjects()) {
            ...
            if (newItem) itemMap[obj] = newItem;
        }
    }
        ...
        for (auto child : obj->getLinkedObject(true)->getOutList()) {
            if (!childSet.insert(child).second) continue;
            sub = child->getNameInDocument();
            sub += ".";
            if (obj->getSubObject(sub.c_str())) createItem(child, item);
        }
    }
}
...
void DlgPropertyLink::on_typeTree_itemSelectionChanged()
{
    ...
    for (auto item : items) selectedTypes.insert(item->data(0, Qt::UserRole).toByteArray());

    if (ui->checkObjectType->isChecked()) filterObjects();
}

void DlgPropertyLink::on_searchBox_textChanged(const QString &text) { itemSearch(text, false); }

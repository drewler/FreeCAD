...
static const char *const yytname[] = {
    "$end",      "error",      "$undefined", "TIDENTIFIER", "TSUB", "TSELECT", "TCOUNT",
    "TSLICE",    "TNAMESPACE", "TNUMBER",    "$accept",     "type", "subname", "count",
...
#line 50 "SelectionFilter.y"
        {
            (yyval.object) = new Node_Object((yyvsp[(1) - (3)].string), (yyvsp[(2) - (3)].string),
                                             (yyvsp[(3) - (3)].slice));
        } break;
...
yyreturn:
    if (yychar != YYEMPTY) yydestruct("Cleanup: discarding lookahead", yytoken, &yylval);
    /* Do not reclaim the symbols of the rule which action triggered

...
void SoFCHighlightAction::finish() { atexit_cleanup(); }


SoFCHighlightAction::SoFCHighlightAction(const SelectionChanges &SelCh) : SelChange(SelCh)
{
...
void SoFCHighlightAction::beginTraversal(SoNode *node) { traverse(node); }

void SoFCHighlightAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }
...
void SoFCSelectionAction::finish() { atexit_cleanup(); }


SoFCSelectionAction::SoFCSelectionAction(const SelectionChanges &SelCh) : SelChange(SelCh)
{
...
void SoFCSelectionAction::beginTraversal(SoNode *node) { traverse(node); }

void SoFCSelectionAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }
...
void SoFCEnableSelectionAction::finish() { atexit_cleanup(); }


SoFCEnableSelectionAction::SoFCEnableSelectionAction(const SbBool &sel) : selection(sel)
{
...
void SoFCEnableSelectionAction::beginTraversal(SoNode *node) { traverse(node); }
...
void SoFCEnableHighlightAction::finish() { atexit_cleanup(); }


SoFCEnableHighlightAction::SoFCEnableHighlightAction(const SbBool &sel) : highlight(sel)
{
...
void SoFCEnableHighlightAction::beginTraversal(SoNode *node) { traverse(node); }
...
void SoFCSelectionColorAction::finish() { atexit_cleanup(); }


SoFCSelectionColorAction::SoFCSelectionColorAction(const SoSFColor &col) : selectionColor(col)
{
...
void SoFCSelectionColorAction::beginTraversal(SoNode *node) { traverse(node); }
...
void SoFCHighlightColorAction::finish() { atexit_cleanup(); }


SoFCHighlightColorAction::SoFCHighlightColorAction(const SoSFColor &col) : highlightColor(col)
{
...
void SoFCHighlightColorAction::beginTraversal(SoNode *node) { traverse(node); }
...
void SoFCDocumentAction::finish() { atexit_cleanup(); }


SoFCDocumentAction::SoFCDocumentAction(const SoSFString &docName) : documentName(docName)
{
...
void SoFCDocumentAction::beginTraversal(SoNode *node) { traverse(node); }

void SoFCDocumentAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }
...
void SoFCDocumentObjectAction::finish() { atexit_cleanup(); }
...
void SoFCDocumentObjectAction::beginTraversal(SoNode *node) { traverse(node); }
...
void SoFCDocumentObjectAction::setHandled() { this->_handled = true; }

SbBool SoFCDocumentObjectAction::isHandled() const { return this->_handled; }
...
const SbViewportRegion &SoGLSelectAction::getViewportRegion() const { return this->vpselect; }
...
void SoGLSelectAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }

void SoGLSelectAction::setHandled() { this->_handled = true; }

SbBool SoGLSelectAction::isHandled() const { return this->_handled; }
...
void SoVisibleFaceAction::beginTraversal(SoNode *node) { traverse(node); }

void SoVisibleFaceAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }

void SoVisibleFaceAction::setHandled() { this->_handled = true; }

SbBool SoVisibleFaceAction::isHandled() const { return this->_handled; }
...
SoUpdateVBOAction::SoUpdateVBOAction() { SO_ACTION_CONSTRUCTOR(SoUpdateVBOAction); }

SoUpdateVBOAction::~SoUpdateVBOAction() {}

void SoUpdateVBOAction::finish() { atexit_cleanup(); }

void SoUpdateVBOAction::beginTraversal(SoNode *node) { traverse(node); }

void SoUpdateVBOAction::callDoAction(SoAction *action, SoNode *node) { node->doAction(action); }
...
public:
    SoBoxSelectionRenderActionP(SoBoxSelectionRenderAction *master)
        : master(master), searchaction(nullptr), selectsearch(nullptr), camerasearch(nullptr),
          bboxaction(nullptr), basecolor(nullptr), postprocpath(nullptr), highlightPath(nullptr),
          localRoot(nullptr), xform(nullptr), cube(nullptr), drawstyle(nullptr)
    {}
...
{
    if (!this->camerasearch) { this->camerasearch = new SoSearchAction; }
...
SoBoxSelectionRenderAction::SoBoxSelectionRenderAction() : inherited(SbViewportRegion())
{
...
SoBoxSelectionRenderAction::~SoBoxSelectionRenderAction()
{
    ...
    if (PRIVATE(this)->highlightPath) { PRIVATE(this)->highlightPath->unref(); }
...
void SoBoxSelectionRenderAction::apply(SoNode *node)
{
    ...
    if (this->hlVisible) {
        if (!PRIVATE(this)->searchaction) { PRIVATE(this)->searchaction = new SoSearchAction; }
...
void SoBoxSelectionRenderAction::apply(SoPath *path)
{
    ...
    if (node && node->getTypeId() == SoFCSelection::getClassTypeId()) {
        ...
        if (PRIVATE(this)->highlightPath == path) {
            ...
            if (!PRIVATE(this)->selectsearch) { PRIVATE(this)->selectsearch = new SoSearchAction; }
            PRIVATE(this)->selectsearch->setType(SoShape::getClassTypeId());
            PRIVATE(this)->selectsearch->setInterest(SoSearchAction::FIRST);
            PRIVATE(this)->selectsearch->apply(selection);
            SoPath *shapepath = PRIVATE(this)->selectsearch->getPath();
...
const SbColor &SoBoxSelectionRenderAction::getColor() { return PRIVATE(this)->basecolor->rgb[0]; }
...
void SoBoxSelectionRenderAction::drawBoxes(SoPath *pathtothis, const SoPathList *pathlist)
{
    ...
    for (i = 0; i < thispos; i++) PRIVATE(this)->postprocpath->append(pathtothis->getNode(i));

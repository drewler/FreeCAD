...
Flag::Flag(QWidget *parent) : QtGLWidget(parent), coord(0.0f, 0.0f, 0.0f)
{
...
void Flag::paintGL() { QOpenGLWidget::paintGL(); }
...
void Flag::setOrigin(const SbVec3f &v) { this->coord = v; }

const SbVec3f &Flag::getOrigin() const { return this->coord; }
...
void Flag::setText(const QString &t) { this->text = t; }

void Flag::resizeEvent(QResizeEvent *e) { QtGLWidget::resizeEvent(e); }
...
void Flag::mouseMoveEvent(QMouseEvent *e)
{
    if (e->buttons() & Qt::LeftButton) {
        ...
        if (viewer) viewer->getSoRenderManager()->scheduleRedraw();
    }
}
...
void Flag::contextMenuEvent(QContextMenuEvent *e)
{
    ...
    if (remove == select) this->deleteLater();
}
...
FlagLayout::FlagLayout(QWidget *parent, int margin, int spacing) : QLayout(parent)
{
    setMargin(margin);
    setSpacing(spacing);
}

FlagLayout::FlagLayout(int spacing) { setSpacing(spacing); }
...
void FlagLayout::addItem(QLayoutItem *item) { add(item, TopLeft); }
...
Qt::Orientations FlagLayout::expandingDirections() const { return Qt::Horizontal | Qt::Vertical; }

bool FlagLayout::hasHeightForWidth() const { return false; }

int FlagLayout::count() const { return list.size(); }
...
QSize FlagLayout::minimumSize() const { return calculateSize(MinimumSize); }
...
void FlagLayout::setGeometry(const QRect &rect)
{
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (position == TopLeft) {
            topHeight += spacing();
            item->setGeometry(QRect(rect.x() + spacing(), topHeight, item->sizeHint().width(),
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (position == TopRight) {
            ...
            item->setGeometry(QRect(rect.x() + rect.width() - rightpos,
                                    rect.height() - bottomHeight, item->sizeHint().width(),
                                    item->sizeHint().height()));
        }
    }
}

QSize FlagLayout::sizeHint() const { return calculateSize(SizeHint); }
...
{
    deleteFlags();
    if (_flagLayout) _flagLayout->deleteLater();
}
...
void GLFlagWindow::deleteFlags()
{
    if (_flagLayout) {
        ...
        if (ct > 0) _viewer->getSoRenderManager()->scheduleRedraw();
    }
}
...
{
    if (_flagLayout) { return _flagLayout->count(); }

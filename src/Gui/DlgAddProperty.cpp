...
DlgAddProperty::DlgAddProperty(QWidget *parent, std::unordered_set<App::PropertyContainer *> &&c)
    : QDialog(parent), containers(std::move(c)), ui(new Ui_DlgAddProperty)
{
    ui->setupUi(this);
    ...
    if (defType.isBad()) defType = App::PropertyString::getClassTypeId();
    ...
    for (const auto &type : types) {
        ...
        if (type == defType) ui->comboType->setCurrentIndex(ui->comboType->count() - 1);
    }
...
void DlgAddProperty::accept()
{
    ...
    for (auto it = containers.begin(); it != containers.end(); ++it) {
        try {
            (*it)->addDynamicProperty(type.c_str(), name.c_str(), group.c_str(),
            ...
            QMessageBox::critical(getMainWindow(), QObject::tr("Add property"),

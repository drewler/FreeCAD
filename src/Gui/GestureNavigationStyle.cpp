...
public:
    ...
    void log() const
    {
        if (isPress(1)) Base::Console().Log("button1 press ");
        if (isPress(2)) Base::Console().Log("button2 press ");
        if (isPress(3)) Base::Console().Log("button3 press ");
        if (isRelease(1)) Base::Console().Log("button1 release ");
        if (isRelease(2)) Base::Console().Log("button2 release ");
        if (isRelease(3)) Base::Console().Log("button3 release ");
        if (isMouseButtonEvent()) Base::Console().Log("%x", modifiers);
        if (isGestureEvent()) {
            Base::Console().Log("Gesture ");
            switch (asGestureEvent()->state) {
                case SoGestureEvent::SbGSStart: Base::Console().Log("start "); break;
                case SoGestureEvent::SbGSEnd: Base::Console().Log("end "); break;
                case SoGestureEvent::SbGSUpdate: Base::Console().Log("data "); break;
                default: Base::Console().Log("??? ");
            }
    ...
    {
        if (!isGestureEvent()) return false;
...
public:
    ...
    explicit IdleState(my_context ctx) : my_base(ctx)
    {
        ...
        if (ns.logging) Base::Console().Log(" -> IdleState\n");
    }
    virtual ~IdleState() {}
    ...
    sc::result react(const NS::Event &ev)
    {
        ...
        switch (ns.getViewingMode()) {
            case NavigationStyle::SEEK_WAIT_MODE: {
                if (ev.isPress(1)) {
                    ns.seekToPoint(
                        ev.inventor_event->getPosition()); // implicitly calls interactiveCountInc()
            ...
            case BOXZOOM: return forward_event();
        }
        ...
        if (ev.isKeyboardEvent()) {
            ...
            switch (kbev->getKey()) {
                case SoKeyboardEvent::H:
                    if (!press) ns.onSetRotationCenter(kbev->getPosition());
                    break;
                case SoKeyboardEvent::PAGE_UP:
                    if (!press) {
                        ns.doZoom(ns.viewer->getSoRenderManager()->getCamera(), ns.getDelta(),
                                  posn);
                    }
                    break;
                case SoKeyboardEvent::PAGE_DOWN:
                    ...
                    break;
                default: ev.flags->processed = false;
            }
        }
...
public:
    ...
    {
        auto &ns = this->outermost_context().ns;
        if (ns.logging) Base::Console().Log(" -> AwaitingMoveState\n");
        ns.setViewingMode(NavigationStyle::IDLE);
        this->base_pos =
        ...
        if (this->hold_timeout == 0) this->hold_timeout = 650; //a fail-safe
    ...
    sc::result react(const NS::Event &ev)
    {
        ...
        if (ev.mbstate() == 0x101) {
            if (ev.isPress(1)) ns.rollDir = -1;
            if (ev.isPress(2)) ns.rollDir = +1;
        }
        ...
        if (ev.isMouseButtonEvent() /* and still not processed*/) { ns.postponedEvents.post(ev); }
                ...
                switch (ev.mbstate()) {
                    case 0x100: {
                        if (!long_click) {
                            ...
                            if (allowSpin) return transit<NS::RotateState>();
                            else {
                                refire();
                                return transit<NS::IdleState>();
                            }
                    ...
                    case 0x001: return transit<NS::PanState>(); break;
                    case (0x101): return transit<NS::TiltState>(); break;
                    default:
...
public:
    ...
    sc::result react(const NS::Event &ev)
    {
        if (ev.isMouseButtonEvent()) {
            ev.flags->processed = true;
            if (ev.mbstate() == 0x101) { return transit<NS::TiltState>(); }
            if (ev.mbstate() == 0) { return transit<NS::IdleState>(); }
        }
        if (ev.isLocation2Event()) {
            ev.flags->processed = true;
...
public:
    ...
    sc::result react(const NS::Event &ev)
    {
        ...
        if (ev.isLocation2Event()) {
            ...
            SbVec2s pos = ev.inventor_event->getPosition();
            auto &ns = this->outermost_context().ns;
            ns.panCamera(ns.viewer->getSoRenderManager()->getCamera(), this->ratio, ns.panningplane,
                         ns.normalizePixelPos(pos), ns.normalizePixelPos(this->base_pos));
            this->base_pos = pos;
        }
        return forward_event();
...
public:
    ...
    virtual ~StickyPanState()
    {
        auto &ns = this->outermost_context().ns;
        ns.button2down =
            false; //a workaround for dealing with Qt not sending UP event after a tap-hold-drag sequence.
    }
    ...
    sc::result react(const NS::Event &ev)
    {
        if (ev.isMouseButtonEvent()) {
            ev.flags->processed = true;
            if (ev.isRelease(1)) { return transit<NS::IdleState>(); }
        }
        if (ev.isLocation2Event()) {
            ev.flags->processed = true;
            SbVec2s pos = ev.inventor_event->getPosition();
...
public:
    ...
    sc::result react(const NS::Event &ev)
    {
        if (ev.isMouseButtonEvent()) {
            ev.flags->processed = true;
            if (ev.mbstate() == 0x001) { return transit<NS::PanState>(); }
            if (ev.mbstate() == 0x100) { return transit<NS::RotateState>(); }
            if (ev.mbstate() == 0) { return transit<NS::IdleState>(); }
        }
        if (ev.isLocation2Event()) {
            ev.flags->processed = true;
    ...
    sc::result react(const NS::Event &ev)
    {
        ...
        if (ev.isGestureEvent()) {
            ...
            if (ev.asGestureEvent()->state == SoGestureEvent::SbGSEnd) {
                ...
                ns.panCamera(ns.viewer->getSoRenderManager()->getCamera(), ratio, ns.panningplane,
                             panDist, SbVec2f(0, 0));
            }
                ...
                ns.doZoom(ns.viewer->getSoRenderManager()->getCamera(),
                          -logf(float(pinch->deltaZoom)), ns.normalizePixelPos(pinch->curCenter));
                if (pinch->deltaAngle != 0.0 && enableTilt) {
                    ns.doRotate(ns.viewer->getSoRenderManager()->getCamera(),
                                float(pinch->deltaAngle), ns.normalizePixelPos(pinch->curCenter));
                }
            }
            else {
                //unknown gesture
...
public:
    ...
    {
        auto &ns = this->outermost_context().ns;
        if (ns.logging) Base::Console().Log(" -> AwaitingReleaseState\n");
    }
    ...
    sc::result react(const NS::Event &ev)
    {
        ...
        if (ev.isLocation2Event()) { ev.flags->processed = true; }
        if (ev.isGestureActive()) {
            ev.flags->processed = true;
...
GestureNavigationStyle::GestureNavigationStyle()
    : naviMachine(new NS::NaviMachine(*this)), postponedEvents(*this)
{
...
{
    switch (mode) {
        case NavigationStyle::SELECTION: return QT_TR_NOOP("Tap OR click left mouse button.");
        ...
        case NavigationStyle::ZOOMING:
            return QT_TR_NOOP(
                "Pinch (place two fingers on the screen && drag them apart from || towards each "
...
SbBool GestureNavigationStyle::processSoEvent(const SoEvent *const ev)
{
    ...
    if (smev.isGestureEvent()) return superclass::processSoEvent(ev);
#endif


    if (!smev.flags->processed) this->naviMachine->processEvent(smev);
    if (!smev.flags->propagated && !smev.flags->processed) return superclass::processSoEvent(ev);
    else
...
bool GestureNavigationStyle::isDraggerUnderCursor(SbVec2s pos)
{
    ...
    if (pick) {
        ...
        for (int i = 0; i < fullpath->getLength(); ++i) {
            if (fullpath->getNode(i)->isOfType(SoDragger::getClassTypeId())) return true;
        }
        return false;
    }
    else {
...
void GestureNavigationStyle::onSetRotationCenter(SbVec2s cursor)
{
    ...
    if (!ret) {
        ...
        Base::Console().Log("No object under cursor! Can't set new center of rotation.\n");
    }
}

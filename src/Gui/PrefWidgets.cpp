 ...
 */
PrefWidget::PrefWidget() : WindowParameter("") {}
...
{
    if (getWindowParameter().isValid()) getWindowParameter()->Detach(this);
}

/** Sets the preference name to \a name. */
void PrefWidget::setEntryName(const QByteArray &name) { m_sPrefName = name; }

/** Sets the preference name to \a name. */
void PrefWidget::setPrefEntry(const QByteArray &name) { setEntryName(name); }

/** Returns the widget's preference name. */
QByteArray PrefWidget::entryName() const { return m_sPrefName; }
...
void PrefWidget::setParamGrpPath(const QByteArray &path)
{
    ...
    if (getWindowParameter().isValid()) {
        if (paramGrpPath() != path) Base::Console().Warning("Widget already attached\n");
    }
#endif
...
/** Sets the preference path to \a path. */
void PrefWidget::setPrefPath(const QByteArray &name) { setParamGrpPath(name); }

/** Returns the widget's preferences path. */
QByteArray PrefWidget::paramGrpPath() const { return m_sPrefGrp; }
...
{
    Q_UNUSED(rCaller);
    if (std::strcmp(sReason, m_sPrefName) == 0) restorePreferences();
}
...
{
    savePreferences();
    if (getWindowParameter().isValid()) getWindowParameter()->Notify(entryName());
#ifdef FC_DEBUG
    else
...
{
#ifdef FC_DEBUG
    if (getWindowParameter().isNull()) qWarning("No parameter group specified!");
#endif
    restorePreferences();
}
...
void PrefWidget::failedToSave(const QString &name) const
{
    ...
    if (objname.isEmpty()) objname = "Undefined";
...
PrefSpinBox::PrefSpinBox(QWidget *parent) : QSpinBox(parent), PrefWidget() {}
...
PrefDoubleSpinBox::PrefDoubleSpinBox(QWidget *parent) : QDoubleSpinBox(parent), PrefWidget() {}
...
PrefLineEdit::PrefLineEdit(QWidget *parent) : QLineEdit(parent), PrefWidget() {}
...
PrefTextEdit::PrefTextEdit(QWidget *parent) : QTextEdit(parent), PrefWidget() {}
...
PrefFileChooser::PrefFileChooser(QWidget *parent) : FileChooser(parent), PrefWidget() {}
...
void PrefFileChooser::restorePreferences()
{
    ...
    QString txt =
        QString::fromUtf8(getWindowParameter()->GetASCII(entryName(), fileName().toUtf8()).c_str());
    setFileName(txt);
}
...
PrefComboBox::PrefComboBox(QWidget *parent) : QComboBox(parent), PrefWidget() {}
...
PrefCheckBox::PrefCheckBox(QWidget *parent) : QCheckBox(parent), PrefWidget() {}
...
PrefRadioButton::PrefRadioButton(QWidget *parent) : QRadioButton(parent), PrefWidget() {}
...
PrefSlider::PrefSlider(QWidget *parent) : QSlider(parent), PrefWidget() {}
...
PrefColorButton::PrefColorButton(QWidget *parent) : ColorButton(parent), PrefWidget() {}
...
PrefUnitSpinBox::PrefUnitSpinBox(QWidget *parent) : QuantitySpinBox(parent), PrefWidget() {}
...
public:
    const QStringList &asStringList() const { return list; }
    int maximumSize() const { return max_size; }
    void setMaximumSize(int num)
    {
        max_size = num;
        while (list.size() > num) list.pop_front();
    }
    void clear() { list.clear(); }
    void append(const QString &value)
    {
        if (!list.isEmpty() && list.back() == value) return;
...
public:
    ...
    QByteArray getHistoryGroupName(QByteArray name) const { return name + "_History"; }

    void restoreHistory(ParameterGrp::handle hGrp)
    {
        std::vector<std::string> hist = hGrp->GetASCIIs("Hist");
        for (const auto &it : hist) history.append(QString::fromStdString(it));
    }
    void clearHistory(ParameterGrp::handle hGrp)
    {
        std::vector<std::string> hist = hGrp->GetASCIIs("Hist");
        for (const auto &it : hist) hGrp->RemoveASCII(it.c_str());
    }
...
void PrefQuantitySpinBox::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (userAction == saveValueAction) { pushToHistory(this->text()); }
    else if (userAction == clearListAction) {
        d->history.clear();
    }
    else if (userAction) {
        QVariant prop = userAction->property("history_value");
        if (prop.isValid()) { lineEdit()->setText(prop.toString()); }
    }
}
...
void PrefQuantitySpinBox::setToLastUsedValue()
{
    ...
    if (!hist.empty()) lineEdit()->setText(hist.front());
}
...
PrefFontBox::PrefFontBox(QWidget *parent) : QFontComboBox(parent), PrefWidget() {}

...
template<class GraphEW> class Edge_writer
{
public:
    explicit Edge_writer(const GraphEW &graphEWIn) : graphEW(graphEWIn) {}
    template<class EdgeW> void operator()(std::ostream &out, const EdgeW & /*edgeW*/) const
    {
        out << "[label=\"";
        out << "edge";
        out << "\"]";
...
template<class GraphVW> class Vertex_writer
{
public:
    explicit Vertex_writer(const GraphVW &graphVWIn) : graphVW(graphVWIn) {}
    template<class VertexW> void operator()(std::ostream &out, const VertexW &vertexW) const
    {
        out << "[label=\"";
...
template<class GraphIn> void outputGraphviz(const GraphIn &graphIn, const std::string &filePath)
{
...
template<class GraphIn> class RakeLeaves
{
...
public:
    ...
    GraphInVertices operator()() const
    {
        ...
        {
            if (boost::out_degree(currentVertex, graph) == 0) out.push_back(currentVertex);
        }
        return out;
    }
...
template<class GraphIn> class DigRoots
{
    using GraphInVertex = boost::graph_traits<Graph>::vertex_descriptor;
    using GraphInVertices = std::vector<GraphInVertex>;
...
public:
    ...
    GraphInVertices operator()() const
    {
        ...
        BGL_FORALL_VERTICES_T(currentVertex, graph, GraphIn)
        {
            if (boost::in_degree(currentVertex, graph) == 0) out.push_back(currentVertex);
        }
        return out;
    }
...
public:
    ...
    template<typename TVertex, typename TGraph> void discover_vertex(TVertex vertex, TGraph &graph)
    {
        Q_UNUSED(graph);
...
using GraphLinkContainer = boost::multi_index_container<
    ...
    BMI::indexed_by<
        ...
        BMI::ordered_unique<BMI::tag<GraphLinkRecord::ByVPDObject>,
                            BMI::member<GraphLinkRecord, const Gui::ViewProviderDocumentObject *,

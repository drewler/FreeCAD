...
FilterBase::FilterBase()
    : name(QString::fromLatin1("empty name")), enabled(true), type(Type::Exclusion)
{}

FilterOrigin::FilterOrigin() : FilterBase() { name = QObject::tr("Origin"); }

bool FilterOrigin::goFilter(const Vertex &vertexIn, const Graph &graphIn,
                            const GraphLinkContainer &linkIn) const
{
    ...
    for (boost::tie(it, itEnd) = boost::in_edges(vertexIn, graphIn); it != itEnd; ++it) {
        ...
        if ((sourceRecord.DObject->getTypeId() == originType)
...
bool FilterTyped::goFilter(const Gui::DAG::Vertex &vertexIn, const Graph &graphIn,
                           const GraphLinkContainer &linkIn) const
{
    Q_UNUSED(graphIn);
    if (type.empty()) return false;
    Base::Type theType = Base::Type::fromName(type.c_str());
    if (theType == Base::Type::badType()) return false;

    const GraphLinkRecord &sourceRecord = findRecord(vertexIn, linkIn);
    if (sourceRecord.DObject->getTypeId() == theType) return true;

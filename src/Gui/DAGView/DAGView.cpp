...
View::View(QWidget *parentIn) : QGraphicsView(parentIn)
{
    ...
    conActive = Application::Instance->signalActiveDocument.connect(
        boost::bind(&View::slotActiveDocument, this, bp::_1));
    conDelete = Application::Instance->signalDeleteDocument.connect(
        boost::bind(&View::slotDeleteDocument, this, bp::_1));
...
{
    if (Gui::Selection().hasSelection()) return;
...
{
    ModelMap::const_iterator it = modelMap.find(&documentIn);
    if (it != modelMap.end()) modelMap.erase(it);
}

void View::awakeSlot()
{
    Model *model = dynamic_cast<Model *>(this->scene());
    if (model) model->awake();
}
...
void View::onSelectionChanged(const SelectionChanges &msg)
{
    switch (msg.Type) {
        ...
        case SelectionChanges::SetSelection:
            if (!msg.pDocName || !msg.pDocName[0]) return;
            break;
        case SelectionChanges::ClrSelection:
            if (!msg.pDocName || !msg.pDocName[0]) {
                for (auto &v : modelMap) { v.second->selectionChanged(msg); }
                return;
            }
            break;
        default: return;
    ...
    if (!model) model = std::make_shared<Model>(this, *doc);

...
Model::Model(QObject *parentIn, const Gui::Document &documentIn) : QGraphicsScene(parentIn)
{
    ...
    connect(this->editingFinishedAction, SIGNAL(triggered()), this, SLOT(editingFinishedSlot()));

    connectNewObject =
        documentIn.signalNewObject.connect(boost::bind(&Model::slotNewObject, this, bp::_1));
    connectDelObject =
        documentIn.signalDeletedObject.connect(boost::bind(&Model::slotDeleteObject, this, bp::_1));
    connectChgObject = documentIn.signalChangedObject.connect(
        boost::bind(&Model::slotChangeObject, this, bp::_1, bp::_2));
    connectEdtObject =
        documentIn.signalInEdit.connect(boost::bind(&Model::slotInEdit, this, bp::_1));
    connectResObject =
        documentIn.signalResetEdit.connect(boost::bind(&Model::slotResetEdit, this, bp::_1));

    for (auto obj : documentIn.getDocument()->getObjects()) {
        auto vpd = Base::freecad_dynamic_cast<Gui::ViewProviderDocumentObject>(
            documentIn.getViewProvider(obj));
        if (vpd) slotNewObject(*vpd);
    }
}

Model::~Model()
{
    if (connectNewObject.connected()) connectNewObject.disconnect();
    if (connectDelObject.connected()) connectDelObject.disconnect();
    if (connectChgObject.connected()) connectChgObject.disconnect();
    if (connectEdtObject.connected()) connectEdtObject.disconnect();
    if (connectResObject.connected()) connectResObject.disconnect();
...
void Model::setupViewConstants()
{
    ...
    if (direction != -1.0 && direction != 1.0) direction = 1.0;
    ...
    forgroundBrushes = {QBrush(Qt::red), QBrush(Qt::darkRed), QBrush(Qt::green),
                        QBrush(Qt::darkGreen), QBrush(Qt::blue), QBrush(Qt::darkBlue),
                        QBrush(Qt::cyan), QBrush(Qt::darkCyan), QBrush(Qt::magenta),
                        QBrush(Qt::darkMagenta),
                        //     QBrush(Qt::yellow), can't read
                        QBrush(Qt::darkYellow), QBrush(Qt::gray), QBrush(Qt::darkGray),
                        QBrush(Qt::lightGray)}; //reserve some of the these for highlight stuff.
}
...
void Model::slotChangeIcon(const ViewProviderDocumentObject &VPDObjectIn,
...
void Model::slotDeleteObject(const ViewProviderDocumentObject &VPDObjectIn)
{
    ...
    if (vertex == lastAddedVertex) lastAddedVertex = Graph::null_vertex();
...
void Model::slotChangeObject(const ViewProviderDocumentObject &VPDObjectIn,
                             const App::Property &propertyIn)
{
    std::string name("Empty Name");
    if (propertyIn.hasName()) name = propertyIn.getName();
    assert(!name.empty());
...
void Model::updateSlot()
{
    ...
    BGL_FORALL_VERTICES(currentVertex, *theGraph, Graph)
    {
        ...
        for (auto &currentOtherDObject : otherDObjects) {
            if (!hasRecord(currentOtherDObject, *graphLink)) continue;
    ...
    BGL_FORALL_VERTICES(currentVertex, *theGraph, Graph)
    {
        if ((*theGraph)[currentVertex].dagVisible
            && (!(*theGraph)[currentVertex].rectangle->scene()))
    ...
    for (const auto &currentVertex : sorted) {
        if (!(*theGraph)[currentVertex].dagVisible) continue;

        if (boost::out_degree(currentVertex, *theGraph) == 0) currentColumn = 0;
        else {
            ...
            for (; it != itEnd; ++it) {
                ...
                if (currentParentIndex < farthestParentIndex) {
                    ...
                    Path::const_iterator end =
                        sorted.begin() + (*theGraph)[currentVertex].topoSortIndex; // 1 before
            ...
            for (const auto &currentParent : parentVertices) {
                if (((*theGraph)[currentParent].column & columnMask).none()) {
                    ...
                    if (!(*theGraph)[currentParent].dagVisible) continue;
                    destinationColumn =
                        static_cast<int>(columnFromMask((*theGraph)[currentParent].column));
                    break;
                }
            }
        ...
        rectangle->setRect(-rowPadding, 0.0, rowPadding,
                           rowHeight); //calculate actual length later.
        ...
        point->setTransform(
            QTransform::fromTranslate(pointSpacing * currentColumn,
                                      rowHeight * currentRow + rowHeight / 2.0 - pointSize / 2.0));
        point->setBrush(currentBrush);

        float cheat = 0.0;
        if (direction == -1) cheat = rowHeight;

        auto visiblePixmap = (*theGraph)[currentVertex].visibleIcon.get();
        visiblePixmap->setTransform(QTransform::fromTranslate(
            0.0, rowHeight * currentRow + cheat)); //calculate x location later.

        auto statePixmap = (*theGraph)[currentVertex].stateIcon.get();
        statePixmap->setTransform(QTransform::fromTranslate(
            0.0, rowHeight * currentRow + cheat)); //calculate x location later.

        auto pixmap = (*theGraph)[currentVertex].icon.get();
        pixmap->setTransform(QTransform::fromTranslate(
            0.0, rowHeight * currentRow + cheat)); //calculate x location later.
        ...
        text->setTransform(QTransform::fromTranslate(0.0,
                                                     rowHeight * currentRow - verticalSpacing * 2.0
                                                         + cheat)); //calculate x location later.
        (*theGraph)[currentVertex].lastVisibleState =
            VisibilityState::None; //force visual update for color.
        ...
        for (; it != itEnd; ++it) {
            ...
            float dependentX =
                pointSpacing * static_cast<int>(columnFromMask((*theGraph)[target].column))
            ...
            if (currentColumn == static_cast<int>(columnFromMask((*theGraph)[target].column)))
                ...
                float yPosition;
                if (direction == -1.0) yPosition = dependentY - 2.0 * radius;
                else
...
void Model::updateStates()
{
    ...
    BGL_FORALL_VERTICES(currentVertex, *theGraph, Graph)
    {
        ...
        VisibilityState currentVisibilityState =
            (record.VPDObject->isShow()) ? (VisibilityState::On) : (VisibilityState::Off);
        if ((currentVisibilityState != (*theGraph)[currentVertex].lastVisibleState)
            ...
            if (record.VPDObject->isShow()) visiblePixmap->setPixmap(visiblePixmapEnabled);
            else
        ...
        if (record.DObject->isError()) currentFeatureState = FeatureState::Fail;
...
void Model::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (rect == currentPrehighlight) return;
...
void Model::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    auto toggleSelect = [](const App::DocumentObject *dObjectIn, RectItem *rectIn) {
        if (rectIn->isSelected())
            Gui::Selection().rmvSelection(dObjectIn->getDocument()->getName(),
                                          dObjectIn->getNameInDocument());
        else
            Gui::Selection().addSelection(dObjectIn->getDocument()->getName(),
                                          dObjectIn->getNameInDocument());
    };

    if (proxy) renameAcceptedSlot();
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (rect) {
            ...
            if (selectionMode == SelectionMode::Single) {
                if (event->modifiers() & Qt::ControlModifier) { toggleSelect(dObject, rect); }
                else if ((event->modifiers() & Qt::ShiftModifier) && lastPickValid) {
                    goShiftSelect();
                }
                else {
                    Gui::Selection().clearSelection(dObject->getDocument()->getName());
                    Gui::Selection().addSelection(dObject->getDocument()->getName(),
                                                  dObject->getNameInDocument());
                }
            }
            if (selectionMode == SelectionMode::Multiple) {
                if ((event->modifiers() & Qt::ShiftModifier) && lastPickValid) { goShiftSelect(); }
                else {
...
std::vector<Gui::DAG::Vertex> Model::getAllSelected()
{
    ...
    BGL_FORALL_VERTICES(currentVertex, *theGraph, Graph)
    {
        if ((*theGraph)[currentVertex].rectangle->isSelected()) out.push_back(currentVertex);
    }
...
void Model::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    if (rect) {
        ...
        if (!rect->isSelected()) {
            ...
            Gui::Selection().addSelection(record.DObject->getDocument()->getName(),
        ...
        if (contextMenu.actions().count() > 0) contextMenu.exec(event->screenPos());
    }
...
void Model::renameRejectedSlot() { finishRename(); }
...
void Model::visiblyIsolate(Gui::DAG::Vertex sourceIn)
{
    ...
    auto testSkipType = [](const App::DocumentObject *dObject,
                           const std::vector<Base::Type> &types) {
        for (const auto &currentType : types) {
            if (dObject->isDerivedFrom(currentType)) return true;
        }
        return false;
    };
    ...
    boost::breadth_first_search(boost::make_reverse_graph(*theGraph), sourceIn,
    ...
    for (const auto &currentVertex : connectedVertices) {
        const GraphLinkRecord &record = findRecord(currentVertex, *graphLink);
        if (testSkipType(record.DObject, skipTypes)) continue;
        const_cast<ViewProviderDocumentObject *>(record.VPDObject)->hide(); //const hack
    }

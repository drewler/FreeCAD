...
bool ViewProviderGroupExtension::extensionCanDragObjects() const { return true; }
...
bool ViewProviderGroupExtension::extensionCanDropObjects() const { return true; }
...
bool ViewProviderGroupExtension::extensionCanDropObject(App::DocumentObject *obj) const
{
    ...
    if (group->hasObject(obj)) return false;

    if (group->allowObject(obj)) return true;
...
void ViewProviderGroupExtension::extensionDropObject(App::DocumentObject *obj)
{
    ...
    QString cmd;
    cmd =
        QString::fromLatin1("App.getDocument(\"%1\").getObject(\"%2\").addObject("
                            "App.getDocument(\"%1\").getObject(\"%3\"))")
            .arg(QString::fromLatin1(doc->getName()), QString::fromLatin1(grp->getNameInDocument()),
...
void ViewProviderGroupExtension::extensionShow()
{
    ...
    if (!getExtendedViewProvider()->isRestoring()) {
        auto *group =
            getExtendedViewProvider()->getObject()->getExtensionByType<App::GroupExtension>();
        for (auto obj : group->Group.getValues()) {
            if (obj && !obj->Visibility.getValue()) obj->Visibility.setValue(true);
        }
    }
...
void ViewProviderGroupExtension::extensionHide()
{
    ...
    if (!getExtendedViewProvider()->isRestoring()
        ...
        auto *group =
            getExtendedViewProvider()->getObject()->getExtensionByType<App::GroupExtension>();
        for (auto obj : group->Group.getValues()) {
            if (obj && obj->Visibility.getValue()) obj->Visibility.setValue(false);
        }
    }
...
bool ViewProviderGroupExtension::extensionOnDelete(const std::vector<std::string> &)
{
    ...
    if (group->Group.getSize() > 0) {
        QMessageBox::StandardButton choice = QMessageBox::question(
            getMainWindow(), QObject::tr("Delete group content?"),
...
namespace Gui
{
EXTENSION_PROPERTY_SOURCE_TEMPLATE(Gui::ViewProviderGroupExtensionPython,

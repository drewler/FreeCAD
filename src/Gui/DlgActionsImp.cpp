...
DlgCustomActionsImp::DlgCustomActionsImp(QWidget *parent)
    : CustomizeActionPage(parent), ui(new Ui_DlgCustomActions)
{
    ui->setupUi(this);
...
bool DlgCustomActionsImp::event(QEvent *e)
{
    ...
    if (e->type() == QEvent::ParentChange || e->type() == QEvent::ParentAboutToChange) {
        ...
        while (topLevel && !topLevel->inherits("QDialog")) topLevel = topLevel->parentWidget();
        if (topLevel) {
            int index = topLevel->metaObject()->indexOfSignal(
                QMetaObject::normalizedSignature("addMacroAction(const QByteArray&)"));
            if (index >= 0) {
                if (e->type() == QEvent::ParentChange) {
                    connect(this, SIGNAL(addMacroAction(const QByteArray &)), topLevel,
                            SIGNAL(addMacroAction(const QByteArray &)));
                    connect(this, SIGNAL(removeMacroAction(const QByteArray &)), topLevel,
                            SIGNAL(removeMacroAction(const QByteArray &)));
                    connect(this, SIGNAL(modifyMacroAction(const QByteArray &)), topLevel,
                            SIGNAL(modifyMacroAction(const QByteArray &)));
                }
                else {
                    disconnect(this, SIGNAL(addMacroAction(const QByteArray &)), topLevel,
                               SIGNAL(addMacroAction(const QByteArray &)));
                    disconnect(this, SIGNAL(removeMacroAction(const QByteArray &)), topLevel,
                               SIGNAL(removeMacroAction(const QByteArray &)));
                    disconnect(this, SIGNAL(modifyMacroAction(const QByteArray &)), topLevel,
                               SIGNAL(modifyMacroAction(const QByteArray &)));
                }
            }
        }
...
void DlgCustomActionsImp::on_buttonAddAction_clicked()
{
    ...
    auto macro = new MacroCommand(
        actionName, ui->actionMacros->itemData(ui->actionMacros->currentIndex()).toBool());
...
#else
    if (ui->pixmapLabel->pixmap()) item->setIcon(0, *ui->pixmapLabel->pixmap());
#endif
    ...
    if (!ui->actionMenu->text().isEmpty()) macro->setMenuText(ui->actionMenu->text().toUtf8());
    ui->actionMenu->clear();
    ...
    if (!ui->actionStatus->text().isEmpty()) macro->setStatusTip(ui->actionStatus->text().toUtf8());
    ui->actionStatus->clear();

    if (!m_sPixmap.isEmpty()) macro->setPixmap(m_sPixmap.toLatin1());
    ui->pixmapLabel->clear();
    m_sPixmap.clear();

    if (!ui->actionAccel->text().isEmpty()) { macro->setAccel(ui->actionAccel->text().toLatin1()); }
...
void DlgCustomActionsImp::on_buttonReplaceAction_clicked()
{
    ...
    if (action) {
        ...
        if (macro->getPixmap()) action->setIcon(Gui::BitmapFactory().pixmap(macro->getPixmap()));
    ...
    if (macro->getPixmap()) item->setIcon(0, Gui::BitmapFactory().pixmap(macro->getPixmap()));
}
...
IconDialog::IconDialog(QWidget *parent) : QDialog(parent), ui(new Ui_DlgChooseIcon)
{
    ui->setupUi(this);
    ui->listWidget->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    // signals and slots connections
    connect(ui->listWidget, SIGNAL(itemClicked(QListWidgetItem *)), this, SLOT(accept()));
    connect(ui->addButton, SIGNAL(clicked()), this, SLOT(onAddIconPath()));
...
IconDialog::~IconDialog() { delete ui; }

QListWidgetItem *IconDialog::currentItem() const { return ui->listWidget->currentItem(); }

void IconDialog::resizeEvent(QResizeEvent *) { ui->listWidget->setFlow(QListView::LeftToRight); }
...
void IconDialog::onAddIconPath()
{
    ...
    for (const auto &path : paths) pathList << QString::fromUtf8(path.c_str());
    ...
    if (dlg.exec()) {
        ...
        for (auto &it : search) { it = QDir::toNativeSeparators(it); }
    ...
    : QDialog(parent), restart(false), maxLines(10)
{
    ...
    for (int row = 0; row < maxRow; row++) {
        ...
        if (row < numPaths) { edit->setText(paths[row]); }
        else {
            edit->hide();
            removeButton->hide();
        }
    ...
    if (numPaths >= this->maxLines) addButton->setDisabled(true);
}
...
void IconFolders::addFolder()
{
        ...
        if (it.first->isHidden()) {
            ...
            if (countHidden == 0) {
                QString dir = QFileDialog::getExistingDirectory(
                    this, IconDialog::tr("Add icon folder"), QString());
    ...
    if (countHidden <= 0) { addButton->setDisabled(true); }
}
...
void IconFolders::removeFolder()
{
    ...
    for (const auto &it : buttonMap) {
        if (it.second == remove) { edit = it.first; }
        else if (edit) {
...
QStringList IconFolders::getPaths() const
{
    QStringList paths;
    for (const auto &it : buttonMap) {
        if (!it.first->isHidden()) { paths << QDir::toNativeSeparators(it.first->text()); }
        else {
            break;
        }
    }

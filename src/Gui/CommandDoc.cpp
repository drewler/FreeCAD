...
StdCmdOpen::StdCmdOpen() : Command("Std_Open")
{
    // setting the
    sGroup = "File";
...
void StdCmdOpen::activated(int iMsg)
{
    ...
    QStringList fileList = FileDialog::getOpenFileNames(
        getMainWindow(), QObject::tr("Open document"), QString(), formatList, &selectedFilter);
    if (fileList.isEmpty()) return;

    // load the files with the associated modules
    SelectModule::Dict dict = SelectModule::importHandler(fileList, selectedFilter);
    if (dict.isEmpty()) {
        QMessageBox::critical(getMainWindow(), qApp->translate("StdCmdOpen", "Cannot open file"),
        ...
        for (SelectModule::Dict::iterator it = dict.begin(); it != dict.end(); ++it) {
            ...
            if (doc && doc->testStatus(App::Document::PartialRestore))
                QMessageBox::critical(
                    getMainWindow(), QObject::tr("Error"),
...
StdCmdImport::StdCmdImport() : Command("Std_Import")
{
    // setting the
    sGroup = "File";
...
void StdCmdImport::activated(int iMsg)
{
    ...
    QStringList fileList = FileDialog::getOpenFileNames(getMainWindow(), QObject::tr("Import file"),
                                                        QString(), formatList, &selectedFilter);
    if (!fileList.isEmpty()) {
        ...
        if (emptyDoc) {
            ...
            for (const auto &view : views) { view->viewAll(); }
        }
    }
}

bool StdCmdImport::isActive() { return (getActiveGuiDocument() ? true : false); }
...
QString createDefaultExportBasename()
{
    ...
    for (int i = 0; i < exportFormatString.size(); ++i) {
        ...
        if (c != QLatin1Char('%')) {
            ...
            if (i < exportFormatString.size() - 1) {
                ...
                if (formatChar == QLatin1Char('F')) { defaultFilename.append(fcstdBasename); }
    ...
    for (const auto &c : invalidCharacters) defaultFilename.replace(c, QLatin1String("_"));
...
void StdCmdExport::activated(int iMsg)
{
    ...
    if (selection.empty()) {
        QMessageBox::warning(
            Gui::getMainWindow(), QString::fromUtf8(QT_TR_NOOP("No selection")),
    ...
    if (!lastExportFilterUsed.isEmpty()) selectedFilter = lastExportFilterUsed;
    ...
    QString fileName = FileDialog::getSaveFileName(getMainWindow(), QObject::tr("Export file"),
                                                   defaultFilename, formatList, &selectedFilter);
    if (!fileName.isEmpty()) {
...
bool StdCmdExport::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdMergeProjects::StdCmdMergeProjects() : Command("Std_MergeProjects")
{
    sAppModule = "File";
    sGroup = "File";
...
void StdCmdMergeProjects::activated(int iMsg)
{
    ...
    QString project = FileDialog::getOpenFileName(
        Gui::getMainWindow(), QString::fromUtf8(QT_TR_NOOP("Merge project")),
...
bool StdCmdMergeProjects::isActive() { return this->hasActiveDocument(); }
...
StdCmdDependencyGraph::StdCmdDependencyGraph() : Command("Std_DependencyGraph")
{
    // setting the
    sGroup = "Tools";
...
bool StdCmdDependencyGraph::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdNew::StdCmdNew() : Command("Std_New")
{
    sGroup = "File";
...
void StdCmdNew::activated(int iMsg)
{
    ...
    QString cmd;
    cmd =
        QString::fromLatin1("App.newDocument(\"%1\")").arg(qApp->translate("StdCmdNew", "Unnamed"));
...
StdCmdSave::StdCmdSave() : Command("Std_Save")
{
    sGroup = "File";
...
bool StdCmdSave::isActive() { return getGuiApplication()->sendHasMsgToActiveView("Save"); }
...
StdCmdSaveAs::StdCmdSaveAs() : Command("Std_SaveAs")
{
    sGroup = "File";
...
bool StdCmdSaveAs::isActive() { return getGuiApplication()->sendHasMsgToActiveView("SaveAs"); }
...
StdCmdSaveCopy::StdCmdSaveCopy() : Command("Std_SaveCopy")
{
    sGroup = "File";
...
bool StdCmdSaveCopy::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdSaveAll::StdCmdSaveAll() : Command("Std_SaveAll")
{
    sGroup = "File";
...
bool StdCmdSaveAll::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdRevert::StdCmdRevert() : Command("Std_Revert")
{
    sGroup = "File";
...
void StdCmdRevert::activated(int iMsg)
{
    ...
    int ret = msgBox.exec();
    if (ret == QMessageBox::Yes) doCommand(Command::App, "App.ActiveDocument.restore()");
}

bool StdCmdRevert::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdProjectInfo::StdCmdProjectInfo() : Command("Std_ProjectInfo")
{
    // setting the
    sGroup = "File";
...
bool StdCmdProjectInfo::isActive() { return (getActiveGuiDocument() ? true : false); }
...
StdCmdProjectUtil::StdCmdProjectUtil() : Command("Std_ProjectUtil")
{
    // setting the
    sGroup = "Tools";
...
bool StdCmdProjectUtil::isActive() { return true; }
...
StdCmdPrint::StdCmdPrint() : Command("Std_Print")
{
    sGroup = "File";
...
bool StdCmdPrint::isActive() { return getGuiApplication()->sendHasMsgToActiveView("Print"); }
...
StdCmdPrintPreview::StdCmdPrintPreview() : Command("Std_PrintPreview")
{
    sGroup = "File";
...
{
    Q_UNUSED(iMsg);
    if (getMainWindow()->activeWindow()) { getMainWindow()->activeWindow()->printPreview(); }
}
...
StdCmdPrintPdf::StdCmdPrintPdf() : Command("Std_PrintPdf")
{
    sGroup = "File";
...
bool StdCmdPrintPdf::isActive() { return getGuiApplication()->sendHasMsgToActiveView("PrintPdf"); }
...
StdCmdQuit::StdCmdQuit() : Command("Std_Quit")
{
    sGroup = "File";
...
StdCmdUndo::StdCmdUndo() : Command("Std_Undo")
{
    sGroup = "Edit";
...
bool StdCmdUndo::isActive() { return getGuiApplication()->sendHasMsgToActiveView("Undo"); }
...
Action *StdCmdUndo::createAction()
{
    ...
    if (getPixmap()) pcAction->setIcon(Gui::BitmapFactory().iconFromTheme(getPixmap()));
...
StdCmdRedo::StdCmdRedo() : Command("Std_Redo")
{
    sGroup = "Edit";
...
bool StdCmdRedo::isActive() { return getGuiApplication()->sendHasMsgToActiveView("Redo"); }
...
StdCmdCut::StdCmdCut() : Command("Std_Cut")
{
    sGroup = "Edit";
...
bool StdCmdCut::isActive() { return getGuiApplication()->sendHasMsgToActiveView("Cut"); }
...
StdCmdCopy::StdCmdCopy() : Command("Std_Copy")
{
    sGroup = "Edit";
...
{
    if (getGuiApplication()->sendHasMsgToFocusView("Copy")) return true;
...
StdCmdPaste::StdCmdPaste() : Command("Std_Paste")
{
    sGroup = "Edit";
...
{
    if (getGuiApplication()->sendHasMsgToFocusView("Paste")) return true;
    QClipboard *cb = QApplication::clipboard();
...
StdCmdDuplicateSelection::StdCmdDuplicateSelection() : Command("Std_DuplicateSelection")
{
    sAppModule = "Edit";
    sGroup = "Edit";
...
void StdCmdDuplicateSelection::activated(int iMsg)
{
    ...
    Base::FileInfo fi(App::Application::getTempFileName());
    {
        ...
        if (!unsaved.empty()) {
            QMessageBox::critical(
                getMainWindow(), QObject::tr("Unsaved document"),
    ...
    if (doc) {
        ...
        if (hasXLink && !doc->isSaved()) {
            int ret = QMessageBox::question(
                getMainWindow(), qApp->translate("Std_DuplicateSelection", "Object dependencies"),
                qApp->translate(
...
bool StdCmdDuplicateSelection::isActive() { return Gui::Selection().hasSelection(); }
...
StdCmdSelectAll::StdCmdSelectAll() : Command("Std_SelectAll")
{
    sGroup = "Edit";
...
bool StdCmdSelectAll::isActive() { return App::GetApplication().getActiveDocument() != nullptr; }
...
StdCmdDelete::StdCmdDelete() : Command("Std_Delete")
{
    sGroup = "Edit";
...
void StdCmdDelete::activated(int iMsg)
{
    ...
    try {
        ...
        if (editDoc) vpedit = dynamic_cast<ViewProviderDocumentObject *>(editDoc->getInEdit());
        if (vpedit) {
            ...
            for (auto it = docs.begin(); it != docs.end(); ++it) {
                ...
                if (itd != outList.end()) {
                    for (auto doc : itd->second) {
                        if (doc != *it) docs.erase(doc);
                    }
                }
            }
            for (auto doc : docs) { FCMD_DOC_CMD(doc, "recompute()"); }
        }
    }
...
bool StdCmdDelete::isActive() { return !Selection().getCompleteSelection().empty(); }
...
StdCmdRefresh::StdCmdRefresh() : Command("Std_Refresh")
{
    sGroup = "Edit";
    ...
    if (!create) eType = eType | NoTransaction;
}
    ...
    if (getActiveGuiDocument()) {
        ...
        try {
            ...
            int ret = QMessageBox::warning(
                getMainWindow(), QObject::tr("Dependency error"),
...
bool StdCmdRefresh::isActive() { return this->getDocument() && this->getDocument()->mustExecute(); }
...
StdCmdTransform::StdCmdTransform() : Command("Std_Transform")
{
    sGroup = "Edit";
...
bool StdCmdTransform::isActive() { return (Gui::Control().activeDialog() == nullptr); }
...
StdCmdPlacement::StdCmdPlacement() : Command("Std_Placement")
{
    sGroup = "Edit";
...
StdCmdTransformManip::StdCmdTransformManip() : Command("Std_TransformManip")
{
    sGroup = "Edit";
    sMenuText = QT_TR_NOOP("Transform");
...
void StdCmdTransformManip::activated(int iMsg)
{
    ...
    if (vp) getActiveGuiDocument()->setEdit(vp, Gui::ViewProvider::Transform);
}
...
StdCmdAlignment::StdCmdAlignment() : Command("Std_Alignment")
{
    sGroup = "Edit";
...
{
    if (ManualAlignment::hasInstance()) return false;
...
StdCmdEdit::StdCmdEdit() : Command("Std_Edit")
{
    sGroup = "Edit";
...
public:
    StdCmdExpression()
        : Command("Std_Expressions"), pcActionCopyAll(nullptr), pcActionCopySel(nullptr),
          pcActionCopyActive(nullptr), pcActionPaste(nullptr)
    {
        sGroup = "Edit";
        ...
        try {
            for (auto &v : exprs) {
                for (auto &v2 : v.second) {
                    ...
                    if (!expressions.empty()) v2.first->setExpressions(std::move(expressions));
                }
            }
            commitCommand();
        }

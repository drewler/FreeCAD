...
*/
void Gui::GuiNativeEvent::tdx_drv_handler(io_connect_t connection, natural_t messageType,
                ...
                switch (msg->command) {
                    ...
                    case kConnexionCmdHandleButtons: {
                        ...
                        for (uint8_t bt = 0; bt < 32; bt++) {
                            if (pressedButtons & 1) mainApp->postButtonEvent(bt, 1);
                            pressedButtons = pressedButtons >> 1;
                        }
                        for (uint8_t bt = 0; bt < 32; bt++) {
                            if (releasedButtons & 1) mainApp->postButtonEvent(bt, 0);
...
Gui::GuiNativeEvent::~GuiNativeEvent()
{
    ...
    if (InstallConnexionHandlers) {
        ...
        if (tdxClientID) UnregisterConnexionClient(tdxClientID);
...
void Gui::GuiNativeEvent::initSpaceball(QMainWindow *window)
{
    ...
    tdxClientID = RegisterConnexionClient(tdxAppID, tdxAppName, kConnexionClientModeTakeOver,
                                          kConnexionMaskAll);
    if (tdxClientID == 0) {

...
static const e3dmouse_virtual_key SpaceMouseWirelessKeys[] = {V3DK_INVALID // there is no button 0
                                                              ,
                                                              V3DK_1,
                                                              V3DK_2,
                                                              V3DK_TOP,
...
static const e3dmouse_virtual_key SpaceExplorerKeys[] = {V3DK_INVALID // there is no button 0
                                                         ,
                                                         V3DK_1,
                                                         V3DK_2,
                                                         V3DK_TOP,
...
*/
I3dMouseParam &Gui::GuiNativeEvent::MouseParams() { return f3dMouseParams; }

/*!
	Access the mouse parameters structure
*/
const I3dMouseParam &Gui::GuiNativeEvent::MouseParams() const { return f3dMouseParams; }
...
static PRAWINPUTDEVICE GetDevicesToRegister(unsigned int *pNumDevices)
{
    ...
    if (pNumDevices) { *pNumDevices = sizeof(sRawInputDevices) / sizeof(sRawInputDevices[0]); }
...
bool Gui::GuiNativeEvent::InitializeRawInput(HWND hwndTarget)
{
    ...
    for (size_t i = 0; i < numDevices; i++) {
        ...
        if (osvi.dwMajorVersion >= 6) { devicesToRegister[i].dwFlags |= RIDEV_DEVNOTIFY; }
    }
...
UINT Gui::GuiNativeEvent::GetRawInputBuffer(PRAWINPUT pData, PUINT pcbSize, UINT cbSizeHeader)
{
    ...
    if (!bIsWow64 || pData == NULL) { return ::GetRawInputBuffer(pData, pcbSize, cbSizeHeader); }
    else {
        ...
        while (PeekMessage(&msg, hwndTarget, WM_INPUT, WM_INPUT, PM_NOREMOVE)) {
            ...
            if (::GetRawInputData(hRawInput, RID_INPUT, pri, &cbSize, cbSizeHeader)
                == static_cast<UINT>(-1)) {
                if (nCount == 0) { return static_cast<UINT>(-1); }
                else {
                    break;
                }
            }
...
void Gui::GuiNativeEvent::On3dmouseInput()
{
    ...
    if (0 == fLast3dmouseInputTime) {
        ...
        if (fLast3dmouseInputTime > dwNow) { dwElapsedTime = ~dwElapsedTime + 1; }
        if (dwElapsedTime < 1) { dwElapsedTime = 1; }
    ...
    while (iterator != fDevice2Data.end()) {
        ...
        if ((--(iterator->second.fTimeToLive)) == 0) { iterator->second.fAxes.assign(6, .0); }
        ...
        if (iterator->second.IsZero()) { iterator = fDevice2Data.erase(iterator); }
        else {
            ++iterator;
        }
        ...
        if (iterator != fDevice2Data.end()) { hNextDevice = iterator->first; }
    ...
    if (!fDevice2Data.empty()) { fLast3dmouseInputTime = dwNow; }
    else {
        fLast3dmouseInputTime = 0;
    }
}
...
void Gui::GuiNativeEvent::OnRawInput(UINT nInputCode, HRAWINPUT hRawInput)
{
    ...
    if (nCount == (UINT)-1) { qDebug("GetRawInputBuffer returned error %d\n", GetLastError()); }
    ...
    if (b3dmouseInput) { On3dmouseInput(); }
}
    ...
    if (::GetRawInputDeviceInfo(pRawInput->header.hDevice, RIDI_DEVICEINFO, &sRidDeviceInfo,
        ...
        switch (sRidDeviceInfo.dwType) {
            ...
            case RIM_TYPEHID:
                ...
                qDebug("\tVendor=0x%x\n\tProduct=0x%x\n\tUsagePage=0x%x\n\tUsage=0x%x\n",
                       sRidDeviceInfo.hid.dwVendorId, sRidDeviceInfo.hid.dwProductId,
                       sRidDeviceInfo.hid.usUsagePage, sRidDeviceInfo.hid.usUsage);
                break;
        }
#endif
                                                        ...
                                                        DWORD dwProductId)
{
    ...
    if (pRawInput->data.hid.bRawData[0] == 0x01) { // Translation vector
        ...
        if (dwKeystate != 0) { fDevice2Keystate[pRawInput->header.hDevice] = dwKeystate; }
        else {
                                                        ...
                                                        DWORD dwProductId)
{
    ...
    if (pRawInput->data.hid.bRawData[0] == 0x01) { // Translation vector
        ...
        if (dwKeyCode != 0) { fDevice2Keystate[pRawInput->header.hDevice] = dwKeyCode; }
        else {
            fDevice2Keystate.erase(pRawInput->header.hDevice);
        }

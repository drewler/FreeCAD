...
LocationWidget::LocationWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(dValue, SIGNAL(activated(int)), this, SLOT(on_direction_activated(int)));
    retranslateUi();
}
...
{
    return Base::Vector3d(this->xValue->value().getValue(), this->yValue->value().getValue(),
...
LocationDialog::LocationDialog(QWidget *parent, Qt::WindowFlags fl) : QDialog(parent, fl) {}
...
void LocationDialog::on_direction_activated(int index) { directionActivated(index); }
...
Base::Vector3d LocationDialogUiImp::getDirection() const { return ui->getDirection(); }

Base::Vector3d LocationDialogUiImp::getPosition() const { return ui->getPosition(); }
...
void LocationDialogUiImp::directionActivated(int index) { ui->directionActivated(this, index); }

...
int LinkViewPy::PyInit(PyObject * /*args*/, PyObject * /*kwd*/) { return 0; }


// returns a string which represent the object e.g. when printed in python
std::string LinkViewPy::representation() const { return "<Link view>"; }
    ...
    PY_TRY
    {
        ...
        if (PyDict_Check(pyObj)) {
            ...
            while (PyDict_Next(pyObj, &pos, &key, &value)) {
                Py::Int idx(key);
                if (value == Py_None) materials[(int)idx] = nullptr;
            ...
            for (auto &v : materials) lv->setMaterial(v.first, v.second);
            Py_Return;
        }
        if (PySequence_Check(pyObj)) {
            ...
            for (size_t i = 0; i < materials.size(); ++i) lv->setMaterial(i, materials[i]);
            Py_Return;
        }
...
PyObject *LinkViewPy::setTransform(PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        if (PyDict_Check(pyObj)) {
            ...
            for (auto &v : mat) lv->setTransform(v.first, *v.second);
            Py_Return;
        }
        if (PySequence_Check(pyObj)) {
            Py::Sequence seq(pyObj);
            ...
            for (size_t i = 0; i < mat.size(); ++i) lv->setTransform(i, *mat[i]);
            Py_Return;
        }

        PyErr_SetString(PyExc_TypeError,
                        "exepcting a type of Matrix, [Matrix,...] or {Int:Matrix,...}");
        return nullptr;
    }
    PY_CATCH;
}
...
PyObject *LinkViewPy::setType(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "h|O!", &type, &PyBool_Type, &sublink)) return nullptr;
...
PyObject *LinkViewPy::setChildren(PyObject *args)
{
    ...
    short type = 0;
    if (!PyArg_ParseTuple(args, "O|Os", &pyObj, &pyVis, &type)) return nullptr;

    PY_TRY
    {
        App::PropertyBoolList vis;
        App::PropertyLinkList links;
        if (pyObj != Py_None) links.setPyObject(pyObj);
        if (pyVis != Py_None) vis.setPyObject(pyVis);
        getLinkViewPtr()->setChildren(links.getValue(), vis.getValue(),
...
PyObject *LinkViewPy::setLink(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O|O", &pyObj, &pySubName)) return nullptr;
    ...
    PY_TRY
    {
        ...
        if (pySubName != Py_None) prop.setPyObject(pySubName);

        if (obj) getLinkViewPtr()->setLink(obj, prop.getValue());
        else
...
void LinkViewPy::setOwner(Py::Object owner)
{
    ...
    if (!owner.isNone()) {
        ...
        vp = static_cast<ViewProviderDocumentObjectPy *>(owner.ptr())
                 ->getViewProviderDocumentObjectPtr();
    }
...
Py::Object LinkViewPy::getSubNames() const
{
    ...
    if (subs.empty()) return Py::Object();
    Py::Tuple ret(subs.size());
    int i = 0;
    for (auto &s : subs) ret.setItem(i++, Py::String(s.c_str()));
    return ret;
}
...
PyObject *LinkViewPy::getElementPicked(PyObject *args)
{
    ...
    if (!pp) throw Py::TypeError("type must be of coin.SoPickedPoint");
    PY_TRY
    {
        std::string name;
        if (!getLinkViewPtr()->linkGetElementPicked(pp, name)) Py_Return;
...
PyObject *LinkViewPy::getDetailPath(PyObject *args)
{
    ...
    if (!pPath) throw Py::TypeError("type must be of coin.SoPath");
    PY_TRY
    {
...
PyObject *LinkViewPy::getBoundBox(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O", &vobj)) return nullptr;
    ViewProviderDocumentObject *vpd = nullptr;
    if (vobj != Py_None) {
...
PyObject *LinkViewPy::getCustomAttributes(const char *) const { return nullptr; }
...
Py::Object LinkViewPy::getRootNode() const
{
    try {
        ...
        PyObject *Ptr =
            Base::Interpreter().createSWIGPointerObj("pivy.coin", "SoSeparator *", node, 1);
        node->ref();
...
Py::Object LinkViewPy::getVisibilities() const
{
    ...
    if (!linked->getSize()) return Py::Object();
...
void LinkViewPy::setVisibilities(Py::Object value)
{
    ...
    if (!value.isNone()) v.setPyObject(value.ptr());
...
PyObject *LinkViewPy::getChildren(PyObject *args)
{
    ...
    if (children.empty()) Py_Return;
    Py::Tuple ret(children.size());
    int i = 0;
    for (auto vp : children) ret.setItem(i++, Py::Object(vp->getPyObject(), true));
    return Py::new_reference_to(ret);
}

Py::Int LinkViewPy::getCount() const { return Py::Int(getLinkViewPtr()->getSize()); }

...
std::string WorkbenchPy::representation() const { return std::string("<Workbench object>"); }
...
PyObject *WorkbenchPy::listMenus(PyObject *args)
{
    PY_TRY
    {
        ...
        Py::List list;
        for (const auto &menu : menus) { list.append(Py::String(menu)); }
...
PyObject *WorkbenchPy::listToolbars(PyObject *args)
{
    PY_TRY
    {
        ...
        Py::List list;
        for (const auto &bar : bars) { list.append(Py::String(bar)); }
        return Py::new_reference_to(list);
    }
    PY_CATCH;
}
...
PyObject *WorkbenchPy::getToolbarItems(PyObject *args)
{
    PY_TRY
    {
        ...
        for (const auto &it : bars) {
            Py::List list;
            for (const auto &jt : it.second) { list.append(Py::String(jt)); }
...
PyObject *WorkbenchPy::getCustomAttributes(const char *) const { return nullptr; }

int WorkbenchPy::setCustomAttributes(const char *, PyObject *) { return 0; }

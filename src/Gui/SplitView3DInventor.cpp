...
AbstractSplitView::AbstractSplitView(Gui::Document *pcDocument, QWidget *parent,
...
bool AbstractSplitView::containsViewProvider(const ViewProvider *vp) const
{
    for (auto it = _viewer.begin(); it != _viewer.end(); ++it) {
        if ((*it)->containsViewProvider(vp)) return true;
    }
...
void AbstractSplitView::OnChange(ParameterGrp::SubjectType &rCaller,
                                 ParameterGrp::MessageType Reason)
{
    ...
    if (strcmp(Reason, "HeadlightColor") == 0) {
        ...
        for (std::vector<View3DInventorViewer *>::iterator it = _viewer.begin();
             it != _viewer.end(); ++it)
        ...
        unsigned long col3 =
            rGrp.GetUnsigned("BackgroundColor3", 3570714879UL); // default color (light yellow/blue)
        unsigned long col4 =
...
void AbstractSplitView::onUpdate() { update(); }

const char *AbstractSplitView::getName() const { return "SplitView3DInventor"; }
...
{
    if (strcmp("ViewFit", pMsg) == 0) { return true; }
    else if (strcmp("ViewBottom", pMsg) == 0) {
        return true;
    }
    else if (strcmp("ViewFront", pMsg) == 0) {
...
{
    if (!_viewerPy) _viewerPy = new AbstractSplitViewPy(this);
    Py_INCREF(_viewerPy);
    return _viewerPy;
}
...
int AbstractSplitView::getSize() { return static_cast<int>(_viewer.size()); }
...
void AbstractSplitViewPy::init_type()
{
    ...
    add_varargs_method("cast_to_base", &AbstractSplitViewPy::cast_to_base,
                       "cast_to_base() cast to MDIView class");
    behaviors().readyType();
}

AbstractSplitViewPy::AbstractSplitViewPy(AbstractSplitView *vi) : base(vi) {}
...
Py::Object AbstractSplitViewPy::repr()
{
    ...
    if (!getSplitViewPtr()) throw Py::RuntimeError("Cannot print representation of deleted object");
...
AbstractSplitView *AbstractSplitViewPy::getSplitViewPtr()
{
    ...
    if (!(view && view->getViewer(0))) throw Py::RuntimeError("Object already deleted");
    return view;
}
...
{
    int viewIndex;
    if (!PyArg_ParseTuple(args.ptr(), "i", &viewIndex)) throw Py::Exception();

    try {
        Gui::View3DInventorViewer *view = getSplitViewPtr()->getViewer(viewIndex);
        if (!view) throw Py::IndexError("Index out of range");
...
Py::Object AbstractSplitViewPy::sequence_item(Py_ssize_t viewIndex)
{
    ...
    if (viewIndex >= view->getSize() || viewIndex < 0) throw Py::IndexError("Index out of range");
...
Py::Object AbstractSplitViewPy::close(const Py::Tuple &args)
{
    ...
    AbstractSplitView *view = getSplitViewPtr();
    view->close();
    if (view->parentWidget()) view->parentWidget()->deleteLater();
...
SplitView3DInventor::SplitView3DInventor(int views, Gui::Document *pcDocument, QWidget *parent,
                                         Qt::WindowFlags wflags)
    : AbstractSplitView(pcDocument, parent, wflags)
{
    ...
    if (views <= 3) {
        ...
        for (int i = 0; i < views; i++) {
            if (glformat) _viewer.push_back(new View3DInventorViewer(f, mainSplitter));
            else
        ...
        for (int i = 2; i < views; i++) {
            if (glformat) _viewer.push_back(new View3DInventorViewer(f, botSplitter));
            else

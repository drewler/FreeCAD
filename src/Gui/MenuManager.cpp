...
{
    if (item) item->appendItem(this);
}

MenuItem::~MenuItem() { clear(); }

void MenuItem::setCommand(const std::string &name) { _name = name; }

std::string MenuItem::command() const { return _name; }

bool MenuItem::hasItems() const { return _items.count() > 0; }
...
{
    for (QList<MenuItem *>::Iterator it = _items.begin(); it != _items.end(); ++it) {
        if ((*it)->_name == name) return this;
    }

    for (QList<MenuItem *>::Iterator it = _items.begin(); it != _items.end(); ++it) {
        if ((*it)->findParentOf(name)) return *it;
    }
...
uint MenuItem::count() const { return _items.count(); }

void MenuItem::appendItem(MenuItem *item) { _items.push_back(item); }
...
MenuItem *MenuItem::afterItem(MenuItem *item) const
{
    ...
    if (pos < 0 || pos + 1 == _items.size()) return nullptr;
...
{
    int pos = _items.indexOf(item);
    if (pos != -1) _items.removeAt(pos);
}

void MenuItem::clear()
{
    for (QList<MenuItem *>::Iterator it = _items.begin(); it != _items.end(); ++it) delete *it;
    _items.clear();
}
...
QList<MenuItem *> MenuItem::getItems() const { return _items; }
...
{
    if (!_instance) _instance = new MenuManager;
    return _instance;
}
...
{
    if (!menuItems) return; // empty menu bar
    ...
    for (QList<MenuItem *>::Iterator it = items.begin(); it != items.end(); ++it) {
        ...
        // flll up the menu
        if (!action->isSeparator()) setup(*it, action->menu());
    }
...
void MenuManager::setup(MenuItem *item, QMenu *menu) const
{
    ...
    for (QList<MenuItem *>::Iterator it = items.begin(); it != items.end(); ++it) {
        ...
        QList<QAction *> used_actions =
            findActions(actions, QString::fromLatin1((*it)->command().c_str()));
        if (used_actions.isEmpty()) {
            if ((*it)->command() == "Separator") {
                ...
                if ((*it)->hasItems()) {
                    ...
                    std::string menuName = (*it)->command();
                    QMenu *submenu =
                        menu->addMenu(QApplication::translate("Workbench", menuName.c_str()));
            ...
            for (QList<QAction *>::Iterator it = used_actions.begin(); it != used_actions.end();
                 ++it) {
        ...
        // fill up the submenu
        if ((*it)->hasItems()) setup(*it, used_actions.front()->menu());
    }
...
void MenuManager::retranslate() const
{
    ...
    QList<QAction *> actions = menuBar->actions();
    for (QList<QAction *>::Iterator it = actions.begin(); it != actions.end(); ++it) {
        if ((*it)->menu()) retranslate((*it)->menu());
    }
}
...
void MenuManager::retranslate(QMenu *menu) const
{
    ...
    if (cmd) { menu->setTitle(QApplication::translate(cmd->className(), cmd->getMenuText())); }
    else {
...
QAction *MenuManager::findAction(const QList<QAction *> &acts, const QString &item) const
{
    for (QList<QAction *>::ConstIterator it = acts.begin(); it != acts.end(); ++it) {
        if ((*it)->data().toString() == item) return *it;
    }
...
QList<QAction *> MenuManager::findActions(const QList<QAction *> &acts, const QString &item) const
{
    ...
    for (QList<QAction *>::ConstIterator it = acts.begin(); it != acts.end(); ++it) {
        if ((*it)->data().toString() == item) {
            ...
            if (item == QLatin1String("Separator")) break;
        }
        else if (first_match)
            break;
    }
...
void MenuManager::setupContextMenu(MenuItem *item, QMenu &menu) const { setup(item, &menu); }

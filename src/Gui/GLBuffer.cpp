...
OpenGLBuffer::OpenGLBuffer(GLenum type)
    : target(type), bufferId(0), context(-1), currentContext(-1), glue(nullptr)
{
...
bool OpenGLBuffer::isVBOSupported(uint32_t ctx)
{
    ...
    if (!glue || !cc_glglue_has_vertex_buffer_object(glue)) return false;
...
bool OpenGLBuffer::create()
{
    ...
    if (!cc_glglue_has_vertex_buffer_object(glue)) return false;
...
bool OpenGLBuffer::isCreated() const { return (bufferId > 0); }
...
{
    if (bufferId > 0) { cc_glglue_glBufferData(glue, target, count, data, GL_STATIC_DRAW); }
}
...
{
    if (bufferId) { cc_glglue_glBindBuffer(glue, target, 0); }
}

GLuint OpenGLBuffer::getBufferId() const { return bufferId; }

uint32_t OpenGLBuffer::getBoundContext() const { return context; }

int OpenGLBuffer::size() const
{
    GLint value = -1;
    if (bufferId > 0) { cc_glglue_glGetBufferParameteriv(glue, target, GL_BUFFER_SIZE, &value); }
    return value;
}
...
OpenGLMultiBuffer::OpenGLMultiBuffer(GLenum type)
    : target(type), currentBuf(nullptr), currentContext(-1), glue(nullptr)
{
    SoContextHandler::addContextDestructionCallback(context_destruction_cb, this);
}
...
{
    if (currentBuf && *currentBuf) { cc_glglue_glBindBuffer(glue, target, 0); }
}

GLuint OpenGLMultiBuffer::getBufferId() const { return currentBuf ? *currentBuf : 0; }
...
void OpenGLMultiBuffer::context_destruction_cb(uint32_t context, void *userdata)
{
    ...
    if (it != self->bufs.end() && it->second) {
        ...
        cc_glglue_glDeleteBuffers(glue, 1, &buffer);
        if (self->currentBuf == &it->second) self->currentBuf = nullptr;
        self->bufs.erase(it);
    }
}

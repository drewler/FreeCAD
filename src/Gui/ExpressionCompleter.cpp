...
public:
    ...
    void setNoProperty(bool enabled) { noProperty = enabled; }
    ...
    void setDocumentObject(const App::DocumentObject *obj, bool checkInList)
    {
        ...
        if (obj) {
            ...
            if (!noProperty && checkInList) inList = obj->getInListEx(true);
        }
        else {
            currentDoc.clear();
            currentObj.clear();
    ...
    {
        if (sizeof(void *) >= sizeof(info)) return info.ptr;
    ...
    static Info getInfo(const QModelIndex &index)
    {
        ...
        if (sizeof(void *) >= sizeof(Info)) return info;
        Info res;
    ...
    void _data(const Info &info, int row, QVariant *v, int *count, bool sep = false) const
    {
        ...
        if (info.d.doc < 0) {
            if (count) *count = docSize + objSize + propSize;
            if (idx >= 0 && v) {
                QString res;
                if (propName) res = QString::fromLatin1(propName);
                else if (obj) {
                    if (idx & 1) res = QString::fromUtf8(quote(obj->Label.getStrValue()).c_str());
                    else
                        res = QString::fromLatin1(obj->getNameInDocument());
                    if (sep && !noProperty) res += QLatin1Char('.');
                }
                else {
                    if (idx & 1) res = QString::fromUtf8(quote(doc->Label.getStrValue()).c_str());
                    else
                        res = QString::fromLatin1(doc->getName());
                    if (sep) res += QLatin1Char('#');
                }
                v->setValue(res);
            }
            return;
        ...
        if (!obj) {
            ...
            const auto &objs = doc->getObjects();
            objSize = (int)objs.size() * 2;
            if (idx < 0 || idx >= objSize || inList.count(obj)) return;
            obj = objs[idx / 2];
            if (info.d.obj < 0) {
                if (count) *count = objSize;
                if (v) {
                    QString res;
                    if (idx & 1) res = QString::fromUtf8(quote(obj->Label.getStrValue()).c_str());
                    else
        ...
        if (noProperty) return;
        if (!propName) {
            idx = row;
            obj->getPropertyNamedList(props);
            propSize = (int)props.size();
            if (idx < 0 || idx >= propSize) return;
            propName = props[idx].first;
            if (count) *count = propSize;
        }
        if (v) *v = QString::fromLatin1(propName);
        return;
    }
    ...
    QModelIndex index(int row, int column, const QModelIndex &parent = QModelIndex()) const override
    {
        ...
        if (!parent.isValid()) {
            ...
            info = getInfo(parent);
            if (info.d.doc <= 0) info.d.doc = parent.row();
...
ExpressionCompleter::ExpressionCompleter(const App::DocumentObject *currentDocObj, QObject *parent,
                                         bool noProperty, bool checkInList)
...
{
    if (!obj || !obj->getNameInDocument()) currentObj = App::DocumentObjectT();
    else
        currentObj = obj;
    setCompletionPrefix(QString());
    checkInList = _checkInList;
    auto m = model();
    if (m) static_cast<ExpressionCompleterModel *>(m)->setDocumentObject(obj, checkInList);
}
...
void ExpressionCompleter::setNoProperty(bool enabled)
{
    noProperty = enabled;
    auto m = model();
    if (m) static_cast<ExpressionCompleterModel *>(m)->setNoProperty(enabled);
}

QString ExpressionCompleter::pathFromIndex(const QModelIndex &index) const
{
    auto m = model();
    if (!m || !index.isValid()) return QString();
...
QStringList ExpressionCompleter::splitPath(const QString &input) const
{
    ...
    while (1) {
        try {
            App::ObjectIdentifier p = ObjectIdentifier::parse(currentObj.getObject(), path);

            std::vector<std::string> sl = p.getStringList();
            auto sli = sl.begin();
            if (retry && !sl.empty()) sl.pop_back();
...
void ExpressionCompleter::slotUpdate(const QString &prefix, int pos)
{
    ...
    auto tokenizeExpression = [](const QString &expr) {
        ...
        std::transform(result.cbegin(), result.cend(), std::back_inserter(tokens),
                       [](const std::tuple<int, int, std::string> &item) {
                           return std::make_tuple(get<0>(item), get<1>(item),
    ...
    // No tokens
    if (tokens.empty()) {
        if (auto p = popup()) p->setVisible(false);
        return;
    }
    ...
    for (auto it = tokens.begin(); it != tokens.end(); ++it) {
        if (get<1>(*it) >= pos) {
            ...
            if (it != tokens.begin() && get<0>(*it) != '.' && get<0>(*it) != '#') it = it - 1;
    ...
    int trim = 0;
    if (prefixEnd > pos) trim = prefixEnd - pos;
    ...
    if (i == static_cast<long>(tokens.size())) prefixStart = prefixEnd;
    else
    ...
    if (!completionPrefix.isEmpty() && widget()->hasFocus()) complete();
    else {
        if (auto p = popup()) p->setVisible(false);
    }
}
...
ExpressionLineEdit::ExpressionLineEdit(QWidget *parent, bool noProperty, char checkPrefix,
                                       bool checkInList)
    : QLineEdit(parent), completer(nullptr), block(true), noProperty(noProperty), exactMatch(false),
      checkInList(checkInList), checkPrefix(checkPrefix)
{
    connect(this, SIGNAL(textEdited(const QString &)), this,
            SLOT(slotTextChanged(const QString &)));
}

void ExpressionLineEdit::setPrefix(char prefix) { checkPrefix = prefix; }
                                           ...
                                           bool _checkInList)
{
    ...
    if (currentDocObj) {
        ...
        if (!exactMatch) completer->setFilterMode(Qt::MatchContains);
        ...
        connect(this, SIGNAL(textChanged2(QString, int)), completer,
                SLOT(slotUpdate(QString, int)));
    }
}
...
{
    noProperty = enabled;
    if (completer) completer->setNoProperty(enabled);
}

void ExpressionLineEdit::setExactMatch(bool enabled)
{
    exactMatch = enabled;
    if (completer) completer->setFilterMode(exactMatch ? Qt::MatchStartsWith : Qt::MatchContains);
}
...
{
    if (completer && completer->popup()) completer->popup()->setVisible(false);
}

void ExpressionLineEdit::slotTextChanged(const QString &text)
{
    if (!block) {
        if (!text.size() || (checkPrefix && text[0] != QLatin1Char(checkPrefix))) return;
...
void ExpressionLineEdit::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (completer) {
        if (action == match) setExactMatch(match->isChecked());
    }
...
ExpressionTextEdit::ExpressionTextEdit(QWidget *parent)
    : QPlainTextEdit(parent), completer(nullptr), block(true), exactMatch(false)
{

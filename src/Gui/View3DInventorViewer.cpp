...
static unsigned char rotate_bitmap[ROTATE_BYTES] = {
    0xf0, 0xef, 0x18, 0xb8, 0x0c, 0x90, 0xe4, 0x83, 0x34, 0x86, 0x1c, 0x83, 0x00, 0x81, 0x00, 0xff,
    0xff, 0x00, 0x81, 0x00, 0xc1, 0x38, 0x61, 0x2c, 0xc1, 0x27, 0x09, 0x30, 0x1d, 0x18, 0xf7, 0x0f};

static unsigned char rotate_mask_bitmap[ROTATE_BYTES] = {
    0xf0, 0xef, 0xf8, 0xff, 0xfc, 0xff, 0xfc, 0xff, 0x3c, 0xfe, 0x1c, 0xff, 0x00, 0xff, 0x00, 0xff,
    0xff, 0x00, 0xff, 0x00, 0xff, 0x38, 0x7f, 0x3c, 0xff, 0x3f, 0xff, 0x3f, 0xff, 0x1f, 0xf7, 0x0f};
...
public:
    ...
    bool eventFilter(QObject *obj, QEvent *event) override
    {
        ...
        if (event->type() == QEvent::Wheel) {
            ...
            if (qAbs(we->angleDelta().x()) > qAbs(we->angleDelta().y())) return true;
        }
        ...
        if (Base::Sequencer().isRunning() && Base::Sequencer().isBlocking()) return false;
...
View3DInventorViewer::View3DInventorViewer(QWidget *parent, const QtGLWidget *sharewidget)
    : Quarter::SoQTQuarterAdaptor(parent, sharewidget),
      SelectionObserver(false, ResolveMode::NoResolve), editViewProvider(nullptr),
      navigation(nullptr), renderType(Native), framebuffer(nullptr), axisCross(nullptr),
      axisGroup(nullptr), editing(false), redirected(false), allowredir(false),
      overrideMode("As Is"), _viewerPy(nullptr)
{
    init();
}

View3DInventorViewer::View3DInventorViewer(const QtGLFormat &format, QWidget *parent,
...
void View3DInventorViewer::init()
{
    ...
    if (!isSeekValuePercentage()) setSeekValueAsPercentage(true);
    ...
    connect(this, &View3DInventorViewer::devicePixelRatioChanged, this,
...
View3DInventorViewer::~View3DInventorViewer()
{
    ...
    if (restoreEditingRoot) { resetEditingRoot(false); }
    ...
    if (this->pcClipPlane) this->pcClipPlane->unref();

    delete this->navigation;

    // Note: When closing the application the main window doesn't exist any more.
    if (getMainWindow()) getMainWindow()->setPaneText(2, QLatin1String(""));
...
void View3DInventorViewer::setDocument(Gui::Document *pcDocument)
{
    ...
    if (pcDocument) {
        ...
        for (auto &sel : sels) {
            SelectionChanges Chng(SelectionChanges::ShowSelection, sel.DocName, sel.FeatName,
                                  sel.SubName);
...
Document *View3DInventorViewer::getDocument() { return guiDocument; }
...
void View3DInventorViewer::checkGroupOnTop(const SelectionChanges &Reason)
{
    if (Reason.Type == SelectionChanges::SetSelection
        ...
        clearGroupOnTop();
        if (Reason.Type == SelectionChanges::ClrSelection) return;
    }
    ...
    if (!getDocument() || !Reason.pDocName || !Reason.pDocName[0] || !Reason.pObjectName) return;
    ...
    if (Reason.Type == SelectionChanges::RmvSelection) {
        ...
        if (index >= 0) {
            ...
            SoSelectionElementAction action(node->getDetail() ? SoSelectionElementAction::Remove
                                                              : SoSelectionElementAction::None,
                                            true);
    ...
    auto pcGroup =
        Reason.Type == SelectionChanges::SetPreselect ? pcGroupOnTopPreSel : pcGroupOnTopSel;

    if (objs.find(key.c_str()) != objs.end()) return;
    auto vp =
        dynamic_cast<ViewProviderDocumentObject *>(Application::Instance->getViewProvider(obj));
    if (!vp || !vp->isSelectable() || !vp->isShow()) return;
    auto svp = vp;
    if (subname && *subname) {
        auto sobj = obj->getSubObject(subname);
        if (!sobj || !sobj->getNameInDocument()) return;
        if (sobj != obj) {
            svp = dynamic_cast<ViewProviderDocumentObject *>(
                Application::Instance->getViewProvider(sobj));
            if (!svp || !svp->isSelectable()) return;
        }
    }
    int onTop;
    ...
    if (Gui::Selection().needPickedList()) onTop = 1;
    ...
    for (auto childVp = vp;; childVp = grpVp) {
        ...
        grpVp =
            dynamic_cast<ViewProviderDocumentObject *>(Application::Instance->getViewProvider(grp));
        if (!grpVp) break;
        ...
        if (childRoot->findChild(childVp->getRoot()) < 0) {
            FC_LOG("cannot find '" << childVp->getObject()->getFullName() << "' in geo group '"
...
void View3DInventorViewer::onSelectionChanged(const SelectionChanges &_Reason)
{
    ...
    switch (Reason.Type) {
        ...
        case SelectionChanges::SetPreselect:
            if (Reason.SubType != SelectionChanges::MsgSource::TreeView) break;
            // fall through
        ...
        case SelectionChanges::ClrSelection: checkGroupOnTop(Reason); break;
        case SelectionChanges::SetPreselectSignal: break;
        default: return;
    }
...
void View3DInventorViewer::addViewProvider(ViewProvider *pcProvider)
{
    ...
    if (root) {
        if (pcProvider->canAddToSceneGraph()) pcViewProviderRoot->addChild(root);
...
{
    if (this->editViewProvider == pcProvider) resetEditingViewProvider();

    SoSeparator *root = pcProvider->getRoot();

    if (root) {
        int index = pcViewProviderRoot->findChild(root);
        if (index >= 0) pcViewProviderRoot->removeChild(index);
...
void View3DInventorViewer::setEditingTransform(const Base::Matrix4D &mat)
{
    if (pcEditingTransform) {
        ...
        pcEditingTransform->setMatrix(SbMatrix(
            dMtrx[0], dMtrx[1], dMtrx[2], dMtrx[3], dMtrx[4], dMtrx[5], dMtrx[6], dMtrx[7],
            dMtrx[8], dMtrx[9], dMtrx[10], dMtrx[11], dMtrx[12], dMtrx[13], dMtrx[14], dMtrx[15]));
    }
}

void View3DInventorViewer::setupEditingRoot(SoNode *node, const Base::Matrix4D *mat)
{
    if (!editViewProvider) return;
    resetEditingRoot(false);
    if (mat) setEditingTransform(*mat);
    else
    ...
    for (int i = 0, count = root->getNumChildren(); i < count; ++i) {
        ...
        if (node != editViewProvider->getTransformNode()) pcEditingRoot->addChild(node);
    }
...
{
    if (!editViewProvider || pcEditingRoot->getNumChildren() <= 1) return;
    ...
    restoreEditingRoot = false;
    auto root = editViewProvider->getRoot();
    if (root->getNumChildren()) FC_ERR("WARNING!!! Editing view provider root node is tampered");
    ...
    try {
        if (updateLinks) ViewProviderLink::updateLinks(editViewProvider);
    }
...
SoPickedPoint *View3DInventorViewer::getPointOnRay(const SbVec3f &pos, const SbVec3f &dir,
...
void View3DInventorViewer::resetEditingViewProvider()
{
    if (this->editViewProvider) {
        ...
        if (heaction && heaction->getGrabber()) heaction->releaseGrabber();
...
{
    if (on && backgroundroot->findChild(pcBackGround) == -1) backgroundroot->addChild(pcBackGround);
...
void View3DInventorViewer::setEnabledFPSCounter(bool on) { fpsEnabled = on; }

void View3DInventorViewer::setEnabledVBO(bool on) { vboEnabled = on; }

bool View3DInventorViewer::isEnabledVBO() const { return vboEnabled; }
...
void View3DInventorViewer::setRenderCache(int mode)
{
    ...
    if (mode < 0) {
        ...
        if (mode == -2) {
            ...
            if (pcViewProviderRoot) pcViewProviderRoot->renderCaching = SoSeparator::AUTO;
            mode = setting;
        }
    }
...
void View3DInventorViewer::setEnabledNaviCube(bool on) { naviCubeEnabled = on; }

bool View3DInventorViewer::isEnabledNaviCube() const { return naviCubeEnabled; }

void View3DInventorViewer::setNaviCubeCorner(int c)
{
    if (naviCube) naviCube->setCorner(static_cast<NaviCube::Corner>(c));
}

NaviCube *View3DInventorViewer::getNavigationCube() const { return naviCube; }
...
bool View3DInventorViewer::hasAxisCross() { return axisGroup; }

void View3DInventorViewer::setNavigationType(Base::Type t)
{
    if (this->navigation && this->navigation->getTypeId() == t) return; // nothing to do

    Base::Type type =
        Base::Type::getTypeIfDerivedFrom(t.getName(), NavigationStyle::getClassTypeId());
...
NavigationStyle *View3DInventorViewer::navigationStyle() const { return this->navigation; }

SoDirectionalLight *View3DInventorViewer::getBacklight() const { return this->backlight; }

void View3DInventorViewer::setBacklight(SbBool on) { this->backlight->on = on; }

SbBool View3DInventorViewer::isBacklight() const { return this->backlight->on.getValue(); }
...
void View3DInventorViewer::setSceneGraph(SoNode *root)
{
    ...
    if (scene && scene->getTypeId().isDerivedFrom(SoSeparator::getClassTypeId())) {
        sa.apply(scene);
        if (!sa.getPath()) static_cast<SoSeparator *>(scene)->insertChild(this->backlight, 0);
    }
}
...
void View3DInventorViewer::savePicture(int w, int h, int s, const QColor &bg, QImage &img) const
{
    ...
    if (saveMethod == "FramebufferObject") { useFramebufferObject = true; }
    ...
    if (w > 0 && h > 0) vp.setWindowSize((short)w, (short)h);
    ...
    QColor bgColor;
    if (!bg.isValid()) {
        if (backgroundroot->findChild(pcBackGround) == -1) { bgColor = this->backgroundColor(); }
        else {
            useBackground = true;
            cb = new SoCallback;
        ...
        if (!useCoinOffscreenRenderer) {
            ...
            if (bgColor.isValid())
                renderer.setBackgroundColor(
                    SbColor4f(bgColor.redF(), bgColor.greenF(), bgColor.blueF(), bgColor.alphaF()));
            if (!renderer.render(root)) throw Base::RuntimeError("Offscreen rendering failed");
            ...
            if (bgColor.isValid())
                renderer.setBackgroundColor(
                    SbColor(bgColor.redF(), bgColor.greenF(), bgColor.blueF()));
            if (!renderer.render(root)) throw Base::RuntimeError("Offscreen rendering failed");
...
void View3DInventorViewer::saveGraphic(int pagesize, const QColor &bgcolor,
...
bool View3DInventorViewer::isSelecting() const { return navigation->isSelecting(); }
...
SbVec2f View3DInventorViewer::screenCoordsOfPath(SoPath *path) const
{
    ...
    // vary within [-0.5,1.5]...
    int width = getGLWidget()->width(), height = getGLWidget()->height();
...
std::list<GLGraphicsItem *> View3DInventorViewer::getGraphicsItems() const { return graphicsItems; }
...
View3DInventorViewer::getGraphicsItemsOfType(const Base::Type &type) const
{
    ...
    for (std::list<GLGraphicsItem *>::const_iterator it = this->graphicsItems.begin();
         it != this->graphicsItems.end(); ++it) {
        if ((*it)->isDerivedFrom(type)) items.push_back(*it);
    }

    return items;
}

void View3DInventorViewer::clearGraphicsItems() { this->graphicsItems.clear(); }

int View3DInventorViewer::getNumSamples()
{
    int samples = App::GetApplication()
                      .GetParameterGroupByPath("User parameter:BaseApp/Preferences/View")
                      ->GetInt("AntiAliasing", 0);

    switch (samples) {
        case View3DInventorViewer::MSAA2x: return 2;
        case View3DInventorViewer::MSAA4x: return 4;
        case View3DInventorViewer::MSAA8x: return 8;
        case View3DInventorViewer::Smoothing: return 1;
        default: return 0;
    }
}
                                                ...
                                                const QColor &bgcolor, QImage &img)
{
    ...
    if (bgopaque.isValid()) {
        ...
        if (alpha < 255) bgopaque.setRgb(255, 255, 255);
...
{
    switch (renderType) {
        case Native: renderScene(); break;
        case Framebuffer: renderFramebuffer(); break;
        case Image: renderGLImage(); break;
    }
}
...
void View3DInventorViewer::renderFramebuffer()
{
    ...
    for (std::list<GLGraphicsItem *>::iterator it = this->graphicsItems.begin();
         it != this->graphicsItems.end(); ++it)
        (*it)->paintGL();

    if (naviCubeEnabled) naviCube->drawNaviCube();
...
void View3DInventorViewer::renderScene()
{
    ...
    try {
        ...
        for (std::set<ViewProvider *>::iterator it = _ViewProviderSet.begin();
             it != _ViewProviderSet.end(); ++it)
            (*it)->hide();
    ...
    if (this->isAnimating()) { this->getSoRenderManager()->scheduleRedraw(); }
...
void View3DInventorViewer::printDimension()
{
    ...
    if (t.isDerivedFrom(SoOrthographicCamera::getClassTypeId())) {
        ...
        float fHeight = static_cast<SoOrthographicCamera *>(getSoRenderManager()->getCamera())
                            ->height.getValue();
        float fWidth = fHeight;

        if (dimX > dimY) fWidth *= ((float)dimX) / ((float)dimY);
        else if (dimX < dimY)
        ...
        QString dim = QString::fromLatin1("%1 x %2").arg(wStr, hStr);
...
void View3DInventorViewer::selectAll()
{
    ...
    if (!objs.empty()) Gui::Selection().setSelection(objs.front()->getDocument()->getName(), objs);
}

bool View3DInventorViewer::processSoEvent(const SoEvent *ev)
{
    if (naviCubeEnabled && naviCube->processSoEvent(ev)) return true;
    if (isRedirectedToSceneGraph()) {
        SbBool processed = inherited::processSoEvent(ev);

        if (!processed) processed = navigation->processEvent(ev);
...
{
    SoCamera *cam = this->getSoRenderManager()->getCamera();
    if (cam) cam->orientation.setValue(SbRotation(SbVec3f(0, 0, -1), dir));
}
...
SbRotation View3DInventorViewer::getCameraOrientation() const
{
    ...
    if (!cam) return SbRotation(0, 0, 0, 1); // this is the default
...
SbVec2f View3DInventorViewer::getNormalizedPosition(const SbVec2s &pnt) const
{
    ...
    // now calculate the real points respecting aspect ratio information
    //
    if (fRatio > 1.0f) { pX = (pX - 0.5f * dX) * fRatio + 0.5f * dX; }
    else if (fRatio < 1.0f) {
        pY = (pY - 0.5f * dY) / fRatio + 0.5f * dY;
    }
...
SbVec3f View3DInventorViewer::getPointOnFocalPlane(const SbVec2s &pnt) const
{
    ...
    if (focalDist < nearDist || focalDist > farDist) focalDist = 0.5f * (nearDist + farDist);
...
void View3DInventorViewer::toggleClippingPlane(int toggle, bool beforeEditing, bool noManip,
                                               const Base::Placement &pla)
{
    ...
    pcClipPlane->ref();
    if (beforeEditing) pcViewProviderRoot->insertChild(pcClipPlane, 0);
    else
        pcViewProviderRoot->insertChild(pcClipPlane,
                                        pcViewProviderRoot->findChild(pcEditingRoot) + 1);
}

bool View3DInventorViewer::hasClippingPlane() const { return !!pcClipPlane; }
...
const SoPickedPoint *View3DInventorViewer::getPickedPoint(SoEventCallback *n) const
{
    if (selectionRoot) {
        ...
        if (!ret.empty()) return ret[0].pp;
        return nullptr;
    }
    return n->getPickedPoint();
}

SbBool View3DInventorViewer::pubSeekToPoint(const SbVec2s &pos) { return this->seekToPoint(pos); }

void View3DInventorViewer::pubSeekToPoint(const SbVec3f &pos) { this->seekToPoint(pos); }
...
void View3DInventorViewer::moveCameraTo(const SbRotation &rot, const SbVec3f &pos, int steps,
                                        int ms)
{
    SoCamera *cam = this->getSoRenderManager()->getCamera();
    if (!cam) return;
...
#if (COIN_MAJOR_VERSION >= 3)
        if (aspectRatio < 1.0f) diff = sphere.getRadius() * 2 - height * aspectRatio;
        else
#endif
...
void View3DInventorViewer::boxZoom(const SbBox2s &box) { navigation->boxZoom(box); }
...
void View3DInventorViewer::viewAll()
{
    ...
    if (isAnimationEnabled()) animatedViewAll(10, 20);
...
void View3DInventorViewer::viewSelection()
{
    ...
    if (cam && bbox.IsValid()) {
        ...
        switch (cam->viewportMapping.getValue()) {
            ...
            case SoCamera::CROP_VIEWPORT_NO_FRAME: aspectratio = 1.0f; break;
            default: break;
        }
...
SbBool View3DInventorViewer::isAnimationEnabled() const { return navigation->isAnimationEnabled(); }

/*!
  Query if the model in the viewer is currently in spinning mode after
  a user drag.
*/
SbBool View3DInventorViewer::isAnimating() const { return navigation->isAnimating(); }
...
void View3DInventorViewer::stopAnimating() { navigation->stopAnimating(); }
...
SbBool View3DInventorViewer::isPopupMenuEnabled() const { return navigation->isPopupMenuEnabled(); }
...
{
    if (enable == this->axiscrossEnabled) { return; }

    this->axiscrossEnabled = enable;

    if (this->isViewing()) { this->getSoRenderManager()->scheduleRedraw(); }
}
...
SbBool View3DInventorViewer::isFeedbackVisible() const { return this->axiscrossEnabled; }
...
int View3DInventorViewer::getFeedbackSize() const { return this->axiscrossSize; }
...
{
    if (this->isViewing() == enable) { return; }
...
void View3DInventorViewer::setCursorRepresentation(int modearg)
{
    ...
    if (glWindow) glWindow->setAttribute(Qt::WA_SetCursor, false);
    ...
    switch (modearg) {
        ...
        case NavigationStyle::INTERACT:
            if (isEditing()) this->getWidget()->setCursor(this->editCursor);
            else
                this->getWidget()->setCursor(QCursor(Qt::ArrowCursor));
            break;

        case NavigationStyle::DRAGGING:
        case NavigationStyle::SPINNING: this->getWidget()->setCursor(spinCursor); break;

        case NavigationStyle::ZOOMING: this->getWidget()->setCursor(zoomCursor); break;

        case NavigationStyle::SEEK_MODE:
        case NavigationStyle::SEEK_WAIT_MODE:
        case NavigationStyle::BOXZOOM: this->getWidget()->setCursor(Qt::CrossCursor); break;

        case NavigationStyle::PANNING: this->getWidget()->setCursor(panCursor); break;
...
SoPath *View3DInventorViewer::pickFilterCB(void *viewer, const SoPickedPoint *pp)
{
    ...
    if (vp && vp->useNewSelectionModel()) {
        ...
        static char buf[513];
        snprintf(buf, 512, "Hovered: %s (%f,%f,%f)", e.c_str(), pp->getPoint()[0],
                 pp->getPoint()[1], pp->getPoint()[2]);
...
void View3DInventorViewer::removeEventCallback(SoType eventtype, SoEventCallbackCB *cb,
                                               void *userdata)
{
...
ViewProvider *View3DInventorViewer::getViewProviderByPath(SoPath *path) const
{
    for (int i = 0; i < path->getLength(); i++) {
        ...
        if (node->isOfType(SoSeparator::getClassTypeId())) {
            ...
            if (it != _ViewProviderMap.end()) { return it->second; }
        }
    }
    return nullptr;
}
...
ViewProvider *View3DInventorViewer::getViewProviderByPathFromTail(SoPath *path) const
{
    ...
    for (int i = 0; i < path->getLength(); i++) {
        ...
        if (node->isOfType(SoSeparator::getClassTypeId())) {
            std::map<SoSeparator *, ViewProvider *>::const_iterator it =
                _ViewProviderMap.find(static_cast<SoSeparator *>(node));
            if (it != _ViewProviderMap.end()) { return it->second; }
        }
    }
...
View3DInventorViewer::getViewProvidersOfType(const Base::Type &typeId) const
{
    ...
    for (std::set<ViewProvider *>::const_iterator it = _ViewProviderSet.begin();
         it != _ViewProviderSet.end(); ++it) {
        if ((*it)->getTypeId().isDerivedFrom(typeId)) { views.push_back(*it); }
    }

    return views;
}

void View3DInventorViewer::turnAllDimensionsOn() { dimensionRoot->whichChild = SO_SWITCH_ALL; }

void View3DInventorViewer::turnAllDimensionsOff() { dimensionRoot->whichChild = SO_SWITCH_NONE; }
...
{
    if (!_viewerPy) _viewerPy = new View3DInventorViewerPy(this);
...
{
    const QMimeData *data = e->mimeData();
    if (data->hasUrls() && selectionRoot && selectionRoot->pcDocument) { e->accept(); }
    else {
        inherited::dragMoveEvent(e);
    }
}

void View3DInventorViewer::dragLeaveEvent(QDragLeaveEvent *e) { inherited::dragLeaveEvent(e); }

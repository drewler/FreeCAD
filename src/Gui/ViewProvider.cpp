...
void coinRemoveAllChildren(SoGroup *group)
{
    ...
    for (; count > 0; --count) group->removeChild(count - 1);
...
ViewProvider::ViewProvider()
    : pcAnnotation(nullptr), pyViewObject(nullptr), overrideMode("As Is"), _iActualMode(-1),
      _iEditMode(-1), viewOverrideMode(-1)
{
...
int ViewProvider::getEditingMode() const { return _iEditMode; }

bool ViewProvider::isEditing() const { return getEditingMode() > -1; }
...
void ViewProvider::unsetEdit(int ModNum) { Q_UNUSED(ModNum); }

void ViewProvider::setEditViewer(View3DInventorViewer *, int ModNum) { Q_UNUSED(ModNum); }

void ViewProvider::unsetEditViewer(View3DInventorViewer *) {}

bool ViewProvider::isUpdatesEnabled() const { return testStatus(UpdateData); }

void ViewProvider::setUpdatesEnabled(bool enable) { setStatus(UpdateData, enable); }

void highlight(const HighlightMode &high) { Q_UNUSED(high); }
...
void ViewProvider::eventCallback(void *ud, SoEventCallback *node)
{
    ...
    try {
        ...
        if (ev->getTypeId().isDerivedFrom(SoKeyboardEvent::getClassTypeId())) {
            ...
            switch (ke->getKey()) {
                case SoKeyboardEvent::ESCAPE:
                    if (self->keyPressed(press, ke->getKey())) { node->setHandled(); }
        ...
        // Mouse Movement handling
            ...
            if (self->mouseMove(ev->getPosition(), viewer)) node->setHandled();
        }
    }
...
void ViewProvider::setTransformation(const SbMatrix &rcMatrix) { pcTransform->setMatrix(rcMatrix); }

SbMatrix ViewProvider::convert(const Base::Matrix4D &rcMatrix)
{
    double dMtrx[16];
    rcMatrix.getGLMatrix(dMtrx);
    return SbMatrix(dMtrx[0], dMtrx[1], dMtrx[2], dMtrx[3], dMtrx[4], dMtrx[5], dMtrx[6], dMtrx[7],
                    dMtrx[8], dMtrx[9], dMtrx[10], dMtrx[11], dMtrx[12], dMtrx[13], dMtrx[14],
                    dMtrx[15]);
}
...
Base::Matrix4D ViewProvider::convert(const SbMatrix &smat)
{
    Base::Matrix4D mat;
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) mat[i][j] = smat[j][i];
    }
    return mat;
}
...
void ViewProvider::setDisplayMaskMode(const char *type)
{
    ...
    if (it != _sDisplayMaskModes.end()) _iActualMode = it->second;
    else
        _iActualMode = -1;
    setModeSwitch();
}
...
{
    std::map<std::string, int>::const_iterator it = _sDisplayMaskModes.find(type);
    if (it != _sDisplayMaskModes.end()) { return pcModeSwitch->getChild(it->second); }
...
void ViewProvider::setDisplayMode(const char *ModeName)
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionSetDisplayMode(ModeName);
}

const char *ViewProvider::getDefaultDisplayMode() const { return nullptr; }
...
std::string ViewProvider::getActiveDisplayMode() const { return _sCurrentMode; }
...
void ViewProvider::hide()
{
    ...
    if (pcModeSwitch->whichChild.getValue() >= 0) {
        ...
        for (auto ext : exts) ext->extensionModeSwitchChange();
    }

    //tell extensions that we hide
    for (Gui::ViewProviderExtension *ext : exts) ext->extensionHide();
}
...
void ViewProvider::show()
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionShow();
}

bool ViewProvider::isShow() const { return pcModeSwitch->whichChild.getValue() != -1; }

void ViewProvider::setVisible(bool s) { s ? show() : hide(); }

bool ViewProvider::isVisible() const { return isShow(); }
...
void ViewProvider::setOverrideMode(const std::string &mode)
{
    if (mode == "As Is") {
        ...
        if (it == _sDisplayMaskModes.end()) return; //view style not supported
        viewOverrideMode = (*it).second;
        overrideMode = mode;
    }
    if (pcModeSwitch->whichChild.getValue() != -1) setModeSwitch();
    else {
...
const string ViewProvider::getOverrideMode() { return overrideMode; }


void ViewProvider::setModeSwitch()
{
    if (viewOverrideMode == -1) pcModeSwitch->whichChild = _iActualMode;
...
std::string ViewProvider::toString() const { return SoFCDB::writeNodesToString(pcRoot); }
...
SoPickedPoint *ViewProvider::getPointOnRay(const SbVec2s &pos,
                                           const View3DInventorViewer *viewer) const
{
    return viewer->getPointOnRay(pos, this);
}

SoPickedPoint *ViewProvider::getPointOnRay(const SbVec3f &pos, const SbVec3f &dir,
                                           const View3DInventorViewer *viewer) const
{
...
bool ViewProvider::mouseButtonPressed(int button, bool pressed, const SbVec2s &cursorPos,
...
bool ViewProvider::mouseWheelEvent(int delta, const SbVec2s &cursorPos,
                                   const View3DInventorViewer *viewer)
{
    (void)delta;
    (void)cursorPos;
...
bool ViewProvider::onDelete(const vector<string> &subNames)
{
    bool del = true;
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) del &= ext->extensionOnDelete(subNames);

    return del;
}

bool ViewProvider::canDelete(App::DocumentObject *) const { return false; }

bool ViewProvider::canDragObject(App::DocumentObject *obj) const
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (ext->extensionCanDragObject(obj)) return true;
    }
...
bool ViewProvider::canDragObjects() const
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (ext->extensionCanDragObjects()) return true;
    }
...
#endif
        if (ext->extensionCanDropObject(obj)) return true;
    }
...
bool ViewProvider::canDropObjects() const
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector)
        if (ext->extensionCanDropObjects()) return true;
...
bool ViewProvider::canDragAndDropObject(App::DocumentObject *obj) const
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (!ext->extensionCanDragAndDropObject(obj)) return false;
    }
...
void ViewProvider::dropObject(App::DocumentObject *obj)
{
    ...
    throw Base::RuntimeError(
        "ViewProvider::dropObject: no extension for dropping given object available.");
}
                                   ...
                                   const std::vector<std::string> &elements) const
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (ext->extensionCanDropObjectEx(obj, owner, subname, elements)) return true;
    }
...
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionUpdateData(prop);
}
...
std::vector<App::DocumentObject *> ViewProvider::claimChildren() const
{
    ...
    for (Gui::ViewProviderExtension *ext : vector) {
        ...
        if (!nvec.empty()) vec.insert(std::end(vec), std::begin(nvec), std::end(nvec));
    }
    return vec;
}
...
bool ViewProvider::getElementPicked(const SoPickedPoint *pp, std::string &subname) const
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (ext->extensionGetElementPicked(pp, subname)) return true;
    }
    subname = getElement(pp ? pp->getDetail() : nullptr);
    return true;
}

bool ViewProvider::getDetailPath(const char *subname, SoFullPath *pPath, bool append,
                                 SoDetail *&det) const
{
    ...
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) {
        if (ext->extensionGetDetailPath(subname, pPath, det)) return true;
    }
    det = getDetail(subname);
    return true;
}

const std::string &ViewProvider::hiddenMarker() { return App::DocumentObject::hiddenMarker(); }
...
int ViewProvider::partialRender(const std::vector<std::string> &elements, bool clear)
{
    ...
    for (auto element : elements) {
        ...
        if (hidden) element.resize(element.size() - hiddenMarker().size());
        path->truncate(0);
        ...
        if (getDetailPath(element.c_str(), path, false, det)) {
            ...
            if (!hidden)
                action.setType(clear ? SoSelectionElementAction::Remove
...
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionBeforeDelete();
}
...
Base::BoundBox3d ViewProvider::getBoundingBox(const char *subname, bool transform,
                                              MDIView *view) const
{
    if (!pcRoot || !pcModeSwitch || pcRoot->findChild(pcModeSwitch) < 0) return Base::BoundBox3d();

    if (!view) view = Application::Instance->activeView();
    auto iview = dynamic_cast<View3DInventor *>(view);
    if (!iview) {
        auto doc = Application::Instance->activeDocument();
        if (doc) {
            auto views = doc->getMDIViewsOfType(View3DInventor::getClassTypeId());
            if (!views.empty()) iview = dynamic_cast<View3DInventor *>(views.front());
        }
        if (!iview) {
            FC_ERR("no view");
    ...
    if (mode < 0) pcModeSwitch->whichChild = getDefaultMode();
    ...
    if (subname && subname[0]) {
        ...
        if (!getDetailPath(subname, &path, true, det)) {
            if (mode < 0) pcModeSwitch->whichChild = mode;
            path.unrefNoDelete();
            return Base::BoundBox3d();
        }
        delete det;
    ...
    if (path.getLength()) bboxAction.apply(&path);
    else
...
{
    auto ext = getExtensionByType<ViewProviderLinkObserver>(true);
    if (ext) ext->setLinkVisible(visible);
}

...
PropertyEditor::PropertyEditor(QWidget *parent)
    : QTreeView(parent), autoexpand(false), autoupdate(false), committing(false), delaybuild(false),
      binding(false), checkDocument(false), closingEditor(false)
{
    ...
    connect(this, SIGNAL(collapsed(const QModelIndex &)), this,
            SLOT(onItemCollapsed(const QModelIndex &)));
    ...
    connect(propertyModel, SIGNAL(rowsRemoved(const QModelIndex &, int, int)), this,
...
void PropertyEditor::setAutomaticExpand(bool v) { autoexpand = v; }

bool PropertyEditor::isAutomaticExpand(bool) const { return autoexpand; }
...
void PropertyEditor::setAutomaticDocumentUpdate(bool v) { autoupdate = v; }

bool PropertyEditor::isAutomaticDocumentUpdate(bool) const { return autoupdate; }

QBrush PropertyEditor::groupBackground() const { return this->background; }

void PropertyEditor::setGroupBackground(const QBrush &c) { this->background = c; }

QColor PropertyEditor::groupTextColor() const { return this->groupColor; }

void PropertyEditor::setGroupTextColor(const QColor &c) { this->groupColor = c; }
    ...
    if (event->type() == QEvent::ShortcutOverride) {
        ...
        if (kevent->modifiers() == Qt::NoModifier || kevent->modifiers() == Qt::ShiftModifier
            ...
            switch (kevent->key()) {
                ...
                case Qt::Key_Left:
                case Qt::Key_Right: kevent->accept();
                default: break;
            }
        }
    }
...
{
    if (editingIndex == index && activeEditor) return;

    closeEditor();

    openPersistentEditor(model()->buddy(index));

    if (!editingIndex.isValid() || !autoupdate) return;
    ...
    if (obj && obj->getNameInDocument()) str << obj->getNameInDocument() << '.';
    else
...
void PropertyEditor::recomputeDocument(App::Document *doc)
{
    try {
        if (doc && !doc->isTransactionEmpty()) {
            ...
            if (doc->isTouched()) doc->recompute();
        }
    }
    // do not re-throw
...
void PropertyEditor::closeEditor(QWidget *editor, QAbstractItemDelegate::EndEditHint hint)
{
    ...
    do {
        ...
        if (!index.isValid() || index == lastIndex) {
            ...
            if (!index.isValid() || index == indexSaved) break;
        }
        lastIndex = index;
...
void PropertyEditor::reset()
{
    ...
    for (int i = 0; i < numRows; ++i) {
        ...
        if (item->childCount() == 0) {
            if (item->isSeparator()) setRowHidden(i, parent, true);
        }
        else
            setEditorMode(index, 0, item->childCount() - 1);
        if (item->isExpanded()) setExpanded(index, true);
    }
}

void PropertyEditor::onRowsMoved(const QModelIndex &parent, int start, int end,
                                 const QModelIndex &dst, int)
{
    if (parent != dst) {
...
void PropertyEditor::rowsInserted(const QModelIndex &parent, int start, int end)
{
    ...
    if (item && item->isSeparator() && item->childCount() == end - start + 1) {
        ...
        if (item->isExpanded()) setExpanded(parent, true);
    }
    ...
    for (int i = start; i < end; ++i) {
        ...
        if (child->isExpanded()) setExpanded(index, true);
    }

    if (parent.isValid()) setEditorMode(parent, start, end);
}
...
void PropertyEditor::rowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    ...
    if (editingIndex.isValid()) {
        if (editingIndex.row() >= start && editingIndex.row() <= end) closeTransaction();
        else {
            removingRows = 1;
...
{
    if (removingRows < 0 && activeEditor) activeEditor->setFocus();
    removingRows = 0;
}

void PropertyEditor::drawBranches(QPainter *painter, const QRect &rect,
...
void PropertyEditor::buildUp(PropertyModel::PropertyList &&props, bool _checkDocument)
{
    ...
    if (!propertyPath.isEmpty()) this->selectedProperty = propertyPath;
...
void PropertyEditor::updateProperty(const App::Property &prop)
{
    ...
    if (!committing) propertyModel->updateProperty(prop);
}
...
void PropertyEditor::removeProperty(const App::Property &prop)
{
    for (PropertyModel::PropertyList::iterator it = propList.begin(); it != propList.end(); ++it) {
        ...
        if (pos != it->second.end()) {
            ...
            if (it->second.empty()) { propList.erase(it); }
...
void PropertyEditor::contextMenuEvent(QContextMenuEvent *)
{
    ...
    if (PropertyView::showAll()) {
        ...
        if (canRemove) menu.addAction(tr("Remove property"))->setData(QVariant(MA_RemoveProp));
...
#define _ACTION_SETUP(_name)                                                                       \
    do {                                                                                           \
        ...
        if (propStatus & (1 << App::Property::_name)) action->setChecked(true);                    \
    } while (0)
#define ACTION_SETUP(_name)                                                                        \
    ...
    switch (action->data().toInt()) {
        case MA_AutoExpand:
            ...
            if (autoexpand) expandAll();
            return;
        case MA_ShowAll: PropertyView::setShowAll(action->isChecked()); return;
#define ACTION_CHECK(_name)                                                                        \
    case MA_##_name:                                                                               \
        for (auto prop : props) prop->setStatus(App::Property::_name, action->isChecked());        \
        break
        ...
        case MA_Touched:
            for (auto prop : props) {
                if (action->isChecked()) prop->touch();
                else
                    prop->purgeTouched();
            }
            break;
        ...
        case MA_AddProp: {
            ...
            if (sels.size() == 1) containers.insert(sels[0].pObject);
                ...
                for (auto prop : props) containers.insert(prop->getContainer());
            }
        ...
        case MA_EditPropGroup: {
            ...
            QString res = QInputDialog::getText(Gui::getMainWindow(), tr("Rename property group"),

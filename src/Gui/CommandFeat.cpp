...
StdCmdFeatRecompute::StdCmdFeatRecompute() : Command("Std_Recompute")
{
    // setting the
    sGroup = "File";
...
void StdCmdFeatRecompute::activated(int iMsg) { Q_UNUSED(iMsg); }
...
StdCmdRandomColor::StdCmdRandomColor() : Command("Std_RandomColor")
{
    sGroup = "File";
...
void StdCmdRandomColor::activated(int iMsg)
{
    ...
    for (const auto &it : sel) {
        ...
        if (vpLink) {
            ...
            cmdGuiObjectArgs(it.pObject, "ShapeMaterial.DiffuseColor=(%.2f,%.2f,%.2f)", fRed, fGrn,
                             fBlu);
            continue;
        }
...
bool StdCmdRandomColor::isActive() { return (Gui::Selection().size() != 0); }
...
StdCmdSendToPythonConsole::StdCmdSendToPythonConsole() : Command("Std_SendToPythonConsole")
{
    // setting the
    sGroup = "Edit";
...
void StdCmdSendToPythonConsole::activated(int iMsg)
{
    ...
    const std::vector<Gui::SelectionObject> &sels = Gui::Selection().getSelectionEx(
        "*", App::DocumentObject::getClassTypeId(), ResolveMode::OldStyleElement, false);

...
public:
    ~DocumentModelIndex() override { qDeleteAll(childItems); }

    void setParent(DocumentModelIndex *parent) { parentItem = parent; }
    DocumentModelIndex *parent() const { return parentItem; }
    void appendChild(DocumentModelIndex *child)
    {
        childItems.append(child);
        child->setParent(this);
    }
    void removeChild(int row) { childItems.removeAt(row); }
    ...
    DocumentModelIndex *child(int row) { return childItems.value(row); }
    int row() const
    {
        if (parentItem)
...
public:
    ...
    {
        if (!documentIcon) documentIcon = new QIcon(Gui::BitmapFactory().pixmap("Document"));
    }
...
int ApplicationIndex::findChild(const Gui::Document &d) const
{
    ...
    for (it = childItems.begin(); it != childItems.end(); ++it, ++child) {
        ...
        if (&doc->d == &d) return child;
    }

    return -1;
}

Qt::ItemFlags ApplicationIndex::flags() const { return Qt::ItemIsEnabled; }

QVariant ApplicationIndex::data(int role) const
{
    if (role == Qt::DecorationRole) { return qApp->windowIcon(); }
...
void DocumentIndex::addToDocument(ViewProviderIndex *vp) { vp_nodes[&vp->v].insert(vp); }

void DocumentIndex::removeFromDocument(ViewProviderIndex *vp) { vp_nodes[&vp->v].erase(vp); }
...
{
    if (role == Qt::DecorationRole) { return *documentIcon; }
    else if (role == Qt::DisplayRole) {
...
void ViewProviderIndex::findViewProviders(const ViewProviderDocumentObject &vp,
                                          QList<ViewProviderIndex *> &index) const
{
    if (&this->v == &vp) index.push_back(const_cast<ViewProviderIndex *>(this));
    QList<DocumentModelIndex *>::const_iterator it;
    for (it = childItems.begin(); it != childItems.end(); ++it) {
        auto v = static_cast<ViewProviderIndex *>(*it);
        v->findViewProviders(vp, index);
...
{
    if (role == Qt::DecorationRole) { return v.getIcon(); }
    else if (role == Qt::DisplayRole) {
...
struct DocumentModelP {
    DocumentModelP() { rootItem = new ApplicationIndex(); }
    ~DocumentModelP() { delete rootItem; }
...
DocumentModel::DocumentModel(QObject *parent) : QAbstractItemModel(parent), d(new DocumentModelP)
{
    ...
    Application::Instance->signalNewDocument.connect(
        boost::bind(&DocumentModel::slotNewDocument, this, bp::_1));
    Application::Instance->signalDeleteDocument.connect(
        boost::bind(&DocumentModel::slotDeleteDocument, this, bp::_1));
    Application::Instance->signalRenameDocument.connect(
        boost::bind(&DocumentModel::slotRenameDocument, this, bp::_1));
    Application::Instance->signalActiveDocument.connect(
        boost::bind(&DocumentModel::slotActiveDocument, this, bp::_1));
    Application::Instance->signalRelabelDocument.connect(
        boost::bind(&DocumentModel::slotRelabelDocument, this, bp::_1));
}
...
void DocumentModel::slotNewDocument(const Gui::Document &Doc)
{
    ...
    Doc.signalChangedObject.connect(
        boost::bind(&DocumentModel::slotChangeObject, this, bp::_1, bp::_2));
...
void DocumentModel::slotInEdit(const Gui::ViewProviderDocumentObject &v) { Q_UNUSED(v); }

void DocumentModel::slotResetEdit(const Gui::ViewProviderDocumentObject &v) { Q_UNUSED(v); }
...
{
    if (prop.isDerivedFrom(App::PropertyLink::getClassTypeId())) return true;
    if (prop.isDerivedFrom(App::PropertyLinkSub::getClassTypeId())) return true;
    if (prop.isDerivedFrom(App::PropertyLinkList::getClassTypeId())) return true;
    if (prop.isDerivedFrom(App::PropertyLinkSubList::getClassTypeId())) return true;
    return false;
}
...
QModelIndex DocumentModel::index(int row, int column, const QModelIndex &parent) const
{
    ...
    if (!parent.isValid()) item = d->rootItem;
    else
...
{
    if (!index.isValid() || index.internalPointer() == d->rootItem) return QModelIndex();
    DocumentModelIndex *item = nullptr;
...
{
    if (!parent.isValid()) return 1; // the root item
    DocumentModelIndex *item = nullptr;
...
bool DocumentModel::setHeaderData(int, Qt::Orientation, const QVariant &, int) { return false; }

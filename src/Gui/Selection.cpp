...
SelectionObserver::SelectionObserver(bool attach, ResolveMode resolve)
    : resolve(resolve), blockedSelection(false)
{
    if (attach) attachSelection();
}

SelectionObserver::SelectionObserver(const ViewProviderDocumentObject *vp, bool attach,
                                     ResolveMode resolve)
    : resolve(resolve), blockedSelection(false)
{
...
SelectionObserver::~SelectionObserver() { detachSelection(); }
...
bool SelectionObserver::isSelectionBlocked() const { return blockedSelection; }

bool SelectionObserver::isSelectionAttached() const { return connectSelection.connected(); }
...
void SelectionObserver::attachSelection()
{
    if (!connectSelection.connected()) {
        ...
        connectSelection =
            signal.connect(boost::bind(&SelectionObserver::_onSelectionChanged, this, bp::_1));
...
void SelectionObserver::detachSelection()
{
    if (connectSelection.connected()) {
        ...
        if (!filterDocName.empty()) Selection().rmvSelectionGate();
    }
}
...
void SelectionObserverPython::removeObserver(const Py::Object &obj)
{
    ...
    for (std::vector<SelectionObserverPython *>::iterator it = _instances.begin();
...
{
    switch (msg.Type) {
        case SelectionChanges::AddSelection: addSelection(msg); break;
        case SelectionChanges::RmvSelection: removeSelection(msg); break;
        case SelectionChanges::SetSelection: setSelection(msg); break;
        case SelectionChanges::ClrSelection: clearSelection(msg); break;
        case SelectionChanges::SetPreselect: setPreselection(msg); break;
        case SelectionChanges::RmvPreselect: removePreselection(msg); break;
        case SelectionChanges::PickedListChanged: pickedListChanged(); break;
        default: break;
    }
}
...
{
    if (py_pickedListChanged.isNone()) return;
...
{
    if (py_addSelection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(4);
...
{
    if (py_removeSelection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(3);
        args.setItem(0, Py::String(msg.pDocName ? msg.pDocName : ""));
...
{
    if (py_setSelection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::String(msg.pDocName ? msg.pDocName : ""));
...
{
    if (py_clearSelection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(1);
        args.setItem(0, Py::String(msg.pDocName ? msg.pDocName : ""));
...
{
    if (py_setPreselection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(3);
        args.setItem(0, Py::String(msg.pDocName ? msg.pDocName : ""));
...
{
    if (py_removePreselection.isNone()) return;
    Base::PyGILStateLocker lock;
    try {
        Py::Tuple args(3);
        args.setItem(0, Py::String(msg.pDocName ? msg.pDocName : ""));
...
bool SelectionSingleton::hasSelection() const { return !_SelList.empty(); }
...
SelectionSingleton::getSelection(const char *pDocName, ResolveMode resolve, bool single) const
{
    ...
    for (auto &sel : _SelList) {
        ...
        if (!obj || (pcDoc && sel.pObject->getDocument() != pcDoc)) continue;
...
bool SelectionSingleton::hasSelection(const char *doc, ResolveMode resolve) const
{
    ...
    for (auto &sel : _SelList) {
        ...
        if (obj && (!pcDoc || sel.pObject->getDocument() == pcDoc)) { return true; }
    }
...
bool SelectionSingleton::hasSubSelection(const char *doc, bool subElement) const
{
    ...
    for (auto &sel : _SelList) {
        ...
        if (sel.SubName.empty()) continue;
        if (subElement && sel.SubName.back() != '.') return true;
        if (sel.pObject != sel.pResolvedObject) return true;
    }
                                                               ...
                                                               bool single) const
{
    ...
    // check the type
    if (typeId == Base::Type::badType()) return temp;
    ...
    for (auto &sel : objList) {
        ...
        if (it != SortMap.end()) {
            ...
            if (subelement && *subelement) {
                ...
                if (resolve != ResolveMode::NoResolve) temp.back()._SubNameSet.insert(subelement);
            }
...
bool SelectionSingleton::needPickedList() const { return _needPickedList; }
    ...
    while (!NotificationQueue.empty()) {
        ...
        switch (msg.Type) {
            ...
            case SelectionChanges::RmvSelection:
                notify = !isSelected(msg.pDocName, msg.pObjectName, msg.pSubName,
...
bool SelectionSingleton::hasPickedList() const { return !_PickedList.empty(); }
                                                                   ...
                                                                   ResolveMode resolve) const
{
    ...
    for (auto &sel : _SelList) {
        ...
        if (pObject) {
            ...
            if (ret.second) temp.push_back(pObject);
        }
    }
                                                                        ...
                                                                        ResolveMode resolve) const
{
    ...
    if (typeId == Base::Type::badType()) return std::vector<App::DocumentObject *>();
    return getObjectsOfType(typeId, pDocName, resolve);
}

unsigned int SelectionSingleton::countObjectsOfType(const Base::Type &typeId, const char *pDocName,
                                                    ResolveMode resolve) const
{
    unsigned int iNbr = 0;
    App::Document *pcDoc = nullptr;
    ...
    for (auto &sel : _SelList) {
        if ((!pcDoc || pcDoc == sel.pDoc) && getObjectOfType(sel, typeId, resolve)) iNbr++;
    }

    return iNbr;
}

unsigned int SelectionSingleton::countObjectsOfType(const char *typeName, const char *pDocName,
                                                    ResolveMode resolve) const
{
    Base::Type typeId = Base::Type::fromName(typeName);
    if (typeId == Base::Type::badType()) return 0;
...
void SelectionSingleton::slotSelectionChanged(const SelectionChanges &msg)
{
    ...
    if (!msg.Object.getSubName().empty()) {
        ...
        SelectionChanges msg2(
            msg.Type, pObject->getDocument()->getName(), pObject->getNameInDocument(),
...
int SelectionSingleton::setPreselect(const char *pDocName, const char *pObjectName,
                                     ...
                                     SelectionChanges::MsgSource signal)
{
    ...
    if (DocName == pDocName && FeatName == pObjectName && SubName == pSubName) { return -1; }

    rmvPreselect();

    if (ActiveGate && signal != SelectionChanges::MsgSource::Internal) {
        App::Document *pDoc = getDocument(pDocName);
        if (!pDoc || !pObjectName) return 0;
        std::pair<std::string, std::string> elementName;
        ...
        if (!ActiveGate->allow(pObject->getDocument(), pObject, subelement)) {
            ...
            msg.append(QString::fromLatin1(" %1.%2.%3 ")
                           .arg(QString::fromLatin1(pDocName), QString::fromLatin1(pObjectName),
...
namespace Gui
{
std::array<std::pair<double, std::string>, 3> schemaTranslatePoint(double x, double y, double z,
                                                                   double precision)
{
...
void SelectionSingleton::setPreselectCoord(float x, float y, float z)
{
    ...
    snprintf(buf, 512, "Preselected: %s.%s.%s (%f %s,%f %s,%f %s)", CurrentPreselection.pDocName,
             CurrentPreselection.pObjectName, CurrentPreselection.pSubName, pts[0].first,
             pts[0].second.c_str(), pts[1].first, pts[1].second.c_str(), pts[2].first,
             pts[2].second.c_str());

    if (getMainWindow()) getMainWindow()->showMessage(QString::fromUtf8(buf));
}
...
const SelectionChanges &SelectionSingleton::getPreselection() const { return CurrentPreselection; }

// add a SelectionGate to control what is selectable
void SelectionSingleton::addSelectionGate(Gui::SelectionGate *gate, ResolveMode resolve)
{
    if (ActiveGate) rmvSelectionGate();
...
void SelectionSingleton::rmvSelectionGate()
{
    if (ActiveGate) {
        ...
        if (doc) {
            ...
            if (mdi) mdi->restoreOverrideCursor();
        }
    }
}
...
{
    if (pDocName && pDocName[0]) return App::GetApplication().getDocument(pDocName);
    else
...
{
    if (!logDisabled) logHasSelection = hasSelection();
    return ++logDisabled;
}
                                      ...
                                      const std::vector<SelObj> *pickedList, bool clearPreselect)
{
    ...
    if (!logDisabled) temp.log(false, clearPreselect);

    _SelList.push_back(temp);
    _SelStackForward.clear();

    if (clearPreselect) rmvPreselect();

    SelectionChanges Chng(SelectionChanges::AddSelection, temp.DocName, temp.FeatName, temp.SubName,
                          temp.TypeName, x, y, z);
...
void SelectionSingleton::selStackPush(bool clearForward, bool overwrite)
{
    ...
    if (clearForward) _SelStackForward.clear();
    if (_SelList.empty()) return;
    if ((int)_SelStackBack.size() >= stackSize) _SelStackBack.pop_front();
    SelStackItem item;
    for (auto &sel : _SelList)
        item.emplace(sel.DocName.c_str(), sel.FeatName.c_str(), sel.SubName.c_str());
    if (!_SelStackBack.empty() && _SelStackBack.back() == item) return;
    if (!overwrite || _SelStackBack.empty()) _SelStackBack.emplace_back();
    _SelStackBack.back() = std::move(item);
}

void SelectionSingleton::selStackGoBack(int count)
{
    if ((int)_SelStackBack.size() < count) count = _SelStackBack.size();
    if (count <= 0) return;
    if (!_SelList.empty()) {
    ...
    while (!_SelStackBack.empty()) {
        ...
        for (auto &sobjT : _SelStackBack.back()) {
            if (sobjT.getSubObject()) {
                addSelection(sobjT.getDocumentName().c_str(), sobjT.getObjectName().c_str(),
...
{
    if ((int)_SelStackForward.size() < count) count = _SelStackForward.size();
    if (count <= 0) return;
    if (!_SelList.empty()) {
        selStackPush(false, true);
        clearCompleteSelection();
...
std::vector<SelectionObject> SelectionSingleton::selStackGet(const char *pDocName,
                                                             ResolveMode resolve, int index) const
{
    const SelStackItem *item = nullptr;
    if (index >= 0) {
        if (index >= (int)_SelStackBack.size()) return {};
        item = &_SelStackBack[_SelStackBack.size() - 1 - index];
    }
    else {
        index = -index - 1;
        if (index >= (int)_SelStackForward.size()) return {};
    ...
    for (auto &sobjT : *item) {
        _SelObj sel;
        if (checkSelection(sobjT.getDocumentName().c_str(), sobjT.getObjectName().c_str(),
                           sobjT.getSubName().c_str(), ResolveMode::NoResolve, sel, &selList)
            == 0) {
...
bool SelectionSingleton::addSelections(const char *pDocName, const char *pObjectName,
                                       const std::vector<std::string> &pSubNames)
{
    ...
    for (const auto &pSubName : pSubNames) {
        ...
        SelectionChanges Chng(SelectionChanges::AddSelection, temp.DocName, temp.FeatName,
                              temp.SubName, temp.TypeName);
    ...
    if (update) getMainWindow()->updateActions();
    return true;
}

bool SelectionSingleton::updateSelection(bool show, const char *pDocName, const char *pObjectName,
                                         const char *pSubName)
{
    if (!pDocName || !pObjectName) return false;
    if (!pSubName) pSubName = "";
    ...
    if (!isSelected(pObject, pSubName, ResolveMode::NoResolve)) return false;
...
void SelectionSingleton::rmvSelection(const char *pDocName, const char *pObjectName,
                                      const char *pSubName, const std::vector<SelObj> *pickedList)
{
    if (pickedList) {
        _PickedList.clear();
        for (const auto &sel : *pickedList) {
    ...
    for (auto It = _SelList.begin(), ItNext = It; It != _SelList.end(); It = ItNext) {
        ++ItNext;
        if (It->DocName != temp.DocName || It->FeatName != temp.FeatName) continue;
        ...
        changes.emplace_back(SelectionChanges::RmvSelection, It->DocName, It->FeatName, It->SubName,
                             It->TypeName);
...
struct SelInfo {
    ...
    SelInfo(const std::string &docName, const std::string &featName, const std::string &subName)
        : DocName(docName), FeatName(featName), SubName(subName)
    {}
};
...
void SelectionSingleton::setVisible(VisibleState vis)
{
    ...
    int visible;
    switch (vis) {
        case VisShow: visible = 1; break;
        case VisToggle: visible = -1; break;
        default: visible = 0;
    }
    ...
    for (auto &sel : _SelList) {
        if (sel.DocName.empty() || sel.FeatName.empty() || !sel.pObject) continue;
    ...
    for (auto &sel : sels) {
        ...
        if (!obj || !obj->getNameInDocument() || (parent && !parent->getNameInDocument())) continue;
        // try call parent object's setElementVisible
        if (parent) {
            // prevent setting the same object visibility more than once
            if (!filter.insert(std::make_pair(obj, parent)).second) continue;
            int visElement = parent->isElementVisible(elementName.c_str());
            if (visElement >= 0) {
                if (visElement > 0) visElement = 1;
                if (visible >= 0) {
                    if (visElement == visible) continue;
                    visElement = visible;
                }
                else {
                ...
                if (!visElement)
                    updateSelection(false, sel.DocName.c_str(), sel.FeatName.c_str(),
                                    sel.SubName.c_str());
                parent->setElementVisible(elementName.c_str(), visElement ? true : false);
                if (visElement)
                    updateSelection(true, sel.DocName.c_str(), sel.FeatName.c_str(),
                                    sel.SubName.c_str());
                continue;
            }
        ...
        if (vp) {
            bool visObject;
            if (visible >= 0) visObject = visible ? true : false;
            else
                visObject = !vp->isShow();

            if (visObject) {
                vp->show();
                updateSelection(visObject, sel.DocName.c_str(), sel.FeatName.c_str(),
                                sel.SubName.c_str());
            }
            else {
                updateSelection(visObject, sel.DocName.c_str(), sel.FeatName.c_str(),
                                      ...
                                      const std::vector<App::DocumentObject *> &sel)
{
    ...
    for (auto obj : sel) {
        ...
        _SelObj temp;
        int ret = checkSelection(pDocName, obj->getNameInDocument(), nullptr,
                                 ResolveMode::NoResolve, temp);
        if (ret != 0) continue;
        touched = true;
        _SelList.push_back(temp);
    }
...
void SelectionSingleton::clearSelection(const char *pDocName, bool clearPreSelect)
{
    ...
    if (pDoc) {
        ...
        if (clearPreSelect && DocName == docName) rmvPreselect();
...
bool SelectionSingleton::isSelected(App::DocumentObject *pObject, const char *pSubName,
                                    ResolveMode resolve) const
{
    if (!pObject || !pObject->getNameInDocument() || !pObject->getDocument()) return false;
    _SelObj sel;
    return checkSelection(pObject->getDocument()->getName(), pObject->getNameInDocument(), pSubName,
                          resolve, sel, &_SelList)
        > 0;
}

int SelectionSingleton::checkSelection(const char *pDocName, const char *pObjectName,
                                       const char *pSubName, ResolveMode resolve, _SelObj &sel,
                                       const std::list<_SelObj> *selList) const
{
    sel.pDoc = getDocument(pDocName);
    if (!sel.pDoc) {
        if (!selList) FC_ERR("Cannot find document");
        return -1;
    }
    pDocName = sel.pDoc->getName();
    sel.DocName = pDocName;

    if (pObjectName) sel.pObject = sel.pDoc->getObject(pObjectName);
    else
        sel.pObject = nullptr;
    if (!sel.pObject) {
        if (!selList) FC_ERR("Object not found");
        return -1;
    }
    if (sel.pObject->testStatus(App::ObjectStatus::Remove)) return -1;
    if (pSubName) sel.SubName = pSubName;
    if (resolve == ResolveMode::NoResolve) TreeWidget::checkTopParent(sel.pObject, sel.SubName);
    ...
    sel.pResolvedObject = App::GeoFeature::resolveElement(
        sel.pObject, pSubName, sel.elementName, false, App::GeoFeature::Normal, nullptr, &element);
    ...
    if (sel.pResolvedObject->testStatus(App::ObjectStatus::Remove)) return -1;
    std::string subname;
    std::string prefix;
    if (pSubName && element) {
    ...
    if (!selList) selList = &_SelList;

    if (!pSubName) pSubName = "";

    for (auto &s : *selList) {
        if (s.DocName == pDocName && s.FeatName == sel.FeatName) {
            if (s.SubName == pSubName) return 1;
    ...
    if (resolve == ResolveMode::OldStyleElement) {
        for (auto &s : *selList) {
            if (s.pResolvedObject != sel.pResolvedObject) continue;
            if (!pSubName[0]) return 1;
            if (!s.elementName.first.empty()) {
                if (s.elementName.first == sel.elementName.first) return 1;
            }
    ...
    for (list<_SelObj>::const_iterator It = _SelList.begin(); It != _SelList.end(); ++It) {
        if (It->pObject == obj) {
            ...
            if (pSubName && strncmp(pSubName, It->SubName.c_str(), It->SubName.length()) == 0) {
                if (pSubName[len] == 0 || pSubName[len - 1] == '.') return It->SubName.c_str();
            }
        }
    }
    return nullptr;
...
void SelectionSingleton::slotDeletedObject(const App::DocumentObject &Obj)
{
    ...
    for (auto it = _SelList.begin(), itNext = it; it != _SelList.end(); it = itNext) {
        ...
        if (it->pResolvedObject == &Obj || it->pObject == &Obj) {
            changes.emplace_back(SelectionChanges::RmvSelection, it->DocName, it->FeatName,
                                 it->SubName, it->TypeName);
            _SelList.erase(it);
        }
    }
    if (!changes.empty()) {
    ...
    if (!_PickedList.empty()) {
        ...
        if (changed) notify(SelectionChanges(SelectionChanges::PickedListChanged));
    }
}
...
SelectionSingleton::SelectionSingleton()
    : CurrentPreselection(SelectionChanges::ClrSelection), _needPickedList(false)
{
    hx = 0;
    hy = 0;
    hz = 0;
    ...
    App::GetApplication().signalDeletedObject.connect(
        boost::bind(&Gui::SelectionSingleton::slotDeletedObject, this, bp::_1));
    signalSelectionChanged.connect(
        boost::bind(&Gui::SelectionSingleton::slotSelectionChanged, this, bp::_1));
}
...
{
    if (!_pcSingleton) _pcSingleton = new SelectionSingleton;
    return *_pcSingleton;
}
...
PyObject *SelectionSingleton::sAddSelection(PyObject * /*self*/, PyObject *args)
{
    ...
    float x = 0, y = 0, z = 0;
    if (PyArg_ParseTuple(args, "ss|sfffO!", &docname, &objname, &subname, &x, &y, &z, &PyBool_Type,
                         &clearPreselect)) {
        Selection().addSelection(docname, objname, subname, x, y, z, nullptr,
    ...
    subname = nullptr;
    x = 0, y = 0, z = 0;
    if (PyArg_ParseTuple(args, "O!|sfffO!", &(App::DocumentObjectPy::Type), &object, &subname, &x,
                         &y, &z, &PyBool_Type, &clearPreselect)) {
    ...
    PyErr_Clear();
    PyObject *sequence;
    if (PyArg_ParseTuple(args, "O!O|O!", &(App::DocumentObjectPy::Type), &object, &sequence,
                         &PyBool_Type, &clearPreselect)) {
        auto docObjPy = static_cast<App::DocumentObjectPy *>(object);
        App::DocumentObject *docObj = docObjPy->getDocumentObjectPtr();
        if (!docObj || !docObj->getNameInDocument()) {
            PyErr_SetString(Base::PyExc_FC_GeneralError, "Cannot check invalid object");
            ...
            if (PyTuple_Check(sequence) || PyList_Check(sequence)) {
                ...
                for (Py::Sequence::iterator it = list.begin(); it != list.end(); ++it) {
                    ...
                    Selection().addSelection(docObj->getDocument()->getName(),
                                             docObj->getNameInDocument(), subname.c_str(), 0, 0, 0,
                                             nullptr, Base::asBoolean(clearPreselect));
                }
                Py_Return;
            }
        }
...
PyObject *SelectionSingleton::sUpdateSelection(PyObject * /*self*/, PyObject *args)
{
    ...
    Selection().updateSelection(Base::asBoolean(show), docObj->getDocument()->getName(),
                                docObj->getNameInDocument(), subname);
...
{
    switch (value) {
        case 0: return ResolveMode::NoResolve;
        case 1: return ResolveMode::OldStyleElement;
        case 2: return ResolveMode::NewStyleElement;
        case 3: return ResolveMode::FollowLink;
        default: throw Base::ValueError("Wrong enum value");
    }
}
} // namespace
...
PyObject *SelectionSingleton::sIsSelected(PyObject * /*self*/, PyObject *args)
{
    ...
    char *subname = nullptr;
    int resolve = 1;
    if (!PyArg_ParseTuple(args, "O!|si", &(App::DocumentObjectPy::Type), &object, &subname,
                          &resolve))
        return nullptr;
...
PyObject *SelectionSingleton::sCountObjectsOfType(PyObject * /*self*/, PyObject *args)
{
    ...
    int resolve = 1;
    if (!PyArg_ParseTuple(args, "s|si", &objecttype, &document, &resolve)) return nullptr;
...
PyObject *SelectionSingleton::sGetSelection(PyObject * /*self*/, PyObject *args)
{
    ...
    try {
        ...
        for (const auto &it : sel) {
            if (noduplicates.insert(it.pObject).second) { selectedObjects.push_back(it.pObject); }
        }
...
PyObject *SelectionSingleton::sEnablePickedList(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O!", &PyBool_Type, &enable)) return nullptr;
...
PyObject *SelectionSingleton::sSetPreselection(PyObject * /*self*/, PyObject *args, PyObject *kwd)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwd, "O!|sfffi", kwlist, &(App::DocumentObjectPy::Type),
                                    &object, &subname, &x, &y, &z, &type)) {
        auto docObjPy = static_cast<App::DocumentObjectPy *>(object);
        App::DocumentObject *docObj = docObjPy->getDocumentObjectPtr();
        if (!docObj || !docObj->getNameInDocument()) {
            PyErr_SetString(Base::PyExc_FC_GeneralError, "Cannot check invalid object");
        ...
        Selection().setPreselect(docObj->getDocument()->getName(), docObj->getNameInDocument(),
...
{
    int resolve = 1;
    if (!PyArg_ParseTuple(args, "|i", &resolve)) return nullptr;
...
PyObject *SelectionSingleton::sGetSelectionEx(PyObject * /*self*/, PyObject *args)
{
    ...
    try {
        ...
        sel = Selection().getSelectionEx(documentName, App::DocumentObject::getClassTypeId(),
                                         toEnum(resolve), Base::asBoolean(single));

        Py::List list;
        for (auto &it : sel) { list.append(Py::asObject(it.getPyObject())); }
        return Py::new_reference_to(list);
    }
    catch (const Base::Exception &e) {
        e.setPyException();
...
{
    char *documentName = nullptr;
    if (!PyArg_ParseTuple(args, "|s", &documentName)) return nullptr;
...
PyObject *SelectionSingleton::sAddSelObserver(PyObject * /*self*/, PyObject *args)
{
    PyObject *o;
    int resolve = 1;
    if (!PyArg_ParseTuple(args, "O|i", &o, &resolve)) return nullptr;
...
PyObject *SelectionSingleton::sAddSelectionGate(PyObject * /*self*/, PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "O!|i", SelectionFilterPy::type_object(), &filterPy, resolve)) {
        ...
        {
            Selection().addSelectionGate(
                new SelectionFilterGatePython(static_cast<SelectionFilterPy *>(filterPy)),
                toEnum(resolve));
            Py_Return;
        }
        PY_CATCH;
...
PyObject *SelectionSingleton::sSetVisible(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O", &visible)) return nullptr;

    PY_TRY
    {
        VisibleState vis;
        if (visible == Py_None) { vis = VisToggle; }
...
PyObject *SelectionSingleton::sHasSelection(PyObject * /*self*/, PyObject *args)
{
    ...
    int resolve = 0;
    if (!PyArg_ParseTuple(args, "|sO!", &doc, &resolve)) return nullptr;

    PY_TRY
    {
        bool ret;
        if (doc || resolve > 0) ret = Selection().hasSelection(doc, toEnum(resolve));
        else
...
PyObject *SelectionSingleton::sHasSubSelection(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|sO!", &doc, &PyBool_Type, &subElement)) return nullptr;
...
PyObject *SelectionSingleton::sGetSelectionFromStack(PyObject * /*self*/, PyObject *args)
{
    ...
    int resolve = 1;
    int index = 0;
    if (!PyArg_ParseTuple(args, "|sii", &documentName, &resolve, &index)) return nullptr;

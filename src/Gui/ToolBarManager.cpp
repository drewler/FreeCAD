...
ToolBarItem::~ToolBarItem() { clear(); }

void ToolBarItem::setCommand(const std::string &name) { _name = name; }

std::string ToolBarItem::command() const { return _name; }

bool ToolBarItem::hasItems() const { return _items.count() > 0; }
...
uint ToolBarItem::count() const { return _items.count(); }

void ToolBarItem::appendItem(ToolBarItem *item) { _items.push_back(item); }
...
QList<ToolBarItem *> ToolBarItem::getItems() const { return _items; }
...
{
    if (!_instance) _instance = new ToolBarManager;
    return _instance;
}
...
{
    if (!toolBarItems) return; // empty menu bar
    ...
    for (QList<ToolBarItem *>::Iterator it = items.begin(); it != items.end(); ++it) {
        ...
        if (!toolbar) {
            toolbar =
                getMainWindow()->addToolBar(QApplication::translate("Workbench",
        ...
        if (toolbar_added) {
            if (top_width > 0 && getMainWindow()->toolBarBreak(toolbar)) top_width = 0;
...
void ToolBarManager::setup(ToolBarItem *item, QToolBar *toolbar) const
{
    ...
    for (QList<ToolBarItem *>::Iterator it = items.begin(); it != items.end(); ++it) {
        ...
        if (!action) {
            if ((*it)->command() == "Separator") { action = toolbar->addSeparator(); }
            else {
...
void ToolBarManager::saveState() const
{
    ...
    QList<QToolBar *> toolbars = toolBars();
    for (QStringList::ConstIterator it = this->toolbarNames.begin(); it != this->toolbarNames.end();
         ++it) {
...
{
    for (auto &tb : toolBars()) { tb->setMovable(moveable); }
}
...
QToolBar *ToolBarManager::findToolBar(const QList<QToolBar *> &toolbars, const QString &item) const
{
    for (QList<QToolBar *>::ConstIterator it = toolbars.begin(); it != toolbars.end(); ++it) {
        if ((*it)->objectName() == item) return *it;
    }
...
QList<QToolBar *> ToolBarManager::toolBars() const
{
    ...
    for (QList<QToolBar *>::Iterator it = bars.begin(); it != bars.end(); ++it) {
        if ((*it)->parentWidget() == mw) tb.push_back(*it);
    }

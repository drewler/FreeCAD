...
static inline bool appendPathSafe(SoPath *path, SoNode *node)
{
    if (path->getLength()) {
        ...
        if (!children || children->find((void *)node) < 0) return false;
    }
    path->append(node);
    return true;
}
...
#define appendPath(_path, _node)                                                                   \
    do {                                                                                           \
        if (!appendPathSafe(_path, _node)) FC_ERR("LinkView: coin path error");                    \
    } while (0)
#else
...
public:
    ...
    static Pointer get(ViewProviderDocumentObject *vp, LinkOwner *owner)
    {
        ...
        if (owner) ext->linkInfo->links.insert(owner);
        return ext->linkInfo;
    }

    static void sensorCB(void *data, SoSensor *) { static_cast<LinkInfo *>(data)->update(); }
    ...
    LinkInfo(ViewProviderDocumentObject *vp) : ref(0), pcLinked(vp)
    {
        FC_LOG("new link to " << pcLinked->getObject()->getFullName());
        connChangeIcon = vp->signalChangeIcon.connect(boost::bind(&LinkInfo::slotChangeIcon, this));
        vp->forceUpdate(true);
    ...
    void remove(LinkOwner *owner) { links.erase(owner); }

    bool isLinked() const
    {
        return pcLinked && pcLinked->getObject() && pcLinked->getObject()->getNameInDocument();
    }

    const char *getLinkedName() const { return pcLinked->getObject()->getNameInDocument(); }

    const char *getLinkedLabel() const { return pcLinked->getObject()->Label.getValue(); }

    const char *getLinkedNameSafe() const
    {
        if (isLinked()) return getLinkedName();
        return "<nil>";
    }

    const char *getDocName() const { return pcLinked->getDocument()->getDocument()->getName(); }
    ...
    void detach(bool unlink)
    {
        ...
        for (const auto &node : pcSnapshots) {
            if (node) coinRemoveAllChildren(node);
        }
    ...
    void updateSwitch(SoSwitch *node = nullptr)
    {
        ...
        for (size_t i = 0; i < pcSwitches.size(); ++i) {
            if (!pcSwitches[i] || (node && node != pcSwitches[i])) continue;
    ...
    bool isVisible() const
    {
        ...
        for (int idx : indices) {
            if (!pcSwitches[idx]) continue;
            if (pcSwitches[idx]->whichChild.getValue() == -1) return false;
        }
        return true;
    }
    ...
    void setVisible(bool visible)
    {
        ...
        for (int idx : indices) {
            if (!pcSwitches[idx]) continue;
            if (!visible) pcSwitches[idx]->whichChild = -1;
    ...
    {
        if (type < 0 || type >= LinkView::SnapshotMax) return nullptr;

        SoSeparator *root;
        if (!isLinked() || !(root = pcLinked->getRoot())) return nullptr;
        ...
        if (pcSnapshot) {
            ...
            if (ViewParams::instance()->getUseSelectionRoot()) pcSnapshot = new SoFCSelectionRoot;
            else
        ...
        for (int i = 0, count = root->getNumChildren(); i < count; ++i) {
            ...
            if (node == pcLinked->getTransformNode()) {
                if (type != LinkView::SnapshotTransform) pcSnapshot->addChild(node);
                else {
            ...
            for (int i = 0, count = pcLinkedSwitch->getNumChildren(); i < count; ++i) {
                ...
                if (pcChildGroup && child == childRoot) pcModeSwitch->addChild(pcChildGroup);
                else
    ...
    void update()
    {
        if (!isLinked() || pcLinked->isRestoring()) return;

        updateChildren();

        for (size_t i = 0; i < pcSnapshots.size(); ++i)
            if (pcSnapshots[i]) getSnapshot(i, true);
    }
    ...
    void updateChildren()
    {
        ...
        if (!pcLinked->getChildRoot()) {
            childSensor.detach();
            if (pcChildGroup) coinRemoveAllChildren(pcChildGroup);
            return;
        }
        ...
        if (!pcChildGroup) pcChildGroup = new SoGroup;
        else
            coinRemoveAllChildren(pcChildGroup);
    ...
    {
        if (!pp || !isLinked() || !pcLinked->isSelectable()) return false;

        if (addname) str << getLinkedName() << '.';
        ...
        if (pcChildGroup && pcSwitch && pcSwitch->whichChild.getValue() >= 0
            ...
            std::string subname;
            if (!pcLinked->getElementPicked(pp, subname)) return false;
            str << subname;
        }
        return true;
    }
...
#define CHECK_NAME(_name, _end)                                                                    \
    do {                                                                                           \
        ...
        for (; *subname && *_n; ++subname, ++_n)                                                   \
            if (*subname != *_n) break;                                                            \
        if (*_n || (*subname != 0 && *subname != _end)) return 0;                                  \
    ...
    bool getDetail(bool checkname, int type, const char *subname, SoDetail *&det,
                   SoFullPath *path) const
    {
        ...
        if (pcSnapshots[type]->findChild(pcSwitches[type]) < 0) {
            if (path) {
                if (!appendPathSafe(path, pcSnapshots[type])) return false;
            }
        ...
        if (path) {
            ...
            if (pcLinked->getChildRoot()) type = LinkView::SnapshotChild;
            else
        ...
        while (1) {
            ...
            if (Data::ComplexGeoData::isMappedElement(dot + 1)) break;
        ...
        for (const auto &v : nodeMap) {
            if (v.second->getDetail(true, type, subname, det, path)) return true;
        }
        if (path) path->truncate(len);
        return false;
    }
    ...
    void slotChangeIcon()
    {
        ...
        if (!isLinked()) return;
        LinkInfoPtr me(this);
        for (auto link : links) link->onLinkedIconChange(me);
    }
    ...
    QIcon getIcon(QPixmap px)
    {
        ...
        if (px.isNull()) return pcLinked->getIcon();
        ...
        if (iconLink.isNull()) {
            ...
            iconLink.addPixmap(
                BitmapFactory().merge(icon.pixmap(iconSize, iconSize, QIcon::Normal, QIcon::Off),
                                      px, BitmapFactoryInst::BottomLeft),
                QIcon::Normal, QIcon::Off);
            iconLink.addPixmap(
                BitmapFactory().merge(icon.pixmap(iconSize, iconSize, QIcon::Normal, QIcon::On), px,
...
#ifdef _MSC_VER
void intrusive_ptr_add_ref(Gui::LinkInfo *px) { px->addref(); }
...
{
    if (linkInfo) return linkInfo->isVisible();
    return true;
}

void ViewProviderLinkObserver::setLinkVisible(bool visible)
{
    if (linkInfo) linkInfo->setVisible(visible);
}

void ViewProviderLinkObserver::extensionBeforeDelete()
{
    if (linkInfo) linkInfo->detach(false);
}
...
void ViewProviderLinkObserver::extensionOnChanged(const App::Property *prop) { (void)prop; }
...
void ViewProviderLinkObserver::extensionModeSwitchChange()
{
    ...
    if (owner && linkInfo) linkInfo->updateSwitch();
}
...
public:
    ...
    {
        if (isLinked() && linkInfo->pcLinked->getObject() == obj) return;
        unlink();
        linkInfo = LinkInfo::get(obj, this);
        if (linkInfo) pcNode->addChild(linkInfo->getSnapshot(LinkView::SnapshotTransform));
    }

    bool isLinked() const { return linkInfo && linkInfo->isLinked(); }
};
...
public:
    ...
    void link(App::DocumentObject *obj)
    {
        ...
        unlink();
        linkInfo = LinkInfo::get(obj, this);
        if (isLinked()) pcRoot->addChild(linkInfo->getSnapshot(handle.childType));
    }
...
LinkView::LinkView() : nodeType(SnapshotTransform), childType((SnapshotType)-1), autoSubLink(true)
{
...
{
    if (PythonObject.is(Py::_None())) PythonObject = Py::Object(new LinkViewPy(this), true);
...
Base::BoundBox3d _getBoundBox(ViewProviderDocumentObject *vpd, SoNode *rootNode)
{
    ...
    if (!view) LINK_THROW(Base::RuntimeError, "no view");
...
{
    if (!vpd) {
        if (!linkOwner || !linkOwner->isLinked()) LINK_THROW(Base::ValueError, "no ViewProvider");
...
{
    if (linkOwner && linkOwner->isLinked()) return linkOwner->pcLinked;
    return nullptr;
}
...
bool LinkView::isLinked() const { return linkInfo && linkInfo->isLinked(); }
...
void LinkView::setDrawStyle(int style, double lineWidth, double pointSize)
{
    ...
    switch (style) {
        case 2: pcDrawStyle->linePattern = 0xf00f; break;
        case 3: pcDrawStyle->linePattern = 0x0f0f; break;
        case 4: pcDrawStyle->linePattern = 0xff88; break;
        default: pcDrawStyle->linePattern = 0xffff;
    }
...
void LinkView::setMaterial(int index, const App::Material *material)
{
    if (index < 0) {
        ...
        for (int i = 0; i < getSize(); ++i) setMaterial(i, nullptr);
    }
                                 ...
                                 const std::vector<std::string> &subs)
{
    ...
    for (const auto &sub : subs) {
        ...
        if (subelement[0]) it->second->subElements.insert(subelement);
    }
    updateLink();
}
...
void LinkView::setTransform(SoTransform *pcTransform, const Base::Matrix4D &mat)
{
    ...
    pcTransform->setMatrix(SbMatrix(dMtrx[0], dMtrx[1], dMtrx[2], dMtrx[3], dMtrx[4], dMtrx[5],
                                    dMtrx[6], dMtrx[7], dMtrx[8], dMtrx[9], dMtrx[10], dMtrx[11],
...
void LinkView::setSize(int _size)
{
    ...
    if (childType < 0 && size == nodeArray.size()) return;
    resetRoot();
    if (!size || childType >= 0) {
        nodeArray.clear();
        nodeMap.clear();
        if (!size && childType < 0) {
            if (pcLinkedRoot) pcLinkRoot->addChild(pcLinkedRoot);
            return;
        }
        childType = SnapshotContainer;
    }
    if (size < nodeArray.size()) {
        for (size_t i = size; i < nodeArray.size(); ++i) nodeMap.erase(nodeArray[i]->pcSwitch);
        nodeArray.resize(size);
    }
    for (const auto &info : nodeArray) pcLinkRoot->addChild(info->pcSwitch);
    ...
    while (nodeArray.size() < size) {
        ...
        if (pcLinkedRoot) info.pcRoot->addChild(pcLinkedRoot);
...
void LinkView::resetRoot()
{
    ...
    if (pcTransform) pcLinkRoot->addChild(pcTransform);
    if (pcShapeHints) pcLinkRoot->addChild(pcShapeHints);
    if (pcDrawStyle) pcLinkRoot->addChild(pcDrawStyle);
}
                           ...
                           const boost::dynamic_bitset<> &vis, SnapshotType type)
{
    ...
    if (type < 0 || type >= SnapshotMax) LINK_THROW(Base::ValueError, "invalid children type");

    resetRoot();

    if (childType < 0) nodeArray.clear();
    childType = type;

    if (nodeArray.size() > children.size()) nodeArray.resize(children.size());
    else
...
std::vector<ViewProviderDocumentObject *> LinkView::getChildren() const
{
    ...
    for (const auto &info : nodeArray) {
        if (info->isLinked()) ret.push_back(info->linkInfo->pcLinked);
    }
    return ret;
}
...
{
    auto link = linkInfo;
    if (autoSubLink && subInfo.size() == 1) link = subInfo.begin()->second->linkInfo;
...
std::vector<std::string> LinkView::getSubNames() const
{
    ...
    for (const auto &v : subInfo) {
        ...
        for (const auto &s : v.second->subElements) ret.push_back(v.first + s);
    }
    return ret;
}
...
void LinkView::setNodeType(SnapshotType type, bool sublink)
{
    ...
    if (nodeType >= 0 && type < 0) {
        ...
        if (isLinked()) replaceLinkedRoot(linkInfo->getSnapshot(type));
        else
...
void LinkView::replaceLinkedRoot(SoSeparator *root)
{
    ...
    if (nodeArray.empty()) {
        if (pcLinkedRoot && root) pcLinkRoot->replaceChild(pcLinkedRoot, root);
        else if (root)
        ...
        if (pcLinkedRoot && root) {
            for (const auto &info : nodeArray) info->pcRoot->replaceChild(pcLinkedRoot, root);
        }
        else if (root) {
            for (const auto &info : nodeArray) info->pcRoot->addChild(root);
        }
        else {
            for (const auto &info : nodeArray) info->pcRoot->removeChild(pcLinkedRoot);
        }
    }
    pcLinkedRoot = root;
}
...
{
    if (info != linkInfo || !linkOwner || !linkOwner->isLinked() || info == linkOwner) return;
...
void LinkView::updateLink()
{
    ...
    if (!linkedRoot) linkedRoot = new SoFCSelectionRoot;
    else {
        SoSelectionElementAction action(SoSelectionElementAction::None, true);
    ...
    for (const auto &v : subInfo) {
        ...
        if (!sub.subElements.empty()) {
            ...
            for (const auto &subelement : sub.subElements) {
                ...
                if (!sub.linkInfo->getDetail(false, SnapshotTransform, subelement.c_str(), det,
                                             &path))
                    continue;
...
bool LinkView::linkGetElementPicked(const SoPickedPoint *pp, std::string &subname) const
{
    ...
    if (!nodeArray.empty()) {
        ...
        if (idx < 0 || idx + 2 >= path->getLength()) return false;
        auto node = path->getNode(idx + 1);
        auto it = nodeMap.find(node);
        if (it == nodeMap.end() || !isElementVisible(it->second)) return false;
        ...
        while (nodeArray[nodeIdx]->isGroup) {
            ...
            idx += 2;
            if (idx >= path->getLength()) return false;
            auto iter = nodeMap.find(path->getNode(idx));
            if (iter == nodeMap.end() || !isElementVisible(iter->second)) return false;
            nodeIdx = iter->second;
        }
        auto &info = *nodeArray[nodeIdx];
        if (nodeIdx == it->second) ss << it->second << '.';
        else
            ss << info.linkInfo->getLinkedName() << '.';
        if (info.isLinked()) {
            if (!info.linkInfo->getElementPicked(false, childType, pp, ss)) return false;
            subname = ss.str();
            return true;
        }
    }
    ...
    for (const auto &v : subInfo) {
        ...
        if (!sub.linkInfo->getElementPicked(false, SnapshotTransform, pp, ss2)) return false;
        ...
        if (!autoSubLink || subInfo.size() > 1) ss << v.first;
        ss << element;
        subname = ss.str();
        return true;
    }
...
{
    if (index > (int)nodeArray.size()) return false;
    auto &info = *nodeArray[index];
    if (info.groupIndex >= 0 && !getGroupHierarchy(info.groupIndex, path)) return false;
...
bool LinkView::linkGetDetailPath(const char *subname, SoFullPath *path, SoDetail *&det) const
{
    ...
    if (nodeArray.empty()) {
        if (!appendPathSafe(path, pcLinkRoot)) return false;
    }
    else {
        int idx = -1;
        if (subname[0] >= '0' && subname[0] <= '9') {
            ...
            while (1) {
                ...
                subname = dot + 1;
                if (!subname[0] || nodeArray[idx]->isGroup == 0) break;
                idx = -1;
            }
        }

        if (idx < 0 || idx >= (int)nodeArray.size()) return false;
    ...
    if (isLinked()) {
        if (nodeType >= 0) {
            if (linkInfo->getDetail(false, nodeType, subname, det, path)) return true;
        }
        else {
            appendPath(path, pcLinkedRoot);
            for (const auto &v : subInfo) {
                auto &sub = *v.second;
                if (!sub.isLinked()) continue;
                const char *nextsub;
                if (autoSubLink && subInfo.size() == 1) nextsub = subname;
                else {
                    if (!boost::algorithm::starts_with(subname, v.first)) continue;
...
QIcon LinkView::getLinkedIcon(QPixmap px) const
{
    auto link = linkInfo;
    if (autoSubLink && subInfo.size() == 1) link = subInfo.begin()->second->linkInfo;
    if (!link || !link->isLinked()) return QIcon();
    return link->getIcon(px);
}

bool LinkView::hasSubs() const { return isLinked() && !subInfo.empty(); }
...
ViewProviderLink::ViewProviderLink()
    : linkType(LinkTypeNone), hasSubName(false), hasSubElement(false), useCenterballDragger(false),
      childVp(nullptr), overlayCacheKey(0)
{
    ...
    ADD_PROPERTY_TYPE(OverrideMaterial, (false), " Link", App::Prop_None,
...
ViewProviderLink::~ViewProviderLink() { linkView->setInvalid(); }

bool ViewProviderLink::isSelectable() const { return !pcDragger && Selectable.getValue(); }
...
void ViewProviderLink::attach(App::DocumentObject *pcObj)
{
    ...
    checkIcon();
    if (pcObj->isDerivedFrom(App::LinkElement::getClassTypeId())) hide();
...
{
    linkView->setOwner(this);
    if (childVp) childVp->reattach(obj);
...
void ViewProviderLink::onChanged(const App::Property *prop)
{
    if (prop == &ChildViewProvider) {
            ...
            if (strcmp(childVp->getTypeId().getName(), getObject()->getViewProviderName()) != 0
                ...
                if (pcModeSwitch->getNumChildren() > 1) {
                    ...
                    pcModeSwitch->replaceChild(
                        1, childVpLink->getSnapshot(LinkView::SnapshotTransform));
                }
            }
        }
    }
        ...
        if (prop == &OverrideMaterial || prop == &ShapeMaterial || prop == &MaterialList
            ...
            if (!DrawStyle.getValue()) linkView->setDrawStyle(0);
            else
                linkView->setDrawStyle(DrawStyle.getValue(), LineWidth.getValue(),
                                       PointSize.getValue());
        }
    }
...
bool ViewProviderLink::setLinkType(App::LinkBaseExtension *ext)
{
    ...
    LinkType type;
    if (hasSubName) type = LinkTypeSubs;
    else
        type = LinkTypeNormal;
    if (linkType != type) linkType = type;
    switch (type) {
        case LinkTypeSubs:
...
{
    if (!pcObject || !pcObject->getNameInDocument()) return nullptr;
...
{
    if (childVp) childVp->updateData(prop);
...
{
    if (!prop) return;
    if (prop == &ext->_ChildCache) { updateElementList(ext); }
    else if (prop == &ext->_LinkTouched) {
        if (linkView->hasSubs()) linkView->updateLink();
        applyColors();
        checkIcon(ext);
    }
    else if (prop == ext->getColoredElementsProperty()) {
        if (!prop->testStatus(App::Property::User3)) applyColors();
    }
    else if (prop == ext->getScaleProperty() || prop == ext->getScaleVectorProperty()) {
        if (!prop->testStatus(App::Property::User3)) {
            const auto &v = ext->getScaleVector();
            if (canScale(v)) pcTransform->scaleFactor.setValue(v.x, v.y, v.z);
        ...
        if (!prop->testStatus(App::Property::User3)) {
            ...
            if (subs.empty() && !sub.empty()) subs.push_back(sub);
            ...
            if (ext->_getShowElementValue()) updateElementList(ext);
            else
        ...
        if (!ext->_getShowElementValue()) {
            ...
            if (linked && linked->getDocument() == getDocument()) linked->hide();
            ...
            if (!elements.empty()) {
                ...
                if (!overrideMaterial) overrideMaterials.clear();
                ...
                if (!hasMaterial) materials.clear();
        ...
        if (!prop->testStatus(App::Property::User3) && linkView->getSize()
            ...
            if (propPlacements && linkView->getSize()) {
                ...
                if (touched.empty()) {
                    ...
                    for (int i : touched) {
                        if (i < 0 || i >= linkView->getSize()) continue;
                        Base::Matrix4D mat;
                        if (propPlacements && propPlacements->getSize() > i)
                            mat = (*propPlacements)[i].toMatrix();
                        if (propScales && propScales->getSize() > i && canScale((*propScales)[i])) {
        ...
        for (size_t i = 0; i < (size_t)linkView->getSize(); ++i) {
            if (vis.size() > i) linkView->setElementVisible(i, vis[i]);
            else
...
void ViewProviderLink::updateElementList(App::LinkBaseExtension *ext)
{
    ...
    if (OverrideMaterialList.getSize() || MaterialList.getSize()) {
        ...
        for (auto obj : elements) {
            ...
            if (MaterialList.getSize() > i) vp->ShapeMaterial.setValue(MaterialList[i]);
        }
...
void ViewProviderLink::checkIcon(const App::LinkBaseExtension *ext)
{
    ...
    if (element) icon = _LinkElementIcon;
...
{
    if (OverrideMaterial.getValue()) linkView->setMaterial(-1, &ShapeMaterial.getValue());
    else {
        for (int i = 0; i < linkView->getSize(); ++i) {
...
void ViewProviderLink::finishRestoring()
{
    ...
    if (ext->getLinkPlacementProperty()) updateDataPrivate(ext, ext->getLinkPlacementProperty());
    else
        updateDataPrivate(ext, ext->getPlacementProperty());
    updateDataPrivate(ext, ext->_getElementCountProperty());
    if (ext->getPlacementListProperty()) updateDataPrivate(ext, ext->getPlacementListProperty());
    else
    ...
    if (childVp) childVp->finishRestoring();
}
...
{
    if (!ext) ext = getLinkExtension();
    auto obj = ext && real ? ext->getTrueLinkedObject(true) : getObject()->getLinkedObject(true);
    if (obj && obj != getObject()) return Application::Instance->getViewProvider(obj);
    return nullptr;
}
...
{
    auto ext = getLinkExtension();
    if (isGroup(ext)) return true;
    if (hasElements(ext)) return false;
    auto linked = getLinkedView(false, ext);
    if (linked) return linked->canDragObject(obj);
    return false;
}
...
bool ViewProviderLink::canDragObjects() const
{
    ...
    if (hasElements(ext)) return false;
    auto linked = getLinkedView(false, ext);
    if (linked) return linked->canDragObjects();
    return false;
}
...
void ViewProviderLink::dragObject(App::DocumentObject *obj)
{
    ...
    if (hasElements(ext)) return;
    auto linked = getLinkedView(false);
    if (linked) linked->dragObject(obj);
}
...
bool ViewProviderLink::canDropObjects() const
{
    ...
    if (isGroup(ext)) return true;
    if (hasElements(ext)) return false;
    if (hasSubElement) return true;
    else if (hasSubName)
        return false;
    auto linked = getLinkedView(false, ext);
    if (linked) return linked->canDropObjects();
    return true;
}

bool ViewProviderLink::canDropObjectEx(App::DocumentObject *obj, App::DocumentObject *owner,
                                       const char *subname,
                                       const std::vector<std::string> &subElements) const
{
    if (pcObject == obj || pcObject == owner) return false;
    auto ext = getLinkExtension();
    if (isGroup(ext)) return true;
    if (!ext || !ext->getLinkedObjectProperty() || hasElements(ext)) return false;
    if (!hasSubName && linkView->isLinked()) {
        auto linked = getLinkedView(false, ext);
        if (linked) {
            auto linkedVdp = freecad_dynamic_cast<ViewProviderDocumentObject>(linked);
            if (linkedVdp) {
                if (linkedVdp->getObject() == obj || linkedVdp->getObject() == owner) return false;
            }
...
std::string ViewProviderLink::dropObjectEx(App::DocumentObject *obj, App::DocumentObject *owner,
                                           const char *subname,
                                           const std::vector<std::string> &subElements)
{
    ...
    if (!ext->getLinkedObjectProperty() || hasElements(ext)) return std::string();

    if (!hasSubName) {
        auto linked = getLinkedView(false, ext);
        if (linked) return linked->dropObjectEx(obj, owner, subname, subElements);
    }
    if (owner) {
        if (!ext->getSubElements().empty()) ext->setLink(-1, owner, subname, subElements);
        else
...
bool ViewProviderLink::canDragAndDropObject(App::DocumentObject *obj) const
{
    ...
    if (!ext->getLinkedObjectProperty() || hasElements(ext)) return false;
    if (!hasSubName) {
        auto linked = getLinkedView(false, ext);
        if (linked) return linked->canDragAndDropObject(obj);
    }
    return false;
}
...
bool ViewProviderLink::getElementPicked(const SoPickedPoint *pp, std::string &subname) const
{
    ...
    if (childVpLink && childVp) {
        ...
        if (idx >= 0) return childVp->getElementPicked(pp, subname);
    }
...
bool ViewProviderLink::getDetailPath(const char *subname, SoFullPath *pPath, bool append,
                                     SoDetail *&det) const
{
    auto ext = getLinkExtension();
    if (!ext) return false;
        ...
        if (auto linked = ext->getLinkedObjectValue()) {
            if (const char *dot = strchr(subname, '.')) {
                if (subname[0] == '$') {
                    ...
                    if (!boost::equals(sub, linked->Label.getValue())) dot = nullptr;
                }
                else {
                    CharRange sub(subname, dot);
                    if (!boost::equals(sub, linked->getNameInDocument())) dot = nullptr;
                }
                if (dot && linked->getSubObject(dot + 1)) subname = dot + 1;
            }
        }
    ...
    if (linkView->linkGetDetailPath(subname, pPath, det)) return true;
    pPath->truncate(len);
    return false;
}
...
{
    auto element = freecad_dynamic_cast<App::LinkElement>(getObject());
    if (element && !element->canDelete()) return false;
    auto ext = getLinkExtension();
    if (ext->isLinkMutated()) {
        ...
        if (linked->getDocument() == doc) {
            ...
            for (const auto &name : objs) doc->removeObject(name.c_str());
        }
    }
    return true;
}
...
{
    auto ext = getLinkExtension();
    if (isGroup(ext) || hasElements(ext) || hasSubElement) return true;
    auto linked = getLinkedView(false, ext);
    if (linked) return linked->canDelete(obj);
    return false;
}
...
{
    if (linkEdit()) return linkView->getLinkedView()->doubleClicked();
        ...
        if (src && !ext->getOnChangeCopyObjects(nullptr, src).empty()) {
            ...
            func->trigger(act, [ext]() {
                try {
                    ...
                    if (applyAll) options |= App::Link::OnChangeCopyOptions::ApplyAll;
                    ...
                    for (auto obj : sels) ext->setOnChangeCopyObject(obj, options);
                    if (!applyAll) ext->monitorOnChangeCopyObjects(ext->getOnChangeCopyObjects());
                    else {
...
void ViewProviderLink::_setupContextMenu(App::LinkBaseExtension *ext, QMenu *menu,
            ...
            if (feature.hasAttr(fname)) {
                ...
                if (ret.isTuple()) {
                    ...
                    if (!PyArg_ParseTuple(ret.ptr(), "O!O!O!", &Base::MatrixPy::Type, &pymat,
                                          &Base::PlacementPy::Type, &pypla, &Base::BoundBoxPy::Type,
                                          &pybbox)) {
...
ViewProvider *ViewProviderLink::startEditing(int mode)
{
    if (mode == ViewProvider::Color) {
        ...
        if (!ext || !ext->getColoredElementsProperty()) {
            if (linkEdit(ext)) return linkView->getLinkedView()->startEditing(mode);
        }
    ...
    if (mode == ViewProvider::Transform) {
        if (_pendingTransform && doc) doc->setEditingTransform(_editingTransform);

        if (!initDraggingPlacement()) return nullptr;
        if (useCenterballDragger) pcDragger = CoinPtr<SoCenterballDragger>(new SoCenterballDragger);
        else
...
bool ViewProviderLink::setEdit(int ModNum)
{
    if (ModNum == ViewProvider::Color) {
        auto ext = getLinkExtension();
        if (!ext || !ext->getColoredElementsProperty()) return false;
...
Base::Placement ViewProviderLink::currentDraggingPlacement() const
{
    ...
    if (!pcDragger) return Base::Placement();
...
{
    if (enable == useCenterballDragger) return;
    if (pcDragger) LINK_THROW(Base::RuntimeError, "Cannot change dragger during dragging");
...
bool ViewProviderLink::callDraggerProxy(const char *fname, bool update)
{
    ...
    try {
        ...
        if (proxy && proxy->getTypeId() == App::PropertyPythonObject::getClassTypeId()) {
            ...
            if (feature.hasAttr(fname)) {
                Py::Callable method(feature.getAttr(fname));
                Py::Tuple args;
                if (method.apply(args).isTrue()) return true;
            }
        }
    }
    catch (Py::Exception &) {
    ...
    if (update) {
        ...
        if (ext) {
            ...
            if (!prop) prop = ext->getPlacementProperty();
            if (prop) {
                auto plaNew = pla * Base::Placement(dragCtx->mat);
                if (prop->getValue() != plaNew) prop->setValue(plaNew);
            }
...
void ViewProviderLink::updateLinks(ViewProvider *vp)
{
    try {
        auto ext = vp->getExtensionByType<ViewProviderLinkObserver>(true);
        if (ext && ext->linkInfo) ext->linkInfo->update();
    }
...
{
    if (!pyViewObject) pyViewObject = new ViewProviderLinkPy(this);
    pyViewObject->IncRef();
    return pyViewObject;
}

PyObject *ViewProviderLink::getPyLinkView() { return linkView->getPyObject(); }
...
std::map<std::string, App::Color> ViewProviderLink::getElementColors(const char *subname) const
{
    ...
    auto ext = getLinkExtension();
    if (!ext || !ext->getColoredElementsProperty()) return colors;
    ...
    if (wildcard == "Face" || wildcard == "Face*" || wildcard.empty()) {
        if (wildcard.size() == 4 || OverrideMaterial.getValue()) {
            ...
            colors["Face"] = c;
            if (wildcard.size() == 4) return colors;
        }
        if (!wildcard.empty()) wildcard.resize(4);
    }
    ...
    if (!wildcard.empty()) {
        for (const auto &sub : subs) {
            ...
            if (boost::starts_with(element, wildcard)) colors[sub.second] = OverrideColorList[i];
            else if (!element[0] && wildcard == "Face")
                colors[sub.second.substr(0, element - sub.second.c_str()) + wildcard] =
                    OverrideColorList[i];
        }
        ...
        while (1) {
            ...
            if (!link || link == vp->getObject()) break;
            auto next = freecad_dynamic_cast<ViewProviderLink>(
                Application::Instance->getViewProvider(link));
            if (!next) break;
            for (const auto &v : next->getElementColors(subname)) colors.insert(v);
            vp = next;
        }
    ...
    for (const auto &sub : subs) {
        ...
        if (offset) colors.emplace(std::string(subname, offset) + sub.second, OverrideColorList[i]);
        else
            colors[sub.second] = OverrideColorList[i];
    }

    if (!subname[0]) return colors;
    ...
    for (const auto &v : colors) {
        ...
        if (!sobj || !pos) continue;
        auto link = sobj->getLinkedObject(true);
        if (!link || link == getObject()) continue;
        ...
        for (const auto &v2 : vp->getElementColors(!pos[0] ? "Face" : pos)) {
            std::string name;
            if (pos[0]) name = v.first.substr(0, pos - v.first.c_str()) + v2.first;
            else
                name = v.first;
...
{
    auto ext = getLinkExtension();
    if (!ext || !ext->getColoredElementsProperty()) return;
    ...
    for (auto &v : subMap) {
        if (element_count == (int)v.second.size()) {
            ...
            for (auto it = v.second.begin(); it != v.second.end();) {
                if (it->second == firstColor) it = v.second.erase(it);
                else
                    ++it;
            }
        }
...
void ViewProviderLink::applyColors()
{
    ...
    for (const auto &v : colors) {
        ...
        if (!sobj || !element) continue;
        if (ViewProvider::hiddenMarker() == element) hideList.emplace(subname, element - subname);
        else
    ...
    for (auto &v : colorMap) {
        ...
        SoDetail *det = nullptr;
        path.truncate(0);
        if (getDetailPath(v.first.c_str(), &path, false, det)) action.apply(&path);
        delete det;
    }
    ...
    for (const auto &sub : hideList) {
        SoDetail *det = nullptr;
        path.truncate(0);
        if (!sub.empty() && getDetailPath(sub.c_str(), &path, false, det)) action.apply(&path);
        delete det;
    }
    path.unrefNoDelete();
}
...
void ViewProviderLink::setOverrideMode(const std::string &mode)
{
    ...
    if (childVp) childVp->setOverrideMode(mode);
}
...
#define CHECK_EXCLUDE_PROP(_name)                                                                  \
    if (strcmp(name, #_name) == 0) return true;
...
App::Property *ViewProviderLink::getPropertyByName(const char *name) const
{
    ...
    if (prop || isExcludedProperties(name)) return prop;
    if (childVp) {
        prop = childVp->getPropertyByName(name);
        if (prop && !prop->testStatus(App::Property::Hidden)) return prop;
        prop = nullptr;
    }
    if (pcObject && pcObject->canLinkProperties()) {
        auto linked = getLinkedViewProvider(nullptr, true);
        if (linked && linked != this) prop = linked->getPropertyByName(name);
    }
    return prop;
}
...
void ViewProviderLink::getPropertyMap(std::map<std::string, App::Property *> &Map) const
{
    ...
    for (const auto &v : childMap) {
        ...
        if (!ret.second) {
            ...
            if (myProp->testStatus(App::Property::Hidden)) ret.first->second = v.second;
        }
    }
}
...
void ViewProviderLink::getPropertyList(std::vector<App::Property *> &List) const
{
    ...
    for (const auto &v : Map) List.push_back(v.second);
}

...
{
    if (!_instance) _instance = new ToolBoxManager;
    return _instance;
}
...
void ToolBoxManager::setToolBox(DockWnd::ToolBox *tb) { _toolBox = tb; }

void ToolBoxManager::setup(ToolBarItem *toolBar) const
{
    if (!toolBar || !_toolBox) return; // empty tool bar
    ...
    for (QList<ToolBarItem *>::Iterator item = items.begin(); item != items.end(); ++item) {
        ...
        for (QList<ToolBarItem *>::Iterator subitem = subitems.begin(); subitem != subitems.end();
             ++subitem) {

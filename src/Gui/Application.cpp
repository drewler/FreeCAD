...
public:
    void newObject(const ViewProvider &vp)
    {
        ...
        if (vpd && vpd->getObject()) map[vpd->getObject()] = vpd;
    }
    void deleteObject(const ViewProvider &vp)
    {
        auto vpd =
            Base::freecad_dynamic_cast<ViewProviderDocumentObject>(const_cast<ViewProvider *>(&vp));
        if (vpd && vpd->getObject()) map.erase(vpd->getObject());
    }
    void deleteDocument(const App::Document &doc)
    {
        for (auto obj : doc.getObjects()) map.erase(obj);
    }
...
struct ApplicationP {
    explicit ApplicationP(bool GUIenabled)
        : activeDocument(nullptr), editDocument(nullptr), isClosing(false), startingUp(true)
    {
        // create the macro manager
        if (GUIenabled) macroMngr = new MacroManager();
        else
            macroMngr = nullptr;
...
{
    PyObject *o;
    if (!PyArg_ParseTuple(args, "O!", &(App::DocumentObjectPy::Type), &o)) return nullptr;
    ...
    try {
        ...
        if (base
            ...
            proxy = Base::Interpreter().createSWIGPointerObj("pivy.coin", type.c_str(),
                                                             (void *)node, 1);
...
static PyObject *FreeCADGui_exportSubgraph(PyObject * /*self*/, PyObject *args)
{
    ...
    PyObject *proxy;
    PyObject *output;
    if (!PyArg_ParseTuple(args, "OO|s", &proxy, &output, &format)) return nullptr;
    ...
    try {
        ...
        if (node) {
            ...
            if (formatStr == "VRML") { SoFCDB::writeToVRML(node, buffer); }
...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
        ...
        if (hViewGrp->GetBool("UseVBO", false)) { (void)coin_setenv("COIN_VBO", "0", true); }
        ...
        if (QLocale().groupSeparator() == QLocale().decimalPoint()) {
            QMessageBox::critical(
                0, QLatin1String("Invalid system settings"),
                QLatin1String(
        ...
        Base::Interpreter().addType(ExpressionBindingPy::type_object(), module,
                                    "ExpressionBinding");
        ...
        Base::Interpreter().addType(&ViewProviderDocumentObjectPy::Type, module,
    ...
    for (; meth->ml_name != nullptr; meth++) {
        ...
        if (!descr) break;
        if (PyDict_SetItemString(dict, meth->ml_name, descr) != 0) break;
        Py_DECREF(descr);
    }

    SoQtOffscreenRendererPy::init_type();
    Base::Interpreter().addType(SoQtOffscreenRendererPy::type_object(), module,
    ...
    if (Module) {
        try {
            if (File.hasExtension("FCStd")) {
                ...
                if (!handled)
                    Command::doCommand(Command::App, "FreeCAD.openDocument('%s')",
                                       unicodepath.c_str());
            }
            else {
        ...
        wc.restoreCursor();
        QMessageBox::warning(
            getMainWindow(), QObject::tr("Unknown filetype"),
...
void Application::importFrom(const char *FileName, const char *DocName, const char *Module)
{
    ...
    if (Module) {
        try {
            ...
            if (File.hasExtension("FCStd")) {
                Command::doCommand(Command::App, "%s.open(u\"%s\")", Module, unicodepath.c_str());
                if (activeDocument()) activeDocument()->setModified(false);
            }
            else {
                ...
                if (doc) {
                    ...
                    if (!pendingCommand) doc->openCommand(QT_TRANSLATE_NOOP("Command", "Import"));
                }

                if (DocName) {
                    Command::doCommand(Command::App, "%s.insert(u\"%s\",\"%s\")", Module,
                                       unicodepath.c_str(), DocName);
                }
                else {
                    Command::doCommand(Command::App, "%s.insert(u\"%s\")", Module,
                                       unicodepath.c_str());
                }

                // Commit the transaction
                if (doc && !pendingCommand) { doc->commitCommand(); }
                ...
                if (doc) {
                    ...
                    if (hGrp->GetBool("AutoFitToView", true)) {
                        ...
                        if (view) {
                            ...
                            if (view->onMsg("ViewFit", &ret)) updateActions(true);
                        }
                    }
                }
            }
            ...
            if (addToRecent) { getMainWindow()->appendRecentFile(filename); }
            FileDialog::setWorkingDirectory(filename);
        }
        catch (const Base::PyException &e) {
            // Usually thrown if the file is invalid somehow
...
void Application::slotNewDocument(const App::Document &Doc, bool isMainDoc)
{
    ...
    if (isMainDoc) pDoc->createView(View3DInventor::getClassTypeId());
}
...
void Application::slotDeleteDocument(const App::Document &Doc)
{
    ...
    if (d->activeDocument == doc->second) setActiveDocument(nullptr);
...
void Application::slotRelabelObject(const ViewProvider &vp) { this->signalRelabelObject(vp); }
...
void Application::slotInEdit(const Gui::ViewProviderDocumentObject &vp) { this->signalInEdit(vp); }
...
void Application::onLastWindowClosed(Gui::Document *pcDoc)
{
    try {
        if (!d->isClosing && pcDoc) {
            ...
            Command::doCommand(Command::Doc, "App.closeDocument(\"%s\")",
                               pcDoc->getDocument()->getName());
            if (!d->activeDocument && !d->documents.empty()) {
                Document *gdoc = nullptr;
                for (auto &v : d->documents) {
                    if (v.second->getDocument()->testStatus(App::Document::TempDoc)) continue;
                    else if (!gdoc)
                        gdoc = v.second;
...
bool Application::sendHasMsgToFocusView(const char *pMsg)
{
    ...
    if (!pView) return false;
    for (auto focus = qApp->focusWidget(); focus; focus = focus->parentWidget()) {
        if (focus == pView) return pView->onHasMsg(pMsg);
    }
    return false;
}
...
{
    if (activeDocument()) return activeDocument()->getActiveView();
    else
        return nullptr;
}
...
void Application::activateView(const Base::Type &type, bool create)
{
    ...
    if (doc) {
        ...
        if (mdiView && mdiView->isDerivedFrom(type)) return;
        std::list<MDIView *> mdiViews = doc->getMDIViewsOfType(type);
        if (!mdiViews.empty()) doc->setActiveWindow(mdiViews.back());
        else if (create)
            doc->createView(type);
    }
}
...
Gui::Document *Application::activeDocument() const { return d->activeDocument; }

Gui::Document *Application::editDocument() const { return d->editDocument; }
...
{
    if (doc == d->editDocument) return;
    if (!doc) d->editDocument = nullptr;
    for (auto &v : d->documents) v.second->_resetEdit();
    d->editDocument = doc;
    updateActions();
}
...
{
    if (d->activeDocument == pcDocument) return; // nothing needs to be done
    ...
    if (pcDocument) {
        ...
        if (d->documents.find(doc) == d->documents.end()) return;
    }
...
Gui::Document *Application::getDocument(const char *name) const
{
    ...
    if (it != d->documents.end()) return it->second;
    else
        return nullptr;
}
...
void Application::attachView(Gui::BaseView *pcView) { d->passive.push_back(pcView); }

void Application::detachView(Gui::BaseView *pcView) { d->passive.remove(pcView); }

void Application::onUpdate()
{
    // update all documents
    std::map<const App::Document *, Gui::Document *>::iterator It;
    for (It = d->documents.begin(); It != d->documents.end(); ++It) It->second->onUpdate();
...
void Application::viewActivated(MDIView *pcView)
{
    ...
    if (!pcView->isPassive()) setActiveDocument(pcView->getGuiDocument());
}


void Application::updateActive() { activeDocument()->onUpdate(); }

void Application::updateActions(bool delay) { getMainWindow()->updateActions(delay); }
...
{
    if (mode.empty()) { return userEditMode; }
    for (auto const &uem : userEditModes) {
        if (uem.second == mode) { return uem.first; }
    }
    return -1;
}
...
{
    if (mode == -1) { return userEditModes.at(userEditMode); }
    if (userEditModes.find(mode) != userEditModes.end()) { return userEditModes.at(mode); }
    return "";
}
...
{
    for (auto const &uem : userEditModes) {
        if (uem.second == mode) { return setUserEditMode(uem.first); }
    }
    return false;
}
...
bool Application::activateWorkbench(const char *name)
{
    ...
    if (oldWb && oldWb->name() == name) return false; // already active
    ...
    try {
        ...
        if (!handler.hasAttr(std::string("__Workbench__"))) {
            ...
            if (Base::Type::fromName(type.c_str())
                ...
                if (!wb) throw Py::RuntimeError("Failed to instantiate workbench of type " + type);
        ...
        if (curWb && curWb->name() == name) ok = true; // already active
        ...
        if (!d->startingUp) Base::Console().Error("%s\n", e.getStackTrace().c_str());
        else
            Base::Console().Log("%s\n", e.getStackTrace().c_str());
...
QPixmap Application::workbenchIcon(const QString &wb) const
{
    ...
    if (pcWorkbench) {
        ...
        QPixmap icon;
        if (BitmapFactory().findPixmapInCache(iconName.c_str(), icon)) return icon;
        ...
        try {
            ...
            if (handler.hasAttr(std::string("Icon"))) {
                ...
                ary.resize(strlen);
                for (int j = 0; j < strlen; j++) ary[j] = content[j];
                ...
                if (!icon.isNull()) { BitmapFactory().addPixmapToCache(iconName.c_str(), icon); }
    ...
    if (!icon.isNull()) {
        ...
        if (!s.isEmpty()) return icon.pixmap(s[0]);
    }
    return QPixmap();
}
...
QStringList Application::workbenches() const
{
...
#endif
        if (hidden.isEmpty()) hidden.push_back(QLatin1String(""));
    }
    if (et != config.end()) {
...
#endif
        if (extra.isEmpty()) extra.push_back(QLatin1String(""));
    }
    ...
    while (PyDict_Next(_pcWorkbenchDictionary, &pos, &key, &value)) {
        ...
        bool ok = true;
        if (!extra.isEmpty() && ok) { ok = (extra.indexOf(QString::fromLatin1(wbName)) != -1); }
        if (!hidden.isEmpty() && ok) { ok = (hidden.indexOf(QString::fromLatin1(wbName)) == -1); }

        // okay the item is visible
        if (ok) wb.push_back(QString::fromLatin1(wbName));
...
bool Application::isClosing() { return d->isClosing; }

MacroManager *Application::macroManager() { return d->macroMngr; }

CommandManager &Application::commandManager() { return d->commandManager; }

Gui::PreferencePackManager *Application::prefPackManager() { return d->prefPackManager; }
...
void messageHandler(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    ...
    switch (type) {
        case QtInfoMsg:
        case QtDebugMsg: Base::Console().Message("%s\n", msg.toUtf8().constData()); break;
        case QtWarningMsg: Base::Console().Warning("%s\n", msg.toUtf8().constData()); break;
        case QtCriticalMsg: Base::Console().Error("%s\n", msg.toUtf8().constData()); break;
        case QtFatalMsg:
            Base::Console().Error("%s\n", msg.toUtf8().constData());
            abort(); // deliberately core dump
    }
#ifdef FC_OS_WIN32
    if (old_qtmsg_handler) (*old_qtmsg_handler)(type, context, msg);
#endif
#else
...
void messageHandlerCoin(const SoError *error, void * /*userdata*/)
{
    if (error && error->getTypeId() == SoDebugError::getClassTypeId()) {
        ...
        switch (dbg->getSeverity()) {
            case SoDebugError::INFO: Base::Console().Message("%s\n", msg); break;
            case SoDebugError::WARNING: Base::Console().Warning("%s\n", msg); break;
            default: // error
                Base::Console().Error("%s\n", msg);
                break;
        }
...
void Application::runApplication()
{
    ...
    if (useSoftwareOpenGL) { QApplication::setAttribute(Qt::AA_UseSoftwareOpenGL); }
    ...
    if (it != cfg.end()) { mainApp.setApplicationName(QString::fromUtf8(it->second.c_str())); }
    else {
    ...
    if (!name.empty()) { QIcon::setThemeName(QString::fromLatin1(name.c_str())); }
    ...
    if (showVersion) {
        ...
        QString title =
            QString::fromLatin1("%1 %2.%3").arg(mainApp.applicationName(), major, minor);
    ...
    QObject::connect(&mainApp, SIGNAL(messageReceived(const QList<QByteArray> &)), &mw,
    ...
    if (!hDocGrp->GetBool("AutoSaveEnabled", true)) timeout = 0;
    ...
    if (it != cfg.end()) {
        ...
        if (QDir(path).isRelative()) { path = QFileInfo(QDir(home), path).absoluteFilePath(); }
    ...
    if (it != cfg.end()) { hidden = true; }

    // show splasher while initializing the GUI
    if (!hidden) mw.startSplasher();
    ...
    if (style.empty()) {
        ...
        if (it != config.end()) style = it->second;
    }
...
void Application::setStyleSheet(const QString &qssFile, bool tiledBackground)
{
    ...
    if (!qssFile.isEmpty() && current != qssFile) {
        ...
        QFile f;
        if (QFile::exists(qssFile)) { f.setFileName(qssFile); }
    ...
    if (!d->startingUp) {
        if (mdi->style()) mdi->style()->unpolish(qApp);
    }
}
...
App::Document *Application::reopen(App::Document *doc)
{
    ...
    if (doc->testStatus(App::Document::PartialDoc)
        ...
        for (auto &file : docs) App::GetApplication().openDocument(file.c_str(), false);
    }

    doc = nullptr;
    for (auto &v : d->documents) {
        if (name == v.first->FileName.getValue()) doc = const_cast<App::Document *>(v.first);
        if (untouchedDocs.count(v.second)) {
            if (!v.second->isModified()) continue;
            bool reset = true;
            for (auto obj : v.second->getDocument()->getObjects()) {
                if (!obj->isTouched()) continue;

...
WindowParameter::WindowParameter(const char *name)
{
    ...
    if (strcmp(name, "") != 0) _handle = getDefaultParameter()->GetGroup(name);
}
...
{
    if (_handle.isValid()) return false; // cannot change parameter group
...
void WindowParameter::OnChange(Base::Subject<const char *> &rCaller, const char *sReason)
{
    Q_UNUSED(rCaller);
    Q_UNUSED(sReason);
    Base::Console().Log("Parameter has changed and window (%s) has not overridden this function!",
                        _handle->GetGroupName());
}

ParameterGrp::handle WindowParameter::getWindowParameter() { return _handle; }

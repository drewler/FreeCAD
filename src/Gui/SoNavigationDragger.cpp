...
void RotTransDragger::initClass() { SO_KIT_INIT_CLASS(RotTransDragger, SoDragger, "Dragger"); }
...
RotTransDragger::RotTransDragger()
{
    ...
    SO_KIT_ADD_CATALOG_ENTRY(surroundScale, SoSurroundScale, true, topSeparator, geomSeparator,
                             true);
    ...
    SO_KIT_ADD_CATALOG_ENTRY(translator, SoTranslate1Dragger, true, topSeparator, geomSeparator,
                             true);
    SO_KIT_ADD_CATALOG_ENTRY(XRotatorSep, SoSeparator, false, topSeparator, geomSeparator, false);
    SO_KIT_ADD_CATALOG_ENTRY(XRotatorRot, SoRotation, true, XRotatorSep, 0, false);
    SO_KIT_ADD_CATALOG_ENTRY(XRotator, SoRotateCylindricalDragger, true, XRotatorSep, 0, true);

    SO_KIT_ADD_CATALOG_ENTRY(YRotator, SoRotateCylindricalDragger, true, topSeparator,
                             geomSeparator, true);

    SO_KIT_ADD_CATALOG_ENTRY(ZRotatorSep, SoSeparator, false, topSeparator, geomSeparator, false);
    SO_KIT_ADD_CATALOG_ENTRY(ZRotatorRot, SoRotation, true, ZRotatorSep, 0, false);
    SO_KIT_ADD_CATALOG_ENTRY(ZRotator, SoRotateCylindricalDragger, true, ZRotatorSep, 0, true);
    ...
    if (SO_KIT_IS_FIRST_INSTANCE())
        readDefaultParts("SoNavigationDraggerLayout.iv", NavigationDraggerLayout,
    ...
    SoDragger *tDragger = SO_GET_ANY_PART(this, "translator", SoTranslate1Dragger);
    (void)tDragger;
...
SbBool RotTransDragger::setUpConnections(SbBool onOff, SbBool doItAlways)
{
    ...
    if (onOff) {
        ...
        if (rotFieldSensor->getAttachedField() != &rotation) rotFieldSensor->attach(&rotation);
    }
    else {
        ...
        if (translFieldSensor->getAttachedField()) translFieldSensor->detach();
        if (rotFieldSensor->getAttachedField()) rotFieldSensor->detach();
...
void RotTransDragger::valueChangedCB(void *, SoDragger *inDragger)
{
    ...
    if (myself->rotation.getValue() != rot) myself->rotation = rot;
    if (myself->translation.getValue() != trans) myself->translation = trans;
...
void RotTransDragger::fieldSensorCB(void *inDragger, SoSensor *)
{
    auto myself = (RotTransDragger *)inDragger;
...
void RotTransDragger::invalidateSurroundScaleCB(void *parent, SoDragger *)
{
    ...
    if (mySS) mySS->invalidate();
}

void RotTransDragger::setDefaultOnNonWritingFields()
{

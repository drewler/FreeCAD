...
void ButtonView::goChangedCommand(const QString &commandName)
{
    ...
    if (model && index.isValid()) model->setCommand(index.row(), commandName);
}
    ...
    BOOST_FOREACH (const boost::property_tree::ptree::value_type &Map,
        ...
        if ("Map" == Map.first) {
            ...
            BOOST_FOREACH (const boost::property_tree::ptree::value_type &kv,
                ...
                if (0 == Attribute.compare("Description")) { ButtonDescription = Value; }
                if (0 == Attribute.compare("KeyCode")) { ButtonCode = Value; }
                if (0 == Attribute.compare("DownTime")) { ButtonDownTime = Value; }
                if (0 == Attribute.compare("Command")) { ButtonCommand = Value; }
            }
...
void ButtonModel::load3DConnexionButtons(const char *RequiredDeviceName)
{
    try {
        ...
        if (!DeviceTree.empty()) { load3DConnexionButtonMapping(DeviceTree); }
    }
...
QVariant ButtonModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) return QVariant(getLabel(index.row()));
    ...
    if (role == Qt::SizeHintRole) return QVariant(QSize(32, 32));
    return QVariant();
}
...
void ButtonModel::goButtonPress(int number)
{
    ...
    if (!spaceballButtonGroup()->HasGroup(numberString.toLatin1())) insertButtonRows(number);
}
...
void ButtonModel::goMacroRemoved(const QByteArray &macroName)
{
    ...
    for (auto &it : groupVector)
        if (std::string(macroName.data()) == it->GetASCII("Command")) it->SetASCII("Command", "");
}
...
QString ButtonModel::getLabel(const int &number) const
{
    if (number > -1 && number < 32) {
        ...
        if (desc.length()) desc = QString::fromUtf8(" \"") + desc + QString::fromUtf8("\"");
...
CommandView::CommandView(QWidget *parent) : QTreeView(parent)
{
    ...
    connect(this, SIGNAL(clicked(const QModelIndex &)), this, SLOT(goClicked(const QModelIndex &)));
}
...
void CommandView::goChangeCommandSelection(const QString &commandName)
{
    ...
    QModelIndexList index(this->model()->match(this->model()->index(0, 0), Qt::UserRole,
                                               QVariant(commandName), 1,
...
CommandNode::~CommandNode() { qDeleteAll(children); }
...
QModelIndex CommandModel::index(int row, int column, const QModelIndex &parent) const
{
    ...
    if (!parent.isValid()) return createIndex(row, column, rootNode->children.at(row));
...
QVariant CommandModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        if (node->nodeType == CommandNode::CommandType)
            return QVariant(
                qApp->translate(node->aCommand->className(), node->aCommand->getMenuText()));
        if (node->nodeType == CommandNode::GroupType) {
            if (node->children.empty()) return QVariant();
    ...
    if (role == Qt::SizeHintRole)
        if (node->nodeType == CommandNode::CommandType) return QVariant(QSize(32, 32));
    if (role == Qt::UserRole) {
        if (node->nodeType == CommandNode::CommandType)
...
Qt::ItemFlags CommandModel::flags(const QModelIndex &index) const
{
    ...
    if (node->nodeType == CommandNode::CommandType) return Qt::ItemIsEnabled | Qt::ItemIsSelectable;
    return Qt::NoItemFlags;
}

CommandNode *CommandModel::nodeFromIndex(const QModelIndex &index) const
{
    if (index.isValid()) return static_cast<CommandNode *>(index.internalPointer());
    return rootNode;
}

void CommandModel::goAddMacro(const QByteArray &macroName)
{
    QModelIndexList indexList(this->match(this->index(0, 0), Qt::UserRole,
                                          QVariant(QString::fromLatin1("Macros")), 1,
                                          Qt::MatchWrap | Qt::MatchRecursive));
    ...
    if (indexList.empty()) {
        ...
        if (location == -1) location = groups.size();
        //add row
...
{
    QModelIndexList macroList(this->match(this->index(0, 0), Qt::UserRole,
                                          QVariant(QString::fromLatin1(macroName.data())), 1,
                                          Qt::MatchWrap | Qt::MatchRecursive));
    if (macroList.isEmpty()) return;

    QModelIndex childIndex(macroList.at(0));
    QModelIndex parentIndex(this->parent(childIndex));
    if (!childIndex.isValid() || !parentIndex.isValid()) return;
...
void CommandModel::initialize()
{
    ...
    QStringList groups(orderedGroups());
    for (const auto &group : groups) groupCommands(group);
}
...
QStringList CommandModel::orderedGroups()
{
    ...
    for (const auto &command : commands) {
        ...
        if (!groups.contains(groupName)) groups << groupName;
    }
    //how to sort?
    groups.sort();
    return groups;
...
QVariant PrintModel::data(const QModelIndex &index, int role) const
{
    ...
    if (index.column() == 1) {
        ...
        if (commandName.isEmpty()) return (QVariant());

        QModelIndexList indexList(commandModel->match(commandModel->index(0, 0), Qt::UserRole,
                                                      QVariant(commandName), 1,
                                                      Qt::MatchWrap | Qt::MatchRecursive));
        if (indexList.isEmpty()) return QVariant();
...
QVariant PrintModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    ...
    if (section == 0) return QVariant(tr("Button"));
    if (section == 1) return QVariant(tr("Command"));
    else
        return QVariant();
}
...
DlgCustomizeSpaceball::DlgCustomizeSpaceball(QWidget *parent)
    : CustomizeActionPage(parent), buttonView(nullptr), buttonModel(nullptr), commandView(nullptr),
      commandModel(nullptr), clearButton(nullptr), printReference(nullptr), devModel(nullptr)
{
    ...
    connect(buttonView, SIGNAL(changeCommandSelection(const QString &)), commandView,
            SLOT(goChangeCommandSelection(const QString &)));
    connect(commandView, SIGNAL(changedCommand(const QString &)), buttonView,
            SLOT(goChangedCommand(const QString &)));
    setupLayout();
...
void DlgCustomizeSpaceball::setupButtonModelView()
{
    ...
    connect(buttonView->selectionModel(),
            SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)), buttonView,
            SLOT(goSelectionChanged(const QItemSelection &, const QItemSelection &)));
}
...
void DlgCustomizeSpaceball::setupLayout()
{
    ...
    QString model = QString::fromStdString(App::GetApplication()
                                               ...
                                               ->GetGroup("Spaceball")
                                               ->GetASCII("Model", ""));
    if (model.length() > 0) { devModel->setCurrentIndex(devModel->findText(model)); }
    else {
...
void DlgCustomizeSpaceball::hideEvent(QHideEvent *event)
{
    ...
    if (buttonView) buttonView->selectionModel()->clear();
    if (commandView) {
...
{
    if (buttonView) buttonView->setFocus();

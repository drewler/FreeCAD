...
static void setLinkLabel(App::DocumentObject *obj, const char *doc, const char *name)
{
    ...
    Command::doCommand(Command::Doc, "App.getDocument('%s').getObject('%s').Label='%s'", doc, name,
                       label);
}
...
StdCmdLinkMakeGroup::StdCmdLinkMakeGroup() : Command("Std_LinkMakeGroup")
{
    sGroup = "Link";
...
bool StdCmdLinkMakeGroup::isActive() { return !!App::GetApplication().getActiveDocument(); }
...
void StdCmdLinkMakeGroup::activated(int option)
{
    ...
    for (auto &sel : Selection().getCompleteSelection()) {
        if (sel.pObject && sel.pObject->getNameInDocument() && objset.insert(sel.pObject).second)
    ...
    try {
        ...
        Command::doCommand(Command::Doc, "App.getDocument('%s').addObject('App::LinkGroup','%s')",
            ...
            for (auto obj : objs) {
                ...
                if (option != 0 || doc != obj->getDocument()) {
                    ...
                    Command::doCommand(Command::Doc,
                                       ...
                                       doc->getName(), name.c_str(), obj->getDocument()->getName(),
...
StdCmdLinkMake::StdCmdLinkMake() : Command("Std_LinkMake")
{
    sGroup = "Link";
...
bool StdCmdLinkMake::isActive() { return !!App::GetApplication().getActiveDocument(); }
...
void StdCmdLinkMake::activated(int)
{
    ...
    for (auto &sel : Selection().getCompleteSelection()) {
        if (sel.pObject && sel.pObject->getNameInDocument()) objs.insert(sel.pObject);
    }
...
StdCmdLinkMakeRelative::StdCmdLinkMakeRelative() : Command("Std_LinkMakeRelative")
{
    sGroup = "Link";
...
bool StdCmdLinkMakeRelative::isActive() { return Selection().hasSubSelection(nullptr, true); }
...
void StdCmdLinkMakeRelative::activated(int)
{
    ...
    try {
        ...
        for (auto &sel : Selection().getCompleteSelection(ResolveMode::NoResolve)) {
            ...
            auto key =
                std::make_pair(sel.pObject, Data::ComplexGeoData::noElementName(sel.SubName));
            auto element = Data::ComplexGeoData::findElementName(sel.SubName);
            auto &info = linkInfo[key];
            info.first = sel.pResolvedObject;
            if (element && element[0]) info.second.emplace_back(element);
        }
...
static void linkConvert(bool unlink)
{
    ...
    try {
        ...
        for (auto &v : infos) {
            ...
            if (!parent || !obj || !parentVp) continue;
            if (!recomputeSet.count(parent)) recomputeSet.emplace(parent, parent);
            auto doc = parent->getDocument();
            App::DocumentObject *replaceObj;
            if (unlink) {
                replaceObj = obj->getLinkedObject(false);
                if (!replaceObj || !replaceObj->getNameInDocument() || replaceObj == obj) continue;
            }
            else {
                auto name = doc->getUniqueObjectName("Link");
                auto link = static_cast<App::Link *>(doc->addObject("App::Link", name.c_str()));
                if (!link) FC_THROWM(Base::RuntimeError, "Failed to create link");
                link->setLink(-1, obj);
            ...
            if (pos == std::string::npos && pos) info.subname.clear();
            else {
                pos = info.subname.rfind('.', pos - 1);
                if (pos == std::string::npos) info.subname.clear();
                else {
        ...
        for (auto &v : recomputeSet) {
            ...
            if (obj) recomputes.push_back(obj);
        }
        if (!recomputes.empty()) recomputes.front()->getDocument()->recompute(recomputes);
...
static bool linkConvertible(bool unlink)
{
    ...
    for (auto &sel : TreeWidget::getSelection()) {
        ...
        if (unlink) {
            auto linked = obj->getLinkedObject(false);
            if (!linked || linked == obj) return false;
        }
        ++count;
    }
    return count != 0;
...
StdCmdLinkReplace::StdCmdLinkReplace() : Command("Std_LinkReplace")
{
    sGroup = "Link";
...
bool StdCmdLinkReplace::isActive() { return linkConvertible(false); }

void StdCmdLinkReplace::activated(int) { linkConvert(false); }

////////////////////////////////////////////////////////////////////////////////////////////

DEF_STD_CMD_A(StdCmdLinkUnlink)

StdCmdLinkUnlink::StdCmdLinkUnlink() : Command("Std_LinkUnlink")
{
    sGroup = "Link";
...
bool StdCmdLinkUnlink::isActive() { return linkConvertible(true); }

void StdCmdLinkUnlink::activated(int) { linkConvert(true); }

////////////////////////////////////////////////////////////////////////////////////////////

DEF_STD_CMD_A(StdCmdLinkImport)

StdCmdLinkImport::StdCmdLinkImport() : Command("Std_LinkImport")
{
    sGroup = "Link";
...
bool StdCmdLinkImport::isActive()
{
    ...
    for (auto &v : links) {
        if (v.first->testStatus(App::Document::PartialDoc)) return false;
    }
    return true;
}
...
void StdCmdLinkImport::activated(int)
{
    ...
    try {
        ...
        for (auto &v : getLinkImportSelections()) {
            ...
            for (auto obj : doc->importLinks(v.second)) obj->Visibility.setValue(false);
        }
        Command::commitCommand();
    }
    catch (const Base::Exception &e) {
...
StdCmdLinkImportAll::StdCmdLinkImportAll() : Command("Std_LinkImportAll")
{
    sGroup = "Link";
...
void StdCmdLinkImportAll::activated(int)
{
    ...
    try {
        ...
        auto doc = App::GetApplication().getActiveDocument();
        if (doc) {
            for (auto obj : doc->importLinks()) obj->Visibility.setValue(false);
        }
        Command::commitCommand();
    }
    catch (const Base::Exception &e) {
...
StdCmdLinkSelectLinked::StdCmdLinkSelectLinked() : Command("Std_LinkSelectLinked")
{
    sGroup = "Link";
...
static App::DocumentObject *getSelectedLink(bool finalLink, std::string *subname = nullptr)
{
    ...
    if (!linkedVp || linkedVp == vp) {
        if (sobj->getDocument() == sels[0].pObject->getDocument()) return nullptr;
        for (const char *dot = strchr(sels[0].SubName, '.'); dot; dot = strchr(dot + 1, '.')) {
            ...
            if (obj->getDocument() != sels[0].pObject->getDocument()) {
                if (finalLink) return sobj == obj ? nullptr : sobj;
                if (subname) *subname = std::string(dot + 1);
                return obj;
            }
        }
    ...
    if (finalLink && linkedVp == vp->getLinkedViewProvider()) return nullptr;

    auto linked = linkedVp->getObject();
    if (!linked || !linked->getNameInDocument()) return nullptr;
    ...
    if (subname && sels[0].pObject != sobj && sels[0].SubName) {
        ...
        for (auto &v : linked->getParents()) {
            if (v.first != sels[0].pObject) continue;
            ...
            if (dot - v.second.c_str() > pre_len
                ...
                if (prefix2.empty() || prefix2.size() > v.second.size()) prefix2 = v.second;
            }
        }
...
bool StdCmdLinkSelectLinked::isActive() { return getSelectedLink(false) != nullptr; }
...
void StdCmdLinkSelectLinked::activated(int)
{
    ...
    Selection().clearCompleteSelection();
    if (!subname.empty()) {
        Selection().addSelection(linked->getDocument()->getName(), linked->getNameInDocument(),
                                 subname.c_str());
        ...
        for (auto tree : trees) tree->selectLinkedObject(linked);
    }
    Selection().selStackPush();
}
...
StdCmdLinkSelectLinkedFinal::StdCmdLinkSelectLinkedFinal() : Command("Std_LinkSelectLinkedFinal")
{
    sGroup = "Link";
...
bool StdCmdLinkSelectLinkedFinal::isActive() { return getSelectedLink(true) != nullptr; }
...
StdCmdLinkSelectAllLinks::StdCmdLinkSelectAllLinks() : Command("Std_LinkSelectAllLinks")
{
    sGroup = "Link";
...
void StdCmdLinkSelectAllLinks::activated(int)
{
    ...
    Selection().clearCompleteSelection();
    const auto trees = getMainWindow()->findChildren<TreeWidget *>();
    for (auto tree : trees) tree->selectAllLinks(sels[0].pObject);
    Selection().selStackPush();
}
...
{
public:
    StdCmdLinkSelectActions() : GroupCommand("Std_LinkSelectActions")
    {
        sGroup = "View";
...
{
public:
    StdCmdLinkActions() : GroupCommand("Std_LinkActions")
    {
        sGroup = "View";

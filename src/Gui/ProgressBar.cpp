...
{
    // not initialized?
    if (!_pclSingleton) { _pclSingleton = new SequencerBar(); }
...
SequencerBar::~SequencerBar() { delete d; }
...
{
    if (d->bar->thread() != QThread::currentThread()) return;
    if (!wasCanceled()) {
        if (d->checkAbortTime.elapsed() < 500) return;
...
void SequencerBar::setProgress(size_t step)
{
    QThread *currentThread = QThread::currentThread();
    QThread *thr = d->bar->thread(); // this is the main thread
    if (thr != currentThread) { QMetaObject::invokeMethod(d->bar, "show", Qt::QueuedConnection); }
    else {
        d->bar->show();
    }
    ...
    if (nTotalSteps == 0) {
        ...
        if (elapsed > 100) {
            ...
            if (thr != currentThread) {
                ...
                if (d->bar->isVisible()) showRemainingTime();
            }
            else {
        ...
        if (elapsed > 1000 && rest > 100) {
            ...
            if (thr != currentThread) {
                QMetaObject::invokeMethod(getMainWindow(), "showMessage",
                                          ...
                                          QGenericReturnArgument(), Q_ARG(QString, status));
            }
            else {
...
void SequencerBar::resetData()
{
    ...
    if (thr != currentThread) {
        ...
        QMetaObject::invokeMethod(getMainWindow(), "showMessage", Qt::/*Blocking*/ QueuedConnection,
                                  QGenericReturnArgument(), Q_ARG(QString, QString()));
        QMetaObject::invokeMethod(getMainWindow(), "setPaneText", Qt::/*Blocking*/ QueuedConnection,
...
void SequencerBar::setText(const char *pszTxt)
{
    ...
    if (thr != currentThread) {
        QMetaObject::invokeMethod(getMainWindow(), "showMessage", Qt::/*Blocking*/ QueuedConnection,
                                  QGenericReturnArgument(), Q_ARG(QString, d->text));
    }
    else {
...
bool SequencerBar::isBlocking() const { return d->guiThread; }

QProgressBar *SequencerBar::getProgressBar(QWidget *parent)
{
    if (!d->bar) d->bar = new ProgressBar(this, parent);
    return d->bar;
}
...
ProgressBar::ProgressBar(SequencerBar *s, QWidget *parent) : QProgressBar(parent), sequencer(s)
{
...
int ProgressBar::minimumDuration() const { return d->minimumDuration; }
...
{
    if (!isVisible() && !sequencer->wasCanceled() && sequencer->isRunning()) { show(); }
}
...
void ProgressBar::resetObserveEventFilter() { d->observeEventFilter = 0; }
    ...
    if (sequencer->isRunning() && e) {
        ...
        switch (e->type()) {
            ...
            case QEvent::ContextMenu: {
                if (!d->isModalDialog(o)) return true;
            } break;

...
void View3DInventorViewerPy::init_type()
{
    ...
    add_varargs_method("getSoRenderManager", &View3DInventorViewerPy::getSoRenderManager,
                       ...
                       "render process\n");
    add_varargs_method(
        "getSoEventManager", &View3DInventorViewerPy::getSoEventManager,
    ...
    add_varargs_method("getSceneGraph", &View3DInventorViewerPy::getSceneGraph,
                       "getSceneGraph() -> SoNode");
    add_varargs_method("setSceneGraph", &View3DInventorViewerPy::setSceneGraph,
                       "setSceneGraph(SoNode)");

    add_varargs_method(
        "seekToPoint", &View3DInventorViewerPy::seekToPoint,
    ...
    add_varargs_method("setFocalDistance", &View3DInventorViewerPy::setFocalDistance,
                       "setFocalDistance(float) -> None\n");
    add_varargs_method("getFocalDistance", &View3DInventorViewerPy::getFocalDistance,
                       "getFocalDistance() -> float\n");
    add_varargs_method("getPoint", &View3DInventorViewerPy::getPointOnFocalPlane,
                       "Same as getPointOnFocalPlane");
    add_varargs_method("getPointOnFocalPlane", &View3DInventorViewerPy::getPointOnFocalPlane,
    ...
    add_varargs_method(
        "setupEditingRoot", &View3DInventorViewerPy::setupEditingRoot,
    ...
    add_varargs_method(
        "resetEditingRoot", &View3DInventorViewerPy::resetEditingRoot,
        "resetEditingRoot(updateLinks=True): restore the editing ViewProvider's root node");
    add_varargs_method(
        "setBackgroundColor", &View3DInventorViewerPy::setBackgroundColor,
    ...
    add_varargs_method(
        "isRedirectedToSceneGraph", &View3DInventorViewerPy::isRedirectedToSceneGraph,
        "isRedirectedToSceneGraph() -> bool: check whether event redirection is enabled.");
    add_varargs_method(
        "setEnabledNaviCube", &View3DInventorViewerPy::setEnabledNaviCube,
...
View3DInventorViewerPy::View3DInventorViewerPy(View3DInventorViewer *vi) : _viewer(vi) {}
...
Py::Object View3DInventorViewerPy::repr()
{
    ...
    if (!_viewer) throw Py::RuntimeError("Cannot print representation of deleted object");
...
Py::Object View3DInventorViewerPy::getSoRenderManager(const Py::Tuple &args)
{
    ...
    try {
        ...
        proxy = Base::Interpreter().createSWIGPointerObj("pivy.coin", "SoRenderManager *",
                                                         (void *)manager, 0);
...
Py::Object View3DInventorViewerPy::getSoEventManager(const Py::Tuple &args)
{
    ...
    try {
        ...
        PyObject *proxy = nullptr;
        proxy = Base::Interpreter().createSWIGPointerObj("pivy.coin", "SoEventManager *",
                                                         (void *)manager, 0);
        return Py::Object(proxy, true);
    }
    catch (const Base::Exception &e) {
...
{
    float distance;
    if (!PyArg_ParseTuple(args.ptr(), "f", &distance)) throw Py::Exception();

    try {
        SoCamera *cam = _viewer->getSoRenderManager()->getCamera();
        if (cam) cam->focalDistance.setValue(distance);
    }
    catch (const Py::Exception &) {
        throw; // re-throw
    }
...
{
    float r = 0.0;
    if (!PyArg_ParseTuple(args.ptr(), "f", &r)) { throw Py::Exception(); }
...
Py::Object View3DInventorViewerPy::setupEditingRoot(const Py::Tuple &args)
{
    ...
    if (pymat != Py_None) mat = static_cast<Base::MatrixPy *>(pymat)->getMatrixPtr();
...
Py::Object View3DInventorViewerPy::resetEditingRoot(const Py::Tuple &args)
{
    ...
    if (!PyArg_ParseTuple(args.ptr(), "|O!", &PyBool_Type, &updateLinks)) { throw Py::Exception(); }
    try {
...
{
    float r, g, b = 0.0;
    if (!PyArg_ParseTuple(args.ptr(), "fff", &r, &g, &b)) { throw Py::Exception(); }
    try {
        SbColor col(r, g, b);
...
{
    PyObject *m = Py_False;
    if (!PyArg_ParseTuple(args.ptr(), "O!", &PyBool_Type, &m)) throw Py::Exception();
...
{
    int pos;
    if (!PyArg_ParseTuple(args.ptr(), "i", &pos)) throw Py::Exception();
    if (pos < 0 || pos > 3) throw Py::IndexError("Value out of range");

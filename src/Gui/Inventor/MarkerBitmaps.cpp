...
void MarkerBitmaps::createBitmap(const std::string &name, int px, int width, int height,
                                 const char *marker)
{
    int byteidx = 0;
    ...
    for (int h = 0; h < height; h++) {
        ...
        for (int w = 0; w < width; w++) {
            if (marker[(h * width) + w] != ' ') { bits |= (0x80 >> (w % 8)); }
...
int MarkerBitmaps::getMarkerIndex(const std::string &name, int px)
{
    ...
    if (it != markerIndex.end()) { return it->second; }
...
std::list<int> MarkerBitmaps::getSupportedSizes(const std::string &name)
{
    ...
    for (std::map<Marker, int>::iterator it = markerIndex.begin(); it != markerIndex.end(); ++it) {
        if (it->first.first == name) sizes.push_back(it->first.second);
    }
    return sizes;
}

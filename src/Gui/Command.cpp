...
CommandBase::CommandBase(const char *sMenu, const char *sToolTip, const char *sWhat,
    ...
    : sMenuText(sMenu), sToolTipText(sToolTip), sWhatsThis(sWhat ? sWhat : sToolTip),
      sStatusTip(sStatus ? sStatus : sToolTip), sPixmap(sPixmap), sAccel(sAcc), _pcAction(nullptr)
{}
...
Action *CommandBase::getAction() const { return _pcAction; }
...
void CommandBase::setMenuText(const char *s) { this->sMenuText = StringCache::New(s); }

void CommandBase::setToolTipText(const char *s) { this->sToolTipText = StringCache::New(s); }

void CommandBase::setStatusTip(const char *s) { this->sStatusTip = StringCache::New(s); }

void CommandBase::setWhatsThis(const char *s) { this->sWhatsThis = StringCache::New(s); }

void CommandBase::setPixmap(const char *s) { this->sPixmap = StringCache::New(s); }

void CommandBase::setAccel(const char *s) { this->sAccel = StringCache::New(s); }
...
Command::Command(const char *name) : CommandBase(nullptr), sName(name), sHelpUrl(nullptr)
{
    sAppModule = "FreeCAD";
    sGroup = "Standard";
...
bool Command::isViewOfType(Base::Type t) const
{
    ...
    if (!v) return false;
    if (v->getTypeId().isDerivedFrom(t)) return true;
    else
        return false;
}
...
Application *Command::getGuiApplication() { return Application::Instance; }
...
{
    if (Name) { return App::GetApplication().getDocument(Name); }
    else {
        Gui::Document *pcDoc = getGuiApplication()->activeDocument();
        if (pcDoc) return pcDoc->getDocument();
        else
            return nullptr;
    }
}
...
App::DocumentObject *Command::getObject(const char *Name) const
{
    ...
    if (pDoc) return pDoc->getObject(Name);
    else
        return nullptr;
}
...
void Command::setupCheckable(int iMsg)
{
    ...
    if (checkable) {
        ...
        switch (triggerSource()) {
            case TriggerNone: checked = !action->isChecked(); break;
            case TriggerAction: checked = _pcAction->isChecked(); break;
            case TriggerChildAction: checked = action->isChecked(); break;
        }
        bool blocked = action->blockSignals(true);
        action->setChecked(checked);
        action->blockSignals(blocked);
        if (action != _pcAction->action()) _pcAction->setChecked(checked, true);
    }
}
...
void Command::invoke(int i, TriggerSource trigger)
{
    ...
    if (displayText.empty()) {
        ...
        if (displayText.empty()) displayText = getName();
    }
...
void Command::_invoke(int id, bool disablelog)
{
    try {
        ...
        if (disablelog) logdisabler.reset(new LogDisabler);
        ...
        if (isActive()) {
            ...
            if (!editDoc && getGuiApplication()->editDocument()) committer.setEnable(false);
        }
    }
...
void Command::testActive()
{
    ...
    if (pcAction) {
        ...
        for (auto action : actions) {
            ...
            if (cmd) action->setEnabled(cmd->isActive());
        }
    }
...
Gui::SelectionSingleton &Command::getSelection() { return Gui::Selection(); }
...
std::string Command::getObjectCmd(const char *Name, const App::Document *doc, const char *prefix,
                                  const char *postfix, bool gui)
{
    if (!doc) doc = App::GetApplication().getActiveDocument();
    if (!doc || !Name) return std::string("None");
...
std::string Command::getObjectCmd(const App::DocumentObject *obj, const char *prefix,
                                  const char *postfix, bool gui)
{
    if (!obj || !obj->getNameInDocument()) return std::string("None");
    return getObjectCmd(obj->getNameInDocument(), obj->getDocument(), prefix, postfix, gui);
}

void Command::setAppModuleName(const char *s) { this->sAppModule = StringCache::New(s); }

void Command::setGroupName(const char *s) { this->sGroup = StringCache::New(s); }
...
{
    if (!sCmdName) sCmdName = "Command";
    App::GetApplication().setActiveTransaction(sCmdName);
}

void Command::commitCommand() { App::GetApplication().closeActiveTransaction(); }

void Command::abortCommand() { App::GetApplication().closeActiveTransaction(true); }

bool Command::hasPendingCommand() { return !!App::GetApplication().getActiveTransaction(); }

bool Command::_blockCmd = false;

void Command::blockCommand(bool block) { Command::_blockCmd = block; }
...
{
    if (!FC_LOG_INSTANCE.isEnabled(FC_LOGLEVEL_LOG)) return;
...
void Command::_runCommand(const char *file, int line, DoCmd_Type eType, const char *sCmd)
{
    ...
    if (eType == Gui) Gui::Application::Instance->macroManager()->addLine(MacroManager::Gui, sCmd);
    else
...
std::string Command::_assureWorkbench(const char *file, int line, const char *sName)
{
    ...
    // if yes, do nothing
    if (actName == sName) return actName;
...
void Command::_copyVisual(const char *file, int line, const char *to, const char *attr,
                          const char *from)
{
    _copyVisual(file, line, to, attr, from, attr);
}

void Command::_copyVisual(const char *file, int line, const char *to, const char *attr_to,
...
void Command::_copyVisual(const char *file, int line, const App::DocumentObject *to,
                          const char *attr_to, const App::DocumentObject *from,
                          const char *attr_from)
{
    if (!from || !from->getNameInDocument() || !to || !to->getNameInDocument()) return;
    ...
    if (it != attrMap.end()) {
        ...
        for (int depth = 0;; ++depth) {
            ...
            if (vp && vp->OverrideMaterial.getValue()) {
                _doCommand(file, line, Gui, "%s.ViewObject.%s=%s.ViewObject.%s", objCmd.c_str(),
                           attr_to, getObjectCmd(obj).c_str(), it->second.c_str());
                return;
            }
            auto linked = obj->getLinkedObject(false, nullptr, false, depth);
            if (!linked || linked == obj) break;
            obj = linked;
        }
    }
    ...
    try {
        _doCommand(
            ...
            objCmd.c_str(), attr_to, getObjectCmd(from).c_str(), attr_from, objCmd.c_str(),
            attr_to);
    }
    catch (Base::Exception & /*e*/) {
        // e.ReportException();
...
const char *Command::endCmdHelp() { return "</body></html>\n\n"; }

void Command::recreateTooltip(const char *context, Action *action)
{
    QString tooltip;
    tooltip.append(QString::fromLatin1("<h3>"));
    tooltip.append(QCoreApplication::translate(context, getMenuText()));
    tooltip.append(QString::fromLatin1("</h3>"));
    ...
    tooltip.append(QCoreApplication::translate(context, getToolTipText()));
    tooltip.append(QString::fromLatin1("<br><i>("));
    tooltip.append(QCoreApplication::translate(context, getWhatsThis()));
    ...
    if (!accel.isEmpty()) {
        ...
        QString ttip = QString::fromLatin1("%1 (%2)").arg(action->toolTip(), accel);
...
const char *Command::keySequenceToAccel(int sk) const
{
    ...
    if (i != strings.end()) return i->second.c_str();
...
void Command::adjustCameraPosition()
{
    ...
    if (doc) {
        ...
        if (!camera || !camera->isOfType(SoOrthographicCamera::getClassTypeId())) return;
...
Action *Command::createAction()
{
    ...
    if (sPixmap) pcAction->setIcon(Gui::BitmapFactory().iconFromTheme(sPixmap));
    return pcAction;
}

void Command::languageChange()
{
    if (_pcAction) { applyCommandData(this->className(), _pcAction); }
}
...
GroupCommand::GroupCommand(const char *name) : Command(name) {}

int GroupCommand::addCommand(Command *cmd, bool reg)
{
    cmds.emplace_back(cmd, cmds.size());
    if (cmd && reg) Application::Instance->commandManager().addCommand(cmd);
...
Action *GroupCommand::createAction()
{
    ...
    for (auto &v : cmds) {
        if (!v.first) pcAction->addAction(QString::fromLatin1(""))->setSeparator(true);
        else
...
{
    if (iMsg < 0 || iMsg >= (int)cmds.size()) return;

    auto &v = cmds[iMsg];
    if (!v.first) return;

    if (triggerSource() != TriggerChildAction) v.first->invoke(0);
...
MacroCommand::MacroCommand(const char *name, bool system)
    : Command(StringCache::New(name)), systemMacro(system)
{
    sGroup = "Macros";
    eType = 0;
    sScriptName = nullptr;
...
Action *MacroCommand::createAction()
{
    ...
    if (pcAction->statusTip().isEmpty()) pcAction->setStatusTip(pcAction->toolTip());
    pcAction->setWhatsThis(QString::fromUtf8(sWhatsThis));
    if (sPixmap) pcAction->setIcon(Gui::BitmapFactory().pixmap(sPixmap));
#ifdef FC_DEBUG
    printConflictingAccelerators();
#endif
    pcAction->setShortcut(QString::fromLatin1(sAccel));
    ...
    if (!accel.isEmpty()) {
        // show shortcut inside tooltip
        QString ttip = QString::fromLatin1("%1 (%2)").arg(pcAction->toolTip(), accel);
...
void MacroCommand::setScriptName(const char *s) { this->sScriptName = StringCache::New(s); }
...
PythonCommand::PythonCommand(const char *name, PyObject *pcPyCommand, const char *pActivationString)
    : Command(StringCache::New(name)), _pcPyCommand(pcPyCommand)
{
    if (pActivationString) Activation = pActivationString;
    ...
    if (!cmdType.empty()) {
        int type = 0;
        if (cmdType.find("AlterDoc") != std::string::npos) type += int(AlterDoc);
        if (cmdType.find("Alter3DView") != std::string::npos) type += int(Alter3DView);
        if (cmdType.find("AlterSelection") != std::string::npos) type += int(AlterSelection);
        if (cmdType.find("ForEdit") != std::string::npos) type += int(ForEdit);
        if (cmdType.find("NoTransaction") != std::string::npos) type += int(NoTransaction);
        eType = type;
    }
}
...
bool PythonCommand::isActive()
{
    try {
        ...
        if (cmd.hasAttr("IsActive")) {
            ...
            if (!PyBool_Check(ret.ptr()) || ret.ptr() != Py_True) return false;
        }
    }
...
{
    if (_pcAction) { applyCommandData(getName(), _pcAction); }
}
...
const char *PythonCommand::getWhatsThis() const
{
    ...
    if (!whatsthis || whatsthis[0] == '\0') whatsthis = this->getName();
    return whatsthis;
}

const char *PythonCommand::getMenuText() const { return getResource("MenuText"); }

const char *PythonCommand::getToolTipText() const { return getResource("ToolTip"); }

const char *PythonCommand::getStatusTip() const { return getResource("StatusTip"); }
...
const char *PythonCommand::getAccel() const { return getResource("Accel"); }
...
bool PythonCommand::isChecked() const
{
    ...
    if (PyBool_Check(item)) { return Base::asBoolean(item); }
    else {
...
Action *PythonGroupCommand::createAction()
{
    ...
    if (strcmp(getResource("Pixmap"), "") != 0) {
        ...
        if (defaultId < 0 || defaultId >= a.size()) defaultId = 0;
        if (a.size() > defaultId) pcAction->setIcon(a[defaultId]->icon());
    }
...
void PythonGroupCommand::languageChange()
{
    ...
    for (const auto &it : a) {
        ...
        if (cmd) {
            ...
            if (!statustip || '\0' == *statustip) { statustip = tooltip; }
...
const char *PythonGroupCommand::getHelpUrl() const { return ""; }
...
const char *PythonGroupCommand::getMenuText() const { return getResource("MenuText"); }

const char *PythonGroupCommand::getToolTipText() const { return getResource("ToolTip"); }

const char *PythonGroupCommand::getStatusTip() const { return getResource("StatusTip"); }
...
const char *PythonGroupCommand::getAccel() const { return getResource("Accel"); }
...
CommandManager::~CommandManager() { clearCommands(); }
...
std::vector<Command *> CommandManager::getModuleCommands(const char *sModName) const
{
    ...
    for (std::map<std::string, Command *>::const_iterator It = _sCommands.begin();
         It != _sCommands.end(); ++It) {
        if (strcmp(It->second->getAppModuleName(), sModName) == 0) vCmds.push_back(It->second);
    }
...
std::vector<Command *> CommandManager::getGroupCommands(const char *sGrpName) const
{
    ...
    for (std::map<std::string, Command *>::const_iterator It = _sCommands.begin();
         It != _sCommands.end(); ++It) {
        if (strcmp(It->second->getGroupName(), sGrpName) == 0) vCmds.push_back(It->second);
    }
...
{
    if (!accel || accel[0] == '\0') return nullptr;

    QString newCombo = QString::fromLatin1(accel);
    if (newCombo.isEmpty()) return nullptr;
    auto newSequence = QKeySequence::fromString(newCombo);
    if (newSequence.count() == 0) return nullptr;
    ...
    for (const auto &cmd : commands) {
        ...
        if (!existingAccel || existingAccel[0] == '\0') continue;
        ...
        if (existingCombo.isEmpty()) continue;
        auto existingSequence = QKeySequence::fromString(existingCombo);
        if (existingSequence.count() == 0) continue;

        // Exact match
        if (existingSequence == newSequence) return cmd;

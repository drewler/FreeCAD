...
{
    switch (mode) {
        case NavigationStyle::SELECTION: return QT_TR_NOOP("Press left mouse button");
        case NavigationStyle::PANNING: return QT_TR_NOOP("Press SHIFT and middle mouse button");
        case NavigationStyle::DRAGGING: return QT_TR_NOOP("Press middle mouse button");
        case NavigationStyle::ZOOMING: return QT_TR_NOOP("Scroll middle mouse button");
...
SbBool BlenderNavigationStyle::processSoEvent(const SoEvent *const ev)
{
        ...
        switch (button) {
            ...
            case SoMouseButtonEvent::BUTTON3:
                if (press) {
                    ...
                    SbViewVolume vv =
                        viewer->getSoRenderManager()->getCamera()->getViewVolume(ratio);
                    this->panningplane = vv.getPlane(
                        viewer->getSoRenderManager()->getCamera()->focalDistance.getValue());
    ...
    if (type.isDerivedFrom(SoLocation2Event::getClassTypeId())) {
        ...
        if (this->currentmode == NavigationStyle::ZOOMING) {
            ...
            float ratio = vp.getViewportAspectRatio();
            panCamera(viewer->getSoRenderManager()->getCamera(), ratio, this->panningplane, posn,
                      prevnormalized);
            newmode = NavigationStyle::SELECTION;
            processed = true;
        }
    ...
    switch (combo) {
        ...
        case CTRLDOWN | BUTTON1DOWN:
            ...
            if (curmode == NavigationStyle::SPINNING || this->lockButton1)
                ...
                newmode = NavigationStyle::SELECTION;
            break;
        case BUTTON1DOWN | BUTTON2DOWN: newmode = NavigationStyle::PANNING; break;
        case SHIFTDOWN | BUTTON3DOWN: newmode = NavigationStyle::PANNING; break;
        case BUTTON3DOWN:
            if (newmode != NavigationStyle::DRAGGING) { saveCursorPosition(ev); }
            newmode = NavigationStyle::DRAGGING;
            break;
        case CTRLDOWN | SHIFTDOWN | BUTTON2DOWN:
        case CTRLDOWN | BUTTON3DOWN: newmode = NavigationStyle::ZOOMING; break;
    ...
    // into selection mode.
    if (this->button1down && this->button3down) this->lockButton1 = true;

    // If not handled in this class, pass on upwards in the inheritance
    // hierarchy.
    if (!processed) processed = inherited::processSoEvent(ev);

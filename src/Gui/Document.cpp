...
Document::Document(App::Document *pcDocument, Application *app)
{
    ...
    d->connectActObjectBlocker =
        boost::signals2::shared_connection_block(d->connectActObject, false);
    d->connectSaveDocument =
        pcDocument->signalSaveDocument.connect(boost::bind(&Gui::Document::Save, this, bp::_1));
    ...
    d->connectUndoDocument =
        pcDocument->signalUndo.connect(boost::bind(&Gui::Document::slotUndoDocument, this, bp::_1));
    d->connectRedoDocument =
        pcDocument->signalRedo.connect(boost::bind(&Gui::Document::slotRedoDocument, this, bp::_1));
...
Document::~Document()
{
    ...
    for (auto &it : temp) it->deleteSelf();

    std::map<const App::DocumentObject *, ViewProviderDocumentObject *>::iterator jt;
    for (jt = d->_ViewProviderMap.begin(); jt != d->_ViewProviderMap.end(); ++jt) delete jt->second;
...
bool Document::setEdit(Gui::ViewProvider *p, int ModNum, const char *subname)
{
    ...
    if (d->_editViewProvider) { _resetEdit(); }
    ...
    if (!subname || !subname[0]) {
        ...
        for (auto &sel : sels) {
            if (!sel.pObject || !sel.pObject->getNameInDocument()) continue;
            if (!parentObj) parentObj = sel.pObject;
        ...
        if (parentObj) {
            ...
            if (vp->getDocument() != this) return vp->getDocument()->setEdit(vp, ModNum, subname);
        }
    }
    ...
    if (!sobj || !sobj->getNameInDocument()) {
        FC_ERR("Invalid sub object '" << obj->getFullName() << '.' << (subname ? subname : "")
                                      << "'");
        return false;
    }
    auto svp = vp;
    ...
    if (view3d) getMainWindow()->setActiveWindow(view3d);
    else
    ...
    if (dlg) dlg->setDocumentName(this->getDocument()->getName());
...
const Base::Matrix4D &Document::getEditingTransform() const { return d->_editingTransform; }
...
void Document::setEditingTransform(const Base::Matrix4D &mat)
{
    ...
    if (activeView) activeView->getViewer()->setEditingTransform(mat);
}

void Document::resetEdit() { Application::Instance->setEditDocument(nullptr); }
...
void Document::_resetEdit()
{
    ...
    if (d->_editViewProvider) {
        for (it = d->baseViews.begin(); it != d->baseViews.end(); ++it) {
            ...
            if (activeView) activeView->getViewer()->resetEditingViewProvider();
        }
        ...
        if (d->_editViewProvider
            && d->_editViewProvider->isDerivedFrom(ViewProviderDocumentObject::getClassTypeId())) {
            auto vpd = static_cast<ViewProviderDocumentObject *>(d->_editViewProvider);
...
ViewProvider *Document::getInEdit(ViewProviderDocumentObject **parentVp, std::string *subname,
                                  int *mode, std::string *subelement) const
{
...
void Document::setAnnotationViewProvider(const char *name, ViewProvider *pcProvider)
{
    ...
    if (it != d->_ViewProviderMapAnnotation.end()) removeAnnotationViewProvider(name);
    ...
    for (vIt = d->baseViews.begin(); vIt != d->baseViews.end(); ++vIt) {
        ...
        if (activeView) activeView->getViewer()->addViewProvider(pcProvider);
    }
}
...
void Document::removeAnnotationViewProvider(const char *name)
{
    ...
    for (vIt = d->baseViews.begin(); vIt != d->baseViews.end(); ++vIt) {
        auto activeView = dynamic_cast<View3DInventor *>(*vIt);
        if (activeView) activeView->getViewer()->removeViewProvider(it->second);
    }
...
std::vector<ViewProvider *> Document::getViewProvidersOfType(const Base::Type &typeId) const
{
    ...
    for (std::map<const App::DocumentObject *, ViewProviderDocumentObject *>::const_iterator it =
         ...
         it != d->_ViewProviderMap.end(); ++it) {
        if (it->second->getTypeId().isDerivedFrom(typeId)) Objects.push_back(it->second);
    }
    return Objects;
}
...
ViewProvider *Document::getViewProviderByName(const char *name) const
{
    ...
    if (pcFeat) {
        ...
        if (it != d->_ViewProviderMap.end()) return it->second;
    }
    else {
        ...
        if (it2 != d->_ViewProviderMapAnnotation.end()) return it2->second;
    }
...
{
    ViewProvider *pcProv = getViewProviderByName(name);
    if (pcProv) pcProv->setTransformation(rclMtrx);
}
...
void Document::slotNewObject(const App::DocumentObject &Obj)
{
    ...
    if (!pcProvider) {
        ...
        for (;;) {
            ...
            Base::Type type = Base::Type::getTypeIfDerivedFrom(
                cName.c_str(), ViewProviderDocumentObject::getClassTypeId(), true);
            ...
            if (!pcProvider) {
                ...
                FC_WARN("View provider type '" << cName << "' does not support "
                                               << Obj.getFullName());
                pcProvider = nullptr;
    ...
    if (pcProvider) {
        ...
        if (d->_isTransacting) { d->_redoViewProviders.push_back(pcProvider); }
    }
}
...
void Document::slotDeletedObject(const App::DocumentObject &Obj)
{
    ...
    if (viewProvider
        && viewProvider->getTypeId().isDerivedFrom(ViewProviderDocumentObject::getClassTypeId())) {
        // go through the views
        for (vIt = d->baseViews.begin(); vIt != d->baseViews.end(); ++vIt) {
            auto activeView = dynamic_cast<View3DInventor *>(*vIt);
            if (activeView) activeView->getViewer()->removeViewProvider(viewProvider);
        }
...
void Document::beforeDelete()
{
    ...
    for (auto &v : d->_ViewProviderMap) v.second->beforeDelete();
}
...
void Document::slotSkipRecompute(const App::Document &doc,
                                 const std::vector<App::DocumentObject *> &objs)
{
    ...
    if (editDoc) {
        ...
        if (vp) obj = vp->getObject();
    }
    if (!obj) obj = doc.getActiveObject();
    if (!obj || !obj->getNameInDocument() || (!objs.empty() && objs.front() != obj)) return;
...
{
    if (d->_isModified == b) return;
    d->_isModified = b;

    std::list<MDIView *> mdis = getMDIViews();
    for (auto &mdi : mdis) { mdi->setWindowModified(b); }
}

bool Document::isModified() const { return d->_isModified; }
...
ViewProviderDocumentObject *Document::getViewProviderByPathFromTail(SoPath *path) const
{
    ...
    for (int i = 0; i < path->getLength(); i++) {
        ...
        if (node->isOfType(SoSeparator::getClassTypeId())) {
            ...
            if (it != d->_CoinMap.end()) return it->second;
        }
    }
...
{
    if (!node || !node->isOfType(SoSeparator::getClassTypeId())) return nullptr;
    auto it = d->_CoinMap.find(static_cast<SoSeparator *>(node));
    if (it != d->_CoinMap.end()) return it->second;
    return nullptr;
}
...
Document::getViewProvidersByPath(SoPath *path) const
{
    ...
    for (int i = 0; i < path->getLength(); i++) {
        ...
        if (node->isOfType(SoSeparator::getClassTypeId())) {
            auto it = d->_CoinMap.find(static_cast<SoSeparator *>(node));
            if (it != d->_CoinMap.end()) ret.emplace_back(it->second, i);
        }
    }
    return ret;
}

App::Document *Document::getDocument() const { return d->_pcDocument; }
...
static bool checkCanonicalPath(const std::map<App::Document *, bool> &docs)
{
    ...
    for (auto doc : App::GetApplication().getDocuments()) {
        ...
        d.push_back(doc);
        if (!warn && d.size() > 1) {
            if (docs.count(d.front()) || docs.count(d.back())) warn = true;
        }
    }
    if (!warn) return true;
    QString msg;
    QTextStream ts(&msg);
    ts << QObject::tr("Identical physical path detected. It may cause unwanted overwrite of "
                      "existing document!\n\n")
       << QObject::tr("Are you sure you want to continue?");

    auto docName = [](App::Document *doc) -> QString {
        if (doc->Label.getStrValue() == doc->getName()) return QString::fromLatin1(doc->getName());
    ...
    int ret = QMessageBox::warning(getMainWindow(), QObject::tr("Identical physical path"), msg,
...
bool Document::askIfSavingFailed(const QString &error)
{
    int ret = QMessageBox::question(
        getMainWindow(), QObject::tr("Could not save document"),
...
bool Document::save()
{
    if (d->_pcDocument->isSaved()) {
        try {
            ...
            if (docs.size() > 1) {
                int ret = QMessageBox::question(
                    getMainWindow(), QObject::tr("Save dependent files"),
            ...
            if (!checkCanonicalPath(dmap)) return false;
            ...
            for (auto doc : docs) {
                ...
                if (!dmap[doc] && doc->mustExecute()) {
                    ...
                    Command::doCommand(Command::Doc, "App.getDocument(\"%s\").recompute()",
                                       doc->getName());
                }
...
bool Document::saveAs()
{
    ...
    QString fn = FileDialog::getSaveFileName(
        getMainWindow(), QObject::tr("Save %1 Document").arg(exe),
    ...
    if (!fn.isEmpty()) {
        ...
        try {
            ...
            Command::doCommand(Command::Doc, "App.getDocument(\"%s\").saveAs(u\"%s\")", DocName,
                               escapedstr.c_str());
...
void Document::saveAll()
{
    ...
    try {
        ...
        int ret = QMessageBox::critical(
            getMainWindow(), QObject::tr("Failed to save document"),
    ...
    if (!checkCanonicalPath(dmap)) return;
...
bool Document::saveCopy()
{
    ...
    QString exe = qApp->applicationName();
    QString fn =
        FileDialog::getSaveFileName(getMainWindow(), QObject::tr("Save %1 Document").arg(exe),
                                    QString::fromUtf8(getDocument()->FileName.getValue()),
    ...
    if (!fn.isEmpty()) {
        ...
        Command::doCommand(Command::Doc, "App.getDocument(\"%s\").saveCopy(\"%s\")", DocName,
...
void Document::RestoreDocFile(Base::Reader &reader)
{
    ...
    if (hasExpansion) {
        ...
        if (tree) {
            ...
            if (docItem) docItem->Restore(*localreader);
        }
    }
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt; i++) {
            ...
            if (!hasExpansion && localreader->hasAttribute("expanded")) {
                ...
                if (strcmp(attr, "1") == 0) { expanded = true; }
            }
        ...
        if (ppReturn && ppReturn[0]) {
            ...
            try {
                ...
                std::list<MDIView *> mdi = getMDIViews();
                for (const auto &it : mdi) {
                    if (it->onHasMsg("SetCamera")) it->onMsg(cameraSettings.c_str(), pReturnIgnore);
                }
            }
            catch (const Base::Exception &e) {
...
void Document::slotFinishRestoreObject(const App::DocumentObject &obj)
{
    ...
    if (vpd) {
        ...
        vpd->finishRestoring();
        if (!vpd->canAddToSceneGraph()) toggleInSceneGraph(vpd);
    }
}
...
void Document::slotFinishRestoreDocument(const App::Document &doc)
{
    ...
    if (act) {
        ...
        if (viewProvider
            && viewProvider->isDerivedFrom(ViewProviderDocumentObject::getClassTypeId())) {
            signalActivatedObject(*(static_cast<ViewProviderDocumentObject *>(viewProvider)));
        }
    }
...
void Document::SaveDocFile(Base::Writer &writer) const
{
    ...
    if (!hasExpansion) writer.Stream() << ">" << std::endl;
    ...
    for (it = d->_ViewProviderMap.begin(); it != d->_ViewProviderMap.end(); ++it) {
        ...
        if (obj->hasExtensions()) writer.Stream() << " Extensions=\"True\"";
    ...
    for (const auto &it : mdi) {
        if (it->onHasMsg("GetCamera")) {
            ...
            if (saveCameraSettings(ppReturn)) break;
        }
    }
...
void Document::exportObjects(const std::vector<App::DocumentObject *> &obj, Base::Writer &writer)
{
    ...
    for (jt = views.begin(); jt != views.end(); ++jt) {
        ...
        writer.Stream() << writer.ind() << "<ViewProvider name=\"" << doc->getExportName() << "\" "
                        << "expanded=\"" << (doc->testStatus(App::Expand) ? 1 : 0) << "\"";
        if (vp->hasExtensions()) writer.Stream() << " Extensions=\"True\"";
                             ...
                             const std::map<std::string, std::string> &nameMapping)
{
    ...
    if (scheme == 1) {
        ...
        for (int i = 0; i < Cnt && it != obj.end(); ++i, ++it) {
            ...
            if (jt != nameMapping.end()) name = jt->second;
            bool expanded = false;
            ...
            if (pObj) {
                ...
                pObj->Restore(*localreader);
                if (expanded && vpd) this->signalExpandObject(*vpd, TreeItemMode::ExpandItem, 0, 0);
            }
            localreader->readEndElement("ViewProvider");
            if (it == obj.end()) break;
        }
    ...
    if (!localreader->getFilenames().empty()) reader.initLocalReader(localreader);
}
                                     ...
                                     App::DocumentObjectGroup *grp)
{
    ...
    for (const auto it : obj) { rootMap[it] = true; }
    ...
    for (const auto it : obj) {
        ...
        if (vp) {
            ...
            for (const auto &jt : child) {
                ...
                if (kt != rootMap.end()) { kt->second = false; }
            }
        }
    }
    ...
    for (const auto &it : rootMap) {
        if (it.second) grp->addObject(it.first);
    }
}

MDIView *Document::createView(const Base::Type &typeId)
{
    if (!typeId.isDerivedFrom(MDIView::getClassTypeId())) return nullptr;
    ...
    if (typeId == View3DInventor::getClassTypeId()) {
        ...
        QString title =
            QString::fromLatin1("%1 : %2[*]").arg(QString::fromUtf8(name)).arg(d->_iWinCount++);
...
{
    if (!bPassiv) d->baseViews.push_back(pcView);
    else
...
void Document::onUpdate()
{
    ...
    for (it = d->baseViews.begin(); it != d->baseViews.end(); ++it) { (*it)->onUpdate(); }

    for (it = d->passiveViews.begin(); it != d->passiveViews.end(); ++it) { (*it)->onUpdate(); }
}
...
void Document::onRelabel()
{
    ...
    for (it = d->baseViews.begin(); it != d->baseViews.end(); ++it) { (*it)->onRelabel(this); }
...
{
    if (d->baseViews.size() <= 1) return true;
    return false;
}
...
{
    if (d->_isClosing) return true;
    if (!getDocument()->isClosable()) {
        QMessageBox::warning(getActiveView(), QObject::tr("Document not closable"),
    ...
    if (checkLink && !App::PropertyXLink::getDocumentInList(getDocument()).empty()) return true;

    if (getDocument()->testStatus(App::Document::TempDoc)) return true;
    ...
    if (ok) {
        ...
        if (!Gui::Control().isAllowedAlterDocument()) {
            ...
            if (name == this->getDocument()->getName()) {
                ...
                // active view.
                if (d->_editViewProvider) this->_resetEdit();
            }
        }
    }
...
std::list<MDIView *> Document::getMDIViewsOfType(const Base::Type &typeId) const
{
    ...
    for (std::list<BaseView *>::const_iterator it = d->baseViews.begin(); it != d->baseViews.end();
         ++it) {
        auto view = dynamic_cast<MDIView *>(*it);
        if (view && view->isDerivedFrom(typeId)) views.push_back(view);
    }
...
bool Document::sendMsgToViews(const char *pMsg)
{
    ...
    for (it = d->baseViews.begin(); it != d->baseViews.end(); ++it) {
        if ((*it)->onMsg(pMsg, pReturnIgnore)) { return true; }
    }
...
MDIView *Document::setActiveView(ViewProviderDocumentObject *vp, Base::Type typeId)
{
    ...
    if (!vp) { view = getActiveView(); }
    else {
        view = vp->getMDIView();
        if (!view) {
            auto obj = vp->getObject();
            if (!obj) { view = getActiveView(); }
            else {
                auto linked = obj->getLinkedObject(true);
                if (linked != obj) {
                    auto vpLinked = dynamic_cast<ViewProviderDocumentObject *>(
                        Application::Instance->getViewProvider(linked));
                    if (vpLinked) view = vpLinked->getMDIView();
                }

                if (!view && typeId.isBad()) {
                    MDIView *active = getActiveView();
                    if (active && active->containsViewProvider(vp)) view = active;
                    else
    ...
    if (!view && !typeId.isBad()) view = createView(typeId);
...
void Document::setActiveWindow(Gui::MDIView *view)
{
    ...
    if (std::find(mdis.begin(), mdis.end(), active) == mdis.end()) return;

    // the view is not part of the document
    if (std::find(mdis.begin(), mdis.end(), view) == mdis.end()) return;
...
Gui::MDIView *Document::getViewOfNode(SoNode *node) const
{
    ...
    for (const auto &mdi : mdis) {
        ...
        if (view->getViewer()->searchNode(node)) return mdi;
    }
...
Gui::MDIView *Document::getEditingViewOfViewProvider(Gui::ViewProvider *vp) const
{
    ...
    for (const auto &mdi : mdis) {
        ...
        // there is only one 3d view which is in edit mode
        if (viewer->hasViewProvider(vp) && viewer->isEditingViewProvider()) return mdi;
    }
 ...
 */
void Document::openCommand(const char *sName) { getDocument()->openTransaction(sName); }

void Document::commitCommand() { getDocument()->commitTransaction(); }

void Document::abortCommand() { getDocument()->abortTransaction(); }

bool Document::hasPendingCommand() const { return getDocument()->hasPendingTransaction(); }
...
bool Document::checkTransactionID(bool undo, int iSteps)
{
    ...
    if (!prompts.empty()) {
        ...
        int ret = QMessageBox::warning(
            getMainWindow(), undo ? QObject::tr("Undo") : QObject::tr("Redo"),
            QString::fromLatin1("%1,\n%2%3")
                .arg(QObject::tr("There are grouped transactions in the following documents with "
...
bool Document::isPerformingTransaction() const { return d->_isTransacting; }
...
void Document::undo(int iSteps)
{
    ...
    if (!checkTransactionID(true, iSteps)) return;

    for (int i = 0; i < iSteps; i++) { getDocument()->undo(); }
...
void Document::redo(int iSteps)
{
    ...
    if (!checkTransactionID(false, iSteps)) return;

    for (int i = 0; i < iSteps; i++) { getDocument()->redo(); }
    App::GetApplication().signalRedo();

    for (auto it : d->_redoViewProviders) handleChildren3D(it);
            ...
            if (!deleting) {
                for (const auto &it : children) {
                    ...
                    if (ChildViewProvider) {
                        auto itOld = oldChildren.find(
                            static_cast<ViewProviderDocumentObject *>(ChildViewProvider));
                        ...
                        if (frontGroup && childFrontNode) frontGroup->addChild(childFrontNode);

                        SoSeparator *childBackNode = ChildViewProvider->getBackRoot();
                        if (backGroup && childBackNode) backGroup->addChild(childBackNode);

                        // cycling to all views of the document to remove the viewprovider from the viewer itself
                        for (std::list<Gui::BaseView *>::iterator vIt = d->baseViews.begin();
                             vIt != d->baseViews.end(); ++vIt) {
                            auto activeView = dynamic_cast<View3DInventor *>(*vIt);
                            if (activeView
...
void Document::toggleInSceneGraph(ViewProvider *vp)
{
    ...
    for (auto view : d->baseViews) {
        ...
        if (!vp->canAddToSceneGraph()) {
            ...
            if (!path) { scenegraph->addChild(root); }
        }
    }
}

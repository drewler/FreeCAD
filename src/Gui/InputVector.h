 ...
 * @author Werner Mayer
 */
template<class Ui> class LocationDialogUi: public LocationDialog, public Ui
{
public:
    ...
    void retranslate()
    {
        ...
        if (this->direction->count() == 0) {
            ...
            this->direction->setItemData(
                0, QVariant::fromValue<Base::Vector3d>(Base::Vector3d(1, 0, 0)));
            this->direction->setItemData(
                1, QVariant::fromValue<Base::Vector3d>(Base::Vector3d(0, 1, 0)));
            this->direction->setItemData(
                2, QVariant::fromValue<Base::Vector3d>(Base::Vector3d(0, 0, 1)));
        }
        else {
    ...
    Base::Vector3d getPosition() const
    {
        return Base::Vector3d(this->xPos->value().getValue(), this->yPos->value().getValue(),
    ...
    Base::Vector3d getDirection() const override
    {
        ...
        if (data.canConvert<Base::Vector3d>()) { return data.value<Base::Vector3d>(); }
        else {
...
protected:
    ...
    {
        if (e->type() == QEvent::LanguageChange) { this->retranslate(); }
        else {
...
private:
    void setDirection(const Base::Vector3d &dir)
    {
        if (dir.Length() < Base::Vector3d::epsilon()) { return; }
            ...
            if (ok) {
                if (dir.Length() < Base::Vector3d::epsilon()) {
                    QMessageBox::critical(
                        this, LocationDialog::tr("Wrong direction"),
 ...
 * @author Werner Mayer
 */
template<class Ui> class LocationUi: public Ui
{
public:
    ...
    bool directionActivated(LocationDialog *dlg, int index)
    {
        ...
        if (index + 1 == this->direction->count()) {
            ...
            if (ok) {
                if (dir.Length() < Base::Vector3d::epsilon()) {
                    QMessageBox::critical(
                        dlg, LocationDialog::tr("Wrong direction"),
                        LocationDialog::tr("Direction must not be the null vector"));
                    return false;
                }
                setDirection(dir);
 ...
 * @author Werner Mayer
 */
template<class Ui> class LocationDialogImp: public LocationDialog
{
public:
    ...
    Base::Vector3d getDirection() const override { return ui.getDirection(); }
...
private:
    void directionActivated(int index) override { ui.directionActivated(this, index); }
 ...
 * @author Werner Mayer
 */
template<class Ui> class LocationImpUi: public AbstractUi
{
public:
    LocationImpUi(Ui *ui) : ui(ui) {}
    ~LocationImpUi() override {}

    boost::any get() override { return ui; }
    ...
    {
        return Base::Vector3d(ui->xPos->value().getValue(), ui->yPos->value().getValue(),

...
PyObject *Gui::Application::sEditDocument(PyObject * /*self*/, PyObject *args)
{
    ...
    if (pcDoc) return pcDoc->getPyObject();
    else
        Py_Return;
}
...
PyObject *Gui::Application::sActiveView(PyObject * /*self*/, PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        if (mdiView && (type.isBad() || mdiView->isDerivedFrom(type))) {
            ...
            if (!res.isNone() || !type.isBad()) return Py::new_reference_to(res);
        }

        if (type.isBad()) type = Gui::View3DInventor::getClassTypeId();
        Instance->activateView(type, true);
        mdiView = Instance->activeView();
        if (mdiView) return mdiView->getPyObject();
...
PyObject *Gui::Application::sActivateView(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "sO!", &typeStr, &PyBool_Type, &create)) return nullptr;
...
PyObject *Application::sGetDocument(PyObject * /*self*/, PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "O!", &(App::DocumentPy::Type), &doc)) {
        Document *pcDoc =
            Instance->getDocument(static_cast<App::DocumentPy *>(doc)->getDocumentPtr());
        if (!pcDoc) {
            PyErr_Format(PyExc_KeyError, "Unknown document instance");
            return nullptr;
        }
...
PyObject *Application::sHide(PyObject * /*self*/, PyObject *args)
{
    ...
    if (pcDoc) pcDoc->setHide(psFeatStr);
...
PyObject *Application::sShow(PyObject * /*self*/, PyObject *args)
{
    ...
    if (pcDoc) pcDoc->setShow(psFeatStr);
...
PyObject *Application::sOpen(PyObject * /*self*/, PyObject *args)
{
    ...
    PY_TRY
    {
        ...
        if (ext == QLatin1String("iv")) {
            if (!Application::Instance->activeDocument()) App::GetApplication().newDocument();
            //QString cmd = QString("Gui.activeDocument().addAnnotation(\"%1\",\"%2\")").arg(fi.baseName()).arg(fi.absoluteFilePath());
            QString cmd =
                QString::fromLatin1("App.ActiveDocument.addObject(\"App::InventorObject\",\"%1\")."
                                    "FileName=\"%2\"\n"
        ...
        else if (ext == QLatin1String("wrl") || ext == QLatin1String("vrml")
            ...
            //QString cmd = QString("Gui.activeDocument().addAnnotation(\"%1\",\"%2\")").arg(fi.baseName()).arg(fi.absoluteFilePath());
            QString cmd =
                QString::fromLatin1("App.ActiveDocument.addObject(\"App::VRMLObject\",\"%1\")."
                                    "VrmlFile=\"%2\"\n"
                                    "App.ActiveDocument.ActiveObject.Label=\"%1\"\n"
                                    "App.ActiveDocument.recompute()")
                    .arg(fi.baseName(), fi.absoluteFilePath());
...
PyObject *Application::sInsert(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "et|s", "utf-8", &Name, &DocName)) return nullptr;
    ...
    PY_TRY
    {
        ...
        if (ext == QLatin1String("iv")) {
            ...
            if (DocName) doc = App::GetApplication().getDocument(DocName);
            else
                doc = App::GetApplication().getActiveDocument();
            if (!doc) doc = App::GetApplication().newDocument(DocName);

            App::DocumentObject *obj =
                doc->addObject("App::InventorObject", (const char *)fi.baseName().toUtf8());
            ...
            App::DocumentObject *obj =
                doc->addObject("App::VRMLObject", (const char *)fi.baseName().toUtf8());
            obj->Label.setValue((const char *)fi.baseName().toUtf8());
...
PyObject *Application::sExport(PyObject * /*self*/, PyObject *args)
{
    PyObject *object;
    char *Name;
    if (!PyArg_ParseTuple(args, "Oet", &object, "utf-8", &Name)) return nullptr;
...
PyObject *Application::sSendActiveView(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|O!", &psCommandStr, &PyBool_Type, &suppress)) return nullptr;
    ...
    if (ppReturn) { return Py_BuildValue("s", ppReturn); }
...
{
    char *fn = nullptr;
    if (!PyArg_ParseTuple(args, "s", &fn)) return nullptr;
...
{
    char *psKey;
    if (!PyArg_ParseTuple(args, "s", &psKey)) return nullptr;
...
{
    PyObject *pcObject;
    if (!PyArg_ParseTuple(args, "O", &pcObject)) return nullptr;
...
{
    char *filePath;
    if (!PyArg_ParseTuple(args, "et", "utf-8", &filePath)) return nullptr;
...
PyObject *Application::sAddIcon(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss*|s", &iconName, &content, &format)) return nullptr;
...
{
    char *iconName;
    if (!PyArg_ParseTuple(args, "s", &iconName)) return nullptr;

    PythonWrapper wrap;
    wrap.loadGuiModule();
    wrap.loadWidgetsModule();
    auto pixmap = BitmapFactory().pixmap(iconName);
    if (!pixmap.isNull()) return Py::new_reference_to(wrap.fromQIcon(new QIcon(pixmap)));
...
PyObject *Application::sAddCommand(PyObject * /*self*/, PyObject *args)
{
    ...
    PyObject *pcCmdObj;
    if (!PyArg_ParseTuple(args, "sO|s", &pName, &pcCmdObj, &pSource)) return nullptr;
    ...
    try {
        ...
        boost::smatch what;
        if (boost::regex_search(file, what, rx)) { group = what[1]; }
        else {
    ...
    try {
        ...
        if (cmd.hasAttr("GetCommands")) {
            ...
            if (!module.empty()) { cmd->setAppModuleName(module.c_str()); }
            if (!group.empty()) { cmd->setGroupName(group.c_str()); }
        ...
        PyErr_SetString(Base::PyExc_FC_GeneralError,
                        "Unknown C++ exception raised in Application::sAddCommand()");
        return nullptr;
    }
...
PyObject *Application::sRunCommand(PyObject * /*self*/, PyObject *args)
{
    char *pName;
    int item = 0;
    if (!PyArg_ParseTuple(args, "s|i", &pName, &item)) return nullptr;
...
{
    char *sCmd = nullptr;
    if (!PyArg_ParseTuple(args, "s", &sCmd)) return nullptr;
...
{
    char *pstr;
    if (!PyArg_ParseTuple(args, "s", &pstr)) return nullptr;
...
PyObject *Application::sShowPreferences(PyObject * /*self*/, PyObject *args)
{
    char *pstr = nullptr;
    int idx = 0;
    if (!PyArg_ParseTuple(args, "|si", &pstr, &idx)) return nullptr;

    Gui::Dialog::DlgPreferencesImp cDlg(getMainWindow());
    if (pstr) cDlg.activateGroupPage(QString::fromUtf8(pstr), idx);
...
PyObject *Application::sCreateViewer(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|is", &num_of_views, &title)) return nullptr;
    ...
    if (num_of_views <= 0) {
        ...
        if (title) viewer->setWindowTitle(QString::fromUtf8(title));
...
PyObject *Application::sGetMarkerIndex(PyObject * /*self*/, PyObject *args)
{
    char *pstr;
    int defSize = 9;
    if (!PyArg_ParseTuple(args, "s|i", &pstr, &defSize)) return nullptr;
    ...
    PY_TRY
    {
        ...
        std::list<std::pair<std::string, std::string>> markerList = {
            {"square", "DIAMOND_FILLED"}, {"cross", "CROSS"},        {"plus", "PLUS"},
            {"empty", "SQUARE_LINE"},     {"quad", "SQUARE_FILLED"}, {"circle", "CIRCLE_LINE"},
        ...
        for (markerStyle = markerList.begin(); markerStyle != markerList.end(); ++markerStyle) {
            if (marker_arg == (*markerStyle).first || marker_arg == (*markerStyle).second) break;
        }

        marker_arg = "CIRCLE_FILLED";

        if (markerStyle != markerList.end()) marker_arg = (*markerStyle).second;
...
{
    char *path, *mod = "";
    if (!PyArg_ParseTuple(args, "s|s", &path, &mod)) return nullptr;
...
PyObject *Application::sListUserEditModes(PyObject * /*self*/, PyObject *args)
{
    ...
    for (auto const &uem : Instance->listUserEditModes()) { ret.append(Py::String(uem.second)); }

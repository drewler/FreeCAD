...
QByteArray PythonOnlineHelp::loadResource(const QString &filename) const
{
    ...
    if (fn == QLatin1String("favicon.ico")) {
        ...
        if (writer.canWrite()) {
            ...
            for (int i = 0; i < (int)navicon_data_len; i++) { res[i] = navicon_data[i]; }
        }
    }
        ...
        QByteArray cmd = "import os, sys, pydoc, pkgutil\n"
                         "\n"
        ...
        PyObject *dict = PyModule_GetDict(main);
        dict = PyDict_Copy(dict);
        QByteArray cmd = "import pydoc\n"
                         "object, name = pydoc.resolve(\"";
        cmd += name.toUtf8();
        cmd += "\")\npage = pydoc.html.page(pydoc.describe(object), pydoc.html.document(object, "
               "name))\n";
        PyObject *result = PyRun_String(cmd.constData(), Py_file_input, dict, dict);
        if (result) {
            Py_DECREF(result);
...
HttpServer::HttpServer(QObject *parent) : QTcpServer(parent), disabled(false) {}
...
void HttpServer::pause() { disabled = true; }

void HttpServer::resume() { disabled = false; }
...
StdCmdPythonHelp::StdCmdPythonHelp() : Command("Std_PythonHelp"), server(nullptr)
{
    sGroup = "Tools";
...
void StdCmdPythonHelp::activated(int iMsg)
{
    ...
    qint16 port = 7465;
    if (!this->server) this->server = new HttpServer();
    ...
    if (this->server->isListening()
        ...
        if (failed) {
            QMessageBox::critical(
                Gui::getMainWindow(), QObject::tr("No Browser"),

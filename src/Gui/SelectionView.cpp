...
SelectionView::SelectionView(Gui::Document *pcDocument, QWidget *parent)
    : DockWindow(pcDocument, parent), SelectionObserver(true, ResolveMode::NoResolve), x(0.0f),
      y(0.0f), z(0.0f), openedAutomatically(false)
{
    ...
    connect(selectionView, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
            SLOT(toggleSelect(QListWidgetItem *)));
    connect(selectionView, SIGNAL(itemEntered(QListWidgetItem *)), this,
            SLOT(preselect(QListWidgetItem *)));
    connect(pickList, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
            SLOT(toggleSelect(QListWidgetItem *)));
    connect(pickList, SIGNAL(itemEntered(QListWidgetItem *)), this,
            SLOT(preselect(QListWidgetItem *)));
    connect(selectionView, SIGNAL(customContextMenuRequested(QPoint)), this,
            SLOT(onItemContextMenu(QPoint)));
    connect(enablePickList, SIGNAL(stateChanged(int)), this, SLOT(onEnablePickList()));
}

SelectionView::~SelectionView() {}

void SelectionView::leaveEvent(QEvent *) { Selection().rmvPreselect(); }
...
void SelectionView::onSelectionChanged(const SelectionChanges &Reason)
{
    ...
    if (Reason.Type == SelectionChanges::AddSelection) {
        ...
        for (std::vector<SelectionSingleton::SelObj>::iterator it = objs.begin(); it != objs.end();
             ++it) {
            // save as user data
            QStringList list;
...
void SelectionView::search(const QString &text)
{
    if (!text.isEmpty()) {
        ...
        if (doc) {
            ...
            selectionView->clear();
            for (std::vector<App::DocumentObject *>::iterator it = objects.begin();
                 it != objects.end(); ++it) {
...
void SelectionView::validateSearch()
{
    if (!searchList.empty()) {
        ...
        if (doc) {
            ...
            for (std::vector<App::DocumentObject *>::iterator it = searchList.begin();
...
{
    if (!item) item = selectionView->currentItem();
    if (!item) return;
    ...
    try {
        ...
        QString cmd = QString::fromLatin1(
                          "Gui.Selection.addSelection(App.getDocument(\"%1\").getObject(\"%2\"))")
...
void SelectionView::deselect()
{
    ...
    QString cmd = QString::fromLatin1(
                      "Gui.Selection.removeSelection(App.getDocument(\"%1\").getObject(\"%2\"))")
                      .arg(elements[0], elements[1]);
    try {
        Gui::Command::runCommand(Gui::Command::Gui, cmd.toLatin1());
    }
    ...
    *objname++ = 0;
    ...
    if (Gui::Selection().isSelected(docname, objname, subname))
        cmd = QString::fromLatin1("Gui.Selection.removeSelection("
                  ...
                  .arg(QString::fromLatin1(docname), QString::fromLatin1(objname),
...
void SelectionView::touch()
{
    ...
    QStringList elements = item->data(Qt::UserRole).toStringList();
    if (elements.size() < 2) return;
    QString cmd = QString::fromLatin1("App.getDocument(\"%1\").getObject(\"%2\").touch()")
                      .arg(elements[0], elements[1]);
    try {
...
void SelectionView::toPython()
{
    ...
    try {
        QString cmd = QString::fromLatin1("obj = App.getDocument(\"%1\").getObject(\"%2\")")
                          .arg(elements[0], elements[1]);
        Gui::Command::runCommand(Gui::Command::Gui, cmd.toLatin1());
    ...
    if (elements.length() > 2) {
        ...
        if (!module.isEmpty() && !property.isEmpty() && supportPart(obj, elements[2])) {
            try {
                ...
                QString cmd =
                    QString::fromLatin1("%1.show(App.getDocument(\"%2\").getObject(\"%3\").%4.%5)")
...
QString SelectionView::getModule(const char *type) const
{
    ...
    while (!typeId.isBad()) {
        ...
        std::string module;
        if (pos != std::string::npos) module = std::string(temp, 0, pos);
        if (module != "App") prefix = module;
        else
            break;
...
QString SelectionView::getProperty(App::DocumentObject *obj) const
{
    ...
    if (obj->getTypeId().isDerivedFrom(App::GeoFeature::getClassTypeId())) {
        ...
        if (App::Property::isValidName(name)) { property = QString::fromLatin1(name); }
    }
...
bool SelectionView::supportPart(App::DocumentObject *obj, const QString &part) const
{
    if (obj->getTypeId().isDerivedFrom(App::GeoFeature::getClassTypeId())) {
        ...
        if (data) {
            ...
            for (auto it : types) {
                if (part.startsWith(QString::fromLatin1(it))) return true;
            }
        }
    }
...
bool SelectionView::onMsg(const char * /*pMsg*/, const char ** /*ppReturn*/) { return false; }

void SelectionView::hideEvent(QHideEvent *ev) { DockWindow::hideEvent(ev); }

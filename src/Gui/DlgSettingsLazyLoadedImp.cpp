...
DlgSettingsLazyLoadedImp::DlgSettingsLazyLoadedImp(QWidget *parent)
    : PreferencePage(parent), ui(new Ui_DlgSettingsLazyLoaded)
{
    ui->setupUi(this);
}
...
void DlgSettingsLazyLoadedImp::loadSettings()
{
    ...
    while (std::getline(stream, workbench, ',')) _backgroundAutoloadedModules.push_back(workbench);
...
void DlgSettingsLazyLoadedImp::buildUnloadedWorkbenchList()
{
    ...
    ui->workbenchTable->horizontalHeader()->setSectionResizeMode(
        0, QHeaderView::ResizeMode::ResizeToContents);
    ui->workbenchTable->horizontalHeader()->setSectionResizeMode(1,
    ...
    for (const auto &wbName : workbenches) {
        ...
        iconLabel->setPixmap(wbIcon.scaled(QSize(20, 20), Qt::AspectRatioMode::KeepAspectRatio,
        ...
        autoloadCheckbox->setToolTip(tr("If checked") + QString::fromUtf8(", ") + wbDisplayName
                                     + QString::fromUtf8(" ")
        ...
        if (wbName.toStdString() == _startupModule) {
            ...
            autoloadCheckbox->setToolTip(
                tr("This is the current startup module, and must be autoloaded. See "
                   "Preferences/General/Autoload to change."));
        }
        ...
        if (WorkbenchManager::instance()->getWorkbench(wbName.toStdString())) {
            ...
            connect(button, &QPushButton::clicked, this,
                    [this, wbName]() { onLoadClicked(wbName); });

...
DlgWorkbenchesImp::DlgWorkbenchesImp(QWidget *parent)
    : CustomizeActionPage(parent), ui(new Ui_DlgWorkbenches)
{
    ui->setupUi(this);
    set_lw_properties(ui->lw_enabled_workbenches);
    set_lw_properties(ui->lw_disabled_workbenches);
    ui->lw_disabled_workbenches->setProperty(
        "OnlyAcceptFrom", QStringList() << ui->lw_enabled_workbenches->objectName());
    ui->lw_disabled_workbenches->setSortingEnabled(true);

    ui->lw_enabled_workbenches->setProperty("OnlyAcceptFrom",
                                            QStringList()
                                                << ui->lw_enabled_workbenches->objectName()
    ...
    for (QStringList::Iterator it = enabled_wbs_list.begin(); it != enabled_wbs_list.end(); ++it) {
        if (workbenches.contains(*it)) { add_workbench(ui->lw_enabled_workbenches, *it); }
        else {
    ...
    for (QStringList::Iterator it = workbenches.begin(); it != workbenches.end(); ++it) {
        if (disabled_wbs_list.contains(*it)) { add_workbench(ui->lw_disabled_workbenches, *it); }
...
void DlgWorkbenchesImp::onAddMacroAction(const QByteArray &macro) { Q_UNUSED(macro); }

void DlgWorkbenchesImp::onRemoveMacroAction(const QByteArray &macro) { Q_UNUSED(macro); }

void DlgWorkbenchesImp::onModifyMacroAction(const QByteArray &macro) { Q_UNUSED(macro); }

void DlgWorkbenchesImp::move_workbench(QListWidgetCustom *lwc_dest, QListWidgetItem *wi)
{
...
void DlgWorkbenchesImp::on_add_to_enabled_workbenches_btn_clicked()
{
    ...
    if (ci) { move_workbench(ui->lw_enabled_workbenches, ci); }
}
...
void DlgWorkbenchesImp::on_remove_from_enabled_workbenches_btn_clicked()
{
    ...
    if (ci) { move_workbench(ui->lw_disabled_workbenches, ci); }
}
...
void DlgWorkbenchesImp::on_shift_workbench_up_btn_clicked() { shift_workbench(true); }

void DlgWorkbenchesImp::on_shift_workbench_down_btn_clicked() { shift_workbench(false); }
...
QStringList DlgWorkbenchesImp::load_enabled_workbenches()
{
    ...
    enabled_wbs = QString::fromStdString(
        hGrp->GetASCII("Enabled", all_workbenches.toStdString().c_str()).c_str());

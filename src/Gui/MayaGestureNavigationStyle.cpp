...
SbBool MayaGestureNavigationStyle::processSoEvent(const SoEvent *const ev)
{
    ...
    if (evIsButton || evIsLoc2) { this->lastmouseposition = posn; }
    ...
    int cntMBBefore = (comboBefore & BUTTON1DOWN
                           ? 1
    ...
    if (evIsLoc2 || evIsButton) { this->mouseMoveThresholdBroken |= this->testMoveThreshold(pos); }
    ...
    if (evIsButton) {
        if (inGesture) {
            inGesture =
                false; //reset the flag when mouse clicks are received, to ensure enabling mouse navigation back.
            setViewingMode(
                NavigationStyle::
                    SELECTION); //exit navigation asap, to proceed with regular processing of the click
        }
    }
    ...
    bool propagated =
        false; //an internal flag indicating that the event has been already passed to inherited, to suppress the automatic doing of this at the end.
    //goto finalize = return processed. Might be important to do something before done (none now).

    // give the nodes in the foreground root the chance to handle events (e.g color bar)
    if (!viewer->isEditing()) { processed = handleEventInForeground(ev); }
    if (processed) goto finalize;
    ...
    if (evIsLoc3) {
        ...
        if (event) this->processMotionEvent(event);
        processed = true;
    }
    if (processed) goto finalize;
        ...
        case NavigationStyle::INTERACT: {
            ...
            if (evIsButton) {
                ...
                switch (button) {
                    ...
                    case SoMouseButtonEvent::BUTTON2:
                        if (press) {
                            if (this->thisClickIsComplex && this->mouseMoveThresholdBroken) {
                                ...
                                pan(viewer->getSoRenderManager()
                                        ->getCamera()); //set up panningplane
                                int &cnt = this->mousedownConsumedCount;
                                this->mousedownConsumedEvents[cnt] =
                                    *event; //hopefully, a shallow copy is enough. There are no pointers stored in events, apparently. Will lose a subclass, though.
                                cnt++;
                                assert(cnt <= 2);
                                ...
                                processed =
                                    true; //just consume this event, and wait for the move threshold to be broken to start dragging/panning
                            }
                        }
                        else { //release
                            ...
                            if (!processed) {
                                ...
                                processed = inherited::processSoEvent(
                                    ev); //explicitly, just for clarity that we are sending a full click sequence.
                                propagated = true;
                            }
                        }
                        break;
                    case SoMouseButtonEvent::BUTTON3: //press the wheel
                        // starts PANNING mode
                        if (press & this->altdown) { setViewingMode(NavigationStyle::PANNING); }
                        else if (press) {
            ...
            if (evIsLoc2) {
                ...
                if (mousedownConsumedCount > 0)
                    processed =
                        true; //if we are still deciding if it's a drag or not, consume mouseMoves.
            }

            //gesture start
            if (evIsGesture
                && /*!this->button1down &&*/ !this
                       ->button2down) { //ignore gestures when mouse buttons are down. Button1down check was disabled because of wrong state after doubleclick on sketcher constraint to edit datum
                auto gesture = static_cast<const SoGestureEvent *>(ev);
                if (gesture->state == SoGestureEvent::SbGSStart
                    || gesture->state
                        == SoGestureEvent::
                            SbGSUpdate) { //even if we didn't get a start, assume the first update is a start (sort-of fail-safe).
        ...
        case NavigationStyle::PANNING: {
            ...
            if (evIsButton) {
                ...
                switch (button) {
                    ...
                    case SoMouseButtonEvent::
                        BUTTON3: // allows to release button3 into SELECTION mode
            ...
            if (evIsLoc2 && !this->inGesture && this->mouseMoveThresholdBroken) {
                if (curmode == NavigationStyle::ZOOMING) { //doesn't happen
                    ...
                    panCamera(viewer->getSoRenderManager()->getCamera(), ratio, this->panningplane,
                              posn, prevnormalized);
                    processed = true;
                }
                    ...
                    if (comboAfter & BUTTON1DOWN && comboAfter & BUTTON2DOWN) {
                        ...
                        this->spin_simplified(viewer->getSoRenderManager()->getCamera(), posn,
                                              prevnormalized);
                        processed = true;
                    }
                }
            ...
            if (evIsGesture) {
                ...
                if (gesture->state == SoGestureEvent::SbGSEnd) {
                    ...
                    if (type.isDerivedFrom(SoGesturePinchEvent::getClassTypeId())) {
                        ...
                        if (this->zoomAtCursor) {
                            ...
                            NavigationStyle::panCamera(viewer->getSoRenderManager()->getCamera(),
                                                       ratio, this->panningplane, panDist,
                                                       SbVec2f(0, 0));
                        }
                    ...
                    if (type.isDerivedFrom(SoGesturePanEvent::getClassTypeId())) {
                        ...
                        NavigationStyle::panCamera(viewer->getSoRenderManager()->getCamera(), ratio,
                                                   this->panningplane, panDist, SbVec2f(0, 0));
                        processed = true;
                    }
                }
                else {

...
BitmapFactoryInst::~BitmapFactoryInst() { delete d; }
...
void BitmapFactoryInst::restoreCustomPaths()
{
    ...
    for (auto &path : paths) { addPath(QString::fromUtf8(path.c_str())); }
}
...
void BitmapFactoryInst::addXPM(const char *name, const char **pXPM) { d->xpmMap[name] = pXPM; }
        ...
        if (!loadPixmap(fileName, icon)) {
            ...
            for (QList<QByteArray>::iterator fm = formats.begin(); fm != formats.end(); ++fm) {
                QString path = QString::fromLatin1("%1.%2").arg(
                    fileName, QString::fromLatin1((*fm).toLower().constData()));
                if (loadPixmap(path, icon)) { break; }
            }
        }
    }
...
QPixmap
BitmapFactoryInst::pixmapFromSvg(const char *name, const QSizeF &size,
                                 const std::map<unsigned long, unsigned long> &colorMapping) const
{
    ...
    if (iconPath.isEmpty()) {
        QString fileName = QString::fromLatin1("icons:") + fn;
        QFileInfo fi(fileName);
        if (fi.exists()) { iconPath = fi.filePath(); }
        else {
...
QPixmap
BitmapFactoryInst::pixmapFromSvg(const QByteArray &originalContents, const QSizeF &size,
                                 const std::map<unsigned long, unsigned long> &colorMapping) const
{
...
QPixmap BitmapFactoryInst::fillRect(int x, int y, int w, int h, const QPixmap &p,
                                    Qt::BGMode bgmode) const
{
    ...
    QPainter pt;
    pt.begin(&b);
    if (bgmode == Qt::OpaqueMode) pt.fillRect(x, y, w, h, Qt::color1); // make opaque
...
QPixmap BitmapFactoryInst::merge(const QPixmap &p1, const QPixmap &p2, Position pos) const
{
    ...
    switch (pos) {
        case TopLeft: break;
        case TopRight: x = p1.width() / dpr1 - p2.width() / dpr2; break;
        case BottomLeft: y = p1.height() / dpr1 - p2.height() / dpr2; break;
        case BottomRight:
...
void BitmapFactoryInst::convert(const QImage &p, SoSFImage &img) const
{
    ...
    if (!table.isEmpty()) {
        if (p.hasAlphaChannel()) {
            if (p.allGray()) numcomponents = 2;
            else
                numcomponents = 4;
        }
        else {
            if (p.allGray()) numcomponents = 1;
            else
                numcomponents = 3;
        }
    }
...
void BitmapFactoryInst::convert(const SoSFImage &p, QImage &img) const
{
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            switch (numcomponents) {
                default:
                case 1: *bits++ = qRgb(line[0], line[0], line[0]); break;
                case 2: *bits++ = qRgba(line[0], line[0], line[0], line[1]); break;
                case 3: *bits++ = qRgb(line[0], line[1], line[2]); break;
                case 4: *bits++ = qRgba(line[0], line[1], line[2], line[3]); break;
            }
...
QIcon BitmapFactoryInst::mergePixmap(const QIcon &base, const QPixmap &px,

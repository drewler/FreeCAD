...
ViewProviderDocumentObject::ViewProviderDocumentObject() : pcObject(nullptr), pcDocument(nullptr)
{
    static const char *dogroup = "Display Options";
    static const char *sgroup = "Selection";

    ADD_PROPERTY_TYPE(DisplayMode, ((long)0), dogroup, App::Prop_None, "Set the display mode");
    ADD_PROPERTY_TYPE(Visibility, (true), dogroup, App::Prop_None,
                      "Show the object in the 3d view");
    ADD_PROPERTY_TYPE(ShowInTree, (true), dogroup, App::Prop_None,
                      "Show the object in the tree view");

    ADD_PROPERTY_TYPE(SelectionStyle, ((long)0), sgroup, App::Prop_None,
...
void ViewProviderDocumentObject::getTaskViewContent(
    std::vector<Gui::TaskView::TaskContent *> &vec) const
{
...
void ViewProviderDocumentObject::startRestoring()
{
    ...
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionStartRestoring();
}

void ViewProviderDocumentObject::finishRestoring()
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionFinishRestoring();
}

bool ViewProviderDocumentObject::isAttachedToDocument() const { return (!testStatus(Detach)); }
...
bool ViewProviderDocumentObject::removeDynamicProperty(const char *name)
{
    ...
    if (document) document->addOrRemovePropertyOfObject(this, prop, false);

    return ViewProvider::removeDynamicProperty(name);
}

App::Property *ViewProviderDocumentObject::addDynamicProperty(const char *type, const char *name,
                                                              ...
                                                              short attr, bool ro, bool hidden)
{
    ...
    if (prop) {
        ...
        App::DocumentObject *docobject = getObject();
        App::Document *document = docobject ? docobject->getDocument() : nullptr;
        if (document) document->addOrRemovePropertyOfObject(this, prop, true);
    }
    return prop;
}
...
void ViewProviderDocumentObject::onBeforeChange(const App::Property *prop)
{
    if (isAttachedToDocument()) {
        ...
        if (doc) { onBeforeChangeProperty(doc, prop); }
    }
...
void ViewProviderDocumentObject::onChanged(const App::Property *prop)
{
    if (prop == &DisplayMode) { setActiveMode(); }
        ...
        if (getRoot()->isOfType(SoFCSelectionRoot::getClassTypeId())) {
            static_cast<SoFCSelectionRoot *>(getRoot())->selectionStyle =
                SelectionStyle.getValue() ? SoFCSelectionRoot::Box : SoFCSelectionRoot::Full;
        }
    }

    if (prop && !prop->testStatus(App::Property::NoModify) && pcDocument
        && !pcDocument->isModified() && testStatus(Gui::ViewStatus::TouchDocument)) {
        if (prop) FC_LOG(prop->getFullName() << " changed");
...
bool ViewProviderDocumentObject::isShowable() const { return _Showable; }

void ViewProviderDocumentObject::setShowable(bool enable)
{
    if (_Showable == enable) return;

    _Showable = enable;
    int which = getModeSwitch()->whichChild.getValue();
    if (_Showable && which == -1 && Visibility.getValue()) { setModeSwitch(); }
    else if (!_Showable) {
        if (which >= 0) ViewProvider::hide();
    }
}
...
void ViewProviderDocumentObject::startDefaultEditMode()
{
    ...
    if (document) { document->setEdit(this, ViewProvider::Default); }
}
...
{
    if (isShowable()) ViewProvider::setModeSwitch();
}

void ViewProviderDocumentObject::show()
{
    if (TreeWidget::isObjectShowable(getObject())) ViewProvider::show();
    else {
        Visibility.setValue(false);
...
{
    if (!pcObject || testStatus(ViewStatus::UpdatingView)) return;

    Base::ObjectStatusLocker<ViewStatus, ViewProviderDocumentObject> lock(ViewStatus::UpdatingView,
                                                                          this);

    // Disable object visibility syncing
    Base::ObjectStatusLocker<App::Property::Status, App::Property> lock2(App::Property::User1,
                                                                         &Visibility);
    ...
    for (const auto &it : Map) { updateData(it.second); }
...
void ViewProviderDocumentObject::attach(App::DocumentObject *pcObj)
{
    ...
    if (aDisplayModesArray.empty()) aDisplayModesArray.emplace_back("");
    ...
    for (std::vector<std::string>::iterator it = aDisplayModesArray.begin();
         it != aDisplayModesArray.end(); ++it) {
    ...
    if (!isRestoring()) {
        ...
        if (defmode) DisplayMode.setValue(defmode);
    }

    //attach the extensions
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionAttach(pcObj);
}

void ViewProviderDocumentObject::reattach(App::DocumentObject *pcObj)
{
    auto vector = getExtensionsDerivedFromType<Gui::ViewProviderExtension>();
    for (Gui::ViewProviderExtension *ext : vector) ext->extensionReattach(pcObj);
}
...
void ViewProviderDocumentObject::update(const App::Property *prop)
{
    ...
    if (prop == &getObject()->Visibility) {
        ...
        // Disable object visibility syncing
        Base::ObjectStatusLocker<App::Property::Status, App::Property> guard(App::Property::User1,
                                                                             &Visibility);
...
{
    if (!pcObject) throw Base::RuntimeError("View provider detached");
    if (pcDocument) { return pcDocument; }
    else {
...
Gui::MDIView *ViewProviderDocumentObject::getInventorView() const
{
    ...
    Gui::MDIView *mdi =
        pGuiDoc->getEditingViewOfViewProvider(const_cast<ViewProviderDocumentObject *>(this));
    if (!mdi) {
...
void ViewProviderDocumentObject::setActiveMode()
{
    ...
    if (!Visibility.getValue()) ViewProvider::hide();
}

bool ViewProviderDocumentObject::canDelete(App::DocumentObject *obj) const
{
    Q_UNUSED(obj)
    if (getObject()->hasExtension(App::GroupExtension::getExtensionClassTypeId())) return true;
    if (getObject()->isDerivedFrom(App::Origin::getClassTypeId())) return true;
    return false;
}

PyObject *ViewProviderDocumentObject::getPyObject()
{
    if (!pyViewObject) pyViewObject = new ViewProviderDocumentObjectPy(this);
    pyViewObject->IncRef();
    return pyViewObject;
}
                                                 ...
                                                 const std::vector<std::string> &elements) const
{
    ...
    if (obj && obj->getDocument() != getObject()->getDocument()) return false;
...
{
    if (!oldObj || !oldObj->getNameInDocument() || !newObj || !newObj->getNameInDocument()) {
        ...
        for (auto o : doc->getObjects()) {
            ...
            for (auto prop : props) {
                ...
                std::unique_ptr<App::Property> copy(
                    linkProp->CopyOnLinkReplace(obj, oldObj, newObj));
                if (!copy) continue;
...
bool ViewProviderDocumentObject::showInTree() const { return ShowInTree.getValue(); }
                                                  ...
                                                  std::string &subname) const
{
    ...
    if (vp->getElementPicked(pp, subname)) str << subname;
    subname = str.str();
    return true;
}
                                               ...
                                               SoDetail *&det) const
{
    ...
    if (!append && len >= 2) len -= 2;
    if (ViewProvider::getDetailPath(subname, path, append, det)) {
        if (det || !subname || !*subname) return true;
    }
    ...
    auto childRoot = getChildRoot();
    if (!childRoot) path->truncate(len);
    else {
        auto idx = pcModeSwitch->whichChild.getValue();
        if (idx < 0 || pcModeSwitch->getChild(idx) != childRoot) return false;
...
ViewProviderDocumentObject *ViewProviderDocumentObject::getLinkedViewProvider(std::string *subname,
                                                                              bool recursive) const
{
    (void)subname;
    auto self = const_cast<ViewProviderDocumentObject *>(this);
    if (!pcObject || !pcObject->getNameInDocument()) return self;
    auto linked = pcObject->getLinkedObject(recursive);
    if (!linked || linked == pcObject) return self;
...
{
    if (pcObject) return pcObject->getFullName() + ".ViewObject";
    return std::string("?");
}

...
QList<QUrl> FileDialog::fetchSidebarUrls()
{
    ...
    for (const auto &it : list) {
        if (QFileInfo::exists(it)) { urls << QUrl::fromLocalFile(it); }
    }
...
bool FileDialog::hasSuffix(const QString &ext) const
{
    ...
    for (const auto &str : filters) {
        if (rx.indexIn(str) != -1) { return true; }
    }
    ...
    if (acceptMode() == QFileDialog::AcceptSave) {
        ...
        if (!files.isEmpty()) {
            ...
            if (!ext.isEmpty() && (suffix.isEmpty() || !hasSuffix(suffix))) {
                ...
                if (fileNameEdit) fileNameEdit->setText(file);
            }
        }
    }
    QFileDialog::accept();
                                    ...
                                    const QString &filter, QString *selectedFilter, Options options)
{
    ...
    if (dirName.isEmpty()) { dirName = getWorkingDirectory(); }
    else {
        QFileInfo fi(dir);
        if (fi.isRelative()) {
        ...
        if (!fi.fileName().isEmpty()) { hasFilename = true; }
        ...
        if (selectedFilter) { filterToSearch = selectedFilter; }
        else {
        ...
        if (index != -1) {
            ...
            if (fi.suffix().isEmpty()) dirName += suffix;
        }
    }

    QString windowTitle = caption;
    if (windowTitle.isEmpty()) windowTitle = FileDialog::tr("Save as");
    ...
    if (DialogOptions::dontUseNativeFileDialog()) {
        ...
        if (hasFilename) dlg.selectFile(dirName);
        ...
        file = QFileDialog::getSaveFileName(parent, windowTitle, dirName, filter, selectedFilter,
                                            options);
                                    ...
                                    const QString &filter, QString *selectedFilter, Options options)
{
    ...
    QString windowTitle = caption;
    if (windowTitle.isEmpty()) windowTitle = FileDialog::tr("Open");
    ...
    if (DialogOptions::dontUseNativeFileDialog()) {
        ...
        file = QFileDialog::getOpenFileName(parent, windowTitle, dirName, filter, selectedFilter,
                                            options);
        file = QDir::fromNativeSeparators(file);
    }
                                         ...
                                         QString *selectedFilter, Options options)
{
    ...
    if (DialogOptions::dontUseNativeFileDialog()) {
        ...
        files = QFileDialog::getOpenFileNames(parent, windowTitle, dirName, filter, selectedFilter,
                                              options);
        for (auto &file : files) { file = QDir::fromNativeSeparators(file); }
    }

    if (!files.isEmpty()) { setWorkingDirectory(files.front()); }
 ...
 */
QString FileDialog::getWorkingDirectory() { return workingDirectory; }
...
void FileDialog::setWorkingDirectory(const QString &dir)
{
    ...
    if (!dir.isEmpty()) {
        QFileInfo info(dir);
        if (!info.exists() || info.isFile()) dirName = info.absolutePath();
        else
...
FileOptionsDialog::FileOptionsDialog(QWidget *parent, Qt::WindowFlags fl)
    : QFileDialog(parent, fl), extensionPos(ExtensionRight)
{
...
void FileOptionsDialog::accept()
{
    ...
    if (fn.startsWith(QLatin1String("*"))) {
        ...
        QRegExp rx(QLatin1String("\\(\\*.(\\w+)"));
        QString suf = selectedNameFilter();
        if (rx.indexIn(suf) >= 0) suf = rx.cap(1);
        if (ext.isEmpty()) setDefaultSuffix(suf);
            ...
            if (fileNameEdit) fileNameEdit->setText(fn);
        }
    }
...
void FileOptionsDialog::toggleExtension()
{
    if (extensionWidget) {
        ...
        if (showIt) {
            ...
            if (extensionPos == ExtensionRight) { setFixedSize(width() + s.width(), height()); }
            else {
...
void FileOptionsDialog::setOptionsWidget(FileOptionsDialog::ExtensionPosition pos, QWidget *w,
                                         bool show)
{
    extensionPos = pos;
    extensionWidget = w;
    if (extensionWidget->parentWidget() != this) extensionWidget->setParent(this);
    ...
    oldSize = size();
    w->hide();
    if (show) toggleExtension();
}

QWidget *FileOptionsDialog::getOptionsWidget() const { return extensionWidget; }
...
QIcon FileIconProvider::icon(IconType type) const { return QFileIconProvider::icon(type); }
...
QIcon FileIconProvider::icon(const QFileInfo &info) const
{
    auto toUrl = [](const QFileInfo &info) {
        ...
        if (fi.isSymLink()) { fileName = fi.symLinkTarget(); }
    ...
    if (info.suffix().toLower() == QLatin1String("fcstd")) {
        ...
        darkIcon.addPixmap(icon.pixmap(w, w, QIcon::Disabled, QIcon::Off), QIcon::Normal,
                           QIcon::Off);
...
FileChooser::FileChooser(QWidget *parent)
    : QWidget(parent), md(File), accMode(AcceptOpen), _filter(QString())
{
 ...
 */
void FileChooser::setFileName(const QString &s) { lineEdit->setText(s); }
...
void FileChooser::chooseFile()
{
    ...
    if (prechosenDirectory.isEmpty()) { prechosenDirectory = FileDialog::getWorkingDirectory(); }

    QFileDialog::Options dlgOpt;
    if (DialogOptions::dontUseNativeFileDialog()) { dlgOpt = QFileDialog::DontUseNativeDialog; }

    QString fn;
    if (mode() == File) {
        if (acceptMode() == AcceptOpen)
            fn = QFileDialog::getOpenFileName(this, tr("Select a file"), prechosenDirectory,
                                              _filter, nullptr, dlgOpt);
        else
            fn = QFileDialog::getSaveFileName(this, tr("Select a file"), prechosenDirectory,
                                              _filter, nullptr, dlgOpt);
    }
    else {
...
{
    if (group->checkedButton()) QDialog::accept();
}

void SelectModule::reject()
{
    if (group->checkedButton()) QDialog::reject();
}

void SelectModule::onButtonClicked()
{
    if (group->checkedButton()) okButton->setEnabled(true);
    else
        okButton->setEnabled(false);
}
...
SelectModule::Dict SelectModule::exportHandler(const QStringList &fileNames, const QString &filter)
{
    ...
    if (!filter.isEmpty()) {
        ...
        if (it != filterList.end()) {
            ...
            for (const auto &fileName : fileNames) { dict[fileName] = module; }
            return dict;
        }
    }
    ...
    for (const auto &fileName : fileNames) {
        ...
        if (!filters.empty()) dict[fileName] = QString::fromLatin1(filters.begin()->second.c_str());
    }
    ...
    for (QMap<QString, SelectModule::Dict>::const_iterator it = filetypeHandler.cbegin();
        ...
        if (it.value().size() > 1) {
            ...
            if (dlg.exec()) {
                ...
                for (const auto &file : files) dict[file] = mod;
            }
        }
    }

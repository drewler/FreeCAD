...
TaskSelectLinkProperty::TaskSelectLinkProperty(const char *sFilter, App::Property *prop,
                                               QWidget *parent)
    : TaskBox(Gui::BitmapFactory().pixmap("mouse_pointer"), tr("edit selection"), true, parent),
      Filter(nullptr), LinkSub(nullptr), LinkList(nullptr)
{
...
void TaskSelectLinkProperty::activate()
{
    ...
    if (LinkSub) {
        ...
        if (StartObject) {
            ...
            for (std::vector<std::string>::const_iterator it = StartValueBuffer.begin();
                 it != StartValueBuffer.end(); ++it) {
...
void TaskSelectLinkProperty::sendSelection2Property()
{
    if (LinkSub) {
        ...
        for (auto &it : sel) temp.push_back(it.getObject());

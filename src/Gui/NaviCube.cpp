...
public:
    ...
    int m_RenderPass;
    Face(int firstVertex, int vertexCount, GLuint textureId, int pickId, int pickTexId,
         GLuint pickTextureId, const QColor &color, int renderPass)
    {
...
private:
    ...
    SbRotation rotateView(SbRotation, int axis, float rotAngle,
                          SbVec3f customAxis = SbVec3f(0, 0, 0)) const;
...
NaviCube::~NaviCube() { delete m_NaviCubeImplementation; }

void NaviCube::drawNaviCube() { m_NaviCubeImplementation->drawNaviCube(); }
...
NaviCubeImplementation::~NaviCubeImplementation()
{
    ...
    delete m_Menu;
    if (m_PickingFramebuffer) delete m_PickingFramebuffer;
    for (vector<Face *>::iterator f = m_Faces.begin(); f != m_Faces.end(); f++) delete *f;
                                                 ...
                                                 int shape)
{
    ...
    if (text) {
        ...
        // Override fromString
        if (hGrp->GetInt("FontWeight") > 0) { sansFont.setWeight(hGrp->GetInt("FontWeight")); }
        if (hGrp->GetInt("FontStretch") > 0) { sansFont.setStretch(hGrp->GetInt("FontStretch")); }
        paint.setFont(sansFont);
        paint.drawText(QRect(0, 0, texSize, texSize), Qt::AlignCenter,
                       qApp->translate("Gui::NaviCube", text));
    }
        ...
        pathEdge.addRect(QRectF(gapi, ((qreal)texSize - sqrt(2) * gapi) * 0.5,
                                (qreal)texSize - 2.0 * gapi, sqrt(2) * gapi));
...
void NaviCubeImplementation::addFace(float gap, const Vector3f &x, const Vector3f &z, int frontTex,
                                     int pickTex, int pickId, bool text)
{
    ...
    // TEX_TOP 			pickId
    Face *FaceFront = new Face(m_IndexArray.size(), 4, m_Textures[pickTex], pickId, pickTex,
                               m_Textures[pickTex], m_FrontFaceColor, 1);
    m_Faces.push_back(FaceFront);

    if (text) {
        Face *FaceText = new Face(m_IndexArray.size(), 4, m_Textures[frontTex], pickId, pickTex,
                                  m_Textures[pickTex], m_TextColor, 2);
        m_Faces.push_back(FaceText);
    }

    for (int i = 0; i < 4; i++) m_IndexArray.push_back(t + i);
}
...
void NaviCubeImplementation::initNaviCube(QtGLWidget *gl)
{
    ...
    sn = sin(90 * M_PI / 180);
    Matrix3f r90x;
    r90x << 1, 0, 0, 0, cs, -sn, 0, sn, cs;

    Matrix3f r90y;
    r90y << cs, 0, sn, 0, 1, 0, -sn, 0, cs;

    Matrix3f r90z;
    r90z << cs, sn, 0, -sn, cs, 0, 0, 0, 1;

    cs = cos(45 * M_PI / 180);
    sn = sin(45 * M_PI / 180);
    Matrix3f r45x;
    r45x << 1, 0, 0, 0, cs, -sn, 0, sn, cs;

    Matrix3f r45z;
    r45z << cs, sn, 0, -sn, cs, 0, 0, 0, 1;
    ...
    m_PickingFramebuffer = new QtGLFramebufferObject(2 * m_CubeWidgetSize, 2 * m_CubeWidgetSize,
                                                     QtGLFramebufferObject::CombinedDepthStencil);
}

void NaviCubeImplementation::drawNaviCube()
{
    glViewport(m_CubeWidgetPosX - m_CubeWidgetSize / 2, m_CubeWidgetPosY - m_CubeWidgetSize / 2,
               m_CubeWidgetSize, m_CubeWidgetSize);
    drawNaviCube(false);
}
...
void NaviCubeImplementation::drawNaviCube(bool pickMode)
{
    ...
    if (pickMode) {
        for (vector<Face *>::iterator f = m_Faces.begin(); f != m_Faces.end(); f++) {
            ...
            glDrawElements(GL_TRIANGLE_FAN, (*f)->m_VertexCount, GL_UNSIGNED_BYTE,
                           (void *)&m_IndexArray[(*f)->m_FirstVertex]);
        }
    }
    else {
        for (int pass = 0; pass < 3; pass++) {
            for (vector<Face *>::iterator f = m_Faces.begin(); f != m_Faces.end(); f++) {
                ...
                //	glBindTexture(GL_TEXTURE_2D, (*f)->m_PickTextureId);
                //} else {
                if (pass != (*f)->m_RenderPass) continue;
                QColor &c =
                    (m_HiliteId == (*f)->m_PickId) && (pass < 2) ? m_HiliteColor : (*f)->m_Color;
...
int NaviCubeImplementation::pickFace(short x, short y)
{
    ...
    if (m_PickingFramebuffer) {
        ...
        glReadPixels(2 * (x - (m_CubeWidgetPosX - m_CubeWidgetSize / 2)),
                     2 * (y - (m_CubeWidgetPosY - m_CubeWidgetSize / 2)), 1, 1, GL_RGBA,
...
SbRotation NaviCubeImplementation::rotateView(SbRotation viewRot, int axis, float rotAngle,
                                              SbVec3f customAxis) const
{
    SbVec3f up;
    ...
    SbVec3f direction;
    switch (axis) {
        default: return viewRot;
        case DIR_UP: direction = up; break;
        case DIR_OUT: direction = out; break;
        case DIR_RIGHT: direction = right; break;
    }

    if (customAxis != SbVec3f(0, 0, 0)) direction = customAxis;
...
void NaviCubeImplementation::handleMenu() { m_Menu->exec(QCursor::pos()); }
...
bool NaviCubeImplementation::mouseReleased(short x, short y)
{
    ...
    if (!m_Dragging) {
        ...
        ParameterGrp::handle hGrpNavi = App::GetApplication().GetParameterGroupByPath(
            "User parameter:BaseApp/Preferences/NaviCube");
        ...
        switch (pick) {
            ...
            case TEX_FRONT:
                ...
                if (toNearest) {
                    if (ViewRotMatrix[0][0] < 0
                        && abs(ViewRotMatrix[0][0]) >= abs(ViewRotMatrix[1][0]))
                        viewRot = rotateView(viewRot, 2, 180);
                    else if (ViewRotMatrix[1][0] > 0
                             && abs(ViewRotMatrix[1][0]) > abs(ViewRotMatrix[0][0]))
            ...
            case TEX_LEFT:
                ...
                if (toNearest) {
                    if (ViewRotMatrix[0][1] > 0
                        && abs(ViewRotMatrix[0][1]) >= abs(ViewRotMatrix[1][1]))
                        viewRot = rotateView(viewRot, 2, 180);
                    else if (ViewRotMatrix[1][1] > 0
                             && abs(ViewRotMatrix[1][1]) > abs(ViewRotMatrix[0][1]))
                        viewRot = rotateView(viewRot, 2, -90);
                    else if (ViewRotMatrix[1][1] < 0
                             && abs(ViewRotMatrix[1][1]) > abs(ViewRotMatrix[0][1]))
                        viewRot = rotateView(viewRot, 2, 90);
            ...
            case TEX_FRONT_LEFT:
                ...
                if (toNearest) {
                    if (ViewRotMatrix[1][2] < 0
                        && abs(ViewRotMatrix[1][2]) >= abs(ViewRotMatrix[0][2]))
                        viewRot = rotateView(viewRot, 2, 180);
                    else if (ViewRotMatrix[0][2] > 0
                             && abs(ViewRotMatrix[0][2]) > abs(ViewRotMatrix[1][2]))
                        viewRot = rotateView(viewRot, 2, -90);
                    else if (ViewRotMatrix[0][2] < 0
                             && abs(ViewRotMatrix[0][2]) > abs(ViewRotMatrix[1][2]))
                        viewRot = rotateView(viewRot, 2, 90);
            ...
            case TEX_TOP_REAR_RIGHT:
                ...
                break;
            case TEX_ARROW_LEFT: viewRot = rotateView(viewRot, DIR_OUT, rotStepAngle); break;
            case TEX_ARROW_RIGHT: viewRot = rotateView(viewRot, DIR_OUT, -rotStepAngle); break;
            case TEX_ARROW_WEST: viewRot = rotateView(viewRot, DIR_UP, -rotStepAngle); break;
            case TEX_ARROW_EAST: viewRot = rotateView(viewRot, DIR_UP, rotStepAngle); break;
            case TEX_ARROW_NORTH: viewRot = rotateView(viewRot, DIR_RIGHT, -rotStepAngle); break;
            case TEX_ARROW_SOUTH: viewRot = rotateView(viewRot, DIR_RIGHT, rotStepAngle); break;
            case TEX_DOT_BACKSIDE: viewRot = rotateView(viewRot, 0, 180); break;
        ...
        if (applyRotation) rotateView(viewRot);
    }
    return true;
}
...
bool NaviCubeImplementation::mouseMoved(short x, short y)
{
    ...
    if (m_MouseDown) {
        if (m_MightDrag && !m_Dragging && !inDragZone(x, y)) m_Dragging = true;
        if (m_Dragging) {
            setHilite(0);
            SbVec2s view = m_View3DInventorViewer->getSoRenderManager()->getSize();
            int width = view[0];
...
bool NaviCubeImplementation::processSoEvent(const SoEvent *ev)
{
    ...
    if (ev->getTypeId().isDerivedFrom(SoMouseButtonEvent::getClassTypeId())) {
        ...
        if (mbev->isButtonPressEvent(mbev, SoMouseButtonEvent::BUTTON1)) return mousePressed(x, y);
        if (mbev->isButtonReleaseEvent(mbev, SoMouseButtonEvent::BUTTON1))
            return mouseReleased(x, y);
    }
    if (ev->getTypeId().isDerivedFrom(SoLocation2Event::getClassTypeId())) return mouseMoved(x, y);
    return false;
}


QString NaviCubeImplementation::str(char *str) { return QString::fromLatin1(str); }
...
ViewIsometricCmd::ViewIsometricCmd() : Command("ViewIsometricCmd")
{
    sGroup = "";
...
ViewOrthographicCmd::ViewOrthographicCmd() : Command("ViewOrthographicCmd")
{
    sGroup = "";
...
ViewPerspectiveCmd::ViewPerspectiveCmd() : Command("ViewPerspectiveCmd")
{
    sGroup = "";
...
ViewZoomToFitCmd::ViewZoomToFitCmd() : Command("ViewZoomToFit")
{
    sGroup = "";
...
QMenu *NaviCubeImplementation::createNaviCubeMenu()
{
    ...
    for (const auto &command : commands) {
        if (command == "Separator") { menu->addSeparator(); }
        else {

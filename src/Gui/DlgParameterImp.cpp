...
DlgParameterImp::DlgParameterImp(QWidget *parent, Qt::WindowFlags fl)
    : QDialog(parent, fl | Qt::WindowMinMaxButtonsHint), ui(new Ui_DlgParameter)
{
    ui->setupUi(this);
    ...
    const std::map<std::string, ParameterManager *> &rcList =
        App::GetApplication().GetParameterSetList();
    ...
    if (ui->parameterSet->count() < 2) ui->parameterSet->hide();

    connect(ui->parameterSet, SIGNAL(activated(int)), this, SLOT(onChangeParameterSet(int)));
    connect(paramGroup, SIGNAL(currentItemChanged(QTreeWidgetItem *, QTreeWidgetItem *)), this,
            SLOT(onGroupSelected(QTreeWidgetItem *)));
...
{
    if (finder.isNull()) finder = new DlgParameterFind(this);
    finder->show();
}
...
void DlgParameterImp::on_findGroupLE_textChanged(const QString &SearchStr)
{
    ...
    if (!foundList.empty()) {
        ...
        if (!ui->findGroupLE->styleSheet().isEmpty()) ui->findGroupLE->setStyleSheet(QString());
        for (QTreeWidgetItem *item : qAsConst(foundList)) {
...
void DlgParameterImp::on_closeButton_clicked() { close(); }

void DlgParameterImp::accept() { close(); }

void DlgParameterImp::reject() { close(); }
...
void DlgParameterImp::onGroupSelected(QTreeWidgetItem *item)
{
    if (item && item->type() == QTreeWidgetItem::UserType + 1) {
        ...
        for (const auto &It3 : mcIntMap) {
            (void)new ParameterInt(paramValue, QString::fromUtf8(It3.first.c_str()), It3.second,
                                   _hcGrp);
        }
        ...
        for (const auto &It4 : mcFloatMap) {
            (void)new ParameterFloat(paramValue, QString::fromUtf8(It4.first.c_str()), It4.second,
                                     _hcGrp);
        }
        ...
        for (const auto &It5 : mcBoolMap) {
            (void)new ParameterBool(paramValue, QString::fromUtf8(It5.first.c_str()), It5.second,
                                    _hcGrp);
        }
        ...
        for (const auto &It6 : mcUIntMap) {
            (void)new ParameterUInt(paramValue, QString::fromUtf8(It6.first.c_str()), It6.second,
                                    _hcGrp);
        }
...
void DlgParameterImp::onChangeParameterSet(int itemPos)
{
    ...
    if (parent) paramGroup->setCurrentItem(parent);
...
ParameterGroup::ParameterGroup(QWidget *parent) : QTreeWidget(parent)
{
...
void ParameterGroup::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (item && item->isSelected()) {
        ...
        if (item->isExpanded()) expandAct->setText(tr("Collapse"));
        else
...
void ParameterGroup::onDeleteSelectedItem()
{
    ...
    if (sel && sel->isSelected() && sel->parent()) {
        if (QMessageBox::question(
                this, tr("Remove group"), tr("Do you really want to remove this parameter group?"),
...
void ParameterGroup::onToggleSelectedItem()
{
    ...
    if (sel && sel->isSelected()) {
        if (sel->isExpanded()) sel->setExpanded(false);
...
void ParameterGroup::onCreateSubgroup()
{
    bool ok;
    QString name =
        QInputDialog::getText(this, QObject::tr("New sub-group"), QObject::tr("Enter the name:"),
...
{
    QString file = FileDialog::getSaveFileName(this, tr("Export parameter to file"), QString(),
...
{
    QString file = FileDialog::getOpenFileName(this, tr("Import parameter from file"), QString(),
                                               QString::fromLatin1("XML (*.FCParam)"));
    if (!file.isEmpty()) {
        QTreeWidgetItem *item = currentItem();
        if (item && item->isSelected()) {
            ...
            try {
                ...
                for (const auto &cSubGrp : cSubGrps) { new ParameterGroupItem(para, cSubGrp); }
...
{
    QTreeWidgetItem *sel = currentItem();
    if (sel && sel->isSelected()) { editItem(sel, 0); }
}
...
ParameterValue::ParameterValue(QWidget *parent) : QTreeWidget(parent)
{
    menuEdit = new QMenu(this);
    ...
    connect(this, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this,
            SLOT(onChangeSelectedItem(QTreeWidgetItem *, int)));
}

ParameterValue::~ParameterValue() {}

void ParameterValue::setCurrentGroup(const Base::Reference<ParameterGrp> &hGrp) { _hcGrp = hGrp; }

Base::Reference<ParameterGrp> ParameterValue::currentGroup() const { return _hcGrp; }
...
{
    QTreeWidgetItem *item = currentItem();
    if (item && item->isSelected()) { menuEdit->popup(event->globalPos()); }
    else {
...
{
    if (item->isSelected() && col > 0) { static_cast<ParameterValueItem *>(item)->changeValue(); }
}

void ParameterValue::onChangeSelectedItem() { onChangeSelectedItem(currentItem(), 1); }
...
void ParameterValue::onCreateTextItem()
{
    bool ok;
    QString name =
        QInputDialog::getText(this, QObject::tr("New text item"), QObject::tr("Enter the name:"),
                              QLineEdit::Normal, QString(), &ok, Qt::MSWindowsFixedSizeDialogHint);

    if (!ok || !Gui::validateInput(this, name)) return;
    ...
    QString val =
        QInputDialog::getText(this, QObject::tr("New text item"), QObject::tr("Enter your text:"),
                              QLineEdit::Normal, QString(), &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (ok && !val.isEmpty()) {
...
void ParameterValue::onCreateIntItem()
{
    bool ok;
    QString name =
        QInputDialog::getText(this, QObject::tr("New integer item"), QObject::tr("Enter the name:"),
                              QLineEdit::Normal, QString(), &ok, Qt::MSWindowsFixedSizeDialogHint);
    ...
    int val = QInputDialog::getInt(this, QObject::tr("New integer item"),
                                   QObject::tr("Enter your number:"), 0, -2147483647, 2147483647, 1,
...
{
    bool ok;
    QString name = QInputDialog::getText(this, QObject::tr("New unsigned item"),
                                         QObject::tr("Enter the name:"), QLineEdit::Normal,
    ...
    DlgInputDialogImp dlg(QObject::tr("Enter your number:"), this, true,
...
void ParameterValue::onCreateFloatItem()
{
    bool ok;
    QString name =
        QInputDialog::getText(this, QObject::tr("New float item"), QObject::tr("Enter the name:"),
                              QLineEdit::Normal, QString(), &ok, Qt::MSWindowsFixedSizeDialogHint);
    ...
    double val = QInputDialog::getDouble(this, QObject::tr("New float item"),
                                         QObject::tr("Enter your number:"), 0, -2147483647,
                                         2147483647, 12, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (ok) {
        ParameterValueItem *pcItem;
...
void ParameterValue::onCreateBoolItem()
{
    bool ok;
    QString name =
        QInputDialog::getText(this, QObject::tr("New Boolean item"), QObject::tr("Enter the name:"),
                              QLineEdit::Normal, QString(), &ok, Qt::MSWindowsFixedSizeDialogHint);
    ...
    QString val =
        QInputDialog::getItem(this, QObject::tr("New boolean item"), QObject::tr("Choose an item:"),
...
ParameterGroupItem::~ParameterGroupItem()
{
    ...
    if (_hcGrp.getRefCount() == 1) _hcGrp->ClearObserver();
}
...
void ParameterGroupItem::fillUp()
{
    ...
    for (const auto &It : vhcParamGrp) (void)new ParameterGroupItem(this, It);
}
...
void ParameterGroupItem::setData(int column, int role, const QVariant &value)
{
    if (role == Qt::EditRole) {
        ...
        if (!Gui::validateInput(treeWidget(), newName)) return;
        ...
        if (item->_hcGrp->HasGroup(newName.toLatin1())) {
            ...
            if (!item->_hcGrp->RenameGrp(oldName.toLatin1(), newName.toLatin1())) return;
        }
    }
...
ParameterText::ParameterText(QTreeWidget *parent, QString label, const char *value,
                             const Base::Reference<ParameterGrp> &hcGrp)
...
{
    bool ok;
    QString txt = QInputDialog::getText(treeWidget(), QObject::tr("Change value"),
                                        QObject::tr("Enter your text:"), QLineEdit::Normal, text(2),
                                        &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (ok) {
        setText(2, txt);
...
void ParameterText::removeFromGroup() { _hcGrp->RemoveASCII(text(0).toLatin1()); }
...
void ParameterText::appendToGroup() { _hcGrp->SetASCII(text(0).toLatin1(), text(2).toUtf8()); }

// --------------------------------------------------------------------

ParameterInt::ParameterInt(QTreeWidget *parent, QString label, long value,
                           const Base::Reference<ParameterGrp> &hcGrp)
    : ParameterValueItem(parent, hcGrp)
{
...
{
    bool ok;
    int num = QInputDialog::getInt(treeWidget(), QObject::tr("Change value"),
                                   QObject::tr("Enter your number:"), text(2).toInt(), -2147483647,
                                   2147483647, 1, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (ok) {
...
void ParameterInt::removeFromGroup() { _hcGrp->RemoveInt(text(0).toLatin1()); }
...
void ParameterInt::appendToGroup() { _hcGrp->SetInt(text(0).toLatin1(), text(2).toLong()); }

// --------------------------------------------------------------------

ParameterUInt::ParameterUInt(QTreeWidget *parent, QString label, unsigned long value,
                             const Base::Reference<ParameterGrp> &hcGrp)
    : ParameterValueItem(parent, hcGrp)
{
...
{
    bool ok;
    DlgInputDialogImp dlg(QObject::tr("Enter your number:"), treeWidget(), true,
                          DlgInputDialogImp::UIntBox);
...
void ParameterUInt::removeFromGroup() { _hcGrp->RemoveUnsigned(text(0).toLatin1()); }
...
void ParameterUInt::appendToGroup() { _hcGrp->SetUnsigned(text(0).toLatin1(), text(2).toULong()); }

// --------------------------------------------------------------------

ParameterFloat::ParameterFloat(QTreeWidget *parent, QString label, double value,
                               const Base::Reference<ParameterGrp> &hcGrp)
    : ParameterValueItem(parent, hcGrp)
{
...
void ParameterFloat::changeValue()
{
    ...
    double num = QInputDialog::getDouble(
        treeWidget(), QObject::tr("Change value"), QObject::tr("Enter your number:"),
...
void ParameterFloat::removeFromGroup() { _hcGrp->RemoveFloat(text(0).toLatin1()); }
...
void ParameterFloat::appendToGroup() { _hcGrp->SetFloat(text(0).toLatin1(), text(2).toDouble()); }
...
void ParameterBool::changeValue()
{
    ...
    QString txt = QInputDialog::getItem(treeWidget(), QObject::tr("Change value"),
                                        QObject::tr("Choose an item:"), list, pos, false, &ok,
...
void ParameterBool::removeFromGroup() { _hcGrp->RemoveBool(text(0).toLatin1()); }

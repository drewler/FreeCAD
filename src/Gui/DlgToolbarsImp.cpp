...
DlgCustomToolbars::DlgCustomToolbars(DlgCustomToolbars::Type t, QWidget *parent)
    : CustomizeActionPage(parent), ui(new Ui_DlgCustomToolbars), type(t)
{
    ui->setupUi(this);
    ...
    for (const auto &workbench : workbenches) {
        ...
        if (mt != QLatin1String("<none>")) {
            if (px.isNull()) ui->workbenchBox->addItem(mt);
            else
...
void DlgCustomToolbars::importCustomToolbars(const QByteArray &name)
{
    ...
    if (!hGrp->HasGroup(name.constData())) return;
    hGrp = hGrp->GetGroup(name.constData());
    if (!hGrp->HasGroup(subgroup)) return;
        ...
        for (const auto &it2 : items) {
            ...
            if (it2.first.substr(0, separator.size()) == separator) {
                ...
                if (pCmd) {
                    ...
                    auto item = new QTreeWidgetItem(toplevel);
                    item->setText(
                        0, tr("%1 module not loaded").arg(QString::fromStdString(it2.second)));
                    item->setData(0, Qt::UserRole, QByteArray(it2.first.c_str()));
    ...
    for (int i = 0; i < ui->toolbarTreeWidget->topLevelItemCount(); i++) {
        ...
        for (int j = 0; j < toplevel->childCount(); j++) {
            ...
            if (commandName == "Separator") {
                ...
                if (pCmd) { hToolGrp->SetASCII(pCmd->getName(), pCmd->getAppModuleName()); }
                else {
...
void DlgCustomToolbars::on_moveActionRightButton_clicked()
{
    ...
    if (item) {
        ...
        if (!current) current = ui->toolbarTreeWidget->topLevelItem(0);
...
void DlgCustomToolbars::on_newButton_clicked()
{
    ...
    text = QInputDialog::getText(this, tr("New toolbar"), tr("Toolbar name:"), QLineEdit::Normal,
                                 text, &ok, Qt::MSWindowsFixedSizeDialogHint);
    if (ok) {
        ...
        for (int i = 0; i < ui->toolbarTreeWidget->topLevelItemCount(); i++) {
            ...
            if (groupName == text) {
                QMessageBox::warning(this, tr("Duplicated name"),
    ...
    if (w && w->name() == std::string((const char *)data.toByteArray())) {
        ...
        if (cmd == "Separator") {
            ...
            if (mgr.addTo(cmd, bars.front())) {
                ...
                if (action && action->data().isNull()) action->setData(cmd);
            }
        }
    }
}
        ...
        for (const auto &action : actions) {
            if (action->data().toByteArray() == cmd) {
                ...
                if (before) {
                    ...
                    if (!group.isEmpty()) setActionGroup(action, group);
                    break;
                }
            }
        ...
        for (QList<QAction *>::Iterator it = actions.begin(); it != actions.end(); ++it) {
            if ((*it)->data().toByteArray() == cmd) {
                ...
                if (*it == actions.back()) {
                    ...
                    if (!group.isEmpty()) setActionGroup(act, group);
                    break;
                }
                ++it;
                QList<QAction *> group = getActionGroup(act);
...
void DlgCustomToolbarsImp::changeEvent(QEvent *e) { DlgCustomToolbars::changeEvent(e); }
...
{
    if (e->type() == QEvent::LanguageChange) { setWindowTitle(tr("Toolbox bars")); }

...
ActionGroup::ActionGroup(const QPixmap &icon, const QString &title, bool expandable,
                         QWidget *parent)
    : QWidget(parent)
{
...
{
    if (m_foldStep) return;

    if (!myHeader->expandable()) return;
...
void ActionGroup::paintEvent(QPaintEvent *event)
{
    ...
    if (myDummy->isVisible()) {
        if (myScheme->groupFoldThaw) {
            if (m_foldDirection < 0) p.setOpacity((double)m_foldStep / myScheme->groupFoldSteps);
            else
...
QSize ActionGroup::minimumSizeHint() const { return QSize(200, 65); }

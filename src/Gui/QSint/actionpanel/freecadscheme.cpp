...
const char *ActionPanelFreeCAD =
    ...
    "QFrame[class='panel'] {"
    "background-color:qlineargradient(x1:1, y1:0.3, x2:1, y2:0, stop:0 rgb(51,51,101), stop:1 "
    "rgb(171,171,193));"
    "}"
...
QString SystemPanelScheme::systemStyle(const QPalette &p) const
{
    ...
    QString style =
        QString::fromLatin1(
            ...
            "color: #006600;"            // todo
            "}")
            .arg(panelBackground1.name(), panelBackground2.name(), headerBackground1.name(),
                 headerBackground2.name(), headerLabelText.name(), headerLabelTextOver.name(),
                 groupBackground.name(), groupBorder.name(), taskLabelText.name())

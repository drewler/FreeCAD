...
TaskHeader::TaskHeader(const QIcon &icon, const QString &title, bool expandable, QWidget *parent)
    : BaseClass(parent), myExpandable(expandable), m_over(false), m_buttonOver(false), m_fold(true),
      m_opacity(0.1), myButton(nullptr)
{
...
{
    if (!myScheme->headerAnimation) return;
...
void TaskHeader::enterEvent(QEvent * /*event*/)
{
    ...
    if (isEnabled()) QTimer::singleShot(100, this, SLOT(animate()));
...
{
    if (myExpandable) { Q_EMIT activated(); }
}
...
void TaskHeader::changeIcons()
{
    ...
    if (m_buttonOver) {
        if (m_fold) myButton->setPixmap(myScheme->headerButtonFoldOver);
        else
            myButton->setPixmap(myScheme->headerButtonUnfoldOver);
    }
    else {
        if (m_fold) myButton->setPixmap(myScheme->headerButtonFold);
        else
...
{
    if (event->button() == Qt::LeftButton) { Q_EMIT activated(); }
}

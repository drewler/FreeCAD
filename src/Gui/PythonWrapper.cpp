...
{
    if (PyObject_TypeCheck(obj, &(Base::QuantityPy::Type))) return toCppPointerConvFuncQuantity;
    else
        return nullptr;
}
...
void registerTypes()
{
    ...
    Shiboken::Conversions::setPythonToCppPointerFunctions(convert, toCppPointerConvFuncQuantity,
    ...
    if (qvariant_conv) {
        ...
        Shiboken::Conversions::addPythonToCppValueConversion(
            qvariant_conv, BaseQuantity_PythonToCpp_QVariant,
            isBaseQuantity_PythonToCpp_QVariantConvertible);
    }

#if defined(HAVE_PYSIDE)
    QMetaType::registerConverter<PySide::PyObjectWrapper, Base::Quantity>(
...
Py::Object qt_wrapInstance(qttype object, const char *className, const char *shiboken,
                           const char *pyside, const char *wrap)
{
...
const char *qt_identifyType(QObject *ptr, const char *pyside)
{
    ...
    while (metaObject) {
        ...
        if (qtmod.getDict().hasKey(className)) return className;
...
template<typename qttype> PyTypeObject *getPyTypeObjectForTypeName()
{
    ...
    if (sbkType) return &(sbkType->type);
#else
...
Py::Object PythonWrapper::fromQIcon(const QIcon *icon)
{
    ...
    PyObject *pyobj = Shiboken::Object::newObject(
        reinterpret_cast<SbkObjectType *>(getPyTypeObjectForTypeName<QIcon>()),
        const_cast<QIcon *>(icon), true, false, typeName);
    if (pyobj) return Py::asObject(pyobj);
#else
    // Access shiboken2/PySide2 via Python
    //
    return qt_wrapInstance<const QIcon *>(icon, "QIcon", "shiboken2", "PySide2.QtGui",
                                          "wrapInstance");
#endif
...
Py::Object PythonWrapper::fromQDir(const QDir &dir)
{
    ...
    PyObject *pyobj = Shiboken::Object::newObject(
        reinterpret_cast<SbkObjectType *>(getPyTypeObjectForTypeName<QDir>()),
...
Py::Object PythonWrapper::fromQObject(QObject *object, const char *className)
{
    ...
    if (type) {
        ...
        PyObject *pyobj =
            Shiboken::Object::newObject(sbk_type, object, false, false, typeName.c_str());
    ...
    // Access shiboken2/PySide2 via Python
    //
    return qt_wrapInstance<QObject *>(object, className, "shiboken2", "PySide2.QtCore",
                                      "wrapInstance");
#endif
#ifdef HAVE_PYQT // Unwrapping using sip/PyQt
    Q_UNUSED(className);
...
Py::Object PythonWrapper::fromQWidget(QWidget *widget, const char *className)
{
    ...
    if (type) {
        ...
        PyObject *pyobj =
            Shiboken::Object::newObject(sbk_type, widget, false, false, typeName.c_str());
        return Py::asObject(pyobj);
    }
    ...
    // Access shiboken2/PySide2 via Python
    //
    return qt_wrapInstance<QWidget *>(widget, className, "shiboken2", "PySide2.QtWidgets",
                                      "wrapInstance");
#endif
...
const char *PythonWrapper::getWrapperName(QObject *obj) const
{
    ...
    while (meta) {
        const char *typeName = meta->className();
        if (names.indexOf(QLatin1String(typeName)) >= 0) return typeName;
        meta = meta->superClass();
    }
#endif
    return "QObject";
...
bool PythonWrapper::loadCoreModule()
{
    ...
    if (!SbkPySide2_QtCoreTypes) {
        ...
        if (requiredModule.isNull()) return false;
...
void PythonWrapper::createChildrenNameAttributes(PyObject *root, QObject *object)
{
    Q_FOREACH (QObject *child, object->children()) {
        ...
        if (!name.isEmpty() && !name.startsWith("_") && !name.startsWith("qt_")) {
            ...
            if (!hasAttr) {
#if defined(HAVE_SHIBOKEN) && defined(HAVE_PYSIDE)
                Shiboken::AutoDecRef pyChild(Shiboken::Conversions::pointerToPython(
                    reinterpret_cast<SbkObjectType *>(getPyTypeObjectForTypeName<QObject>()),
                    child));
                PyObject_SetAttrString(root, name.constData(), pyChild);
#else
                const char *className = qt_identifyType(child, "PySide2.QtWidgets");
                if (!className) {
                    if (qobject_cast<QWidget *>(child)) className = "QWidget";
                    else
                        className = "QObject";
                }

                Py::Object pyChild(qt_wrapInstance<QObject *>(child, className, "shiboken2",
                                                              "PySide2.QtWidgets", "wrapInstance"));
...
#if defined(HAVE_SHIBOKEN) && defined(HAVE_PYSIDE)
    if (parent) {
        Shiboken::AutoDecRef pyParent(Shiboken::Conversions::pointerToPython(
            reinterpret_cast<SbkObjectType *>(getPyTypeObjectForTypeName<QWidget>()), parent));

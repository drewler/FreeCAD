...
SyntaxHighlighter::~SyntaxHighlighter() { delete d; }
...
void SyntaxHighlighter::setColor(const QString &type, const QColor &col)
{
    ...
    if (old == col) return;
    if (type == QLatin1String("Text")) d->cNormalText = col;
...
{
    if (type == QLatin1String("Text")) return d->cNormalText;
    else if (type == QLatin1String("Comment"))
        return d->cComment;
    else if (type == QLatin1String("Block comment"))
...
{
    if (type == SyntaxHighlighter::Text) return d->cNormalText;
...
int SyntaxHighlighter::maximumUserState() const { return 8; }

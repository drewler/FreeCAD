...
DeflateOutputStreambuf::DeflateOutputStreambuf(streambuf *outbuf, bool user_init, bool del_outbuf)
    : FilterOutputStreambuf(outbuf, del_outbuf), _zs_initialized(false), _invecsize(1000),
      _invec(_invecsize), _outvecsize(1000), _outvec(_outvecsize)
{
...
DeflateOutputStreambuf::~DeflateOutputStreambuf() { closeStream(); }
...
bool DeflateOutputStreambuf::init(int comp_level)
{
    ...
    if (_zs_initialized) { // just reset it
        ...
        err = deflateInit2(&_zs, comp_level, Z_DEFLATED, -MAX_WBITS, default_mem_level,
                           Z_DEFAULT_STRATEGY);
...
int DeflateOutputStreambuf::overflow(int c)
{
    ...
    while ((_zs.avail_in > 0 || _zs.avail_out == 0) && err == Z_OK) {
        if (_zs.avail_out == 0) flushOutvec();

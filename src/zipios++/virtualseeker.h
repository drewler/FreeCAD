...
VirtualSeeker::VirtualSeeker(int start_offset, int end_offset)
    : _s_off(start_offset), _e_off(end_offset)
{}
...
int VirtualSeeker::startOffset() const { return _s_off; }


int VirtualSeeker::endOffset() const { return _e_off; }

void VirtualSeeker::vseekg(istream &is, int offset, ios::seekdir sd) const
{
    if (sd == ios::cur) is.seekg(offset, sd);
    else if (sd == ios::beg)
...
int VirtualSeeker::vtellg(istream &is) const { return static_cast<int>(is.tellg()) - _s_off; }

...
FileCollection *ZipFile::clone() const { return new ZipFile(*this); }


ZipFile::~ZipFile() { close(); }

void ZipFile::close() { _valid = false; }

istream *ZipFile::getInputStream(const ConstEntryPointer &entry)
{
    if (!_valid) throw InvalidStateException("Attempt to use an invalid FileCollection");
    return getInputStream(entry->getName());
}

istream *ZipFile::getInputStream(const string &entry_name, MatchPath matchpath)
{
    if (!_valid) throw InvalidStateException("Attempt to use an invalid ZipFile");
    ...
    if (!ent) return nullptr;
        ...
        return new ZipInputStream(
            _filename,
            static_cast<const ZipCDirEntry *>(ent.get())->getLocalHeaderOffset()
                + _vs.startOffset());
}
...
bool ZipFile::readCentralDirectory(istream &_zipfile)
{
    ...
    while ((entry_num < _eocd.totalCount())) {
        ...
        if (!_zipfile) {
            if (_zipfile.bad())
                ...
                throw FCollException("Zip file consistency problem. Failure while reading zip file "
                                     "central directory");
    ...
    if (remaining != _eocd.eocdOffSetFromEnd())
        throw FCollException("Zip file consistency problem. Zip file data fields are inconsistent "
                             "with zip file layout");
...
#else
    cerr << error_str
         << endl; // define operator<< for exception class if such a class replaces string
#endif
}

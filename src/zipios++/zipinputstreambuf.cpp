...
ZipInputStreambuf::ZipInputStreambuf(streambuf *inbuf, int s_pos, bool del_inbuf)
    : InflateInputStreambuf(inbuf, s_pos, del_inbuf), _open_entry(false)
{
...
void ZipInputStreambuf::closeEntry()
{
    ...
    int position = _inbuf->pubseekoff(0, ios::cur, ios::in);
    if (position != _data_start + static_cast<int>(_curr_entry.getCompressedSize()))
        _inbuf->pubseekoff(_data_start + _curr_entry.getCompressedSize(), ios::beg, ios::in);
}
...
ConstEntryPointer ZipInputStreambuf::getNextEntry()
{
    ...
    if (_curr_entry.isValid()) {
        ...
        if (_curr_entry.getMethod() == DEFLATED) {
            ...
            setg(&(_outvec[0]), &(_outvec[0]) + _outvecsize, &(_outvec[0]) + _outvecsize);
            //        cerr << "stored" << endl ;
        }
        else {
...
{
    if (!_open_entry) return EOF; // traits_type::eof()
    if (_curr_entry.getMethod() == DEFLATED) return InflateInputStreambuf::underflow();

    // Ok, we're are stored, so we handle it ourselves.
    int num_b = min(_remain, _outvecsize);
    int g = _inbuf->sgetn(&(_outvec[0]), num_b);
    setg(&(_outvec[0]), &(_outvec[0]), &(_outvec[0]) + g);
    _remain -= g;
    if (g > 0) return static_cast<unsigned char>(*gptr());
    else

...
InflateInputStreambuf::InflateInputStreambuf(streambuf *inbuf, int s_pos, bool del_inbuf)
    : FilterInputStreambuf(inbuf, del_inbuf), _zs_initialized(false), _invecsize(1000),
...
int InflateInputStreambuf::underflow()
{
    ...
    setg(&(_outvec[0]), &(_outvec[0]), &(_outvec[0]) + inflated_bytes);
    ...
    if (inflated_bytes > 0) return static_cast<unsigned char>(*gptr());
    else

...
struct boost::filesystem::dir_it::representation {
    representation() : m_handle(nullptr), m_refcount(1), m_stat_p(false) {}

    representation(std::string const &dirname)
        : m_handle(opendir(dirname.c_str())), m_refcount(1), m_directory(dirname), m_stat_p(false)
    {
        if (m_directory.size() == 0) m_directory = "./";
        if (m_directory[m_directory.size() - 1] != '/') m_directory += '/';
        operator++();
    }
    ...
    {
        if (!m_stat_p) stat((m_directory + m_current).c_str(), &m_stat);
        return m_stat;
    }
    ...
    {
        int c = chown((m_directory + m_current).c_str(), uid, get_stat().st_gid);
        (void)c;
    }
    void change_group(gid_t gid)
    {
        int c = chown((m_directory + m_current).c_str(), get_stat().st_uid, gid);
        (void)c;
    }
...
template<> bool get<is_char_device>(dir_it const &it)
{
    return S_ISCHR(it.rep->get_stat().st_mode);
}
template<> bool get<is_block_device>(dir_it const &it)
{
    return S_ISBLK(it.rep->get_stat().st_mode);
}
...
template<> bool get<group_execute>(dir_it const &it)
{
    return it.rep->get_stat().st_mode & S_IXGRP;
}
...
template<> bool get<other_execute>(dir_it const &it)
{
    return it.rep->get_stat().st_mode & S_IXOTH;
}
...
template<> std::string get<uname>(dir_it const &it)
{
    ...
    if (!pw) throw unknown_uid(it.rep->get_stat().st_uid);
    return pw->pw_name;
}
template<> void set<uname>(dir_it const &it, std::string name)
{
    struct passwd *pw = getpwnam(name.c_str());
    if (pw) it.rep->change_owner(pw->pw_uid);
    else
...
struct boost::filesystem::dir_it::representation {
    representation() : m_handle(-1), m_refcount(1) {}

    representation(std::string const &dirname)
        : m_handle(_findfirst((dirname + "\\*").c_str(), &m_data)), m_refcount(1)
    {}

    ~representation()
    {
        if (m_handle != -1) _findclose(m_handle);
    }
    ...
    struct _finddata_t const &get_data() const { return m_data; }
...
template<> size_t get<size>(dir_it const &it) { return it.rep->get_data().size; }
template<> mtime::value_type get<mtime>(dir_it const &it) { return &it.rep->get_data().time_write; }
...
template<> bool get<user_read>(dir_it const & /*it*/) { return true; }
...
#else
get<size>::operator size::value_type() const { return m_it.rep->get_data().size; }
get<mtime>::operator mtime::value_type() const { return &m_it.rep->get_data().time_write; }
...
get<user_read>::operator user_read::value_type() const { return true; }
...
boost::filesystem::dir_it::dir_it() : rep(new representation()) {}

boost::filesystem::dir_it::dir_it(std::string const &dirname) : rep(new representation(dirname)) {}

boost::filesystem::dir_it::dir_it(boost::filesystem::dir_it const &it) : rep(it.rep->reference()) {}

boost::filesystem::dir_it::~dir_it() { delete rep->release(); }
...
std::string boost::filesystem::dir_it::operator*() const { return *(*rep); }

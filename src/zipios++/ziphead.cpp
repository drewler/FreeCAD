...
bool operator==(const ZipLocalEntry &zlh, const ZipCDirEntry &ze)
{
    ...
    //    cerr << "----- END -----" << endl ;
    return (zlh.extract_version == ze.extract_version && zlh.gp_bitfield == ze.gp_bitfield
            && zlh.compress_method == ze.compress_method && zlh.last_mod_ftime == ze.last_mod_ftime
            && zlh.last_mod_fdate == ze.last_mod_fdate && zlh.filename_len == ze.filename_len &&
...
uint32 ZipLocalEntry::getCompressedSize() const { return compress_size; }

uint32 ZipLocalEntry::getCrc() const { return crc_32; }

vector<unsigned char> ZipLocalEntry::getExtra() const { return extra_field; }
...
string ZipLocalEntry::getName() const { return filename; }
...
uint32 ZipLocalEntry::getSize() const { return uncompress_size; }
...
bool ZipLocalEntry::isValid() const { return _valid; }
...
void ZipLocalEntry::setCompressedSize(uint32 size) { compress_size = size; }

void ZipLocalEntry::setCrc(uint32 crc) { crc_32 = crc; }
...
void ZipLocalEntry::setSize(uint32 size) { uncompress_size = size; }
...
int ZipLocalEntry::getLocalHeaderSize() const { return 30 + filename.size() + extra_field.size(); }
...
FileEntry *ZipLocalEntry::clone() const { return new ZipLocalEntry(*this); }
...
string ZipCDirEntry::getComment() const { return file_comment; }

uint32 ZipCDirEntry::getLocalHeaderOffset() const { return rel_offset_loc_head; }

void ZipCDirEntry::setLocalHeaderOffset(uint32 offset) { rel_offset_loc_head = offset; }
...
FileEntry *ZipCDirEntry::clone() const { return new ZipCDirEntry(*this); }
...
{
    if ((buf.size() - pos < sizeof(uint32)) || (!checkSignature(&(buf[pos])))) return false;

    eocd_offset_from_end = buf.size() - pos;
    pos += sizeof(uint32);
    disk_num = ztohs(&(buf[pos]));
    pos += sizeof(uint16);
    ...
    pos += sizeof(uint16);
    cdir_size = ztohl(&(buf[pos]));
    pos += sizeof(uint32);
    cdir_offset = ztohl(&(buf[pos]));
    pos += sizeof(uint32);
    zip_comment_len = ztohs(&(buf[pos]));
    pos += sizeof(uint16);

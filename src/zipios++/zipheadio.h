...
inline uint16 ztohs(unsigned char *buf)
{
    ...
    out = (static_cast<uint16>(buf[0]) << 8) + (static_cast<uint16>(buf[1]));
...
{
    uint32 out;
    out = (static_cast<uint32>(buf[0]) << 24) + (static_cast<uint32>(buf[1]) << 16)
...
{
    uint16 out;
    out = (static_cast<uint16>(buf[1]) << 8) + (static_cast<uint16>(buf[0]));
    return out;
}
...
{
    uint32 out;
    out = (static_cast<uint32>(buf[3]) << 24) + (static_cast<uint32>(buf[2]) << 16)
        + (static_cast<uint32>(buf[1]) << 8) + (static_cast<uint32>(buf[0]));
...
inline uint32 htozl(unsigned char *buf) { return ztohl(buf); }

// htozs (host-to-zip-short)
inline uint16 htozs(unsigned char *buf) { return ztohs(buf); }
...
inline void writeByteSeq(ostream &os, const string &con) { os << con; }
...
{
    if (!vec.empty()) os.rdbuf()->sputn(reinterpret_cast<const char *>(&(vec[0])), vec.size());
}

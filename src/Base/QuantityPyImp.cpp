...
std::string QuantityPy::representation() const
{
    ...
    if (!unit.isEmpty()) ret << " " << unit.getString().toUtf8().constData();
#endif
...
PyObject *QuantityPy::toStr(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|i", &prec)) return nullptr;
...
int QuantityPy::PyInit(PyObject *args, PyObject * /*kwd*/)
{
    ...
    if (PyArg_ParseTuple(args, "|diiiiiiii", &f, &i1, &i2, &i3, &i4, &i5, &i6, &i7, &i8)) {
        if (f < DOUBLE_MAX) {
            *self = Quantity(f,
                             Unit {static_cast<int8_t>(i1), static_cast<int8_t>(i2),
                                   static_cast<int8_t>(i3), static_cast<int8_t>(i4),
                                   static_cast<int8_t>(i5), static_cast<int8_t>(i6),
                                   static_cast<int8_t>(i7), static_cast<int8_t>(i8)});
        }
        return 0;
    }
...
{
    if (!PyObject_TypeCheck(self, &(QuantityPy::Type))) { return 1; }
...
Py::Float QuantityPy::getValue() const { return Py::Float(getQuantityPtr()->getValue()); }

void QuantityPy::setValue(Py::Float arg) { getQuantityPtr()->setValue(arg); }
...
void QuantityPy::setUnit(Py::Object arg)
{
    ...
    if (!arg.isType(UnitType)) throw Py::AttributeError("Not yet implemented");
...
void QuantityPy::setFormat(Py::Dict arg)
{
    ...
    if (arg.hasKey("NumberFormat")) {
        ...
        if (item.isNumeric()) {
            ...
            if (fmtstr.size() != 1) throw Py::ValueError("Invalid format character");

            bool ok;
            fmt.format = Base::QuantityFormat::toFormat(fmtstr[0], &ok);
            if (!ok) throw Py::ValueError("Invalid format character");
        }
    }
    ...
    if (arg.hasKey("Denominator")) {
        ...
        if (fracInch <= 0) throw Py::ValueError("Denominator must be higher than zero");
        // bitwise check
        if (fracInch & (fracInch - 1)) throw Py::ValueError("Denominator must be a power of two");
...
PyObject *QuantityPy::getCustomAttributes(const char *attr) const
{
    ...
    if (strcmp(attr, "Torr") == 0) { py = new QuantityPy(new Quantity(Quantity::Torr)); }

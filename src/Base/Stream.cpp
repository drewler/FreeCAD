...
Stream::ByteOrder Stream::byteOrder() const { return _swap ? BigEndian : LittleEndian; }

void Stream::setByteOrder(ByteOrder bo) { _swap = (bo == BigEndian); }
...
std::streambuf::int_type ByteArrayOStreambuf::overflow(std::streambuf::int_type c)
{
    if (c != EOF) {
        ...
        if (_buffer->write(&z, 1) != 1) { return EOF; }
    }
    return c;
}
                                                      ...
                                                      std::ios_base::openmode /*mode*/)
{
    ...
    switch (way) {
        case std::ios_base::beg: endpos = off; break;
        case std::ios_base::cur: endpos = curpos + off; break;
        case std::ios_base::end: endpos = _buffer->size(); break;
        default: return pos_type(off_type(-1));
    }

    if (endpos != curpos) {
        if (!_buffer->seek(endpos)) endpos = -1;
    }
...
std::streamsize ByteArrayIStreambuf::showmanyc() { return _end - _cur; }
...
std::streambuf::int_type IODeviceOStreambuf::overflow(std::streambuf::int_type c)
{
    if (c != EOF) {
        char z = static_cast<char>(c);
        if (device->write(&z, 1) != 1) { return EOF; }
    }
    return c;
}
                                                     ...
                                                     std::ios_base::openmode /*mode*/)
{
    ...
    switch (way) {
        case std::ios_base::beg: endpos = off; break;
        case std::ios_base::cur: endpos = curpos + off; break;
        case std::ios_base::end: endpos = device->size(); break;
        default: return pos_type(off_type(-1));
    }

    if (endpos != curpos) {
        if (!device->seek(endpos)) endpos = -1;
    }
...
PyStreambuf::PyStreambuf(PyObject *o, std::size_t buf_size, std::size_t put_back)
    : inp(o), type(Unknown), put_back(std::max(put_back, std::size_t(1))),
...
PyStreambuf::int_type PyStreambuf::underflow()
{
    ...
    try {
        ...
        if (res.isBytes()) { c = static_cast<std::string>(Py::Bytes(res)); }
...
PyStreambuf::int_type PyStreambuf::overflow(PyStreambuf::int_type ch)
{
    ...
    if (ch != EOF) {
        char z = ch;
        if (!writeStr(&z, 1)) return traits_type::eof();
    }
...
#else
    if (!writeStr(s, num)) return 0;
    return num;
#endif
}

PyStreambuf::pos_type PyStreambuf::seekoff(PyStreambuf::off_type offset, PyStreambuf::seekdir dir,
...
Streambuf::int_type Streambuf::pbackfail(int_type ch)
{
    ...
    if (_cur == _beg || (ch != traits_type::eof() && ch != _cur[-1])) return traits_type::eof();

    return static_cast<Streambuf::int_type>(*--_cur) & 0x000000ff;
}

std::streamsize Streambuf::showmanyc() { return _end - _cur; }

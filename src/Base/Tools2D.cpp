...
bool BoundBox2d::Intersect(const Line2d &rclLine) const
{
    ...
    if (clThisLine.IntersectAndContain(rclLine, clVct)) return true;
...
bool BoundBox2d::Intersect(const Polygon2d &rclPoly) const
{
    ...
    for (i = 0; i < rclPoly.GetCtVectors(); i++)
        if (Contains(rclPoly[i])) return true; /***** RETURN INTERSECTION *********/
    ...
    for (i = 0; i < rclPoly.GetCtVectors(); i++) {
        ...
        if (Intersect(clLine)) return true; /***** RETURN INTERSECTION *********/
    }
    // no intersection
    return false;
}
...
bool Line2d::Intersect(const Line2d &rclLine, Vector2d &rclV) const
{
    ...
    // calc coefficients
    if (fabs(clV2.x - clV1.x) > 1e-10) m1 = (clV2.y - clV1.y) / (clV2.x - clV1.x);
    else
        m1 = DOUBLE_MAX;
...
bool Line2d::Intersect(const Vector2d &rclV, double eps) const
{
    ...
    if (dot < -eps || dot > len + eps) return false;
    return true;
}
...
bool Line2d::IntersectAndContain(const Line2d &rclLine, Vector2d &rclV) const
{
    ...
    if (rc) rc = Contains(rclV) && rclLine.Contains(rclV);
    return rc;
}
...
static short _CalcTorsion(double *pfLine, double fX, double fY)
{
    ...
    for (i = 0; i < 2; i++) {
        if (pfLine[i * 2] <= fX) sQuad[i] = (pfLine[i * 2 + 1] > fY) ? 0 : 3;
        else
    ...
    if (abs(sQuad[0] - sQuad[1]) == 3) return (sQuad[0] == 0) ? 1 : -1;
                          ...
                          std::list<Polygon2d> &rclResultPolygonList) const
{
    ...
    //
    if ((rclPolygon.GetCtVectors() < 2) || (GetCtVectors() < 2)) return;
    ...
    for (size_t ulVec = 0; ulVec < (ulPolyCt - 1); ulVec++) {
        ...
        {
            for (std::set<double>::iterator pF = afIntersections.begin();
                 pF != afIntersections.end(); ++pF) {
                // intersection point
    ...
    // add last segment
    if (clResultPolygon.GetCtVectors() > 0) rclResultPolygonList.push_back(clResultPolygon);
}

bool Polygon2d::Intersect(const Polygon2d &other) const
{
    if (other.GetCtVectors() < 2 || GetCtVectors() < 2) return false;

    for (auto &v : _aclVct) {
        if (other.Contains(v)) return true;
    }

    if (Contains(other[0])) return true;
    ...
    for (size_t j = 1; j < other.GetCtVectors(); ++j) {
        ...
        for (size_t i = 0; i < GetCtVectors(); ++i) {
            ...
            Vector2d v;
            if (line.IntersectAndContain(line2, v)) return true;
        }
    }
    return false;
}
...
{
    if (_aclVct.size() < 2) return false;
    ...
    for (size_t i = 0; i < numPts; i++) {
        ...
        Line2d clLine(clPt0, clPt1);
        if (clLine.Intersect(rclV, eps)) return true;
    }

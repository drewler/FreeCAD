...
template<class vecT> struct vec_traits {
};

template<> struct vec_traits<Vector3f> {
...
template<> struct vec_traits<Vector3d> {
...
template<> struct vec_traits<Rotation> {
...
_Vec make_vec(const std::tuple<float_type, float_type, float_type> &&t)
{
    ...
    return _Vec(float_traits_type(std::get<0>(t)), float_traits_type(std::get<1>(t)),
...
_Vec make_vec(const std::tuple<float_type, float_type, float_type, float_type> &&t)
{
    ...
    using float_traits_type = typename traits_type::float_type;
    return _Vec(float_traits_type(std::get<0>(t)), float_traits_type(std::get<1>(t)),
                float_traits_type(std::get<2>(t)), float_traits_type(std::get<3>(t)));
}

template<class _Vec1, class _Vec2> inline _Vec1 convertTo(const _Vec2 &v)
{

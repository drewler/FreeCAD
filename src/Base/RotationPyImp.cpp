...
int RotationPy::PyInit(PyObject *args, PyObject *kwds)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O!d", kw_deg, &(Base::VectorPy::Type), &o,
                                    &angle)) {
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwds, "O!d", kw_rad, &(Base::VectorPy::Type), &o,
                                    &angle)) {
    ...
    if (PyArg_ParseTuple(args, "dddddddddddddddd", &a11, &a12, &a13, &a14, &a21, &a22, &a23, &a24,
        ...
        Matrix4D mtx(a11, a12, a13, a14, a21, a22, a23, a24, a31, a32, a33, a34, a41, a42, a43,
                     a44);
    ...
    PyErr_Clear();
    if (PyArg_ParseTuple(args, "ddddddddd", &a11, &a12, &a13, &a21, &a22, &a23, &a31, &a32, &a33)) {
        Matrix4D mtx(a11, a12, a13, a14, a21, a22, a23, a24, a31, a32, a33, a34, a41, a42, a43,
                     a44);
        getRotationPtr()->setValue(mtx);
        return 0;
    ...
    PyErr_Clear();
    PyObject *v1, *v2;
    if (PyArg_ParseTuple(args, "O!O!", &(Base::VectorPy::Type), &v1, &(Base::VectorPy::Type),
                         &v2)) {
    ...
    if (PyArg_ParseTuple(args, "O!O!O!|s", &(Base::VectorPy::Type), &v1, &(Base::VectorPy::Type),
                         &v2, &(Base::VectorPy::Type), &v3, &priority)) {
        Py::Vector xdir(v1, false);
        Py::Vector ydir(v2, false);
        Py::Vector zdir(v3, false);
        if (!priority) priority = "ZXY";
        try {
            *getRotationPtr() = (Rotation::makeRotationByAxes(xdir.toVector(), ydir.toVector(),
                                                              zdir.toVector(), priority));
        }
...
{
    PyObject *rot;
    if (!PyArg_ParseTuple(args, "O!", &(RotationPy::Type), &rot)) return nullptr;
...
PyObject *RotationPy::slerp(PyObject *args)
{
    PyObject *rot;
    double t;
    if (!PyArg_ParseTuple(args, "O!d", &(RotationPy::Type), &rot, &t)) return nullptr;
...
{
    double A, B, C;
    if (!PyArg_ParseTuple(args, "ddd", &A, &B, &C)) return nullptr;
...
PyObject *RotationPy::setEulerAngles(PyObject *args)
{
    const char *seq;
    double A, B, C;
    if (!PyArg_ParseTuple(args, "sddd", &seq, &A, &B, &C)) return nullptr;
...
PyObject *RotationPy::toEulerAngles(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|s", &seq)) return nullptr;
    if (!seq) {
        Py::List res;
...
PyObject *RotationPy::isSame(PyObject *args)
{
    PyObject *rot;
    double tol = 0.0;
    if (!PyArg_ParseTuple(args, "O!|d", &(RotationPy::Type), &rot, &tol)) return nullptr;
...
int RotationPy::number_nonzero_handler(PyObject * /*self*/) { return 1; }

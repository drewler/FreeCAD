...
QString UnitsSchemaImperial1::schemaTranslate(const Quantity &quant, double &factor,
                                              QString &unitString)
{
...
QString UnitsSchemaImperialDecimal::schemaTranslate(const Base::Quantity &quant, double &factor,
                                                    QString &unitString)
{
    //double UnitValue = std::abs(quant.getValue());
    Unit unit = quant.getUnit();
...
QString UnitsSchemaImperialBuilding::schemaTranslate(const Quantity &quant, double &factor,
                                                     QString &unitString)
{
    ...
    if (unit == Unit::Length) {
        ...
        if (ntot == 0) return QString::fromLatin1("0");
...
QString UnitsSchemaImperialCivil::schemaTranslate(const Base::Quantity &quant, double &factor,
                                                  QString &unitString)
{
    //    double UnitValue = std::abs(quant.getValue());
    Unit unit = quant.getUnit();
    ...
    // this schema expresses angles in degrees + minutes + seconds
        ...
        if ((outMin > 0) || (outSec > 0)) { output << outMin << minuteString.toUtf8().constData(); }
        if (outSec > 0) { output << outSec << secondString.toUtf8().constData(); }

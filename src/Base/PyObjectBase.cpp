...
typedef struct {
    PyObject_HEAD PyObject *baseobject;
    PyObject *weakreflist; /* List of weak references */
} PyBaseProxy;

// Constructor
PyObjectBase::PyObjectBase(void *p, PyTypeObject *T)
    : _pcTwinPointer(p), baseProxy(nullptr), attrDict(nullptr)
{
...
static void PyBaseProxy_dealloc(PyObject *self)
{
    /* Clear weakrefs first before calling any destructors */
    if (reinterpret_cast<PyBaseProxy *>(self)->weakreflist) PyObject_ClearWeakRefs(self);
    Py_TYPE(self)->tp_free(self);
}

static PyTypeObject PyBaseProxyType = {
    PyVarObject_HEAD_INIT(nullptr, 0) "PyBaseProxy", /*tp_name*/
...
PyObject *createWeakRef(PyObjectBase *ptr)
{
    ...
    if (!init) {
        init = true;
        if (PyType_Ready(&PyBaseProxyType) < 0) return nullptr;
    }
...
#ifdef ATTR_TRACKING
    if (ret == 0) { static_cast<PyObjectBase *>(obj)->startNotify(); }
#endif
    return ret;
}
...
{
    if (streq(attr, "softspace")) return -1; // filter out softspace
    PyObject *w;
    // As fallback solution use Python's default method to get generic attributes
    ...
    if (w) {
        ...
        // Throw an exception for unknown attributes
        PyTypeObject *tp = Py_TYPE(this);
        PyErr_Format(PyExc_AttributeError, "%.50s instance has no attribute '%.400s'", tp->tp_name,
                     attr);
        return -1;
    }
}
...
void PyObjectBase::resetAttribute()
{
    if (attrDict) {
        ...
        if (attr) { PyDict_DelItem(attrDict, key1); }
        if (inst) { PyDict_DelItem(attrDict, key2); }
        Py_DECREF(key1);
        Py_DECREF(key2);
    }
}
...
{
    if (!attrDict) { attrDict = PyDict_New(); }
...
void PyObjectBase::trackAttribute(const char *attr, PyObject *obj)
{
    ...
    if (obj_ref) { PyDict_SetItemString(attrDict, attr, obj_ref); }
}

void PyObjectBase::untrackAttribute(const char *attr)
{
    if (attrDict) { PyDict_DelItemString(attrDict, attr); }
}

void PyObjectBase::clearAttributes()
{
    if (attrDict) { PyDict_Clear(attrDict); }
}

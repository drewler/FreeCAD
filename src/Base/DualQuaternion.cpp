...
{
    return DualQuat(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);
}
...
Base::DualQuat Base::operator*(Base::DualQuat a, Base::DualQuat b)
{
    return DualQuat(a.w * b.x + a.x * b.w + a.y * b.z - a.z * b.y,
...
{
    return DualQuat(a.x * b, a.y * b, a.z * b, a.w * b);
}

Base::DualQuat Base::operator*(double a, Base::DualQuat b)
{
    return DualQuat(b.x * a, b.y * a, b.z * a, b.w * a);
}
...
Base::DualQuat::DualQuat(Base::DualQuat re, Base::DualQuat du)
    : x(re.x.re, du.x.re), y(re.y.re, du.y.re), z(re.z.re, du.z.re), w(re.w.re, du.w.re)
{
...
Base::DualQuat Base::DualQuat::pow(double t, bool shorten) const
{
    ...
    if (shorten) {
        if (dot(self, identity())
            < -1e-12) { //using negative tolerance instead of zero, for stability in situations the choice is ambiguous (180-degree rotations)
            self = -self;
        }
    }
    ...
    DualQuat l =
        self.real().vec() * normmult; //abusing DualQuat to store vectors. Very handy in this case.
    ...
    //back to quaternion
    return DualQuat(l * sin(theta / 2) + DualQuat(0, 0, 0, cos(theta / 2)),

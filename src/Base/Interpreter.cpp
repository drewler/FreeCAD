...
void PyException::raiseException()
{
    ...
    if (PP_PyDict_Object) {
        ...
        if (_isReported) edict.setItem("breported", Py::True());
...
SystemExitException::SystemExitException()
{
    ...
    if (value) {
        ...
        if (PyLong_Check(value)) { errCode = PyLong_AsLong(value); }
        else {
            const char *str = PyUnicode_AsUTF8(value);
            if (str) errMsg = errMsg + ": " + str;
        }
    }
...
public:
    ...
    Py::Object write(const Py::Tuple &) { return Py::None(); }
    Py::Object flush(const Py::Tuple &) { return Py::None(); }
};

// ---------------------------------------------------------

InterpreterSingleton::InterpreterSingleton() { this->_global = nullptr; }
...
std::string InterpreterSingleton::runString(const char *sCmd)
{
    ...
    if (!module) throw PyException();    /* not incref'd */
    dict = PyModule_GetDict(module);     /* get dict namespace */
    if (!dict) throw PyException();      /* not incref'd */


    presult = PyRun_String(sCmd, Py_file_input, dict, dict); /* eval direct */
    if (!presult) {
        if (PyErr_ExceptionMatches(PyExc_SystemExit)) throw SystemExitException();
        else {
...
std::string InterpreterSingleton::runStringWithKey(const char *psCmd, const char *key,
                                                   const char *key_initial_value)
{
    ...
    if (!key_return_value.isString()) key_return_value = key_return_value.str();
...
void InterpreterSingleton::systemExit()
{
    ...
    fflush(stdout);
    if (!value || value == Py_None) goto done;
...
void InterpreterSingleton::runInteractiveString(const char *sCmd)
{
    ...
    if (!presult) {
        ...
        if (errdata) {
            if (PyUnicode_Check(errdata)) exc.setMessage(PyUnicode_AsUTF8(errdata));
        }
...
void InterpreterSingleton::runFile(const char *pxFileName, bool local)
{
    ...
    if (fp) {
        ...
        if (local) { dict = PyDict_Copy(dict); }
        else {
...
void InterpreterSingleton::cleanupModules()
{
    ...
    for (auto it : _modules) { delete it; }
    _modules.clear();
#endif
#endif
}
...
const char *InterpreterSingleton::init(int argc, char *argv[])
{
    if (!Py_IsInitialized()) {
        ...
        for (int i = 0; i < argc; i++) { _argv[i] = Py_DecodeLocale(argv[i], nullptr); }
...
void initInterpreter(int argc, char *argv[])
{
    ...
    if (PyStatus_Exception(status)) { throw Base::RuntimeError("Failed to set config"); }

    status = Py_InitializeFromConfig(&config);
    if (PyStatus_Exception(status)) { throw Base::RuntimeError("Failed to init from config"); }
...
int InterpreterSingleton::cleanup(void (*func)()) { return Py_AtExit(func); }
...
{
    // not initialized!
    if (!_pcSingleton) _pcSingleton = new InterpreterSingleton();
    return *_pcSingleton;
}
...
void InterpreterSingleton::runMethod(PyObject *pobject, const char *method, const char *resfmt,
                                     ...
                                     const char *argfmt, ...) /* convert to python */
{
    ...
    if (!pmeth) { /* get callable object */
        ...
        throw AttributeError(
            "Error running InterpreterSingleton::RunMethod() method not defined"); /* bound method? has self */
    }
...
{
    if (sFileName) _cDebugFileName = sFileName;
    else
        _cDebugFileName = "";
}
...
PyObject *InterpreterSingleton::createSWIGPointerObj(const char *Module, const char *TypeName,
                                                     void *Pointer, int own)
{
    int result = 0;
...
bool InterpreterSingleton::convertSWIGPointerObj(const char *Module, const char *TypeName,
                                                 PyObject *obj, void **ptr, int flags)
{
    int result = 0;
    PyGILStateLocker locker;
    (void)Module;

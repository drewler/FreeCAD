...
Vector3<_Precision>::Vector3(_Precision fx, _Precision fy, _Precision fz) : x(fx), y(fy), z(fz)
{}

template<class _Precision> _Precision &Vector3<_Precision>::operator[](unsigned short usIndex)
{
    switch (usIndex) {
        case 0: return x;
        case 1: return y;
...
template<class _Precision> Vector3<_Precision> Vector3<_Precision>::operator-() const
{
...
template<class _Precision> Vector3<_Precision> &Vector3<_Precision>::operator*=(_Precision fScale)
{
    x *= fScale;
    y *= fScale;
    z *= fScale;
...
template<class _Precision> Vector3<_Precision> &Vector3<_Precision>::operator/=(_Precision fDiv)
{
    x /= fDiv;
    y /= fDiv;
    z /= fDiv;
...
template<class _Precision> Vector3<_Precision> Vector3<_Precision>::operator/(_Precision fDiv) const
{
                                          ...
                                          const Vector3<_Precision> &endVct) const
{
    ...
    if (crossproduct.Length() > traits_type::epsilon()) return false;

    if (dotproduct < 0) return false;

    if (dotproduct > vectorAB.Sqr()) return false;
...
template<class _Precision>
void Vector3<_Precision>::ProjectToPlane(const Vector3 &rclBase, const Vector3 &rclNorm,
...
template<class _Precision> _Precision Vector3<_Precision>::Length() const
{
...
template<class _Precision> _Precision Vector3<_Precision>::Sqr() const
{
...
template<class _Precision> void Vector3<_Precision>::ScaleX(_Precision f) { x *= f; }

template<class _Precision> void Vector3<_Precision>::ScaleY(_Precision f) { y *= f; }

template<class _Precision> void Vector3<_Precision>::ScaleZ(_Precision f) { z *= f; }
...
template<class _Precision> void Vector3<_Precision>::MoveX(_Precision f) { x += f; }

template<class _Precision> void Vector3<_Precision>::MoveY(_Precision f) { y += f; }

template<class _Precision> void Vector3<_Precision>::MoveZ(_Precision f) { z += f; }
...
template<class _Precision> void Vector3<_Precision>::RotateX(_Precision f)
{
    Vector3 cPt(*this);
...
template<class _Precision> void Vector3<_Precision>::RotateY(_Precision f)
{
    Vector3 cPt(*this);
    _Precision fsin, fcos;
...
template<class _Precision> void Vector3<_Precision>::RotateZ(_Precision f)
{
    Vector3 cPt(*this);
    _Precision fsin, fcos;
...
template<class _Precision> Vector3<_Precision> &Vector3<_Precision>::Normalize()
{
...
template<class _Precision> bool Vector3<_Precision>::IsNull() const
{
    _Precision n {0.0};
    return (x == n) && (y == n) && (z == n);
}

template<class _Precision> _Precision Vector3<_Precision>::GetAngle(const Vector3 &rcVect) const
{
    ...
    if (dot <= -1.0) return traits_type::pi();
    else if (dot >= 1.0)
        return 0.0;

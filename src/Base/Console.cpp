...
public:
    ...
    {
        if (!instance) instance = new ConsoleOutput;
        return instance;
    }
    ...
    void customEvent(QEvent *ev) override
    {
        if (ev->type() == QEvent::User) {
            ...
            switch (ce->msgtype) {
                case ConsoleSingleton::MsgType_Txt: Console().NotifyMessage(ce->msg.c_str()); break;
                case ConsoleSingleton::MsgType_Log: Console().NotifyLog(ce->msg.c_str()); break;
                case ConsoleSingleton::MsgType_Wrn: Console().NotifyWarning(ce->msg.c_str()); break;
                case ConsoleSingleton::MsgType_Err: Console().NotifyError(ce->msg.c_str()); break;
            }
        }
    }
...
ConsoleSingleton::ConsoleSingleton()
    : _bVerbose(true), _bCanRefresh(true), connectionMode(Direct)
#ifdef FC_DEBUG
      ,
...
ConsoleSingleton::~ConsoleSingleton()
{
    ...
    for (std::set<ILogger *>::iterator Iter = _aclObservers.begin(); Iter != _aclObservers.end();
         ++Iter)
        delete (*Iter);
}
...
ConsoleMsgFlags ConsoleSingleton::SetEnabledMsgType(const char *sObs, ConsoleMsgFlags type, bool b)
{
    ...
    if (pObs) {
        ...
        if (type & MsgType_Err) {
            if (pObs->bErr != b) flags |= MsgType_Err;
            pObs->bErr = b;
        }
        if (type & MsgType_Wrn) {
            if (pObs->bWrn != b) flags |= MsgType_Wrn;
            pObs->bWrn = b;
        }
        if (type & MsgType_Txt) {
            if (pObs->bMsg != b) flags |= MsgType_Txt;
            pObs->bMsg = b;
        }
        if (type & MsgType_Log) {
            if (pObs->bLog != b) flags |= MsgType_Log;
            pObs->bLog = b;
        }
        return flags;
    }
...
bool ConsoleSingleton::IsMsgTypeEnabled(const char *sObs, FreeCAD_ConsoleMsgType type) const
{
    ...
    if (pObs) {
        switch (type) {
            case MsgType_Txt: return pObs->bMsg;
            case MsgType_Log: return pObs->bLog;
            case MsgType_Wrn: return pObs->bWrn;
            case MsgType_Err: return pObs->bErr;
...
void ConsoleSingleton::SetConnectionMode(ConnectionMode mode)
{
    ...
    if (connectionMode == Queued) { ConsoleOutput::getInstance(); }
}
...
#define FC_CONSOLE_FMT(_type, _type2)                                                              \
    ...
    va_end(namelessVars);                                                                          \
    if (connectionMode == Direct) Notify##_type(format);                                           \
    else                                                                                           \
        QCoreApplication::postEvent(ConsoleOutput::getInstance(),                                  \
                                    new ConsoleEvent(MsgType_##_type2, format));
 ...
 *  @see Log
 */
void ConsoleSingleton::Warning(const char *pMsg, ...) { FC_CONSOLE_FMT(Warning, Wrn); }
 ...
 *  @see Log
 */
void ConsoleSingleton::Error(const char *pMsg, ...) { FC_CONSOLE_FMT(Error, Err); }
...
{
    if (_bVerbose) { FC_CONSOLE_FMT(Log, Log); }
}
 ...
 *  @see ILogger
 */
void ConsoleSingleton::DetachObserver(ILogger *pcObserver) { _aclObservers.erase(pcObserver); }

void ConsoleSingleton::NotifyMessage(const char *sMsg)
{
    for (std::set<ILogger *>::iterator Iter = _aclObservers.begin(); Iter != _aclObservers.end();
         ++Iter) {
        if ((*Iter)->bMsg) (*Iter)->SendLog(sMsg, LogStyle::Message); // send string to the listener
    }
}
...
void ConsoleSingleton::NotifyWarning(const char *sMsg)
{
    for (std::set<ILogger *>::iterator Iter = _aclObservers.begin(); Iter != _aclObservers.end();
         ++Iter) {
        if ((*Iter)->bWrn) (*Iter)->SendLog(sMsg, LogStyle::Warning); // send string to the listener
    }
}
...
void ConsoleSingleton::NotifyError(const char *sMsg)
{
    for (std::set<ILogger *>::iterator Iter = _aclObservers.begin(); Iter != _aclObservers.end();
         ++Iter) {
        if ((*Iter)->bErr) (*Iter)->SendLog(sMsg, LogStyle::Error); // send string to the listener
    }
}
...
void ConsoleSingleton::NotifyLog(const char *sMsg)
{
    for (std::set<ILogger *>::iterator Iter = _aclObservers.begin(); Iter != _aclObservers.end();
         ++Iter) {
        if ((*Iter)->bLog) (*Iter)->SendLog(sMsg, LogStyle::Log); // send string to the listener
    }
}
...
{
    const char *OName;
    for (std::set<ILogger *>::const_iterator Iter = _aclObservers.begin();
         Iter != _aclObservers.end(); ++Iter) {
...
{
    if (!tag) tag = "";
    if (_logLevels.find(tag) != _logLevels.end()) return &_logLevels[tag];
...
{
    if (_bCanRefresh) qApp->processEvents(QEventLoop::ExcludeUserInputEvents);
}

void ConsoleSingleton::EnableRefresh(bool enable) { _bCanRefresh = enable; }
...
{
    PyObject *output;
    if (!PyArg_ParseTuple(args, "O", &output)) return nullptr;
    PY_TRY
    {
        const char *string = nullptr;
        PyObject *unicode = nullptr;
        if (PyUnicode_Check(output)) { string = PyUnicode_AsUTF8(output); }
        else {
            unicode = PyObject_Str(output);
            if (unicode) string = PyUnicode_AsUTF8(unicode);
        }
        if (string) func(string); /*process message*/
        Py_XDECREF(unicode);
    }
    PY_CATCH
    Py_Return;
...
{
    return FC_PYCONSOLE_MSG([](const char *msg) { Instance().Message("%s", msg); }, args);
}

PyObject *ConsoleSingleton::sPyWarning(PyObject * /*self*/, PyObject *args)
{
    return FC_PYCONSOLE_MSG([](const char *msg) { Instance().Warning("%s", msg); }, args);
}

PyObject *ConsoleSingleton::sPyError(PyObject * /*self*/, PyObject *args)
{
    return FC_PYCONSOLE_MSG([](const char *msg) { Instance().Error("%s", msg); }, args);
}

PyObject *ConsoleSingleton::sPyLog(PyObject * /*self*/, PyObject *args)
{
    return FC_PYCONSOLE_MSG([](const char *msg) { Instance().Log("%s", msg); }, args);
}
...
PyObject *ConsoleSingleton::sPyGetStatus(PyObject * /*self*/, PyObject *args)
{
    char *pstr1;
    char *pstr2;
    if (!PyArg_ParseTuple(args, "ss", &pstr1, &pstr2)) return nullptr;
    ...
    PY_TRY
    {
        ...
        if (strcmp(pstr2, "Log") == 0) b = pObs->bLog;
            ...
            Py_Error(Base::PyExc_FC_GeneralError,
                     "Unknown message type (use 'Log', 'Err', 'Msg' or 'Wrn')");
...
PyObject *ConsoleSingleton::sPySetStatus(PyObject * /*self*/, PyObject *args)
{
    ...
    char *pstr2;
    PyObject *pyStatus;
    if (!PyArg_ParseTuple(args, "ssO!", &pstr1, &pstr2, &PyBool_Type, &pyStatus)) return nullptr;
    ...
    PY_TRY
    {
        ...
        ILogger *pObs = Instance().Get(pstr1);
        if (pObs) {
            if (strcmp(pstr2, "Log") == 0) pObs->bLog = status;
            else if (strcmp(pstr2, "Wrn") == 0)
                pObs->bWrn = status;
            else if (strcmp(pstr2, "Msg") == 0)
                pObs->bMsg = status;
...
PyObject *ConsoleSingleton::sPyGetObservers(PyObject * /*self*/, PyObject *args)
{
    ...
    {
        Py::List list;
        for (auto i : Instance()._aclObservers) list.append(Py::String(i->Name() ? i->Name() : ""));

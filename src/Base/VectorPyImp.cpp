...
Py_ssize_t VectorPy::sequence_length(PyObject *) { return 3; }
...
PyObject *VectorPy::mapping_subscript(PyObject *self, PyObject *item)
{
    ...
    PyErr_Format(PyExc_TypeError, "Vector indices must be integers or slices, not %.200s",
...
PyObject *VectorPy::isEqual(PyObject *args)
{
    PyObject *obj;
    double tolerance = 0;
    if (!PyArg_ParseTuple(args, "O!d", &(VectorPy::Type), &obj, &tolerance)) return nullptr;
...
PyObject *VectorPy::scale(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ddd", &factorX, &factorY, &factorZ)) return nullptr;
    VectorPy::PointerType ptr = getVectorPtr();
...
{
    PyObject *start, *end;
    if (!PyArg_ParseTuple(args, "OO", &start, &end)) return nullptr;
...
{
    PyObject *base, *line;
    if (!PyArg_ParseTuple(args, "OO", &base, &line)) return nullptr;
...
{
    PyObject *pnt;
    if (!PyArg_ParseTuple(args, "O!", &(VectorPy::Type), &pnt)) return nullptr;
...
PyObject *VectorPy::getCustomAttributes(const char * /*attr*/) const { return nullptr; }

int VectorPy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }
...
int VectorPy::number_nonzero_handler(PyObject * /*self*/) { return 1; }

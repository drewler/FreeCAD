...
public:
    ...
    inline const Vector2d &value() const { return v; }
    inline void setValue(const Vector2d &n) { v = n; }
...
{
public:
    explicit Vector(PyObject *pyob, bool owned) : Object(pyob, owned) { validate(); }

    Vector(const Vector &ob) : Object(*ob) { validate(); }

    explicit Vector(const Base::Vector3d &);
    explicit Vector(const Base::Vector3f &);
    bool accepts(PyObject *pyob) const override;

    Vector(const Object &other) : Object(other.ptr()) { validate(); }
    Vector &operator=(const Object &rhs) { return (*this = *rhs); }
...
template<class T, class PyT, T *(PyT::*valuePtr)() const> class GeometryT: public Object
{
public:
    explicit GeometryT(PyObject *pyob, bool owned) : Object(pyob, owned) { validate(); }
    GeometryT(const GeometryT &ob) : Object(*ob) { validate(); }
    explicit GeometryT()
    {
    ...
    GeometryT(const Object &other) : Object(other.ptr()) { validate(); }
    bool accepts(PyObject *pyob) const override { return pyob && Geometry_TypeCheck(pyob); }
    GeometryT &operator=(const Object &rhs) { return (*this = *rhs); }
...
private:
    static int Geometry_TypeCheck(PyObject *obj) { return PyObject_TypeCheck(obj, &(PyT::Type)); }
};

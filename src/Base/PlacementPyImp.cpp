...
int PlacementPy::PyInit(PyObject *args, PyObject * /*kwd*/)
{
    ...
    PyObject *d;
    double angle;
    if (PyArg_ParseTuple(args, "O!O!d", &(Base::VectorPy::Type), &o, &(Base::VectorPy::Type), &d,
                         &angle)) {
    ...
    PyErr_Clear();
    if (PyArg_ParseTuple(args, "O!O!", &(Base::VectorPy::Type), &o, &(Base::RotationPy::Type),
                         &d)) {
    ...
    PyErr_Clear();
    PyObject *c;
    if (PyArg_ParseTuple(args, "O!O!O!", &(Base::VectorPy::Type), &o, &(Base::RotationPy::Type), &d,
...
PyObject *PlacementPy::translate(PyObject *args) { return move(args); }
...
PyObject *PlacementPy::rotate(PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "(ddd)(ddd)d|O!", keywords, &center.x, &center.y,
                                     &center.z, &axis.x, &axis.y, &axis.z, &angle, &PyBool_Type,
                                     &pyComp))
        return nullptr;
...
PyObject *PlacementPy::pow(PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "d|O!", &t, &(PyBool_Type), &shorten)) return nullptr;
...
PyObject *PlacementPy::sclerp(PyObject *args)
{
    ...
    double t;
    PyObject *shorten = Py_True;
    if (!PyArg_ParseTuple(args, "O!d|O!", &(PlacementPy::Type), &pyplm2, &t, &(PyBool_Type),
                          &shorten))
        return nullptr;
...
PyObject *PlacementPy::slerp(PyObject *args)
{
    PyObject *pyplm2;
    double t;
    if (!PyArg_ParseTuple(args, "O!d", &(PlacementPy::Type), &pyplm2, &t)) return nullptr;
    Base::Placement plm2 = static_cast<Base::PlacementPy *>(pyplm2)->value();
...
PyObject *PlacementPy::isSame(PyObject *args)
{
    PyObject *plm;
    double tol = 0.0;
    if (!PyArg_ParseTuple(args, "O!|d", &PlacementPy::Type, &plm, &tol)) return nullptr;
...
Py::Object PlacementPy::getBase() const { return Py::Vector(getPlacementPtr()->getPosition()); }
...
void PlacementPy::setRotation(Py::Object arg)
{
    ...
    if (tuple.accepts(arg.ptr())) {
        tuple = arg;
        getPlacementPtr()->setRotation(Base::Rotation(
            static_cast<double>(Py::Float(tuple[0])), static_cast<double>(Py::Float(tuple[1])),
            static_cast<double>(Py::Float(tuple[2])), static_cast<double>(Py::Float(tuple[3]))));
        return;
    }
...
Py::Object PlacementPy::getMatrix() const { return Py::Matrix(getPlacementPtr()->toMatrix()); }

void PlacementPy::setMatrix(Py::Object arg)
{
    Py::Matrix mat;
    if (!mat.accepts(arg.ptr())) throw Py::TypeError("Expect type Matrix");
    mat = arg;
    getPlacementPtr()->fromMatrix(mat);
}
...
int PlacementPy::setCustomAttributes(const char * /*attr*/, PyObject * /*obj*/) { return 0; }
...
int PlacementPy::number_nonzero_handler(PyObject * /*self*/) { return 1; }

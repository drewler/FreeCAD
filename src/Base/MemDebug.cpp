...
protected:
    ...
    static int __cdecl sAllocHook(int nAllocType, void *pvData, size_t nSize, int nBlockUse,
...
MemDebug::MemDebug()
{
    ...
    fprintf(logFile, "Memory Allocation Log File for FreeCAD, run at %s on %s.\n", timeStr,
            dateStr);
    fputs("-------------------------------------------------------------------\n", logFile);
...
*/
int __cdecl MemDebug::sAllocHook(int nAllocType, void *pvData, size_t nSize, int nBlockUse,
                                 long lRequest, const unsigned char *szFileName, int nLine)
{
    ...
    if (nBlockUse != 4) return (7);

    fprintf(logFile, "%s (#%7d) %12Iu byte (%s) in %s line %d", operation[nAllocType], lRequest,
            nSize, blockType[nBlockUse], szFileName, nLine);
...
bool MemCheck::checkMemory() { return _CrtCheckMemory() ? true : false; }

bool MemCheck::dumpLeaks() { return _CrtDumpMemoryLeaks() ? true : false; }

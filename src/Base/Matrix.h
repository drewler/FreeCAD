...
public:
   ...
   */
    ...
    /// Construction
    Matrix4D(float a11, float a12, float a13, float a14, float a21, float a22, float a23, float a24,
             float a31, float a32, float a33, float a34, float a41, float a42, float a43,
             float a44);
    /// Construction
    Matrix4D(double a11, double a12, double a13, double a14, double a21, double a22, double a23,
    ...
    void move(double x, double y, double z) { move(Vector3d(x, y, z)); }
    ...
    void scale(double x, double y, double z) { scale(Vector3d(x, y, z)); }
    /// scale for the x,y,z value
    void scale(const Vector3f &rclVct);
    void scale(const Vector3d &rclVct);
    /// uniform scale
    void scale(float scalexyz) { scale(Vector3f(scalexyz, scalexyz, scalexyz)); }
    void scale(double scalexyz) { scale(Vector3d(scalexyz, scalexyz, scalexyz)); }
    ...
    bool toAxisAngle(Vector3d &rclBase, Vector3d &rclDir, double &fAngle,
...
inline Matrix4D Matrix4D::operator+(const Matrix4D &rclMtrx) const
{
    ...
    for (iz = 0; iz < 4; iz++) {
        for (is = 0; is < 4; is++) { clMat.dMtrx4D[iz][is] = dMtrx4D[iz][is] + rclMtrx[iz][is]; }
    }
...
inline Matrix4D &Matrix4D::operator+=(const Matrix4D &rclMtrx)
{
    ...
    for (iz = 0; iz < 4; iz++) {
        for (is = 0; is < 4; is++) { dMtrx4D[iz][is] += rclMtrx[iz][is]; }
    }
...
inline Matrix4D &Matrix4D::operator=(const Matrix4D &rclMtrx)
{
    ...
    for (iz = 0; iz < 4; iz++) {
        for (is = 0; is < 4; is++) { dMtrx4D[iz][is] = rclMtrx.dMtrx4D[iz][is]; }
    }
...
inline Vector3f Matrix4D::operator*(const Vector3f &rclVct) const
{
    ...
    return Vector3f(static_cast<float>(dMtrx4D[0][0] * x + dMtrx4D[0][1] * y + dMtrx4D[0][2] * z
                                       + dMtrx4D[0][3]),
                    static_cast<float>(dMtrx4D[1][0] * x + dMtrx4D[1][1] * y + dMtrx4D[1][2] * z
                                       + dMtrx4D[1][3]),
                    static_cast<float>(dMtrx4D[2][0] * x + dMtrx4D[2][1] * y + dMtrx4D[2][2] * z
                                       + dMtrx4D[2][3]));
}

inline Vector3d Matrix4D::operator*(const Vector3d &rclVct) const
{
    return Vector3d((dMtrx4D[0][0] * rclVct.x + dMtrx4D[0][1] * rclVct.y + dMtrx4D[0][2] * rclVct.z
                     + dMtrx4D[0][3]),
                    (dMtrx4D[1][0] * rclVct.x + dMtrx4D[1][1] * rclVct.y + dMtrx4D[1][2] * rclVct.z
                     + dMtrx4D[1][3]),
                    (dMtrx4D[2][0] * rclVct.x + dMtrx4D[2][1] * rclVct.y + dMtrx4D[2][2] * rclVct.z
                     + dMtrx4D[2][3]));
}
...
{
    double x =
        (dMtrx4D[0][0] * src.x + dMtrx4D[0][1] * src.y + dMtrx4D[0][2] * src.z + dMtrx4D[0][3]);
    double y =
        (dMtrx4D[1][0] * src.x + dMtrx4D[1][1] * src.y + dMtrx4D[1][2] * src.z + dMtrx4D[1][3]);
    double z =
        (dMtrx4D[2][0] * src.x + dMtrx4D[2][1] * src.y + dMtrx4D[2][2] * src.z + dMtrx4D[2][3]);
    dst.Set(x, y, z);
}
...
inline void Matrix4D::multVec(const Vector3f &src, Vector3f &dst) const
{
    ...
    double x = (dMtrx4D[0][0] * sx + dMtrx4D[0][1] * sy + dMtrx4D[0][2] * sz + dMtrx4D[0][3]);
    double y = (dMtrx4D[1][0] * sx + dMtrx4D[1][1] * sy + dMtrx4D[1][2] * sz + dMtrx4D[1][3]);
    double z = (dMtrx4D[2][0] * sx + dMtrx4D[2][1] * sy + dMtrx4D[2][2] * sz + dMtrx4D[2][3]);
    dst.Set(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
}
...
inline Matrix4D Matrix4D::operator*(double scalar) const
{
    ...
    for (unsigned short i = 0; i < 4; i++) {
        for (unsigned short j = 0; j < 4; j++) { matrix.dMtrx4D[i][j] = dMtrx4D[i][j] * scalar; }
    }
...
{
    for (unsigned short i = 0; i < 4; i++) {
        for (unsigned short j = 0; j < 4; j++) { dMtrx4D[i][j] *= scalar; }
    }
    return *this;
}
...
inline bool Matrix4D::operator!=(const Matrix4D &rclMtrx) const { return !((*this) == rclMtrx); }

inline Vector3f &operator*=(Vector3f &rclVect, const Matrix4D &rclMtrx)
{
    rclVect = rclMtrx * rclVect;
    return rclVect;
}

inline double *Matrix4D::operator[](unsigned short usNdx) { return dMtrx4D[usNdx]; }

inline const double *Matrix4D::operator[](unsigned short usNdx) const { return dMtrx4D[usNdx]; }

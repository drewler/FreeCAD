 ...
 */
Builder3D::Builder3D() : bStartEndOpen(false)
{
...
void Builder3D::addPoint(float x, float y, float z) { result << x << " " << y << " " << z << ","; }


/// add a vector to a point set
void Builder3D::addPoint(const Vector3f &vec) { addPoint(vec.x, vec.y, vec.z); }
/**
...
void Builder3D::addSinglePoint(float x, float y, float z, short pointSize, float color_r,
...
void Builder3D::addSinglePoint(const Base::Vector3f &vec, short pointSize, float color_r,
                               float color_g, float color_b)
{
 ...
 */
void Builder3D::addText(float pos_x, float pos_y, float pos_z, const char *text, float color_r,
                        float color_g, float color_b)
{
    // addSinglePoint() not between startXXX() and endXXX() allowed
    assert(!bStartEndOpen);
...
void Builder3D::addText(const Base::Vector3f &vec, const char *text, float color_r, float color_g,
                        float color_b)
{
...
void Builder3D::addSingleLine(const Vector3f &pt1, const Vector3f &pt2, short lineSize,
                              ...
                              unsigned short linePattern)
{
    ...
    result << "Separator { "
           ...
           << "Coordinate3 { "
           << "point [ " << pt1.x << " " << pt1.y << " " << pt1.z << "," << pt2.x << " " << pt2.y
           << " " << pt2.z << "] "
           << "} "
           << "LineSet { } "
           << "} ";
...
void Builder3D::addSingleArrow(const Vector3f &pt1, const Vector3f &pt2, short lineSize,
                               float color_r, float color_g, float color_b,
                               unsigned short /*linePattern*/)
{
    ...
    result << "Separator { "
           ...
           << "Coordinate3 { "
           << "point [ " << pt1.x << " " << pt1.y << " " << pt1.z << "," << pt2s.x << " " << pt2s.y
           << " " << pt2s.z << "] "
           << "} "
           << "LineSet { } "
           << "Transform { "
...
void Builder3D::addSingleTriangle(const Vector3f &pt0, const Vector3f &pt1, const Vector3f &pt2,
                                  bool filled, short lineSize, float color_r, float color_g,
                                  float color_b)
{
    std::string fs = "";
    if (filled) { fs = "IndexedFaceSet { coordIndex[ 0, 1, 2, -1 ] } "; }

    result << "Separator { "
           << "Material { diffuseColor " << color_r << " " << color_g << " " << color_b << "} "
           << "DrawStyle { lineWidth " << lineSize << "} "
           << "Coordinate3 { "
           << "point [ " << pt0.x << " " << pt0.y << " " << pt0.z << "," << pt1.x << " " << pt1.y
...
void Builder3D::addTransformation(const Base::Vector3f &translation,
                                  const Base::Vector3f &rotationaxis, float fAngle)
{
    result << "Transform {";
    result << "  translation " << translation.x << " " << translation.y << " " << translation.z;
    result << "  rotation " << rotationaxis.x << " " << rotationaxis.y << " " << rotationaxis.z
           << " " << fAngle;
    result << "}";
}
...
void Builder3D::saveToLog()
{
    ...
    if (obs) { obs->SendLog(result.str().c_str(), Base::LogStyle::Log); }
}
...
void Builder3D::saveToFile(const char *FileName)
{
    ...
    if (!file) throw FileException("Builder3D::saveToFile(): Can not open file...");
...
InventorBuilder::InventorBuilder(std::ostream &output) : result(output), indent(0)
{
    result << "#Inventor V2.1 ascii \n\n";
    beginSeparator();
}

InventorBuilder::~InventorBuilder() { close(); }
...
void InventorBuilder::addMaterial(float color_r, float color_g, float color_b, float color_a)
{
    ...
    result << Base::blanks(indent) << "  diffuseColor " << color_r << " " << color_g << " "
           << color_b << '\n';
...
{
    result << Base::blanks(indent) << "MaterialBinding { value " << bind << " } \n";
}

void InventorBuilder::addDrawStyle(short pointSize, short lineWidth, unsigned short linePattern,
                                   const char *style)
{
...
/// add a vector to a point set
void InventorBuilder::addPoint(const Vector3f &vec) { addPoint(vec.x, vec.y, vec.z); }
 ...
 * @see endPoints()
 */
void InventorBuilder::addPointSet() { result << Base::blanks(indent) << "PointSet { } \n"; }
 ...
 * @see endPoints()
 */
void InventorBuilder::addLineSet() { result << Base::blanks(indent) << "LineSet { } \n"; }
 ...
 * @param color_b blue part of the text color (0.0 - 1.0).
 */
void InventorBuilder::addText(float pos_x, float pos_y, float pos_z, const char *text,
                              float color_r, float color_g, float color_b)
{
    result << Base::blanks(indent) << "Separator { \n"
           << Base::blanks(indent) << "  Material { diffuseColor " << color_r << " " << color_g
           << " " << color_b << "} \n"
           << Base::blanks(indent) << "  Transform { translation " << pos_x << " " << pos_y << " "
           << pos_z << "} \n"
...
void InventorBuilder::addText(const Vector3f &vec, const char *text, float color_r, float color_g,
                              float color_b)
{
    addText(vec.x, vec.y, vec.z, text, color_r, color_g, color_b);
}
                                     ...
                                     unsigned short /*linePattern*/)
{
    ...
    result << Base::blanks(indent) << "Separator { \n"
           ...
           << Base::blanks(indent) << "    point [ " << pt1.x << " " << pt1.y << " " << pt1.z << ","
           << pt2s.x << " " << pt2s.y << " " << pt2s.z << " ] \n"
           << Base::blanks(indent) << "  } \n"
           << Base::blanks(indent) << "  LineSet { } \n"
           << Base::blanks(indent) << "  Transform { \n"
           << Base::blanks(indent) << "    translation " << cpt.x << " " << cpt.y << " " << cpt.z
           << " \n"
           << Base::blanks(indent) << "    rotation " << rot.x << " " << rot.y << " " << rot.z
           << " " << a << '\n'
           << Base::blanks(indent) << "  } \n"
 ...
 */
void InventorBuilder::addLineSet(const std::vector<Vector3f> &points, short lineSize, float color_r,
                                 float color_g, float color_b, unsigned short linePattern)
{
    char lp[20];
    sprintf(lp, "0x%x", linePattern);
...
void InventorBuilder::addIndexedFaceSet(const std::vector<int> &indices)
{
    ...
    for (std::vector<int>::const_iterator it = indices.begin(); it != indices.end(); ++it) {
        if (index % 8 == 0) result << Base::blanks(indent);
        if (it != it_last_f) result << *it << ", ";
        else
            result << *it << " ] \n";
        if (++index % 8 == 0) result << '\n';
    }
...
void InventorBuilder::addSingleTriangle(const Vector3f &pt0, const Vector3f &pt1,
                                        const Vector3f &pt2, bool filled, short lineSize,
                                        float color_r, float color_g, float color_b)
{
    std::string fs = "";
    if (filled) { fs = "    FaceSet { } "; }
                                      ...
                                      const std::vector<float> &vKnots)
{
    result << "  Separator { \n"
           << "    Coordinate3 { \n"
           << "      point [ ";
    for (std::vector<Base::Vector3f>::const_iterator it = controlPoints.begin();
         it != controlPoints.end(); ++it) {
        if (it != controlPoints.begin()) result << ",\n          ";
        result << it->x << " " << it->y << " " << it->z;
    }
...
void InventorBuilder::addTransformation(const Vector3f &translation, const Vector3f &rotationaxis,
                                        float fAngle)
{
    result << Base::blanks(indent) << "Transform {\n";
    result << Base::blanks(indent) << "  translation " << translation.x << " " << translation.y
           << " " << translation.z << '\n';
    result << Base::blanks(indent) << "  rotation " << rotationaxis.x << " " << rotationaxis.y
           << " " << rotationaxis.z << " " << fAngle << '\n';
    result << Base::blanks(indent) << "}" << '\n';
}

// -----------------------------------------------------------------------------

template<typename T> std::vector<T> InventorLoader::readData(const char *fieldName) const
{
...
{
    if (data.size() % 3 != 0) throw std::string("Reading failed");
...
std::vector<InventorLoader::Face> InventorLoader::convert(const std::vector<int32_t> &data) const
{
    ...
    for (const auto it : data) {
        if (it == 3) { faces.emplace_back(coordIndex, coordIndex + 1, coordIndex + 2); }
        else if (it == 4) {
...
InventorLoader::convert(const std::vector<std::vector<int32_t>> &coordIndex) const
{
    ...
    for (const auto &it : coordIndex) {
        if (it.size() == 3) { faces.emplace_back(it[0], it[1], it[2]); }
        else if (it.size() == 4) {
...
bool InventorLoader::read()
{
    ...
    if (line.find("#Inventor V2.1 ascii") == std::string::npos) return false;

    while (std::getline(inp, line)) {
        // read the normals if they are defined
        if (line.find("Normal {") != std::string::npos) { readNormals(); }
...
bool InventorLoader::isValid() const
{
    ...
    auto inRange = [value](const Face &f) {
        if (f.p1 < 0 || f.p1 >= value) return false;
        if (f.p2 < 0 || f.p2 >= value) return false;
        if (f.p3 < 0 || f.p3 >= value) return false;
        return true;
    };
    for (auto it : faces) {

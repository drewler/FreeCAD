...
QuantityFormat::QuantityFormat()
    : option(OmitGroupSeparator | RejectGroupSeparator), format(Fixed),
      precision(UnitsApi::getDecimals()), denominator(defaultDenominator)
{}

QuantityFormat::QuantityFormat(QuantityFormat::NumberFormat format, int decimals)
    : option(OmitGroupSeparator | RejectGroupSeparator), format(format),
      precision(decimals < 0 ? UnitsApi::getDecimals() : decimals), denominator(defaultDenominator)
{}

// ----------------------------------------------------------------------------

Quantity::Quantity() { this->_Value = 0.0; }

Quantity::Quantity(const Quantity &that) { *this = that; }
...
double Quantity::getValueAs(const Quantity &q) const { return _Value / q.getValue(); }
...
Quantity Quantity::operator*(double p) const { return Quantity(this->_Value * p, this->_Unit); }
...
Quantity Quantity::pow(const Quantity &p) const
{
    ...
    return Quantity(std::pow(this->_Value, p._Value),
                    this->_Unit.pow(static_cast<signed char>(p._Value)));
}

Quantity Quantity::pow(double p) const
{
    return Quantity(std::pow(this->_Value, p), this->_Unit.pow((short)p));
}
...
Quantity Quantity::operator-() const { return Quantity(-(this->_Value), this->_Unit); }
...
bool Quantity::isDimensionless() const { return isValid() && _Unit.isEmpty(); }

// true if it has a number and a valid unit
bool Quantity::isQuantity() const { return isValid() && !_Unit.isEmpty(); }

// true if it has a number with or without a unit
bool Quantity::isValid() const { return !boost::math::isnan(_Value); }

void Quantity::setInvalid() { _Value = std::numeric_limits<double>::quiet_NaN(); }
...
Quantity Quantity::Torr(101.325 / 760.0,
                        Unit(-1, 1,
                             -2)); // Torr is a defined fraction of Pascal (kg/m/s^2 or N/m^2)
Quantity Quantity::mTorr(0.101325 / 760.0,
                         Unit(-1, 1,
                              -2)); // Torr is a defined fraction of Pascal (kg/m/s^2 or N/m^2)
Quantity Quantity::yTorr(0.000101325 / 760.0,
                         Unit(-1, 1,
                              -2)); // Torr is a defined fraction of Pascal (kg/m/s^2 or N/m^2)
...
// error func
void Quantity_yyerror(char *errorinfo) { throw Base::ParserError(errorinfo); }

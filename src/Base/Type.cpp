...
struct Base::TypeData {
    TypeData(const char *theName, const Type type = Type::badType(),
             const Type theParent = Type::badType(), Type::instantiationMethod method = nullptr)
...
Type::Type(const Type &type) : index(type.index) {}
...
void *Type::createInstanceByName(const char *TypeName, bool bLoadModule)
{
    ...
    if (bLoadModule) importModule(TypeName);

    // now the type should be in the type map
    Type t = fromName(TypeName);
    if (t == badType()) return nullptr;
...
string Type::getModuleName(const char *ClassName)
{
    ...
    if (pos != std::string::npos) return string(temp, 0, pos);
    else
        return string();
}
...
Type Type::fromName(const char *name)
{
    ...
    if (pos != typemap.end()) return typedata[pos->second]->type;
    else
        return Type::badType();
}
...
{
    if (key < typedata.size()) return typedata[key]->type;
    else
        return Type::badType();
}

const char *Type::getName() const { return typedata[index]->name.c_str(); }

const Type Type::getParent() const { return typedata[index]->parent; }
...
int Type::getAllDerivedFrom(const Type type, std::vector<Type> &List)
{
    ...
    for (std::vector<TypeData *>::const_iterator it = typedata.begin(); it != typedata.end();
         ++it) {
...
int Type::getNumTypes() { return static_cast<int>(typedata.size()); }

Type Type::getTypeIfDerivedFrom(const char *name, const Type parent, bool bLoadModule)
{
    if (bLoadModule) importModule(name);

    Type type = fromName(name);

    if (type.isDerivedFrom(parent)) return type;
    else
        return Type::badType();
}

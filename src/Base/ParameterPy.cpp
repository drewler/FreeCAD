...
{
public:
    ParameterGrpObserver(const Py::Object &obj) { inst = obj; }
    ...
    void OnChange(ParameterGrp::SubjectType &rCaller, ParameterGrp::MessageType Reason) override
    {
        ...
        try {
            ...
            if (Reason && Reason[0] != '\0') args.setItem(1, Py::String(Reason));
            method.apply(args);
        }
        catch (Py::Exception &) {
    ...
    bool isEqual(const Py::Object &obj) const { return this->inst.is(obj); }
...
Py::Object ParameterGrpPy::getGroups(const Py::Tuple &args)
{
    ...
    Py::List list;
    for (const auto &it : handle) { list.append(Py::String(it->GetGroupName())); }
...
Py::Object ParameterGrpPy::setBool(const Py::Tuple &args)
{
    char *pstr;
    int Bool;
    if (!PyArg_ParseTuple(args.ptr(), "si", &pstr, &Bool)) throw Py::Exception();
...
Py::Object ParameterGrpPy::getBool(const Py::Tuple &args)
{
    char *pstr;
    int Bool = 0;
    if (!PyArg_ParseTuple(args.ptr(), "s|i", &pstr, &Bool)) throw Py::Exception();
...
{
    char *filter = nullptr;
    if (!PyArg_ParseTuple(args.ptr(), "|s", &filter)) throw Py::Exception();

    std::vector<std::pair<std::string, bool>> map = _cParamGrp->GetBoolMap(filter);
    Py::List list;
    for (const auto &it : map) { list.append(Py::String(it.first)); }
...
Py::Object ParameterGrpPy::setInt(const Py::Tuple &args)
{
    char *pstr;
    int Int;
    if (!PyArg_ParseTuple(args.ptr(), "si", &pstr, &Int)) throw Py::Exception();
...
Py::Object ParameterGrpPy::getInt(const Py::Tuple &args)
{
    char *pstr;
    int Int = 0;
    if (!PyArg_ParseTuple(args.ptr(), "s|i", &pstr, &Int)) throw Py::Exception();
...
Py::Object ParameterGrpPy::setUnsigned(const Py::Tuple &args)
{
    char *pstr;
    unsigned int UInt;
    if (!PyArg_ParseTuple(args.ptr(), "sI", &pstr, &UInt)) throw Py::Exception();
...
Py::Object ParameterGrpPy::getUnsigned(const Py::Tuple &args)
{
    char *pstr;
    unsigned int UInt = 0;
    if (!PyArg_ParseTuple(args.ptr(), "s|I", &pstr, &UInt)) throw Py::Exception();
...
Py::Object ParameterGrpPy::setFloat(const Py::Tuple &args)
{
    char *pstr;
    double Float;
    if (!PyArg_ParseTuple(args.ptr(), "sd", &pstr, &Float)) throw Py::Exception();
...
Py::Object ParameterGrpPy::getFloat(const Py::Tuple &args)
{
    char *pstr;
    double Float = 0.0;
    if (!PyArg_ParseTuple(args.ptr(), "s|d", &pstr, &Float)) throw Py::Exception();
...
Py::Object ParameterGrpPy::setString(const Py::Tuple &args)
{
    char *pstr;
    char *str;
    if (!PyArg_ParseTuple(args.ptr(), "ss", &pstr, &str)) throw Py::Exception();
...
Py::Object ParameterGrpPy::getString(const Py::Tuple &args)
{
    char *pstr;
    char *str = "";
    if (!PyArg_ParseTuple(args.ptr(), "s|s", &pstr, &str)) throw Py::Exception();
...
Py::Object ParameterGrpPy::attach(const Py::Tuple &args)
{
    ...
    for (ParameterGrpObserverList::iterator it = _observers.begin(); it != _observers.end(); ++it) {
        if ((*it)->isEqual(o)) { throw Py::RuntimeError("Object is already attached."); }
    }
...
Py::Object ParameterGrpPy::getContents(const Py::Tuple &args)
{
    ...
    if (_cParamGrp->IsEmpty()) return Py::None();

    Py::List list;
    // filling up Text nodes
    std::vector<std::pair<std::string, std::string>> mcTextMap = _cParamGrp->GetASCIIMap();
    for (std::vector<std::pair<std::string, std::string>>::iterator It2 = mcTextMap.begin();
         It2 != mcTextMap.end(); ++It2) {
        Py::Tuple t2(3);
    ...
    for (std::vector<std::pair<std::string, long>>::iterator It3 = mcIntMap.begin();
         It3 != mcIntMap.end(); ++It3) {
        Py::Tuple t3(3);
    ...
    for (std::vector<std::pair<std::string, double>>::iterator It4 = mcFloatMap.begin();
         It4 != mcFloatMap.end(); ++It4) {
        Py::Tuple t4(3);
    ...
    for (std::vector<std::pair<std::string, bool>>::iterator It5 = mcBoolMap.begin();
         It5 != mcBoolMap.end(); ++It5) {
        Py::Tuple t5(3);
    ...
    for (std::vector<std::pair<std::string, unsigned long>>::iterator It6 = mcUIntMap.begin();
         It6 != mcUIntMap.end(); ++It6) {
        Py::Tuple t6(3);

...
static inline void checkRange(const char *op, int length, int mass, int time, int electricCurrent,
                              int thermodynamicTemperature, int amountOfSubstance,
                              int luminousIntensity, int angle)
{
...
Unit::Unit(int8_t Length, int8_t Mass, int8_t Time, int8_t ElectricCurrent,
           int8_t ThermodynamicTemperature, int8_t AmountOfSubstance, int8_t LuminousIntensity,
           int8_t Angle)
{
    checkRange("unit", Length, Mass, Time, ElectricCurrent, ThermodynamicTemperature,
...
Unit::Unit(const Unit &that) { this->Sig = that.Sig; }
...
{
    checkRange("pow()", Sig.Length * exp, Sig.Mass * exp, Sig.Time * exp, Sig.ElectricCurrent * exp,
               Sig.ThermodynamicTemperature * exp, Sig.AmountOfSubstance * exp,
               Sig.LuminousIntensity * exp, Sig.Angle * exp);
...
{
    return (this->Sig.Length == 0) && (this->Sig.Mass == 0) && (this->Sig.Time == 0)
        && (this->Sig.ElectricCurrent == 0) && (this->Sig.ThermodynamicTemperature == 0)
        && (this->Sig.AmountOfSubstance == 0) && (this->Sig.LuminousIntensity == 0)
        && (this->Sig.Angle == 0);
}

bool Unit::operator==(const Unit &that) const
{
    return (this->Sig.Length == that.Sig.Length) && (this->Sig.Mass == that.Sig.Mass)
...
{
    checkRange("* operator", Sig.Length + right.Sig.Length, Sig.Mass + right.Sig.Mass,
               Sig.Time + right.Sig.Time, Sig.ElectricCurrent + right.Sig.ElectricCurrent,
               Sig.ThermodynamicTemperature + right.Sig.ThermodynamicTemperature,
               Sig.AmountOfSubstance + right.Sig.AmountOfSubstance,
               Sig.LuminousIntensity + right.Sig.LuminousIntensity, Sig.Angle + right.Sig.Angle);
    ...
    result.Sig.ThermodynamicTemperature =
        Sig.ThermodynamicTemperature + right.Sig.ThermodynamicTemperature;
...
QString Unit::getString() const
{
    ...
    if (Sig.Length > 0 || Sig.Mass > 0 || Sig.Time > 0 || Sig.ElectricCurrent > 0
        || Sig.ThermodynamicTemperature > 0 || Sig.AmountOfSubstance > 0
        || Sig.LuminousIntensity > 0 || Sig.Angle > 0) {

        bool mult = false;
        if (Sig.Length > 0) {
            mult = true;
            ret << "mm";
            if (Sig.Length > 1) ret << "^" << Sig.Length;
        }
        ...
        if (Sig.ElectricCurrent > 0) {
            ...
            mult = true;
            ret << "A";
            if (Sig.ElectricCurrent > 1) ret << "^" << Sig.ElectricCurrent;
        }
        ...
        if (Sig.ThermodynamicTemperature > 0) {
            ...
            mult = true;
            ret << "K";
            if (Sig.ThermodynamicTemperature > 1) ret << "^" << Sig.ThermodynamicTemperature;
        }
        ...
        if (Sig.AmountOfSubstance > 0) {
            ...
            mult = true;
            ret << "mol";
            if (Sig.AmountOfSubstance > 1) ret << "^" << Sig.AmountOfSubstance;
        }
        ...
        if (Sig.LuminousIntensity > 0) {
            ...
            mult = true;
            ret << "cd";
            if (Sig.LuminousIntensity > 1) ret << "^" << Sig.LuminousIntensity;
        }
    ...
    if (Sig.Length < 0 || Sig.Mass < 0 || Sig.Time < 0 || Sig.ElectricCurrent < 0
        ...
        if (Sig.Length < 0) {
            ret << "mm";
            mult = true;
            if (Sig.Length < -1) ret << "^" << abs(Sig.Length);
        }
        ...
        if (Sig.Mass < 0) {
            ...
            mult = true;
            ret << "kg";
            if (Sig.Mass < -1) ret << "^" << abs(Sig.Mass);
        }
        ...
        if (Sig.Time < 0) {
            ...
            mult = true;
            ret << "s";
            if (Sig.Time < -1) ret << "^" << abs(Sig.Time);
        }
        ...
        if (Sig.ElectricCurrent < 0) {
            ...
            mult = true;
            ret << "A";
            if (Sig.ElectricCurrent < -1) ret << "^" << abs(Sig.ElectricCurrent);
        }
        ...
        if (Sig.ThermodynamicTemperature < 0) {
            ...
            mult = true;
            ret << "K";
            if (Sig.ThermodynamicTemperature < -1) ret << "^" << abs(Sig.ThermodynamicTemperature);
        }
        ...
        if (Sig.AmountOfSubstance < 0) {
            ...
            mult = true;
            ret << "mol";
            if (Sig.AmountOfSubstance < -1) ret << "^" << abs(Sig.AmountOfSubstance);
        }
        ...
        if (Sig.LuminousIntensity < 0) {
            ...
            mult = true;
            ret << "cd";
            if (Sig.LuminousIntensity < -1) ret << "^" << abs(Sig.LuminousIntensity);
        }
        ...
        if (Sig.Angle < 0) {
            ...
            mult = true;
            ret << "deg";
            if (Sig.Angle < -1) ret << "^" << abs(Sig.Angle);
        }
...
Unit Unit::KinematicViscosity(
    2, 0, -1); // SI unit: m^2/s, https://en.wikipedia.org/wiki/Viscosity#Kinematic_viscosity
Unit Unit::VacuumPermittivity(
    -3, -1, 4,
    2); // SI unit: A²*s⁴/kg/m³ https://en.wikipedia.org/wiki/Permittivity#Vacuum_permittivity
